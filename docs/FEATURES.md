...
- **Calendar recurrence**: pin a task to weekdays (e.g. Mon & Thu), the *nth weekday of the month* (1st–5th or last, e.g. "1st Saturday" for smoke-alarm checks), or a fixed *day of the month* (clamped to the month length). **End-of-month options** (2.18+, #83): *last day of the month*, *last business day* (weekend dates roll back to Friday), and a **±N-day offset** on any calendar pattern — so "two days before the last working day" is three clicks, not a special case
