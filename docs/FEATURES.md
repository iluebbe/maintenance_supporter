...
**Create an object from a template** — pick one of 94 templates and get an
...
![Select mode: two objects ticked, Delete and Archive in the bar](images/objects-bulk-select.png)
...
- Scheduling modes: **time-based** (recurring interval), **calendar recurrence** (specific weekdays, *nth weekday of the month* — e.g. "1st Saturday", a *day of the month*, or — since 2.89 — the events of a **Home Assistant calendar entity**), **sensor-based** (triggered by entity state), **one-time** (single due date, marked *done* once completed), **manual**
...
- **Calendar entity as schedule** (2.89+, #187 / D#157): pick any `calendar.*` entity as the task's schedule and the task comes due **once per event**, on the event's start date (timed events count on their local start day). The next due date is always the **first event after the last completion** — completing the task on pickup day moves it to the next pickup, never back onto the one just handled, so a waste-collection calendar (all the `waste_collection_schedule`-style integrations, an ICS subscription, a Google/CalDAV calendar — or simply Home Assistant's built-in *Local Calendar* with the dates typed in for the year) drives "put the bins out" without the re-fire loop a state trigger on the calendar sensor produces. A ±N-day offset applies like on the other calendar kinds ("the evening before" = offset −1), as do the seasonal window and the series end. Events are read through `calendar.get_events` (60 days back, 400 days ahead), refreshed every 15 minutes and immediately after a user action; a calendar that is temporarily unavailable keeps its last known dates. With no known upcoming event the task simply has no due date (like a manual task). **Event titles** (2.92+, #189): the task also shows *which* events its next date stands for — one Waste Collection Schedule calendar names every pickup after the bin, so the list reads *Put the bins out · Residual waste, Paper* (several events on one day are joined). The titles appear after the task name on the dashboard, the Today tab, the object page, the task page and the Lovelace card, and in the notification text, the *Maintenance* to-do list, mirrored to-do rows and the integration's calendar; the task's own name never changes (it stays stable for entity ids, search and reference numbers). The task sensor carries them as `next_event_titles`. Available in the task dialog (entity picker, live preview of the next dates) and both config flows
...
- **Time-of-day scheduling** (optional, advanced): tasks flip to OVERDUE at a configured `HH:MM` in HA's timezone instead of at midnight. Works on every kind that has a due date — intervals, weekdays, nth weekday, day of month, calendar-entity and one-time tasks (2.77+; intervals only before), so "every Sunday at 21:00" is the Weekdays kind plus a time. Calendar events become timed 30-min blocks so mobile calendars can set real reminders. Enable under Settings → Features.
...
- 94 object templates in 10 categories — Vehicle, Home & HVAC, Building & Safety, Household & Routines, Garden & Outdoor, Pool, Appliances, Pets, Tech & IT, Health (car, e-bike, caravan & motorhome, seasonal tires, HVAC, heat pump, split AC / air-to-air heat pump, evaporative cooler, solar PV, balcony solar, ventilation system, domestic water installation, fireplace, sauna, heating oil tank, smoke detectors, electrical safety, windows & doors, drains & fixtures, damp & mould, frost protection, snow on the roof, flat roof & roof terrace, backflow valve & lifting station, septic system, small sewage treatment plant, private well, water storage tank, rainwater tank, sump pump, radon, termites, mosquitoes, wildfire, storm and earthquake preparation, emergency supplies, holiday home / cabin, tatami & futon care, inverter battery backup, garage door, standby generator, hot tub, robot vacuums, espresso machine, gas cooker & LPG cylinder, air purifier, RO water filter, irrigation, snow blower, roof moss, wooden deck & facade, litter box, aquarium, printer, NAS, UPS, wallbox, CPAP, hearing aids, …) — fully localized, curatable via Settings → Template gallery
- **Home profile and recommendations** (2.93+): the template gallery opens with *Recommended for your home* — the basics for a house or an apartment, what your climate and region call for (frost protection where winters freeze, snow on the roof in severe winters, damp and mould checks and roof moss in maritime climates, flat roofs and mosquitoes around the Mediterranean, termite checks, wildfire and storm preparation where they matter, earthquake preparedness in seismic regions, a radon test in countries with radon-prone areas, winter tires in the Alps and the north; the sauna in Finland, the holiday cabin in Russia and Scandinavia, tatami care in Japan, the water storage tank in Brazil, Mexico, India and the Middle East, the inverter battery in India and South Africa, the air-to-air heat pump in Norway, Sweden, Finland, Australia and New Zealand, bushfire preparation and termite protection across Australia, the house water installation in Germany, Austria and Switzerland, the air purifier in China, Korea and wildfire regions, the gas cooker and LPG cylinder in India, Brazil and Mexico) and the equipment your home evidently has — the garage door where a garage was found, lawn care with a garden, the sump pump (North America) or the backflow valve (Germany, Austria, Switzerland) with a basement, the UPS template where NUT or apcupsd monitors one. Equipment only some homes have (a septic tank, an oil tank, a treatment plant) is not pushed on everyone. Each recommendation names its reason. Templates your home type rarely has (a pool in an apartment) move to the end of their category — nothing is hidden. The profile is worked out locally: house or apartment from your Home Assistant floors, area names (in any of the 22 languages) and a few devices (a lawn mower, a garage door or gate); the climate from your home location and an offline climate map (Köppen-Geiger 1991–2020 plus the coldest and warmest monthly temperatures, Beck et al. 2023, CC BY 4.0). *Settings → Home profile* shows what was detected and why, and lets you set house or apartment yourself.
- **Seasons that fit your location** (2.93+): template tasks that belong to a season get a real seasonal window or a fixed date instead of a hint in the notes — mowing April–October, the ash pan October–April, winterize the irrigation on 15 October, gutters in April and November, spring start-up of the evaporative cooler. South of the equator the months are mirrored automatically; where there is no winter at all (Miami, Singapore), year-round tasks like mowing keep their plain interval and winter-only ones (closing the pool, winterizing the irrigation or the caravan) are left out. Everything stays editable in the task dialog. Where a duty depends on the country, the task note says so for your country only — the French boiler and chimney-sweeping duties, the SPANC inspection, the Polish and Swedish chimney rules, the German smoke-alarm check (DIN 14676), the UK Gas Safe engineer, the Italian flue-gas check; the German chimney-sweep notice (Feuerstättenbescheid), the heating checks in Belgium, Spain, Czechia, Poland and Russia, the Finnish, Danish, Czech, Italian and US chimney rules, the smoke-alarm rules of France, the Netherlands, Belgium, Japan and Queensland, the pool-barrier rules of France, Australia and New Zealand, gas-hose and gas-installation rules (India, Brazil, China, France, Spain), the periodic vehicle test in ten European countries plus the NSW pink slip and the NZ Warrant of Fitness, the Australian safety-switch, relief-valve, gas-heater, wastewater-plant and flue rules. Where the country's rhythm differs, the task gets it too — the MOT is yearly in the UK, the RCD test monthly in North America.
- **Legal notes are guidance, not legal advice.** The duties and intervals named in template notes were researched in September 2026 from public sources (standards, authorities, manufacturers). Rules vary by state, département or municipality and change over time — local regulations and the manufacturer's instructions always take precedence. The template gallery says so above the templates.
...
- Daily notification limits — fair and priority-aware (2.85+): the last 10 % of the limit are kept for high-priority tasks and low-priority tasks stop at 20 %; a task turned away yesterday and never served goes first today; a task that already got a message today yields its repeat to tasks still waiting; within one object's refresh high-priority tasks are announced first
- Mobile actionable notifications via Companion App: Complete, Skip, Snooze
- **Your own notification rule** (2.80+, #165) — every notification the integration is about to send also fires the event `maintenance_supporter_notification` (kind, status, object and task with their reference numbers, priority, due data, the notify target, title, message and the payload's `data`), so an automation can route it anywhere — into a notification hub such as Ticker, or straight to Telegram / Pushover with their own fields. Two settings under *Settings → Notifications → Your own notification rule*: **Only fire the event, send nothing myself** hands the whole delivery to your automation (repeat intervals, quiet hours and the daily cap still decide *when* the event fires), and **Extra notification data (template)** — JSON or YAML with Jinja, rendered with the same variables — is merged into every notify call's `data` (your keys win), which is how a Ticker `category`, a Pushover `priority` or a Telegram `parse_mode` rides along without an integration-specific option. The *Send test* button walks the same path, so both can be verified from Settings. Since 2.84 (#178) the event and the template also carry the task's **labels** and **notes**, the object's **area**, the task type and schedule kind, the last completion, the task's status-sensor entity id and the trigger entity — everything an automation needs to route or format on its own; the full field table is in the architecture docs, and *Send test* fills sample values for all of them. Both settings also sit in the integration's *Configure* dialog → *Notification Settings* (2.81+), and event-only needs **no notify service** at all — the event is the delivery (2.81+, #173)

  ![Settings: your own notification rule](images/notification-rule.png)
- **Completion notifications** (2.83+, #173 follow-up) — opt-in household news when a task is completed, with the reason (*completed in the panel*, *via QR scan*, *via NFC tag*, *from the to-do list*, *by voice*, *from the notification button*, *checked off the shopping list*, *by a service call*, *sensor recovered*) and, when known, who did it. *Automatic completions only* announces just the ones nobody made on the spot — sensor recovery, shopping-list check-off, an automation's service call — which is the mode for "tell me when the house did something by itself"; *All completions* is the shared household log on the phone. Sent to the household service only (the person who just pressed Complete needs no push), respects the per-task mute, the scope view, quiet hours and the daily cap, and walks the same notification event as everything else (`kind: completed`, `category: activity`, `reason`). Settings → Notifications → *Completion notifications*
- **No notifications for this task** (2.81+, #173) — a checkbox in the task dialog (next to the skip lock) mutes one task: no status notifications, repeats, lead-time reminders and no seat in a bundle (it does not count toward the bundle threshold either). The dashboard, its entities and the weekly digest are unaffected; the task header shows a bell-off badge
- **Notification icons** (2.89+, #185) — reminders sent to the Companion app carry an icon that matches the task: cleaning tasks get a broom, inspections a magnifier, replacements the swap arrows, calibrations a tuner, service a wrench, readings a counter, the battery fleet a battery, and summaries, warranty and budget alerts, the quiet-hours catch-up, completions and the Settings test send have their own. Any task can override its icon (*Notification icon* in the task dialog or the options-flow task editor, any `mdi:` name; the dialog previews it and names the default that applies when it is empty). Android renders the icon; iOS ignores it. Your own *Extra notification data* template keeps the last word — a `notification_icon` key there is never overwritten — and the `maintenance_supporter_notification` event carries the resolved icon in `data` so automations can reuse it

![Notification icon in the task dialog](images/task-dialog-notify-icon.png)
- **Triggers notify promptly** (2.81+, #175) — a sensor trigger flipping on or off asks the coordinator for a refresh right away (debounced ten seconds), so the *triggered* reminder and the notification event follow within seconds instead of waiting for the next 5-minute update tick
...
### Search (2.78+)
- **One search for everything, one tap away** — the magnifier in the panel's tab bar (wide screens) or header (phones, object and task pages) opens it; `/` opens it from anywhere in the panel. Results come **grouped**: Objects · Tasks · Spare parts · Documents · In documents · History notes, best hit first.
- **Forgiving on purpose** — every word of the query has to match *somewhere*, in any order (`spülm filt` finds the dishwasher's filter task); a word matches by exact / prefix / substring or **one typo** (`reinigne`); case, diacritics and `ue/oe/ae/ss` spellings do not matter (`kuhlschrank`, `spuel`). Separators are ignored, so `e24` finds `E-24` and `wm14t5` finds `WM-14-T5`.
- **Covers what you remember** — objects by name, manufacturer, model, serial number and notes; tasks by name, labels, notes and object; spare parts by name and part number; documents by title, file name and tags — **and by their contents**: the search reads the text layer of uploaded PDFs and text files and answers with the **page and a snippet** ("Fehlercode E24 … page 12"); opening the hit lands in the PDF on that page. **History notes** across every task are searched too — a note hit opens the task's history with the filter pre-filled.
- **What the content search can and cannot see** — born-digital PDFs (manufacturer manuals, invoices) and scans that already carry an OCR text layer (Paperless-ngx, scanner apps) are indexed. There is **no OCR** in Home Assistant: photos and scan-only PDFs stay invisible to the content search (their titles and tags are still found). The **Document storage** card shows how much of the library is indexed and how many files have no text layer. Extraction runs in the background after an upload; the existing library is indexed a couple of minutes after Home Assistant starts, one file at a time, so a Pi stays responsive.
- **Long document lists get a filter** — an object's or task's documents section shows a filter box from eight documents up, with the same forgiving matching.

![Global search: a manual found by its contents, with page and snippet](images/search.png)

### Documents & manuals (2.11.0+)
- **Attach files to any object** — PDFs, manuals, photos, receipts, spare-part lists. Stored **backup-safe under `/config`** (so they ride along in every Home Assistant backup) and **content-addressed with reference counting**, so uploading the same file to several objects keeps a single shared copy — a hint tells you when an upload deduped and cost no extra space. The full-text search's extracted text (2.78+) sits next to the files under `maintenance_supporter/docs/text/` — same backup scope, removed with the file
...
- **Document storage sensor** (2.11.0+) — one global **diagnostic** entity (since 2.92) `sensor.maintenance_supporter_document_storage` (`device_class: data_size`) reporting the physical footprint of attached documents, with `dedup_savings_bytes`, `document_count`, and per-object / per-category breakdowns as attributes
...
- **Core**: `maintenance_type`, `schedule_type`, `interval_days`, `interval_unit`, `interval_anchor`, `due_date`, `warning_days`, `last_performed`, `next_due`, `next_event_titles` (2.92+, #189 — calendar-entity schedules only: the titles of the events behind `next_due`, e.g. `["Residual waste", "Paper"]`; absent otherwise), `days_until_due`, `parent_object`, `times_performed`, `total_cost`, `average_duration`, `notes`, `documentation_url`, `priority` (#134 — route automations on it: `state_attr(..., 'priority') == 'high'`; tasks without an explicit value read `normal`)
