...
| `battery_auto_record_recovery` (2.88+, #181) | bool | `false` | — | **Record a replacement automatically when a low battery recovers**: when a battery that was low rises above `battery_recovered_percent`, the replacement date is written to its Battery Notes note and the type's spare cells are taken from stock (the note's `battery_quantity`, once per day) — the same as tapping *Replaced* in the roster, without the tap. Off by default because a level that climbs back on its own (a warmer room, a re-seated cell) is not a swap. Batteries without a Battery Notes note, rechargeables and the level-less rows are never auto-recorded. Settings → General, under the recovery threshold |
...
| `member_display` (2.76+, #169) | map | `{}` | per HA user id: `initials` ≤3 chars, `color` from the 12-colour palette | Per-member **avatar** overrides — the initials-in-a-colour circle shown next to assigned tasks (Today tab, dashboard rows, task page, Lovelace card). Without an entry the initials derive from the user's name and the colour from a stable hash of the user id, so members differ by default. Edited under **Settings tab → Member avatars**; user ids are instance-bound, so the settings export leaves this key out |
| `home_type` (2.93+) | string | `auto` | `auto`, `house`, `apartment` | The home profile's dwelling type. `auto` guesses it from your Home Assistant floors, area names and a few devices; `house` / `apartment` override the guess. Only affects the template gallery's recommendations (*Recommended for your home*) — set under **Settings tab → Home profile** |
...
| `notify_completed` (2.83+) | enum | `off` | `off` / `automatic` / `all` | **Completion notifications** (#173 follow-up): household news when a task is completed, with the reason (panel, QR / NFC scan, to-do list, voice, notification button, shopping-list check-off, service call, sensor recovery) and, when known, who did it. `automatic` announces only completions nobody made on the spot (shopping_list, service, auto_recovery). Sent to the household service only, never to the actor's devices; the per-task mute, the scope view, quiet hours and the daily cap apply, repeat intervals and snooze do not. Also fires `maintenance_supporter_notification` with `kind: completed`, `category: activity`, `reason`, `completed_by`, `completed_by_name`, `completed_at`. Settings → Notifications → *Completion notifications* (also in the Configure dialog → Notification Settings) |
| `notify_event_only` (2.80+) | bool | `false` | — | **Your own notification rule** (#165): fire `maintenance_supporter_notification` and send nothing yourself — an automation is the whole delivery. Repeat intervals, quiet hours and the daily cap still gate *when* the event fires; the send counts as delivered for rate limiting. Settings → Notifications → *Only fire the event, send nothing myself* (also in the Configure dialog → Notification Settings, 2.81+). Needs no notify service (2.81+): with an empty service the event still fires |
| `notify_extra_data` (2.80+) | string (template) | `""` | ≤ 2000 chars | JSON or YAML with Jinja, rendered per notification with every field of the notification event — `kind`, `status`, `priority`, `labels`, `notes`, `object_name`, `area_name`, `task_name`, `object_ref`, `task_ref`, `task_type`, `days_until_due`, `next_due`, `last_performed`, `responsible_user_id`, `sensor_entity_id`, `url`, `target`, `title`, `message` … (the full table: Architecture → *Notification event fields*; `tasks` for bundles carries the same per task). The result must be a mapping; it is merged into the notify call's `data` — your keys override the integration's (`tag`, `url`, `clickAction`, `actions`). A template that fails to render or yields a non-mapping is logged once and ignored — the plain notification still goes out. Example for Ticker: `{"category": "maintenance", "critical": {{ priority == "high" }}, "navigate_to": "{{ url }}"}`; for Pushover: `{"priority": {{ 1 if status == "overdue" else 0 }}}`. Also merged into the Settings *Send test* |
| `notify_scope_view_id` (2.26+) | string | `""` | max 64 chars | Restrict **all** status-change notifications to tasks matching a saved view's **label, user and priority filters** (pick the view under Settings → Notifications → *"Notify only for view"*). Combined with a *"High priority"* view this routes urgent tasks to push while a daily digest covers the rest (#134). Display-only view dimensions (status, sorting, grouping, archived) are ignored; a *"current user"* filter can't be resolved server-side and imposes no restriction; if the referenced view was deleted, notifications fall back to all tasks. Empty = notify for everything |
...
| `documentation_url` (1.4.0+) | string (URL) | `""` | Link to PDF manual / vendor page / setup guide for this object. Only `http://` and `https://` URLs are accepted; `javascript:`, `data:`, and protocol-relative URLs are rejected. Shown as a clickable link in the panel object-detail header AND on every task-detail page belonging to this object (1.4.1+) so the manual is always one click away from any maintenance task. When left empty, the header and the objects-table "manual" column fall back to attached documents in the "manual" category (2.47+) |
| `ha_device_id` (2.19+) | string (device id) | `""` | Attach the object to an **existing HA device**: its task entities (sensor / binary sensor / buttons) then appear on that device's page instead of an own virtual device, and the owning integration's device metadata stays untouched — the object contributes **no device of its own** while linked. The area of the linked device is adopted by the object, so moving the appliance moves the object with it. If the linked device disappears, the object falls back to its own device and a **fixable Repairs notice** appears (relink to the suggested best match, or remove the link); the stored id is never cleared for you, so a link that dangles today (after a JSON restore on another instance, or while an integration is removed) still works when the device comes back. Maintenance Supporter's own devices are rejected as link targets — the object's twin carries the appliance's exact name and used to be an easy mispick. Set via the panel object dialog ("Link to existing device") |
...
| `ref_no` (2.79+) | int | *(assigned)* | Reference number (`#8`), assigned once when the object's entry is first set up — in creation order — and never reused; a copy or a successor gets a fresh one. Read-only; a JSON backup carries it (a colliding import is renumbered). `next_task_ref` (int) is the object's high-water counter for its tasks |
| `notes` (1.4.10+) | string (multiline, ≤2000 chars) | `""` | Free-form notes about the object — part numbers, replacement procedures, settings reminders, "spare key in garage drawer", etc. Rendered as **Markdown** since 2.67 (bold, lists, links; newlines preserved). Shown in a dedicated, left-bordered block under the object meta in the panel object-detail header. Editable via panel object dialog or *Object settings* in the config flow |
...
| `schedule_type` | enum | `time_based` | — | Scheduling mode: `time_based`, `sensor_based`, `one_time`, `manual`. The calendar kinds `weekdays`, `nth_weekday`, `day_of_month` and `calendar` (2.89+) are configured via the nested `schedule` object (below) — they can't be expressed by the flat fields — and are reported back under their own `schedule_type` value |
...
| `schedule_time` | string (HH:MM) | *(none)* | `00:00–23:59` | Optional time-of-day at which the task flips from `due_soon` to `overdue` on the due date. Requires the `advanced_schedule_time_visible` feature flag. Available on every kind that produces a due date — `time_based`, `one_time`, `weekdays`, `nth_weekday`, `day_of_month` (2.77+; interval only before) and `calendar` (2.89+). Not offered on `manual` or `sensor_based` tasks, which have no due date to refine. Interpreted in HA's configured timezone. Empty/unset → midnight semantic (historical behaviour). |
...
| `notes` | string (multiline, ≤2000 chars) | `""` | — | General notes about the task. Rendered as **Markdown** on the task detail since 2.67 |
...
| `ref_no` (2.79+) | int | *(assigned)* | ≥1 | Reference number within the object (`#8.3` = object 8, task 3), assigned on the coordinator's next refresh after creation, in creation order, never reused. Read-only, carried by JSON backups. Each completed history entry carries its own `ref_no` (`#8.3-2`), handed out in the completion choke point; skips carry none |
...
| `notify_icon` (#185, 2.89+) | string | — | `mdi:` name, ≤64 | **Notification icon** for this task's push notifications (`data.notification_icon`, rendered by the Android Companion app). Empty = the maintenance type's default (cleaning `mdi:broom`, inspection `mdi:magnify`, replacement `mdi:swap-horizontal`, calibration `mdi:tune`, service `mdi:wrench`, reading `mdi:counter`, custom `mdi:wrench-clock`). An invalid name is refused (`invalid_icon`); a `notification_icon` key in the *Extra notification data* template still wins |
...
Every completion fires `maintenance_supporter_task_completed` on the HA event bus regardless of whether `on_complete_action` is set. Since 2.75 the payload also carries the task's status sensor as `entity_id` (which is what makes the entry appear under that sensor in the HA logbook) and the recorded readings (`reading_value` / `reading_values`). The payload's `completed_at` field carries the history entry's own timestamp — for a backdated completion (the optional *Completed at* field in the dialog, or the service parameter of the same name) that is the chosen past moment, so automations that aggregate by period (e.g. yearly cost buckets) attribute backfilled entries correctly (#133). Backfills of a moment older than the task's latest completion carry `backfill: true` and deliberately do **not** run the task's `on_complete_action` — resetting a device counter today for work done months ago would act on the wrong state. Power users who prefer YAML automations can subscribe directly:
