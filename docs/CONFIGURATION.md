...
| `battery_lifetime_months` (2.83+, D#162) | dict | `{}` | type → 1–240 months, ≤100 entries | **Typical battery lifetimes** per type (`{"CR2032": 24}`), the forecast anchor for batteries without a percentage. Keys are canonical Battery Notes types (aliases folded: LR6 → AA). Precedence: this value → learned from the fleet's own replacements (median, ≥3) → built-in table → 12 months. Settings → *Typical battery lifetimes* |
