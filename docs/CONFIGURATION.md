...
| `notify_extra_data` (2.80+) | string (template) | `""` | ≤ 2000 chars | JSON or YAML with Jinja, rendered per notification with `kind`, `status`, `priority`, `object_name`, `task_name`, `object_ref`, `task_ref`, `days_until_due`, `next_due`, `responsible_user_id`, `url`, `target`, `title`, `message` (and `tasks` for bundles). The result must be a mapping; it is merged into the notify call's `data` — your keys override the integration's (`tag`, `url`, `clickAction`, `actions`). A template that fails to render or yields a non-mapping is logged once and ignored — the plain notification still goes out. Example for Ticker: `{"category": "maintenance", "critical": {{ priority == "high" }}, "navigate_to": "{{ url }}"}`; for Pushover: `{"priority": {{ 1 if status == "overdue" else 0 }}}`. Also merged into the Settings *Send test* |
