...
Point a task at the calendar your waste-collection integration (or an ICS subscription) exposes — or, for a handful of dates you know in advance, create a *Local Calendar* (Settings → Integrations → Add → Local Calendar) and type them in: schedule type **Calendar entity**, pick `calendar.waste_collection`, optionally an offset of −1 day for "the evening before". The task comes due on each pickup date and, once completed, moves to the **next** pickup — it never re-fires for the collection you just handled, which is exactly what a state trigger on the calendar's sensor could not do. When one calendar carries every bin (Waste Collection Schedule's default), the task names the next pickup — *Put the bins out · Residual waste, Paper* — on the dashboard, in the notification and on the to-do list (2.92+), so one task covers all bins. The same pattern fits street-sweeping calendars, chimney-sweep appointments, or a shared family calendar with "change the pool filter" entries.
