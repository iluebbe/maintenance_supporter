...
| B4 | Skip / missed semantics / snooze (notification + panel) | P, N, W | test_ws_tasks_actions, test_notification_manager, test_journey_snooze (snooze silences reminders; survives an entry RELOAD but a full restart forgets it — the in-memory-only contract, pinned via the new `simulate_full_restart` harness) | closed |
| B5 | Being reminded: due-soon → overdue → lead-times → digest → quiet hours/vacation interplay | (time) | test_notification_manager, test_daily_tick, test_vacation, test_journey_biographies (frozen clock walks ok → due_soon push → same-day silence → overdue push → complete → next cycle re-arms), test_journey_vacation (during the window a normal overdue task is silent while an exempt one still fires; decision persists across restart; window ends → notifications resume, not "used up") — closed |
| B6 | Sensor-triggered flow: threshold crosses → TRIGGERED → notification → complete → re-arm (or auto-complete on recovery) | (state) | test_triggers*, test_auto_complete_recovery, test_journey_triggers (threshold: restart-while-high re-arms), test_journey_counter_trigger (delta counter re-baselines on complete AND the baseline persists across restart), test_journey_compound_trigger (AND state round-trips the Store's _compound_N reconstruction across restart), test_journey_auto_complete (sensor recovery auto-completes exactly once, no double-count across restart, 120 s race guard) | closed |
