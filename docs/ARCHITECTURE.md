...
**Version:** 2.55.0 | 200 source files (129 Python + 71 TypeScript) | **98% test coverage** (3,297 backend tests + 402 frontend tests)
...
│
├── websocket/                   (7,135 lines)  86 WS commands, split by domain
...
86 commands organized by function. The authoritative inventory (command → permission tier) is `tests/test_ws_permission_matrix.py`, which fails if a handler is added without a tier.
...
The backend exposes 86 WS commands; most are consumed by the Lit panel. A couple (`task/list`, `templates`) are genuinely obsolete for the panel but kept as public API.
