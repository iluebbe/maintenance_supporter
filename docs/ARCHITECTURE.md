...
**Version:** 2.56.0 | 200 source files (129 Python + 71 TypeScript) | **98% test coverage** (3,314 backend tests + 423 frontend tests)
...
│
├── websocket/                   (7,135 lines)  87 WS commands, split by domain
...
87 commands organized by function. The authoritative inventory (command → permission tier) is `tests/test_ws_permission_matrix.py`, which fails if a handler is added without a tier.
...
The backend exposes 87 WS commands; most are consumed by the Lit panel. A couple (`task/list`, `templates`) are genuinely obsolete for the panel but kept as public API.
