...
**Version:** 2.86.0 | 249 source files (145 Python + 104 TypeScript) | **98% test coverage** (4,048 backend tests + 855 frontend tests)
...
+-------------------+    | - sensor_predictor (degradation + env)
                         ...
                         | - signatures/ (123 integrations, 235 signals)
...
│
├── websocket/                   (7,135 lines)  96 WS commands, split by domain
...
**Four single-file bundles** (the card bundle also carries the panel-card wrapper, #174) — `maintenance-panel.js` (~568 KB), `maintenance-card.js` (~263 KB), `maintenance-calendar-card.js` (~103 KB), `maintenance-strategy-shim.js` (~3.6 KB, the dashboard-strategy self-heal shim) — plus the code-split `maintenance-dashboard-strategy.js` entry (~14 KB, output to `strategy/` with content-hashed chunks). UI translations are no longer bundled (only `en.json` is); the other 21 languages load at runtime from `frontend/locales/`, which is what shrank these bundles ~50–80%.
...
**The panel as a card (2.84, #174).** `maintenance-panel-card.ts` (in the card bundle) defines `maintenance-supporter-panel-card`: a plain custom element that lazy-imports the panel bundle from the URL HA registered for the sidebar panel (`hass.panels["maintenance-supporter"].config._panel_custom.module_url`) — the same module instance, so `maintenance-supporter-panel` is defined once whichever surface loads first — and mounts one panel element with `embedded = true`, `presets` (`tab` / `view` from the card config) and `narrow` derived from the card's width (870 px, HA's own breakpoint). The panel keeps its bounded box with `.content` as the scroller, so the card gives it a definite height: the viewport below the card's top edge (measured on mount / resize, never on scroll) or the configured `height`. Embedded mode in the panel: no `ha-menu-button`, no safe-area padding on `.panel`, `_onLocationChanged` accepts the dashboard's path, and the presets open a tab / saved view once without persisting the choice.
...
96 commands organized by function. The authoritative inventory (command → permission tier) is `tests/test_ws_permission_matrix.py`, which fails if a handler is added without a tier.
...
The backend exposes 96 WS commands; most are consumed by the Lit panel. A couple (`task/list`, `templates`) are genuinely obsolete for the panel but kept as public API.
...
### Reference numbers (2.79, #170)

Object `8`, task `8.3`, completion `8.3-2` — short numbers a printed booklet, a photo caption or a note can carry, unlike the UUIDs the storage is keyed by. The rules that make them trustworthy on paper, all in `helpers/reference_numbers.py`:

- **Assigned once, never reused, never renumbered.** Each level keeps a high-water counter: `next_object_ref` in a small Store (`.storage/maintenance_supporter.reference_numbers`), `object.next_task_ref` on the object, `next_history_ref` in the task's dynamic Store state. A deleted 8.2 does not turn 8.3 into 8.2; the next task is 8.4. Counters are also lifted by numbers found in the data (an import carrying higher numbers moves the mark up), so a restore never hands out a number twice.
- **Creation order, lazily.** Objects are numbered in `async_setup_entry` — all unnumbered entries in one pass sorted by `ConfigEntry.created_at`, under a lock, so concurrent setup at boot cannot decide who is 1 and who is 2. Tasks are numbered at the top of the coordinator refresh (`_assign_task_refs`; every creation path — panel, options flow, import, template, adopt, fleet setup — ends in a refresh, so this is the one hook). Completions are numbered in `complete_maintenance` right after `task.complete()`, the choke point every completion surface passes; backdated completions get the next number (they are ids, not ranks). Copies and successors drop the number (`strip_task_runtime_state`, `strip_object_reference`).
- **Surfaces.** `object`/`objects` and the task summaries carry `ref_no`; history entries carry theirs inline. The panel formats in `helpers/reference.ts` (twin of the Python formatter), shows muted `.ref-chip`s on the object page, task page, history entries and an optional objects-table column, prints them on the booklet and the work sheet, and the global search treats a typed reference as exact: `8` / `8.3` from the loaded data, `8.3-2` through `maintenance_supporter/search` (which answers with that one completion). JSON export/import round-trips `ref_no` + `next_task_ref`; CSV deliberately does not (a sheet must not renumber).
...
- **Kind-agnostic.** The model, the calendar and the WS layer never looked at the schedule kind — `schedule_time` refines whatever `next_due` the schedule produced. Both editors nevertheless gated the field to the interval kind until 2.77 (#168); `SCHEDULE_TIME_KINDS` (`config_flow_helpers.py`, mirrored in `task-dialog.ts`) now names the date-producing kinds — interval, `weekdays`, `nth_weekday`, `day_of_month`, `one_time` — and "every Tuesday at 19:00" is simply the native **Weekdays** kind plus a time. `manual` and `sensor_based` have no due date and never see the field; the dialog omits (not nulls) `schedule_time` for them so a no-op edit cannot wipe a stored value.
