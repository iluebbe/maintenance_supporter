...
**Version:** 2.92.0 | 256 source files (150 Python + 106 TypeScript) | **98% test coverage** (4,346 backend tests + 902 frontend tests)
...
`operator_write_enabled` defaults **off**, so out of the box every non-admin is read-only. When operator mode is active the panel hides every create/edit/delete control (Settings tab, both `+ New Object` buttons, `New Maintenance Task`, object Edit / Add Task / Delete, the NFC-link affordance, and the write entries of the per-task more-menu); the household actions — `Complete`, `Skip`, `Reset`, `Postpone`, `Snooze` (read tier on the server) — plus `QR Code` and the work sheet remain. The Lovelace quick-action dialogs, the card header and the groups card apply the same rule (`helpers/permissions.ts canWrite()`, tied to the WS tiers by `tests/test_frontend_permission_parity.py`).
