...
**Version:** 2.59.0 | 205 source files (132 Python + 73 TypeScript) | **98% test coverage** (3,390 backend tests + 434 frontend tests)
...
│
├── websocket/                   (7,135 lines)  88 WS commands, split by domain
...
88 commands organized by function. The authoritative inventory (command → permission tier) is `tests/test_ws_permission_matrix.py`, which fails if a handler is added without a tier.
...
The backend exposes 88 WS commands; most are consumed by the Lit panel. A couple (`task/list`, `templates`) are genuinely obsolete for the panel but kept as public API.
