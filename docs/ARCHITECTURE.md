...
**Version:** 2.62.0 | 206 source files (132 Python + 74 TypeScript) | **98% test coverage** (3,428 backend tests + 450 frontend tests)
...
│
├── websocket/                   (7,135 lines)  90 WS commands, split by domain
...
90 commands organized by function. The authoritative inventory (command → permission tier) is `tests/test_ws_permission_matrix.py`, which fails if a handler is added without a tier.
...
The backend exposes 90 WS commands; most are consumed by the Lit panel. A couple (`task/list`, `templates`) are genuinely obsolete for the panel but kept as public API.
...
| test-coverage (>95%) | Silver | Yes (98%, 3,295 tests) |
