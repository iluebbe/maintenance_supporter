...
Create the object once (or pick it from 73 ready-made templates), give it a
...
| [ARCHITECTURE.md](docs/ARCHITECTURE.md) | Internals: data flow, trigger engine, WebSocket API (97 commands), tests |
...
— self-assessed **Platinum** (every Bronze → Platinum rule done or exempt with a reason), with the per-rule self-assessment in
