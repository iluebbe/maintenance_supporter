    ...
    `}_renderCondition(t,e){let i=this._lang,o=e+1;return r`
      <div class="compound-condition">
        ...
        <div class="select-row">
          ...
          >
            ${st.map(u=>r`<option value=${u} ?selected=${u===t.type}>${s(u,i)}</option>`)}
          </select>
        </div>
      ...
      <div class="select-row">
        ...
        <div class="select-row">
          ...
          <select .value=${this._nth} @change=${o=>this._nth=o.target.value}>
            ${i.map(([o,u])=>r`<option value=${o} ?selected=${o===this._nth}>${u}</option>`)}
          </select>
        </div>
        <div class="select-row">
          <label>${s("recurrence_weekday",t)}</label>
          <select .value=${this._nthWeekday} @change=${o=>this._nthWeekday=o.target.value}>
            ${e.map((o,u)=>r`<option value=${String(u)} ?selected=${String(u)===this._nthWeekday}>${o}</option>`)}
          </select>
        </div>
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${yt.map(i=>r`<option value=${i} ?selected=${i===this._type}>${s(i,t)}</option>`)}
            </select>
          </div>
          <div class="select-row">
          ...
          <div class="select-row">
            ...
            >
              ${$t.map(i=>r`<option value=${i} ?selected=${i===this._scheduleType}>${s(i,t)}</option>`)}
            </select>
          </div>
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function Rt(n,d,t){let e=n.confidence_interval_low,i=n.confidence_interval_high,o=d.suggested_interval??d.interval_days??0,u=d.interval_days??0,_=Math.max(0,e-5),m=i+5-_,y=(e-_)/m*100,I=(i-e)/m*100,z=(o-_)/m*100,w=u>0?(u-_)/m*100:-1;return r`
  ...
  `}function nt(n,d,t){let e=n.degradation_trend!=null&&n.degradation_trend!=="insufficient_data",i=n.days_until_threshold!=null,o=n.environmental_factor!=null&&n.environmental_factor!==1;if(!e&&!i&&!o)return c;let u=n.degradation_trend==="rising"?"M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z":n.degradation_trend==="falling"?"M16,18L18.29,15.71L13.41,10.83L9.41,14.83L2,7.41L3.41,6L9.41,12L13.41,8L19.71,14.29L22,12V18H16Z":"M22,12L18,8V11H3V13H18V16L22,12Z";return r`
      ...
      <div class="prediction-grid">
        ...
        ${i?r`
          <div class="prediction-item">
            ...
            ${n.threshold_prediction_date?r`<span class="prediction-date">${D(n.threshold_prediction_date,d)}</span>`:c}
        ...
        ${o&&t.environmental?r`
          <div class="prediction-item">
            ...
            ${n.environmental_entity?r`<span class="prediction-entity entity-link" @click=${_=>et(_,n.environmental_entity)}>${n.environmental_entity}</span>`:c}
          </div>
        `:c}
      </div>
    </div>
  `}function ot(n,d,t,e){let i=Math.max(n||1,d);return r`
  ...
  `}var lt=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"];function dt(n,d,t){if(!t.seasonal||!n.seasonal_factor||n.seasonal_factor===1)return c;let e=lt.map(_=>s(_,d)),i=new Date().getMonth(),o=n.seasonal_factors||n.interval_analysis?.seasonal_factors||null,u=o&&o.length===12?o:e.map((_,p)=>{let m=n.seasonal_factor||1,y=Math.sin((p-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,m+y))});return r`
    <div class="seasonal-card-compact">
      <h4>${s("seasonal_awareness",d)}</h4>
      <div class="seasonal-mini-chart">
        ${u.map((_,p)=>{let m=_*40,y=_<.9?"low":_>1.1?"high":"normal";return r`
  ...
  `}function ct(n,d){return Pt(n,d)}function Pt(n,d){let t=n.seasonal_factors??n.interval_analysis?.seasonal_factors;if(!t||t.length!==12)return c;let e=n.interval_analysis?.seasonal_reason,i=new Date().getMonth(),o=300,u=100,_=8,m=u-_-4,y=Math.max(...t,1.5),I=o/12,z=I*.65,w=_+m-1/y*m;return r`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${o} ${u}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",d)}">
        ...
        ${t.map((H,S)=>{let U=H/y*m,K=S*I+(I-z)/2,q=_+m-U,j=S===i,O=H<1?"var(--success-color, #4caf50)":H>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return M`
  ...
  `}var b=class extends ${constructor(){super(...arguments);this._open=!1;this._entryId=null;this._taskId=null;this._task=null;this._objectName="";this._busy=!1;this._error="";this._showSkip=!1;this._showReset=!1;this._showDetails=!1;this._showAdaptive=!1;this._skipReason="";this._resetDate="";this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._toast="";this._featuresLoaded=!1}get _lang(){return this.hass?.language||"en"}async openFor(t,e){this._entryId=t,this._taskId=e,this._error="",this._showSkip=!1,this._showReset=!1,this._showAdaptive=!1,this._skipReason="",this._resetDate=new Date().toISOString().slice(0,10),this._open=!0,await Promise.all([this._loadTask(),this._loadFeatures()])}async _loadFeatures(){if(!this._featuresLoaded)try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});t?.features&&(this._features={...this._features,...t.features}),this._featuresLoaded=!0}catch{}}close(){this._open=!1,this._task=null,this._error=""}async _loadTask(){if(!(!this._entryId||!this._taskId))try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._objectName=t.object?.name||"";let e=(t.tasks||[]).find(i=>i.id===this._taskId);this._task=e??null}catch(t){this._error=k(t,this._lang)}}async _runWs(t){this._busy=!0,this._error="";try{return await this.hass.connection.sendMessagePromise(t),this._busy=!1,!0}catch(e){return this._error=k(e,this._lang),this._busy=!1,!1}}_notifyChanged(t){this.dispatchEvent(new CustomEvent("task-action-fired",{detail:{entry_id:this._entryId,task_id:this._taskId,action:t},bubbles:!0,composed:!0}))}_onComplete(){!this._entryId||!this._taskId||!this._task||import("./dialog-mount-FFUYRDBT.js").then(({openCompleteDialog:t})=>{t({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,checklist:this._task.checklist||[],adaptive_enabled:!!this._task.adaptive_config?.enabled})&&(this._notifyChanged("complete"),this.close())})}async _onSkipConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/skip",entry_id:this._entryId,task_id:this._taskId,reason:this._skipReason.trim()||null})&&(this._notifyChanged("skip"),this.close())}async _onResetConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/reset",entry_id:this._entryId,task_id:this._taskId,date:this._resetDate||void 0})&&(this._notifyChanged("reset"),this.close())}_onEdit(){!this._entryId||!this._taskId||import("./dialog-mount-FFUYRDBT.js").then(({openEditTaskDialog:t})=>{t(this._entryId,this._taskId),this.close()})}_onQr(){!this._entryId||!this._taskId||!this._task||import("./dialog-mount-FFUYRDBT.js").then(({openQrDialog:t})=>{t({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,object_name:this._objectName}),this.close()})}async _onDelete(){if(!this._entryId||!this._taskId)return;let t=s("delete_task_confirm",this._lang)||`Delete "${this._task?.name}"?`;if(!window.confirm(t))return;await this._runWs({type:"maintenance_supporter/task/delete",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("delete"),this.close())}async _onArchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/archive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("archive"),this.close())}async _onUnarchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/unarchive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("unarchive"),this.close())}_onOpenInPanel(){if(!this._entryId||!this._taskId)return;let t=`/maintenance-supporter?entry_id=${encodeURIComponent(this._entryId)}&task_id=${encodeURIComponent(this._taskId)}`;history.pushState(null,"",t),window.dispatchEvent(new CustomEvent("location-changed")),this.close()}async _applySuggestion(){if(!this._entryId||!this._taskId||!this._task?.suggested_interval)return;await this._runWs({type:"maintenance_supporter/task/apply_suggestion",entry_id:this._entryId,task_id:this._taskId,interval:this._task.suggested_interval})&&(this._toast=s("suggestion_applied",this._lang)||"Applied",this._notifyChanged("apply_suggestion"),await this._loadTask(),setTimeout(()=>{this._toast=""},2500))}async _reanalyzeInterval(){if(!(!this._entryId||!this._taskId)){this._busy=!0,this._error="";try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:this._entryId,task_id:this._taskId});this._toast=t.recommended_interval?`${s("reanalyze_result",this._lang)||"Recomputed"}: ${t.recommended_interval}d (${t.data_points} pts)`:s("reanalyze_insufficient_data",this._lang)||"Not enough data",await this._loadTask(),setTimeout(()=>{this._toast=""},3500)}catch(t){this._error=k(t,this._lang)}finally{this._busy=!1}}}_onEditHistoryEntry(t){!this._entryId||!this._taskId||import("./dialog-mount-FFUYRDBT.js").then(({openHistoryEditDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,original_timestamp:t.timestamp,type:t.type,timestamp:t.timestamp,notes:t.notes??null,cost:t.cost??null,duration:t.duration??null,completed_by:t.completed_by??null})})}_renderRecommendation(t){if(!this._features.adaptive||!t.suggested_interval||t.suggested_interval===t.interval_days)return c;let e=this._lang;return r`
    ...
    `}_renderDetails(t){let e=this._lang,i=t.history||[],o=i.filter(p=>p.type==="completed"),u=o.reduce((p,m)=>p+(typeof m.cost=="number"?m.cost:0),0),_=(()=>{let p=o.map(m=>typeof m.duration=="number"?m.duration:null).filter(m=>m!=null);return p.length?Math.round(p.reduce((m,y)=>m+y,0)/p.length):null})();return r`
      <div class="details">
        ...
        ${i.length===0?r`<div class="history-empty">${s("history_empty",e)||"No history yet."}</div>`:r`
              <div class="history-list">
                ${[...i].reverse().slice(0,20).map(p=>{let m=["completed","reset","skipped"].includes(p.type);return r`
                    <div class="history-entry">
                      <div class="history-line">
                        <span class="history-type type-${p.type}">${s(p.type,e)}</span>
                        <span class="history-date">${X(p.timestamp,e)}</span>
                        ${m?r`<button class="history-edit"
      ...
      <div class="dialog" role="dialog" aria-modal="true">
        ${e?r`
              <div class="header">
                ...
                <div class="object">
                  <button class="link-inline" @click=${()=>{this._entryId&&import("./dialog-mount-FFUYRDBT.js").then(({openObjectQuickActions:o})=>{o(this._entryId),this.close()})}}>${this._objectName}</button>
                </div>
                <div class="quick-info">
                  ${e.next_due?r`<span><strong>${s("next_due",t)||"Next due"}:</strong> ${D(e.next_due,t)}</span>`:c}
                  ${e.last_performed?r`<span><strong>${s("last_performed",t)||"Last"}:</strong> ${D(e.last_performed,t)}</span>`:c}
  ...
  `],a([g({attribute:!1})],b.prototype,"hass",2),a([l()],b.prototype,"_open",2),a([l()],b.prototype,"_entryId",2),a([l()],b.prototype,"_taskId",2),a([l()],b.prototype,"_task",2),a([l()],b.prototype,"_objectName",2),a([l()],b.prototype,"_busy",2),a([l()],b.prototype,"_error",2),a([l()],b.prototype,"_showSkip",2),a([l()],b.prototype,"_showReset",2),a([l()],b.prototype,"_showDetails",2),a([l()],b.prototype,"_showAdaptive",2),a([l()],b.prototype,"_skipReason",2),a([l()],b.prototype,"_resetDate",2),a([l()],b.prototype,"_features",2),a([l()],b.prototype,"_toast",2);customElements.get("maintenance-task-quick-actions-dialog")||customElements.define("maintenance-task-quick-actions-dialog",b);var C=class extends ${constructor(){super(...arguments);this._open=!1;this._entryId=null;this._data=null;this._busy=!1;this._error=""}get _lang(){return this.hass?.language||"en"}async openFor(t){this._entryId=t,this._error="",this._open=!0,await this._load()}close(){this._open=!1,this._data=null,this._error=""}async _load(){if(this._entryId)try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._data=t}catch(t){this._error=k(t,this._lang)}}_onEditObject(){!this._entryId||!this._data||import("./dialog-mount-FFUYRDBT.js").then(({openEditObjectDialog:t})=>{t(this._entryId,this._data.object),this.close()})}_onAddTask(){this._entryId&&import("./dialog-mount-FFUYRDBT.js").then(({openCreateTaskDialog:t})=>{t(),this.close()})}async _onDelete(){if(!this._entryId||!this._data)return;let t=s("delete_object_confirm",this._lang)||`Delete "${this._data.object.name}" and all its tasks?`;if(window.confirm(t)){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:this._entryId}),this.dispatchEvent(new CustomEvent("object-deleted",{detail:{entry_id:this._entryId},bubbles:!0,composed:!0})),this.close()}catch(e){this._error=k(e,this._lang)}finally{this._busy=!1}}}async _onArchiveObject(){if(!this._entryId||!this._data)return;let t=!!this._data.object.archived;if(!t){let e=s("confirm_archive_object",this._lang)||"Archive this object and its tasks?";if(!window.confirm(e))return}this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:t?"maintenance_supporter/object/unarchive":"maintenance_supporter/object/archive",entry_id:this._entryId}),this.dispatchEvent(new CustomEvent("object-changed",{detail:{entry_id:this._entryId},bubbles:!0,composed:!0})),this.close()}catch(e){this._error=k(e,this._lang)}finally{this._busy=!1}}_onTaskClick(t){this._entryId&&import("./dialog-mount-FFUYRDBT.js").then(({openTaskQuickActions:e})=>{e(this._entryId,t)})}render(){if(!this._open)return c;let t=this._lang,e=this._data,i=e?.object,o=e?.tasks||[],u=this.hass?.user?.is_admin??!0;return r`
      <div class="backdrop" @click=${this.close}></div>
      <div class="dialog" role="dialog" aria-modal="true">
        ${e&&i?r`
              <div class="header">
      ...
      <div class="meta">
        ${i.map(([o,u])=>r`
            <div class="meta-item">
              ...
              <span class="meta-value">${/^https?:\/\//i.test(u)?r`<a href="${u}" target="_blank" rel="noopener noreferrer">${u}</a>`:u}</span>
            </div>
          `)}
      </div>
    `}};C.styles=x`
  ...
  `,a([g({attribute:!1})],C.prototype,"hass",2),a([l()],C.prototype,"_open",2),a([l()],C.prototype,"_entryId",2),a([l()],C.prototype,"_data",2),a([l()],C.prototype,"_busy",2),a([l()],C.prototype,"_error",2);customElements.get("maintenance-object-quick-actions-dialog")||customElements.define("maintenance-object-quick-actions-dialog",C);var ht="maintenance-object-dialog",_t="maintenance-task-dialog",Ft="maintenance-history-edit-dialog",zt="maintenance-complete-dialog",qt="maintenance-qr-dialog",Mt="maintenance-task-quick-actions-dialog",Nt="maintenance-object-quick-actions-dialog";function G(){return document.querySelector("home-assistant")?.hass}function P(n){let d=document.body.querySelector(n);return d||(d=document.createElement(n),document.body.appendChild(d)),d}function F(n){let d=G();if(!d)return!1;n.hass=d;let t=d.language||"en";return Z(t)||J(t).then(()=>{n.requestUpdate?.()}),!0}var pt={features:{adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1},defaultWarningDays:7},B=null;function ut(n){return B||(B=n.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).then(d=>({features:d.features??pt.features,defaultWarningDays:d.general?.default_warning_days??7})).catch(()=>pt),B)}function ri(){let n=P(ht);return F(n)?(n.openCreate(),!0):!1}function ni(n,d){let t=P(ht);return F(t)?(t.openEdit(n,d),!0):!1}function oi(){let n=P(_t);if(!F(n))return!1;let d=G();return d?((async()=>{let t=await ut(d),e=n;e.checklistsEnabled=t.features.checklists,e.scheduleTimeEnabled=t.features.schedule_time,e.completionActionsEnabled=t.features.completion_actions,e.defaultWarningDays=t.defaultWarningDays,e.openCreate()})(),!0):!1}function li(n,d){let t=P(_t);if(!F(t))return!1;let e=G();return e?((async()=>{try{let[i,o]=await Promise.all([e.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:n}),ut(e)]),u=(i.tasks||[]).find(p=>p.id===d);if(!u){console.warn(`openEditTaskDialog: task ${d} not found in entry ${n}`);return}let _=t;_.checklistsEnabled=o.features.checklists,_.scheduleTimeEnabled=o.features.schedule_time,_.completionActionsEnabled=o.features.completion_actions,_.defaultWarningDays=o.defaultWarningDays,await _.openEdit(n,u)}catch(i){console.warn("openEditTaskDialog: failed to load task/features",i)}})(),!0):!1}function di(n){let d=P(Ft);return F(d)?(d.openEdit(n),!0):!1}function ci(n){let d=P(zt);return F(d)?(d.entryId=n.entry_id,d.taskId=n.task_id,d.taskName=n.task_name,d.checklist=n.checklist??[],d.adaptiveEnabled=!!n.adaptive_enabled,d.lang=G()?.language||"en",d.open(),!0):!1}function pi(n){let d=P(qt);return F(d)?(d.openForTask(n.entry_id,n.task_id,n.object_name,n.task_name),!0):!1}function hi(n,d){let t=P(Mt);return F(t)?(t.openFor(n,d),!0):!1}function _i(n){let d=P(Nt);return F(d)?(d.openFor(n),!0):!1}export{ci as openCompleteDialog,ri as openCreateObjectDialog,oi as openCreateTaskDialog,ni as openEditObjectDialog,li as openEditTaskDialog,di as openHistoryEditDialog,_i as openObjectQuickActions,pi as openQrDialog,hi as openTaskQuickActions};
