...
`).map(m=>m.trim()).filter(Boolean).slice(0,100);t.checklist=p.length?p:null}if(this.completionActionsEnabled){let p=this._actionService.trim();if(p&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(p)){let I={service:p},F=this._actionTargetEntity.trim();F&&(I.target={entity_id:F});let w=this._buildActionData();Object.keys(w).length>0&&(I.data=w),t.on_complete_action=I}else t.on_complete_action=null;let m={};this._qcNotes.trim()&&(m.notes=this._qcNotes.trim());let u=parseFloat(this._qcCost);!isNaN(u)&&u>=0&&(m.cost=u);let y=parseInt(this._qcDuration,10);!isNaN(y)&&y>=0&&(m.duration=y),this._qcFeedback&&(m.feedback=this._qcFeedback),t.quick_complete_defaults=Object.keys(m).length?m:null}let i=await this.hass.connection.sendMessagePromise(t),o=this._taskId||i?.task_id,_=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(o&&this._scheduleType==="sensor_based"&&_)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:o,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(t){this._error=k(t,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return c;let t=this._lang,e=this._triggerType==="compound";return r`
      ...
      <div class="select-row">
        ...
        >
          ${Et.map(i=>r`<option value=${i} ?selected=${i===this._triggerType}>${s(i,t)}</option>`)}
        </select>
      </div>
    ...
    `}_patchCondition(t,e){this._compoundConditions=this._compoundConditions.map((i,o)=>o===t?{...i,...e}:i)}_addCondition(){this._compoundConditions=[...this._compoundConditions,It()]}_removeCondition(t){this._compoundConditions=this._compoundConditions.filter((e,i)=>i!==t)}_renderCompoundEditor(){let t=this._lang;return r`
      <div class="select-row">
    ...
    `}_renderConditionTypeFields(t,e){let i=this._lang;return t.type==="threshold"?r`
      ...
      ${this._endsMode==="until"?r`
        ...
        ></ms-textfield>`:c}
    `}_renderCalendarFields(){let t=this._lang,e=Lt(t);if(this._scheduleType==="weekdays")return r`
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${xt.map(i=>r`<option value=${i} ?selected=${i===this._type}>${s(i,t)}</option>`)}
            </select>
          </div>
          ...
          <div class="select-row">
            ...
            >
              ${kt.map(i=>r`<option value=${i} ?selected=${i===this._priority}>${s("priority_"+i,t)}</option>`)}
            </select>
          </div>
          <div class="field">
          ...
          <div class="select-row">
            ...
            >
              ${wt.map(i=>r`<option value=${i} ?selected=${i===this._scheduleType}>${s(i,t)}</option>`)}
            </select>
          </div>
...
<script>setTimeout(()=>window.print(),300)<\/script>
</body></html>`),o.document.close()}_downloadSvg(t,e){let i=decodeURIComponent(t.svg_data_uri.replace("data:image/svg+xml,","")),o=new Blob([i],{type:"image/svg+xml"}),_=URL.createObjectURL(o),p=document.createElement("a");p.href=_;let m=this._taskName?`${this._objectName}-${this._taskName}`:this._objectName;p.download=`qr-${Ht(m)}-${e}.svg`,p.click(),URL.revokeObjectURL(_)}_close(){this._open=!1,this._viewResult=null,this._completeResult=null,this._error="",this._loading=!1}render(){if(!this._open)return r``;let t=this.lang||this.hass?.language||"en",e=this._taskName?`${s("qr_code",t)}: ${this._objectName} \u2014 ${this._taskName}`:`${s("qr_code",t)}: ${this._objectName}`,i=!!this._viewResult;return r`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${e}</div>
        <div class="content">
  ...
  `}function Mt(n,d,t){let e=n.confidence_interval_low,i=n.confidence_interval_high,o=d.suggested_interval??d.interval_days??0,_=d.interval_days??0,p=Math.max(0,e-5),u=i+5-p,y=(e-p)/u*100,I=(i-e)/u*100,F=(o-p)/u*100,w=_>0?(_-p)/u*100:-1;return r`
  ...
  `}function ht(n,d){return Ft(n,d)}function Ft(n,d){let t=n.seasonal_factors??n.interval_analysis?.seasonal_factors;if(!t||t.length!==12)return c;let e=n.interval_analysis?.seasonal_reason,i=new Date().getMonth(),o=300,_=100,p=8,u=_-p-4,y=Math.max(...t,1.5),I=o/12,F=I*.65,w=p+u-1/y*u;return r`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${o} ${_}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",d)}">
        ...
        ${t.map((H,S)=>{let j=H/y*u,K=S*I+(I-F)/2,q=p+u-j,D=S===i,O=H<1?"var(--success-color, #4caf50)":H>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return z`
      ...
      <div class="dialog" role="dialog" aria-modal="true">
        ${e?r`
              <div class="header">
                ...
                <div class="object">
                  <button class="link-inline" @click=${()=>{this._entryId&&import("./dialog-mount-G5UC7O4D.js").then(({openObjectQuickActions:o})=>{o(this._entryId),this.close()})}}>${this._objectName}</button>
                </div>
  ...
  `,a([g({attribute:!1})],C.prototype,"hass",2),a([l()],C.prototype,"_open",2),a([l()],C.prototype,"_entryId",2),a([l()],C.prototype,"_data",2),a([l()],C.prototype,"_busy",2),a([l()],C.prototype,"_error",2);customElements.get("maintenance-object-quick-actions-dialog")||customElements.define("maintenance-object-quick-actions-dialog",C);var ut="maintenance-object-dialog",mt="maintenance-task-dialog",qt="maintenance-history-edit-dialog",zt="maintenance-complete-dialog",Nt="maintenance-qr-dialog",Ut="maintenance-task-quick-actions-dialog",jt="maintenance-object-quick-actions-dialog";function G(){return document.querySelector("home-assistant")?.hass}function R(n){let d=document.body.querySelector(n);return d||(d=document.createElement(n),document.body.appendChild(d)),d}function M(n){let d=G();if(!d)return!1;n.hass=d;let t=d.language||"en";return Z(t)||J(t).then(()=>{n.requestUpdate?.()}),!0}var _t={features:{adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1},defaultWarningDays:7},B=null;function gt(n){return B||(B=n.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).then(d=>({features:d.features??_t.features,defaultWarningDays:d.general?.default_warning_days??7})).catch(()=>_t),B)}function oi(){let n=R(ut);return M(n)?(n.openCreate(),!0):!1}function li(n,d){let t=R(ut);return M(t)?(t.openEdit(n,d),!0):!1}function di(){let n=R(mt);if(!M(n))return!1;let d=G();return d?((async()=>{let t=await gt(d),e=n;e.checklistsEnabled=t.features.checklists,e.scheduleTimeEnabled=t.features.schedule_time,e.completionActionsEnabled=t.features.completion_actions,e.defaultWarningDays=t.defaultWarningDays,e.openCreate()})(),!0):!1}function ci(n,d){let t=R(mt);if(!M(t))return!1;let e=G();return e?((async()=>{try{let[i,o]=await Promise.all([e.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:n}),gt(e)]),_=(i.tasks||[]).find(m=>m.id===d);if(!_){console.warn(`openEditTaskDialog: task ${d} not found in entry ${n}`);return}let p=t;p.checklistsEnabled=o.features.checklists,p.scheduleTimeEnabled=o.features.schedule_time,p.completionActionsEnabled=o.features.completion_actions,p.defaultWarningDays=o.defaultWarningDays,await p.openEdit(n,_)}catch(i){console.warn("openEditTaskDialog: failed to load task/features",i)}})(),!0):!1}function pi(n){let d=R(qt);return M(d)?(d.openEdit(n),!0):!1}function hi(n){let d=R(zt);return M(d)?(d.entryId=n.entry_id,d.taskId=n.task_id,d.taskName=n.task_name,d.checklist=n.checklist??[],d.adaptiveEnabled=!!n.adaptive_enabled,d.lang=G()?.language||"en",d.open(),!0):!1}function _i(n){let d=R(Nt);return M(d)?(d.openForTask(n.entry_id,n.task_id,n.object_name,n.task_name),!0):!1}function ui(n,d){let t=R(Ut);return M(t)?(t.openFor(n,d),!0):!1}function mi(n){let d=R(jt);return M(d)?(d.openFor(n),!0):!1}export{hi as openCompleteDialog,oi as openCreateObjectDialog,di as openCreateTaskDialog,li as openEditObjectDialog,ci as openEditTaskDialog,pi as openHistoryEditDialog,mi as openObjectQuickActions,_i as openQrDialog,ui as openTaskQuickActions};
