...
`).map(_=>_.trim()).filter(Boolean).slice(0,100);t.checklist=h.length?h:null}if(this.completionActionsEnabled){let h=this._actionService.trim();if(h&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(h)){let y={service:h},b=this._actionTargetEntity.trim();b&&(y.target={entity_id:b});let $=this._buildActionData();Object.keys($).length>0&&(y.data=$),t.on_complete_action=y}else t.on_complete_action=null;let _={};this._qcNotes.trim()&&(_.notes=this._qcNotes.trim());let u=parseFloat(this._qcCost);!isNaN(u)&&u>=0&&(_.cost=u);let v=parseInt(this._qcDuration,10);!isNaN(v)&&v>=0&&(_.duration=v),this._qcFeedback&&(_.feedback=this._qcFeedback),t.quick_complete_defaults=Object.keys(_).length?_:null}let i=await this.hass.connection.sendMessagePromise(t),n=this._taskId||i?.task_id,d=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(n&&this._scheduleType==="sensor_based"&&d)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:n,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}if(n&&this._adaptiveSnapshot()!==this._adaptiveInitial){let h=parseFloat(this._adaptiveAlpha),_=parseInt(this._adaptiveMin,10),u=parseInt(this._adaptiveMax,10);try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_adaptive",entry_id:this._entryId,task_id:n,enabled:this._adaptiveEnabled,...h>=.1&&h<=.9?{ewa_alpha:h}:{},...!isNaN(_)&&_>=1?{min_interval_days:_}:{},...!isNaN(u)&&u>=1?{max_interval_days:u}:{},seasonal_enabled:this._adaptiveSeasonal,sensor_prediction_enabled:this._adaptivePrediction}),this._adaptiveInitial=this._adaptiveSnapshot()}catch{}}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(t){this._error=I(t,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1,this._pickerProbeTimer!==void 0&&(clearTimeout(this._pickerProbeTimer),this._pickerProbeTimer=void 0),this._pickerProbeStrikes=0}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return c;let t=this._lang,e=this._triggerType==="compound";return a`
      ...
      <div class="select-row">
        ...
        >
          ${Ft.map(i=>a`<option value=${i} ?selected=${i===this._triggerType}>${s(i,t)}</option>`)}
        </select>
      </div>
    ...
    `}_patchCondition(t,e){this._compoundConditions=this._compoundConditions.map((i,n)=>n===t?{...i,...e}:i)}_addCondition(){this._compoundConditions=[...this._compoundConditions,Nt()]}_removeCondition(t){this._compoundConditions=this._compoundConditions.filter((e,i)=>i!==t)}_renderCompoundEditor(){let t=this._lang;return a`
      <div class="select-row">
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${Rt.map(i=>a`<option value=${i} ?selected=${i===this._priority}>${s("priority_"+i,t)}</option>`)}
            </select>
          </div>
          <div class="field">
          ...
          <div class="select-row">
            ...
            >
              ${qt.map(i=>a`<option value=${i} ?selected=${i===this._scheduleType}>${s(i,t)}</option>`)}
            </select>
          </div>
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          ${this.parts.length?a`<div class="used-parts">
                ...
                ${this.parts.map(e=>{let i=O({part_id:e.id,entry_id:e.entry_id}),n=this._usedParts[i],d=n!==void 0,h=e.entry_id?{part_id:e.id,quantity:1,entry_id:e.entry_id}:{part_id:e.id,quantity:1};return a`<div class="used-part-row">
...
<script>setTimeout(()=>window.print(),300)<\/script>
</body></html>`),n.document.close()}_downloadSvg(t,e){let i=decodeURIComponent(t.svg_data_uri.replace("data:image/svg+xml,","")),n=new Blob([i],{type:"image/svg+xml"}),d=URL.createObjectURL(n),h=document.createElement("a");h.href=d;let _=this._taskName?`${this._objectName}-${this._taskName}`:this._objectName;h.download=`qr-${Dt(_)}-${e}.svg`,h.click(),URL.revokeObjectURL(d)}_close(){this._open=!1,this._viewResult=null,this._completeResult=null,this._error="",this._loading=!1}render(){if(!this._open)return a``;let t=this.lang||this.hass?.language||"en",e=this._taskName?`${s("qr_code",t)}: ${this._objectName} \u2014 ${this._taskName}`:`${s("qr_code",t)}: ${this._objectName}`,i=!!this._viewResult;return a`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${e}</div>
        <div class="content">
  ...
  `}function Kt(o,p,t){let e=o.confidence_interval_low,i=o.confidence_interval_high,n=p.suggested_interval??p.interval_days??0,d=p.interval_days??0,h=Math.max(0,e-5),u=i+5-h,v=(e-h)/u*100,y=(i-e)/u*100,b=(n-h)/u*100,$=d>0?(d-h)/u*100:-1;return a`
      ...
      <div class="prediction-grid">
        ...
        ${i?a`
          <div class="prediction-item">
            ...
            ${o.threshold_prediction_date?a`<span class="prediction-date">${z(o.threshold_prediction_date,p)}</span>`:c}
        ...
        ${n&&t.environmental?a`
          <div class="prediction-item">
            ...
            ${o.environmental_entity?a`<span class="prediction-entity entity-link" @click=${h=>dt(h,o.environmental_entity)}>${o.environmental_entity}</span>`:c}
          </div>
        `:c}
      </div>
    </div>
  `}function yt(o,p,t,e){let i=Math.max(o||1,p);return a`
  ...
  `}var bt=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"];function $t(o,p,t){if(!t.seasonal||!o.seasonal_factor||o.seasonal_factor===1)return c;let e=bt.map(h=>s(h,p)),i=new Date().getMonth(),n=o.seasonal_factors||o.interval_analysis?.seasonal_factors||null,d=n&&n.length===12?n:e.map((h,_)=>{let u=o.seasonal_factor||1,v=Math.sin((_-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,u+v))});return a`
  ...
  `}function xt(o,p){return Gt(o,p)}function Gt(o,p){let t=o.seasonal_factors??o.interval_analysis?.seasonal_factors;if(!t||t.length!==12)return c;let e=o.interval_analysis?.seasonal_reason,i=new Date().getMonth(),n=300,d=100,h=8,u=d-h-4,v=Math.max(...t,1.5),y=n/12,b=y*.65,$=h+u-1/v*u;return a`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${n} ${d}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",p)}">
        ...
        ${t.map((R,q)=>{let V=R/v*u,J=q*y+(y-b)/2,U=h+u-V,B=q===i,W=R<1?"var(--success-color, #4caf50)":R>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return j`
                ...
                ${[...i].reverse().slice(0,20).map(_=>{let u=["completed","reset","skipped"].includes(_.type);return a`
                    <div class="history-entry">
                      <div class="history-line">
                        ...
                        <span class="history-date">${nt(_.timestamp,e)}</span>
      ...
      <div class="dialog" role="dialog" aria-modal="true">
        ${e?a`
              <div class="header">
                ...
                <div class="object">
                  <button class="link-inline" @click=${()=>{this._entryId&&import("./dialog-mount-FV7TZSVY.js").then(({openObjectQuickActions:n})=>{n(this._entryId),this.close()})}}>${this._objectName}</button>
                </div>
                      ...
                      <div class="task-list">
                        ${n.map(h=>a`
                          <div class="task-row" @click=${()=>this._onTaskClick(h.id)}>
                            <span class="status-dot" style="background: ${K[h.status]||"#ccc"}"></span>
  ...
  `,r([g({attribute:!1})],H.prototype,"hass",2),r([l()],H.prototype,"_open",2),r([l()],H.prototype,"_entryId",2),r([l()],H.prototype,"_data",2),r([l()],H.prototype,"_busy",2),r([l()],H.prototype,"_error",2);customElements.get("maintenance-object-quick-actions-dialog")||customElements.define("maintenance-object-quick-actions-dialog",H);var Et="maintenance-object-dialog",It="maintenance-task-dialog",Yt="maintenance-history-edit-dialog",Qt="maintenance-complete-dialog",Zt="maintenance-qr-dialog",Jt="maintenance-task-quick-actions-dialog",Xt="maintenance-object-quick-actions-dialog";function Z(){return document.querySelector("home-assistant")?.hass}function te(){return document.querySelector("home-assistant")?.shadowRoot??document.body}function N(o){let p=te(),t=p.querySelector(o)??document.body.querySelector(o);return t?t.parentNode!==p&&p.appendChild(t):(t=document.createElement(o),p.appendChild(t)),t}function M(o){let p=Z();if(!p)return!1;o.hass=p;let t=p.language||"en";return rt(t)||at(t).then(()=>{o.requestUpdate?.()}),!0}var wt={features:{adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1},defaultWarningDays:7},Q=null;function Tt(o){return Q||(Q=o.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).then(p=>({features:p.features??wt.features,defaultWarningDays:p.general?.default_warning_days??7})).catch(()=>wt),Q)}function Pi(){let o=N(Et);return M(o)?(o.openCreate(),!0):!1}function Ci(o,p){let t=N(Et);return M(t)?(t.openEdit(o,p),!0):!1}function Hi(){let o=N(It);if(!M(o))return!1;let p=Z();return p?((async()=>{let t=await Tt(p),e=o;e.checklistsEnabled=t.features.checklists,e.scheduleTimeEnabled=t.features.schedule_time,e.completionActionsEnabled=t.features.completion_actions,e.defaultWarningDays=t.defaultWarningDays,e.openCreate()})(),!0):!1}function Ri(o,p){let t=N(It);if(!M(t))return!1;let e=Z();return e?((async()=>{try{let[i,n]=await Promise.all([e.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:o}),Tt(e)]),d=(i.tasks||[]).find(_=>_.id===p);if(!d){console.warn(`openEditTaskDialog: task ${p} not found in entry ${o}`);return}let h=t;h.checklistsEnabled=n.features.checklists,h.scheduleTimeEnabled=n.features.schedule_time,h.completionActionsEnabled=n.features.completion_actions,h.defaultWarningDays=n.defaultWarningDays,await h.openEdit(o,d)}catch(i){console.warn("openEditTaskDialog: failed to load task/features",i)}})(),!0):!1}function qi(o){let p=N(Yt);return M(p)?(p.openEdit(o),!0):!1}function Fi(o){let p=N(Qt);return M(p)?(p.entryId=o.entry_id,p.taskId=o.task_id,p.taskName=o.task_name,p.checklist=o.checklist??[],p.adaptiveEnabled=!!o.adaptive_enabled,p.requiredFields=o.required_completion_fields??[],p.lang=Z()?.language||"en",p.open(),!0):!1}function Ni(o){let p=N(Zt);return M(p)?(p.openForTask(o.entry_id,o.task_id,o.object_name,o.task_name),!0):!1}function Mi(o,p){let t=N(Jt);return M(t)?(t.openFor(o,p),!0):!1}function Oi(o){let p=N(Xt);return M(p)?(p.openFor(o),!0):!1}export{Fi as openCompleteDialog,Pi as openCreateObjectDialog,Hi as openCreateTaskDialog,Ci as openEditObjectDialog,Ri as openEditTaskDialog,qi as openHistoryEditDialog,Oi as openObjectQuickActions,Ni as openQrDialog,Mi as openTaskQuickActions};
