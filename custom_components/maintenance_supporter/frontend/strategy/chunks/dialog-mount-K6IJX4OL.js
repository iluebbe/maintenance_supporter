...
<script>setTimeout(()=>window.print(),300)<\/script>
</body></html>`),o.document.close()}_downloadSvg(t,e){let i=decodeURIComponent(t.svg_data_uri.replace("data:image/svg+xml,","")),o=new Blob([i],{type:"image/svg+xml"}),_=URL.createObjectURL(o),p=document.createElement("a");p.href=_;let m=this._taskName?`${this._objectName}-${this._taskName}`:this._objectName;p.download=`qr-${Ct(m)}-${e}.svg`,p.click(),URL.revokeObjectURL(_)}_close(){this._open=!1,this._viewResult=null,this._completeResult=null,this._error="",this._loading=!1}render(){if(!this._open)return r``;let t=this.lang||this.hass?.language||"en",e=this._taskName?`${s("qr_code",t)}: ${this._objectName} \u2014 ${this._taskName}`:`${s("qr_code",t)}: ${this._objectName}`,i=!!this._viewResult;return r`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${e}</div>
        <div class="content">
  ...
  `,a([g({attribute:!1})],T.prototype,"hass",2),a([g()],T.prototype,"lang",2),a([l()],T.prototype,"_open",2),a([l()],T.prototype,"_loading",2),a([l()],T.prototype,"_error",2),a([l()],T.prototype,"_viewResult",2),a([l()],T.prototype,"_completeResult",2),a([l()],T.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",T);function nt(n,d){let t=n.interval_analysis,e=t?.weibull_beta,i=t?.weibull_eta;if(e==null||i==null||i<=0)return c;let o=n.interval_days??0,_=n.suggested_interval??o;return r`
    <div class="weibull-section">
      ...
      ${St(e,i,o,_,d)}
      ${Pt(t,d)}
      ${t?.confidence_interval_low!=null?Rt(t,n,d):c}
    </div>
  ...
  `}function St(n,d,t,e,i){let w=Math.max(t,e,d,1)*1.3,H=50,S=[];for(let A=0;A<=H;A++){let L=A/H*w,ft=1-Math.exp(-Math.pow(L/d,n)),bt=32+L/w*260,yt=136-ft*128;S.push([bt,yt])}let j=S.map(([A,L])=>`${A.toFixed(1)},${L.toFixed(1)}`).join(" "),K="M32,136 "+S.map(([A,L])=>`L${A.toFixed(1)},${L.toFixed(1)}`).join(" ")+` L${S[H][0].toFixed(1)},136 Z`,q=32+t/w*260,D=1-Math.exp(-Math.pow(t/d,n)),O=136-D*128,gt=((1-D)*100).toFixed(0),Y=32+e/w*260,vt=[0,.25,.5,.75,1];return r`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_weibull",i)}">
        ${vt.map(A=>{let L=136-A*128;return z`
  ...
  `}function Rt(n,d,t){let e=n.confidence_interval_low,i=n.confidence_interval_high,o=d.suggested_interval??d.interval_days??0,_=d.interval_days??0,p=Math.max(0,e-5),u=i+5-p,y=(e-p)/u*100,I=(i-e)/u*100,F=(o-p)/u*100,w=_>0?(_-p)/u*100:-1;return r`
  ...
  `}function pt(n,d){return Mt(n,d)}function Mt(n,d){let t=n.seasonal_factors??n.interval_analysis?.seasonal_factors;if(!t||t.length!==12)return c;let e=n.interval_analysis?.seasonal_reason,i=new Date().getMonth(),o=300,_=100,p=8,u=_-p-4,y=Math.max(...t,1.5),I=o/12,F=I*.65,w=p+u-1/y*u;return r`
      ...
      <div class="dialog" role="dialog" aria-modal="true">
        ${e?r`
              <div class="header">
                ...
                <div class="object">
                  <button class="link-inline" @click=${()=>{this._entryId&&import("./dialog-mount-K6IJX4OL.js").then(({openObjectQuickActions:o})=>{o(this._entryId),this.close()})}}>${this._objectName}</button>
                </div>
