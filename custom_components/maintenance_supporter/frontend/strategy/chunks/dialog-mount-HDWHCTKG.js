...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function wt(n,l,t,e,s){let I=Math.max(t,e,l,1)*1.3,S=50,C=[];for(let T=0;T<=S;T++){let L=T/S*I,ut=1-Math.exp(-Math.pow(L/l,n)),mt=32+L/I*260,gt=136-ut*128;C.push([mt,gt])}let D=C.map(([T,L])=>`${T.toFixed(1)},${L.toFixed(1)}`).join(" "),K="M32,136 "+C.map(([T,L])=>`L${T.toFixed(1)},${L.toFixed(1)}`).join(" ")+` L${C[S][0].toFixed(1)},136 Z`,F=32+t/I*260,j=1-Math.exp(-Math.pow(t/l,n)),W=136-j*128,ht=((1-j)*100).toFixed(0),Z=32+e/I*260,_t=[0,.25,.5,.75,1];return r`
  ...
  `}function It(n,l,t){let e=n.confidence_interval_low,s=n.confidence_interval_high,c=l.suggested_interval??l.interval_days??0,_=l.interval_days??0,h=Math.max(0,e-5),g=s+5-h,y=(e-h)/g*100,E=(s-e)/g*100,P=(c-h)/g*100,I=_>0?(_-h)/g*100:-1;return r`
  ...
  `}function ot(n,l){return At(n,l)}function At(n,l){let t=n.seasonal_factors??n.interval_analysis?.seasonal_factors;if(!t||t.length!==12)return d;let e=n.interval_analysis?.seasonal_reason,s=new Date().getMonth(),c=300,_=100,h=8,g=_-h-4,y=Math.max(...t,1.5),E=c/12,P=E*.65,I=h+g-1/y*g;return r`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${c} ${_}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_seasonal",l)}">
        ...
        ${t.map((S,C)=>{let D=S/y*g,K=C*E+(E-P)/2,F=h+g-D,j=C===s,W=S<1?"var(--success-color, #4caf50)":S>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return M`
      ...
      <div class="dialog" role="dialog" aria-modal="true">
        ${e?r`
              <div class="header">
                ...
                <div class="object">
                  <button class="link-inline" @click=${()=>{this._entryId&&import("./dialog-mount-HDWHCTKG.js").then(({openObjectQuickActions:c})=>{c(this._entryId),this.close()})}}>${this._objectName}</button>
                </div>
