...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function Ht(n,d,t,e,i){let w=Math.max(t,e,d,1)*1.3,H=50,S=[];for(let A=0;A<=H;A++){let L=A/H*w,vt=1-Math.exp(-Math.pow(L/d,n)),ft=32+L/w*260,bt=136-vt*128;S.push([ft,bt])}let D=S.map(([A,L])=>`${A.toFixed(1)},${L.toFixed(1)}`).join(" "),K="M32,136 "+S.map(([A,L])=>`L${A.toFixed(1)},${L.toFixed(1)}`).join(" ")+` L${S[H][0].toFixed(1)},136 Z`,q=32+t/w*260,j=1-Math.exp(-Math.pow(t/d,n)),O=136-j*128,mt=((1-j)*100).toFixed(0),Y=32+e/w*260,gt=[0,.25,.5,.75,1];return r`
      ...
      <div class="dialog" role="dialog" aria-modal="true">
        ${e?r`
              <div class="header">
                ...
                <div class="object">
                  <button class="link-inline" @click=${()=>{this._entryId&&import("./dialog-mount-MWBCC7OP.js").then(({openObjectQuickActions:o})=>{o(this._entryId),this.close()})}}>${this._objectName}</button>
                </div>
                      ...
                      <div class="task-list">
                        ${o.map(_=>r`
                          <div class="task-row" @click=${()=>this._onTaskClick(_.id)}>
                            <span class="status-dot" style="background: ${V[_.status]||"#ccc"}"></span>
