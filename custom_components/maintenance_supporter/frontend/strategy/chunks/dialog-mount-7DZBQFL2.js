...
import{a as dt,b as ft,c as ct,d as bt,e as yt,f as xt,g as $t,h as kt}from"./chunk-HCTB4FTI.js";import"./chunk-HBXVZDNV.js";import{A,a as w,b as a,c as J,d as _,e as E,f as g,g as c,i as s,k as L,l as wt,m as Et,n as It,o as St,p as S,q as Tt,r as Y,s as At,t as Pt,u as pt,v as Ct,w as Lt,x as qt,y as Rt,z as Ht}from"./chunk-J3IGT6RD.js";import{a as o,b as it}from"./chunk-LRS2DZQN.js";var P=class extends E{constructor(){super(...arguments);this.label="";this.value="";this.placeholder="";this.type="text";this.required=!1;this.disabled=!1;this.multiline=!1;this.rows=3}_onInput(t){let e=t.target.value;this.value=e,this.dispatchEvent(new CustomEvent("input",{bubbles:!0,composed:!0,detail:{value:e}}))}render(){return a`
      <label class="field">
      ...
      <div class="consumes-row">
        <label class="consumes-check">
          <input
            type="checkbox"
            .checked=${r!==void 0}
            @change=${p=>{let h={...this._consumesParts};p.target.checked?h[i]=h[i]||d:delete h[i],this._consumesParts=h}}
          />
        ...
        ${r!==void 0?a`<input
              ...
              @input=${p=>{let h=parseFloat(p.target.value);this._consumesParts={...this._consumesParts,[i]:{...d,quantity:Number.isFinite(h)&&h>=.01?h:1}}}}
            />`:_}
      </div>
    `}_toggleRequired(t,e){let i=new Set(this._requiredCompletion);e?i.add(t):i.delete(t),this._requiredCompletion=[...i]}_patchReading(t,e){this._readings=this._readings.map(i=>i.id===t?{...i,...e}:i)}_renderReadingsEditor(t){let e=$t(this._readings);return a`
      <div class="readings-editor">
        <div class="field-label">${s("readings_section",t)}</div>
        <div class="field-help">${s("readings_hint",t)}</div>
        ${this._readings.map(i=>a`
          ${e.has(i.id)?a`<div class="field-help reading-dup">${s("reading_duplicate_name",t)}</div>`:_}
          <div class="reading-row">
            <ms-textfield
              class="reading-name"
              label="${s("reading_name_label",t)}"
              .value=${i.name}
              @input=${r=>this._patchReading(i.id,{name:r.target.value})}
            ></ms-textfield>
            <ms-textfield
              class="reading-unit"
              label="${s("reading_unit_short",t)}"
              .value=${i.unit||""}
              @input=${r=>this._patchReading(i.id,{unit:r.target.value})}
            ></ms-textfield>
            <mwc-icon-button class="phase-remove reading-remove" @click=${()=>this._readings=this._readings.filter(r=>r.id!==i.id)}>
        ...
        <div class="phase-def">
          ...
          <label class="req-option phase-req-toggle">
            <input
              ...
              @change=${r=>this._patchPhaseDef(i.id,{reqOverride:r.target.checked})}
            />
            ...
            <div class="required-completion phase-req-fields">
              ${ut.map(r=>a`
                <label class="req-option">
                  <input
                    type="checkbox"
                    .checked=${i.reqFields.includes(r)}
                    @change=${d=>{let p=d.target.checked,h=new Set(i.reqFields);p?h.add(r):h.delete(r),this._patchPhaseDef(i.id,{reqFields:[...h]})}}
                  />
                  <span>${s(tt[r],t)}</span>
                </label>
              `)}
...
`).map(h=>h.trim()).filter(Boolean).slice(0,100);t.checklist=p.length?p:null}if(this.completionActionsEnabled){let p=this._actionService.trim();if(p&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(p)){let y={service:p},x=this._actionTargetEntity.trim();x&&(y.target={entity_id:x});let f=this._buildActionData();Object.keys(f).length>0&&(y.data=f),t.on_complete_action=y}else t.on_complete_action=null;let h={};this._qcNotes.trim()&&(h.notes=this._qcNotes.trim());let u=parseFloat(this._qcCost);!isNaN(u)&&u>=0&&(h.cost=u);let v=parseInt(this._qcDuration,10);!isNaN(v)&&v>=0&&(h.duration=v),this._qcFeedback&&(h.feedback=this._qcFeedback),t.quick_complete_defaults=Object.keys(h).length?h:null}let i=await this.hass.connection.sendMessagePromise(t),r=this._taskId||i?.task_id,d=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(r&&this._scheduleType==="sensor_based"&&d)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:r,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}if(r&&this._adaptiveSnapshot()!==this._adaptiveInitial){let p=parseFloat(this._adaptiveAlpha),h=parseInt(this._adaptiveMin,10),u=parseInt(this._adaptiveMax,10);try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_adaptive",entry_id:this._entryId,task_id:r,enabled:this._adaptiveEnabled,...p>=.1&&p<=.9?{ewa_alpha:p}:{},...!isNaN(h)&&h>=1?{min_interval_days:h}:{},...!isNaN(u)&&u>=1?{max_interval_days:u}:{},seasonal_enabled:this._adaptiveSeasonal,sensor_prediction_enabled:this._adaptivePrediction}),this._adaptiveInitial=this._adaptiveSnapshot()}catch{}}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(t){this._error=A(t,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1,this._pickerProbeTimer!==void 0&&(clearTimeout(this._pickerProbeTimer),this._pickerProbeTimer=void 0),this._pickerProbeStrikes=0}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return _;let t=this._lang,e=this._triggerType==="compound";return a`
      ...
      <div class="select-row">
        ...
        >
          ${xe.map(i=>a`<option value=${i} ?selected=${i===this._triggerType}>${s(i,t)}</option>`)}
        </select>
      </div>
      ${e?this._renderCompoundEditor():a`
        ...
        <ha-form
          ...
          .hass=${this.hass}
          .schema=${[{name:"trigger_entities",selector:{entity:{multiple:!0,domain:ht}}}]}
      ...
      ${this._intervalDays?a`
          ...
          `:_}
    `}_patchCondition(t,e){this._compoundConditions=this._compoundConditions.map((i,r)=>r===t?{...i,...e}:i)}_addCondition(){this._compoundConditions=[...this._compoundConditions,$e()]}_removeCondition(t){this._compoundConditions=this._compoundConditions.filter((e,i)=>i!==t)}_renderCompoundEditor(){let t=this._lang;return a`
      <div class="select-row">
    ...
    `}_renderCondition(t,e){let i=this._lang,r=e+1;return a`
      <div class="compound-condition">
        ...
        <ha-form
          class="entity-picker-form"
          .hass=${this.hass}
          .schema=${[{name:"condition_entities",selector:{entity:{multiple:!0,domain:ht}}}]}
        ...
        <div class="select-row">
          ...
          >
            ${Vt.map(d=>a`<option value=${d} ?selected=${d===t.type}>${s(d,i)}</option>`)}
          </select>
        </div>
    ...
    `}_buildSchedule(){let t=i=>{let r=parseInt(this._calOffset,10)||0;return r&&(i.offset=Math.max(-15,Math.min(r,15))),i};if(this._scheduleType==="weekdays")return t({kind:"weekdays",weekdays:[...this._weekdays].sort((i,r)=>i-r)});if(this._scheduleType==="nth_weekday")return t({kind:"nth_weekday",nth:parseInt(this._nth,10),weekday:parseInt(this._nthWeekday,10)});let e={kind:"day_of_month",day:this._domLastDay?-1:parseInt(this._domDay,10)||1};return this._domBusiness&&(e.business=!0),t(e)}_recurrenceExtras(){let t={};if(this._seasonMonths.length&&(t.season_months=[...this._seasonMonths].sort((e,i)=>e-i)),this._endsMode==="count"){let e=parseInt(this._endsCount,10);e>=1&&(t.ends={count:e})}else this._endsMode==="until"&&this._endsUntil&&(t.ends={until:this._endsUntil});return t}_toggleSeasonMonth(t){this._seasonMonths=this._seasonMonths.includes(t)?this._seasonMonths.filter(e=>e!==t):[...this._seasonMonths,t]}_renderRecurrenceExtras(){let t=this._lang;if(!(this._scheduleType==="time_based"||rt.includes(this._scheduleType)))return _;let i=Se(t);return a`
      ...
      ${this._endsMode==="until"?a`
        ...
        ></ms-date-field>`:_}
    `}_renderCalendarFields(){let t=this._lang,e=Ie(t);if(this._scheduleType==="weekdays")return a`
      ...
      ></ms-textfield>`}_thresholdLimitsOverlap(){let t=parseFloat(this._triggerAbove),e=parseFloat(this._triggerBelow);return!isNaN(t)&&!isNaN(e)&&e>t}_renderTriggerLiveHint(){if(this._triggerType==="compound")return _;let t=this._triggerType==="threshold"&&this._thresholdLimitsOverlap()?a`<div class="trigger-live-hint warn">${s("trigger_hint_overlap",this._lang)}</div>`:_,e=this._triggerEntityId||this._triggerEntityIds[0];if(!e||!this.hass?.states)return t;let i=this.hass.states[e];if(!i)return t;let r=this._lang,d=i.attributes?.unit_of_measurement,p=typeof d=="string"&&d?` ${d}`:"",h=this._triggerAttribute?i.attributes?.[this._triggerAttribute]:i.state,u=typeof h=="number"?h:parseFloat(String(h)),v=h!=="unknown"&&h!=="unavailable"&&h!=null&&!isNaN(u),y=f=>S(f,r,{maximumFractionDigits:1}),x=[];if(this._triggerType==="threshold"){let f=parseFloat(this._triggerAbove),T=parseFloat(this._triggerBelow);if(isNaN(f)&&isNaN(T))return _;v&&x.push(s("trigger_hint_now",r).replace("{value}",y(u)+p)),isNaN(f)||x.push(s("trigger_hint_above",r).replace("{target}",y(f)+p)),isNaN(T)||x.push(s("trigger_hint_below",r).replace("{target}",y(T)+p))}else if(this._triggerType==="counter"){let f=parseFloat(this._triggerTargetValue);if(isNaN(f))return _;this._triggerDeltaMode?this._taskId?x.push(s("trigger_hint_counter_delta_edit",r).replace("{target}",y(f)+p)):v?x.push(s("trigger_hint_counter_delta",r).replace("{value}",y(u)+p).replace("{due}",y(u+f)+p).replace("{target}",y(f)+p)):x.push(s("trigger_hint_counter_delta_edit",r).replace("{target}",y(f)+p)):(v&&x.push(s("trigger_hint_now",r).replace("{value}",y(u)+p)),x.push(s("trigger_hint_counter_abs",r).replace("{target}",y(f)+p)))}else if(this._triggerType==="runtime"){let f=parseFloat(this._triggerRuntimeHours);if(isNaN(f))return _;x.push(s("trigger_hint_runtime",r).replace("{hours}",y(f))),x.push(s("trigger_hint_state_now",r).replace("{value}",String(i.state)))}else if(this._triggerType==="state_change"){let f=parseInt(this._triggerTargetChanges,10)||1,T=this._triggerToState.trim();x.push((T?s("trigger_hint_state_change_to",r).replace("{state}",T):s("trigger_hint_state_change",r)).replace("{count}",String(f))),x.push(s("trigger_hint_state_now",r).replace("{value}",String(i.state)))}return x.length?a`<div class="trigger-live-hint">${x.join(" ")}</div>${t}`:t}_renderTriggerTypeFields(){let t=this._lang;return this._triggerType==="threshold"?a`
        <ms-textfield
      ...
      `:this._triggerType==="state_change"?a`
        ...
        ${(this._triggerTargetChanges||"1")==="1"&&(this._triggerFromState||this._triggerToState)?a`<div class="field-help">${s("state_latch_help",t)}</div>`:_}
        <ms-textfield
          label="${s("for_at_least_minutes",t)}"
          type="number"
          min="0"
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${fe.map(i=>a`<option value=${i} ?selected=${i===this._type}>${s(i,t)}</option>`)}
            </select>
          </div>
          ...
          <div class="select-row">
            ...
            >
              ${be.map(i=>a`<option value=${i} ?selected=${i===this._priority}>${s("priority_"+i,t)}</option>`)}
            </select>
          </div>
          <div class="field">
          ...
          <div class="select-row">
            ...
            >
              ${ye.map(i=>a`<option value=${i} ?selected=${i===this._scheduleType}>${s(i,t)}</option>`)}
            </select>
          </div>
          ...
          ${this._scheduleType==="sensor_based"?a`
            ...
            <ha-form
              class="entity-picker-form"
              .hass=${this.hass}
              .schema=${[{name:"environmental_entity",selector:{entity:{domain:jt,device_class:Ut}}}]}
          ...
          ${this._requireTagScan?a`<div class="field-help">${s("require_tag_scan_help",t)}</div>`:_}
          <label class="req-option">
            <input
              type="checkbox"
          ...
          ${this._allowSkip?_:a`<div class="field-help">${s("disallow_skip_help",t)}</div>`}
          <label class="req-option">
            <input
              type="checkbox"
          ...
          ${this._notifyEnabled?_:a`<div class="field-help">${s("no_notifications_help",t)}</div>`}
  ...
  `,o([g({attribute:!1})],m.prototype,"hass",2),o([g({type:Boolean,attribute:"checklists-enabled"})],m.prototype,"checklistsEnabled",2),o([g({type:Boolean,attribute:"schedule-time-enabled"})],m.prototype,"scheduleTimeEnabled",2),o([g({type:Boolean,attribute:"completion-actions-enabled"})],m.prototype,"completionActionsEnabled",2),o([g({type:Number,attribute:"default-warning-days"})],m.prototype,"defaultWarningDays",2),o([c()],m.prototype,"parts",2),o([c()],m.prototype,"_foreignOwners",2),o([c()],m.prototype,"_open",2),o([c()],m.prototype,"_entityPickerFallback",2),o([c()],m.prototype,"_loading",2),o([c()],m.prototype,"_error",2),o([c()],m.prototype,"_entryId",2),o([c()],m.prototype,"_taskId",2),o([c()],m.prototype,"_objectChoices",2),o([c()],m.prototype,"_name",2),o([c()],m.prototype,"_type",2),o([c()],m.prototype,"_scheduleType",2),o([c()],m.prototype,"_intervalDays",2),o([c()],m.prototype,"_intervalUnit",2),o([c()],m.prototype,"_dueDate",2),o([c()],m.prototype,"_warningDays",2),o([c()],m.prototype,"_earliestCompletionDays",2),o([c()],m.prototype,"_intervalAnchor",2),o([c()],m.prototype,"_weekdays",2),o([c()],m.prototype,"_nth",2),o([c()],m.prototype,"_nthWeekday",2),o([c()],m.prototype,"_domDay",2),o([c()],m.prototype,"_domLastDay",2),o([c()],m.prototype,"_domBusiness",2),o([c()],m.prototype,"_calOffset",2),o([c()],m.prototype,"_seasonMonths",2),o([c()],m.prototype,"_endsMode",2),o([c()],m.prototype,"_endsCount",2),o([c()],m.prototype,"_endsUntil",2),o([c()],m.prototype,"_schedulePreview",2),o([c()],m.prototype,"_schedulePreviewEnded",2),o([c()],m.prototype,"_notes",2),o([c()],m.prototype,"_documentationUrl",2),o([c()],m.prototype,"_customIcon",2),o([c()],m.prototype,"_priority",2),o([c()],m.prototype,"_labels",2),o([c()],m.prototype,"_enabled",2),o([c()],m.prototype,"_triggerEntityId",2),o([c()],m.prototype,"_triggerEntityIds",2),o([c()],m.prototype,"_triggerEntityLogic",2),o([c()],m.prototype,"_triggerAttribute",2),o([c()],m.prototype,"_triggerType",2),o([c()],m.prototype,"_triggerAbove",2),o([c()],m.prototype,"_triggerBelow",2),o([c()],m.prototype,"_triggerEquals",2),o([c()],m.prototype,"_triggerNotEquals",2),o([c()],m.prototype,"_triggerForMinutes",2),o([c()],m.prototype,"_triggerCombinator",2),o([c()],m.prototype,"_triggerTargetValue",2),o([c()],m.prototype,"_triggerDeltaMode",2),o([c()],m.prototype,"_triggerBaselineValue",2),o([c()],m.prototype,"_liveBaselineValue",2),o([c()],m.prototype,"_autoCompleteOnRecovery",2),o([c()],m.prototype,"_triggerFromState",2),o([c()],m.prototype,"_triggerToState",2),o([c()],m.prototype,"_triggerTargetChanges",2),o([c()],m.prototype,"_triggerRuntimeHours",2),o([c()],m.prototype,"_triggerRuntimeMaxSession",2),o([c()],m.prototype,"_triggerOnStates",2),o([c()],m.prototype,"_compoundLogic",2),o([c()],m.prototype,"_compoundConditions",2),o([c()],m.prototype,"_suggestedAttributes",2),o([c()],m.prototype,"_availableAttributes",2),o([c()],m.prototype,"_entityDomain",2),o([c()],m.prototype,"_lastPerformed",2),o([c()],m.prototype,"_nfcTagId",2),o([c()],m.prototype,"_requireTagScan",2),o([c()],m.prototype,"_allowSkip",2),o([c()],m.prototype,"_notifyEnabled",2),o([c()],m.prototype,"_readingUnit",2),o([c()],m.prototype,"_readings",2),o([c()],m.prototype,"_consumesParts",2),o([c()],m.prototype,"_partsLoadFailed",2),o([c()],m.prototype,"_availableTags",2),o([c()],m.prototype,"_responsibleUserId",2),o([c()],m.prototype,"_assigneePool",2),o([c()],m.prototype,"_rotationStrategy",2),o([c()],m.prototype,"_availableUsers",2),o([c()],m.prototype,"_checklistText",2),o([c()],m.prototype,"_phaseDefs",2),o([c()],m.prototype,"_phaseSeq",2),o([c()],m.prototype,"_requiredCompletion",2),o([c()],m.prototype,"_scheduleTime",2),o([c()],m.prototype,"_scheduleTimeOn",2),o([c()],m.prototype,"_actionService",2),o([c()],m.prototype,"_actionTargetEntity",2),o([c()],m.prototype,"_actionData",2),o([c()],m.prototype,"_actionDataJsonFallback",2),o([c()],m.prototype,"_actionTesting",2),o([c()],m.prototype,"_actionTestResult",2),o([c()],m.prototype,"_actionTestError",2),o([c()],m.prototype,"_qcNotes",2),o([c()],m.prototype,"_qcCost",2),o([c()],m.prototype,"_qcDuration",2),o([c()],m.prototype,"_qcFeedback",2),o([c()],m.prototype,"_environmentalEntity",2),o([c()],m.prototype,"_environmentalAttribute",2),o([c()],m.prototype,"_adaptiveEnabled",2),o([c()],m.prototype,"_adaptiveAlpha",2),o([c()],m.prototype,"_adaptiveMin",2),o([c()],m.prototype,"_adaptiveMax",2),o([c()],m.prototype,"_adaptiveSeasonal",2),o([c()],m.prototype,"_adaptivePrediction",2),o([c()],m.prototype,"_conditionAttrOptions",2);var _t=m;customElements.get("maintenance-task-dialog")||customElements.define("maintenance-task-dialog",_t);function nt(){if(window.externalApp!==void 0)return!0;let l=typeof navigator<"u"&&navigator.userAgent||"";return/Home Assistant\//.test(l)&&/Android/.test(l)}async function at(n,l,t){let e=new FormData;e.append("entry_id",l),e.append("tags","photo"),e.append("file",t,t.name);let i=await fetch("/api/maintenance_supporter/document/upload",{method:"POST",headers:{Authorization:`Bearer ${n.auth?.data?.access_token??""}`},body:e});if(i.status===413)throw new Error("doc_too_large");if(!i.ok)throw new Error("doc_upload_failed");let r=await i.json();if(!r.id)throw new Error("doc_upload_failed");return r.id}async function Z(n,l){await Promise.all(l.map(t=>n.connection.sendMessagePromise({type:"maintenance_supporter/documents/delete",doc_id:t}).catch(()=>{})))}function Bt(){if(!nt())return!1;let n=typeof navigator<"u"?navigator.mediaDevices:void 0;return!!n&&typeof n.getUserMedia=="function"}var Te=1920,Ae=.88,G=class extends E{constructor(){super(...arguments);this.lang="en";this._open=!1;this._busy=!1;this._stream=null}async open(){if(this._open)return;let t=typeof navigator<"u"?navigator.mediaDevices:void 0;if(!t||typeof t.getUserMedia!="function"){this._unavailable("no_media_devices");return}try{this._stream=await t.getUserMedia({video:{facingMode:{ideal:"environment"}},audio:!1})}catch(i){this._unavailable(i instanceof Error?i.name||i.message:String(i));return}this._open=!0,await this.updateComplete;let e=this._video;if(e&&this._stream){e.srcObject=this._stream;try{await e.play()}catch{}}}close(){this._stopStream(),this._open=!1,this._busy=!1}disconnectedCallback(){super.disconnectedCallback(),this._stopStream()}get _video(){return this.shadowRoot?.querySelector("video")??null}_stopStream(){for(let e of this._stream?.getTracks()??[])e.stop();this._stream=null;let t=this._video;t&&(t.srcObject=null)}_unavailable(t){this._stopStream(),this._open=!1,this.dispatchEvent(new CustomEvent("capture-unavailable",{detail:{reason:t},bubbles:!0,composed:!0}))}async _shoot(){let t=this._video;if(!(!t||this._busy)){this._busy=!0;try{let e=t.videoWidth||640,i=t.videoHeight||480,r=Math.min(1,Te/Math.max(e,i)),d=document.createElement("canvas");d.width=Math.round(e*r),d.height=Math.round(i*r);let p=d.getContext("2d");if(!p)throw new Error("no_canvas");p.drawImage(t,0,0,d.width,d.height);let h=await new Promise(y=>d.toBlob(y,"image/jpeg",Ae));if(!h)throw new Error("no_blob");let u=new Date().toISOString().replace(/[-:]/g,"").slice(0,15),v=new File([h],`photo-${u}.jpg`,{type:"image/jpeg"});this.close(),this.dispatchEvent(new CustomEvent("photo-captured",{detail:{file:v},bubbles:!0,composed:!0}))}catch(e){this._unavailable(e instanceof Error?e.message:String(e))}finally{this._busy=!1}}}render(){if(!this._open)return _;let t=this.lang;return a`
  ...
  `,o([g({type:String})],G.prototype,"lang",2),o([c()],G.prototype,"_open",2),o([c()],G.prototype,"_busy",2);customElements.get("ms-camera-capture")||customElements.define("ms-camera-capture",G);var b=class extends E{constructor(){super(...arguments);this.entryId="";this.taskId="";this.taskName="";this.lang="en";this.checklist=[];this.adaptiveEnabled=!1;this.taskType="";this.readingUnit="";this.readings=[];this.readingHistory=[];this.restockDefault=null;this.restockUnitCost=null;this.currencySymbol="";this.parts=[];this.consumesParts=[];this.consumesInfo=[];this.requiredFields=[];this.phaseLabel="";this.requireTagScan=!1;this.viaTagScan=!1;this._open=!1;this._notes="";this._cost="";this._duration="";this._loading=!1;this._error="";this._checklistState={};this._feedback="needed";this._photos=[];this._uploadedIds=[];this._photoUploading=!1;this._readingValue="";this._readingValues={};this._restockQty="";this._completedAt="";this._usedParts={};this.checklistPrefill={};this._singlePick=nt();this._inAppCamera=Bt()}open(t={}){this._open||(this._open=!0,this.viaTagScan=!!t.viaTagScan,this._notes="",this._cost="",this._duration="",this._error="",this._checklistState=Object.fromEntries(this.checklist.map((e,i)=>[String(i),!!this.checklistPrefill[e]]).filter(([,e])=>e)),this._feedback="needed",this._photos.forEach(e=>URL.revokeObjectURL(e.preview)),this._photos=[],this._uploadedIds=[],this._photoUploading=!1,this._readingValue="",this._readingValues={},this._restockQty=this.restockDefault!==null?String(this.restockDefault):"",this._completedAt="",this._usedParts=Object.fromEntries(this.consumesParts.map(e=>[O(e),{...e}])))}_toggleCheck(t){let e=String(t);this._checklistState={...this._checklistState,[e]:!this._checklistState[e]}}_setFeedback(t){this._feedback=t}_onCameraClick(t){!this._inAppCamera||this._photoUploading||(t.preventDefault(),this.shadowRoot?.querySelector("ms-camera-capture")?.open())}_onCameraUnavailable(){this._inAppCamera=!1,this.shadowRoot?.querySelector(".photo-pick-camera input")?.click()}async _onPhotoInput(t){let e=t.target,i=Array.from(e.files??[]);e.value="",await this._addPhotoFiles(i)}async _addPhotoFiles(t){if(t.length===0)return;let e=10-this._photos.length,i=t.slice(0,Math.max(e,0));this._photoUploading=!0,this._error="";try{for(let r of i){let d=await at(this.hass,this.entryId,r);this._uploadedIds=[...this._uploadedIds,d],this._photos=[...this._photos,{id:d,preview:URL.createObjectURL(r)}]}t.length>i.length&&(this._error=s("photos_limit",this.lang).replace("{max}",String(10)))}catch(r){let d=r instanceof Error&&r.message==="doc_too_large"?"doc_too_large":"doc_upload_failed";this._error=s(d,this.lang)}finally{this._photoUploading=!1}}_removePhoto(t){let e=this._photos.find(i=>i.id===t);e&&URL.revokeObjectURL(e.preview),this._photos=this._photos.filter(i=>i.id!==t),this._uploadedIds.includes(t)&&(this._uploadedIds=this._uploadedIds.filter(i=>i!==t),Z(this.hass,[t]))}async _complete(){this._loading=!0,this._error="";try{let t={type:"maintenance_supporter/task/complete",entry_id:this.entryId,task_id:this.taskId};if(this._notes&&(t.notes=this._notes),this._cost){let e=parseFloat(this._cost);!isNaN(e)&&e>=0&&(t.cost=e)}if(this._duration){let e=parseInt(this._duration,10);!isNaN(e)&&e>=0&&(t.duration=e)}if(this.checklist.length>0&&(t.checklist_state=this._checklistState),this.adaptiveEnabled&&(t.feedback=this._feedback),this._photos.length>0&&(t.photo_doc_ids=this._photos.map(e=>e.id)),this.viaTagScan&&(t.via_tag_scan=!0),this._completedAt){if(new Date(this._completedAt).getTime()>Date.now()){this._error=s("completed_at_future_error",this.lang),this._loading=!1;return}t.completed_at=this._completedAt.length===16?`${this._completedAt}:00`:this._completedAt}if(this.readings.length>0){let e={};for(let i of this.readings){let r=(this._readingValues[i.id]??"").trim();if(r==="")continue;let d=parseFloat(r.replace(",","."));isNaN(d)||(e[i.id]=d)}Object.keys(e).length>0&&(t.reading_values=e)}else if(this._readingValue!==""){let e=parseFloat(this._readingValue);isNaN(e)||(t.reading_value=e)}if(this.restockDefault!==null&&this._restockQty!==""){let e=parseFloat(this._restockQty);!isNaN(e)&&e>=1&&(t.restock_quantity=e)}this.parts.length>0&&(t.used_parts=Object.values(this._usedParts).filter(e=>Number.isFinite(e.quantity)&&e.quantity>0).map(e=>e.entry_id?{part_id:e.part_id,quantity:e.quantity,entry_id:e.entry_id}:{part_id:e.part_id,quantity:e.quantity})),await this.hass.connection.sendMessagePromise(t),this._uploadedIds=[],this._open=!1,this.dispatchEvent(new CustomEvent("task-completed"))}catch(t){this._error=A(t,this.lang,s("save_error",this.lang))}finally{this._loading=!1}}_renderReadingField(t,e){let i=this._completedAt?new Date(this._completedAt).getTime():NaN,r=yt(this.readingHistory,t.id,isNaN(i)?void 0:i),d=t.unit||this.readingUnit,p=(this._readingValues[t.id]??"").trim(),h=p===""?NaN:parseFloat(p.replace(",",".")),u=r!==void 0&&!isNaN(h)&&h<r.value,v=r!==void 0?S(r.value,e,{maximumFractionDigits:3}):"";return a`
      <label class="field reading-field">
        <span class="field-label">${t.name}${d?` (${d})`:""}</span>
        <input type="text" inputmode="decimal" class="field-input"
          placeholder=${r!==void 0?s("reading_last",e).replace("{value}",v):""}
          .value=${this._readingValues[t.id]??""}
          @input=${y=>{this._readingValues={...this._readingValues,[t.id]:y.target.value}}} />
        ${u?a`<span class="reading-warn">${s("reading_below_last",e).replace("{value}",v)}</span>`:_}
      </label>`}get _missingRequired(){let t={notes:this._notes.trim()!=="",cost:this._cost.trim()!=="",duration:this._duration.trim()!=="",photo:this._photos.length>0,user:!!this.hass?.user};return this.requiredFields.filter(e=>!t[e])}_req(t){return this.requiredFields.includes(t)?a`<span class="req-mark" aria-hidden="true">*</span>`:_}_partsCostSuggestion(){if(this.restockDefault!==null){let i=parseFloat(this._restockQty);return this.restockUnitCost==null||!Number.isFinite(i)||i<=0?null:Math.round(this.restockUnitCost*i*100)/100}if(!this.parts.length)return null;let t=0,e=!1;for(let i of Object.values(this._usedParts)){let r=this.parts.find(d=>O({part_id:d.id,entry_id:d.entry_id})===O(i));r?.cost!=null&&(t+=r.cost*(i.quantity||1),e=!0)}return e?Math.round(t*100)/100:null}_renderCostSuggestion(t){if(this._cost.trim()!=="")return _;let e=this._partsCostSuggestion();if(e==null||e<=0)return _;let i=Tt(e,this.currencySymbol,t);return a`<button
      type="button"
      class="cost-suggestion"
      @click=${()=>this._cost=String(Math.round(e*100)/100)}
    >${s("cost_from_parts",t).replace("{amount}",i)}</button>`}_close(){if(this._open=!1,this._uploadedIds.length>0){let t=this._uploadedIds;this._uploadedIds=[],Z(this.hass,t)}}_pickCompletedAt(){let t=new Date,e=i=>String(i).padStart(2,"0");this._completedAt=`${t.getFullYear()}-${e(t.getMonth()+1)}-${e(t.getDate())}T${e(t.getHours())}:${e(t.getMinutes())}:00`}render(){if(!this._open)return a``;let t=this.lang||this.hass?.language||"en";return a`
      <ha-dialog open @closed=${this._close}>
        ...
        ${this.requireTagScan&&!this.viaTagScan?a`<div class="scan-required-note">${s("require_tag_scan_hint",t)}</div>`:_}
        <div class="content">
          ${this._error?a`<div class="error">${this._error}</div>`:_}
          ...
          ${this.parts.length?a`<div class="used-parts">
                ...
                ${this.parts.map(e=>{let i=O({part_id:e.id,entry_id:e.entry_id}),r=this._usedParts[i],d=r!==void 0,p=e.entry_id?{part_id:e.id,quantity:1,entry_id:e.entry_id}:{part_id:e.id,quantity:1};return a`<div class="used-part-row">
                    <label class="used-part-check">
                      <input type="checkbox" .checked=${d}
                        @change=${h=>{let u={...this._usedParts};h.target.checked?u[i]=u[i]||p:delete u[i],this._usedParts=u}} />
                      <span
                    ...
                    ${d?a`<input class="used-part-qty" type="number" min="0.01" max="999" step="0.01"
                          .value=${String(r.quantity)}
                          @input=${h=>{let u=parseFloat(h.target.value);this._usedParts={...this._usedParts,[i]:{...p,quantity:Number.isFinite(u)&&u>=.01?u:1}}}} />`:_}
                  </div>`})}
          ...
          <div class="field">
            ...
            ${this._photos.length>0?a`<div class="photo-strip">
                  ${this._photos.map(e=>a`
            ...
            ${this._photos.length<10?a`<div class="photo-pickers">
                  ...
                  <label class="photo-pick photo-pick-gallery">
                    ...
                    <span>${s(this._singlePick?"choose_photo":"choose_photos",t)}</span>
                    <input type="file" accept="image/*" ?multiple=${!this._singlePick}
                      ?disabled=${this._photoUploading}
                      @change=${this._onPhotoInput} />
                  </label>
                </div>
                ${this._singlePick?a`<div class="photo-limit photo-android-hint">${s("photos_android_hint",t)}</div>`:_}
                ${this._inAppCamera?a`<ms-camera-capture .lang=${t}
                  @photo-captured=${e=>this._addPhotoFiles([e.detail.file])}
                  @capture-unavailable=${this._onCameraUnavailable}></ms-camera-capture>`:_}`:a`<div class="photo-limit">${s("photos_limit",t).replace("{max}",String(10))}</div>`}
          </div>
        ...
        <div class="dialog-actions">
          ...
          <ha-button
            ...
            title=${this._missingRequired.length?this._missingRequired.map(e=>s("err_required",t).replace("{field}",s(tt[e]??e,t))).join(" \xB7 "):""}
          >
  ...
  `],o([g({attribute:!1})],b.prototype,"hass",2),o([g()],b.prototype,"entryId",2),o([g()],b.prototype,"taskId",2),o([g()],b.prototype,"taskName",2),o([g()],b.prototype,"lang",2),o([g({type:Array})],b.prototype,"checklist",2),o([g({type:Boolean})],b.prototype,"adaptiveEnabled",2),o([g()],b.prototype,"taskType",2),o([g()],b.prototype,"readingUnit",2),o([g({attribute:!1})],b.prototype,"readings",2),o([g({attribute:!1})],b.prototype,"readingHistory",2),o([g({attribute:!1})],b.prototype,"restockDefault",2),o([g({attribute:!1})],b.prototype,"restockUnitCost",2),o([g()],b.prototype,"currencySymbol",2),o([g({attribute:!1})],b.prototype,"parts",2),o([g({attribute:!1})],b.prototype,"consumesParts",2),o([g({type:Array})],b.prototype,"consumesInfo",2),o([g({type:Array})],b.prototype,"requiredFields",2),o([g()],b.prototype,"phaseLabel",2),o([g({type:Boolean})],b.prototype,"requireTagScan",2),o([g({type:Boolean})],b.prototype,"viaTagScan",2),o([c()],b.prototype,"_open",2),o([c()],b.prototype,"_notes",2),o([c()],b.prototype,"_cost",2),o([c()],b.prototype,"_duration",2),o([c()],b.prototype,"_loading",2),o([c()],b.prototype,"_error",2),o([c()],b.prototype,"_checklistState",2),o([c()],b.prototype,"_feedback",2),o([c()],b.prototype,"_photos",2),o([c()],b.prototype,"_photoUploading",2),o([c()],b.prototype,"_readingValue",2),o([c()],b.prototype,"_readingValues",2),o([c()],b.prototype,"_restockQty",2),o([c()],b.prototype,"_completedAt",2),o([c()],b.prototype,"_usedParts",2),o([g({attribute:!1})],b.prototype,"checklistPrefill",2),o([c()],b.prototype,"_inAppCamera",2);customElements.get("maintenance-complete-dialog")||customElements.define("maintenance-complete-dialog",b);function Wt(n,l,t){let e=new Blob([n],{type:t}),i=URL.createObjectURL(e),r=document.createElement("a");r.href=i,r.download=l,r.target="_blank",r.rel="noopener",r.style.display="none",document.body.appendChild(r),r.dispatchEvent(new MouseEvent("click")),document.body.removeChild(r),setTimeout(()=>URL.revokeObjectURL(i),6e4)}async function Pe(n,l,t=300){return(await n.connection.sendMessagePromise({type:"auth/sign_path",path:l,expires:t})).path}async function Kt(n,l,t=300){return Pe(n,`/api/maintenance_supporter/document/${l}`,t)}var W=class extends E{constructor(){super(...arguments);this.docId="";this._url="";this._failed=!1;this._signedFor=""}updated(){this.hass&&this.docId&&this._signedFor!==this.docId&&(this._signedFor=this.docId,this._url="",this._failed=!1,this._sign())}async _sign(){try{this._url=await Kt(this.hass,this.docId)}catch{this._failed=!0}}render(){return this._failed||!this.docId?_:this._url?a`
      <a href=${this._url} target="_blank" rel="noopener" class="wrap">
        <img src=${this._url} alt="" loading="lazy"
          @error=${()=>this._failed=!0} />
      </a>`:a`<div class="ph"></div>`}};W.styles=w`
  ...
  `,o([g({attribute:!1})],W.prototype,"hass",2),o([g()],W.prototype,"docId",2),o([c()],W.prototype,"_url",2),o([c()],W.prototype,"_failed",2);customElements.get("maintenance-history-photo")||customElements.define("maintenance-history-photo",W);var C=class extends E{constructor(){super(...arguments);this._open=!1;this._saving=!1;this._error="";this._draft=null;this._originalSnapshot=null;this._partOptions=null;this._partQty={};this._partQtyOriginal="";this._photos=[];this._photosOriginal="";this._readingRows=[];this._readingText={};this._readingsOriginal="";this._uploadedIds=[];this._photoUploading=!1}get _lang(){return L(this.hass)}openEdit(t){this._draft={...t},this._originalSnapshot={...t},this._error="",this._open=!0,this._partOptions=null,this._partQty={},this._partQtyOriginal="",this._photos=[...t.photo_doc_ids??[]],this._photosOriginal=JSON.stringify(this._photos),this._uploadedIds=[],this._photoUploading=!1,this._seedReadings(t),this._loadPartOptions()}_seedReadings(t){let e=[],i=new Set;for(let d of t.readings??[])i.has(d.id)||(i.add(d.id),e.push({id:d.id,name:d.name,unit:d.unit??null}));for(let d of t.reading_values??[])i.has(d.id)||(i.add(d.id),e.push({id:d.id,name:d.name,unit:d.unit??null}));this._readingRows=e;let r={};for(let d of t.reading_values??[])r[d.id]=String(d.value);this._readingText=r,this._readingsOriginal=JSON.stringify(this._readingNumbers())}_readingNumbers(){let t={};for(let e of this._readingRows){let i=(this._readingText[e.id]??"").trim();if(i==="")continue;let r=parseFloat(i.replace(",","."));isNaN(r)||(t[e.id]=r)}return t}async _onPhotoInput(t){let e=t.target,i=Array.from(e.files??[]);e.value="";let r=this._draft;if(i.length===0||!r)return;let d=10-this._photos.length,p=i.slice(0,Math.max(d,0));this._photoUploading=!0,this._error="";try{for(let h of p){let u=await at(this.hass,r.entry_id,h);this._uploadedIds=[...this._uploadedIds,u],this._photos=[...this._photos,u]}i.length>p.length&&(this._error=s("photos_limit",this._lang).replace("{max}",String(10)))}catch(h){let u=h instanceof Error&&h.message==="doc_too_large"?"doc_too_large":"doc_upload_failed";this._error=s(u,this._lang)}finally{this._photoUploading=!1}}_removePhoto(t){this._photos=this._photos.filter(e=>e!==t),this._uploadedIds.includes(t)&&(this._uploadedIds=this._uploadedIds.filter(e=>e!==t),Z(this.hass,[t]))}async _loadPartOptions(){let t=this._draft;if(t)try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/parts/overview"}),i=[];for(let d of e.parts||[]){let p=d.entry_id===t.entry_id,h=d.consumers.some(u=>u.entry_id===t.entry_id&&u.task_id===t.task_id);!p&&!h||i.push({part_id:d.part_id,name:d.name,entry_id:d.entry_id,foreign:!p,object_name:d.object_name})}for(let d of t.used_parts||[]){let p=d.entry_id||t.entry_id;i.some(h=>h.part_id===d.part_id&&h.entry_id===p)||i.push({part_id:d.part_id,name:d.name||d.part_id,entry_id:p,foreign:p!==t.entry_id,object_name:null})}let r={};for(let d of t.used_parts||[])r[`${d.entry_id||t.entry_id}:${d.part_id}`]=d.quantity??1;this._partOptions=i,this._partQty=r,this._partQtyOriginal=this._partSelectionKey()}catch{this._partOptions=[]}}_partSelectionKey(){return JSON.stringify(Object.entries(this._partQty).filter(([,t])=>t>0).sort(([t],[e])=>t.localeCompare(e)))}close(){if(this._open=!1,this._error="",this._draft=null,this._originalSnapshot=null,this._uploadedIds.length>0){let t=this._uploadedIds;this._uploadedIds=[],Z(this.hass,t)}}_set(t,e){this._draft&&(this._draft={...this._draft,[t]:e})}async _delete(){if(!this._draft||!this._originalSnapshot)return;let t=this._lang;if(window.confirm(s("history_delete_confirm",t))){this._saving=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/history/delete",entry_id:this._draft.entry_id,task_id:this._draft.task_id,timestamp:this._originalSnapshot.original_timestamp}),this.dispatchEvent(new CustomEvent("history-entry-saved",{detail:{entry_id:this._draft.entry_id,task_id:this._draft.task_id,deleted:!0},bubbles:!0,composed:!0})),this.close()}catch(e){this._error=A(e,t)}finally{this._saving=!1}}}async _save(){if(!(!this._draft||!this._originalSnapshot)){this._saving=!0,this._error="";try{let t={type:"maintenance_supporter/task/history/update",entry_id:this._draft.entry_id,task_id:this._draft.task_id,original_timestamp:this._originalSnapshot.original_timestamp};if(this._draft.timestamp!==this._originalSnapshot.timestamp&&(t.timestamp=this._draft.timestamp),this._draft.notes!==this._originalSnapshot.notes&&(t.notes=this._draft.notes),this._draft.cost!==this._originalSnapshot.cost&&(t.cost=this._draft.cost),this._draft.duration!==this._originalSnapshot.duration&&(t.duration=this._draft.duration),this._draft.completed_by!==this._originalSnapshot.completed_by&&(t.completed_by=this._draft.completed_by),this._partOptions!==null&&this._partSelectionKey()!==this._partQtyOriginal&&(t.used_parts=(this._partOptions||[]).filter(i=>(this._partQty[`${i.entry_id}:${i.part_id}`]||0)>0).map(i=>({part_id:i.part_id,quantity:this._partQty[`${i.entry_id}:${i.part_id}`],...i.foreign?{entry_id:i.entry_id}:{}}))),this._draft.reading_value!==this._originalSnapshot.reading_value&&(t.reading_value=this._draft.reading_value??null),this._readingRows.length>0&&JSON.stringify(this._readingNumbers())!==this._readingsOriginal){let i=this._readingNumbers(),r={};for(let d of this._readingRows)r[d.id]=i[d.id]??null;t.reading_values=r}if(JSON.stringify(this._photos)!==this._photosOriginal&&(t.photo_doc_ids=[...this._photos]),Object.keys(t).filter(i=>!["type","entry_id","task_id","original_timestamp"].includes(i)).length===0){this.close();return}await this.hass.connection.sendMessagePromise(t),this._uploadedIds=[],this.dispatchEvent(new CustomEvent("history-entry-saved",{detail:{entry_id:this._draft.entry_id,task_id:this._draft.task_id,new_timestamp:this._draft.timestamp},bubbles:!0,composed:!0})),this.close()}catch(t){this._error=A(t,this._lang)}finally{this._saving=!1}}}render(){if(!this._open||!this._draft)return _;let t=this._lang,e=this._draft;return a`
      ...
      <div class="dialog" role="dialog" aria-modal="true">
            ...
            ${this._partOptions.map(i=>{let r=`${i.entry_id}:${i.part_id}`,d=this._partQty[r]||0;return a`
                <label class="part-row-edit">
                  <input type="checkbox" .checked=${d>0}
                    @change=${p=>{let h=p.target.checked;this._partQty={...this._partQty,[r]:h?1:0}}} />
                  ...
                  ${d>0?a`
                    <input class="part-qty" type="number" min="0.01" max="999" step="0.01"
                      .value=${String(d)}
                      @input=${p=>{let h=parseFloat(p.target.value);!isNaN(h)&&h>0&&(this._partQty={...this._partQty,[r]:h})}} />
                  `:_}
                </label>
              `})}
          </div>
        ...
        <div class="photos-block">
          ...
          ${this._photos.length>0?a`
            <div class="photo-strip">
              ${this._photos.map(i=>a`
          ...
          ${this._photos.length<10?a`
            <label class="photo-add">
              ...
              <input type="file" accept="image/*" multiple
                ?disabled=${this._photoUploading}
                @change=${this._onPhotoInput} />
            </label>`:a`<span class="photos-hint">${s("photos_limit",t).replace("{max}",String(10))}</span>`}
    ...
    `}_renderReadings(t,e){return this._readingRows.length>0?a`
        <div class="readings-block">
          <span class="parts-title">${s("readings_section",e)}</span>
          ${this._readingRows.map(i=>a`
            <label class="reading-row-edit">
              ...
              <input type="text" inputmode="decimal" class="reading-row-input"
                ...
                @input=${r=>{this._readingText={...this._readingText,[i.id]:r.target.value}}} />
            </label>
          `)}
        </div>`:t.reading_value==null&&t.task_type!=="reading"?_:a`
      <label>
        <span>${s("reading_value_label",e)}${t.reading_unit?` (${t.reading_unit})`:""}</span>
        <input type="number" step="any"
          .value=${t.reading_value!=null?String(t.reading_value):""}
          @input=${i=>{let r=i.target.value,d=r===""?NaN:Number(r);this._set("reading_value",isNaN(d)?null:d)}} />
      </label>`}};C.styles=w`
    :host { display: contents; }
    .backdrop {
  ...
  `,o([g({attribute:!1})],C.prototype,"hass",2),o([c()],C.prototype,"_open",2),o([c()],C.prototype,"_saving",2),o([c()],C.prototype,"_error",2),o([c()],C.prototype,"_draft",2),o([c()],C.prototype,"_partOptions",2),o([c()],C.prototype,"_partQty",2),o([c()],C.prototype,"_photos",2),o([c()],C.prototype,"_readingRows",2),o([c()],C.prototype,"_readingText",2),o([c()],C.prototype,"_photoUploading",2);customElements.get("maintenance-history-edit-dialog")||customElements.define("maintenance-history-edit-dialog",C);function X(n){return n.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function Yt(n){return!n.startsWith("data:image/svg+xml,")&&!n.startsWith("data:image/png;base64,")?"":X(n)}function Ce(n){return n.replace(/[/\\:*?"<>|#%]+/g,"").replace(/\s+/g,"-").toLowerCase().substring(0,100)}var q=class extends E{constructor(){super(...arguments);this.lang="en";this._open=!1;this._loading=!1;this._error="";this._viewResult=null;this._completeResult=null;this._urlMode="companion";this._entryId="";this._taskId=null;this._objectName="";this._taskName="";this._generateSeq=0}openForObject(t,e){this._entryId=t,this._taskId=null,this._objectName=e,this._taskName="",this._urlMode="companion",this._error="",this._viewResult=null,this._completeResult=null,this._open=!0,this._generate()}openForTask(t,e,i,r){this._entryId=t,this._taskId=e,this._objectName=i,this._taskName=r,this._urlMode="companion",this._error="",this._viewResult=null,this._completeResult=null,this._open=!0,this._generate()}async _generate(){let t=++this._generateSeq;this._loading=!0,this._error="",this._viewResult=null,this._completeResult=null;try{let e={type:"maintenance_supporter/qr/generate",entry_id:this._entryId,url_mode:this._urlMode};this._taskId&&(e.task_id=this._taskId);let i=[this.hass.connection.sendMessagePromise({...e,action:"view"})];this._taskId&&i.push(this.hass.connection.sendMessagePromise({...e,action:"complete"}));let r=await Promise.all(i);if(t!==this._generateSeq)return;this._viewResult=r[0],r.length>1&&(this._completeResult=r[1])}catch(e){if(t!==this._generateSeq)return;let i=e?.code,r=e?.message;this._error=i==="no_url"||typeof r=="string"&&r.includes("No Home Assistant URL")?s("qr_error_no_url",this.lang):s("qr_error",this.lang)}finally{t===this._generateSeq&&(this._loading=!1)}}_setUrlMode(t){this._urlMode!==t&&(this._urlMode=t,this._generate())}_print(){if(!this._viewResult)return;let t=this._viewResult,e=t.label.task_name?`${t.label.object_name} \u2014 ${t.label.task_name}`:t.label.object_name,i=[t.label.manufacturer,t.label.model].filter(Boolean).join(" "),r=window.open("","_blank","width=600,height=500");if(!r)return;let d=this.lang||"en",p=X(e),h=X(i),u=!!this._completeResult,v=X(s("qr_action_view",d)),y=X(s("qr_action_complete",d));r.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8">
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `,o([g({attribute:!1})],q.prototype,"hass",2),o([g()],q.prototype,"lang",2),o([c()],q.prototype,"_open",2),o([c()],q.prototype,"_loading",2),o([c()],q.prototype,"_error",2),o([c()],q.prototype,"_viewResult",2),o([c()],q.prototype,"_completeResult",2),o([c()],q.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",q);function Gt(n){let l=n.getFullYear(),t=String(n.getMonth()+1).padStart(2,"0"),e=String(n.getDate()).padStart(2,"0");return`${l}-${t}-${e}`}function Le(n,l){if(l<=0)return 0;let t=typeof n=="number"&&Number.isFinite(n)?Math.trunc(n):0;return t<0?0:t%l}function qe(n){return!!(n?.phases&&n.phase_sequence&&n.phase_sequence.length>0)}function mt(n){if(!n||!qe(n))return null;let l=n.phase_sequence,t=Le(n.phase_cursor,l.length),e=l[t],i=n.phases?.[e];return i?{id:e,name:i.name,index:t,count:l.length,notes:i.notes,checklist:i.checklist!==void 0?i.checklist:n.checklist??[],consumesParts:i.consumes_parts!==void 0?i.consumes_parts:n.consumes_parts??[],requiredFields:i.required_completion_fields!==void 0?i.required_completion_fields:n.required_completion_fields??[]}:null}function et(n){let l=mt(n);return l?`${l.index+1}/${l.count} \xB7 ${l.name}`:""}function Qt(n){let l=n.task??null,t=l?mt(l):null,e=t?t.consumesParts:l?.consumes_parts||[],i=!!l?.part_ref,r=n.objects.find(h=>h.entry_id===n.entryId)?.parts||[],d=i?r.find(h=>h.id===l.part_ref.part_id):void 0,p=n.checklistsEnabled??!0;return{entry_id:n.entryId,task_id:n.taskId,task_name:n.taskName,checklist:t?p?t.checklist:[]:n.checklist??[],adaptive_enabled:!!n.adaptiveEnabled,required_completion_fields:t?t.requiredFields:l?.required_completion_fields||[],task_type:l?.type||"",reading_unit:l?.reading_unit||"",readings:l?.readings||[],reading_history:bt(l?.history),parts:i?[]:Dt({consumes_parts:e},n.entryId,n.objects,n.lang),consumes_parts:i?[]:e,phase_label:t?et(l):"",require_tag_scan:!!l?.require_tag_scan,restock_default:i?d?.restock_quantity??1:null,restock_unit_cost:i?d?.cost??null:null,currency_symbol:n.currencySymbol??"",consumes_info:e.map(h=>Ft(h,n.entryId,n.objects,n.lang)),checklist_prefill:l?.checklist_progress||{},via_tag_scan:!!n.viaTagScan}}function Jt(n,l,t){n.entryId=l.entry_id,n.taskId=l.task_id,n.taskName=l.task_name,n.lang=t,n.checklist=l.checklist??[],n.adaptiveEnabled=!!l.adaptive_enabled,n.requiredFields=l.required_completion_fields??[],n.taskType=l.task_type??"",n.readingUnit=l.reading_unit??"",n.readings=l.readings??[],n.readingHistory=l.reading_history??[],n.parts=l.parts??[],n.consumesParts=l.consumes_parts??[],n.phaseLabel=l.phase_label??"",n.requireTagScan=!!l.require_tag_scan,n.restockDefault=l.restock_default??null,n.restockUnitCost=l.restock_unit_cost??null,n.currencySymbol=l.currency_symbol??"",n.consumesInfo=l.consumes_info??[],n.checklistPrefill=l.checklist_prefill??{},n.viaTagScan=!!l.via_tag_scan,n.open({viaTagScan:!!l.via_tag_scan})}function $(n){return n.toFixed(1)}function Zt(n,l){let t=n.interval_analysis,e=t?.weibull_beta,i=t?.weibull_eta;if(e==null||i==null||i<=0)return _;let r=n.interval_days??0,d=n.suggested_interval??r;return a`
    <div class="weibull-section">
      ...
      ${He(e,i,r,d,l)}
      ${Ne(t,l)}
      ${t?.confidence_interval_low!=null?Me(t,n,l):_}
    </div>
  `}function Re(n,l){let t,e,i;return n<.8?(t="early_failures",e="M13,14H11V10H13M13,18H11V16H13M1,21H23L12,2L1,21Z",i="beta_early_failures"):n<=1.2?(t="random_failures",e="M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2M13,17H11V15H13V17M13,13H11V7H13V13Z",i="beta_random_failures"):n<=3.5?(t="wear_out",e="M12,2A10,10 0 0,1 22,12A10,10 0 0,1 12,22A10,10 0 0,1 2,12A10,10 0 0,1 12,2M12,4A8,8 0 0,0 4,12A8,8 0 0,0 12,20A8,8 0 0,0 20,12A8,8 0 0,0 12,4M12,6A6,6 0 0,1 18,12H12V6Z",i="beta_wear_out"):(t="highly_predictable",e="M12,2A10,10 0 0,1 22,12A10,10 0 0,1 12,22A10,10 0 0,1 2,12A10,10 0 0,1 12,2M11,16.5L18,9.5L16.59,8.09L11,13.67L7.91,10.59L6.5,12L11,16.5Z",i="beta_highly_predictable"),a`
    <span class="beta-badge ${t}">
      <ha-svg-icon path="${e}"></ha-svg-icon>
      ${s(i,l)} (\u03B2=${S(n,l,2)})
    </span>
  `}function He(n,l,t,e,i){let f=Math.max(t,e,l,1)*1.3,T=50,R=[];for(let H=0;H<=T;H++){let M=H/T*f,he=1-Math.exp(-Math.pow(M/l,n)),ue=32+M/f*260,_e=136-he*128;R.push([ue,_e])}let B=R.map(([H,M])=>`${$(H)},${$(M)}`).join(" "),K="M32,136 "+R.map(([H,M])=>`L${$(H)},${$(M)}`).join(" ")+` L${$(R[T][0])},136 Z`,N=32+t/f*260,j=1-Math.exp(-Math.pow(t/l,n)),Q=136-j*128,ce=S((1-j)*100,i,0),vt=32+e/f*260,pe=[0,.25,.5,.75,1];return a`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_weibull",i)}">
        ${pe.map(H=>{let M=136-H*128;return J`
            <line x1="${32}" y1="${$(M)}" x2="${292}" y2="${$(M)}"
              stroke="var(--divider-color)" stroke-width="0.5" stroke-dasharray="${H===.5?"4,3":_}" />
            <text x="${28}" y="${$(M+3)}" fill="var(--secondary-text-color)"
              font-size="8" text-anchor="end">${S(H*100,i,0)}%</text>
          `})}
        ...
        ${t>0?J`
          <line x1="${$(N)}" y1="${8}" x2="${$(N)}" y2="${$(136)}"
            stroke="var(--primary-color, #03a9f4)" stroke-width="1.5" stroke-dasharray="4,3" />
          <circle cx="${$(N)}" cy="${$(Q)}" r="3"
            fill="var(--primary-color, #03a9f4)" />
          <text x="${$(N+4)}" y="${$(Q-6)}" fill="var(--primary-color, #03a9f4)"
            font-size="9" font-weight="600">R=${ce}%</text>
        `:_}

        ${e>0&&e!==t?J`
          <line x1="${$(vt)}" y1="${8}" x2="${$(vt)}" y2="${$(136)}"
  ...
  `}function Ne(n,l){return a`
    <div class="weibull-info-row">
      <div class="weibull-info-item">
        <span>${s("characteristic_life",l)}</span>
        <span class="weibull-info-value">${Math.round(n.weibull_eta)} ${s("days",l)}</span>
      </div>
      ${n.weibull_r_squared!=null?a`
        <div class="weibull-info-item">
          <span>${s("weibull_r_squared",l)}</span>
          <span class="weibull-info-value">${S(n.weibull_r_squared,l,3)}</span>
        </div>
      `:_}
    </div>
  `}function Me(n,l,t){let e=n.confidence_interval_low,i=n.confidence_interval_high,r=l.suggested_interval??l.interval_days??0,d=l.interval_days??0,p=Math.max(0,e-5),u=i+5-p,v=(e-p)/u*100,y=(i-e)/u*100,x=(r-p)/u*100,f=d>0?(d-p)/u*100:-1;return a`
    <div class="confidence-range">
      <div class="confidence-range-title">
        ${s("confidence_interval",t)}: ${r} ${s("days",t)} (${e}\u2013${i})
      </div>
      <div class="confidence-bar">
        <div class="confidence-fill" style="left:${$(v)}%;width:${$(y)}%"></div>
        ${f>=0?a`<div class="confidence-marker current" style="left:${$(f)}%"></div>`:_}
      ...
      <div class="prediction-grid">
        ${e?a`
          <div class="prediction-item">
            ...
            <span class="prediction-value ${n.degradation_trend}">${s("trend_"+n.degradation_trend,l)}</span>
            ${n.degradation_rate!=null?a`<span class="prediction-rate">${n.degradation_rate>0?"+":""}${S(n.degradation_rate,l,Math.abs(n.degradation_rate)>=10?0:1)} ${n.trigger_entity_info?.unit_of_measurement||""}/${s("day_short",l)}</span>`:_}
          </div>
        `:_}
        ${i?a`
          <div class="prediction-item">
            ...
            <span class="prediction-value prediction-days${n.days_until_threshold===0?" exceeded":n.sensor_prediction_urgency?" urgent":""}">${n.days_until_threshold===0?s("threshold_exceeded",l):"~"+Math.round(n.days_until_threshold)+" "+s("days",l)}</span>
            ${n.threshold_prediction_date?a`<span class="prediction-date">${Y(n.threshold_prediction_date,l)}</span>`:_}
            ${n.threshold_prediction_confidence?a`<span class="confidence-dot ${n.threshold_prediction_confidence}"></span>`:_}
            ${(n.prediction_cycles??0)>0?a`<span class="prediction-cycles">${s("prediction_cycles",l)}: ${n.prediction_cycles}</span>`:_}
          </div>
        `:_}
        ${r&&t.environmental?a`
          <div class="prediction-item">
            ...
            <span class="prediction-value">${S(n.environmental_factor,l,2)}x</span>
            ${n.environmental_entity?a`<span class="prediction-entity entity-link" @click=${p=>qt(p,n.environmental_entity)}>${n.environmental_entity}</span>`:_}
          </div>
        `:_}
      </div>
    </div>
  `}function te(n,l,t,e){let i=Math.max(n||1,l);return a`
  ...
  `}var ee=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"];function ie(n,l,t){if(!t.seasonal||!n.seasonal_factor||n.seasonal_factor===1)return _;let e=ee.map(p=>s(p,l)),i=new Date().getMonth(),r=n.seasonal_factors||n.interval_analysis?.seasonal_factors||null,d=r&&r.length===12?r:e.map((p,h)=>{let u=n.seasonal_factor||1,v=Math.sin((h-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,u+v))});return a`
    <div class="seasonal-card-compact">
      <h4>${s("seasonal_awareness",l)}</h4>
      <div class="seasonal-mini-chart">
        ${d.map((p,h)=>{let u=p*40,v=p<.9?"low":p>1.1?"high":"normal";return a`
            <div class="seasonal-bar ${v} ${h===i?"current":""}"
                 style="height: ${u}px"
                 title="${e[h]}: ${S(p,l,2)}x">
            </div>
          `})}
  ...
  `}function se(n,l){return Oe(n,l)}function Oe(n,l){let t=n.seasonal_factors??n.interval_analysis?.seasonal_factors;if(!t||t.length!==12)return _;let e=n.interval_analysis?.seasonal_reason,i=new Date().getMonth(),r=300,d=100,p=8,u=d-p-4,v=Math.max(...t,1.5),y=r/12,x=y*.65,f=p+u-1/v*u;return a`
    <div class="seasonal-chart">
      <div class="seasonal-chart-title">
        ...
        ${e?a`<span class="source-tag">${e==="learned"?s("seasonal_learned",l):s("seasonal_manual",l)}</span>`:_}
      </div>
      <svg viewBox="0 0 ${r} ${d}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",l)}">
        <line x1="0" y1="${$(f)}" x2="${r}" y2="${$(f)}"
          stroke="var(--divider-color)" stroke-width="1" stroke-dasharray="4,3" />
        ${t.map((T,R)=>{let B=T/v*u,K=R*y+(y-x)/2,N=p+u-B,j=R===i,Q=T<1?"var(--success-color, #4caf50)":T>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return J`
            <rect x="${$(K)}" y="${$(N)}"
              width="${$(x)}" height="${$(B)}"
      ...
      <div class="seasonal-labels">
        ${ee.map((T,R)=>a`<span class="seasonal-label ${R===i?"active-month":""}">${s(T,l)}</span>`)}
      </div>
    </div>
  `}var I=class extends E{constructor(){super(...arguments);this._open=!1;this._entryId=null;this._taskId=null;this._task=null;this._objectName="";this._busy=!1;this._error="";this._showSkip=!1;this._showReset=!1;this._showDetails=!1;this._showAdaptive=!1;this._skipReason="";this._resetDate="";this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._toast="";this._featuresLoaded=!1;this._currencySymbol=""}get _lang(){return L(this.hass)}async openFor(t,e){this._entryId=t,this._taskId=e,this._error="",this._showSkip=!1,this._showReset=!1,this._showAdaptive=!1,this._skipReason="",this._resetDate=Gt(new Date),this._open=!0,await Promise.all([this._loadTask(),this._loadFeatures()])}async _loadFeatures(){if(!this._featuresLoaded)try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});t?.features&&(this._features={...this._features,...t.features}),this._currencySymbol=t?.budget?.currency_symbol||"",It(t?.budget),this._featuresLoaded=!0}catch{}}close(){this._open=!1,this._task=null,this._error=""}async _loadTask(){if(!(!this._entryId||!this._taskId))try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._objectName=t.object?.name||"";let e=(t.tasks||[]).find(i=>i.id===this._taskId);this._task=e??null}catch(t){this._error=A(t,this._lang)}}async _runWs(t){this._busy=!0,this._error="";try{return await this.hass.connection.sendMessagePromise(t),this._busy=!1,!0}catch(e){return this._error=A(e,this._lang),this._busy=!1,!1}}_notifyChanged(t){this.dispatchEvent(new CustomEvent("task-action-fired",{detail:{entry_id:this._entryId,task_id:this._taskId,action:t},bubbles:!0,composed:!0}))}_onComplete(){!this._entryId||!this._taskId||!this._task||import("./dialog-mount-7DZBQFL2.js").then(async({openCompleteDialog:t})=>{let e=this._task,i=[];try{i=(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects",compact:!0})).objects||[]}catch{}t(Qt({entryId:this._entryId,taskId:this._taskId,taskName:e.name,task:e,objects:i,lang:this._lang,checklist:e.checklist||[],adaptiveEnabled:!!e.adaptive_config?.enabled,currencySymbol:this._currencySymbol}))&&(this._notifyChanged("complete"),this.close())})}async _onSkipConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/skip",entry_id:this._entryId,task_id:this._taskId,reason:this._skipReason.trim()||null})&&(this._notifyChanged("skip"),this.close())}async _onResetConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/reset",entry_id:this._entryId,task_id:this._taskId,date:this._resetDate||void 0})&&(this._notifyChanged("reset"),this.close())}_onEdit(){!this._entryId||!this._taskId||import("./dialog-mount-7DZBQFL2.js").then(({openEditTaskDialog:t})=>{t(this._entryId,this._taskId),this.close()})}_onQr(){!this._entryId||!this._taskId||!this._task||import("./dialog-mount-7DZBQFL2.js").then(({openQrDialog:t})=>{t({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,object_name:this._objectName}),this.close()})}async _onDelete(){if(!this._entryId||!this._taskId)return;let t=s("delete_task_confirm",this._lang)||`Delete "${this._task?.name}"?`;if(!window.confirm(t))return;await this._runWs({type:"maintenance_supporter/task/delete",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("delete"),this.close())}async _onArchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/archive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("archive"),this.close())}async _onUnarchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/unarchive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("unarchive"),this.close())}_onOpenInPanel(){if(!this._entryId||!this._taskId)return;let t=`/maintenance-supporter?entry_id=${encodeURIComponent(this._entryId)}&task_id=${encodeURIComponent(this._taskId)}`;history.pushState(null,"",t),window.dispatchEvent(new CustomEvent("location-changed")),this.close()}async _applySuggestion(){if(!this._entryId||!this._taskId||!this._task?.suggested_interval)return;await this._runWs({type:"maintenance_supporter/task/apply_suggestion",entry_id:this._entryId,task_id:this._taskId,interval:this._task.suggested_interval})&&(this._toast=s("suggestion_applied",this._lang)||"Applied",this._notifyChanged("apply_suggestion"),await this._loadTask(),setTimeout(()=>{this._toast=""},2500))}async _reanalyzeInterval(){if(!(!this._entryId||!this._taskId)){this._busy=!0,this._error="";try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:this._entryId,task_id:this._taskId});this._toast=t.recommended_interval?`${s("reanalyze_result",this._lang)||"Recomputed"}: ${Pt(t.recommended_interval,"days",this._lang)} (${t.data_points} pts)`:s("reanalyze_insufficient_data",this._lang)||"Not enough data",await this._loadTask(),setTimeout(()=>{this._toast=""},3500)}catch(t){this._error=A(t,this._lang)}finally{this._busy=!1}}}_onEditHistoryEntry(t){!this._entryId||!this._taskId||import("./dialog-mount-7DZBQFL2.js").then(({openHistoryEditDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,original_timestamp:t.timestamp,type:t.type,timestamp:t.timestamp,notes:t.notes??null,cost:t.cost??null,duration:t.duration??null,completed_by:t.completed_by??null,used_parts:t.used_parts??null,photo_doc_ids:dt(t),reading_value:t.reading_value??null,reading_values:ct(t),readings:this._task?.readings??[],task_type:this._task?.type??null,reading_unit:this._task?.reading_unit??null})})}_renderRecommendation(t){if(!this._features.adaptive||!t.suggested_interval||t.suggested_interval===t.interval_days)return _;let e=this._lang;return a`
      <div class="recommendation-card">
        <h4>${s("suggested_interval",e)}</h4>
        ${te(t.interval_days,t.suggested_interval,t.interval_confidence||"medium",e)}
    ...
    `}_renderAdaptive(t){let e=this._lang,i=this._features.adaptive&&t.suggested_interval&&t.suggested_interval!==t.interval_days,r=t.degradation_trend!=null&&t.degradation_trend!=="insufficient_data"||t.days_until_threshold!=null||t.environmental_factor!=null&&t.environmental_factor!==1,d=this._features.adaptive&&t.interval_analysis?.weibull_beta!=null&&t.interval_analysis?.weibull_eta!=null,p=this._features.seasonal&&t.seasonal_factor&&t.seasonal_factor!==1;return!i&&!r&&!d&&!p?a`<div class="adaptive-empty">
      ...
      <div class="adaptive-stack">
        ...
        ${r?Xt(t,e,this._features):_}
        ${d?Zt(t,e):_}
        ${p?a`
          ${ie(t,e,this._features)}
          ${t.seasonal_factors?.length===12||t.interval_analysis?.seasonal_factors?.length===12?se(t,e):_}
        `:_}
      </div>
    `}_renderHistoryReadings(t,e,i){let r=ct(t),d=p=>S(p,i,{maximumFractionDigits:3});if(r.length>0)return a`<div class="history-readings">
        ${r.map(p=>{let h=xt(e,t,p.id);return a`<span class="history-reading"><span class="history-reading-name">${p.name}</span>
            <span class="history-reading-value">${d(p.value)}${p.unit?` ${p.unit}`:""}${h==null?"":` (${h>=0?"+":""}${d(h)})`}</span></span>`})}
        ...
        <span class="history-reading-name">${s("reading_label",i)}</span>
        <span class="history-reading-value">${d(t.reading_value)}${p}</span></span></div>`}return _}_renderDetails(t){let e=this._lang,i=t.history||[],r=i.filter(h=>h.type==="completed"),d=r.reduce((h,u)=>h+(typeof u.cost=="number"?u.cost:0),0),p=(()=>{let h=r.map(u=>typeof u.duration=="number"?u.duration:null).filter(u=>u!=null);return h.length?Math.round(h.reduce((u,v)=>u+v,0)/h.length):null})();return a`
      <div class="details">
        <div class="stats-grid">
          ...
          <div class="stat">
            ...
            <span class="stat-value">${S(d,e,2)}</span>
          </div>
          <div class="stat">
        ...
        ${i.length===0?a`<div class="history-empty">${s("history_empty",e)||"No history yet."}</div>`:a`
              <div class="history-list">
                ${[...i].reverse().slice(0,20).map(h=>{let u=["completed","reset","skipped"].includes(h.type);return a`
                    <div class="history-entry">
                      <div class="history-line">
                        <span class="history-type type-${h.type}">${s(h.type,e)}</span>
                        <span class="history-date">${At(h.timestamp,e)}</span>
                      ...
                      ${h.notes?a`<div class="history-notes">${h.notes}</div>`:_}
                      ${this._renderHistoryReadings(h,i,e)}
                      ${(()=>{let v=dt(h);return v.length?a`<div class="history-photos">
                              ${v.map(y=>a`<maintenance-history-photo .hass=${this.hass} .docId=${y}></maintenance-history-photo>`)}
                            </div>`:_})()}
                      ${h.cost!=null||h.duration!=null?a`<div class="history-meta">
                            ${h.cost!=null?a`<span>💰 ${S(h.cost,e,2)}</span>`:_}
                            ${h.duration!=null?a`<span>⏱️ ${h.duration}m</span>`:_}
                          </div>`:_}
                    </div>
                  `})}
    ...
    `}render(){if(!this._open)return _;let t=this._lang,e=this._task,i=this.hass?.user?.is_admin??!0;return a`
      ...
      <div class="dialog" role="dialog" aria-modal="true">
        ${e?a`
              <div class="header">
                <div class="title">
                  <span class="status-dot" style="background: ${it[e.status]||"#ccc"}"></span>
                  <span class="task-name">${e.name}</span>
                </div>
                <div class="object">
                  <button class="link-inline" @click=${()=>{this._entryId&&import("./dialog-mount-7DZBQFL2.js").then(({openObjectQuickActions:r})=>{r(this._entryId),this.close()})}}>${this._objectName}</button>
                </div>
                <div class="quick-info">
                  ${e.next_due?a`<span><strong>${s("next_due",t)||"Next due"}:</strong> ${Y(e.next_due,t)}</span>`:_}
                  ${e.last_performed?a`<span><strong>${s("last_performed",t)||"Last"}:</strong> ${Y(e.last_performed,t)}</span>`:_}
                  ${e.schedule?.kind&&!["manual","one_time"].includes(e.schedule.kind)||e.interval_days!=null?a`<span><strong>${s("interval",t)||"Interval"}:</strong> ${Lt(e,t)}</span>`:_}
                  ${et(e)?a`<span><strong>${s("phase_current",t)}:</strong> ${et(e)}</span>`:_}
                </div>
              </div>
  ...
  `],o([g({attribute:!1})],I.prototype,"hass",2),o([c()],I.prototype,"_open",2),o([c()],I.prototype,"_entryId",2),o([c()],I.prototype,"_taskId",2),o([c()],I.prototype,"_task",2),o([c()],I.prototype,"_objectName",2),o([c()],I.prototype,"_busy",2),o([c()],I.prototype,"_error",2),o([c()],I.prototype,"_showSkip",2),o([c()],I.prototype,"_showReset",2),o([c()],I.prototype,"_showDetails",2),o([c()],I.prototype,"_showAdaptive",2),o([c()],I.prototype,"_skipReason",2),o([c()],I.prototype,"_resetDate",2),o([c()],I.prototype,"_features",2),o([c()],I.prototype,"_toast",2);customElements.get("maintenance-task-quick-actions-dialog")||customElements.define("maintenance-task-quick-actions-dialog",I);function re(n){return!!n&&/^https?:\/\//i.test(n)}function ne(n){return n?customElements.get("ha-markdown")?a`<ha-markdown class="notes-md" .content=${n} breaks></ha-markdown>`:a`${n}`:_}var D=class extends E{constructor(){super(...arguments);this._open=!1;this._entryId=null;this._data=null;this._busy=!1;this._error=""}get _lang(){return L(this.hass)}async openFor(t){this._entryId=t,this._error="",this._open=!0,await this._load()}close(){this._open=!1,this._data=null,this._error=""}async _load(){if(this._entryId)try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._data=t}catch(t){this._error=A(t,this._lang)}}_onEditObject(){!this._entryId||!this._data||import("./dialog-mount-7DZBQFL2.js").then(({openEditObjectDialog:t})=>{t(this._entryId,this._data.object),this.close()})}_onAddTask(){this._entryId&&import("./dialog-mount-7DZBQFL2.js").then(({openCreateTaskDialog:t})=>{t(this._entryId),this.close()})}async _onDelete(){if(!this._entryId||!this._data)return;let t=s("delete_object_confirm",this._lang)||`Delete "${this._data.object.name}" and all its tasks?`;if(window.confirm(t)){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:this._entryId}),this.dispatchEvent(new CustomEvent("object-deleted",{detail:{entry_id:this._entryId},bubbles:!0,composed:!0})),this.close()}catch(e){this._error=A(e,this._lang)}finally{this._busy=!1}}}async _onArchiveObject(){if(!this._entryId||!this._data)return;let t=!!this._data.object.archived;if(!t){let e=s("confirm_archive_object",this._lang)||"Archive this object and its tasks?";if(!window.confirm(e))return}this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:t?"maintenance_supporter/object/unarchive":"maintenance_supporter/object/archive",entry_id:this._entryId}),this.dispatchEvent(new CustomEvent("object-changed",{detail:{entry_id:this._entryId},bubbles:!0,composed:!0})),this.close()}catch(e){this._error=A(e,this._lang)}finally{this._busy=!1}}_onTaskClick(t){this._entryId&&import("./dialog-mount-7DZBQFL2.js").then(({openTaskQuickActions:e})=>{e(this._entryId,t)})}render(){if(!this._open)return _;let t=this._lang,e=this._data,i=e?.object,r=e?.tasks||[],d=this.hass?.user?.is_admin??!0;return a`
      <div class="backdrop" @click=${this.close}></div>
      <div class="dialog" role="dialog" aria-modal="true">
        ${e&&i?a`
              <div class="header">
                      ...
                      <div class="task-list">
                        ${r.map(p=>a`
                          <div class="task-row" @click=${()=>this._onTaskClick(p.id)}>
                            <span class="status-dot" style="background: ${it[p.status]||"#ccc"}"></span>
              ...
              ${i.notes?a`
                    <div class="notes-section">
                      ...
                      <div class="notes-body">${ne(i.notes)}</div>
                    </div>
                  `:_}
      ...
      <div class="meta">
        ${i.map(([r,d])=>a`
            <div class="meta-item">
              ...
              <span class="meta-value">${re(d)?a`<a href="${d}" target="_blank" rel="noopener noreferrer">${d}</a>`:d}</span>
            </div>
          `)}
      </div>
    `}};D.styles=w`
  ...
  `,o([g({attribute:!1})],D.prototype,"hass",2),o([c()],D.prototype,"_open",2),o([c()],D.prototype,"_entryId",2),o([c()],D.prototype,"_data",2),o([c()],D.prototype,"_busy",2),o([c()],D.prototype,"_error",2);customElements.get("maintenance-object-quick-actions-dialog")||customElements.define("maintenance-object-quick-actions-dialog",D);var gt={features:{adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1},defaultWarningDays:7,rowActionStyle:"buttons_compact"};function ae(){let n=window;return n.__msSettingsCache??={promise:null}}function ot(n){let l=ae();if(l.promise)return l.promise;let t=n.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).then(e=>({features:e.features??gt.features,defaultWarningDays:e.general?.default_warning_days??7,rowActionStyle:e.general?.row_action_style??gt.rowActionStyle})).catch(()=>(l.promise===t&&(l.promise=null),gt));return l.promise=t,t}function oe(){ae().promise=null}var le="maintenance-object-dialog",de="maintenance-task-dialog",Fe="maintenance-history-edit-dialog",De="maintenance-complete-dialog",je="maintenance-qr-dialog",Ue="maintenance-task-quick-actions-dialog",ze="maintenance-object-quick-actions-dialog";function lt(){return document.querySelector("home-assistant")?.hass}function Ve(){return document.querySelector("home-assistant")?.shadowRoot??document.body}function z(n){let l=Ve(),t=l.querySelector(n)??document.body.querySelector(n);return t?t.parentNode!==l&&l.appendChild(t):(t=document.createElement(n),l.appendChild(t)),t}function V(n){let l=lt();if(!l)return!1;n.hass=l;let t=L(l);return wt(t)||Et(t).then(()=>{n.requestUpdate?.()}),St(l.locale,l.config?.country),!0}function Pr(n){return ot(n).then(l=>l.rowActionStyle)}function Cr(){oe()}function Lr(){let n=z(le);return V(n)?(n.openCreate(),!0):!1}function qr(n,l){let t=z(le);return V(t)?(t.openEdit(n,l),!0):!1}function Rr(n="",l){let t=z(de);if(!V(t))return!1;let e=lt();return e?((async()=>{let i=await ot(e),r=t;r.checklistsEnabled=i.features.checklists,r.scheduleTimeEnabled=i.features.schedule_time,r.completionActionsEnabled=i.features.completion_actions,r.defaultWarningDays=i.defaultWarningDays,r.openCreate(n,l)})(),!0):!1}function Hr(n,l){let t=z(de);if(!V(t))return!1;let e=lt();return e?((async()=>{try{let[i,r]=await Promise.all([e.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:n}),ot(e)]),d=(i.tasks||[]).find(h=>h.id===l);if(!d){console.warn(`openEditTaskDialog: task ${l} not found in entry ${n}`);return}let p=t;p.checklistsEnabled=r.features.checklists,p.scheduleTimeEnabled=r.features.schedule_time,p.completionActionsEnabled=r.features.completion_actions,p.defaultWarningDays=r.defaultWarningDays,await p.openEdit(n,d)}catch(i){console.warn("openEditTaskDialog: failed to load task/features",i)}})(),!0):!1}function Nr(n){let l=z(Fe);return V(l)?(l.openEdit(n),!0):!1}function Mr(n){let l=z(De);return V(l)?(Jt(l,n,lt()?.language||"en"),!0):!1}function Or(n){let l=z(je);return V(l)?(l.openForTask(n.entry_id,n.task_id,n.object_name,n.task_name),!0):!1}function Fr(n,l){let t=z(Ue);return V(t)?(t.openFor(n,l),!0):!1}function Dr(n){let l=z(ze);return V(l)?(l.openFor(n),!0):!1}export{Cr as __resetSettingsCacheForTests,Pr as getRowActionStyle,Mr as openCompleteDialog,Lr as openCreateObjectDialog,Rr as openCreateTaskDialog,qr as openEditObjectDialog,Hr as openEditTaskDialog,Nr as openHistoryEditDialog,Dr as openObjectQuickActions,Or as openQrDialog,Fr as openTaskQuickActions};
