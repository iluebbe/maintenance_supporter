...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function xt(n,o,t,e,s){let I=Math.max(t,e,o,1)*1.3,S=50,C=[];for(let T=0;T<=S;T++){let L=T/S*I,ut=1-Math.exp(-Math.pow(L/o,n)),_t=32+L/I*260,mt=136-ut*128;C.push([_t,mt])}let j=C.map(([T,L])=>`${T.toFixed(1)},${L.toFixed(1)}`).join(" "),K="M32,136 "+C.map(([T,L])=>`L${T.toFixed(1)},${L.toFixed(1)}`).join(" ")+` L${C[S][0].toFixed(1)},136 Z`,F=32+t/I*260,D=1-Math.exp(-Math.pow(t/o,n)),V=136-D*128,pt=((1-D)*100).toFixed(0),Z=32+e/I*260,ht=[0,.25,.5,.75,1];return r`
  ...
  `}function kt(n,o,t){let e=n.confidence_interval_low,s=n.confidence_interval_high,c=o.suggested_interval??o.interval_days??0,u=o.interval_days??0,d=Math.max(0,e-5),g=s+5-d,b=(e-d)/g*100,E=(s-e)/g*100,z=(c-d)/g*100,I=u>0?(u-d)/g*100:-1;return r`
  ...
  `}function it(n,o,t){let e=n.degradation_trend!=null&&n.degradation_trend!=="insufficient_data",s=n.days_until_threshold!=null,c=n.environmental_factor!=null&&n.environmental_factor!==1;if(!e&&!s&&!c)return p;let u=n.degradation_trend==="rising"?"M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z":n.degradation_trend==="falling"?"M16,18L18.29,15.71L13.41,10.83L9.41,14.83L2,7.41L3.41,6L9.41,12L13.41,8L19.71,14.29L22,12V18H16Z":"M22,12L18,8V11H3V13H18V16L22,12Z";return r`
  ...
  `}function nt(n,o){return wt(n,o)}function wt(n,o){let t=n.seasonal_factors??n.interval_analysis?.seasonal_factors;if(!t||t.length!==12)return p;let e=n.interval_analysis?.seasonal_reason,s=new Date().getMonth(),c=300,u=100,d=8,g=u-d-4,b=Math.max(...t,1.5),E=c/12,z=E*.65,I=d+g-1/b*g;return r`
      ...
      <div class="dialog" role="dialog" aria-modal="true">
        ${e?r`
              <div class="header">
                ...
                <div class="object">
                  <button class="link-inline" @click=${()=>{this._entryId&&import("./dialog-mount-TRBVWQZI.js").then(({openObjectQuickActions:c})=>{c(this._entryId),this.close()})}}>${this._objectName}</button>
                </div>
