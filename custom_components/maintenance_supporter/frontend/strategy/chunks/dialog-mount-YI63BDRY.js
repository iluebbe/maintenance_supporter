  ...
  `}function ot(n,l){return At(n,l)}function At(n,l){let t=n.seasonal_factors??n.interval_analysis?.seasonal_factors;if(!t||t.length!==12)return d;let e=n.interval_analysis?.seasonal_reason,s=new Date().getMonth(),c=300,_=100,h=8,g=_-h-4,y=Math.max(...t,1.5),E=c/12,P=E*.65,I=h+g-1/y*g;return r`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${c} ${_}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_seasonal",l)}">
        ...
        ${t.map((S,C)=>{let j=S/y*g,K=C*E+(E-P)/2,F=h+g-j,D=C===s,W=S<1?"var(--success-color, #4caf50)":S>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return M`
      ...
      <div class="dialog" role="dialog" aria-modal="true">
        ${e?r`
              <div class="header">
                ...
                <div class="object">
                  <button class="link-inline" @click=${()=>{this._entryId&&import("./dialog-mount-YI63BDRY.js").then(({openObjectQuickActions:c})=>{c(this._entryId),this.close()})}}>${this._objectName}</button>
                </div>
