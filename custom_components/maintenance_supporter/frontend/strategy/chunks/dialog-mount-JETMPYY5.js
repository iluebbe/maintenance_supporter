...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function yt(n,o,t,e,s){let I=Math.max(t,e,o,1)*1.3,S=50,C=[];for(let T=0;T<=S;T++){let L=T/S*I,ht=1-Math.exp(-Math.pow(L/o,n)),ut=32+L/I*260,_t=136-ht*128;C.push([ut,_t])}let j=C.map(([T,L])=>`${T.toFixed(1)},${L.toFixed(1)}`).join(" "),K="M32,136 "+C.map(([T,L])=>`L${T.toFixed(1)},${L.toFixed(1)}`).join(" ")+` L${C[S][0].toFixed(1)},136 Z`,F=32+t/I*260,D=1-Math.exp(-Math.pow(t/o,n)),V=136-D*128,dt=((1-D)*100).toFixed(0),Z=32+e/I*260,pt=[0,.25,.5,.75,1];return r`
      ...
      <div class="dialog" role="dialog" aria-modal="true">
        ${e?r`
              <div class="header">
                ...
                <div class="object">
                  <button class="link-inline" @click=${()=>{this._entryId&&import("./dialog-mount-JETMPYY5.js").then(({openObjectQuickActions:c})=>{c(this._entryId),this.close()})}}>${this._objectName}</button>
                </div>
