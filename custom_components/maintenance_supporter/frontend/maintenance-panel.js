...
`;var W={overviewTab:"msp-overview-tab",collapsedSections:"msp-collapsed-sections",chartRange:"msp-chart-range",chartHideOutliers:"msp-chart-hide-outliers",taskSort:"maintenance_supporter_sort",objectSort:"maintenance_supporter_object_sort",groupBy:"maintenance_supporter_groupby",objectView:"maintenance_supporter_object_view"};var Ss={days:1,weeks:7,months:30.4368,years:365.25};function Kt(n,r){return!n||n<=0?0:n*(Ss[r||"days"]??1)}function yt(n,r,e){let t=Kt(n,e);if(t<=0||r==null)return{pct:0,overflow:!1};let i=(t-r)/t*100;return{pct:Math.max(0,Math.min(100,i)),overflow:i>100}}function O(n){return String(n??"").replace(/[&<>"']/g,r=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[r])}function Ei(n,r,e,t,i,a){let l=[[e.manufacturer,n.manufacturer],[e.model,n.model],[e.serial,n.serial_number],[e.installed,n.installation_date?t(n.installation_date):null],[e.warranty,n.warranty_expiry?t(n.warranty_expiry):null]].filter(([,u])=>!!u),c=r.map(u=>{let g=e.scheduleLabel(u);return`<tr>
      <td>${O(u.name)}</td>
    ...
    </tr>`}).join(""),p=r.reduce((u,g)=>u+(g.total_cost??0),0);return`<!DOCTYPE html><html><head><meta charset="utf-8">
...
<style>
  * { box-sizing: border-box; }
  ...
  <table>
    ...
    <tfoot><tr><td colspan="7">${O(e.totalCost)}</td><td class="num">${p.toFixed(2)} ${O(i)}</td></tr></tfoot>
  </table>
  ${n.notes?`<div class="notes"><strong>${O(e.notes)}:</strong>
${O(n.notes)}</div>`:""}
</body></html>`}function Yt(n,r=new Date){if(!n)return{kind:"none",days:null,date:null};let e=new Date(`${n}T00:00:00`);if(isNaN(e.getTime()))return{kind:"none",days:null,date:null};let t=Date.UTC(r.getFullYear(),r.getMonth(),r.getDate()),i=Date.UTC(e.getFullYear(),e.getMonth(),e.getDate()),a=Math.round((i-t)/864e5);return a<0?{kind:"expired",days:a,date:n}:a<=60?{kind:"expiring",days:a,date:n}:{kind:"valid",days:a,date:n}}var ze=[{key:"name",labelKey:"name",required:!0},{key:"manufacturer",labelKey:"manufacturer"},{key:"model",labelKey:"model"},{key:"serial_number",labelKey:"serial_number_label"},{key:"installation_date",labelKey:"installed"},{key:"warranty_expiry",labelKey:"warranty"},{key:"area_id",labelKey:"area"},{key:"documentation_url",labelKey:"documentation_url_label"},{key:"notes",labelKey:"object_notes_label"},{key:"task_count",labelKey:"tasks"},{key:"actions",labelKey:"actions"}],As=ze.map(n=>n.key),xt=["name","manufacturer","model","serial_number","installation_date","warranty_expiry","area_id","task_count","actions"];function tt(n){if(!Array.isArray(n))return[...xt];let r=new Set,e=[];for(let t of n)typeof t=="string"&&As.includes(t)&&!r.has(t)&&(r.add(t),e.push(t));return e.length?(e.includes("name")||e.unshift("name"),e):[...xt]}function $t(n,r,e){let t=new Blob([n],{type:e}),i=URL.createObjectURL(t),a=document.createElement("a");a.href=i,a.download=r,a.target="_blank",a.rel="noopener",a.style.display="none",document.body.appendChild(a),a.dispatchEvent(new MouseEvent("click")),document.body.removeChild(a),setTimeout(()=>URL.revokeObjectURL(i),6e4)}function wt(n,r){let e=document.createElement("a");e.href=n,e.download=r,e.target="_blank",e.rel="noopener",e.style.display="none",document.body.appendChild(e),e.dispatchEvent(new MouseEvent("click")),document.body.removeChild(e)}var D=n=>String(n??"").replace(/[&<>"']/g,r=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[r]);function Ti(n,r,e,t,i,a,l,c,p,u=[]){let g=[[e.object,D(r)],[e.type,D(e.typeLabel(n.type))],[e.interval,D(i(n))],[e.nextDue,n.next_due?D(t(n.next_due)):"\u2014"],[e.lastDone,n.last_performed?D(t(n.last_performed)):D(e.never)]];n.priority&&n.priority!=="normal"&&g.push([e.priority,D(n.priority)]);let m=(n.checklist||[]).map(f=>`<li><span class="box"></span>${D(f)}</li>`).join(""),v=(f,$)=>f?`<figure class="qr"><img src="${f}" alt="" /><figcaption>${D($)}</figcaption></figure>`:"";return`<!DOCTYPE html>
...
<body>
  ...
  <table class="meta">
    ${g.map(([f,$])=>`<tr><td>${D(f)}</td><td>${$}</td></tr>`).join("")}
  </table>
...
`;var kt=class{constructor(r){this._cache=new Map;this._pending=new Map;this._hass=r}updateHass(r){this._hass=r}async getDetailStats(r,e,t=30){return this._getStats(r,t<=35?"hour":"day",t,e)}async getMiniStats(r,e){return this._getStats(r,"day",14,e)}async getBatchMiniStats(r){let e=new Map,t=[];for(let p of r){let u=`${p.entityId}:day:14`,g=this._cache.get(u);g&&Date.now()-g.fetchedAt<3e5?e.set(p.entityId,g.points):t.push(p)}if(t.length===0)return e;let i=t.filter(p=>p.isCounter).map(p=>p.entityId),a=t.filter(p=>!p.isCounter).map(p=>p.entityId),l=new Date(Date.now()-336*60*60*1e3).toISOString(),c=[];return i.length>0&&c.push(this._fetchBatch(i,"day",l,["state","sum","change"],!0,e)),a.length>0&&c.push(this._fetchBatch(a,"day",l,["mean","min","max"],!1,e)),await Promise.all(c),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(r,e,t,i){let a=`${r}:${e}:${t}`,l=this._cache.get(a);if(l&&Date.now()-l.fetchedAt<3e5)return l.points;if(this._pending.has(a))return this._pending.get(a);let c=this._fetchAndNormalize(r,e,t,i,a);this._pending.set(a,c);try{return await c}finally{this._pending.delete(a)}}async _fetchAndNormalize(r,e,t,i,a){let l=new Date(Date.now()-t*24*60*60*1e3).toISOString(),c=i?["state","sum","change"]:["mean","min","max"];try{let u=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:l,statistic_ids:[r],period:e,types:c}))[r]||[],g=this._normalizeRows(u,i);return this._cache.set(a,{entityId:r,fetchedAt:Date.now(),period:e,points:g}),g}catch(p){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${r}:`,p),[]}}async _fetchBatch(r,e,t,i,a,l){try{let c=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:r,period:e,types:i});for(let p of r){let u=c[p]||[],g=this._normalizeRows(u,a);l.set(p,g),this._cache.set(`${p}:${e}:14`,{entityId:p,fetchedAt:Date.now(),period:e,points:g})}}catch(c){console.warn("[maintenance-supporter] Batch statistics fetch failed:",c)}}_normalizeRows(r,e){let t=[];for(let i of r){let a=null;if(e?a=i.state??null:a=i.mean??null,a===null)continue;let l={ts:i.start,val:a};e||(i.min!=null&&(l.min=i.min),i.max!=null&&(l.max=i.max)),t.push(l)}return t.sort((i,a)=>i.ts-a.ts),t}};var he=class{constructor(r){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=r}updateHass(r){this.hass=r}async getUsers(r=!1){let e=Date.now();if(!r&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(r,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:r,task_id:e,user_id:t})}async getTasksByUser(r){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:r})).tasks}getUserName(r){return!r||!this.usersCache?null:this.usersCache.find(t=>t.id===r)?.name||null}getUser(r){return!r||!this.usersCache?null:this.usersCache.find(e=>e.id===r)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(r){return r?r===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var Z=class extends A{constructor(){super(...arguments);this.label="";this.value="";this.placeholder="";this.type="text";this.required=!1;this.disabled=!1}_onInput(e){let t=e.target.value;this.value=t,this.dispatchEvent(new CustomEvent("input",{bubbles:!0,composed:!0,detail:{value:t}}))}render(){return o`
  ...
  `,d([b({attribute:!1})],U.prototype,"hass",2),d([b({attribute:!1})],U.prototype,"objects",2),d([_()],U.prototype,"_open",2),d([_()],U.prototype,"_loading",2),d([_()],U.prototype,"_error",2),d([_()],U.prototype,"_name",2),d([_()],U.prototype,"_manufacturer",2),d([_()],U.prototype,"_model",2),d([_()],U.prototype,"_serialNumber",2),d([_()],U.prototype,"_areaId",2),d([_()],U.prototype,"_installationDate",2),d([_()],U.prototype,"_warrantyExpiry",2),d([_()],U.prototype,"_documentationUrl",2),d([_()],U.prototype,"_notes",2),d([_()],U.prototype,"_haDeviceId",2),d([_()],U.prototype,"_parentEntryId",2),d([_()],U.prototype,"_entryId",2);customElements.get("maintenance-object-dialog")||customElements.define("maintenance-object-dialog",U);function ue(n){let r=n??0;return r<1024?`${r} B`:r<1024*1024?`${(r/1024).toFixed(1)} KB`:`${(r/(1024*1024)).toFixed(1)} MB`}var Et=["manual","warranty","invoice","spare_parts","photo","other"],Ms={manual:"mdi:book-open-variant",warranty:"mdi:shield-check",invoice:"mdi:receipt-text-outline",spare_parts:"mdi:cog-outline",photo:"mdi:image-outline",other:"mdi:file-document-outline"},F=class extends A{constructor(){super(...arguments);this.canWrite=!1;this._docs=[];this._loaded=!1;this._busy=!1;this._error="";this._hint="";this._addingLink=!1;this._linkUrl="";this._linkTitle="";this._category="manual";this._thumbs={};this._lightboxUrl="";this._editingId="";this._editTitle="";this._editCategory="manual";this._dragOver=!1;this._loadedFor=null;this._localeReady=!1}_isImage(e){return e.kind==="file"&&(e.mime||"").startsWith("image/")}async _sign(e){return(await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300})).path}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),this.hass&&!this._localeReady&&(this._localeReady=!0,V(this._lang).then(()=>this.requestUpdate())),this.hass&&this.entryId&&this._loadedFor!==this.entryId&&(this._loadedFor=this.entryId,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:this.entryId});this._docs=e.documents||[],this._loaded=!0,this._error="",this._thumbs={},this._loadThumbs()}catch(e){this._error=C(e,this._lang),this._loaded=!0}}async _loadThumbs(){await Promise.all(this._docs.filter(e=>this._isImage(e)).map(async e=>{try{let t=await this._sign(e);this._thumbs={...this._thumbs,[e.id]:t}}catch{}}))}_category_of(e){return(e.tags||[]).find(i=>Et.includes(i))||"other"}_labelKeydown(e){(e.key==="Enter"||e.key===" ")&&(e.preventDefault(),e.currentTarget.querySelector("input")?.click())}_onFileInput(e){let t=e.target,i=Array.from(t.files??[]);i.length&&this._uploadFiles(i),t.value=""}_onCameraInput(e){let t=e.target,i=Array.from(t.files??[]);i.length&&this._uploadFiles(i,"photo"),t.value=""}_onDrop(e){if(e.preventDefault(),this._dragOver=!1,!this.canWrite||this._busy)return;let t=Array.from(e.dataTransfer?.files??[]);t.length&&this._uploadFiles(t)}_onDragOver(e){this.canWrite&&(e.preventDefault(),this._dragOver=!0)}_onDragLeave(e){let t=e.relatedTarget;(!t||!e.currentTarget.contains(t))&&(this._dragOver=!1)}async _uploadFiles(e,t){let i=t??this._category;this._busy=!0,this._error="",this._hint="";let a=0,l=0;try{for(let c of e){let p=new FormData;p.append("entry_id",this.entryId),p.append("tags",i),p.append("file",c,c.name);let u=await fetch("/api/maintenance_supporter/document/upload",{method:"POST",headers:{Authorization:`Bearer ${this.hass.auth?.data?.access_token??""}`},body:p});if(!u.ok){this._error=u.status===413?s("doc_too_large",this._lang):s("doc_upload_failed",this._lang);continue}let g=await u.json();g.duplicate_in_object?l++:g.deduped&&a++}l?this._hint=s("doc_dup_in_object",this._lang):a&&(this._hint=s("doc_deduped",this._lang)),await this._load()}catch{this._error=s("doc_upload_failed",this._lang)}finally{this._busy=!1}}async _download(e){try{let t=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:30});wt(t.path,e.filename||e.title||"document")}catch(t){this._error=C(t,this._lang)}}async _preview(e){if(this._isImage(e)){this._lightboxUrl=this._thumbs[e.id]||await this._sign(e);return}let t=window.open("about:blank","_blank");try{let i=await this._sign(e);t&&(t.location.href=new URL(i,window.location.origin).href)}catch(i){t&&t.close(),this._error=C(i,this._lang)}}_openDoc(e){e.kind==="file"?this._preview(e):le(e.url)&&window.open(e.url,"_blank","noopener")}_startEdit(e){this._editingId=e.id,this._editTitle=e.title||"",this._editCategory=this._category_of(e),this._addingLink=!1,this._error=""}_cancelEdit(){this._editingId=""}async _saveEdit(e){let t=(e.tags||[]).filter(a=>!Et.includes(a)),i=e.kind==="file"?[this._editCategory,...t]:e.tags??[];this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,title:this._editTitle.trim()||e.filename||e.url||"",tags:i}),this._editingId="",await this._load()}catch(a){this._error=C(a,this._lang)}finally{this._busy=!1}}async _delete(e){let t=e.title||e.filename||e.url||"";if(window.confirm(s("doc_delete_confirm",this._lang).replace("{name}",t))){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/delete",doc_id:e.id}),await this._load()}catch(i){this._error=C(i,this._lang)}finally{this._busy=!1}}}async _addLink(){let e=this._linkUrl.trim();if(e){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/add_link",entry_id:this.entryId,url:e,title:this._linkTitle.trim()||null}),this._linkUrl="",this._linkTitle="",this._addingLink=!1,await this._load()}catch(t){this._error=C(t,this._lang,s("doc_link_invalid",this._lang))}finally{this._busy=!1}}}render(){let e=this._lang;return o`
      <div
      ...
      <div class="doc-header">
        ...
        ${this.canWrite?o`
              <div class="doc-actions">
                ...
                >
                  ${Et.map(t=>o`<option value=${t}>${s(`doc_cat_${t}`,e)}</option>`)}
                </select>
                <label
    ...
    `}_renderDoc(e,t){if(this._editingId===e.id)return this._renderEdit(e,t);let i=e.kind==="file",a=this._category_of(e),l=i?`${s(`doc_cat_${a}`,t)} \xB7 ${ue(e.size)}`:s("doc_link_badge",t),c=this._thumbs[e.id];return o`
      <div class="doc-row">
        ...
        <div
          ...
          @keydown=${p=>{(p.key==="Enter"||p.key===" ")&&(p.preventDefault(),this._openDoc(e))}}
        >
    ...
    `}_renderEdit(e,t){let i=e.kind==="file";return o`
      <div class="doc-row editing">
        ...
        ${i?o`<select
            ...
            >
              ${Et.map(a=>o`<option value=${a} ?selected=${a===this._editCategory}>${s(`doc_cat_${a}`,t)}</option>`)}
            </select>`:h}
  ...
  `,d([b({attribute:!1})],F.prototype,"hass",2),d([b({attribute:!1})],F.prototype,"entryId",2),d([b({type:Boolean})],F.prototype,"canWrite",2),d([_()],F.prototype,"_docs",2),d([_()],F.prototype,"_loaded",2),d([_()],F.prototype,"_busy",2),d([_()],F.prototype,"_error",2),d([_()],F.prototype,"_hint",2),d([_()],F.prototype,"_addingLink",2),d([_()],F.prototype,"_linkUrl",2),d([_()],F.prototype,"_linkTitle",2),d([_()],F.prototype,"_category",2),d([_()],F.prototype,"_thumbs",2),d([_()],F.prototype,"_lightboxUrl",2),d([_()],F.prototype,"_editingId",2),d([_()],F.prototype,"_editTitle",2),d([_()],F.prototype,"_editCategory",2),d([_()],F.prototype,"_dragOver",2);customElements.get("maintenance-documents-section")||customElements.define("maintenance-documents-section",F);var Is=["manual","warranty","invoice","spare_parts","photo","other"],Ls={manual:"mdi:book-open-variant",warranty:"mdi:shield-check",invoice:"mdi:receipt-text-outline",spare_parts:"mdi:cog-outline",photo:"mdi:image-outline",other:"mdi:file-document-outline"},X=class extends A{constructor(){super(...arguments);this.canWrite=!1;this._docs=[];this._loaded=!1;this._busy=!1;this._error="";this._attachId="";this._loadedKey="";this._localeReady=!1}get _lang(){return this.hass?.language||"en"}get _refId(){return this.partId||this.taskId||""}get _linkField(){return this.partId?"part_ids":"task_ids"}updated(e){super.updated(e),this.hass&&!this._localeReady&&(this._localeReady=!0,V(this._lang).then(()=>this.requestUpdate()));let t=`${this.entryId}|${this._refId}`;this.hass&&this.entryId&&this._refId&&this._loadedKey!==t&&(this._loadedKey=t,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:this.entryId});this._docs=e.documents||[],this._loaded=!0,this._error=""}catch(e){this._error=C(e,this._lang),this._loaded=!0}}_links(e){return e[this._linkField]||[]}_linked(){return this._docs.filter(e=>this._links(e).includes(this._refId))}_available(){return this._docs.filter(e=>!this._links(e).includes(this._refId))}async _setLinks(e,t){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,[this._linkField]:t}),await this._load()}catch(i){this._error=C(i,this._lang)}finally{this._busy=!1}}_link(){let e=this._docs.find(t=>t.id===this._attachId);e&&(this._attachId="",this._setLinks(e,[...this._links(e),this._refId]))}_unlink(e){this._setLinks(e,this._links(e).filter(t=>t!==this._refId))}_isPdf(e){return e.mime==="application/pdf"||(e.filename||"").toLowerCase().endsWith(".pdf")}_pageFor(e){return this._isPdf(e)&&this.taskId?e.task_pages?.[this.taskId]:void 0}async _open(e){if(e.kind==="weblink"){window.open(e.url,"_blank","noopener");return}let t=this._pageFor(e),i=t?`#page=${t}`:"",a=window.open("about:blank","_blank");try{let l=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300});a&&(a.location.href=new URL(l.path+i,window.location.origin).href)}catch(l){a&&a.close(),this._error=C(l,this._lang)}}async _setPage(e,t){if(this.taskId){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,task_pages:{[this.taskId]:t}}),await this._load()}catch(i){this._error=C(i,this._lang)}finally{this._busy=!1}}}async _download(e){try{let t=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:30});wt(t.path,e.filename||e.title||"document")}catch(t){this._error=C(t,this._lang)}}render(){if(!this._loaded||this._docs.length===0)return h;let e=this._lang,t=this._linked(),i=this._available();return o`
      <div class="task-docs">
    ...
    `}_renderRow(e,t){let i=e.kind==="file",a=this._isPdf(e),l=this._pageFor(e),c=(e.tags||[]).find(u=>Is.includes(u))||"other",p=i?ue(e.size):s("doc_link_badge",t);return o`
      <div class="tdoc-row">
        <ha-icon class="tdoc-icon" icon=${i?Ls[c]:"mdi:link-variant"}></ha-icon>
        <div
          class="tdoc-info"
          role="button"
          tabindex="0"
        ...
        >
          ...
          <div class="tdoc-meta">
            ${p}${l?o` · <span class="tdoc-pagetag">${s("doc_page",t)} ${l}</span>`:h}
          </div>
        </div>
...
`).map(p=>p.trim()).filter(Boolean).slice(0,100);e.checklist=c.length?c:null}if(this.completionActionsEnabled){let c=this._actionService.trim();if(c&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(c)){let m={service:c},v=this._actionTargetEntity.trim();v&&(m.target={entity_id:v});let f=this._buildActionData();Object.keys(f).length>0&&(m.data=f),e.on_complete_action=m}else e.on_complete_action=null;let p={};this._qcNotes.trim()&&(p.notes=this._qcNotes.trim());let u=parseFloat(this._qcCost);!isNaN(u)&&u>=0&&(p.cost=u);let g=parseInt(this._qcDuration,10);!isNaN(g)&&g>=0&&(p.duration=g),this._qcFeedback&&(p.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(p).length?p:null}let i=await this.hass.connection.sendMessagePromise(e),a=this._taskId||i?.task_id,l=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(a&&this._scheduleType==="sensor_based"&&l)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:a,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=C(e,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return h;let e=this._lang,t=this._triggerType==="compound";return o`
      ...
      <div class="select-row">
        ...
        >
          ${Ds.map(i=>o`<option value=${i} ?selected=${i===this._triggerType}>${s(i,e)}</option>`)}
        </select>
      </div>
    ...
    `}_patchCondition(e,t){this._compoundConditions=this._compoundConditions.map((i,a)=>a===e?{...i,...t}:i)}_addCondition(){this._compoundConditions=[...this._compoundConditions,qs()]}_removeCondition(e){this._compoundConditions=this._compoundConditions.filter((t,i)=>i!==e)}_renderCompoundEditor(){let e=this._lang;return o`
      <div class="select-row">
    ...
    `}_renderCondition(e,t){let i=this._lang,a=t+1;return o`
      <div class="compound-condition">
        ...
        <div class="select-row">
          ...
          >
            ${ji.map(l=>o`<option value=${l} ?selected=${l===e.type}>${s(l,i)}</option>`)}
          </select>
        </div>
    ...
    `}_buildSchedule(){let e=i=>{let a=parseInt(this._calOffset,10)||0;return a&&(i.offset=Math.max(-15,Math.min(a,15))),i};if(this._scheduleType==="weekdays")return e({kind:"weekdays",weekdays:[...this._weekdays].sort((i,a)=>i-a)});if(this._scheduleType==="nth_weekday")return e({kind:"nth_weekday",nth:parseInt(this._nth,10),weekday:parseInt(this._nthWeekday,10)});let t={kind:"day_of_month",day:this._domLastDay?-1:parseInt(this._domDay,10)||1};return this._domBusiness&&(t.business=!0),e(t)}_recurrenceExtras(){let e={};if(this._seasonMonths.length&&(e.season_months=[...this._seasonMonths].sort((t,i)=>t-i)),this._endsMode==="count"){let t=parseInt(this._endsCount,10);t>=1&&(e.ends={count:t})}else this._endsMode==="until"&&this._endsUntil&&(e.ends={until:this._endsUntil});return e}_toggleSeasonMonth(e){this._seasonMonths=this._seasonMonths.includes(e)?this._seasonMonths.filter(t=>t!==e):[...this._seasonMonths,e]}_renderRecurrenceExtras(){let e=this._lang;if(!(this._scheduleType==="time_based"||Gt.includes(this._scheduleType)))return h;let i=Vs(e);return o`
      ...
      ${this._endsMode==="until"?o`
        ...
        ></ms-textfield>`:h}
    `}_renderCalendarFields(){let e=this._lang,t=Bs(e);if(this._scheduleType==="weekdays")return o`
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${zs.map(i=>o`<option value=${i} ?selected=${i===this._type}>${s(i,e)}</option>`)}
            </select>
          </div>
                  ...
                  ${this.parts.map(i=>{let a=this._consumesParts[i.id];return o`
                      <div class="consumes-row">
                        ...
                        ${a!==void 0?o`<input
                              ...
                              step="0.01"
                              .value=${String(a)}
                              @input=${l=>{let c=parseFloat(l.target.value);this._consumesParts={...this._consumesParts,[i.id]:Number.isFinite(c)&&c>=.01?c:1}}}
                            />`:h}
                      </div>
                    `})}
                </div>
          ...
          <div class="select-row">
            ...
            >
              ${Os.map(i=>o`<option value=${i} ?selected=${i===this._priority}>${s("priority_"+i,e)}</option>`)}
            </select>
          </div>
          <div class="field">
  ...
  `,d([b({attribute:!1})],y.prototype,"hass",2),d([b({type:Boolean,attribute:"checklists-enabled"})],y.prototype,"checklistsEnabled",2),d([b({type:Boolean,attribute:"schedule-time-enabled"})],y.prototype,"scheduleTimeEnabled",2),d([b({type:Boolean,attribute:"completion-actions-enabled"})],y.prototype,"completionActionsEnabled",2),d([b({type:Number,attribute:"default-warning-days"})],y.prototype,"defaultWarningDays",2),d([_()],y.prototype,"parts",2),d([_()],y.prototype,"_open",2),d([_()],y.prototype,"_loading",2),d([_()],y.prototype,"_error",2),d([_()],y.prototype,"_entryId",2),d([_()],y.prototype,"_taskId",2),d([_()],y.prototype,"_objectChoices",2),d([_()],y.prototype,"_name",2),d([_()],y.prototype,"_type",2),d([_()],y.prototype,"_scheduleType",2),d([_()],y.prototype,"_intervalDays",2),d([_()],y.prototype,"_intervalUnit",2),d([_()],y.prototype,"_dueDate",2),d([_()],y.prototype,"_warningDays",2),d([_()],y.prototype,"_earliestCompletionDays",2),d([_()],y.prototype,"_intervalAnchor",2),d([_()],y.prototype,"_weekdays",2),d([_()],y.prototype,"_nth",2),d([_()],y.prototype,"_nthWeekday",2),d([_()],y.prototype,"_domDay",2),d([_()],y.prototype,"_domLastDay",2),d([_()],y.prototype,"_domBusiness",2),d([_()],y.prototype,"_calOffset",2),d([_()],y.prototype,"_seasonMonths",2),d([_()],y.prototype,"_endsMode",2),d([_()],y.prototype,"_endsCount",2),d([_()],y.prototype,"_endsUntil",2),d([_()],y.prototype,"_schedulePreview",2),d([_()],y.prototype,"_schedulePreviewEnded",2),d([_()],y.prototype,"_notes",2),d([_()],y.prototype,"_documentationUrl",2),d([_()],y.prototype,"_customIcon",2),d([_()],y.prototype,"_priority",2),d([_()],y.prototype,"_labels",2),d([_()],y.prototype,"_enabled",2),d([_()],y.prototype,"_triggerEntityId",2),d([_()],y.prototype,"_triggerEntityIds",2),d([_()],y.prototype,"_triggerEntityLogic",2),d([_()],y.prototype,"_triggerAttribute",2),d([_()],y.prototype,"_triggerType",2),d([_()],y.prototype,"_triggerAbove",2),d([_()],y.prototype,"_triggerBelow",2),d([_()],y.prototype,"_triggerForMinutes",2),d([_()],y.prototype,"_triggerTargetValue",2),d([_()],y.prototype,"_triggerDeltaMode",2),d([_()],y.prototype,"_triggerBaselineValue",2),d([_()],y.prototype,"_liveBaselineValue",2),d([_()],y.prototype,"_autoCompleteOnRecovery",2),d([_()],y.prototype,"_triggerFromState",2),d([_()],y.prototype,"_triggerToState",2),d([_()],y.prototype,"_triggerTargetChanges",2),d([_()],y.prototype,"_triggerRuntimeHours",2),d([_()],y.prototype,"_triggerOnStates",2),d([_()],y.prototype,"_compoundLogic",2),d([_()],y.prototype,"_compoundConditions",2),d([_()],y.prototype,"_suggestedAttributes",2),d([_()],y.prototype,"_availableAttributes",2),d([_()],y.prototype,"_entityDomain",2),d([_()],y.prototype,"_lastPerformed",2),d([_()],y.prototype,"_nfcTagId",2),d([_()],y.prototype,"_readingUnit",2),d([_()],y.prototype,"_consumesParts",2),d([_()],y.prototype,"_partsLoadFailed",2),d([_()],y.prototype,"_availableTags",2),d([_()],y.prototype,"_responsibleUserId",2),d([_()],y.prototype,"_assigneePool",2),d([_()],y.prototype,"_rotationStrategy",2),d([_()],y.prototype,"_availableUsers",2),d([_()],y.prototype,"_checklistText",2),d([_()],y.prototype,"_requiredCompletion",2),d([_()],y.prototype,"_scheduleTime",2),d([_()],y.prototype,"_actionService",2),d([_()],y.prototype,"_actionTargetEntity",2),d([_()],y.prototype,"_actionData",2),d([_()],y.prototype,"_actionDataJsonFallback",2),d([_()],y.prototype,"_actionTesting",2),d([_()],y.prototype,"_actionTestResult",2),d([_()],y.prototype,"_actionTestError",2),d([_()],y.prototype,"_qcNotes",2),d([_()],y.prototype,"_qcCost",2),d([_()],y.prototype,"_qcDuration",2),d([_()],y.prototype,"_qcFeedback",2),d([_()],y.prototype,"_environmentalEntity",2),d([_()],y.prototype,"_environmentalAttribute",2);var Qt=y;customElements.get("maintenance-task-dialog")||customElements.define("maintenance-task-dialog",Qt);var P=class extends A{constructor(){super(...arguments);this.entryId="";this.taskId="";this.taskName="";this.lang="en";this.checklist=[];this.adaptiveEnabled=!1;this.taskType="";this.readingUnit="";this.restockDefault=null;this.parts=[];this.consumesParts=[];this.consumesInfo=[];this.requiredFields=[];this._open=!1;this._notes="";this._cost="";this._duration="";this._loading=!1;this._error="";this._checklistState={};this._feedback="needed";this._photoDocId="";this._photoPreview="";this._photoUploading=!1;this._readingValue="";this._restockQty="";this._usedParts={}}open(){this._open||(this._open=!0,this._notes="",this._cost="",this._duration="",this._error="",this._checklistState={},this._feedback="needed",this._photoDocId="",this._photoPreview="",this._photoUploading=!1,this._readingValue="",this._restockQty=this.restockDefault!==null?String(this.restockDefault):"",this._usedParts=Object.fromEntries(this.consumesParts.map(e=>[e.part_id,e.quantity])))}_toggleCheck(e){let t=String(e);this._checklistState={...this._checklistState,[t]:!this._checklistState[t]}}_setFeedback(e){this._feedback=e}async _onPhotoInput(e){let t=e.target,i=t.files?.[0];if(t.value="",!!i){this._photoUploading=!0,this._error="";try{let a=new FormData;a.append("entry_id",this.entryId),a.append("tags","photo"),a.append("file",i,i.name);let l=await fetch("/api/maintenance_supporter/document/upload",{method:"POST",headers:{Authorization:`Bearer ${this.hass.auth?.data?.access_token??""}`},body:a});if(!l.ok){this._error=l.status===413?s("doc_too_large",this.lang):s("doc_upload_failed",this.lang);return}let c=await l.json();c.id&&(this._photoDocId=c.id,this._photoPreview=URL.createObjectURL(i))}catch{this._error=s("doc_upload_failed",this.lang)}finally{this._photoUploading=!1}}}_removePhoto(){this._photoPreview&&URL.revokeObjectURL(this._photoPreview),this._photoDocId="",this._photoPreview=""}async _complete(){this._loading=!0,this._error="";try{let e={type:"maintenance_supporter/task/complete",entry_id:this.entryId,task_id:this.taskId};if(this._notes&&(e.notes=this._notes),this._cost){let t=parseFloat(this._cost);!isNaN(t)&&t>=0&&(e.cost=t)}if(this._duration){let t=parseInt(this._duration,10);!isNaN(t)&&t>=0&&(e.duration=t)}if(this.checklist.length>0&&(e.checklist_state=this._checklistState),this.adaptiveEnabled&&(e.feedback=this._feedback),this._photoDocId&&(e.photo_doc_id=this._photoDocId),this._readingValue!==""){let t=parseFloat(this._readingValue);isNaN(t)||(e.reading_value=t)}if(this.restockDefault!==null&&this._restockQty!==""){let t=parseFloat(this._restockQty);!isNaN(t)&&t>=1&&(e.restock_quantity=t)}this.parts.length>0&&(e.used_parts=Object.entries(this._usedParts).filter(([,t])=>Number.isFinite(t)&&t>0).map(([t,i])=>({part_id:t,quantity:i}))),await this.hass.connection.sendMessagePromise(e),this._open=!1,this.dispatchEvent(new CustomEvent("task-completed"))}catch(e){this._error=C(e,this.lang,s("save_error",this.lang))}finally{this._loading=!1}}get _missingRequired(){let e={notes:this._notes.trim()!=="",cost:this._cost.trim()!=="",duration:this._duration.trim()!=="",photo:this._photoDocId!=="",user:!!this.hass?.user};return this.requiredFields.filter(t=>!e[t])}_req(e){return this.requiredFields.includes(e)?o`<span class="req-mark" aria-hidden="true">*</span>`:h}_close(){this._open=!1}render(){if(!this._open)return o``;let e=this.lang||this.hass?.language||"en";return o`
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          ${this.parts.length?o`<div class="used-parts">
                ...
                ${this.parts.map(t=>{let i=this._usedParts[t.id],a=i!==void 0;return o`<div class="used-part-row">
                    <label class="used-part-check">
                      <input type="checkbox" .checked=${a}
                        @change=${l=>{let c={...this._usedParts};l.target.checked?c[t.id]=c[t.id]||1:delete c[t.id],this._usedParts=c}} />
                      <span>${t.name}${t.stock!==null&&t.stock!==void 0?` (${t.stock}${t.unit?" "+t.unit:""})`:""}</span>
                    </label>
                    ${a?o`<input class="used-part-qty" type="number" min="0.01" max="999" step="0.01"
                          .value=${String(i)}
                          @input=${l=>{let c=parseFloat(l.target.value);this._usedParts={...this._usedParts,[t.id]:Number.isFinite(c)&&c>=.01?c:1}}} />`:h}
                  </div>`})}
        ...
        <div class="dialog-actions">
          ...
          <ha-button
            ...
            title=${this._missingRequired.length?this._missingRequired.map(t=>s("err_required",e).replace("{field}",s(Tt[t]??t,e))).join(" \xB7 "):""}
          >
  ...
  `],d([b({attribute:!1})],P.prototype,"hass",2),d([b()],P.prototype,"entryId",2),d([b()],P.prototype,"taskId",2),d([b()],P.prototype,"taskName",2),d([b()],P.prototype,"lang",2),d([b({type:Array})],P.prototype,"checklist",2),d([b({type:Boolean})],P.prototype,"adaptiveEnabled",2),d([b()],P.prototype,"taskType",2),d([b()],P.prototype,"readingUnit",2),d([b({attribute:!1})],P.prototype,"restockDefault",2),d([b({attribute:!1})],P.prototype,"parts",2),d([b({attribute:!1})],P.prototype,"consumesParts",2),d([b({type:Array})],P.prototype,"consumesInfo",2),d([b({type:Array})],P.prototype,"requiredFields",2),d([_()],P.prototype,"_open",2),d([_()],P.prototype,"_notes",2),d([_()],P.prototype,"_cost",2),d([_()],P.prototype,"_duration",2),d([_()],P.prototype,"_loading",2),d([_()],P.prototype,"_error",2),d([_()],P.prototype,"_checklistState",2),d([_()],P.prototype,"_feedback",2),d([_()],P.prototype,"_photoDocId",2),d([_()],P.prototype,"_photoPreview",2),d([_()],P.prototype,"_photoUploading",2),d([_()],P.prototype,"_readingValue",2),d([_()],P.prototype,"_restockQty",2),d([_()],P.prototype,"_usedParts",2);customElements.get("maintenance-complete-dialog")||customElements.define("maintenance-complete-dialog",P);function Oe(n){return n.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function Ci(n){return!n.startsWith("data:image/svg+xml,")&&!n.startsWith("data:image/png;base64,")?"":Oe(n)}function Ws(n){return n.replace(/[/\\:*?"<>|#%]+/g,"").replace(/\s+/g,"-").toLowerCase().substring(0,100)}var oe=class extends A{constructor(){super(...arguments);this.lang="en";this._open=!1;this._loading=!1;this._error="";this._viewResult=null;this._completeResult=null;this._urlMode="companion";this._entryId="";this._taskId=null;this._objectName="";this._taskName="";this._generateSeq=0}openForObject(e,t){this._entryId=e,this._taskId=null,this._objectName=t,this._taskName="",this._urlMode="companion",this._error="",this._viewResult=null,this._completeResult=null,this._open=!0,this._generate()}openForTask(e,t,i,a){this._entryId=e,this._taskId=t,this._objectName=i,this._taskName=a,this._urlMode="companion",this._error="",this._viewResult=null,this._completeResult=null,this._open=!0,this._generate()}async _generate(){let e=++this._generateSeq;this._loading=!0,this._error="",this._viewResult=null,this._completeResult=null;try{let t={type:"maintenance_supporter/qr/generate",entry_id:this._entryId,url_mode:this._urlMode};this._taskId&&(t.task_id=this._taskId);let i=[this.hass.connection.sendMessagePromise({...t,action:"view"})];this._taskId&&i.push(this.hass.connection.sendMessagePromise({...t,action:"complete"}));let a=await Promise.all(i);if(e!==this._generateSeq)return;this._viewResult=a[0],a.length>1&&(this._completeResult=a[1])}catch(t){if(e!==this._generateSeq)return;let i=t?.code,a=t?.message;this._error=i==="no_url"||typeof a=="string"&&a.includes("No Home Assistant URL")?s("qr_error_no_url",this.lang):s("qr_error",this.lang)}finally{e===this._generateSeq&&(this._loading=!1)}}_setUrlMode(e){this._urlMode!==e&&(this._urlMode=e,this._generate())}_print(){if(!this._viewResult)return;let e=this._viewResult,t=e.label.task_name?`${e.label.object_name} \u2014 ${e.label.task_name}`:e.label.object_name,i=[e.label.manufacturer,e.label.model].filter(Boolean).join(" "),a=window.open("","_blank","width=600,height=500");if(!a)return;let l=this.lang||"en",c=Oe(t),p=Oe(i),u=!!this._completeResult,g=Oe(s("qr_action_view",l)),m=Oe(s("qr_action_complete",l));a.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8">
<title>${c}</title>
<style>
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `,d([b({attribute:!1})],ie.prototype,"hass",2),d([_()],ie.prototype,"_open",2),d([_()],ie.prototype,"_loading",2),d([_()],ie.prototype,"_adopting",2),d([_()],ie.prototype,"_error",2),d([_()],ie.prototype,"_sensors",2),d([_()],ie.prototype,"_selected",2),d([_()],ie.prototype,"_users",2),d([_()],ie.prototype,"_responsible",2);customElements.get("maintenance-adopt-problem-sensors-dialog")||customElements.define("maintenance-adopt-problem-sensors-dialog",ie);var ee=class extends A{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._adopting=!1;this._error="";this._setups=[];this._selected=new Set;this._baselines=new Map;this._targets=new Map;this._objects=[];this._localeReady=!1;this._toggle=e=>{let t=new Set(this._selected);t.has(e)?t.delete(e):t.add(e),this._selected=t};this._adopt=async()=>{if(!(this._selected.size===0||this._adopting)){this._adopting=!0,this._error="";try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/integration_setups/adopt",selections:[...this._selected].map(t=>{let i={device_id:t},a=this._targets.get(t);a&&(i.entry_id=a);let l=this._setups.find(c=>c.device_id===t);for(let c of l?.tasks??[]){let p=this._baselines.get(`${t} ${c.task_name}`),u=p?parseFloat(p):NaN;!isNaN(u)&&u>=0&&((i.baselines??={})[c.task_name]=u)}return i})});this.dispatchEvent(new CustomEvent("integration-setups-adopted",{bubbles:!0,composed:!0,detail:e})),this._open=!1}catch(e){this._error=C(e,this._lang)}finally{this._adopting=!1}}}}get _lang(){return this.hass?.language||"en"}updated(e){e.has("hass")&&this.hass&&!this._localeReady&&(this._localeReady=!0,V(this._lang).then(()=>this.requestUpdate()))}async open(){this._open=!0,this._loading=!0,this._error="",this._setups=[],this._selected=new Set;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/integration_setups/discover"});this._setups=e.setups||[],this._selected=new Set(this._setups.map(t=>t.device_id)),this._baselines=new Map,this._targets=new Map;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"});this._objects=(t.objects||[]).map(i=>({entry_id:i.entry_id,name:i.object?.name||i.entry_id})).sort((i,a)=>i.name.localeCompare(a.name))}catch{this._objects=[]}}catch(e){this._error=C(e,this._lang)}finally{this._loading=!1}}_close(){this._open=!1}render(){if(!this._open)return o``;let e=this._lang;return o`
      <div class="overlay" @click=${this._close}>
                          ...
                          <div class="row-main">
                            ...
                            <div class="row-target" @click=${l=>l.preventDefault()}>
                              ...
                              ${i&&this._objects.length>0?o`
                                    <select
                                      class="target-select"
                                      @change=${l=>{let c=new Map(this._targets),p=l.target.value;p?c.set(t.device_id,p):c.delete(t.device_id),this._targets=c}}
                                    >
                            ...
                            ${i?t.tasks.filter(l=>l.direction==="usage_delta").map(l=>{let c=`${t.device_id} ${l.task_name}`;return o`
                                      <div class="baseline-field" @click=${p=>p.preventDefault()}>
                                        ...
                                        <input
                                          ...
                                          @input=${p=>{let u=new Map(this._baselines);u.set(c,p.target.value),this._baselines=u}}
                                        />
                                      </div>
                                    `}):h}
                          </div>
  ...
  `,d([b({attribute:!1})],be.prototype,"hass",2),d([_()],be.prototype,"_ov",2),d([_()],be.prototype,"_loading",2),d([_()],be.prototype,"_marking",2),d([_()],be.prototype,"_error",2);customElements.get("maintenance-battery-fleet-section")||customElements.define("maintenance-battery-fleet-section",be);var Ri=5;function it(n){let r=n.getFullYear(),e=String(n.getMonth()+1).padStart(2,"0"),t=String(n.getDate()).padStart(2,"0");return`${r}-${e}-${t}`}function Ks(n,r){let e=[];for(let t=0;t<r;t++){let i=new Date(n);i.setDate(i.getDate()+t),i.setHours(0,0,0,0),e.push(it(i))}return e}function St(n,r){let[e,t,i]=n.split("-").map(Number),a=new Date(e,t-1,i);return a.setDate(a.getDate()+r),it(a)}function Ys(n){if(!n||n.length===0)return null;let r=n.map(e=>e.cost).filter(e=>typeof e=="number");return r.length===0?null:r.reduce((e,t)=>e+t,0)/r.length}function Gs(n){let{windowStart:r,windowEnd:e,task:t,entryId:i,objectName:a}=n,l=[],c=(m,v)=>({date:m,entry_id:i,task_id:t.id,task_name:t.name,object_name:a,status:v&&(t.status==="overdue"||t.status==="triggered")?"ok":t.status,days_until_due:v?null:t.days_until_due??null,projected:v,schedule_type:t.schedule_type,interval_days:t.interval_days??null,interval_unit:t.interval_unit??null,responsible_user_id:t.responsible_user_id??null,avg_cost:Ys(t.history),adaptive_enabled:!!t.adaptive_config?.enabled,prediction_confidence:t.threshold_prediction_confidence??null}),p=Math.max(1,Math.round(Kt(t.interval_days,t.interval_unit)));if(t.status==="overdue"||t.status==="triggered"){if(l.push(c(r,!1)),t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let m=St(r,p),v=1;for(;m<=e&&v<Ri;)l.push(c(m,!0)),v++,m=St(m,p)}return l}let u=t.next_due;if(typeof u!="string"||!u)return l;let g=u.slice(0,10);if(g>=r&&g<=e)l.push(c(g,!1));else if(g>e)return l;if(t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let m=St(g,p),v=l.length;for(;m<=e&&v<Ri;)m>=r&&(l.push(c(m,!0)),v++),m=St(m,p)}return l}var Mi={overdue:0,triggered:1,due_soon:2,ok:3};function Ii(n,r,e,t=null){let i=Ks(r,e),a=i[0],l=i[i.length-1],c=[];for(let u of n){let g=u.object?.name||"",m=u.entry_id,v=u.tasks||[];for(let f of v){if(t&&f.responsible_user_id!==t||f.enabled===!1)continue;let $=Gs({windowStart:a,windowEnd:l,task:f,entryId:m,objectName:g});c.push(...$)}}let p=new Map;for(let u of i)p.set(u,[]);for(let u of c){let g=p.get(u.date);g&&g.push(u)}for(let[,u]of p)u.sort((g,m)=>{let v=Mi[g.status]??99,f=Mi[m.status]??99;if(v!==f)return v-f;if(g.projected!==m.projected)return g.projected?1:-1;let $=g.object_name.localeCompare(m.object_name);return $!==0?$:g.task_name.localeCompare(m.task_name)});return i.map(u=>({date:u,events:p.get(u)??[]}))}var Qs={completed:"ok",reset:"ok",skipped:"due_soon",triggered:"triggered",trigger_replaced:"triggered",trigger_removed:"ok"};function Zs(n,r){let e=[];for(let t=r-1;t>=0;t--){let i=new Date(n);i.setDate(i.getDate()-t),i.setHours(0,0,0,0),e.push(it(i))}return e}function Li(n,r,e,t=null){let i=Zs(r,e),a=i[0],l=i[i.length-1],c=new Map;for(let u of i)c.set(u,[]);for(let u of n){let g=u.object?.name||"",m=u.entry_id,v=u.tasks||[];for(let f of v){if(t&&f.responsible_user_id!==t)continue;let $=f.history||[];for(let w of $){if(typeof w?.timestamp!="string")continue;let R=w.timestamp.slice(0,10);if(R<a||R>l)continue;let I=c.get(R);if(!I)continue;let x=w.type??"completed";I.push({date:R,entry_id:m,task_id:f.id,task_name:f.name,object_name:g,status:Qs[x]??"ok",days_until_due:null,projected:!1,schedule_type:f.schedule_type,interval_days:f.interval_days??null,responsible_user_id:f.responsible_user_id??null,avg_cost:typeof w.cost=="number"?w.cost:null,adaptive_enabled:!!f.adaptive_config?.enabled,prediction_confidence:null,history_timestamp:w.timestamp,history_type:x,history_cost:typeof w.cost=="number"?w.cost:null,history_notes:typeof w.notes=="string"?w.notes:null,history_duration:typeof w.duration=="number"?w.duration:null})}}}let p={completed:0,reset:1,skipped:2,triggered:3,trigger_replaced:4};for(let[,u]of c)u.sort((g,m)=>{let v=p[g.history_type??""]??99,f=p[m.history_type??""]??99;if(v!==f)return v-f;let $=g.object_name.localeCompare(m.object_name);return $!==0?$:g.task_name.localeCompare(m.task_name)});return i.map(u=>({date:u,events:c.get(u)??[]}))}var Pi=j`
  .cal-controls {
    display: flex;
    gap: 12px;
    align-items: center;
...
`;var se=class extends A{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._objectFilter="";this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter),typeof e.object_filter=="string"&&(this._objectFilter=e.object_filter)}getCardSize(){return 6}get _lang(){return this.hass?.language||"en"}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){super.updated(e),e.has("hass")&&mt(this.hass?.locale);let t=this.hass?.language;if(t&&!gt(t)&&V(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{let e=await this.hass.connection.subscribeMessage(t=>{let i=t;this._objects=i.objects},{type:"maintenance_supporter/subscribe"});if(!this.isConnected){e();return}this._unsub=e}catch{}}_onEventClick(e){if(e.history_timestamp){this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}));return}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}render(){if(!this.hass)return h;let e=this._lang,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.title,l=null;this._userFilter&&(l=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let c=this._config.show_object_filter!==!1&&this._objects.length>1,p=null;if(this._objectFilter){let x=this._objectFilter.toLowerCase();p=this._objects.find(S=>S.entry_id===this._objectFilter||S.object.name.toLowerCase()===x)?.entry_id??null}let u=p?this._objects.filter(x=>x.entry_id===p):this._objects,g=new Date;g.setHours(0,0,0,0);let m=this._pastDays>0,v=m?Li(u,g,this._pastDays,l):Ii(u,g,this._windowDays,l),f=it(g),$=this._windowDays===365||m,w=$?v.filter(x=>x.events.length>0):v,R=x=>{let L=`cal-status-${x.status}`,S=x.projected?"cal-event-projected":"",te=x.status==="overdue"&&x.days_until_due!=null?` (${ke(x.days_until_due,e)})`:"",Q=x.projected&&x.interval_days?o`<span class="cal-event-recur">${x.interval_unit&&x.interval_unit!=="days"?`${x.interval_days} ${s("unit_"+x.interval_unit,e)}`:s("cal_every_n_days",e).replace("{n}",String(x.interval_days))}</span>`:h,Y=x.schedule_type==="sensor_based",N=Y?o`<ha-icon class="cal-event-icon cal-source-sensor"
      ...
      `},I=x=>{let[L,S,te]=x.date.split("-").map(Number),Q=new Date(L,S-1,te),Y=x.date===f,N=Q.toLocaleDateString(e,{weekday:"short"}),q=Q.toLocaleDateString(e,{month:"long"});return o`
        <div class="cal-day-row">
          ...
          <div class="cal-day-content">
            ...
            ${x.events.length===0?o`<div class="cal-empty">${s("cal_no_events",e)}</div>`:x.events.map(R)}
          </div>
        </div>
      `};return o`
      <ha-card .header=${a}>
              ...
              <div class="cal-controls">
                ${t?o`
                      <div class="cal-window-chips cal-past-chips" title="${s("cal_past_windows",e)||"Past windows"}">
                        ${[30,90].map(x=>o`
                          <button class="cal-window-chip cal-past-chip ${this._pastDays===x?"active":""}"
                            @click=${()=>{this._pastDays=x}}>
                            −${x}d
                          </button>
                        `)}
                      </div>
                      ...
                      <div class="cal-window-chips" title="${s("cal_forward_windows",e)||"Forward windows"}">
                        ${[7,14,30,365].map(x=>o`
                          <button class="cal-window-chip ${this._pastDays===0&&this._windowDays===x?"active":""}"
                            @click=${()=>{this._windowDays=x,this._pastDays=0}}>
                            ${x===365?"+1y":`+${x}d`}
                          </button>
                        `)}
                      </div>
                ...
                ${i?o`
                      <select class="cal-user-filter"
                        ...
                        @change=${x=>{this._userFilter=x.target.value}}>
                ...
                ${c?o`
                      <select class="cal-user-filter"
                        .value=${p??""}
                        @change=${x=>{this._objectFilter=x.target.value}}>
                        <option value="">${s("all_objects",e)}</option>
                        ${[...this._objects].sort((x,L)=>x.object.name.localeCompare(L.object.name)).map(x=>o`<option value=${x.entry_id} ?selected=${x.entry_id===p}>${x.object.name}</option>`)}
                      </select>
                    `:h}
              </div>
            `:h}
        <div class="cal-rolling">
          ${w.length===0&&$?o`<div class="cal-empty">${s("cal_no_events",e)}</div>`:w.map(I)}
        </div>
      </ha-card>
    `}};se.styles=[bt,Pi,j`
      :host { display: block; }
      ha-card { padding: 0; overflow: hidden; }
    `],d([b({attribute:!1})],se.prototype,"hass",2),d([_()],se.prototype,"_config",2),d([_()],se.prototype,"_objects",2),d([_()],se.prototype,"_stats",2),d([_()],se.prototype,"_windowDays",2),d([_()],se.prototype,"_pastDays",2),d([_()],se.prototype,"_userFilter",2),d([_()],se.prototype,"_objectFilter",2),d([_()],se.prototype,"_unsub",2);var Js=[{value:7,label:"Week (7 days)"},{value:14,label:"Fortnight (14 days)"},{value:30,label:"Month (30 days, default)"},{value:365,label:"Year (365 days, empty days collapsed)"}],He=class extends A{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"}}setConfig(e){this._config={...e}}_valueChanged(e,t){let i={...this._config,[e]:t};e==="show_window_chips"&&t===!0&&delete i.show_window_chips,e==="show_user_filter"&&t===!0&&delete i.show_user_filter,e==="show_object_filter"&&t===!0&&delete i.show_object_filter,e==="title"&&(!t||typeof t=="string"&&t.trim()==="")&&delete i.title,e==="user_filter"&&t===""&&delete i.user_filter,this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}render(){let e=this._config.window_days??30,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.user_filter??"",l=this._config.title??"";return o`
      <div class="editor">
        <div class="row">
        ...
        <div class="row">
          ...
          >
            ${Js.map(c=>o`<option value="${c.value}" ?selected=${c.value===e}>${c.label}</option>`)}
          </select>
        </div>
  ...
  `],d([b({attribute:!1})],ae.prototype,"hass",2),d([_()],ae.prototype,"_open",2),d([_()],ae.prototype,"_title",2),d([_()],ae.prototype,"_message",2),d([_()],ae.prototype,"_confirmText",2),d([_()],ae.prototype,"_danger",2),d([_()],ae.prototype,"_inputLabel",2),d([_()],ae.prototype,"_inputType",2),d([_()],ae.prototype,"_inputValue",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",ae);var Oi={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},Hi=n=>(...r)=>({_$litDirective$:n,values:r}),jt=class{constructor(r){}get _$AU(){return this._$AM._$AU}_$AT(r,e,t){this._$Ct=r,this._$AM=e,this._$Ci=t}_$AS(r,e){return this.update(r,e)}update(r,e){return this.render(...e)}};var st=class extends jt{constructor(r){if(super(r),this.it=h,r.type!==Oi.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(r){if(r===h||r==null)return this._t=void 0,this.it=r;if(r===fe)return r;if(typeof r!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(r===this.it)return this._t;this.it=r;let e=[r];return e.raw=e,this._t={_$litType$:this.constructor.resultType,strings:e,values:[]}}};st.directiveName="unsafeHTML",st.resultType=1;var Di=Hi(st);var ea=["EUR","USD","GBP","JPY","CHF","CAD","AUD","NZD","CNY","INR","BRL","CZK","PLN","RUB","SEK","NOK","DKK","UAH"],M=class extends A{constructor(){super(...arguments);this.budget=null;this._settings=null;this._loading=!0;this._importCsv="";this._importLoading=!1;this._includeHistory=!0;this._toast="";this._testingNotification=!1;this._personTargets=[];this._testingUser="";this._users=[];this._savedViews=[];this._vacEnabled=!1;this._vacStart="";this._vacEnd="";this._vacBuffer=3;this._vacExempt=new Set;this._vacIsActive=!1;this._vacWindowEnd=null;this._vacAllTasks=[];this._vacPreview=[];this._vacPreviewLoading=!1;this._vacSaving=!1;this._qrObjects=[];this._qrSelectedEntries=new Set;this._qrActions=new Set(["view"]);this._qrUrlMode="companion";this._qrBatchLoading=!1;this._qrBatchResults=[];this._qrObjectsLoaded=!1;this._exportObjects=[];this._exportSelectedEntries=new Set;this._exportObjectsLoaded=!1;this._docArchiveLoading=!1;this._loaded=!1;this._userService=null;this._sendTestNotification=async e=>{e?this._testingUser=e:this._testingNotification=!0;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/test_notification",...e?{user_id:e}:{}}),i=t.message||(t.success?s("test_notification_success",this._lang):s("test_notification_failed",this._lang));this._showToast(i)}catch{this._showToast(s("test_notification_failed",this._lang))}finally{e?this._testingUser="":this._testingNotification=!1}};this._allTemplates=[];this._templateCategories={};this._tplOpenGroups=new Set;this._templatesRequested=!1}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._loaded?(this._loaded=!0,this._userService=new he(this.hass),this._loadSettings(),this._loadUsers()):e.has("hass")&&this.hass&&this._userService&&this._userService.updateHass(this.hass)}async _loadUsers(){if(this._userService){try{this._users=await this._userService.getUsers()}catch{this._users=[]}this._loadNotifyTargets()}}async _loadNotifyTargets(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/notify/user_targets"});this._personTargets=e.targets||[]}catch{this._personTargets=[]}}async _loadSettings(){this._loading=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});this._settings=e,this._hydrateVacationFromSettings()}catch{}try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"});this._savedViews=e.views||[]}catch{}this._loading=!1}_hydrateVacationFromSettings(){let e=this._settings?.vacation;e&&(this._vacEnabled=e.enabled,this._vacStart=e.start||"",this._vacEnd=e.end||"",this._vacBuffer=e.buffer_days,this._vacExempt=new Set(e.exempt_task_ids||[]),this._vacIsActive=e.is_active,this._vacWindowEnd=e.window_end)}async _updateSetting(e,t){try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/update",settings:{[e]:t}});this._settings=i,this._showToast(s("settings_saved",this._lang)),this.dispatchEvent(new CustomEvent("settings-changed"))}catch{this._showToast(s("action_error",this._lang))}}_showToast(e){this._toast=e,setTimeout(()=>{this._toast=""},3e3)}_downloadFile(e,t,i){$t(e,t,i)}render(){let e=this._lang;return this._loading||!this._settings?o`<div class="settings-loading">Loading…</div>`:o`
    ...
    `}scrollToSection(e){requestAnimationFrame(()=>{let t=this.shadowRoot;if(!t)return;let i=t.querySelector(`[data-section="${e}"]`)??t.querySelector(`[data-section-alt="${e}"]`);i&&i.scrollIntoView({behavior:"smooth",block:"start"})})}_renderPanelAccess(e){let t=new Set(this._settings.admin_panel_user_ids||[]),i=this._users.filter(c=>!c.is_admin),a=this._settings.operator_write_enabled??!1,l=(c,p)=>{let u=new Set(t);p?u.add(c):u.delete(c),this._updateSetting("admin_panel_user_ids",[...u])};return o`
      <div class="settings-section">
        ...
        ${a?i.length===0?o`<div class="setting-row hint">${s("no_non_admin_users",e)}</div>`:i.map(c=>o`
              <label class="setting-row">
                ...
                <input type="checkbox"
                  .checked=${t.has(c.id)}
                  @change=${p=>l(c.id,p.target.checked)} />
              </label>
            `):h}
      </div>
    ...
    `}async _loadTemplates(){if(!this._templatesRequested){this._templatesRequested=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/templates",language:this._lang});this._allTemplates=e.templates||[],this._templateCategories=e.categories||{}}catch{}}}_renderTemplateToggles(e){this._loadTemplates();let t=new Set(this._settings.disabled_template_ids||[]),i=new Map;for(let l of Object.keys(this._templateCategories))i.set(l,[]);for(let l of this._allTemplates)i.has(l.category)||i.set(l.category,[]),i.get(l.category).push(l);let a=l=>this._templateCategories[l]?.["name_"+e]||this._templateCategories[l]?.name_en||l;return o`
      <div class="settings-section" data-section="templates">
        ...
        ${[...i.entries()].filter(([,l])=>l.length>0).map(([l,c])=>{let p=c.filter(g=>!t.has(g.id)).length,u=this._tplOpenGroups.has(l);return o`
            <div class="tpl-group">
              <div
              ...
              >
                ...
                <span class="tpl-group-count">${p}/${c.length}</span>
                <input
                  type="checkbox"
    ...
    `}_toggleColumn(e,t){let i=new Set(tt(this._settings.objects_table_columns));t?i.add(e):i.delete(e);let a=ze.filter(l=>l.required||i.has(l.key)).map(l=>l.key);this._updateSetting("objects_table_columns",a)}_renderGeneral(e){let t=this._settings.general,i=t.notify_targets??[],a=this._settings.budget;return o`
      <div class="settings-section">
        ...
        <label class="setting-row">
          ...
          <select @change=${l=>this._updateSetting("budget_currency",l.target.value)}>
            ${ea.map(l=>o`<option value=${l} ?selected=${a.currency===l}>${l}</option>`)}
          </select>
        </label>
        <label class="setting-row">
    ...
    `}_renderVacationTaskList(e){let t=new Map;for(let a of this._vacAllTasks){let l=t.get(a.object_name)||[];l.push(a),t.set(a.object_name,l)}return[...t.entries()].sort(([a],[l])=>a.localeCompare(l)).map(([a,l])=>o`
      <div class="vac-task-group">
        ...
        ${l.sort((c,p)=>c.task_name.localeCompare(p.task_name)).map(c=>o`
            <label class="vac-task-row">
              <input type="checkbox"
                .checked=${this._vacExempt.has(c.task_id)}
                @change=${p=>this._toggleVacationExempt(c.task_id,p.target.checked)} />
                  ...
                  ${this._qrObjects.length===0?o`<div class="hint">${s("no_objects",e)}</div>`:this._qrObjects.map(c=>o`
                      <label class="qr-object-row">
                        <input type="checkbox"
                          ...
                          @change=${p=>this._toggleQrObject(c.entry_id,p.target.checked)} />
                        <span>${c.name}</span>
                  ...
                  ${["view","complete","skip"].map(c=>o`
                    <label class="qr-action-chip ${this._qrActions.has(c)?"active":""}">
                      <input type="checkbox"
                        ...
                        @change=${p=>this._toggleQrAction(c,p.target.checked)} />
                      ${s("qr_action_"+c,e)}
                    </label>
                  `)}
                </div>
...
<body>
  ...
  `,d([b({attribute:!1})],M.prototype,"hass",2),d([b({attribute:!1})],M.prototype,"features",2),d([b({attribute:!1})],M.prototype,"budget",2),d([_()],M.prototype,"_settings",2),d([_()],M.prototype,"_loading",2),d([_()],M.prototype,"_importCsv",2),d([_()],M.prototype,"_importLoading",2),d([_()],M.prototype,"_includeHistory",2),d([_()],M.prototype,"_toast",2),d([_()],M.prototype,"_testingNotification",2),d([_()],M.prototype,"_personTargets",2),d([_()],M.prototype,"_testingUser",2),d([_()],M.prototype,"_users",2),d([_()],M.prototype,"_savedViews",2),d([_()],M.prototype,"_vacEnabled",2),d([_()],M.prototype,"_vacStart",2),d([_()],M.prototype,"_vacEnd",2),d([_()],M.prototype,"_vacBuffer",2),d([_()],M.prototype,"_vacExempt",2),d([_()],M.prototype,"_vacIsActive",2),d([_()],M.prototype,"_vacWindowEnd",2),d([_()],M.prototype,"_vacAllTasks",2),d([_()],M.prototype,"_vacPreview",2),d([_()],M.prototype,"_vacPreviewLoading",2),d([_()],M.prototype,"_vacSaving",2),d([_()],M.prototype,"_qrObjects",2),d([_()],M.prototype,"_qrSelectedEntries",2),d([_()],M.prototype,"_qrActions",2),d([_()],M.prototype,"_qrUrlMode",2),d([_()],M.prototype,"_qrBatchLoading",2),d([_()],M.prototype,"_qrBatchResults",2),d([_()],M.prototype,"_qrObjectsLoaded",2),d([_()],M.prototype,"_exportObjects",2),d([_()],M.prototype,"_exportSelectedEntries",2),d([_()],M.prototype,"_exportObjectsLoaded",2),d([_()],M.prototype,"_docArchiveLoading",2),d([_()],M.prototype,"_allTemplates",2),d([_()],M.prototype,"_templateCategories",2),d([_()],M.prototype,"_tplOpenGroups",2);customElements.define("maintenance-settings-view",M);var re=class extends A{constructor(){super(...arguments);this.objects=[];this._summary=null;this._loaded=!1;this._busy=!1;this._error="";this._query="";this._results=[];this._expanded=!1;this._initiallyLoaded=!1;this._searchTimer=0}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._initiallyLoaded&&(this._initiallyLoaded=!0,this._load(),V(this._lang).then(()=>this.requestUpdate()))}async _load(){this._busy=!0;try{this._summary=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/storage"}),this._error=""}catch(e){this._error=C(e,this._lang)}finally{this._loaded=!0,this._busy=!1}}_nameFor(e){return this.objects.find(i=>i.object?.id===e)?.object?.name||e.slice(0,8)}_entryFor(e){return this.objects.find(t=>t.object?.id===e)?.entry_id}_toggle(){this._expanded=!this._expanded}_openObject(e){this.dispatchEvent(new CustomEvent("open-object",{detail:{entry_id:e},bubbles:!0,composed:!0}))}_onSearch(e){this._query=e.target.value,clearTimeout(this._searchTimer),this._searchTimer=window.setTimeout(()=>{this._doSearch()},250)}async _doSearch(){let e=this._query.trim();if(!e){this._results=[];return}try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/search",query:e});this._results=t.results||[]}catch(t){this._error=C(t,this._lang),this._results=[]}}async _openResult(e){if(e.kind==="weblink"){window.open(e.url,"_blank","noopener");return}let t=window.open("about:blank","_blank");try{let i=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300});t&&(t.location.href=new URL(i.path,window.location.origin).href)}catch(i){t&&t.close(),this._error=C(i,this._lang)}}_renderResult(e,t){return o`
  ...
  `,d([b({attribute:!1})],re.prototype,"hass",2),d([b({attribute:!1})],re.prototype,"objects",2),d([_()],re.prototype,"_summary",2),d([_()],re.prototype,"_loaded",2),d([_()],re.prototype,"_busy",2),d([_()],re.prototype,"_error",2),d([_()],re.prototype,"_query",2),d([_()],re.prototype,"_results",2),d([_()],re.prototype,"_expanded",2);customElements.get("maintenance-storage-section-card")||customElements.define("maintenance-storage-section-card",re);var ta=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"],ce=class extends A{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._entryId="";this._taskId="";this._values=new Array(12).fill("");this._save=async()=>{let e=this._buildOverrides();if(e!==null){this._loading=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/seasonal_overrides",entry_id:this._entryId,task_id:this._taskId,overrides:e}),this._open=!1,this.dispatchEvent(new CustomEvent("overrides-saved"))}catch(t){this._error=C(t,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}};this._clearAll=async()=>{this._loading=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/seasonal_overrides",entry_id:this._entryId,task_id:this._taskId,overrides:{}}),this._values=new Array(12).fill(""),this._open=!1,this.dispatchEvent(new CustomEvent("overrides-saved"))}catch(e){this._error=C(e,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}open(e,t,i){if(this._entryId=e,this._taskId=t,this._values=new Array(12).fill(""),i)for(let[a,l]of Object.entries(i)){let c=parseInt(a,10);c>=1&&c<=12&&typeof l=="number"&&(this._values[c-1]=l.toString())}this._error="",this._open=!0}_close(){this._open=!1}_buildOverrides(){let e={};for(let t=0;t<12;t++){let i=this._values[t].trim();if(!i)continue;let a=parseFloat(i);if(Number.isNaN(a))return this._error=`${s("month_"+["jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec"][t],this._lang)}: ${s("seasonal_override_invalid",this._lang)}`,null;if(a<.1||a>5)return this._error=s("seasonal_override_range",this._lang),null;e[t+1]=a}return e}render(){if(!this._open)return o``;let e=this._lang;return o`
  ...
  `,d([b({attribute:!1})],ne.prototype,"hass",2),d([b({attribute:!1})],ne.prototype,"objects",2),d([_()],ne.prototype,"_open",2),d([_()],ne.prototype,"_loading",2),d([_()],ne.prototype,"_error",2),d([_()],ne.prototype,"_groupId",2),d([_()],ne.prototype,"_name",2),d([_()],ne.prototype,"_description",2),d([_()],ne.prototype,"_selected",2);customElements.get("maintenance-group-dialog")||customElements.define("maintenance-group-dialog",ne);var _e=class extends A{constructor(){super(...arguments);this._open=!1;this._busy=!1;this._error="";this._name="";this._views=[];this._filters=null;this._localeReady=!1;this._save=async()=>{let e=this._name.trim();if(!(!e||this._busy||!this._filters)){this._busy=!0,this._error="";try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/save",name:e,filters:this._filters});this._name="",this._emitChanged(t.views||[])}catch(t){this._error=C(t,this._lang)}finally{this._busy=!1}}};this._delete=async e=>{if(!this._busy){this._busy=!0,this._error="";try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/delete",view_id:e});this._emitChanged(t.views||[])}catch(t){this._error=C(t,this._lang)}finally{this._busy=!1}}}}get _lang(){return this.hass?.language||"en"}updated(e){e.has("hass")&&this.hass&&!this._localeReady&&(this._localeReady=!0,V(this._lang).then(()=>this.requestUpdate()))}async open(e,t){this._open=!0,this._error="",this._name="",this._filters=e,this._views=t}_close(){this._open=!1}_emitChanged(e){this._views=e,this.dispatchEvent(new CustomEvent("saved-views-changed",{bubbles:!0,composed:!0,detail:{views:e}}))}render(){if(!this._open)return o``;let e=this._lang;return o`
      <div class="overlay" @click=${this._close}>
        <div class="card" @click=${t=>t.stopPropagation()}>
  ...
  `,d([b({attribute:!1})],_e.prototype,"hass",2),d([_()],_e.prototype,"_open",2),d([_()],_e.prototype,"_busy",2),d([_()],_e.prototype,"_error",2),d([_()],_e.prototype,"_name",2),d([_()],_e.prototype,"_views",2);customElements.get("maintenance-saved-views-dialog")||customElements.define("maintenance-saved-views-dialog",_e);var ia=60,sa=20,qi=30;function Zt(n){let r=n.trigger_config??null;if(!r)return h;let e=r.type||"threshold",t=n.trigger_entity_info?.unit_of_measurement??"",i=0,a="";if(e==="threshold"){let p=n.trigger_current_value??null;if(p==null)return h;let u=r.trigger_above,g=r.trigger_below;if(u!=null){let m=g??0,v=u-m||1;i=Math.min(100,Math.max(0,(p-m)/v*100)),a=`${p.toFixed(1)} / ${u} ${t}`}else if(g!=null){let v=n.trigger_entity_info?.max??(g*2||100),f=v-g||1;i=Math.min(100,Math.max(0,(v-p)/f*100)),a=`${p.toFixed(1)} / ${g} ${t}`}else return h}else if(e==="counter"){let p=r.trigger_target_value||1,u;if(r.trigger_delta_mode?(u=n.trigger_current_delta??null,u==null&&n.trigger_baseline_value!=null&&n.trigger_current_value!=null&&(u=n.trigger_current_value-n.trigger_baseline_value)):u=n.trigger_current_value??null,u==null)return h;i=Math.min(100,Math.max(0,u/p*100)),a=`${u.toFixed(1)} / ${p} ${t}`}else if(e==="state_change"){let p=r.trigger_target_changes||1,u=n.trigger_current_value??null;if(u==null)return h;i=Math.min(100,Math.max(0,u/p*100)),a=`${Math.round(u)} / ${p}`}else if(e==="runtime"){let p=r.trigger_runtime_hours||100,u=n.trigger_current_value??null;if(u==null)return h;i=Math.min(100,Math.max(0,u/p*100)),a=`${u.toFixed(1)}h / ${p}h`}else if(e==="compound"){let p=r.compound_logic||r.operator||"AND",u=r.conditions?.length||0;a=`${p} (${u})`,i=n.trigger_active?100:0}else return h;let l=i>=100,c=i>90?"var(--error-color, #f44336)":i>70?"var(--warning-color, #ff9800)":"var(--primary-color)";return o`
  ...
  `}function Jt(n,r,e){if(!n.trigger_config?.entity_id)return h;let t=n.trigger_config.entity_id,i=r.get(t)||[],a=[];if(i.length>=2)a=i.map(S=>({ts:S.ts,val:S.val}));else{if(!n.history)return h;for(let S of n.history)S.trigger_value!=null&&a.push({ts:new Date(S.timestamp).getTime(),val:S.trigger_value})}if(n.trigger_current_value!=null&&a.push({ts:Date.now(),val:n.trigger_current_value}),a.length<2)return h;a.sort((S,te)=>S.ts-te.ts);let l=ia,c=sa,p=a.map(S=>S.val),u=Math.min(...p),g=Math.max(...p),m=g-u||1;u-=m*.1,g+=m*.1;let v=a[0].ts,$=a[a.length-1].ts-v||1,w=S=>(S-v)/$*l,R=S=>2+(1-(S-u)/(g-u))*(c-4),I=a;if(I.length>qi){let S=Math.ceil(I.length/qi);I=I.filter((te,Q)=>Q%S===0||Q===I.length-1)}let x=I.map(S=>`${w(S.ts).toFixed(1)},${R(S.val).toFixed(1)}`).join(" "),L=n.trigger_active?"var(--error-color, #f44336)":"var(--primary-color)";return o`
    <svg class="mini-sparkline" viewBox="0 0 ${l} ${c}" preserveAspectRatio="none" role="img" aria-label="${s("chart_mini_sparkline",e)}">
      <polyline points="${x}" fill="none" stroke="${L}" stroke-width="1.5" stroke-linejoin="round" />
    </svg>
  `}function Ni(n,r){let e=r;if(n.days_until_due==null||!n.interval_days||n.interval_days<=0)return h;let{pct:t,overflow:i}=yt(n.interval_days,n.days_until_due,n.interval_unit),a="var(--success-color, #4caf50)";return n.status==="overdue"?a="var(--error-color, #f44336)":n.status==="due_soon"&&(a="var(--warning-color, #ff9800)"),o`
  ...
  `}function at(n,r,e=4){if(!isFinite(n)||!isFinite(r))return{ticks:[],niceMin:0,niceMax:1};if(n===r){let u=Math.abs(n)*.1||1;n-=u,r+=u}let t=r-n,i=Math.pow(10,Math.floor(Math.log10(t/Math.max(1,e)))),a=i;for(let u of[1,2,5,10])if(a=i*u,t/a<=e+.5)break;let l=Math.floor(n/a)*a,c=Math.ceil(r/a)*a,p=[];for(let u=l;u<=c+a*1e-6;u+=a)p.push(Math.abs(u)<a*1e-9?0:u);return{ticks:p,niceMin:l,niceMax:c}}function ge(n){let r=Math.abs(n);return r>=1e6?De((n/1e6).toFixed(r>=1e7?0:1))+"M":r>=1e4?De((n/1e3).toFixed(0))+"k":r>=1e3?De((n/1e3).toFixed(1))+"k":r>=100?n.toFixed(0):r>=10||r>=1?De(n.toFixed(1)):r===0?"0":De(n.toFixed(2))}function De(n){return n.replace(/\.0+$/,"").replace(/(\.\d*[1-9])0+$/,"$1")}function Me(n,r,e){let t=n.toLocaleString(e,{maximumFractionDigits:Math.abs(n)>=100?0:1});return r?`${t} ${r}`:t}function qe(n,r,e){let t=new Date(n),i=e?{month:"short",day:"numeric",year:"2-digit"}:{month:"short",day:"numeric"};return t.toLocaleDateString(r,i)}function Xt(n,r){return new Date(n).toLocaleDateString(r,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function Ct(n,r){return new Date(n).getFullYear()!==new Date(r).getFullYear()}function Rt(n,r,e){if(e<2||r<=n)return[n,r];let t=[];for(let i=0;i<e;i++)t.push(n+(r-n)*i/(e-1));return t}var Mt=210,de=46,xe=14,$e=12,Fi=14,aa=20+Fi,ra=[{days:7,key:"chart_range_7d"},{days:30,key:"chart_range_30d"},{days:90,key:"chart_range_90d"},{days:365,key:"chart_range_1y"}],K=class extends A{constructor(){super(...arguments);this.points=[];this.events=[];this.unit="";this.lang="en";this.thresholdAbove=null;this.thresholdBelow=null;this.targetValue=null;this.forceZero=!1;this.projection=null;this.rangeDays=30;this.showRange=!0;this.busy=!1;this.hideOutliers=!1;this.showOutlierToggle=!0;this._width=0;this._hover=null;this._ro=null}connectedCallback(){super.connectedCallback(),this._ro=new ResizeObserver(e=>{let t=Math.floor(e[0]?.contentRect?.width||0);t&&Math.abs(t-this._width)>2&&(this._width=t)}),this._ro.observe(this)}disconnectedCallback(){super.disconnectedCallback(),this._ro?.disconnect(),this._ro=null}_emitRange(e){e!==this.rangeDays&&this.dispatchEvent(new CustomEvent("range-change",{detail:{days:e},bubbles:!0,composed:!0}))}_toggleOutliers(){this.dispatchEvent(new CustomEvent("outlier-toggle",{detail:{hide:!this.hideOutliers},bubbles:!0,composed:!0}))}render(){let e=this._width||320,t=[...this.points].sort((a,l)=>a.ts-l.ts),i=this.lang;return o`
    ...
    `}_renderSvg(e,t){let i=this.lang,a=e-de-xe,l=Mt-aa,c=l-$e,p=1/0,u=-1/0;for(let E of t)p=Math.min(p,E.min??E.val),u=Math.max(u,E.max??E.val);this.thresholdAbove!=null&&(p=Math.min(p,this.thresholdAbove),u=Math.max(u,this.thresholdAbove)),this.thresholdBelow!=null&&(p=Math.min(p,this.thresholdBelow),u=Math.max(u,this.thresholdBelow)),this.targetValue!=null&&(p=Math.min(p,this.targetValue),u=Math.max(u,this.targetValue)),this.forceZero&&(p=Math.min(p,0));let g=(u-p||1)*.06,m=this.forceZero&&p>=0?0:p-g,{ticks:v,niceMin:f,niceMax:$}=at(m,u+g,4);this.forceZero&&p>=0&&f<0&&(f=0,v=v.filter(E=>E>=0));let w=t[0].ts,R=t[t.length-1].ts,I=R-w||1,x=Ct(w,R),L=E=>de+(E-w)/I*a,S=E=>$e+(1-(E-f)/($-f||1))*c,te=t.map(E=>`${L(E.ts).toFixed(1)},${S(E.val).toFixed(1)}`).join(" "),Q=`M${L(t[0].ts).toFixed(1)},${l} `+t.map(E=>`L${L(E.ts).toFixed(1)},${S(E.val).toFixed(1)}`).join(" ")+` L${L(t[t.length-1].ts).toFixed(1)},${l} Z`,Y="",N=t.filter(E=>E.min!=null&&E.max!=null);if(N.length>=2){let E=N.map(z=>`${L(z.ts).toFixed(1)},${S(z.max).toFixed(1)}`),k=[...N].reverse().map(z=>`${L(z.ts).toFixed(1)},${S(z.min).toFixed(1)}`);Y=`M${E[0]} `+E.slice(1).map(z=>`L${z}`).join(" ")+` L${k.join(" L")} Z`}let q=[];if(this.thresholdBelow!=null){let E=S(this.thresholdBelow);q.push({y:E,h:Math.max(0,l-E),lineY:E,label:`\u25BC ${ge(this.thresholdBelow)}`,labelY:Math.min(l-4,E+13)})}if(this.thresholdAbove!=null){let E=S(this.thresholdAbove);q.push({y:$e,h:Math.max(0,E-$e),lineY:E,label:`\u25B2 ${ge(this.thresholdAbove)}`,labelY:Math.max($e+11,E-5)})}let pe=t[t.length-1],me=(this.events||[]).filter(E=>E.ts>=w&&E.ts<=R),Ie=Rt(w,R,Math.max(2,Math.min(5,Math.floor(a/110)+1))),G=this._hover;return o`
      <div class="svg-holder">
        <svg
          ...
          @pointermove=${E=>this._onPointer(E,t,L,S,e)}
          @pointerdown=${E=>this._onPointer(E,t,L,S,e)}
        ...
        >
          ...
          ${v.map(E=>{let k=S(E);return k<$e-1||k>l+1?h:H`
          ...
          ${this.targetValue!=null?H`<line x1="${de}" y1="${S(this.targetValue).toFixed(1)}" x2="${e-xe}" y2="${S(this.targetValue).toFixed(1)}"
                stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="6,4" />
              <text x="${e-xe-4}" y="${(S(this.targetValue)-5).toFixed(1)}" text-anchor="end" class="zone-label">◆ ${ge(this.targetValue)} ${this.unit}</text>`:h}

          ${this.projection&&this.projection.length===2?H`<line x1="${L(this.projection[0].ts).toFixed(1)}" y1="${S(this.projection[0].val).toFixed(1)}"
                x2="${Math.min(L(this.projection[1].ts),e-xe).toFixed(1)}" y2="${S(Math.max(f,Math.min($,this.projection[1].val))).toFixed(1)}"
                stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.8" />`:h}

          ${Ie.map((E,k)=>{let z=L(E),It=k===0?"start":k===Ie.length-1?"end":"middle";return H`<text x="${z.toFixed(1)}" y="${Mt-5}" text-anchor="${It}" class="tick-label">${qe(E,i,x)}</text>`})}

          <line x1="${de}" y1="${l}" x2="${e-xe}" y2="${l}" stroke="var(--divider-color)" stroke-width="1" />

          ${me.map(E=>{let k=L(E.ts),z=E.type==="completed"?"var(--success-color, #4caf50)":E.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return H`
              <line x1="${k.toFixed(1)}" y1="${$e}" x2="${k.toFixed(1)}" y2="${l}" stroke="${z}" stroke-width="1" opacity="0.14" />
              <rect x="${(k-1.5).toFixed(1)}" y="${l+3}" width="3" height="${Fi-6}" rx="1.5" fill="${z}">
                <title>${Xt(E.ts,i)}</title>
              </rect>`})}
          ...
          ${G?H`
                ...
                <circle cx="${G.x.toFixed(1)}" cy="${G.y.toFixed(1)}" r="4.5" fill="var(--primary-color)"
                  stroke="var(--card-background-color, #fff)" stroke-width="2" />`:H`<circle cx="${L(pe.ts).toFixed(1)}" cy="${S(pe.val).toFixed(1)}" r="4" fill="var(--primary-color)"
        ...
        ${G?o`<div
            ...
            >
              <div class="hover-date">${Xt(G.p.ts,i)}</div>
              <div class="hover-val">
                ${Me(G.p.val,this.unit,i)}
                ${G.p.min!=null&&G.p.max!=null?o`<span class="hover-range">(${ge(G.p.min)}–${ge(G.p.max)})</span>`:h}
              </div>
            </div>`:h}
      </div>
    `}_onPointer(e,t,i,a,l){let p=e.currentTarget.getBoundingClientRect(),u=(e.clientX-p.left)/p.width*l;if(u<de-8||u>l-xe+8){this._hover=null;return}let g=t[0],m=1/0;for(let v of t){let f=Math.abs(i(v.ts)-u);f<m&&(m=f,g=v)}this._hover={x:i(g.ts),y:a(g.val),p:g}}};K.styles=j`
  ...
  `,d([b({attribute:!1})],K.prototype,"points",2),d([b({attribute:!1})],K.prototype,"events",2),d([b()],K.prototype,"unit",2),d([b()],K.prototype,"lang",2),d([b({attribute:!1})],K.prototype,"thresholdAbove",2),d([b({attribute:!1})],K.prototype,"thresholdBelow",2),d([b({attribute:!1})],K.prototype,"targetValue",2),d([b({type:Boolean})],K.prototype,"forceZero",2),d([b({attribute:!1})],K.prototype,"projection",2),d([b({attribute:!1})],K.prototype,"rangeDays",2),d([b({type:Boolean})],K.prototype,"showRange",2),d([b({type:Boolean})],K.prototype,"busy",2),d([b({type:Boolean})],K.prototype,"hideOutliers",2),d([b({type:Boolean})],K.prototype,"showOutlierToggle",2),d([_()],K.prototype,"_width",2),d([_()],K.prototype,"_hover",2);customElements.get("maintenance-trigger-chart")||customElements.define("maintenance-trigger-chart",K);function na(n){if(n.length<4)return n;let r=n.map(u=>u.val).sort((u,g)=>u-g),e=u=>{let g=(r.length-1)*u,m=Math.floor(g),v=Math.ceil(g);return r[m]+(r[v]-r[m])*(g-m)},t=e(.25),i=e(.75),a=i-t;if(a===0)return n;let l=t-1.5*a,c=i+1.5*a,p=n.filter(u=>u.val>=l&&u.val<=c);return p.length>=2?p:n}function Ui(n,r){let e=n.trigger_config;if(!e)return h;let t=r.lang,i=n.trigger_entity_info,a=n.trigger_entity_infos,l=i?.friendly_name||e.entity_id||"\u2014",c=e.entity_id||"",p=e.entity_ids||(c?[c]:[]),u=i?.unit_of_measurement||"",g=n.trigger_current_value,m=e.type||"threshold",v=p.length>1,f=oa(n,u,r);return o`
    <h3>${s("trigger",t)}</h3>
    <div class="trigger-card">
      <div class="trigger-header">
        ...
        <div>
          ${v?o`
            ...
            <div class="trigger-entity-id">${p.map(($,w)=>o`${w>0?", ":""}<span class="entity-link" @click=${R=>Re(R,$)}>${$}</span>`)}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `:o`
            <div class="trigger-entity-name">${l}</div>
            <div class="trigger-entity-id">${c?o`<span class="entity-link" @click=${$=>Re($,c)}>${c}</span>`:""}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `}
        </div>
      ...
      ${f?la(f,t):g!=null?o`
              <div class="trigger-value-row">
                <span class="trigger-current ${n.trigger_active?"active":""}">${typeof g=="number"?Me(g,"",t):g}</span>
      ...
      <div class="trigger-limits">
        ...
        ${m==="compound"?o`
          ...
          ${(e.conditions||[]).map(($,w)=>o`
            <span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${w+1}. ${s($.type||"unknown",t)}: ${$.entity_id?o`<span class="entity-link" @click=${R=>Re(R,$.entity_id)}>${$.entity_id}</span>`:""}</span>
          `)}
        `:h}
      </div>
      ...
      ${a&&a.length>1?o`
        <div class="trigger-entity-list">
          ${a.map($=>o`
            <span class="trigger-entity-id">${$.friendly_name} (<span class="entity-link" @click=${w=>Re(w,$.entity_id)}>${$.entity_id}</span>)</span>
          `)}
        </div>
      `:h}
  ...
  `}function oa(n,r,e){let t=n.trigger_config,i=n.trigger_current_value;if(!t||i==null)return null;switch(t.type||"threshold"){case"counter":{let a=t.trigger_target_value;if(a==null||a<=0)return null;let l=Bi(n,Vi(n,e));return{progress:Math.max(0,i-(l?.value??i)),target:a,unit:r,meter:i}}case"state_change":{let a=t.trigger_target_changes;return a==null||a<=0?null:{progress:Math.max(0,i),target:a,unit:"",meter:null}}case"runtime":{let a=t.trigger_runtime_hours;return a==null||a<=0?null:{progress:Math.max(0,i),target:a,unit:"h",meter:null}}}return null}function Bi(n,r){if(n.trigger_baseline_value!=null)return{value:n.trigger_baseline_value,ts:ei(n)};if(!r.length)return null;let e=ei(n);if(e==null)return{value:r[0].val,ts:null};let t=r[0],i=Math.abs(r[0].ts-e);for(let a of r){let l=Math.abs(a.ts-e);l<i&&(t=a,i=l)}return{value:t.val,ts:e}}function ei(n){let r=[...n.history].filter(e=>e.type==="completed"||e.type==="reset").sort((e,t)=>new Date(t.timestamp).getTime()-new Date(e.timestamp).getTime())[0];return r?new Date(r.timestamp).getTime():null}function la(n,r){let e=Math.min(999,Math.round(n.progress/n.target*100)),t=e>=100?"over":e>=75?"near":"ok";return o`
    <div class="counter-progress">
      <div class="counter-progress-nums">
        <span class="counter-progress-main">${Me(n.progress,"",r)}<span class="counter-progress-target"> / ${Me(n.target,n.unit,r)}</span></span>
      ...
      <div class="counter-progress-caption">
        ${s("chart_since_service",r)}${n.meter!=null?o` · ${s("current",r)}: ${Me(n.meter,n.unit,r)}`:h}
      </div>
    </div>
  `}function Vi(n,r){let e=n.trigger_config;if(!e)return[];let t=e.type||"threshold",i=e.entity_id||"",a=t==="runtime"?[]:r.detailStatsData.get(i)||[],l=r.isCounterEntity(e),c=[];if(a.length>=2)for(let p of a){let u={ts:p.ts,val:p.val};!l&&p.min!=null&&p.max!=null&&(u.min=p.min,u.max=p.max),c.push(u)}else for(let p of n.history)p.trigger_value!=null&&c.push({ts:new Date(p.timestamp).getTime(),val:p.trigger_value});return n.trigger_current_value!=null&&c.push({ts:Date.now(),val:n.trigger_current_value}),c.sort((p,u)=>p.ts-u.ts),c}function ca(n,r,e){let t=n.trigger_config;if(!t)return h;let i=t.type||"threshold",a=t.entity_id||"",l=Vi(n,e);i==="runtime"&&t.trigger_runtime_hours&&n.trigger_current_value!=null&&(l=[{ts:ei(n)??l[0]?.ts??Date.now()-864e5,val:0},{ts:Date.now(),val:Math.max(0,n.trigger_current_value)}]),e.hideOutliers&&(l=na(l));let c=l.length<2&&!!a&&e.hasStatsService&&!e.detailStatsData.has(a);if(l.length<2&&!c)return h;let p=!!a&&e.detailStatsData.has(a)&&(e.detailStatsData.get(a)?.length??0)<2,u=Date.now()-e.rangeDays*864e5,g=l.filter(w=>w.ts>=u);g.length>=2&&(l=g);let m=null,v=!1;if(i==="counter"&&t.trigger_target_value!=null&&l.length){let w=Bi(n,l);if(w){if(w.ts!=null){let R=l.filter(I=>I.ts>=w.ts);R.length>=2&&(l=R)}l=l.map(R=>({...R,val:Math.max(0,R.val-w.value)}))}m=t.trigger_target_value,v=!0}else i==="state_change"&&t.trigger_target_changes?(m=t.trigger_target_changes,v=!0):i==="runtime"&&t.trigger_runtime_hours&&(m=t.trigger_runtime_hours,v=!0);let f=null;if(m==null&&n.degradation_rate!=null&&(n.degradation_trend!=="stable"||n.days_until_threshold!=null)&&n.degradation_trend!=="insufficient_data"&&l.length>=2){let w=l[l.length-1];f=[w,{ts:w.ts+30*864e5,val:w.val+n.degradation_rate*30}]}let $=n.history.filter(w=>["completed","skipped","reset"].includes(w.type)).map(w=>({ts:new Date(w.timestamp).getTime(),type:w.type}));return o`
    <maintenance-trigger-chart
      ...
      .busy=${c}
      @range-change=${w=>e.setRangeDays(w.detail.days)}
      @outlier-toggle=${w=>e.setHideOutliers(w.detail.hide)}
    ></maintenance-trigger-chart>
    ${p&&!c?o`<div class="chart-note">
          <ha-icon icon="mdi:information-outline"></ha-icon>
          ${s("chart_no_stats",e.lang)}
        </div>`:h}
  `}function Wi(n,r,e){let t=n.degradation_trend!=null&&n.degradation_trend!=="insufficient_data",i=n.days_until_threshold!=null,a=n.environmental_factor!=null&&n.environmental_factor!==1;if(!t&&!i&&!a)return h;let l=n.degradation_trend==="rising"?"M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z":n.degradation_trend==="falling"?"M16,18L18.29,15.71L13.41,10.83L9.41,14.83L2,7.41L3.41,6L9.41,12L13.41,8L19.71,14.29L22,12V18H16Z":"M22,12L18,8V11H3V13H18V16L22,12Z";return o`
      ...
      <div class="prediction-grid">
        ...
        ${a&&e.environmental?o`
          <div class="prediction-item">
            ...
            ${n.environmental_entity?o`<span class="prediction-entity entity-link" @click=${c=>Re(c,n.environmental_entity)}>${n.environmental_entity}</span>`:h}
          </div>
        `:h}
      </div>
    </div>
  `}function Ki(n,r){let e=n.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return h;let a=n.interval_days??0,l=n.suggested_interval??a;return o`
    ...
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_weibull",i)}">
        ...
        ${e>0?H`
          ...
          <circle cx="${x.toFixed(1)}" cy="${S.toFixed(1)}" r="3"
            fill="var(--primary-color, #03a9f4)" />
          <text x="${(x+4).toFixed(1)}" y="${(S-6).toFixed(1)}" fill="var(--primary-color, #03a9f4)"
  ...
  `}function ua(n,r,e){let t=n.confidence_interval_low,i=n.confidence_interval_high,a=r.suggested_interval??r.interval_days??0,l=r.interval_days??0,c=Math.max(0,t-5),u=i+5-c,g=(t-c)/u*100,m=(i-t)/u*100,v=(a-c)/u*100,f=l>0?(l-c)/u*100:-1;return o`
  ...
  `}function Yi(n,r,e,t){let i=Math.max(n||1,r);return o`
  ...
  `}var Gi=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"];function Qi(n,r,e){if(!e.seasonal||!n.seasonal_factor||n.seasonal_factor===1)return h;let t=Gi.map(c=>s(c,r)),i=new Date().getMonth(),a=n.seasonal_factors||n.interval_analysis?.seasonal_factors||null,l=a&&a.length===12?a:t.map((c,p)=>{let u=n.seasonal_factor||1,g=Math.sin((p-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,u+g))});return o`
    <div class="seasonal-card-compact">
      <h4>${s("seasonal_awareness",r)}</h4>
      <div class="seasonal-mini-chart">
        ${l.map((c,p)=>{let u=c*40,g=c<.9?"low":c>1.1?"high":"normal";return o`
            <div class="seasonal-bar ${g} ${p===i?"current":""}"
                 style="height: ${u}px"
  ...
  `}function Zi(n,r){return _a(n,r)}function _a(n,r){let e=n.seasonal_factors??n.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return h;let t=n.interval_analysis?.seasonal_reason,i=new Date().getMonth(),a=300,l=100,c=8,u=l-c-4,g=Math.max(...e,1.5),m=a/12,v=m*.65,f=c+u-1/g*u;return o`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${a} ${l}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",r)}">
        ...
        ${e.map(($,w)=>{let R=$/g*u,I=w*m+(m-v)/2,x=c+u-R,L=w===i,S=$<1?"var(--success-color, #4caf50)":$>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return H`
            <rect x="${I.toFixed(1)}" y="${x.toFixed(1)}"
              width="${v.toFixed(1)}" height="${R.toFixed(1)}"
              fill="${S}" opacity="${L?1:.5}" rx="2" />
          `})}
      </svg>
      <div class="seasonal-labels">
        ${Gi.map(($,w)=>o`<span class="seasonal-label ${w===i?"active-month":""}">${s($,r)}</span>`)}
      </div>
    </div>
  `}var ga=200,rt=10,ma=22;function Ji(n,r,e,t){let i=n.history.filter(c=>c.type==="completed"&&(c.cost!=null||c.duration!=null));if(i.length<2)return h;let a=i.some(c=>(c.cost??0)>0),l=i.some(c=>(c.duration??0)>0);return!a&&!l?h:o`
  ...
  `}function va(n,r,e){let t=n.history.filter(k=>k.type==="completed"&&(k.cost!=null||k.duration!=null)).map(k=>({ts:new Date(k.timestamp).getTime(),cost:k.cost??0,duration:k.duration??0})).sort((k,z)=>k.ts-z.ts);if(t.length<2)return h;let i=t.some(k=>k.cost>0),a=t.some(k=>k.duration>0);if(!i&&!a)return h;let l=e!=="duration"&&i,c=e!=="cost"&&a,p=l||!c&&i,u=c||!l&&a,g=640,m=ga,v=p?44:12,f=u?44:12,$=g-v-f,w=m-ma,R=w-rt,I=t[0].ts,x=t[t.length-1].ts,L=(x-I||864e5)*.05,S=I-L,te=x+L,Q=Ct(I,x),Y=k=>v+(k-S)/(te-S)*$,N=at(0,Math.max(...t.map(k=>k.cost))||1,3),q=at(0,Math.max(...t.map(k=>k.duration))||1,3),pe=k=>rt+(1-k/(N.niceMax||1))*R,me=k=>rt+(1-k/(q.niceMax||1))*R,Ie=t.length>1?Math.min(...t.slice(1).map((k,z)=>Y(k.ts)-Y(t[z].ts))):$,G=Math.max(6,Math.min(22,Ie*.55)),E=Rt(I,x,Math.max(2,Math.min(4,t.length)));return o`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${g} ${m}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_history",r)}">
        ${p?N.ticks.map(k=>{let z=pe(k);return z<rt-1||z>w+1?h:H`
        ...
        ${u?q.ticks.map(k=>{let z=me(k);return z<rt-1||z>w+1?h:H`<text x="${g-f+6}" y="${(z+3.5).toFixed(1)}" text-anchor="start" fill="var(--accent-color, #ff9800)" font-size="10.5">${ge(k)}m</text>`}):h}

        ${p?t.filter(k=>k.cost>0).map(k=>H`
          <rect x="${(Y(k.ts)-G/2).toFixed(1)}" y="${pe(k.cost).toFixed(1)}" width="${G.toFixed(1)}" height="${(w-pe(k.cost)).toFixed(1)}"
            fill="var(--primary-color)" opacity="0.6" rx="2">
            <title>${qe(k.ts,r,!0)}: ${k.cost.toLocaleString(r)}€${k.duration?` \xB7 ${k.duration}m`:""}</title>
          </rect>
        `):h}
        ${u?H`
          ...
          ${t.map(k=>H`
            <circle cx="${Y(k.ts).toFixed(1)}" cy="${me(k.duration).toFixed(1)}" r="3.5" fill="var(--accent-color, #ff9800)">
              <title>${qe(k.ts,r,!0)}: ${k.duration}m${k.cost?` \xB7 ${k.cost.toLocaleString(r)}\u20AC`:""}</title>
            </circle>
          `)}
        `:h}

        <line x1="${v}" y1="${w}" x2="${g-f}" y2="${w}" stroke="var(--divider-color)" stroke-width="1" />
        ${E.map((k,z)=>{let It=z===0?"start":z===E.length-1?"end":"middle";return H`<text x="${Y(k).toFixed(1)}" y="${m-6}" text-anchor="${It}" fill="var(--secondary-text-color)" font-size="10">${qe(k,r,Q)}</text>`})}
      </svg>
    </div>
    <div class="chart-legend">
      ${p?o`<span class="legend-item"><span class="legend-swatch" style="background:var(--primary-color);opacity:0.6"></span>${s("cost",r)}</span>`:h}
      ${u?o`<span class="legend-item"><span class="legend-swatch" style="background:var(--accent-color, #ff9800)"></span>${s("duration",r)}</span>`:h}
    </div>
  `}var Ee=class extends A{constructor(){super(...arguments);this.docId="";this._url="";this._failed=!1;this._signedFor=""}updated(){this.hass&&this.docId&&this._signedFor!==this.docId&&(this._signedFor=this.docId,this._url="",this._failed=!1,this._sign())}async _sign(){try{let e=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${this.docId}`,expires:300});this._url=e.path}catch{this._failed=!0}}render(){return this._failed||!this.docId?h:this._url?o`
      <a href=${this._url} target="_blank" rel="noopener" class="wrap">
        <img src=${this._url} alt="" loading="lazy"
          @error=${()=>this._failed=!0} />
      </a>`:o`<div class="ph"></div>`}};Ee.styles=j`
  ...
  `,d([b({attribute:!1})],Ee.prototype,"hass",2),d([b()],Ee.prototype,"docId",2),d([_()],Ee.prototype,"_url",2),d([_()],Ee.prototype,"_failed",2);customElements.get("maintenance-history-photo")||customElements.define("maintenance-history-photo",Ee);var fa=["completed","skipped","missed","reset","triggered"];function Xi(n,r){let e=r.lang;return o`
    <div class="history-filters-new">
      <div class="filter-chips">
        ${fa.map(t=>{let i=n.history.filter(a=>a.type===t).length;return i===0?h:o`
  ...
  `}function es(n,r){let e=r.lang,t=r.filter?n.history.filter(i=>i.type===r.filter):n.history;if(r.search){let i=r.search.toLowerCase();t=t.filter(a=>a.notes?.toLowerCase().includes(i))}return t.length===0?o`<p class="empty">${s("no_history",e)}</p>`:o`
    <div class="history-timeline">
      ${[...t].reverse().map(i=>ba(i,r))}
    </div>
  `}function ba(n,r){let e=r.lang,t=["completed","reset","skipped"].includes(n.type);return o`
    <div class="history-entry">
      <div class="history-icon ${n.type}">
        <ha-icon .icon=${Qe[n.type]||"mdi:circle"}></ha-icon>
      </div>
      <div class="history-content">
        ...
        <div class="history-date">${vt(n.timestamp,e)}</div>
  ...
  `}function ti(n,r){if(!n.responsible_user_id)return h;let e=r(n.responsible_user_id);return e?o`
    <span class="user-badge">
      <ha-icon icon="mdi:account"></ha-icon>
      ${e}
    </span>
  `:h}function ya(n,r){let e=r.lang,t=r.isOperator,i=n.archived?"archived":n.is_done?"done":n.status==="due_soon"?"warning":n.status||"ok",a=n.archived?s("archived",e):n.is_done?s("completed",e):s(n.status||"ok",e);return o`
  ...
  `}function xa(n){let r=n.lang;return o`
    <div class="tab-bar">
  ...
  `}function ts(n,r,e,t){let i=t.collapsedSections.has(n);return o`
  ...
  `}function $a(n,r){if(!r.features.checklists)return h;let e=n.checklist||[];if(e.length===0)return h;let t=r.lang;return o`
  ...
  `}function wa(n,r){let e=le(n.documentation_url)?n.documentation_url:null,t=le(r.objectDocUrl)?r.objectDocUrl:null;if(!n.notes&&!e&&!t)return h;let i=r.lang;return o`
  ...
  `}function ka(n,r){let e=r.lang,t=n.times_performed>0?n.total_cost/n.times_performed:0,i=n.days_until_due!==null&&n.days_until_due!==void 0?n.days_until_due<0?"overdue":n.days_until_due<=n.warning_days?"warning":"":"";return o`
    <div class="kpi-bar">
      <div class="kpi-card">
      ...
      <div class="kpi-card">
        ...
        <div class="kpi-value">${et(n,e)}</div>
  ...
  `}function Ea(n,r){let e=r.lang;if(!r.features.adaptive||!n.suggested_interval||n.suggested_interval===n.interval_days)return h;if(r.suggestionDismissed)return h;let t=n.suggested_interval;return o`
    <div class="recommendation-card">
      <h4>${s("suggested_interval",e)}</h4>
      ${Yi(n.interval_days,t,n.interval_confidence||"medium",e)}
  ...
  `}function Ta(n,r){let e=r.lang,t=n.history.slice(-3).reverse();if(t.length===0)return h;let i=a=>{switch(a){case"completed":return"\u2713";case"triggered":return"\u2297";case"skipped":return"\u21B7";case"reset":return"\u21BA";default:return"\xB7"}};return o`
    <div class="recent-activities">
      <h3>${s("recent_activities",e)}</h3>
      ${t.map(a=>o`
        <div class="activity-item">
          <span class="activity-icon">${i(a.type)}</span>
          <span class="activity-date">${vt(a.timestamp,e)}</span>
  ...
  `}function Sa(n,r){let e=r.lang,t=r.features.adaptive&&n.suggested_interval&&n.suggested_interval!==n.interval_days,i=r.features.seasonal&&n.seasonal_factor&&n.seasonal_factor!==1,a=t||i,l=r.features.adaptive&&n.interval_analysis?.weibull_beta!=null&&n.interval_analysis?.weibull_eta!=null,c=r.features.seasonal&&(n.seasonal_factors?.length===12||n.interval_analysis?.seasonal_factors?.length===12);return o`
    <div class="tab-content overview-tab">
      ...
      <div class="two-column-layout ${a?"":"single-column"}">
        ...
        <div class="right-column">
          ${Ji(n,e,r.costDurationToggle,p=>r.setCostDurationToggle(p))}
        </div>
      </div>
      ${l?ts("weibull","weibull_reliability_curve",Ki(n,e),r):h}
      ${c?ts("seasonal","seasonal_chart_title",o`
            ${Zi(n,e)}
  ...
  `}function Aa(n,r){return o`
    <div class="tab-content history-tab">
      ${Xi(n,r.history)}
      ${es(n,r.history)}
    </div>
  `}function ja(n,r){switch(r.activeTab){case"overview":return Sa(n,r);case"history":return Aa(n,r);default:return h}}function is(n,r){return o`
  ...
  `}var nt=class extends A{createRenderRoot(){return this}render(){return!this.task||!this.ctx?h:o`${is(this.task,this.ctx)}`}};d([b({attribute:!1})],nt.prototype,"task",2),d([b({attribute:!1})],nt.prototype,"ctx",2);customElements.get("maintenance-task-detail-view")||customElements.define("maintenance-task-detail-view",nt);function ss(n){if(n.total<=0)return{start:0,end:0,padTop:0,padBottom:0};let r=n.overscan??12,e=Math.max(1,n.step??6),t=Math.max(1,n.rowHeight),i=Math.floor((n.scrollTop-n.listTop)/t),a=Math.ceil(n.viewportHeight/t)+1,l=Math.max(0,i-r);l=Math.floor(l/e)*e;let c=Math.min(n.total,Math.max(i,0)+a+r);return c=Math.min(n.total,Math.ceil(c/e)*e),l>=c&&(l=Math.min(l,Math.max(0,n.total-1)),c=Math.min(n.total,l+Math.max(a,1))),{start:l,end:c,padTop:l*t,padBottom:(n.total-c)*t}}var T=class extends A{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._filterLabel=null;this._savedViews=[];this._activeViewId="";this._unsub=null;this._chartRangeDays=(()=>{try{let e=parseInt(localStorage.getItem(W.chartRange)||"",10);return[7,30,90,365].includes(e)?e:30}catch{return 30}})();this._hideOutliers=(()=>{try{return localStorage.getItem(W.chartHideOutliers)==="1"}catch{return!1}})();this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._adminPanelUserIds=[];this._operatorWriteEnabled=!1;this._defaultWarningDays=7;this._actionLoading=!1;this._moreMenuOpen=!1;this._objMenuOpen=!1;this._toastMessage="";this._toastUndo=null;this._toastActionLabel="";this._filtersOpen=!1;this._actionsMenuOpen=!1;this._batteryFleetSetupAvailable=!1;this._staleBundle=!1;this._staleChecked=!1;this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab=(()=>{try{let e=localStorage.getItem(W.overviewTab);return e==="today"||e==="calendar"?e:"dashboard"}catch{return"dashboard"}})();this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._objectSortMode="alphabetical";this._groupByMode="none";this._objectViewMode="cards";this._objectsTableColumns=xt;this._showArchived=!1;this._bulkMode=!1;this._bulkSelected=new Set;this._virtStart=0;this._virtEnd=0;this._virtRowHeight=53;this._virtTotalRows=0;this._virtScrollAttached=!1;this._virtRaf=0;this._collapsedSections=(()=>{try{return new Set(JSON.parse(localStorage.getItem(W.collapsedSections)||"[]"))}catch{return new Set}})();this._paletteOpen=!1;this._paletteQuery="";this._paletteActive=0;this._templateGalleryOpen=!1;this._templates=[];this._templateCategories={};this._templateBusy=!1;this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._onVirtualScroll=()=>{this._virtRaf||(this._virtRaf=requestAnimationFrame(()=>{this._virtRaf=0,this._updateVirtualWindow()}))};this._deepLinkHandled=!1;this._paletteKeydown=e=>{if(e.key==="/"&&!e.ctrlKey&&!e.metaKey&&!e.altKey&&!this._paletteOpen){let i=e.composedPath()[0];if(i instanceof HTMLElement&&(i.tagName==="INPUT"||i.tagName==="TEXTAREA"||i.tagName==="SELECT"||i.isContentEditable))return;e.preventDefault(),this._openPalette();return}if(!this._paletteOpen)return;let t=this._paletteResults;if(e.key==="Escape")e.preventDefault(),this._closePalette();else if(e.key==="ArrowDown")e.preventDefault(),this._paletteActive=Math.min(this._paletteActive+1,t.length-1);else if(e.key==="ArrowUp")e.preventDefault(),this._paletteActive=Math.max(this._paletteActive-1,0);else if(e.key==="Enter"){e.preventDefault();let i=t[this._paletteActive];i&&this._selectPaletteResult(i)}};this._onDialogEvent=async()=>{try{await this._loadData()}catch{}};this._onCalendarLlCustom=e=>{let t=e.detail;t?.type==="maintenance-supporter:open-task"&&t.entry_id&&t.task_id&&(e.stopPropagation(),this._showTask(t.entry_id,t.task_id))};this._onHistoryEntrySaved=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"en"}get _isOperator(){let e=this.hass?.user;return e?e.is_admin?!1:!(this._operatorWriteEnabled&&this._adminPanelUserIds.includes(e.id)):!0}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler),window.addEventListener("keydown",this._paletteKeydown),window.addEventListener("resize",this._onVirtualScroll,{passive:!0});try{let e=localStorage.getItem(W.taskSort);e&&["due_date","object","type","task_name","area","assigned_user","group"].includes(e)&&(this._sortMode=e);let t=localStorage.getItem(W.objectSort);t&&["alphabetical","due_soonest","task_count"].includes(t)&&(this._objectSortMode=t);let i=localStorage.getItem(W.groupBy);i&&["none","area","group","user"].includes(i)&&(this._groupByMode=i);let a=localStorage.getItem(W.objectView);(a==="cards"||a==="table")&&(this._objectViewMode=a)}catch{}}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),window.removeEventListener("keydown",this._paletteKeydown),window.removeEventListener("resize",this._onVirtualScroll),this.shadowRoot?.querySelector(".content")?.removeEventListener("scroll",this._onVirtualScroll),this._virtScrollAttached=!1,this._virtRaf&&cancelAnimationFrame(this._virtRaf),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){super.updated(e),e.has("hass")&&mt(this.hass?.locale);let t=this.hass?.language;if(t&&!gt(t)&&V(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new kt(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new he(this.hass),this._userService.getUsers())}let i=this.shadowRoot?.querySelector(".content");i&&!this._virtScrollAttached&&(i.addEventListener("scroll",this._onVirtualScroll,{passive:!0}),this._virtScrollAttached=!0),this._updateVirtualWindow()}_updateVirtualWindow(){let e=this.shadowRoot?.querySelector(".content"),t=this.shadowRoot?.querySelector(".task-table.virtual");if(!e||!t)return;let i=t.querySelector(".task-row:not(.virt-sizer)");i&&i.offsetHeight>20&&(this._virtRowHeight=i.offsetHeight);let a=t.getBoundingClientRect().top-e.getBoundingClientRect().top+e.scrollTop,l=ss({scrollTop:e.scrollTop,viewportHeight:e.clientHeight,listTop:a,rowHeight:this._virtRowHeight,total:this._virtTotalRows});(l.start!==this._virtStart||l.end!==this._virtEnd)&&(this._virtStart=l.start,this._virtEnd=l.end)}async _loadData(){let[e,t,i,a,l,c]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"}).catch(()=>null)]);if(c&&(this._savedViews=c.views||[]),e&&(this._objects=e.objects),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/overview"}).then(p=>{this._batteryFleetSetupAvailable=!!p.available&&!p.configured}).catch(()=>{this._batteryFleetSetupAvailable=!1}),this._staleChecked||(this._staleChecked=!0,this.hass.connection.sendMessagePromise({type:"maintenance_supporter/version"}).then(p=>{this._staleBundle=bi(p?.version)}).catch(()=>{})),t&&(this._stats=t),i&&(this._budget=i),a&&(this._groups=a.groups||{}),l){let p=l;this._features=p.features,this._adminPanelUserIds=p.admin_panel_user_ids||[],this._operatorWriteEnabled=p.operator_write_enabled??!1;let u=p.general?.default_warning_days;typeof u=="number"&&u>=0&&u<=365&&(this._defaultWarningDays=u),this._objectsTableColumns=tt(p.objects_table_columns)}this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("ms_action"),i=()=>{let g=window.location.pathname+window.location.hash;history.replaceState(history.state,"",g)};if(t==="add_object"){this._deepLinkHandled=!0,i(),requestAnimationFrame(()=>{this.shadowRoot?.querySelector("maintenance-object-dialog")?.openCreate()});return}if(t==="open_vacation"||t==="open_budget"||t==="open_groups"||t==="open_settings"){this._deepLinkHandled=!0,i(),this._overviewTab="settings",requestAnimationFrame(()=>{let g=this.shadowRoot?.querySelector("maintenance-settings-view"),m=t.replace("open_","");g?.scrollToSection?.(m)});return}let a=e.get("entry_id");if(!a)return;this._deepLinkHandled=!0;let l=e.get("task_id"),c=e.get("action"),p=window.location.pathname+window.location.hash;history.replaceState(history.state,"",p);let u=this._getObject(a);if(!u){this._showOverview();return}if(l){let g=u.tasks.find(m=>m.id===l);if(!g){this._showObject(a);return}this._showTask(a,l),c==="complete"?requestAnimationFrame(()=>{this._openCompleteDialog(a,l,g.name,this._features.checklists?g.checklist:void 0,this._features.adaptive&&!!g.adaptive_config?.enabled)}):c==="quick_complete"&&requestAnimationFrame(()=>{this._handleQuickComplete(a,l,g)})}else this._showObject(a)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let i=await this._statsService.getDetailStats(e,t,this._chartRangeDays),a=new Map(this._detailStatsData);a.set(e,i),this._detailStatsData=a}_setChartRange(e){if(e===this._chartRangeDays)return;this._chartRangeDays=e;try{localStorage.setItem(W.chartRange,String(e))}catch{}let t=this._selectedEntryId&&this._selectedTaskId?this._getTask(this._selectedEntryId,this._selectedTaskId):null,i=t?.trigger_config?.entity_id;if(i){let a=new Map(this._detailStatsData);a.delete(i),this._detailStatsData=a,this._fetchDetailStats(i,this._isCounterEntity(t.trigger_config))}}_setHideOutliers(e){if(e!==this._hideOutliers){this._hideOutliers=e;try{localStorage.setItem(W.chartHideOutliers,e?"1":"0")}catch{}}}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let i of this._objects)for(let a of i.tasks){let l=a.trigger_config?.entity_id;l&&e.push({entityId:l,isCounter:this._isCounterEntity(a.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{let e=await this.hass.connection.subscribeMessage(t=>{let i=t;this._objects=i.objects},{type:"maintenance_supporter/subscribe"});if(!this.isConnected){e();return}this._unsub=e}catch{}}get _taskRows(){let e=[];for(let m of this._objects)for(let v of m.tasks){if(!this._showArchived&&v.archived||this._filterStatus&&v.status!==this._filterStatus)continue;if(this._filterUser){let $=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(v.responsible_user_id!==$)continue}if(this._filterLabel&&!(v.labels||[]).includes(this._filterLabel))continue;let f=[];for(let $ of Object.values(this._groups))$.task_refs?.some(w=>w.entry_id===m.entry_id&&w.task_id===v.id)&&f.push($.name);e.push({entry_id:m.entry_id,task_id:v.id,object_name:m.object.name,task_name:v.name,type:v.type,schedule_type:v.schedule_type,status:v.status,days_until_due:v.days_until_due??null,next_due:v.next_due??null,trigger_active:v.trigger_active,trigger_current_value:v.trigger_current_value??null,trigger_current_delta:v.trigger_current_delta??null,trigger_config:v.trigger_config??null,trigger_entity_info:v.trigger_entity_info??null,times_performed:v.times_performed,total_cost:v.total_cost,interval_days:v.interval_days??null,interval_unit:v.interval_unit??null,interval_anchor:v.interval_anchor??null,is_done:v.is_done??!1,archived:v.archived??!1,history:v.history||[],enabled:v.enabled,nfc_tag_id:v.nfc_tag_id??null,priority:v.priority??"normal",labels:v.labels??[],area_id:m.object.area_id??null,responsible_user_id:v.responsible_user_id??null,group_names:f})}let t={overdue:0,triggered:1,due_soon:2,ok:3},i=(m,v)=>(t[m.status]??9)-(t[v.status]??9),a=(m,v)=>(m.days_until_due??99999)-(v.days_until_due??99999),l=(m,v)=>i(m,v)||a(m,v),c=m=>m.area_id&&this.hass?.areas?.[m.area_id]?.name||"",p=m=>m.responsible_user_id&&this._userService?.getUserName(m.responsible_user_id)||"",u=m=>m.group_names[0]||"",g={due_date:l,object:(m,v)=>m.object_name.localeCompare(v.object_name)||l(m,v),type:(m,v)=>m.type.localeCompare(v.type)||l(m,v),task_name:(m,v)=>m.task_name.localeCompare(v.task_name),area:(m,v)=>{let f=c(m),$=c(v);return!f&&$?1:f&&!$?-1:f.localeCompare($)||l(m,v)},assigned_user:(m,v)=>{let f=p(m),$=p(v);return!f&&$?1:f&&!$?-1:f.localeCompare($)||l(m,v)},group:(m,v)=>{let f=u(m),$=u(v);return!f&&$?1:f&&!$?-1:f.localeCompare($)||l(m,v)}};return e.sort(g[this._sortMode]),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(a=>a.id===t)}_pushPanelState(e,t,i){let a={msp_view:e,msp_entry:t||null,msp_task:i||null};history.pushState(a,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let i=this._getTask(t.msp_entry,t.msp_task);i?.trigger_config?.entity_id&&this._fetchDetailStats(i.trigger_config.entity_id,this._isCounterEntity(i.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1,this._scrollContentToTop()}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null,this._scrollContentToTop()}_filterByStatus(e){this._filterStatus=e,this._activeViewId="",this._overviewTab!=="dashboard"&&(this._overviewTab="dashboard"),this._scrollContentToTop()}get _allLabels(){let e=new Set;for(let t of this._objects)for(let i of t.tasks)for(let a of i.labels||[])e.add(a);return[...e].sort((t,i)=>t.localeCompare(i))}get _currentFilters(){return{status:this._filterStatus,user_id:this._filterUser,label:this._filterLabel,archived:this._showArchived,sort_mode:this._sortMode,group_by:this._groupByMode}}_applyView(e){if(this._activeViewId=e,!e)return;let t=this._savedViews.find(a=>a.id===e);if(!t)return;let i=t.filters;this._filterStatus=i.status||"",this._filterUser=i.user_id||null,this._filterLabel=i.label||null,this._showArchived=!!i.archived,["due_date","object","type","task_name","area","assigned_user","group"].includes(i.sort_mode)&&(this._sortMode=i.sort_mode),["none","area","group","user"].includes(i.group_by)&&(this._groupByMode=i.group_by);try{localStorage.setItem(W.taskSort,this._sortMode),localStorage.setItem(W.groupBy,this._groupByMode)}catch{}this._overviewTab!=="dashboard"&&(this._overviewTab="dashboard")}_openSavedViewsDialog(){this.shadowRoot.querySelector("maintenance-saved-views-dialog")?.open(this._currentFilters,this._savedViews)}_onSavedViewsChanged(e){this._savedViews=e.detail.views||[],this._activeViewId&&!this._savedViews.some(t=>t.id===this._activeViewId)&&(this._activeViewId="")}_scrollContentToTop(){requestAnimationFrame(()=>{let e=this.shadowRoot?.querySelector(".content");e&&e.scrollTo({top:0,behavior:"smooth"})})}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null,this._scrollContentToTop()}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null,this._scrollContentToTop();let i=this._getTask(e,t);if(i?.trigger_config?.entity_id){let a=i.trigger_config.entity_id,l=this._isCounterEntity(i.trigger_config);this._fetchDetailStats(a,l)}}_showToast(e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastUndo=null,this._toastActionLabel="",this._toastMessage=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}_showActionToast(e,t,i){this._showUndoToast(e,i),this._toastActionLabel=t}_showUndoToast(e,t){this._toastTimer&&clearTimeout(this._toastTimer),this._toastActionLabel="",this._toastMessage=e,this._toastUndo=t,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastUndo=null,this._toastTimer=null},7e3)}_runToastUndo(){let e=this._toastUndo;this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage="",this._toastUndo=null,this._toastTimer=null,e?.()}_openPalette(){this._paletteQuery="",this._paletteActive=0,this._paletteOpen=!0,this.updateComplete.then(()=>{this.shadowRoot?.querySelector(".palette-input")?.focus()})}_closePalette(){this._paletteOpen=!1,this._paletteQuery=""}get _paletteResults(){let e=this._paletteQuery.trim().toLowerCase(),t=[];for(let i of this._objects){let a=i.object.name||"";(!e||a.toLowerCase().includes(e))&&t.push({kind:"object",entryId:i.entry_id,label:a,sub:s("object",this._lang)});for(let l of i.tasks){if(l.archived)continue;let c=l.name||"",p=(l.labels||[]).some(u=>u.toLowerCase().includes(e));if(!e||c.toLowerCase().includes(e)||a.toLowerCase().includes(e)||p){let u=(l.labels||[]).length?`  #${(l.labels||[]).join(" #")}`:"";t.push({kind:"task",entryId:i.entry_id,taskId:l.id,label:c,sub:a+u})}}if(t.length>60)break}return t.slice(0,40)}_selectPaletteResult(e){this._closePalette(),e.kind==="task"&&e.taskId?this._showTask(e.entryId,e.taskId):this._showObject(e.entryId)}_renderPalette(){if(!this._paletteOpen)return h;let e=this._lang,t=this._paletteResults;return o`
    ...
    `}_statusBadge(e,t,i){let a=this._lang,l=e?"archived":t?"done":i,c=e?"archived":t?"completed":i,p=e?s("archived",a):t?s("completed",a):s(i,a);return o`<span class="status-badge ${l}"><ha-icon icon="${Qe[c]||"mdi:circle-medium"}"></ha-icon>${p}</span>`}_setOverviewTab(e){this._overviewTab=e;try{localStorage.setItem(W.overviewTab,e)}catch{}this._scrollContentToTop()}_renderToday(){let e=this._lang,t=this._taskRows,i=u=>`${u.entry_id}:${u.task_id}`,a=t.filter(u=>u.status==="overdue"||u.trigger_active),l=new Set(a.map(i)),c=t.filter(u=>!l.has(i(u))&&u.days_until_due===0);c.forEach(u=>l.add(i(u)));let p=t.filter(u=>!l.has(i(u))&&u.days_until_due!=null&&u.days_until_due>0&&u.days_until_due<=7);return a.length+c.length+p.length===0?o`
      ...
      <div class="today-view">
        ...
        ${this._renderTodaySection("today_this_week",p,"")}
      </div>
    ...
    `}_renderDashboard(){let e=this._stats,t=this._taskRows,i=this._lang,a=this._isOperator,l=this._objects.reduce((p,u)=>p+u.tasks.filter(g=>g.archived).length,0),c=(this._filterStatus?1:0)+(this._filterUser?1:0)+(this._filterLabel?1:0)+(this._activeViewId?1:0);return o`
      ...
      <div class="filter-bar ${this.narrow&&!this._filtersOpen?"collapsed":""}">
        ...
        <label class="filter-field">
          ...
          <select
            ...
            @change=${p=>{let u=p.target.value;this._filterUser=u||null,this._activeViewId=""}}
          >
        ...
        ${this._allLabels.length>0?o`
          <label class="filter-field">
            ...
            <select
              ...
              @change=${p=>{let u=p.target.value;this._filterLabel=u||null,this._activeViewId=""}}
            >
        ...
        <label class="filter-field">
          ...
          <select
            ...
            @change=${p=>{this._sortMode=p.target.value,this._activeViewId="";try{localStorage.setItem(W.taskSort,this._sortMode)}catch{}}}
          >
        ...
        <label class="filter-field">
          ...
          <select
            ...
            @change=${p=>{this._groupByMode=p.target.value,this._activeViewId="";try{localStorage.setItem(W.groupBy,this._groupByMode)}catch{}}}
          >
      ...
      ${a?h:o`<maintenance-storage-section-card
            .hass=${this.hass}
            .objects=${this._objects}
            @open-object=${p=>{let u=p.detail?.entry_id;u&&this._showObject(u)}}
    ...
    `}_renderTaskTable(e){let t=this._bulkMode?" bulk":"";if(this._virtTotalRows=e.length,this.narrow||e.length<120)return o`
      ...
      `;let i=e.length,a=this._virtRowHeight,l=Math.max(0,Math.min(this._virtStart,i)),c=this._virtEnd>0?Math.min(this._virtEnd,i):Math.min(i,40);c<l&&(l=0,c=Math.min(i,40));let p=l*a,u=(i-c)*a;return o`
    ...
    `}_renderVirtSizerRow(e){let t=this._lang,i="",a=!1,l=!1,c=!1;for(let p of e){let u=p.archived?s("archived",t):p.is_done?s("completed",t):s(p.status,t);u.length>i.length&&(i=u),p.enabled||(a=!0),p.nfc_tag_id&&(l=!0),(p.priority==="high"||p.priority==="low")&&(c=!0)}return o`
    ...
    `}_renderGroupedTasks(e,t){let i=new Map,a=s("unassigned",t);for(let p of e){let u=[];this._groupByMode==="area"?u=[(p.area_id?this.hass?.areas?.[p.area_id]?.name:null)||a]:this._groupByMode==="user"?u=[(p.responsible_user_id?this._userService?.getUserName(p.responsible_user_id):null)||a]:this._groupByMode==="group"&&(u=p.group_names.length>0?p.group_names:[a]);for(let g of u)i.has(g)||i.set(g,[]),i.get(g).push(p)}let l=[...i.entries()].sort(([p],[u])=>p===a&&u!==a?1:u===a&&p!==a?-1:p.localeCompare(u)),c=this._groupByMode==="area"?"mdi:map-marker-outline":this._groupByMode==="group"?"mdi:folder-outline":"mdi:account-outline";return o`
      ${l.map(([p,u])=>o`
    ...
    `}_setObjectViewMode(e){this._objectViewMode=e,localStorage.setItem(W.objectView,e)}async _exportObjectsCsv(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects/csv"}),t=new Date().toISOString().slice(0,10);$t(e.csv,`maintenance_objects_${t}.csv`,"text/csv;charset=utf-8")}catch{this._showToast(s("action_error",this._lang))}}_renderObjectsTable(e){let t=this._lang,i=this._objectsTableColumns;return o`
      <div class="objects-table-wrap">
        <table class="objects-table">
          <thead>
            <tr>
              ${i.map(a=>{let l=ze.find(p=>p.key===a),c=l&&l.key!=="actions"?s(l.labelKey,t):"";return o`<th class="oc-${a}">${c}</th>`})}
            </tr>
          </thead>
          <tbody>
            ${e.map(a=>o`
    ...
    `}_openGroupCreate(){this.shadowRoot.querySelector("maintenance-group-dialog")?.openCreate()}_openGroupEdit(e){let t=this._groups[e];t&&this.shadowRoot.querySelector("maintenance-group-dialog")?.openEdit(e,t)}async _deleteGroup(e,t){let i=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(i?await i.confirm({title:s("delete_group",this._lang),message:s("delete_group_confirm",this._lang).replace("{name}",t),confirmText:s("delete",this._lang)}):confirm(`${s("delete_group_confirm",this._lang).replace("{name}",t)}`))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/group/delete",group_id:e}),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}_renderBudgetBar(){let e=this._budget;if(!e)return h;let t=this._lang,i=e.currency_symbol||Ce,a=[];e.monthly_budget>0&&a.push({label:s("budget_monthly",t),spent:e.monthly_spent,budget:e.monthly_budget}),e.yearly_budget>0&&a.push({label:s("budget_yearly",t),spent:e.yearly_spent,budget:e.yearly_budget});let l=[];return e.monthly_budget>0||l.push({label:s("budget_monthly",t),spent:e.monthly_spent||0}),e.yearly_budget>0||l.push({label:s("budget_yearly",t),spent:e.yearly_spent||0}),o`
      <div class="budget-bars">
        ${a.map(c=>{let p=Math.min(100,Math.max(0,c.spent/c.budget*100)),u=p>=100?"var(--error-color, #f44336)":p>=e.alert_threshold_pct?"var(--warning-color, #ff9800)":"var(--success-color, #4caf50)";return o`
            <div class="budget-item">
              ...
              <div class="budget-bar">
                <div class="budget-bar-fill" style="width:${p}%; background:${u}"></div>
              </div>
            </div>
          `})}
        ${l.map(c=>o`
    ...
    `}_renderOverviewRow(e){let t=this._lang,i=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,a=0,l=Ge.ok,c=!1;if(i&&e.days_until_due!==null){let v=yt(e.interval_days,e.days_until_due,e.interval_unit);a=v.pct,c=v.overflow,e.status==="overdue"?l=Ge.overdue:e.status==="due_soon"&&(l=Ge.due_soon)}let p=e.area_id?this.hass?.areas?.[e.area_id]?.name:null,u=e.responsible_user_id?this._userService?.getUserName(e.responsible_user_id):null,g=e.group_names.length>0||p||u,m=this._bulkMode&&this._bulkSelected.has(this._bulkKey(e));return o`
      <div class="task-row${e.enabled?"":" task-disabled"}${m?" bulk-selected":""}">
        ...
        <span class="task-sub${g?"":" task-sub-empty"}">
          ...
          ${p?o`
            <span class="sub-chip">
              <ha-icon icon="mdi:map-marker-outline"></ha-icon>${p}
            </span>`:h}
          ${u?o`
        ...
        <span class="due-cell" @click=${()=>this._showTask(e.entry_id,e.task_id)}>
          ...
          ${e.trigger_config?Zt(e):!i&&e.trigger_active?o`<span style="color:var(--maint-triggered-color);font-weight:600">⚡</span>`:h}
          ${Jt(e,this._miniStatsData,this._lang)}
        </span>
    ...
    `}_openCompleteDialogForRow(e){let i=this._objects.find(a=>a.entry_id===e.entry_id)?.tasks.find(a=>a.id===e.task_id);this._openCompleteDialog(e.entry_id,e.task_id,e.task_name,this._features.checklists?i?.checklist:void 0,this._features.adaptive&&!!i?.adaptive_config?.enabled)}_renderObjectDetail(){if(!this._selectedEntryId)return h;let e=this._getObject(this._selectedEntryId);if(!e)return o`<p>Object not found.</p>`;let t=e.object,i=this._lang,a=this._isOperator,l=e.tasks.filter(p=>p.archived).length,c=e.tasks.filter(p=>this._showArchived||!p.archived);return o`
      <div class="detail-section">
        ...
        ${e.tasks.length===0?o`<div class="empty-state-centered">
            ...
            </div>`:o`<div class="task-table">${[...c].sort((p,u)=>{let g={overdue:0,triggered:1,due_soon:2,ok:3};return(g[p.status]??9)-(g[u.status]??9)||(p.days_until_due??99999)-(u.days_until_due??99999)}).map(p=>o`
              <div class="task-row${p.enabled?"":" task-disabled"}">
                <span class="cell-badges">
                  ${this._statusBadge(!!p.archived,!!p.is_done,p.status)}
                  ${p.enabled?h:o`<span class="badge-disabled">${s("disabled",i)}</span>`}
                  ${p.nfc_tag_id?o`<span class="nfc-badge" title="${s("nfc_linked",i)}"><ha-icon icon="mdi:nfc-variant"></ha-icon></span>`:h}
                  ${p.document_count?o`<span class="doc-badge" title="${p.document_count} ${s("documents",i)}"><ha-icon icon="mdi:paperclip"></ha-icon>${p.document_count}</span>`:h}
                </span>
                <span class="cell task-name" @click=${()=>this._showTask(e.entry_id,p.id)}>${p.name}</span>
                <span class="task-sub${p.responsible_user_id?"":" task-sub-empty"}">${ti(p,u=>this._userService?.getUserName(u)??null)}</span>
                <span class="cell type">${s(p.type,i)}</span>
                <span class="due-cell" @click=${()=>this._showTask(e.entry_id,p.id)}>
                  <span class="due-text">${ke(p.days_until_due,i)}</span>
                  ${p.trigger_config?Zt(p):h}
                  ${Jt(p,this._miniStatsData,this._lang)}
                </span>
                <span class="row-actions">
                  <mwc-icon-button class="btn-complete" title="${s("complete",i)}" @click=${u=>{u.stopPropagation(),this._openCompleteDialog(e.entry_id,p.id,p.name,this._features.checklists?p.checklist:void 0,this._features.adaptive&&!!p.adaptive_config?.enabled)}}>
                    <ha-icon icon="mdi:check"></ha-icon>
                  </mwc-icon-button>
                  <mwc-icon-button class="btn-skip" title="${s("skip",i)}" .disabled=${this._actionLoading} @click=${u=>{u.stopPropagation(),this._promptSkipTask(e.entry_id,p.id)}}>
                    <ha-icon icon="mdi:skip-next"></ha-icon>
                  </mwc-icon-button>
                </span>
              </div>
