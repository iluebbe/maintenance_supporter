...
`;var La={days:1,weeks:7,months:30.4368,years:365.25};function it(l,s){return!l||l<=0?0:l*(La[s||"days"]??1)}function st(l,s,e){let t=it(l,e);if(t<=0||s==null)return{pct:0,overflow:!1};let a=(t-s)/t*100;return{pct:Math.max(0,Math.min(100,a)),overflow:a>100}}function ot(l,s=new Date){if(!l)return{kind:"none",days:null,date:null};let e=new Date(`${l}T00:00:00`);if(isNaN(e.getTime()))return{kind:"none",days:null,date:null};let t=Date.UTC(s.getFullYear(),s.getMonth(),s.getDate()),a=Date.UTC(e.getFullYear(),e.getMonth(),e.getDate()),i=Math.round((a-t)/864e5);return i<0?{kind:"expired",days:i,date:l}:i<=60?{kind:"expiring",days:i,date:l}:{kind:"valid",days:i,date:l}}var pe=[{key:"name",labelKey:"name",required:!0},{key:"manufacturer",labelKey:"manufacturer"},{key:"model",labelKey:"model"},{key:"serial_number",labelKey:"serial_number_label"},{key:"installation_date",labelKey:"installed"},{key:"warranty_expiry",labelKey:"warranty"},{key:"area_id",labelKey:"area"},{key:"documentation_url",labelKey:"documentation_url_label"},{key:"notes",labelKey:"object_notes_label"},{key:"task_count",labelKey:"tasks"},{key:"actions",labelKey:"actions"}],Ha=pe.map(l=>l.key),Ie=["name","manufacturer","model","serial_number","installation_date","warranty_expiry","area_id","task_count","actions"];function $e(l){if(!Array.isArray(l))return[...Ie];let s=new Set,e=[];for(let t of l)typeof t=="string"&&Ha.includes(t)&&!s.has(t)&&(s.add(t),e.push(t));return e.length?(e.includes("name")||e.unshift("name"),e):[...Ie]}var Mt=q`
  :host {
    display: block;
    height: 100%;
...
`;var Pe=class{constructor(s){this._cache=new Map;this._pending=new Map;this._hass=s}updateHass(s){this._hass=s}async getDetailStats(s,e){return this._getStats(s,"hour",30,e)}async getMiniStats(s,e){return this._getStats(s,"day",14,e)}async getBatchMiniStats(s){let e=new Map,t=[];for(let u of s){let p=`${u.entityId}:day`,h=this._cache.get(p);h&&Date.now()-h.fetchedAt<3e5?e.set(u.entityId,h.points):t.push(u)}if(t.length===0)return e;let a=t.filter(u=>u.isCounter).map(u=>u.entityId),i=t.filter(u=>!u.isCounter).map(u=>u.entityId),o=new Date(Date.now()-336*60*60*1e3).toISOString(),c=[];return a.length>0&&c.push(this._fetchBatch(a,"day",o,["state","sum","change"],!0,e)),i.length>0&&c.push(this._fetchBatch(i,"day",o,["mean","min","max"],!1,e)),await Promise.all(c),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(s,e,t,a){let i=`${s}:${e}`,o=this._cache.get(i);if(o&&Date.now()-o.fetchedAt<3e5)return o.points;if(this._pending.has(i))return this._pending.get(i);let c=this._fetchAndNormalize(s,e,t,a,i);this._pending.set(i,c);try{return await c}finally{this._pending.delete(i)}}async _fetchAndNormalize(s,e,t,a,i){let o=new Date(Date.now()-t*24*60*60*1e3).toISOString(),c=a?["state","sum","change"]:["mean","min","max"];try{let p=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:o,statistic_ids:[s],period:e,types:c}))[s]||[],h=this._normalizeRows(p,a);return this._cache.set(i,{entityId:s,fetchedAt:Date.now(),period:e,points:h}),h}catch(u){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${s}:`,u),[]}}async _fetchBatch(s,e,t,a,i,o){try{let c=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:s,period:e,types:a});for(let u of s){let p=c[u]||[],h=this._normalizeRows(p,i);o.set(u,h),this._cache.set(`${u}:${e}`,{entityId:u,fetchedAt:Date.now(),period:e,points:h})}}catch(c){console.warn("[maintenance-supporter] Batch statistics fetch failed:",c)}}_normalizeRows(s,e){let t=[];for(let a of s){let i=null;if(e?i=a.state??null:i=a.mean??null,i===null)continue;let o={ts:a.start,val:i};e||(a.min!=null&&(o.min=a.min),a.max!=null&&(o.max=a.max)),t.push(o)}return t.sort((a,i)=>a.ts-i.ts),t}};var oe=class{constructor(s){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=s}updateHass(s){this.hass=s}async getUsers(s=!1){let e=Date.now();if(!s&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(s,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:s,task_id:e,user_id:t})}async getTasksByUser(s){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:s})).tasks}getUserName(s){return!s||!this.usersCache?null:this.usersCache.find(t=>t.id===s)?.name||null}getUser(s){return!s||!this.usersCache?null:this.usersCache.find(e=>e.id===s)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(s){return s?s===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var Oa={name:"name",task_type:"maintenance_type",schedule_type:"schedule_type",interval_days:"interval_days",interval_anchor:"interval_anchor",warning_days:"warning_days",last_performed:"last_performed_optional",notes:"notes_optional",documentation_url:"documentation_url_optional",custom_icon:"custom_icon_optional",nfc_tag_id:"nfc_tag_id_optional",responsible_user_id:"responsible_user",entity_slug:"entity_slug",entity_id:"entity_id",area_id:"area_id_optional",manufacturer:"manufacturer_optional",model:"model_optional",serial_number:"serial_number_optional",installation_date:"installation_date_optional",warranty_expiry:"warranty_expiry_optional",checklist:"checklist_steps_optional",reason:"reason",feedback:"feedback",cost:"cost",duration:"duration",description:"description_optional",group_name:"name",group_description:"description_optional",environmental_entity:"environmental_entity_optional",environmental_attribute:"environmental_attribute_optional",trigger_above:"trigger_above",trigger_below:"trigger_below",trigger_for_minutes:"trigger_for_minutes"};function Ua(l,s){let e=Oa[l];if(!e)return l;let t=n(e,s);return t&&t!==e?t:l}function Va(l){let e=l.match(/data\['([^']+)'\]/)?.[1],t;return(t=l.match(/length of value must be at most (\d+)/))?{field:e,rule:"too_long",param:t[1]}:(t=l.match(/length of value must be at least (\d+)/))?{field:e,rule:"too_short",param:t[1]}:(t=l.match(/value must be at most (\S+)/))?{field:e,rule:"value_too_high",param:t[1]}:(t=l.match(/value must be at least (\S+)/))?{field:e,rule:"value_too_low",param:t[1]}:/required key not provided/.test(l)?{field:e,rule:"required"}:(t=l.match(/expected (\w+)/))?{field:e,rule:"wrong_type",param:t[1]}:/value must be one of/.test(l)?{field:e,rule:"invalid_choice"}:/not a valid value/.test(l)?{field:e,rule:"invalid_value"}:{field:e,rule:"unknown"}}function W(l,s,e){if(typeof l=="string")return l;if(typeof l!="object"||l===null)return e;let t=l,a=t.message||t.error?.message||"";if(!a)return e;let i=Va(a),o=i.field?Ua(i.field,s):"",c=u=>n(u,s).replace("{field}",o).replace("{n}",i.param??"");switch(i.rule){case"too_long":return c("err_too_long");case"too_short":return c("err_too_short");case"value_too_high":return c("err_value_too_high");case"value_too_low":return c("err_value_too_low");case"required":return c("err_required");case"wrong_type":return c("err_wrong_type").replace("{type}",i.param??"");case"invalid_choice":return c("err_invalid_choice");case"invalid_value":return c("err_invalid_value");default:return a||e}}var V=class extends T{constructor(){super(...arguments);this.label="";this.value="";this.placeholder="";this.type="text";this.required=!1;this.disabled=!1}_onInput(e){let t=e.target.value;this.value=t,this.dispatchEvent(new CustomEvent("input",{bubbles:!0,composed:!0,detail:{value:t}}))}render(){return r`
      <label class="field">
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <ms-textfield
            ...
            @input=${a=>this._warrantyExpiry=a.target.value}
          ></ms-textfield>
...
`),this._scheduleTime=t.schedule_time||"";let i=t.on_complete_action;if(i&&i.service){this._actionService=i.service;let u=i.target?.entity_id;this._actionTargetEntity=Array.isArray(u)?u[0]||"":u||"",this._actionData=i.data&&typeof i.data=="object"?{...i.data}:{},this._actionDataJsonFallback=""}else this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="";let o=t.quick_complete_defaults;this._qcNotes=o?.notes||"",this._qcCost=o?.cost!=null?String(o.cost):"",this._qcDuration=o?.duration!=null?String(o.duration):"",this._qcFeedback=o?.feedback||"";let c=t.adaptive_config||{};if(this._environmentalEntity=c.environmental_entity||"",this._environmentalAttribute=c.environmental_attribute||"",this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute,t.trigger_config){let u=t.trigger_config;this._triggerEntityId=u.entity_id||"",this._triggerEntityIds=u.entity_ids||(u.entity_id?[u.entity_id]:[]),this._triggerEntityLogic=u.entity_logic||"any",this._triggerAttribute=u.attribute||"",this._triggerType=u.type||"threshold",this._triggerAbove=u.trigger_above?.toString()||"",this._triggerBelow=u.trigger_below?.toString()||"",this._triggerForMinutes=u.trigger_for_minutes?.toString()||"0",this._triggerTargetValue=u.trigger_target_value?.toString()||"",this._triggerDeltaMode=u.trigger_delta_mode||!1,this._triggerFromState=u.trigger_from_state||"",this._triggerToState=u.trigger_to_state||"",this._triggerTargetChanges=u.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=u.trigger_runtime_hours?.toString()||""}else this._resetTriggerFields();this._triggerEntityId&&this._fetchEntityAttributes(this._triggerEntityId),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._intervalUnit="days",this._dueDate="",this._warningDays=String(this.defaultWarningDays),this._intervalAnchor="completion",this._weekdays=[],this._nth="1",this._nthWeekday="5",this._domDay="1",this._notes="",this._documentationUrl="",this._customIcon="",this._enabled=!0,this._lastPerformed="",this._nfcTagId="",this._responsibleUserId=null,this._checklistText="",this._scheduleTime="",this._environmentalEntity="",this._environmentalAttribute="",this._environmentalInitial="",this._environmentalAttributeInitial="",this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="",this._actionTesting=!1,this._actionTestResult="",this._qcNotes="",this._qcCost="",this._qcDuration="",this._qcFeedback="",this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerForMinutes="0",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours=""}async _loadUsers(){this._userService||(this._userService=new oe(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}async _testAction(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e)){this._actionTestResult="error",this._actionTestError="Invalid service format (expected 'domain.service')",setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3);return}let[t,a]=e.split(".");if(!this.hass?.services?.[t]?.[a]){this._actionTestResult="error",this._actionTestError=`Service "${e}" is not registered in Home Assistant. Check spelling and that the integration providing it is loaded.`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}let i=this._actionTargetEntity.trim();if(i){let o=i.split(".")[0];if(o!==t&&!new Set(["homeassistant","scene","notify","persistent_notification"]).has(t)){this._actionTestResult="error",this._actionTestError=`Service "${e}" only works on ${t}.* entities; entity "${i}" is in ${o}.* \u2014 pick a service that matches the entity domain (e.g. ${o}.${a})`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}if(!this.hass.states?.[i]){this._actionTestResult="error",this._actionTestError=`Target entity "${i}" not found in Home Assistant \u2014 the entity may have been renamed or its integration removed.`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}}this._actionTestResult="ok",setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3)}_buildActionData(){if(this._actionDataJsonFallback.trim())try{let e=JSON.parse(this._actionDataJsonFallback);if(e&&typeof e=="object"&&!Array.isArray(e))return e}catch{}return{...this._actionData}}_serviceSchema(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e))return null;let[t,a]=e.split("."),i=this.hass?.services?.[t]?.[a]?.fields;return!i||Object.keys(i).length===0?null:Object.entries(i).map(([o,c])=>({name:o,required:!!c.required,selector:c.selector||{text:{}}}))}_renderCompletionActionsSection(e){if(!this.completionActionsEnabled)return d;let t=this._serviceSchema();return r`
      <details class="ca-section">
        ...
        <ha-service-picker
          ...
          @value-changed=${a=>{this._actionService=a.detail.value||"";let i=this._serviceSchema();if(i){let o=new Set(i.map(c=>c.name));this._actionData=Object.fromEntries(Object.entries(this._actionData).filter(([c])=>o.has(c)))}}}
        ></ha-service-picker>
        <ha-form
          .hass=${this.hass}
          ...
          @value-changed=${a=>{let i=a.detail.value;this._actionTargetEntity=i.target_entity||""}}
        ></ha-form>
        ...
        <div class="ca-test-row">
          ...
          ${this._actionTestResult==="ok"?r`<span class="ca-test-ok">${n("on_complete_action_test_success",e)}</span>`:d}
...
`).map(c=>c.trim()).filter(Boolean).slice(0,100);e.checklist=o.length?o:null}if(this.completionActionsEnabled){let o=this._actionService.trim();if(o&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(o)){let h={service:o},m=this._actionTargetEntity.trim();m&&(h.target={entity_id:m});let v=this._buildActionData();Object.keys(v).length>0&&(h.data=v),e.on_complete_action=h}else e.on_complete_action=null;let c={};this._qcNotes.trim()&&(c.notes=this._qcNotes.trim());let u=parseFloat(this._qcCost);!isNaN(u)&&u>=0&&(c.cost=u);let p=parseInt(this._qcDuration,10);!isNaN(p)&&p>=0&&(c.duration=p),this._qcFeedback&&(c.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(c).length?c:null}let t=await this.hass.connection.sendMessagePromise(e),a=this._taskId||t?.task_id,i=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(a&&this._scheduleType==="sensor_based"&&i)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:a,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=W(e,this._lang,n("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return d;let e=this._lang;return r`
      ...
      <ms-textfield
        ...
        @input=${t=>{let i=t.target.value.split(",").map(o=>o.trim()).filter(Boolean);this._triggerEntityId=i[0]||"",this._triggerEntityIds=i,i[0]&&this._fetchEntityAttributes(i[0])}}
      ></ms-textfield>
      ...
      ${this._availableAttributes.length>0?r`
          <div class="select-row">
            <label>${n("attribute_optional",e)}</label>
            <select
              .value=${this._triggerAttribute}
              @change=${t=>this._triggerAttribute=t.target.value}
            >
              <option value="" ?selected=${!this._triggerAttribute}>${n("use_entity_state",e)}</option>
      ...
      <div class="select-row">
        ...
        >
          ${Ka.map(t=>r`<option value=${t} ?selected=${t===this._triggerType}>${n(t,e)}</option>`)}
        </select>
      </div>
    ...
    `}_renderUnitSelect(){let e=this._lang;return r`
      <div class="select-row">
        ...
        <div class="weekday-chips">
          ${t.map((a,i)=>r`
            ...
            >${a}</button>`)}
        </div>`;if(this._scheduleType==="nth_weekday"){let a=[["1",n("ord_1",e)],["2",n("ord_2",e)],["3",n("ord_3",e)],["4",n("ord_4",e)],["5",n("ord_5",e)],["-1",n("ord_last",e)]];return r`
        <div class="select-row">
          <label>${n("recurrence_occurrence",e)}</label>
          <select .value=${this._nth} @change=${i=>this._nth=i.target.value}>
            ${a.map(([i,o])=>r`<option value=${i} ?selected=${i===this._nth}>${o}</option>`)}
          </select>
        </div>
        <div class="select-row">
          <label>${n("recurrence_weekday",e)}</label>
          <select .value=${this._nthWeekday} @change=${i=>this._nthWeekday=i.target.value}>
            ${t.map((i,o)=>r`<option value=${String(o)} ?selected=${String(o)===this._nthWeekday}>${i}</option>`)}
          </select>
        ...
        <ms-textfield
          ...
          max="31"
          .value=${this._domDay}
          @input=${a=>this._domDay=a.target.value}
        ></ms-textfield>`:d}_renderTriggerTypeFields(){let e=this._lang;return this._triggerType==="threshold"?r`
        <ms-textfield
      ...
      `:this._triggerType==="runtime"?r`
        ...
        ></ms-textfield>
      `:d}render(){if(!this._open)return r``;let e=this._lang,t=this._taskId?n("edit_task",e):n("new_task",e);return r`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
          ${this._error?r`<div class="error">${this._error}</div>`:d}
          ...
          <div class="select-row">
            ...
            >
              ${Ba.map(a=>r`<option value=${a} ?selected=${a===this._type}>${n(a,e)}</option>`)}
            </select>
          </div>
          <div class="select-row">
            ...
            >
              ${Wa.map(a=>r`<option value=${a} ?selected=${a===this._scheduleType}>${n(a,e)}</option>`)}
            </select>
          </div>
          ${this._scheduleType==="time_based"?r`
                ...
                ${this.scheduleTimeEnabled?r`
                  <ms-textfield
                    label="${n("schedule_time_optional",e)}"
                    type="time"
          ...
          ${this._scheduleType==="one_time"?r`
                <ms-textfield
                  ...
                  type="date"
                  .value=${this._dueDate}
                  @input=${a=>this._dueDate=a.target.value}
                ></ms-textfield>
              `:d}
          <ms-textfield
          ...
          <div class="select-row">
            ...
            <select
              ...
              @change=${a=>{let i=a.target.value;this._responsibleUserId=i||null}}
            >
          ...
          ${this._scheduleType==="sensor_based"?r`
            ...
            ${this._environmentalEntity?r`
              <ms-textfield
                label="${n("environmental_attribute_optional",e)}"
  ...
  `,_([x({attribute:!1})],w.prototype,"hass",2),_([x({type:Boolean,attribute:"checklists-enabled"})],w.prototype,"checklistsEnabled",2),_([x({type:Boolean,attribute:"schedule-time-enabled"})],w.prototype,"scheduleTimeEnabled",2),_([x({type:Boolean,attribute:"completion-actions-enabled"})],w.prototype,"completionActionsEnabled",2),_([x({type:Number,attribute:"default-warning-days"})],w.prototype,"defaultWarningDays",2),_([g()],w.prototype,"_open",2),_([g()],w.prototype,"_loading",2),_([g()],w.prototype,"_error",2),_([g()],w.prototype,"_entryId",2),_([g()],w.prototype,"_taskId",2),_([g()],w.prototype,"_objectChoices",2),_([g()],w.prototype,"_name",2),_([g()],w.prototype,"_type",2),_([g()],w.prototype,"_scheduleType",2),_([g()],w.prototype,"_intervalDays",2),_([g()],w.prototype,"_intervalUnit",2),_([g()],w.prototype,"_dueDate",2),_([g()],w.prototype,"_warningDays",2),_([g()],w.prototype,"_intervalAnchor",2),_([g()],w.prototype,"_weekdays",2),_([g()],w.prototype,"_nth",2),_([g()],w.prototype,"_nthWeekday",2),_([g()],w.prototype,"_domDay",2),_([g()],w.prototype,"_notes",2),_([g()],w.prototype,"_documentationUrl",2),_([g()],w.prototype,"_customIcon",2),_([g()],w.prototype,"_enabled",2),_([g()],w.prototype,"_triggerEntityId",2),_([g()],w.prototype,"_triggerEntityIds",2),_([g()],w.prototype,"_triggerEntityLogic",2),_([g()],w.prototype,"_triggerAttribute",2),_([g()],w.prototype,"_triggerType",2),_([g()],w.prototype,"_triggerAbove",2),_([g()],w.prototype,"_triggerBelow",2),_([g()],w.prototype,"_triggerForMinutes",2),_([g()],w.prototype,"_triggerTargetValue",2),_([g()],w.prototype,"_triggerDeltaMode",2),_([g()],w.prototype,"_triggerFromState",2),_([g()],w.prototype,"_triggerToState",2),_([g()],w.prototype,"_triggerTargetChanges",2),_([g()],w.prototype,"_triggerRuntimeHours",2),_([g()],w.prototype,"_suggestedAttributes",2),_([g()],w.prototype,"_availableAttributes",2),_([g()],w.prototype,"_entityDomain",2),_([g()],w.prototype,"_lastPerformed",2),_([g()],w.prototype,"_nfcTagId",2),_([g()],w.prototype,"_availableTags",2),_([g()],w.prototype,"_responsibleUserId",2),_([g()],w.prototype,"_availableUsers",2),_([g()],w.prototype,"_checklistText",2),_([g()],w.prototype,"_scheduleTime",2),_([g()],w.prototype,"_actionService",2),_([g()],w.prototype,"_actionTargetEntity",2),_([g()],w.prototype,"_actionData",2),_([g()],w.prototype,"_actionDataJsonFallback",2),_([g()],w.prototype,"_actionTesting",2),_([g()],w.prototype,"_actionTestResult",2),_([g()],w.prototype,"_actionTestError",2),_([g()],w.prototype,"_qcNotes",2),_([g()],w.prototype,"_qcCost",2),_([g()],w.prototype,"_qcDuration",2),_([g()],w.prototype,"_qcFeedback",2),_([g()],w.prototype,"_environmentalEntity",2),_([g()],w.prototype,"_environmentalAttribute",2);customElements.get("maintenance-task-dialog")||customElements.define("maintenance-task-dialog",w);var F=class extends T{constructor(){super(...arguments);this.entryId="";this.taskId="";this.taskName="";this.lang="en";this.checklist=[];this.adaptiveEnabled=!1;this._open=!1;this._notes="";this._cost="";this._duration="";this._loading=!1;this._error="";this._checklistState={};this._feedback="needed"}open(){this._open||(this._open=!0,this._notes="",this._cost="",this._duration="",this._error="",this._checklistState={},this._feedback="needed")}_toggleCheck(e){let t=String(e);this._checklistState={...this._checklistState,[t]:!this._checklistState[t]}}_setFeedback(e){this._feedback=e}async _complete(){this._loading=!0,this._error="";try{let e={type:"maintenance_supporter/task/complete",entry_id:this.entryId,task_id:this.taskId};if(this._notes&&(e.notes=this._notes),this._cost){let t=parseFloat(this._cost);!isNaN(t)&&t>=0&&(e.cost=t)}if(this._duration){let t=parseInt(this._duration,10);!isNaN(t)&&t>=0&&(e.duration=t)}this.checklist.length>0&&(e.checklist_state=this._checklistState),this.adaptiveEnabled&&(e.feedback=this._feedback),await this.hass.connection.sendMessagePromise(e),this._open=!1,this.dispatchEvent(new CustomEvent("task-completed"))}catch(e){this._error=W(e,this.lang,n("save_error",this.lang))}finally{this._loading=!1}}_close(){this._open=!1}render(){if(!this._open)return r``;let e=this.lang||this.hass?.language||"en";return r`
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <label class="field">
            <span class="field-label">${n("notes_optional",e)}</span>
            <input type="text" class="field-input"
              .value=${this._notes}
              @input=${t=>this._notes=t.target.value} />
          </label>
          <label class="field">
            <span class="field-label">${n("cost_optional",e)}</span>
            <input type="number" step="0.01" min="0" class="field-input"
              .value=${this._cost}
              @input=${t=>this._cost=t.target.value} />
          </label>
          <label class="field">
            <span class="field-label">${n("duration_minutes",e)}</span>
  ...
  `,_([x({attribute:!1})],F.prototype,"hass",2),_([x()],F.prototype,"entryId",2),_([x()],F.prototype,"taskId",2),_([x()],F.prototype,"taskName",2),_([x()],F.prototype,"lang",2),_([x({type:Array})],F.prototype,"checklist",2),_([x({type:Boolean})],F.prototype,"adaptiveEnabled",2),_([g()],F.prototype,"_open",2),_([g()],F.prototype,"_notes",2),_([g()],F.prototype,"_cost",2),_([g()],F.prototype,"_duration",2),_([g()],F.prototype,"_loading",2),_([g()],F.prototype,"_error",2),_([g()],F.prototype,"_checklistState",2),_([g()],F.prototype,"_feedback",2);customElements.get("maintenance-complete-dialog")||customElements.define("maintenance-complete-dialog",F);function ge(l){return l.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function Nt(l){return!l.startsWith("data:image/svg+xml,")&&!l.startsWith("data:image/png;base64,")?"":ge(l)}function Za(l){return l.replace(/[/\\:*?"<>|#%]+/g,"").replace(/\s+/g,"-").toLowerCase().substring(0,100)}var J=class extends T{constructor(){super(...arguments);this.lang="en";this._open=!1;this._loading=!1;this._error="";this._viewResult=null;this._completeResult=null;this._urlMode="companion";this._entryId="";this._taskId=null;this._objectName="";this._taskName="";this._generateSeq=0}openForObject(e,t){this._entryId=e,this._taskId=null,this._objectName=t,this._taskName="",this._urlMode="companion",this._error="",this._viewResult=null,this._completeResult=null,this._open=!0,this._generate()}openForTask(e,t,a,i){this._entryId=e,this._taskId=t,this._objectName=a,this._taskName=i,this._urlMode="companion",this._error="",this._viewResult=null,this._completeResult=null,this._open=!0,this._generate()}async _generate(){let e=++this._generateSeq;this._loading=!0,this._error="",this._viewResult=null,this._completeResult=null;try{let t={type:"maintenance_supporter/qr/generate",entry_id:this._entryId,url_mode:this._urlMode};this._taskId&&(t.task_id=this._taskId);let a=[this.hass.connection.sendMessagePromise({...t,action:"view"})];this._taskId&&a.push(this.hass.connection.sendMessagePromise({...t,action:"complete"}));let i=await Promise.all(a);if(e!==this._generateSeq)return;this._viewResult=i[0],i.length>1&&(this._completeResult=i[1])}catch(t){if(e!==this._generateSeq)return;let a=t?.code,i=t?.message;this._error=a==="no_url"||typeof i=="string"&&i.includes("No Home Assistant URL")?n("qr_error_no_url",this.lang):n("qr_error",this.lang)}finally{e===this._generateSeq&&(this._loading=!1)}}_setUrlMode(e){this._urlMode!==e&&(this._urlMode=e,this._generate())}_print(){if(!this._viewResult)return;let e=this._viewResult,t=e.label.task_name?`${e.label.object_name} \u2014 ${e.label.task_name}`:e.label.object_name,a=[e.label.manufacturer,e.label.model].filter(Boolean).join(" "),i=window.open("","_blank","width=600,height=500");if(!i)return;let o=this.lang||"en",c=ge(t),u=ge(a),p=!!this._completeResult,h=ge(n("qr_action_view",o)),m=ge(n("qr_action_complete",o));i.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8">
<title>${c}</title>
<style>
...
<div class="url">${ge(this._viewResult.url)}</div>
<script>setTimeout(()=>window.print(),300)<\/script>
</body></html>`),i.document.close()}_downloadSvg(e,t){let a=decodeURIComponent(e.svg_data_uri.replace("data:image/svg+xml,","")),i=new Blob([a],{type:"image/svg+xml"}),o=URL.createObjectURL(i),c=document.createElement("a");c.href=o;let u=this._taskName?`${this._objectName}-${this._taskName}`:this._objectName;c.download=`qr-${Za(u)}-${t}.svg`,c.click(),URL.revokeObjectURL(o)}_close(){this._open=!1,this._viewResult=null,this._completeResult=null,this._error="",this._loading=!1}render(){if(!this._open)return r``;let e=this.lang||this.hass?.language||"en",t=this._taskName?`${n("qr_code",e)}: ${this._objectName} \u2014 ${this._taskName}`:`${n("qr_code",e)}: ${this._objectName}`,a=!!this._viewResult;return r`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
          ${this._loading?r`<div class="loading">${n("qr_generating",e)}</div>`:this._error?r`<div class="error">${this._error}</div>`:a?r`
                    <div class="qr-pair">
                      <div class="qr-item">
                        <img
  ...
  `,_([x({attribute:!1})],J.prototype,"hass",2),_([x()],J.prototype,"lang",2),_([g()],J.prototype,"_open",2),_([g()],J.prototype,"_loading",2),_([g()],J.prototype,"_error",2),_([g()],J.prototype,"_viewResult",2),_([g()],J.prototype,"_completeResult",2),_([g()],J.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",J);var Dt=5;function ze(l){let s=l.getFullYear(),e=String(l.getMonth()+1).padStart(2,"0"),t=String(l.getDate()).padStart(2,"0");return`${s}-${e}-${t}`}function Ja(l,s){let e=[];for(let t=0;t<s;t++){let a=new Date(l);a.setDate(a.getDate()+t),a.setHours(0,0,0,0),e.push(ze(a))}return e}function Fe(l,s){let[e,t,a]=l.split("-").map(Number),i=new Date(e,t-1,a);return i.setDate(i.getDate()+s),ze(i)}function Ya(l){if(!l||l.length===0)return null;let s=l.map(e=>e.cost).filter(e=>typeof e=="number");return s.length===0?null:s.reduce((e,t)=>e+t,0)/s.length}function Xa(l){let{windowStart:s,windowEnd:e,task:t,entryId:a,objectName:i}=l,o=[],c=(m,v)=>({date:m,entry_id:a,task_id:t.id,task_name:t.name,object_name:i,status:v&&(t.status==="overdue"||t.status==="triggered")?"ok":t.status,days_until_due:v?null:t.days_until_due??null,projected:v,schedule_type:t.schedule_type,interval_days:t.interval_days??null,interval_unit:t.interval_unit??null,responsible_user_id:t.responsible_user_id??null,avg_cost:Ya(t.history),adaptive_enabled:!!t.adaptive_config?.enabled,prediction_confidence:t.threshold_prediction_confidence??null}),u=Math.max(1,Math.round(it(t.interval_days,t.interval_unit)));if(t.status==="overdue"||t.status==="triggered"){if(o.push(c(s,!1)),t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let m=Fe(s,u),v=1;for(;m<=e&&v<Dt;)o.push(c(m,!0)),v++,m=Fe(m,u)}return o}let p=t.next_due;if(typeof p!="string"||!p)return o;let h=p.slice(0,10);if(h>=s&&h<=e)o.push(c(h,!1));else if(h>e)return o;if(t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let m=Fe(h,u),v=o.length;for(;m<=e&&v<Dt;)m>=s&&(o.push(c(m,!0)),v++),m=Fe(m,u)}return o}var Rt={overdue:0,triggered:1,due_soon:2,ok:3};function It(l,s,e,t=null){let a=Ja(s,e),i=a[0],o=a[a.length-1],c=[];for(let p of l){let h=p.object?.name||"",m=p.entry_id,v=p.tasks||[];for(let f of v){if(t&&f.responsible_user_id!==t||f.enabled===!1)continue;let k=Xa({windowStart:i,windowEnd:o,task:f,entryId:m,objectName:h});c.push(...k)}}let u=new Map;for(let p of a)u.set(p,[]);for(let p of c){let h=u.get(p.date);h&&h.push(p)}for(let[,p]of u)p.sort((h,m)=>{let v=Rt[h.status]??99,f=Rt[m.status]??99;if(v!==f)return v-f;if(h.projected!==m.projected)return h.projected?1:-1;let k=h.object_name.localeCompare(m.object_name);return k!==0?k:h.task_name.localeCompare(m.task_name)});return a.map(p=>({date:p,events:u.get(p)??[]}))}var en={completed:"ok",reset:"ok",skipped:"due_soon",triggered:"triggered",trigger_replaced:"triggered"};function tn(l,s){let e=[];for(let t=s-1;t>=0;t--){let a=new Date(l);a.setDate(a.getDate()-t),a.setHours(0,0,0,0),e.push(ze(a))}return e}function Pt(l,s,e,t=null){let a=tn(s,e),i=a[0],o=a[a.length-1],c=new Map;for(let p of a)c.set(p,[]);for(let p of l){let h=p.object?.name||"",m=p.entry_id,v=p.tasks||[];for(let f of v){if(t&&f.responsible_user_id!==t)continue;let k=f.history||[];for(let b of k){if(typeof b?.timestamp!="string")continue;let N=b.timestamp.slice(0,10);if(N<i||N>o)continue;let C=c.get(N);if(!C)continue;let E=b.type??"completed";C.push({date:N,entry_id:m,task_id:f.id,task_name:f.name,object_name:h,status:en[E]??"ok",days_until_due:null,projected:!1,schedule_type:f.schedule_type,interval_days:f.interval_days??null,responsible_user_id:f.responsible_user_id??null,avg_cost:typeof b.cost=="number"?b.cost:null,adaptive_enabled:!!f.adaptive_config?.enabled,prediction_confidence:null,history_timestamp:b.timestamp,history_type:E,history_cost:typeof b.cost=="number"?b.cost:null,history_notes:typeof b.notes=="string"?b.notes:null,history_duration:typeof b.duration=="number"?b.duration:null})}}}let u={completed:0,reset:1,skipped:2,triggered:3,trigger_replaced:4};for(let[,p]of c)p.sort((h,m)=>{let v=u[h.history_type??""]??99,f=u[m.history_type??""]??99;if(v!==f)return v-f;let k=h.object_name.localeCompare(m.object_name);return k!==0?k:h.task_name.localeCompare(m.task_name)});return a.map(p=>({date:p,events:c.get(p)??[]}))}var Ft=q`
  .cal-controls {
    display: flex;
    gap: 12px;
    align-items: center;
...
`;var Y=class extends T{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter)}getCardSize(){return 6}get _lang(){return this.hass?.language||"en"}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}_onEventClick(e){if(e.history_timestamp){this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}));return}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}render(){if(!this.hass)return d;let e=this._lang,t=this._config.show_window_chips!==!1,a=this._config.show_user_filter!==!1,i=this._config.title,o=null;this._userFilter&&(o=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let c=new Date;c.setHours(0,0,0,0);let u=this._pastDays>0,p=u?Pt(this._objects,c,this._pastDays,o):It(this._objects,c,this._windowDays,o),h=ze(c),m=this._windowDays===365||u,v=m?p.filter(b=>b.events.length>0):p,f=b=>{let N=`cal-status-${b.status}`,C=b.projected?"cal-event-projected":"",E=b.status==="overdue"&&b.days_until_due!=null?` (${Math.abs(b.days_until_due)}d ${n("overdue",e).toLowerCase()})`:"",$=b.projected&&b.interval_days?r`<span class="cal-event-recur">${b.interval_unit&&b.interval_unit!=="days"?`${b.interval_days} ${n("unit_"+b.interval_unit,e)}`:n("cal_every_n_days",e).replace("{n}",String(b.interval_days))}</span>`:d,M=b.schedule_type==="sensor_based",L=M?r`<ha-icon class="cal-event-icon cal-source-sensor"
                title="${n("cal_source_sensor",e)}" icon="mdi:trending-up"></ha-icon>`:r`<ha-icon class="cal-event-icon cal-source-time"
                title="${b.adaptive_enabled?n("cal_source_time_adaptive",e):n("cal_source_time",e)}"
                icon="${b.adaptive_enabled?"mdi:clock-time-four-outline":"mdi:clock-outline"}"></ha-icon>`,B=M&&b.prediction_confidence&&b.status!=="triggered"&&!b.projected?r`<span class="cal-event-prediction cal-conf-${b.prediction_confidence}">
            ${n("cal_predicted",e)} · ${n(`cal_confidence_${b.prediction_confidence}`,e)}
          </span>`:d,Q=this._stats?.budget?.currency_symbol||"\u20AC",D=b.history_type?n(b.history_type,e):n(b.status,e);return r`
        <div class="cal-event ${C}"
          @click=${()=>this._onEventClick(b)}>
          ${L}
          <span class="cal-status-pill ${N}">${D}</span>
          <div class="cal-event-body">
            <div class="cal-event-title">${b.object_name} · ${b.task_name}${E}</div>
            ${B}
            ${$}
          </div>
          ${b.avg_cost!=null&&b.avg_cost>0?r`<span class="cal-event-cost">${b.avg_cost.toFixed(0)} ${Q}</span>`:d}
        </div>
      `},k=b=>{let[N,C,E]=b.date.split("-").map(Number),$=new Date(N,C-1,E),M=b.date===h,L=$.toLocaleDateString(e,{weekday:"short"}),B=$.toLocaleDateString(e,{month:"long"});return r`
        <div class="cal-day-row">
          <div class="cal-day-pill ${M?"cal-today":""}">
            <span class="cal-pill-weekday">${L}</span>
            <span class="cal-pill-day">${$.getDate()}</span>
          </div>
          <div class="cal-day-content">
            <div class="cal-day-header">
              <span class="cal-day-month">${B}</span>
              ${M?r`<span class="cal-day-today-badge">${n("today",e)}</span>`:d}
            </div>
            ${b.events.length===0?r`<div class="cal-empty">${n("cal_no_events",e)}</div>`:b.events.map(f)}
          </div>
        </div>
      `};return r`
      <ha-card .header=${i}>
        ${t||a?r`
              <div class="cal-controls">
                ${t?r`
                      <div class="cal-window-chips cal-past-chips" title="${n("cal_past_windows",e)||"Past windows"}">
                        ${[30,90].map(b=>r`
                      ...
                      <div class="cal-window-chips" title="${n("cal_forward_windows",e)||"Forward windows"}">
                        ${[7,14,30,365].map(b=>r`
        ...
        <div class="cal-rolling">
          ${v.length===0&&m?r`<div class="cal-empty">${n("cal_no_events",e)}</div>`:v.map(k)}
        </div>
      </ha-card>
    `}};Y.styles=[Re,Ft,q`
      :host { display: block; }
      ha-card { padding: 0; overflow: hidden; }
    `],_([x({attribute:!1})],Y.prototype,"hass",2),_([g()],Y.prototype,"_config",2),_([g()],Y.prototype,"_objects",2),_([g()],Y.prototype,"_stats",2),_([g()],Y.prototype,"_windowDays",2),_([g()],Y.prototype,"_pastDays",2),_([g()],Y.prototype,"_userFilter",2),_([g()],Y.prototype,"_unsub",2);var an=[{value:7,label:"Week (7 days)"},{value:14,label:"Fortnight (14 days)"},{value:30,label:"Month (30 days, default)"},{value:365,label:"Year (365 days, empty days collapsed)"}],me=class extends T{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"}}setConfig(e){this._config={...e}}_valueChanged(e,t){let a={...this._config,[e]:t};e==="show_window_chips"&&t===!0&&delete a.show_window_chips,e==="show_user_filter"&&t===!0&&delete a.show_user_filter,e==="title"&&(!t||typeof t=="string"&&t.trim()==="")&&delete a.title,e==="user_filter"&&t===""&&delete a.user_filter,this._config=a,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:a},bubbles:!0,composed:!0}))}render(){let e=this._config.window_days??30,t=this._config.show_window_chips!==!1,a=this._config.show_user_filter!==!1,i=this._config.user_filter??"",o=this._config.title??"";return r`
      <div class="editor">
        <div class="row">
        ...
        <div class="row">
          ...
          >
            ${an.map(c=>r`<option value="${c.value}" ?selected=${c.value===e}>${c.label}</option>`)}
          </select>
        </div>
  ...
  `,_([x({attribute:!1})],me.prototype,"hass",2),_([g()],me.prototype,"_config",2);customElements.get("maintenance-supporter-calendar-card")||customElements.define("maintenance-supporter-calendar-card",Y);customElements.get("maintenance-supporter-calendar-card-editor")||customElements.define("maintenance-supporter-calendar-card-editor",me);var Le=window;Le.customCards=Le.customCards||[];var Lt="maintenance-supporter-calendar-card",nn=Le.customCards.some(l=>l.type===Lt);nn||Le.customCards.push({type:Lt,name:"Maintenance Supporter \u2014 Calendar",description:"Rolling calendar of maintenance tasks with 7/14/30/365 day windows, source icons, and prediction-confidence pills.",preview:!0});var ae=class extends T{constructor(){super(...arguments);this._open=!1;this._saving=!1;this._error="";this._draft=null;this._originalSnapshot=null}get _lang(){return this.hass?.language||"en"}openEdit(e){this._draft={...e},this._originalSnapshot={...e},this._error="",this._open=!0}close(){this._open=!1,this._error="",this._draft=null,this._originalSnapshot=null}_set(e,t){this._draft&&(this._draft={...this._draft,[e]:t})}async _save(){if(!(!this._draft||!this._originalSnapshot)){this._saving=!0,this._error="";try{let e={type:"maintenance_supporter/task/history/update",entry_id:this._draft.entry_id,task_id:this._draft.task_id,original_timestamp:this._originalSnapshot.original_timestamp};if(this._draft.timestamp!==this._originalSnapshot.timestamp&&(e.timestamp=this._draft.timestamp),this._draft.notes!==this._originalSnapshot.notes&&(e.notes=this._draft.notes),this._draft.cost!==this._originalSnapshot.cost&&(e.cost=this._draft.cost),this._draft.duration!==this._originalSnapshot.duration&&(e.duration=this._draft.duration),this._draft.completed_by!==this._originalSnapshot.completed_by&&(e.completed_by=this._draft.completed_by),Object.keys(e).filter(a=>!["type","entry_id","task_id","original_timestamp"].includes(a)).length===0){this.close();return}await this.hass.connection.sendMessagePromise(e),this.dispatchEvent(new CustomEvent("history-entry-saved",{detail:{entry_id:this._draft.entry_id,task_id:this._draft.task_id,new_timestamp:this._draft.timestamp},bubbles:!0,composed:!0})),this.close()}catch(e){this._error=W(e,this._lang)}finally{this._saving=!1}}}render(){if(!this._open||!this._draft)return d;let e=this._lang,t=this._draft;return r`
      <div class="backdrop" @click=${this.close}></div>
      <div class="dialog" role="dialog" aria-modal="true">
        <h2>${n("history_edit_title",e)||"Edit history entry"}</h2>
        <div class="entry-type">
          <ha-icon icon="mdi:tag-outline"></ha-icon>
          <span>${n(t.type,e)||t.type}</span>
        </div>
        <label>
          <span>${n("history_edit_timestamp",e)||"Timestamp"}</span>
          <input type="datetime-local"
            .value=${t.timestamp.length>=16?t.timestamp.slice(0,16):t.timestamp}
            @change=${a=>{let i=a.target.value;this._set("timestamp",i.length===16?`${i}:00`:i)}} />
        </label>
        <label>
          <span>${n("notes",e)||"Notes"}</span>
          <textarea
            rows="3"
            @input=${a=>{let i=a.target.value;this._set("notes",i||null)}}
            .value=${t.notes??""}></textarea>
        </label>
        <div class="row">
          <label>
            <span>${n("cost",e)||"Cost"}</span>
            <input type="number" min="0" step="0.01"
              .value=${t.cost!=null?String(t.cost):""}
              @input=${a=>{let i=a.target.value;this._set("cost",i?Number(i):null)}} />
          </label>
          <label>
            <span>${n("duration",e)||"Duration (min)"}</span>
            <input type="number" min="0"
              .value=${t.duration!=null?String(t.duration):""}
              @input=${a=>{let i=a.target.value;this._set("duration",i?Number(i):null)}} />
          </label>
        </div>
        ${this._error?r`<div class="error">${this._error}</div>`:d}
        <div class="actions">
          ...
          <button class="save" @click=${this._save} ?disabled=${this._saving}>
            ${this._saving?n("saving",e)||"Saving\u2026":n("save",e)||"Save"}
          </button>
        </div>
      </div>
    `}};ae.styles=q`
  ...
  `,_([x({attribute:!1})],G.prototype,"hass",2),_([g()],G.prototype,"_open",2),_([g()],G.prototype,"_title",2),_([g()],G.prototype,"_message",2),_([g()],G.prototype,"_confirmText",2),_([g()],G.prototype,"_danger",2),_([g()],G.prototype,"_inputLabel",2),_([g()],G.prototype,"_inputType",2),_([g()],G.prototype,"_inputValue",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",G);var Ht={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},Ot=l=>(...s)=>({_$litDirective$:l,values:s}),He=class{constructor(s){}get _$AU(){return this._$AM._$AU}_$AT(s,e,t){this._$Ct=s,this._$AM=e,this._$Ci=t}_$AS(s,e){return this.update(s,e)}update(s,e){return this.render(...e)}};var je=class extends He{constructor(s){if(super(s),this.it=d,s.type!==Ht.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(s){if(s===d||s==null)return this._t=void 0,this.it=s;if(s===te)return s;if(typeof s!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(s===this.it)return this._t;this.it=s;let e=[s];return e.raw=e,this._t={_$litType$:this.constructor.resultType,strings:e,values:[]}}};je.directiveName="unsafeHTML",je.resultType=1;var Ut=Ot(je);var sn=["EUR","USD","GBP","JPY","CHF","CAD","AUD","CNY","INR","BRL","CZK","PLN","RUB","SEK","NOK","DKK","UAH"],A=class extends T{constructor(){super(...arguments);this.budget=null;this._settings=null;this._loading=!0;this._importCsv="";this._importLoading=!1;this._includeHistory=!0;this._toast="";this._testingNotification=!1;this._users=[];this._vacEnabled=!1;this._vacStart="";this._vacEnd="";this._vacBuffer=3;this._vacExempt=new Set;this._vacIsActive=!1;this._vacWindowEnd=null;this._vacAllTasks=[];this._vacPreview=[];this._vacPreviewLoading=!1;this._vacSaving=!1;this._qrObjects=[];this._qrSelectedEntries=new Set;this._qrActions=new Set(["view"]);this._qrUrlMode="companion";this._qrBatchLoading=!1;this._qrBatchResults=[];this._qrObjectsLoaded=!1;this._loaded=!1;this._userService=null;this._sendTestNotification=async()=>{this._testingNotification=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/test_notification"}),t=e.message||(e.success?n("test_notification_success",this._lang):n("test_notification_failed",this._lang));this._showToast(t)}catch{this._showToast(n("test_notification_failed",this._lang))}finally{this._testingNotification=!1}}}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._loaded?(this._loaded=!0,this._userService=new oe(this.hass),this._loadSettings(),this._loadUsers()):e.has("hass")&&this.hass&&this._userService&&this._userService.updateHass(this.hass)}async _loadUsers(){if(this._userService)try{this._users=await this._userService.getUsers()}catch{this._users=[]}}async _loadSettings(){this._loading=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});this._settings=e,this._hydrateVacationFromSettings()}catch{}this._loading=!1}_hydrateVacationFromSettings(){let e=this._settings?.vacation;e&&(this._vacEnabled=e.enabled,this._vacStart=e.start||"",this._vacEnd=e.end||"",this._vacBuffer=e.buffer_days,this._vacExempt=new Set(e.exempt_task_ids||[]),this._vacIsActive=e.is_active,this._vacWindowEnd=e.window_end)}async _updateSetting(e,t){try{let a=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/update",settings:{[e]:t}});this._settings=a,this._showToast(n("settings_saved",this._lang)),this.dispatchEvent(new CustomEvent("settings-changed"))}catch{this._showToast(n("action_error",this._lang))}}_showToast(e){this._toast=e,setTimeout(()=>{this._toast=""},3e3)}_downloadFile(e,t,a){let i=new Blob([e],{type:a}),o=URL.createObjectURL(i),c=document.createElement("a");c.href=o,c.download=t,c.click(),URL.revokeObjectURL(o)}render(){let e=this._lang;return this._loading||!this._settings?r`<div class="settings-loading">Loading…</div>`:r`
    ...
    `}scrollToSection(e){requestAnimationFrame(()=>{let t=this.shadowRoot;if(!t)return;let a=t.querySelector(`[data-section="${e}"]`)??t.querySelector(`[data-section-alt="${e}"]`);a&&a.scrollIntoView({behavior:"smooth",block:"start"})})}_renderPanelAccess(e){let t=new Set(this._settings.admin_panel_user_ids||[]),a=this._users.filter(c=>!c.is_admin),i=this._settings.operator_write_enabled??!1,o=(c,u)=>{let p=new Set(t);u?p.add(c):p.delete(c),this._updateSetting("admin_panel_user_ids",[...p])};return r`
      <div class="settings-section">
        <h3>${n("settings_panel_access",e)} ${i&&t.size>0?r`<span class="section-badge">${t.size}</span>`:d}</h3>
        <p class="section-desc">${n("settings_panel_access_desc",e)}</p>
        <label class="setting-row">
          <span>
            <span class="setting-label">${n("settings_operator_write",e)}</span>
            <span class="setting-desc">${n("settings_operator_write_desc",e)}</span>
          </span>
          <input type="checkbox"
            .checked=${i}
            @change=${c=>this._updateSetting("operator_write_enabled",c.target.checked)} />
        </label>
        ${i?a.length===0?r`<div class="setting-row hint">${n("no_non_admin_users",e)}</div>`:a.map(c=>r`
              <label class="setting-row">
                <span>
                  <span class="setting-label">${c.name||c.id.slice(0,8)}</span>
                  <span class="setting-desc">${c.is_owner?n("owner_label",e):""}</span>
                </span>
                <input type="checkbox"
                  .checked=${t.has(c.id)}
                  @change=${u=>o(c.id,u.target.checked)} />
              </label>
            `):d}
      </div>
    `}_renderFeatures(e){let t=this._settings.features,a=[{key:"adaptive",settingKey:"advanced_adaptive_visible",label:n("feat_adaptive",e),desc:n("feat_adaptive_desc",e)},{key:"predictions",settingKey:"advanced_predictions_visible",label:n("feat_predictions",e),desc:n("feat_predictions_desc",e)},{key:"seasonal",settingKey:"advanced_seasonal_visible",label:n("feat_seasonal",e),desc:n("feat_seasonal_desc",e)},{key:"environmental",settingKey:"advanced_environmental_visible",label:n("feat_environmental",e),desc:n("feat_environmental_desc",e)},{key:"budget",settingKey:"advanced_budget_visible",label:n("feat_budget",e),desc:n("feat_budget_desc",e)},{key:"groups",settingKey:"advanced_groups_visible",label:n("feat_groups",e),desc:n("feat_groups_desc",e)},{key:"checklists",settingKey:"advanced_checklists_visible",label:n("feat_checklists",e),desc:n("feat_checklists_desc",e)},{key:"schedule_time",settingKey:"advanced_schedule_time_visible",label:n("feat_schedule_time",e),desc:n("feat_schedule_time_desc",e)},{key:"completion_actions",settingKey:"advanced_completion_actions_visible",label:n("feat_completion_actions",e),desc:n("feat_completion_actions_desc",e)}];return r`
      <div class="settings-section" data-section="settings" data-section-alt="groups">
        <h3>${n("settings_features",e)}</h3>
        <p class="section-desc">${n("settings_features_desc",e)}</p>
        ${a.map(i=>r`
          <label class="setting-row">
            <span>
            ...
            <input type="checkbox" .checked=${t[i.key]}
              @change=${o=>this._updateSetting(i.settingKey,o.target.checked)} />
          </label>
        `)}
      </div>
    `}_renderObjectsColumns(e){let t=$e(this._settings.objects_table_columns);return r`
      <div class="settings-section" data-section="objects_table_columns">
        <h3>${n("objects_table_columns_label",e)}</h3>
        <p class="section-desc">${n("objects_table_columns_hint",e)}</p>
        ${pe.map(a=>r`
          <label class="setting-row">
            <span class="setting-label">${n(a.labelKey,e)}</span>
            <input
              type="checkbox"
    ...
    `}_toggleColumn(e,t){let a=new Set($e(this._settings.objects_table_columns));t?a.add(e):a.delete(e);let i=pe.filter(o=>o.required||a.has(o.key)).map(o=>o.key);this._updateSetting("objects_table_columns",i)}_renderGeneral(e){let t=this._settings.general,a=this._settings.budget;return r`
      <div class="settings-section">
        <h3>${n("settings_general",e)}</h3>
        <label class="setting-row">
          <span class="setting-label">${n("settings_default_warning",e)}</span>
          <input type="number" min="1" max="365" .value=${String(t.default_warning_days)}
            @change=${i=>{let o=parseInt(i.target.value,10);o>=1&&o<=365&&this._updateSetting("default_warning_days",o)}} />
        </label>
        <label class="setting-row">
          <span class="setting-label">${n("settings_currency",e)}</span>
          <select @change=${i=>this._updateSetting("budget_currency",i.target.value)}>
            ${sn.map(i=>r`<option value=${i} ?selected=${a.currency===i}>${i}</option>`)}
          </select>
        </label>
        <label class="setting-row">
          <span class="setting-label">${n("settings_panel_enabled",e)}</span>
          <input type="checkbox" .checked=${t.panel_enabled}
            @change=${i=>this._updateSetting("panel_enabled",i.target.checked)} />
        </label>
        ${t.panel_enabled?r`
          <label class="setting-row">
            <span class="setting-label">${n("settings_panel_title",e)}</span>
            <input type="text" .value=${t.panel_title??""}
              placeholder="Maintenance"
              maxlength="50"
              @change=${i=>this._updateSetting("panel_title",i.target.value.trim())} />
          </label>
        `:""}
        <label class="setting-row">
          <span class="setting-label">${n("settings_notifications",e)}</span>
    ...
    `}_renderNotifications(e){let t=this._settings.notifications,a=this._settings.actions;return r`
      <div class="settings-section">
        <h3>${n("settings_notifications",e)}</h3>

        <label class="setting-row">
          <span>
            <span class="setting-label">${n("settings_notify_due_soon",e)}</span>
          </span>
        ...
        ${t.due_soon_enabled?r`
          <label class="setting-row sub-row">
            <span class="setting-desc">${n("settings_interval_hours",e)}</span>
        ...
        <label class="setting-row">
          <span>
            <span class="setting-label">${n("settings_notify_overdue",e)}</span>
          </span>
        ...
        <label class="setting-row">
          <span>
            <span class="setting-label">${n("settings_notify_triggered",e)}</span>
          </span>
        ...
        <label class="setting-row">
          <span class="setting-label">${n("settings_quiet_hours",e)}</span>
        ...
        ${t.quiet_hours_enabled?r`
          <div class="setting-row sub-row">
            <span class="setting-desc">${n("settings_quiet_start",e)}</span>
            <input type="time" .value=${t.quiet_hours_start}
              @change=${i=>this._updateSetting("quiet_hours_start",i.target.value)} />
          </div>
          <div class="setting-row sub-row">
            <span class="setting-desc">${n("settings_quiet_end",e)}</span>
        ...
        <label class="setting-row">
          <span class="setting-label">${n("settings_max_per_day",e)}</span>
        ...
        <label class="setting-row">
          <span class="setting-label">${n("settings_bundling",e)}</span>
        ...
        ${t.bundling_enabled?r`
          <label class="setting-row sub-row">
            <span class="setting-desc">${n("settings_bundle_threshold",e)}</span>
    ...
    `}_renderBudget(e){let t=this._settings.budget;return r`
      <div class="settings-section" data-section="budget">
        <h3>${n("settings_budget",e)}</h3>
        <label class="setting-row">
          <span class="setting-label">${n("settings_budget_monthly",e)}</span>
        ...
        <label class="setting-row">
          <span class="setting-label">${n("settings_budget_yearly",e)}</span>
        ...
        <label class="setting-row">
          <span class="setting-label">${n("settings_budget_alerts",e)}</span>
          <input type="checkbox" .checked=${t.alerts_enabled}
            @change=${a=>this._updateSetting("budget_alerts_enabled",a.target.checked)} />
        </label>
        ${t.alerts_enabled?r`
          <label class="setting-row sub-row">
            <span class="setting-desc">${n("settings_budget_threshold",e)}</span>
    ...
    `}_renderVacation(e){let t=this._vacEnabled&&!this._vacIsActive&&this._vacWindowEnd&&new Date(this._vacWindowEnd)<new Date,a=this._vacExempt.size;return r`
      <div class="settings-section vacation-section" data-section="vacation">
        <h3>
          ${n("vacation_title",e)}
          ${this._vacIsActive?r`<span class="vac-badge active">${n("vacation_active",e)}</span>`:d}
          ${t?r`<span class="vac-badge stale">${n("vacation_ended",e)}</span>`:d}
        </h3>
        <p class="section-desc">${n("vacation_desc",e)}</p>
        ...
        <details class="vac-exempt-panel">
          <summary>
            ${n("vacation_exempt_title",e)}
            ${a>0?r`<span class="section-badge">${a}</span>`:d}
          </summary>
          <p class="section-desc">${n("vacation_exempt_desc",e)}</p>
          ${this._vacAllTasks.length===0?r`<button @click=${this._loadAllTasksForVacation}>${n("vacation_load_tasks",e)}</button>`:r`
        ...
        ${this._vacStart&&this._vacEnd?r`
          <div class="vac-preview-toolbar">
            <button @click=${this._loadVacationPreview} ?disabled=${this._vacPreviewLoading}>
              ${this._vacPreviewLoading?"\u2026":n("vacation_preview_btn",e)}
            </button>
            ${this._vacPreview.length>0?r`<span class="vac-preview-count">${this._vacPreview.length} ${n("vacation_preview_affected",e)}</span>`:d}
          </div>
        ...
        ${this._vacIsActive||t?r`<button class="vac-end-now" @click=${this._endVacationNow}>
              ${n("vacation_end_now",e)}
            </button>`:d}
      </div>
    `}_renderVacationTaskList(e){let t=new Map;for(let i of this._vacAllTasks){let o=t.get(i.object_name)||[];o.push(i),t.set(i.object_name,o)}return[...t.entries()].sort(([i],[o])=>i.localeCompare(o)).map(([i,o])=>r`
      <div class="vac-task-group">
        <div class="vac-task-group-name">${i||n("no_objects",e)}</div>
        ${o.sort((c,u)=>c.task_name.localeCompare(u.task_name)).map(c=>r`
    ...
    `)}_renderVacationPreview(e){return r`
      <div class="vac-preview-list">
        ${this._vacPreview.map(t=>{let a=t.events.map(o=>{let c=`vacation_event_${o.status}`;return`${o.date} (${n(c,e)})`}).join(" \xB7 "),i=!t.will_suppress;return r`
            <div class="vac-preview-row ${i?"exempt":""}">
              <div class="vac-preview-info">
                <div class="vac-preview-name">
                  ...
                  ${t.kind==="sensor_based"?r`<span class="vac-preview-kind">${n("vacation_sensor_based",e)}</span>`:d}
                </div>
                <div class="vac-preview-events">${a}</div>
              </div>
              <div class="vac-preview-actions">
                <button @click=${()=>this._previewActionComplete(t)}>${n("qr_action_complete",e)}</button>
                ${t.kind==="time_based"?r`<button @click=${()=>this._previewActionSkip(t)}>${n("qr_action_skip",e)}</button>`:d}
                <button class=${i?"vac-notify-on":""}
                  @click=${()=>this._toggleVacationExempt(t.task_id,!i)}>
                  ${i?n("vacation_action_unsilence",e):n("vacation_action_notify",e)}
                </button>
              </div>
            </div>
          `})}
      </div>
    `}async _loadAllTasksForVacation(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),t=[];for(let a of e.objects||[])for(let i of a.tasks||[])t.push({entry_id:a.entry_id,object_name:a.object.name||"",task_id:i.id,task_name:i.name||""});this._vacAllTasks=t}catch{this._showToast(n("action_error",this._lang))}}async _saveVacation(e){if(!this._vacSaving){this._vacSaving=!0;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/vacation/update",...e});this._vacEnabled=t.enabled,this._vacStart=t.start||"",this._vacEnd=t.end||"",this._vacBuffer=t.buffer_days,this._vacExempt=new Set(t.exempt_task_ids||[]),this._vacIsActive=t.is_active,this._vacWindowEnd=t.window_end,this.dispatchEvent(new CustomEvent("settings-changed"))}catch(t){let a=t?.message||n("action_error",this._lang);this._showToast(a)}finally{this._vacSaving=!1}}}_toggleVacationEnabled(e){this._saveVacation({enabled:e})}_setVacationDate(e,t){let a={};a[e]=t||null,this._saveVacation(a)}_setVacationBuffer(e){e<0||e>14||this._saveVacation({buffer_days:e})}_toggleVacationExempt(e,t){let a=new Set(this._vacExempt);t?a.add(e):a.delete(e),this._saveVacation({exempt_task_ids:[...a]})}async _loadVacationPreview(){this._vacPreviewLoading=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/vacation/preview"});this._vacPreview=e.rows||[]}catch{this._showToast(n("action_error",this._lang))}finally{this._vacPreviewLoading=!1}}async _previewActionComplete(e){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/complete",entry_id:e.entry_id,task_id:e.task_id}),this._showToast(n("vacation_marked_complete",this._lang)),await this._loadVacationPreview()}catch{this._showToast(n("action_error",this._lang))}}async _previewActionSkip(e){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/skip",entry_id:e.entry_id,task_id:e.task_id,reason:"Skipped before vacation"}),this._showToast(n("vacation_marked_skip",this._lang)),await this._loadVacationPreview()}catch{this._showToast(n("action_error",this._lang))}}async _endVacationNow(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/vacation/end_now"});this._vacEnabled=e.enabled,this._vacEnd=e.end||"",this._vacIsActive=e.is_active,this._vacWindowEnd=e.window_end,this.dispatchEvent(new CustomEvent("settings-changed")),this._showToast(n("vacation_ended",this._lang))}catch{this._showToast(n("action_error",this._lang))}}_renderPrintQr(e){let t=this._qrSelectedEntries.size||this._qrObjects.length,a=this._qrActions.size,i=t*a,o=i>200;return r`
      <div class="settings-section qr-print-section">
        <h3>${n("qr_print_title",e)}</h3>
        <p class="section-desc">${n("qr_print_desc",e)}</p>

        ${this._qrObjectsLoaded?r`
            <details open class="qr-filter-panel">
              <summary>${n("qr_print_filter",e)}</summary>

              <div class="qr-filter-group">
                <div class="qr-filter-label">${n("qr_print_objects",e)}</div>
                <div class="qr-object-list">
                  ${this._qrObjects.length===0?r`<div class="hint">${n("no_objects",e)}</div>`:this._qrObjects.map(c=>r`
              ...
              <div class="qr-filter-group">
                <div class="qr-filter-label">${n("qr_print_actions",e)}</div>
              ...
              <div class="qr-filter-group">
                <div class="qr-filter-label">${n("qr_print_url_mode",e)}</div>
                <select .value=${this._qrUrlMode}
                  @change=${c=>{this._qrUrlMode=c.target.value}}>
                  <option value="companion">${n("qr_mode_companion",e)}</option>
                  <option value="local">${n("qr_mode_local",e)}</option>
                  <option value="server">${n("qr_mode_server",e)}</option>
                </select>
              </div>

              <div class="qr-filter-group qr-filter-actions">
                <div class="qr-estimate ${o?"error":""}">
                  ${n("qr_print_estimate",e)}: <strong>${i}</strong>
                  ${o?r` — ${n("qr_print_over_limit",e)}`:d}
                </div>
                <button
                  ?disabled=${this._qrBatchLoading||o||a===0}
                  @click=${this._generateBatch}>
                  ${this._qrBatchLoading?n("qr_print_generating",e):n("qr_print_generate",e)}
                </button>
              </div>
            </details>
            ...
            ${this._qrBatchResults.length>0?r`
                <div class="qr-results-toolbar">
                  <span>${this._qrBatchResults.length} ${n("qr_print_ready",e)}</span>
                  <button @click=${this._printQrs}>${n("qr_print_print_button",e)}</button>
                </div>
                <div class="qr-print-grid">
                  ${this._qrBatchResults.map(c=>r`
                    <div class="qr-print-cell">
                      <div class="qr-svg">${Ut(c.svg)}</div>
                      <div class="qr-label">
                        <div class="qr-label-obj">${c.object_name}</div>
                        <div class="qr-label-task">${c.task_name}</div>
                        <div class="qr-label-action">${n("qr_action_"+c.action,e)}</div>
                      </div>
                    </div>
                  `)}
                </div>
              `:d}
          `:r`<button @click=${this._loadQrObjects}>${n("qr_print_load",e)}</button>`}
      </div>
    `}async _loadQrObjects(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"});this._qrObjects=(e.objects||[]).map(t=>({entry_id:t.entry_id,name:t.object.name,task_count:(t.tasks||[]).length})).sort((t,a)=>t.name.localeCompare(a.name)),this._qrObjectsLoaded=!0}catch{this._showToast(n("action_error",this._lang))}}_toggleQrObject(e,t){let a=new Set(this._qrSelectedEntries);if(a.size===0)for(let i of this._qrObjects)a.add(i.entry_id);t?a.add(e):a.delete(e),a.size===this._qrObjects.length&&a.clear(),this._qrSelectedEntries=a}_toggleQrAction(e,t){let a=new Set(this._qrActions);t?a.add(e):a.delete(e),this._qrActions=a}async _generateBatch(){this._qrBatchLoading=!0,this._qrBatchResults=[];try{let e={type:"maintenance_supporter/qr/batch_generate",actions:[...this._qrActions],url_mode:this._qrUrlMode};this._qrSelectedEntries.size>0&&(e.entry_ids=[...this._qrSelectedEntries]);let t=await this.hass.connection.sendMessagePromise(e);this._qrBatchResults=t.qrs||[],this._qrBatchResults.length===0&&this._showToast(n("qr_print_empty",this._lang))}catch(e){let t=e?.message||n("action_error",this._lang);this._showToast(t)}finally{this._qrBatchLoading=!1}}_printQrs(){if(this._qrBatchResults.length===0)return;let e=this._lang,t=this._qrBatchResults.map(c=>{let u=n("qr_action_"+c.action,e);return`
        <div class="cell">
        ...
        </div>`}).join(""),a=n("qr_print_title",e),i=`<!DOCTYPE html>
...
<body>
  <div class="toolbar">
    ...
    <button onclick="window.print()">${this._escapeHtml(n("qr_print_print_button",e))}</button>
  </div>
  <div class="grid">${t}</div>
  <script>window.addEventListener("load", function () { setTimeout(function () { window.print(); }, 250); });<\/script>
</body>
</html>`,o=window.open("","_blank","width=900,height=1100");if(!o){window.print();return}o.document.open(),o.document.write(i),o.document.close()}_escapeHtml(e){return e.replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}_renderImportExport(e){return r`
      <div class="settings-section">
        <h3>${n("settings_import_export",e)}</h3>
        <div class="settings-actions">
          <label class="export-history-toggle">
            ...
            ${n("settings_include_history",e)}
          </label>
        </div>
        <div class="settings-actions">
          <button @click=${this._exportJson}>${n("settings_export_json",e)}</button>
          <button @click=${this._exportYaml}>${n("settings_export_yaml",e)}</button>
          <button @click=${this._exportCsv}>${n("settings_export_csv",e)}</button>
        </div>
        <div class="import-section">
          <textarea class="import-area" .value=${this._importCsv}
            placeholder=${n("settings_import_placeholder",e)}
          ...
          <div class="settings-actions">
            <button ?disabled=${!this._importCsv.trim()||this._importLoading}
              ...
              ${this._importLoading?"\u2026":n("settings_import_btn",e)}
            </button>
          </div>
        </div>
      </div>
    `}async _exportJson(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/export",format:"json",include_history:this._includeHistory}),t=new Date().toISOString().slice(0,10);this._downloadFile(e.data,`maintenance_export_${t}.json`,"application/json"),this._showToast(n("settings_export_success",this._lang))}catch{this._showToast(n("action_error",this._lang))}}async _exportYaml(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/export",format:"yaml",include_history:this._includeHistory}),t=new Date().toISOString().slice(0,10);this._downloadFile(e.data,`maintenance_export_${t}.yaml`,"application/yaml"),this._showToast(n("settings_export_success",this._lang))}catch{this._showToast(n("action_error",this._lang))}}async _exportCsv(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/csv/export"}),t=new Date().toISOString().slice(0,10);this._downloadFile(e.csv,`maintenance_export_${t}.csv`,"text/csv"),this._showToast(n("settings_export_success",this._lang))}catch{this._showToast(n("action_error",this._lang))}}async _importCsvAction(){let e=this._importCsv.trim();if(e){this._importLoading=!0;try{let t=e.startsWith("object_name"),i=(await this.hass.connection.sendMessagePromise(t?{type:"maintenance_supporter/csv/import",csv_content:e}:{type:"maintenance_supporter/json/import",json_content:e})).created??0;this._showToast(n("settings_import_success",this._lang).replace("{count}",String(i))),this._importCsv="",this.dispatchEvent(new CustomEvent("settings-changed"))}catch{this._showToast(n("action_error",this._lang))}this._importLoading=!1}}};A.styles=q`
    :host { display: block; }
  ...
  `,_([x({attribute:!1})],A.prototype,"hass",2),_([x({attribute:!1})],A.prototype,"features",2),_([x({attribute:!1})],A.prototype,"budget",2),_([g()],A.prototype,"_settings",2),_([g()],A.prototype,"_loading",2),_([g()],A.prototype,"_importCsv",2),_([g()],A.prototype,"_importLoading",2),_([g()],A.prototype,"_includeHistory",2),_([g()],A.prototype,"_toast",2),_([g()],A.prototype,"_testingNotification",2),_([g()],A.prototype,"_users",2),_([g()],A.prototype,"_vacEnabled",2),_([g()],A.prototype,"_vacStart",2),_([g()],A.prototype,"_vacEnd",2),_([g()],A.prototype,"_vacBuffer",2),_([g()],A.prototype,"_vacExempt",2),_([g()],A.prototype,"_vacIsActive",2),_([g()],A.prototype,"_vacWindowEnd",2),_([g()],A.prototype,"_vacAllTasks",2),_([g()],A.prototype,"_vacPreview",2),_([g()],A.prototype,"_vacPreviewLoading",2),_([g()],A.prototype,"_vacSaving",2),_([g()],A.prototype,"_qrObjects",2),_([g()],A.prototype,"_qrSelectedEntries",2),_([g()],A.prototype,"_qrActions",2),_([g()],A.prototype,"_qrUrlMode",2),_([g()],A.prototype,"_qrBatchLoading",2),_([g()],A.prototype,"_qrBatchResults",2),_([g()],A.prototype,"_qrObjectsLoaded",2);customElements.define("maintenance-settings-view",A);var on=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"],X=class extends T{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._entryId="";this._taskId="";this._values=new Array(12).fill("");this._save=async()=>{let e=this._buildOverrides();if(e!==null){this._loading=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/seasonal_overrides",entry_id:this._entryId,task_id:this._taskId,overrides:e}),this._open=!1,this.dispatchEvent(new CustomEvent("overrides-saved"))}catch(t){this._error=W(t,this._lang,n("save_error",this._lang))}finally{this._loading=!1}}};this._clearAll=async()=>{this._loading=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/seasonal_overrides",entry_id:this._entryId,task_id:this._taskId,overrides:{}}),this._values=new Array(12).fill(""),this._open=!1,this.dispatchEvent(new CustomEvent("overrides-saved"))}catch(e){this._error=W(e,this._lang,n("save_error",this._lang))}finally{this._loading=!1}}}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}open(e,t,a){if(this._entryId=e,this._taskId=t,this._values=new Array(12).fill(""),a)for(let[i,o]of Object.entries(a)){let c=parseInt(i,10);c>=1&&c<=12&&typeof o=="number"&&(this._values[c-1]=o.toString())}this._error="",this._open=!0}_close(){this._open=!1}_buildOverrides(){let e={};for(let t=0;t<12;t++){let a=this._values[t].trim();if(!a)continue;let i=parseFloat(a);if(Number.isNaN(i))return this._error=`${n("month_"+["jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec"][t],this._lang)}: ${n("seasonal_override_invalid",this._lang)}`,null;if(i<.1||i>5)return this._error=n("seasonal_override_range",this._lang),null;e[t+1]=i}return e}render(){if(!this._open)return r``;let e=this._lang;return r`
      <ha-dialog open @closed=${this._close} heading="${n("seasonal_overrides_title",e)}">
        <div class="content">
          <p class="hint">${n("seasonal_overrides_hint",e)}</p>
          ${this._error?r`<div class="error">${this._error}</div>`:d}
          <div class="months">
            ${on.map((t,a)=>r`
              <label class="month">
                <span class="mn">${n(t,e)}</span>
                <input type="number" step="0.1" min="0.1" max="5.0"
                  placeholder="1.0"
                  .value=${this._values[a]}
                  @input=${i=>{let o=[...this._values];o[a]=i.target.value,this._values=o}} />
              </label>
            `)}
          </div>
        </div>
    ...
    .spacer { flex: 1; }
  `,_([x({attribute:!1})],X.prototype,"hass",2),_([g()],X.prototype,"_open",2),_([g()],X.prototype,"_loading",2),_([g()],X.prototype,"_error",2),_([g()],X.prototype,"_entryId",2),_([g()],X.prototype,"_taskId",2),_([g()],X.prototype,"_values",2);customElements.get("maintenance-seasonal-overrides-dialog")||customElements.define("maintenance-seasonal-overrides-dialog",X);var K=class extends T{constructor(){super(...arguments);this.objects=[];this._open=!1;this._loading=!1;this._error="";this._groupId=null;this._name="";this._description="";this._selected=new Set;this._toggleTask=(e,t)=>{let a=`${e}:${t}`,i=new Set(this._selected);i.has(a)?i.delete(a):i.add(a),this._selected=i};this._save=async()=>{let e=this._name.trim();if(!e){this._error=n("group_name_required",this._lang);return}this._loading=!0,this._error="";try{let t=this._buildTaskRefs();this._groupId?await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/group/update",group_id:this._groupId,name:e,description:this._description,task_refs:t}):await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/group/create",name:e,description:this._description,task_refs:t}),this._open=!1,this.dispatchEvent(new CustomEvent("group-saved"))}catch(t){this._error=W(t,this._lang,n("save_error",this._lang))}finally{this._loading=!1}}}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}openCreate(){this._reset(),this._open=!0}openEdit(e,t){this._reset(),this._groupId=e,this._name=t.name,this._description=t.description||"",this._selected=new Set(t.task_refs.map(a=>`${a.entry_id}:${a.task_id}`)),this._open=!0}_reset(){this._groupId=null,this._name="",this._description="",this._selected=new Set,this._error=""}_close(){this._open=!1}_buildTaskRefs(){return[...this._selected].map(e=>{let[t,a]=e.split(":",2);return{entry_id:t,task_id:a}})}render(){if(!this._open)return r``;let e=this._lang,t=this._groupId?n("edit_group",e):n("new_group",e);return r`
      <ha-dialog open @closed=${this._close} heading="${t}">
        <div class="content">
          ...
          ></ms-textfield>
          <ms-textfield
            label="${n("description_optional",e)}"
            .value=${this._description}
            @input=${a=>this._description=a.target.value}
          ></ms-textfield>

          <div class="section-title">${n("group_select_tasks",e)}</div>
          ${this.objects.length===0?r`<div class="hint">${n("no_objects",e)}</div>`:r`
              <div class="objects">
                ${[...this.objects].sort((a,i)=>a.object.name.localeCompare(i.object.name)).map(a=>r`
                  <div class="object-block">
                    <div class="object-name">${a.object.name}</div>
                    ${a.tasks.length===0?r`<div class="hint small">${n("no_tasks_short",e)}</div>`:[...a.tasks].sort((i,o)=>i.name.localeCompare(o.name)).map(i=>{let o=`${a.entry_id}:${i.id}`,c=this._selected.has(o);return r`
                          <label class="task-row">
                            <input type="checkbox"
                              .checked=${c}
                              @change=${()=>this._toggleTask(a.entry_id,i.id)} />
                            <span>${i.name}</span>
                          </label>
                        `})}
                  </div>
          ...
          <div class="selected-count">
            ${n("selected",e)}: ${this._selected.size}
          </div>
        </div>
        <div class="dialog-actions">
          <ha-button appearance="plain" @click=${this._close}>
  ...
  `,_([x({attribute:!1})],K.prototype,"hass",2),_([x({attribute:!1})],K.prototype,"objects",2),_([g()],K.prototype,"_open",2),_([g()],K.prototype,"_loading",2),_([g()],K.prototype,"_error",2),_([g()],K.prototype,"_groupId",2),_([g()],K.prototype,"_name",2),_([g()],K.prototype,"_description",2),_([g()],K.prototype,"_selected",2);customElements.get("maintenance-group-dialog")||customElements.define("maintenance-group-dialog",K);var rn=300,ln=140,rt=27;function Bt(l,s){let e=l.trigger_config;if(!e)return d;let t=s.lang,a=l.trigger_entity_info,i=l.trigger_entity_infos,o=a?.friendly_name||e.entity_id||"\u2014",c=e.entity_id||"",u=e.entity_ids||(c?[c]:[]),p=a?.unit_of_measurement||"",h=l.trigger_current_value,m=e.type||"threshold",v=u.length>1;return r`
    <h3>${n("trigger",t)}</h3>
    <div class="trigger-card">
      <div class="trigger-header">
        ...
        <div>
          ${v?r`
            <div class="trigger-entity-name">${u.length} ${n("entities",t)} (${e.entity_logic||"any"})</div>
            <div class="trigger-entity-id">${u.map((f,k)=>r`${k>0?", ":""}<span class="entity-link" @click=${b=>de(b,f)}>${f}</span>`)}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `:r`
            <div class="trigger-entity-name">${o}</div>
            <div class="trigger-entity-id">${c?r`<span class="entity-link" @click=${f=>de(f,c)}>${c}</span>`:""}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `}
        </div>
        <span class="status-badge ${l.trigger_active?"triggered":"ok"}" style="margin-left: auto;">
          ${l.trigger_active?n("triggered",t):n("ok",t)}
        </span>
      </div>

      ${h!=null?r`
            <div class="trigger-value-row">
              <span class="trigger-current ${l.trigger_active?"active":""}">${typeof h=="number"?h.toFixed(1):h}</span>
      ...
      <div class="trigger-limits">
        ${m==="threshold"?r`
          ${e.trigger_above!=null?r`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${n("threshold_above",t)}: ${e.trigger_above} ${p}</span>`:d}
          ${e.trigger_below!=null?r`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${n("threshold_below",t)}: ${e.trigger_below} ${p}</span>`:d}
          ${e.trigger_for_minutes?r`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${n("for_minutes",t)}: ${e.trigger_for_minutes}</span>`:d}
        `:d}
        ${m==="counter"?r`
          ${e.trigger_target_value!=null?r`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${n("target_value",t)}: ${e.trigger_target_value} ${p}</span>`:d}
        `:d}
        ${m==="state_change"?r`
          ${e.trigger_target_changes!=null?r`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${n("target_changes",t)}: ${e.trigger_target_changes}</span>`:d}
        `:d}
        ${m==="runtime"?r`
          ${e.trigger_runtime_hours!=null?r`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${n("runtime_hours",t)}: ${e.trigger_runtime_hours}h</span>`:d}
        `:d}
        ${m==="compound"?r`
          ...
          ${(e.conditions||[]).map((f,k)=>r`
            <span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${k+1}. ${n(f.type||"unknown",t)}: ${f.entity_id?r`<span class="entity-link" @click=${b=>de(b,f.entity_id)}>${f.entity_id}</span>`:""}</span>
          `)}
        `:d}
        ${a?.min!=null?r`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${n("min",t)}: ${a.min} ${p}</span>`:d}
        ${a?.max!=null?r`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${n("max",t)}: ${a.max} ${p}</span>`:d}
      </div>

      ${i&&i.length>1?r`
        <div class="trigger-entity-list">
          ${i.map(f=>r`
            <span class="trigger-entity-id">${f.friendly_name} (<span class="entity-link" @click=${k=>de(k,f.entity_id)}>${f.entity_id}</span>)</span>
          `)}
        </div>
      `:d}
  ...
  `}function cn(l,s,e){let t=l.trigger_config;if(!t)return d;let a=t.type||"threshold",i=a==="counter"&&t.trigger_delta_mode,o=e.isCounterEntity(t),c=t.entity_id||"",u=e.detailStatsData.get(c)||[],p=[],h=!1;if(u.length>=2)for(let y of u){let S=y.val;i&&l.trigger_baseline_value!=null&&(S-=l.trigger_baseline_value);let H={ts:y.ts,val:S};!o&&y.min!=null&&y.max!=null&&(H.min=i&&l.trigger_baseline_value!=null?y.min-l.trigger_baseline_value:y.min,H.max=i&&l.trigger_baseline_value!=null?y.max-l.trigger_baseline_value:y.max,h=!0),p.push(H)}else for(let y of l.history)y.trigger_value!=null&&p.push({ts:new Date(y.timestamp).getTime(),val:y.trigger_value});if(l.trigger_current_value!=null){let y=l.trigger_current_value;i&&l.trigger_baseline_value!=null&&(y-=l.trigger_baseline_value),p.push({ts:Date.now(),val:y})}if(p.length<2&&c&&e.hasStatsService&&!e.detailStatsData.has(c))return r`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
      <ha-icon icon="mdi:chart-line" style="--mdc-icon-size:16px;margin-right:8px;"></ha-icon>
      ${n("loading_chart",e.lang)}
    </div>`;if(p.length<2)return d;p.sort((y,S)=>y.ts-S.ts);let m=rn,v=ln,f=30,k=2,b=8,N=16,C=p.map(y=>y.val),E=Math.min(...C),$=Math.max(...C);if(h)for(let y of p)y.min!=null&&(E=Math.min(E,y.min)),y.max!=null&&($=Math.max($,y.max));t.trigger_above!=null&&($=Math.max($,t.trigger_above),E=Math.min(E,t.trigger_above)),t.trigger_below!=null&&(E=Math.min(E,t.trigger_below),$=Math.max($,t.trigger_below));let M=null,L=null;if(a==="counter"&&t.trigger_target_value!=null){if(l.trigger_baseline_value!=null)M=l.trigger_baseline_value;else if(p.length>0){let y=[...l.history].filter(S=>S.type==="completed"||S.type==="reset").sort((S,H)=>new Date(H.timestamp).getTime()-new Date(S.timestamp).getTime())[0];if(y){let S=new Date(y.timestamp).getTime(),H=p[0],U=Math.abs(p[0].ts-S);for(let ne of p){let ie=Math.abs(ne.ts-S);ie<U&&(H=ne,U=ie)}M=H.val}else M=p[0].val}M!=null?(L=M+t.trigger_target_value,$=Math.max($,L),E=Math.min(E,M)):($=Math.max($,t.trigger_target_value),E=Math.min(E,0))}i&&l.trigger_baseline_value!=null&&(E=Math.min(E,0));let B=$-E||1;E-=B*.1,$+=B*.1;let Q=p[0].ts,D=p[p.length-1].ts,z=D-Q||1,I=y=>f+(y-Q)/z*(m-f-k),R=y=>b+(1-(y-E)/($-E))*(v-b-N),Oe=p.map(y=>`${I(y.ts).toFixed(1)},${R(y.val).toFixed(1)}`).join(" "),ea=`M${I(p[0].ts).toFixed(1)},${v-N} `+p.map(y=>`L${I(y.ts).toFixed(1)},${R(y.val).toFixed(1)}`).join(" ")+` L${I(p[p.length-1].ts).toFixed(1)},${v-N} Z`,Ue="";if(h){let y=p.filter(S=>S.min!=null&&S.max!=null);if(y.length>=2){let S=y.map(U=>`${I(U.ts).toFixed(1)},${R(U.max).toFixed(1)}`),H=[...y].reverse().map(U=>`${I(U.ts).toFixed(1)},${R(U.min).toFixed(1)}`);Ue=`M${S[0]} `+S.slice(1).map(U=>`L${U}`).join(" ")+` L${H[0]} `+H.slice(1).map(U=>`L${U}`).join(" ")+" Z"}}let lt=p[p.length-1],ta=I(lt.ts),aa=R(lt.val),ct=y=>Math.abs(y)>=1e4?(y/1e3).toFixed(0)+"k":y>=1e3?(y/1e3).toFixed(1)+"k":y.toFixed(y<10?1:0),na=ct($),ia=ct(E),sa=l.history.filter(y=>["completed","skipped","reset"].includes(y.type)).map(y=>({ts:new Date(y.timestamp).getTime(),type:y.type})).filter(y=>y.ts>=Q&&y.ts<=D),Ve=p;if(p.length>rt){let y=(p.length-1)/(rt-1);Ve=[];for(let S=0;S<rt;S++)Ve.push(p[Math.round(S*y)])}return r`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${m} ${v}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_sparkline",e.lang)}">
        <text x="${f-3}" y="${b+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${na}</text>
        <text x="${f-3}" y="${v-N+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${ia}</text>
        <text x="${f}" y="${v-1}" text-anchor="start" fill="var(--secondary-text-color)" font-size="7">${new Date(Q).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        <text x="${m-k}" y="${v-1}" text-anchor="end" fill="var(--secondary-text-color)" font-size="7">${new Date(D).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        ${Ue?P`<path d="${Ue}" fill="var(--primary-color)" opacity="0.08" />`:d}
        <path d="${ea}" fill="var(--primary-color)" opacity="0.15" />
        <polyline points="${Oe}" fill="none" stroke="var(--primary-color)" stroke-width="2" stroke-linejoin="round" />
        ${l.degradation_rate!=null&&l.degradation_trend!=="stable"&&l.degradation_trend!=="insufficient_data"&&p.length>=2?(()=>{let y=p[p.length-1],S=30,H=y.ts+S*864e5,U=y.val+l.degradation_rate*S,ne=Math.min(H,D+(D-Q)*.3),ie=Math.max(E,Math.min($,U)),oa=I(y.ts),ra=R(y.val),la=I(ne),ca=R(ie);return P`<line x1="${oa.toFixed(1)}" y1="${ra.toFixed(1)}" x2="${la.toFixed(1)}" y2="${ca.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():d}
        ${a==="threshold"&&t.trigger_above!=null?P`<line x1="${f}" y1="${R(t.trigger_above).toFixed(1)}" x2="${m}" y2="${R(t.trigger_above).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${m-2}" y="${R(t.trigger_above)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25B2 ${t.trigger_above}</text>`:d}
        ${a==="threshold"&&t.trigger_below!=null?P`<line x1="${f}" y1="${R(t.trigger_below).toFixed(1)}" x2="${m}" y2="${R(t.trigger_below).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${m-2}" y="${R(t.trigger_below)+11}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25BC ${t.trigger_below}</text>`:d}
        ${a==="counter"&&L!=null?P`<line x1="${f}" y1="${R(L).toFixed(1)}" x2="${m}" y2="${R(L).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${m-2}" y="${R(L)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">${n("target_value",e.lang)}: +${t.trigger_target_value}</text>`:d}
        ${a==="counter"&&M!=null?P`<line x1="${f}" y1="${R(M).toFixed(1)}" x2="${m}" y2="${R(M).toFixed(1)}" stroke="var(--secondary-text-color)" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
                <text x="${f+2}" y="${R(M)+11}" text-anchor="start" fill="var(--secondary-text-color)" font-size="8">${n("baseline",e.lang)}</text>`:d}
        <circle cx="${ta.toFixed(1)}" cy="${aa.toFixed(1)}" r="3.5" fill="var(--primary-color)" />
        ${sa.map(y=>{let S=I(y.ts),H=y.type==="completed"?"var(--success-color, #4caf50)":y.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return P`
            <line x1="${S.toFixed(1)}" y1="${b}" x2="${S.toFixed(1)}" y2="${v-N}" stroke="${H}" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
            <circle cx="${S.toFixed(1)}" cy="${b+2}" r="5" fill="${H}" opacity="0.8" />
            <text x="${S.toFixed(1)}" y="${b+6}" text-anchor="middle" fill="white" font-size="7" font-weight="bold">${y.type==="completed"?"\u2713":y.type==="skipped"?"\u23ED":"\u21BA"}</text>
          `})}
        ${Ve.map(y=>{let S=I(y.ts),H=R(y.val),U=new Date(y.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),ne=`${y.val.toFixed(1)} ${s}`;return h&&y.min!=null&&y.max!=null&&(ne+=` (${y.min.toFixed(1)}\u2013${y.max.toFixed(1)})`),P`<circle cx="${S.toFixed(1)}" cy="${H.toFixed(1)}" r="8" fill="transparent" tabindex="0"
            @mouseenter=${ie=>Vt(ie,`${U}
${ne}`,e.setTooltip)}
            @focus=${ie=>Vt(ie,`${U}
${ne}`,e.setTooltip)}
...
`).map(y=>r`<div>${y}</div>`)}
  ...
  `}function Vt(l,s,e){let t=l.currentTarget,a=t.closest(".sparkline-container");if(!a)return;let i=a.getBoundingClientRect(),o=t.getBoundingClientRect();e({x:o.left-i.left+o.width/2,y:o.top-i.top-8,text:s})}function Wt(l,s,e){let t=l.degradation_trend!=null&&l.degradation_trend!=="insufficient_data",a=l.days_until_threshold!=null,i=l.environmental_factor!=null&&l.environmental_factor!==1;if(!t&&!a&&!i)return d;let o=l.degradation_trend==="rising"?"M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z":l.degradation_trend==="falling"?"M16,18L18.29,15.71L13.41,10.83L9.41,14.83L2,7.41L3.41,6L9.41,12L13.41,8L19.71,14.29L22,12V18H16Z":"M22,12L18,8V11H3V13H18V16L22,12Z";return r`
    <div class="prediction-section">
      ${l.sensor_prediction_urgency?r`
        <div class="prediction-urgency-banner">
          <ha-svg-icon path="M1,21H23L12,2L1,21M12,18A1,1 0 0,1 11,17A1,1 0 0,1 12,16A1,1 0 0,1 13,17A1,1 0 0,1 12,18M13,15H11V10H13V15Z"></ha-svg-icon>
          ${n("sensor_prediction_urgency",s).replace("{days}",String(Math.round(l.days_until_threshold||0)))}
        </div>
      `:d}
      ...
      <div class="prediction-grid">
        ${t?r`
          <div class="prediction-item">
            ...
            <span class="prediction-value ${l.degradation_trend}">${n("trend_"+l.degradation_trend,s)}</span>
            ${l.degradation_rate!=null?r`<span class="prediction-rate">${l.degradation_rate>0?"+":""}${Math.abs(l.degradation_rate)>=10?Math.round(l.degradation_rate).toLocaleString():l.degradation_rate.toFixed(1)} ${l.trigger_entity_info?.unit_of_measurement||""}/${n("day_short",s)}</span>`:d}
          </div>
        `:d}
        ${a?r`
          <div class="prediction-item">
            ...
            <span class="prediction-value prediction-days${l.days_until_threshold===0?" exceeded":l.sensor_prediction_urgency?" urgent":""}">${l.days_until_threshold===0?n("threshold_exceeded",s):"~"+Math.round(l.days_until_threshold)+" "+n("days",s)}</span>
            ${l.threshold_prediction_date?r`<span class="prediction-date">${Z(l.threshold_prediction_date,s)}</span>`:d}
        ...
        ${i&&e.environmental?r`
          <div class="prediction-item">
            ...
            ${l.environmental_entity?r`<span class="prediction-entity entity-link" @click=${c=>de(c,l.environmental_entity)}>${l.environmental_entity}</span>`:d}
          </div>
        `:d}
      </div>
    </div>
  `}function Gt(l,s){let e=l.interval_analysis,t=e?.weibull_beta,a=e?.weibull_eta;if(t==null||a==null||a<=0)return d;let i=l.interval_days??0,o=l.suggested_interval??i;return r`
    <div class="weibull-section">
      ...
      ${_n(t,a,i,o,s)}
      ${un(e,s)}
      ${e?.confidence_interval_low!=null?pn(e,l,s):d}
    </div>
  `}function dn(l,s){let e,t,a;return l<.8?(e="early_failures",t="M13,14H11V10H13M13,18H11V16H13M1,21H23L12,2L1,21Z",a="beta_early_failures"):l<=1.2?(e="random_failures",t="M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2M13,17H11V15H13V17M13,13H11V7H13V13Z",a="beta_random_failures"):l<=3.5?(e="wear_out",t="M12,2A10,10 0 0,1 22,12A10,10 0 0,1 12,22A10,10 0 0,1 2,12A10,10 0 0,1 12,2M12,4A8,8 0 0,0 4,12A8,8 0 0,0 12,20A8,8 0 0,0 20,12A8,8 0 0,0 12,4M12,6A6,6 0 0,1 18,12H12V6Z",a="beta_wear_out"):(e="highly_predictable",t="M12,2A10,10 0 0,1 22,12A10,10 0 0,1 12,22A10,10 0 0,1 2,12A10,10 0 0,1 12,2M11,16.5L18,9.5L16.59,8.09L11,13.67L7.91,10.59L6.5,12L11,16.5Z",a="beta_highly_predictable"),r`
    <span class="beta-badge ${e}">
      <ha-svg-icon path="${t}"></ha-svg-icon>
      ${n(a,s)} (\u03B2=${l.toFixed(2)})
    </span>
  `}function _n(l,s,e,t,a){let f=Math.max(e,t,s,1)*1.3,k=50,b=[];for(let D=0;D<=k;D++){let z=D/k*f,I=1-Math.exp(-Math.pow(z/s,l)),R=32+z/f*260,Oe=136-I*128;b.push([R,Oe])}let N=b.map(([D,z])=>`${D.toFixed(1)},${z.toFixed(1)}`).join(" "),C="M32,136 "+b.map(([D,z])=>`L${D.toFixed(1)},${z.toFixed(1)}`).join(" ")+` L${b[k][0].toFixed(1)},136 Z`,E=32+e/f*260,$=1-Math.exp(-Math.pow(e/s,l)),M=136-$*128,L=((1-$)*100).toFixed(0),B=32+t/f*260,Q=[0,.25,.5,.75,1];return r`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_weibull",a)}">
        ${Q.map(D=>{let z=136-D*128;return P`
        ...
        ${e>0?P`
          ...
          <circle cx="${E.toFixed(1)}" cy="${M.toFixed(1)}" r="3"
            fill="var(--primary-color, #03a9f4)" />
          <text x="${(E+4).toFixed(1)}" y="${(M-6).toFixed(1)}" fill="var(--primary-color, #03a9f4)"
    ...
    <div class="chart-legend">
      <span class="legend-item"><span class="legend-swatch" style="background:var(--primary-color, #03a9f4)"></span> ${n("weibull_failure_probability",a)}</span>
      ${e>0?r`<span class="legend-item"><span class="legend-swatch" style="background:var(--primary-color, #03a9f4); opacity:0.5"></span> ${n("current_interval_marker",a)}</span>`:d}
      ${t>0&&t!==e?r`<span class="legend-item"><span class="legend-swatch" style="background:var(--success-color, #4caf50)"></span> ${n("recommended_marker",a)}</span>`:d}
    </div>
  `}function un(l,s){return r`
    <div class="weibull-info-row">
      <div class="weibull-info-item">
        <span>${n("characteristic_life",s)}</span>
        <span class="weibull-info-value">${Math.round(l.weibull_eta)} ${n("days",s)}</span>
      </div>
  ...
  `}function pn(l,s,e){let t=l.confidence_interval_low,a=l.confidence_interval_high,i=s.suggested_interval??s.interval_days??0,o=s.interval_days??0,c=Math.max(0,t-5),p=a+5-c,h=(t-c)/p*100,m=(a-t)/p*100,v=(i-c)/p*100,f=o>0?(o-c)/p*100:-1;return r`
    <div class="confidence-range">
      <div class="confidence-range-title">
        ${n("confidence_interval",e)}: ${i} ${n("days",e)} (${t}\u2013${a})
      </div>
      <div class="confidence-bar">
        <div class="confidence-fill" style="left:${h.toFixed(1)}%;width:${m.toFixed(1)}%"></div>
      ...
      <div class="confidence-labels">
        ...
        <span class="confidence-text high">${n("confidence_aggressive",e)} (${a}${n("days",e).charAt(0)})</span>
      </div>
    </div>
  `}function Kt(l,s,e,t){let a=Math.max(l||1,s);return r`
    <div class="interval-comparison">
      <div class="interval-bar">
        <div class="interval-label">
          ${n("current",t)}: ${l??"\u2014"} ${l!=null?n("days",t):""}
        </div>
        <div class="interval-visual current"
          style="width: ${l!=null?Math.min(l/a*100,100):0}%"></div>
      </div>
      <div class="interval-bar">
        <div class="interval-label">
  ...
  `}var Qt=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"];function Zt(l,s,e){if(!e.seasonal||!l.seasonal_factor||l.seasonal_factor===1)return d;let t=Qt.map(c=>n(c,s)),a=new Date().getMonth(),i=l.seasonal_factors||l.interval_analysis?.seasonal_factors||null,o=i&&i.length===12?i:t.map((c,u)=>{let p=l.seasonal_factor||1,h=Math.sin((u-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,p+h))});return r`
    <div class="seasonal-card-compact">
      <h4>${n("seasonal_awareness",s)}</h4>
      <div class="seasonal-mini-chart">
        ${o.map((c,u)=>{let p=c*40,h=c<.9?"low":c>1.1?"high":"normal";return r`
            <div class="seasonal-bar ${h} ${u===a?"current":""}"
                 style="height: ${p}px"
      ...
      <div class="seasonal-legend">
        <span class="legend-item"><span class="dot low"></span> ${n("shorter",s)||"K\xFCrzer"}</span>
        <span class="legend-item"><span class="dot normal"></span> ${n("normal",s)||"Normal"}</span>
        <span class="legend-item"><span class="dot high"></span> ${n("longer",s)||"L\xE4nger"}</span>
      </div>
    </div>
  `}function Jt(l,s){return gn(l,s)}function gn(l,s){let e=l.seasonal_factors??l.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return d;let t=l.interval_analysis?.seasonal_reason,a=new Date().getMonth(),i=300,o=100,c=8,p=o-c-4,h=Math.max(...e,1.5),m=i/12,v=m*.65,f=c+p-1/h*p;return r`
    <div class="seasonal-chart">
      <div class="seasonal-chart-title">
        ...
        ${t?r`<span class="source-tag">${t==="learned"?n("seasonal_learned",s):n("seasonal_manual",s)}</span>`:d}
      </div>
      <svg viewBox="0 0 ${i} ${o}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_seasonal",s)}">
        <line x1="0" y1="${f.toFixed(1)}" x2="${i}" y2="${f.toFixed(1)}"
          stroke="var(--divider-color)" stroke-width="1" stroke-dasharray="4,3" />
        ${e.map((k,b)=>{let N=k/h*p,C=b*m+(m-v)/2,E=c+p-N,$=b===a,M=k<1?"var(--success-color, #4caf50)":k>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return P`
            <rect x="${C.toFixed(1)}" y="${E.toFixed(1)}"
              width="${v.toFixed(1)}" height="${N.toFixed(1)}"
              fill="${M}" opacity="${$?1:.5}" rx="2" />
          `})}
      </svg>
      <div class="seasonal-labels">
        ${Qt.map((k,b)=>r`<span class="seasonal-label ${b===a?"active-month":""}">${n(k,s)}</span>`)}
      </div>
    </div>
  `}var mn=300,hn=200;function Yt(l,s,e,t){let a=l.history.filter(c=>c.type==="completed"&&(c.cost!=null||c.duration!=null));if(a.length<2)return d;let i=a.some(c=>(c.cost??0)>0),o=a.some(c=>(c.duration??0)>0);return!i&&!o?d:r`
    <div class="cost-duration-card">
      <div class="card-header">
        <h3>${n("cost_duration_chart",s)}</h3>
  ...
  `}function vn(l,s,e){let t=l.history.filter(z=>z.type==="completed"&&(z.cost!=null||z.duration!=null)).map(z=>({ts:new Date(z.timestamp).getTime(),cost:z.cost??0,duration:z.duration??0})).sort((z,I)=>z.ts-I.ts);if(t.length<2)return d;let a=t.some(z=>z.cost>0),i=t.some(z=>z.duration>0);if(!a&&!i)return d;let o=e!=="duration"&&a,c=e!=="cost"&&i,u=o||!c&&a,p=c||!o&&i,h=mn,m=hn,v=u?32:8,f=p?32:8,k=8,b=20,N=h-v-f,C=m-k-b,E=Math.max(...t.map(z=>z.cost))||1,$=Math.max(...t.map(z=>z.duration))||1,M=Math.min(20,N/t.length*.6),L=N/t.length,B=z=>v+L*z+L/2,Q=z=>k+C-z/E*C,D=z=>k+C-z/$*C;return r`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${h} ${m}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_history",s)}">
        ${u?t.map((z,I)=>P`
          <rect x="${(B(I)-M/2).toFixed(1)}" y="${Q(z.cost).toFixed(1)}" width="${M.toFixed(1)}" height="${(k+C-Q(z.cost)).toFixed(1)}"
            fill="var(--primary-color)" opacity="0.6" rx="2" />
        `):d}
        ${p?P`
          <polyline points="${t.map((z,I)=>`${B(I).toFixed(1)},${D(z.duration).toFixed(1)}`).join(" ")}"
            fill="none" stroke="var(--accent-color, #ff9800)" stroke-width="2" stroke-linejoin="round" />
          ${t.map((z,I)=>P`
            <circle cx="${B(I).toFixed(1)}" cy="${D(z.duration).toFixed(1)}" r="3" fill="var(--accent-color, #ff9800)" />
          `)}
        `:d}
        <text x="${v}" y="${m-2}" text-anchor="start" fill="var(--secondary-text-color)" font-size="7">${new Date(t[0].ts).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        <text x="${h-f}" y="${m-2}" text-anchor="end" fill="var(--secondary-text-color)" font-size="7">${new Date(t[t.length-1].ts).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        ${u?P`
          <text x="${v-3}" y="${k+4}" text-anchor="end" fill="var(--primary-color)" font-size="7">${E.toFixed(0)}\u20AC</text>
          <text x="${v-3}" y="${k+C+3}" text-anchor="end" fill="var(--primary-color)" font-size="7">0\u20AC</text>
        `:d}
        ${p?P`
          <text x="${h-f+3}" y="${k+4}" text-anchor="start" fill="var(--accent-color, #ff9800)" font-size="7">${$.toFixed(0)}m</text>
          <text x="${h-f+3}" y="${k+C+3}" text-anchor="start" fill="var(--accent-color, #ff9800)" font-size="7">0m</text>
        `:d}
      </svg>
    </div>
    <div class="chart-legend">
      ${u?r`<span class="legend-item"><span class="legend-swatch" style="background:var(--primary-color);opacity:0.6"></span>${n("cost",s)}</span>`:d}
      ${p?r`<span class="legend-item"><span class="legend-swatch" style="background:var(--accent-color, #ff9800)"></span>${n("duration",s)}</span>`:d}
    </div>
  `}var fn=60,bn=20,Xt=30,j=class extends T{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._sparklineTooltip=null;this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._adminPanelUserIds=[];this._operatorWriteEnabled=!1;this._defaultWarningDays=7;this._actionLoading=!1;this._moreMenuOpen=!1;this._toastMessage="";this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab="dashboard";this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._objectSortMode="alphabetical";this._groupByMode="none";this._objectViewMode="cards";this._objectsTableColumns=Ie;this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._deepLinkHandled=!1;this._onDialogEvent=async()=>{try{await this._loadData()}catch{}};this._onCalendarLlCustom=e=>{let t=e.detail;t?.type==="maintenance-supporter:open-task"&&t.entry_id&&t.task_id&&(e.stopPropagation(),this._showTask(t.entry_id,t.task_id))};this._onHistoryEntrySaved=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"en"}get _isOperator(){let e=this.hass?.user;return e?e.is_admin?!1:!(this._operatorWriteEnabled&&this._adminPanelUserIds.includes(e.id)):!0}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler);let e=localStorage.getItem("maintenance_supporter_sort");e&&["due_date","object","type","task_name","area","assigned_user","group"].includes(e)&&(this._sortMode=e);let t=localStorage.getItem("maintenance_supporter_object_sort");t&&["alphabetical","due_soonest","task_count"].includes(t)&&(this._objectSortMode=t);let a=localStorage.getItem("maintenance_supporter_groupby");a&&["none","area","group","user"].includes(a)&&(this._groupByMode=a);let i=localStorage.getItem("maintenance_supporter_object_view");(i==="cards"||i==="table")&&(this._objectViewMode=i)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new Pe(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new oe(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,a,i,o]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);if(e&&(this._objects=e.objects),t&&(this._stats=t),a&&(this._budget=a),i&&(this._groups=i.groups||{}),o){let c=o;this._features=c.features,this._adminPanelUserIds=c.admin_panel_user_ids||[],this._operatorWriteEnabled=c.operator_write_enabled??!1;let u=c.general?.default_warning_days;typeof u=="number"&&u>=0&&u<=365&&(this._defaultWarningDays=u),this._objectsTableColumns=$e(c.objects_table_columns)}this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("ms_action"),a=()=>{let h=window.location.pathname+window.location.hash;history.replaceState(history.state,"",h)};if(t==="add_object"){this._deepLinkHandled=!0,a(),requestAnimationFrame(()=>{this.shadowRoot?.querySelector("maintenance-object-dialog")?.openCreate()});return}if(t==="open_vacation"||t==="open_budget"||t==="open_groups"||t==="open_settings"){this._deepLinkHandled=!0,a(),this._overviewTab="settings",requestAnimationFrame(()=>{let h=this.shadowRoot?.querySelector("maintenance-settings-view"),m=t.replace("open_","");h?.scrollToSection?.(m)});return}let i=e.get("entry_id");if(!i)return;this._deepLinkHandled=!0;let o=e.get("task_id"),c=e.get("action"),u=window.location.pathname+window.location.hash;history.replaceState(history.state,"",u);let p=this._getObject(i);if(!p){this._showOverview();return}if(o){let h=p.tasks.find(m=>m.id===o);if(!h){this._showObject(i);return}this._showTask(i,o),c==="complete"?requestAnimationFrame(()=>{this._openCompleteDialog(i,o,h.name,this._features.checklists?h.checklist:void 0,this._features.adaptive&&!!h.adaptive_config?.enabled)}):c==="quick_complete"&&requestAnimationFrame(()=>{this._handleQuickComplete(i,o,h)})}else this._showObject(i)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let a=await this._statsService.getDetailStats(e,t),i=new Map(this._detailStatsData);i.set(e,a),this._detailStatsData=i}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let a of this._objects)for(let i of a.tasks){let o=i.trigger_config?.entity_id;o&&e.push({entityId:o,isCounter:this._isCounterEntity(i.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let m of this._objects)for(let v of m.tasks){if(this._filterStatus&&v.status!==this._filterStatus)continue;if(this._filterUser){let k=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(v.responsible_user_id!==k)continue}let f=[];for(let k of Object.values(this._groups))k.task_refs?.some(b=>b.entry_id===m.entry_id&&b.task_id===v.id)&&f.push(k.name);e.push({entry_id:m.entry_id,task_id:v.id,object_name:m.object.name,task_name:v.name,type:v.type,schedule_type:v.schedule_type,status:v.status,days_until_due:v.days_until_due??null,next_due:v.next_due??null,trigger_active:v.trigger_active,trigger_current_value:v.trigger_current_value??null,trigger_current_delta:v.trigger_current_delta??null,trigger_config:v.trigger_config??null,trigger_entity_info:v.trigger_entity_info??null,times_performed:v.times_performed,total_cost:v.total_cost,interval_days:v.interval_days??null,interval_unit:v.interval_unit??null,interval_anchor:v.interval_anchor??null,is_done:v.is_done??!1,history:v.history||[],enabled:v.enabled,nfc_tag_id:v.nfc_tag_id??null,area_id:m.object.area_id??null,responsible_user_id:v.responsible_user_id??null,group_names:f})}let t={overdue:0,triggered:1,due_soon:2,ok:3},a=(m,v)=>(t[m.status]??9)-(t[v.status]??9),i=(m,v)=>(m.days_until_due??99999)-(v.days_until_due??99999),o=(m,v)=>a(m,v)||i(m,v),c=m=>m.area_id&&this.hass?.areas?.[m.area_id]?.name||"",u=m=>m.responsible_user_id&&this._userService?.getUserName(m.responsible_user_id)||"",p=m=>m.group_names[0]||"",h={due_date:o,object:(m,v)=>m.object_name.localeCompare(v.object_name)||o(m,v),type:(m,v)=>m.type.localeCompare(v.type)||o(m,v),task_name:(m,v)=>m.task_name.localeCompare(v.task_name),area:(m,v)=>{let f=c(m),k=c(v);return!f&&k?1:f&&!k?-1:f.localeCompare(k)||o(m,v)},assigned_user:(m,v)=>{let f=u(m),k=u(v);return!f&&k?1:f&&!k?-1:f.localeCompare(k)||o(m,v)},group:(m,v)=>{let f=p(m),k=p(v);return!f&&k?1:f&&!k?-1:f.localeCompare(k)||o(m,v)}};return e.sort(h[this._sortMode]),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(i=>i.id===t)}_pushPanelState(e,t,a){let i={msp_view:e,msp_entry:t||null,msp_task:a||null};history.pushState(i,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let a=this._getTask(t.msp_entry,t.msp_task);a?.trigger_config?.entity_id&&this._fetchDetailStats(a.trigger_config.entity_id,this._isCounterEntity(a.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1,this._scrollContentToTop()}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null,this._scrollContentToTop()}_filterByStatus(e){this._filterStatus=e,this._overviewTab!=="dashboard"&&(this._overviewTab="dashboard"),this._scrollContentToTop()}_scrollContentToTop(){requestAnimationFrame(()=>{let e=this.shadowRoot?.querySelector(".content");e&&e.scrollTo({top:0,behavior:"smooth"})})}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null,this._scrollContentToTop()}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null,this._scrollContentToTop();let a=this._getTask(e,t);if(a?.trigger_config?.entity_id){let i=a.trigger_config.entity_id,o=this._isCounterEntity(a.trigger_config);this._fetchDetailStats(i,o)}}_showToast(e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}async _deleteObject(e){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:n("delete",this._lang),message:n("confirm_delete_object",this._lang),confirmText:n("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}async _deleteTask(e,t){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:n("delete",this._lang),message:n("confirm_delete_task",this._lang),confirmText:n("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}async _skipTask(e,t,a){this._actionLoading=!0;try{let i={type:"maintenance_supporter/task/skip",entry_id:e,task_id:t};a&&(i.reason=a),await this.hass.connection.sendMessagePromise(i),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}finally{this._actionLoading=!1}}async _resetTask(e,t,a){this._actionLoading=!0;try{let i={type:"maintenance_supporter/task/reset",entry_id:e,task_id:t};a&&(i.date=a),await this.hass.connection.sendMessagePromise(i),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}finally{this._actionLoading=!1}}async _applySuggestion(e,t,a){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:a}),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}_openSeasonalOverrides(e){let t=this.shadowRoot.querySelector("maintenance-seasonal-overrides-dialog");if(!t||!this._selectedEntryId)return;let a=e.adaptive_config?.seasonal_overrides;t.open(this._selectedEntryId,e.id,a)}async _reanalyzeInterval(e,t){try{let a=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:e,task_id:t});a.recommended_interval?this._showToast(`${n("reanalyze_result",this._lang)}: ${a.recommended_interval} ${n("days",this._lang)} (${n(`confidence_${a.confidence}`,this._lang)}, ${a.data_points} ${n("data_points",this._lang)})`):this._showToast(n("reanalyze_insufficient_data",this._lang)),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}async _promptSkipTask(e,t){let a=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!a)return;let i=await a.prompt({title:n("skip",this._lang),message:n("skip_reason_prompt",this._lang),confirmText:n("skip",this._lang),inputLabel:n("reason_optional",this._lang),inputType:"text"});i.confirmed&&this._skipTask(e,t,i.value||void 0)}async _promptResetTask(e,t){let a=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!a)return;let i=await a.prompt({title:n("reset",this._lang),message:n("reset_date_prompt",this._lang),confirmText:n("reset",this._lang),inputLabel:n("reset_date_optional",this._lang),inputType:"date"});i.confirmed&&this._resetTask(e,t,i.value||void 0)}_dismissSuggestion(e,t){e&&t&&this._dismissedSuggestions.add(`${e}_${t}`),this.requestUpdate()}async _handleQuickComplete(e,t,a){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/quick_complete",entry_id:e,task_id:t}),this._showToast(n("quick_complete_success",this._lang))}catch(i){(i?.code||"")==="no_defaults"?this._openCompleteDialog(e,t,a.name,this._features.checklists?a.checklist:void 0,this._features.adaptive&&!!a.adaptive_config?.enabled):this._showToast(n("action_error",this._lang))}}_openCompleteDialog(e,t,a,i,o){let c=this.shadowRoot.querySelector("maintenance-complete-dialog");c&&(c.entryId=e,c.taskId=t,c.taskName=a,c.lang=this._lang,c.checklist=i||[],c.adaptiveEnabled=!!o,c.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,a,i){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,a,i)}render(){return r`
      <div class="panel">
    ...
    `}_renderOverview(){let e=this._lang,t=!!this.hass?.user?.is_admin,a=this._stats;return!t&&this._overviewTab==="settings"&&(this._overviewTab="dashboard"),r`
      ${a?r`
            <div class="stats-bar">
              <div class="stat-item clickable"
                   @click=${()=>this._showAllObjects()}
                   title=${n("show_all_objects",e)}>
                <span class="stat-value">${a.total_objects}</span>
                <span class="stat-label">${n("objects",e)}</span>
              </div>
              <div class="stat-item clickable"
                   @click=${()=>this._filterByStatus("")}
                   title=${n("show_all_tasks",e)}>
                <span class="stat-value">${a.total_tasks}</span>
                <span class="stat-label">${n("tasks",e)}</span>
              </div>
              <div class="stat-item clickable ${this._filterStatus==="overdue"&&this._overviewTab==="dashboard"?"active":""}"
                   ...
                   title=${n("filter_to_overdue",e)}>
                <span class="stat-value" style="color: var(--error-color)">${a.overdue}</span>
                <span class="stat-label">${n("overdue",e)}</span>
              </div>
              <div class="stat-item clickable ${this._filterStatus==="due_soon"&&this._overviewTab==="dashboard"?"active":""}"
                   ...
                   title=${n("filter_to_due_soon",e)}>
                <span class="stat-value" style="color: var(--warning-color)">${a.due_soon}</span>
                <span class="stat-label">${n("due_soon",e)}</span>
              </div>
              <div class="stat-item clickable ${this._filterStatus==="triggered"&&this._overviewTab==="dashboard"?"active":""}"
                   ...
                   title=${n("filter_to_triggered",e)}>
                <span class="stat-value" style="color: #ff5722">${a.triggered}</span>
                <span class="stat-label">${n("triggered",e)}</span>
              </div>
            </div>
          `:d}
      <div class="tab-bar">
      ...
      ${this._overviewTab==="dashboard"?this._renderDashboard():this._overviewTab==="calendar"?r`
    ...
    `}_renderDashboard(){let e=this._stats,t=this._taskRows,a=this._lang,i=this._isOperator;return r`
      ${this._features.budget?this._renderBudgetBar():d}

      <div class="filter-bar">
        <label class="filter-field">
          <span class="filter-label">${n("filter_label",a)}</span>
          <select
            .value=${this._filterStatus}
            @change=${o=>this._filterStatus=o.target.value}
          >
            <option value="">${n("all",a)}</option>
            <option value="overdue">${n("overdue",a)}</option>
            <option value="due_soon">${n("due_soon",a)}</option>
            <option value="triggered">${n("triggered",a)}</option>
            <option value="ok">${n("ok",a)}</option>
          </select>
        </label>
        <label class="filter-field">
          <span class="filter-label">${n("user_label",a)}</span>
          <select
            .value=${this._filterUser||""}
            @change=${o=>{let c=o.target.value;this._filterUser=c||null}}
          >
            <option value="">${n("all_users",a)}</option>
            <option value="current_user">${n("my_tasks",a)}</option>
          </select>
        </label>
        <label class="filter-field">
          <span class="filter-label">${n("sort_label",a)}</span>
          <select
          ...
          >
            <option value="due_date" ?selected=${this._sortMode==="due_date"}>${n("sort_due_date",a)}</option>
            <option value="object" ?selected=${this._sortMode==="object"}>${n("sort_object",a)}</option>
            <option value="type" ?selected=${this._sortMode==="type"}>${n("sort_type",a)}</option>
            <option value="task_name" ?selected=${this._sortMode==="task_name"}>${n("sort_task_name",a)}</option>
            <option value="area" ?selected=${this._sortMode==="area"}>${n("sort_area",a)}</option>
            <option value="assigned_user" ?selected=${this._sortMode==="assigned_user"}>${n("sort_assigned_user",a)}</option>
            <option value="group" ?selected=${this._sortMode==="group"}>${n("sort_group",a)}</option>
          </select>
        </label>
        <label class="filter-field">
          <span class="filter-label">${n("group_by_label",a)}</span>
          <select
          ...
          >
            <option value="none" ?selected=${this._groupByMode==="none"}>${n("groupby_none",a)}</option>
            <option value="area" ?selected=${this._groupByMode==="area"}>${n("groupby_area",a)}</option>
            ${this._features.groups?r`<option value="group" ?selected=${this._groupByMode==="group"}>${n("groupby_group",a)}</option>`:d}
            <option value="user" ?selected=${this._groupByMode==="user"}>${n("groupby_user",a)}</option>
          </select>
        </label>
        ${i?d:r`
          <ha-button
      ...
      ${this._features.groups&&!i?this._renderGroupsSection():d}
    `}_renderGroupedTasks(e,t){let a=new Map,i=n("unassigned",t);for(let u of e){let p=[];this._groupByMode==="area"?p=[(u.area_id?this.hass?.areas?.[u.area_id]?.name:null)||i]:this._groupByMode==="user"?p=[(u.responsible_user_id?this._userService?.getUserName(u.responsible_user_id):null)||i]:this._groupByMode==="group"&&(p=u.group_names.length>0?u.group_names:[i]);for(let h of p)a.has(h)||a.set(h,[]),a.get(h).push(u)}let o=[...a.entries()].sort(([u],[p])=>u===i&&p!==i?1:p===i&&u!==i?-1:u.localeCompare(p)),c=this._groupByMode==="area"?"mdi:map-marker-outline":this._groupByMode==="group"?"mdi:folder-outline":"mdi:account-outline";return r`
      ${o.map(([u,p])=>r`
      ...
      `)}
    `}_warrantyLabel(e,t,a){return e.kind==="expired"?n("warranty_expired",a):e.kind==="expiring"?n("warranty_expires_in",a).replace("{days}",String(e.days??0)):n("warranty_valid_until",a).replace("{date}",Z(t,a))}_renderWarrantyMeta(e,t){let a=ot(e);return r`<p class="meta">${n("warranty",t)}:
      <span class="warranty-chip warranty-${a.kind}">${this._warrantyLabel(a,e,t)}</span></p>`}_renderAllObjects(){let e=this._lang,t=this._isOperator,a=this._objectViewMode==="table"&&!this.narrow,i=p=>{let h=1/0;for(let m of p.tasks){let v=m.days_until_due;v!=null&&v<h&&(h=v)}return h},o=[...this._objects];this._objectSortMode==="alphabetical"?o.sort((p,h)=>p.object.name.localeCompare(h.object.name)):this._objectSortMode==="task_count"?o.sort((p,h)=>h.tasks.length-p.tasks.length||p.object.name.localeCompare(h.object.name)):o.sort((p,h)=>i(p)-i(h)||p.object.name.localeCompare(h.object.name));let c=()=>{let p=new Map;for(let h of o){let m=h.object.area_id,v=m?this.hass?.areas?.[m]?.name||n("unassigned",e):n("no_area",e);p.has(v)||p.set(v,[]),p.get(v).push(h)}return new Map([...p.entries()].sort(([h],[m])=>h.localeCompare(m)))},u=p=>{let h=p.tasks.some(m=>m.status==="overdue"||m.status==="triggered");return r`
        <div class="object-card${h?" object-card-overdue":""}" @click=${()=>this._showObject(p.entry_id)}>
          ${h?r`<span class="overdue-dot" title="${n("has_overdue",e)}"></span>`:d}
          <div class="object-card-header">
            <span class="object-card-name">${p.object.name}</span>
            <span class="object-card-count">${p.tasks.length} ${n("tasks_lower",e)}</span>
          </div>
          ${p.object.manufacturer||p.object.model?r`<div class="object-card-meta">${[p.object.manufacturer,p.object.model].filter(Boolean).join(" ")}</div>`:d}
          ${p.tasks.length===0?r`<div class="object-card-empty">${n("no_tasks_yet",e)}</div>`:d}
        </div>
      `};return r`
      <div class="breadcrumb">
        ...
        <span>${n("all_objects",e)}</span>
      </div>
      <div class="filter-bar">
        <label class="filter-field">
          <span class="filter-label">${n("sort_label",e)}</span>
          <select
            .value=${this._objectSortMode}
            @change=${p=>{this._objectSortMode=p.target.value,localStorage.setItem("maintenance_supporter_object_sort",this._objectSortMode)}}
          >
            <option value="alphabetical" ?selected=${this._objectSortMode==="alphabetical"}>${n("sort_alphabetical",e)}</option>
            <option value="due_soonest" ?selected=${this._objectSortMode==="due_soonest"}>${n("sort_due_soonest",e)}</option>
            <option value="task_count" ?selected=${this._objectSortMode==="task_count"}>${n("sort_task_count",e)}</option>
          </select>
        </label>
        ${this.narrow?d:r`
          <div class="view-toggle" role="group" aria-label="${n("view_mode_label",e)}">
            <button
              class="view-toggle-btn${a?"":" active"}"
              title="${n("view_cards",e)}"
              @click=${()=>this._setObjectViewMode("cards")}
            ><ha-icon icon="mdi:view-grid-outline"></ha-icon></button>
            <button
              class="view-toggle-btn${a?" active":""}"
              title="${n("view_table",e)}"
        ...
        ${a?d:r`
        <label class="filter-field">
          <span class="filter-label">${n("group_by_label",e)}</span>
          <select
            .value=${this._groupByMode}
            @change=${p=>{this._groupByMode=p.target.value,localStorage.setItem("maintenance_supporter_groupby",this._groupByMode)}}
          >
            <option value="none" ?selected=${this._groupByMode==="none"}>${n("groupby_none",e)}</option>
            <option value="area" ?selected=${this._groupByMode==="area"}>${n("groupby_area",e)}</option>
          </select>
        </label>
        `}
        ${t?d:r`
        ...
        <ha-button appearance="plain" @click=${()=>this._exportObjectsCsv()}>
          <ha-icon icon="mdi:file-delimited-outline"></ha-icon> ${n("settings_export_csv",e)}
        </ha-button>
      </div>
      ${a?this._renderObjectsTable(o):this._groupByMode==="area"?r`
          ${[...c().entries()].map(([p,h])=>r`
            <details class="group-section" open>
              <summary class="group-section-header">
              ...
              <div class="objects-grid">${h.map(u)}</div>
            </details>
          `)}
        `:r`<div class="objects-grid">${o.map(u)}</div>`}
    `}_setObjectViewMode(e){this._objectViewMode=e,localStorage.setItem("maintenance_supporter_object_view",e)}async _exportObjectsCsv(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects/csv"}),t=new Date().toISOString().slice(0,10),a=new Blob([e.csv],{type:"text/csv;charset=utf-8"}),i=URL.createObjectURL(a),o=document.createElement("a");o.href=i,o.download=`maintenance_objects_${t}.csv`,o.click(),URL.revokeObjectURL(i)}catch{this._showToast(n("action_error",this._lang))}}_renderObjectsTable(e){let t=this._lang,a=this._objectsTableColumns;return r`
      <div class="objects-table-wrap">
        <table class="objects-table">
          <thead>
            <tr>
              ${a.map(i=>{let o=pe.find(u=>u.key===i),c=o&&o.key!=="actions"?n(o.labelKey,t):"";return r`<th class="oc-${i}">${c}</th>`})}
            </tr>
          </thead>
          <tbody>
            ${e.map(i=>r`
              <tr class="objects-table-row" @click=${()=>this._showObject(i.entry_id)}>
                ${a.map(o=>this._renderObjectCell(o,i,t))}
              </tr>
            `)}
          </tbody>
        </table>
      </div>
    `}_renderObjectCell(e,t,a){let i=t.object;switch(e){case"name":return r`<td class="oc-name"><span class="objects-table-name">${i.name}</span></td>`;case"manufacturer":return r`<td class="oc-manufacturer">${i.manufacturer||"\u2014"}</td>`;case"model":return r`<td class="oc-model">${i.model||"\u2014"}</td>`;case"serial_number":return r`<td class="oc-serial_number">${i.serial_number||"\u2014"}</td>`;case"installation_date":return r`<td class="oc-installation_date">${i.installation_date?Z(i.installation_date,a):"\u2014"}</td>`;case"warranty_expiry":return r`<td class="oc-warranty_expiry">${this._renderWarrantyCell(i.warranty_expiry,a)}</td>`;case"area_id":{let o=i.area_id?this.hass?.areas?.[i.area_id]?.name||i.area_id:"\u2014";return r`<td class="oc-area_id">${o}</td>`}case"documentation_url":return r`<td class="oc-documentation_url">${i.documentation_url&&/^https?:\/\//i.test(i.documentation_url)?r`<a href=${i.documentation_url} target="_blank" rel="noopener noreferrer"
                @click=${o=>o.stopPropagation()}><ha-icon icon="mdi:file-document-outline"></ha-icon></a>`:"\u2014"}</td>`;case"notes":return r`<td class="oc-notes" title=${i.notes||""}>${i.notes||"\u2014"}</td>`;case"task_count":return r`<td class="oc-task_count">${t.tasks.length}</td>`;case"actions":return r`<td class="oc-actions">
          <mwc-icon-button title="${n("qr_code",a)}" @click=${o=>{o.stopPropagation(),this._openQrForObject(t.entry_id,i.name)}}>
            <ha-icon icon="mdi:qrcode"></ha-icon>
          </mwc-icon-button>
        </td>`;default:return r`<td></td>`}}_renderWarrantyCell(e,t){let a=ot(e);return a.kind==="none"?r`<span class="warranty-none">—</span>`:r`<span class="warranty-chip warranty-${a.kind}">${this._warrantyLabel(a,e,t)}</span>`}async _onSettingsChanged(){await this._loadData()}_renderGroupsSection(){if(!this._features.groups)return d;let e=Object.entries(this._groups),t=this._lang;return r`
      <div class="groups-section">
        ...
        ${e.length===0?r`<div class="hint">${n("no_groups",t)}</div>`:r`
            <div class="groups-grid">
              ${e.map(([a,i])=>{let o=i.task_refs.map(c=>this._getTask(c.entry_id,c.task_id)?.name).filter(Boolean);return r`
                  <div class="group-card">
                    <div class="group-card-head">
                      <div class="group-card-name">${i.name}</div>
                      <div class="group-card-actions">
                        <mwc-icon-button title="${n("edit",t)}" @click=${()=>this._openGroupEdit(a)}>
                        ...
                        <mwc-icon-button title="${n("delete",t)}" @click=${()=>this._deleteGroup(a,i.name)}>
                          <ha-svg-icon path="M19 4h-3.5l-1-1h-5l-1 1H5v2h14M6 19a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V7H6v12z"></ha-svg-icon>
                        </mwc-icon-button>
                      </div>
                    </div>
                    ${i.description?r`<div class="group-card-desc">${i.description}</div>`:d}
                    <div class="group-card-tasks">
                      ${o.length>0?o.map(c=>r`<span class="group-task-chip">${c}</span>`):r`<span style="font-size:12px;color:var(--secondary-text-color)">${n("no_tasks_short",t)}</span>`}
                    </div>
                  </div>
                `})}
            </div>
    ...
    `}_openGroupCreate(){this.shadowRoot.querySelector("maintenance-group-dialog")?.openCreate()}_openGroupEdit(e){let t=this._groups[e];t&&this.shadowRoot.querySelector("maintenance-group-dialog")?.openEdit(e,t)}async _deleteGroup(e,t){let a=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(a?await a.confirm({title:n("delete_group",this._lang),message:n("delete_group_confirm",this._lang).replace("{name}",t),confirmText:n("delete",this._lang)}):confirm(`${n("delete_group_confirm",this._lang).replace("{name}",t)}`))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/group/delete",group_id:e}),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}_renderBudgetBar(){let e=this._budget;if(!e)return d;let t=this._lang,a=e.currency_symbol||"\u20AC",i=[];return e.monthly_budget>0&&i.push({label:n("budget_monthly",t),spent:e.monthly_spent,budget:e.monthly_budget}),e.yearly_budget>0&&i.push({label:n("budget_yearly",t),spent:e.yearly_spent,budget:e.yearly_budget}),i.length===0?d:r`
      <div class="budget-bars">
        ${i.map(o=>{let c=Math.min(100,Math.max(0,o.spent/o.budget*100)),u=c>=100?"var(--error-color, #f44336)":c>=e.alert_threshold_pct?"var(--warning-color, #ff9800)":"var(--success-color, #4caf50)";return r`
    ...
    `}_renderOverviewRow(e){let t=this._lang,a=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,i=0,o=Ne.ok,c=!1;if(a&&e.days_until_due!==null){let m=st(e.interval_days,e.days_until_due,e.interval_unit);i=m.pct,c=m.overflow,e.status==="overdue"?o=Ne.overdue:e.status==="due_soon"&&(o=Ne.due_soon)}let u=e.area_id?this.hass?.areas?.[e.area_id]?.name:null,p=e.responsible_user_id?this._userService?.getUserName(e.responsible_user_id):null,h=e.group_names.length>0||u||p;return r`
      <div class="task-row${e.enabled?"":" task-disabled"}">
        <span class="cell-badges">
          <span class="status-badge ${e.is_done?"done":e.status}">${e.is_done?n("completed",t):n(e.status,t)}</span>
          ${e.enabled?d:r`<span class="badge-disabled">${n("disabled",t)}</span>`}
          ${e.nfc_tag_id?r`<span class="nfc-badge" title="${n("nfc_linked",t)}"><ha-icon icon="mdi:nfc-variant"></ha-icon></span>`:d}
        </span>
        ...
        <span class="task-sub${h?"":" task-sub-empty"}">
          ${e.group_names.length>0?r`
            <span class="sub-chip" title="${n("groups",t)}">
          ...
          ${p?r`
            <span class="sub-chip" title="${n("responsible_user",t)}">
        ...
        <span class="due-cell" @click=${()=>this._showTask(e.entry_id,e.task_id)}>
          ...
          ${a?r`<div class="days-bar"><div class="days-bar-fill${c?" overflow":""}" style="width:${i}%;background:${o}"></div></div>`:d}
        ...
        <span class="row-actions">
          <mwc-icon-button class="btn-complete" title="${n("complete",t)}" @click=${m=>{m.stopPropagation(),this._openCompleteDialogForRow(e)}}>
            <ha-icon icon="mdi:check"></ha-icon>
          </mwc-icon-button>
          <mwc-icon-button class="btn-skip" title="${n("skip",t)}" .disabled=${this._actionLoading} @click=${m=>{m.stopPropagation(),this._promptSkipTask(e.entry_id,e.task_id)}}>
            <ha-icon icon="mdi:skip-next"></ha-icon>
          </mwc-icon-button>
        </span>
      </div>
    `}_openCompleteDialogForRow(e){let a=this._objects.find(i=>i.entry_id===e.entry_id)?.tasks.find(i=>i.id===e.task_id);this._openCompleteDialog(e.entry_id,e.task_id,e.task_name,this._features.checklists?a?.checklist:void 0,this._features.adaptive&&!!a?.adaptive_config?.enabled)}_renderTriggerProgress(e){let t=e.trigger_config??null;if(!t)return d;let a=t.type||"threshold",i=e.trigger_entity_info?.unit_of_measurement??"",o=0,c="";if(a==="threshold"){let h=e.trigger_current_value??null;if(h==null)return d;let m=t.trigger_above,v=t.trigger_below;if(m!=null){let f=v??0,k=m-f||1;o=Math.min(100,Math.max(0,(h-f)/k*100)),c=`${h.toFixed(1)} / ${m} ${i}`}else if(v!=null){let k=e.trigger_entity_info?.max??(v*2||100),b=k-v||1;o=Math.min(100,Math.max(0,(k-h)/b*100)),c=`${h.toFixed(1)} / ${v} ${i}`}else return d}else if(a==="counter"){let h=t.trigger_target_value||1,v=e.trigger_current_delta??null??e.trigger_current_value??null;if(v==null)return d;o=Math.min(100,Math.max(0,v/h*100)),c=`${v.toFixed(1)} / ${h} ${i}`}else if(a==="state_change"){let h=t.trigger_target_changes||1,m=e.trigger_current_value??null;if(m==null)return d;o=Math.min(100,Math.max(0,m/h*100)),c=`${Math.round(m)} / ${h}`}else if(a==="runtime"){let h=t.trigger_runtime_hours||100,m=e.trigger_current_value??null;if(m==null)return d;o=Math.min(100,Math.max(0,m/h*100)),c=`${m.toFixed(1)}h / ${h}h`}else if(a==="compound"){let h=t.compound_logic||t.operator||"AND",m=t.conditions?.length||0;c=`${h} (${m})`,o=e.trigger_active?100:0}else return d;let u=o>=100,p=o>90?"var(--error-color, #f44336)":o>70?"var(--warning-color, #ff9800)":"var(--primary-color)";return r`
      <div class="trigger-progress">
        <div class="trigger-progress-bar">
          <div class="trigger-progress-fill${u?" overflow":""}" style="width:${o}%;background:${p}"></div>
        </div>
        <span class="trigger-progress-label">${c}</span>
      </div>
    `}_renderMiniSparkline(e){if(!e.trigger_config?.entity_id)return d;let t=e.trigger_config.entity_id,a=this._miniStatsData.get(t)||[],i=[];if(a.length>=2)i=a.map($=>({ts:$.ts,val:$.val}));else{if(!e.history)return d;for(let $ of e.history)$.trigger_value!=null&&i.push({ts:new Date($.timestamp).getTime(),val:$.trigger_value})}if(e.trigger_current_value!=null&&i.push({ts:Date.now(),val:e.trigger_current_value}),i.length<2)return d;i.sort(($,M)=>$.ts-M.ts);let o=fn,c=bn,u=i.map($=>$.val),p=Math.min(...u),h=Math.max(...u),m=h-p||1;p-=m*.1,h+=m*.1;let v=i[0].ts,k=i[i.length-1].ts-v||1,b=$=>($-v)/k*o,N=$=>2+(1-($-p)/(h-p))*(c-4),C=i;if(C.length>Xt){let $=Math.ceil(C.length/Xt);C=C.filter((M,L)=>L%$===0||L===C.length-1)}let E=C.map($=>`${b($.ts).toFixed(1)},${N($.val).toFixed(1)}`).join(" ");return r`
      <svg class="mini-sparkline" viewBox="0 0 ${o} ${c}" preserveAspectRatio="none" role="img" aria-label="${n("chart_mini_sparkline",this._lang)}">
        <polyline points="${E}" fill="none" stroke="var(--primary-color)" stroke-width="1.5" stroke-linejoin="round" />
      </svg>
    `}_renderDaysProgress(e){let t=this._lang;if(e.days_until_due==null||!e.interval_days||e.interval_days<=0)return d;let{pct:a,overflow:i}=st(e.interval_days,e.days_until_due,e.interval_unit),o="var(--success-color, #4caf50)";return e.status==="overdue"?o="var(--error-color, #f44336)":e.status==="due_soon"&&(o="var(--warning-color, #ff9800)"),r`
      <div class="days-progress">
        <div class="days-progress-labels">
          <span>${e.last_performed?`${n("last_performed",t)}: ${Z(e.last_performed,t)}`:""}</span>
          <span>${e.next_due?`${n("next_due",t)}: ${Z(e.next_due,t)}`:""}</span>
        </div>
        <div class="days-progress-bar" role="progressbar" aria-valuenow="${Math.round(a)}" aria-valuemin="0" aria-valuemax="100" aria-label="${n("days_progress",t)}">
          <div class="days-progress-fill${i?" overflow":""}" style="width:${a}%;background:${o}"></div>
        </div>
        <div class="days-progress-text">${De(e.days_until_due,t)}</div>
      </div>
    `}_renderObjectDetail(){if(!this._selectedEntryId)return d;let e=this._getObject(this._selectedEntryId);if(!e)return r`<p>Object not found.</p>`;let t=e.object,a=this._lang,i=this._isOperator;return r`
      <div class="detail-section">
        <div class="detail-header">
          <h2>${t.name}</h2>
          <div class="action-buttons">
            ${i?d:r`
              <ha-button appearance="plain" @click=${()=>{this.shadowRoot.querySelector("maintenance-object-dialog")?.openEdit(e.entry_id,t)}}>${n("edit",a)}</ha-button>
              <ha-button appearance="filled" @click=${()=>{this.shadowRoot.querySelector("maintenance-task-dialog")?.openCreate(e.entry_id)}}>${n("add_task",a)}</ha-button>
              <ha-button variant="danger" appearance="plain" @click=${()=>this._deleteObject(e.entry_id)}>${n("delete",a)}</ha-button>
            `}
            <ha-button appearance="plain" @click=${()=>this._openQrForObject(e.entry_id,t.name)}><ha-icon icon="mdi:qrcode"></ha-icon> ${n("qr_code",a)}</ha-button>
          </div>
        </div>
        ${t.manufacturer||t.model?r`<p class="meta">${[t.manufacturer,t.model].filter(Boolean).join(" ")}</p>`:d}
        ${t.serial_number?r`<p class="meta">${n("serial_number_label",a)}: ${t.serial_number}</p>`:d}
        ${t.documentation_url&&/^https?:\/\//i.test(t.documentation_url)?r`<p class="meta">${n("documentation_url_label",a)}:
              <a href=${t.documentation_url} target="_blank" rel="noopener noreferrer">${t.documentation_url}</a>
            </p>`:d}
        ${t.installation_date?r`<p class="meta">${n("installed",a)}: ${Z(t.installation_date,a)}</p>`:d}
        ${t.warranty_expiry?this._renderWarrantyMeta(t.warranty_expiry,a):d}
        ${t.notes?r`<div class="object-notes">
              <div class="object-notes-label">${n("object_notes_label",a)}</div>
              <div class="object-notes-body">${t.notes}</div>
            </div>`:d}

        <h3>${n("tasks",a)} (${e.tasks.length})</h3>
        ${e.tasks.length===0?r`<div class="empty-state-centered">
              <p class="empty">${n("no_tasks_yet",a)}</p>
              <ha-button appearance="filled" @click=${()=>{this.shadowRoot.querySelector("maintenance-task-dialog")?.openCreate(e.entry_id)}}>${n("add_first_task",a)}</ha-button>
            </div>`:r`<div class="task-table">${[...e.tasks].sort((o,c)=>{let u={overdue:0,triggered:1,due_soon:2,ok:3};return(u[o.status]??9)-(u[c.status]??9)||(o.days_until_due??99999)-(c.days_until_due??99999)}).map(o=>r`
              <div class="task-row${o.enabled?"":" task-disabled"}">
                <span class="cell-badges">
                  <span class="status-badge ${o.is_done?"done":o.status}">${o.is_done?n("completed",a):n(o.status,a)}</span>
                  ${o.enabled?d:r`<span class="badge-disabled">${n("disabled",a)}</span>`}
                  ${o.nfc_tag_id?r`<span class="nfc-badge" title="${n("nfc_linked",a)}"><ha-icon icon="mdi:nfc-variant"></ha-icon></span>`:d}
                </span>
                <span class="cell task-name" @click=${()=>this._showTask(e.entry_id,o.id)}>${o.name}</span>
                ${this._renderUserBadge(o)}
                <span class="cell type">${n(o.type,a)}</span>
                <span class="due-cell" @click=${()=>this._showTask(e.entry_id,o.id)}>
                  <span class="due-text">${De(o.days_until_due,a)}</span>
                  ${o.trigger_config?this._renderTriggerProgress(o):d}
                  ${this._renderMiniSparkline(o)}
                </span>
                <span class="row-actions">
                  <mwc-icon-button class="btn-complete" title="${n("complete",a)}" @click=${c=>{c.stopPropagation(),this._openCompleteDialog(e.entry_id,o.id,o.name,this._features.checklists?o.checklist:void 0,this._features.adaptive&&!!o.adaptive_config?.enabled)}}>
                    <ha-icon icon="mdi:check"></ha-icon>
                  </mwc-icon-button>
                  <mwc-icon-button class="btn-skip" title="${n("skip",a)}" .disabled=${this._actionLoading} @click=${c=>{c.stopPropagation(),this._promptSkipTask(e.entry_id,o.id)}}>
                    <ha-icon icon="mdi:skip-next"></ha-icon>
                  </mwc-icon-button>
                </span>
              </div>
    ...
    `}_renderTaskHeader(e){let t=this._lang,i=this._getObject(this._selectedEntryId)?.object.name||"",o=this._isOperator,c=e.is_done?"done":e.status==="due_soon"?"warning":e.status||"ok",u=e.is_done?n("completed",t):n(e.status||"ok",t);return r`
      <div class="task-header">
        <div class="task-header-title">
          <span class="task-name-breadcrumb" @click=${()=>this._view="task"}>${e.name}</span>
          <span class="breadcrumb-separator">·</span>
          <span class="object-name-breadcrumb" @click=${()=>this._showObject(this._selectedEntryId)}>${i}</span>
          <span class="status-chip ${c}">${u}</span>
          ${this._renderUserBadge(e)}
          ${e.nfc_tag_id?r`<span class="nfc-badge" title="${n("nfc_tag_id",t)}: ${e.nfc_tag_id}"><ha-icon icon="mdi:nfc-variant"></ha-icon> NFC</span>`:o?d:r`<span class="nfc-badge unlinked" title="${n("nfc_link_hint",t)}"
        ...
        <div class="task-header-actions">
          ...
          <ha-button appearance="plain" @click=${()=>{let p=this._getObject(this._selectedEntryId)?.object;this._openQrForTask(this._selectedEntryId,this._selectedTaskId,p?.name||"",e.name)}}><ha-icon icon="mdi:qrcode"></ha-icon> ${n("qr_code",t)}</ha-button>
          ${o?d:r`
      ...
      <div class="tab-content overview-tab">
        ...
        <div class="two-column-layout ${o?"":"single-column"}">
          ...
          <div class="right-column">
            ${Yt(e,t,this._costDurationToggle,p=>{this._costDurationToggle=p})}
          </div>
        </div>
        ${c?Gt(e,t):d}
        ${u?r`
    ...
    `}_renderChecklistCard(e){if(!this._features.checklists)return d;let t=e.checklist||[];if(t.length===0)return d;let a=this._lang;return r`
      <div class="checklist-preview-card">
        <div class="checklist-preview-header">
          ...
          <span>${n("checklist",a)} (${t.length})</span>
        </div>
    ...
    `}_renderTaskMeta(e){let t=e.documentation_url&&/^https?:\/\//i.test(e.documentation_url)?e.documentation_url:null,a=this._selectedEntryId?this._getObject(this._selectedEntryId):void 0,i=a?.object?.documentation_url,o=i&&/^https?:\/\//i.test(i)?i:null;if(!e.notes&&!t&&!o)return d;let c=this._lang;return r`
      <div class="task-meta-card">
        ...
        ${t?r`
          <div class="task-meta-row task-meta-link">
            ...
            <a href="${t}" target="_blank" rel="noopener noreferrer">${n("documentation_label",c)}</a>
          </div>
        `:d}
        ${o?r`
          <div class="task-meta-row task-meta-link">
            <ha-icon icon="mdi:book-open-variant"></ha-icon>
            <a href="${o}" target="_blank" rel="noopener noreferrer">${n("documentation_url_label",c)} (${a?.object?.name||""})</a>
          </div>
        `:d}
      </div>
    `}_renderKPIBar(e){let t=this._lang,a=e.times_performed>0?e.total_cost/e.times_performed:0,i=e.days_until_due!==null&&e.days_until_due!==void 0?e.days_until_due<0?"overdue":e.days_until_due<=e.warning_days?"warning":"":"";return r`
      <div class="kpi-bar">
        <div class="kpi-card">
          <div class="kpi-label">${n("next_due",t)}</div>
          <div class="kpi-value">${e.next_due?Z(e.next_due,t):"\u2014"}</div>
          ${this._features.schedule_time&&e.schedule_time?r`<div class="kpi-subtext">${n("at_time",t)} ${e.schedule_time}</div>`:d}
        </div>
        ...
        <div class="kpi-card">
          ...
          <div class="kpi-value">${Ct(e,t)}</div>
        ...
        <div class="kpi-card">
          ...
          <div class="kpi-value">${e.last_performed?Z(e.last_performed,t):"\u2014"}</div>
        </div>
        <div class="kpi-card">
    ...
    `}_renderRecommendationCard(e){let t=this._lang;if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return d;if(this._selectedEntryId&&this._selectedTaskId&&this._dismissedSuggestions.has(`${this._selectedEntryId}_${this._selectedTaskId}`))return d;let a=e.suggested_interval;return r`
      <div class="recommendation-card">
        <h4>${n("suggested_interval",t)}</h4>
        ${Kt(e.interval_days,a,e.interval_confidence||"medium",t)}
        <div class="recommendation-actions">
          <ha-button appearance="filled"
            @click=${()=>this._applySuggestion(this._selectedEntryId,this._selectedTaskId,a)}>
    ...
    `}_renderRecentActivities(e){let t=this._lang,a=e.history.slice(-3).reverse();if(a.length===0)return d;let i=o=>{switch(o){case"completed":return"\u2713";case"triggered":return"\u2297";case"skipped":return"\u21B7";case"reset":return"\u21BA";default:return"\xB7"}};return r`
      <div class="recent-activities">
        <h3>${n("recent_activities",t)}</h3>
        ${a.map(o=>r`
          <div class="activity-item">
            <span class="activity-icon">${i(o.type)}</span>
            <span class="activity-date">${nt(o.timestamp,t)}</span>
    ...
    `}_renderHistoryFilters(e){let t=this._lang;return r`
      <div class="history-filters-new">
        <div class="filter-chips">
          ${["completed","skipped","reset","triggered"].map(a=>{let i=e.history.filter(o=>o.type===a).length;return i===0?d:r`
              <span class="filter-chip ${this._historyFilter===a?"active":""}"
                @click=${()=>{this._historyFilter=this._historyFilter===a?null:a}}>
                ${n(a,t)} (${i})
              </span>
            `})}
          ${this._historyFilter?r`<span class="filter-chip clear" @click=${()=>{this._historyFilter=null}}>${n("show_all",t)}</span>`:d}
        </div>
        <div class="filter-controls">
          <input type="text" class="search-input" placeholder="${n("search_notes",t)}..." .value=${this._historySearch} @input=${a=>this._historySearch=a.target.value} />
        </div>
      </div>
    `}_renderHistoryList(e){let t=this._lang,a=this._historyFilter?e.history.filter(i=>i.type===this._historyFilter):e.history;if(this._historySearch){let i=this._historySearch.toLowerCase();a=a.filter(o=>o.notes?.toLowerCase().includes(i))}return a.length===0?r`<p class="empty">${n("no_history",t)}</p>`:r`
      <div class="history-timeline">
        ${[...a].reverse().map(i=>this._renderHistoryEntry(i))}
      </div>
    ...
    `}_renderHistoryEntry(e){let t=this._lang,a=["completed","reset","skipped"].includes(e.type);return r`
      <div class="history-entry">
        ...
        <div class="history-content">
          <div class="history-row">
            <strong>${n(e.type,t)}</strong>
            ${a?r`<button class="history-edit-btn"
                       title=${n("history_edit_button",t)||"Edit entry"}
                       @click=${()=>this._openHistoryEdit(e)}>
                  <ha-icon icon="mdi:pencil"></ha-icon>
                </button>`:d}
          </div>
          <div class="history-date">${nt(e.timestamp,t)}</div>
          ${e.notes?r`<div>${e.notes}</div>`:d}
          <div class="history-details">
            ${e.cost!=null?r`<span>${n("cost",t)}: ${e.cost.toFixed(2)} ${this._budget?.currency_symbol||"\u20AC"}</span>`:d}
            ${e.duration!=null?r`<span>${n("duration",t)}: ${e.duration} min</span>`:d}
            ${e.trigger_value!=null?r`<span>${n("trigger_val",t)}: ${e.trigger_value}</span>`:d}
          </div>
        </div>
      </div>
    `}_openHistoryEdit(e){if(!this._selectedEntryId||!this._selectedTaskId)return;let t={entry_id:this._selectedEntryId,task_id:this._selectedTaskId,original_timestamp:e.timestamp,type:e.type,timestamp:e.timestamp,notes:e.notes??null,cost:e.cost??null,duration:e.duration??null,completed_by:e.completed_by??null};this.shadowRoot?.querySelector("maintenance-history-edit-dialog")?.openEdit(t)}};j.styles=[Re,Mt],_([x({attribute:!1})],j.prototype,"hass",2),_([x({type:Boolean,reflect:!0})],j.prototype,"narrow",2),_([x({attribute:!1})],j.prototype,"panel",2),_([g()],j.prototype,"_objects",2),_([g()],j.prototype,"_stats",2),_([g()],j.prototype,"_view",2),_([g()],j.prototype,"_selectedEntryId",2),_([g()],j.prototype,"_selectedTaskId",2),_([g()],j.prototype,"_filterStatus",2),_([g()],j.prototype,"_filterUser",2),_([g()],j.prototype,"_unsub",2),_([g()],j.prototype,"_sparklineTooltip",2),_([g()],j.prototype,"_historyFilter",2),_([g()],j.prototype,"_budget",2),_([g()],j.prototype,"_groups",2),_([g()],j.prototype,"_detailStatsData",2),_([g()],j.prototype,"_miniStatsData",2),_([g()],j.prototype,"_features",2),_([g()],j.prototype,"_adminPanelUserIds",2),_([g()],j.prototype,"_operatorWriteEnabled",2),_([g()],j.prototype,"_defaultWarningDays",2),_([g()],j.prototype,"_actionLoading",2),_([g()],j.prototype,"_moreMenuOpen",2),_([g()],j.prototype,"_toastMessage",2),_([g()],j.prototype,"_overviewTab",2),_([g()],j.prototype,"_activeTab",2),_([g()],j.prototype,"_costDurationToggle",2),_([g()],j.prototype,"_historySearch",2),_([g()],j.prototype,"_sortMode",2),_([g()],j.prototype,"_objectSortMode",2),_([g()],j.prototype,"_groupByMode",2),_([g()],j.prototype,"_objectViewMode",2),_([g()],j.prototype,"_objectsTableColumns",2),j=_([Et("maintenance-supporter-panel")],j);export{j as MaintenanceSupporterPanel};
