...
<body>
  ...
  `}function ja(l,s,e){let t=l.trigger_config;if(!t)return d;let a=t.type||"threshold",n=a==="counter"&&t.trigger_delta_mode,o=e.isCounterEntity(t),c=t.entity_id||"",_=e.detailStatsData.get(c)||[],g=[],m=!1;if(_.length>=2)for(let f of _){let j=f.val;n&&l.trigger_baseline_value!=null&&(j-=l.trigger_baseline_value);let F={ts:f.ts,val:j};!o&&f.min!=null&&f.max!=null&&(F.min=n&&l.trigger_baseline_value!=null?f.min-l.trigger_baseline_value:f.min,F.max=n&&l.trigger_baseline_value!=null?f.max-l.trigger_baseline_value:f.max,m=!0),g.push(F)}else for(let f of l.history)f.trigger_value!=null&&g.push({ts:new Date(f.timestamp).getTime(),val:f.trigger_value});if(l.trigger_current_value!=null){let f=l.trigger_current_value;n&&l.trigger_baseline_value!=null&&(f-=l.trigger_baseline_value),g.push({ts:Date.now(),val:f})}if(g.length<2&&c&&e.hasStatsService&&!e.detailStatsData.has(c))return r`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
      <ha-icon icon="mdi:chart-line" style="--mdc-icon-size:16px;margin-right:8px;"></ha-icon>
      ${i("loading_chart",e.lang)}
    </div>`;if(g.length<2)return d;g.sort((f,j)=>f.ts-j.ts);let h=za,v=Ea,b=30,k=2,S=8,O=16,M=g.map(f=>f.val),A=Math.min(...M),x=Math.max(...M);if(m)for(let f of g)f.min!=null&&(A=Math.min(A,f.min)),f.max!=null&&(x=Math.max(x,f.max));t.trigger_above!=null&&(x=Math.max(x,t.trigger_above),A=Math.min(A,t.trigger_above)),t.trigger_below!=null&&(A=Math.min(A,t.trigger_below),x=Math.max(x,t.trigger_below));let C=null,U=null;if(a==="counter"&&t.trigger_target_value!=null){if(l.trigger_baseline_value!=null)C=l.trigger_baseline_value;else if(g.length>0){let f=[...l.history].filter(j=>j.type==="completed"||j.type==="reset").sort((j,F)=>new Date(F.timestamp).getTime()-new Date(j.timestamp).getTime())[0];if(f){let j=new Date(f.timestamp).getTime(),F=g[0],L=Math.abs(g[0].ts-j);for(let X of g){let ee=Math.abs(X.ts-j);ee<L&&(F=X,L=ee)}C=F.val}else C=g[0].val}C!=null?(U=C+t.trigger_target_value,x=Math.max(x,U),A=Math.min(A,C)):(x=Math.max(x,t.trigger_target_value),A=Math.min(A,0))}n&&l.trigger_baseline_value!=null&&(A=Math.min(A,0));let Z=x-A||1;A-=Z*.1,x+=Z*.1;let K=g[0].ts,P=g[g.length-1].ts,w=P-K||1,N=f=>b+(f-K)/w*(h-b-k),T=f=>S+(1-(f-A)/(x-A))*(v-S-O),qe=g.map(f=>`${N(f.ts).toFixed(1)},${T(f.val).toFixed(1)}`).join(" "),Tt=`M${N(g[0].ts).toFixed(1)},${v-O} `+g.map(f=>`L${N(f.ts).toFixed(1)},${T(f.val).toFixed(1)}`).join(" ")+` L${N(g[g.length-1].ts).toFixed(1)},${v-O} Z`,Te="";if(m){let f=g.filter(j=>j.min!=null&&j.max!=null);if(f.length>=2){let j=f.map(L=>`${N(L.ts).toFixed(1)},${T(L.max).toFixed(1)}`),F=[...f].reverse().map(L=>`${N(L.ts).toFixed(1)},${T(L.min).toFixed(1)}`);Te=`M${j[0]} `+j.slice(1).map(L=>`L${L}`).join(" ")+` L${F[0]} `+F.slice(1).map(L=>`L${L}`).join(" ")+" Z"}}let Qe=g[g.length-1],Mt=N(Qe.ts),Ct=T(Qe.val),Ge=f=>Math.abs(f)>=1e4?(f/1e3).toFixed(0)+"k":f>=1e3?(f/1e3).toFixed(1)+"k":f.toFixed(f<10?1:0),Nt=Ge(x),It=Ge(A),Rt=l.history.filter(f=>["completed","skipped","reset"].includes(f.type)).map(f=>({ts:new Date(f.timestamp).getTime(),type:f.type})).filter(f=>f.ts>=K&&f.ts<=P),Me=g;if(g.length>We){let f=(g.length-1)/(We-1);Me=[];for(let j=0;j<We;j++)Me.push(g[Math.round(j*f)])}return r`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${h} ${v}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_sparkline",e.lang)}">
        ...
        ${l.degradation_rate!=null&&l.degradation_trend!=="stable"&&l.degradation_trend!=="insufficient_data"&&g.length>=2?(()=>{let f=g[g.length-1],j=30,F=f.ts+j*864e5,L=f.val+l.degradation_rate*j,X=Math.min(F,P+(P-K)*.3),ee=Math.max(A,Math.min(x,L)),Pt=N(f.ts),Dt=T(f.val),Ft=N(X),Lt=T(ee);return R`<line x1="${Pt.toFixed(1)}" y1="${Dt.toFixed(1)}" x2="${Ft.toFixed(1)}" y2="${Lt.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():d}
        ...
        ${Rt.map(f=>{let j=N(f.ts),F=f.type==="completed"?"var(--success-color, #4caf50)":f.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return R`
            <line x1="${j.toFixed(1)}" y1="${S}" x2="${j.toFixed(1)}" y2="${v-O}" stroke="${F}" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
          ...
          `})}
        ${Me.map(f=>{let j=N(f.ts),F=T(f.val),L=new Date(f.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),X=`${f.val.toFixed(1)} ${s}`;return m&&f.min!=null&&f.max!=null&&(X+=` (${f.min.toFixed(1)}\u2013${f.max.toFixed(1)})`),R`<circle cx="${j.toFixed(1)}" cy="${F.toFixed(1)}" r="8" fill="transparent" tabindex="0"
...
`).map(f=>r`<div>${f}</div>`)}
  ...
  `}function At(l,s){return Ma(l,s)}function Ma(l,s){let e=l.seasonal_factors??l.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return d;let t=l.interval_analysis?.seasonal_reason,a=new Date().getMonth(),n=300,o=100,c=8,g=o-c-4,m=Math.max(...e,1.5),h=n/12,v=h*.65,b=c+g-1/m*g;return r`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${n} ${o}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_seasonal",s)}">
        ...
        ${e.map((k,S)=>{let O=k/m*g,M=S*h+(h-v)/2,A=c+g-O,x=S===a,C=k<1?"var(--success-color, #4caf50)":k>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return R`
  ...
  `}function Ia(l,s,e){let t=l.history.filter(w=>w.type==="completed"&&(w.cost!=null||w.duration!=null)).map(w=>({ts:new Date(w.timestamp).getTime(),cost:w.cost??0,duration:w.duration??0})).sort((w,N)=>w.ts-N.ts);if(t.length<2)return d;let a=t.some(w=>w.cost>0),n=t.some(w=>w.duration>0);if(!a&&!n)return d;let o=e!=="duration"&&a,c=e!=="cost"&&n,_=o||!c&&a,g=c||!o&&n,m=Ca,h=Na,v=_?32:8,b=g?32:8,k=8,S=20,O=m-v-b,M=h-k-S,A=Math.max(...t.map(w=>w.cost))||1,x=Math.max(...t.map(w=>w.duration))||1,C=Math.min(20,O/t.length*.6),U=O/t.length,Z=w=>v+U*w+U/2,K=w=>k+M-w/A*M,P=w=>k+M-w/x*M;return r`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${m} ${h}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_history",s)}">
        ...
        ${_?R`
          <text x="${v-3}" y="${k+4}" text-anchor="end" fill="var(--primary-color)" font-size="7">${A.toFixed(0)}\u20AC</text>
