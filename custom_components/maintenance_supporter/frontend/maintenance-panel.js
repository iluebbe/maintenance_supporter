...
`;var we=class{constructor(a){this._cache=new Map;this._pending=new Map;this._hass=a}updateHass(a){this._hass=a}async getDetailStats(a,e){return this._getStats(a,"hour",30,e)}async getMiniStats(a,e){return this._getStats(a,"day",14,e)}async getBatchMiniStats(a){let e=new Map,t=[];for(let p of a){let u=`${p.entityId}:day`,m=this._cache.get(u);m&&Date.now()-m.fetchedAt<3e5?e.set(p.entityId,m.points):t.push(p)}if(t.length===0)return e;let i=t.filter(p=>p.isCounter).map(p=>p.entityId),s=t.filter(p=>!p.isCounter).map(p=>p.entityId),o=new Date(Date.now()-336*60*60*1e3).toISOString(),c=[];return i.length>0&&c.push(this._fetchBatch(i,"day",o,["state","sum","change"],!0,e)),s.length>0&&c.push(this._fetchBatch(s,"day",o,["mean","min","max"],!1,e)),await Promise.all(c),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(a,e,t,i){let s=`${a}:${e}`,o=this._cache.get(s);if(o&&Date.now()-o.fetchedAt<3e5)return o.points;if(this._pending.has(s))return this._pending.get(s);let c=this._fetchAndNormalize(a,e,t,i,s);this._pending.set(s,c);try{return await c}finally{this._pending.delete(s)}}async _fetchAndNormalize(a,e,t,i,s){let o=new Date(Date.now()-t*24*60*60*1e3).toISOString(),c=i?["state","sum","change"]:["mean","min","max"];try{let u=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:o,statistic_ids:[a],period:e,types:c}))[a]||[],m=this._normalizeRows(u,i);return this._cache.set(s,{entityId:a,fetchedAt:Date.now(),period:e,points:m}),m}catch(p){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${a}:`,p),[]}}async _fetchBatch(a,e,t,i,s,o){try{let c=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:a,period:e,types:i});for(let p of a){let u=c[p]||[],m=this._normalizeRows(u,s);o.set(p,m),this._cache.set(`${p}:${e}`,{entityId:p,fetchedAt:Date.now(),period:e,points:m})}}catch(c){console.warn("[maintenance-supporter] Batch statistics fetch failed:",c)}}_normalizeRows(a,e){let t=[];for(let i of a){let s=null;if(e?s=i.state??null:s=i.mean??null,s===null)continue;let o={ts:i.start,val:s};e||(i.min!=null&&(o.min=i.min),i.max!=null&&(o.max=i.max)),t.push(o)}return t.sort((i,s)=>i.ts-s.ts),t}};var re=class{constructor(a){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=a}updateHass(a){this.hass=a}async getUsers(a=!1){let e=Date.now();if(!a&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(a,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:a,task_id:e,user_id:t})}async getTasksByUser(a){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:a})).tasks}getUserName(a){return!a||!this.usersCache?null:this.usersCache.find(t=>t.id===a)?.name||null}getUser(a){return!a||!this.usersCache?null:this.usersCache.find(e=>e.id===a)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(a){return a?a===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var q=class extends L{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._name="";this._manufacturer="";this._model="";this._serialNumber="";this._areaId="";this._installationDate="";this._entryId=null}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}openCreate(){this._entryId=null,this._name="",this._manufacturer="",this._model="",this._serialNumber="",this._areaId="",this._installationDate="",this._error="",this._open=!0}openEdit(e,t){this._entryId=e,this._name=t.name||"",this._manufacturer=t.manufacturer||"",this._model=t.model||"",this._serialNumber=t.serial_number||"",this._areaId=t.area_id||"",this._installationDate=t.installation_date||"",this._error="",this._open=!0}async _save(){if(this._name.trim()){this._loading=!0,this._error="";try{this._entryId?await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/update",entry_id:this._entryId,name:this._name,manufacturer:this._manufacturer||null,model:this._model||null,serial_number:this._serialNumber||null,area_id:this._areaId||null,installation_date:this._installationDate||null}):await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/create",name:this._name,manufacturer:this._manufacturer||null,model:this._model||null,serial_number:this._serialNumber||null,area_id:this._areaId||null,installation_date:this._installationDate||null}),this._open=!1,this.dispatchEvent(new CustomEvent("object-saved"))}catch{this._error=n("save_error",this._lang)}finally{this._loading=!1}}}_close(){this._open=!1}render(){if(!this._open)return l``;let e=this._lang,t=this._entryId?n("edit_object",e):n("new_object",e);return l`
  ...
  `,_([S({attribute:!1})],q.prototype,"hass",2),_([g()],q.prototype,"_open",2),_([g()],q.prototype,"_loading",2),_([g()],q.prototype,"_error",2),_([g()],q.prototype,"_name",2),_([g()],q.prototype,"_manufacturer",2),_([g()],q.prototype,"_model",2),_([g()],q.prototype,"_serialNumber",2),_([g()],q.prototype,"_areaId",2),_([g()],q.prototype,"_installationDate",2),_([g()],q.prototype,"_entryId",2);customElements.get("maintenance-object-dialog")||customElements.define("maintenance-object-dialog",q);var ai=["cleaning","inspection","replacement","calibration","service","custom"],si=["time_based","sensor_based","manual"],ni=["threshold","counter","state_change","runtime"],$=class extends L{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._entryId="";this._taskId=null;this._name="";this._type="custom";this._scheduleType="time_based";this._intervalDays="30";this._warningDays="7";this._intervalAnchor="completion";this._notes="";this._documentationUrl="";this._customIcon="";this._enabled=!0;this._triggerEntityId="";this._triggerEntityIds=[];this._triggerEntityLogic="any";this._triggerAttribute="";this._triggerType="threshold";this._triggerAbove="";this._triggerBelow="";this._triggerForMinutes="0";this._triggerTargetValue="";this._triggerDeltaMode=!1;this._triggerFromState="";this._triggerToState="";this._triggerTargetChanges="";this._triggerRuntimeHours="";this._suggestedAttributes=[];this._availableAttributes=[];this._entityDomain="";this._lastPerformed="";this._nfcTagId="";this._availableTags=[];this._responsibleUserId=null;this._availableUsers=[];this._userService=null}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}async openCreate(e){this._entryId=e,this._taskId=null,this._error="",this._resetFields(),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}async openEdit(e,t){if(this._entryId=e,this._taskId=t.id,this._error="",this._name=t.name,this._type=t.type,this._scheduleType=t.schedule_type,this._intervalDays=t.interval_days?.toString()||"30",this._warningDays=t.warning_days.toString(),this._intervalAnchor=t.interval_anchor||"completion",this._notes=t.notes||"",this._documentationUrl=t.documentation_url||"",this._customIcon=t.custom_icon||"",this._enabled=t.enabled!==!1,this._lastPerformed=t.last_performed||"",this._nfcTagId=t.nfc_tag_id||"",this._responsibleUserId=t.responsible_user_id||null,t.trigger_config){let i=t.trigger_config;this._triggerEntityId=i.entity_id||"",this._triggerEntityIds=i.entity_ids||(i.entity_id?[i.entity_id]:[]),this._triggerEntityLogic=i.entity_logic||"any",this._triggerAttribute=i.attribute||"",this._triggerType=i.type||"threshold",this._triggerAbove=i.trigger_above?.toString()||"",this._triggerBelow=i.trigger_below?.toString()||"",this._triggerForMinutes=i.trigger_for_minutes?.toString()||"0",this._triggerTargetValue=i.trigger_target_value?.toString()||"",this._triggerDeltaMode=i.trigger_delta_mode||!1,this._triggerFromState=i.trigger_from_state||"",this._triggerToState=i.trigger_to_state||"",this._triggerTargetChanges=i.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=i.trigger_runtime_hours?.toString()||""}else this._resetTriggerFields();this._triggerEntityId&&this._fetchEntityAttributes(this._triggerEntityId),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._warningDays="7",this._intervalAnchor="completion",this._notes="",this._documentationUrl="",this._customIcon="",this._enabled=!0,this._lastPerformed="",this._nfcTagId="",this._responsibleUserId=null,this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerForMinutes="0",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours=""}async _loadUsers(){this._userService||(this._userService=new re(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}async _loadTags(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tags/list"});this._availableTags=e.tags||[]}catch{this._availableTags=[]}}async _fetchEntityAttributes(e){if(!e||!this.hass){this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="";return}try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/entity/attributes",entity_id:e});this._entityDomain=t.domain||"",this._suggestedAttributes=t.suggested_attributes||[],this._availableAttributes=t.available_attributes||[]}catch{this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain=""}}async _save(){if(this._name.trim()){this._loading=!0,this._error="";try{let e={type:this._taskId?"maintenance_supporter/task/update":"maintenance_supporter/task/create",entry_id:this._entryId,name:this._name,task_type:this._type,schedule_type:this._scheduleType,warning_days:parseInt(this._warningDays,10)||7};if(this._taskId&&(e.task_id=this._taskId),this._scheduleType!=="manual"?this._intervalDays?(e.interval_days=parseInt(this._intervalDays,10),e.interval_anchor=this._intervalAnchor):this._taskId&&(e.interval_days=null,e.interval_anchor="completion"):this._taskId&&(e.interval_days=null,e.interval_anchor="completion"),e.notes=this._notes||null,e.documentation_url=this._documentationUrl||null,e.custom_icon=this._customIcon||null,e.enabled=this._enabled,e.last_performed=this._lastPerformed||null,e.nfc_tag_id=this._nfcTagId||null,e.responsible_user_id=this._responsibleUserId,this._scheduleType==="sensor_based"&&this._triggerEntityId){let t=this._triggerEntityIds.length>0?this._triggerEntityIds:[this._triggerEntityId],i={entity_id:t[0],entity_ids:t,type:this._triggerType};if(this._triggerAttribute&&(i.attribute=this._triggerAttribute),t.length>1&&(i.entity_logic=this._triggerEntityLogic),this._triggerType==="threshold"){if(this._triggerAbove){let s=parseFloat(this._triggerAbove);isNaN(s)||(i.trigger_above=s)}if(this._triggerBelow){let s=parseFloat(this._triggerBelow);isNaN(s)||(i.trigger_below=s)}if(this._triggerForMinutes){let s=parseInt(this._triggerForMinutes,10);isNaN(s)||(i.trigger_for_minutes=s)}}else if(this._triggerType==="counter"){if(this._triggerTargetValue){let s=parseFloat(this._triggerTargetValue);isNaN(s)||(i.trigger_target_value=s)}i.trigger_delta_mode=this._triggerDeltaMode}else if(this._triggerType==="state_change"){if(this._triggerFromState&&(i.trigger_from_state=this._triggerFromState),this._triggerToState&&(i.trigger_to_state=this._triggerToState),this._triggerTargetChanges){let s=parseInt(this._triggerTargetChanges,10);isNaN(s)||(i.trigger_target_changes=s)}}else if(this._triggerType==="runtime"&&this._triggerRuntimeHours){let s=parseFloat(this._triggerRuntimeHours);isNaN(s)||(i.trigger_runtime_hours=s)}e.trigger_config=i}else this._taskId&&(e.trigger_config=null);await this.hass.connection.sendMessagePromise(e),this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch{this._error=n("save_error",this._lang)}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return d;let e=this._lang;return l`
      ...
      <div class="select-row">
        ...
        >
          ${ni.map(t=>l`<option value=${t} ?selected=${t===this._triggerType}>${n(t,e)}</option>`)}
        </select>
      </div>
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${ai.map(i=>l`<option value=${i} ?selected=${i===this._type}>${n(i,e)}</option>`)}
            </select>
          </div>
          <div class="select-row">
            ...
            >
              ${si.map(i=>l`<option value=${i} ?selected=${i===this._scheduleType}>${n(i,e)}</option>`)}
            </select>
          </div>
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `,_([S({attribute:!1})],U.prototype,"hass",2),_([g()],U.prototype,"_open",2),_([g()],U.prototype,"_title",2),_([g()],U.prototype,"_message",2),_([g()],U.prototype,"_confirmText",2),_([g()],U.prototype,"_danger",2),_([g()],U.prototype,"_inputLabel",2),_([g()],U.prototype,"_inputType",2),_([g()],U.prototype,"_inputValue",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",U);var oi=["EUR","USD","GBP","JPY","CHF","CAD","AUD","CNY","INR","BRL"],B=class extends L{constructor(){super(...arguments);this.budget=null;this._settings=null;this._loading=!0;this._importCsv="";this._importLoading=!1;this._includeHistory=!0;this._toast="";this._loaded=!1}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._loaded&&(this._loaded=!0,this._loadSettings())}async _loadSettings(){this._loading=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});this._settings=e}catch{}this._loading=!1}async _updateSetting(e,t){try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/update",settings:{[e]:t}});this._settings=i,this._showToast(n("settings_saved",this._lang)),this.dispatchEvent(new CustomEvent("settings-changed"))}catch{this._showToast(n("action_error",this._lang))}}_showToast(e){this._toast=e,setTimeout(()=>{this._toast=""},3e3)}_downloadFile(e,t,i){let s=new Blob([e],{type:i}),o=URL.createObjectURL(s),c=document.createElement("a");c.href=o,c.download=t,c.click(),URL.revokeObjectURL(o)}render(){let e=this._lang;return this._loading||!this._settings?l`<div class="settings-loading">Loading…</div>`:l`
  ...
  `,_([S({attribute:!1})],B.prototype,"hass",2),_([S({attribute:!1})],B.prototype,"features",2),_([S({attribute:!1})],B.prototype,"budget",2),_([g()],B.prototype,"_settings",2),_([g()],B.prototype,"_loading",2),_([g()],B.prototype,"_importCsv",2),_([g()],B.prototype,"_importLoading",2),_([g()],B.prototype,"_includeHistory",2),_([g()],B.prototype,"_toast",2);customElements.define("maintenance-settings-view",B);var li=300,di=140,qe=27;function ht(r,a){let e=r.trigger_config;if(!e)return d;let t=a.lang,i=r.trigger_entity_info,s=r.trigger_entity_infos,o=i?.friendly_name||e.entity_id||"\u2014",c=e.entity_id||"",p=e.entity_ids||(c?[c]:[]),u=i?.unit_of_measurement||"",m=r.trigger_current_value,v=e.type||"threshold",b=p.length>1;return l`
    <h3>${n("trigger",t)}</h3>
    <div class="trigger-card">
      <div class="trigger-header">
        ...
        <div>
          ${b?l`
            ...
            <div class="trigger-entity-id">${p.map((f,k)=>l`${k>0?", ":""}<span class="entity-link" @click=${T=>ae(T,f)}>${f}</span>`)}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `:l`
      ...
      <div class="trigger-limits">
        ${v==="threshold"?l`
          ${e.trigger_above!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${n("threshold_above",t)}: ${e.trigger_above} ${u}</span>`:d}
          ${e.trigger_below!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${n("threshold_below",t)}: ${e.trigger_below} ${u}</span>`:d}
        ...
        `:d}
        ${v==="counter"?l`
          ${e.trigger_target_value!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${n("target_value",t)}: ${e.trigger_target_value} ${u}</span>`:d}
        `:d}
        ${v==="state_change"?l`
        ...
        ${v==="compound"?l`
          ...
          `)}
        `:d}
        ${i?.min!=null?l`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${n("min",t)}: ${i.min} ${u}</span>`:d}
        ${i?.max!=null?l`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${n("max",t)}: ${i.max} ${u}</span>`:d}
      </div>
  ...
  `}function ci(r,a,e){let t=r.trigger_config;if(!t)return d;let i=t.type||"threshold",s=i==="counter"&&t.trigger_delta_mode,o=e.isCounterEntity(t),c=t.entity_id||"",p=e.detailStatsData.get(c)||[],u=[],m=!1;if(p.length>=2)for(let h of p){let w=h.val;s&&r.trigger_baseline_value!=null&&(w-=r.trigger_baseline_value);let R={ts:h.ts,val:w};!o&&h.min!=null&&h.max!=null&&(R.min=s&&r.trigger_baseline_value!=null?h.min-r.trigger_baseline_value:h.min,R.max=s&&r.trigger_baseline_value!=null?h.max-r.trigger_baseline_value:h.max,m=!0),u.push(R)}else for(let h of r.history)h.trigger_value!=null&&u.push({ts:new Date(h.timestamp).getTime(),val:h.trigger_value});if(r.trigger_current_value!=null){let h=r.trigger_current_value;s&&r.trigger_baseline_value!=null&&(h-=r.trigger_baseline_value),u.push({ts:Date.now(),val:h})}if(u.length<2&&c&&e.hasStatsService&&!e.detailStatsData.has(c))return l`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
      <ha-icon icon="mdi:chart-line" style="--mdc-icon-size:16px;margin-right:8px;"></ha-icon>
      ${n("loading_chart",e.lang)}
    </div>`;if(u.length<2)return d;u.sort((h,w)=>h.ts-w.ts);let v=li,b=di,f=30,k=2,T=8,H=16,C=u.map(h=>h.val),A=Math.min(...C),y=Math.max(...C);if(m)for(let h of u)h.min!=null&&(A=Math.min(A,h.min)),h.max!=null&&(y=Math.max(y,h.max));t.trigger_above!=null&&(y=Math.max(y,t.trigger_above),A=Math.min(A,t.trigger_above)),t.trigger_below!=null&&(A=Math.min(A,t.trigger_below),y=Math.max(y,t.trigger_below));let z=null,O=null;if(i==="counter"&&t.trigger_target_value!=null){if(r.trigger_baseline_value!=null)z=r.trigger_baseline_value;else if(u.length>0){let h=[...r.history].filter(w=>w.type==="completed"||w.type==="reset").sort((w,R)=>new Date(R.timestamp).getTime()-new Date(w.timestamp).getTime())[0];if(h){let w=new Date(h.timestamp).getTime(),R=u[0],N=Math.abs(u[0].ts-w);for(let J of u){let Z=Math.abs(J.ts-w);Z<N&&(R=J,N=Z)}z=R.val}else z=u[0].val}z!=null?(O=z+t.trigger_target_value,y=Math.max(y,O),A=Math.min(A,z)):(y=Math.max(y,t.trigger_target_value),A=Math.min(A,0))}s&&r.trigger_baseline_value!=null&&(A=Math.min(A,0));let G=y-A||1;A-=G*.1,y+=G*.1;let W=u[0].ts,D=u[u.length-1].ts,x=D-W||1,I=h=>f+(h-W)/x*(v-f-k),M=h=>T+(1-(h-A)/(y-A))*(b-T-H),Ee=u.map(h=>`${I(h.ts).toFixed(1)},${M(h.val).toFixed(1)}`).join(" "),kt=`M${I(u[0].ts).toFixed(1)},${b-H} `+u.map(h=>`L${I(h.ts).toFixed(1)},${M(h.val).toFixed(1)}`).join(" ")+` L${I(u[u.length-1].ts).toFixed(1)},${b-H} Z`,Ae="";if(m){let h=u.filter(w=>w.min!=null&&w.max!=null);if(h.length>=2){let w=h.map(N=>`${I(N.ts).toFixed(1)},${M(N.max).toFixed(1)}`),R=[...h].reverse().map(N=>`${I(N.ts).toFixed(1)},${M(N.min).toFixed(1)}`);Ae=`M${w[0]} `+w.slice(1).map(N=>`L${N}`).join(" ")+` L${R[0]} `+R.slice(1).map(N=>`L${N}`).join(" ")+" Z"}}let Oe=u[u.length-1],wt=I(Oe.ts),Et=M(Oe.val),Ue=h=>Math.abs(h)>=1e4?(h/1e3).toFixed(0)+"k":h>=1e3?(h/1e3).toFixed(1)+"k":h.toFixed(h<10?1:0),At=Ue(y),St=Ue(A),Tt=r.history.filter(h=>["completed","skipped","reset"].includes(h.type)).map(h=>({ts:new Date(h.timestamp).getTime(),type:h.type})).filter(h=>h.ts>=W&&h.ts<=D),Se=u;if(u.length>qe){let h=(u.length-1)/(qe-1);Se=[];for(let w=0;w<qe;w++)Se.push(u[Math.round(w*h)])}return l`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${v} ${b}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_sparkline",e.lang)}">
        ...
        ${Tt.map(h=>{let w=I(h.ts),R=h.type==="completed"?"var(--success-color, #4caf50)":h.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return j`
            <line x1="${w.toFixed(1)}" y1="${T}" x2="${w.toFixed(1)}" y2="${b-H}" stroke="${R}" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
          ...
          `})}
        ${Se.map(h=>{let w=I(h.ts),R=M(h.val),N=new Date(h.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),J=`${h.val.toFixed(1)} ${a}`;return m&&h.min!=null&&h.max!=null&&(J+=` (${h.min.toFixed(1)}\u2013${h.max.toFixed(1)})`),j`<circle cx="${w.toFixed(1)}" cy="${R.toFixed(1)}" r="8" fill="transparent" tabindex="0"
            @mouseenter=${Z=>gt(Z,`${N}
${J}`,e.setTooltip)}
            @focus=${Z=>gt(Z,`${N}
${J}`,e.setTooltip)}
...
`).map(h=>l`<div>${h}</div>`)}
      ...
      <div class="prediction-grid">
        ...
        ${i?l`
          <div class="prediction-item">
            ...
            ${r.threshold_prediction_date?l`<span class="prediction-date">${Q(r.threshold_prediction_date,a)}</span>`:d}
  ...
  `}function vt(r,a){let e=r.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return d;let s=r.interval_days??0,o=r.suggested_interval??s;return l`
    <div class="weibull-section">
      ...
      ${pi(t,i,s,o,a)}
      ${_i(e,a)}
      ${e?.confidence_interval_low!=null?gi(e,r,a):d}
    </div>
  ...
  `}function pi(r,a,e,t,i){let f=Math.max(e,t,a,1)*1.3,k=50,T=[];for(let D=0;D<=k;D++){let x=D/k*f,I=1-Math.exp(-Math.pow(x/a,r)),M=32+x/f*260,Ee=136-I*128;T.push([M,Ee])}let H=T.map(([D,x])=>`${D.toFixed(1)},${x.toFixed(1)}`).join(" "),C="M32,136 "+T.map(([D,x])=>`L${D.toFixed(1)},${x.toFixed(1)}`).join(" ")+` L${T[k][0].toFixed(1)},136 Z`,A=32+e/f*260,y=1-Math.exp(-Math.pow(e/a,r)),z=136-y*128,O=((1-y)*100).toFixed(0),G=32+t/f*260,W=[0,.25,.5,.75,1];return l`
  ...
  `}function gi(r,a,e){let t=r.confidence_interval_low,i=r.confidence_interval_high,s=a.suggested_interval??a.interval_days??0,o=a.interval_days??0,c=Math.max(0,t-5),u=i+5-c,m=(t-c)/u*100,v=(i-t)/u*100,b=(s-c)/u*100,f=o>0?(o-c)/u*100:-1;return l`
  ...
  `}function yt(r,a){return hi(r,a)}function hi(r,a){let e=r.seasonal_factors??r.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return d;let t=r.interval_analysis?.seasonal_reason,i=new Date().getMonth(),s=300,o=100,c=8,u=o-c-4,m=Math.max(...e,1.5),v=s/12,b=v*.65,f=c+u-1/m*u;return l`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${s} ${o}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_seasonal",a)}">
        ...
        ${e.map((k,T)=>{let H=k/m*u,C=T*v+(v-b)/2,A=c+u-H,y=T===i,z=k<1?"var(--success-color, #4caf50)":k>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return j`
  ...
  `}var mi=300,vi=200;function xt(r,a,e,t){let i=r.history.filter(c=>c.type==="completed"&&(c.cost!=null||c.duration!=null));if(i.length<2)return d;let s=i.some(c=>(c.cost??0)>0),o=i.some(c=>(c.duration??0)>0);return!s&&!o?d:l`
  ...
  `}function fi(r,a,e){let t=r.history.filter(x=>x.type==="completed"&&(x.cost!=null||x.duration!=null)).map(x=>({ts:new Date(x.timestamp).getTime(),cost:x.cost??0,duration:x.duration??0})).sort((x,I)=>x.ts-I.ts);if(t.length<2)return d;let i=t.some(x=>x.cost>0),s=t.some(x=>x.duration>0);if(!i&&!s)return d;let o=e!=="duration"&&i,c=e!=="cost"&&s,p=o||!c&&i,u=c||!o&&s,m=mi,v=vi,b=p?32:8,f=u?32:8,k=8,T=20,H=m-b-f,C=v-k-T,A=Math.max(...t.map(x=>x.cost))||1,y=Math.max(...t.map(x=>x.duration))||1,z=Math.min(20,H/t.length*.6),O=H/t.length,G=x=>b+O*x+O/2,W=x=>k+C-x/A*C,D=x=>k+C-x/y*C;return l`
    <div class="sparkline-container">
  ...
  `}var bi=60,yi=20,$t=30,E=class extends L{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._sparklineTooltip=null;this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1};this._actionLoading=!1;this._moreMenuOpen=!1;this._toastMessage="";this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab="dashboard";this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._deepLinkHandled=!1;this._onDialogEvent=async()=>{try{await this._loadData()}catch{}}}get _lang(){return this.hass?.language||"en"}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler);let e=localStorage.getItem("maintenance_supporter_sort");e&&["due_date","object","type","task_name"].includes(e)&&(this._sortMode=e)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new we(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new re(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,i,s,o]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);e&&(this._objects=e.objects),t&&(this._stats=t),i&&(this._budget=i),s&&(this._groups=s.groups||{}),o&&(this._features=o.features),this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("entry_id");if(!t)return;this._deepLinkHandled=!0;let i=e.get("task_id"),s=e.get("action"),o=window.location.pathname+window.location.hash;history.replaceState(history.state,"",o);let c=this._getObject(t);if(!c){this._showOverview();return}if(i){let p=c.tasks.find(u=>u.id===i);if(!p){this._showObject(t);return}this._showTask(t,i),s==="complete"&&requestAnimationFrame(()=>{this._openCompleteDialog(t,i,p.name,this._features.checklists?p.checklist:void 0,this._features.adaptive&&!!p.adaptive_config?.enabled)})}else this._showObject(t)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let i=await this._statsService.getDetailStats(e,t),s=new Map(this._detailStatsData);s.set(e,i),this._detailStatsData=s}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let i of this._objects)for(let s of i.tasks){let o=s.trigger_config?.entity_id;o&&e.push({entityId:o,isCounter:this._isCounterEntity(s.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let p of this._objects)for(let u of p.tasks)if(!(this._filterStatus&&u.status!==this._filterStatus)){if(this._filterUser){let m=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(u.responsible_user_id!==m)continue}e.push({entry_id:p.entry_id,task_id:u.id,object_name:p.object.name,task_name:u.name,type:u.type,schedule_type:u.schedule_type,status:u.status,days_until_due:u.days_until_due??null,next_due:u.next_due??null,trigger_active:u.trigger_active,trigger_current_value:u.trigger_current_value??null,trigger_current_delta:u.trigger_current_delta??null,trigger_config:u.trigger_config??null,trigger_entity_info:u.trigger_entity_info??null,times_performed:u.times_performed,total_cost:u.total_cost,interval_days:u.interval_days??null,interval_anchor:u.interval_anchor??null,history:u.history||[],enabled:u.enabled,nfc_tag_id:u.nfc_tag_id??null})}let t={overdue:0,triggered:1,due_soon:2,ok:3},i=(p,u)=>(t[p.status]??9)-(t[u.status]??9),s=(p,u)=>(p.days_until_due??99999)-(u.days_until_due??99999),o=(p,u)=>i(p,u)||s(p,u),c={due_date:o,object:(p,u)=>p.object_name.localeCompare(u.object_name)||o(p,u),type:(p,u)=>p.type.localeCompare(u.type)||o(p,u),task_name:(p,u)=>p.task_name.localeCompare(u.task_name)};return e.sort(c[this._sortMode]),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(s=>s.id===t)}_pushPanelState(e,t,i){let s={msp_view:e,msp_entry:t||null,msp_task:i||null};history.pushState(s,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let i=this._getTask(t.msp_entry,t.msp_task);i?.trigger_config?.entity_id&&this._fetchDetailStats(i.trigger_config.entity_id,this._isCounterEntity(i.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null;let i=this._getTask(e,t);if(i?.trigger_config?.entity_id){let s=i.trigger_config.entity_id,o=this._isCounterEntity(i.trigger_config);this._fetchDetailStats(s,o)}}_showToast(e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}async _deleteObject(e){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:n("delete",this._lang),message:n("confirm_delete_object",this._lang),confirmText:n("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}async _deleteTask(e,t){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:n("delete",this._lang),message:n("confirm_delete_task",this._lang),confirmText:n("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}async _skipTask(e,t,i){this._actionLoading=!0;try{let s={type:"maintenance_supporter/task/skip",entry_id:e,task_id:t};i&&(s.reason=i),await this.hass.connection.sendMessagePromise(s),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}finally{this._actionLoading=!1}}async _resetTask(e,t,i){this._actionLoading=!0;try{let s={type:"maintenance_supporter/task/reset",entry_id:e,task_id:t};i&&(s.date=i),await this.hass.connection.sendMessagePromise(s),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}finally{this._actionLoading=!1}}async _applySuggestion(e,t,i){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:i}),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}async _promptSkipTask(e,t){let i=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!i)return;let s=await i.prompt({title:n("skip",this._lang),message:n("skip_reason_prompt",this._lang),confirmText:n("skip",this._lang),inputLabel:n("reason_optional",this._lang),inputType:"text"});s.confirmed&&this._skipTask(e,t,s.value||void 0)}async _promptResetTask(e,t){let i=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!i)return;let s=await i.prompt({title:n("reset",this._lang),message:n("reset_date_prompt",this._lang),confirmText:n("reset",this._lang),inputLabel:n("reset_date_optional",this._lang),inputType:"date"});s.confirmed&&this._resetTask(e,t,s.value||void 0)}_dismissSuggestion(e,t){e&&t&&this._dismissedSuggestions.add(`${e}_${t}`),this.requestUpdate()}_openCompleteDialog(e,t,i,s,o){let c=this.shadowRoot.querySelector("maintenance-complete-dialog");c&&(c.entryId=e,c.taskId=t,c.taskName=i,c.lang=this._lang,c.checklist=s||[],c.adaptiveEnabled=!!o,c.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,i,s){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,i,s)}render(){return l`
    ...
    `}_renderOverviewRow(e){let t=this._lang,i=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,s=0,o=$e.ok,c=!1;if(i&&e.days_until_due!==null){let p=(e.interval_days-e.days_until_due)/e.interval_days*100;s=Math.max(0,Math.min(100,p)),c=p>100,e.status==="overdue"?o=$e.overdue:e.status==="due_soon"&&(o=$e.due_soon)}return l`
    ...
    `}_renderMiniSparkline(e){if(!e.trigger_config?.entity_id)return d;let t=e.trigger_config.entity_id,i=this._miniStatsData.get(t)||[],s=[];if(i.length>=2)s=i.map(y=>({ts:y.ts,val:y.val}));else{if(!e.history)return d;for(let y of e.history)y.trigger_value!=null&&s.push({ts:new Date(y.timestamp).getTime(),val:y.trigger_value})}if(e.trigger_current_value!=null&&s.push({ts:Date.now(),val:e.trigger_current_value}),s.length<2)return d;s.sort((y,z)=>y.ts-z.ts);let o=bi,c=yi,p=s.map(y=>y.val),u=Math.min(...p),m=Math.max(...p),v=m-u||1;u-=v*.1,m+=v*.1;let b=s[0].ts,k=s[s.length-1].ts-b||1,T=y=>(y-b)/k*o,H=y=>2+(1-(y-u)/(m-u))*(c-4),C=s;if(C.length>$t){let y=Math.ceil(C.length/$t);C=C.filter((z,O)=>O%y===0||O===C.length-1)}let A=C.map(y=>`${T(y.ts).toFixed(1)},${H(y.val).toFixed(1)}`).join(" ");return l`
    ...
    `}_renderObjectDetail(){if(!this._selectedEntryId)return d;let e=this._getObject(this._selectedEntryId);if(!e)return l`<p>Object not found.</p>`;let t=e.object,i=this._lang;return l`
      <div class="detail-section">
        ...
        ${t.installation_date?l`<p class="meta">${n("installed",i)}: ${Q(t.installation_date,i)}</p>`:d}
    ...
    `}};E.styles=[ut,pt],_([S({attribute:!1})],E.prototype,"hass",2),_([S({type:Boolean,reflect:!0})],E.prototype,"narrow",2),_([S({attribute:!1})],E.prototype,"panel",2),_([g()],E.prototype,"_objects",2),_([g()],E.prototype,"_stats",2),_([g()],E.prototype,"_view",2),_([g()],E.prototype,"_selectedEntryId",2),_([g()],E.prototype,"_selectedTaskId",2),_([g()],E.prototype,"_filterStatus",2),_([g()],E.prototype,"_filterUser",2),_([g()],E.prototype,"_unsub",2),_([g()],E.prototype,"_sparklineTooltip",2),_([g()],E.prototype,"_historyFilter",2),_([g()],E.prototype,"_budget",2),_([g()],E.prototype,"_groups",2),_([g()],E.prototype,"_detailStatsData",2),_([g()],E.prototype,"_miniStatsData",2),_([g()],E.prototype,"_features",2),_([g()],E.prototype,"_actionLoading",2),_([g()],E.prototype,"_moreMenuOpen",2),_([g()],E.prototype,"_toastMessage",2),_([g()],E.prototype,"_overviewTab",2),_([g()],E.prototype,"_activeTab",2),_([g()],E.prototype,"_costDurationToggle",2),_([g()],E.prototype,"_historySearch",2),_([g()],E.prototype,"_sortMode",2),E=_([ot("maintenance-supporter-panel")],E);export{E as MaintenanceSupporterPanel};
