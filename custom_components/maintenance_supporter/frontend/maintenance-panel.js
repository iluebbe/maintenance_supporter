...
`;var bt=class{constructor(r){this._cache=new Map;this._pending=new Map;this._hass=r}updateHass(r){this._hass=r}async getDetailStats(r,e,t=30){return this._getStats(r,t<=35?"hour":"day",t,e)}async getMiniStats(r,e){return this._getStats(r,"day",14,e)}async getBatchMiniStats(r){let e=new Map,t=[];for(let h of r){let u=`${h.entityId}:day:14`,g=this._cache.get(u);g&&Date.now()-g.fetchedAt<3e5?e.set(h.entityId,g.points):t.push(h)}if(t.length===0)return e;let i=t.filter(h=>h.isCounter).map(h=>h.entityId),a=t.filter(h=>!h.isCounter).map(h=>h.entityId),l=new Date(Date.now()-336*60*60*1e3).toISOString(),c=[];return i.length>0&&c.push(this._fetchBatch(i,"day",l,["state","sum","change"],!0,e)),a.length>0&&c.push(this._fetchBatch(a,"day",l,["mean","min","max"],!1,e)),await Promise.all(c),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(r,e,t,i){let a=`${r}:${e}:${t}`,l=this._cache.get(a);if(l&&Date.now()-l.fetchedAt<3e5)return l.points;if(this._pending.has(a))return this._pending.get(a);let c=this._fetchAndNormalize(r,e,t,i,a);this._pending.set(a,c);try{return await c}finally{this._pending.delete(a)}}async _fetchAndNormalize(r,e,t,i,a){let l=new Date(Date.now()-t*24*60*60*1e3).toISOString(),c=i?["state","sum","change"]:["mean","min","max"];try{let u=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:l,statistic_ids:[r],period:e,types:c}))[r]||[],g=this._normalizeRows(u,i);return this._cache.set(a,{entityId:r,fetchedAt:Date.now(),period:e,points:g}),g}catch(h){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${r}:`,h),[]}}async _fetchBatch(r,e,t,i,a,l){try{let c=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:r,period:e,types:i});for(let h of r){let u=c[h]||[],g=this._normalizeRows(u,a);l.set(h,g),this._cache.set(`${h}:${e}:14`,{entityId:h,fetchedAt:Date.now(),period:e,points:g})}}catch(c){console.warn("[maintenance-supporter] Batch statistics fetch failed:",c)}}_normalizeRows(r,e){let t=[];for(let i of r){let a=null;if(e?a=i.state??null:a=i.mean??null,a===null)continue;let l={ts:i.start,val:a};e||(i.min!=null&&(l.min=i.min),i.max!=null&&(l.max=i.max)),t.push(l)}return t.sort((i,a)=>i.ts-a.ts),t}};var ye=class{constructor(r){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=r}updateHass(r){this.hass=r}async getUsers(r=!1){let e=Date.now();if(!r&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(r,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:r,task_id:e,user_id:t})}async getTasksByUser(r){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:r})).tasks}getUserName(r){return!r||!this.usersCache?null:this.usersCache.find(t=>t.id===r)?.name||null}getUser(r){return!r||!this.usersCache?null:this.usersCache.find(e=>e.id===r)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(r){return r?r===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var G=class extends S{constructor(){super(...arguments);this.label="";this.value="";this.placeholder="";this.type="text";this.required=!1;this.disabled=!1}_onInput(e){let t=e.target.value;this.value=t,this.dispatchEvent(new CustomEvent("input",{bubbles:!0,composed:!0,detail:{value:t}}))}render(){return o`
  ...
  `,d([y({attribute:!1})],Z.prototype,"hass",2),d([y({attribute:!1})],Z.prototype,"entryId",2),d([y({attribute:!1})],Z.prototype,"parts",2),d([y({type:Boolean})],Z.prototype,"canWrite",2),d([_()],Z.prototype,"_editing",2),d([_()],Z.prototype,"_busy",2),d([_()],Z.prototype,"_error",2),d([_()],Z.prototype,"_restockFor",2),d([_()],Z.prototype,"_restockQty",2),d([_()],Z.prototype,"_restockInvalid",2);customElements.define("maintenance-parts-section",Z);var $s=["manual","warranty","invoice","spare_parts","photo","other"],ws={manual:"mdi:book-open-variant",warranty:"mdi:shield-check",invoice:"mdi:receipt-text-outline",spare_parts:"mdi:cog-outline",photo:"mdi:image-outline",other:"mdi:file-document-outline"},ee=class extends S{constructor(){super(...arguments);this.canWrite=!1;this._docs=[];this._loaded=!1;this._busy=!1;this._error="";this._attachId="";this._loadedKey="";this._localeReady=!1}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),this.hass&&!this._localeReady&&(this._localeReady=!0,Y(this._lang).then(()=>this.requestUpdate()));let t=`${this.entryId}|${this.taskId}`;this.hass&&this.entryId&&this.taskId&&this._loadedKey!==t&&(this._loadedKey=t,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:this.entryId});this._docs=e.documents||[],this._loaded=!0,this._error=""}catch(e){this._error=M(e,this._lang),this._loaded=!0}}_linked(){return this._docs.filter(e=>(e.task_ids||[]).includes(this.taskId))}_available(){return this._docs.filter(e=>!(e.task_ids||[]).includes(this.taskId))}async _setTaskIds(e,t){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,task_ids:t}),await this._load()}catch(i){this._error=M(i,this._lang)}finally{this._busy=!1}}_link(){let e=this._docs.find(t=>t.id===this._attachId);e&&(this._attachId="",this._setTaskIds(e,[...e.task_ids||[],this.taskId]))}_unlink(e){this._setTaskIds(e,(e.task_ids||[]).filter(t=>t!==this.taskId))}_isPdf(e){return e.mime==="application/pdf"||(e.filename||"").toLowerCase().endsWith(".pdf")}_pageFor(e){return this._isPdf(e)?e.task_pages?.[this.taskId]:void 0}async _open(e){if(e.kind==="weblink"){window.open(e.url,"_blank","noopener");return}let t=this._pageFor(e),i=t?`#page=${t}`:"",a=window.open("about:blank","_blank");try{let l=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300});a&&(a.location.href=new URL(l.path+i,window.location.origin).href)}catch(l){a&&a.close(),this._error=M(l,this._lang)}}async _setPage(e,t){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,task_pages:{[this.taskId]:t}}),await this._load()}catch(i){this._error=M(i,this._lang)}finally{this._busy=!1}}async _download(e){try{let t=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:30});vt(t.path,e.filename||e.title||"document")}catch(t){this._error=M(t,this._lang)}}render(){if(!this._loaded||this._docs.length===0)return p;let e=this._lang,t=this._linked(),i=this._available();return o`
    ...
    `}_renderRow(e,t){let i=e.kind==="file",a=this._isPdf(e),l=e.task_pages?.[this.taskId],c=(e.tags||[]).find(u=>$s.includes(u))||"other",h=i?de(e.size):s("doc_link_badge",t);return o`
      <div class="tdoc-row">
        <ha-icon class="tdoc-icon" icon=${i?ws[c]:"mdi:link-variant"}></ha-icon>
        <div
          class="tdoc-info"
          role="button"
          tabindex="0"
...
`).map(h=>h.trim()).filter(Boolean).slice(0,100);e.checklist=c.length?c:null}if(this.completionActionsEnabled){let c=this._actionService.trim();if(c&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(c)){let m={service:c},v=this._actionTargetEntity.trim();v&&(m.target={entity_id:v});let b=this._buildActionData();Object.keys(b).length>0&&(m.data=b),e.on_complete_action=m}else e.on_complete_action=null;let h={};this._qcNotes.trim()&&(h.notes=this._qcNotes.trim());let u=parseFloat(this._qcCost);!isNaN(u)&&u>=0&&(h.cost=u);let g=parseInt(this._qcDuration,10);!isNaN(g)&&g>=0&&(h.duration=g),this._qcFeedback&&(h.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(h).length?h:null}let i=await this.hass.connection.sendMessagePromise(e),a=this._taskId||i?.task_id,l=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(a&&this._scheduleType==="sensor_based"&&l)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:a,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=M(e,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return p;let e=this._lang,t=this._triggerType==="compound";return o`
    ...
    `}_renderConditionTypeFields(e,t){let i=this._lang;return e.type==="threshold"?o`
      ...
      ${this._endsMode==="until"?o`
        ...
        ></ms-textfield>`:p}
    `}_renderCalendarFields(){let e=this._lang,t=Ms(e);if(this._scheduleType==="weekdays")return o`
...
`;var re=class extends S{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter)}getCardSize(){return 6}get _lang(){return this.hass?.language||"en"}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){super.updated(e);let t=this.hass?.language;if(t&&!dt(t)&&Y(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{let e=await this.hass.connection.subscribeMessage(t=>{let i=t;this._objects=i.objects},{type:"maintenance_supporter/subscribe"});if(!this.isConnected){e();return}this._unsub=e}catch{}}_onEventClick(e){if(e.history_timestamp){this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}));return}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}render(){if(!this.hass)return p;let e=this._lang,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.title,l=null;this._userFilter&&(l=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let c=new Date;c.setHours(0,0,0,0);let h=this._pastDays>0,u=h?Ei(this._objects,c,this._pastDays,l):ki(this._objects,c,this._windowDays,l),g=Je(c),m=this._windowDays===365||h,v=m?u.filter(f=>f.events.length>0):u,b=f=>{let j=`cal-status-${f.status}`,C=f.projected?"cal-event-projected":"",L=f.status==="overdue"&&f.days_until_due!=null?` (${Math.abs(f.days_until_due)}d ${s("overdue",e).toLowerCase()})`:"",R=f.projected&&f.interval_days?o`<span class="cal-event-recur">${f.interval_unit&&f.interval_unit!=="days"?`${f.interval_days} ${s("unit_"+f.interval_unit,e)}`:s("cal_every_n_days",e).replace("{n}",String(f.interval_days))}</span>`:p,T=f.schedule_type==="sensor_based",J=T?o`<ha-icon class="cal-event-icon cal-source-sensor"
                ...
                icon="${f.adaptive_enabled?"mdi:clock-time-four-outline":"mdi:clock-outline"}"></ha-icon>`,X=T&&f.prediction_confidence&&f.status!=="triggered"&&!f.projected?o`<span class="cal-event-prediction cal-conf-${f.prediction_confidence}">
            ${s("cal_predicted",e)} · ${s(`cal_confidence_${f.prediction_confidence}`,e)}
          </span>`:p,Q=this._stats?.budget?.currency_symbol||Se,U=f.history_type?s(f.history_type,e):s(f.status,e);return o`
        <div class="cal-event ${C}"
          ...
          ${f.avg_cost!=null&&f.avg_cost>0?o`<span class="cal-event-cost">${f.avg_cost.toFixed(0)} ${Q}</span>`:p}
        </div>
      `},$=f=>{let[j,C,L]=f.date.split("-").map(Number),R=new Date(j,C-1,L),T=f.date===g,J=R.toLocaleDateString(e,{weekday:"short"}),X=R.toLocaleDateString(e,{month:"long"});return o`
        <div class="cal-day-row">
          <div class="cal-day-pill ${T?"cal-today":""}">
            <span class="cal-pill-weekday">${J}</span>
...
<body>
  ...
  `}function Bt(n,r,e){if(!n.trigger_config?.entity_id)return p;let t=n.trigger_config.entity_id,i=r.get(t)||[],a=[];if(i.length>=2)a=i.map(T=>({ts:T.ts,val:T.val}));else{if(!n.history)return p;for(let T of n.history)T.trigger_value!=null&&a.push({ts:new Date(T.timestamp).getTime(),val:T.trigger_value})}if(n.trigger_current_value!=null&&a.push({ts:Date.now(),val:n.trigger_current_value}),a.length<2)return p;a.sort((T,J)=>T.ts-J.ts);let l=Us,c=Bs,h=a.map(T=>T.val),u=Math.min(...h),g=Math.max(...h),m=g-u||1;u-=m*.1,g+=m*.1;let v=a[0].ts,$=a[a.length-1].ts-v||1,f=T=>(T-v)/$*l,j=T=>2+(1-(T-u)/(g-u))*(c-4),C=a;if(C.length>Mi){let T=Math.ceil(C.length/Mi);C=C.filter((J,X)=>X%T===0||X===C.length-1)}let L=C.map(T=>`${f(T.ts).toFixed(1)},${j(T.val).toFixed(1)}`).join(" "),R=n.trigger_active?"var(--error-color, #f44336)":"var(--primary-color)";return o`
  ...
  `}function Ii(n,r){let e=r;if(n.days_until_due==null||!n.interval_days||n.interval_days<=0)return p;let{pct:t,overflow:i}=_t(n.interval_days,n.days_until_due,n.interval_unit),a="var(--success-color, #4caf50)";return n.status==="overdue"?a="var(--error-color, #f44336)":n.status==="due_soon"&&(a="var(--warning-color, #ff9800)"),o`
    ...
    `}_renderSvg(e,t){let i=this.lang,a=e-ce-me,l=Et-Vs,c=l-ve,h=1/0,u=-1/0;for(let k of t)h=Math.min(h,k.min??k.val),u=Math.max(u,k.max??k.val);this.thresholdAbove!=null&&(h=Math.min(h,this.thresholdAbove),u=Math.max(u,this.thresholdAbove)),this.thresholdBelow!=null&&(h=Math.min(h,this.thresholdBelow),u=Math.max(u,this.thresholdBelow)),this.targetValue!=null&&(h=Math.min(h,this.targetValue),u=Math.max(u,this.targetValue)),this.forceZero&&(h=Math.min(h,0));let g=(u-h||1)*.06,m=this.forceZero&&h>=0?0:h-g,{ticks:v,niceMin:b,niceMax:$}=et(m,u+g,4);this.forceZero&&h>=0&&b<0&&(b=0,v=v.filter(k=>k>=0));let f=t[0].ts,j=t[t.length-1].ts,C=j-f||1,L=wt(f,j),R=k=>ce+(k-f)/C*a,T=k=>ve+(1-(k-b)/($-b||1))*c,J=t.map(k=>`${R(k.ts).toFixed(1)},${T(k.val).toFixed(1)}`).join(" "),X=`M${R(t[0].ts).toFixed(1)},${l} `+t.map(k=>`L${R(k.ts).toFixed(1)},${T(k.val).toFixed(1)}`).join(" ")+` L${R(t[t.length-1].ts).toFixed(1)},${l} Z`,Q="",U=t.filter(k=>k.min!=null&&k.max!=null);if(U.length>=2){let k=U.map(P=>`${R(P.ts).toFixed(1)},${T(P.max).toFixed(1)}`),w=[...U].reverse().map(P=>`${R(P.ts).toFixed(1)},${T(P.min).toFixed(1)}`);Q=`M${k[0]} `+k.slice(1).map(P=>`L${P}`).join(" ")+` L${w.join(" L")} Z`}let N=[];if(this.thresholdBelow!=null){let k=T(this.thresholdBelow);N.push({y:k,h:Math.max(0,l-k),lineY:k,label:`\u25BC ${he(this.thresholdBelow)}`,labelY:Math.min(l-4,k+13)})}if(this.thresholdAbove!=null){let k=T(this.thresholdAbove);N.push({y:ve,h:Math.max(0,k-ve),lineY:k,label:`\u25B2 ${he(this.thresholdAbove)}`,labelY:Math.max(ve+11,k-5)})}let be=t[t.length-1],$e=(this.events||[]).filter(k=>k.ts>=f&&k.ts<=j),Ce=kt(f,j,Math.max(2,Math.min(5,Math.floor(a/110)+1))),K=this._hover;return o`
      <div class="svg-holder">
        ...
        >
          ...
          ${Q?D`<path d="${Q}" fill="var(--primary-color)" opacity="0.08" clip-path="url(#plot)" />`:p}
          <path d="${X}" fill="var(--primary-color)" opacity="0.10" clip-path="url(#plot)" />
          <polyline points="${J}" fill="none" stroke="var(--primary-color)" stroke-width="2"
            stroke-linejoin="round" stroke-linecap="round" clip-path="url(#plot)" />
          ${N.length?D`<polyline points="${J}" fill="none" stroke="var(--error-color, #f44336)" stroke-width="2"
      ...
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_weibull",i)}">
        ...
        ${e>0?D`
          ...
          <text x="${(L+4).toFixed(1)}" y="${(T-6).toFixed(1)}" fill="var(--primary-color, #03a9f4)"
            font-size="9" font-weight="600">R=${J}%</text>
        `:p}
  ...
  `}function aa(n,r,e){let t=n.history.filter(w=>w.type==="completed"&&(w.cost!=null||w.duration!=null)).map(w=>({ts:new Date(w.timestamp).getTime(),cost:w.cost??0,duration:w.duration??0})).sort((w,P)=>w.ts-P.ts);if(t.length<2)return p;let i=t.some(w=>w.cost>0),a=t.some(w=>w.duration>0);if(!i&&!a)return p;let l=e!=="duration"&&i,c=e!=="cost"&&a,h=l||!c&&i,u=c||!l&&a,g=640,m=ia,v=h?44:12,b=u?44:12,$=g-v-b,f=m-sa,j=f-tt,C=t[0].ts,L=t[t.length-1].ts,R=(L-C||864e5)*.05,T=C-R,J=L+R,X=wt(C,L),Q=w=>v+(w-T)/(J-T)*$,U=et(0,Math.max(...t.map(w=>w.cost))||1,3),N=et(0,Math.max(...t.map(w=>w.duration))||1,3),be=w=>tt+(1-w/(U.niceMax||1))*j,$e=w=>tt+(1-w/(N.niceMax||1))*j,Ce=t.length>1?Math.min(...t.slice(1).map((w,P)=>Q(w.ts)-Q(t[P].ts))):$,K=Math.max(6,Math.min(22,Ce*.55)),k=kt(C,L,Math.max(2,Math.min(4,t.length)));return o`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${g} ${m}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_history",r)}">
        ...
        ${h?t.filter(w=>w.cost>0).map(w=>D`
          <rect x="${(Q(w.ts)-K/2).toFixed(1)}" y="${be(w.cost).toFixed(1)}" width="${K.toFixed(1)}" height="${(f-be(w.cost)).toFixed(1)}"
            fill="var(--primary-color)" opacity="0.6" rx="2">
            <title>${De(w.ts,r,!0)}: ${w.cost.toLocaleString(r)}€${w.duration?` \xB7 ${w.duration}m`:""}</title>
          </rect>
        `):p}
        ${u?D`
          <polyline points="${t.map(w=>`${Q(w.ts).toFixed(1)},${$e(w.duration).toFixed(1)}`).join(" ")}"
            fill="none" stroke="var(--accent-color, #ff9800)" stroke-width="2" stroke-linejoin="round" />
          ${t.map(w=>D`
            <circle cx="${Q(w.ts).toFixed(1)}" cy="${$e(w.duration).toFixed(1)}" r="3.5" fill="var(--accent-color, #ff9800)">
        ...
        ${k.map((w,P)=>{let Tt=P===0?"start":P===k.length-1?"end":"middle";return D`<text x="${Q(w).toFixed(1)}" y="${m-6}" text-anchor="${Tt}" fill="var(--secondary-text-color)" font-size="10">${De(w,r,X)}</text>`})}
      </svg>
    </div>
    <div class="chart-legend">
