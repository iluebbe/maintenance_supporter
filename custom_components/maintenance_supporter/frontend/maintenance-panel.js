...
`;var te=class extends S{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter)}getCardSize(){return 6}get _lang(){return this.hass?.language||"en"}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){super.updated(e);let t=this.hass?.language;if(t&&!it(t)&&ae(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{let e=await this.hass.connection.subscribeMessage(t=>{let i=t;this._objects=i.objects},{type:"maintenance_supporter/subscribe"});if(!this.isConnected){e();return}this._unsub=e}catch{}}_onEventClick(e){if(e.history_timestamp){this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}));return}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}render(){if(!this.hass)return h;let e=this._lang,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.title,l=null;this._userFilter&&(l=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let c=new Date;c.setHours(0,0,0,0);let d=this._pastDays>0,u=d?gi(this._objects,c,this._pastDays,l):_i(this._objects,c,this._windowDays,l),g=Ue(c),m=this._windowDays===365||d,v=m?u.filter(b=>b.events.length>0):u,f=b=>{let C=`cal-status-${b.status}`,P=b.projected?"cal-event-projected":"",H=b.status==="overdue"&&b.days_until_due!=null?` (${Math.abs(b.days_until_due)}d ${s("overdue",e).toLowerCase()})`:"",R=b.projected&&b.interval_days?o`<span class="cal-event-recur">${b.interval_unit&&b.interval_unit!=="days"?`${b.interval_days} ${s("unit_"+b.interval_unit,e)}`:s("cal_every_n_days",e).replace("{n}",String(b.interval_days))}</span>`:h,T=b.schedule_type==="sensor_based",Y=T?o`<ha-icon class="cal-event-icon cal-source-sensor"
      ...
      `},w=b=>{let[C,P,H]=b.date.split("-").map(Number),R=new Date(C,P-1,H),T=b.date===g,Y=R.toLocaleDateString(e,{weekday:"short"}),G=R.toLocaleDateString(e,{month:"long"});return o`
...
<body>
  ...
  `}function Pt(n,r,e){if(!n.trigger_config?.entity_id)return h;let t=n.trigger_config.entity_id,i=r.get(t)||[],a=[];if(i.length>=2)a=i.map(T=>({ts:T.ts,val:T.val}));else{if(!n.history)return h;for(let T of n.history)T.trigger_value!=null&&a.push({ts:new Date(T.timestamp).getTime(),val:T.trigger_value})}if(n.trigger_current_value!=null&&a.push({ts:Date.now(),val:n.trigger_current_value}),a.length<2)return h;a.sort((T,Y)=>T.ts-Y.ts);let l=Rs,c=Is,d=a.map(T=>T.val),u=Math.min(...d),g=Math.max(...d),m=g-u||1;u-=m*.1,g+=m*.1;let v=a[0].ts,w=a[a.length-1].ts-v||1,b=T=>(T-v)/w*l,C=T=>2+(1-(T-u)/(g-u))*(c-4),P=a;if(P.length>xi){let T=Math.ceil(P.length/xi);P=P.filter((Y,G)=>G%T===0||G===P.length-1)}let H=P.map(T=>`${b(T.ts).toFixed(1)},${C(T.val).toFixed(1)}`).join(" "),R=n.trigger_active?"var(--error-color, #f44336)":"var(--primary-color)";return o`
  ...
  `}function $i(n,r){let e=r;if(n.days_until_due==null||!n.interval_days||n.interval_days<=0)return h;let{pct:t,overflow:i}=nt(n.interval_days,n.days_until_due,n.interval_unit),a="var(--success-color, #4caf50)";return n.status==="overdue"?a="var(--error-color, #f44336)":n.status==="due_soon"&&(a="var(--warning-color, #ff9800)"),o`
  ...
  `}function Ve(n,r,e=4){if(!isFinite(n)||!isFinite(r))return{ticks:[],niceMin:0,niceMax:1};if(n===r){let u=Math.abs(n)*.1||1;n-=u,r+=u}let t=r-n,i=Math.pow(10,Math.floor(Math.log10(t/Math.max(1,e)))),a=i;for(let u of[1,2,5,10])if(a=i*u,t/a<=e+.5)break;let l=Math.floor(n/a)*a,c=Math.ceil(r/a)*a,d=[];for(let u=l;u<=c+a*1e-6;u+=a)d.push(Math.abs(u)<a*1e-9?0:u);return{ticks:d,niceMin:l,niceMax:c}}function ne(n){let r=Math.abs(n);return r>=1e6?Me((n/1e6).toFixed(r>=1e7?0:1))+"M":r>=1e4?Me((n/1e3).toFixed(0))+"k":r>=1e3?Me((n/1e3).toFixed(1))+"k":r>=100?n.toFixed(0):r>=10||r>=1?Me(n.toFixed(1)):r===0?"0":Me(n.toFixed(2))}function Me(n){return n.replace(/\.0+$/,"").replace(/(\.\d*[1-9])0+$/,"$1")}function we(n,r,e){let t=n.toLocaleString(e,{maximumFractionDigits:Math.abs(n)>=100?0:1});return r?`${t} ${r}`:t}function Re(n,r,e){let t=new Date(n),i=e?{month:"short",day:"numeric",year:"2-digit"}:{month:"short",day:"numeric"};return t.toLocaleDateString(r,i)}function Dt(n,r){return new Date(n).toLocaleDateString(r,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function gt(n,r){return new Date(n).getFullYear()!==new Date(r).getFullYear()}function mt(n,r,e){if(e<2||r<=n)return[n,r];let t=[];for(let i=0;i<e;i++)t.push(n+(r-n)*i/(e-1));return t}var vt=210,se=46,de=14,pe=12,wi=14,Ls=20+wi,zs=[{days:7,key:"chart_range_7d"},{days:30,key:"chart_range_30d"},{days:90,key:"chart_range_90d"},{days:365,key:"chart_range_1y"}],U=class extends S{constructor(){super(...arguments);this.points=[];this.events=[];this.unit="";this.lang="en";this.thresholdAbove=null;this.thresholdBelow=null;this.targetValue=null;this.forceZero=!1;this.projection=null;this.rangeDays=30;this.showRange=!0;this.busy=!1;this.hideOutliers=!1;this.showOutlierToggle=!0;this._width=0;this._hover=null;this._ro=null}connectedCallback(){super.connectedCallback(),this._ro=new ResizeObserver(e=>{let t=Math.floor(e[0]?.contentRect?.width||0);t&&Math.abs(t-this._width)>2&&(this._width=t)}),this._ro.observe(this)}disconnectedCallback(){super.disconnectedCallback(),this._ro?.disconnect(),this._ro=null}_emitRange(e){e!==this.rangeDays&&this.dispatchEvent(new CustomEvent("range-change",{detail:{days:e},bubbles:!0,composed:!0}))}_toggleOutliers(){this.dispatchEvent(new CustomEvent("outlier-toggle",{detail:{hide:!this.hideOutliers},bubbles:!0,composed:!0}))}render(){let e=this._width||320,t=[...this.points].sort((a,l)=>a.ts-l.ts),i=this.lang;return o`
    ...
    `}_renderSvg(e,t){let i=this.lang,a=e-se-de,l=vt-Ls,c=l-pe,d=1/0,u=-1/0;for(let k of t)d=Math.min(d,k.min??k.val),u=Math.max(u,k.max??k.val);this.thresholdAbove!=null&&(d=Math.min(d,this.thresholdAbove),u=Math.max(u,this.thresholdAbove)),this.thresholdBelow!=null&&(d=Math.min(d,this.thresholdBelow),u=Math.max(u,this.thresholdBelow)),this.targetValue!=null&&(d=Math.min(d,this.targetValue),u=Math.max(u,this.targetValue)),this.forceZero&&(d=Math.min(d,0));let g=(u-d||1)*.06,m=this.forceZero&&d>=0?0:d-g,{ticks:v,niceMin:f,niceMax:w}=Ve(m,u+g,4);this.forceZero&&d>=0&&f<0&&(f=0,v=v.filter(k=>k>=0));let b=t[0].ts,C=t[t.length-1].ts,P=C-b||1,H=gt(b,C),R=k=>se+(k-b)/P*a,T=k=>pe+(1-(k-f)/(w-f||1))*c,Y=t.map(k=>`${R(k.ts).toFixed(1)},${T(k.val).toFixed(1)}`).join(" "),G=`M${R(t[0].ts).toFixed(1)},${l} `+t.map(k=>`L${R(k.ts).toFixed(1)},${T(k.val).toFixed(1)}`).join(" ")+` L${R(t[t.length-1].ts).toFixed(1)},${l} Z`,K="",F=t.filter(k=>k.min!=null&&k.max!=null);if(F.length>=2){let k=F.map(I=>`${R(I.ts).toFixed(1)},${T(I.max).toFixed(1)}`),$=[...F].reverse().map(I=>`${R(I.ts).toFixed(1)},${T(I.min).toFixed(1)}`);K=`M${k[0]} `+k.slice(1).map(I=>`L${I}`).join(" ")+` L${$.join(" L")} Z`}let q=[];if(this.thresholdBelow!=null){let k=T(this.thresholdBelow);q.push({y:k,h:Math.max(0,l-k),lineY:k,label:`\u25BC ${ne(this.thresholdBelow)}`,labelY:Math.min(l-4,k+13)})}if(this.thresholdAbove!=null){let k=T(this.thresholdAbove);q.push({y:pe,h:Math.max(0,k-pe),lineY:k,label:`\u25B2 ${ne(this.thresholdAbove)}`,labelY:Math.max(pe+11,k-5)})}let he=t[t.length-1],me=(this.events||[]).filter(k=>k.ts>=b&&k.ts<=C),ke=mt(b,C,Math.max(2,Math.min(5,Math.floor(a/110)+1))),B=this._hover;return o`
      <div class="svg-holder">
        ...
        >
          ...
          ${v.map(k=>{let $=T(k);return $<pe-1||$>l+1?h:z`
          ...
          ${this.targetValue!=null?z`<line x1="${se}" y1="${T(this.targetValue).toFixed(1)}" x2="${e-de}" y2="${T(this.targetValue).toFixed(1)}"
                stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="6,4" />
              <text x="${e-de-4}" y="${(T(this.targetValue)-5).toFixed(1)}" text-anchor="end" class="zone-label">◆ ${ne(this.targetValue)} ${this.unit}</text>`:h}
          ...
          ${ke.map((k,$)=>{let I=R(k),bt=$===0?"start":$===ke.length-1?"end":"middle";return z`<text x="${I.toFixed(1)}" y="${vt-5}" text-anchor="${bt}" class="tick-label">${Re(k,i,H)}</text>`})}
        ...
        ${B?o`<div
            ...
            >
              ...
              <div class="hover-val">
                ...
                ${B.p.min!=null&&B.p.max!=null?o`<span class="hover-range">(${ne(B.p.min)}–${ne(B.p.max)})</span>`:h}
              </div>
            </div>`:h}
      </div>
  ...
  `}function Si(n,r,e){let t=n.degradation_trend!=null&&n.degradation_trend!=="insufficient_data",i=n.days_until_threshold!=null,a=n.environmental_factor!=null&&n.environmental_factor!==1;if(!t&&!i&&!a)return h;let l=n.degradation_trend==="rising"?"M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z":n.degradation_trend==="falling"?"M16,18L18.29,15.71L13.41,10.83L9.41,14.83L2,7.41L3.41,6L9.41,12L13.41,8L19.71,14.29L22,12V18H16Z":"M22,12L18,8V11H3V13H18V16L22,12Z";return o`
      ...
      <div class="prediction-grid">
        ...
        ${a&&e.environmental?o`
          <div class="prediction-item">
            ...
            ${n.environmental_entity?o`<span class="prediction-entity entity-link" @click=${c=>$e(c,n.environmental_entity)}>${n.environmental_entity}</span>`:h}
          </div>
        `:h}
      </div>
    </div>
  `}function Ai(n,r){let e=n.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return h;let a=n.interval_days??0,l=n.suggested_interval??a;return o`
    <div class="weibull-section">
      ...
      ${Ns(t,i,a,l,r)}
      ${Fs(e,r)}
      ${e?.confidence_interval_low!=null?Us(e,n,r):h}
    </div>
  ...
  `}function Ri(n,r){return Bs(n,r)}function Bs(n,r){let e=n.seasonal_factors??n.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return h;let t=n.interval_analysis?.seasonal_reason,i=new Date().getMonth(),a=300,l=100,c=8,u=l-c-4,g=Math.max(...e,1.5),m=a/12,v=m*.65,f=c+u-1/g*u;return o`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${a} ${l}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",r)}">
        ...
        ${e.map((w,b)=>{let C=w/g*u,P=b*m+(m-v)/2,H=c+u-C,R=b===i,T=w<1?"var(--success-color, #4caf50)":w>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return z`
            <rect x="${P.toFixed(1)}" y="${H.toFixed(1)}"
  ...
  `}var Vs=200,We=10,Ws=22;function Ii(n,r,e,t){let i=n.history.filter(c=>c.type==="completed"&&(c.cost!=null||c.duration!=null));if(i.length<2)return h;let a=i.some(c=>(c.cost??0)>0),l=i.some(c=>(c.duration??0)>0);return!a&&!l?h:o`
  ...
  `}function Ks(n,r,e){let t=n.history.filter($=>$.type==="completed"&&($.cost!=null||$.duration!=null)).map($=>({ts:new Date($.timestamp).getTime(),cost:$.cost??0,duration:$.duration??0})).sort(($,I)=>$.ts-I.ts);if(t.length<2)return h;let i=t.some($=>$.cost>0),a=t.some($=>$.duration>0);if(!i&&!a)return h;let l=e!=="duration"&&i,c=e!=="cost"&&a,d=l||!c&&i,u=c||!l&&a,g=640,m=Vs,v=d?44:12,f=u?44:12,w=g-v-f,b=m-Ws,C=b-We,P=t[0].ts,H=t[t.length-1].ts,R=(H-P||864e5)*.05,T=P-R,Y=H+R,G=gt(P,H),K=$=>v+($-T)/(Y-T)*w,F=Ve(0,Math.max(...t.map($=>$.cost))||1,3),q=Ve(0,Math.max(...t.map($=>$.duration))||1,3),he=$=>We+(1-$/(F.niceMax||1))*C,me=$=>We+(1-$/(q.niceMax||1))*C,ke=t.length>1?Math.min(...t.slice(1).map(($,I)=>K($.ts)-K(t[I].ts))):w,B=Math.max(6,Math.min(22,ke*.55)),k=mt(P,H,Math.max(2,Math.min(4,t.length)));return o`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${g} ${m}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_history",r)}">
        ${d?F.ticks.map($=>{let I=he($);return I<We-1||I>b+1?h:z`
            ...
            <text x="${v-6}" y="${(I+3.5).toFixed(1)}" text-anchor="end" fill="var(--primary-color)" font-size="10.5">${ne($)}€</text>`}):h}
        ${u?q.ticks.map($=>{let I=me($);return I<We-1||I>b+1?h:z`<text x="${g-f+6}" y="${(I+3.5).toFixed(1)}" text-anchor="start" fill="var(--accent-color, #ff9800)" font-size="10.5">${ne($)}m</text>`}):h}
  ...
  `,p([y({attribute:!1})],ge.prototype,"hass",2),p([y()],ge.prototype,"docId",2),p([_()],ge.prototype,"_url",2),p([_()],ge.prototype,"_failed",2);customElements.get("maintenance-history-photo")||customElements.define("maintenance-history-photo",ge);var Ys=["completed","skipped","missed","reset","triggered"];function Li(n,r){let e=r.lang;return o`
    <div class="history-filters-new">
      <div class="filter-chips">
        ${Ys.map(t=>{let i=n.history.filter(a=>a.type===t).length;return i===0?h:o`
  ...
  `}function Ot(n,r){if(!n.responsible_user_id)return h;let e=r(n.responsible_user_id);return e?o`
    <span class="user-badge">
      <ha-icon icon="mdi:account"></ha-icon>
      ${e}
    </span>
  `:h}function Qs(n,r){let e=r.lang,t=r.isOperator,i=n.archived?"archived":n.is_done?"done":n.status==="due_soon"?"warning":n.status||"ok",a=n.archived?s("archived",e):n.is_done?s("completed",e):s(n.status||"ok",e);return o`
  ...
  `}function Zs(n,r){if(!r.features.checklists)return h;let e=n.checklist||[];if(e.length===0)return h;let t=r.lang;return o`
  ...
  `}function Xs(n,r){let e=n.documentation_url&&/^https?:\/\//i.test(n.documentation_url)?n.documentation_url:null,t=r.objectDocUrl&&/^https?:\/\//i.test(r.objectDocUrl)?r.objectDocUrl:null;if(!n.notes&&!e&&!t)return h;let i=r.lang;return o`
  ...
  `}function ta(n,r){let e=r.lang;if(!r.features.adaptive||!n.suggested_interval||n.suggested_interval===n.interval_days)return h;if(r.suggestionDismissed)return h;let t=n.suggested_interval;return o`
  ...
  `}function ia(n,r){let e=r.lang,t=n.history.slice(-3).reverse();if(t.length===0)return h;let i=a=>{switch(a){case"completed":return"\u2713";case"triggered":return"\u2297";case"skipped":return"\u21B7";case"reset":return"\u21BA";default:return"\xB7"}};return o`
  ...
  `}function sa(n,r){let e=r.lang,t=r.features.adaptive&&n.suggested_interval&&n.suggested_interval!==n.interval_days,i=r.features.seasonal&&n.seasonal_factor&&n.seasonal_factor!==1,a=t||i,l=r.features.adaptive&&n.interval_analysis?.weibull_beta!=null&&n.interval_analysis?.weibull_eta!=null,c=r.features.seasonal&&(n.seasonal_factors?.length===12||n.interval_analysis?.seasonal_factors?.length===12);return o`
    <div class="tab-content overview-tab">
      ...
      ${l?Pi("weibull","weibull_reliability_curve",Ai(n,e),r):h}
  ...
  `}function ra(n,r){switch(r.activeTab){case"overview":return sa(n,r);case"history":return aa(n,r);default:return h}}function Di(n,r){return o`
