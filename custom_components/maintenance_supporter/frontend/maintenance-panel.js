...
`;var Da={days:1,weeks:7,months:30.4368,years:365.25};function tt(r,n){return!r||r<=0?0:r*(Da[n||"days"]??1)}function at(r,n,e){let t=tt(r,e);if(t<=0||n==null)return{pct:0,overflow:!1};let a=(t-n)/t*100;return{pct:Math.max(0,Math.min(100,a)),overflow:a>100}}var St=q`
  :host {
    display: block;
    height: 100%;
...
`;var De=class{constructor(n){this._cache=new Map;this._pending=new Map;this._hass=n}updateHass(n){this._hass=n}async getDetailStats(n,e){return this._getStats(n,"hour",30,e)}async getMiniStats(n,e){return this._getStats(n,"day",14,e)}async getBatchMiniStats(n){let e=new Map,t=[];for(let _ of n){let u=`${_.entityId}:day`,h=this._cache.get(u);h&&Date.now()-h.fetchedAt<3e5?e.set(_.entityId,h.points):t.push(_)}if(t.length===0)return e;let a=t.filter(_=>_.isCounter).map(_=>_.entityId),s=t.filter(_=>!_.isCounter).map(_=>_.entityId),o=new Date(Date.now()-336*60*60*1e3).toISOString(),c=[];return a.length>0&&c.push(this._fetchBatch(a,"day",o,["state","sum","change"],!0,e)),s.length>0&&c.push(this._fetchBatch(s,"day",o,["mean","min","max"],!1,e)),await Promise.all(c),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(n,e,t,a){let s=`${n}:${e}`,o=this._cache.get(s);if(o&&Date.now()-o.fetchedAt<3e5)return o.points;if(this._pending.has(s))return this._pending.get(s);let c=this._fetchAndNormalize(n,e,t,a,s);this._pending.set(s,c);try{return await c}finally{this._pending.delete(s)}}async _fetchAndNormalize(n,e,t,a,s){let o=new Date(Date.now()-t*24*60*60*1e3).toISOString(),c=a?["state","sum","change"]:["mean","min","max"];try{let u=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:o,statistic_ids:[n],period:e,types:c}))[n]||[],h=this._normalizeRows(u,a);return this._cache.set(s,{entityId:n,fetchedAt:Date.now(),period:e,points:h}),h}catch(_){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${n}:`,_),[]}}async _fetchBatch(n,e,t,a,s,o){try{let c=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:n,period:e,types:a});for(let _ of n){let u=c[_]||[],h=this._normalizeRows(u,s);o.set(_,h),this._cache.set(`${_}:${e}`,{entityId:_,fetchedAt:Date.now(),period:e,points:h})}}catch(c){console.warn("[maintenance-supporter] Batch statistics fetch failed:",c)}}_normalizeRows(n,e){let t=[];for(let a of n){let s=null;if(e?s=a.state??null:s=a.mean??null,s===null)continue;let o={ts:a.start,val:s};e||(a.min!=null&&(o.min=a.min),a.max!=null&&(o.max=a.max)),t.push(o)}return t.sort((a,s)=>a.ts-s.ts),t}};var oe=class{constructor(n){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=n}updateHass(n){this.hass=n}async getUsers(n=!1){let e=Date.now();if(!n&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(n,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:n,task_id:e,user_id:t})}async getTasksByUser(n){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:n})).tasks}getUserName(n){return!n||!this.usersCache?null:this.usersCache.find(t=>t.id===n)?.name||null}getUser(n){return!n||!this.usersCache?null:this.usersCache.find(e=>e.id===n)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(n){return n?n===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var Ra={name:"name",task_type:"maintenance_type",schedule_type:"schedule_type",interval_days:"interval_days",interval_anchor:"interval_anchor",warning_days:"warning_days",last_performed:"last_performed_optional",notes:"notes_optional",documentation_url:"documentation_url_optional",custom_icon:"custom_icon_optional",nfc_tag_id:"nfc_tag_id_optional",responsible_user_id:"responsible_user",entity_slug:"entity_slug",entity_id:"entity_id",area_id:"area_id_optional",manufacturer:"manufacturer_optional",model:"model_optional",serial_number:"serial_number_optional",installation_date:"installation_date_optional",checklist:"checklist_steps_optional",reason:"reason",feedback:"feedback",cost:"cost",duration:"duration",description:"description_optional",group_name:"name",group_description:"description_optional",environmental_entity:"environmental_entity_optional",environmental_attribute:"environmental_attribute_optional",trigger_above:"trigger_above",trigger_below:"trigger_below",trigger_for_minutes:"trigger_for_minutes"};function Ia(r,n){let e=Ra[r];if(!e)return r;let t=i(e,n);return t&&t!==e?t:r}function Pa(r){let e=r.match(/data\['([^']+)'\]/)?.[1],t;return(t=r.match(/length of value must be at most (\d+)/))?{field:e,rule:"too_long",param:t[1]}:(t=r.match(/length of value must be at least (\d+)/))?{field:e,rule:"too_short",param:t[1]}:(t=r.match(/value must be at most (\S+)/))?{field:e,rule:"value_too_high",param:t[1]}:(t=r.match(/value must be at least (\S+)/))?{field:e,rule:"value_too_low",param:t[1]}:/required key not provided/.test(r)?{field:e,rule:"required"}:(t=r.match(/expected (\w+)/))?{field:e,rule:"wrong_type",param:t[1]}:/value must be one of/.test(r)?{field:e,rule:"invalid_choice"}:/not a valid value/.test(r)?{field:e,rule:"invalid_value"}:{field:e,rule:"unknown"}}function W(r,n,e){if(typeof r=="string")return r;if(typeof r!="object"||r===null)return e;let t=r,a=t.message||t.error?.message||"";if(!a)return e;let s=Pa(a),o=s.field?Ia(s.field,n):"",c=_=>i(_,n).replace("{field}",o).replace("{n}",s.param??"");switch(s.rule){case"too_long":return c("err_too_long");case"too_short":return c("err_too_short");case"value_too_high":return c("err_value_too_high");case"value_too_low":return c("err_value_too_low");case"required":return c("err_required");case"wrong_type":return c("err_wrong_type").replace("{type}",s.param??"");case"invalid_choice":return c("err_invalid_choice");case"invalid_value":return c("err_invalid_value");default:return a||e}}var B=class extends T{constructor(){super(...arguments);this.label="";this.value="";this.placeholder="";this.type="text";this.required=!1;this.disabled=!1}_onInput(e){let t=e.target.value;this.value=t,this.dispatchEvent(new CustomEvent("input",{bubbles:!0,composed:!0,detail:{value:t}}))}render(){return l`
      <label class="field">
        ${this.label?l`<span class="label">${this.label}${this.required?l`<span class="req">*</span>`:d}</span>`:d}
        <input
          .value=${this.value??""}
          .type=${this.type}
        ...
        />
        ${this.helper?l`<span class="helper">${this.helper}</span>`:d}
      </label>
    `}};B.styles=q`
    :host { display: block; }
    .field {
...
`),this._scheduleTime=t.schedule_time||"";let s=t.on_complete_action;if(s&&s.service){this._actionService=s.service;let _=s.target?.entity_id;this._actionTargetEntity=Array.isArray(_)?_[0]||"":_||"",this._actionData=s.data&&typeof s.data=="object"?{...s.data}:{},this._actionDataJsonFallback=""}else this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="";let o=t.quick_complete_defaults;this._qcNotes=o?.notes||"",this._qcCost=o?.cost!=null?String(o.cost):"",this._qcDuration=o?.duration!=null?String(o.duration):"",this._qcFeedback=o?.feedback||"";let c=t.adaptive_config||{};if(this._environmentalEntity=c.environmental_entity||"",this._environmentalAttribute=c.environmental_attribute||"",this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute,t.trigger_config){let _=t.trigger_config;this._triggerEntityId=_.entity_id||"",this._triggerEntityIds=_.entity_ids||(_.entity_id?[_.entity_id]:[]),this._triggerEntityLogic=_.entity_logic||"any",this._triggerAttribute=_.attribute||"",this._triggerType=_.type||"threshold",this._triggerAbove=_.trigger_above?.toString()||"",this._triggerBelow=_.trigger_below?.toString()||"",this._triggerForMinutes=_.trigger_for_minutes?.toString()||"0",this._triggerTargetValue=_.trigger_target_value?.toString()||"",this._triggerDeltaMode=_.trigger_delta_mode||!1,this._triggerFromState=_.trigger_from_state||"",this._triggerToState=_.trigger_to_state||"",this._triggerTargetChanges=_.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=_.trigger_runtime_hours?.toString()||""}else this._resetTriggerFields();this._triggerEntityId&&this._fetchEntityAttributes(this._triggerEntityId),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._intervalUnit="days",this._dueDate="",this._warningDays=String(this.defaultWarningDays),this._intervalAnchor="completion",this._weekdays=[],this._nth="1",this._nthWeekday="5",this._domDay="1",this._notes="",this._documentationUrl="",this._customIcon="",this._enabled=!0,this._lastPerformed="",this._nfcTagId="",this._responsibleUserId=null,this._checklistText="",this._scheduleTime="",this._environmentalEntity="",this._environmentalAttribute="",this._environmentalInitial="",this._environmentalAttributeInitial="",this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="",this._actionTesting=!1,this._actionTestResult="",this._qcNotes="",this._qcCost="",this._qcDuration="",this._qcFeedback="",this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerForMinutes="0",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours=""}async _loadUsers(){this._userService||(this._userService=new oe(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}async _testAction(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e)){this._actionTestResult="error",this._actionTestError="Invalid service format (expected 'domain.service')",setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3);return}let[t,a]=e.split(".");if(!this.hass?.services?.[t]?.[a]){this._actionTestResult="error",this._actionTestError=`Service "${e}" is not registered in Home Assistant. Check spelling and that the integration providing it is loaded.`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}let s=this._actionTargetEntity.trim();if(s){let o=s.split(".")[0];if(o!==t&&!new Set(["homeassistant","scene","notify","persistent_notification"]).has(t)){this._actionTestResult="error",this._actionTestError=`Service "${e}" only works on ${t}.* entities; entity "${s}" is in ${o}.* \u2014 pick a service that matches the entity domain (e.g. ${o}.${a})`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}if(!this.hass.states?.[s]){this._actionTestResult="error",this._actionTestError=`Target entity "${s}" not found in Home Assistant \u2014 the entity may have been renamed or its integration removed.`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}}this._actionTestResult="ok",setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3)}_buildActionData(){if(this._actionDataJsonFallback.trim())try{let e=JSON.parse(this._actionDataJsonFallback);if(e&&typeof e=="object"&&!Array.isArray(e))return e}catch{}return{...this._actionData}}_serviceSchema(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e))return null;let[t,a]=e.split("."),s=this.hass?.services?.[t]?.[a]?.fields;return!s||Object.keys(s).length===0?null:Object.entries(s).map(([o,c])=>({name:o,required:!!c.required,selector:c.selector||{text:{}}}))}_renderCompletionActionsSection(e){if(!this.completionActionsEnabled)return d;let t=this._serviceSchema();return l`
      <details class="ca-section">
        ...
        <ha-service-picker
          ...
          @value-changed=${a=>{this._actionService=a.detail.value||"";let s=this._serviceSchema();if(s){let o=new Set(s.map(c=>c.name));this._actionData=Object.fromEntries(Object.entries(this._actionData).filter(([c])=>o.has(c)))}}}
        ></ha-service-picker>
        <ha-form
          .hass=${this.hass}
          ...
          @value-changed=${a=>{let s=a.detail.value;this._actionTargetEntity=s.target_entity||""}}
        ></ha-form>
        ...
        <div class="ca-test-row">
          ...
          ${this._actionTestResult==="ok"?l`<span class="ca-test-ok">${i("on_complete_action_test_success",e)}</span>`:d}
          ${this._actionTestResult==="error"?l`<div class="ca-test-error-block">
                ...
                ${this._actionTestError?l`<div class="ca-test-error-detail">${this._actionTestError}</div>`:d}
              </div>`:d}
        </div>
      </details>
...
`).map(c=>c.trim()).filter(Boolean).slice(0,100);e.checklist=o.length?o:null}if(this.completionActionsEnabled){let o=this._actionService.trim();if(o&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(o)){let h={service:o},m=this._actionTargetEntity.trim();m&&(h.target={entity_id:m});let v=this._buildActionData();Object.keys(v).length>0&&(h.data=v),e.on_complete_action=h}else e.on_complete_action=null;let c={};this._qcNotes.trim()&&(c.notes=this._qcNotes.trim());let _=parseFloat(this._qcCost);!isNaN(_)&&_>=0&&(c.cost=_);let u=parseInt(this._qcDuration,10);!isNaN(u)&&u>=0&&(c.duration=u),this._qcFeedback&&(c.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(c).length?c:null}let t=await this.hass.connection.sendMessagePromise(e),a=this._taskId||t?.task_id,s=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(a&&this._scheduleType==="sensor_based"&&s)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:a,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=W(e,this._lang,i("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return d;let e=this._lang;return l`
      ...
      <ms-textfield
        ...
        @input=${t=>{let s=t.target.value.split(",").map(o=>o.trim()).filter(Boolean);this._triggerEntityId=s[0]||"",this._triggerEntityIds=s,s[0]&&this._fetchEntityAttributes(s[0])}}
      ></ms-textfield>
      ...
      <div class="select-row">
        ...
        >
          ${Oa.map(t=>l`<option value=${t} ?selected=${t===this._triggerType}>${i(t,e)}</option>`)}
        </select>
      </div>
    ...
    `}_renderUnitSelect(){let e=this._lang;return l`
      <div class="select-row">
        ...
        >
          ${["days","weeks","months","years"].map(t=>l`<option value=${t} ?selected=${t===this._intervalUnit}>${i("unit_"+t,e)}</option>`)}
        </select>
      </div>`}_toggleWeekday(e){this._weekdays=this._weekdays.includes(e)?this._weekdays.filter(t=>t!==e):[...this._weekdays,e]}_buildSchedule(){return this._scheduleType==="weekdays"?{kind:"weekdays",weekdays:[...this._weekdays].sort((e,t)=>e-t)}:this._scheduleType==="nth_weekday"?{kind:"nth_weekday",nth:parseInt(this._nth,10),weekday:parseInt(this._nthWeekday,10)}:{kind:"day_of_month",day:parseInt(this._domDay,10)||1}}_renderCalendarFields(){let e=this._lang,t=Ua(e);if(this._scheduleType==="weekdays")return l`
        <label class="field-label">${i("recurrence_on_days",e)}</label>
        <div class="weekday-chips">
          ${t.map((a,s)=>l`
            <button
              type="button"
              class="weekday-chip ${this._weekdays.includes(s)?"selected":""}"
              @click=${()=>this._toggleWeekday(s)}
            >${a}</button>`)}
        </div>`;if(this._scheduleType==="nth_weekday"){let a=[["1",i("ord_1",e)],["2",i("ord_2",e)],["3",i("ord_3",e)],["4",i("ord_4",e)],["5",i("ord_5",e)],["-1",i("ord_last",e)]];return l`
        <div class="select-row">
          <label>${i("recurrence_occurrence",e)}</label>
          <select .value=${this._nth} @change=${s=>this._nth=s.target.value}>
            ${a.map(([s,o])=>l`<option value=${s} ?selected=${s===this._nth}>${o}</option>`)}
          </select>
        </div>
        <div class="select-row">
          <label>${i("recurrence_weekday",e)}</label>
          <select .value=${this._nthWeekday} @change=${s=>this._nthWeekday=s.target.value}>
            ${t.map((s,o)=>l`<option value=${String(o)} ?selected=${String(o)===this._nthWeekday}>${s}</option>`)}
          </select>
        ...
        ></ms-textfield>`:d}_renderTriggerTypeFields(){let e=this._lang;return this._triggerType==="threshold"?l`
        <ms-textfield
      ...
      `:this._triggerType==="runtime"?l`
        ...
        ></ms-textfield>
      `:d}render(){if(!this._open)return l``;let e=this._lang,t=this._taskId?i("edit_task",e):i("new_task",e);return l`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
          ${this._error?l`<div class="error">${this._error}</div>`:d}
          ...
          <div class="select-row">
            ...
            >
              ${Fa.map(a=>l`<option value=${a} ?selected=${a===this._type}>${i(a,e)}</option>`)}
            </select>
          </div>
          <div class="select-row">
            ...
            >
              ${La.map(a=>l`<option value=${a} ?selected=${a===this._scheduleType}>${i(a,e)}</option>`)}
            </select>
          </div>
  ...
  `,p([x({attribute:!1})],w.prototype,"hass",2),p([x({type:Boolean,attribute:"checklists-enabled"})],w.prototype,"checklistsEnabled",2),p([x({type:Boolean,attribute:"schedule-time-enabled"})],w.prototype,"scheduleTimeEnabled",2),p([x({type:Boolean,attribute:"completion-actions-enabled"})],w.prototype,"completionActionsEnabled",2),p([x({type:Number,attribute:"default-warning-days"})],w.prototype,"defaultWarningDays",2),p([g()],w.prototype,"_open",2),p([g()],w.prototype,"_loading",2),p([g()],w.prototype,"_error",2),p([g()],w.prototype,"_entryId",2),p([g()],w.prototype,"_taskId",2),p([g()],w.prototype,"_objectChoices",2),p([g()],w.prototype,"_name",2),p([g()],w.prototype,"_type",2),p([g()],w.prototype,"_scheduleType",2),p([g()],w.prototype,"_intervalDays",2),p([g()],w.prototype,"_intervalUnit",2),p([g()],w.prototype,"_dueDate",2),p([g()],w.prototype,"_warningDays",2),p([g()],w.prototype,"_intervalAnchor",2),p([g()],w.prototype,"_weekdays",2),p([g()],w.prototype,"_nth",2),p([g()],w.prototype,"_nthWeekday",2),p([g()],w.prototype,"_domDay",2),p([g()],w.prototype,"_notes",2),p([g()],w.prototype,"_documentationUrl",2),p([g()],w.prototype,"_customIcon",2),p([g()],w.prototype,"_enabled",2),p([g()],w.prototype,"_triggerEntityId",2),p([g()],w.prototype,"_triggerEntityIds",2),p([g()],w.prototype,"_triggerEntityLogic",2),p([g()],w.prototype,"_triggerAttribute",2),p([g()],w.prototype,"_triggerType",2),p([g()],w.prototype,"_triggerAbove",2),p([g()],w.prototype,"_triggerBelow",2),p([g()],w.prototype,"_triggerForMinutes",2),p([g()],w.prototype,"_triggerTargetValue",2),p([g()],w.prototype,"_triggerDeltaMode",2),p([g()],w.prototype,"_triggerFromState",2),p([g()],w.prototype,"_triggerToState",2),p([g()],w.prototype,"_triggerTargetChanges",2),p([g()],w.prototype,"_triggerRuntimeHours",2),p([g()],w.prototype,"_suggestedAttributes",2),p([g()],w.prototype,"_availableAttributes",2),p([g()],w.prototype,"_entityDomain",2),p([g()],w.prototype,"_lastPerformed",2),p([g()],w.prototype,"_nfcTagId",2),p([g()],w.prototype,"_availableTags",2),p([g()],w.prototype,"_responsibleUserId",2),p([g()],w.prototype,"_availableUsers",2),p([g()],w.prototype,"_checklistText",2),p([g()],w.prototype,"_scheduleTime",2),p([g()],w.prototype,"_actionService",2),p([g()],w.prototype,"_actionTargetEntity",2),p([g()],w.prototype,"_actionData",2),p([g()],w.prototype,"_actionDataJsonFallback",2),p([g()],w.prototype,"_actionTesting",2),p([g()],w.prototype,"_actionTestResult",2),p([g()],w.prototype,"_actionTestError",2),p([g()],w.prototype,"_qcNotes",2),p([g()],w.prototype,"_qcCost",2),p([g()],w.prototype,"_qcDuration",2),p([g()],w.prototype,"_qcFeedback",2),p([g()],w.prototype,"_environmentalEntity",2),p([g()],w.prototype,"_environmentalAttribute",2);customElements.get("maintenance-task-dialog")||customElements.define("maintenance-task-dialog",w);var F=class extends T{constructor(){super(...arguments);this.entryId="";this.taskId="";this.taskName="";this.lang="en";this.checklist=[];this.adaptiveEnabled=!1;this._open=!1;this._notes="";this._cost="";this._duration="";this._loading=!1;this._error="";this._checklistState={};this._feedback="needed"}open(){this._open||(this._open=!0,this._notes="",this._cost="",this._duration="",this._error="",this._checklistState={},this._feedback="needed")}_toggleCheck(e){let t=String(e);this._checklistState={...this._checklistState,[t]:!this._checklistState[t]}}_setFeedback(e){this._feedback=e}async _complete(){this._loading=!0,this._error="";try{let e={type:"maintenance_supporter/task/complete",entry_id:this.entryId,task_id:this.taskId};if(this._notes&&(e.notes=this._notes),this._cost){let t=parseFloat(this._cost);!isNaN(t)&&t>=0&&(e.cost=t)}if(this._duration){let t=parseInt(this._duration,10);!isNaN(t)&&t>=0&&(e.duration=t)}this.checklist.length>0&&(e.checklist_state=this._checklistState),this.adaptiveEnabled&&(e.feedback=this._feedback),await this.hass.connection.sendMessagePromise(e),this._open=!1,this.dispatchEvent(new CustomEvent("task-completed"))}catch(e){this._error=W(e,this.lang,i("save_error",this.lang))}finally{this._loading=!1}}_close(){this._open=!1}render(){if(!this._open)return l``;let e=this.lang||this.hass?.language||"en";return l`
      <ha-dialog open @closed=${this._close}>
...
<div class="qr-row">
  <div class="qr-col">
    <img src="${Tt(this._viewResult.svg_data_uri)}" alt="QR Info" />
    <div class="qr-label">${h}</div>
  </div>
  ${u?`<div class="qr-col">
    <img src="${Tt(this._completeResult.svg_data_uri)}" alt="QR Complete" />
    <div class="qr-label">${m}</div>
  </div>`:""}
</div>
<div class="url">${ue(this._viewResult.url)}</div>
<script>setTimeout(()=>window.print(),300)<\/script>
</body></html>`),s.document.close()}_downloadSvg(e,t){let a=decodeURIComponent(e.svg_data_uri.replace("data:image/svg+xml,","")),s=new Blob([a],{type:"image/svg+xml"}),o=URL.createObjectURL(s),c=document.createElement("a");c.href=o;let _=this._taskName?`${this._objectName}-${this._taskName}`:this._objectName;c.download=`qr-${Ba(_)}-${t}.svg`,c.click(),URL.revokeObjectURL(o)}_close(){this._open=!1,this._viewResult=null,this._completeResult=null,this._error="",this._loading=!1}render(){if(!this._open)return l``;let e=this.lang||this.hass?.language||"en",t=this._taskName?`${i("qr_code",e)}: ${this._objectName} \u2014 ${this._taskName}`:`${i("qr_code",e)}: ${this._objectName}`,a=!!this._viewResult;return l`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
  ...
  `,p([x({attribute:!1})],Z.prototype,"hass",2),p([x()],Z.prototype,"lang",2),p([g()],Z.prototype,"_open",2),p([g()],Z.prototype,"_loading",2),p([g()],Z.prototype,"_error",2),p([g()],Z.prototype,"_viewResult",2),p([g()],Z.prototype,"_completeResult",2),p([g()],Z.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",Z);var qt=5;function xe(r){let n=r.getFullYear(),e=String(r.getMonth()+1).padStart(2,"0"),t=String(r.getDate()).padStart(2,"0");return`${n}-${e}-${t}`}function Va(r,n){let e=[];for(let t=0;t<n;t++){let a=new Date(r);a.setDate(a.getDate()+t),a.setHours(0,0,0,0),e.push(xe(a))}return e}function Re(r,n){let[e,t,a]=r.split("-").map(Number),s=new Date(e,t-1,a);return s.setDate(s.getDate()+n),xe(s)}function Wa(r){if(!r||r.length===0)return null;let n=r.map(e=>e.cost).filter(e=>typeof e=="number");return n.length===0?null:n.reduce((e,t)=>e+t,0)/n.length}function Ga(r){let{windowStart:n,windowEnd:e,task:t,entryId:a,objectName:s}=r,o=[],c=(m,v)=>({date:m,entry_id:a,task_id:t.id,task_name:t.name,object_name:s,status:v&&(t.status==="overdue"||t.status==="triggered")?"ok":t.status,days_until_due:v?null:t.days_until_due??null,projected:v,schedule_type:t.schedule_type,interval_days:t.interval_days??null,interval_unit:t.interval_unit??null,responsible_user_id:t.responsible_user_id??null,avg_cost:Wa(t.history),adaptive_enabled:!!t.adaptive_config?.enabled,prediction_confidence:t.threshold_prediction_confidence??null}),_=Math.max(1,Math.round(tt(t.interval_days,t.interval_unit)));if(t.status==="overdue"||t.status==="triggered"){if(o.push(c(n,!1)),t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let m=Re(n,_),v=1;for(;m<=e&&v<qt;)o.push(c(m,!0)),v++,m=Re(m,_)}return o}let u=t.next_due;if(typeof u!="string"||!u)return o;let h=u.slice(0,10);if(h>=n&&h<=e)o.push(c(h,!1));else if(h>e)return o;if(t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let m=Re(h,_),v=o.length;for(;m<=e&&v<qt;)m>=n&&(o.push(c(m,!0)),v++),m=Re(m,_)}return o}var Ct={overdue:0,triggered:1,due_soon:2,ok:3};function Mt(r,n,e,t=null){let a=Va(n,e),s=a[0],o=a[a.length-1],c=[];for(let u of r){let h=u.object?.name||"",m=u.entry_id,v=u.tasks||[];for(let f of v){if(t&&f.responsible_user_id!==t||f.enabled===!1)continue;let k=Ga({windowStart:s,windowEnd:o,task:f,entryId:m,objectName:h});c.push(...k)}}let _=new Map;for(let u of a)_.set(u,[]);for(let u of c){let h=_.get(u.date);h&&h.push(u)}for(let[,u]of _)u.sort((h,m)=>{let v=Ct[h.status]??99,f=Ct[m.status]??99;if(v!==f)return v-f;if(h.projected!==m.projected)return h.projected?1:-1;let k=h.object_name.localeCompare(m.object_name);return k!==0?k:h.task_name.localeCompare(m.task_name)});return a.map(u=>({date:u,events:_.get(u)??[]}))}var Qa={completed:"ok",reset:"ok",skipped:"due_soon",triggered:"triggered",trigger_replaced:"triggered"};function Ka(r,n){let e=[];for(let t=n-1;t>=0;t--){let a=new Date(r);a.setDate(a.getDate()-t),a.setHours(0,0,0,0),e.push(xe(a))}return e}function Nt(r,n,e,t=null){let a=Ka(n,e),s=a[0],o=a[a.length-1],c=new Map;for(let u of a)c.set(u,[]);for(let u of r){let h=u.object?.name||"",m=u.entry_id,v=u.tasks||[];for(let f of v){if(t&&f.responsible_user_id!==t)continue;let k=f.history||[];for(let b of k){if(typeof b?.timestamp!="string")continue;let N=b.timestamp.slice(0,10);if(N<s||N>o)continue;let C=c.get(N);if(!C)continue;let E=b.type??"completed";C.push({date:N,entry_id:m,task_id:f.id,task_name:f.name,object_name:h,status:Qa[E]??"ok",days_until_due:null,projected:!1,schedule_type:f.schedule_type,interval_days:f.interval_days??null,responsible_user_id:f.responsible_user_id??null,avg_cost:typeof b.cost=="number"?b.cost:null,adaptive_enabled:!!f.adaptive_config?.enabled,prediction_confidence:null,history_timestamp:b.timestamp,history_type:E,history_cost:typeof b.cost=="number"?b.cost:null,history_notes:typeof b.notes=="string"?b.notes:null,history_duration:typeof b.duration=="number"?b.duration:null})}}}let _={completed:0,reset:1,skipped:2,triggered:3,trigger_replaced:4};for(let[,u]of c)u.sort((h,m)=>{let v=_[h.history_type??""]??99,f=_[m.history_type??""]??99;if(v!==f)return v-f;let k=h.object_name.localeCompare(m.object_name);return k!==0?k:h.task_name.localeCompare(m.task_name)});return a.map(u=>({date:u,events:c.get(u)??[]}))}var Dt=q`
  .cal-controls {
    display: flex;
    gap: 12px;
    align-items: center;
...
`;var J=class extends T{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter)}getCardSize(){return 6}get _lang(){return this.hass?.language||"en"}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}_onEventClick(e){if(e.history_timestamp){this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}));return}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}render(){if(!this.hass)return d;let e=this._lang,t=this._config.show_window_chips!==!1,a=this._config.show_user_filter!==!1,s=this._config.title,o=null;this._userFilter&&(o=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let c=new Date;c.setHours(0,0,0,0);let _=this._pastDays>0,u=_?Nt(this._objects,c,this._pastDays,o):Mt(this._objects,c,this._windowDays,o),h=xe(c),m=this._windowDays===365||_,v=m?u.filter(b=>b.events.length>0):u,f=b=>{let N=`cal-status-${b.status}`,C=b.projected?"cal-event-projected":"",E=b.status==="overdue"&&b.days_until_due!=null?` (${Math.abs(b.days_until_due)}d ${i("overdue",e).toLowerCase()})`:"",$=b.projected&&b.interval_days?l`<span class="cal-event-recur">${b.interval_unit&&b.interval_unit!=="days"?`${b.interval_days} ${i("unit_"+b.interval_unit,e)}`:i("cal_every_n_days",e).replace("{n}",String(b.interval_days))}</span>`:d,M=b.schedule_type==="sensor_based",L=M?l`<ha-icon class="cal-event-icon cal-source-sensor"
        ...
        <div class="cal-event ${C}"
          ...
          ${b.avg_cost!=null&&b.avg_cost>0?l`<span class="cal-event-cost">${b.avg_cost.toFixed(0)} ${K}</span>`:d}
        </div>
      `},k=b=>{let[N,C,E]=b.date.split("-").map(Number),$=new Date(N,C-1,E),M=b.date===h,L=$.toLocaleDateString(e,{weekday:"short"}),V=$.toLocaleDateString(e,{month:"long"});return l`
        <div class="cal-day-row">
          ...
          <div class="cal-day-content">
            <div class="cal-day-header">
              <span class="cal-day-month">${V}</span>
              ${M?l`<span class="cal-day-today-badge">${i("today",e)}</span>`:d}
            </div>
            ${b.events.length===0?l`<div class="cal-empty">${i("cal_no_events",e)}</div>`:b.events.map(f)}
          </div>
        </div>
      `};return l`
      <ha-card .header=${s}>
        ...
        <div class="cal-rolling">
          ${v.length===0&&m?l`<div class="cal-empty">${i("cal_no_events",e)}</div>`:v.map(k)}
        </div>
      </ha-card>
    `}};J.styles=[Ne,Dt,q`
      :host { display: block; }
      ha-card { padding: 0; overflow: hidden; }
    `],p([x({attribute:!1})],J.prototype,"hass",2),p([g()],J.prototype,"_config",2),p([g()],J.prototype,"_objects",2),p([g()],J.prototype,"_stats",2),p([g()],J.prototype,"_windowDays",2),p([g()],J.prototype,"_pastDays",2),p([g()],J.prototype,"_userFilter",2),p([g()],J.prototype,"_unsub",2);var Za=[{value:7,label:"Week (7 days)"},{value:14,label:"Fortnight (14 days)"},{value:30,label:"Month (30 days, default)"},{value:365,label:"Year (365 days, empty days collapsed)"}],ge=class extends T{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"}}setConfig(e){this._config={...e}}_valueChanged(e,t){let a={...this._config,[e]:t};e==="show_window_chips"&&t===!0&&delete a.show_window_chips,e==="show_user_filter"&&t===!0&&delete a.show_user_filter,e==="title"&&(!t||typeof t=="string"&&t.trim()==="")&&delete a.title,e==="user_filter"&&t===""&&delete a.user_filter,this._config=a,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:a},bubbles:!0,composed:!0}))}render(){let e=this._config.window_days??30,t=this._config.show_window_chips!==!1,a=this._config.show_user_filter!==!1,s=this._config.user_filter??"",o=this._config.title??"";return l`
      <div class="editor">
        <div class="row">
        ...
        <div class="row">
          ...
          >
            ${Za.map(c=>l`<option value="${c.value}" ?selected=${c.value===e}>${c.label}</option>`)}
          </select>
        </div>
      ...
      <div class="dialog" role="dialog" aria-modal="true">
        ...
        <label>
          ...
          <input type="datetime-local"
            ...
            @change=${a=>{let s=a.target.value;this._set("timestamp",s.length===16?`${s}:00`:s)}} />
        </label>
        <label>
          <span>${i("notes",e)||"Notes"}</span>
          <textarea
            rows="3"
            @input=${a=>{let s=a.target.value;this._set("notes",s||null)}}
        ...
        <div class="row">
          <label>
            ...
            <input type="number" min="0" step="0.01"
              ...
              @input=${a=>{let s=a.target.value;this._set("cost",s?Number(s):null)}} />
          </label>
          <label>
            <span>${i("duration",e)||"Duration (min)"}</span>
            <input type="number" min="0"
              .value=${t.duration!=null?String(t.duration):""}
              @input=${a=>{let s=a.target.value;this._set("duration",s?Number(s):null)}} />
          </label>
        </div>
        ${this._error?l`<div class="error">${this._error}</div>`:d}
        <div class="actions">
  ...
  `,p([x({attribute:!1})],G.prototype,"hass",2),p([g()],G.prototype,"_open",2),p([g()],G.prototype,"_title",2),p([g()],G.prototype,"_message",2),p([g()],G.prototype,"_confirmText",2),p([g()],G.prototype,"_danger",2),p([g()],G.prototype,"_inputLabel",2),p([g()],G.prototype,"_inputType",2),p([g()],G.prototype,"_inputValue",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",G);var It={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},Pt=r=>(...n)=>({_$litDirective$:r,values:n}),Pe=class{constructor(n){}get _$AU(){return this._$AM._$AU}_$AT(n,e,t){this._$Ct=n,this._$AM=e,this._$Ci=t}_$AS(n,e){return this.update(n,e)}update(n,e){return this.render(...e)}};var $e=class extends Pe{constructor(n){if(super(n),this.it=d,n.type!==It.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(n){if(n===d||n==null)return this._t=void 0,this.it=n;if(n===te)return n;if(typeof n!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(n===this.it)return this._t;this.it=n;let e=[n];return e.raw=e,this._t={_$litType$:this.constructor.resultType,strings:e,values:[]}}};$e.directiveName="unsafeHTML",$e.resultType=1;var Ft=Pt($e);var Ya=["EUR","USD","GBP","JPY","CHF","CAD","AUD","CNY","INR","BRL","CZK","PLN","RUB","SEK","NOK","DKK","UAH"],A=class extends T{constructor(){super(...arguments);this.budget=null;this._settings=null;this._loading=!0;this._importCsv="";this._importLoading=!1;this._includeHistory=!0;this._toast="";this._testingNotification=!1;this._users=[];this._vacEnabled=!1;this._vacStart="";this._vacEnd="";this._vacBuffer=3;this._vacExempt=new Set;this._vacIsActive=!1;this._vacWindowEnd=null;this._vacAllTasks=[];this._vacPreview=[];this._vacPreviewLoading=!1;this._vacSaving=!1;this._qrObjects=[];this._qrSelectedEntries=new Set;this._qrActions=new Set(["view"]);this._qrUrlMode="companion";this._qrBatchLoading=!1;this._qrBatchResults=[];this._qrObjectsLoaded=!1;this._loaded=!1;this._userService=null;this._sendTestNotification=async()=>{this._testingNotification=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/test_notification"}),t=e.message||(e.success?i("test_notification_success",this._lang):i("test_notification_failed",this._lang));this._showToast(t)}catch{this._showToast(i("test_notification_failed",this._lang))}finally{this._testingNotification=!1}}}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._loaded?(this._loaded=!0,this._userService=new oe(this.hass),this._loadSettings(),this._loadUsers()):e.has("hass")&&this.hass&&this._userService&&this._userService.updateHass(this.hass)}async _loadUsers(){if(this._userService)try{this._users=await this._userService.getUsers()}catch{this._users=[]}}async _loadSettings(){this._loading=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});this._settings=e,this._hydrateVacationFromSettings()}catch{}this._loading=!1}_hydrateVacationFromSettings(){let e=this._settings?.vacation;e&&(this._vacEnabled=e.enabled,this._vacStart=e.start||"",this._vacEnd=e.end||"",this._vacBuffer=e.buffer_days,this._vacExempt=new Set(e.exempt_task_ids||[]),this._vacIsActive=e.is_active,this._vacWindowEnd=e.window_end)}async _updateSetting(e,t){try{let a=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/update",settings:{[e]:t}});this._settings=a,this._showToast(i("settings_saved",this._lang)),this.dispatchEvent(new CustomEvent("settings-changed"))}catch{this._showToast(i("action_error",this._lang))}}_showToast(e){this._toast=e,setTimeout(()=>{this._toast=""},3e3)}_downloadFile(e,t,a){let s=new Blob([e],{type:a}),o=URL.createObjectURL(s),c=document.createElement("a");c.href=o,c.download=t,c.click(),URL.revokeObjectURL(o)}render(){let e=this._lang;return this._loading||!this._settings?l`<div class="settings-loading">Loading…</div>`:l`
    ...
    `}scrollToSection(e){requestAnimationFrame(()=>{let t=this.shadowRoot;if(!t)return;let a=t.querySelector(`[data-section="${e}"]`)??t.querySelector(`[data-section-alt="${e}"]`);a&&a.scrollIntoView({behavior:"smooth",block:"start"})})}_renderPanelAccess(e){let t=new Set(this._settings.admin_panel_user_ids||[]),a=this._users.filter(o=>!o.is_admin),s=(o,c)=>{let _=new Set(t);c?_.add(o):_.delete(o),this._updateSetting("admin_panel_user_ids",[..._])};return l`
      <div class="settings-section">
        <h3>${i("settings_panel_access",e)} ${t.size>0?l`<span class="section-badge">${t.size}</span>`:d}</h3>
        <p class="section-desc">${i("settings_panel_access_desc",e)}</p>
        ${a.length===0?l`<div class="setting-row hint">${i("no_non_admin_users",e)}</div>`:a.map(o=>l`
        ...
        ${a.map(s=>l`
          <label class="setting-row">
            ...
            <input type="checkbox" .checked=${t[s.key]}
              @change=${o=>this._updateSetting(s.settingKey,o.target.checked)} />
          </label>
        `)}
      </div>
    `}_renderGeneral(e){let t=this._settings.general,a=this._settings.budget;return l`
      <div class="settings-section">
        ...
        <label class="setting-row">
          ...
          <input type="number" min="1" max="365" .value=${String(t.default_warning_days)}
            @change=${s=>{let o=parseInt(s.target.value,10);o>=1&&o<=365&&this._updateSetting("default_warning_days",o)}} />
        </label>
        <label class="setting-row">
          <span class="setting-label">${i("settings_currency",e)}</span>
          <select @change=${s=>this._updateSetting("budget_currency",s.target.value)}>
            ${Ya.map(s=>l`<option value=${s} ?selected=${a.currency===s}>${s}</option>`)}
          </select>
        </label>
        <label class="setting-row">
          <span class="setting-label">${i("settings_panel_enabled",e)}</span>
          <input type="checkbox" .checked=${t.panel_enabled}
            @change=${s=>this._updateSetting("panel_enabled",s.target.checked)} />
        </label>
        <label class="setting-row">
          <span class="setting-label">${i("settings_notifications",e)}</span>
          <input type="checkbox" .checked=${t.notifications_enabled}
            @change=${s=>this._updateSetting("notifications_enabled",s.target.checked)} />
        </label>
        ${t.notifications_enabled?l`
          <label class="setting-row">
            <span class="setting-label">${i("settings_notify_service",e)}</span>
            <input type="text" .value=${t.notify_service}
              @change=${s=>this._updateSetting("notify_service",s.target.value.trim())} />
          </label>
    ...
    `}_renderVacation(e){let t=this._vacEnabled&&!this._vacIsActive&&this._vacWindowEnd&&new Date(this._vacWindowEnd)<new Date,a=this._vacExempt.size;return l`
      <div class="settings-section vacation-section" data-section="vacation">
        <h3>
          ${i("vacation_title",e)}
          ${this._vacIsActive?l`<span class="vac-badge active">${i("vacation_active",e)}</span>`:d}
          ${t?l`<span class="vac-badge stale">${i("vacation_ended",e)}</span>`:d}
        </h3>
        <p class="section-desc">${i("vacation_desc",e)}</p>

        <label class="vac-toggle">
          <input type="checkbox" .checked=${this._vacEnabled}
            @change=${s=>this._toggleVacationEnabled(s.target.checked)} />
          ${i("vacation_enable",e)}
        </label>
        ...
        <details class="vac-exempt-panel">
          <summary>
            ...
            ${a>0?l`<span class="section-badge">${a}</span>`:d}
          </summary>
          <p class="section-desc">${i("vacation_exempt_desc",e)}</p>
          ${this._vacAllTasks.length===0?l`<button @click=${this._loadAllTasksForVacation}>${i("vacation_load_tasks",e)}</button>`:l`
        ...
        ${this._vacStart&&this._vacEnd?l`
          <div class="vac-preview-toolbar">
            ...
            ${this._vacPreview.length>0?l`<span class="vac-preview-count">${this._vacPreview.length} ${i("vacation_preview_affected",e)}</span>`:d}
          </div>
          ${this._vacPreview.length>0?this._renderVacationPreview(e):d}
        `:d}

        ${this._vacIsActive||t?l`<button class="vac-end-now" @click=${this._endVacationNow}>
              ${i("vacation_end_now",e)}
            </button>`:d}
      </div>
    `}_renderVacationTaskList(e){let t=new Map;for(let s of this._vacAllTasks){let o=t.get(s.object_name)||[];o.push(s),t.set(s.object_name,o)}return[...t.entries()].sort(([s],[o])=>s.localeCompare(o)).map(([s,o])=>l`
      <div class="vac-task-group">
        <div class="vac-task-group-name">${s||i("no_objects",e)}</div>
        ${o.sort((c,_)=>c.task_name.localeCompare(_.task_name)).map(c=>l`
    ...
    `)}_renderVacationPreview(e){return l`
      <div class="vac-preview-list">
        ${this._vacPreview.map(t=>{let a=t.events.map(o=>{let c=`vacation_event_${o.status}`;return`${o.date} (${i(c,e)})`}).join(" \xB7 "),s=!t.will_suppress;return l`
            <div class="vac-preview-row ${s?"exempt":""}">
              <div class="vac-preview-info">
                <div class="vac-preview-name">
                  <strong>${t.object_name}</strong> · ${t.task_name}
                  ${t.kind==="sensor_based"?l`<span class="vac-preview-kind">${i("vacation_sensor_based",e)}</span>`:d}
                </div>
        ...
        ${this._qrObjectsLoaded?l`
            <details open class="qr-filter-panel">
              ...
              <div class="qr-filter-group">
                ...
                <div class="qr-object-list">
                  ${this._qrObjects.length===0?l`<div class="hint">${i("no_objects",e)}</div>`:this._qrObjects.map(c=>l`
            ...
            ${this._qrBatchResults.length>0?l`
                ...
                <div class="qr-print-grid">
                  ${this._qrBatchResults.map(c=>l`
                    <div class="qr-print-cell">
                      <div class="qr-svg">${Ft(c.svg)}</div>
                      <div class="qr-label">
...
<body>
...
</html>`,o=window.open("","_blank","width=900,height=1100");if(!o){window.print();return}o.document.open(),o.document.write(s),o.document.close()}_escapeHtml(e){return e.replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}_renderImportExport(e){return l`
      <div class="settings-section">
  ...
  `,p([x({attribute:!1})],A.prototype,"hass",2),p([x({attribute:!1})],A.prototype,"features",2),p([x({attribute:!1})],A.prototype,"budget",2),p([g()],A.prototype,"_settings",2),p([g()],A.prototype,"_loading",2),p([g()],A.prototype,"_importCsv",2),p([g()],A.prototype,"_importLoading",2),p([g()],A.prototype,"_includeHistory",2),p([g()],A.prototype,"_toast",2),p([g()],A.prototype,"_testingNotification",2),p([g()],A.prototype,"_users",2),p([g()],A.prototype,"_vacEnabled",2),p([g()],A.prototype,"_vacStart",2),p([g()],A.prototype,"_vacEnd",2),p([g()],A.prototype,"_vacBuffer",2),p([g()],A.prototype,"_vacExempt",2),p([g()],A.prototype,"_vacIsActive",2),p([g()],A.prototype,"_vacWindowEnd",2),p([g()],A.prototype,"_vacAllTasks",2),p([g()],A.prototype,"_vacPreview",2),p([g()],A.prototype,"_vacPreviewLoading",2),p([g()],A.prototype,"_vacSaving",2),p([g()],A.prototype,"_qrObjects",2),p([g()],A.prototype,"_qrSelectedEntries",2),p([g()],A.prototype,"_qrActions",2),p([g()],A.prototype,"_qrUrlMode",2),p([g()],A.prototype,"_qrBatchLoading",2),p([g()],A.prototype,"_qrBatchResults",2),p([g()],A.prototype,"_qrObjectsLoaded",2);customElements.define("maintenance-settings-view",A);var Xa=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"],Y=class extends T{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._entryId="";this._taskId="";this._values=new Array(12).fill("");this._save=async()=>{let e=this._buildOverrides();if(e!==null){this._loading=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/seasonal_overrides",entry_id:this._entryId,task_id:this._taskId,overrides:e}),this._open=!1,this.dispatchEvent(new CustomEvent("overrides-saved"))}catch(t){this._error=W(t,this._lang,i("save_error",this._lang))}finally{this._loading=!1}}};this._clearAll=async()=>{this._loading=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/seasonal_overrides",entry_id:this._entryId,task_id:this._taskId,overrides:{}}),this._values=new Array(12).fill(""),this._open=!1,this.dispatchEvent(new CustomEvent("overrides-saved"))}catch(e){this._error=W(e,this._lang,i("save_error",this._lang))}finally{this._loading=!1}}}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}open(e,t,a){if(this._entryId=e,this._taskId=t,this._values=new Array(12).fill(""),a)for(let[s,o]of Object.entries(a)){let c=parseInt(s,10);c>=1&&c<=12&&typeof o=="number"&&(this._values[c-1]=o.toString())}this._error="",this._open=!0}_close(){this._open=!1}_buildOverrides(){let e={};for(let t=0;t<12;t++){let a=this._values[t].trim();if(!a)continue;let s=parseFloat(a);if(Number.isNaN(s))return this._error=`${i("month_"+["jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec"][t],this._lang)}: ${i("seasonal_override_invalid",this._lang)}`,null;if(s<.1||s>5)return this._error=i("seasonal_override_range",this._lang),null;e[t+1]=s}return e}render(){if(!this._open)return l``;let e=this._lang;return l`
            ...
            ${Xa.map((t,a)=>l`
              <label class="month">
                ...
                <input type="number" step="0.1" min="0.1" max="5.0"
                  ...
                  @input=${s=>{let o=[...this._values];o[a]=s.target.value,this._values=o}} />
              </label>
            `)}
          </div>
        </div>
              ...
              <div class="objects">
                ${[...this.objects].sort((a,s)=>a.object.name.localeCompare(s.object.name)).map(a=>l`
                  <div class="object-block">
                    ...
                    ${a.tasks.length===0?l`<div class="hint small">${i("no_tasks_short",e)}</div>`:[...a.tasks].sort((s,o)=>s.name.localeCompare(o.name)).map(s=>{let o=`${a.entry_id}:${s.id}`,c=this._selected.has(o);return l`
  ...
  `,p([x({attribute:!1})],Q.prototype,"hass",2),p([x({attribute:!1})],Q.prototype,"objects",2),p([g()],Q.prototype,"_open",2),p([g()],Q.prototype,"_loading",2),p([g()],Q.prototype,"_error",2),p([g()],Q.prototype,"_groupId",2),p([g()],Q.prototype,"_name",2),p([g()],Q.prototype,"_description",2),p([g()],Q.prototype,"_selected",2);customElements.get("maintenance-group-dialog")||customElements.define("maintenance-group-dialog",Q);var ei=300,ti=140,it=27;function Ht(r,n){let e=r.trigger_config;if(!e)return d;let t=n.lang,a=r.trigger_entity_info,s=r.trigger_entity_infos,o=a?.friendly_name||e.entity_id||"\u2014",c=e.entity_id||"",_=e.entity_ids||(c?[c]:[]),u=a?.unit_of_measurement||"",h=r.trigger_current_value,m=e.type||"threshold",v=_.length>1;return l`
    <h3>${i("trigger",t)}</h3>
      ...
      <div class="trigger-header">
        ...
        <div>
          ${v?l`
            ...
            <div class="trigger-entity-id">${_.map((f,k)=>l`${k>0?", ":""}<span class="entity-link" @click=${b=>de(b,f)}>${f}</span>`)}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `:l`
            <div class="trigger-entity-name">${o}</div>
            <div class="trigger-entity-id">${c?l`<span class="entity-link" @click=${f=>de(f,c)}>${c}</span>`:""}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `}
        </div>
        <span class="status-badge ${r.trigger_active?"triggered":"ok"}" style="margin-left: auto;">
          ${r.trigger_active?i("triggered",t):i("ok",t)}
        </span>
      </div>

      ${h!=null?l`
            <div class="trigger-value-row">
              <span class="trigger-current ${r.trigger_active?"active":""}">${typeof h=="number"?h.toFixed(1):h}</span>
              ${u?l`<span class="trigger-unit">${u}</span>`:d}
            </div>
          `:d}

      <div class="trigger-limits">
        ${m==="threshold"?l`
          ${e.trigger_above!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${i("threshold_above",t)}: ${e.trigger_above} ${u}</span>`:d}
          ${e.trigger_below!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${i("threshold_below",t)}: ${e.trigger_below} ${u}</span>`:d}
          ${e.trigger_for_minutes?l`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${i("for_minutes",t)}: ${e.trigger_for_minutes}</span>`:d}
        `:d}
        ${m==="counter"?l`
          ${e.trigger_target_value!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${i("target_value",t)}: ${e.trigger_target_value} ${u}</span>`:d}
        `:d}
        ${m==="state_change"?l`
          ${e.trigger_target_changes!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${i("target_changes",t)}: ${e.trigger_target_changes}</span>`:d}
        `:d}
        ${m==="runtime"?l`
          ${e.trigger_runtime_hours!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${i("runtime_hours",t)}: ${e.trigger_runtime_hours}h</span>`:d}
        `:d}
        ${m==="compound"?l`
          ...
          ${(e.conditions||[]).map((f,k)=>l`
            <span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${k+1}. ${i(f.type||"unknown",t)}: ${f.entity_id?l`<span class="entity-link" @click=${b=>de(b,f.entity_id)}>${f.entity_id}</span>`:""}</span>
          `)}
        `:d}
        ${a?.min!=null?l`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${i("min",t)}: ${a.min} ${u}</span>`:d}
        ${a?.max!=null?l`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${i("max",t)}: ${a.max} ${u}</span>`:d}
      </div>
  ...
  `}function ai(r,n,e){let t=r.trigger_config;if(!t)return d;let a=t.type||"threshold",s=a==="counter"&&t.trigger_delta_mode,o=e.isCounterEntity(t),c=t.entity_id||"",_=e.detailStatsData.get(c)||[],u=[],h=!1;if(_.length>=2)for(let y of _){let S=y.val;s&&r.trigger_baseline_value!=null&&(S-=r.trigger_baseline_value);let H={ts:y.ts,val:S};!o&&y.min!=null&&y.max!=null&&(H.min=s&&r.trigger_baseline_value!=null?y.min-r.trigger_baseline_value:y.min,H.max=s&&r.trigger_baseline_value!=null?y.max-r.trigger_baseline_value:y.max,h=!0),u.push(H)}else for(let y of r.history)y.trigger_value!=null&&u.push({ts:new Date(y.timestamp).getTime(),val:y.trigger_value});if(r.trigger_current_value!=null){let y=r.trigger_current_value;s&&r.trigger_baseline_value!=null&&(y-=r.trigger_baseline_value),u.push({ts:Date.now(),val:y})}if(u.length<2&&c&&e.hasStatsService&&!e.detailStatsData.has(c))return l`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
      <ha-icon icon="mdi:chart-line" style="--mdc-icon-size:16px;margin-right:8px;"></ha-icon>
      ${i("loading_chart",e.lang)}
    </div>`;if(u.length<2)return d;u.sort((y,S)=>y.ts-S.ts);let m=ei,v=ti,f=30,k=2,b=8,N=16,C=u.map(y=>y.val),E=Math.min(...C),$=Math.max(...C);if(h)for(let y of u)y.min!=null&&(E=Math.min(E,y.min)),y.max!=null&&($=Math.max($,y.max));t.trigger_above!=null&&($=Math.max($,t.trigger_above),E=Math.min(E,t.trigger_above)),t.trigger_below!=null&&(E=Math.min(E,t.trigger_below),$=Math.max($,t.trigger_below));let M=null,L=null;if(a==="counter"&&t.trigger_target_value!=null){if(r.trigger_baseline_value!=null)M=r.trigger_baseline_value;else if(u.length>0){let y=[...r.history].filter(S=>S.type==="completed"||S.type==="reset").sort((S,H)=>new Date(H.timestamp).getTime()-new Date(S.timestamp).getTime())[0];if(y){let S=new Date(y.timestamp).getTime(),H=u[0],O=Math.abs(u[0].ts-S);for(let ie of u){let ne=Math.abs(ie.ts-S);ne<O&&(H=ie,O=ne)}M=H.val}else M=u[0].val}M!=null?(L=M+t.trigger_target_value,$=Math.max($,L),E=Math.min(E,M)):($=Math.max($,t.trigger_target_value),E=Math.min(E,0))}s&&r.trigger_baseline_value!=null&&(E=Math.min(E,0));let V=$-E||1;E-=V*.1,$+=V*.1;let K=u[0].ts,D=u[u.length-1].ts,z=D-K||1,I=y=>f+(y-K)/z*(m-f-k),R=y=>b+(1-(y-E)/($-E))*(v-b-N),Fe=u.map(y=>`${I(y.ts).toFixed(1)},${R(y.val).toFixed(1)}`).join(" "),Zt=`M${I(u[0].ts).toFixed(1)},${v-N} `+u.map(y=>`L${I(y.ts).toFixed(1)},${R(y.val).toFixed(1)}`).join(" ")+` L${I(u[u.length-1].ts).toFixed(1)},${v-N} Z`,Le="";if(h){let y=u.filter(S=>S.min!=null&&S.max!=null);if(y.length>=2){let S=y.map(O=>`${I(O.ts).toFixed(1)},${R(O.max).toFixed(1)}`),H=[...y].reverse().map(O=>`${I(O.ts).toFixed(1)},${R(O.min).toFixed(1)}`);Le=`M${S[0]} `+S.slice(1).map(O=>`L${O}`).join(" ")+` L${H[0]} `+H.slice(1).map(O=>`L${O}`).join(" ")+" Z"}}let nt=u[u.length-1],Jt=I(nt.ts),Yt=R(nt.val),st=y=>Math.abs(y)>=1e4?(y/1e3).toFixed(0)+"k":y>=1e3?(y/1e3).toFixed(1)+"k":y.toFixed(y<10?1:0),Xt=st($),ea=st(E),ta=r.history.filter(y=>["completed","skipped","reset"].includes(y.type)).map(y=>({ts:new Date(y.timestamp).getTime(),type:y.type})).filter(y=>y.ts>=K&&y.ts<=D),He=u;if(u.length>it){let y=(u.length-1)/(it-1);He=[];for(let S=0;S<it;S++)He.push(u[Math.round(S*y)])}return l`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${m} ${v}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_sparkline",e.lang)}">
        <text x="${f-3}" y="${b+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${Xt}</text>
        <text x="${f-3}" y="${v-N+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${ea}</text>
        ...
        ${Le?P`<path d="${Le}" fill="var(--primary-color)" opacity="0.08" />`:d}
        <path d="${Zt}" fill="var(--primary-color)" opacity="0.15" />
        <polyline points="${Fe}" fill="none" stroke="var(--primary-color)" stroke-width="2" stroke-linejoin="round" />
        ${r.degradation_rate!=null&&r.degradation_trend!=="stable"&&r.degradation_trend!=="insufficient_data"&&u.length>=2?(()=>{let y=u[u.length-1],S=30,H=y.ts+S*864e5,O=y.val+r.degradation_rate*S,ie=Math.min(H,D+(D-K)*.3),ne=Math.max(E,Math.min($,O)),aa=I(y.ts),ia=R(y.val),na=I(ie),sa=R(ne);return P`<line x1="${aa.toFixed(1)}" y1="${ia.toFixed(1)}" x2="${na.toFixed(1)}" y2="${sa.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():d}
        ...
        <circle cx="${Jt.toFixed(1)}" cy="${Yt.toFixed(1)}" r="3.5" fill="var(--primary-color)" />
        ${ta.map(y=>{let S=I(y.ts),H=y.type==="completed"?"var(--success-color, #4caf50)":y.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return P`
          ...
          `})}
        ${He.map(y=>{let S=I(y.ts),H=R(y.val),O=new Date(y.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),ie=`${y.val.toFixed(1)} ${n}`;return h&&y.min!=null&&y.max!=null&&(ie+=` (${y.min.toFixed(1)}\u2013${y.max.toFixed(1)})`),P`<circle cx="${S.toFixed(1)}" cy="${H.toFixed(1)}" r="8" fill="transparent" tabindex="0"
            @mouseenter=${ne=>Lt(ne,`${O}
${ie}`,e.setTooltip)}
            @focus=${ne=>Lt(ne,`${O}
${ie}`,e.setTooltip)}
...
`).map(y=>l`<div>${y}</div>`)}
  ...
  `}function Lt(r,n,e){let t=r.currentTarget,a=t.closest(".sparkline-container");if(!a)return;let s=a.getBoundingClientRect(),o=t.getBoundingClientRect();e({x:o.left-s.left+o.width/2,y:o.top-s.top-8,text:n})}function Ot(r,n,e){let t=r.degradation_trend!=null&&r.degradation_trend!=="insufficient_data",a=r.days_until_threshold!=null,s=r.environmental_factor!=null&&r.environmental_factor!==1;if(!t&&!a&&!s)return d;let o=r.degradation_trend==="rising"?"M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z":r.degradation_trend==="falling"?"M16,18L18.29,15.71L13.41,10.83L9.41,14.83L2,7.41L3.41,6L9.41,12L13.41,8L19.71,14.29L22,12V18H16Z":"M22,12L18,8V11H3V13H18V16L22,12Z";return l`
    <div class="prediction-section">
      ${r.sensor_prediction_urgency?l`
        <div class="prediction-urgency-banner">
          <ha-svg-icon path="M1,21H23L12,2L1,21M12,18A1,1 0 0,1 11,17A1,1 0 0,1 12,16A1,1 0 0,1 13,17A1,1 0 0,1 12,18M13,15H11V10H13V15Z"></ha-svg-icon>
          ${i("sensor_prediction_urgency",n).replace("{days}",String(Math.round(r.days_until_threshold||0)))}
        </div>
      `:d}
      ...
      <div class="prediction-grid">
        ${t?l`
          <div class="prediction-item">
            ...
            <span class="prediction-value ${r.degradation_trend}">${i("trend_"+r.degradation_trend,n)}</span>
            ${r.degradation_rate!=null?l`<span class="prediction-rate">${r.degradation_rate>0?"+":""}${Math.abs(r.degradation_rate)>=10?Math.round(r.degradation_rate).toLocaleString():r.degradation_rate.toFixed(1)} ${r.trigger_entity_info?.unit_of_measurement||""}/${i("day_short",n)}</span>`:d}
          </div>
        `:d}
        ${a?l`
          <div class="prediction-item">
            ...
            <span class="prediction-value prediction-days${r.days_until_threshold===0?" exceeded":r.sensor_prediction_urgency?" urgent":""}">${r.days_until_threshold===0?i("threshold_exceeded",n):"~"+Math.round(r.days_until_threshold)+" "+i("days",n)}</span>
            ${r.threshold_prediction_date?l`<span class="prediction-date">${X(r.threshold_prediction_date,n)}</span>`:d}
        ...
        ${s&&e.environmental?l`
          <div class="prediction-item">
            ...
            ${r.environmental_entity?l`<span class="prediction-entity entity-link" @click=${c=>de(c,r.environmental_entity)}>${r.environmental_entity}</span>`:d}
          </div>
        `:d}
      </div>
    </div>
  `}function Ut(r,n){let e=r.interval_analysis,t=e?.weibull_beta,a=e?.weibull_eta;if(t==null||a==null||a<=0)return d;let s=r.interval_days??0,o=r.suggested_interval??s;return l`
  ...
  `}function ni(r,n,e,t,a){let f=Math.max(e,t,n,1)*1.3,k=50,b=[];for(let D=0;D<=k;D++){let z=D/k*f,I=1-Math.exp(-Math.pow(z/n,r)),R=32+z/f*260,Fe=136-I*128;b.push([R,Fe])}let N=b.map(([D,z])=>`${D.toFixed(1)},${z.toFixed(1)}`).join(" "),C="M32,136 "+b.map(([D,z])=>`L${D.toFixed(1)},${z.toFixed(1)}`).join(" ")+` L${b[k][0].toFixed(1)},136 Z`,E=32+e/f*260,$=1-Math.exp(-Math.pow(e/n,r)),M=136-$*128,L=((1-$)*100).toFixed(0),V=32+t/f*260,K=[0,.25,.5,.75,1];return l`
    ...
    <div class="chart-legend">
      ...
      ${e>0?l`<span class="legend-item"><span class="legend-swatch" style="background:var(--primary-color, #03a9f4); opacity:0.5"></span> ${i("current_interval_marker",a)}</span>`:d}
      ${t>0&&t!==e?l`<span class="legend-item"><span class="legend-swatch" style="background:var(--success-color, #4caf50)"></span> ${i("recommended_marker",a)}</span>`:d}
    </div>
  `}function si(r,n){return l`
    <div class="weibull-info-row">
      <div class="weibull-info-item">
        <span>${i("characteristic_life",n)}</span>
        <span class="weibull-info-value">${Math.round(r.weibull_eta)} ${i("days",n)}</span>
      </div>
  ...
  `}function oi(r,n,e){let t=r.confidence_interval_low,a=r.confidence_interval_high,s=n.suggested_interval??n.interval_days??0,o=n.interval_days??0,c=Math.max(0,t-5),u=a+5-c,h=(t-c)/u*100,m=(a-t)/u*100,v=(s-c)/u*100,f=o>0?(o-c)/u*100:-1;return l`
  ...
  `}function Bt(r,n,e,t){let a=Math.max(r||1,n);return l`
    <div class="interval-comparison">
      <div class="interval-bar">
        <div class="interval-label">
          ${i("current",t)}: ${r??"\u2014"} ${r!=null?i("days",t):""}
        </div>
        <div class="interval-visual current"
          style="width: ${r!=null?Math.min(r/a*100,100):0}%"></div>
      </div>
      <div class="interval-bar">
        <div class="interval-label">
          ${i("recommended",t)}: ${n} ${i("days",t)}
          <span class="confidence-badge ${e}">${i(`confidence_${e}`,t)}</span>
        </div>
        <div class="interval-visual suggested"
          style="width: ${Math.min(n/a*100,100)}%"></div>
      </div>
    </div>
  `}var Vt=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"];function Wt(r,n,e){if(!e.seasonal||!r.seasonal_factor||r.seasonal_factor===1)return d;let t=Vt.map(c=>i(c,n)),a=new Date().getMonth(),s=r.seasonal_factors||r.interval_analysis?.seasonal_factors||null,o=s&&s.length===12?s:t.map((c,_)=>{let u=r.seasonal_factor||1,h=Math.sin((_-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,u+h))});return l`
    <div class="seasonal-card-compact">
      <h4>${i("seasonal_awareness",n)}</h4>
      <div class="seasonal-mini-chart">
        ${o.map((c,_)=>{let u=c*40,h=c<.9?"low":c>1.1?"high":"normal";return l`
            <div class="seasonal-bar ${h} ${_===a?"current":""}"
                 style="height: ${u}px"
  ...
  `}function Gt(r,n){return ri(r,n)}function ri(r,n){let e=r.seasonal_factors??r.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return d;let t=r.interval_analysis?.seasonal_reason,a=new Date().getMonth(),s=300,o=100,c=8,u=o-c-4,h=Math.max(...e,1.5),m=s/12,v=m*.65,f=c+u-1/h*u;return l`
    <div class="seasonal-chart">
      <div class="seasonal-chart-title">
        ...
        ${t?l`<span class="source-tag">${t==="learned"?i("seasonal_learned",n):i("seasonal_manual",n)}</span>`:d}
      </div>
      <svg viewBox="0 0 ${s} ${o}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_seasonal",n)}">
      ...
      <div class="seasonal-labels">
        ${Vt.map((k,b)=>l`<span class="seasonal-label ${b===a?"active-month":""}">${i(k,n)}</span>`)}
      </div>
    </div>
  `}var li=300,ci=200;function Qt(r,n,e,t){let a=r.history.filter(c=>c.type==="completed"&&(c.cost!=null||c.duration!=null));if(a.length<2)return d;let s=a.some(c=>(c.cost??0)>0),o=a.some(c=>(c.duration??0)>0);return!s&&!o?d:l`
  ...
  `}function di(r,n,e){let t=r.history.filter(z=>z.type==="completed"&&(z.cost!=null||z.duration!=null)).map(z=>({ts:new Date(z.timestamp).getTime(),cost:z.cost??0,duration:z.duration??0})).sort((z,I)=>z.ts-I.ts);if(t.length<2)return d;let a=t.some(z=>z.cost>0),s=t.some(z=>z.duration>0);if(!a&&!s)return d;let o=e!=="duration"&&a,c=e!=="cost"&&s,_=o||!c&&a,u=c||!o&&s,h=li,m=ci,v=_?32:8,f=u?32:8,k=8,b=20,N=h-v-f,C=m-k-b,E=Math.max(...t.map(z=>z.cost))||1,$=Math.max(...t.map(z=>z.duration))||1,M=Math.min(20,N/t.length*.6),L=N/t.length,V=z=>v+L*z+L/2,K=z=>k+C-z/E*C,D=z=>k+C-z/$*C;return l`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${h} ${m}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_history",n)}">
        ${_?t.map((z,I)=>P`
          <rect x="${(V(I)-M/2).toFixed(1)}" y="${K(z.cost).toFixed(1)}" width="${M.toFixed(1)}" height="${(k+C-K(z.cost)).toFixed(1)}"
            fill="var(--primary-color)" opacity="0.6" rx="2" />
        `):d}
        ${u?P`
          <polyline points="${t.map((z,I)=>`${V(I).toFixed(1)},${D(z.duration).toFixed(1)}`).join(" ")}"
            fill="none" stroke="var(--accent-color, #ff9800)" stroke-width="2" stroke-linejoin="round" />
          ${t.map((z,I)=>P`
            <circle cx="${V(I).toFixed(1)}" cy="${D(z.duration).toFixed(1)}" r="3" fill="var(--accent-color, #ff9800)" />
          `)}
        `:d}
  ...
  `}var _i=60,pi=20,Kt=30,j=class extends T{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._sparklineTooltip=null;this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._adminPanelUserIds=[];this._defaultWarningDays=7;this._actionLoading=!1;this._moreMenuOpen=!1;this._toastMessage="";this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab="dashboard";this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._objectSortMode="alphabetical";this._groupByMode="none";this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._deepLinkHandled=!1;this._onDialogEvent=async()=>{try{await this._loadData()}catch{}};this._onCalendarLlCustom=e=>{let t=e.detail;t?.type==="maintenance-supporter:open-task"&&t.entry_id&&t.task_id&&(e.stopPropagation(),this._showTask(t.entry_id,t.task_id))};this._onHistoryEntrySaved=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"en"}get _isOperator(){let e=this.hass?.user;return e?e.is_admin?!1:!this._adminPanelUserIds.includes(e.id):!0}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler);let e=localStorage.getItem("maintenance_supporter_sort");e&&["due_date","object","type","task_name","area","assigned_user","group"].includes(e)&&(this._sortMode=e);let t=localStorage.getItem("maintenance_supporter_object_sort");t&&["alphabetical","due_soonest","task_count"].includes(t)&&(this._objectSortMode=t);let a=localStorage.getItem("maintenance_supporter_groupby");a&&["none","area","group","user"].includes(a)&&(this._groupByMode=a)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new De(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new oe(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,a,s,o]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);if(e&&(this._objects=e.objects),t&&(this._stats=t),a&&(this._budget=a),s&&(this._groups=s.groups||{}),o){let c=o;this._features=c.features,this._adminPanelUserIds=c.admin_panel_user_ids||[];let _=c.general?.default_warning_days;typeof _=="number"&&_>=0&&_<=365&&(this._defaultWarningDays=_)}this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("ms_action"),a=()=>{let h=window.location.pathname+window.location.hash;history.replaceState(history.state,"",h)};if(t==="add_object"){this._deepLinkHandled=!0,a(),requestAnimationFrame(()=>{this.shadowRoot?.querySelector("maintenance-object-dialog")?.openCreate()});return}if(t==="open_vacation"||t==="open_budget"||t==="open_groups"||t==="open_settings"){this._deepLinkHandled=!0,a(),this._overviewTab="settings",requestAnimationFrame(()=>{let h=this.shadowRoot?.querySelector("maintenance-settings-view"),m=t.replace("open_","");h?.scrollToSection?.(m)});return}let s=e.get("entry_id");if(!s)return;this._deepLinkHandled=!0;let o=e.get("task_id"),c=e.get("action"),_=window.location.pathname+window.location.hash;history.replaceState(history.state,"",_);let u=this._getObject(s);if(!u){this._showOverview();return}if(o){let h=u.tasks.find(m=>m.id===o);if(!h){this._showObject(s);return}this._showTask(s,o),c==="complete"?requestAnimationFrame(()=>{this._openCompleteDialog(s,o,h.name,this._features.checklists?h.checklist:void 0,this._features.adaptive&&!!h.adaptive_config?.enabled)}):c==="quick_complete"&&requestAnimationFrame(()=>{this._handleQuickComplete(s,o,h)})}else this._showObject(s)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let a=await this._statsService.getDetailStats(e,t),s=new Map(this._detailStatsData);s.set(e,a),this._detailStatsData=s}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let a of this._objects)for(let s of a.tasks){let o=s.trigger_config?.entity_id;o&&e.push({entityId:o,isCounter:this._isCounterEntity(s.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let m of this._objects)for(let v of m.tasks){if(this._filterStatus&&v.status!==this._filterStatus)continue;if(this._filterUser){let k=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(v.responsible_user_id!==k)continue}let f=[];for(let k of Object.values(this._groups))k.task_refs?.some(b=>b.entry_id===m.entry_id&&b.task_id===v.id)&&f.push(k.name);e.push({entry_id:m.entry_id,task_id:v.id,object_name:m.object.name,task_name:v.name,type:v.type,schedule_type:v.schedule_type,status:v.status,days_until_due:v.days_until_due??null,next_due:v.next_due??null,trigger_active:v.trigger_active,trigger_current_value:v.trigger_current_value??null,trigger_current_delta:v.trigger_current_delta??null,trigger_config:v.trigger_config??null,trigger_entity_info:v.trigger_entity_info??null,times_performed:v.times_performed,total_cost:v.total_cost,interval_days:v.interval_days??null,interval_unit:v.interval_unit??null,interval_anchor:v.interval_anchor??null,is_done:v.is_done??!1,history:v.history||[],enabled:v.enabled,nfc_tag_id:v.nfc_tag_id??null,area_id:m.object.area_id??null,responsible_user_id:v.responsible_user_id??null,group_names:f})}let t={overdue:0,triggered:1,due_soon:2,ok:3},a=(m,v)=>(t[m.status]??9)-(t[v.status]??9),s=(m,v)=>(m.days_until_due??99999)-(v.days_until_due??99999),o=(m,v)=>a(m,v)||s(m,v),c=m=>m.area_id&&this.hass?.areas?.[m.area_id]?.name||"",_=m=>m.responsible_user_id&&this._userService?.getUserName(m.responsible_user_id)||"",u=m=>m.group_names[0]||"",h={due_date:o,object:(m,v)=>m.object_name.localeCompare(v.object_name)||o(m,v),type:(m,v)=>m.type.localeCompare(v.type)||o(m,v),task_name:(m,v)=>m.task_name.localeCompare(v.task_name),area:(m,v)=>{let f=c(m),k=c(v);return!f&&k?1:f&&!k?-1:f.localeCompare(k)||o(m,v)},assigned_user:(m,v)=>{let f=_(m),k=_(v);return!f&&k?1:f&&!k?-1:f.localeCompare(k)||o(m,v)},group:(m,v)=>{let f=u(m),k=u(v);return!f&&k?1:f&&!k?-1:f.localeCompare(k)||o(m,v)}};return e.sort(h[this._sortMode]),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(s=>s.id===t)}_pushPanelState(e,t,a){let s={msp_view:e,msp_entry:t||null,msp_task:a||null};history.pushState(s,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let a=this._getTask(t.msp_entry,t.msp_task);a?.trigger_config?.entity_id&&this._fetchDetailStats(a.trigger_config.entity_id,this._isCounterEntity(a.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1,this._scrollContentToTop()}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null,this._scrollContentToTop()}_filterByStatus(e){this._filterStatus=e,this._overviewTab!=="dashboard"&&(this._overviewTab="dashboard"),this._scrollContentToTop()}_scrollContentToTop(){requestAnimationFrame(()=>{let e=this.shadowRoot?.querySelector(".content");e&&e.scrollTo({top:0,behavior:"smooth"})})}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null,this._scrollContentToTop()}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null,this._scrollContentToTop();let a=this._getTask(e,t);if(a?.trigger_config?.entity_id){let s=a.trigger_config.entity_id,o=this._isCounterEntity(a.trigger_config);this._fetchDetailStats(s,o)}}_showToast(e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}async _deleteObject(e){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:i("delete",this._lang),message:i("confirm_delete_object",this._lang),confirmText:i("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}}async _deleteTask(e,t){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:i("delete",this._lang),message:i("confirm_delete_task",this._lang),confirmText:i("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}}async _skipTask(e,t,a){this._actionLoading=!0;try{let s={type:"maintenance_supporter/task/skip",entry_id:e,task_id:t};a&&(s.reason=a),await this.hass.connection.sendMessagePromise(s),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}finally{this._actionLoading=!1}}async _resetTask(e,t,a){this._actionLoading=!0;try{let s={type:"maintenance_supporter/task/reset",entry_id:e,task_id:t};a&&(s.date=a),await this.hass.connection.sendMessagePromise(s),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}finally{this._actionLoading=!1}}async _applySuggestion(e,t,a){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:a}),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}}_openSeasonalOverrides(e){let t=this.shadowRoot.querySelector("maintenance-seasonal-overrides-dialog");if(!t||!this._selectedEntryId)return;let a=e.adaptive_config?.seasonal_overrides;t.open(this._selectedEntryId,e.id,a)}async _reanalyzeInterval(e,t){try{let a=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:e,task_id:t});a.recommended_interval?this._showToast(`${i("reanalyze_result",this._lang)}: ${a.recommended_interval} ${i("days",this._lang)} (${i(`confidence_${a.confidence}`,this._lang)}, ${a.data_points} ${i("data_points",this._lang)})`):this._showToast(i("reanalyze_insufficient_data",this._lang)),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}}async _promptSkipTask(e,t){let a=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!a)return;let s=await a.prompt({title:i("skip",this._lang),message:i("skip_reason_prompt",this._lang),confirmText:i("skip",this._lang),inputLabel:i("reason_optional",this._lang),inputType:"text"});s.confirmed&&this._skipTask(e,t,s.value||void 0)}async _promptResetTask(e,t){let a=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!a)return;let s=await a.prompt({title:i("reset",this._lang),message:i("reset_date_prompt",this._lang),confirmText:i("reset",this._lang),inputLabel:i("reset_date_optional",this._lang),inputType:"date"});s.confirmed&&this._resetTask(e,t,s.value||void 0)}_dismissSuggestion(e,t){e&&t&&this._dismissedSuggestions.add(`${e}_${t}`),this.requestUpdate()}async _handleQuickComplete(e,t,a){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/quick_complete",entry_id:e,task_id:t}),this._showToast(i("quick_complete_success",this._lang))}catch(s){(s?.code||"")==="no_defaults"?this._openCompleteDialog(e,t,a.name,this._features.checklists?a.checklist:void 0,this._features.adaptive&&!!a.adaptive_config?.enabled):this._showToast(i("action_error",this._lang))}}_openCompleteDialog(e,t,a,s,o){let c=this.shadowRoot.querySelector("maintenance-complete-dialog");c&&(c.entryId=e,c.taskId=t,c.taskName=a,c.lang=this._lang,c.checklist=s||[],c.adaptiveEnabled=!!o,c.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,a,s){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,a,s)}render(){return l`
      ...
      <div class="filter-bar">
        ...
        <label class="filter-field">
          ...
          >
            ...
            ${this._features.groups?l`<option value="group" ?selected=${this._groupByMode==="group"}>${i("groupby_group",a)}</option>`:d}
    ...
    `}_renderGroupedTasks(e,t){let a=new Map,s=i("unassigned",t);for(let _ of e){let u=[];this._groupByMode==="area"?u=[(_.area_id?this.hass?.areas?.[_.area_id]?.name:null)||s]:this._groupByMode==="user"?u=[(_.responsible_user_id?this._userService?.getUserName(_.responsible_user_id):null)||s]:this._groupByMode==="group"&&(u=_.group_names.length>0?_.group_names:[s]);for(let h of u)a.has(h)||a.set(h,[]),a.get(h).push(_)}let o=[...a.entries()].sort(([_],[u])=>_===s&&u!==s?1:u===s&&_!==s?-1:_.localeCompare(u)),c=this._groupByMode==="area"?"mdi:map-marker-outline":this._groupByMode==="group"?"mdi:folder-outline":"mdi:account-outline";return l`
      ${o.map(([_,u])=>l`
      ...
      `)}
    `}_renderAllObjects(){let e=this._lang,t=this._isOperator,a=_=>{let u=1/0;for(let h of _.tasks){let m=h.days_until_due;m!=null&&m<u&&(u=m)}return u},s=[...this._objects];this._objectSortMode==="alphabetical"?s.sort((_,u)=>_.object.name.localeCompare(u.object.name)):this._objectSortMode==="task_count"?s.sort((_,u)=>u.tasks.length-_.tasks.length||_.object.name.localeCompare(u.object.name)):s.sort((_,u)=>a(_)-a(u)||_.object.name.localeCompare(u.object.name));let o=()=>{let _=new Map;for(let u of s){let h=u.object.area_id,m=h?this.hass?.areas?.[h]?.name||i("unassigned",e):i("no_area",e);_.has(m)||_.set(m,[]),_.get(m).push(u)}return new Map([..._.entries()].sort(([u],[h])=>u.localeCompare(h)))},c=_=>{let u=_.tasks.some(h=>h.status==="overdue"||h.status==="triggered");return l`
        <div class="object-card${u?" object-card-overdue":""}" @click=${()=>this._showObject(_.entry_id)}>
          ${u?l`<span class="overdue-dot" title="${i("has_overdue",e)}"></span>`:d}
          <div class="object-card-header">
            <span class="object-card-name">${_.object.name}</span>
            <span class="object-card-count">${_.tasks.length} ${i("tasks_lower",e)}</span>
          </div>
          ${_.object.manufacturer||_.object.model?l`<div class="object-card-meta">${[_.object.manufacturer,_.object.model].filter(Boolean).join(" ")}</div>`:d}
          ${_.tasks.length===0?l`<div class="object-card-empty">${i("no_tasks_yet",e)}</div>`:d}
        </div>
      `};return l`
      ...
      ${this._groupByMode==="area"?l`
          ${[...o().entries()].map(([_,u])=>l`
            <details class="group-section" open>
              ...
              <div class="objects-grid">${u.map(c)}</div>
            </details>
          `)}
        `:l`<div class="objects-grid">${s.map(c)}</div>`}
    `}async _onSettingsChanged(){await this._loadData()}_renderGroupsSection(){if(!this._features.groups)return d;let e=Object.entries(this._groups),t=this._lang;return l`
      <div class="groups-section">
        ...
        ${e.length===0?l`<div class="hint">${i("no_groups",t)}</div>`:l`
            <div class="groups-grid">
              ${e.map(([a,s])=>{let o=s.task_refs.map(c=>this._getTask(c.entry_id,c.task_id)?.name).filter(Boolean);return l`
                  <div class="group-card">
                    ...
                    ${s.description?l`<div class="group-card-desc">${s.description}</div>`:d}
                    <div class="group-card-tasks">
                      ${o.length>0?o.map(c=>l`<span class="group-task-chip">${c}</span>`):l`<span style="font-size:12px;color:var(--secondary-text-color)">${i("no_tasks_short",t)}</span>`}
                    </div>
                  </div>
                `})}
            </div>
    ...
    `}_renderOverviewRow(e){let t=this._lang,a=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,s=0,o=Ce.ok,c=!1;if(a&&e.days_until_due!==null){let m=at(e.interval_days,e.days_until_due,e.interval_unit);s=m.pct,c=m.overflow,e.status==="overdue"?o=Ce.overdue:e.status==="due_soon"&&(o=Ce.due_soon)}let _=e.area_id?this.hass?.areas?.[e.area_id]?.name:null,u=e.responsible_user_id?this._userService?.getUserName(e.responsible_user_id):null,h=e.group_names.length>0||_||u;return l`
      <div class="task-row${e.enabled?"":" task-disabled"}">
        <span class="cell-badges">
          ...
          ${e.enabled?d:l`<span class="badge-disabled">${i("disabled",t)}</span>`}
          ${e.nfc_tag_id?l`<span class="nfc-badge" title="${i("nfc_linked",t)}"><ha-icon icon="mdi:nfc-variant"></ha-icon></span>`:d}
        </span>
        ...
        <span class="due-cell" @click=${()=>this._showTask(e.entry_id,e.task_id)}>
          ...
          ${a?l`<div class="days-bar"><div class="days-bar-fill${c?" overflow":""}" style="width:${s}%;background:${o}"></div></div>`:d}
    ...
    `}_openCompleteDialogForRow(e){let a=this._objects.find(s=>s.entry_id===e.entry_id)?.tasks.find(s=>s.id===e.task_id);this._openCompleteDialog(e.entry_id,e.task_id,e.task_name,this._features.checklists?a?.checklist:void 0,this._features.adaptive&&!!a?.adaptive_config?.enabled)}_renderTriggerProgress(e){let t=e.trigger_config??null;if(!t)return d;let a=t.type||"threshold",s=e.trigger_entity_info?.unit_of_measurement??"",o=0,c="";if(a==="threshold"){let h=e.trigger_current_value??null;if(h==null)return d;let m=t.trigger_above,v=t.trigger_below;if(m!=null){let f=v??0,k=m-f||1;o=Math.min(100,Math.max(0,(h-f)/k*100)),c=`${h.toFixed(1)} / ${m} ${s}`}else if(v!=null){let k=e.trigger_entity_info?.max??(v*2||100),b=k-v||1;o=Math.min(100,Math.max(0,(k-h)/b*100)),c=`${h.toFixed(1)} / ${v} ${s}`}else return d}else if(a==="counter"){let h=t.trigger_target_value||1,v=e.trigger_current_delta??null??e.trigger_current_value??null;if(v==null)return d;o=Math.min(100,Math.max(0,v/h*100)),c=`${v.toFixed(1)} / ${h} ${s}`}else if(a==="state_change"){let h=t.trigger_target_changes||1,m=e.trigger_current_value??null;if(m==null)return d;o=Math.min(100,Math.max(0,m/h*100)),c=`${Math.round(m)} / ${h}`}else if(a==="runtime"){let h=t.trigger_runtime_hours||100,m=e.trigger_current_value??null;if(m==null)return d;o=Math.min(100,Math.max(0,m/h*100)),c=`${m.toFixed(1)}h / ${h}h`}else if(a==="compound"){let h=t.compound_logic||t.operator||"AND",m=t.conditions?.length||0;c=`${h} (${m})`,o=e.trigger_active?100:0}else return d;let _=o>=100,u=o>90?"var(--error-color, #f44336)":o>70?"var(--warning-color, #ff9800)":"var(--primary-color)";return l`
      <div class="trigger-progress">
        <div class="trigger-progress-bar">
          <div class="trigger-progress-fill${_?" overflow":""}" style="width:${o}%;background:${u}"></div>
        </div>
        <span class="trigger-progress-label">${c}</span>
      </div>
    `}_renderMiniSparkline(e){if(!e.trigger_config?.entity_id)return d;let t=e.trigger_config.entity_id,a=this._miniStatsData.get(t)||[],s=[];if(a.length>=2)s=a.map($=>({ts:$.ts,val:$.val}));else{if(!e.history)return d;for(let $ of e.history)$.trigger_value!=null&&s.push({ts:new Date($.timestamp).getTime(),val:$.trigger_value})}if(e.trigger_current_value!=null&&s.push({ts:Date.now(),val:e.trigger_current_value}),s.length<2)return d;s.sort(($,M)=>$.ts-M.ts);let o=_i,c=pi,_=s.map($=>$.val),u=Math.min(..._),h=Math.max(..._),m=h-u||1;u-=m*.1,h+=m*.1;let v=s[0].ts,k=s[s.length-1].ts-v||1,b=$=>($-v)/k*o,N=$=>2+(1-($-u)/(h-u))*(c-4),C=s;if(C.length>Kt){let $=Math.ceil(C.length/Kt);C=C.filter((M,L)=>L%$===0||L===C.length-1)}let E=C.map($=>`${b($.ts).toFixed(1)},${N($.val).toFixed(1)}`).join(" ");return l`
    ...
    `}_renderDaysProgress(e){let t=this._lang;if(e.days_until_due==null||!e.interval_days||e.interval_days<=0)return d;let{pct:a,overflow:s}=at(e.interval_days,e.days_until_due,e.interval_unit),o="var(--success-color, #4caf50)";return e.status==="overdue"?o="var(--error-color, #f44336)":e.status==="due_soon"&&(o="var(--warning-color, #ff9800)"),l`
      <div class="days-progress">
        <div class="days-progress-labels">
          <span>${e.last_performed?`${i("last_performed",t)}: ${X(e.last_performed,t)}`:""}</span>
          <span>${e.next_due?`${i("next_due",t)}: ${X(e.next_due,t)}`:""}</span>
        </div>
        <div class="days-progress-bar" role="progressbar" aria-valuenow="${Math.round(a)}" aria-valuemin="0" aria-valuemax="100" aria-label="${i("days_progress",t)}">
          <div class="days-progress-fill${s?" overflow":""}" style="width:${a}%;background:${o}"></div>
        </div>
        <div class="days-progress-text">${Me(e.days_until_due,t)}</div>
      </div>
    `}_renderObjectDetail(){if(!this._selectedEntryId)return d;let e=this._getObject(this._selectedEntryId);if(!e)return l`<p>Object not found.</p>`;let t=e.object,a=this._lang,s=this._isOperator;return l`
      <div class="detail-section">
        ...
        ${t.serial_number?l`<p class="meta">${i("serial_number_label",a)}: ${t.serial_number}</p>`:d}
        ${t.documentation_url&&/^https?:\/\//i.test(t.documentation_url)?l`<p class="meta">${i("documentation_url_label",a)}:
              <a href=${t.documentation_url} target="_blank" rel="noopener noreferrer">${t.documentation_url}</a>
            </p>`:d}
        ${t.installation_date?l`<p class="meta">${i("installed",a)}: ${X(t.installation_date,a)}</p>`:d}
        ${t.notes?l`<div class="object-notes">
        ...
        ${e.tasks.length===0?l`<div class="empty-state-centered">
              ...
              <div class="task-row${o.enabled?"":" task-disabled"}">
                ...
                <span class="due-cell" @click=${()=>this._showTask(e.entry_id,o.id)}>
                  <span class="due-text">${Me(o.days_until_due,a)}</span>
    ...
    `}_renderTaskHeader(e){let t=this._lang,s=this._getObject(this._selectedEntryId)?.object.name||"",o=this._isOperator,c=e.is_done?"done":e.status==="due_soon"?"warning":e.status||"ok",_=e.is_done?i("completed",t):i(e.status||"ok",t);return l`
      <div class="task-header">
        <div class="task-header-title">
          ...
          ${e.nfc_tag_id?l`<span class="nfc-badge" title="${i("nfc_tag_id",t)}: ${e.nfc_tag_id}"><ha-icon icon="mdi:nfc-variant"></ha-icon> NFC</span>`:o?d:l`<span class="nfc-badge unlinked" title="${i("nfc_link_hint",t)}"
      ...
      <div class="tab-content overview-tab">
        ...
        <div class="two-column-layout ${o?"":"single-column"}">
          ...
          <div class="right-column">
            ${Qt(e,t,this._costDurationToggle,u=>{this._costDurationToggle=u})}
          </div>
        </div>
        ${c?Ut(e,t):d}
        ${_?l`
    ...
    `}_renderChecklistCard(e){if(!this._features.checklists)return d;let t=e.checklist||[];if(t.length===0)return d;let a=this._lang;return l`
    ...
    `}_renderTaskMeta(e){let t=e.documentation_url&&/^https?:\/\//i.test(e.documentation_url)?e.documentation_url:null,a=this._selectedEntryId?this._getObject(this._selectedEntryId):void 0,s=a?.object?.documentation_url,o=s&&/^https?:\/\//i.test(s)?s:null;if(!e.notes&&!t&&!o)return d;let c=this._lang;return l`
    ...
    `}_renderKPIBar(e){let t=this._lang,a=e.times_performed>0?e.total_cost/e.times_performed:0,s=e.days_until_due!==null&&e.days_until_due!==void 0?e.days_until_due<0?"overdue":e.days_until_due<=e.warning_days?"warning":"":"";return l`
      <div class="kpi-bar">
        <div class="kpi-card">
          ...
          <div class="kpi-value">${e.next_due?X(e.next_due,t):"\u2014"}</div>
          ${this._features.schedule_time&&e.schedule_time?l`<div class="kpi-subtext">${i("at_time",t)} ${e.schedule_time}</div>`:d}
        </div>
        ...
        <div class="kpi-card">
          ...
          <div class="kpi-value">${At(e,t)}</div>
        ...
        <div class="kpi-card">
          ...
          <div class="kpi-value">${e.last_performed?X(e.last_performed,t):"\u2014"}</div>
        </div>
        <div class="kpi-card">
    ...
    `}_renderRecommendationCard(e){let t=this._lang;if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return d;if(this._selectedEntryId&&this._selectedTaskId&&this._dismissedSuggestions.has(`${this._selectedEntryId}_${this._selectedTaskId}`))return d;let a=e.suggested_interval;return l`
      <div class="recommendation-card">
        <h4>${i("suggested_interval",t)}</h4>
        ${Bt(e.interval_days,a,e.interval_confidence||"medium",t)}
    ...
    `}_renderRecentActivities(e){let t=this._lang,a=e.history.slice(-3).reverse();if(a.length===0)return d;let s=o=>{switch(o){case"completed":return"\u2713";case"triggered":return"\u2297";case"skipped":return"\u21B7";case"reset":return"\u21BA";default:return"\xB7"}};return l`
      <div class="recent-activities">
        <h3>${i("recent_activities",t)}</h3>
        ${a.map(o=>l`
          <div class="activity-item">
            <span class="activity-icon">${s(o.type)}</span>
            <span class="activity-date">${et(o.timestamp,t)}</span>
    ...
    `}_renderHistoryFilters(e){let t=this._lang;return l`
      <div class="history-filters-new">
        <div class="filter-chips">
          ${["completed","skipped","reset","triggered"].map(a=>{let s=e.history.filter(o=>o.type===a).length;return s===0?d:l`
              <span class="filter-chip ${this._historyFilter===a?"active":""}"
                @click=${()=>{this._historyFilter=this._historyFilter===a?null:a}}>
                ${i(a,t)} (${s})
              </span>
            `})}
          ${this._historyFilter?l`<span class="filter-chip clear" @click=${()=>{this._historyFilter=null}}>${i("show_all",t)}</span>`:d}
        </div>
    ...
    `}_renderHistoryList(e){let t=this._lang,a=this._historyFilter?e.history.filter(s=>s.type===this._historyFilter):e.history;if(this._historySearch){let s=this._historySearch.toLowerCase();a=a.filter(o=>o.notes?.toLowerCase().includes(s))}return a.length===0?l`<p class="empty">${i("no_history",t)}</p>`:l`
    ...
    `}_renderHistoryEntry(e){let t=this._lang,a=["completed","reset","skipped"].includes(e.type);return l`
      <div class="history-entry">
        <div class="history-icon ${e.type}">
          <ha-icon .icon=${Et[e.type]||"mdi:circle"}></ha-icon>
        </div>
        <div class="history-content">
          <div class="history-row">
            <strong>${i(e.type,t)}</strong>
            ${a?l`<button class="history-edit-btn"
                       title=${i("history_edit_button",t)||"Edit entry"}
                       @click=${()=>this._openHistoryEdit(e)}>
                  <ha-icon icon="mdi:pencil"></ha-icon>
                </button>`:d}
          </div>
          <div class="history-date">${et(e.timestamp,t)}</div>
          ${e.notes?l`<div>${e.notes}</div>`:d}
          <div class="history-details">
            ${e.cost!=null?l`<span>${i("cost",t)}: ${e.cost.toFixed(2)} ${this._budget?.currency_symbol||"\u20AC"}</span>`:d}
            ${e.duration!=null?l`<span>${i("duration",t)}: ${e.duration} min</span>`:d}
            ${e.trigger_value!=null?l`<span>${i("trigger_val",t)}: ${e.trigger_value}</span>`:d}
          </div>
        </div>
      </div>
    `}_openHistoryEdit(e){if(!this._selectedEntryId||!this._selectedTaskId)return;let t={entry_id:this._selectedEntryId,task_id:this._selectedTaskId,original_timestamp:e.timestamp,type:e.type,timestamp:e.timestamp,notes:e.notes??null,cost:e.cost??null,duration:e.duration??null,completed_by:e.completed_by??null};this.shadowRoot?.querySelector("maintenance-history-edit-dialog")?.openEdit(t)}};j.styles=[Ne,St],p([x({attribute:!1})],j.prototype,"hass",2),p([x({type:Boolean,reflect:!0})],j.prototype,"narrow",2),p([x({attribute:!1})],j.prototype,"panel",2),p([g()],j.prototype,"_objects",2),p([g()],j.prototype,"_stats",2),p([g()],j.prototype,"_view",2),p([g()],j.prototype,"_selectedEntryId",2),p([g()],j.prototype,"_selectedTaskId",2),p([g()],j.prototype,"_filterStatus",2),p([g()],j.prototype,"_filterUser",2),p([g()],j.prototype,"_unsub",2),p([g()],j.prototype,"_sparklineTooltip",2),p([g()],j.prototype,"_historyFilter",2),p([g()],j.prototype,"_budget",2),p([g()],j.prototype,"_groups",2),p([g()],j.prototype,"_detailStatsData",2),p([g()],j.prototype,"_miniStatsData",2),p([g()],j.prototype,"_features",2),p([g()],j.prototype,"_adminPanelUserIds",2),p([g()],j.prototype,"_defaultWarningDays",2),p([g()],j.prototype,"_actionLoading",2),p([g()],j.prototype,"_moreMenuOpen",2),p([g()],j.prototype,"_toastMessage",2),p([g()],j.prototype,"_overviewTab",2),p([g()],j.prototype,"_activeTab",2),p([g()],j.prototype,"_costDurationToggle",2),p([g()],j.prototype,"_historySearch",2),p([g()],j.prototype,"_sortMode",2),p([g()],j.prototype,"_objectSortMode",2),p([g()],j.prototype,"_groupByMode",2),j=p([xt("maintenance-supporter-panel")],j);export{j as MaintenanceSupporterPanel};
