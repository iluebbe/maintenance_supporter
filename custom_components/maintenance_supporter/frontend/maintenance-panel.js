      ...
      <ha-dialog open @closed=${this._close} .heading=${t}>
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${Vt.map(i=>o`<option value=${i}>${n(i,e)}</option>`)}
            </select>
          </div>
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `,_([P({attribute:!1})],W.prototype,"hass",2),_([m()],W.prototype,"_open",2),_([m()],W.prototype,"_title",2),_([m()],W.prototype,"_message",2),_([m()],W.prototype,"_confirmText",2),_([m()],W.prototype,"_danger",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",W);var Wt=60,Kt=20,Gt=300,Zt=140,Yt=300,Qt=200,ct=30,Jt=27,A=class extends z{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._sparklineTooltip=null;this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1};this._actionLoading=!1;this._moreMenuOpen=!1;this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._deepLinkHandled=!1;this._onDialogEvent=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"de"}connectedCallback(){super.connectedCallback()}disconnectedCallback(){super.disconnectedCallback(),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new $e(this.hass),this._fetchMiniStatsForOverview()),this._userService||(this._userService=new ne(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,i,r,s]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);e&&(this._objects=e.objects),t&&(this._stats=t),i&&(this._budget=i),r&&(this._groups=r.groups||{}),s&&(this._features=s.features),this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("entry_id");if(!t)return;this._deepLinkHandled=!0;let i=e.get("task_id"),r=e.get("action");i?(this._showTask(t,i),r==="complete"&&(history.replaceState(null,"",window.location.pathname+window.location.hash),requestAnimationFrame(()=>{let d=this._getObject(t)?.tasks.find(u=>u.id===i);d&&this._openCompleteDialog(t,i,d.name,this._features.checklists?d.checklist:void 0,this._features.adaptive&&!!d.adaptive_config?.enabled)}))):this._showObject(t)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let i=await this._statsService.getDetailStats(e,t),r=new Map(this._detailStatsData);r.set(e,i),this._detailStatsData=r}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let i of this._objects)for(let r of i.tasks){let s=r.trigger_config?.entity_id;s&&e.push({entityId:s,isCounter:this._isCounterEntity(r.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let i of this._objects)for(let r of i.tasks)if(!(this._filterStatus&&r.status!==this._filterStatus)){if(this._filterUser){let s=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(r.responsible_user_id!==s)continue}e.push({entry_id:i.entry_id,task_id:r.id,object_name:i.object.name,task_name:r.name,type:r.type,schedule_type:r.schedule_type,status:r.status,days_until_due:r.days_until_due??null,next_due:r.next_due??null,trigger_active:r.trigger_active,trigger_current_value:r.trigger_current_value??null,trigger_current_delta:r.trigger_current_delta??null,trigger_config:r.trigger_config??null,trigger_entity_info:r.trigger_entity_info??null,times_performed:r.times_performed,total_cost:r.total_cost,interval_days:r.interval_days??null,history:r.history||[],enabled:r.enabled})}let t={overdue:0,triggered:1,due_soon:2,ok:3};return e.sort((i,r)=>(t[i.status]??9)-(t[r.status]??9)),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(r=>r.id===t)}_showOverview(){this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1}_showObject(e){this._view="object",this._selectedEntryId=e,this._selectedTaskId=null}_showTask(e,t){this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._historyFilter=null;let i=this._getTask(e,t);if(i?.trigger_config?.entity_id){let r=i.trigger_config.entity_id,s=this._isCounterEntity(i.trigger_config);this._fetchDetailStats(r,s)}}async _deleteObject(e){await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:n("delete",this._lang),message:n("confirm_delete_object",this._lang),confirmText:n("delete",this._lang),danger:!0})&&(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData())}async _deleteTask(e,t){await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:n("delete",this._lang),message:n("confirm_delete_task",this._lang),confirmText:n("delete",this._lang),danger:!0})&&(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData())}async _skipTask(e,t){this._actionLoading=!0;try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/skip",entry_id:e,task_id:t}),await this._loadData()}finally{this._actionLoading=!1}}async _resetTask(e,t){this._actionLoading=!0;try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/reset",entry_id:e,task_id:t}),await this._loadData()}finally{this._actionLoading=!1}}async _applySuggestion(e,t,i){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:i}),await this._loadData()}_dismissSuggestion(){this._loadData()}_openCompleteDialog(e,t,i,r,s){let d=this.shadowRoot.querySelector("maintenance-complete-dialog");d&&(d.entryId=e,d.taskId=t,d.taskName=i,d.lang=this._lang,d.checklist=r||[],d.adaptiveEnabled=!!s,d.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,i,r){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,i,r)}async _exportCsv(){let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/csv/export"}),t=new Blob([e.csv],{type:"text/csv;charset=utf-8;"}),i=URL.createObjectURL(t),r=document.createElement("a");r.href=i,r.download="maintenance_export.csv",r.click(),URL.revokeObjectURL(i)}_triggerImportCsv(){let e=this.shadowRoot.querySelector("input[type=file]");e&&(e.value="",e.click())}async _handleCsvFile(e){let i=e.target.files?.[0];if(!i)return;let r=await i.text(),s=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/csv/import",csv_content:r});await this._loadData(),await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:"Import",message:n("import_result",this._lang).replace("{created}",String(s.created)).replace("{total}",String(s.total)),confirmText:"OK"})}render(){return o`
    ...
    `}_renderOverviewRow(e){let t=this._lang,i=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,r=0,s=ye.ok,d=!1;if(i&&e.days_until_due!==null){let u=(e.interval_days-e.days_until_due)/e.interval_days*100;r=Math.max(0,Math.min(100,u)),d=u>100,e.status==="overdue"?s=ye.overdue:e.status==="due_soon"&&(s=ye.due_soon)}return o`
    ...
    `}_openCompleteDialogForRow(e){let i=this._objects.find(r=>r.entry_id===e.entry_id)?.tasks.find(r=>r.id===e.task_id);this._openCompleteDialog(e.entry_id,e.task_id,e.task_name,this._features.checklists?i?.checklist:void 0,this._features.adaptive&&!!i?.adaptive_config?.enabled)}_renderTriggerProgress(e){let t=e.trigger_config??null;if(!t)return l;let i=t.type||"threshold",r=e.trigger_entity_info?.unit_of_measurement??"",s=0,d="";if(i==="threshold"){let p=e.trigger_current_value??null;if(p==null)return l;let h=t.trigger_above,f=t.trigger_below;if(h!=null){let $=f??0,b=h-$||1;s=Math.min(100,Math.max(0,(p-$)/b*100)),d=`${p.toFixed(1)} / ${h} ${r}`}else if(f!=null){let b=e.trigger_entity_info?.max??(p*2||100),C=b-f||1;s=Math.min(100,Math.max(0,(b-p)/C*100)),d=`${p.toFixed(1)} / ${f} ${r}`}else return l}else if(i==="counter"){let p=t.trigger_target_value||1,f=e.trigger_current_delta??null??e.trigger_current_value??null;if(f==null)return l;s=Math.min(100,Math.max(0,f/p*100)),d=`${f.toFixed(1)} / ${p} ${r}`}else if(i==="state_change"){let p=t.trigger_target_changes||1,h=e.trigger_current_value??null;if(h==null)return l;s=Math.min(100,Math.max(0,h/p*100)),d=`${Math.round(h)} / ${p}`}else if(i==="runtime"){let p=t.trigger_runtime_hours||100,h=e.trigger_current_value??null;if(h==null)return l;s=Math.min(100,Math.max(0,h/p*100)),d=`${h.toFixed(1)}h / ${p}h`}else if(i==="compound"){let p=t.compound_logic||t.operator||"AND",h=t.conditions?.length||0;d=`${p} (${h})`,s=e.trigger_active?100:0}else return l;let u=s>=100,v=s>90?"var(--error-color, #f44336)":s>70?"var(--warning-color, #ff9800)":"var(--primary-color)";return o`
      <div class="trigger-progress">
        <div class="trigger-progress-bar">
          <div class="trigger-progress-fill${u?" overflow":""}" style="width:${s}%;background:${v}"></div>
        </div>
        <span class="trigger-progress-label">${d}</span>
      </div>
    `}_renderMiniSparkline(e){if(!e.trigger_config?.entity_id)return l;let t=e.trigger_config.entity_id,i=this._miniStatsData.get(t)||[],r=[];if(i.length>=2)r=i.map(y=>({ts:y.ts,val:y.val}));else{if(!e.history)return l;for(let y of e.history)y.trigger_value!=null&&r.push({ts:new Date(y.timestamp).getTime(),val:y.trigger_value})}if(e.trigger_current_value!=null&&r.push({ts:Date.now(),val:e.trigger_current_value}),r.length<2)return l;r.sort((y,T)=>y.ts-T.ts);let s=Wt,d=Kt,u=r.map(y=>y.val),v=Math.min(...u),p=Math.max(...u),h=p-v||1;v-=h*.1,p+=h*.1;let f=r[0].ts,b=r[r.length-1].ts-f||1,C=y=>(y-f)/b*s,L=y=>2+(1-(y-v)/(p-v))*(d-4),E=r;if(E.length>ct){let y=Math.ceil(E.length/ct);E=E.filter((T,M)=>M%y===0||M===E.length-1)}let H=E.map(y=>`${C(y.ts).toFixed(1)},${L(y.val).toFixed(1)}`).join(" ");return o`
    ...
    `}_renderDaysProgress(e){let t=this._lang;if(e.days_until_due==null||!e.interval_days||e.interval_days<=0)return l;let r=(e.interval_days-e.days_until_due)/e.interval_days*100,s=Math.max(0,Math.min(100,r)),d=r>100,u="var(--success-color, #4caf50)";return e.status==="overdue"?u="var(--error-color, #f44336)":e.status==="due_soon"&&(u="var(--warning-color, #ff9800)"),o`
      <div class="days-progress">
        ...
        <div class="days-progress-bar" role="progressbar" aria-valuenow="${Math.round(s)}" aria-valuemin="0" aria-valuemax="100" aria-label="${n("days_progress",t)}">
          <div class="days-progress-fill${d?" overflow":""}" style="width:${s}%;background:${u}"></div>
        </div>
    ...
    `}_renderObjectDetail(){if(!this._selectedEntryId)return l;let e=this._getObject(this._selectedEntryId);if(!e)return o`<p>Object not found.</p>`;let t=e.object,i=this._lang;return o`
      <div class="detail-section">
        ...
        ${t.installation_date?o`<p class="meta">${n("installed",i)}: ${te(t.installation_date,i)}</p>`:l}
        ...
        ${e.tasks.length===0?o`<p class="empty">${n("no_tasks_short",i)}</p>`:e.tasks.map(r=>o`
              <div class="task-row${r.enabled?"":" task-disabled"}">
                ...
                <span class="row-actions">
                  ...
                  <mwc-icon-button class="btn-skip" title="${n("skip",i)}" .disabled=${this._actionLoading} @click=${s=>{s.stopPropagation(),this._skipTask(e.entry_id,r.id)}}>
                    <ha-icon icon="mdi:skip-next"></ha-icon>
                  </mwc-icon-button>
                </span>
              </div>
    ...
    `}_renderAnalysisTab(e){let t=this._lang;if(!(this._features.adaptive||this._features.seasonal))return o`
      ...
      `;let r=this._features.adaptive&&e.interval_analysis?.weibull_beta!=null&&e.interval_analysis?.weibull_eta!=null,s=this._features.seasonal&&(e.seasonal_factors?.length===12||e.interval_analysis?.seasonal_factors?.length===12);if(!(r||s)){let u=e.schedule_type==="manual",v=e.interval_analysis?.data_points??0,p=Math.min(100,Math.max(0,v/5*100));return o`
        <div class="tab-content analysis-tab">
          ${this._renderTriggerSection(e)}
          ${this._renderPredictionSection(e)}
          <div class="analysis-empty-state">
            ...
            <p class="empty-hint">
              ${u?n("analysis_manual_task_hint",t):n("analysis_not_enough_data_hint",t)}
            </p>
            ${!u&&v>0?o`
    ...
    `}_renderHistoryChart(e){let t=e.history.filter(x=>x.type==="completed"&&(x.cost!=null||x.duration!=null)).map(x=>({ts:new Date(x.timestamp).getTime(),cost:x.cost??0,duration:x.duration??0})).sort((x,D)=>x.ts-D.ts);if(t.length<2)return l;let i=t.some(x=>x.cost>0),r=t.some(x=>x.duration>0);if(!i&&!r)return l;let s=this._costDurationToggle!=="duration"&&i,d=this._costDurationToggle!=="cost"&&r,u=s||!d&&i,v=d||!s&&r,p=Yt,h=Qt,f=u?32:8,$=v?32:8,b=8,C=20,L=p-f-$,E=h-b-C,H=Math.max(...t.map(x=>x.cost))||1,y=Math.max(...t.map(x=>x.duration))||1,T=Math.min(20,L/t.length*.6),M=L/t.length,R=x=>f+M*x+M/2,V=x=>b+E-x/H*E,B=x=>b+E-x/y*E;return o`
      <div class="sparkline-container">
        <svg class="history-chart" viewBox="0 0 ${p} ${h}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_history",this._lang)}">
          <!-- Cost bars -->
          ${u?t.map((x,D)=>I`
            <rect x="${(R(D)-T/2).toFixed(1)}" y="${V(x.cost).toFixed(1)}" width="${T.toFixed(1)}" height="${(b+E-V(x.cost)).toFixed(1)}"
          ...
          ${u?I`
            ...
            <text x="${f-3}" y="${b+E+3}" text-anchor="end" fill="var(--primary-color)" font-size="7">0\u20AC</text>
          `:l}
          ${v?I`
            <text x="${p-$+3}" y="${b+4}" text-anchor="start" fill="var(--accent-color, #ff9800)" font-size="7">${y.toFixed(0)}m</text>
            <text x="${p-$+3}" y="${b+E+3}" text-anchor="start" fill="var(--accent-color, #ff9800)" font-size="7">0m</text>
          `:l}
        </svg>
      </div>
      <div class="chart-legend">
        ${u?o`<span class="legend-item"><span class="legend-swatch" style="background:var(--primary-color);opacity:0.6"></span>${n("cost",this._lang)}</span>`:l}
    ...
    `}_renderSeasonalChart(e){let t=e.seasonal_factors??e.interval_analysis?.seasonal_factors;if(!t||t.length!==12)return l;let i=this._lang,r=e.interval_analysis?.seasonal_reason,s=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"],d=new Date().getMonth(),u=300,v=100,p=8,f=v-p-4,$=Math.max(...t,1.5),b=u/12,C=b*.65,L=p+f-1/$*f;return o`
      <div class="seasonal-chart">
        ...
        <svg viewBox="0 0 ${u} ${v}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_seasonal",i)}">
          <!-- Baseline at 1.0 -->
          <line x1="0" y1="${L.toFixed(1)}" x2="${u}" y2="${L.toFixed(1)}"
            stroke="var(--divider-color)" stroke-width="1" stroke-dasharray="4,3" />
          <!-- Bars -->
          ${t.map((E,H)=>{let y=E/$*f,T=H*b+(b-C)/2,M=p+f-y,R=H===d,V=E<1?"var(--success-color, #4caf50)":E>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return I`
              <rect x="${T.toFixed(1)}" y="${M.toFixed(1)}"
                width="${C.toFixed(1)}" height="${y.toFixed(1)}"
                fill="${V}" opacity="${R?1:.5}" rx="2" />
            `})}
        </svg>
        <div class="seasonal-labels">
          ${s.map((E,H)=>o`<span class="seasonal-label ${H===d?"active-month":""}">${n(E,i)}</span>`)}
        </div>
      </div>
    `}_renderWeibullSection(e){let t=e.interval_analysis,i=t?.weibull_beta,r=t?.weibull_eta;if(i==null||r==null)return l;let s=this._lang,d=t?.weibull_r_squared,u=e.interval_days??0,v=e.suggested_interval??u;return o`
    ...
    `}_renderWeibullChart(e,t,i,r){let b=Math.max(i,r,t,1)*1.3,C=50,L=[];for(let x=0;x<=C;x++){let D=x/C*b,N=1-Math.exp(-Math.pow(D/t,e)),S=32+D/b*260,we=136-N*128;L.push([S,we])}let E=L.map(([x,D])=>`${x.toFixed(1)},${D.toFixed(1)}`).join(" "),H="M32,136 "+L.map(([x,D])=>`L${x.toFixed(1)},${D.toFixed(1)}`).join(" ")+` L${L[C][0].toFixed(1)},136 Z`,y=32+i/b*260,T=1-Math.exp(-Math.pow(i/t,e)),M=136-T*128,R=((1-T)*100).toFixed(0),V=32+r/b*260,B=[0,.25,.5,.75,1];return o`
      <div class="weibull-chart">
        <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_weibull",this._lang)}">
          <!-- Grid lines -->
          ${B.map(x=>{let D=136-x*128;return I`
          ...
          ${i>0?I`
            ...
            <circle cx="${y.toFixed(1)}" cy="${M.toFixed(1)}" r="3"
              fill="var(--primary-color, #03a9f4)" />
            <text x="${(y+4).toFixed(1)}" y="${(M-6).toFixed(1)}" fill="var(--primary-color, #03a9f4)"
    ...
    `}_renderConfidenceInterval(e,t,i){let r=e.confidence_interval_low,s=e.confidence_interval_high,d=t.suggested_interval??t.interval_days??0,u=t.interval_days??0,v=Math.max(0,r-5),h=s+5-v,f=(r-v)/h*100,$=(s-r)/h*100,b=(d-v)/h*100,C=u>0?(u-v)/h*100:-1;return o`
        ...
        <div class="prediction-grid">
          ...
          ${i?o`
            <div class="prediction-item">
              ...
              ${e.threshold_prediction_date?o`<span class="prediction-date">${te(e.threshold_prediction_date,s)}</span>`:l}
    ...
    `}_renderTriggerSection(e){let t=e.trigger_config;if(!t)return l;let i=this._lang,r=e.trigger_entity_info,s=e.trigger_entity_infos,d=r?.friendly_name||t.entity_id||"\u2014",u=t.entity_id||"",v=t.entity_ids||(u?[u]:[]),p=r?.unit_of_measurement||"",h=e.trigger_current_value,f=t.type||"threshold",$=v.length>1;return o`
      <h3>${n("trigger",i)}</h3>
      <div class="trigger-card">
        ...
        <div class="trigger-limits">
          ${f==="threshold"?o`
            ${t.trigger_above!=null?o`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${n("threshold_above",i)}: ${t.trigger_above} ${p}</span>`:l}
            ${t.trigger_below!=null?o`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${n("threshold_below",i)}: ${t.trigger_below} ${p}</span>`:l}
          ...
          `:l}
          ${f==="counter"?o`
            ${t.trigger_target_value!=null?o`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${n("target_value",i)}: ${t.trigger_target_value} ${p}</span>`:l}
          `:l}
          ${f==="state_change"?o`
          ...
          ${f==="compound"?o`
            ...
            ${(t.conditions||[]).map((b,C)=>o`
              <span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${C+1}. ${n(b.trigger_config?.type||"unknown",i)}: ${b.trigger_config?.entity_id||""}</span>
            `)}
          `:l}
          ${r?.min!=null?o`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${n("min",i)}: ${r.min} ${p}</span>`:l}
          ${r?.max!=null?o`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${n("max",i)}: ${r.max} ${p}</span>`:l}
        </div>
    ...
    `}_renderSparkline(e,t){let i=e.trigger_config;if(!i)return l;let r=i.type||"threshold",s=r==="counter"&&i.trigger_delta_mode,d=this._isCounterEntity(i),u=i.entity_id||"",v=this._detailStatsData.get(u)||[],p=[],h=!1;if(v.length>=2)for(let g of v){let k=g.val;s&&e.trigger_baseline_value!=null&&(k-=e.trigger_baseline_value);let j={ts:g.ts,val:k};!d&&g.min!=null&&g.max!=null&&(j.min=s&&e.trigger_baseline_value!=null?g.min-e.trigger_baseline_value:g.min,j.max=s&&e.trigger_baseline_value!=null?g.max-e.trigger_baseline_value:g.max,h=!0),p.push(j)}else for(let g of e.history)g.trigger_value!=null&&p.push({ts:new Date(g.timestamp).getTime(),val:g.trigger_value});if(e.trigger_current_value!=null){let g=e.trigger_current_value;s&&e.trigger_baseline_value!=null&&(g-=e.trigger_baseline_value),p.push({ts:Date.now(),val:g})}if(p.length<2&&u&&this._statsService&&!this._detailStatsData.has(u))return o`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
        <ha-icon icon="mdi:chart-line" style="--mdc-icon-size:16px;margin-right:8px;"></ha-icon>
        ${n("loading_chart",this._lang)}
      </div>`;if(p.length<2)return l;p.sort((g,k)=>g.ts-k.ts);let f=Gt,$=Zt,b=30,C=2,L=8,E=16,H=p.map(g=>g.val),y=Math.min(...H),T=Math.max(...H);if(h)for(let g of p)g.min!=null&&(y=Math.min(y,g.min)),g.max!=null&&(T=Math.max(T,g.max));i.trigger_above!=null&&(T=Math.max(T,i.trigger_above),y=Math.min(y,i.trigger_above)),i.trigger_below!=null&&(y=Math.min(y,i.trigger_below),T=Math.max(T,i.trigger_below));let M=null,R=null;if(r==="counter"&&i.trigger_target_value!=null){if(e.trigger_baseline_value!=null)M=e.trigger_baseline_value;else if(p.length>0){let g=[...e.history].filter(k=>k.type==="completed"||k.type==="reset").sort((k,j)=>new Date(j.timestamp).getTime()-new Date(k.timestamp).getTime())[0];if(g){let k=new Date(g.timestamp).getTime(),j=p[0],F=Math.abs(p[0].ts-k);for(let G of p){let Z=Math.abs(G.ts-k);Z<F&&(j=G,F=Z)}M=j.val}else M=p[0].val}M!=null?(R=M+i.trigger_target_value,T=Math.max(T,R),y=Math.min(y,M)):(T=Math.max(T,i.trigger_target_value),y=Math.min(y,0))}s&&e.trigger_baseline_value!=null&&(y=Math.min(y,0));let V=T-y||1;y-=V*.1,T+=V*.1;let B=p[0].ts,x=p[p.length-1].ts,D=x-B||1,N=g=>b+(g-B)/D*(f-b-C),S=g=>L+(1-(g-y)/(T-y))*($-L-E),we=p.map(g=>`${N(g.ts).toFixed(1)},${S(g.val).toFixed(1)}`).join(" "),dt=`M${N(p[0].ts).toFixed(1)},${$-E} `+p.map(g=>`L${N(g.ts).toFixed(1)},${S(g.val).toFixed(1)}`).join(" ")+` L${N(p[p.length-1].ts).toFixed(1)},${$-E} Z`,ke="";if(h){let g=p.filter(k=>k.min!=null&&k.max!=null);if(g.length>=2){let k=g.map(F=>`${N(F.ts).toFixed(1)},${S(F.max).toFixed(1)}`),j=[...g].reverse().map(F=>`${N(F.ts).toFixed(1)},${S(F.min).toFixed(1)}`);ke=`M${k[0]} `+k.slice(1).map(F=>`L${F}`).join(" ")+` L${j[0]} `+j.slice(1).map(F=>`L${F}`).join(" ")+" Z"}}let Ne=p[p.length-1],ut=N(Ne.ts),pt=S(Ne.val),Oe=g=>Math.abs(g)>=1e4?(g/1e3).toFixed(0)+"k":g>=1e3?(g/1e3).toFixed(1)+"k":g.toFixed(g<10?1:0),gt=Oe(T),_t=Oe(y),ht=e.history.filter(g=>["completed","skipped","reset"].includes(g.type)).map(g=>({ts:new Date(g.timestamp).getTime(),type:g.type})).filter(g=>g.ts>=B&&g.ts<=x),Ee=Jt,Ae=p;if(p.length>Ee){let g=(p.length-1)/(Ee-1);Ae=[];for(let k=0;k<Ee;k++)Ae.push(p[Math.round(k*g)])}return o`
      <div class="sparkline-container">
        <svg class="sparkline-svg" viewBox="0 0 ${f} ${$}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_sparkline",this._lang)}">
          <!-- Y-axis labels -->
          <text x="${b-3}" y="${L+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${gt}</text>
          <text x="${b-3}" y="${$-E+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${_t}</text>
          <!-- X-axis labels -->
          ...
          <text x="${f-C}" y="${$-1}" text-anchor="end" fill="var(--secondary-text-color)" font-size="7">${new Date(x).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
          <!-- Min/max band (mean-type entities) -->
          ${ke?I`<path d="${ke}" fill="var(--primary-color)" opacity="0.08" />`:l}
          <!-- Area fill -->
          ...
          ${e.degradation_rate!=null&&e.degradation_trend!=="stable"&&e.degradation_trend!=="insufficient_data"&&p.length>=2?(()=>{let g=p[p.length-1],k=30,j=g.ts+k*864e5,F=g.val+e.degradation_rate*k,G=Math.min(j,x+(x-B)*.3),Z=Math.max(y,Math.min(T,F)),mt=N(g.ts),vt=S(g.val),ft=N(G),bt=S(Z);return I`<line x1="${mt.toFixed(1)}" y1="${vt.toFixed(1)}" x2="${ft.toFixed(1)}" y2="${bt.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():l}
          <!-- Threshold lines (for threshold type) -->
          ${r==="threshold"&&i.trigger_above!=null?I`<line x1="${b}" y1="${S(i.trigger_above).toFixed(1)}" x2="${f}" y2="${S(i.trigger_above).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                  <text x="${f-2}" y="${S(i.trigger_above)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">▲ ${i.trigger_above}</text>`:l}
          ${r==="threshold"&&i.trigger_below!=null?I`<line x1="${b}" y1="${S(i.trigger_below).toFixed(1)}" x2="${f}" y2="${S(i.trigger_below).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
          ...
          ${r==="counter"&&R!=null?I`<line x1="${b}" y1="${S(R).toFixed(1)}" x2="${f}" y2="${S(R).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                  <text x="${f-2}" y="${S(R)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">${n("target_value",this._lang)}: +${i.trigger_target_value}</text>`:l}
          <!-- Baseline line (for counter — value at last maintenance) -->
          ${r==="counter"&&M!=null?I`<line x1="${b}" y1="${S(M).toFixed(1)}" x2="${f}" y2="${S(M).toFixed(1)}" stroke="var(--secondary-text-color)" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
                  <text x="${b+2}" y="${S(M)+11}" text-anchor="start" fill="var(--secondary-text-color)" font-size="8">${n("baseline",this._lang)}</text>`:l}
          <!-- Current dot -->
          <circle cx="${ut.toFixed(1)}" cy="${pt.toFixed(1)}" r="3.5" fill="var(--primary-color)" />
          <!-- Event markers -->
          ${ht.map(g=>{let k=N(g.ts),j=g.type==="completed"?"var(--success-color, #4caf50)":g.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return I`
          ...
          ${Ae.map(g=>{let k=N(g.ts),j=S(g.val),F=new Date(g.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),G=`${g.val.toFixed(1)} ${t}`;return h&&g.min!=null&&g.max!=null&&(G+=` (${g.min.toFixed(1)}\u2013${g.max.toFixed(1)})`),I`<circle cx="${k.toFixed(1)}" cy="${j.toFixed(1)}" r="8" fill="transparent" tabindex="0"
              @mouseenter=${Z=>this._showSparklineTooltip(Z,`${F}
${G}`)}
              @focus=${Z=>this._showSparklineTooltip(Z,`${F}
${G}`)}
...
`).map(g=>o`<div>${g}</div>`)}
    ...
    `],_([P({attribute:!1})],A.prototype,"hass",2),_([P({type:Boolean})],A.prototype,"narrow",2),_([P({attribute:!1})],A.prototype,"panel",2),_([m()],A.prototype,"_objects",2),_([m()],A.prototype,"_stats",2),_([m()],A.prototype,"_view",2),_([m()],A.prototype,"_selectedEntryId",2),_([m()],A.prototype,"_selectedTaskId",2),_([m()],A.prototype,"_filterStatus",2),_([m()],A.prototype,"_filterUser",2),_([m()],A.prototype,"_unsub",2),_([m()],A.prototype,"_sparklineTooltip",2),_([m()],A.prototype,"_historyFilter",2),_([m()],A.prototype,"_budget",2),_([m()],A.prototype,"_groups",2),_([m()],A.prototype,"_detailStatsData",2),_([m()],A.prototype,"_miniStatsData",2),_([m()],A.prototype,"_features",2),_([m()],A.prototype,"_actionLoading",2),_([m()],A.prototype,"_moreMenuOpen",2),_([m()],A.prototype,"_activeTab",2),_([m()],A.prototype,"_costDurationToggle",2),_([m()],A.prototype,"_historySearch",2),A=_([re("maintenance-supporter-panel")],A);export{A as MaintenanceSupporterPanel};
