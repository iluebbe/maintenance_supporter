...
`).map(l=>l.trim()).filter(Boolean).slice(0,100);e.checklist=o.length?o:null}if(this.completionActionsEnabled){let o=this._actionService.trim();if(o&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(o)){let m={service:o},h=this._actionTargetEntity.trim();h&&(m.target={entity_id:h});let v=this._buildActionData();Object.keys(v).length>0&&(m.data=v),e.on_complete_action=m}else e.on_complete_action=null;let l={};this._qcNotes.trim()&&(l.notes=this._qcNotes.trim());let _=parseFloat(this._qcCost);!isNaN(_)&&_>=0&&(l.cost=_);let u=parseInt(this._qcDuration,10);!isNaN(u)&&u>=0&&(l.duration=u),this._qcFeedback&&(l.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(l).length?l:null}let t=await this.hass.connection.sendMessagePromise(e),a=this._taskId||t?.task_id,n=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(a&&this._scheduleType==="sensor_based"&&n)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:a,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=V(e,this._lang,i("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return d;let e=this._lang;return r`
      ...
      <div class="select-row">
        ...
        >
          ${Ca.map(t=>r`<option value=${t} ?selected=${t===this._triggerType}>${i(t,e)}</option>`)}
        </select>
      </div>
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${Ta.map(a=>r`<option value=${a} ?selected=${a===this._type}>${i(a,e)}</option>`)}
            </select>
          </div>
          <div class="select-row">
            ...
            >
              ${qa.map(a=>r`<option value=${a} ?selected=${a===this._scheduleType}>${i(a,e)}</option>`)}
            </select>
          </div>
...
<div class="qr-row">
  <div class="qr-col">
    <img src="${$t(this._viewResult.svg_data_uri)}" alt="QR Info" />
    <div class="qr-label">${m}</div>
  </div>
  ${u?`<div class="qr-col">
    <img src="${$t(this._completeResult.svg_data_uri)}" alt="QR Complete" />
    <div class="qr-label">${h}</div>
  </div>`:""}
</div>
<div class="url">${pe(this._viewResult.url)}</div>
<script>setTimeout(()=>window.print(),300)<\/script>
</body></html>`),n.document.close()}_downloadSvg(e,t){let a=decodeURIComponent(e.svg_data_uri.replace("data:image/svg+xml,","")),n=new Blob([a],{type:"image/svg+xml"}),o=URL.createObjectURL(n),l=document.createElement("a");l.href=o;let _=this._taskName?`${this._objectName}-${this._taskName}`:this._objectName;l.download=`qr-${Ma(_)}-${t}.svg`,l.click(),URL.revokeObjectURL(o)}_close(){this._open=!1,this._viewResult=null,this._completeResult=null,this._error="",this._loading=!1}render(){if(!this._open)return r``;let e=this.lang||this.hass?.language||"en",t=this._taskName?`${i("qr_code",e)}: ${this._objectName} \u2014 ${this._taskName}`:`${i("qr_code",e)}: ${this._objectName}`,a=!!this._viewResult;return r`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
  ...
  `,p([z({attribute:!1})],K.prototype,"hass",2),p([z()],K.prototype,"lang",2),p([g()],K.prototype,"_open",2),p([g()],K.prototype,"_loading",2),p([g()],K.prototype,"_error",2),p([g()],K.prototype,"_viewResult",2),p([g()],K.prototype,"_completeResult",2),p([g()],K.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",K);function xe(c){let s=c.getFullYear(),e=String(c.getMonth()+1).padStart(2,"0"),t=String(c.getDate()).padStart(2,"0");return`${s}-${e}-${t}`}function Na(c,s){let e=[];for(let t=0;t<s;t++){let a=new Date(c);a.setDate(a.getDate()+t),a.setHours(0,0,0,0),e.push(xe(a))}return e}function Ne(c,s){let[e,t,a]=c.split("-").map(Number),n=new Date(e,t-1,a);return n.setDate(n.getDate()+s),xe(n)}function Da(c){if(!c||c.length===0)return null;let s=c.map(e=>e.cost).filter(e=>typeof e=="number");return s.length===0?null:s.reduce((e,t)=>e+t,0)/s.length}function Ra(c){let{windowStart:s,windowEnd:e,task:t,entryId:a,objectName:n}=c,o=[],l=(m,h)=>({date:m,entry_id:a,task_id:t.id,task_name:t.name,object_name:n,status:h&&(t.status==="overdue"||t.status==="triggered")?"ok":t.status,days_until_due:h?null:t.days_until_due??null,projected:h,schedule_type:t.schedule_type,interval_days:t.interval_days??null,responsible_user_id:t.responsible_user_id??null,avg_cost:Da(t.history),adaptive_enabled:!!t.adaptive_config?.enabled,prediction_confidence:t.threshold_prediction_confidence??null});if(t.status==="overdue"||t.status==="triggered"){if(o.push(l(s,!1)),t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let m=Ne(s,t.interval_days),h=1;for(;m<=e&&h<5;)o.push(l(m,!0)),h++,m=Ne(m,t.interval_days)}return o}let _=t.next_due;if(typeof _!="string"||!_)return o;let u=_.slice(0,10);if(u>=s&&u<=e)o.push(l(u,!1));else if(u>e)return o;if(t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let m=Ne(u,t.interval_days),h=o.length;for(;m<=e&&h<5;)m>=s&&(o.push(l(m,!0)),h++),m=Ne(m,t.interval_days)}return o}var zt={overdue:0,triggered:1,due_soon:2,ok:3};function Et(c,s,e,t=null){let a=Na(s,e),n=a[0],o=a[a.length-1],l=[];for(let u of c){let m=u.object?.name||"",h=u.entry_id,v=u.tasks||[];for(let f of v){if(t&&f.responsible_user_id!==t||f.enabled===!1)continue;let k=Ra({windowStart:n,windowEnd:o,task:f,entryId:h,objectName:m});l.push(...k)}}let _=new Map;for(let u of a)_.set(u,[]);for(let u of l){let m=_.get(u.date);m&&m.push(u)}for(let[,u]of _)u.sort((m,h)=>{let v=zt[m.status]??99,f=zt[h.status]??99;if(v!==f)return v-f;if(m.projected!==h.projected)return m.projected?1:-1;let k=m.object_name.localeCompare(h.object_name);return k!==0?k:m.task_name.localeCompare(h.task_name)});return a.map(u=>({date:u,events:_.get(u)??[]}))}var Ia={completed:"ok",reset:"ok",skipped:"due_soon",triggered:"triggered",trigger_replaced:"triggered"};function Pa(c,s){let e=[];for(let t=s-1;t>=0;t--){let a=new Date(c);a.setDate(a.getDate()-t),a.setHours(0,0,0,0),e.push(xe(a))}return e}function jt(c,s,e,t=null){let a=Pa(s,e),n=a[0],o=a[a.length-1],l=new Map;for(let u of a)l.set(u,[]);for(let u of c){let m=u.object?.name||"",h=u.entry_id,v=u.tasks||[];for(let f of v){if(t&&f.responsible_user_id!==t)continue;let k=f.history||[];for(let b of k){if(typeof b?.timestamp!="string")continue;let N=b.timestamp.slice(0,10);if(N<n||N>o)continue;let q=l.get(N);if(!q)continue;let E=b.type??"completed";q.push({date:N,entry_id:h,task_id:f.id,task_name:f.name,object_name:m,status:Ia[E]??"ok",days_until_due:null,projected:!1,schedule_type:f.schedule_type,interval_days:f.interval_days??null,responsible_user_id:f.responsible_user_id??null,avg_cost:typeof b.cost=="number"?b.cost:null,adaptive_enabled:!!f.adaptive_config?.enabled,prediction_confidence:null,history_timestamp:b.timestamp,history_type:E,history_cost:typeof b.cost=="number"?b.cost:null,history_notes:typeof b.notes=="string"?b.notes:null,history_duration:typeof b.duration=="number"?b.duration:null})}}}let _={completed:0,reset:1,skipped:2,triggered:3,trigger_replaced:4};for(let[,u]of l)u.sort((m,h)=>{let v=_[m.history_type??""]??99,f=_[h.history_type??""]??99;if(v!==f)return v-f;let k=m.object_name.localeCompare(h.object_name);return k!==0?k:m.task_name.localeCompare(h.task_name)});return a.map(u=>({date:u,events:l.get(u)??[]}))}var At=C`
  .cal-controls {
    display: flex;
    gap: 12px;
    align-items: center;
...
`;var Z=class extends T{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter)}getCardSize(){return 6}get _lang(){return this.hass?.language||"en"}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}_onEventClick(e){if(e.history_timestamp){this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}));return}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}render(){if(!this.hass)return d;let e=this._lang,t=this._config.show_window_chips!==!1,a=this._config.show_user_filter!==!1,n=this._config.title,o=null;this._userFilter&&(o=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let l=new Date;l.setHours(0,0,0,0);let _=this._pastDays>0,u=_?jt(this._objects,l,this._pastDays,o):Et(this._objects,l,this._windowDays,o),m=xe(l),h=this._windowDays===365||_,v=h?u.filter(b=>b.events.length>0):u,f=b=>{let N=`cal-status-${b.status}`,q=b.projected?"cal-event-projected":"",E=b.status==="overdue"&&b.days_until_due!=null?` (${Math.abs(b.days_until_due)}d ${i("overdue",e).toLowerCase()})`:"",w=b.projected&&b.interval_days?r`<span class="cal-event-recur">${i("cal_every_n_days",e).replace("{n}",String(b.interval_days))}</span>`:d,M=b.schedule_type==="sensor_based",L=M?r`<ha-icon class="cal-event-icon cal-source-sensor"
          ...
          </span>`:d,Q=this._stats?.budget?.currency_symbol||"\u20AC";return r`
      ...
      `},k=b=>{let[N,q,E]=b.date.split("-").map(Number),w=new Date(N,q-1,E),M=b.date===m,L=w.toLocaleDateString(e,{weekday:"short"}),B=w.toLocaleDateString(e,{month:"long"});return r`
        <div class="cal-day-row">
          <div class="cal-day-pill ${M?"cal-today":""}">
            <span class="cal-pill-weekday">${L}</span>
            <span class="cal-pill-day">${w.getDate()}</span>
          </div>
      ...
      <ha-card .header=${n}>
        ...
        <div class="cal-rolling">
          ${v.length===0&&h?r`<div class="cal-empty">${i("cal_no_events",e)}</div>`:v.map(k)}
        </div>
      </ha-card>
    `}};Z.styles=[Ce,At,C`
      :host { display: block; }
      ha-card { padding: 0; overflow: hidden; }
    `],p([z({attribute:!1})],Z.prototype,"hass",2),p([g()],Z.prototype,"_config",2),p([g()],Z.prototype,"_objects",2),p([g()],Z.prototype,"_stats",2),p([g()],Z.prototype,"_windowDays",2),p([g()],Z.prototype,"_pastDays",2),p([g()],Z.prototype,"_userFilter",2),p([g()],Z.prototype,"_unsub",2);var Fa=[{value:7,label:"Week (7 days)"},{value:14,label:"Fortnight (14 days)"},{value:30,label:"Month (30 days, default)"},{value:365,label:"Year (365 days, empty days collapsed)"}],ue=class extends T{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"}}setConfig(e){this._config={...e}}_valueChanged(e,t){let a={...this._config,[e]:t};e==="show_window_chips"&&t===!0&&delete a.show_window_chips,e==="show_user_filter"&&t===!0&&delete a.show_user_filter,e==="title"&&(!t||typeof t=="string"&&t.trim()==="")&&delete a.title,e==="user_filter"&&t===""&&delete a.user_filter,this._config=a,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:a},bubbles:!0,composed:!0}))}render(){let e=this._config.window_days??30,t=this._config.show_window_chips!==!1,a=this._config.show_user_filter!==!1,n=this._config.user_filter??"",o=this._config.title??"";return r`
      <div class="editor">
        <div class="row">
        ...
        <div class="row">
          ...
          >
            ${Fa.map(l=>r`<option value="${l.value}" ?selected=${l.value===e}>${l.label}</option>`)}
          </select>
        </div>
  ...
  `,p([z({attribute:!1})],ue.prototype,"hass",2),p([g()],ue.prototype,"_config",2);customElements.get("maintenance-supporter-calendar-card")||customElements.define("maintenance-supporter-calendar-card",Z);customElements.get("maintenance-supporter-calendar-card-editor")||customElements.define("maintenance-supporter-calendar-card-editor",ue);var De=window;De.customCards=De.customCards||[];var St="maintenance-supporter-calendar-card",La=De.customCards.some(c=>c.type===St);La||De.customCards.push({type:St,name:"Maintenance Supporter \u2014 Calendar",description:"Rolling calendar of maintenance tasks with 7/14/30/365 day windows, source icons, and prediction-confidence pills.",preview:!0});var ee=class extends T{constructor(){super(...arguments);this._open=!1;this._saving=!1;this._error="";this._draft=null;this._originalSnapshot=null}get _lang(){return this.hass?.language||"en"}openEdit(e){this._draft={...e},this._originalSnapshot={...e},this._error="",this._open=!0}close(){this._open=!1,this._error="",this._draft=null,this._originalSnapshot=null}_set(e,t){this._draft&&(this._draft={...this._draft,[e]:t})}async _save(){if(!(!this._draft||!this._originalSnapshot)){this._saving=!0,this._error="";try{let e={type:"maintenance_supporter/task/history/update",entry_id:this._draft.entry_id,task_id:this._draft.task_id,original_timestamp:this._originalSnapshot.original_timestamp};if(this._draft.timestamp!==this._originalSnapshot.timestamp&&(e.timestamp=this._draft.timestamp),this._draft.notes!==this._originalSnapshot.notes&&(e.notes=this._draft.notes),this._draft.cost!==this._originalSnapshot.cost&&(e.cost=this._draft.cost),this._draft.duration!==this._originalSnapshot.duration&&(e.duration=this._draft.duration),this._draft.completed_by!==this._originalSnapshot.completed_by&&(e.completed_by=this._draft.completed_by),Object.keys(e).filter(a=>!["type","entry_id","task_id","original_timestamp"].includes(a)).length===0){this.close();return}await this.hass.connection.sendMessagePromise(e),this.dispatchEvent(new CustomEvent("history-entry-saved",{detail:{entry_id:this._draft.entry_id,task_id:this._draft.task_id,new_timestamp:this._draft.timestamp},bubbles:!0,composed:!0})),this.close()}catch(e){this._error=V(e,this._lang)}finally{this._saving=!1}}}render(){if(!this._open||!this._draft)return d;let e=this._lang,t=this._draft;return r`
  ...
  `,p([z({attribute:!1})],W.prototype,"hass",2),p([g()],W.prototype,"_open",2),p([g()],W.prototype,"_title",2),p([g()],W.prototype,"_message",2),p([g()],W.prototype,"_confirmText",2),p([g()],W.prototype,"_danger",2),p([g()],W.prototype,"_inputLabel",2),p([g()],W.prototype,"_inputType",2),p([g()],W.prototype,"_inputValue",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",W);var Tt={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},qt=c=>(...s)=>({_$litDirective$:c,values:s}),Re=class{constructor(s){}get _$AU(){return this._$AM._$AU}_$AT(s,e,t){this._$Ct=s,this._$AM=e,this._$Ci=t}_$AS(s,e){return this.update(s,e)}update(s,e){return this.render(...e)}};var we=class extends Re{constructor(s){if(super(s),this.it=d,s.type!==Tt.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(s){if(s===d||s==null)return this._t=void 0,this.it=s;if(s===X)return s;if(typeof s!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(s===this.it)return this._t;this.it=s;let e=[s];return e.raw=e,this._t={_$litType$:this.constructor.resultType,strings:e,values:[]}}};we.directiveName="unsafeHTML",we.resultType=1;var Ct=qt(we);var Ha=["EUR","USD","GBP","JPY","CHF","CAD","AUD","CNY","INR","BRL","CZK","PLN","RUB","SEK","NOK","DKK","UAH"],A=class extends T{constructor(){super(...arguments);this.budget=null;this._settings=null;this._loading=!0;this._importCsv="";this._importLoading=!1;this._includeHistory=!0;this._toast="";this._testingNotification=!1;this._users=[];this._vacEnabled=!1;this._vacStart="";this._vacEnd="";this._vacBuffer=3;this._vacExempt=new Set;this._vacIsActive=!1;this._vacWindowEnd=null;this._vacAllTasks=[];this._vacPreview=[];this._vacPreviewLoading=!1;this._vacSaving=!1;this._qrObjects=[];this._qrSelectedEntries=new Set;this._qrActions=new Set(["view"]);this._qrUrlMode="companion";this._qrBatchLoading=!1;this._qrBatchResults=[];this._qrObjectsLoaded=!1;this._loaded=!1;this._userService=null;this._sendTestNotification=async()=>{this._testingNotification=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/test_notification"}),t=e.message||(e.success?i("test_notification_success",this._lang):i("test_notification_failed",this._lang));this._showToast(t)}catch{this._showToast(i("test_notification_failed",this._lang))}finally{this._testingNotification=!1}}}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._loaded?(this._loaded=!0,this._userService=new se(this.hass),this._loadSettings(),this._loadUsers()):e.has("hass")&&this.hass&&this._userService&&this._userService.updateHass(this.hass)}async _loadUsers(){if(this._userService)try{this._users=await this._userService.getUsers()}catch{this._users=[]}}async _loadSettings(){this._loading=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});this._settings=e,this._hydrateVacationFromSettings()}catch{}this._loading=!1}_hydrateVacationFromSettings(){let e=this._settings?.vacation;e&&(this._vacEnabled=e.enabled,this._vacStart=e.start||"",this._vacEnd=e.end||"",this._vacBuffer=e.buffer_days,this._vacExempt=new Set(e.exempt_task_ids||[]),this._vacIsActive=e.is_active,this._vacWindowEnd=e.window_end)}async _updateSetting(e,t){try{let a=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/update",settings:{[e]:t}});this._settings=a,this._showToast(i("settings_saved",this._lang)),this.dispatchEvent(new CustomEvent("settings-changed"))}catch{this._showToast(i("action_error",this._lang))}}_showToast(e){this._toast=e,setTimeout(()=>{this._toast=""},3e3)}_downloadFile(e,t,a){let n=new Blob([e],{type:a}),o=URL.createObjectURL(n),l=document.createElement("a");l.href=o,l.download=t,l.click(),URL.revokeObjectURL(o)}render(){let e=this._lang;return this._loading||!this._settings?r`<div class="settings-loading">Loading…</div>`:r`
    ...
    `}scrollToSection(e){requestAnimationFrame(()=>{let t=this.shadowRoot;if(!t)return;let a=t.querySelector(`[data-section="${e}"]`)??t.querySelector(`[data-section-alt="${e}"]`);a&&a.scrollIntoView({behavior:"smooth",block:"start"})})}_renderPanelAccess(e){let t=new Set(this._settings.admin_panel_user_ids||[]),a=this._users.filter(o=>!o.is_admin),n=(o,l)=>{let _=new Set(t);l?_.add(o):_.delete(o),this._updateSetting("admin_panel_user_ids",[..._])};return r`
      ...
      <div class="settings-section">
        ...
        <label class="setting-row">
          ...
          <select @change=${n=>this._updateSetting("budget_currency",n.target.value)}>
            ${Ha.map(n=>r`<option value=${n} ?selected=${a.currency===n}>${n}</option>`)}
          </select>
        </label>
        <label class="setting-row">
    ...
    `}_renderVacationTaskList(e){let t=new Map;for(let n of this._vacAllTasks){let o=t.get(n.object_name)||[];o.push(n),t.set(n.object_name,o)}return[...t.entries()].sort(([n],[o])=>n.localeCompare(o)).map(([n,o])=>r`
      <div class="vac-task-group">
        ...
        ${o.sort((l,_)=>l.task_name.localeCompare(_.task_name)).map(l=>r`
            <label class="vac-task-row">
              <input type="checkbox"
                .checked=${this._vacExempt.has(l.task_id)}
                @change=${_=>this._toggleVacationExempt(l.task_id,_.target.checked)} />
                  ...
                  ${this._qrObjects.length===0?r`<div class="hint">${i("no_objects",e)}</div>`:this._qrObjects.map(l=>r`
                      <label class="qr-object-row">
                        <input type="checkbox"
                          ...
                          @change=${_=>this._toggleQrObject(l.entry_id,_.target.checked)} />
                        <span>${l.name}</span>
                  ...
                  ${["view","complete","skip"].map(l=>r`
                    <label class="qr-action-chip ${this._qrActions.has(l)?"active":""}">
                      <input type="checkbox"
                        ...
                        @change=${_=>this._toggleQrAction(l,_.target.checked)} />
                      ${i("qr_action_"+l,e)}
                    </label>
                  `)}
                </div>
...
<body>
  ...
  `,p([z({attribute:!1})],A.prototype,"hass",2),p([z({attribute:!1})],A.prototype,"features",2),p([z({attribute:!1})],A.prototype,"budget",2),p([g()],A.prototype,"_settings",2),p([g()],A.prototype,"_loading",2),p([g()],A.prototype,"_importCsv",2),p([g()],A.prototype,"_importLoading",2),p([g()],A.prototype,"_includeHistory",2),p([g()],A.prototype,"_toast",2),p([g()],A.prototype,"_testingNotification",2),p([g()],A.prototype,"_users",2),p([g()],A.prototype,"_vacEnabled",2),p([g()],A.prototype,"_vacStart",2),p([g()],A.prototype,"_vacEnd",2),p([g()],A.prototype,"_vacBuffer",2),p([g()],A.prototype,"_vacExempt",2),p([g()],A.prototype,"_vacIsActive",2),p([g()],A.prototype,"_vacWindowEnd",2),p([g()],A.prototype,"_vacAllTasks",2),p([g()],A.prototype,"_vacPreview",2),p([g()],A.prototype,"_vacPreviewLoading",2),p([g()],A.prototype,"_vacSaving",2),p([g()],A.prototype,"_qrObjects",2),p([g()],A.prototype,"_qrSelectedEntries",2),p([g()],A.prototype,"_qrActions",2),p([g()],A.prototype,"_qrUrlMode",2),p([g()],A.prototype,"_qrBatchLoading",2),p([g()],A.prototype,"_qrBatchResults",2),p([g()],A.prototype,"_qrObjectsLoaded",2);customElements.define("maintenance-settings-view",A);var Oa=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"],J=class extends T{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._entryId="";this._taskId="";this._values=new Array(12).fill("");this._save=async()=>{let e=this._buildOverrides();if(e!==null){this._loading=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/seasonal_overrides",entry_id:this._entryId,task_id:this._taskId,overrides:e}),this._open=!1,this.dispatchEvent(new CustomEvent("overrides-saved"))}catch(t){this._error=V(t,this._lang,i("save_error",this._lang))}finally{this._loading=!1}}};this._clearAll=async()=>{this._loading=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/seasonal_overrides",entry_id:this._entryId,task_id:this._taskId,overrides:{}}),this._values=new Array(12).fill(""),this._open=!1,this.dispatchEvent(new CustomEvent("overrides-saved"))}catch(e){this._error=V(e,this._lang,i("save_error",this._lang))}finally{this._loading=!1}}}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}open(e,t,a){if(this._entryId=e,this._taskId=t,this._values=new Array(12).fill(""),a)for(let[n,o]of Object.entries(a)){let l=parseInt(n,10);l>=1&&l<=12&&typeof o=="number"&&(this._values[l-1]=o.toString())}this._error="",this._open=!0}_close(){this._open=!1}_buildOverrides(){let e={};for(let t=0;t<12;t++){let a=this._values[t].trim();if(!a)continue;let n=parseFloat(a);if(Number.isNaN(n))return this._error=`${i("month_"+["jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec"][t],this._lang)}: ${i("seasonal_override_invalid",this._lang)}`,null;if(n<.1||n>5)return this._error=i("seasonal_override_range",this._lang),null;e[t+1]=n}return e}render(){if(!this._open)return r``;let e=this._lang;return r`
  ...
  `,p([z({attribute:!1})],G.prototype,"hass",2),p([z({attribute:!1})],G.prototype,"objects",2),p([g()],G.prototype,"_open",2),p([g()],G.prototype,"_loading",2),p([g()],G.prototype,"_error",2),p([g()],G.prototype,"_groupId",2),p([g()],G.prototype,"_name",2),p([g()],G.prototype,"_description",2),p([g()],G.prototype,"_selected",2);customElements.get("maintenance-group-dialog")||customElements.define("maintenance-group-dialog",G);var Ua=300,Ba=140,Xe=27;function Nt(c,s){let e=c.trigger_config;if(!e)return d;let t=s.lang,a=c.trigger_entity_info,n=c.trigger_entity_infos,o=a?.friendly_name||e.entity_id||"\u2014",l=e.entity_id||"",_=e.entity_ids||(l?[l]:[]),u=a?.unit_of_measurement||"",m=c.trigger_current_value,h=e.type||"threshold",v=_.length>1;return r`
    <h3>${i("trigger",t)}</h3>
    <div class="trigger-card">
      <div class="trigger-header">
        ...
        <div>
          ${v?r`
            ...
            <div class="trigger-entity-id">${_.map((f,k)=>r`${k>0?", ":""}<span class="entity-link" @click=${b=>ce(b,f)}>${f}</span>`)}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `:r`
            <div class="trigger-entity-name">${o}</div>
            <div class="trigger-entity-id">${l?r`<span class="entity-link" @click=${f=>ce(f,l)}>${l}</span>`:""}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `}
        </div>
      ...
      ${m!=null?r`
            <div class="trigger-value-row">
              <span class="trigger-current ${c.trigger_active?"active":""}">${typeof m=="number"?m.toFixed(1):m}</span>
      ...
      <div class="trigger-limits">
        ...
        ${h==="compound"?r`
          ...
          ${(e.conditions||[]).map((f,k)=>r`
            <span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${k+1}. ${i(f.type||"unknown",t)}: ${f.entity_id?r`<span class="entity-link" @click=${b=>ce(b,f.entity_id)}>${f.entity_id}</span>`:""}</span>
          `)}
        `:d}
      ...
      ${n&&n.length>1?r`
        <div class="trigger-entity-list">
          ${n.map(f=>r`
            <span class="trigger-entity-id">${f.friendly_name} (<span class="entity-link" @click=${k=>ce(k,f.entity_id)}>${f.entity_id}</span>)</span>
          `)}
        </div>
      `:d}
  ...
  `}function Va(c,s,e){let t=c.trigger_config;if(!t)return d;let a=t.type||"threshold",n=a==="counter"&&t.trigger_delta_mode,o=e.isCounterEntity(t),l=t.entity_id||"",_=e.detailStatsData.get(l)||[],u=[],m=!1;if(_.length>=2)for(let y of _){let S=y.val;n&&c.trigger_baseline_value!=null&&(S-=c.trigger_baseline_value);let H={ts:y.ts,val:S};!o&&y.min!=null&&y.max!=null&&(H.min=n&&c.trigger_baseline_value!=null?y.min-c.trigger_baseline_value:y.min,H.max=n&&c.trigger_baseline_value!=null?y.max-c.trigger_baseline_value:y.max,m=!0),u.push(H)}else for(let y of c.history)y.trigger_value!=null&&u.push({ts:new Date(y.timestamp).getTime(),val:y.trigger_value});if(c.trigger_current_value!=null){let y=c.trigger_current_value;n&&c.trigger_baseline_value!=null&&(y-=c.trigger_baseline_value),u.push({ts:Date.now(),val:y})}if(u.length<2&&l&&e.hasStatsService&&!e.detailStatsData.has(l))return r`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
      <ha-icon icon="mdi:chart-line" style="--mdc-icon-size:16px;margin-right:8px;"></ha-icon>
      ${i("loading_chart",e.lang)}
    </div>`;if(u.length<2)return d;u.sort((y,S)=>y.ts-S.ts);let h=Ua,v=Ba,f=30,k=2,b=8,N=16,q=u.map(y=>y.val),E=Math.min(...q),w=Math.max(...q);if(m)for(let y of u)y.min!=null&&(E=Math.min(E,y.min)),y.max!=null&&(w=Math.max(w,y.max));t.trigger_above!=null&&(w=Math.max(w,t.trigger_above),E=Math.min(E,t.trigger_above)),t.trigger_below!=null&&(E=Math.min(E,t.trigger_below),w=Math.max(w,t.trigger_below));let M=null,L=null;if(a==="counter"&&t.trigger_target_value!=null){if(c.trigger_baseline_value!=null)M=c.trigger_baseline_value;else if(u.length>0){let y=[...c.history].filter(S=>S.type==="completed"||S.type==="reset").sort((S,H)=>new Date(H.timestamp).getTime()-new Date(S.timestamp).getTime())[0];if(y){let S=new Date(y.timestamp).getTime(),H=u[0],O=Math.abs(u[0].ts-S);for(let te of u){let ae=Math.abs(te.ts-S);ae<O&&(H=te,O=ae)}M=H.val}else M=u[0].val}M!=null?(L=M+t.trigger_target_value,w=Math.max(w,L),E=Math.min(E,M)):(w=Math.max(w,t.trigger_target_value),E=Math.min(E,0))}n&&c.trigger_baseline_value!=null&&(E=Math.min(E,0));let B=w-E||1;E-=B*.1,w+=B*.1;let Q=u[0].ts,P=u[u.length-1].ts,$=P-Q||1,R=y=>f+(y-Q)/$*(h-f-k),D=y=>b+(1-(y-E)/(w-E))*(v-b-N),Ie=u.map(y=>`${R(y.ts).toFixed(1)},${D(y.val).toFixed(1)}`).join(" "),Ot=`M${R(u[0].ts).toFixed(1)},${v-N} `+u.map(y=>`L${R(y.ts).toFixed(1)},${D(y.val).toFixed(1)}`).join(" ")+` L${R(u[u.length-1].ts).toFixed(1)},${v-N} Z`,Pe="";if(m){let y=u.filter(S=>S.min!=null&&S.max!=null);if(y.length>=2){let S=y.map(O=>`${R(O.ts).toFixed(1)},${D(O.max).toFixed(1)}`),H=[...y].reverse().map(O=>`${R(O.ts).toFixed(1)},${D(O.min).toFixed(1)}`);Pe=`M${S[0]} `+S.slice(1).map(O=>`L${O}`).join(" ")+` L${H[0]} `+H.slice(1).map(O=>`L${O}`).join(" ")+" Z"}}let et=u[u.length-1],Ut=R(et.ts),Bt=D(et.val),tt=y=>Math.abs(y)>=1e4?(y/1e3).toFixed(0)+"k":y>=1e3?(y/1e3).toFixed(1)+"k":y.toFixed(y<10?1:0),Vt=tt(w),Wt=tt(E),Gt=c.history.filter(y=>["completed","skipped","reset"].includes(y.type)).map(y=>({ts:new Date(y.timestamp).getTime(),type:y.type})).filter(y=>y.ts>=Q&&y.ts<=P),Fe=u;if(u.length>Xe){let y=(u.length-1)/(Xe-1);Fe=[];for(let S=0;S<Xe;S++)Fe.push(u[Math.round(S*y)])}return r`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${h} ${v}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_sparkline",e.lang)}">
        <text x="${f-3}" y="${b+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${Vt}</text>
        <text x="${f-3}" y="${v-N+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${Wt}</text>
        ...
        <text x="${h-k}" y="${v-1}" text-anchor="end" fill="var(--secondary-text-color)" font-size="7">${new Date(P).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        ${Pe?I`<path d="${Pe}" fill="var(--primary-color)" opacity="0.08" />`:d}
        <path d="${Ot}" fill="var(--primary-color)" opacity="0.15" />
        <polyline points="${Ie}" fill="none" stroke="var(--primary-color)" stroke-width="2" stroke-linejoin="round" />
        ${c.degradation_rate!=null&&c.degradation_trend!=="stable"&&c.degradation_trend!=="insufficient_data"&&u.length>=2?(()=>{let y=u[u.length-1],S=30,H=y.ts+S*864e5,O=y.val+c.degradation_rate*S,te=Math.min(H,P+(P-Q)*.3),ae=Math.max(E,Math.min(w,O)),Qt=R(y.ts),Kt=D(y.val),Zt=R(te),Jt=D(ae);return I`<line x1="${Qt.toFixed(1)}" y1="${Kt.toFixed(1)}" x2="${Zt.toFixed(1)}" y2="${Jt.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():d}
        ${a==="threshold"&&t.trigger_above!=null?I`<line x1="${f}" y1="${D(t.trigger_above).toFixed(1)}" x2="${h}" y2="${D(t.trigger_above).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${h-2}" y="${D(t.trigger_above)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25B2 ${t.trigger_above}</text>`:d}
        ${a==="threshold"&&t.trigger_below!=null?I`<line x1="${f}" y1="${D(t.trigger_below).toFixed(1)}" x2="${h}" y2="${D(t.trigger_below).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${h-2}" y="${D(t.trigger_below)+11}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25BC ${t.trigger_below}</text>`:d}
        ${a==="counter"&&L!=null?I`<line x1="${f}" y1="${D(L).toFixed(1)}" x2="${h}" y2="${D(L).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${h-2}" y="${D(L)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">${i("target_value",e.lang)}: +${t.trigger_target_value}</text>`:d}
        ${a==="counter"&&M!=null?I`<line x1="${f}" y1="${D(M).toFixed(1)}" x2="${h}" y2="${D(M).toFixed(1)}" stroke="var(--secondary-text-color)" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
                <text x="${f+2}" y="${D(M)+11}" text-anchor="start" fill="var(--secondary-text-color)" font-size="8">${i("baseline",e.lang)}</text>`:d}
        <circle cx="${Ut.toFixed(1)}" cy="${Bt.toFixed(1)}" r="3.5" fill="var(--primary-color)" />
        ${Gt.map(y=>{let S=R(y.ts),H=y.type==="completed"?"var(--success-color, #4caf50)":y.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return I`
          ...
          `})}
        ${Fe.map(y=>{let S=R(y.ts),H=D(y.val),O=new Date(y.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),te=`${y.val.toFixed(1)} ${s}`;return m&&y.min!=null&&y.max!=null&&(te+=` (${y.min.toFixed(1)}\u2013${y.max.toFixed(1)})`),I`<circle cx="${S.toFixed(1)}" cy="${H.toFixed(1)}" r="8" fill="transparent" tabindex="0"
            @mouseenter=${ae=>Mt(ae,`${O}
${te}`,e.setTooltip)}
            @focus=${ae=>Mt(ae,`${O}
${te}`,e.setTooltip)}
...
`).map(y=>r`<div>${y}</div>`)}
      ...
      <div class="prediction-grid">
        ...
        ${a?r`
          <div class="prediction-item">
            ...
            ${c.threshold_prediction_date?r`<span class="prediction-date">${ne(c.threshold_prediction_date,s)}</span>`:d}
        ...
        ${n&&e.environmental?r`
          <div class="prediction-item">
            ...
            ${c.environmental_entity?r`<span class="prediction-entity entity-link" @click=${l=>ce(l,c.environmental_entity)}>${c.environmental_entity}</span>`:d}
          </div>
        `:d}
      </div>
    </div>
  `}function Rt(c,s){let e=c.interval_analysis,t=e?.weibull_beta,a=e?.weibull_eta;if(t==null||a==null||a<=0)return d;let n=c.interval_days??0,o=c.suggested_interval??n;return r`
  ...
  `}function Ka(c,s,e){let t=c.confidence_interval_low,a=c.confidence_interval_high,n=s.suggested_interval??s.interval_days??0,o=s.interval_days??0,l=Math.max(0,t-5),u=a+5-l,m=(t-l)/u*100,h=(a-t)/u*100,v=(n-l)/u*100,f=o>0?(o-l)/u*100:-1;return r`
  ...
  `}var It=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"];function Pt(c,s,e){if(!e.seasonal||!c.seasonal_factor||c.seasonal_factor===1)return d;let t=It.map(l=>i(l,s)),a=new Date().getMonth(),n=c.seasonal_factors||c.interval_analysis?.seasonal_factors||null,o=n&&n.length===12?n:t.map((l,_)=>{let u=c.seasonal_factor||1,m=Math.sin((_-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,u+m))});return r`
    <div class="seasonal-card-compact">
      <h4>${i("seasonal_awareness",s)}</h4>
      <div class="seasonal-mini-chart">
        ${o.map((l,_)=>{let u=l*40,m=l<.9?"low":l>1.1?"high":"normal";return r`
            <div class="seasonal-bar ${m} ${_===a?"current":""}"
                 style="height: ${u}px"
                 title="${t[_]}: ${l.toFixed(2)}x">
            </div>
          `})}
      </div>
  ...
  `}function Ft(c,s){return Za(c,s)}function Za(c,s){let e=c.seasonal_factors??c.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return d;let t=c.interval_analysis?.seasonal_reason,a=new Date().getMonth(),n=300,o=100,l=8,u=o-l-4,m=Math.max(...e,1.5),h=n/12,v=h*.65,f=l+u-1/m*u;return r`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${n} ${o}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_seasonal",s)}">
        ...
        ${e.map((k,b)=>{let N=k/m*u,q=b*h+(h-v)/2,E=l+u-N,w=b===a,M=k<1?"var(--success-color, #4caf50)":k>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return I`
            <rect x="${q.toFixed(1)}" y="${E.toFixed(1)}"
              width="${v.toFixed(1)}" height="${N.toFixed(1)}"
              fill="${M}" opacity="${w?1:.5}" rx="2" />
          `})}
      </svg>
      <div class="seasonal-labels">
        ${It.map((k,b)=>r`<span class="seasonal-label ${b===a?"active-month":""}">${i(k,s)}</span>`)}
      </div>
    </div>
  `}var Ja=300,Ya=200;function Lt(c,s,e,t){let a=c.history.filter(l=>l.type==="completed"&&(l.cost!=null||l.duration!=null));if(a.length<2)return d;let n=a.some(l=>(l.cost??0)>0),o=a.some(l=>(l.duration??0)>0);return!n&&!o?d:r`
  ...
  `}function Xa(c,s,e){let t=c.history.filter($=>$.type==="completed"&&($.cost!=null||$.duration!=null)).map($=>({ts:new Date($.timestamp).getTime(),cost:$.cost??0,duration:$.duration??0})).sort(($,R)=>$.ts-R.ts);if(t.length<2)return d;let a=t.some($=>$.cost>0),n=t.some($=>$.duration>0);if(!a&&!n)return d;let o=e!=="duration"&&a,l=e!=="cost"&&n,_=o||!l&&a,u=l||!o&&n,m=Ja,h=Ya,v=_?32:8,f=u?32:8,k=8,b=20,N=m-v-f,q=h-k-b,E=Math.max(...t.map($=>$.cost))||1,w=Math.max(...t.map($=>$.duration))||1,M=Math.min(20,N/t.length*.6),L=N/t.length,B=$=>v+L*$+L/2,Q=$=>k+q-$/E*q,P=$=>k+q-$/w*q;return r`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${m} ${h}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_history",s)}">
        ${_?t.map(($,R)=>I`
          <rect x="${(B(R)-M/2).toFixed(1)}" y="${Q($.cost).toFixed(1)}" width="${M.toFixed(1)}" height="${(k+q-Q($.cost)).toFixed(1)}"
            fill="var(--primary-color)" opacity="0.6" rx="2" />
        `):d}
        ${u?I`
          <polyline points="${t.map(($,R)=>`${B(R).toFixed(1)},${P($.duration).toFixed(1)}`).join(" ")}"
            fill="none" stroke="var(--accent-color, #ff9800)" stroke-width="2" stroke-linejoin="round" />
          ${t.map(($,R)=>I`
            <circle cx="${B(R).toFixed(1)}" cy="${P($.duration).toFixed(1)}" r="3" fill="var(--accent-color, #ff9800)" />
          `)}
        `:d}
        ...
        <text x="${m-f}" y="${h-2}" text-anchor="end" fill="var(--secondary-text-color)" font-size="7">${new Date(t[t.length-1].ts).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        ${_?I`
          <text x="${v-3}" y="${k+4}" text-anchor="end" fill="var(--primary-color)" font-size="7">${E.toFixed(0)}\u20AC</text>
          <text x="${v-3}" y="${k+q+3}" text-anchor="end" fill="var(--primary-color)" font-size="7">0\u20AC</text>
        `:d}
        ${u?I`
          <text x="${m-f+3}" y="${k+4}" text-anchor="start" fill="var(--accent-color, #ff9800)" font-size="7">${w.toFixed(0)}m</text>
          <text x="${m-f+3}" y="${k+q+3}" text-anchor="start" fill="var(--accent-color, #ff9800)" font-size="7">0m</text>
        `:d}
      </svg>
    </div>
    <div class="chart-legend">
  ...
  `}var ei=60,ti=20,Ht=30,j=class extends T{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._sparklineTooltip=null;this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._adminPanelUserIds=[];this._defaultWarningDays=7;this._actionLoading=!1;this._moreMenuOpen=!1;this._toastMessage="";this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab="dashboard";this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._objectSortMode="alphabetical";this._groupByMode="none";this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._deepLinkHandled=!1;this._onDialogEvent=async()=>{try{await this._loadData()}catch{}};this._onCalendarLlCustom=e=>{let t=e.detail;t?.type==="maintenance-supporter:open-task"&&t.entry_id&&t.task_id&&(e.stopPropagation(),this._showTask(t.entry_id,t.task_id))};this._onHistoryEntrySaved=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"en"}get _isOperator(){let e=this.hass?.user;return e?e.is_admin?!1:!this._adminPanelUserIds.includes(e.id):!0}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler);let e=localStorage.getItem("maintenance_supporter_sort");e&&["due_date","object","type","task_name","area","assigned_user","group"].includes(e)&&(this._sortMode=e);let t=localStorage.getItem("maintenance_supporter_object_sort");t&&["alphabetical","due_soonest","task_count"].includes(t)&&(this._objectSortMode=t);let a=localStorage.getItem("maintenance_supporter_groupby");a&&["none","area","group","user"].includes(a)&&(this._groupByMode=a)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new Me(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new se(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,a,n,o]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);if(e&&(this._objects=e.objects),t&&(this._stats=t),a&&(this._budget=a),n&&(this._groups=n.groups||{}),o){let l=o;this._features=l.features,this._adminPanelUserIds=l.admin_panel_user_ids||[];let _=l.general?.default_warning_days;typeof _=="number"&&_>=0&&_<=365&&(this._defaultWarningDays=_)}this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("ms_action"),a=()=>{let m=window.location.pathname+window.location.hash;history.replaceState(history.state,"",m)};if(t==="add_object"){this._deepLinkHandled=!0,a(),requestAnimationFrame(()=>{this.shadowRoot?.querySelector("maintenance-object-dialog")?.openCreate()});return}if(t==="open_vacation"||t==="open_budget"||t==="open_groups"||t==="open_settings"){this._deepLinkHandled=!0,a(),this._overviewTab="settings",requestAnimationFrame(()=>{let m=this.shadowRoot?.querySelector("maintenance-settings-view"),h=t.replace("open_","");m?.scrollToSection?.(h)});return}let n=e.get("entry_id");if(!n)return;this._deepLinkHandled=!0;let o=e.get("task_id"),l=e.get("action"),_=window.location.pathname+window.location.hash;history.replaceState(history.state,"",_);let u=this._getObject(n);if(!u){this._showOverview();return}if(o){let m=u.tasks.find(h=>h.id===o);if(!m){this._showObject(n);return}this._showTask(n,o),l==="complete"?requestAnimationFrame(()=>{this._openCompleteDialog(n,o,m.name,this._features.checklists?m.checklist:void 0,this._features.adaptive&&!!m.adaptive_config?.enabled)}):l==="quick_complete"&&requestAnimationFrame(()=>{this._handleQuickComplete(n,o,m)})}else this._showObject(n)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let a=await this._statsService.getDetailStats(e,t),n=new Map(this._detailStatsData);n.set(e,a),this._detailStatsData=n}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let a of this._objects)for(let n of a.tasks){let o=n.trigger_config?.entity_id;o&&e.push({entityId:o,isCounter:this._isCounterEntity(n.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let h of this._objects)for(let v of h.tasks){if(this._filterStatus&&v.status!==this._filterStatus)continue;if(this._filterUser){let k=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(v.responsible_user_id!==k)continue}let f=[];for(let k of Object.values(this._groups))k.task_refs?.some(b=>b.entry_id===h.entry_id&&b.task_id===v.id)&&f.push(k.name);e.push({entry_id:h.entry_id,task_id:v.id,object_name:h.object.name,task_name:v.name,type:v.type,schedule_type:v.schedule_type,status:v.status,days_until_due:v.days_until_due??null,next_due:v.next_due??null,trigger_active:v.trigger_active,trigger_current_value:v.trigger_current_value??null,trigger_current_delta:v.trigger_current_delta??null,trigger_config:v.trigger_config??null,trigger_entity_info:v.trigger_entity_info??null,times_performed:v.times_performed,total_cost:v.total_cost,interval_days:v.interval_days??null,interval_anchor:v.interval_anchor??null,history:v.history||[],enabled:v.enabled,nfc_tag_id:v.nfc_tag_id??null,area_id:h.object.area_id??null,responsible_user_id:v.responsible_user_id??null,group_names:f})}let t={overdue:0,triggered:1,due_soon:2,ok:3},a=(h,v)=>(t[h.status]??9)-(t[v.status]??9),n=(h,v)=>(h.days_until_due??99999)-(v.days_until_due??99999),o=(h,v)=>a(h,v)||n(h,v),l=h=>h.area_id&&this.hass?.areas?.[h.area_id]?.name||"",_=h=>h.responsible_user_id&&this._userService?.getUserName(h.responsible_user_id)||"",u=h=>h.group_names[0]||"",m={due_date:o,object:(h,v)=>h.object_name.localeCompare(v.object_name)||o(h,v),type:(h,v)=>h.type.localeCompare(v.type)||o(h,v),task_name:(h,v)=>h.task_name.localeCompare(v.task_name),area:(h,v)=>{let f=l(h),k=l(v);return!f&&k?1:f&&!k?-1:f.localeCompare(k)||o(h,v)},assigned_user:(h,v)=>{let f=_(h),k=_(v);return!f&&k?1:f&&!k?-1:f.localeCompare(k)||o(h,v)},group:(h,v)=>{let f=u(h),k=u(v);return!f&&k?1:f&&!k?-1:f.localeCompare(k)||o(h,v)}};return e.sort(m[this._sortMode]),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(n=>n.id===t)}_pushPanelState(e,t,a){let n={msp_view:e,msp_entry:t||null,msp_task:a||null};history.pushState(n,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let a=this._getTask(t.msp_entry,t.msp_task);a?.trigger_config?.entity_id&&this._fetchDetailStats(a.trigger_config.entity_id,this._isCounterEntity(a.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null}_filterByStatus(e){this._filterStatus=e,this._overviewTab!=="dashboard"&&(this._overviewTab="dashboard"),requestAnimationFrame(()=>{this.shadowRoot?.querySelector(".filter-bar")?.scrollIntoView({behavior:"smooth",block:"start"})})}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null;let a=this._getTask(e,t);if(a?.trigger_config?.entity_id){let n=a.trigger_config.entity_id,o=this._isCounterEntity(a.trigger_config);this._fetchDetailStats(n,o)}}_showToast(e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}async _deleteObject(e){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:i("delete",this._lang),message:i("confirm_delete_object",this._lang),confirmText:i("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}}async _deleteTask(e,t){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:i("delete",this._lang),message:i("confirm_delete_task",this._lang),confirmText:i("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}}async _skipTask(e,t,a){this._actionLoading=!0;try{let n={type:"maintenance_supporter/task/skip",entry_id:e,task_id:t};a&&(n.reason=a),await this.hass.connection.sendMessagePromise(n),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}finally{this._actionLoading=!1}}async _resetTask(e,t,a){this._actionLoading=!0;try{let n={type:"maintenance_supporter/task/reset",entry_id:e,task_id:t};a&&(n.date=a),await this.hass.connection.sendMessagePromise(n),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}finally{this._actionLoading=!1}}async _applySuggestion(e,t,a){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:a}),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}}_openSeasonalOverrides(e){let t=this.shadowRoot.querySelector("maintenance-seasonal-overrides-dialog");if(!t||!this._selectedEntryId)return;let a=e.adaptive_config?.seasonal_overrides;t.open(this._selectedEntryId,e.id,a)}async _reanalyzeInterval(e,t){try{let a=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:e,task_id:t});a.recommended_interval?this._showToast(`${i("reanalyze_result",this._lang)}: ${a.recommended_interval} ${i("days",this._lang)} (${i(`confidence_${a.confidence}`,this._lang)}, ${a.data_points} ${i("data_points",this._lang)})`):this._showToast(i("reanalyze_insufficient_data",this._lang)),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}}async _promptSkipTask(e,t){let a=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!a)return;let n=await a.prompt({title:i("skip",this._lang),message:i("skip_reason_prompt",this._lang),confirmText:i("skip",this._lang),inputLabel:i("reason_optional",this._lang),inputType:"text"});n.confirmed&&this._skipTask(e,t,n.value||void 0)}async _promptResetTask(e,t){let a=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!a)return;let n=await a.prompt({title:i("reset",this._lang),message:i("reset_date_prompt",this._lang),confirmText:i("reset",this._lang),inputLabel:i("reset_date_optional",this._lang),inputType:"date"});n.confirmed&&this._resetTask(e,t,n.value||void 0)}_dismissSuggestion(e,t){e&&t&&this._dismissedSuggestions.add(`${e}_${t}`),this.requestUpdate()}async _handleQuickComplete(e,t,a){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/quick_complete",entry_id:e,task_id:t}),this._showToast(i("quick_complete_success",this._lang))}catch(n){(n?.code||"")==="no_defaults"?this._openCompleteDialog(e,t,a.name,this._features.checklists?a.checklist:void 0,this._features.adaptive&&!!a.adaptive_config?.enabled):this._showToast(i("action_error",this._lang))}}_openCompleteDialog(e,t,a,n,o){let l=this.shadowRoot.querySelector("maintenance-complete-dialog");l&&(l.entryId=e,l.taskId=t,l.taskName=a,l.lang=this._lang,l.checklist=n||[],l.adaptiveEnabled=!!o,l.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,a,n){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,a,n)}render(){return r`
    ...
    `}_renderGroupedTasks(e,t){let a=new Map,n=i("unassigned",t);for(let _ of e){let u=[];this._groupByMode==="area"?u=[(_.area_id?this.hass?.areas?.[_.area_id]?.name:null)||n]:this._groupByMode==="user"?u=[(_.responsible_user_id?this._userService?.getUserName(_.responsible_user_id):null)||n]:this._groupByMode==="group"&&(u=_.group_names.length>0?_.group_names:[n]);for(let m of u)a.has(m)||a.set(m,[]),a.get(m).push(_)}let o=[...a.entries()].sort(([_],[u])=>_===n&&u!==n?1:u===n&&_!==n?-1:_.localeCompare(u)),l=this._groupByMode==="area"?"mdi:map-marker-outline":this._groupByMode==="group"?"mdi:folder-outline":"mdi:account-outline";return r`
      ${o.map(([_,u])=>r`
        <details class="group-section" open>
          ...
          <div class="task-table">
            ${u.map(m=>this._renderOverviewRow(m))}
          </div>
        </details>
      `)}
    `}_renderAllObjects(){let e=this._lang,t=this._isOperator,a=_=>{let u=1/0;for(let m of _.tasks){let h=m.days_until_due;h!=null&&h<u&&(u=h)}return u},n=[...this._objects];this._objectSortMode==="alphabetical"?n.sort((_,u)=>_.object.name.localeCompare(u.object.name)):this._objectSortMode==="task_count"?n.sort((_,u)=>u.tasks.length-_.tasks.length||_.object.name.localeCompare(u.object.name)):n.sort((_,u)=>a(_)-a(u)||_.object.name.localeCompare(u.object.name));let o=()=>{let _=new Map;for(let u of n){let m=u.object.area_id,h=m?this.hass?.areas?.[m]?.name||i("unassigned",e):i("no_area",e);_.has(h)||_.set(h,[]),_.get(h).push(u)}return new Map([..._.entries()].sort(([u],[m])=>u.localeCompare(m)))},l=_=>{let u=_.tasks.some(m=>m.status==="overdue"||m.status==="triggered");return r`
    ...
    `}_openGroupCreate(){this.shadowRoot.querySelector("maintenance-group-dialog")?.openCreate()}_openGroupEdit(e){let t=this._groups[e];t&&this.shadowRoot.querySelector("maintenance-group-dialog")?.openEdit(e,t)}async _deleteGroup(e,t){let a=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(a?await a.confirm({title:i("delete_group",this._lang),message:i("delete_group_confirm",this._lang).replace("{name}",t),confirmText:i("delete",this._lang)}):confirm(`${i("delete_group_confirm",this._lang).replace("{name}",t)}`))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/group/delete",group_id:e}),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}}_renderBudgetBar(){let e=this._budget;if(!e)return d;let t=this._lang,a=e.currency_symbol||"\u20AC",n=[];return e.monthly_budget>0&&n.push({label:i("budget_monthly",t),spent:e.monthly_spent,budget:e.monthly_budget}),e.yearly_budget>0&&n.push({label:i("budget_yearly",t),spent:e.yearly_spent,budget:e.yearly_budget}),n.length===0?d:r`
      <div class="budget-bars">
        ${n.map(o=>{let l=Math.min(100,Math.max(0,o.spent/o.budget*100)),_=l>=100?"var(--error-color, #f44336)":l>=e.alert_threshold_pct?"var(--warning-color, #ff9800)":"var(--success-color, #4caf50)";return r`
            <div class="budget-item">
              ...
              <div class="budget-bar">
                <div class="budget-bar-fill" style="width:${l}%; background:${_}"></div>
              </div>
            </div>
          `})}
      </div>
    `}_renderOverviewRow(e){let t=this._lang,a=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,n=0,o=Te.ok,l=!1;if(a&&e.days_until_due!==null){let h=(e.interval_days-e.days_until_due)/e.interval_days*100;n=Math.max(0,Math.min(100,h)),l=h>100,e.status==="overdue"?o=Te.overdue:e.status==="due_soon"&&(o=Te.due_soon)}let _=e.area_id?this.hass?.areas?.[e.area_id]?.name:null,u=e.responsible_user_id?this._userService?.getUserName(e.responsible_user_id):null,m=e.group_names.length>0||_||u;return r`
      <div class="task-row${e.enabled?"":" task-disabled"}">
        ...
        <span class="due-cell" @click=${()=>this._showTask(e.entry_id,e.task_id)}>
          <span class="due-text">${qe(e.days_until_due,t)}</span>
    ...
    `}_openCompleteDialogForRow(e){let a=this._objects.find(n=>n.entry_id===e.entry_id)?.tasks.find(n=>n.id===e.task_id);this._openCompleteDialog(e.entry_id,e.task_id,e.task_name,this._features.checklists?a?.checklist:void 0,this._features.adaptive&&!!a?.adaptive_config?.enabled)}_renderTriggerProgress(e){let t=e.trigger_config??null;if(!t)return d;let a=t.type||"threshold",n=e.trigger_entity_info?.unit_of_measurement??"",o=0,l="";if(a==="threshold"){let m=e.trigger_current_value??null;if(m==null)return d;let h=t.trigger_above,v=t.trigger_below;if(h!=null){let f=v??0,k=h-f||1;o=Math.min(100,Math.max(0,(m-f)/k*100)),l=`${m.toFixed(1)} / ${h} ${n}`}else if(v!=null){let k=e.trigger_entity_info?.max??(v*2||100),b=k-v||1;o=Math.min(100,Math.max(0,(k-m)/b*100)),l=`${m.toFixed(1)} / ${v} ${n}`}else return d}else if(a==="counter"){let m=t.trigger_target_value||1,v=e.trigger_current_delta??null??e.trigger_current_value??null;if(v==null)return d;o=Math.min(100,Math.max(0,v/m*100)),l=`${v.toFixed(1)} / ${m} ${n}`}else if(a==="state_change"){let m=t.trigger_target_changes||1,h=e.trigger_current_value??null;if(h==null)return d;o=Math.min(100,Math.max(0,h/m*100)),l=`${Math.round(h)} / ${m}`}else if(a==="runtime"){let m=t.trigger_runtime_hours||100,h=e.trigger_current_value??null;if(h==null)return d;o=Math.min(100,Math.max(0,h/m*100)),l=`${h.toFixed(1)}h / ${m}h`}else if(a==="compound"){let m=t.compound_logic||t.operator||"AND",h=t.conditions?.length||0;l=`${m} (${h})`,o=e.trigger_active?100:0}else return d;let _=o>=100,u=o>90?"var(--error-color, #f44336)":o>70?"var(--warning-color, #ff9800)":"var(--primary-color)";return r`
    ...
    `}_renderMiniSparkline(e){if(!e.trigger_config?.entity_id)return d;let t=e.trigger_config.entity_id,a=this._miniStatsData.get(t)||[],n=[];if(a.length>=2)n=a.map(w=>({ts:w.ts,val:w.val}));else{if(!e.history)return d;for(let w of e.history)w.trigger_value!=null&&n.push({ts:new Date(w.timestamp).getTime(),val:w.trigger_value})}if(e.trigger_current_value!=null&&n.push({ts:Date.now(),val:e.trigger_current_value}),n.length<2)return d;n.sort((w,M)=>w.ts-M.ts);let o=ei,l=ti,_=n.map(w=>w.val),u=Math.min(..._),m=Math.max(..._),h=m-u||1;u-=h*.1,m+=h*.1;let v=n[0].ts,k=n[n.length-1].ts-v||1,b=w=>(w-v)/k*o,N=w=>2+(1-(w-u)/(m-u))*(l-4),q=n;if(q.length>Ht){let w=Math.ceil(q.length/Ht);q=q.filter((M,L)=>L%w===0||L===q.length-1)}let E=q.map(w=>`${b(w.ts).toFixed(1)},${N(w.val).toFixed(1)}`).join(" ");return r`
    ...
    `}_renderDaysProgress(e){let t=this._lang;if(e.days_until_due==null||!e.interval_days||e.interval_days<=0)return d;let n=(e.interval_days-e.days_until_due)/e.interval_days*100,o=Math.max(0,Math.min(100,n)),l=n>100,_="var(--success-color, #4caf50)";return e.status==="overdue"?_="var(--error-color, #f44336)":e.status==="due_soon"&&(_="var(--warning-color, #ff9800)"),r`
      <div class="days-progress">
        <div class="days-progress-labels">
          <span>${e.last_performed?`${i("last_performed",t)}: ${ne(e.last_performed,t)}`:""}</span>
          <span>${e.next_due?`${i("next_due",t)}: ${ne(e.next_due,t)}`:""}</span>
        </div>
        <div class="days-progress-bar" role="progressbar" aria-valuenow="${Math.round(o)}" aria-valuemin="0" aria-valuemax="100" aria-label="${i("days_progress",t)}">
          <div class="days-progress-fill${l?" overflow":""}" style="width:${o}%;background:${_}"></div>
        </div>
        <div class="days-progress-text">${qe(e.days_until_due,t)}</div>
      </div>
    `}_renderObjectDetail(){if(!this._selectedEntryId)return d;let e=this._getObject(this._selectedEntryId);if(!e)return r`<p>Object not found.</p>`;let t=e.object,a=this._lang,n=this._isOperator;return r`
      <div class="detail-section">
        ...
        ${t.installation_date?r`<p class="meta">${i("installed",a)}: ${ne(t.installation_date,a)}</p>`:d}
        ...
        ${e.tasks.length===0?r`<div class="empty-state-centered">
            ...
            </div>`:[...e.tasks].sort((o,l)=>{let _={overdue:0,triggered:1,due_soon:2,ok:3};return(_[o.status]??9)-(_[l.status]??9)||(o.days_until_due??99999)-(l.days_until_due??99999)}).map(o=>r`
              <div class="task-row${o.enabled?"":" task-disabled"}">
                ...
                <span class="due-cell" @click=${()=>this._showTask(e.entry_id,o.id)}>
                  <span class="due-text">${qe(o.days_until_due,a)}</span>
    ...
    `}_renderTaskHeader(e){let t=this._lang,n=this._getObject(this._selectedEntryId)?.object.name||"",o=this._isOperator,l=e.status==="due_soon"?"warning":e.status||"ok",_=i(e.status||"ok",t);return r`
      <div class="task-header">
        <div class="task-header-title">
          ...
          <span class="status-chip ${l}">${_}</span>
    ...
    `}_renderTabContent(e){switch(this._activeTab){case"overview":return this._renderOverviewTab(e);case"history":return this._renderHistoryTab(e);default:return d}}get _sparklineCtx(){return{lang:this._lang,detailStatsData:this._detailStatsData,hasStatsService:!!this._statsService,isCounterEntity:e=>this._isCounterEntity(e),tooltip:this._sparklineTooltip,setTooltip:e=>{this._sparklineTooltip=e}}}_renderOverviewTab(e){let t=this._lang,a=this._features.adaptive&&e.suggested_interval&&e.suggested_interval!==e.interval_days,n=this._features.seasonal&&e.seasonal_factor&&e.seasonal_factor!==1,o=a||n,l=this._features.adaptive&&e.interval_analysis?.weibull_beta!=null&&e.interval_analysis?.weibull_eta!=null,_=this._features.seasonal&&(e.seasonal_factors?.length===12||e.interval_analysis?.seasonal_factors?.length===12);return r`
      <div class="tab-content overview-tab">
        ...
        ${Nt(e,this._sparklineCtx)}
        ${Dt(e,t,this._features)}
        <div class="two-column-layout ${o?"":"single-column"}">
          ${o?r`
            <div class="left-column">
              ${this._renderRecommendationCard(e)}
              ${Pt(e,t,this._features)}
            </div>
          `:d}
          <div class="right-column">
            ${Lt(e,t,this._costDurationToggle,u=>{this._costDurationToggle=u})}
          </div>
        </div>
        ${l?Rt(e,t):d}
        ${_?r`
    ...
    `}_renderKPIBar(e){let t=this._lang,a=e.times_performed>0?e.total_cost/e.times_performed:0,n=e.days_until_due!==null&&e.days_until_due!==void 0?e.days_until_due<0?"overdue":e.days_until_due<=e.warning_days?"warning":"":"";return r`
      <div class="kpi-bar">
        <div class="kpi-card">
          ...
          <div class="kpi-value">${e.next_due?ne(e.next_due,t):"\u2014"}</div>
        ...
        <div class="kpi-card">
          ...
          <div class="kpi-value">${e.last_performed?ne(e.last_performed,t):"\u2014"}</div>
        </div>
        <div class="kpi-card">
    ...
    `}_renderRecommendationCard(e){let t=this._lang;if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return d;if(this._selectedEntryId&&this._selectedTaskId&&this._dismissedSuggestions.has(`${this._selectedEntryId}_${this._selectedTaskId}`))return d;let a=e.interval_days,n=e.suggested_interval,o=e.interval_confidence||"medium",l=Math.max(a||1,n);return r`
      <div class="recommendation-card">
        <h4>${i("suggested_interval",t)}</h4>
        <div class="interval-comparison">
          <div class="interval-bar">
            <div class="interval-label">${i("current",t)||"Aktuell"}: ${a??"\u2014"} ${a!=null?i("days",t):""}</div>
            <div class="interval-visual current" style="width: ${a!=null?Math.min(a/l*100,100):0}%"></div>
          </div>
          <div class="interval-bar">
            <div class="interval-label">${i("recommended",t)}: ${n} ${i("days",t)}
              <span class="confidence-badge ${o}">${i(`confidence_${o}`,t)}</span>
            </div>
            <div class="interval-visual suggested" style="width: ${Math.min(n/l*100,100)}%"></div>
          </div>
        </div>
      ...
      <div class="recent-activities">
        ...
        ${a.map(o=>r`
          <div class="activity-item">
            ...
            <span class="activity-date">${Ye(o.timestamp,t)}</span>
    ...
    `}_renderHistoryEntry(e){let t=this._lang,a=["completed","reset","skipped"].includes(e.type);return r`
      <div class="history-entry">
        <div class="history-icon ${e.type}">
          <ha-icon .icon=${kt[e.type]||"mdi:circle"}></ha-icon>
        </div>
        <div class="history-content">
          ...
          <div class="history-date">${Ye(e.timestamp,t)}</div>
