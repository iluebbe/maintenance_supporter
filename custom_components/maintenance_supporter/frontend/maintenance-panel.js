...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `,h([N({attribute:!1})],q.prototype,"hass",2),h([N()],q.prototype,"lang",2),h([f()],q.prototype,"_open",2),h([f()],q.prototype,"_loading",2),h([f()],q.prototype,"_error",2),h([f()],q.prototype,"_result",2),h([f()],q.prototype,"_action",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",q);var Vt=60,Wt=20,Gt=300,Kt=140,Zt=300,Yt=200,lt=30,Jt=27,S=class extends H{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._sparklineTooltip=null;this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1};this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._statsService=null;this._userService=null;this._deepLinkHandled=!1;this._onDialogEvent=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"de"}connectedCallback(){super.connectedCallback(),this._loadData(),this._subscribe()}disconnectedCallback(){super.disconnectedCallback(),this._unsub&&(this._unsub(),this._unsub=null),this._statsService?.clearCache(),this._statsService=null}updated(e){super.updated(e),e.has("hass")&&this.hass&&(this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new xe(this.hass),this._fetchMiniStatsForOverview()),this._userService||(this._userService=new re(this.hass),this._userService.getUsers()))}async _loadData(){let[e,t,i,r,s]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);e&&(this._objects=e.objects),t&&(this._stats=t),i&&(this._budget=i),r&&(this._groups=r.groups||{}),s&&(this._features=s.features),this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("entry_id");if(!t)return;this._deepLinkHandled=!0;let i=e.get("task_id"),r=e.get("action"),s=window.location.pathname+window.location.hash;history.replaceState(null,"",s),i?(this._showTask(t,i),r==="complete"&&requestAnimationFrame(()=>{let p=this._getObject(t)?.tasks.find(m=>m.id===i);p&&this._openCompleteDialog(t,i,p.name,this._features.checklists?p.checklist:void 0,this._features.adaptive&&!!p.adaptive_config?.enabled)})):this._showObject(t)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let i=await this._statsService.getDetailStats(e,t),r=new Map(this._detailStatsData);r.set(e,i),this._detailStatsData=r}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=new Map(this._miniStatsData),t=[];for(let i of this._objects)for(let r of i.tasks){let s=r.trigger_config?.entity_id;if(!s)continue;let d=this._isCounterEntity(r.trigger_config);t.push(this._statsService.getMiniStats(s,d).then(p=>{e.set(s,p)}))}t.length>0&&(await Promise.all(t),this._miniStatsData=e)}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let i of this._objects)for(let r of i.tasks)if(!(this._filterStatus&&r.status!==this._filterStatus)){if(this._filterUser){let s=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(r.responsible_user_id!==s)continue}e.push({entry_id:i.entry_id,task_id:r.id,object_name:i.object.name,task_name:r.name,type:r.type,schedule_type:r.schedule_type,status:r.status,days_until_due:r.days_until_due??null,next_due:r.next_due??null,trigger_active:r.trigger_active,trigger_current_value:r.trigger_current_value??null,trigger_current_delta:r.trigger_current_delta??null,trigger_config:r.trigger_config??null,trigger_entity_info:r.trigger_entity_info??null,times_performed:r.times_performed,total_cost:r.total_cost,interval_days:r.interval_days??null,history:r.history||[],enabled:r.enabled})}let t={overdue:0,triggered:1,due_soon:2,ok:3};return e.sort((i,r)=>(t[i.status]??9)-(t[r.status]??9)),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(r=>r.id===t)}_showOverview(){this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null}_showObject(e){this._view="object",this._selectedEntryId=e,this._selectedTaskId=null}_showTask(e,t){this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._historyFilter=null;let i=this._getTask(e,t);if(i?.trigger_config?.entity_id){let r=i.trigger_config.entity_id,s=this._isCounterEntity(i.trigger_config);this._fetchDetailStats(r,s)}}async _deleteObject(e){confirm(n("confirm_delete_object",this._lang))&&(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData())}async _deleteTask(e,t){confirm(n("confirm_delete_task",this._lang))&&(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData())}async _skipTask(e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/skip",entry_id:e,task_id:t}),await this._loadData()}async _resetTask(e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/reset",entry_id:e,task_id:t}),await this._loadData()}async _applySuggestion(e,t,i){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:i}),await this._loadData()}_dismissSuggestion(){this._loadData()}_openCompleteDialog(e,t,i,r,s){let d=this.shadowRoot.querySelector("maintenance-complete-dialog");d&&(d.entryId=e,d.taskId=t,d.taskName=i,d.lang=this._lang,d.checklist=r||[],d.adaptiveEnabled=!!s,d.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,i,r){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,i,r)}async _exportCsv(){let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/csv/export"}),t=new Blob([e.csv],{type:"text/csv;charset=utf-8;"}),i=URL.createObjectURL(t),r=document.createElement("a");r.href=i,r.download="maintenance_export.csv",r.click(),URL.revokeObjectURL(i)}_triggerImportCsv(){let e=this.shadowRoot.querySelector("input[type=file]");e&&(e.value="",e.click())}async _handleCsvFile(e){let i=e.target.files?.[0];if(!i)return;let r=await i.text(),s=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/csv/import",csv_content:r});await this._loadData(),alert(`Imported ${s.created} of ${s.total} objects.`)}render(){return o`
    ...
    `}_openCompleteDialogForRow(e){let i=this._objects.find(r=>r.entry_id===e.entry_id)?.tasks.find(r=>r.id===e.task_id);this._openCompleteDialog(e.entry_id,e.task_id,e.task_name,this._features.checklists?i?.checklist:void 0,this._features.adaptive&&!!i?.adaptive_config?.enabled)}_renderTriggerProgress(e){let t=e.trigger_config??null;if(!t)return l;let i=t.type||"threshold",r=e.trigger_entity_info?.unit_of_measurement??"",s=0,d="";if(i==="threshold"){let u=e.trigger_current_value??null;if(u==null)return l;let _=t.trigger_above,v=t.trigger_below;if(_!=null){let $=v??0,b=_-$||1;s=Math.min(100,Math.max(0,(u-$)/b*100)),d=`${u.toFixed(1)} / ${_} ${r}`}else if(v!=null){let b=e.trigger_entity_info?.max??(u*2||100),M=b-v||1;s=Math.min(100,Math.max(0,(b-u)/M*100)),d=`${u.toFixed(1)} / ${v} ${r}`}else return l}else if(i==="counter"){let u=t.trigger_target_value||1,v=e.trigger_current_delta??null??e.trigger_current_value??null;if(v==null)return l;s=Math.min(100,Math.max(0,v/u*100)),d=`${v.toFixed(1)} / ${u} ${r}`}else if(i==="state_change"){let u=t.trigger_target_changes||1,_=e.trigger_current_value??null;if(_==null)return l;s=Math.min(100,Math.max(0,_/u*100)),d=`${Math.round(_)} / ${u}`}else if(i==="runtime"){let u=t.trigger_runtime_hours||100,_=e.trigger_current_value??null;if(_==null)return l;s=Math.min(100,Math.max(0,_/u*100)),d=`${_.toFixed(1)}h / ${u}h`}else if(i==="compound"){let u=t.compound_logic||t.operator||"AND",_=t.conditions?.length||0;d=`${u} (${_})`,s=e.trigger_active?100:0}else return l;let p=s>=100,m=s>90?"var(--error-color, #f44336)":s>70?"var(--warning-color, #ff9800)":"var(--primary-color)";return o`
    ...
    `}_renderAnalysisTab(e){let t=this._lang;if(!(this._features.adaptive||this._features.seasonal))return o`
      ...
      `;let r=this._features.adaptive&&e.interval_analysis?.weibull_beta!=null&&e.interval_analysis?.weibull_eta!=null,s=this._features.seasonal&&(e.seasonal_factors?.length===12||e.interval_analysis?.seasonal_factors?.length===12);if(!(r||s)){let p=e.schedule_type==="manual",m=e.interval_analysis?.data_points??0,u=Math.min(100,Math.max(0,m/5*100));return o`
        <div class="tab-content analysis-tab">
          ${this._renderTriggerSection(e)}
          ${this._renderPredictionSection(e)}
          <div class="analysis-empty-state">
    ...
    `}_renderSeasonalCardCompact(e){let t=this._lang;if(!this._features.seasonal||!e.seasonal_factor||e.seasonal_factor===1)return l;let r=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"].map(m=>n(m,t)),s=new Date().getMonth(),d=e.seasonal_factors||e.interval_analysis?.seasonal_factors||null,p=d&&d.length===12?d:r.map((m,u)=>{let _=e.seasonal_factor||1,v=Math.sin((u-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,_+v))});return o`
      <div class="seasonal-card-compact">
        <h4>${n("seasonal_awareness",t)}</h4>
        <div class="seasonal-mini-chart">
          ${p.map((m,u)=>{let _=m*40,v=m<.9?"low":m>1.1?"high":"normal";return o`
              <div class="seasonal-bar ${v} ${u===s?"current":""}"
                   style="height: ${_}px"
                   title="${r[u]}: ${m.toFixed(2)}x">
              </div>
            `})}
        </div>
    ...
    `}_renderHistoryChart(e){let t=e.history.filter(x=>x.type==="completed"&&(x.cost!=null||x.duration!=null)).map(x=>({ts:new Date(x.timestamp).getTime(),cost:x.cost??0,duration:x.duration??0})).sort((x,D)=>x.ts-D.ts);if(t.length<2)return l;let i=t.some(x=>x.cost>0),r=t.some(x=>x.duration>0);if(!i&&!r)return l;let s=this._costDurationToggle!=="duration"&&i,d=this._costDurationToggle!=="cost"&&r,p=s||!d&&i,m=d||!s&&r,u=Zt,_=Yt,v=p?32:8,$=m?32:8,b=8,M=20,L=u-v-$,E=_-b-M,P=Math.max(...t.map(x=>x.cost))||1,y=Math.max(...t.map(x=>x.duration))||1,A=Math.min(20,L/t.length*.6),C=L/t.length,I=x=>v+C*x+C/2,B=x=>b+E-x/P*E,V=x=>b+E-x/y*E;return o`
      <div class="sparkline-container">
        <svg class="history-chart" viewBox="0 0 ${u} ${_}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_history",this._lang)}">
          <!-- Cost bars -->
          ${p?t.map((x,D)=>F`
            <rect x="${(I(D)-A/2).toFixed(1)}" y="${B(x.cost).toFixed(1)}" width="${A.toFixed(1)}" height="${(b+E-B(x.cost)).toFixed(1)}"
              fill="var(--primary-color)" opacity="0.6" rx="2" />
          `):l}
          <!-- Duration line -->
          ${m?F`
            <polyline points="${t.map((x,D)=>`${I(D).toFixed(1)},${V(x.duration).toFixed(1)}`).join(" ")}"
              fill="none" stroke="var(--accent-color, #ff9800)" stroke-width="2" stroke-linejoin="round" />
            ${t.map((x,D)=>F`
              <circle cx="${I(D).toFixed(1)}" cy="${V(x.duration).toFixed(1)}" r="3" fill="var(--accent-color, #ff9800)" />
            `)}
          `:l}
          <!-- X-axis labels -->
          ...
          <!-- Y-axis labels -->
          ${p?F`
            <text x="${v-3}" y="${b+4}" text-anchor="end" fill="var(--primary-color)" font-size="7">${P.toFixed(0)}\u20AC</text>
            <text x="${v-3}" y="${b+E+3}" text-anchor="end" fill="var(--primary-color)" font-size="7">0\u20AC</text>
          `:l}
          ${m?F`
      ...
      <div class="chart-legend">
        ...
        ${m?o`<span class="legend-item"><span class="legend-swatch" style="background:var(--accent-color, #ff9800)"></span>${n("duration",this._lang)}</span>`:l}
      </div>
    `}_renderSeasonalChart(e){let t=e.seasonal_factors??e.interval_analysis?.seasonal_factors;if(!t||t.length!==12)return l;let i=this._lang,r=e.interval_analysis?.seasonal_reason,s=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"],d=new Date().getMonth(),p=300,m=100,u=8,v=m-u-4,$=Math.max(...t,1.5),b=p/12,M=b*.65,L=u+v-1/$*v;return o`
      <div class="seasonal-chart">
        ...
        <svg viewBox="0 0 ${p} ${m}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_seasonal",i)}">
          ...
          <!-- Bars -->
          ${t.map((E,P)=>{let y=E/$*v,A=P*b+(b-M)/2,C=u+v-y,I=P===d,B=E<1?"var(--success-color, #4caf50)":E>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return F`
              <rect x="${A.toFixed(1)}" y="${C.toFixed(1)}"
                width="${M.toFixed(1)}" height="${y.toFixed(1)}"
                fill="${B}" opacity="${I?1:.5}" rx="2" />
            `})}
        </svg>
        <div class="seasonal-labels">
          ${s.map((E,P)=>o`<span class="seasonal-label ${P===d?"active-month":""}">${n(E,i)}</span>`)}
        </div>
      </div>
    `}_renderWeibullSection(e){let t=e.interval_analysis,i=t?.weibull_beta,r=t?.weibull_eta;if(i==null||r==null)return l;let s=this._lang,d=t?.weibull_r_squared,p=e.interval_days??0,m=e.suggested_interval??p;return o`
    ...
    `}_renderWeibullChart(e,t,i,r){let b=Math.max(i,r,t)*1.3,M=50,L=[];for(let x=0;x<=M;x++){let D=x/M*b,R=1-Math.exp(-Math.pow(D/t,e)),T=32+D/b*260,$e=136-R*128;L.push([T,$e])}let E=L.map(([x,D])=>`${x.toFixed(1)},${D.toFixed(1)}`).join(" "),P="M32,136 "+L.map(([x,D])=>`L${x.toFixed(1)},${D.toFixed(1)}`).join(" ")+` L${L[M][0].toFixed(1)},136 Z`,y=32+i/b*260,A=1-Math.exp(-Math.pow(i/t,e)),C=136-A*128,I=((1-A)*100).toFixed(0),B=32+r/b*260,V=[0,.25,.5,.75,1];return o`
      <div class="weibull-chart">
        <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_weibull",this._lang)}">
          <!-- Grid lines -->
          ${V.map(x=>{let D=136-x*128;return F`
          ...
          ${i>0?F`
            ...
            <text x="${(y+4).toFixed(1)}" y="${(C-6).toFixed(1)}" fill="var(--primary-color, #03a9f4)"
              font-size="9" font-weight="600">R=${I}%</text>
          `:l}
    ...
    `}_renderConfidenceInterval(e,t,i){let r=e.confidence_interval_low,s=e.confidence_interval_high,d=t.suggested_interval??t.interval_days??0,p=t.interval_days??0,m=Math.max(0,r-5),_=s+5-m,v=(r-m)/_*100,$=(s-r)/_*100,b=(d-m)/_*100,M=p>0?(p-m)/_*100:-1;return o`
    ...
    `}_renderTriggerSection(e){let t=e.trigger_config;if(!t)return l;let i=this._lang,r=e.trigger_entity_info,s=e.trigger_entity_infos,d=r?.friendly_name||t.entity_id||"\u2014",p=t.entity_id||"",m=t.entity_ids||(p?[p]:[]),u=r?.unit_of_measurement||"",_=e.trigger_current_value,v=t.type||"threshold",$=m.length>1;return o`
      <h3>${n("trigger",i)}</h3>
    ...
    `}_renderSparkline(e,t){let i=e.trigger_config;if(!i)return l;let r=i.type||"threshold",s=r==="counter"&&i.trigger_delta_mode,d=this._isCounterEntity(i),p=i.entity_id||"",m=this._detailStatsData.get(p)||[],u=[],_=!1;if(m.length>=2)for(let g of m){let k=g.val;s&&e.trigger_baseline_value!=null&&(k-=e.trigger_baseline_value);let j={ts:g.ts,val:k};!d&&g.min!=null&&g.max!=null&&(j.min=s&&e.trigger_baseline_value!=null?g.min-e.trigger_baseline_value:g.min,j.max=s&&e.trigger_baseline_value!=null?g.max-e.trigger_baseline_value:g.max,_=!0),u.push(j)}else for(let g of e.history)g.trigger_value!=null&&u.push({ts:new Date(g.timestamp).getTime(),val:g.trigger_value});if(e.trigger_current_value!=null){let g=e.trigger_current_value;s&&e.trigger_baseline_value!=null&&(g-=e.trigger_baseline_value),u.push({ts:Date.now(),val:g})}if(u.length<2&&p&&this._statsService&&!this._detailStatsData.has(p))return o`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
        <ha-icon icon="mdi:chart-line" style="--mdc-icon-size:16px;margin-right:8px;"></ha-icon>
        ${n("loading_chart",this._lang)}
      </div>`;if(u.length<2)return l;u.sort((g,k)=>g.ts-k.ts);let v=Gt,$=Kt,b=30,M=2,L=8,E=16,P=u.map(g=>g.val),y=Math.min(...P),A=Math.max(...P);if(_)for(let g of u)g.min!=null&&(y=Math.min(y,g.min)),g.max!=null&&(A=Math.max(A,g.max));i.trigger_above!=null&&(A=Math.max(A,i.trigger_above),y=Math.min(y,i.trigger_above)),i.trigger_below!=null&&(y=Math.min(y,i.trigger_below),A=Math.max(A,i.trigger_below));let C=null,I=null;if(r==="counter"&&i.trigger_target_value!=null){if(e.trigger_baseline_value!=null)C=e.trigger_baseline_value;else if(u.length>0){let g=[...e.history].filter(k=>k.type==="completed"||k.type==="reset").sort((k,j)=>new Date(j.timestamp).getTime()-new Date(k.timestamp).getTime())[0];if(g){let k=new Date(g.timestamp).getTime(),j=u[0],z=Math.abs(u[0].ts-k);for(let G of u){let K=Math.abs(G.ts-k);K<z&&(j=G,z=K)}C=j.val}else C=u[0].val}C!=null?(I=C+i.trigger_target_value,A=Math.max(A,I),y=Math.min(y,C)):(A=Math.max(A,i.trigger_target_value),y=Math.min(y,0))}s&&e.trigger_baseline_value!=null&&(y=Math.min(y,0));let B=A-y||1;y-=B*.1,A+=B*.1;let V=u[0].ts,x=u[u.length-1].ts,D=x-V||1,R=g=>b+(g-V)/D*(v-b-M),T=g=>L+(1-(g-y)/(A-y))*($-L-E),$e=u.map(g=>`${R(g.ts).toFixed(1)},${T(g.val).toFixed(1)}`).join(" "),ct=`M${R(u[0].ts).toFixed(1)},${$-E} `+u.map(g=>`L${R(g.ts).toFixed(1)},${T(g.val).toFixed(1)}`).join(" ")+` L${R(u[u.length-1].ts).toFixed(1)},${$-E} Z`,we="";if(_){let g=u.filter(k=>k.min!=null&&k.max!=null);if(g.length>=2){let k=g.map(z=>`${R(z.ts).toFixed(1)},${T(z.max).toFixed(1)}`),j=[...g].reverse().map(z=>`${R(z.ts).toFixed(1)},${T(z.min).toFixed(1)}`);we=`M${k[0]} `+k.slice(1).map(z=>`L${z}`).join(" ")+` L${j[0]} `+j.slice(1).map(z=>`L${z}`).join(" ")+" Z"}}let Re=u[u.length-1],dt=R(Re.ts),ut=T(Re.val),Ne=g=>Math.abs(g)>=1e4?(g/1e3).toFixed(0)+"k":g>=1e3?(g/1e3).toFixed(1)+"k":g.toFixed(g<10?1:0),pt=Ne(A),gt=Ne(y),_t=e.history.filter(g=>["completed","skipped","reset"].includes(g.type)).map(g=>({ts:new Date(g.timestamp).getTime(),type:g.type})).filter(g=>g.ts>=V&&g.ts<=x),ke=Jt,Ee=u;if(u.length>ke){let g=(u.length-1)/(ke-1);Ee=[];for(let k=0;k<ke;k++)Ee.push(u[Math.round(k*g)])}return o`
      <div class="sparkline-container">
        <svg class="sparkline-svg" viewBox="0 0 ${v} ${$}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_sparkline",this._lang)}">
          ...
          <!-- X-axis labels -->
          <text x="${b}" y="${$-1}" text-anchor="start" fill="var(--secondary-text-color)" font-size="7">${new Date(V).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
          <text x="${v-M}" y="${$-1}" text-anchor="end" fill="var(--secondary-text-color)" font-size="7">${new Date(x).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
          ...
          ${e.degradation_rate!=null&&e.degradation_trend!=="stable"&&e.degradation_trend!=="insufficient_data"&&u.length>=2?(()=>{let g=u[u.length-1],k=30,j=g.ts+k*864e5,z=g.val+e.degradation_rate*k,G=Math.min(j,x+(x-V)*.3),K=Math.max(y,Math.min(A,z)),ht=R(g.ts),mt=T(g.val),vt=R(G),ft=T(K);return F`<line x1="${ht.toFixed(1)}" y1="${mt.toFixed(1)}" x2="${vt.toFixed(1)}" y2="${ft.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():l}
          <!-- Threshold lines (for threshold type) -->
          ${r==="threshold"&&i.trigger_above!=null?F`<line x1="${b}" y1="${T(i.trigger_above).toFixed(1)}" x2="${v}" y2="${T(i.trigger_above).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                  <text x="${v-2}" y="${T(i.trigger_above)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">▲ ${i.trigger_above}</text>`:l}
          ${r==="threshold"&&i.trigger_below!=null?F`<line x1="${b}" y1="${T(i.trigger_below).toFixed(1)}" x2="${v}" y2="${T(i.trigger_below).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
          ...
          ${r==="counter"&&I!=null?F`<line x1="${b}" y1="${T(I).toFixed(1)}" x2="${v}" y2="${T(I).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                  <text x="${v-2}" y="${T(I)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">${n("target_value",this._lang)}: +${i.trigger_target_value}</text>`:l}
          <!-- Baseline line (for counter — value at last maintenance) -->
          ${r==="counter"&&C!=null?F`<line x1="${b}" y1="${T(C).toFixed(1)}" x2="${v}" y2="${T(C).toFixed(1)}" stroke="var(--secondary-text-color)" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
                  <text x="${b+2}" y="${T(C)+11}" text-anchor="start" fill="var(--secondary-text-color)" font-size="8">${n("baseline",this._lang)}</text>`:l}
          <!-- Current dot -->
          ...
          ${Ee.map(g=>{let k=R(g.ts),j=T(g.val),z=new Date(g.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),G=`${g.val.toFixed(1)} ${t}`;return _&&g.min!=null&&g.max!=null&&(G+=` (${g.min.toFixed(1)}\u2013${g.max.toFixed(1)})`),F`<circle cx="${k.toFixed(1)}" cy="${j.toFixed(1)}" r="8" fill="transparent" tabindex="0"
...
`).map(g=>o`<div>${g}</div>`)}
    ...
    `],h([N({attribute:!1})],S.prototype,"hass",2),h([N({type:Boolean})],S.prototype,"narrow",2),h([N({attribute:!1})],S.prototype,"panel",2),h([f()],S.prototype,"_objects",2),h([f()],S.prototype,"_stats",2),h([f()],S.prototype,"_view",2),h([f()],S.prototype,"_selectedEntryId",2),h([f()],S.prototype,"_selectedTaskId",2),h([f()],S.prototype,"_filterStatus",2),h([f()],S.prototype,"_filterUser",2),h([f()],S.prototype,"_unsub",2),h([f()],S.prototype,"_sparklineTooltip",2),h([f()],S.prototype,"_historyFilter",2),h([f()],S.prototype,"_budget",2),h([f()],S.prototype,"_groups",2),h([f()],S.prototype,"_detailStatsData",2),h([f()],S.prototype,"_miniStatsData",2),h([f()],S.prototype,"_features",2),h([f()],S.prototype,"_activeTab",2),h([f()],S.prototype,"_costDurationToggle",2),h([f()],S.prototype,"_historySearch",2),S=h([ae("maintenance-supporter-panel")],S);export{S as MaintenanceSupporterPanel};
