...
`;var Ke=class{constructor(r){this._cache=new Map;this._pending=new Map;this._hass=r}updateHass(r){this._hass=r}async getDetailStats(r,e){return this._getStats(r,"hour",30,e)}async getMiniStats(r,e){return this._getStats(r,"day",14,e)}async getBatchMiniStats(r){let e=new Map,t=[];for(let u of r){let _=`${u.entityId}:day`,m=this._cache.get(_);m&&Date.now()-m.fetchedAt<3e5?e.set(u.entityId,m.points):t.push(u)}if(t.length===0)return e;let i=t.filter(u=>u.isCounter).map(u=>u.entityId),a=t.filter(u=>!u.isCounter).map(u=>u.entityId),l=new Date(Date.now()-336*60*60*1e3).toISOString(),c=[];return i.length>0&&c.push(this._fetchBatch(i,"day",l,["state","sum","change"],!0,e)),a.length>0&&c.push(this._fetchBatch(a,"day",l,["mean","min","max"],!1,e)),await Promise.all(c),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(r,e,t,i){let a=`${r}:${e}`,l=this._cache.get(a);if(l&&Date.now()-l.fetchedAt<3e5)return l.points;if(this._pending.has(a))return this._pending.get(a);let c=this._fetchAndNormalize(r,e,t,i,a);this._pending.set(a,c);try{return await c}finally{this._pending.delete(a)}}async _fetchAndNormalize(r,e,t,i,a){let l=new Date(Date.now()-t*24*60*60*1e3).toISOString(),c=i?["state","sum","change"]:["mean","min","max"];try{let _=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:l,statistic_ids:[r],period:e,types:c}))[r]||[],m=this._normalizeRows(_,i);return this._cache.set(a,{entityId:r,fetchedAt:Date.now(),period:e,points:m}),m}catch(u){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${r}:`,u),[]}}async _fetchBatch(r,e,t,i,a,l){try{let c=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:r,period:e,types:i});for(let u of r){let _=c[u]||[],m=this._normalizeRows(_,a);l.set(u,m),this._cache.set(`${u}:${e}`,{entityId:u,fetchedAt:Date.now(),period:e,points:m})}}catch(c){console.warn("[maintenance-supporter] Batch statistics fetch failed:",c)}}_normalizeRows(r,e){let t=[];for(let i of r){let a=null;if(e?a=i.state??null:a=i.mean??null,a===null)continue;let l={ts:i.start,val:a};e||(i.min!=null&&(l.min=i.min),i.max!=null&&(l.max=i.max)),t.push(l)}return t.sort((i,a)=>i.ts-a.ts),t}};var pe=class{constructor(r){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=r}updateHass(r){this.hass=r}async getUsers(r=!1){let e=Date.now();if(!r&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(r,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:r,task_id:e,user_id:t})}async getTasksByUser(r){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:r})).tasks}getUserName(r){return!r||!this.usersCache?null:this.usersCache.find(t=>t.id===r)?.name||null}getUser(r){return!r||!this.usersCache?null:this.usersCache.find(e=>e.id===r)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(r){return r?r===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var Wi={name:"name",task_type:"maintenance_type",schedule_type:"schedule_type",interval_days:"interval_days",interval_anchor:"interval_anchor",warning_days:"warning_days",last_performed:"last_performed_optional",notes:"notes_optional",documentation_url:"documentation_url_optional",custom_icon:"custom_icon_optional",nfc_tag_id:"nfc_tag_id_optional",responsible_user_id:"responsible_user",entity_slug:"entity_slug",entity_id:"entity_id",area_id:"area_id_optional",manufacturer:"manufacturer_optional",model:"model_optional",serial_number:"serial_number_optional",installation_date:"installation_date_optional",warranty_expiry:"warranty_expiry_optional",checklist:"checklist_steps_optional",reason:"reason",feedback:"feedback",cost:"cost",duration:"duration",description:"description_optional",group_name:"name",group_description:"description_optional",environmental_entity:"environmental_entity_optional",environmental_attribute:"environmental_attribute_optional",trigger_above:"trigger_above",trigger_below:"trigger_below",trigger_for_minutes:"trigger_for_minutes"};function Vi(o,r){let e=Wi[o];if(!e)return o;let t=s(e,r);return t&&t!==e?t:o}function Ki(o){let e=o.match(/data\['([^']+)'\]/)?.[1],t;return(t=o.match(/length of value must be at most (\d+)/))?{field:e,rule:"too_long",param:t[1]}:(t=o.match(/length of value must be at least (\d+)/))?{field:e,rule:"too_short",param:t[1]}:(t=o.match(/value must be at most (\S+)/))?{field:e,rule:"value_too_high",param:t[1]}:(t=o.match(/value must be at least (\S+)/))?{field:e,rule:"value_too_low",param:t[1]}:/required key not provided/.test(o)?{field:e,rule:"required"}:(t=o.match(/expected (\w+)/))?{field:e,rule:"wrong_type",param:t[1]}:/value must be one of/.test(o)?{field:e,rule:"invalid_choice"}:/not a valid value/.test(o)?{field:e,rule:"invalid_value"}:{field:e,rule:"unknown"}}function M(o,r,e){if(typeof o=="string")return o;if(typeof o!="object"||o===null)return e;let t=o,i=t.message||t.error?.message||"";if(!i)return e;let a=Ki(i),l=a.field?Vi(a.field,r):"",c=u=>s(u,r).replace("{field}",l).replace("{n}",a.param??"");switch(a.rule){case"too_long":return c("err_too_long");case"too_short":return c("err_too_short");case"value_too_high":return c("err_value_too_high");case"value_too_low":return c("err_value_too_low");case"required":return c("err_required");case"wrong_type":return c("err_wrong_type").replace("{type}",a.param??"");case"invalid_choice":return c("err_invalid_choice");case"invalid_value":return c("err_invalid_value");default:return i||e}}var V=class extends A{constructor(){super(...arguments);this.label="";this.value="";this.placeholder="";this.type="text";this.required=!1;this.disabled=!1}_onInput(e){let t=e.target.value;this.value=t,this.dispatchEvent(new CustomEvent("input",{bubbles:!0,composed:!0,detail:{value:t}}))}render(){return n`
    ...
    `}_renderDoc(e,t){if(this._editingId===e.id)return this._renderEdit(e,t);let i=e.kind==="file",a=this._category_of(e),l=i?`${s(`doc_cat_${a}`,t)} \xB7 ${ae(e.size)}`:s("doc_link_badge",t),c=this._thumbs[e.id];return n`
      <div class="doc-row">
        ...
        <div
          ...
          @keydown=${u=>{(u.key==="Enter"||u.key===" ")&&(u.preventDefault(),this._openDoc(e))}}
        >
  ...
  `,p([w({attribute:!1})],q.prototype,"hass",2),p([w({attribute:!1})],q.prototype,"entryId",2),p([w({type:Boolean})],q.prototype,"canWrite",2),p([h()],q.prototype,"_docs",2),p([h()],q.prototype,"_loaded",2),p([h()],q.prototype,"_busy",2),p([h()],q.prototype,"_error",2),p([h()],q.prototype,"_hint",2),p([h()],q.prototype,"_addingLink",2),p([h()],q.prototype,"_linkUrl",2),p([h()],q.prototype,"_linkTitle",2),p([h()],q.prototype,"_category",2),p([h()],q.prototype,"_thumbs",2),p([h()],q.prototype,"_lightboxUrl",2),p([h()],q.prototype,"_editingId",2),p([h()],q.prototype,"_editTitle",2),p([h()],q.prototype,"_editCategory",2),p([h()],q.prototype,"_dragOver",2);customElements.get("maintenance-documents-section")||customElements.define("maintenance-documents-section",q);var Gi=["manual","warranty","invoice","spare_parts","photo","other"],Qi={manual:"mdi:book-open-variant",warranty:"mdi:shield-check",invoice:"mdi:receipt-text-outline",spare_parts:"mdi:cog-outline",photo:"mdi:image-outline",other:"mdi:file-document-outline"},Y=class extends A{constructor(){super(...arguments);this.canWrite=!1;this._docs=[];this._loaded=!1;this._busy=!1;this._error="";this._attachId="";this._loadedKey="";this._localeReady=!1}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),this.hass&&!this._localeReady&&(this._localeReady=!0,se(this._lang).then(()=>this.requestUpdate()));let t=`${this.entryId}|${this.taskId}`;this.hass&&this.entryId&&this.taskId&&this._loadedKey!==t&&(this._loadedKey=t,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:this.entryId});this._docs=e.documents||[],this._loaded=!0,this._error=""}catch(e){this._error=M(e,this._lang),this._loaded=!0}}_linked(){return this._docs.filter(e=>(e.task_ids||[]).includes(this.taskId))}_available(){return this._docs.filter(e=>!(e.task_ids||[]).includes(this.taskId))}async _setTaskIds(e,t){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,task_ids:t}),await this._load()}catch(i){this._error=M(i,this._lang)}finally{this._busy=!1}}_link(){let e=this._docs.find(t=>t.id===this._attachId);e&&(this._attachId="",this._setTaskIds(e,[...e.task_ids||[],this.taskId]))}_unlink(e){this._setTaskIds(e,(e.task_ids||[]).filter(t=>t!==this.taskId))}_isPdf(e){return e.mime==="application/pdf"||(e.filename||"").toLowerCase().endsWith(".pdf")}_pageFor(e){return this._isPdf(e)?e.task_pages?.[this.taskId]:void 0}async _open(e){if(e.kind==="weblink"){window.open(e.url,"_blank","noopener");return}let t=this._pageFor(e),i=t?`#page=${t}`:"",a=window.open("about:blank","_blank");try{let l=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300});a&&(a.location.href=new URL(l.path+i,window.location.origin).href)}catch(l){a&&a.close(),this._error=M(l,this._lang)}}async _setPage(e,t){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,task_pages:{[this.taskId]:t}}),await this._load()}catch(i){this._error=M(i,this._lang)}finally{this._busy=!1}}async _download(e){try{let t=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:30});Ve(t.path,e.filename||e.title||"document")}catch(t){this._error=M(t,this._lang)}}render(){if(!this._loaded||this._docs.length===0)return d;let e=this._lang,t=this._linked(),i=this._available();return n`
    ...
    `}_renderRow(e,t){let i=e.kind==="file",a=this._isPdf(e),l=e.task_pages?.[this.taskId],c=(e.tags||[]).find(_=>Gi.includes(_))||"other",u=i?ae(e.size):s("doc_link_badge",t);return n`
      <div class="tdoc-row">
        <ha-icon class="tdoc-icon" icon=${i?Qi[c]:"mdi:link-variant"}></ha-icon>
        <div
          class="tdoc-info"
          role="button"
          tabindex="0"
        ...
        >
          ...
          <div class="tdoc-meta">
            ${u}${l?n` · <span class="tdoc-pagetag">${s("doc_page",t)} ${l}</span>`:d}
          </div>
        </div>
        ${this.canWrite&&a?n`<input
              ...
              .value=${l?String(l):""}
              ?disabled=${this._busy}
              @change=${_=>{let m=parseInt(_.target.value,10);this._setPage(e,Number.isFinite(m)&&m>=1?m:0)}}
            />`:d}
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `,p([w({attribute:!1})],ee.prototype,"hass",2),p([w()],ee.prototype,"lang",2),p([h()],ee.prototype,"_open",2),p([h()],ee.prototype,"_loading",2),p([h()],ee.prototype,"_error",2),p([h()],ee.prototype,"_viewResult",2),p([h()],ee.prototype,"_completeResult",2),p([h()],ee.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",ee);var Yt=5;function Ie(o){let r=o.getFullYear(),e=String(o.getMonth()+1).padStart(2,"0"),t=String(o.getDate()).padStart(2,"0");return`${r}-${e}-${t}`}function ss(o,r){let e=[];for(let t=0;t<r;t++){let i=new Date(o);i.setDate(i.getDate()+t),i.setHours(0,0,0,0),e.push(Ie(i))}return e}function Ge(o,r){let[e,t,i]=o.split("-").map(Number),a=new Date(e,t-1,i);return a.setDate(a.getDate()+r),Ie(a)}function as(o){if(!o||o.length===0)return null;let r=o.map(e=>e.cost).filter(e=>typeof e=="number");return r.length===0?null:r.reduce((e,t)=>e+t,0)/r.length}function rs(o){let{windowStart:r,windowEnd:e,task:t,entryId:i,objectName:a}=o,l=[],c=(g,v)=>({date:g,entry_id:i,task_id:t.id,task_name:t.name,object_name:a,status:v&&(t.status==="overdue"||t.status==="triggered")?"ok":t.status,days_until_due:v?null:t.days_until_due??null,projected:v,schedule_type:t.schedule_type,interval_days:t.interval_days??null,interval_unit:t.interval_unit??null,responsible_user_id:t.responsible_user_id??null,avg_cost:as(t.history),adaptive_enabled:!!t.adaptive_config?.enabled,prediction_confidence:t.threshold_prediction_confidence??null}),u=Math.max(1,Math.round(vt(t.interval_days,t.interval_unit)));if(t.status==="overdue"||t.status==="triggered"){if(l.push(c(r,!1)),t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let g=Ge(r,u),v=1;for(;g<=e&&v<Yt;)l.push(c(g,!0)),v++,g=Ge(g,u)}return l}let _=t.next_due;if(typeof _!="string"||!_)return l;let m=_.slice(0,10);if(m>=r&&m<=e)l.push(c(m,!1));else if(m>e)return l;if(t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let g=Ge(m,u),v=l.length;for(;g<=e&&v<Yt;)g>=r&&(l.push(c(g,!0)),v++),g=Ge(g,u)}return l}var Gt={overdue:0,triggered:1,due_soon:2,ok:3};function Qt(o,r,e,t=null){let i=ss(r,e),a=i[0],l=i[i.length-1],c=[];for(let _ of o){let m=_.object?.name||"",g=_.entry_id,v=_.tasks||[];for(let b of v){if(t&&b.responsible_user_id!==t||b.enabled===!1)continue;let x=rs({windowStart:a,windowEnd:l,task:b,entryId:g,objectName:m});c.push(...x)}}let u=new Map;for(let _ of i)u.set(_,[]);for(let _ of c){let m=u.get(_.date);m&&m.push(_)}for(let[,_]of u)_.sort((m,g)=>{let v=Gt[m.status]??99,b=Gt[g.status]??99;if(v!==b)return v-b;if(m.projected!==g.projected)return m.projected?1:-1;let x=m.object_name.localeCompare(g.object_name);return x!==0?x:m.task_name.localeCompare(g.task_name)});return i.map(_=>({date:_,events:u.get(_)??[]}))}var ns={completed:"ok",reset:"ok",skipped:"due_soon",triggered:"triggered",trigger_replaced:"triggered"};function os(o,r){let e=[];for(let t=r-1;t>=0;t--){let i=new Date(o);i.setDate(i.getDate()-t),i.setHours(0,0,0,0),e.push(Ie(i))}return e}function Jt(o,r,e,t=null){let i=os(r,e),a=i[0],l=i[i.length-1],c=new Map;for(let _ of i)c.set(_,[]);for(let _ of o){let m=_.object?.name||"",g=_.entry_id,v=_.tasks||[];for(let b of v){if(t&&b.responsible_user_id!==t)continue;let x=b.history||[];for(let f of x){if(typeof f?.timestamp!="string")continue;let P=f.timestamp.slice(0,10);if(P<a||P>l)continue;let R=c.get(P);if(!R)continue;let S=f.type??"completed";R.push({date:P,entry_id:g,task_id:b.id,task_name:b.name,object_name:m,status:ns[S]??"ok",days_until_due:null,projected:!1,schedule_type:b.schedule_type,interval_days:b.interval_days??null,responsible_user_id:b.responsible_user_id??null,avg_cost:typeof f.cost=="number"?f.cost:null,adaptive_enabled:!!b.adaptive_config?.enabled,prediction_confidence:null,history_timestamp:f.timestamp,history_type:S,history_cost:typeof f.cost=="number"?f.cost:null,history_notes:typeof f.notes=="string"?f.notes:null,history_duration:typeof f.duration=="number"?f.duration:null})}}}let u={completed:0,reset:1,skipped:2,triggered:3,trigger_replaced:4};for(let[,_]of c)_.sort((m,g)=>{let v=u[m.history_type??""]??99,b=u[g.history_type??""]??99;if(v!==b)return v-b;let x=m.object_name.localeCompare(g.object_name);return x!==0?x:m.task_name.localeCompare(g.task_name)});return i.map(_=>({date:_,events:c.get(_)??[]}))}var Zt=j`
  .cal-controls {
    display: flex;
    gap: 12px;
    align-items: center;
...
`;var te=class extends A{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter)}getCardSize(){return 6}get _lang(){return this.hass?.language||"en"}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){super.updated(e);let t=this.hass?.language;if(t&&!Fe(t)&&se(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}_onEventClick(e){if(e.history_timestamp){this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}));return}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}render(){if(!this.hass)return d;let e=this._lang,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.title,l=null;this._userFilter&&(l=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let c=new Date;c.setHours(0,0,0,0);let u=this._pastDays>0,_=u?Jt(this._objects,c,this._pastDays,l):Qt(this._objects,c,this._windowDays,l),m=Ie(c),g=this._windowDays===365||u,v=g?_.filter(f=>f.events.length>0):_,b=f=>{let P=`cal-status-${f.status}`,R=f.projected?"cal-event-projected":"",S=f.status==="overdue"&&f.days_until_due!=null?` (${Math.abs(f.days_until_due)}d ${s("overdue",e).toLowerCase()})`:"",k=f.projected&&f.interval_days?n`<span class="cal-event-recur">${f.interval_unit&&f.interval_unit!=="days"?`${f.interval_days} ${s("unit_"+f.interval_unit,e)}`:s("cal_every_n_days",e).replace("{n}",String(f.interval_days))}</span>`:d,z=f.schedule_type==="sensor_based",N=z?n`<ha-icon class="cal-event-icon cal-source-sensor"
          ...
          </span>`:d,Z=this._stats?.budget?.currency_symbol||"\u20AC",L=f.history_type?s(f.history_type,e):s(f.status,e);return n`
        <div class="cal-event ${R}"
          @click=${()=>this._onEventClick(f)}>
          ${N}
          <span class="cal-status-pill ${P}">${L}</span>
          <div class="cal-event-body">
            <div class="cal-event-title">${f.object_name} · ${f.task_name}${S}</div>
            ${K}
            ${k}
          </div>
          ${f.avg_cost!=null&&f.avg_cost>0?n`<span class="cal-event-cost">${f.avg_cost.toFixed(0)} ${Z}</span>`:d}
        </div>
      `},x=f=>{let[P,R,S]=f.date.split("-").map(Number),k=new Date(P,R-1,S),z=f.date===m,N=k.toLocaleDateString(e,{weekday:"short"}),K=k.toLocaleDateString(e,{month:"long"});return n`
      ...
      <ha-card .header=${a}>
        ...
        <div class="cal-rolling">
          ${v.length===0&&g?n`<div class="cal-empty">${s("cal_no_events",e)}</div>`:v.map(x)}
        </div>
      </ha-card>
    `}};te.styles=[Ue,Zt,j`
      :host { display: block; }
      ha-card { padding: 0; overflow: hidden; }
    `],p([w({attribute:!1})],te.prototype,"hass",2),p([h()],te.prototype,"_config",2),p([h()],te.prototype,"_objects",2),p([h()],te.prototype,"_stats",2),p([h()],te.prototype,"_windowDays",2),p([h()],te.prototype,"_pastDays",2),p([h()],te.prototype,"_userFilter",2),p([h()],te.prototype,"_unsub",2);var ls=[{value:7,label:"Week (7 days)"},{value:14,label:"Fortnight (14 days)"},{value:30,label:"Month (30 days, default)"},{value:365,label:"Year (365 days, empty days collapsed)"}],ye=class extends A{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"}}setConfig(e){this._config={...e}}_valueChanged(e,t){let i={...this._config,[e]:t};e==="show_window_chips"&&t===!0&&delete i.show_window_chips,e==="show_user_filter"&&t===!0&&delete i.show_user_filter,e==="title"&&(!t||typeof t=="string"&&t.trim()==="")&&delete i.title,e==="user_filter"&&t===""&&delete i.user_filter,this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}render(){let e=this._config.window_days??30,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.user_filter??"",l=this._config.title??"";return n`
      <div class="editor">
        <div class="row">
...
<body>
  ...
  `,p([w({attribute:!1})],J.prototype,"hass",2),p([w({attribute:!1})],J.prototype,"objects",2),p([h()],J.prototype,"_open",2),p([h()],J.prototype,"_loading",2),p([h()],J.prototype,"_error",2),p([h()],J.prototype,"_groupId",2),p([h()],J.prototype,"_name",2),p([h()],J.prototype,"_description",2),p([h()],J.prototype,"_selected",2);customElements.get("maintenance-group-dialog")||customElements.define("maintenance-group-dialog",J);var us=300,hs=140,yt=27;function ai(o,r){let e=o.trigger_config;if(!e)return d;let t=r.lang,i=o.trigger_entity_info,a=o.trigger_entity_infos,l=i?.friendly_name||e.entity_id||"\u2014",c=e.entity_id||"",u=e.entity_ids||(c?[c]:[]),_=i?.unit_of_measurement||"",m=o.trigger_current_value,g=e.type||"threshold",v=u.length>1;return n`
    <h3>${s("trigger",t)}</h3>
    <div class="trigger-card">
      ...
      ${m!=null?n`
            <div class="trigger-value-row">
              ...
              ${_?n`<span class="trigger-unit">${_}</span>`:d}
            </div>
          `:d}

      <div class="trigger-limits">
        ${g==="threshold"?n`
          ${e.trigger_above!=null?n`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("threshold_above",t)}: ${e.trigger_above} ${_}</span>`:d}
          ${e.trigger_below!=null?n`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("threshold_below",t)}: ${e.trigger_below} ${_}</span>`:d}
        ...
        `:d}
        ${g==="counter"?n`
          ${e.trigger_target_value!=null?n`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("target_value",t)}: ${e.trigger_target_value} ${_}</span>`:d}
        `:d}
        ${g==="state_change"?n`
        ...
        ${g==="compound"?n`
          ...
          `)}
        `:d}
        ${i?.min!=null?n`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${s("min",t)}: ${i.min} ${_}</span>`:d}
        ${i?.max!=null?n`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${s("max",t)}: ${i.max} ${_}</span>`:d}
      </div>
  ...
  `}function _s(o,r,e){let t=o.trigger_config;if(!t)return d;let i=t.type||"threshold",a=i==="counter"&&t.trigger_delta_mode,l=e.isCounterEntity(t),c=t.entity_id||"",u=e.detailStatsData.get(c)||[],_=[],m=!1;if(u.length>=2)for(let y of u){let I=y.val;a&&o.trigger_baseline_value!=null&&(I-=o.trigger_baseline_value);let U={ts:y.ts,val:I};!l&&y.min!=null&&y.max!=null&&(U.min=a&&o.trigger_baseline_value!=null?y.min-o.trigger_baseline_value:y.min,U.max=a&&o.trigger_baseline_value!=null?y.max-o.trigger_baseline_value:y.max,m=!0),_.push(U)}else for(let y of o.history)y.trigger_value!=null&&_.push({ts:new Date(y.timestamp).getTime(),val:y.trigger_value});if(o.trigger_current_value!=null){let y=o.trigger_current_value;a&&o.trigger_baseline_value!=null&&(y-=o.trigger_baseline_value),_.push({ts:Date.now(),val:y})}if(_.length<2&&c&&e.hasStatsService&&!e.detailStatsData.has(c))return n`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
      <ha-icon icon="mdi:chart-line" style="--mdc-icon-size:16px;margin-right:8px;"></ha-icon>
      ${s("loading_chart",e.lang)}
    </div>`;if(_.length<2)return d;_.sort((y,I)=>y.ts-I.ts);let g=us,v=hs,b=30,x=2,f=8,P=16,R=_.map(y=>y.val),S=Math.min(...R),k=Math.max(...R);if(m)for(let y of _)y.min!=null&&(S=Math.min(S,y.min)),y.max!=null&&(k=Math.max(k,y.max));t.trigger_above!=null&&(k=Math.max(k,t.trigger_above),S=Math.min(S,t.trigger_above)),t.trigger_below!=null&&(S=Math.min(S,t.trigger_below),k=Math.max(k,t.trigger_below));let z=null,N=null;if(i==="counter"&&t.trigger_target_value!=null){if(o.trigger_baseline_value!=null)z=o.trigger_baseline_value;else if(_.length>0){let y=[...o.history].filter(I=>I.type==="completed"||I.type==="reset").sort((I,U)=>new Date(U.timestamp).getTime()-new Date(I.timestamp).getTime())[0];if(y){let I=new Date(y.timestamp).getTime(),U=_[0],W=Math.abs(_[0].ts-I);for(let le of _){let ce=Math.abs(le.ts-I);ce<W&&(U=le,W=ce)}z=U.val}else z=_[0].val}z!=null?(N=z+t.trigger_target_value,k=Math.max(k,N),S=Math.min(S,z)):(k=Math.max(k,t.trigger_target_value),S=Math.min(S,0))}a&&o.trigger_baseline_value!=null&&(S=Math.min(S,0));let K=k-S||1;S-=K*.1,k+=K*.1;let Z=_[0].ts,L=_[_.length-1].ts,E=L-Z||1,D=y=>b+(y-Z)/E*(g-b-x),H=y=>f+(1-(y-S)/(k-S))*(v-f-P),Ze=_.map(y=>`${D(y.ts).toFixed(1)},${H(y.val).toFixed(1)}`).join(" "),hi=`M${D(_[0].ts).toFixed(1)},${v-P} `+_.map(y=>`L${D(y.ts).toFixed(1)},${H(y.val).toFixed(1)}`).join(" ")+` L${D(_[_.length-1].ts).toFixed(1)},${v-P} Z`,Xe="";if(m){let y=_.filter(I=>I.min!=null&&I.max!=null);if(y.length>=2){let I=y.map(W=>`${D(W.ts).toFixed(1)},${H(W.max).toFixed(1)}`),U=[...y].reverse().map(W=>`${D(W.ts).toFixed(1)},${H(W.min).toFixed(1)}`);Xe=`M${I[0]} `+I.slice(1).map(W=>`L${W}`).join(" ")+` L${U[0]} `+U.slice(1).map(W=>`L${W}`).join(" ")+" Z"}}let xt=_[_.length-1],_i=D(xt.ts),gi=H(xt.val),$t=y=>Math.abs(y)>=1e4?(y/1e3).toFixed(0)+"k":y>=1e3?(y/1e3).toFixed(1)+"k":y.toFixed(y<10?1:0),mi=$t(k),vi=$t(S),bi=o.history.filter(y=>["completed","skipped","reset"].includes(y.type)).map(y=>({ts:new Date(y.timestamp).getTime(),type:y.type})).filter(y=>y.ts>=Z&&y.ts<=L),et=_;if(_.length>yt){let y=(_.length-1)/(yt-1);et=[];for(let I=0;I<yt;I++)et.push(_[Math.round(I*y)])}return n`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${g} ${v}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_sparkline",e.lang)}">
        ...
        <text x="${b-3}" y="${v-P+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${vi}</text>
        <text x="${b}" y="${v-1}" text-anchor="start" fill="var(--secondary-text-color)" font-size="7">${new Date(Z).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        <text x="${g-x}" y="${v-1}" text-anchor="end" fill="var(--secondary-text-color)" font-size="7">${new Date(L).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        ...
        ${o.degradation_rate!=null&&o.degradation_trend!=="stable"&&o.degradation_trend!=="insufficient_data"&&_.length>=2?(()=>{let y=_[_.length-1],I=30,U=y.ts+I*864e5,W=y.val+o.degradation_rate*I,le=Math.min(U,L+(L-Z)*.3),ce=Math.max(S,Math.min(k,W)),fi=D(y.ts),yi=H(y.val),xi=D(le),$i=H(ce);return O`<line x1="${fi.toFixed(1)}" y1="${yi.toFixed(1)}" x2="${xi.toFixed(1)}" y2="${$i.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():d}
        ${i==="threshold"&&t.trigger_above!=null?O`<line x1="${b}" y1="${H(t.trigger_above).toFixed(1)}" x2="${g}" y2="${H(t.trigger_above).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${g-2}" y="${H(t.trigger_above)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25B2 ${t.trigger_above}</text>`:d}
        ${i==="threshold"&&t.trigger_below!=null?O`<line x1="${b}" y1="${H(t.trigger_below).toFixed(1)}" x2="${g}" y2="${H(t.trigger_below).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${g-2}" y="${H(t.trigger_below)+11}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25BC ${t.trigger_below}</text>`:d}
        ${i==="counter"&&N!=null?O`<line x1="${b}" y1="${H(N).toFixed(1)}" x2="${g}" y2="${H(N).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${g-2}" y="${H(N)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">${s("target_value",e.lang)}: +${t.trigger_target_value}</text>`:d}
        ${i==="counter"&&z!=null?O`<line x1="${b}" y1="${H(z).toFixed(1)}" x2="${g}" y2="${H(z).toFixed(1)}" stroke="var(--secondary-text-color)" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
        ...
        ${bi.map(y=>{let I=D(y.ts),U=y.type==="completed"?"var(--success-color, #4caf50)":y.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return O`
            <line x1="${I.toFixed(1)}" y1="${f}" x2="${I.toFixed(1)}" y2="${v-P}" stroke="${U}" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
          ...
          `})}
        ${et.map(y=>{let I=D(y.ts),U=H(y.val),W=new Date(y.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),le=`${y.val.toFixed(1)} ${r}`;return m&&y.min!=null&&y.max!=null&&(le+=` (${y.min.toFixed(1)}\u2013${y.max.toFixed(1)})`),O`<circle cx="${I.toFixed(1)}" cy="${U.toFixed(1)}" r="8" fill="transparent" tabindex="0"
            @mouseenter=${ce=>si(ce,`${W}
${le}`,e.setTooltip)}
            @focus=${ce=>si(ce,`${W}
${le}`,e.setTooltip)}
...
`).map(y=>n`<div>${y}</div>`)}
      ...
      <div class="prediction-grid">
        ...
        ${i?n`
          <div class="prediction-item">
            ...
            ${o.threshold_prediction_date?n`<span class="prediction-date">${X(o.threshold_prediction_date,r)}</span>`:d}
  ...
  `}function ms(o,r,e,t,i){let b=Math.max(e,t,r,1)*1.3,x=50,f=[];for(let L=0;L<=x;L++){let E=L/x*b,D=1-Math.exp(-Math.pow(E/r,o)),H=32+E/b*260,Ze=136-D*128;f.push([H,Ze])}let P=f.map(([L,E])=>`${L.toFixed(1)},${E.toFixed(1)}`).join(" "),R="M32,136 "+f.map(([L,E])=>`L${L.toFixed(1)},${E.toFixed(1)}`).join(" ")+` L${f[x][0].toFixed(1)},136 Z`,S=32+e/b*260,k=1-Math.exp(-Math.pow(e/r,o)),z=136-k*128,N=((1-k)*100).toFixed(0),K=32+t/b*260,Z=[0,.25,.5,.75,1];return n`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_weibull",i)}">
        ${Z.map(L=>{let E=136-L*128;return O`
            <line x1="${32}" y1="${E.toFixed(1)}" x2="${292}" y2="${E.toFixed(1)}"
              stroke="var(--divider-color)" stroke-width="0.5" stroke-dasharray="${L===.5?"4,3":d}" />
  ...
  `}function bs(o,r,e){let t=o.confidence_interval_low,i=o.confidence_interval_high,a=r.suggested_interval??r.interval_days??0,l=r.interval_days??0,c=Math.max(0,t-5),_=i+5-c,m=(t-c)/_*100,g=(i-t)/_*100,v=(a-c)/_*100,b=l>0?(l-c)/_*100:-1;return n`
  ...
  `}function di(o,r){return fs(o,r)}function fs(o,r){let e=o.seasonal_factors??o.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return d;let t=o.interval_analysis?.seasonal_reason,i=new Date().getMonth(),a=300,l=100,c=8,_=l-c-4,m=Math.max(...e,1.5),g=a/12,v=g*.65,b=c+_-1/m*_;return n`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${a} ${l}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",r)}">
        ...
        ${e.map((x,f)=>{let P=x/m*_,R=f*g+(g-v)/2,S=c+_-P,k=f===i,z=x<1?"var(--success-color, #4caf50)":x>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return O`
            <rect x="${R.toFixed(1)}" y="${S.toFixed(1)}"
              width="${v.toFixed(1)}" height="${P.toFixed(1)}"
  ...
  `}function $s(o,r,e){let t=o.history.filter(E=>E.type==="completed"&&(E.cost!=null||E.duration!=null)).map(E=>({ts:new Date(E.timestamp).getTime(),cost:E.cost??0,duration:E.duration??0})).sort((E,D)=>E.ts-D.ts);if(t.length<2)return d;let i=t.some(E=>E.cost>0),a=t.some(E=>E.duration>0);if(!i&&!a)return d;let l=e!=="duration"&&i,c=e!=="cost"&&a,u=l||!c&&i,_=c||!l&&a,m=ys,g=xs,v=u?32:8,b=_?32:8,x=8,f=20,P=m-v-b,R=g-x-f,S=Math.max(...t.map(E=>E.cost))||1,k=Math.max(...t.map(E=>E.duration))||1,z=Math.min(20,P/t.length*.6),N=P/t.length,K=E=>v+N*E+N/2,Z=E=>x+R-E/S*R,L=E=>x+R-E/k*R;return n`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${m} ${g}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_history",r)}">
        ...
        <text x="${m-b}" y="${g-2}" text-anchor="end" fill="var(--secondary-text-color)" font-size="7">${new Date(t[t.length-1].ts).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        ${u?O`
  ...
  `}var ws=60,ks=20,ui=30,T=class extends A{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._sparklineTooltip=null;this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._adminPanelUserIds=[];this._operatorWriteEnabled=!1;this._defaultWarningDays=7;this._actionLoading=!1;this._moreMenuOpen=!1;this._toastMessage="";this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab="dashboard";this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._objectSortMode="alphabetical";this._groupByMode="none";this._objectViewMode="cards";this._objectsTableColumns=Be;this._showArchived=!1;this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._deepLinkHandled=!1;this._onDialogEvent=async()=>{try{await this._loadData()}catch{}};this._onCalendarLlCustom=e=>{let t=e.detail;t?.type==="maintenance-supporter:open-task"&&t.entry_id&&t.task_id&&(e.stopPropagation(),this._showTask(t.entry_id,t.task_id))};this._onHistoryEntrySaved=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"en"}get _isOperator(){let e=this.hass?.user;return e?e.is_admin?!1:!(this._operatorWriteEnabled&&this._adminPanelUserIds.includes(e.id)):!0}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler);let e=localStorage.getItem("maintenance_supporter_sort");e&&["due_date","object","type","task_name","area","assigned_user","group"].includes(e)&&(this._sortMode=e);let t=localStorage.getItem("maintenance_supporter_object_sort");t&&["alphabetical","due_soonest","task_count"].includes(t)&&(this._objectSortMode=t);let i=localStorage.getItem("maintenance_supporter_groupby");i&&["none","area","group","user"].includes(i)&&(this._groupByMode=i);let a=localStorage.getItem("maintenance_supporter_object_view");(a==="cards"||a==="table")&&(this._objectViewMode=a)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){super.updated(e);let t=this.hass?.language;if(t&&!Fe(t)&&se(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new Ke(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new pe(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,i,a,l]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);if(e&&(this._objects=e.objects),t&&(this._stats=t),i&&(this._budget=i),a&&(this._groups=a.groups||{}),l){let c=l;this._features=c.features,this._adminPanelUserIds=c.admin_panel_user_ids||[],this._operatorWriteEnabled=c.operator_write_enabled??!1;let u=c.general?.default_warning_days;typeof u=="number"&&u>=0&&u<=365&&(this._defaultWarningDays=u),this._objectsTableColumns=Ce(c.objects_table_columns)}this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("ms_action"),i=()=>{let m=window.location.pathname+window.location.hash;history.replaceState(history.state,"",m)};if(t==="add_object"){this._deepLinkHandled=!0,i(),requestAnimationFrame(()=>{this.shadowRoot?.querySelector("maintenance-object-dialog")?.openCreate()});return}if(t==="open_vacation"||t==="open_budget"||t==="open_groups"||t==="open_settings"){this._deepLinkHandled=!0,i(),this._overviewTab="settings",requestAnimationFrame(()=>{let m=this.shadowRoot?.querySelector("maintenance-settings-view"),g=t.replace("open_","");m?.scrollToSection?.(g)});return}let a=e.get("entry_id");if(!a)return;this._deepLinkHandled=!0;let l=e.get("task_id"),c=e.get("action"),u=window.location.pathname+window.location.hash;history.replaceState(history.state,"",u);let _=this._getObject(a);if(!_){this._showOverview();return}if(l){let m=_.tasks.find(g=>g.id===l);if(!m){this._showObject(a);return}this._showTask(a,l),c==="complete"?requestAnimationFrame(()=>{this._openCompleteDialog(a,l,m.name,this._features.checklists?m.checklist:void 0,this._features.adaptive&&!!m.adaptive_config?.enabled)}):c==="quick_complete"&&requestAnimationFrame(()=>{this._handleQuickComplete(a,l,m)})}else this._showObject(a)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let i=await this._statsService.getDetailStats(e,t),a=new Map(this._detailStatsData);a.set(e,i),this._detailStatsData=a}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let i of this._objects)for(let a of i.tasks){let l=a.trigger_config?.entity_id;l&&e.push({entityId:l,isCounter:this._isCounterEntity(a.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let g of this._objects)for(let v of g.tasks){if(!this._showArchived&&v.archived||this._filterStatus&&v.status!==this._filterStatus)continue;if(this._filterUser){let x=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(v.responsible_user_id!==x)continue}let b=[];for(let x of Object.values(this._groups))x.task_refs?.some(f=>f.entry_id===g.entry_id&&f.task_id===v.id)&&b.push(x.name);e.push({entry_id:g.entry_id,task_id:v.id,object_name:g.object.name,task_name:v.name,type:v.type,schedule_type:v.schedule_type,status:v.status,days_until_due:v.days_until_due??null,next_due:v.next_due??null,trigger_active:v.trigger_active,trigger_current_value:v.trigger_current_value??null,trigger_current_delta:v.trigger_current_delta??null,trigger_config:v.trigger_config??null,trigger_entity_info:v.trigger_entity_info??null,times_performed:v.times_performed,total_cost:v.total_cost,interval_days:v.interval_days??null,interval_unit:v.interval_unit??null,interval_anchor:v.interval_anchor??null,is_done:v.is_done??!1,archived:v.archived??!1,history:v.history||[],enabled:v.enabled,nfc_tag_id:v.nfc_tag_id??null,area_id:g.object.area_id??null,responsible_user_id:v.responsible_user_id??null,group_names:b})}let t={overdue:0,triggered:1,due_soon:2,ok:3},i=(g,v)=>(t[g.status]??9)-(t[v.status]??9),a=(g,v)=>(g.days_until_due??99999)-(v.days_until_due??99999),l=(g,v)=>i(g,v)||a(g,v),c=g=>g.area_id&&this.hass?.areas?.[g.area_id]?.name||"",u=g=>g.responsible_user_id&&this._userService?.getUserName(g.responsible_user_id)||"",_=g=>g.group_names[0]||"",m={due_date:l,object:(g,v)=>g.object_name.localeCompare(v.object_name)||l(g,v),type:(g,v)=>g.type.localeCompare(v.type)||l(g,v),task_name:(g,v)=>g.task_name.localeCompare(v.task_name),area:(g,v)=>{let b=c(g),x=c(v);return!b&&x?1:b&&!x?-1:b.localeCompare(x)||l(g,v)},assigned_user:(g,v)=>{let b=u(g),x=u(v);return!b&&x?1:b&&!x?-1:b.localeCompare(x)||l(g,v)},group:(g,v)=>{let b=_(g),x=_(v);return!b&&x?1:b&&!x?-1:b.localeCompare(x)||l(g,v)}};return e.sort(m[this._sortMode]),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(a=>a.id===t)}_pushPanelState(e,t,i){let a={msp_view:e,msp_entry:t||null,msp_task:i||null};history.pushState(a,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let i=this._getTask(t.msp_entry,t.msp_task);i?.trigger_config?.entity_id&&this._fetchDetailStats(i.trigger_config.entity_id,this._isCounterEntity(i.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1,this._scrollContentToTop()}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null,this._scrollContentToTop()}_filterByStatus(e){this._filterStatus=e,this._overviewTab!=="dashboard"&&(this._overviewTab="dashboard"),this._scrollContentToTop()}_scrollContentToTop(){requestAnimationFrame(()=>{let e=this.shadowRoot?.querySelector(".content");e&&e.scrollTo({top:0,behavior:"smooth"})})}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null,this._scrollContentToTop()}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null,this._scrollContentToTop();let i=this._getTask(e,t);if(i?.trigger_config?.entity_id){let a=i.trigger_config.entity_id,l=this._isCounterEntity(i.trigger_config);this._fetchDetailStats(a,l)}}_showToast(e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}async _deleteObject(e){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("delete",this._lang),message:s("confirm_delete_object",this._lang),confirmText:s("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _deleteTask(e,t){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("delete",this._lang),message:s("confirm_delete_task",this._lang),confirmText:s("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _toggleArchiveTask(e,t,i){this._actionLoading=!0;try{await this.hass.connection.sendMessagePromise({type:i?"maintenance_supporter/task/unarchive":"maintenance_supporter/task/archive",entry_id:e,task_id:t}),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}finally{this._actionLoading=!1}}async _toggleArchiveObject(e,t){if(!(!t&&!await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("archive_object",this._lang),message:s("confirm_archive_object",this._lang),confirmText:s("archive_object",this._lang)})))try{await this.hass.connection.sendMessagePromise({type:t?"maintenance_supporter/object/unarchive":"maintenance_supporter/object/archive",entry_id:e}),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _skipTask(e,t,i){this._actionLoading=!0;try{let a={type:"maintenance_supporter/task/skip",entry_id:e,task_id:t};i&&(a.reason=i),await this.hass.connection.sendMessagePromise(a),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}finally{this._actionLoading=!1}}async _resetTask(e,t,i){this._actionLoading=!0;try{let a={type:"maintenance_supporter/task/reset",entry_id:e,task_id:t};i&&(a.date=i),await this.hass.connection.sendMessagePromise(a),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}finally{this._actionLoading=!1}}async _applySuggestion(e,t,i){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:i}),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}_openSeasonalOverrides(e){let t=this.shadowRoot.querySelector("maintenance-seasonal-overrides-dialog");if(!t||!this._selectedEntryId)return;let i=e.adaptive_config?.seasonal_overrides;t.open(this._selectedEntryId,e.id,i)}async _reanalyzeInterval(e,t){try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:e,task_id:t});i.recommended_interval?this._showToast(`${s("reanalyze_result",this._lang)}: ${i.recommended_interval} ${s("days",this._lang)} (${s(`confidence_${i.confidence}`,this._lang)}, ${i.data_points} ${s("data_points",this._lang)})`):this._showToast(s("reanalyze_insufficient_data",this._lang)),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _promptSkipTask(e,t){let i=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!i)return;let a=await i.prompt({title:s("skip",this._lang),message:s("skip_reason_prompt",this._lang),confirmText:s("skip",this._lang),inputLabel:s("reason_optional",this._lang),inputType:"text"});a.confirmed&&this._skipTask(e,t,a.value||void 0)}async _promptResetTask(e,t){let i=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!i)return;let a=await i.prompt({title:s("reset",this._lang),message:s("reset_date_prompt",this._lang),confirmText:s("reset",this._lang),inputLabel:s("reset_date_optional",this._lang),inputType:"date"});a.confirmed&&this._resetTask(e,t,a.value||void 0)}_dismissSuggestion(e,t){e&&t&&this._dismissedSuggestions.add(`${e}_${t}`),this.requestUpdate()}async _handleQuickComplete(e,t,i){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/quick_complete",entry_id:e,task_id:t}),this._showToast(s("quick_complete_success",this._lang))}catch(a){(a?.code||"")==="no_defaults"?this._openCompleteDialog(e,t,i.name,this._features.checklists?i.checklist:void 0,this._features.adaptive&&!!i.adaptive_config?.enabled):this._showToast(s("action_error",this._lang))}}_openCompleteDialog(e,t,i,a,l){let c=this.shadowRoot.querySelector("maintenance-complete-dialog");c&&(c.entryId=e,c.taskId=t,c.taskName=i,c.lang=this._lang,c.checklist=a||[],c.adaptiveEnabled=!!l,c.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,i,a){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,i,a)}render(){return n`
    ...
    `}_renderDashboard(){let e=this._stats,t=this._taskRows,i=this._lang,a=this._isOperator,l=this._objects.reduce((c,u)=>c+u.tasks.filter(_=>_.archived).length,0);return n`
    ...
    `}_renderGroupedTasks(e,t){let i=new Map,a=s("unassigned",t);for(let u of e){let _=[];this._groupByMode==="area"?_=[(u.area_id?this.hass?.areas?.[u.area_id]?.name:null)||a]:this._groupByMode==="user"?_=[(u.responsible_user_id?this._userService?.getUserName(u.responsible_user_id):null)||a]:this._groupByMode==="group"&&(_=u.group_names.length>0?u.group_names:[a]);for(let m of _)i.has(m)||i.set(m,[]),i.get(m).push(u)}let l=[...i.entries()].sort(([u],[_])=>u===a&&_!==a?1:_===a&&u!==a?-1:u.localeCompare(_)),c=this._groupByMode==="area"?"mdi:map-marker-outline":this._groupByMode==="group"?"mdi:folder-outline":"mdi:account-outline";return n`
      ...
      `)}
    `}_warrantyLabel(e,t,i){return e.kind==="expired"?s("warranty_expired",i):e.kind==="expiring"?s("warranty_expires_in",i).replace("{days}",String(e.days??0)):s("warranty_valid_until",i).replace("{date}",X(t,i))}_renderWarrantyMeta(e,t){let i=ft(e);return n`<p class="meta">${s("warranty",t)}:
      <span class="warranty-chip warranty-${i.kind}">${this._warrantyLabel(i,e,t)}</span></p>`}_renderAllObjects(){let e=this._lang,t=this._isOperator,i=this._objectViewMode==="table"&&!this.narrow,a=this._objects.filter(m=>m.object.archived).length,l=m=>{let g=1/0;for(let v of m.tasks){let b=v.days_until_due;b!=null&&b<g&&(g=b)}return g},c=this._objects.filter(m=>this._showArchived||!m.object.archived);this._objectSortMode==="alphabetical"?c.sort((m,g)=>m.object.name.localeCompare(g.object.name)):this._objectSortMode==="task_count"?c.sort((m,g)=>g.tasks.length-m.tasks.length||m.object.name.localeCompare(g.object.name)):c.sort((m,g)=>l(m)-l(g)||m.object.name.localeCompare(g.object.name));let u=()=>{let m=new Map;for(let g of c){let v=g.object.area_id,b=v?this.hass?.areas?.[v]?.name||s("unassigned",e):s("no_area",e);m.has(b)||m.set(b,[]),m.get(b).push(g)}return new Map([...m.entries()].sort(([g],[v])=>g.localeCompare(v)))},_=m=>{let g=m.tasks.some(v=>v.status==="overdue"||v.status==="triggered");return n`
        <div class="object-card${g?" object-card-overdue":""}" @click=${()=>this._showObject(m.entry_id)}>
          ${g?n`<span class="overdue-dot" title="${s("has_overdue",e)}"></span>`:d}
      ...
      ${i?this._renderObjectsTable(c):this._groupByMode==="area"?n`
          ${[...u().entries()].map(([m,g])=>n`
            <details class="group-section" open>
              ...
              <div class="objects-grid">${g.map(_)}</div>
            </details>
          `)}
        `:n`<div class="objects-grid">${c.map(_)}</div>`}
    ...
    `}_renderOverviewRow(e){let t=this._lang,i=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,a=0,l=Oe.ok,c=!1;if(i&&e.days_until_due!==null){let g=bt(e.interval_days,e.days_until_due,e.interval_unit);a=g.pct,c=g.overflow,e.status==="overdue"?l=Oe.overdue:e.status==="due_soon"&&(l=Oe.due_soon)}let u=e.area_id?this.hass?.areas?.[e.area_id]?.name:null,_=e.responsible_user_id?this._userService?.getUserName(e.responsible_user_id):null,m=e.group_names.length>0||u||_;return n`
    ...
    `}_openCompleteDialogForRow(e){let i=this._objects.find(a=>a.entry_id===e.entry_id)?.tasks.find(a=>a.id===e.task_id);this._openCompleteDialog(e.entry_id,e.task_id,e.task_name,this._features.checklists?i?.checklist:void 0,this._features.adaptive&&!!i?.adaptive_config?.enabled)}_renderTriggerProgress(e){let t=e.trigger_config??null;if(!t)return d;let i=t.type||"threshold",a=e.trigger_entity_info?.unit_of_measurement??"",l=0,c="";if(i==="threshold"){let m=e.trigger_current_value??null;if(m==null)return d;let g=t.trigger_above,v=t.trigger_below;if(g!=null){let b=v??0,x=g-b||1;l=Math.min(100,Math.max(0,(m-b)/x*100)),c=`${m.toFixed(1)} / ${g} ${a}`}else if(v!=null){let x=e.trigger_entity_info?.max??(v*2||100),f=x-v||1;l=Math.min(100,Math.max(0,(x-m)/f*100)),c=`${m.toFixed(1)} / ${v} ${a}`}else return d}else if(i==="counter"){let m=t.trigger_target_value||1,v=e.trigger_current_delta??null??e.trigger_current_value??null;if(v==null)return d;l=Math.min(100,Math.max(0,v/m*100)),c=`${v.toFixed(1)} / ${m} ${a}`}else if(i==="state_change"){let m=t.trigger_target_changes||1,g=e.trigger_current_value??null;if(g==null)return d;l=Math.min(100,Math.max(0,g/m*100)),c=`${Math.round(g)} / ${m}`}else if(i==="runtime"){let m=t.trigger_runtime_hours||100,g=e.trigger_current_value??null;if(g==null)return d;l=Math.min(100,Math.max(0,g/m*100)),c=`${g.toFixed(1)}h / ${m}h`}else if(i==="compound"){let m=t.compound_logic||t.operator||"AND",g=t.conditions?.length||0;c=`${m} (${g})`,l=e.trigger_active?100:0}else return d;let u=l>=100,_=l>90?"var(--error-color, #f44336)":l>70?"var(--warning-color, #ff9800)":"var(--primary-color)";return n`
      <div class="trigger-progress">
        <div class="trigger-progress-bar">
          <div class="trigger-progress-fill${u?" overflow":""}" style="width:${l}%;background:${_}"></div>
        </div>
        <span class="trigger-progress-label">${c}</span>
      </div>
    `}_renderMiniSparkline(e){if(!e.trigger_config?.entity_id)return d;let t=e.trigger_config.entity_id,i=this._miniStatsData.get(t)||[],a=[];if(i.length>=2)a=i.map(k=>({ts:k.ts,val:k.val}));else{if(!e.history)return d;for(let k of e.history)k.trigger_value!=null&&a.push({ts:new Date(k.timestamp).getTime(),val:k.trigger_value})}if(e.trigger_current_value!=null&&a.push({ts:Date.now(),val:e.trigger_current_value}),a.length<2)return d;a.sort((k,z)=>k.ts-z.ts);let l=ws,c=ks,u=a.map(k=>k.val),_=Math.min(...u),m=Math.max(...u),g=m-_||1;_-=g*.1,m+=g*.1;let v=a[0].ts,x=a[a.length-1].ts-v||1,f=k=>(k-v)/x*l,P=k=>2+(1-(k-_)/(m-_))*(c-4),R=a;if(R.length>ui){let k=Math.ceil(R.length/ui);R=R.filter((z,N)=>N%k===0||N===R.length-1)}let S=R.map(k=>`${f(k.ts).toFixed(1)},${P(k.val).toFixed(1)}`).join(" ");return n`
    ...
    `}_renderDaysProgress(e){let t=this._lang;if(e.days_until_due==null||!e.interval_days||e.interval_days<=0)return d;let{pct:i,overflow:a}=bt(e.interval_days,e.days_until_due,e.interval_unit),l="var(--success-color, #4caf50)";return e.status==="overdue"?l="var(--error-color, #f44336)":e.status==="due_soon"&&(l="var(--warning-color, #ff9800)"),n`
      <div class="days-progress">
        <div class="days-progress-labels">
          <span>${e.last_performed?`${s("last_performed",t)}: ${X(e.last_performed,t)}`:""}</span>
          <span>${e.next_due?`${s("next_due",t)}: ${X(e.next_due,t)}`:""}</span>
        </div>
    ...
    `}_renderObjectDetail(){if(!this._selectedEntryId)return d;let e=this._getObject(this._selectedEntryId);if(!e)return n`<p>Object not found.</p>`;let t=e.object,i=this._lang,a=this._isOperator,l=e.tasks.filter(u=>u.archived).length,c=e.tasks.filter(u=>this._showArchived||!u.archived);return n`
      <div class="detail-section">
        ...
        ${t.installation_date?n`<p class="meta">${s("installed",i)}: ${X(t.installation_date,i)}</p>`:d}
        ...
        ${e.tasks.length===0?n`<div class="empty-state-centered">
            ...
            </div>`:n`<div class="task-table">${[...c].sort((u,_)=>{let m={overdue:0,triggered:1,due_soon:2,ok:3};return(m[u.status]??9)-(m[_.status]??9)||(u.days_until_due??99999)-(_.days_until_due??99999)}).map(u=>n`
              <div class="task-row${u.enabled?"":" task-disabled"}">
                ...
                <span class="row-actions">
                  <mwc-icon-button class="btn-complete" title="${s("complete",i)}" @click=${_=>{_.stopPropagation(),this._openCompleteDialog(e.entry_id,u.id,u.name,this._features.checklists?u.checklist:void 0,this._features.adaptive&&!!u.adaptive_config?.enabled)}}>
                    <ha-icon icon="mdi:check"></ha-icon>
                  </mwc-icon-button>
                  <mwc-icon-button class="btn-skip" title="${s("skip",i)}" .disabled=${this._actionLoading} @click=${_=>{_.stopPropagation(),this._promptSkipTask(e.entry_id,u.id)}}>
                    <ha-icon icon="mdi:skip-next"></ha-icon>
                  </mwc-icon-button>
                </span>
              </div>
      ...
      <div class="tab-content overview-tab">
        ...
        <div class="two-column-layout ${l?"":"single-column"}">
          ...
          <div class="right-column">
            ${pi(e,t,this._costDurationToggle,_=>{this._costDurationToggle=_})}
          </div>
        </div>
        ${c?ni(e,t):d}
        ${u?n`
