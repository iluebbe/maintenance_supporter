...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function di(r,a,e){let t=r.trigger_config;if(!t)return c;let i=t.type||"threshold",n=i==="counter"&&t.trigger_delta_mode,o=e.isCounterEntity(t),d=t.entity_id||"",p=e.detailStatsData.get(d)||[],g=[],m=!1;if(p.length>=2)for(let h of p){let k=h.val;n&&r.trigger_baseline_value!=null&&(k-=r.trigger_baseline_value);let R={ts:h.ts,val:k};!o&&h.min!=null&&h.max!=null&&(R.min=n&&r.trigger_baseline_value!=null?h.min-r.trigger_baseline_value:h.min,R.max=n&&r.trigger_baseline_value!=null?h.max-r.trigger_baseline_value:h.max,m=!0),g.push(R)}else for(let h of r.history)h.trigger_value!=null&&g.push({ts:new Date(h.timestamp).getTime(),val:h.trigger_value});if(r.trigger_current_value!=null){let h=r.trigger_current_value;n&&r.trigger_baseline_value!=null&&(h-=r.trigger_baseline_value),g.push({ts:Date.now(),val:h})}if(g.length<2&&d&&e.hasStatsService&&!e.detailStatsData.has(d))return l`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
    ...
    </div>`;if(g.length<2)return c;g.sort((h,k)=>h.ts-k.ts);let v=li,b=ci,f=30,w=2,T=8,H=16,C=g.map(h=>h.val),E=Math.min(...C),y=Math.max(...C);if(m)for(let h of g)h.min!=null&&(E=Math.min(E,h.min)),h.max!=null&&(y=Math.max(y,h.max));t.trigger_above!=null&&(y=Math.max(y,t.trigger_above),E=Math.min(E,t.trigger_above)),t.trigger_below!=null&&(E=Math.min(E,t.trigger_below),y=Math.max(y,t.trigger_below));let z=null,O=null;if(i==="counter"&&t.trigger_target_value!=null){if(r.trigger_baseline_value!=null)z=r.trigger_baseline_value;else if(g.length>0){let h=[...r.history].filter(k=>k.type==="completed"||k.type==="reset").sort((k,R)=>new Date(R.timestamp).getTime()-new Date(k.timestamp).getTime())[0];if(h){let k=new Date(h.timestamp).getTime(),R=g[0],N=Math.abs(g[0].ts-k);for(let J of g){let Z=Math.abs(J.ts-k);Z<N&&(R=J,N=Z)}z=R.val}else z=g[0].val}z!=null?(O=z+t.trigger_target_value,y=Math.max(y,O),E=Math.min(E,z)):(y=Math.max(y,t.trigger_target_value),E=Math.min(E,0))}n&&r.trigger_baseline_value!=null&&(E=Math.min(E,0));let G=y-E||1;E-=G*.1,y+=G*.1;let W=g[0].ts,L=g[g.length-1].ts,x=L-W||1,I=h=>f+(h-W)/x*(v-f-w),M=h=>T+(1-(h-E)/(y-E))*(b-T-H),Ee=g.map(h=>`${I(h.ts).toFixed(1)},${M(h.val).toFixed(1)}`).join(" "),wt=`M${I(g[0].ts).toFixed(1)},${b-H} `+g.map(h=>`L${I(h.ts).toFixed(1)},${M(h.val).toFixed(1)}`).join(" ")+` L${I(g[g.length-1].ts).toFixed(1)},${b-H} Z`,Ae="";if(m){let h=g.filter(k=>k.min!=null&&k.max!=null);if(h.length>=2){let k=h.map(N=>`${I(N.ts).toFixed(1)},${M(N.max).toFixed(1)}`),R=[...h].reverse().map(N=>`${I(N.ts).toFixed(1)},${M(N.min).toFixed(1)}`);Ae=`M${k[0]} `+k.slice(1).map(N=>`L${N}`).join(" ")+` L${R[0]} `+R.slice(1).map(N=>`L${N}`).join(" ")+" Z"}}let Oe=g[g.length-1],kt=I(Oe.ts),Et=M(Oe.val),Ue=h=>Math.abs(h)>=1e4?(h/1e3).toFixed(0)+"k":h>=1e3?(h/1e3).toFixed(1)+"k":h.toFixed(h<10?1:0),At=Ue(y),St=Ue(E),Tt=r.history.filter(h=>["completed","skipped","reset"].includes(h.type)).map(h=>({ts:new Date(h.timestamp).getTime(),type:h.type})).filter(h=>h.ts>=W&&h.ts<=L),Se=g;if(g.length>qe){let h=(g.length-1)/(qe-1);Se=[];for(let k=0;k<qe;k++)Se.push(g[Math.round(k*h)])}return l`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${v} ${b}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_sparkline",e.lang)}">
        ...
        ${r.degradation_rate!=null&&r.degradation_trend!=="stable"&&r.degradation_trend!=="insufficient_data"&&g.length>=2?(()=>{let h=g[g.length-1],k=30,R=h.ts+k*864e5,N=h.val+r.degradation_rate*k,J=Math.min(R,L+(L-W)*.3),Z=Math.max(E,Math.min(y,N)),Mt=I(h.ts),Ct=M(h.val),zt=I(J),It=M(Z);return D`<line x1="${Mt.toFixed(1)}" y1="${Ct.toFixed(1)}" x2="${zt.toFixed(1)}" y2="${It.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():c}
        ...
        ${i==="counter"&&O!=null?D`<line x1="${f}" y1="${M(O).toFixed(1)}" x2="${v}" y2="${M(O).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
        ...
        ${Tt.map(h=>{let k=I(h.ts),R=h.type==="completed"?"var(--success-color, #4caf50)":h.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return D`
          ...
          `})}
        ${Se.map(h=>{let k=I(h.ts),R=M(h.val),N=new Date(h.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),J=`${h.val.toFixed(1)} ${a}`;return m&&h.min!=null&&h.max!=null&&(J+=` (${h.min.toFixed(1)}\u2013${h.max.toFixed(1)})`),D`<circle cx="${k.toFixed(1)}" cy="${R.toFixed(1)}" r="8" fill="transparent" tabindex="0"
            @mouseenter=${Z=>gt(Z,`${N}
${J}`,e.setTooltip)}
            @focus=${Z=>gt(Z,`${N}
${J}`,e.setTooltip)}
...
`).map(h=>l`<div>${h}</div>`)}
  ...
  `}function pi(r,a,e,t,i){let f=Math.max(e,t,a,1)*1.3,w=50,T=[];for(let L=0;L<=w;L++){let x=L/w*f,I=1-Math.exp(-Math.pow(x/a,r)),M=32+x/f*260,Ee=136-I*128;T.push([M,Ee])}let H=T.map(([L,x])=>`${L.toFixed(1)},${x.toFixed(1)}`).join(" "),C="M32,136 "+T.map(([L,x])=>`L${L.toFixed(1)},${x.toFixed(1)}`).join(" ")+` L${T[w][0].toFixed(1)},136 Z`,E=32+e/f*260,y=1-Math.exp(-Math.pow(e/a,r)),z=136-y*128,O=((1-y)*100).toFixed(0),G=32+t/f*260,W=[0,.25,.5,.75,1];return l`
