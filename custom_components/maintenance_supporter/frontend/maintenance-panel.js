...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function li(r,a,e){let t=r.trigger_config;if(!t)return c;let i=t.type||"threshold",n=i==="counter"&&t.trigger_delta_mode,o=e.isCounterEntity(t),d=t.entity_id||"",p=e.detailStatsData.get(d)||[],g=[],m=!1;if(p.length>=2)for(let h of p){let k=h.val;n&&r.trigger_baseline_value!=null&&(k-=r.trigger_baseline_value);let j={ts:h.ts,val:k};!o&&h.min!=null&&h.max!=null&&(j.min=n&&r.trigger_baseline_value!=null?h.min-r.trigger_baseline_value:h.min,j.max=n&&r.trigger_baseline_value!=null?h.max-r.trigger_baseline_value:h.max,m=!0),g.push(j)}else for(let h of r.history)h.trigger_value!=null&&g.push({ts:new Date(h.timestamp).getTime(),val:h.trigger_value});if(r.trigger_current_value!=null){let h=r.trigger_current_value;n&&r.trigger_baseline_value!=null&&(h-=r.trigger_baseline_value),g.push({ts:Date.now(),val:h})}if(g.length<2&&d&&e.hasStatsService&&!e.detailStatsData.has(d))return l`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
    ...
    </div>`;if(g.length<2)return c;g.sort((h,k)=>h.ts-k.ts);let v=ri,b=oi,f=30,w=2,T=8,N=16,C=g.map(h=>h.val),E=Math.min(...C),y=Math.max(...C);if(m)for(let h of g)h.min!=null&&(E=Math.min(E,h.min)),h.max!=null&&(y=Math.max(y,h.max));t.trigger_above!=null&&(y=Math.max(y,t.trigger_above),E=Math.min(E,t.trigger_above)),t.trigger_below!=null&&(E=Math.min(E,t.trigger_below),y=Math.max(y,t.trigger_below));let I=null,q=null;if(i==="counter"&&t.trigger_target_value!=null){if(r.trigger_baseline_value!=null)I=r.trigger_baseline_value;else if(g.length>0){let h=[...r.history].filter(k=>k.type==="completed"||k.type==="reset").sort((k,j)=>new Date(j.timestamp).getTime()-new Date(k.timestamp).getTime())[0];if(h){let k=new Date(h.timestamp).getTime(),j=g[0],P=Math.abs(g[0].ts-k);for(let J of g){let Z=Math.abs(J.ts-k);Z<P&&(j=J,P=Z)}I=j.val}else I=g[0].val}I!=null?(q=I+t.trigger_target_value,y=Math.max(y,q),E=Math.min(E,I)):(y=Math.max(y,t.trigger_target_value),E=Math.min(E,0))}n&&r.trigger_baseline_value!=null&&(E=Math.min(E,0));let G=y-E||1;E-=G*.1,y+=G*.1;let W=g[0].ts,D=g[g.length-1].ts,x=D-W||1,z=h=>f+(h-W)/x*(v-f-w),M=h=>T+(1-(h-E)/(y-E))*(b-T-N),Ae=g.map(h=>`${z(h.ts).toFixed(1)},${M(h.val).toFixed(1)}`).join(" "),$t=`M${z(g[0].ts).toFixed(1)},${b-N} `+g.map(h=>`L${z(h.ts).toFixed(1)},${M(h.val).toFixed(1)}`).join(" ")+` L${z(g[g.length-1].ts).toFixed(1)},${b-N} Z`,Se="";if(m){let h=g.filter(k=>k.min!=null&&k.max!=null);if(h.length>=2){let k=h.map(P=>`${z(P.ts).toFixed(1)},${M(P.max).toFixed(1)}`),j=[...h].reverse().map(P=>`${z(P.ts).toFixed(1)},${M(P.min).toFixed(1)}`);Se=`M${k[0]} `+k.slice(1).map(P=>`L${P}`).join(" ")+` L${j[0]} `+j.slice(1).map(P=>`L${P}`).join(" ")+" Z"}}let qe=g[g.length-1],wt=z(qe.ts),kt=M(qe.val),Oe=h=>Math.abs(h)>=1e4?(h/1e3).toFixed(0)+"k":h>=1e3?(h/1e3).toFixed(1)+"k":h.toFixed(h<10?1:0),Et=Oe(y),At=Oe(E),St=r.history.filter(h=>["completed","skipped","reset"].includes(h.type)).map(h=>({ts:new Date(h.timestamp).getTime(),type:h.type})).filter(h=>h.ts>=W&&h.ts<=D),Te=g;if(g.length>Ne){let h=(g.length-1)/(Ne-1);Te=[];for(let k=0;k<Ne;k++)Te.push(g[Math.round(k*h)])}return l`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${v} ${b}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_sparkline",e.lang)}">
        ...
        ${St.map(h=>{let k=z(h.ts),j=h.type==="completed"?"var(--success-color, #4caf50)":h.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return L`
          ...
          `})}
        ${Te.map(h=>{let k=z(h.ts),j=M(h.val),P=new Date(h.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),J=`${h.val.toFixed(1)} ${a}`;return m&&h.min!=null&&h.max!=null&&(J+=` (${h.min.toFixed(1)}\u2013${h.max.toFixed(1)})`),L`<circle cx="${k.toFixed(1)}" cy="${j.toFixed(1)}" r="8" fill="transparent" tabindex="0"
            @mouseenter=${Z=>_t(Z,`${P}
${J}`,e.setTooltip)}
            @focus=${Z=>_t(Z,`${P}
${J}`,e.setTooltip)}
