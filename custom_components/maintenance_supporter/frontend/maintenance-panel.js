  ...
  `}function Va(c,s,e){let t=c.trigger_config;if(!t)return d;let a=t.type||"threshold",n=a==="counter"&&t.trigger_delta_mode,o=e.isCounterEntity(t),l=t.entity_id||"",_=e.detailStatsData.get(l)||[],u=[],m=!1;if(_.length>=2)for(let y of _){let S=y.val;n&&c.trigger_baseline_value!=null&&(S-=c.trigger_baseline_value);let H={ts:y.ts,val:S};!o&&y.min!=null&&y.max!=null&&(H.min=n&&c.trigger_baseline_value!=null?y.min-c.trigger_baseline_value:y.min,H.max=n&&c.trigger_baseline_value!=null?y.max-c.trigger_baseline_value:y.max,m=!0),u.push(H)}else for(let y of c.history)y.trigger_value!=null&&u.push({ts:new Date(y.timestamp).getTime(),val:y.trigger_value});if(c.trigger_current_value!=null){let y=c.trigger_current_value;n&&c.trigger_baseline_value!=null&&(y-=c.trigger_baseline_value),u.push({ts:Date.now(),val:y})}if(u.length<2&&l&&e.hasStatsService&&!e.detailStatsData.has(l))return r`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
    ...
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${h} ${v}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_sparkline",e.lang)}">
        ...
        ${c.degradation_rate!=null&&c.degradation_trend!=="stable"&&c.degradation_trend!=="insufficient_data"&&u.length>=2?(()=>{let y=u[u.length-1],S=30,H=y.ts+S*864e5,O=y.val+c.degradation_rate*S,te=Math.min(H,P+(P-K)*.3),ae=Math.max(E,Math.min(w,O)),Kt=D(y.ts),Gt=R(y.val),Zt=D(te),Jt=R(ae);return I`<line x1="${Kt.toFixed(1)}" y1="${Gt.toFixed(1)}" x2="${Zt.toFixed(1)}" y2="${Jt.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():d}
        ...
        ${Qt.map(y=>{let S=D(y.ts),H=y.type==="completed"?"var(--success-color, #4caf50)":y.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return I`
...
`).map(y=>r`<div>${y}</div>`)}
  ...
  `}function Dt(c,s){let e=c.interval_analysis,t=e?.weibull_beta,a=e?.weibull_eta;if(t==null||a==null||a<=0)return d;let n=c.interval_days??0,o=c.suggested_interval??n;return r`
    <div class="weibull-section">
      ...
      ${Qa(t,a,n,o,s)}
      ${Ka(e,s)}
      ${e?.confidence_interval_low!=null?Ga(e,c,s):d}
    </div>
  ...
  `}function Qa(c,s,e,t,a){let f=Math.max(e,t,s,1)*1.3,k=50,b=[];for(let P=0;P<=k;P++){let $=P/k*f,D=1-Math.exp(-Math.pow($/s,c)),R=32+$/f*260,Ie=136-D*128;b.push([R,Ie])}let N=b.map(([P,$])=>`${P.toFixed(1)},${$.toFixed(1)}`).join(" "),q="M32,136 "+b.map(([P,$])=>`L${P.toFixed(1)},${$.toFixed(1)}`).join(" ")+` L${b[k][0].toFixed(1)},136 Z`,E=32+e/f*260,w=1-Math.exp(-Math.pow(e/s,c)),M=136-w*128,L=((1-w)*100).toFixed(0),B=32+t/f*260,K=[0,.25,.5,.75,1];return r`
  ...
  `}function Ga(c,s,e){let t=c.confidence_interval_low,a=c.confidence_interval_high,n=s.suggested_interval??s.interval_days??0,o=s.interval_days??0,l=Math.max(0,t-5),u=a+5-l,m=(t-l)/u*100,h=(a-t)/u*100,v=(n-l)/u*100,f=o>0?(o-l)/u*100:-1;return r`
