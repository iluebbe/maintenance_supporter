  ...
  `,_([P({attribute:!1})],W.prototype,"hass",2),_([m()],W.prototype,"_open",2),_([m()],W.prototype,"_title",2),_([m()],W.prototype,"_message",2),_([m()],W.prototype,"_confirmText",2),_([m()],W.prototype,"_danger",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",W);var Wt=60,Kt=20,Gt=300,Zt=140,Yt=300,Qt=200,ct=30,Jt=27,A=class extends z{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._sparklineTooltip=null;this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1};this._actionLoading=!1;this._moreMenuOpen=!1;this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._deepLinkHandled=!1;this._onDialogEvent=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"de"}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new $e(this.hass),this._fetchMiniStatsForOverview()),this._userService||(this._userService=new ne(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,i,r,n]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);e&&(this._objects=e.objects),t&&(this._stats=t),i&&(this._budget=i),r&&(this._groups=r.groups||{}),n&&(this._features=n.features),this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("entry_id");if(!t)return;this._deepLinkHandled=!0;let i=e.get("task_id"),r=e.get("action");i?(this._showTask(t,i),r==="complete"&&(history.replaceState(null,"",window.location.pathname+window.location.hash),requestAnimationFrame(()=>{let d=this._getObject(t)?.tasks.find(u=>u.id===i);d&&this._openCompleteDialog(t,i,d.name,this._features.checklists?d.checklist:void 0,this._features.adaptive&&!!d.adaptive_config?.enabled)}))):this._showObject(t)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let i=await this._statsService.getDetailStats(e,t),r=new Map(this._detailStatsData);r.set(e,i),this._detailStatsData=r}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let i of this._objects)for(let r of i.tasks){let n=r.trigger_config?.entity_id;n&&e.push({entityId:n,isCounter:this._isCounterEntity(r.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let i of this._objects)for(let r of i.tasks)if(!(this._filterStatus&&r.status!==this._filterStatus)){if(this._filterUser){let n=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(r.responsible_user_id!==n)continue}e.push({entry_id:i.entry_id,task_id:r.id,object_name:i.object.name,task_name:r.name,type:r.type,schedule_type:r.schedule_type,status:r.status,days_until_due:r.days_until_due??null,next_due:r.next_due??null,trigger_active:r.trigger_active,trigger_current_value:r.trigger_current_value??null,trigger_current_delta:r.trigger_current_delta??null,trigger_config:r.trigger_config??null,trigger_entity_info:r.trigger_entity_info??null,times_performed:r.times_performed,total_cost:r.total_cost,interval_days:r.interval_days??null,history:r.history||[],enabled:r.enabled})}let t={overdue:0,triggered:1,due_soon:2,ok:3};return e.sort((i,r)=>(t[i.status]??9)-(t[r.status]??9)),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(r=>r.id===t)}_pushPanelState(e,t,i){let r={msp_view:e,msp_entry:t||null,msp_task:i||null};history.pushState(r,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let i=this._getTask(t.msp_entry,t.msp_task);i?.trigger_config?.entity_id&&this._fetchDetailStats(i.trigger_config.entity_id,this._isCounterEntity(i.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._historyFilter=null;let i=this._getTask(e,t);if(i?.trigger_config?.entity_id){let r=i.trigger_config.entity_id,n=this._isCounterEntity(i.trigger_config);this._fetchDetailStats(r,n)}}async _deleteObject(e){await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("delete",this._lang),message:s("confirm_delete_object",this._lang),confirmText:s("delete",this._lang),danger:!0})&&(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData())}async _deleteTask(e,t){await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("delete",this._lang),message:s("confirm_delete_task",this._lang),confirmText:s("delete",this._lang),danger:!0})&&(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData())}async _skipTask(e,t){this._actionLoading=!0;try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/skip",entry_id:e,task_id:t}),await this._loadData()}finally{this._actionLoading=!1}}async _resetTask(e,t){this._actionLoading=!0;try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/reset",entry_id:e,task_id:t}),await this._loadData()}finally{this._actionLoading=!1}}async _applySuggestion(e,t,i){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:i}),await this._loadData()}_dismissSuggestion(){this._loadData()}_openCompleteDialog(e,t,i,r,n){let d=this.shadowRoot.querySelector("maintenance-complete-dialog");d&&(d.entryId=e,d.taskId=t,d.taskName=i,d.lang=this._lang,d.checklist=r||[],d.adaptiveEnabled=!!n,d.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,i,r){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,i,r)}async _exportCsv(){let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/csv/export"}),t=new Blob([e.csv],{type:"text/csv;charset=utf-8;"}),i=URL.createObjectURL(t),r=document.createElement("a");r.href=i,r.download="maintenance_export.csv",r.click(),URL.revokeObjectURL(i)}_triggerImportCsv(){let e=this.shadowRoot.querySelector("input[type=file]");e&&(e.value="",e.click())}async _handleCsvFile(e){let i=e.target.files?.[0];if(!i)return;let r=await i.text(),n=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/csv/import",csv_content:r});await this._loadData(),await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:"Import",message:s("import_result",this._lang).replace("{created}",String(n.created)).replace("{total}",String(n.total)),confirmText:"OK"})}render(){return o`
    ...
    `}_renderTabContent(e){switch(this._activeTab){case"overview":return this._renderOverviewTab(e);case"history":return this._renderHistoryTab(e);default:return l}}_renderOverviewTab(e){let t=this._lang,i=this._features.adaptive&&e.suggested_interval&&e.suggested_interval!==e.interval_days,r=this._features.seasonal&&e.seasonal_factor&&e.seasonal_factor!==1,n=i||r,d=this._features.adaptive&&e.interval_analysis?.weibull_beta!=null&&e.interval_analysis?.weibull_eta!=null,u=this._features.seasonal&&(e.seasonal_factors?.length===12||e.interval_analysis?.seasonal_factors?.length===12);return o`
      <div class="tab-content overview-tab">
        ...
        ${d?this._renderWeibullCardExpanded(e):l}
        ${u?this._renderSeasonalCardExpanded(e):l}
    ...
    `}_renderHistoryChart(e){let t=e.history.filter(x=>x.type==="completed"&&(x.cost!=null||x.duration!=null)).map(x=>({ts:new Date(x.timestamp).getTime(),cost:x.cost??0,duration:x.duration??0})).sort((x,L)=>x.ts-L.ts);if(t.length<2)return l;let i=t.some(x=>x.cost>0),r=t.some(x=>x.duration>0);if(!i&&!r)return l;let n=this._costDurationToggle!=="duration"&&i,d=this._costDurationToggle!=="cost"&&r,u=n||!d&&i,v=d||!n&&r,p=Yt,h=Qt,f=u?32:8,$=v?32:8,b=8,C=20,D=p-f-$,E=h-b-C,H=Math.max(...t.map(x=>x.cost))||1,y=Math.max(...t.map(x=>x.duration))||1,S=Math.min(20,D/t.length*.6),M=D/t.length,R=x=>f+M*x+M/2,V=x=>b+E-x/H*E,B=x=>b+E-x/y*E;return o`
      <div class="sparkline-container">
        <svg class="history-chart" viewBox="0 0 ${p} ${h}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_history",this._lang)}">
          <!-- Cost bars -->
          ${u?t.map((x,L)=>F`
            <rect x="${(R(L)-S/2).toFixed(1)}" y="${V(x.cost).toFixed(1)}" width="${S.toFixed(1)}" height="${(b+E-V(x.cost)).toFixed(1)}"
              fill="var(--primary-color)" opacity="0.6" rx="2" />
          `):l}
          <!-- Duration line -->
          ${v?F`
            <polyline points="${t.map((x,L)=>`${R(L).toFixed(1)},${B(x.duration).toFixed(1)}`).join(" ")}"
              fill="none" stroke="var(--accent-color, #ff9800)" stroke-width="2" stroke-linejoin="round" />
            ${t.map((x,L)=>F`
              <circle cx="${R(L).toFixed(1)}" cy="${B(x.duration).toFixed(1)}" r="3" fill="var(--accent-color, #ff9800)" />
            `)}
          `:l}
          <!-- X-axis labels -->
    ...
    `}_renderSeasonalChart(e){let t=e.seasonal_factors??e.interval_analysis?.seasonal_factors;if(!t||t.length!==12)return l;let i=this._lang,r=e.interval_analysis?.seasonal_reason,n=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"],d=new Date().getMonth(),u=300,v=100,p=8,f=v-p-4,$=Math.max(...t,1.5),b=u/12,C=b*.65,D=p+f-1/$*f;return o`
      <div class="seasonal-chart">
        ...
        <svg viewBox="0 0 ${u} ${v}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",i)}">
          <!-- Baseline at 1.0 -->
          <line x1="0" y1="${D.toFixed(1)}" x2="${u}" y2="${D.toFixed(1)}"
        ...
        <div class="seasonal-labels">
          ${n.map((E,H)=>o`<span class="seasonal-label ${H===d?"active-month":""}">${s(E,i)}</span>`)}
        </div>
      </div>
    ...
    `}_renderWeibullChart(e,t,i,r){let b=Math.max(i,r,t,1)*1.3,C=50,D=[];for(let x=0;x<=C;x++){let L=x/C*b,N=1-Math.exp(-Math.pow(L/t,e)),T=32+L/b*260,we=136-N*128;D.push([T,we])}let E=D.map(([x,L])=>`${x.toFixed(1)},${L.toFixed(1)}`).join(" "),H="M32,136 "+D.map(([x,L])=>`L${x.toFixed(1)},${L.toFixed(1)}`).join(" ")+` L${D[C][0].toFixed(1)},136 Z`,y=32+i/b*260,S=1-Math.exp(-Math.pow(i/t,e)),M=136-S*128,R=((1-S)*100).toFixed(0),V=32+r/b*260,B=[0,.25,.5,.75,1];return o`
      <div class="weibull-chart">
        <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_weibull",this._lang)}">
          <!-- Grid lines -->
          ${B.map(x=>{let L=136-x*128;return F`
    ...
    `}_renderConfidenceInterval(e,t,i){let r=e.confidence_interval_low,n=e.confidence_interval_high,d=t.suggested_interval??t.interval_days??0,u=t.interval_days??0,v=Math.max(0,r-5),h=n+5-v,f=(r-v)/h*100,$=(n-r)/h*100,b=(d-v)/h*100,C=u>0?(u-v)/h*100:-1;return o`
        ...
        <div class="trigger-limits">
          ...
          ${f==="compound"?o`
            ...
            ${(t.conditions||[]).map((b,C)=>o`
              <span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${C+1}. ${s(b.trigger_config?.type||"unknown",i)}: ${b.trigger_config?.entity_id||""}</span>
            `)}
          `:l}
    ...
    `}_renderSparkline(e,t){let i=e.trigger_config;if(!i)return l;let r=i.type||"threshold",n=r==="counter"&&i.trigger_delta_mode,d=this._isCounterEntity(i),u=i.entity_id||"",v=this._detailStatsData.get(u)||[],p=[],h=!1;if(v.length>=2)for(let g of v){let k=g.val;n&&e.trigger_baseline_value!=null&&(k-=e.trigger_baseline_value);let j={ts:g.ts,val:k};!d&&g.min!=null&&g.max!=null&&(j.min=n&&e.trigger_baseline_value!=null?g.min-e.trigger_baseline_value:g.min,j.max=n&&e.trigger_baseline_value!=null?g.max-e.trigger_baseline_value:g.max,h=!0),p.push(j)}else for(let g of e.history)g.trigger_value!=null&&p.push({ts:new Date(g.timestamp).getTime(),val:g.trigger_value});if(e.trigger_current_value!=null){let g=e.trigger_current_value;n&&e.trigger_baseline_value!=null&&(g-=e.trigger_baseline_value),p.push({ts:Date.now(),val:g})}if(p.length<2&&u&&this._statsService&&!this._detailStatsData.has(u))return o`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
      ...
      </div>`;if(p.length<2)return l;p.sort((g,k)=>g.ts-k.ts);let f=Gt,$=Zt,b=30,C=2,D=8,E=16,H=p.map(g=>g.val),y=Math.min(...H),S=Math.max(...H);if(h)for(let g of p)g.min!=null&&(y=Math.min(y,g.min)),g.max!=null&&(S=Math.max(S,g.max));i.trigger_above!=null&&(S=Math.max(S,i.trigger_above),y=Math.min(y,i.trigger_above)),i.trigger_below!=null&&(y=Math.min(y,i.trigger_below),S=Math.max(S,i.trigger_below));let M=null,R=null;if(r==="counter"&&i.trigger_target_value!=null){if(e.trigger_baseline_value!=null)M=e.trigger_baseline_value;else if(p.length>0){let g=[...e.history].filter(k=>k.type==="completed"||k.type==="reset").sort((k,j)=>new Date(j.timestamp).getTime()-new Date(k.timestamp).getTime())[0];if(g){let k=new Date(g.timestamp).getTime(),j=p[0],I=Math.abs(p[0].ts-k);for(let G of p){let Z=Math.abs(G.ts-k);Z<I&&(j=G,I=Z)}M=j.val}else M=p[0].val}M!=null?(R=M+i.trigger_target_value,S=Math.max(S,R),y=Math.min(y,M)):(S=Math.max(S,i.trigger_target_value),y=Math.min(y,0))}n&&e.trigger_baseline_value!=null&&(y=Math.min(y,0));let V=S-y||1;y-=V*.1,S+=V*.1;let B=p[0].ts,x=p[p.length-1].ts,L=x-B||1,N=g=>b+(g-B)/L*(f-b-C),T=g=>D+(1-(g-y)/(S-y))*($-D-E),we=p.map(g=>`${N(g.ts).toFixed(1)},${T(g.val).toFixed(1)}`).join(" "),dt=`M${N(p[0].ts).toFixed(1)},${$-E} `+p.map(g=>`L${N(g.ts).toFixed(1)},${T(g.val).toFixed(1)}`).join(" ")+` L${N(p[p.length-1].ts).toFixed(1)},${$-E} Z`,ke="";if(h){let g=p.filter(k=>k.min!=null&&k.max!=null);if(g.length>=2){let k=g.map(I=>`${N(I.ts).toFixed(1)},${T(I.max).toFixed(1)}`),j=[...g].reverse().map(I=>`${N(I.ts).toFixed(1)},${T(I.min).toFixed(1)}`);ke=`M${k[0]} `+k.slice(1).map(I=>`L${I}`).join(" ")+` L${j[0]} `+j.slice(1).map(I=>`L${I}`).join(" ")+" Z"}}let Ne=p[p.length-1],ut=N(Ne.ts),pt=T(Ne.val),Oe=g=>Math.abs(g)>=1e4?(g/1e3).toFixed(0)+"k":g>=1e3?(g/1e3).toFixed(1)+"k":g.toFixed(g<10?1:0),gt=Oe(S),_t=Oe(y),ht=e.history.filter(g=>["completed","skipped","reset"].includes(g.type)).map(g=>({ts:new Date(g.timestamp).getTime(),type:g.type})).filter(g=>g.ts>=B&&g.ts<=x),Ee=Jt,Ae=p;if(p.length>Ee){let g=(p.length-1)/(Ee-1);Ae=[];for(let k=0;k<Ee;k++)Ae.push(p[Math.round(k*g)])}return o`
      <div class="sparkline-container">
        <svg class="sparkline-svg" viewBox="0 0 ${f} ${$}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_sparkline",this._lang)}">
          <!-- Y-axis labels -->
          <text x="${b-3}" y="${D+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${gt}</text>
          ...
          ${r==="counter"&&R!=null?F`<line x1="${b}" y1="${T(R).toFixed(1)}" x2="${f}" y2="${T(R).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                  <text x="${f-2}" y="${T(R)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">${s("target_value",this._lang)}: +${i.trigger_target_value}</text>`:l}
          ...
          ${r==="counter"&&M!=null?F`<line x1="${b}" y1="${T(M).toFixed(1)}" x2="${f}" y2="${T(M).toFixed(1)}" stroke="var(--secondary-text-color)" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
                  <text x="${b+2}" y="${T(M)+11}" text-anchor="start" fill="var(--secondary-text-color)" font-size="8">${s("baseline",this._lang)}</text>`:l}
          <!-- Current dot -->
          ...
          ${ht.map(g=>{let k=N(g.ts),j=g.type==="completed"?"var(--success-color, #4caf50)":g.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return F`
              <line x1="${k.toFixed(1)}" y1="${D}" x2="${k.toFixed(1)}" y2="${$-E}" stroke="${j}" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
              <circle cx="${k.toFixed(1)}" cy="${D+2}" r="5" fill="${j}" opacity="0.8" />
              <text x="${k.toFixed(1)}" y="${D+6}" text-anchor="middle" fill="white" font-size="7" font-weight="bold">${g.type==="completed"?"\u2713":g.type==="skipped"?"\u23ED":"\u21BA"}</text>
            `})}
