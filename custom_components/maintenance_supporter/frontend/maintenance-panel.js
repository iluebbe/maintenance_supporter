...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function Aa(l,n,e){let t=l.trigger_config;if(!t)return d;let a=t.type||"threshold",s=a==="counter"&&t.trigger_delta_mode,o=e.isCounterEntity(t),c=t.entity_id||"",_=e.detailStatsData.get(c)||[],g=[],m=!1;if(_.length>=2)for(let f of _){let A=f.val;s&&l.trigger_baseline_value!=null&&(A-=l.trigger_baseline_value);let F={ts:f.ts,val:A};!o&&f.min!=null&&f.max!=null&&(F.min=s&&l.trigger_baseline_value!=null?f.min-l.trigger_baseline_value:f.min,F.max=s&&l.trigger_baseline_value!=null?f.max-l.trigger_baseline_value:f.max,m=!0),g.push(F)}else for(let f of l.history)f.trigger_value!=null&&g.push({ts:new Date(f.timestamp).getTime(),val:f.trigger_value});if(l.trigger_current_value!=null){let f=l.trigger_current_value;s&&l.trigger_baseline_value!=null&&(f-=l.trigger_baseline_value),g.push({ts:Date.now(),val:f})}if(g.length<2&&c&&e.hasStatsService&&!e.detailStatsData.has(c))return r`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
    ...
    </div>`;if(g.length<2)return d;g.sort((f,A)=>f.ts-A.ts);let h=za,v=Ea,b=30,k=2,S=8,H=16,M=g.map(f=>f.val),j=Math.min(...M),x=Math.max(...M);if(m)for(let f of g)f.min!=null&&(j=Math.min(j,f.min)),f.max!=null&&(x=Math.max(x,f.max));t.trigger_above!=null&&(x=Math.max(x,t.trigger_above),j=Math.min(j,t.trigger_above)),t.trigger_below!=null&&(j=Math.min(j,t.trigger_below),x=Math.max(x,t.trigger_below));let C=null,U=null;if(a==="counter"&&t.trigger_target_value!=null){if(l.trigger_baseline_value!=null)C=l.trigger_baseline_value;else if(g.length>0){let f=[...l.history].filter(A=>A.type==="completed"||A.type==="reset").sort((A,F)=>new Date(F.timestamp).getTime()-new Date(A.timestamp).getTime())[0];if(f){let A=new Date(f.timestamp).getTime(),F=g[0],L=Math.abs(g[0].ts-A);for(let X of g){let ee=Math.abs(X.ts-A);ee<L&&(F=X,L=ee)}C=F.val}else C=g[0].val}C!=null?(U=C+t.trigger_target_value,x=Math.max(x,U),j=Math.min(j,C)):(x=Math.max(x,t.trigger_target_value),j=Math.min(j,0))}s&&l.trigger_baseline_value!=null&&(j=Math.min(j,0));let Z=x-j||1;j-=Z*.1,x+=Z*.1;let K=g[0].ts,P=g[g.length-1].ts,w=P-K||1,N=f=>b+(f-K)/w*(h-b-k),T=f=>S+(1-(f-j)/(x-j))*(v-S-H),qe=g.map(f=>`${N(f.ts).toFixed(1)},${T(f.val).toFixed(1)}`).join(" "),Tt=`M${N(g[0].ts).toFixed(1)},${v-H} `+g.map(f=>`L${N(f.ts).toFixed(1)},${T(f.val).toFixed(1)}`).join(" ")+` L${N(g[g.length-1].ts).toFixed(1)},${v-H} Z`,Te="";if(m){let f=g.filter(A=>A.min!=null&&A.max!=null);if(f.length>=2){let A=f.map(L=>`${N(L.ts).toFixed(1)},${T(L.max).toFixed(1)}`),F=[...f].reverse().map(L=>`${N(L.ts).toFixed(1)},${T(L.min).toFixed(1)}`);Te=`M${A[0]} `+A.slice(1).map(L=>`L${L}`).join(" ")+` L${F[0]} `+F.slice(1).map(L=>`L${L}`).join(" ")+" Z"}}let Qe=g[g.length-1],Mt=N(Qe.ts),Ct=T(Qe.val),Ge=f=>Math.abs(f)>=1e4?(f/1e3).toFixed(0)+"k":f>=1e3?(f/1e3).toFixed(1)+"k":f.toFixed(f<10?1:0),Nt=Ge(x),Rt=Ge(j),It=l.history.filter(f=>["completed","skipped","reset"].includes(f.type)).map(f=>({ts:new Date(f.timestamp).getTime(),type:f.type})).filter(f=>f.ts>=K&&f.ts<=P),Me=g;if(g.length>We){let f=(g.length-1)/(We-1);Me=[];for(let A=0;A<We;A++)Me.push(g[Math.round(A*f)])}return r`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${h} ${v}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_sparkline",e.lang)}">
        ...
        <text x="${b-3}" y="${v-H+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${Rt}</text>
        ...
        ${Te?I`<path d="${Te}" fill="var(--primary-color)" opacity="0.08" />`:d}
        ...
        ${l.degradation_rate!=null&&l.degradation_trend!=="stable"&&l.degradation_trend!=="insufficient_data"&&g.length>=2?(()=>{let f=g[g.length-1],A=30,F=f.ts+A*864e5,L=f.val+l.degradation_rate*A,X=Math.min(F,P+(P-K)*.3),ee=Math.max(j,Math.min(x,L)),Pt=N(f.ts),Dt=T(f.val),Ft=N(X),Lt=T(ee);return I`<line x1="${Pt.toFixed(1)}" y1="${Dt.toFixed(1)}" x2="${Ft.toFixed(1)}" y2="${Lt.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():d}
        ${a==="threshold"&&t.trigger_above!=null?I`<line x1="${b}" y1="${T(t.trigger_above).toFixed(1)}" x2="${h}" y2="${T(t.trigger_above).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${h-2}" y="${T(t.trigger_above)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25B2 ${t.trigger_above}</text>`:d}
        ${a==="threshold"&&t.trigger_below!=null?I`<line x1="${b}" y1="${T(t.trigger_below).toFixed(1)}" x2="${h}" y2="${T(t.trigger_below).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${h-2}" y="${T(t.trigger_below)+11}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25BC ${t.trigger_below}</text>`:d}
        ${a==="counter"&&U!=null?I`<line x1="${b}" y1="${T(U).toFixed(1)}" x2="${h}" y2="${T(U).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
        ...
        ${a==="counter"&&C!=null?I`<line x1="${b}" y1="${T(C).toFixed(1)}" x2="${h}" y2="${T(C).toFixed(1)}" stroke="var(--secondary-text-color)" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
        ...
        ${It.map(f=>{let A=N(f.ts),F=f.type==="completed"?"var(--success-color, #4caf50)":f.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return I`
            <line x1="${A.toFixed(1)}" y1="${S}" x2="${A.toFixed(1)}" y2="${v-H}" stroke="${F}" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
          ...
          `})}
        ${Me.map(f=>{let A=N(f.ts),F=T(f.val),L=new Date(f.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),X=`${f.val.toFixed(1)} ${n}`;return m&&f.min!=null&&f.max!=null&&(X+=` (${f.min.toFixed(1)}\u2013${f.max.toFixed(1)})`),I`<circle cx="${A.toFixed(1)}" cy="${F.toFixed(1)}" r="8" fill="transparent" tabindex="0"
...
`).map(f=>r`<div>${f}</div>`)}
  ...
  `}function Sa(l,n,e,t,a){let b=Math.max(e,t,n,1)*1.3,k=50,S=[];for(let P=0;P<=k;P++){let w=P/k*b,N=1-Math.exp(-Math.pow(w/n,l)),T=32+w/b*260,qe=136-N*128;S.push([T,qe])}let H=S.map(([P,w])=>`${P.toFixed(1)},${w.toFixed(1)}`).join(" "),M="M32,136 "+S.map(([P,w])=>`L${P.toFixed(1)},${w.toFixed(1)}`).join(" ")+` L${S[k][0].toFixed(1)},136 Z`,j=32+e/b*260,x=1-Math.exp(-Math.pow(e/n,l)),C=136-x*128,U=((1-x)*100).toFixed(0),Z=32+t/b*260,K=[0,.25,.5,.75,1];return r`
  ...
  `}function jt(l,n){return Ma(l,n)}function Ma(l,n){let e=l.seasonal_factors??l.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return d;let t=l.interval_analysis?.seasonal_reason,a=new Date().getMonth(),s=300,o=100,c=8,g=o-c-4,m=Math.max(...e,1.5),h=s/12,v=h*.65,b=c+g-1/m*g;return r`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${s} ${o}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_seasonal",n)}">
        ...
        ${e.map((k,S)=>{let H=k/m*g,M=S*h+(h-v)/2,j=c+g-H,x=S===a,C=k<1?"var(--success-color, #4caf50)":k>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return I`
  ...
  `}function Ra(l,n,e){let t=l.history.filter(w=>w.type==="completed"&&(w.cost!=null||w.duration!=null)).map(w=>({ts:new Date(w.timestamp).getTime(),cost:w.cost??0,duration:w.duration??0})).sort((w,N)=>w.ts-N.ts);if(t.length<2)return d;let a=t.some(w=>w.cost>0),s=t.some(w=>w.duration>0);if(!a&&!s)return d;let o=e!=="duration"&&a,c=e!=="cost"&&s,_=o||!c&&a,g=c||!o&&s,m=Ca,h=Na,v=_?32:8,b=g?32:8,k=8,S=20,H=m-v-b,M=h-k-S,j=Math.max(...t.map(w=>w.cost))||1,x=Math.max(...t.map(w=>w.duration))||1,C=Math.min(20,H/t.length*.6),U=H/t.length,Z=w=>v+U*w+U/2,K=w=>k+M-w/j*M,P=w=>k+M-w/x*M;return r`
    <div class="sparkline-container">
  ...
  `}var Ia=60,Pa=20,qt=30,z=class extends q{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._sparklineTooltip=null;this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._adminPanelUserIds=[];this._defaultWarningDays=7;this._actionLoading=!1;this._moreMenuOpen=!1;this._toastMessage="";this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab="dashboard";this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._objectSortMode="alphabetical";this._groupByMode="none";this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._deepLinkHandled=!1;this._onDialogEvent=async()=>{try{await this._loadData()}catch{}}}get _lang(){return this.hass?.language||"en"}get _isOperator(){let e=this.hass?.user;return e?e.is_admin?!1:!this._adminPanelUserIds.includes(e.id):!0}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler);let e=localStorage.getItem("maintenance_supporter_sort");e&&["due_date","object","type","task_name","area","assigned_user","group"].includes(e)&&(this._sortMode=e);let t=localStorage.getItem("maintenance_supporter_object_sort");t&&["alphabetical","due_soonest","task_count"].includes(t)&&(this._objectSortMode=t);let a=localStorage.getItem("maintenance_supporter_groupby");a&&["none","area","group","user"].includes(a)&&(this._groupByMode=a)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new je(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new ie(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,a,s,o]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);if(e&&(this._objects=e.objects),t&&(this._stats=t),a&&(this._budget=a),s&&(this._groups=s.groups||{}),o){let c=o;this._features=c.features,this._adminPanelUserIds=c.admin_panel_user_ids||[];let _=c.general?.default_warning_days;typeof _=="number"&&_>=0&&_<=365&&(this._defaultWarningDays=_)}this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("entry_id");if(!t)return;this._deepLinkHandled=!0;let a=e.get("task_id"),s=e.get("action"),o=window.location.pathname+window.location.hash;history.replaceState(history.state,"",o);let c=this._getObject(t);if(!c){this._showOverview();return}if(a){let _=c.tasks.find(g=>g.id===a);if(!_){this._showObject(t);return}this._showTask(t,a),s==="complete"?requestAnimationFrame(()=>{this._openCompleteDialog(t,a,_.name,this._features.checklists?_.checklist:void 0,this._features.adaptive&&!!_.adaptive_config?.enabled)}):s==="quick_complete"&&requestAnimationFrame(()=>{this._handleQuickComplete(t,a,_)})}else this._showObject(t)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let a=await this._statsService.getDetailStats(e,t),s=new Map(this._detailStatsData);s.set(e,a),this._detailStatsData=s}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let a of this._objects)for(let s of a.tasks){let o=s.trigger_config?.entity_id;o&&e.push({entityId:o,isCounter:this._isCounterEntity(s.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let h of this._objects)for(let v of h.tasks){if(this._filterStatus&&v.status!==this._filterStatus)continue;if(this._filterUser){let k=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(v.responsible_user_id!==k)continue}let b=[];for(let k of Object.values(this._groups))k.task_refs?.some(S=>S.entry_id===h.entry_id&&S.task_id===v.id)&&b.push(k.name);e.push({entry_id:h.entry_id,task_id:v.id,object_name:h.object.name,task_name:v.name,type:v.type,schedule_type:v.schedule_type,status:v.status,days_until_due:v.days_until_due??null,next_due:v.next_due??null,trigger_active:v.trigger_active,trigger_current_value:v.trigger_current_value??null,trigger_current_delta:v.trigger_current_delta??null,trigger_config:v.trigger_config??null,trigger_entity_info:v.trigger_entity_info??null,times_performed:v.times_performed,total_cost:v.total_cost,interval_days:v.interval_days??null,interval_anchor:v.interval_anchor??null,history:v.history||[],enabled:v.enabled,nfc_tag_id:v.nfc_tag_id??null,area_id:h.object.area_id??null,responsible_user_id:v.responsible_user_id??null,group_names:b})}let t={overdue:0,triggered:1,due_soon:2,ok:3},a=(h,v)=>(t[h.status]??9)-(t[v.status]??9),s=(h,v)=>(h.days_until_due??99999)-(v.days_until_due??99999),o=(h,v)=>a(h,v)||s(h,v),c=h=>h.area_id&&this.hass?.areas?.[h.area_id]?.name||"",_=h=>h.responsible_user_id&&this._userService?.getUserName(h.responsible_user_id)||"",g=h=>h.group_names[0]||"",m={due_date:o,object:(h,v)=>h.object_name.localeCompare(v.object_name)||o(h,v),type:(h,v)=>h.type.localeCompare(v.type)||o(h,v),task_name:(h,v)=>h.task_name.localeCompare(v.task_name),area:(h,v)=>{let b=c(h),k=c(v);return!b&&k?1:b&&!k?-1:b.localeCompare(k)||o(h,v)},assigned_user:(h,v)=>{let b=_(h),k=_(v);return!b&&k?1:b&&!k?-1:b.localeCompare(k)||o(h,v)},group:(h,v)=>{let b=g(h),k=g(v);return!b&&k?1:b&&!k?-1:b.localeCompare(k)||o(h,v)}};return e.sort(m[this._sortMode]),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(s=>s.id===t)}_pushPanelState(e,t,a){let s={msp_view:e,msp_entry:t||null,msp_task:a||null};history.pushState(s,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let a=this._getTask(t.msp_entry,t.msp_task);a?.trigger_config?.entity_id&&this._fetchDetailStats(a.trigger_config.entity_id,this._isCounterEntity(a.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null;let a=this._getTask(e,t);if(a?.trigger_config?.entity_id){let s=a.trigger_config.entity_id,o=this._isCounterEntity(a.trigger_config);this._fetchDetailStats(s,o)}}_showToast(e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}async _deleteObject(e){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:i("delete",this._lang),message:i("confirm_delete_object",this._lang),confirmText:i("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}}async _deleteTask(e,t){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:i("delete",this._lang),message:i("confirm_delete_task",this._lang),confirmText:i("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}}async _skipTask(e,t,a){this._actionLoading=!0;try{let s={type:"maintenance_supporter/task/skip",entry_id:e,task_id:t};a&&(s.reason=a),await this.hass.connection.sendMessagePromise(s),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}finally{this._actionLoading=!1}}async _resetTask(e,t,a){this._actionLoading=!0;try{let s={type:"maintenance_supporter/task/reset",entry_id:e,task_id:t};a&&(s.date=a),await this.hass.connection.sendMessagePromise(s),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}finally{this._actionLoading=!1}}async _applySuggestion(e,t,a){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:a}),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}}_openSeasonalOverrides(e){let t=this.shadowRoot.querySelector("maintenance-seasonal-overrides-dialog");if(!t||!this._selectedEntryId)return;let a=e.adaptive_config?.seasonal_overrides;t.open(this._selectedEntryId,e.id,a)}async _reanalyzeInterval(e,t){try{let a=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:e,task_id:t});a.recommended_interval?this._showToast(`${i("reanalyze_result",this._lang)}: ${a.recommended_interval} ${i("days",this._lang)} (${i(`confidence_${a.confidence}`,this._lang)}, ${a.data_points} ${i("data_points",this._lang)})`):this._showToast(i("reanalyze_insufficient_data",this._lang)),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}}async _promptSkipTask(e,t){let a=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!a)return;let s=await a.prompt({title:i("skip",this._lang),message:i("skip_reason_prompt",this._lang),confirmText:i("skip",this._lang),inputLabel:i("reason_optional",this._lang),inputType:"text"});s.confirmed&&this._skipTask(e,t,s.value||void 0)}async _promptResetTask(e,t){let a=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!a)return;let s=await a.prompt({title:i("reset",this._lang),message:i("reset_date_prompt",this._lang),confirmText:i("reset",this._lang),inputLabel:i("reset_date_optional",this._lang),inputType:"date"});s.confirmed&&this._resetTask(e,t,s.value||void 0)}_dismissSuggestion(e,t){e&&t&&this._dismissedSuggestions.add(`${e}_${t}`),this.requestUpdate()}async _handleQuickComplete(e,t,a){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/quick_complete",entry_id:e,task_id:t}),this._showToast(i("quick_complete_success",this._lang))}catch(s){(s?.code||"")==="no_defaults"?this._openCompleteDialog(e,t,a.name,this._features.checklists?a.checklist:void 0,this._features.adaptive&&!!a.adaptive_config?.enabled):this._showToast(i("action_error",this._lang))}}_openCompleteDialog(e,t,a,s,o){let c=this.shadowRoot.querySelector("maintenance-complete-dialog");c&&(c.entryId=e,c.taskId=t,c.taskName=a,c.lang=this._lang,c.checklist=s||[],c.adaptiveEnabled=!!o,c.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,a,s){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,a,s)}render(){return r`
    ...
    `}_renderMiniSparkline(e){if(!e.trigger_config?.entity_id)return d;let t=e.trigger_config.entity_id,a=this._miniStatsData.get(t)||[],s=[];if(a.length>=2)s=a.map(x=>({ts:x.ts,val:x.val}));else{if(!e.history)return d;for(let x of e.history)x.trigger_value!=null&&s.push({ts:new Date(x.timestamp).getTime(),val:x.trigger_value})}if(e.trigger_current_value!=null&&s.push({ts:Date.now(),val:e.trigger_current_value}),s.length<2)return d;s.sort((x,C)=>x.ts-C.ts);let o=Ia,c=Pa,_=s.map(x=>x.val),g=Math.min(..._),m=Math.max(..._),h=m-g||1;g-=h*.1,m+=h*.1;let v=s[0].ts,k=s[s.length-1].ts-v||1,S=x=>(x-v)/k*o,H=x=>2+(1-(x-g)/(m-g))*(c-4),M=s;if(M.length>qt){let x=Math.ceil(M.length/qt);M=M.filter((C,U)=>U%x===0||U===M.length-1)}let j=M.map(x=>`${S(x.ts).toFixed(1)},${H(x.val).toFixed(1)}`).join(" ");return r`
