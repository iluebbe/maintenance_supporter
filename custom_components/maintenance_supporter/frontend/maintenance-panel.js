...
`;var Ee=class{constructor(i){this._cache=new Map;this._pending=new Map;this._hass=i}updateHass(i){this._hass=i}async getDetailStats(i,e){return this._getStats(i,"hour",30,e)}async getMiniStats(i,e){return this._getStats(i,"day",14,e)}async getBatchMiniStats(i){let e=new Map,t=[];for(let p of i){let _=`${p.entityId}:day`,m=this._cache.get(_);m&&Date.now()-m.fetchedAt<3e5?e.set(p.entityId,m.points):t.push(p)}if(t.length===0)return e;let a=t.filter(p=>p.isCounter).map(p=>p.entityId),s=t.filter(p=>!p.isCounter).map(p=>p.entityId),o=new Date(Date.now()-336*60*60*1e3).toISOString(),c=[];return a.length>0&&c.push(this._fetchBatch(a,"day",o,["state","sum","change"],!0,e)),s.length>0&&c.push(this._fetchBatch(s,"day",o,["mean","min","max"],!1,e)),await Promise.all(c),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(i,e,t,a){let s=`${i}:${e}`,o=this._cache.get(s);if(o&&Date.now()-o.fetchedAt<3e5)return o.points;if(this._pending.has(s))return this._pending.get(s);let c=this._fetchAndNormalize(i,e,t,a,s);this._pending.set(s,c);try{return await c}finally{this._pending.delete(s)}}async _fetchAndNormalize(i,e,t,a,s){let o=new Date(Date.now()-t*24*60*60*1e3).toISOString(),c=a?["state","sum","change"]:["mean","min","max"];try{let _=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:o,statistic_ids:[i],period:e,types:c}))[i]||[],m=this._normalizeRows(_,a);return this._cache.set(s,{entityId:i,fetchedAt:Date.now(),period:e,points:m}),m}catch(p){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${i}:`,p),[]}}async _fetchBatch(i,e,t,a,s,o){try{let c=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:i,period:e,types:a});for(let p of i){let _=c[p]||[],m=this._normalizeRows(_,s);o.set(p,m),this._cache.set(`${p}:${e}`,{entityId:p,fetchedAt:Date.now(),period:e,points:m})}}catch(c){console.warn("[maintenance-supporter] Batch statistics fetch failed:",c)}}_normalizeRows(i,e){let t=[];for(let a of i){let s=null;if(e?s=a.state??null:s=a.mean??null,s===null)continue;let o={ts:a.start,val:s};e||(a.min!=null&&(o.min=a.min),a.max!=null&&(o.max=a.max)),t.push(o)}return t.sort((a,s)=>a.ts-s.ts),t}};var oe=class{constructor(i){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=i}updateHass(i){this.hass=i}async getUsers(i=!1){let e=Date.now();if(!i&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(i,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:i,task_id:e,user_id:t})}async getTasksByUser(i){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:i})).tasks}getUserName(i){return!i||!this.usersCache?null:this.usersCache.find(t=>t.id===i)?.name||null}getUser(i){return!i||!this.usersCache?null:this.usersCache.find(e=>e.id===i)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(i){return i?i===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var H=class extends C{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._name="";this._manufacturer="";this._model="";this._serialNumber="";this._areaId="";this._installationDate="";this._entryId=null}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}openCreate(){this._entryId=null,this._name="",this._manufacturer="",this._model="",this._serialNumber="",this._areaId="",this._installationDate="",this._error="",this._open=!0}openEdit(e,t){this._entryId=e,this._name=t.name||"",this._manufacturer=t.manufacturer||"",this._model=t.model||"",this._serialNumber=t.serial_number||"",this._areaId=t.area_id||"",this._installationDate=t.installation_date||"",this._error="",this._open=!0}async _save(){if(this._name.trim()){this._loading=!0,this._error="";try{this._entryId?await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/update",entry_id:this._entryId,name:this._name,manufacturer:this._manufacturer||null,model:this._model||null,serial_number:this._serialNumber||null,area_id:this._areaId||null,installation_date:this._installationDate||null}):await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/create",name:this._name,manufacturer:this._manufacturer||null,model:this._model||null,serial_number:this._serialNumber||null,area_id:this._areaId||null,installation_date:this._installationDate||null}),this._open=!1,this.dispatchEvent(new CustomEvent("object-saved"))}catch{this._error=n("save_error",this._lang)}finally{this._loading=!1}}}_close(){this._open=!1}render(){if(!this._open)return l``;let e=this._lang,t=this._entryId?n("edit_object",e):n("new_object",e);return l`
  ...
  `,u([S({attribute:!1})],H.prototype,"hass",2),u([g()],H.prototype,"_open",2),u([g()],H.prototype,"_loading",2),u([g()],H.prototype,"_error",2),u([g()],H.prototype,"_name",2),u([g()],H.prototype,"_manufacturer",2),u([g()],H.prototype,"_model",2),u([g()],H.prototype,"_serialNumber",2),u([g()],H.prototype,"_areaId",2),u([g()],H.prototype,"_installationDate",2),u([g()],H.prototype,"_entryId",2);customElements.get("maintenance-object-dialog")||customElements.define("maintenance-object-dialog",H);var ra=["cleaning","inspection","replacement","calibration","service","custom"],oa=["time_based","sensor_based","manual"],la=["threshold","counter","state_change","runtime"],y=class extends C{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._entryId="";this._taskId=null;this._name="";this._type="custom";this._scheduleType="time_based";this._intervalDays="30";this._warningDays="7";this._intervalAnchor="completion";this._notes="";this._documentationUrl="";this._customIcon="";this._enabled=!0;this._triggerEntityId="";this._triggerEntityIds=[];this._triggerEntityLogic="any";this._triggerAttribute="";this._triggerType="threshold";this._triggerAbove="";this._triggerBelow="";this._triggerForMinutes="0";this._triggerTargetValue="";this._triggerDeltaMode=!1;this._triggerFromState="";this._triggerToState="";this._triggerTargetChanges="";this._triggerRuntimeHours="";this._suggestedAttributes=[];this._availableAttributes=[];this._entityDomain="";this._lastPerformed="";this._nfcTagId="";this._availableTags=[];this._responsibleUserId=null;this._availableUsers=[];this._environmentalEntity="";this._environmentalAttribute="";this._environmentalInitial="";this._userService=null}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}async openCreate(e){this._entryId=e,this._taskId=null,this._error="",this._resetFields(),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}async openEdit(e,t){this._entryId=e,this._taskId=t.id,this._error="",this._name=t.name,this._type=t.type,this._scheduleType=t.schedule_type,this._intervalDays=t.interval_days?.toString()||"30",this._warningDays=t.warning_days.toString(),this._intervalAnchor=t.interval_anchor||"completion",this._notes=t.notes||"",this._documentationUrl=t.documentation_url||"",this._customIcon=t.custom_icon||"",this._enabled=t.enabled!==!1,this._lastPerformed=t.last_performed||"",this._nfcTagId=t.nfc_tag_id||"",this._responsibleUserId=t.responsible_user_id||null;let a=t.adaptive_config||{};if(this._environmentalEntity=a.environmental_entity||"",this._environmentalAttribute=a.environmental_attribute||"",this._environmentalInitial=this._environmentalEntity,t.trigger_config){let s=t.trigger_config;this._triggerEntityId=s.entity_id||"",this._triggerEntityIds=s.entity_ids||(s.entity_id?[s.entity_id]:[]),this._triggerEntityLogic=s.entity_logic||"any",this._triggerAttribute=s.attribute||"",this._triggerType=s.type||"threshold",this._triggerAbove=s.trigger_above?.toString()||"",this._triggerBelow=s.trigger_below?.toString()||"",this._triggerForMinutes=s.trigger_for_minutes?.toString()||"0",this._triggerTargetValue=s.trigger_target_value?.toString()||"",this._triggerDeltaMode=s.trigger_delta_mode||!1,this._triggerFromState=s.trigger_from_state||"",this._triggerToState=s.trigger_to_state||"",this._triggerTargetChanges=s.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=s.trigger_runtime_hours?.toString()||""}else this._resetTriggerFields();this._triggerEntityId&&this._fetchEntityAttributes(this._triggerEntityId),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._warningDays="7",this._intervalAnchor="completion",this._notes="",this._documentationUrl="",this._customIcon="",this._enabled=!0,this._lastPerformed="",this._nfcTagId="",this._responsibleUserId=null,this._environmentalEntity="",this._environmentalAttribute="",this._environmentalInitial="",this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerForMinutes="0",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours=""}async _loadUsers(){this._userService||(this._userService=new oe(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}async _loadTags(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tags/list"});this._availableTags=e.tags||[]}catch{this._availableTags=[]}}async _fetchEntityAttributes(e){if(!e||!this.hass){this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="";return}try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/entity/attributes",entity_id:e});this._entityDomain=t.domain||"",this._suggestedAttributes=t.suggested_attributes||[],this._availableAttributes=t.available_attributes||[]}catch{this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain=""}}async _save(){if(this._name.trim()){this._loading=!0,this._error="";try{let e={type:this._taskId?"maintenance_supporter/task/update":"maintenance_supporter/task/create",entry_id:this._entryId,name:this._name,task_type:this._type,schedule_type:this._scheduleType,warning_days:parseInt(this._warningDays,10)||7};if(this._taskId&&(e.task_id=this._taskId),this._scheduleType!=="manual"?this._intervalDays?(e.interval_days=parseInt(this._intervalDays,10),e.interval_anchor=this._intervalAnchor):this._taskId&&(e.interval_days=null,e.interval_anchor="completion"):this._taskId&&(e.interval_days=null,e.interval_anchor="completion"),e.notes=this._notes||null,e.documentation_url=this._documentationUrl||null,e.custom_icon=this._customIcon||null,e.enabled=this._enabled,e.last_performed=this._lastPerformed||null,e.nfc_tag_id=this._nfcTagId||null,e.responsible_user_id=this._responsibleUserId,this._scheduleType==="sensor_based"&&this._triggerEntityId){let s=this._triggerEntityIds.length>0?this._triggerEntityIds:[this._triggerEntityId],o={entity_id:s[0],entity_ids:s,type:this._triggerType};if(this._triggerAttribute&&(o.attribute=this._triggerAttribute),s.length>1&&(o.entity_logic=this._triggerEntityLogic),this._triggerType==="threshold"){if(this._triggerAbove){let c=parseFloat(this._triggerAbove);isNaN(c)||(o.trigger_above=c)}if(this._triggerBelow){let c=parseFloat(this._triggerBelow);isNaN(c)||(o.trigger_below=c)}if(this._triggerForMinutes){let c=parseInt(this._triggerForMinutes,10);isNaN(c)||(o.trigger_for_minutes=c)}}else if(this._triggerType==="counter"){if(this._triggerTargetValue){let c=parseFloat(this._triggerTargetValue);isNaN(c)||(o.trigger_target_value=c)}o.trigger_delta_mode=this._triggerDeltaMode}else if(this._triggerType==="state_change"){if(this._triggerFromState&&(o.trigger_from_state=this._triggerFromState),this._triggerToState&&(o.trigger_to_state=this._triggerToState),this._triggerTargetChanges){let c=parseInt(this._triggerTargetChanges,10);isNaN(c)||(o.trigger_target_changes=c)}}else if(this._triggerType==="runtime"&&this._triggerRuntimeHours){let c=parseFloat(this._triggerRuntimeHours);isNaN(c)||(o.trigger_runtime_hours=c)}e.trigger_config=o}else this._taskId&&(e.trigger_config=null);let t=await this.hass.connection.sendMessagePromise(e),a=this._taskId||t?.task_id;if(a&&this._scheduleType==="sensor_based"&&this._environmentalEntity!==this._environmentalInitial)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:a,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null})}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch{this._error=n("save_error",this._lang)}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return d;let e=this._lang;return l`
      ...
      <div class="select-row">
        ...
        >
          ${la.map(t=>l`<option value=${t} ?selected=${t===this._triggerType}>${n(t,e)}</option>`)}
        </select>
      </div>
    ...
    `}_renderTriggerTypeFields(){let e=this._lang;return this._triggerType==="threshold"?l`
      ...
      `:this._triggerType==="runtime"?l`
        ...
        ></ha-textfield>
      `:d}render(){if(!this._open)return l``;let e=this._lang,t=this._taskId?n("edit_task",e):n("new_task",e);return l`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
          ${this._error?l`<div class="error">${this._error}</div>`:d}
          ...
          <div class="select-row">
            ...
            >
              ${ra.map(a=>l`<option value=${a} ?selected=${a===this._type}>${n(a,e)}</option>`)}
            </select>
          </div>
          <div class="select-row">
            ...
            >
              ${oa.map(a=>l`<option value=${a} ?selected=${a===this._scheduleType}>${n(a,e)}</option>`)}
            </select>
          </div>
  ...
  `,u([S({attribute:!1})],y.prototype,"hass",2),u([g()],y.prototype,"_open",2),u([g()],y.prototype,"_loading",2),u([g()],y.prototype,"_error",2),u([g()],y.prototype,"_entryId",2),u([g()],y.prototype,"_taskId",2),u([g()],y.prototype,"_name",2),u([g()],y.prototype,"_type",2),u([g()],y.prototype,"_scheduleType",2),u([g()],y.prototype,"_intervalDays",2),u([g()],y.prototype,"_warningDays",2),u([g()],y.prototype,"_intervalAnchor",2),u([g()],y.prototype,"_notes",2),u([g()],y.prototype,"_documentationUrl",2),u([g()],y.prototype,"_customIcon",2),u([g()],y.prototype,"_enabled",2),u([g()],y.prototype,"_triggerEntityId",2),u([g()],y.prototype,"_triggerEntityIds",2),u([g()],y.prototype,"_triggerEntityLogic",2),u([g()],y.prototype,"_triggerAttribute",2),u([g()],y.prototype,"_triggerType",2),u([g()],y.prototype,"_triggerAbove",2),u([g()],y.prototype,"_triggerBelow",2),u([g()],y.prototype,"_triggerForMinutes",2),u([g()],y.prototype,"_triggerTargetValue",2),u([g()],y.prototype,"_triggerDeltaMode",2),u([g()],y.prototype,"_triggerFromState",2),u([g()],y.prototype,"_triggerToState",2),u([g()],y.prototype,"_triggerTargetChanges",2),u([g()],y.prototype,"_triggerRuntimeHours",2),u([g()],y.prototype,"_suggestedAttributes",2),u([g()],y.prototype,"_availableAttributes",2),u([g()],y.prototype,"_entityDomain",2),u([g()],y.prototype,"_lastPerformed",2),u([g()],y.prototype,"_nfcTagId",2),u([g()],y.prototype,"_availableTags",2),u([g()],y.prototype,"_responsibleUserId",2),u([g()],y.prototype,"_availableUsers",2),u([g()],y.prototype,"_environmentalEntity",2),u([g()],y.prototype,"_environmentalAttribute",2);customElements.get("maintenance-task-dialog")||customElements.define("maintenance-task-dialog",y);var N=class extends C{constructor(){super(...arguments);this.entryId="";this.taskId="";this.taskName="";this.lang="en";this.checklist=[];this.adaptiveEnabled=!1;this._open=!1;this._notes="";this._cost="";this._duration="";this._loading=!1;this._error="";this._checklistState={};this._feedback="needed"}open(){this._open||(this._open=!0,this._notes="",this._cost="",this._duration="",this._error="",this._checklistState={},this._feedback="needed")}_toggleCheck(e){let t=String(e);this._checklistState={...this._checklistState,[t]:!this._checklistState[t]}}_setFeedback(e){this._feedback=e}async _complete(){this._loading=!0,this._error="";try{let e={type:"maintenance_supporter/task/complete",entry_id:this.entryId,task_id:this.taskId};if(this._notes&&(e.notes=this._notes),this._cost){let t=parseFloat(this._cost);!isNaN(t)&&t>=0&&(e.cost=t)}if(this._duration){let t=parseInt(this._duration,10);!isNaN(t)&&t>=0&&(e.duration=t)}this.checklist.length>0&&(e.checklist_state=this._checklistState),this.adaptiveEnabled&&(e.feedback=this._feedback),await this.hass.connection.sendMessagePromise(e),this._open=!1,this.dispatchEvent(new CustomEvent("task-completed"))}catch{this._error=n("save_error",this.lang)}finally{this._loading=!1}}_close(){this._open=!1}render(){if(!this._open)return l``;let e=this.lang||this.hass?.language||"en";return l`
      <ha-dialog open @closed=${this._close}>
  ...
  `,u([S({attribute:!1})],N.prototype,"hass",2),u([S()],N.prototype,"entryId",2),u([S()],N.prototype,"taskId",2),u([S()],N.prototype,"taskName",2),u([S()],N.prototype,"lang",2),u([S({type:Array})],N.prototype,"checklist",2),u([S({type:Boolean})],N.prototype,"adaptiveEnabled",2),u([g()],N.prototype,"_open",2),u([g()],N.prototype,"_notes",2),u([g()],N.prototype,"_cost",2),u([g()],N.prototype,"_duration",2),u([g()],N.prototype,"_loading",2),u([g()],N.prototype,"_error",2),u([g()],N.prototype,"_checklistState",2),u([g()],N.prototype,"_feedback",2);customElements.get("maintenance-complete-dialog")||customElements.define("maintenance-complete-dialog",N);function le(r){return r.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function gt(r){return!r.startsWith("data:image/svg+xml,")&&!r.startsWith("data:image/png;base64,")?"":le(r)}function da(r){return r.replace(/[/\\:*?"<>|#%]+/g,"").replace(/\s+/g,"-").toLowerCase().substring(0,100)}var V=class extends C{constructor(){super(...arguments);this.lang="en";this._open=!1;this._loading=!1;this._error="";this._viewResult=null;this._completeResult=null;this._urlMode="companion";this._entryId="";this._taskId=null;this._objectName="";this._taskName="";this._generateSeq=0}openForObject(e,t){this._entryId=e,this._taskId=null,this._objectName=t,this._taskName="",this._urlMode="companion",this._error="",this._viewResult=null,this._completeResult=null,this._open=!0,this._generate()}openForTask(e,t,a,s){this._entryId=e,this._taskId=t,this._objectName=a,this._taskName=s,this._urlMode="companion",this._error="",this._viewResult=null,this._completeResult=null,this._open=!0,this._generate()}async _generate(){let e=++this._generateSeq;this._loading=!0,this._error="",this._viewResult=null,this._completeResult=null;try{let t={type:"maintenance_supporter/qr/generate",entry_id:this._entryId,url_mode:this._urlMode};this._taskId&&(t.task_id=this._taskId);let a=[this.hass.connection.sendMessagePromise({...t,action:"view"})];this._taskId&&a.push(this.hass.connection.sendMessagePromise({...t,action:"complete"}));let s=await Promise.all(a);if(e!==this._generateSeq)return;this._viewResult=s[0],s.length>1&&(this._completeResult=s[1])}catch(t){if(e!==this._generateSeq)return;let a=t?.code,s=t?.message;this._error=a==="no_url"||typeof s=="string"&&s.includes("No Home Assistant URL")?n("qr_error_no_url",this.lang):n("qr_error",this.lang)}finally{e===this._generateSeq&&(this._loading=!1)}}_setUrlMode(e){this._urlMode!==e&&(this._urlMode=e,this._generate())}_print(){if(!this._viewResult)return;let e=this._viewResult,t=e.label.task_name?`${e.label.object_name} \u2014 ${e.label.task_name}`:e.label.object_name,a=[e.label.manufacturer,e.label.model].filter(Boolean).join(" "),s=window.open("","_blank","width=600,height=500");if(!s)return;let o=this.lang||"en",c=le(t),p=le(a),_=!!this._completeResult,m=le(n("qr_action_view",o)),v=le(n("qr_action_complete",o));s.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8">
<title>${c}</title>
<style>
...
<script>setTimeout(()=>window.print(),300)<\/script>
</body></html>`),s.document.close()}_downloadSvg(e,t){let a=decodeURIComponent(e.svg_data_uri.replace("data:image/svg+xml,","")),s=new Blob([a],{type:"image/svg+xml"}),o=URL.createObjectURL(s),c=document.createElement("a");c.href=o;let p=this._taskName?`${this._objectName}-${this._taskName}`:this._objectName;c.download=`qr-${da(p)}-${t}.svg`,c.click(),URL.revokeObjectURL(o)}_close(){this._open=!1,this._viewResult=null,this._completeResult=null,this._error="",this._loading=!1}render(){if(!this._open)return l``;let e=this.lang||this.hass?.language||"en",t=this._taskName?`${n("qr_code",e)}: ${this._objectName} \u2014 ${this._taskName}`:`${n("qr_code",e)}: ${this._objectName}`,a=!!this._viewResult;return l`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
          ${this._loading?l`<div class="loading">${n("qr_generating",e)}</div>`:this._error?l`<div class="error">${this._error}</div>`:a?l`
                    <div class="qr-pair">
                      <div class="qr-item">
                        <img
  ...
  `,u([S({attribute:!1})],B.prototype,"hass",2),u([g()],B.prototype,"_open",2),u([g()],B.prototype,"_title",2),u([g()],B.prototype,"_message",2),u([g()],B.prototype,"_confirmText",2),u([g()],B.prototype,"_danger",2),u([g()],B.prototype,"_inputLabel",2),u([g()],B.prototype,"_inputType",2),u([g()],B.prototype,"_inputValue",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",B);var ca=["EUR","USD","GBP","JPY","CHF","CAD","AUD","CNY","INR","BRL"],U=class extends C{constructor(){super(...arguments);this.budget=null;this._settings=null;this._loading=!0;this._importCsv="";this._importLoading=!1;this._includeHistory=!0;this._toast="";this._testingNotification=!1;this._loaded=!1;this._sendTestNotification=async()=>{this._testingNotification=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/test_notification"}),t=e.message||(e.success?n("test_notification_success",this._lang):n("test_notification_failed",this._lang));this._showToast(t)}catch{this._showToast(n("test_notification_failed",this._lang))}finally{this._testingNotification=!1}}}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._loaded&&(this._loaded=!0,this._loadSettings())}async _loadSettings(){this._loading=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});this._settings=e}catch{}this._loading=!1}async _updateSetting(e,t){try{let a=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/update",settings:{[e]:t}});this._settings=a,this._showToast(n("settings_saved",this._lang)),this.dispatchEvent(new CustomEvent("settings-changed"))}catch{this._showToast(n("action_error",this._lang))}}_showToast(e){this._toast=e,setTimeout(()=>{this._toast=""},3e3)}_downloadFile(e,t,a){let s=new Blob([e],{type:a}),o=URL.createObjectURL(s),c=document.createElement("a");c.href=o,c.download=t,c.click(),URL.revokeObjectURL(o)}render(){let e=this._lang;return this._loading||!this._settings?l`<div class="settings-loading">Loading…</div>`:l`
      ...
      <div class="settings-section">
        ...
        <label class="setting-row">
          ...
          <select @change=${a=>this._updateSetting("budget_currency",a.target.value)}>
            ${ca.map(a=>l`<option value=${a} ?selected=${t.currency===a}>${a}</option>`)}
          </select>
        </label>
        <label class="setting-row">
  ...
  `,u([S({attribute:!1})],U.prototype,"hass",2),u([S({attribute:!1})],U.prototype,"features",2),u([S({attribute:!1})],U.prototype,"budget",2),u([g()],U.prototype,"_settings",2),u([g()],U.prototype,"_loading",2),u([g()],U.prototype,"_importCsv",2),u([g()],U.prototype,"_importLoading",2),u([g()],U.prototype,"_includeHistory",2),u([g()],U.prototype,"_toast",2),u([g()],U.prototype,"_testingNotification",2);customElements.define("maintenance-settings-view",U);var _a=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"],W=class extends C{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._entryId="";this._taskId="";this._values=new Array(12).fill("");this._save=async()=>{let e=this._buildOverrides();if(e!==null){this._loading=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/seasonal_overrides",entry_id:this._entryId,task_id:this._taskId,overrides:e}),this._open=!1,this.dispatchEvent(new CustomEvent("overrides-saved"))}catch(t){this._error=t?.message||n("save_error",this._lang)}finally{this._loading=!1}}};this._clearAll=async()=>{this._loading=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/seasonal_overrides",entry_id:this._entryId,task_id:this._taskId,overrides:{}}),this._values=new Array(12).fill(""),this._open=!1,this.dispatchEvent(new CustomEvent("overrides-saved"))}catch(e){this._error=e?.message||n("save_error",this._lang)}finally{this._loading=!1}}}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}open(e,t,a){if(this._entryId=e,this._taskId=t,this._values=new Array(12).fill(""),a)for(let[s,o]of Object.entries(a)){let c=parseInt(s,10);c>=1&&c<=12&&typeof o=="number"&&(this._values[c-1]=o.toString())}this._error="",this._open=!0}_close(){this._open=!1}_buildOverrides(){let e={};for(let t=0;t<12;t++){let a=this._values[t].trim();if(!a)continue;let s=parseFloat(a);if(Number.isNaN(s))return this._error=`${n("month_"+["jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec"][t],this._lang)}: ${n("seasonal_override_invalid",this._lang)}`,null;if(s<.1||s>5)return this._error=n("seasonal_override_range",this._lang),null;e[t+1]=s}return e}render(){if(!this._open)return l``;let e=this._lang;return l`
    ...
    .spacer { flex: 1; }
  `,u([S({attribute:!1})],W.prototype,"hass",2),u([g()],W.prototype,"_open",2),u([g()],W.prototype,"_loading",2),u([g()],W.prototype,"_error",2),u([g()],W.prototype,"_entryId",2),u([g()],W.prototype,"_taskId",2),u([g()],W.prototype,"_values",2);customElements.get("maintenance-seasonal-overrides-dialog")||customElements.define("maintenance-seasonal-overrides-dialog",W);var ua=300,pa=140,Oe=27;function mt(r,i){let e=r.trigger_config;if(!e)return d;let t=i.lang,a=r.trigger_entity_info,s=r.trigger_entity_infos,o=a?.friendly_name||e.entity_id||"\u2014",c=e.entity_id||"",p=e.entity_ids||(c?[c]:[]),_=a?.unit_of_measurement||"",m=r.trigger_current_value,v=e.type||"threshold",b=p.length>1;return l`
    <h3>${n("trigger",t)}</h3>
    <div class="trigger-card">
      <div class="trigger-header">
        ...
        <div>
          ${b?l`
            ...
            <div class="trigger-entity-id">${p.map((f,k)=>l`${k>0?", ":""}<span class="entity-link" @click=${T=>ne(T,f)}>${f}</span>`)}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `:l`
            <div class="trigger-entity-name">${o}</div>
            <div class="trigger-entity-id">${c?l`<span class="entity-link" @click=${f=>ne(f,c)}>${c}</span>`:""}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `}
        </div>
        <span class="status-badge ${r.trigger_active?"triggered":"ok"}" style="margin-left: auto;">
          ${r.trigger_active?n("triggered",t):n("ok",t)}
        </span>
      </div>
      ...
      <div class="trigger-limits">
        ${v==="threshold"?l`
          ${e.trigger_above!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${n("threshold_above",t)}: ${e.trigger_above} ${_}</span>`:d}
          ${e.trigger_below!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${n("threshold_below",t)}: ${e.trigger_below} ${_}</span>`:d}
          ${e.trigger_for_minutes?l`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${n("for_minutes",t)}: ${e.trigger_for_minutes}</span>`:d}
        `:d}
        ${v==="counter"?l`
          ${e.trigger_target_value!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${n("target_value",t)}: ${e.trigger_target_value} ${_}</span>`:d}
        `:d}
        ${v==="state_change"?l`
          ${e.trigger_target_changes!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${n("target_changes",t)}: ${e.trigger_target_changes}</span>`:d}
        `:d}
        ${v==="runtime"?l`
          ${e.trigger_runtime_hours!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${n("runtime_hours",t)}: ${e.trigger_runtime_hours}h</span>`:d}
        `:d}
        ${v==="compound"?l`
          ...
          ${(e.conditions||[]).map((f,k)=>l`
            <span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${k+1}. ${n(f.type||"unknown",t)}: ${f.entity_id?l`<span class="entity-link" @click=${T=>ne(T,f.entity_id)}>${f.entity_id}</span>`:""}</span>
          `)}
        `:d}
        ${a?.min!=null?l`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${n("min",t)}: ${a.min} ${_}</span>`:d}
        ${a?.max!=null?l`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${n("max",t)}: ${a.max} ${_}</span>`:d}
      </div>

      ${s&&s.length>1?l`
        <div class="trigger-entity-list">
          ${s.map(f=>l`
            <span class="trigger-entity-id">${f.friendly_name} (<span class="entity-link" @click=${k=>ne(k,f.entity_id)}>${f.entity_id}</span>)</span>
          `)}
        </div>
      `:d}
  ...
  `}function ga(r,i,e){let t=r.trigger_config;if(!t)return d;let a=t.type||"threshold",s=a==="counter"&&t.trigger_delta_mode,o=e.isCounterEntity(t),c=t.entity_id||"",p=e.detailStatsData.get(c)||[],_=[],m=!1;if(p.length>=2)for(let h of p){let w=h.val;s&&r.trigger_baseline_value!=null&&(w-=r.trigger_baseline_value);let R={ts:h.ts,val:w};!o&&h.min!=null&&h.max!=null&&(R.min=s&&r.trigger_baseline_value!=null?h.min-r.trigger_baseline_value:h.min,R.max=s&&r.trigger_baseline_value!=null?h.max-r.trigger_baseline_value:h.max,m=!0),_.push(R)}else for(let h of r.history)h.trigger_value!=null&&_.push({ts:new Date(h.timestamp).getTime(),val:h.trigger_value});if(r.trigger_current_value!=null){let h=r.trigger_current_value;s&&r.trigger_baseline_value!=null&&(h-=r.trigger_baseline_value),_.push({ts:Date.now(),val:h})}if(_.length<2&&c&&e.hasStatsService&&!e.detailStatsData.has(c))return l`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
      <ha-icon icon="mdi:chart-line" style="--mdc-icon-size:16px;margin-right:8px;"></ha-icon>
      ${n("loading_chart",e.lang)}
    </div>`;if(_.length<2)return d;_.sort((h,w)=>h.ts-w.ts);let v=ua,b=pa,f=30,k=2,T=8,q=16,z=_.map(h=>h.val),A=Math.min(...z),x=Math.max(...z);if(m)for(let h of _)h.min!=null&&(A=Math.min(A,h.min)),h.max!=null&&(x=Math.max(x,h.max));t.trigger_above!=null&&(x=Math.max(x,t.trigger_above),A=Math.min(A,t.trigger_above)),t.trigger_below!=null&&(A=Math.min(A,t.trigger_below),x=Math.max(x,t.trigger_below));let I=null,O=null;if(a==="counter"&&t.trigger_target_value!=null){if(r.trigger_baseline_value!=null)I=r.trigger_baseline_value;else if(_.length>0){let h=[...r.history].filter(w=>w.type==="completed"||w.type==="reset").sort((w,R)=>new Date(R.timestamp).getTime()-new Date(w.timestamp).getTime())[0];if(h){let w=new Date(h.timestamp).getTime(),R=_[0],P=Math.abs(_[0].ts-w);for(let Z of _){let Q=Math.abs(Z.ts-w);Q<P&&(R=Z,P=Q)}I=R.val}else I=_[0].val}I!=null?(O=I+t.trigger_target_value,x=Math.max(x,O),A=Math.min(A,I)):(x=Math.max(x,t.trigger_target_value),A=Math.min(A,0))}s&&r.trigger_baseline_value!=null&&(A=Math.min(A,0));let K=x-A||1;A-=K*.1,x+=K*.1;let G=_[0].ts,D=_[_.length-1].ts,$=D-G||1,j=h=>f+(h-G)/$*(v-f-k),M=h=>T+(1-(h-A)/(x-A))*(b-T-q),Ae=_.map(h=>`${j(h.ts).toFixed(1)},${M(h.val).toFixed(1)}`).join(" "),wt=`M${j(_[0].ts).toFixed(1)},${b-q} `+_.map(h=>`L${j(h.ts).toFixed(1)},${M(h.val).toFixed(1)}`).join(" ")+` L${j(_[_.length-1].ts).toFixed(1)},${b-q} Z`,Se="";if(m){let h=_.filter(w=>w.min!=null&&w.max!=null);if(h.length>=2){let w=h.map(P=>`${j(P.ts).toFixed(1)},${M(P.max).toFixed(1)}`),R=[...h].reverse().map(P=>`${j(P.ts).toFixed(1)},${M(P.min).toFixed(1)}`);Se=`M${w[0]} `+w.slice(1).map(P=>`L${P}`).join(" ")+` L${R[0]} `+R.slice(1).map(P=>`L${P}`).join(" ")+" Z"}}let Ue=_[_.length-1],Et=j(Ue.ts),At=M(Ue.val),Be=h=>Math.abs(h)>=1e4?(h/1e3).toFixed(0)+"k":h>=1e3?(h/1e3).toFixed(1)+"k":h.toFixed(h<10?1:0),St=Be(x),Tt=Be(A),Mt=r.history.filter(h=>["completed","skipped","reset"].includes(h.type)).map(h=>({ts:new Date(h.timestamp).getTime(),type:h.type})).filter(h=>h.ts>=G&&h.ts<=D),Te=_;if(_.length>Oe){let h=(_.length-1)/(Oe-1);Te=[];for(let w=0;w<Oe;w++)Te.push(_[Math.round(w*h)])}return l`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${v} ${b}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_sparkline",e.lang)}">
        <text x="${f-3}" y="${T+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${St}</text>
        <text x="${f-3}" y="${b-q+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${Tt}</text>
        <text x="${f}" y="${b-1}" text-anchor="start" fill="var(--secondary-text-color)" font-size="7">${new Date(G).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        ...
        ${Se?L`<path d="${Se}" fill="var(--primary-color)" opacity="0.08" />`:d}
        <path d="${wt}" fill="var(--primary-color)" opacity="0.15" />
        <polyline points="${Ae}" fill="none" stroke="var(--primary-color)" stroke-width="2" stroke-linejoin="round" />
        ${r.degradation_rate!=null&&r.degradation_trend!=="stable"&&r.degradation_trend!=="insufficient_data"&&_.length>=2?(()=>{let h=_[_.length-1],w=30,R=h.ts+w*864e5,P=h.val+r.degradation_rate*w,Z=Math.min(R,D+(D-G)*.3),Q=Math.max(A,Math.min(x,P)),Ct=j(h.ts),zt=M(h.val),It=j(Z),jt=M(Q);return L`<line x1="${Ct.toFixed(1)}" y1="${zt.toFixed(1)}" x2="${It.toFixed(1)}" y2="${jt.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():d}
        ${a==="threshold"&&t.trigger_above!=null?L`<line x1="${f}" y1="${M(t.trigger_above).toFixed(1)}" x2="${v}" y2="${M(t.trigger_above).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${v-2}" y="${M(t.trigger_above)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25B2 ${t.trigger_above}</text>`:d}
        ${a==="threshold"&&t.trigger_below!=null?L`<line x1="${f}" y1="${M(t.trigger_below).toFixed(1)}" x2="${v}" y2="${M(t.trigger_below).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${v-2}" y="${M(t.trigger_below)+11}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25BC ${t.trigger_below}</text>`:d}
        ${a==="counter"&&O!=null?L`<line x1="${f}" y1="${M(O).toFixed(1)}" x2="${v}" y2="${M(O).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${v-2}" y="${M(O)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">${n("target_value",e.lang)}: +${t.trigger_target_value}</text>`:d}
        ${a==="counter"&&I!=null?L`<line x1="${f}" y1="${M(I).toFixed(1)}" x2="${v}" y2="${M(I).toFixed(1)}" stroke="var(--secondary-text-color)" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
                <text x="${f+2}" y="${M(I)+11}" text-anchor="start" fill="var(--secondary-text-color)" font-size="8">${n("baseline",e.lang)}</text>`:d}
        <circle cx="${Et.toFixed(1)}" cy="${At.toFixed(1)}" r="3.5" fill="var(--primary-color)" />
        ${Mt.map(h=>{let w=j(h.ts),R=h.type==="completed"?"var(--success-color, #4caf50)":h.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return L`
          ...
          `})}
        ${Te.map(h=>{let w=j(h.ts),R=M(h.val),P=new Date(h.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),Z=`${h.val.toFixed(1)} ${i}`;return m&&h.min!=null&&h.max!=null&&(Z+=` (${h.min.toFixed(1)}\u2013${h.max.toFixed(1)})`),L`<circle cx="${w.toFixed(1)}" cy="${R.toFixed(1)}" r="8" fill="transparent" tabindex="0"
            @mouseenter=${Q=>ht(Q,`${P}
${Z}`,e.setTooltip)}
            @focus=${Q=>ht(Q,`${P}
${Z}`,e.setTooltip)}
...
`).map(h=>l`<div>${h}</div>`)}
  ...
  `}function ht(r,i,e){let t=r.currentTarget,a=t.closest(".sparkline-container");if(!a)return;let s=a.getBoundingClientRect(),o=t.getBoundingClientRect();e({x:o.left-s.left+o.width/2,y:o.top-s.top-8,text:i})}function vt(r,i,e){let t=r.degradation_trend!=null&&r.degradation_trend!=="insufficient_data",a=r.days_until_threshold!=null,s=r.environmental_factor!=null&&r.environmental_factor!==1;if(!t&&!a&&!s)return d;let o=r.degradation_trend==="rising"?"M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z":r.degradation_trend==="falling"?"M16,18L18.29,15.71L13.41,10.83L9.41,14.83L2,7.41L3.41,6L9.41,12L13.41,8L19.71,14.29L22,12V18H16Z":"M22,12L18,8V11H3V13H18V16L22,12Z";return l`
    <div class="prediction-section">
      ${r.sensor_prediction_urgency?l`
        <div class="prediction-urgency-banner">
          <ha-svg-icon path="M1,21H23L12,2L1,21M12,18A1,1 0 0,1 11,17A1,1 0 0,1 12,16A1,1 0 0,1 13,17A1,1 0 0,1 12,18M13,15H11V10H13V15Z"></ha-svg-icon>
          ${n("sensor_prediction_urgency",i).replace("{days}",String(Math.round(r.days_until_threshold||0)))}
        </div>
      `:d}
      <div class="prediction-title">
        <ha-svg-icon path="M2,2V4H7V2H2M22,2V4H13V2H22M7,7V9H2V7H7M22,7V9H13V7H22M7,12V14H2V12H7M22,12V14H13V12H22M7,17V19H2V17H7M22,17V19H13V17H22M9,2V19L12,22L15,19V2H9M11,4H13V17.17L12,18.17L11,17.17V4Z"></ha-svg-icon>
        ${n("sensor_prediction",i)}
      </div>
      <div class="prediction-grid">
        ${t?l`
          <div class="prediction-item">
            <ha-svg-icon path="${o}"></ha-svg-icon>
            <span class="prediction-label">${n("degradation_trend",i)}</span>
            <span class="prediction-value ${r.degradation_trend}">${n("trend_"+r.degradation_trend,i)}</span>
            ${r.degradation_rate!=null?l`<span class="prediction-rate">${r.degradation_rate>0?"+":""}${Math.abs(r.degradation_rate)>=10?Math.round(r.degradation_rate).toLocaleString():r.degradation_rate.toFixed(1)} ${r.trigger_entity_info?.unit_of_measurement||""}/${n("day_short",i)}</span>`:d}
          </div>
        `:d}
        ${a?l`
          <div class="prediction-item">
            <ha-svg-icon path="M12,20A7,7 0 0,1 5,13A7,7 0 0,1 12,6A7,7 0 0,1 19,13A7,7 0 0,1 12,20M12,4A9,9 0 0,0 3,13A9,9 0 0,0 12,22A9,9 0 0,0 21,13A9,9 0 0,0 12,4M12.5,8H11V14L15.75,16.85L16.5,15.62L12.5,13.25V8M7.88,3.39L6.6,1.86L2,5.71L3.29,7.24L7.88,3.39M22,5.72L17.4,1.86L16.11,3.39L20.71,7.25L22,5.72Z"></ha-svg-icon>
            <span class="prediction-label">${n("days_until_threshold",i)}</span>
            <span class="prediction-value prediction-days${r.days_until_threshold===0?" exceeded":r.sensor_prediction_urgency?" urgent":""}">${r.days_until_threshold===0?n("threshold_exceeded",i):"~"+Math.round(r.days_until_threshold)+" "+n("days",i)}</span>
            ${r.threshold_prediction_date?l`<span class="prediction-date">${X(r.threshold_prediction_date,i)}</span>`:d}
  ...
  `}function ma(r,i,e,t,a){let f=Math.max(e,t,i,1)*1.3,k=50,T=[];for(let D=0;D<=k;D++){let $=D/k*f,j=1-Math.exp(-Math.pow($/i,r)),M=32+$/f*260,Ae=136-j*128;T.push([M,Ae])}let q=T.map(([D,$])=>`${D.toFixed(1)},${$.toFixed(1)}`).join(" "),z="M32,136 "+T.map(([D,$])=>`L${D.toFixed(1)},${$.toFixed(1)}`).join(" ")+` L${T[k][0].toFixed(1)},136 Z`,A=32+e/f*260,x=1-Math.exp(-Math.pow(e/i,r)),I=136-x*128,O=((1-x)*100).toFixed(0),K=32+t/f*260,G=[0,.25,.5,.75,1];return l`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_weibull",a)}">
        ...
        ${e>0?L`
          ...
          <circle cx="${A.toFixed(1)}" cy="${I.toFixed(1)}" r="3"
            fill="var(--primary-color, #03a9f4)" />
          <text x="${(A+4).toFixed(1)}" y="${(I-6).toFixed(1)}" fill="var(--primary-color, #03a9f4)"
    ...
    <div class="chart-legend">
      ...
      ${e>0?l`<span class="legend-item"><span class="legend-swatch" style="background:var(--primary-color, #03a9f4); opacity:0.5"></span> ${n("current_interval_marker",a)}</span>`:d}
      ${t>0&&t!==e?l`<span class="legend-item"><span class="legend-swatch" style="background:var(--success-color, #4caf50)"></span> ${n("recommended_marker",a)}</span>`:d}
    </div>
  `}function va(r,i){return l`
    <div class="weibull-info-row">
      <div class="weibull-info-item">
        <span>${n("characteristic_life",i)}</span>
        <span class="weibull-info-value">${Math.round(r.weibull_eta)} ${n("days",i)}</span>
      </div>
      ${r.weibull_r_squared!=null?l`
        <div class="weibull-info-item">
          <span>${n("weibull_r_squared",i)}</span>
          <span class="weibull-info-value">${r.weibull_r_squared.toFixed(3)}</span>
        </div>
      `:d}
    </div>
  `}function fa(r,i,e){let t=r.confidence_interval_low,a=r.confidence_interval_high,s=i.suggested_interval??i.interval_days??0,o=i.interval_days??0,c=Math.max(0,t-5),_=a+5-c,m=(t-c)/_*100,v=(a-t)/_*100,b=(s-c)/_*100,f=o>0?(o-c)/_*100:-1;return l`
    <div class="confidence-range">
      <div class="confidence-range-title">
        ${n("confidence_interval",e)}: ${s} ${n("days",e)} (${t}\u2013${a})
      </div>
  ...
  `}var bt=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"];function yt(r,i,e){if(!e.seasonal||!r.seasonal_factor||r.seasonal_factor===1)return d;let t=bt.map(c=>n(c,i)),a=new Date().getMonth(),s=r.seasonal_factors||r.interval_analysis?.seasonal_factors||null,o=s&&s.length===12?s:t.map((c,p)=>{let _=r.seasonal_factor||1,m=Math.sin((p-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,_+m))});return l`
    <div class="seasonal-card-compact">
      <h4>${n("seasonal_awareness",i)}</h4>
      <div class="seasonal-mini-chart">
        ${o.map((c,p)=>{let _=c*40,m=c<.9?"low":c>1.1?"high":"normal";return l`
            <div class="seasonal-bar ${m} ${p===a?"current":""}"
                 style="height: ${_}px"
      ...
      <div class="seasonal-legend">
        <span class="legend-item"><span class="dot low"></span> ${n("shorter",i)||"K\xFCrzer"}</span>
        <span class="legend-item"><span class="dot normal"></span> ${n("normal",i)||"Normal"}</span>
        <span class="legend-item"><span class="dot high"></span> ${n("longer",i)||"L\xE4nger"}</span>
      </div>
    </div>
  `}function xt(r,i){return ba(r,i)}function ba(r,i){let e=r.seasonal_factors??r.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return d;let t=r.interval_analysis?.seasonal_reason,a=new Date().getMonth(),s=300,o=100,c=8,_=o-c-4,m=Math.max(...e,1.5),v=s/12,b=v*.65,f=c+_-1/m*_;return l`
    <div class="seasonal-chart">
      <div class="seasonal-chart-title">
        ...
        ${t?l`<span class="source-tag">${t==="learned"?n("seasonal_learned",i):n("seasonal_manual",i)}</span>`:d}
      </div>
      <svg viewBox="0 0 ${s} ${o}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_seasonal",i)}">
        <line x1="0" y1="${f.toFixed(1)}" x2="${s}" y2="${f.toFixed(1)}"
          stroke="var(--divider-color)" stroke-width="1" stroke-dasharray="4,3" />
        ${e.map((k,T)=>{let q=k/m*_,z=T*v+(v-b)/2,A=c+_-q,x=T===a,I=k<1?"var(--success-color, #4caf50)":k>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return L`
            <rect x="${z.toFixed(1)}" y="${A.toFixed(1)}"
              width="${b.toFixed(1)}" height="${q.toFixed(1)}"
              fill="${I}" opacity="${x?1:.5}" rx="2" />
          `})}
      </svg>
      <div class="seasonal-labels">
        ${bt.map((k,T)=>l`<span class="seasonal-label ${T===a?"active-month":""}">${n(k,i)}</span>`)}
      </div>
    </div>
  `}var ya=300,xa=200;function $t(r,i,e,t){let a=r.history.filter(c=>c.type==="completed"&&(c.cost!=null||c.duration!=null));if(a.length<2)return d;let s=a.some(c=>(c.cost??0)>0),o=a.some(c=>(c.duration??0)>0);return!s&&!o?d:l`
    <div class="cost-duration-card">
      <div class="card-header">
        <h3>${n("cost_duration_chart",i)}</h3>
  ...
  `}function $a(r,i,e){let t=r.history.filter($=>$.type==="completed"&&($.cost!=null||$.duration!=null)).map($=>({ts:new Date($.timestamp).getTime(),cost:$.cost??0,duration:$.duration??0})).sort(($,j)=>$.ts-j.ts);if(t.length<2)return d;let a=t.some($=>$.cost>0),s=t.some($=>$.duration>0);if(!a&&!s)return d;let o=e!=="duration"&&a,c=e!=="cost"&&s,p=o||!c&&a,_=c||!o&&s,m=ya,v=xa,b=p?32:8,f=_?32:8,k=8,T=20,q=m-b-f,z=v-k-T,A=Math.max(...t.map($=>$.cost))||1,x=Math.max(...t.map($=>$.duration))||1,I=Math.min(20,q/t.length*.6),O=q/t.length,K=$=>b+O*$+O/2,G=$=>k+z-$/A*z,D=$=>k+z-$/x*z;return l`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${m} ${v}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_history",i)}">
        ${p?t.map(($,j)=>L`
          <rect x="${(K(j)-I/2).toFixed(1)}" y="${G($.cost).toFixed(1)}" width="${I.toFixed(1)}" height="${(k+z-G($.cost)).toFixed(1)}"
            fill="var(--primary-color)" opacity="0.6" rx="2" />
        `):d}
        ${_?L`
          <polyline points="${t.map(($,j)=>`${K(j).toFixed(1)},${D($.duration).toFixed(1)}`).join(" ")}"
            fill="none" stroke="var(--accent-color, #ff9800)" stroke-width="2" stroke-linejoin="round" />
          ${t.map(($,j)=>L`
            <circle cx="${K(j).toFixed(1)}" cy="${D($.duration).toFixed(1)}" r="3" fill="var(--accent-color, #ff9800)" />
          `)}
        `:d}
  ...
  `}var ka=60,wa=20,kt=30,E=class extends C{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._sparklineTooltip=null;this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1};this._actionLoading=!1;this._moreMenuOpen=!1;this._toastMessage="";this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab="dashboard";this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._deepLinkHandled=!1;this._onDialogEvent=async()=>{try{await this._loadData()}catch{}}}get _lang(){return this.hass?.language||"en"}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler);let e=localStorage.getItem("maintenance_supporter_sort");e&&["due_date","object","type","task_name"].includes(e)&&(this._sortMode=e)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new Ee(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new oe(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,a,s,o]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);e&&(this._objects=e.objects),t&&(this._stats=t),a&&(this._budget=a),s&&(this._groups=s.groups||{}),o&&(this._features=o.features),this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("entry_id");if(!t)return;this._deepLinkHandled=!0;let a=e.get("task_id"),s=e.get("action"),o=window.location.pathname+window.location.hash;history.replaceState(history.state,"",o);let c=this._getObject(t);if(!c){this._showOverview();return}if(a){let p=c.tasks.find(_=>_.id===a);if(!p){this._showObject(t);return}this._showTask(t,a),s==="complete"&&requestAnimationFrame(()=>{this._openCompleteDialog(t,a,p.name,this._features.checklists?p.checklist:void 0,this._features.adaptive&&!!p.adaptive_config?.enabled)})}else this._showObject(t)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let a=await this._statsService.getDetailStats(e,t),s=new Map(this._detailStatsData);s.set(e,a),this._detailStatsData=s}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let a of this._objects)for(let s of a.tasks){let o=s.trigger_config?.entity_id;o&&e.push({entityId:o,isCounter:this._isCounterEntity(s.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let p of this._objects)for(let _ of p.tasks)if(!(this._filterStatus&&_.status!==this._filterStatus)){if(this._filterUser){let m=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(_.responsible_user_id!==m)continue}e.push({entry_id:p.entry_id,task_id:_.id,object_name:p.object.name,task_name:_.name,type:_.type,schedule_type:_.schedule_type,status:_.status,days_until_due:_.days_until_due??null,next_due:_.next_due??null,trigger_active:_.trigger_active,trigger_current_value:_.trigger_current_value??null,trigger_current_delta:_.trigger_current_delta??null,trigger_config:_.trigger_config??null,trigger_entity_info:_.trigger_entity_info??null,times_performed:_.times_performed,total_cost:_.total_cost,interval_days:_.interval_days??null,interval_anchor:_.interval_anchor??null,history:_.history||[],enabled:_.enabled,nfc_tag_id:_.nfc_tag_id??null})}let t={overdue:0,triggered:1,due_soon:2,ok:3},a=(p,_)=>(t[p.status]??9)-(t[_.status]??9),s=(p,_)=>(p.days_until_due??99999)-(_.days_until_due??99999),o=(p,_)=>a(p,_)||s(p,_),c={due_date:o,object:(p,_)=>p.object_name.localeCompare(_.object_name)||o(p,_),type:(p,_)=>p.type.localeCompare(_.type)||o(p,_),task_name:(p,_)=>p.task_name.localeCompare(_.task_name)};return e.sort(c[this._sortMode]),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(s=>s.id===t)}_pushPanelState(e,t,a){let s={msp_view:e,msp_entry:t||null,msp_task:a||null};history.pushState(s,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let a=this._getTask(t.msp_entry,t.msp_task);a?.trigger_config?.entity_id&&this._fetchDetailStats(a.trigger_config.entity_id,this._isCounterEntity(a.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null;let a=this._getTask(e,t);if(a?.trigger_config?.entity_id){let s=a.trigger_config.entity_id,o=this._isCounterEntity(a.trigger_config);this._fetchDetailStats(s,o)}}_showToast(e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}async _deleteObject(e){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:n("delete",this._lang),message:n("confirm_delete_object",this._lang),confirmText:n("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}async _deleteTask(e,t){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:n("delete",this._lang),message:n("confirm_delete_task",this._lang),confirmText:n("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}async _skipTask(e,t,a){this._actionLoading=!0;try{let s={type:"maintenance_supporter/task/skip",entry_id:e,task_id:t};a&&(s.reason=a),await this.hass.connection.sendMessagePromise(s),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}finally{this._actionLoading=!1}}async _resetTask(e,t,a){this._actionLoading=!0;try{let s={type:"maintenance_supporter/task/reset",entry_id:e,task_id:t};a&&(s.date=a),await this.hass.connection.sendMessagePromise(s),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}finally{this._actionLoading=!1}}async _applySuggestion(e,t,a){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:a}),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}_openSeasonalOverrides(e){let t=this.shadowRoot.querySelector("maintenance-seasonal-overrides-dialog");if(!t||!this._selectedEntryId)return;let a=e.adaptive_config?.seasonal_overrides;t.open(this._selectedEntryId,e.id,a)}async _reanalyzeInterval(e,t){try{let a=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:e,task_id:t});a.recommended_interval?this._showToast(`${n("reanalyze_result",this._lang)}: ${a.recommended_interval} ${n("days",this._lang)} (${n(`confidence_${a.confidence}`,this._lang)}, ${a.data_points} ${n("data_points",this._lang)})`):this._showToast(n("reanalyze_insufficient_data",this._lang)),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}async _promptSkipTask(e,t){let a=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!a)return;let s=await a.prompt({title:n("skip",this._lang),message:n("skip_reason_prompt",this._lang),confirmText:n("skip",this._lang),inputLabel:n("reason_optional",this._lang),inputType:"text"});s.confirmed&&this._skipTask(e,t,s.value||void 0)}async _promptResetTask(e,t){let a=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!a)return;let s=await a.prompt({title:n("reset",this._lang),message:n("reset_date_prompt",this._lang),confirmText:n("reset",this._lang),inputLabel:n("reset_date_optional",this._lang),inputType:"date"});s.confirmed&&this._resetTask(e,t,s.value||void 0)}_dismissSuggestion(e,t){e&&t&&this._dismissedSuggestions.add(`${e}_${t}`),this.requestUpdate()}_openCompleteDialog(e,t,a,s,o){let c=this.shadowRoot.querySelector("maintenance-complete-dialog");c&&(c.entryId=e,c.taskId=t,c.taskName=a,c.lang=this._lang,c.checklist=s||[],c.adaptiveEnabled=!!o,c.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,a,s){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,a,s)}render(){return l`
    ...
    `}_renderDashboard(){let e=this._stats,t=this._taskRows,a=this._lang;return l`
      ...
      <div class="filter-bar">
        ...
        <select
          ...
          @change=${s=>{let o=s.target.value;this._filterUser=o||null}}
        >
        ...
        ${this._objects.map(t=>l`
          <div class="object-card" @click=${()=>this._showObject(t.entry_id)}>
            <div class="object-card-header">
              ...
              <span class="object-card-count">${t.tasks.length} ${n("tasks_lower",e)}</span>
            </div>
            ...
            ${t.tasks.length===0?l`<div class="object-card-empty">${n("no_tasks_yet",e)}</div>`:d}
          </div>
        `)}
      </div>
          ...
          ${e.map(([a,s])=>{let o=s.task_refs.map(c=>this._getTask(c.entry_id,c.task_id)?.name).filter(Boolean);return l`
              <div class="group-card">
                ...
                <div class="group-card-tasks">
                  ${o.length>0?o.map(c=>l`<span class="group-task-chip">${c}</span>`):l`<span style="font-size:12px;color:var(--secondary-text-color)">${n("no_tasks_short",t)}</span>`}
                </div>
              </div>
            `})}
        </div>
      </div>
    `}_renderBudgetBar(){let e=this._budget;if(!e)return d;let t=this._lang,a=e.currency_symbol||"\u20AC",s=[];return e.monthly_budget>0&&s.push({label:n("budget_monthly",t),spent:e.monthly_spent,budget:e.monthly_budget}),e.yearly_budget>0&&s.push({label:n("budget_yearly",t),spent:e.yearly_spent,budget:e.yearly_budget}),s.length===0?d:l`
      <div class="budget-bars">
        ${s.map(o=>{let c=Math.min(100,Math.max(0,o.spent/o.budget*100)),p=c>=100?"var(--error-color, #f44336)":c>=e.alert_threshold_pct?"var(--warning-color, #ff9800)":"var(--success-color, #4caf50)";return l`
    ...
    `}_renderOverviewRow(e){let t=this._lang,a=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,s=0,o=ke.ok,c=!1;if(a&&e.days_until_due!==null){let p=(e.interval_days-e.days_until_due)/e.interval_days*100;s=Math.max(0,Math.min(100,p)),c=p>100,e.status==="overdue"?o=ke.overdue:e.status==="due_soon"&&(o=ke.due_soon)}return l`
      <div class="task-row${e.enabled?"":" task-disabled"}">
        <span class="status-badge ${e.status}">${n(e.status,t)}</span>
        ${e.enabled?d:l`<span class="badge-disabled">${n("disabled",t)}</span>`}
        ${e.nfc_tag_id?l`<span class="nfc-badge" title="${n("nfc_linked",t)}"><ha-icon icon="mdi:nfc-variant"></ha-icon></span>`:d}
    ...
    `}_openCompleteDialogForRow(e){let a=this._objects.find(s=>s.entry_id===e.entry_id)?.tasks.find(s=>s.id===e.task_id);this._openCompleteDialog(e.entry_id,e.task_id,e.task_name,this._features.checklists?a?.checklist:void 0,this._features.adaptive&&!!a?.adaptive_config?.enabled)}_renderTriggerProgress(e){let t=e.trigger_config??null;if(!t)return d;let a=t.type||"threshold",s=e.trigger_entity_info?.unit_of_measurement??"",o=0,c="";if(a==="threshold"){let m=e.trigger_current_value??null;if(m==null)return d;let v=t.trigger_above,b=t.trigger_below;if(v!=null){let f=b??0,k=v-f||1;o=Math.min(100,Math.max(0,(m-f)/k*100)),c=`${m.toFixed(1)} / ${v} ${s}`}else if(b!=null){let k=e.trigger_entity_info?.max??(b*2||100),T=k-b||1;o=Math.min(100,Math.max(0,(k-m)/T*100)),c=`${m.toFixed(1)} / ${b} ${s}`}else return d}else if(a==="counter"){let m=t.trigger_target_value||1,b=e.trigger_current_delta??null??e.trigger_current_value??null;if(b==null)return d;o=Math.min(100,Math.max(0,b/m*100)),c=`${b.toFixed(1)} / ${m} ${s}`}else if(a==="state_change"){let m=t.trigger_target_changes||1,v=e.trigger_current_value??null;if(v==null)return d;o=Math.min(100,Math.max(0,v/m*100)),c=`${Math.round(v)} / ${m}`}else if(a==="runtime"){let m=t.trigger_runtime_hours||100,v=e.trigger_current_value??null;if(v==null)return d;o=Math.min(100,Math.max(0,v/m*100)),c=`${v.toFixed(1)}h / ${m}h`}else if(a==="compound"){let m=t.compound_logic||t.operator||"AND",v=t.conditions?.length||0;c=`${m} (${v})`,o=e.trigger_active?100:0}else return d;let p=o>=100,_=o>90?"var(--error-color, #f44336)":o>70?"var(--warning-color, #ff9800)":"var(--primary-color)";return l`
    ...
    `}_renderMiniSparkline(e){if(!e.trigger_config?.entity_id)return d;let t=e.trigger_config.entity_id,a=this._miniStatsData.get(t)||[],s=[];if(a.length>=2)s=a.map(x=>({ts:x.ts,val:x.val}));else{if(!e.history)return d;for(let x of e.history)x.trigger_value!=null&&s.push({ts:new Date(x.timestamp).getTime(),val:x.trigger_value})}if(e.trigger_current_value!=null&&s.push({ts:Date.now(),val:e.trigger_current_value}),s.length<2)return d;s.sort((x,I)=>x.ts-I.ts);let o=ka,c=wa,p=s.map(x=>x.val),_=Math.min(...p),m=Math.max(...p),v=m-_||1;_-=v*.1,m+=v*.1;let b=s[0].ts,k=s[s.length-1].ts-b||1,T=x=>(x-b)/k*o,q=x=>2+(1-(x-_)/(m-_))*(c-4),z=s;if(z.length>kt){let x=Math.ceil(z.length/kt);z=z.filter((I,O)=>O%x===0||O===z.length-1)}let A=z.map(x=>`${T(x.ts).toFixed(1)},${q(x.val).toFixed(1)}`).join(" ");return l`
    ...
    `}_renderDaysProgress(e){let t=this._lang;if(e.days_until_due==null||!e.interval_days||e.interval_days<=0)return d;let s=(e.interval_days-e.days_until_due)/e.interval_days*100,o=Math.max(0,Math.min(100,s)),c=s>100,p="var(--success-color, #4caf50)";return e.status==="overdue"?p="var(--error-color, #f44336)":e.status==="due_soon"&&(p="var(--warning-color, #ff9800)"),l`
      <div class="days-progress">
        <div class="days-progress-labels">
          <span>${e.last_performed?`${n("last_performed",t)}: ${X(e.last_performed,t)}`:""}</span>
          <span>${e.next_due?`${n("next_due",t)}: ${X(e.next_due,t)}`:""}</span>
        </div>
    ...
    `}_renderObjectDetail(){if(!this._selectedEntryId)return d;let e=this._getObject(this._selectedEntryId);if(!e)return l`<p>Object not found.</p>`;let t=e.object,a=this._lang;return l`
      <div class="detail-section">
        ...
        ${t.serial_number?l`<p class="meta">${n("serial_number_label",a)}: ${t.serial_number}</p>`:d}
        ${t.installation_date?l`<p class="meta">${n("installed",a)}: ${X(t.installation_date,a)}</p>`:d}
        ...
        ${e.tasks.length===0?l`<div class="empty-state-centered">
            ...
            </div>`:[...e.tasks].sort((s,o)=>{let c={overdue:0,triggered:1,due_soon:2,ok:3};return(c[s.status]??9)-(c[o.status]??9)||(s.days_until_due??99999)-(o.days_until_due??99999)}).map(s=>l`
              <div class="task-row${s.enabled?"":" task-disabled"}">
                <span class="status-badge ${s.status}">${n(s.status,a)}</span>
                ${s.enabled?d:l`<span class="badge-disabled">${n("disabled",a)}</span>`}
                ${s.nfc_tag_id?l`<span class="nfc-badge" title="${n("nfc_linked",a)}"><ha-icon icon="mdi:nfc-variant"></ha-icon></span>`:d}
                <span class="cell task-name" @click=${()=>this._showTask(e.entry_id,s.id)}>${s.name}</span>
                ${this._renderUserBadge(s)}
                <span class="cell type">${n(s.type,a)}</span>
                <span class="due-cell" @click=${()=>this._showTask(e.entry_id,s.id)}>
                  <span class="due-text">${we(s.days_until_due,a)}</span>
                  ${s.trigger_config?this._renderTriggerProgress(s):d}
                  ${this._renderMiniSparkline(s)}
                </span>
                <span class="row-actions">
                  <mwc-icon-button class="btn-complete" title="${n("complete",a)}" @click=${o=>{o.stopPropagation(),this._openCompleteDialog(e.entry_id,s.id,s.name,this._features.checklists?s.checklist:void 0,this._features.adaptive&&!!s.adaptive_config?.enabled)}}>
                    <ha-icon icon="mdi:check"></ha-icon>
                  </mwc-icon-button>
                  <mwc-icon-button class="btn-skip" title="${n("skip",a)}" .disabled=${this._actionLoading} @click=${o=>{o.stopPropagation(),this._promptSkipTask(e.entry_id,s.id)}}>
                    <ha-icon icon="mdi:skip-next"></ha-icon>
                  </mwc-icon-button>
                </span>
              </div>
    ...
    `}_renderTaskHeader(e){let t=this._lang,s=this._getObject(this._selectedEntryId)?.object.name||"",o=e.status==="due_soon"?"warning":e.status||"ok",c=n(e.status||"ok",t);return l`
      <div class="task-header">
        <div class="task-header-title">
          ...
          ${e.nfc_tag_id?l`<span class="nfc-badge" title="${n("nfc_tag_id",t)}: ${e.nfc_tag_id}"><ha-icon icon="mdi:nfc-variant"></ha-icon> NFC</span>`:l`<span class="nfc-badge unlinked" title="${n("nfc_link_hint",t)}"
      ...
      <div class="tab-content overview-tab">
        ...
        <div class="two-column-layout ${o?"":"single-column"}">
          ...
          <div class="right-column">
            ${$t(e,t,this._costDurationToggle,_=>{this._costDurationToggle=_})}
          </div>
        </div>
        ${c?ft(e,t):d}
        ${p?l`
      ...
      <div class="task-meta-card">
        ...
        ${t?l`
          <div class="task-meta-row task-meta-link">
            ...
            <a href="${t}" target="_blank" rel="noopener noreferrer">${n("documentation_label",a)}</a>
          </div>
        `:d}
      </div>
    ...
    `}_renderRecommendationCard(e){let t=this._lang;if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return d;if(this._selectedEntryId&&this._selectedTaskId&&this._dismissedSuggestions.has(`${this._selectedEntryId}_${this._selectedTaskId}`))return d;let a=e.interval_days,s=e.suggested_interval,o=e.interval_confidence||"medium",c=Math.max(a||1,s);return l`
      <div class="recommendation-card">
        <h4>${n("suggested_interval",t)}</h4>
        <div class="interval-comparison">
          <div class="interval-bar">
            <div class="interval-label">${n("current",t)||"Aktuell"}: ${a??"\u2014"} ${a!=null?n("days",t):""}</div>
            <div class="interval-visual current" style="width: ${a!=null?Math.min(a/c*100,100):0}%"></div>
          </div>
          <div class="interval-bar">
            <div class="interval-label">${n("recommended",t)}: ${s} ${n("days",t)}
              <span class="confidence-badge ${o}">${n(`confidence_${o}`,t)}</span>
            </div>
            <div class="interval-visual suggested" style="width: ${Math.min(s/c*100,100)}%"></div>
          </div>
        </div>
    ...
    `}_renderRecentActivities(e){let t=this._lang,a=e.history.slice(0,3);if(a.length===0)return d;let s=o=>{switch(o){case"completed":return"\u2713";case"triggered":return"\u2297";case"skipped":return"\u21B7";case"reset":return"\u21BA";default:return"\xB7"}};return l`
      ...
      <div class="history-filters-new">
        <div class="filter-chips">
          ${["completed","skipped","reset","triggered"].map(a=>{let s=e.history.filter(o=>o.type===a).length;return s===0?d:l`
            ...
            `})}
          ${this._historyFilter?l`<span class="filter-chip clear" @click=${()=>{this._historyFilter=null}}>${n("show_all",t)}</span>`:d}
        </div>
    ...
    `}_renderHistoryList(e){let t=this._lang,a=this._historyFilter?e.history.filter(s=>s.type===this._historyFilter):e.history;if(this._historySearch){let s=this._historySearch.toLowerCase();a=a.filter(o=>o.notes?.toLowerCase().includes(s))}return a.length===0?l`<p class="empty">${n("no_history",t)}</p>`:l`
      ...
      <div class="history-entry">
        ...
        <div class="history-content">
          ...
          <div class="history-details">
            ${e.cost!=null?l`<span>${n("cost",t)}: ${e.cost.toFixed(2)} ${this._budget?.currency_symbol||"\u20AC"}</span>`:d}
            ${e.duration!=null?l`<span>${n("duration",t)}: ${e.duration} min</span>`:d}
            ${e.trigger_value!=null?l`<span>${n("trigger_val",t)}: ${e.trigger_value}</span>`:d}
          </div>
        </div>
      </div>
    `}};E.styles=[ut,pt],u([S({attribute:!1})],E.prototype,"hass",2),u([S({type:Boolean,reflect:!0})],E.prototype,"narrow",2),u([S({attribute:!1})],E.prototype,"panel",2),u([g()],E.prototype,"_objects",2),u([g()],E.prototype,"_stats",2),u([g()],E.prototype,"_view",2),u([g()],E.prototype,"_selectedEntryId",2),u([g()],E.prototype,"_selectedTaskId",2),u([g()],E.prototype,"_filterStatus",2),u([g()],E.prototype,"_filterUser",2),u([g()],E.prototype,"_unsub",2),u([g()],E.prototype,"_sparklineTooltip",2),u([g()],E.prototype,"_historyFilter",2),u([g()],E.prototype,"_budget",2),u([g()],E.prototype,"_groups",2),u([g()],E.prototype,"_detailStatsData",2),u([g()],E.prototype,"_miniStatsData",2),u([g()],E.prototype,"_features",2),u([g()],E.prototype,"_actionLoading",2),u([g()],E.prototype,"_moreMenuOpen",2),u([g()],E.prototype,"_toastMessage",2),u([g()],E.prototype,"_overviewTab",2),u([g()],E.prototype,"_activeTab",2),u([g()],E.prototype,"_costDurationToggle",2),u([g()],E.prototype,"_historySearch",2),u([g()],E.prototype,"_sortMode",2),E=u([lt("maintenance-supporter-panel")],E);export{E as MaintenanceSupporterPanel};
