...
<body>
  ...
  `}function As(n,r,e,t,i){let b=Math.max(e,t,r,1)*1.3,w=50,f=[];for(let q=0;q<=w;q++){let D=q/w*b,pe=1-Math.exp(-Math.pow(D/r,n)),_e=32+D/b*260,ye=136-pe*128;f.push([_e,ye])}let j=f.map(([q,D])=>`${q.toFixed(1)},${D.toFixed(1)}`).join(" "),H="M32,136 "+f.map(([q,D])=>`L${q.toFixed(1)},${D.toFixed(1)}`).join(" ")+` L${f[w][0].toFixed(1)},136 Z`,L=32+e/b*260,M=1-Math.exp(-Math.pow(e/r,n)),E=136-M*128,W=((1-M)*100).toFixed(0),K=32+t/b*260,V=[0,.25,.5,.75,1];return o`
  ...
  `}function Ps(n,r,e){let t=n.history.filter($=>$.type==="completed"&&($.cost!=null||$.duration!=null)).map($=>({ts:new Date($.timestamp).getTime(),cost:$.cost??0,duration:$.duration??0})).sort(($,R)=>$.ts-R.ts);if(t.length<2)return p;let i=t.some($=>$.cost>0),a=t.some($=>$.duration>0);if(!i&&!a)return p;let l=e!=="duration"&&i,c=e!=="cost"&&a,d=l||!c&&i,_=c||!l&&a,g=640,m=Ms,v=d?44:12,b=_?44:12,w=g-v-b,f=m-Rs,j=f-Ne,H=t[0].ts,L=t[t.length-1].ts,M=(L-H||864e5)*.05,E=H-M,W=L+M,K=ct(H,L),V=$=>v+($-E)/(W-E)*w,q=Fe(0,Math.max(...t.map($=>$.cost))||1,3),D=Fe(0,Math.max(...t.map($=>$.duration))||1,3),pe=$=>Ne+(1-$/(q.niceMax||1))*j,_e=$=>Ne+(1-$/(D.niceMax||1))*j,ye=t.length>1?Math.min(...t.slice(1).map(($,R)=>V($.ts)-V(t[R].ts))):w,U=Math.max(6,Math.min(22,ye*.55)),k=dt(H,L,Math.max(2,Math.min(4,t.length)));return o`
