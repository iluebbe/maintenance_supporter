...
<body>
  ...
  `}function tt(n,r,e=4){if(!isFinite(n)||!isFinite(r))return{ticks:[],niceMin:0,niceMax:1};if(n===r){let u=Math.abs(n)*.1||1;n-=u,r+=u}let t=r-n,i=Math.pow(10,Math.floor(Math.log10(t/Math.max(1,e)))),a=i;for(let u of[1,2,5,10])if(a=i*u,t/a<=e+.5)break;let l=Math.floor(n/a)*a,c=Math.ceil(r/a)*a,h=[];for(let u=l;u<=c+a*1e-6;u+=a)h.push(Math.abs(u)<a*1e-9?0:u);return{ticks:h,niceMin:l,niceMax:c}}function ue(n){let r=Math.abs(n);return r>=1e6?De((n/1e6).toFixed(r>=1e7?0:1))+"M":r>=1e4?De((n/1e3).toFixed(0))+"k":r>=1e3?De((n/1e3).toFixed(1))+"k":r>=100?n.toFixed(0):r>=10||r>=1?De(n.toFixed(1)):r===0?"0":De(n.toFixed(2))}function De(n){return n.replace(/\.0+$/,"").replace(/(\.\d*[1-9])0+$/,"$1")}function Ce(n,r,e){let t=n.toLocaleString(e,{maximumFractionDigits:Math.abs(n)>=100?0:1});return r?`${t} ${r}`:t}function Oe(n,r,e){let t=new Date(n),i=e?{month:"short",day:"numeric",year:"2-digit"}:{month:"short",day:"numeric"};return t.toLocaleDateString(r,i)}function Gt(n,r){return new Date(n).toLocaleDateString(r,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function St(n,r){return new Date(n).getFullYear()!==new Date(r).getFullYear()}function At(n,r,e){if(e<2||r<=n)return[n,r];let t=[];for(let i=0;i<e;i++)t.push(n+(r-n)*i/(e-1));return t}var jt=210,de=46,ve=14,be=12,Hi=14,Zs=20+Hi,Js=[{days:7,key:"chart_range_7d"},{days:30,key:"chart_range_30d"},{days:90,key:"chart_range_90d"},{days:365,key:"chart_range_1y"}],V=class extends S{constructor(){super(...arguments);this.points=[];this.events=[];this.unit="";this.lang="en";this.thresholdAbove=null;this.thresholdBelow=null;this.targetValue=null;this.forceZero=!1;this.projection=null;this.rangeDays=30;this.showRange=!0;this.busy=!1;this.hideOutliers=!1;this.showOutlierToggle=!0;this._width=0;this._hover=null;this._ro=null}connectedCallback(){super.connectedCallback(),this._ro=new ResizeObserver(e=>{let t=Math.floor(e[0]?.contentRect?.width||0);t&&Math.abs(t-this._width)>2&&(this._width=t)}),this._ro.observe(this)}disconnectedCallback(){super.disconnectedCallback(),this._ro?.disconnect(),this._ro=null}_emitRange(e){e!==this.rangeDays&&this.dispatchEvent(new CustomEvent("range-change",{detail:{days:e},bubbles:!0,composed:!0}))}_toggleOutliers(){this.dispatchEvent(new CustomEvent("outlier-toggle",{detail:{hide:!this.hideOutliers},bubbles:!0,composed:!0}))}render(){let e=this._width||320,t=[...this.points].sort((a,l)=>a.ts-l.ts),i=this.lang;return o`
    ...
    `}_renderSvg(e,t){let i=this.lang,a=e-de-ve,l=jt-Zs,c=l-be,h=1/0,u=-1/0;for(let k of t)h=Math.min(h,k.min??k.val),u=Math.max(u,k.max??k.val);this.thresholdAbove!=null&&(h=Math.min(h,this.thresholdAbove),u=Math.max(u,this.thresholdAbove)),this.thresholdBelow!=null&&(h=Math.min(h,this.thresholdBelow),u=Math.max(u,this.thresholdBelow)),this.targetValue!=null&&(h=Math.min(h,this.targetValue),u=Math.max(u,this.targetValue)),this.forceZero&&(h=Math.min(h,0));let g=(u-h||1)*.06,m=this.forceZero&&h>=0?0:h-g,{ticks:v,niceMin:b,niceMax:$}=tt(m,u+g,4);this.forceZero&&h>=0&&b<0&&(b=0,v=v.filter(k=>k>=0));let f=t[0].ts,j=t[t.length-1].ts,I=j-f||1,L=St(f,j),M=k=>de+(k-f)/I*a,T=k=>be+(1-(k-b)/($-b||1))*c,X=t.map(k=>`${M(k.ts).toFixed(1)},${T(k.val).toFixed(1)}`).join(" "),ee=`M${M(t[0].ts).toFixed(1)},${l} `+t.map(k=>`L${M(k.ts).toFixed(1)},${T(k.val).toFixed(1)}`).join(" ")+` L${M(t[t.length-1].ts).toFixed(1)},${l} Z`,Z="",U=t.filter(k=>k.min!=null&&k.max!=null);if(U.length>=2){let k=U.map(P=>`${M(P.ts).toFixed(1)},${T(P.max).toFixed(1)}`),w=[...U].reverse().map(P=>`${M(P.ts).toFixed(1)},${T(P.min).toFixed(1)}`);Z=`M${k[0]} `+k.slice(1).map(P=>`L${P}`).join(" ")+` L${w.join(" L")} Z`}let N=[];if(this.thresholdBelow!=null){let k=T(this.thresholdBelow);N.push({y:k,h:Math.max(0,l-k),lineY:k,label:`\u25BC ${ue(this.thresholdBelow)}`,labelY:Math.min(l-4,k+13)})}if(this.thresholdAbove!=null){let k=T(this.thresholdAbove);N.push({y:be,h:Math.max(0,k-be),lineY:k,label:`\u25B2 ${ue(this.thresholdAbove)}`,labelY:Math.max(be+11,k-5)})}let fe=t[t.length-1],we=(this.events||[]).filter(k=>k.ts>=f&&k.ts<=j),Ie=At(f,j,Math.max(2,Math.min(5,Math.floor(a/110)+1))),Y=this._hover;return o`
      <div class="svg-holder">
        ...
        >
          ...
          ${Ie.map((k,w)=>{let P=M(k),Ct=w===0?"start":w===Ie.length-1?"end":"middle";return D`<text x="${P.toFixed(1)}" y="${jt-5}" text-anchor="${Ct}" class="tick-label">${Oe(k,i,L)}</text>`})}

          <line x1="${de}" y1="${l}" x2="${e-ve}" y2="${l}" stroke="var(--divider-color)" stroke-width="1" />

          ${we.map(k=>{let w=M(k.ts),P=k.type==="completed"?"var(--success-color, #4caf50)":k.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return D`
              <line x1="${w.toFixed(1)}" y1="${be}" x2="${w.toFixed(1)}" y2="${l}" stroke="${P}" stroke-width="1" opacity="0.14" />
              <rect x="${(w-1.5).toFixed(1)}" y="${l+3}" width="3" height="${Hi-6}" rx="1.5" fill="${P}">
  ...
  `}function da(n,r,e){let t=n.history.filter(w=>w.type==="completed"&&(w.cost!=null||w.duration!=null)).map(w=>({ts:new Date(w.timestamp).getTime(),cost:w.cost??0,duration:w.duration??0})).sort((w,P)=>w.ts-P.ts);if(t.length<2)return p;let i=t.some(w=>w.cost>0),a=t.some(w=>w.duration>0);if(!i&&!a)return p;let l=e!=="duration"&&i,c=e!=="cost"&&a,h=l||!c&&i,u=c||!l&&a,g=640,m=la,v=h?44:12,b=u?44:12,$=g-v-b,f=m-ca,j=f-it,I=t[0].ts,L=t[t.length-1].ts,M=(L-I||864e5)*.05,T=I-M,X=L+M,ee=St(I,L),Z=w=>v+(w-T)/(X-T)*$,U=tt(0,Math.max(...t.map(w=>w.cost))||1,3),N=tt(0,Math.max(...t.map(w=>w.duration))||1,3),fe=w=>it+(1-w/(U.niceMax||1))*j,we=w=>it+(1-w/(N.niceMax||1))*j,Ie=t.length>1?Math.min(...t.slice(1).map((w,P)=>Z(w.ts)-Z(t[P].ts))):$,Y=Math.max(6,Math.min(22,Ie*.55)),k=At(I,L,Math.max(2,Math.min(4,t.length)));return o`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${g} ${m}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_history",r)}">
        ...
        ${k.map((w,P)=>{let Ct=P===0?"start":P===k.length-1?"end":"middle";return D`<text x="${Z(w).toFixed(1)}" y="${m-6}" text-anchor="${Ct}" fill="var(--secondary-text-color)" font-size="10">${Oe(w,r,ee)}</text>`})}
      </svg>
    </div>
    <div class="chart-legend">
