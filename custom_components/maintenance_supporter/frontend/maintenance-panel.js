...
`;function He(o){let n=window;n.customCards=n.customCards||[],n.customCards.some(t=>t.type===o.type)||n.customCards.push(o)}var it=class extends C{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._objectFilter="";this._configuredObjects=[];this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(t){if(this._config={...t},t.past_days&&[30,90].includes(t.past_days)?this._pastDays=t.past_days:t.window_days&&[7,14,30,365].includes(t.window_days)&&(this._windowDays=t.window_days,this._pastDays=0),typeof t.user_filter=="string"&&(this._userFilter=t.user_filter),typeof t.object_filter=="string")this._objectFilter=t.object_filter,this._configuredObjects=[];else if(Array.isArray(t.object_filter)){let e=t.object_filter.filter(i=>typeof i=="string"&&i!=="");this._objectFilter=e.length===1?e[0]:"",this._configuredObjects=e.length>1?e:[]}}getCardSize(){return 6}get _lang(){return L(this.hass)}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(t){if(super.updated(t),Ft(this,t),t.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[t,e]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=t.objects,this._stats=e}catch{}}async _subscribe(){try{let t=await this.hass.connection.subscribeMessage(e=>{let i=e;this._objects=i.objects},{type:"maintenance_supporter/subscribe"});if(!this.isConnected){t();return}this._unsub=t}catch{}}_onEventClick(t){if(t.history_timestamp){this._openHistoryEntry(t);return}Ce(t.entry_id,t.task_id)||this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:t.entry_id,task_id:t.task_id},bubbles:!0,composed:!0}))}async _openHistoryEntry(t){try{let i=(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:t.entry_id})).tasks?.find(l=>l.id===t.task_id)?.history?.find(l=>l.timestamp===t.history_timestamp);if(!i||Me({entry_id:t.entry_id,task_id:t.task_id,original_timestamp:t.history_timestamp,type:i.type||"completed",timestamp:i.timestamp||t.history_timestamp,notes:i.notes??null,cost:i.cost??null,duration:i.duration??null,completed_by:i.completed_by??null,used_parts:i.used_parts??null}))return}catch{}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:t.entry_id,task_id:t.task_id,original_timestamp:t.history_timestamp},bubbles:!0,composed:!0}))}render(){if(!this.hass)return d;let t=this._lang,e=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.title,l=null;this._userFilter&&(l=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let p=f=>{let K=f.toLowerCase();return this._objects.find(et=>et.entry_id===f||et.object.name.toLowerCase()===K)?.entry_id??null},c=new Set(this._configuredObjects.map(p).filter(f=>f!==null)),h=c.size?this._objects.filter(f=>c.has(f.entry_id)):this._objects,g=this._config.show_object_filter!==!1&&h.length>1,v=this._objectFilter?p(this._objectFilter):null,_=v&&h.some(f=>f.entry_id===v)?h.filter(f=>f.entry_id===v):h,y=new Date;y.setHours(0,0,0,0);let w=this._pastDays>0,S=w?we(_,y,this._pastDays,l):$e(_,y,this._windowDays,l),O=xe(y),k=this._windowDays===365||w,P=k?S.filter(f=>f.events.length>0):S,ct=f=>{let K=`cal-status-${f.status}`,tt=f.projected?"cal-event-projected":"",et=f.status==="overdue"&&f.days_until_due!=null?` (${_t(f.days_until_due,t)})`:"",ot=f.projected&&f.interval_days?r`<span class="cal-event-recur">${f.interval_unit&&f.interval_unit!=="days"?`${f.interval_days} ${s("unit_"+f.interval_unit,t)}`:s("cal_every_n_days",t).replace("{n}",String(f.interval_days))}</span>`:d,U=f.schedule_type==="sensor_based",dt=U?r`<ha-icon class="cal-event-icon cal-source-sensor"
  ...
  `}function ae(o,n,t){if(!o.trigger_config?.entity_id)return d;let e=o.trigger_config.entity_id,i=n.get(e)||[],a=[];if(i.length>=2)a=i.map(j=>({ts:j.ts,val:j.val}));else{if(!o.history)return d;for(let j of o.history)j.trigger_value!=null&&a.push({ts:new Date(j.timestamp).getTime(),val:j.trigger_value})}if(o.trigger_current_value!=null&&a.push({ts:Date.now(),val:o.trigger_current_value}),a.length<2)return d;a.sort((j,f)=>j.ts-f.ts);let l=hi,p=ui,c=a.map(j=>j.val),h=Math.min(...c),g=Math.max(...c),v=g-h||1;h-=v*.1,g+=v*.1;let _=a[0].ts,w=a[a.length-1].ts-_||1,S=j=>(j-_)/w*l,O=j=>2+(1-(j-h)/(g-h))*(p-4),k=a;if(k.length>Ne){let j=Math.ceil(k.length/Ne);k=k.filter((f,K)=>K%j===0||K===k.length-1)}let P=k.map(j=>`${S(j.ts).toFixed(1)},${O(j.val).toFixed(1)}`).join(" "),ct=o.trigger_active?"var(--error-color, #f44336)":"var(--primary-color)";return r`
    ...
    `}_renderSvg(t,e){let i=this.lang,a=t-lt-gt,l=Kt-gi,p=l-mt,c=1/0,h=-1/0;for(let b of e)c=Math.min(c,b.min??b.val),h=Math.max(h,b.max??b.val);this.thresholdAbove!=null&&(c=Math.min(c,this.thresholdAbove),h=Math.max(h,this.thresholdAbove)),this.thresholdBelow!=null&&(c=Math.min(c,this.thresholdBelow),h=Math.max(h,this.thresholdBelow)),this.targetValue!=null&&(c=Math.min(c,this.targetValue),h=Math.max(h,this.targetValue)),this.forceZero&&(c=Math.min(c,0));let g=(h-c||1)*.06,v=this.forceZero&&c>=0?0:c-g,{ticks:_,niceMin:y,niceMax:w}=Rt(v,h+g,4);this.forceZero&&c>=0&&y<0&&(y=0,_=_.filter(b=>b>=0));let S=e[0].ts,O=this.projection&&this.projection.length===2?this.projection[1].ts:null,k=O!=null?Math.max(e[e.length-1].ts,O):e[e.length-1].ts,P=k-S||1,ct=Yt(S,k),j=b=>lt+(b-S)/P*a,f=b=>mt+(1-(b-y)/(w-y||1))*p,K=e.map(b=>`${j(b.ts).toFixed(1)},${f(b.val).toFixed(1)}`).join(" "),tt=`M${j(e[0].ts).toFixed(1)},${l} `+e.map(b=>`L${j(b.ts).toFixed(1)},${f(b.val).toFixed(1)}`).join(" ")+` L${j(e[e.length-1].ts).toFixed(1)},${l} Z`,et="",ot=e.filter(b=>b.min!=null&&b.max!=null);if(ot.length>=2){let b=ot.map(Q=>`${j(Q.ts).toFixed(1)},${f(Q.max).toFixed(1)}`),E=[...ot].reverse().map(Q=>`${j(Q.ts).toFixed(1)},${f(Q.min).toFixed(1)}`);et=`M${b[0]} `+b.slice(1).map(Q=>`L${Q}`).join(" ")+` L${E.join(" L")} Z`}let U=[];if(this.thresholdBelow!=null){let b=f(this.thresholdBelow);U.push({y:b,h:Math.max(0,l-b),lineY:b,label:`\u25BC ${ut(this.thresholdBelow)}`,labelY:Math.min(l-4,b+13)})}if(this.thresholdAbove!=null){let b=f(this.thresholdAbove);U.push({y:mt,h:Math.max(0,b-mt),lineY:b,label:`\u25B2 ${ut(this.thresholdAbove)}`,labelY:Math.max(mt+11,b-5)})}let dt=e[e.length-1],yt=(this.events||[]).filter(b=>b.ts>=S&&b.ts<=k),xt=Gt(S,k,Math.max(2,Math.min(5,Math.floor(a/110)+1))),N=this._hover;return r`
      <div class="svg-holder">
        ...
        >
          ...
          ${xt.map((b,E)=>{let Q=j(b),Xe=E===0?"start":E===xt.length-1?"end":"middle";return F`<text x="${Q.toFixed(1)}" y="${Kt-5}" text-anchor="${Xe}" class="tick-label">${St(b,i,ct)}</text>`})}
  ...
  `}function $i(o,n,t){let e=o.history.filter(b=>b.type==="completed"&&(b.cost!=null||b.duration!=null)).map(b=>({ts:new Date(b.timestamp).getTime(),cost:b.cost??0,duration:b.duration??0})).sort((b,E)=>b.ts-E.ts);if(e.length<2)return d;let i=e.some(b=>b.cost>0),a=e.some(b=>b.duration>0);if(!i&&!a)return d;let l=t!=="duration"&&i,p=t!=="cost"&&a,c=l||!p&&i,h=p||!l&&a,g=640,v=yi,_=c?44:12,y=h?44:12,w=g-_-y,S=v-xi,O=S-At,k=e[0].ts,P=e[e.length-1].ts,ct=(P-k||864e5)*.05,j=k-ct,f=P+ct,K=Yt(k,P),tt=b=>_+(b-j)/(f-j)*w,et=Rt(0,Math.max(...e.map(b=>b.cost))||1,3),ot=Rt(0,Math.max(...e.map(b=>b.duration))||1,3),U=b=>At+(1-b/(et.niceMax||1))*O,dt=b=>At+(1-b/(ot.niceMax||1))*O,yt=e.length>1?Math.min(...e.slice(1).map((b,E)=>tt(b.ts)-tt(e[E].ts))):w,xt=Math.max(6,Math.min(22,yt*.55)),N=Gt(k,P,Math.max(2,Math.min(4,e.length)));return r`
  ...
  `:d}function ji(o,n){let t=n.lang,e=n.isOperator,i=o.archived?"archived":o.is_done?"done":o.status==="due_soon"?"warning":o.status||"ok",a=o.archived?s("archived",t):o.is_done?s("completed",t):s(o.status||"ok",t);return r`
  ...
  `}function Ti(o){let n=o.lang;return r`
    <div class="tab-bar">
  ...
  `}function Di(o,n){let t=n.lang;if(!n.features.adaptive||!o.suggested_interval||o.suggested_interval===o.interval_days)return d;if(n.suggestionDismissed)return d;let e=o.suggested_interval;return r`
    <div class="recommendation-card">
      ...
      ${Te(o.interval_days,e,o.interval_confidence||"medium",t)}
