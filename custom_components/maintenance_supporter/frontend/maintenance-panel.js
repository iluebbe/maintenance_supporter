...
`;var ee=class extends S{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter)}getCardSize(){return 6}get _lang(){return this.hass?.language||"en"}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){super.updated(e);let t=this.hass?.language;if(t&&!Je(t)&&se(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}_onEventClick(e){if(e.history_timestamp){this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}));return}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}render(){if(!this.hass)return p;let e=this._lang,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.title,l=null;this._userFilter&&(l=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let c=new Date;c.setHours(0,0,0,0);let d=this._pastDays>0,_=d?li(this._objects,c,this._pastDays,l):oi(this._objects,c,this._windowDays,l),g=qe(c),m=this._windowDays===365||d,v=m?_.filter(b=>b.events.length>0):_,f=b=>{let j=`cal-status-${b.status}`,L=b.projected?"cal-event-projected":"",H=b.status==="overdue"&&b.days_until_due!=null?` (${Math.abs(b.days_until_due)}d ${s("overdue",e).toLowerCase()})`:"",M=b.projected&&b.interval_days?o`<span class="cal-event-recur">${b.interval_unit&&b.interval_unit!=="days"?`${b.interval_days} ${s("unit_"+b.interval_unit,e)}`:s("cal_every_n_days",e).replace("{n}",String(b.interval_days))}</span>`:p,E=b.schedule_type==="sensor_based",W=E?o`<ha-icon class="cal-event-icon cal-source-sensor"
      ...
      `},w=b=>{let[j,L,H]=b.date.split("-").map(Number),M=new Date(j,L-1,H),E=b.date===g,W=M.toLocaleDateString(e,{weekday:"short"}),K=M.toLocaleDateString(e,{month:"long"});return o`
...
<body>
  ...
  `,u([y({attribute:!1})],Z.prototype,"hass",2),u([y({attribute:!1})],Z.prototype,"objects",2),u([h()],Z.prototype,"_open",2),u([h()],Z.prototype,"_loading",2),u([h()],Z.prototype,"_error",2),u([h()],Z.prototype,"_groupId",2),u([h()],Z.prototype,"_name",2),u([h()],Z.prototype,"_description",2),u([h()],Z.prototype,"_selected",2);customElements.get("maintenance-group-dialog")||customElements.define("maintenance-group-dialog",Z);function Fe(n,r,e=4){if(!isFinite(n)||!isFinite(r))return{ticks:[],niceMin:0,niceMax:1};if(n===r){let _=Math.abs(n)*.1||1;n-=_,r+=_}let t=r-n,i=Math.pow(10,Math.floor(Math.log10(t/Math.max(1,e)))),a=i;for(let _ of[1,2,5,10])if(a=i*_,t/a<=e+.5)break;let l=Math.floor(n/a)*a,c=Math.ceil(r/a)*a,d=[];for(let _=l;_<=c+a*1e-6;_+=a)d.push(Math.abs(_)<a*1e-9?0:_);return{ticks:d,niceMin:l,niceMax:c}}function re(n){let r=Math.abs(n);return r>=1e6?Se((n/1e6).toFixed(r>=1e7?0:1))+"M":r>=1e4?Se((n/1e3).toFixed(0))+"k":r>=1e3?Se((n/1e3).toFixed(1))+"k":r>=100?n.toFixed(0):r>=10||r>=1?Se(n.toFixed(1)):r===0?"0":Se(n.toFixed(2))}function Se(n){return n.replace(/\.0+$/,"").replace(/(\.\d*[1-9])0+$/,"$1")}function fe(n,r,e){let t=n.toLocaleString(e,{maximumFractionDigits:Math.abs(n)>=100?0:1});return r?`${t} ${r}`:t}function Ae(n,r,e){let t=new Date(n),i=e?{month:"short",day:"numeric",year:"2-digit"}:{month:"short",day:"numeric"};return t.toLocaleDateString(r,i)}function Ct(n,r){return new Date(n).toLocaleDateString(r,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function ct(n,r){return new Date(n).getFullYear()!==new Date(r).getFullYear()}function dt(n,r,e){if(e<2||r<=n)return[n,r];let t=[];for(let i=0;i<e;i++)t.push(n+(r-n)*i/(e-1));return t}var pt=210,ie=46,ce=14,de=12,_i=14,$s=20+_i,ws=[{days:7,key:"chart_range_7d"},{days:30,key:"chart_range_30d"},{days:90,key:"chart_range_90d"},{days:365,key:"chart_range_1y"}],O=class extends S{constructor(){super(...arguments);this.points=[];this.events=[];this.unit="";this.lang="en";this.thresholdAbove=null;this.thresholdBelow=null;this.targetValue=null;this.forceZero=!1;this.projection=null;this.rangeDays=30;this.showRange=!0;this.busy=!1;this.hideOutliers=!1;this.showOutlierToggle=!0;this._width=0;this._hover=null;this._ro=null}connectedCallback(){super.connectedCallback(),this._ro=new ResizeObserver(e=>{let t=Math.floor(e[0]?.contentRect?.width||0);t&&Math.abs(t-this._width)>2&&(this._width=t)}),this._ro.observe(this)}disconnectedCallback(){super.disconnectedCallback(),this._ro?.disconnect(),this._ro=null}_emitRange(e){e!==this.rangeDays&&this.dispatchEvent(new CustomEvent("range-change",{detail:{days:e},bubbles:!0,composed:!0}))}_toggleOutliers(){this.dispatchEvent(new CustomEvent("outlier-toggle",{detail:{hide:!this.hideOutliers},bubbles:!0,composed:!0}))}render(){let e=this._width||320,t=[...this.points].sort((a,l)=>a.ts-l.ts),i=this.lang;return o`
    ...
    `}_renderSvg(e,t){let i=this.lang,a=e-ie-ce,l=pt-$s,c=l-de,d=1/0,_=-1/0;for(let k of t)d=Math.min(d,k.min??k.val),_=Math.max(_,k.max??k.val);this.thresholdAbove!=null&&(d=Math.min(d,this.thresholdAbove),_=Math.max(_,this.thresholdAbove)),this.thresholdBelow!=null&&(d=Math.min(d,this.thresholdBelow),_=Math.max(_,this.thresholdBelow)),this.targetValue!=null&&(d=Math.min(d,this.targetValue),_=Math.max(_,this.targetValue)),this.forceZero&&(d=Math.min(d,0));let g=(_-d||1)*.06,m=this.forceZero&&d>=0?0:d-g,{ticks:v,niceMin:f,niceMax:w}=Fe(m,_+g,4);this.forceZero&&d>=0&&f<0&&(f=0,v=v.filter(k=>k>=0));let b=t[0].ts,j=t[t.length-1].ts,L=j-b||1,H=ct(b,j),M=k=>ie+(k-b)/L*a,E=k=>de+(1-(k-f)/(w-f||1))*c,W=t.map(k=>`${M(k.ts).toFixed(1)},${E(k.val).toFixed(1)}`).join(" "),K=`M${M(t[0].ts).toFixed(1)},${l} `+t.map(k=>`L${M(k.ts).toFixed(1)},${E(k.val).toFixed(1)}`).join(" ")+` L${M(t[t.length-1].ts).toFixed(1)},${l} Z`,V="",q=t.filter(k=>k.min!=null&&k.max!=null);if(q.length>=2){let k=q.map(R=>`${M(R.ts).toFixed(1)},${E(R.max).toFixed(1)}`),$=[...q].reverse().map(R=>`${M(R.ts).toFixed(1)},${E(R.min).toFixed(1)}`);V=`M${k[0]} `+k.slice(1).map(R=>`L${R}`).join(" ")+` L${$.join(" L")} Z`}let D=[];if(this.thresholdBelow!=null){let k=E(this.thresholdBelow);D.push({y:k,h:Math.max(0,l-k),lineY:k,label:`\u25BC ${re(this.thresholdBelow)}`,labelY:Math.min(l-4,k+13)})}if(this.thresholdAbove!=null){let k=E(this.thresholdAbove);D.push({y:de,h:Math.max(0,k-de),lineY:k,label:`\u25B2 ${re(this.thresholdAbove)}`,labelY:Math.max(de+11,k-5)})}let pe=t[t.length-1],_e=(this.events||[]).filter(k=>k.ts>=b&&k.ts<=j),ye=dt(b,j,Math.max(2,Math.min(5,Math.floor(a/110)+1))),U=this._hover;return o`
      <div class="svg-holder">
        ...
        >
          ...
          ${ye.map((k,$)=>{let R=M(k),ut=$===0?"start":$===ye.length-1?"end":"middle";return P`<text x="${R.toFixed(1)}" y="${pt-5}" text-anchor="${ut}" class="tick-label">${Ae(k,i,H)}</text>`})}
  ...
  `}function ki(n,r){return Ms(n,r)}function Ms(n,r){let e=n.seasonal_factors??n.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return p;let t=n.interval_analysis?.seasonal_reason,i=new Date().getMonth(),a=300,l=100,c=8,_=l-c-4,g=Math.max(...e,1.5),m=a/12,v=m*.65,f=c+_-1/g*_;return o`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${a} ${l}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",r)}">
        ...
        ${e.map((w,b)=>{let j=w/g*_,L=b*m+(m-v)/2,H=c+_-j,M=b===i,E=w<1?"var(--success-color, #4caf50)":w>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return P`
  ...
  `}function Ls(n,r,e){let t=n.history.filter($=>$.type==="completed"&&($.cost!=null||$.duration!=null)).map($=>({ts:new Date($.timestamp).getTime(),cost:$.cost??0,duration:$.duration??0})).sort(($,R)=>$.ts-R.ts);if(t.length<2)return p;let i=t.some($=>$.cost>0),a=t.some($=>$.duration>0);if(!i&&!a)return p;let l=e!=="duration"&&i,c=e!=="cost"&&a,d=l||!c&&i,_=c||!l&&a,g=640,m=Rs,v=d?44:12,f=_?44:12,w=g-v-f,b=m-Ps,j=b-Ne,L=t[0].ts,H=t[t.length-1].ts,M=(H-L||864e5)*.05,E=L-M,W=H+M,K=ct(L,H),V=$=>v+($-E)/(W-E)*w,q=Fe(0,Math.max(...t.map($=>$.cost))||1,3),D=Fe(0,Math.max(...t.map($=>$.duration))||1,3),pe=$=>Ne+(1-$/(q.niceMax||1))*j,_e=$=>Ne+(1-$/(D.niceMax||1))*j,ye=t.length>1?Math.min(...t.slice(1).map(($,R)=>V($.ts)-V(t[R].ts))):w,U=Math.max(6,Math.min(22,ye*.55)),k=dt(L,H,Math.max(2,Math.min(4,t.length)));return o`
  ...
  `}function Mt(n,r,e){if(!n.trigger_config?.entity_id)return p;let t=n.trigger_config.entity_id,i=r.get(t)||[],a=[];if(i.length>=2)a=i.map(E=>({ts:E.ts,val:E.val}));else{if(!n.history)return p;for(let E of n.history)E.trigger_value!=null&&a.push({ts:new Date(E.timestamp).getTime(),val:E.trigger_value})}if(n.trigger_current_value!=null&&a.push({ts:Date.now(),val:n.trigger_current_value}),a.length<2)return p;a.sort((E,W)=>E.ts-W.ts);let l=Hs,c=zs,d=a.map(E=>E.val),_=Math.min(...d),g=Math.max(...d),m=g-_||1;_-=m*.1,g+=m*.1;let v=a[0].ts,w=a[a.length-1].ts-v||1,b=E=>(E-v)/w*l,j=E=>2+(1-(E-_)/(g-_))*(c-4),L=a;if(L.length>Ti){let E=Math.ceil(L.length/Ti);L=L.filter((W,K)=>K%E===0||K===L.length-1)}let H=L.map(E=>`${b(E.ts).toFixed(1)},${j(E.val).toFixed(1)}`).join(" "),M=n.trigger_active?"var(--error-color, #f44336)":"var(--primary-color)";return o`
