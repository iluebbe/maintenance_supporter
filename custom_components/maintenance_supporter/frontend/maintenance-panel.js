...
`;var Xi={days:1,weeks:7,months:30.4368,years:365.25};function Me(n,r){return!n||n<=0?0:n*(Xi[r||"days"]??1)}function se(n,r,t){let e=Me(n,t);if(e<=0||r==null)return{pct:0,overflow:!1};let i=(e-r)/e*100;return{pct:Math.max(0,Math.min(100,i)),overflow:i>100}}function L(n){return String(n??"").replace(/[&<>"']/g,r=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[r])}function ai(n,r,t,e,i,a){let l=[[t.manufacturer,n.manufacturer],[t.model,n.model],[t.serial,n.serial_number],[t.installed,n.installation_date?e(n.installation_date):null],[t.warranty,n.warranty_expiry?e(n.warranty_expiry):null]].filter(([,h])=>!!h),c=r.map(h=>{let g=t.scheduleLabel(h);return`<tr>
      <td>${L(h.name)}</td>
...
${L(n.notes)}</div>`:""}
</body></html>`}function Re(n,r=new Date){if(!n)return{kind:"none",days:null,date:null};let t=new Date(`${n}T00:00:00`);if(isNaN(t.getTime()))return{kind:"none",days:null,date:null};let e=Date.UTC(r.getFullYear(),r.getMonth(),r.getDate()),i=Date.UTC(t.getFullYear(),t.getMonth(),t.getDate()),a=Math.round((i-e)/864e5);return a<0?{kind:"expired",days:a,date:n}:a<=60?{kind:"expiring",days:a,date:n}:{kind:"valid",days:a,date:n}}var Tt=[{key:"name",labelKey:"name",required:!0},{key:"manufacturer",labelKey:"manufacturer"},{key:"model",labelKey:"model"},{key:"serial_number",labelKey:"serial_number_label"},{key:"installation_date",labelKey:"installed"},{key:"warranty_expiry",labelKey:"warranty"},{key:"area_id",labelKey:"area"},{key:"documentation_url",labelKey:"documentation_url_label"},{key:"notes",labelKey:"object_notes_label"},{key:"task_count",labelKey:"tasks"},{key:"actions",labelKey:"actions"}],ts=Tt.map(n=>n.key),ae=["name","manufacturer","model","serial_number","installation_date","warranty_expiry","area_id","task_count","actions"];function Ot(n){if(!Array.isArray(n))return[...ae];let r=new Set,t=[];for(let e of n)typeof e=="string"&&ts.includes(e)&&!r.has(e)&&(r.add(e),t.push(e));return t.length?(t.includes("name")||t.unshift("name"),t):[...ae]}function re(n,r,t){let e=new Blob([n],{type:t}),i=URL.createObjectURL(e),a=document.createElement("a");a.href=i,a.download=r,a.target="_blank",a.rel="noopener",a.style.display="none",document.body.appendChild(a),a.dispatchEvent(new MouseEvent("click")),document.body.removeChild(a),setTimeout(()=>URL.revokeObjectURL(i),6e4)}function ne(n,r){let t=document.createElement("a");t.href=n,t.download=r,t.target="_blank",t.rel="noopener",t.style.display="none",document.body.appendChild(t),t.dispatchEvent(new MouseEvent("click")),document.body.removeChild(t)}var ri=A`
  :host {
    display: block;
    height: 100%;
...
`;var oe=class{constructor(r){this._cache=new Map;this._pending=new Map;this._hass=r}updateHass(r){this._hass=r}async getDetailStats(r,t,e=30){return this._getStats(r,e<=35?"hour":"day",e,t)}async getMiniStats(r,t){return this._getStats(r,"day",14,t)}async getBatchMiniStats(r){let t=new Map,e=[];for(let d of r){let h=`${d.entityId}:day:14`,g=this._cache.get(h);g&&Date.now()-g.fetchedAt<3e5?t.set(d.entityId,g.points):e.push(d)}if(e.length===0)return t;let i=e.filter(d=>d.isCounter).map(d=>d.entityId),a=e.filter(d=>!d.isCounter).map(d=>d.entityId),l=new Date(Date.now()-336*60*60*1e3).toISOString(),c=[];return i.length>0&&c.push(this._fetchBatch(i,"day",l,["state","sum","change"],!0,t)),a.length>0&&c.push(this._fetchBatch(a,"day",l,["mean","min","max"],!1,t)),await Promise.all(c),t}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(r,t,e,i){let a=`${r}:${t}:${e}`,l=this._cache.get(a);if(l&&Date.now()-l.fetchedAt<3e5)return l.points;if(this._pending.has(a))return this._pending.get(a);let c=this._fetchAndNormalize(r,t,e,i,a);this._pending.set(a,c);try{return await c}finally{this._pending.delete(a)}}async _fetchAndNormalize(r,t,e,i,a){let l=new Date(Date.now()-e*24*60*60*1e3).toISOString(),c=i?["state","sum","change"]:["mean","min","max"];try{let h=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:l,statistic_ids:[r],period:t,types:c}))[r]||[],g=this._normalizeRows(h,i);return this._cache.set(a,{entityId:r,fetchedAt:Date.now(),period:t,points:g}),g}catch(d){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${r}:`,d),[]}}async _fetchBatch(r,t,e,i,a,l){try{let c=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:e,statistic_ids:r,period:t,types:i});for(let d of r){let h=c[d]||[],g=this._normalizeRows(h,a);l.set(d,g),this._cache.set(`${d}:${t}:14`,{entityId:d,fetchedAt:Date.now(),period:t,points:g})}}catch(c){console.warn("[maintenance-supporter] Batch statistics fetch failed:",c)}}_normalizeRows(r,t){let e=[];for(let i of r){let a=null;if(t?a=i.state??null:a=i.mean??null,a===null)continue;let l={ts:i.start,val:a};t||(i.min!=null&&(l.min=i.min),i.max!=null&&(l.max=i.max)),e.push(l)}return e.sort((i,a)=>i.ts-a.ts),e}};var gt=class{constructor(r){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=r}updateHass(r){this.hass=r}async getUsers(r=!1){let t=Date.now();if(!r&&this.usersCache&&t-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=e.users,this.cacheTimestamp=t,this.usersCache}catch(e){return console.error("Failed to fetch users:",e),this.usersCache||[]}}async assignUser(r,t,e){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:r,task_id:t,user_id:e})}async getTasksByUser(r){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:r})).tasks}getUserName(r){return!r||!this.usersCache?null:this.usersCache.find(e=>e.id===r)?.name||null}getUser(r){return!r||!this.usersCache?null:this.usersCache.find(t=>t.id===r)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(r){return r?r===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var es={name:"name",task_type:"maintenance_type",schedule_type:"schedule_type",interval_days:"interval_days",interval_anchor:"interval_anchor",warning_days:"warning_days",last_performed:"last_performed_optional",notes:"notes_optional",documentation_url:"documentation_url_optional",custom_icon:"custom_icon_optional",nfc_tag_id:"nfc_tag_id_optional",responsible_user_id:"responsible_user",entity_slug:"entity_slug",entity_id:"entity_id",area_id:"area_id_optional",manufacturer:"manufacturer_optional",model:"model_optional",serial_number:"serial_number_optional",installation_date:"installation_date_optional",warranty_expiry:"warranty_expiry_optional",checklist:"checklist_steps_optional",reason:"reason",feedback:"feedback",cost:"cost",duration:"duration",description:"description_optional",group_name:"name",group_description:"description_optional",environmental_entity:"environmental_entity_optional",environmental_attribute:"environmental_attribute_optional",trigger_above:"trigger_above",trigger_below:"trigger_below",trigger_for_minutes:"trigger_for_minutes"};function is(n,r){let t=es[n];if(!t)return n;let e=s(t,r);return e&&e!==t?e:n}function ss(n){let t=n.match(/data\['([^']+)'\]/)?.[1],e;return(e=n.match(/length of value must be at most (\d+)/))?{field:t,rule:"too_long",param:e[1]}:(e=n.match(/length of value must be at least (\d+)/))?{field:t,rule:"too_short",param:e[1]}:(e=n.match(/value must be at most (\S+)/))?{field:t,rule:"value_too_high",param:e[1]}:(e=n.match(/value must be at least (\S+)/))?{field:t,rule:"value_too_low",param:e[1]}:/required key not provided/.test(n)?{field:t,rule:"required"}:(e=n.match(/expected (\w+)/))?{field:t,rule:"wrong_type",param:e[1]}:/value must be one of/.test(n)?{field:t,rule:"invalid_choice"}:/not a valid value/.test(n)?{field:t,rule:"invalid_value"}:{field:t,rule:"unknown"}}function I(n,r,t){if(t=t??s("action_error",r),typeof n=="string")return n;if(typeof n!="object"||n===null)return t;let e=n,i=e.message||e.error?.message||"";if(!i)return t;let a=ss(i),l=a.field?is(a.field,r):"",c=d=>s(d,r).replace("{field}",l).replace("{n}",a.param??"");switch(a.rule){case"too_long":return c("err_too_long");case"too_short":return c("err_too_short");case"value_too_high":return c("err_value_too_high");case"value_too_low":return c("err_value_too_low");case"required":return c("err_required");case"wrong_type":return c("err_wrong_type").replace("{type}",a.param??"");case"invalid_choice":return c("err_invalid_choice");case"invalid_value":return c("err_invalid_value");default:return i||t}}var V=class extends S{constructor(){super(...arguments);this.label="";this.value="";this.placeholder="";this.type="text";this.required=!1;this.disabled=!1}_onInput(t){let e=t.target.value;this.value=e,this.dispatchEvent(new CustomEvent("input",{bubbles:!0,composed:!0,detail:{value:e}}))}render(){return o`
      ...
      <div class="doc-header">
        ...
        ${this.canWrite?o`
              <div class="doc-actions">
                ...
                >
                  ${le.map(e=>o`<option value=${e}>${s(`doc_cat_${e}`,t)}</option>`)}
                </select>
                <label
    ...
    `}_renderEdit(t,e){let i=t.kind==="file";return o`
      <div class="doc-row editing">
        ...
        ${i?o`<select
            ...
            >
              ${le.map(a=>o`<option value=${a} ?selected=${a===this._editCategory}>${s(`doc_cat_${a}`,e)}</option>`)}
            </select>`:p}
  ...
  `,u([y({attribute:!1})],D.prototype,"hass",2),u([y({attribute:!1})],D.prototype,"entryId",2),u([y({type:Boolean})],D.prototype,"canWrite",2),u([_()],D.prototype,"_docs",2),u([_()],D.prototype,"_loaded",2),u([_()],D.prototype,"_busy",2),u([_()],D.prototype,"_error",2),u([_()],D.prototype,"_hint",2),u([_()],D.prototype,"_addingLink",2),u([_()],D.prototype,"_linkUrl",2),u([_()],D.prototype,"_linkTitle",2),u([_()],D.prototype,"_category",2),u([_()],D.prototype,"_thumbs",2),u([_()],D.prototype,"_lightboxUrl",2),u([_()],D.prototype,"_editingId",2),u([_()],D.prototype,"_editTitle",2),u([_()],D.prototype,"_editCategory",2),u([_()],D.prototype,"_dragOver",2);customElements.get("maintenance-documents-section")||customElements.define("maintenance-documents-section",D);var rs=["manual","warranty","invoice","spare_parts","photo","other"],ns={manual:"mdi:book-open-variant",warranty:"mdi:shield-check",invoice:"mdi:receipt-text-outline",spare_parts:"mdi:cog-outline",photo:"mdi:image-outline",other:"mdi:file-document-outline"},Q=class extends S{constructor(){super(...arguments);this.canWrite=!1;this._docs=[];this._loaded=!1;this._busy=!1;this._error="";this._attachId="";this._loadedKey="";this._localeReady=!1}get _lang(){return this.hass?.language||"en"}updated(t){super.updated(t),this.hass&&!this._localeReady&&(this._localeReady=!0,at(this._lang).then(()=>this.requestUpdate()));let e=`${this.entryId}|${this.taskId}`;this.hass&&this.entryId&&this.taskId&&this._loadedKey!==e&&(this._loadedKey=e,this._load())}async _load(){try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:this.entryId});this._docs=t.documents||[],this._loaded=!0,this._error=""}catch(t){this._error=I(t,this._lang),this._loaded=!0}}_linked(){return this._docs.filter(t=>(t.task_ids||[]).includes(this.taskId))}_available(){return this._docs.filter(t=>!(t.task_ids||[]).includes(this.taskId))}async _setTaskIds(t,e){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:t.id,task_ids:e}),await this._load()}catch(i){this._error=I(i,this._lang)}finally{this._busy=!1}}_link(){let t=this._docs.find(e=>e.id===this._attachId);t&&(this._attachId="",this._setTaskIds(t,[...t.task_ids||[],this.taskId]))}_unlink(t){this._setTaskIds(t,(t.task_ids||[]).filter(e=>e!==this.taskId))}_isPdf(t){return t.mime==="application/pdf"||(t.filename||"").toLowerCase().endsWith(".pdf")}_pageFor(t){return this._isPdf(t)?t.task_pages?.[this.taskId]:void 0}async _open(t){if(t.kind==="weblink"){window.open(t.url,"_blank","noopener");return}let e=this._pageFor(t),i=e?`#page=${e}`:"",a=window.open("about:blank","_blank");try{let l=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${t.id}`,expires:300});a&&(a.location.href=new URL(l.path+i,window.location.origin).href)}catch(l){a&&a.close(),this._error=I(l,this._lang)}}async _setPage(t,e){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:t.id,task_pages:{[this.taskId]:e}}),await this._load()}catch(i){this._error=I(i,this._lang)}finally{this._busy=!1}}async _download(t){try{let e=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${t.id}`,expires:30});ne(e.path,t.filename||t.title||"document")}catch(e){this._error=I(e,this._lang)}}render(){if(!this._loaded||this._docs.length===0)return p;let t=this._lang,e=this._linked(),i=this._available();return o`
    ...
    `}_renderRow(t,e){let i=t.kind==="file",a=this._isPdf(t),l=t.task_pages?.[this.taskId],c=(t.tags||[]).find(h=>rs.includes(h))||"other",d=i?rt(t.size):s("doc_link_badge",e);return o`
      <div class="tdoc-row">
        <ha-icon class="tdoc-icon" icon=${i?ns[c]:"mdi:link-variant"}></ha-icon>
        <div
          class="tdoc-info"
          role="button"
          tabindex="0"
...
`).map(c=>c.trim()).filter(Boolean).slice(0,100);t.checklist=l.length?l:null}if(this.completionActionsEnabled){let l=this._actionService.trim();if(l&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(l)){let g={service:l},m=this._actionTargetEntity.trim();m&&(g.target={entity_id:m});let v=this._buildActionData();Object.keys(v).length>0&&(g.data=v),t.on_complete_action=g}else t.on_complete_action=null;let c={};this._qcNotes.trim()&&(c.notes=this._qcNotes.trim());let d=parseFloat(this._qcCost);!isNaN(d)&&d>=0&&(c.cost=d);let h=parseInt(this._qcDuration,10);!isNaN(h)&&h>=0&&(c.duration=h),this._qcFeedback&&(c.feedback=this._qcFeedback),t.quick_complete_defaults=Object.keys(c).length?c:null}let e=await this.hass.connection.sendMessagePromise(t),i=this._taskId||e?.task_id,a=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(i&&this._scheduleType==="sensor_based"&&a)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:i,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(t){this._error=I(t,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return p;let t=this._lang,e=this._triggerType==="compound";return o`
      ...
      <div class="select-row">
        ...
        >
          ${ds.map(i=>o`<option value=${i} ?selected=${i===this._triggerType}>${s(i,t)}</option>`)}
        </select>
      </div>
    ...
    `}_patchCondition(t,e){this._compoundConditions=this._compoundConditions.map((i,a)=>a===t?{...i,...e}:i)}_addCondition(){this._compoundConditions=[...this._compoundConditions,ps()]}_removeCondition(t){this._compoundConditions=this._compoundConditions.filter((e,i)=>i!==t)}_renderCompoundEditor(){let t=this._lang;return o`
      <div class="select-row">
    ...
    `}_renderCondition(t,e){let i=this._lang,a=e+1;return o`
      <div class="compound-condition">
        ...
        <div class="select-row">
          ...
          >
            ${ni.map(l=>o`<option value=${l} ?selected=${l===t.type}>${s(l,i)}</option>`)}
          </select>
        </div>
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${os.map(i=>o`<option value=${i} ?selected=${i===this._type}>${s(i,t)}</option>`)}
            </select>
          </div>
          <div class="select-row">
            ...
            >
              ${ls.map(i=>o`<option value=${i} ?selected=${i===this._scheduleType}>${s(i,t)}</option>`)}
            </select>
          </div>
...
<div class="qr-row">
  <div class="qr-col">
    <img src="${oi(this._viewResult.svg_data_uri)}" alt="QR Info" />
    <div class="qr-label">${g}</div>
  </div>
  ${h?`<div class="qr-col">
    <img src="${oi(this._completeResult.svg_data_uri)}" alt="QR Complete" />
    <div class="qr-label">${m}</div>
  </div>`:""}
</div>
<div class="url">${St(this._viewResult.url)}</div>
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `,u([y({attribute:!1})],tt.prototype,"hass",2),u([y()],tt.prototype,"lang",2),u([_()],tt.prototype,"_open",2),u([_()],tt.prototype,"_loading",2),u([_()],tt.prototype,"_error",2),u([_()],tt.prototype,"_viewResult",2),u([_()],tt.prototype,"_completeResult",2),u([_()],tt.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",tt);var li=5;function Ft(n){let r=n.getFullYear(),t=String(n.getMonth()+1).padStart(2,"0"),e=String(n.getDate()).padStart(2,"0");return`${r}-${t}-${e}`}function ms(n,r){let t=[];for(let e=0;e<r;e++){let i=new Date(n);i.setDate(i.getDate()+e),i.setHours(0,0,0,0),t.push(Ft(i))}return t}function ce(n,r){let[t,e,i]=n.split("-").map(Number),a=new Date(t,e-1,i);return a.setDate(a.getDate()+r),Ft(a)}function vs(n){if(!n||n.length===0)return null;let r=n.map(t=>t.cost).filter(t=>typeof t=="number");return r.length===0?null:r.reduce((t,e)=>t+e,0)/r.length}function bs(n){let{windowStart:r,windowEnd:t,task:e,entryId:i,objectName:a}=n,l=[],c=(m,v)=>({date:m,entry_id:i,task_id:e.id,task_name:e.name,object_name:a,status:v&&(e.status==="overdue"||e.status==="triggered")?"ok":e.status,days_until_due:v?null:e.days_until_due??null,projected:v,schedule_type:e.schedule_type,interval_days:e.interval_days??null,interval_unit:e.interval_unit??null,responsible_user_id:e.responsible_user_id??null,avg_cost:vs(e.history),adaptive_enabled:!!e.adaptive_config?.enabled,prediction_confidence:e.threshold_prediction_confidence??null}),d=Math.max(1,Math.round(Me(e.interval_days,e.interval_unit)));if(e.status==="overdue"||e.status==="triggered"){if(l.push(c(r,!1)),e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0){let m=ce(r,d),v=1;for(;m<=t&&v<li;)l.push(c(m,!0)),v++,m=ce(m,d)}return l}let h=e.next_due;if(typeof h!="string"||!h)return l;let g=h.slice(0,10);if(g>=r&&g<=t)l.push(c(g,!1));else if(g>t)return l;if(e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0){let m=ce(g,d),v=l.length;for(;m<=t&&v<li;)m>=r&&(l.push(c(m,!0)),v++),m=ce(m,d)}return l}var ci={overdue:0,triggered:1,due_soon:2,ok:3};function di(n,r,t,e=null){let i=ms(r,t),a=i[0],l=i[i.length-1],c=[];for(let h of n){let g=h.object?.name||"",m=h.entry_id,v=h.tasks||[];for(let f of v){if(e&&f.responsible_user_id!==e||f.enabled===!1)continue;let w=bs({windowStart:a,windowEnd:l,task:f,entryId:m,objectName:g});c.push(...w)}}let d=new Map;for(let h of i)d.set(h,[]);for(let h of c){let g=d.get(h.date);g&&g.push(h)}for(let[,h]of d)h.sort((g,m)=>{let v=ci[g.status]??99,f=ci[m.status]??99;if(v!==f)return v-f;if(g.projected!==m.projected)return g.projected?1:-1;let w=g.object_name.localeCompare(m.object_name);return w!==0?w:g.task_name.localeCompare(m.task_name)});return i.map(h=>({date:h,events:d.get(h)??[]}))}var fs={completed:"ok",reset:"ok",skipped:"due_soon",triggered:"triggered",trigger_replaced:"triggered",trigger_removed:"ok"};function ys(n,r){let t=[];for(let e=r-1;e>=0;e--){let i=new Date(n);i.setDate(i.getDate()-e),i.setHours(0,0,0,0),t.push(Ft(i))}return t}function pi(n,r,t,e=null){let i=ys(r,t),a=i[0],l=i[i.length-1],c=new Map;for(let h of i)c.set(h,[]);for(let h of n){let g=h.object?.name||"",m=h.entry_id,v=h.tasks||[];for(let f of v){if(e&&f.responsible_user_id!==e)continue;let w=f.history||[];for(let b of w){if(typeof b?.timestamp!="string")continue;let j=b.timestamp.slice(0,10);if(j<a||j>l)continue;let z=c.get(j);if(!z)continue;let H=b.type??"completed";z.push({date:j,entry_id:m,task_id:f.id,task_name:f.name,object_name:g,status:fs[H]??"ok",days_until_due:null,projected:!1,schedule_type:f.schedule_type,interval_days:f.interval_days??null,responsible_user_id:f.responsible_user_id??null,avg_cost:typeof b.cost=="number"?b.cost:null,adaptive_enabled:!!f.adaptive_config?.enabled,prediction_confidence:null,history_timestamp:b.timestamp,history_type:H,history_cost:typeof b.cost=="number"?b.cost:null,history_notes:typeof b.notes=="string"?b.notes:null,history_duration:typeof b.duration=="number"?b.duration:null})}}}let d={completed:0,reset:1,skipped:2,triggered:3,trigger_replaced:4};for(let[,h]of c)h.sort((g,m)=>{let v=d[g.history_type??""]??99,f=d[m.history_type??""]??99;if(v!==f)return v-f;let w=g.object_name.localeCompare(m.object_name);return w!==0?w:g.task_name.localeCompare(m.task_name)});return i.map(h=>({date:h,events:c.get(h)??[]}))}var ui=A`
  .cal-controls {
    display: flex;
    gap: 12px;
    align-items: center;
...
`;var et=class extends S{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(t){this._config={...t},t.past_days&&[30,90].includes(t.past_days)?this._pastDays=t.past_days:t.window_days&&[7,14,30,365].includes(t.window_days)&&(this._windowDays=t.window_days,this._pastDays=0),typeof t.user_filter=="string"&&(this._userFilter=t.user_filter)}getCardSize(){return 6}get _lang(){return this.hass?.language||"en"}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(t){super.updated(t);let e=this.hass?.language;if(e&&!te(e)&&at(e).then(()=>this.requestUpdate()),t.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[t,e]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=t.objects,this._stats=e}catch{}}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(t=>{let e=t;this._objects=e.objects},{type:"maintenance_supporter/subscribe"})}catch{}}_onEventClick(t){if(t.history_timestamp){this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:t.entry_id,task_id:t.task_id,original_timestamp:t.history_timestamp},bubbles:!0,composed:!0}));return}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:t.entry_id,task_id:t.task_id},bubbles:!0,composed:!0}))}render(){if(!this.hass)return p;let t=this._lang,e=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.title,l=null;this._userFilter&&(l=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let c=new Date;c.setHours(0,0,0,0);let d=this._pastDays>0,h=d?pi(this._objects,c,this._pastDays,l):di(this._objects,c,this._windowDays,l),g=Ft(c),m=this._windowDays===365||d,v=m?h.filter(b=>b.events.length>0):h,f=b=>{let j=`cal-status-${b.status}`,z=b.projected?"cal-event-projected":"",H=b.status==="overdue"&&b.days_until_due!=null?` (${Math.abs(b.days_until_due)}d ${s("overdue",t).toLowerCase()})`:"",M=b.projected&&b.interval_days?o`<span class="cal-event-recur">${b.interval_unit&&b.interval_unit!=="days"?`${b.interval_days} ${s("unit_"+b.interval_unit,t)}`:s("cal_every_n_days",t).replace("{n}",String(b.interval_days))}</span>`:p,T=b.schedule_type==="sensor_based",Y=T?o`<ha-icon class="cal-event-icon cal-source-sensor"
          ...
          </span>`:p,W=this._stats?.budget?.currency_symbol||_t,O=b.history_type?s(b.history_type,t):s(b.status,t);return o`
    ...
    `],u([y({attribute:!1})],et.prototype,"hass",2),u([_()],et.prototype,"_config",2),u([_()],et.prototype,"_objects",2),u([_()],et.prototype,"_stats",2),u([_()],et.prototype,"_windowDays",2),u([_()],et.prototype,"_pastDays",2),u([_()],et.prototype,"_userFilter",2),u([_()],et.prototype,"_unsub",2);var xs=[{value:7,label:"Week (7 days)"},{value:14,label:"Fortnight (14 days)"},{value:30,label:"Month (30 days, default)"},{value:365,label:"Year (365 days, empty days collapsed)"}],At=class extends S{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"}}setConfig(t){this._config={...t}}_valueChanged(t,e){let i={...this._config,[t]:e};t==="show_window_chips"&&e===!0&&delete i.show_window_chips,t==="show_user_filter"&&e===!0&&delete i.show_user_filter,t==="title"&&(!e||typeof e=="string"&&e.trim()==="")&&delete i.title,t==="user_filter"&&e===""&&delete i.user_filter,this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}render(){let t=this._config.window_days??30,e=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.user_filter??"",l=this._config.title??"";return o`
      <div class="editor">
        <div class="row">
        ...
        <div class="row">
          ...
          >
            ${xs.map(c=>o`<option value="${c.value}" ?selected=${c.value===t}>${c.label}</option>`)}
          </select>
        </div>
  ...
  `,u([y({attribute:!1})],J.prototype,"hass",2),u([_()],J.prototype,"_open",2),u([_()],J.prototype,"_title",2),u([_()],J.prototype,"_message",2),u([_()],J.prototype,"_confirmText",2),u([_()],J.prototype,"_danger",2),u([_()],J.prototype,"_inputLabel",2),u([_()],J.prototype,"_inputType",2),u([_()],J.prototype,"_inputValue",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",J);var _i={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},gi=n=>(...r)=>({_$litDirective$:n,values:r}),pe=class{constructor(r){}get _$AU(){return this._$AM._$AU}_$AT(r,t,e){this._$Ct=r,this._$AM=t,this._$Ci=e}_$AS(r,t){return this.update(r,t)}update(r,t){return this.render(...t)}};var Nt=class extends pe{constructor(r){if(super(r),this.it=p,r.type!==_i.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(r){if(r===p||r==null)return this._t=void 0,this.it=r;if(r===lt)return r;if(typeof r!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(r===this.it)return this._t;this.it=r;let t=[r];return t.raw=t,this._t={_$litType$:this.constructor.resultType,strings:t,values:[]}}};Nt.directiveName="unsafeHTML",Nt.resultType=1;var mi=gi(Nt);var ws=["EUR","USD","GBP","JPY","CHF","CAD","AUD","CNY","INR","BRL","CZK","PLN","RUB","SEK","NOK","DKK","UAH"],C=class extends S{constructor(){super(...arguments);this.budget=null;this._settings=null;this._loading=!0;this._importCsv="";this._importLoading=!1;this._includeHistory=!0;this._toast="";this._testingNotification=!1;this._users=[];this._vacEnabled=!1;this._vacStart="";this._vacEnd="";this._vacBuffer=3;this._vacExempt=new Set;this._vacIsActive=!1;this._vacWindowEnd=null;this._vacAllTasks=[];this._vacPreview=[];this._vacPreviewLoading=!1;this._vacSaving=!1;this._qrObjects=[];this._qrSelectedEntries=new Set;this._qrActions=new Set(["view"]);this._qrUrlMode="companion";this._qrBatchLoading=!1;this._qrBatchResults=[];this._qrObjectsLoaded=!1;this._loaded=!1;this._userService=null;this._sendTestNotification=async()=>{this._testingNotification=!0;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/test_notification"}),e=t.message||(t.success?s("test_notification_success",this._lang):s("test_notification_failed",this._lang));this._showToast(e)}catch{this._showToast(s("test_notification_failed",this._lang))}finally{this._testingNotification=!1}}}get _lang(){return this.hass?.language||"en"}updated(t){super.updated(t),t.has("hass")&&this.hass&&!this._loaded?(this._loaded=!0,this._userService=new gt(this.hass),this._loadSettings(),this._loadUsers()):t.has("hass")&&this.hass&&this._userService&&this._userService.updateHass(this.hass)}async _loadUsers(){if(this._userService)try{this._users=await this._userService.getUsers()}catch{this._users=[]}}async _loadSettings(){this._loading=!0;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});this._settings=t,this._hydrateVacationFromSettings()}catch{}this._loading=!1}_hydrateVacationFromSettings(){let t=this._settings?.vacation;t&&(this._vacEnabled=t.enabled,this._vacStart=t.start||"",this._vacEnd=t.end||"",this._vacBuffer=t.buffer_days,this._vacExempt=new Set(t.exempt_task_ids||[]),this._vacIsActive=t.is_active,this._vacWindowEnd=t.window_end)}async _updateSetting(t,e){try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/update",settings:{[t]:e}});this._settings=i,this._showToast(s("settings_saved",this._lang)),this.dispatchEvent(new CustomEvent("settings-changed"))}catch{this._showToast(s("action_error",this._lang))}}_showToast(t){this._toast=t,setTimeout(()=>{this._toast=""},3e3)}_downloadFile(t,e,i){re(t,e,i)}render(){let t=this._lang;return this._loading||!this._settings?o`<div class="settings-loading">Loading…</div>`:o`
    ...
    `}_renderObjectsColumns(t){let e=Ot(this._settings.objects_table_columns);return o`
    ...
    `}_toggleColumn(t,e){let i=new Set(Ot(this._settings.objects_table_columns));e?i.add(t):i.delete(t);let a=Tt.filter(l=>l.required||i.has(l.key)).map(l=>l.key);this._updateSetting("objects_table_columns",a)}_renderGeneral(t){let e=this._settings.general,i=e.notify_targets??[],a=this._settings.budget;return o`
      <div class="settings-section">
...
<body>
  ...
  `,u([y({attribute:!1})],X.prototype,"hass",2),u([y({attribute:!1})],X.prototype,"objects",2),u([_()],X.prototype,"_open",2),u([_()],X.prototype,"_loading",2),u([_()],X.prototype,"_error",2),u([_()],X.prototype,"_groupId",2),u([_()],X.prototype,"_name",2),u([_()],X.prototype,"_description",2),u([_()],X.prototype,"_selected",2);customElements.get("maintenance-group-dialog")||customElements.define("maintenance-group-dialog",X);function Ut(n,r,t=4){if(!isFinite(n)||!isFinite(r))return{ticks:[],niceMin:0,niceMax:1};if(n===r){let h=Math.abs(n)*.1||1;n-=h,r+=h}let e=r-n,i=Math.pow(10,Math.floor(Math.log10(e/Math.max(1,t)))),a=i;for(let h of[1,2,5,10])if(a=i*h,e/a<=t+.5)break;let l=Math.floor(n/a)*a,c=Math.ceil(r/a)*a,d=[];for(let h=l;h<=c+a*1e-6;h+=a)d.push(Math.abs(h)<a*1e-9?0:h);return{ticks:d,niceMin:l,niceMax:c}}function nt(n){let r=Math.abs(n);return r>=1e6?jt((n/1e6).toFixed(r>=1e7?0:1))+"M":r>=1e4?jt((n/1e3).toFixed(0))+"k":r>=1e3?jt((n/1e3).toFixed(1))+"k":r>=100?n.toFixed(0):r>=10||r>=1?jt(n.toFixed(1)):r===0?"0":jt(n.toFixed(2))}function jt(n){return n.replace(/\.0+$/,"").replace(/(\.\d*[1-9])0+$/,"$1")}function xt(n,r,t){let e=n.toLocaleString(t,{maximumFractionDigits:Math.abs(n)>=100?0:1});return r?`${e} ${r}`:e}function Ct(n,r,t){let e=new Date(n),i=t?{month:"short",day:"numeric",year:"2-digit"}:{month:"short",day:"numeric"};return e.toLocaleDateString(r,i)}function Le(n,r){return new Date(n).toLocaleDateString(r,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function ue(n,r){return new Date(n).getFullYear()!==new Date(r).getFullYear()}function he(n,r,t){if(t<2||r<=n)return[n,r];let e=[];for(let i=0;i<t;i++)e.push(n+(r-n)*i/(t-1));return e}var _e=210,st=46,dt=14,pt=12,vi=14,Es=20+vi,Ts=[{days:7,key:"chart_range_7d"},{days:30,key:"chart_range_30d"},{days:90,key:"chart_range_90d"},{days:365,key:"chart_range_1y"}],F=class extends S{constructor(){super(...arguments);this.points=[];this.events=[];this.unit="";this.lang="en";this.thresholdAbove=null;this.thresholdBelow=null;this.targetValue=null;this.forceZero=!1;this.projection=null;this.rangeDays=30;this.showRange=!0;this.busy=!1;this.hideOutliers=!1;this.showOutlierToggle=!0;this._width=0;this._hover=null;this._ro=null}connectedCallback(){super.connectedCallback(),this._ro=new ResizeObserver(t=>{let e=Math.floor(t[0]?.contentRect?.width||0);e&&Math.abs(e-this._width)>2&&(this._width=e)}),this._ro.observe(this)}disconnectedCallback(){super.disconnectedCallback(),this._ro?.disconnect(),this._ro=null}_emitRange(t){t!==this.rangeDays&&this.dispatchEvent(new CustomEvent("range-change",{detail:{days:t},bubbles:!0,composed:!0}))}_toggleOutliers(){this.dispatchEvent(new CustomEvent("outlier-toggle",{detail:{hide:!this.hideOutliers},bubbles:!0,composed:!0}))}render(){let t=this._width||320,e=[...this.points].sort((a,l)=>a.ts-l.ts),i=this.lang;return o`
    ...
    `}_renderSvg(t,e){let i=this.lang,a=t-st-dt,l=_e-Es,c=l-pt,d=1/0,h=-1/0;for(let k of e)d=Math.min(d,k.min??k.val),h=Math.max(h,k.max??k.val);this.thresholdAbove!=null&&(d=Math.min(d,this.thresholdAbove),h=Math.max(h,this.thresholdAbove)),this.thresholdBelow!=null&&(d=Math.min(d,this.thresholdBelow),h=Math.max(h,this.thresholdBelow)),this.targetValue!=null&&(d=Math.min(d,this.targetValue),h=Math.max(h,this.targetValue)),this.forceZero&&(d=Math.min(d,0));let g=(h-d||1)*.06,m=this.forceZero&&d>=0?0:d-g,{ticks:v,niceMin:f,niceMax:w}=Ut(m,h+g,4);this.forceZero&&d>=0&&f<0&&(f=0,v=v.filter(k=>k>=0));let b=e[0].ts,j=e[e.length-1].ts,z=j-b||1,H=ue(b,j),M=k=>st+(k-b)/z*a,T=k=>pt+(1-(k-f)/(w-f||1))*c,Y=e.map(k=>`${M(k.ts).toFixed(1)},${T(k.val).toFixed(1)}`).join(" "),G=`M${M(e[0].ts).toFixed(1)},${l} `+e.map(k=>`L${M(k.ts).toFixed(1)},${T(k.val).toFixed(1)}`).join(" ")+` L${M(e[e.length-1].ts).toFixed(1)},${l} Z`,W="",O=e.filter(k=>k.min!=null&&k.max!=null);if(O.length>=2){let k=O.map(R=>`${M(R.ts).toFixed(1)},${T(R.max).toFixed(1)}`),$=[...O].reverse().map(R=>`${M(R.ts).toFixed(1)},${T(R.min).toFixed(1)}`);W=`M${k[0]} `+k.slice(1).map(R=>`L${R}`).join(" ")+` L${$.join(" L")} Z`}let q=[];if(this.thresholdBelow!=null){let k=T(this.thresholdBelow);q.push({y:k,h:Math.max(0,l-k),lineY:k,label:`\u25BC ${nt(this.thresholdBelow)}`,labelY:Math.min(l-4,k+13)})}if(this.thresholdAbove!=null){let k=T(this.thresholdAbove);q.push({y:pt,h:Math.max(0,k-pt),lineY:k,label:`\u25B2 ${nt(this.thresholdAbove)}`,labelY:Math.max(pt+11,k-5)})}let ut=e[e.length-1],mt=(this.events||[]).filter(k=>k.ts>=b&&k.ts<=j),$t=he(b,j,Math.max(2,Math.min(5,Math.floor(a/110)+1))),B=this._hover;return o`
      <div class="svg-holder">
        ...
        >
          ...
          ${$t.map((k,$)=>{let R=M(k),ge=$===0?"start":$===$t.length-1?"end":"middle";return P`<text x="${R.toFixed(1)}" y="${_e-5}" text-anchor="${ge}" class="tick-label">${Ct(k,i,H)}</text>`})}

          <line x1="${st}" y1="${l}" x2="${t-dt}" y2="${l}" stroke="var(--divider-color)" stroke-width="1" />

          ${mt.map(k=>{let $=M(k.ts),R=k.type==="completed"?"var(--success-color, #4caf50)":k.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return P`
              <line x1="${$.toFixed(1)}" y1="${pt}" x2="${$.toFixed(1)}" y2="${l}" stroke="${R}" stroke-width="1" opacity="0.14" />
              <rect x="${($-1.5).toFixed(1)}" y="${l+3}" width="3" height="${vi-6}" rx="1.5" fill="${R}">
                <title>${Le(k.ts,i)}</title>
              </rect>`})}
    ...
    <div class="trigger-card">
      <div class="trigger-header">
        ...
        <div>
          ${v?o`
            ...
            <div class="trigger-entity-id">${d.map((w,b)=>o`${b>0?", ":""}<span class="entity-link" @click=${j=>yt(j,w)}>${w}</span>`)}${t.attribute?` \u2192 ${t.attribute}`:""}</div>
          `:o`
            <div class="trigger-entity-name">${l}</div>
            <div class="trigger-entity-id">${c?o`<span class="entity-link" @click=${w=>yt(w,c)}>${c}</span>`:""}${t.attribute?` \u2192 ${t.attribute}`:""}</div>
          `}
        </div>
      ...
      ${f?js(f,e):g!=null?o`
              <div class="trigger-value-row">
                <span class="trigger-current ${n.trigger_active?"active":""}">${typeof g=="number"?xt(g,"",e):g}</span>
      ...
      <div class="trigger-limits">
        ...
        ${m==="compound"?o`
          ...
          ${(t.conditions||[]).map((w,b)=>o`
            <span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${b+1}. ${s(w.type||"unknown",e)}: ${w.entity_id?o`<span class="entity-link" @click=${j=>yt(j,w.entity_id)}>${w.entity_id}</span>`:""}</span>
          `)}
        `:p}
      </div>
      ...
      ${a&&a.length>1?o`
        <div class="trigger-entity-list">
          ${a.map(w=>o`
            <span class="trigger-entity-id">${w.friendly_name} (<span class="entity-link" @click=${b=>yt(b,w.entity_id)}>${w.entity_id}</span>)</span>
          `)}
        </div>
      `:p}
  ...
  `}function As(n,r,t){let e=n.trigger_config,i=n.trigger_current_value;if(!e||i==null)return null;switch(e.type||"threshold"){case"counter":{let a=e.trigger_target_value;if(a==null||a<=0)return null;let l=yi(n,xi(n,t));return{progress:Math.max(0,i-(l?.value??i)),target:a,unit:r,meter:i}}case"state_change":{let a=e.trigger_target_changes;return a==null||a<=0?null:{progress:Math.max(0,i),target:a,unit:"",meter:null}}case"runtime":{let a=e.trigger_runtime_hours;return a==null||a<=0?null:{progress:Math.max(0,i),target:a,unit:"h",meter:null}}}return null}function yi(n,r){if(n.trigger_baseline_value!=null)return{value:n.trigger_baseline_value,ts:bi(n)};if(!r.length)return null;let t=bi(n);if(t==null)return{value:r[0].val,ts:null};let e=r[0],i=Math.abs(r[0].ts-t);for(let a of r){let l=Math.abs(a.ts-t);l<i&&(e=a,i=l)}return{value:e.val,ts:t}}function bi(n){let r=[...n.history].filter(t=>t.type==="completed"||t.type==="reset").sort((t,e)=>new Date(e.timestamp).getTime()-new Date(t.timestamp).getTime())[0];return r?new Date(r.timestamp).getTime():null}function js(n,r){let t=Math.min(999,Math.round(n.progress/n.target*100)),e=t>=100?"over":t>=75?"near":"ok";return o`
    <div class="counter-progress">
      <div class="counter-progress-nums">
        <span class="counter-progress-main">${xt(n.progress,"",r)}<span class="counter-progress-target"> / ${xt(n.target,n.unit,r)}</span></span>
  ...
  `}function $i(n,r,t){let e=n.degradation_trend!=null&&n.degradation_trend!=="insufficient_data",i=n.days_until_threshold!=null,a=n.environmental_factor!=null&&n.environmental_factor!==1;if(!e&&!i&&!a)return p;let l=n.degradation_trend==="rising"?"M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z":n.degradation_trend==="falling"?"M16,18L18.29,15.71L13.41,10.83L9.41,14.83L2,7.41L3.41,6L9.41,12L13.41,8L19.71,14.29L22,12V18H16Z":"M22,12L18,8V11H3V13H18V16L22,12Z";return o`
      ...
      <div class="prediction-grid">
        ...
        ${a&&t.environmental?o`
          <div class="prediction-item">
            ...
            ${n.environmental_entity?o`<span class="prediction-entity entity-link" @click=${c=>yt(c,n.environmental_entity)}>${n.environmental_entity}</span>`:p}
          </div>
        `:p}
      </div>
    </div>
  `}function wi(n,r){let t=n.interval_analysis,e=t?.weibull_beta,i=t?.weibull_eta;if(e==null||i==null||i<=0)return p;let a=n.interval_days??0,l=n.suggested_interval??a;return o`
  ...
  `}function Ls(n,r,t){let e=n.confidence_interval_low,i=n.confidence_interval_high,a=r.suggested_interval??r.interval_days??0,l=r.interval_days??0,c=Math.max(0,e-5),h=i+5-c,g=(e-c)/h*100,m=(i-e)/h*100,v=(a-c)/h*100,f=l>0?(l-c)/h*100:-1;return o`
  ...
  `}function ki(n,r,t,e){let i=Math.max(n||1,r);return o`
  ...
  `}var Ei=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"];function Ti(n,r,t){if(!t.seasonal||!n.seasonal_factor||n.seasonal_factor===1)return p;let e=Ei.map(c=>s(c,r)),i=new Date().getMonth(),a=n.seasonal_factors||n.interval_analysis?.seasonal_factors||null,l=a&&a.length===12?a:e.map((c,d)=>{let h=n.seasonal_factor||1,g=Math.sin((d-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,h+g))});return o`
  ...
  `}function Si(n,r){return Ps(n,r)}function Ps(n,r){let t=n.seasonal_factors??n.interval_analysis?.seasonal_factors;if(!t||t.length!==12)return p;let e=n.interval_analysis?.seasonal_reason,i=new Date().getMonth(),a=300,l=100,c=8,h=l-c-4,g=Math.max(...t,1.5),m=a/12,v=m*.65,f=c+h-1/g*h;return o`
    <div class="seasonal-chart">
      ...
      <div class="seasonal-labels">
        ${Ei.map((w,b)=>o`<span class="seasonal-label ${b===i?"active-month":""}">${s(w,r)}</span>`)}
      </div>
    </div>
  `}var zs=200,Bt=10,Hs=22;function Ai(n,r,t,e){let i=n.history.filter(c=>c.type==="completed"&&(c.cost!=null||c.duration!=null));if(i.length<2)return p;let a=i.some(c=>(c.cost??0)>0),l=i.some(c=>(c.duration??0)>0);return!a&&!l?p:o`
  ...
  `}function Ds(n,r,t){let e=n.history.filter($=>$.type==="completed"&&($.cost!=null||$.duration!=null)).map($=>({ts:new Date($.timestamp).getTime(),cost:$.cost??0,duration:$.duration??0})).sort(($,R)=>$.ts-R.ts);if(e.length<2)return p;let i=e.some($=>$.cost>0),a=e.some($=>$.duration>0);if(!i&&!a)return p;let l=t!=="duration"&&i,c=t!=="cost"&&a,d=l||!c&&i,h=c||!l&&a,g=640,m=zs,v=d?44:12,f=h?44:12,w=g-v-f,b=m-Hs,j=b-Bt,z=e[0].ts,H=e[e.length-1].ts,M=(H-z||864e5)*.05,T=z-M,Y=H+M,G=ue(z,H),W=$=>v+($-T)/(Y-T)*w,O=Ut(0,Math.max(...e.map($=>$.cost))||1,3),q=Ut(0,Math.max(...e.map($=>$.duration))||1,3),ut=$=>Bt+(1-$/(O.niceMax||1))*j,mt=$=>Bt+(1-$/(q.niceMax||1))*j,$t=e.length>1?Math.min(...e.slice(1).map(($,R)=>W($.ts)-W(e[R].ts))):w,B=Math.max(6,Math.min(22,$t*.55)),k=he(z,H,Math.max(2,Math.min(4,e.length)));return o`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${g} ${m}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_history",r)}">
        ${d?O.ticks.map($=>{let R=ut($);return R<Bt-1||R>b+1?p:P`
        ...
        ${h?q.ticks.map($=>{let R=mt($);return R<Bt-1||R>b+1?p:P`<text x="${g-f+6}" y="${(R+3.5).toFixed(1)}" text-anchor="start" fill="var(--accent-color, #ff9800)" font-size="10.5">${nt($)}m</text>`}):p}
        ...
        ${d?e.filter($=>$.cost>0).map($=>P`
          <rect x="${(W($.ts)-B/2).toFixed(1)}" y="${ut($.cost).toFixed(1)}" width="${B.toFixed(1)}" height="${(b-ut($.cost)).toFixed(1)}"
            ...
            <title>${Ct($.ts,r,!0)}: ${$.cost.toLocaleString(r)}€${$.duration?` \xB7 ${$.duration}m`:""}</title>
          </rect>
        `):p}
        ${h?P`
          <polyline points="${e.map($=>`${W($.ts).toFixed(1)},${mt($.duration).toFixed(1)}`).join(" ")}"
            fill="none" stroke="var(--accent-color, #ff9800)" stroke-width="2" stroke-linejoin="round" />
          ${e.map($=>P`
            <circle cx="${W($.ts).toFixed(1)}" cy="${mt($.duration).toFixed(1)}" r="3.5" fill="var(--accent-color, #ff9800)">
              <title>${Ct($.ts,r,!0)}: ${$.duration}m${$.cost?` \xB7 ${$.cost.toLocaleString(r)}\u20AC`:""}</title>
            </circle>
          `)}
        `:p}

        <line x1="${v}" y1="${b}" x2="${g-f}" y2="${b}" stroke="var(--divider-color)" stroke-width="1" />
        ${k.map(($,R)=>{let ge=R===0?"start":R===k.length-1?"end":"middle";return P`<text x="${W($).toFixed(1)}" y="${m-6}" text-anchor="${ge}" fill="var(--secondary-text-color)" font-size="10">${Ct($,r,G)}</text>`})}
      </svg>
    </div>
    <div class="chart-legend">
  ...
  `}function ze(n,r,t){if(!n.trigger_config?.entity_id)return p;let e=n.trigger_config.entity_id,i=r.get(e)||[],a=[];if(i.length>=2)a=i.map(T=>({ts:T.ts,val:T.val}));else{if(!n.history)return p;for(let T of n.history)T.trigger_value!=null&&a.push({ts:new Date(T.timestamp).getTime(),val:T.trigger_value})}if(n.trigger_current_value!=null&&a.push({ts:Date.now(),val:n.trigger_current_value}),a.length<2)return p;a.sort((T,Y)=>T.ts-Y.ts);let l=qs,c=Os,d=a.map(T=>T.val),h=Math.min(...d),g=Math.max(...d),m=g-h||1;h-=m*.1,g+=m*.1;let v=a[0].ts,w=a[a.length-1].ts-v||1,b=T=>(T-v)/w*l,j=T=>2+(1-(T-h)/(g-h))*(c-4),z=a;if(z.length>ji){let T=Math.ceil(z.length/ji);z=z.filter((Y,G)=>G%T===0||G===z.length-1)}let H=z.map(T=>`${b(T.ts).toFixed(1)},${j(T.val).toFixed(1)}`).join(" "),M=n.trigger_active?"var(--error-color, #f44336)":"var(--primary-color)";return o`
  ...
  `}function Ci(n,r){let t=r;if(n.days_until_due==null||!n.interval_days||n.interval_days<=0)return p;let{pct:e,overflow:i}=se(n.interval_days,n.days_until_due,n.interval_unit),a="var(--success-color, #4caf50)";return n.status==="overdue"?a="var(--error-color, #f44336)":n.status==="due_soon"&&(a="var(--warning-color, #ff9800)"),o`
    <div class="days-progress">
      ...
      <div class="days-progress-text">${Et(n.days_until_due,t)}</div>
    </div>
  `}var Fs=["completed","skipped","reset","triggered"];function Ii(n,r){let t=r.lang;return o`
    <div class="history-filters-new">
      <div class="filter-chips">
        ${Fs.map(e=>{let i=n.history.filter(a=>a.type===e).length;return i===0?p:o`
  ...
  `}function Mi(n,r){let t=r.lang,e=r.filter?n.history.filter(i=>i.type===r.filter):n.history;if(r.search){let i=r.search.toLowerCase();e=e.filter(a=>a.notes?.toLowerCase().includes(i))}return e.length===0?o`<p class="empty">${s("no_history",t)}</p>`:o`
    <div class="history-timeline">
      ${[...e].reverse().map(i=>Ns(i,r))}
    </div>
  `}function Ns(n,r){let t=r.lang,e=["completed","reset","skipped"].includes(n.type);return o`
    <div class="history-entry">
      ...
      <div class="history-content">
        ...
        <div class="history-date">${ee(n.timestamp,t)}</div>
  ...
  `}var E=class extends S{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._chartRangeDays=(()=>{try{let t=parseInt(localStorage.getItem("msp-chart-range")||"",10);return[7,30,90,365].includes(t)?t:30}catch{return 30}})();this._hideOutliers=(()=>{try{return localStorage.getItem("msp-chart-hide-outliers")==="1"}catch{return!1}})();this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._adminPanelUserIds=[];this._operatorWriteEnabled=!1;this._defaultWarningDays=7;this._actionLoading=!1;this._moreMenuOpen=!1;this._toastMessage="";this._toastUndo=null;this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab=(()=>{try{let t=localStorage.getItem("msp-overview-tab");return t==="today"||t==="calendar"?t:"dashboard"}catch{return"dashboard"}})();this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._objectSortMode="alphabetical";this._groupByMode="none";this._objectViewMode="cards";this._objectsTableColumns=ae;this._showArchived=!1;this._bulkMode=!1;this._bulkSelected=new Set;this._collapsedSections=(()=>{try{return new Set(JSON.parse(localStorage.getItem("msp-collapsed-sections")||"[]"))}catch{return new Set}})();this._paletteOpen=!1;this._paletteQuery="";this._paletteActive=0;this._templateGalleryOpen=!1;this._templates=[];this._templateCategories={};this._templateBusy=!1;this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=t=>this._onPopState(t);this._deepLinkHandled=!1;this._paletteKeydown=t=>{if((t.ctrlKey||t.metaKey)&&(t.key==="k"||t.key==="K")){t.preventDefault(),this._paletteOpen?this._closePalette():this._openPalette();return}if(!this._paletteOpen)return;let e=this._paletteResults;if(t.key==="Escape")t.preventDefault(),this._closePalette();else if(t.key==="ArrowDown")t.preventDefault(),this._paletteActive=Math.min(this._paletteActive+1,e.length-1);else if(t.key==="ArrowUp")t.preventDefault(),this._paletteActive=Math.max(this._paletteActive-1,0);else if(t.key==="Enter"){t.preventDefault();let i=e[this._paletteActive];i&&this._selectPaletteResult(i)}};this._onDialogEvent=async()=>{try{await this._loadData()}catch{}};this._onCalendarLlCustom=t=>{let e=t.detail;e?.type==="maintenance-supporter:open-task"&&e.entry_id&&e.task_id&&(t.stopPropagation(),this._showTask(e.entry_id,e.task_id))};this._onHistoryEntrySaved=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"en"}get _isOperator(){let t=this.hass?.user;return t?t.is_admin?!1:!(this._operatorWriteEnabled&&this._adminPanelUserIds.includes(t.id)):!0}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler),window.addEventListener("keydown",this._paletteKeydown);let t=localStorage.getItem("maintenance_supporter_sort");t&&["due_date","object","type","task_name","area","assigned_user","group"].includes(t)&&(this._sortMode=t);let e=localStorage.getItem("maintenance_supporter_object_sort");e&&["alphabetical","due_soonest","task_count"].includes(e)&&(this._objectSortMode=e);let i=localStorage.getItem("maintenance_supporter_groupby");i&&["none","area","group","user"].includes(i)&&(this._groupByMode=i);let a=localStorage.getItem("maintenance_supporter_object_view");(a==="cards"||a==="table")&&(this._objectViewMode=a)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),window.removeEventListener("keydown",this._paletteKeydown),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(t){super.updated(t);let e=this.hass?.language;if(e&&!te(e)&&at(e).then(()=>this.requestUpdate()),t.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new oe(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new gt(this.hass),this._userService.getUsers())}}async _loadData(){let[t,e,i,a,l]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);if(t&&(this._objects=t.objects),e&&(this._stats=e),i&&(this._budget=i),a&&(this._groups=a.groups||{}),l){let c=l;this._features=c.features,this._adminPanelUserIds=c.admin_panel_user_ids||[],this._operatorWriteEnabled=c.operator_write_enabled??!1;let d=c.general?.default_warning_days;typeof d=="number"&&d>=0&&d<=365&&(this._defaultWarningDays=d),this._objectsTableColumns=Ot(c.objects_table_columns)}this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let t=new URLSearchParams(window.location.search),e=t.get("ms_action"),i=()=>{let g=window.location.pathname+window.location.hash;history.replaceState(history.state,"",g)};if(e==="add_object"){this._deepLinkHandled=!0,i(),requestAnimationFrame(()=>{this.shadowRoot?.querySelector("maintenance-object-dialog")?.openCreate()});return}if(e==="open_vacation"||e==="open_budget"||e==="open_groups"||e==="open_settings"){this._deepLinkHandled=!0,i(),this._overviewTab="settings",requestAnimationFrame(()=>{let g=this.shadowRoot?.querySelector("maintenance-settings-view"),m=e.replace("open_","");g?.scrollToSection?.(m)});return}let a=t.get("entry_id");if(!a)return;this._deepLinkHandled=!0;let l=t.get("task_id"),c=t.get("action"),d=window.location.pathname+window.location.hash;history.replaceState(history.state,"",d);let h=this._getObject(a);if(!h){this._showOverview();return}if(l){let g=h.tasks.find(m=>m.id===l);if(!g){this._showObject(a);return}this._showTask(a,l),c==="complete"?requestAnimationFrame(()=>{this._openCompleteDialog(a,l,g.name,this._features.checklists?g.checklist:void 0,this._features.adaptive&&!!g.adaptive_config?.enabled)}):c==="quick_complete"&&requestAnimationFrame(()=>{this._handleQuickComplete(a,l,g)})}else this._showObject(a)}_isCounterEntity(t){if(!t)return!1;let e=t.type||"threshold";return e==="counter"||e==="state_change"}async _fetchDetailStats(t,e){if(!this._statsService)return;let i=await this._statsService.getDetailStats(t,e,this._chartRangeDays),a=new Map(this._detailStatsData);a.set(t,i),this._detailStatsData=a}_setChartRange(t){if(t===this._chartRangeDays)return;this._chartRangeDays=t;try{localStorage.setItem("msp-chart-range",String(t))}catch{}let e=this._selectedEntryId&&this._selectedTaskId?this._getTask(this._selectedEntryId,this._selectedTaskId):null,i=e?.trigger_config?.entity_id;if(i){let a=new Map(this._detailStatsData);a.delete(i),this._detailStatsData=a,this._fetchDetailStats(i,this._isCounterEntity(e.trigger_config))}}_setHideOutliers(t){if(t!==this._hideOutliers){this._hideOutliers=t;try{localStorage.setItem("msp-chart-hide-outliers",t?"1":"0")}catch{}}}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let t=[];for(let i of this._objects)for(let a of i.tasks){let l=a.trigger_config?.entity_id;l&&t.push({entityId:l,isCounter:this._isCounterEntity(a.trigger_config)})}if(t.length===0)return;let e=await this._statsService.getBatchMiniStats(t);this._miniStatsData=new Map([...this._miniStatsData,...e])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(t=>{let e=t;this._objects=e.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let t=[];for(let m of this._objects)for(let v of m.tasks){if(!this._showArchived&&v.archived||this._filterStatus&&v.status!==this._filterStatus)continue;if(this._filterUser){let w=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(v.responsible_user_id!==w)continue}let f=[];for(let w of Object.values(this._groups))w.task_refs?.some(b=>b.entry_id===m.entry_id&&b.task_id===v.id)&&f.push(w.name);t.push({entry_id:m.entry_id,task_id:v.id,object_name:m.object.name,task_name:v.name,type:v.type,schedule_type:v.schedule_type,status:v.status,days_until_due:v.days_until_due??null,next_due:v.next_due??null,trigger_active:v.trigger_active,trigger_current_value:v.trigger_current_value??null,trigger_current_delta:v.trigger_current_delta??null,trigger_config:v.trigger_config??null,trigger_entity_info:v.trigger_entity_info??null,times_performed:v.times_performed,total_cost:v.total_cost,interval_days:v.interval_days??null,interval_unit:v.interval_unit??null,interval_anchor:v.interval_anchor??null,is_done:v.is_done??!1,archived:v.archived??!1,history:v.history||[],enabled:v.enabled,nfc_tag_id:v.nfc_tag_id??null,area_id:m.object.area_id??null,responsible_user_id:v.responsible_user_id??null,group_names:f})}let e={overdue:0,triggered:1,due_soon:2,ok:3},i=(m,v)=>(e[m.status]??9)-(e[v.status]??9),a=(m,v)=>(m.days_until_due??99999)-(v.days_until_due??99999),l=(m,v)=>i(m,v)||a(m,v),c=m=>m.area_id&&this.hass?.areas?.[m.area_id]?.name||"",d=m=>m.responsible_user_id&&this._userService?.getUserName(m.responsible_user_id)||"",h=m=>m.group_names[0]||"",g={due_date:l,object:(m,v)=>m.object_name.localeCompare(v.object_name)||l(m,v),type:(m,v)=>m.type.localeCompare(v.type)||l(m,v),task_name:(m,v)=>m.task_name.localeCompare(v.task_name),area:(m,v)=>{let f=c(m),w=c(v);return!f&&w?1:f&&!w?-1:f.localeCompare(w)||l(m,v)},assigned_user:(m,v)=>{let f=d(m),w=d(v);return!f&&w?1:f&&!w?-1:f.localeCompare(w)||l(m,v)},group:(m,v)=>{let f=h(m),w=h(v);return!f&&w?1:f&&!w?-1:f.localeCompare(w)||l(m,v)}};return t.sort(g[this._sortMode]),t}_getObject(t){return this._objects.find(e=>e.entry_id===t)}_getTask(t,e){return this._getObject(t)?.tasks.find(a=>a.id===e)}_pushPanelState(t,e,i){let a={msp_view:t,msp_entry:e||null,msp_task:i||null};history.pushState(a,"")}_onPopState(t){let e=t.state;if(e?.msp_view&&(this._view=e.msp_view,this._selectedEntryId=e.msp_entry||null,this._selectedTaskId=e.msp_task||null,this._moreMenuOpen=!1,e.msp_view==="task"&&e.msp_entry&&e.msp_task)){this._historyFilter=null;let i=this._getTask(e.msp_entry,e.msp_task);i?.trigger_config?.entity_id&&this._fetchDetailStats(i.trigger_config.entity_id,this._isCounterEntity(i.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1,this._scrollContentToTop()}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null,this._scrollContentToTop()}_filterByStatus(t){this._filterStatus=t,this._overviewTab!=="dashboard"&&(this._overviewTab="dashboard"),this._scrollContentToTop()}_scrollContentToTop(){requestAnimationFrame(()=>{let t=this.shadowRoot?.querySelector(".content");t&&t.scrollTo({top:0,behavior:"smooth"})})}_showObject(t){this._pushPanelState("object",t),this._view="object",this._selectedEntryId=t,this._selectedTaskId=null,this._scrollContentToTop()}_showTask(t,e){this._pushPanelState("task",t,e),this._view="task",this._selectedEntryId=t,this._selectedTaskId=e,this._activeTab="overview",this._historyFilter=null,this._scrollContentToTop();let i=this._getTask(t,e);if(i?.trigger_config?.entity_id){let a=i.trigger_config.entity_id,l=this._isCounterEntity(i.trigger_config);this._fetchDetailStats(a,l)}}_showToast(t){this._toastTimer&&clearTimeout(this._toastTimer),this._toastUndo=null,this._toastMessage=t,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}_showUndoToast(t,e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage=t,this._toastUndo=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastUndo=null,this._toastTimer=null},7e3)}_runToastUndo(){let t=this._toastUndo;this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage="",this._toastUndo=null,this._toastTimer=null,t?.()}_openPalette(){this._paletteQuery="",this._paletteActive=0,this._paletteOpen=!0,this.updateComplete.then(()=>{this.shadowRoot?.querySelector(".palette-input")?.focus()})}_closePalette(){this._paletteOpen=!1,this._paletteQuery=""}get _paletteResults(){let t=this._paletteQuery.trim().toLowerCase(),e=[];for(let i of this._objects){let a=i.object.name||"";(!t||a.toLowerCase().includes(t))&&e.push({kind:"object",entryId:i.entry_id,label:a,sub:s("object",this._lang)});for(let l of i.tasks){if(l.archived)continue;let c=l.name||"";(!t||c.toLowerCase().includes(t)||a.toLowerCase().includes(t))&&e.push({kind:"task",entryId:i.entry_id,taskId:l.id,label:c,sub:a})}if(e.length>60)break}return e.slice(0,40)}_selectPaletteResult(t){this._closePalette(),t.kind==="task"&&t.taskId?this._showTask(t.entryId,t.taskId):this._showObject(t.entryId)}_renderPalette(){if(!this._paletteOpen)return p;let t=this._lang,e=this._paletteResults;return o`
    ...
    `}_statusBadge(t,e,i){let a=this._lang,l=t?"archived":e?"done":i,c=t?"archived":e?"completed":i,d=t?s("archived",a):e?s("completed",a):s(i,a);return o`<span class="status-badge ${l}"><ha-icon icon="${Xt[c]||"mdi:circle-medium"}"></ha-icon>${d}</span>`}_setOverviewTab(t){this._overviewTab=t;try{localStorage.setItem("msp-overview-tab",t)}catch{}this._scrollContentToTop()}_renderToday(){let t=this._lang,e=this._taskRows,i=h=>`${h.entry_id}:${h.task_id}`,a=e.filter(h=>h.status==="overdue"||h.trigger_active),l=new Set(a.map(i)),c=e.filter(h=>!l.has(i(h))&&h.days_until_due===0);c.forEach(h=>l.add(i(h)));let d=e.filter(h=>!l.has(i(h))&&h.days_until_due!=null&&h.days_until_due>0&&h.days_until_due<=7);return a.length+c.length+d.length===0?o`
        ...
        ${e.map(l=>o`
          <div class="today-row" @click=${()=>this._showTask(l.entry_id,l.task_id)}>
            ...
            <div class="today-main">
              ...
              <div class="today-object">${l.object_name} · ${Et(l.days_until_due,a)}</div>
            </div>
    ...
    `}_renderGroupedTasks(t,e){let i=new Map,a=s("unassigned",e);for(let d of t){let h=[];this._groupByMode==="area"?h=[(d.area_id?this.hass?.areas?.[d.area_id]?.name:null)||a]:this._groupByMode==="user"?h=[(d.responsible_user_id?this._userService?.getUserName(d.responsible_user_id):null)||a]:this._groupByMode==="group"&&(h=d.group_names.length>0?d.group_names:[a]);for(let g of h)i.has(g)||i.set(g,[]),i.get(g).push(d)}let l=[...i.entries()].sort(([d],[h])=>d===a&&h!==a?1:h===a&&d!==a?-1:d.localeCompare(h)),c=this._groupByMode==="area"?"mdi:map-marker-outline":this._groupByMode==="group"?"mdi:folder-outline":"mdi:account-outline";return o`
      ...
      `)}
    `}_warrantyLabel(t,e,i){return t.kind==="expired"?s("warranty_expired",i):t.kind==="expiring"?s("warranty_expires_in",i).replace("{days}",String(t.days??0)):s("warranty_valid_until",i).replace("{date}",K(e,i))}_renderWarrantyMeta(t,e){let i=Re(t);return o`<p class="meta">${s("warranty",e)}:
    ...
    `}_renderOverviewRow(t){let e=this._lang,i=t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0,a=0,l=Zt.ok,c=!1;if(i&&t.days_until_due!==null){let v=se(t.interval_days,t.days_until_due,t.interval_unit);a=v.pct,c=v.overflow,t.status==="overdue"?l=Zt.overdue:t.status==="due_soon"&&(l=Zt.due_soon)}let d=t.area_id?this.hass?.areas?.[t.area_id]?.name:null,h=t.responsible_user_id?this._userService?.getUserName(t.responsible_user_id):null,g=t.group_names.length>0||d||h,m=this._bulkMode&&this._bulkSelected.has(this._bulkKey(t));return o`
      <div class="task-row${t.enabled?"":" task-disabled"}${m?" bulk-selected":""}">
        ...
        <span class="due-cell" @click=${()=>this._showTask(t.entry_id,t.task_id)}>
          <span class="due-text">${Et(t.days_until_due,e)}</span>
          ${i?o`<div class="days-bar"><div class="days-bar-fill${c?" overflow":""}" style="width:${a}%;background:${l}"></div></div>`:p}
          ${t.trigger_config?Pe(t):!i&&t.trigger_active?o`<span style="color:var(--maint-triggered-color);font-weight:600">⚡</span>`:p}
        ...
        ${t.tasks.length===0?o`<div class="empty-state-centered">
              ...
              <div class="task-row${d.enabled?"":" task-disabled"}">
                ...
                <span class="due-cell" @click=${()=>this._showTask(t.entry_id,d.id)}>
                  <span class="due-text">${Et(d.days_until_due,i)}</span>
                  ${d.trigger_config?Pe(d):p}
                  ${ze(d,this._miniStatsData,this._lang)}
                </span>
                <span class="row-actions">
    ...
    `}_renderOverviewTab(t){let e=this._lang,i=this._features.adaptive&&t.suggested_interval&&t.suggested_interval!==t.interval_days,a=this._features.seasonal&&t.seasonal_factor&&t.seasonal_factor!==1,l=i||a,c=this._features.adaptive&&t.interval_analysis?.weibull_beta!=null&&t.interval_analysis?.weibull_eta!=null,d=this._features.seasonal&&(t.seasonal_factors?.length===12||t.interval_analysis?.seasonal_factors?.length===12);return o`
      <div class="tab-content overview-tab">
        ...
        ${Ci(t,this._lang)}
        ${fi(t,this._sparklineCtx)}
        ${$i(t,e,this._features)}
        <div class="two-column-layout ${l?"":"single-column"}">
          ${l?o`
            <div class="left-column">
              ${this._renderRecommendationCard(t)}
              ${Ti(t,e,this._features)}
            </div>
          `:p}
          <div class="right-column">
            ${Ai(t,e,this._costDurationToggle,h=>{this._costDurationToggle=h})}
          </div>
        </div>
        ${c?this._collapsible("weibull","weibull_reliability_curve",wi(t,e)):p}
    ...
    `}_historyCtx(){return{lang:this._lang,filter:this._historyFilter,search:this._historySearch,currencySymbol:this._budget?.currency_symbol||_t,setFilter:t=>{this._historyFilter=t},setSearch:t=>{this._historySearch=t},openEdit:t=>this._openHistoryEdit(t)}}_renderHistoryTab(t){let e=this._historyCtx();return o`
    ...
    `}_renderKPIBar(t){let e=this._lang,i=t.times_performed>0?t.total_cost/t.times_performed:0,a=t.days_until_due!==null&&t.days_until_due!==void 0?t.days_until_due<0?"overdue":t.days_until_due<=t.warning_days?"warning":"":"";return o`
      <div class="kpi-bar">
        ...
        <div class="kpi-card">
          ...
          <div class="kpi-value">${Ie(t,e)}</div>
    ...
    `}_renderRecommendationCard(t){let e=this._lang;if(!this._features.adaptive||!t.suggested_interval||t.suggested_interval===t.interval_days)return p;if(this._selectedEntryId&&this._selectedTaskId&&this._dismissedSuggestions.has(`${this._selectedEntryId}_${this._selectedTaskId}`))return p;let i=t.suggested_interval;return o`
      <div class="recommendation-card">
        ...
        ${ki(t.interval_days,i,t.interval_confidence||"medium",e)}
      ...
      <div class="recent-activities">
        ...
        ${i.map(l=>o`
          <div class="activity-item">
            ...
            <span class="activity-date">${ee(l.timestamp,e)}</span>
            <span class="activity-note">${l.notes||"\u2014"}</span>
            ${l.cost?o`<span class="activity-badge">${l.cost.toFixed(0)}${this._budget?.currency_symbol||_t}</span>`:p}
