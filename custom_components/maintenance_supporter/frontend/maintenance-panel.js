...
`;var $e=class{constructor(a){this._cache=new Map;this._pending=new Map;this._hass=a}updateHass(a){this._hass=a}async getDetailStats(a,e){return this._getStats(a,"hour",30,e)}async getMiniStats(a,e){return this._getStats(a,"day",14,e)}async getBatchMiniStats(a){let e=new Map,t=[];for(let p of a){let u=`${p.entityId}:day`,m=this._cache.get(u);m&&Date.now()-m.fetchedAt<3e5?e.set(p.entityId,m.points):t.push(p)}if(t.length===0)return e;let i=t.filter(p=>p.isCounter).map(p=>p.entityId),r=t.filter(p=>!p.isCounter).map(p=>p.entityId),o=new Date(Date.now()-336*60*60*1e3).toISOString(),d=[];return i.length>0&&d.push(this._fetchBatch(i,"day",o,["state","sum","change"],!0,e)),r.length>0&&d.push(this._fetchBatch(r,"day",o,["mean","min","max"],!1,e)),await Promise.all(d),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(a,e,t,i){let r=`${a}:${e}`,o=this._cache.get(r);if(o&&Date.now()-o.fetchedAt<3e5)return o.points;if(this._pending.has(r))return this._pending.get(r);let d=this._fetchAndNormalize(a,e,t,i,r);this._pending.set(r,d);try{return await d}finally{this._pending.delete(r)}}async _fetchAndNormalize(a,e,t,i,r){let o=new Date(Date.now()-t*24*60*60*1e3).toISOString(),d=i?["state","sum","change"]:["mean","min","max"];try{let u=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:o,statistic_ids:[a],period:e,types:d}))[a]||[],m=this._normalizeRows(u,i);return this._cache.set(r,{entityId:a,fetchedAt:Date.now(),period:e,points:m}),m}catch(p){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${a}:`,p),[]}}async _fetchBatch(a,e,t,i,r,o){try{let d=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:a,period:e,types:i});for(let p of a){let u=d[p]||[],m=this._normalizeRows(u,r);o.set(p,m),this._cache.set(`${p}:${e}`,{entityId:p,fetchedAt:Date.now(),period:e,points:m})}}catch(d){console.warn("[maintenance-supporter] Batch statistics fetch failed:",d)}}_normalizeRows(a,e){let t=[];for(let i of a){let r=null;if(e?r=i.state??null:r=i.mean??null,r===null)continue;let o={ts:i.start,val:r};e||(i.min!=null&&(o.min=i.min),i.max!=null&&(o.max=i.max)),t.push(o)}return t.sort((i,r)=>i.ts-r.ts),t}};var ne=class{constructor(a){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=a}async getUsers(a=!1){let e=Date.now();if(!a&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(a,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:a,task_id:e,user_id:t})}async getTasksByUser(a){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:a})).tasks}getUserName(a){return!a||!this.usersCache?null:this.usersCache.find(t=>t.id===a)?.name||null}getUser(a){return!a||!this.usersCache?null:this.usersCache.find(e=>e.id===a)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(a){return a?a===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var U=class extends I{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._name="";this._manufacturer="";this._model="";this._entryId=null}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}openCreate(){this._entryId=null,this._name="",this._manufacturer="",this._model="",this._open=!0}openEdit(e,t){this._entryId=e,this._name=t.name||"",this._manufacturer=t.manufacturer||"",this._model=t.model||"",this._open=!0}async _save(){if(this._name.trim()){this._loading=!0;try{this._entryId?await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/update",entry_id:this._entryId,name:this._name,manufacturer:this._manufacturer||null,model:this._model||null}):await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/create",name:this._name,manufacturer:this._manufacturer||null,model:this._model||null}),this._open=!1,this.dispatchEvent(new CustomEvent("object-saved"))}finally{this._loading=!1}}}_close(){this._open=!1}render(){if(!this._open)return l``;let e=this._lang,t=this._entryId?s("edit_object",e):s("new_object",e);return l`
  ...
  `,g([S({attribute:!1})],U.prototype,"hass",2),g([_()],U.prototype,"_open",2),g([_()],U.prototype,"_loading",2),g([_()],U.prototype,"_name",2),g([_()],U.prototype,"_manufacturer",2),g([_()],U.prototype,"_model",2),g([_()],U.prototype,"_entryId",2),U=g([re("maintenance-object-dialog")],U);var Jt=["cleaning","inspection","replacement","calibration","service","custom"],Xt=["time_based","sensor_based","manual"],ei=["threshold","counter","state_change","runtime"],w=class extends I{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._entryId="";this._taskId=null;this._name="";this._type="custom";this._scheduleType="time_based";this._intervalDays="30";this._warningDays="7";this._notes="";this._documentationUrl="";this._triggerEntityId="";this._triggerEntityIds=[];this._triggerEntityLogic="any";this._triggerAttribute="";this._triggerType="threshold";this._triggerAbove="";this._triggerBelow="";this._triggerForMinutes="0";this._triggerTargetValue="";this._triggerDeltaMode=!1;this._triggerFromState="";this._triggerToState="";this._triggerTargetChanges="";this._triggerRuntimeHours="";this._responsibleUserId=null;this._availableUsers=[];this._userService=null}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}async openCreate(e){this._entryId=e,this._taskId=null,this._resetFields(),await this._loadUsers(),this._open=!0}async openEdit(e,t){if(this._entryId=e,this._taskId=t.id,this._name=t.name,this._type=t.type,this._scheduleType=t.schedule_type,this._intervalDays=t.interval_days?.toString()||"30",this._warningDays=t.warning_days.toString(),this._notes=t.notes||"",this._documentationUrl=t.documentation_url||"",this._responsibleUserId=t.responsible_user_id||null,t.trigger_config){let i=t.trigger_config;this._triggerEntityId=i.entity_id||"",this._triggerEntityIds=i.entity_ids||(i.entity_id?[i.entity_id]:[]),this._triggerEntityLogic=i.entity_logic||"any",this._triggerAttribute=i.attribute||"",this._triggerType=i.type||"threshold",this._triggerAbove=i.trigger_above?.toString()||"",this._triggerBelow=i.trigger_below?.toString()||"",this._triggerForMinutes=i.trigger_for_minutes?.toString()||"0",this._triggerTargetValue=i.trigger_target_value?.toString()||"",this._triggerDeltaMode=i.trigger_delta_mode||!1,this._triggerFromState=i.trigger_from_state||"",this._triggerToState=i.trigger_to_state||"",this._triggerTargetChanges=i.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=i.trigger_runtime_hours?.toString()||""}else this._resetTriggerFields();await this._loadUsers(),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._warningDays="7",this._notes="",this._documentationUrl="",this._responsibleUserId=null,this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerForMinutes="0",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours=""}async _loadUsers(){this._userService||(this._userService=new ne(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}async _save(){if(this._name.trim()){this._loading=!0;try{let e={type:this._taskId?"maintenance_supporter/task/update":"maintenance_supporter/task/create",entry_id:this._entryId,name:this._name,task_type:this._type,schedule_type:this._scheduleType,warning_days:parseInt(this._warningDays,10)||7};if(this._taskId&&(e.task_id=this._taskId),this._scheduleType!=="manual"&&this._intervalDays&&(e.interval_days=parseInt(this._intervalDays,10)),this._notes&&(e.notes=this._notes),this._documentationUrl&&(e.documentation_url=this._documentationUrl),this._responsibleUserId&&(e.responsible_user_id=this._responsibleUserId),this._scheduleType==="sensor_based"&&this._triggerEntityId){let t=this._triggerEntityIds.length>0?this._triggerEntityIds:[this._triggerEntityId],i={entity_id:t[0],entity_ids:t,type:this._triggerType};this._triggerAttribute&&(i.attribute=this._triggerAttribute),t.length>1&&(i.entity_logic=this._triggerEntityLogic),this._triggerType==="threshold"?(this._triggerAbove&&(i.trigger_above=parseFloat(this._triggerAbove)),this._triggerBelow&&(i.trigger_below=parseFloat(this._triggerBelow)),this._triggerForMinutes&&(i.trigger_for_minutes=parseInt(this._triggerForMinutes,10))):this._triggerType==="counter"?(this._triggerTargetValue&&(i.trigger_target_value=parseFloat(this._triggerTargetValue)),i.trigger_delta_mode=this._triggerDeltaMode):this._triggerType==="state_change"?(this._triggerFromState&&(i.trigger_from_state=this._triggerFromState),this._triggerToState&&(i.trigger_to_state=this._triggerToState),this._triggerTargetChanges&&(i.trigger_target_changes=parseInt(this._triggerTargetChanges,10))):this._triggerType==="runtime"&&this._triggerRuntimeHours&&(i.trigger_runtime_hours=parseFloat(this._triggerRuntimeHours)),e.trigger_config=i}await this.hass.connection.sendMessagePromise(e),this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return c;let e=this._lang;return l`
      ...
      <div class="select-row">
        ...
        >
          ${ei.map(t=>l`<option value=${t}>${s(t,e)}</option>`)}
        </select>
      </div>
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `,g([S({attribute:!1})],B.prototype,"hass",2),g([_()],B.prototype,"_open",2),g([_()],B.prototype,"_title",2),g([_()],B.prototype,"_message",2),g([_()],B.prototype,"_confirmText",2),g([_()],B.prototype,"_danger",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",B);var ti=300,ii=140,Re=27;function pt(n,a){let e=n.trigger_config;if(!e)return c;let t=a.lang,i=n.trigger_entity_info,r=n.trigger_entity_infos,o=i?.friendly_name||e.entity_id||"\u2014",d=e.entity_id||"",p=e.entity_ids||(d?[d]:[]),u=i?.unit_of_measurement||"",m=n.trigger_current_value,v=e.type||"threshold",b=p.length>1;return l`
    <h3>${s("trigger",t)}</h3>
    <div class="trigger-card">
      ...
      <div class="trigger-limits">
        ${v==="threshold"?l`
          ${e.trigger_above!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("threshold_above",t)}: ${e.trigger_above} ${u}</span>`:c}
          ${e.trigger_below!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("threshold_below",t)}: ${e.trigger_below} ${u}</span>`:c}
        ...
        `:c}
        ${v==="counter"?l`
          ${e.trigger_target_value!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("target_value",t)}: ${e.trigger_target_value} ${u}</span>`:c}
        `:c}
        ${v==="state_change"?l`
        ...
        ${v==="compound"?l`
          ...
          `)}
        `:c}
        ${i?.min!=null?l`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${s("min",t)}: ${i.min} ${u}</span>`:c}
        ${i?.max!=null?l`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${s("max",t)}: ${i.max} ${u}</span>`:c}
      </div>
  ...
  `}function ai(n,a,e){let t=n.trigger_config;if(!t)return c;let i=t.type||"threshold",r=i==="counter"&&t.trigger_delta_mode,o=e.isCounterEntity(t),d=t.entity_id||"",p=e.detailStatsData.get(d)||[],u=[],m=!1;if(p.length>=2)for(let h of p){let k=h.val;r&&n.trigger_baseline_value!=null&&(k-=n.trigger_baseline_value);let F={ts:h.ts,val:k};!o&&h.min!=null&&h.max!=null&&(F.min=r&&n.trigger_baseline_value!=null?h.min-n.trigger_baseline_value:h.min,F.max=r&&n.trigger_baseline_value!=null?h.max-n.trigger_baseline_value:h.max,m=!0),u.push(F)}else for(let h of n.history)h.trigger_value!=null&&u.push({ts:new Date(h.timestamp).getTime(),val:h.trigger_value});if(n.trigger_current_value!=null){let h=n.trigger_current_value;r&&n.trigger_baseline_value!=null&&(h-=n.trigger_baseline_value),u.push({ts:Date.now(),val:h})}if(u.length<2&&d&&e.hasStatsService&&!e.detailStatsData.has(d))return l`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
      <ha-icon icon="mdi:chart-line" style="--mdc-icon-size:16px;margin-right:8px;"></ha-icon>
      ${s("loading_chart",e.lang)}
    </div>`;if(u.length<2)return c;u.sort((h,k)=>h.ts-k.ts);let v=ti,b=ii,f=30,$=2,T=8,R=16,C=u.map(h=>h.val),E=Math.min(...C),y=Math.max(...C);if(m)for(let h of u)h.min!=null&&(E=Math.min(E,h.min)),h.max!=null&&(y=Math.max(y,h.max));t.trigger_above!=null&&(y=Math.max(y,t.trigger_above),E=Math.min(E,t.trigger_above)),t.trigger_below!=null&&(E=Math.min(E,t.trigger_below),y=Math.max(y,t.trigger_below));let L=null,O=null;if(i==="counter"&&t.trigger_target_value!=null){if(n.trigger_baseline_value!=null)L=n.trigger_baseline_value;else if(u.length>0){let h=[...n.history].filter(k=>k.type==="completed"||k.type==="reset").sort((k,F)=>new Date(F.timestamp).getTime()-new Date(k.timestamp).getTime())[0];if(h){let k=new Date(h.timestamp).getTime(),F=u[0],H=Math.abs(u[0].ts-k);for(let K of u){let Z=Math.abs(K.ts-k);Z<H&&(F=K,H=Z)}L=F.val}else L=u[0].val}L!=null?(O=L+t.trigger_target_value,y=Math.max(y,O),E=Math.min(E,L)):(y=Math.max(y,t.trigger_target_value),E=Math.min(E,0))}r&&n.trigger_baseline_value!=null&&(E=Math.min(E,0));let W=y-E||1;E-=W*.1,y+=W*.1;let V=u[0].ts,z=u[u.length-1].ts,x=z-V||1,D=h=>f+(h-V)/x*(v-f-$),M=h=>T+(1-(h-E)/(y-E))*(b-T-R),we=u.map(h=>`${D(h.ts).toFixed(1)},${M(h.val).toFixed(1)}`).join(" "),bt=`M${D(u[0].ts).toFixed(1)},${b-R} `+u.map(h=>`L${D(h.ts).toFixed(1)},${M(h.val).toFixed(1)}`).join(" ")+` L${D(u[u.length-1].ts).toFixed(1)},${b-R} Z`,ke="";if(m){let h=u.filter(k=>k.min!=null&&k.max!=null);if(h.length>=2){let k=h.map(H=>`${D(H.ts).toFixed(1)},${M(H.max).toFixed(1)}`),F=[...h].reverse().map(H=>`${D(H.ts).toFixed(1)},${M(H.min).toFixed(1)}`);ke=`M${k[0]} `+k.slice(1).map(H=>`L${H}`).join(" ")+` L${F[0]} `+F.slice(1).map(H=>`L${H}`).join(" ")+" Z"}}let Ne=u[u.length-1],yt=D(Ne.ts),xt=M(Ne.val),Oe=h=>Math.abs(h)>=1e4?(h/1e3).toFixed(0)+"k":h>=1e3?(h/1e3).toFixed(1)+"k":h.toFixed(h<10?1:0),$t=Oe(y),wt=Oe(E),kt=n.history.filter(h=>["completed","skipped","reset"].includes(h.type)).map(h=>({ts:new Date(h.timestamp).getTime(),type:h.type})).filter(h=>h.ts>=V&&h.ts<=z),Ee=u;if(u.length>Re){let h=(u.length-1)/(Re-1);Ee=[];for(let k=0;k<Re;k++)Ee.push(u[Math.round(k*h)])}return l`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${v} ${b}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_sparkline",e.lang)}">
        ...
        ${n.degradation_rate!=null&&n.degradation_trend!=="stable"&&n.degradation_trend!=="insufficient_data"&&u.length>=2?(()=>{let h=u[u.length-1],k=30,F=h.ts+k*864e5,H=h.val+n.degradation_rate*k,K=Math.min(F,z+(z-V)*.3),Z=Math.max(E,Math.min(y,H)),Et=D(h.ts),At=M(h.val),St=D(K),Tt=M(Z);return j`<line x1="${Et.toFixed(1)}" y1="${At.toFixed(1)}" x2="${St.toFixed(1)}" y2="${Tt.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():c}
        ...
        ${kt.map(h=>{let k=D(h.ts),F=h.type==="completed"?"var(--success-color, #4caf50)":h.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return j`
            <line x1="${k.toFixed(1)}" y1="${T}" x2="${k.toFixed(1)}" y2="${b-R}" stroke="${F}" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
          ...
          `})}
        ${Ee.map(h=>{let k=D(h.ts),F=M(h.val),H=new Date(h.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),K=`${h.val.toFixed(1)} ${a}`;return m&&h.min!=null&&h.max!=null&&(K+=` (${h.min.toFixed(1)}\u2013${h.max.toFixed(1)})`),j`<circle cx="${k.toFixed(1)}" cy="${F.toFixed(1)}" r="8" fill="transparent" tabindex="0"
            @mouseenter=${Z=>dt(Z,`${H}
${K}`,e.setTooltip)}
            @focus=${Z=>dt(Z,`${H}
${K}`,e.setTooltip)}
...
`).map(h=>l`<div>${h}</div>`)}
  ...
  `}function gt(n,a){let e=n.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return c;let r=n.interval_days??0,o=n.suggested_interval??r;return l`
  ...
  `}function oi(n,a,e){let t=n.confidence_interval_low,i=n.confidence_interval_high,r=a.suggested_interval??a.interval_days??0,o=a.interval_days??0,d=Math.max(0,t-5),u=i+5-d,m=(t-d)/u*100,v=(i-t)/u*100,b=(r-d)/u*100,f=o>0?(o-d)/u*100:-1;return l`
  ...
  `}function mt(n,a){return li(n,a)}function li(n,a){let e=n.seasonal_factors??n.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return c;let t=n.interval_analysis?.seasonal_reason,i=new Date().getMonth(),r=300,o=100,d=8,u=o-d-4,m=Math.max(...e,1.5),v=r/12,b=v*.65,f=d+u-1/m*u;return l`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${r} ${o}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",a)}">
        ...
        ${e.map(($,T)=>{let R=$/m*u,C=T*v+(v-b)/2,E=d+u-R,y=T===i,L=$<1?"var(--success-color, #4caf50)":$>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return j`
      ...
      <div class="seasonal-labels">
        ${ht.map(($,T)=>l`<span class="seasonal-label ${T===i?"active-month":""}">${s($,a)}</span>`)}
      </div>
    </div>
  `}var ci=300,di=200;function vt(n,a,e,t){let i=n.history.filter(d=>d.type==="completed"&&(d.cost!=null||d.duration!=null));if(i.length<2)return c;let r=i.some(d=>(d.cost??0)>0),o=i.some(d=>(d.duration??0)>0);return!r&&!o?c:l`
  ...
  `}var ui=60,gi=20,ft=30,A=class extends I{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._sparklineTooltip=null;this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1};this._actionLoading=!1;this._moreMenuOpen=!1;this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._deepLinkHandled=!1;this._onDialogEvent=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"de"}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new $e(this.hass),this._fetchMiniStatsForOverview()),this._userService||(this._userService=new ne(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,i,r,o]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);e&&(this._objects=e.objects),t&&(this._stats=t),i&&(this._budget=i),r&&(this._groups=r.groups||{}),o&&(this._features=o.features),this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("entry_id");if(!t)return;this._deepLinkHandled=!0;let i=e.get("task_id"),r=e.get("action");i?(this._showTask(t,i),r==="complete"&&(history.replaceState(null,"",window.location.pathname+window.location.hash),requestAnimationFrame(()=>{let d=this._getObject(t)?.tasks.find(p=>p.id===i);d&&this._openCompleteDialog(t,i,d.name,this._features.checklists?d.checklist:void 0,this._features.adaptive&&!!d.adaptive_config?.enabled)}))):this._showObject(t)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let i=await this._statsService.getDetailStats(e,t),r=new Map(this._detailStatsData);r.set(e,i),this._detailStatsData=r}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let i of this._objects)for(let r of i.tasks){let o=r.trigger_config?.entity_id;o&&e.push({entityId:o,isCounter:this._isCounterEntity(r.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let i of this._objects)for(let r of i.tasks)if(!(this._filterStatus&&r.status!==this._filterStatus)){if(this._filterUser){let o=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(r.responsible_user_id!==o)continue}e.push({entry_id:i.entry_id,task_id:r.id,object_name:i.object.name,task_name:r.name,type:r.type,schedule_type:r.schedule_type,status:r.status,days_until_due:r.days_until_due??null,next_due:r.next_due??null,trigger_active:r.trigger_active,trigger_current_value:r.trigger_current_value??null,trigger_current_delta:r.trigger_current_delta??null,trigger_config:r.trigger_config??null,trigger_entity_info:r.trigger_entity_info??null,times_performed:r.times_performed,total_cost:r.total_cost,interval_days:r.interval_days??null,history:r.history||[],enabled:r.enabled})}let t={overdue:0,triggered:1,due_soon:2,ok:3};return e.sort((i,r)=>(t[i.status]??9)-(t[r.status]??9)),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(r=>r.id===t)}_pushPanelState(e,t,i){let r={msp_view:e,msp_entry:t||null,msp_task:i||null};history.pushState(r,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let i=this._getTask(t.msp_entry,t.msp_task);i?.trigger_config?.entity_id&&this._fetchDetailStats(i.trigger_config.entity_id,this._isCounterEntity(i.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null;let i=this._getTask(e,t);if(i?.trigger_config?.entity_id){let r=i.trigger_config.entity_id,o=this._isCounterEntity(i.trigger_config);this._fetchDetailStats(r,o)}}async _deleteObject(e){await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("delete",this._lang),message:s("confirm_delete_object",this._lang),confirmText:s("delete",this._lang),danger:!0})&&(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData())}async _deleteTask(e,t){await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("delete",this._lang),message:s("confirm_delete_task",this._lang),confirmText:s("delete",this._lang),danger:!0})&&(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData())}async _skipTask(e,t){this._actionLoading=!0;try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/skip",entry_id:e,task_id:t}),await this._loadData()}finally{this._actionLoading=!1}}async _resetTask(e,t){this._actionLoading=!0;try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/reset",entry_id:e,task_id:t}),await this._loadData()}finally{this._actionLoading=!1}}async _applySuggestion(e,t,i){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:i}),await this._loadData()}_dismissSuggestion(){this._loadData()}_openCompleteDialog(e,t,i,r,o){let d=this.shadowRoot.querySelector("maintenance-complete-dialog");d&&(d.entryId=e,d.taskId=t,d.taskName=i,d.lang=this._lang,d.checklist=r||[],d.adaptiveEnabled=!!o,d.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,i,r){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,i,r)}async _exportCsv(){let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/csv/export"}),t=new Blob([e.csv],{type:"text/csv;charset=utf-8;"}),i=URL.createObjectURL(t),r=document.createElement("a");r.href=i,r.download="maintenance_export.csv",r.click(),URL.revokeObjectURL(i)}_triggerImportCsv(){let e=this.shadowRoot.querySelector("input[type=file]");e&&(e.value="",e.click())}async _handleCsvFile(e){let i=e.target.files?.[0];if(!i)return;let r=await i.text(),o=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/csv/import",csv_content:r});await this._loadData(),await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:"Import",message:s("import_result",this._lang).replace("{created}",String(o.created)).replace("{total}",String(o.total)),confirmText:"OK"})}render(){return l`
    ...
    `}_renderOverviewRow(e){let t=this._lang,i=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,r=0,o=ye.ok,d=!1;if(i&&e.days_until_due!==null){let p=(e.interval_days-e.days_until_due)/e.interval_days*100;r=Math.max(0,Math.min(100,p)),d=p>100,e.status==="overdue"?o=ye.overdue:e.status==="due_soon"&&(o=ye.due_soon)}return l`
      <div class="task-row${e.enabled?"":" task-disabled"}">
        ...
        <span class="row-actions">
          ...
          <mwc-icon-button class="btn-skip" title="${s("skip",t)}" .disabled=${this._actionLoading} @click=${p=>{p.stopPropagation(),this._skipTask(e.entry_id,e.task_id)}}>
      ...
      <div class="tab-content overview-tab">
        ...
        <div class="two-column-layout ${o?"":"single-column"}">
          ...
          <div class="right-column">
            ${vt(e,t,this._costDurationToggle,u=>{this._costDurationToggle=u})}
          </div>
        </div>
        ${d?gt(e,t):c}
        ${p?mt(e,t):c}
