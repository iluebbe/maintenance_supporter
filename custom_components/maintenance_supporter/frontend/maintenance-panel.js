...
`;var it=class{constructor(r){this._cache=new Map;this._pending=new Map;this._hass=r}updateHass(r){this._hass=r}async getDetailStats(r,e,t=30){return this._getStats(r,t<=35?"hour":"day",t,e)}async getMiniStats(r,e){return this._getStats(r,"day",14,e)}async getBatchMiniStats(r){let e=new Map,t=[];for(let d of r){let g=`${d.entityId}:day:14`,_=this._cache.get(g);_&&Date.now()-_.fetchedAt<3e5?e.set(d.entityId,_.points):t.push(d)}if(t.length===0)return e;let i=t.filter(d=>d.isCounter).map(d=>d.entityId),a=t.filter(d=>!d.isCounter).map(d=>d.entityId),l=new Date(Date.now()-336*60*60*1e3).toISOString(),c=[];return i.length>0&&c.push(this._fetchBatch(i,"day",l,["state","sum","change"],!0,e)),a.length>0&&c.push(this._fetchBatch(a,"day",l,["mean","min","max"],!1,e)),await Promise.all(c),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(r,e,t,i){let a=`${r}:${e}:${t}`,l=this._cache.get(a);if(l&&Date.now()-l.fetchedAt<3e5)return l.points;if(this._pending.has(a))return this._pending.get(a);let c=this._fetchAndNormalize(r,e,t,i,a);this._pending.set(a,c);try{return await c}finally{this._pending.delete(a)}}async _fetchAndNormalize(r,e,t,i,a){let l=new Date(Date.now()-t*24*60*60*1e3).toISOString(),c=i?["state","sum","change"]:["mean","min","max"];try{let g=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:l,statistic_ids:[r],period:e,types:c}))[r]||[],_=this._normalizeRows(g,i);return this._cache.set(a,{entityId:r,fetchedAt:Date.now(),period:e,points:_}),_}catch(d){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${r}:`,d),[]}}async _fetchBatch(r,e,t,i,a,l){try{let c=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:r,period:e,types:i});for(let d of r){let g=c[d]||[],_=this._normalizeRows(g,a);l.set(d,_),this._cache.set(`${d}:${e}:14`,{entityId:d,fetchedAt:Date.now(),period:e,points:_})}}catch(c){console.warn("[maintenance-supporter] Batch statistics fetch failed:",c)}}_normalizeRows(r,e){let t=[];for(let i of r){let a=null;if(e?a=i.state??null:a=i.mean??null,a===null)continue;let l={ts:i.start,val:a};e||(i.min!=null&&(l.min=i.min),i.max!=null&&(l.max=i.max)),t.push(l)}return t.sort((i,a)=>i.ts-a.ts),t}};var ue=class{constructor(r){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=r}updateHass(r){this.hass=r}async getUsers(r=!1){let e=Date.now();if(!r&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(r,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:r,task_id:e,user_id:t})}async getTasksByUser(r){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:r})).tasks}getUserName(r){return!r||!this.usersCache?null:this.usersCache.find(t=>t.id===r)?.name||null}getUser(r){return!r||!this.usersCache?null:this.usersCache.find(e=>e.id===r)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(r){return r?r===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var Vi={name:"name",task_type:"maintenance_type",schedule_type:"schedule_type",interval_days:"interval_days",interval_anchor:"interval_anchor",warning_days:"warning_days",last_performed:"last_performed_optional",notes:"notes_optional",documentation_url:"documentation_url_optional",custom_icon:"custom_icon_optional",nfc_tag_id:"nfc_tag_id_optional",responsible_user_id:"responsible_user",entity_slug:"entity_slug",entity_id:"entity_id",area_id:"area_id_optional",manufacturer:"manufacturer_optional",model:"model_optional",serial_number:"serial_number_optional",installation_date:"installation_date_optional",warranty_expiry:"warranty_expiry_optional",checklist:"checklist_steps_optional",reason:"reason",feedback:"feedback",cost:"cost",duration:"duration",description:"description_optional",group_name:"name",group_description:"description_optional",environmental_entity:"environmental_entity_optional",environmental_attribute:"environmental_attribute_optional",trigger_above:"trigger_above",trigger_below:"trigger_below",trigger_for_minutes:"trigger_for_minutes"};function Ki(o,r){let e=Vi[o];if(!e)return o;let t=s(e,r);return t&&t!==e?t:o}function Yi(o){let e=o.match(/data\['([^']+)'\]/)?.[1],t;return(t=o.match(/length of value must be at most (\d+)/))?{field:e,rule:"too_long",param:t[1]}:(t=o.match(/length of value must be at least (\d+)/))?{field:e,rule:"too_short",param:t[1]}:(t=o.match(/value must be at most (\S+)/))?{field:e,rule:"value_too_high",param:t[1]}:(t=o.match(/value must be at least (\S+)/))?{field:e,rule:"value_too_low",param:t[1]}:/required key not provided/.test(o)?{field:e,rule:"required"}:(t=o.match(/expected (\w+)/))?{field:e,rule:"wrong_type",param:t[1]}:/value must be one of/.test(o)?{field:e,rule:"invalid_choice"}:/not a valid value/.test(o)?{field:e,rule:"invalid_value"}:{field:e,rule:"unknown"}}function M(o,r,e){if(e=e??s("action_error",r),typeof o=="string")return o;if(typeof o!="object"||o===null)return e;let t=o,i=t.message||t.error?.message||"";if(!i)return e;let a=Yi(i),l=a.field?Ki(a.field,r):"",c=d=>s(d,r).replace("{field}",l).replace("{n}",a.param??"");switch(a.rule){case"too_long":return c("err_too_long");case"too_short":return c("err_too_short");case"value_too_high":return c("err_value_too_high");case"value_too_low":return c("err_value_too_low");case"required":return c("err_required");case"wrong_type":return c("err_wrong_type").replace("{type}",a.param??"");case"invalid_choice":return c("err_invalid_choice");case"invalid_value":return c("err_invalid_value");default:return i||e}}var B=class extends T{constructor(){super(...arguments);this.label="";this.value="";this.placeholder="";this.type="text";this.required=!1;this.disabled=!1}_onInput(e){let t=e.target.value;this.value=t,this.dispatchEvent(new CustomEvent("input",{bubbles:!0,composed:!0,detail:{value:t}}))}render(){return n`
      <label class="field">
        ...
        />
        ${this.helper?n`<span class="helper">${this.helper}</span>`:p}
      </label>
    `}};B.styles=A`
    :host { display: block; }
    .field {
  ...
  `,h([y({attribute:!1})],O.prototype,"hass",2),h([u()],O.prototype,"_open",2),h([u()],O.prototype,"_loading",2),h([u()],O.prototype,"_error",2),h([u()],O.prototype,"_name",2),h([u()],O.prototype,"_manufacturer",2),h([u()],O.prototype,"_model",2),h([u()],O.prototype,"_serialNumber",2),h([u()],O.prototype,"_areaId",2),h([u()],O.prototype,"_installationDate",2),h([u()],O.prototype,"_warrantyExpiry",2),h([u()],O.prototype,"_documentationUrl",2),h([u()],O.prototype,"_notes",2),h([u()],O.prototype,"_entryId",2);customElements.get("maintenance-object-dialog")||customElements.define("maintenance-object-dialog",O);function ae(o){let r=o??0;return r<1024?`${r} B`:r<1024*1024?`${(r/1024).toFixed(1)} KB`:`${(r/(1024*1024)).toFixed(1)} MB`}var st=["manual","warranty","invoice","spare_parts","photo","other"],Gi={manual:"mdi:book-open-variant",warranty:"mdi:shield-check",invoice:"mdi:receipt-text-outline",spare_parts:"mdi:cog-outline",photo:"mdi:image-outline",other:"mdi:file-document-outline"},H=class extends T{constructor(){super(...arguments);this.canWrite=!1;this._docs=[];this._loaded=!1;this._busy=!1;this._error="";this._hint="";this._addingLink=!1;this._linkUrl="";this._linkTitle="";this._category="manual";this._thumbs={};this._lightboxUrl="";this._editingId="";this._editTitle="";this._editCategory="manual";this._dragOver=!1;this._loadedFor=null;this._localeReady=!1}_isImage(e){return e.kind==="file"&&(e.mime||"").startsWith("image/")}async _sign(e){return(await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300})).path}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),this.hass&&!this._localeReady&&(this._localeReady=!0,se(this._lang).then(()=>this.requestUpdate())),this.hass&&this.entryId&&this._loadedFor!==this.entryId&&(this._loadedFor=this.entryId,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:this.entryId});this._docs=e.documents||[],this._loaded=!0,this._error="",this._thumbs={},this._loadThumbs()}catch(e){this._error=M(e,this._lang),this._loaded=!0}}async _loadThumbs(){await Promise.all(this._docs.filter(e=>this._isImage(e)).map(async e=>{try{let t=await this._sign(e);this._thumbs={...this._thumbs,[e.id]:t}}catch{}}))}_category_of(e){return(e.tags||[]).find(i=>st.includes(i))||"other"}_labelKeydown(e){(e.key==="Enter"||e.key===" ")&&(e.preventDefault(),e.currentTarget.querySelector("input")?.click())}_onFileInput(e){let t=e.target,i=Array.from(t.files??[]);i.length&&this._uploadFiles(i),t.value=""}_onCameraInput(e){let t=e.target,i=Array.from(t.files??[]);i.length&&this._uploadFiles(i,"photo"),t.value=""}_onDrop(e){if(e.preventDefault(),this._dragOver=!1,!this.canWrite||this._busy)return;let t=Array.from(e.dataTransfer?.files??[]);t.length&&this._uploadFiles(t)}_onDragOver(e){this.canWrite&&(e.preventDefault(),this._dragOver=!0)}_onDragLeave(e){let t=e.relatedTarget;(!t||!e.currentTarget.contains(t))&&(this._dragOver=!1)}async _uploadFiles(e,t){let i=t??this._category;this._busy=!0,this._error="",this._hint="";let a=0,l=0;try{for(let c of e){let d=new FormData;d.append("entry_id",this.entryId),d.append("tags",i),d.append("file",c,c.name);let g=await fetch("/api/maintenance_supporter/document/upload",{method:"POST",headers:{Authorization:`Bearer ${this.hass.auth?.data?.access_token??""}`},body:d});if(!g.ok){this._error=g.status===413?s("doc_too_large",this._lang):s("doc_upload_failed",this._lang);continue}let _=await g.json();_.duplicate_in_object?l++:_.deduped&&a++}l?this._hint=s("doc_dup_in_object",this._lang):a&&(this._hint=s("doc_deduped",this._lang)),await this._load()}catch{this._error=s("doc_upload_failed",this._lang)}finally{this._busy=!1}}async _download(e){try{let t=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:30});tt(t.path,e.filename||e.title||"document")}catch(t){this._error=M(t,this._lang)}}async _preview(e){if(this._isImage(e)){this._lightboxUrl=this._thumbs[e.id]||await this._sign(e);return}let t=window.open("about:blank","_blank");try{let i=await this._sign(e);t&&(t.location.href=new URL(i,window.location.origin).href)}catch(i){t&&t.close(),this._error=M(i,this._lang)}}_openDoc(e){e.kind==="file"?this._preview(e):e.url&&window.open(e.url,"_blank","noopener")}_startEdit(e){this._editingId=e.id,this._editTitle=e.title||"",this._editCategory=this._category_of(e),this._addingLink=!1,this._error=""}_cancelEdit(){this._editingId=""}async _saveEdit(e){let t=(e.tags||[]).filter(a=>!st.includes(a)),i=e.kind==="file"?[this._editCategory,...t]:e.tags??[];this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,title:this._editTitle.trim()||e.filename||e.url||"",tags:i}),this._editingId="",await this._load()}catch(a){this._error=M(a,this._lang)}finally{this._busy=!1}}async _delete(e){let t=e.title||e.filename||e.url||"";if(window.confirm(s("doc_delete_confirm",this._lang).replace("{name}",t))){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/delete",doc_id:e.id}),await this._load()}catch(i){this._error=M(i,this._lang)}finally{this._busy=!1}}}async _addLink(){let e=this._linkUrl.trim();if(e){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/add_link",entry_id:this.entryId,url:e,title:this._linkTitle.trim()||null}),this._linkUrl="",this._linkTitle="",this._addingLink=!1,await this._load()}catch(t){this._error=M(t,this._lang,s("doc_link_invalid",this._lang))}finally{this._busy=!1}}}render(){let e=this._lang;return n`
      ...
      ${this._error?n`<div class="doc-msg error">${this._error}</div>`:p}
      ${this._hint?n`<div class="doc-msg hint">${this._hint}</div>`:p}
  ...
  `,h([y({attribute:!1})],H.prototype,"hass",2),h([y({attribute:!1})],H.prototype,"entryId",2),h([y({type:Boolean})],H.prototype,"canWrite",2),h([u()],H.prototype,"_docs",2),h([u()],H.prototype,"_loaded",2),h([u()],H.prototype,"_busy",2),h([u()],H.prototype,"_error",2),h([u()],H.prototype,"_hint",2),h([u()],H.prototype,"_addingLink",2),h([u()],H.prototype,"_linkUrl",2),h([u()],H.prototype,"_linkTitle",2),h([u()],H.prototype,"_category",2),h([u()],H.prototype,"_thumbs",2),h([u()],H.prototype,"_lightboxUrl",2),h([u()],H.prototype,"_editingId",2),h([u()],H.prototype,"_editTitle",2),h([u()],H.prototype,"_editCategory",2),h([u()],H.prototype,"_dragOver",2);customElements.get("maintenance-documents-section")||customElements.define("maintenance-documents-section",H);var Qi=["manual","warranty","invoice","spare_parts","photo","other"],Zi={manual:"mdi:book-open-variant",warranty:"mdi:shield-check",invoice:"mdi:receipt-text-outline",spare_parts:"mdi:cog-outline",photo:"mdi:image-outline",other:"mdi:file-document-outline"},K=class extends T{constructor(){super(...arguments);this.canWrite=!1;this._docs=[];this._loaded=!1;this._busy=!1;this._error="";this._attachId="";this._loadedKey="";this._localeReady=!1}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),this.hass&&!this._localeReady&&(this._localeReady=!0,se(this._lang).then(()=>this.requestUpdate()));let t=`${this.entryId}|${this.taskId}`;this.hass&&this.entryId&&this.taskId&&this._loadedKey!==t&&(this._loadedKey=t,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:this.entryId});this._docs=e.documents||[],this._loaded=!0,this._error=""}catch(e){this._error=M(e,this._lang),this._loaded=!0}}_linked(){return this._docs.filter(e=>(e.task_ids||[]).includes(this.taskId))}_available(){return this._docs.filter(e=>!(e.task_ids||[]).includes(this.taskId))}async _setTaskIds(e,t){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,task_ids:t}),await this._load()}catch(i){this._error=M(i,this._lang)}finally{this._busy=!1}}_link(){let e=this._docs.find(t=>t.id===this._attachId);e&&(this._attachId="",this._setTaskIds(e,[...e.task_ids||[],this.taskId]))}_unlink(e){this._setTaskIds(e,(e.task_ids||[]).filter(t=>t!==this.taskId))}_isPdf(e){return e.mime==="application/pdf"||(e.filename||"").toLowerCase().endsWith(".pdf")}_pageFor(e){return this._isPdf(e)?e.task_pages?.[this.taskId]:void 0}async _open(e){if(e.kind==="weblink"){window.open(e.url,"_blank","noopener");return}let t=this._pageFor(e),i=t?`#page=${t}`:"",a=window.open("about:blank","_blank");try{let l=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300});a&&(a.location.href=new URL(l.path+i,window.location.origin).href)}catch(l){a&&a.close(),this._error=M(l,this._lang)}}async _setPage(e,t){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,task_pages:{[this.taskId]:t}}),await this._load()}catch(i){this._error=M(i,this._lang)}finally{this._busy=!1}}async _download(e){try{let t=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:30});tt(t.path,e.filename||e.title||"document")}catch(t){this._error=M(t,this._lang)}}render(){if(!this._loaded||this._docs.length===0)return p;let e=this._lang,t=this._linked(),i=this._available();return n`
    ...
    `}_renderRow(e,t){let i=e.kind==="file",a=this._isPdf(e),l=e.task_pages?.[this.taskId],c=(e.tags||[]).find(g=>Qi.includes(g))||"other",d=i?ae(e.size):s("doc_link_badge",t);return n`
      <div class="tdoc-row">
        ...
        >
          ...
          <div class="tdoc-meta">
            ${d}${l?n` · <span class="tdoc-pagetag">${s("doc_page",t)} ${l}</span>`:p}
          </div>
        </div>
...
`),this._scheduleTime=t.schedule_time||"";let a=t.on_complete_action;if(a&&a.service){this._actionService=a.service;let d=a.target?.entity_id;this._actionTargetEntity=Array.isArray(d)?d[0]||"":d||"",this._actionData=a.data&&typeof a.data=="object"?{...a.data}:{},this._actionDataJsonFallback=""}else this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="";let l=t.quick_complete_defaults;this._qcNotes=l?.notes||"",this._qcCost=l?.cost!=null?String(l.cost):"",this._qcDuration=l?.duration!=null?String(l.duration):"",this._qcFeedback=l?.feedback||"";let c=t.adaptive_config||{};if(this._environmentalEntity=c.environmental_entity||"",this._environmentalAttribute=c.environmental_attribute||"",this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute,t.trigger_config){let d=t.trigger_config;this._triggerEntityId=d.entity_id||"",this._triggerEntityIds=d.entity_ids||(d.entity_id?[d.entity_id]:[]),this._triggerEntityLogic=d.entity_logic||"any",this._triggerAttribute=d.attribute||"",this._triggerType=d.type||"threshold",this._triggerAbove=d.trigger_above?.toString()||"",this._triggerBelow=d.trigger_below?.toString()||"",this._triggerForMinutes=d.trigger_for_minutes?.toString()||"0",this._triggerTargetValue=d.trigger_target_value?.toString()||"",this._triggerDeltaMode=d.trigger_delta_mode||!1,this._triggerFromState=d.trigger_from_state||"",this._triggerToState=d.trigger_to_state||"",this._triggerTargetChanges=d.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=d.trigger_runtime_hours?.toString()||""}else this._resetTriggerFields();this._triggerEntityId&&this._fetchEntityAttributes(this._triggerEntityId),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._intervalUnit="days",this._dueDate="",this._warningDays=String(this.defaultWarningDays),this._intervalAnchor="completion",this._weekdays=[],this._nth="1",this._nthWeekday="5",this._domDay="1",this._notes="",this._documentationUrl="",this._customIcon="",this._enabled=!0,this._lastPerformed="",this._nfcTagId="",this._responsibleUserId=null,this._checklistText="",this._scheduleTime="",this._environmentalEntity="",this._environmentalAttribute="",this._environmentalInitial="",this._environmentalAttributeInitial="",this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="",this._actionTesting=!1,this._actionTestResult="",this._qcNotes="",this._qcCost="",this._qcDuration="",this._qcFeedback="",this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerForMinutes="0",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours=""}async _loadUsers(){this._userService||(this._userService=new ue(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}async _testAction(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e)){this._actionTestResult="error",this._actionTestError="Invalid service format (expected 'domain.service')",setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3);return}let[t,i]=e.split(".");if(!this.hass?.services?.[t]?.[i]){this._actionTestResult="error",this._actionTestError=`Service "${e}" is not registered in Home Assistant. Check spelling and that the integration providing it is loaded.`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}let a=this._actionTargetEntity.trim();if(a){let l=a.split(".")[0];if(l!==t&&!new Set(["homeassistant","scene","notify","persistent_notification"]).has(t)){this._actionTestResult="error",this._actionTestError=`Service "${e}" only works on ${t}.* entities; entity "${a}" is in ${l}.* \u2014 pick a service that matches the entity domain (e.g. ${l}.${i})`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}if(!this.hass.states?.[a]){this._actionTestResult="error",this._actionTestError=`Target entity "${a}" not found in Home Assistant \u2014 the entity may have been renamed or its integration removed.`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}}this._actionTestResult="ok",setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3)}_buildActionData(){if(this._actionDataJsonFallback.trim())try{let e=JSON.parse(this._actionDataJsonFallback);if(e&&typeof e=="object"&&!Array.isArray(e))return e}catch{}return{...this._actionData}}_serviceSchema(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e))return null;let[t,i]=e.split("."),a=this.hass?.services?.[t]?.[i]?.fields;return!a||Object.keys(a).length===0?null:Object.entries(a).map(([l,c])=>({name:l,required:!!c.required,selector:c.selector||{text:{}}}))}_renderCompletionActionsSection(e){if(!this.completionActionsEnabled)return p;let t=this._serviceSchema();return n`
      <details class="ca-section">
        ...
        <div class="ca-test-row">
          ...
          ${this._actionTestResult==="ok"?n`<span class="ca-test-ok">${s("on_complete_action_test_success",e)}</span>`:p}
          ${this._actionTestResult==="error"?n`<div class="ca-test-error-block">
                ...
                ${this._actionTestError?n`<div class="ca-test-error-detail">${this._actionTestError}</div>`:p}
              </div>`:p}
        </div>
      </details>
...
`).map(c=>c.trim()).filter(Boolean).slice(0,100);e.checklist=l.length?l:null}if(this.completionActionsEnabled){let l=this._actionService.trim();if(l&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(l)){let _={service:l},m=this._actionTargetEntity.trim();m&&(_.target={entity_id:m});let v=this._buildActionData();Object.keys(v).length>0&&(_.data=v),e.on_complete_action=_}else e.on_complete_action=null;let c={};this._qcNotes.trim()&&(c.notes=this._qcNotes.trim());let d=parseFloat(this._qcCost);!isNaN(d)&&d>=0&&(c.cost=d);let g=parseInt(this._qcDuration,10);!isNaN(g)&&g>=0&&(c.duration=g),this._qcFeedback&&(c.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(c).length?c:null}let t=await this.hass.connection.sendMessagePromise(e),i=this._taskId||t?.task_id,a=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(i&&this._scheduleType==="sensor_based"&&a)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:i,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=M(e,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return p;let e=this._lang;return n`
    ...
    `}_renderUnitSelect(){let e=this._lang;return n`
      <div class="select-row">
        ...
        ></ms-textfield>`:p}_renderTriggerTypeFields(){let e=this._lang;return this._triggerType==="threshold"?n`
        <ms-textfield
      ...
      `:this._triggerType==="runtime"?n`
        ...
        ></ms-textfield>
      `:p}render(){if(!this._open)return n``;let e=this._lang,t=this._taskId?s("edit_task",e):s("new_task",e);return n`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
          ${this._error?n`<div class="error">${this._error}</div>`:p}
...
`;var X=class extends T{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter)}getCardSize(){return 6}get _lang(){return this.hass?.language||"en"}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){super.updated(e);let t=this.hass?.language;if(t&&!Qe(t)&&se(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}_onEventClick(e){if(e.history_timestamp){this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}));return}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}render(){if(!this.hass)return p;let e=this._lang,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.title,l=null;this._userFilter&&(l=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let c=new Date;c.setHours(0,0,0,0);let d=this._pastDays>0,g=d?ri(this._objects,c,this._pastDays,l):ai(this._objects,c,this._windowDays,l),_=qe(c),m=this._windowDays===365||d,v=m?g.filter(b=>b.events.length>0):g,f=b=>{let j=`cal-status-${b.status}`,z=b.projected?"cal-event-projected":"",L=b.status==="overdue"&&b.days_until_due!=null?` (${Math.abs(b.days_until_due)}d ${s("overdue",e).toLowerCase()})`:"",E=b.projected&&b.interval_days?n`<span class="cal-event-recur">${b.interval_unit&&b.interval_unit!=="days"?`${b.interval_days} ${s("unit_"+b.interval_unit,e)}`:s("cal_every_n_days",e).replace("{n}",String(b.interval_days))}</span>`:p,I=b.schedule_type==="sensor_based",V=I?n`<ha-icon class="cal-event-icon cal-source-sensor"
                ...
                icon="${b.adaptive_enabled?"mdi:clock-time-four-outline":"mdi:clock-outline"}"></ha-icon>`,ie=I&&b.prediction_confidence&&b.status!=="triggered"&&!b.projected?n`<span class="cal-event-prediction cal-conf-${b.prediction_confidence}">
            ${s("cal_predicted",e)} · ${s(`cal_confidence_${b.prediction_confidence}`,e)}
          </span>`:p,W=this._stats?.budget?.currency_symbol||"\u20AC",D=b.history_type?s(b.history_type,e):s(b.status,e);return n`
        <div class="cal-event ${z}"
          ...
          ${b.avg_cost!=null&&b.avg_cost>0?n`<span class="cal-event-cost">${b.avg_cost.toFixed(0)} ${W}</span>`:p}
        </div>
      `},x=b=>{let[j,z,L]=b.date.split("-").map(Number),E=new Date(j,z-1,L),I=b.date===_,V=E.toLocaleDateString(e,{weekday:"short"}),ie=E.toLocaleDateString(e,{month:"long"});return n`
        <div class="cal-day-row">
          ...
          <div class="cal-day-content">
            <div class="cal-day-header">
              <span class="cal-day-month">${ie}</span>
              ${I?n`<span class="cal-day-today-badge">${s("today",e)}</span>`:p}
            </div>
    ...
    `],h([y({attribute:!1})],X.prototype,"hass",2),h([u()],X.prototype,"_config",2),h([u()],X.prototype,"_objects",2),h([u()],X.prototype,"_stats",2),h([u()],X.prototype,"_windowDays",2),h([u()],X.prototype,"_pastDays",2),h([u()],X.prototype,"_userFilter",2),h([u()],X.prototype,"_unsub",2);var cs=[{value:7,label:"Week (7 days)"},{value:14,label:"Fortnight (14 days)"},{value:30,label:"Month (30 days, default)"},{value:365,label:"Year (365 days, empty days collapsed)"}],Ee=class extends T{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"}}setConfig(e){this._config={...e}}_valueChanged(e,t){let i={...this._config,[e]:t};e==="show_window_chips"&&t===!0&&delete i.show_window_chips,e==="show_user_filter"&&t===!0&&delete i.show_user_filter,e==="title"&&(!t||typeof t=="string"&&t.trim()==="")&&delete i.title,e==="user_filter"&&t===""&&delete i.user_filter,this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}render(){let e=this._config.window_days??30,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.user_filter??"",l=this._config.title??"";return n`
      <div class="editor">
        <div class="row">
  ...
  `,h([y({attribute:!1})],le.prototype,"hass",2),h([u()],le.prototype,"_open",2),h([u()],le.prototype,"_saving",2),h([u()],le.prototype,"_error",2),h([u()],le.prototype,"_draft",2);customElements.get("maintenance-history-edit-dialog")||customElements.define("maintenance-history-edit-dialog",le);var Y=class extends T{constructor(){super(...arguments);this._open=!1;this._title="";this._message="";this._confirmText="";this._danger=!1;this._inputLabel="";this._inputType="";this._inputValue="";this._resolve=null;this._promptResolve=null}confirm(e){return this._title=e.title,this._message=e.message,this._confirmText=e.confirmText||"OK",this._danger=e.danger||!1,this._inputLabel="",this._inputType="",this._inputValue="",this._open=!0,new Promise(t=>{this._resolve=t,this._promptResolve=null})}prompt(e){return this._title=e.title,this._message=e.message,this._confirmText=e.confirmText||"OK",this._danger=e.danger||!1,this._inputLabel=e.inputLabel||"",this._inputType=e.inputType||"text",this._inputValue=e.inputValue||"",this._open=!0,new Promise(t=>{this._promptResolve=t,this._resolve=null})}_cancel(){this._open=!1,this._promptResolve&&(this._promptResolve({confirmed:!1,value:""}),this._promptResolve=null),this._resolve?.(!1),this._resolve=null}_confirmAction(){this._open=!1,this._promptResolve&&(this._promptResolve({confirmed:!0,value:this._inputValue}),this._promptResolve=null),this._resolve?.(!0),this._resolve=null}render(){if(!this._open)return p;let e=this.hass?.language||"en";return n`
  ...
  `,h([y({attribute:!1})],Y.prototype,"hass",2),h([u()],Y.prototype,"_open",2),h([u()],Y.prototype,"_title",2),h([u()],Y.prototype,"_message",2),h([u()],Y.prototype,"_confirmText",2),h([u()],Y.prototype,"_danger",2),h([u()],Y.prototype,"_inputLabel",2),h([u()],Y.prototype,"_inputType",2),h([u()],Y.prototype,"_inputValue",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",Y);var li={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},ci=o=>(...r)=>({_$litDirective$:o,values:r}),nt=class{constructor(r){}get _$AU(){return this._$AM._$AU}_$AT(r,e,t){this._$Ct=r,this._$AM=e,this._$Ci=t}_$AS(r,e){return this.update(r,e)}update(r,e){return this.render(...e)}};var De=class extends nt{constructor(r){if(super(r),this.it=p,r.type!==li.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(r){if(r===p||r==null)return this._t=void 0,this.it=r;if(r===oe)return r;if(typeof r!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(r===this.it)return this._t;this.it=r;let e=[r];return e.raw=e,this._t={_$litType$:this.constructor.resultType,strings:e,values:[]}}};De.directiveName="unsafeHTML",De.resultType=1;var di=ci(De);var ps=["EUR","USD","GBP","JPY","CHF","CAD","AUD","CNY","INR","BRL","CZK","PLN","RUB","SEK","NOK","DKK","UAH"],C=class extends T{constructor(){super(...arguments);this.budget=null;this._settings=null;this._loading=!0;this._importCsv="";this._importLoading=!1;this._includeHistory=!0;this._toast="";this._testingNotification=!1;this._users=[];this._vacEnabled=!1;this._vacStart="";this._vacEnd="";this._vacBuffer=3;this._vacExempt=new Set;this._vacIsActive=!1;this._vacWindowEnd=null;this._vacAllTasks=[];this._vacPreview=[];this._vacPreviewLoading=!1;this._vacSaving=!1;this._qrObjects=[];this._qrSelectedEntries=new Set;this._qrActions=new Set(["view"]);this._qrUrlMode="companion";this._qrBatchLoading=!1;this._qrBatchResults=[];this._qrObjectsLoaded=!1;this._loaded=!1;this._userService=null;this._sendTestNotification=async()=>{this._testingNotification=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/test_notification"}),t=e.message||(e.success?s("test_notification_success",this._lang):s("test_notification_failed",this._lang));this._showToast(t)}catch{this._showToast(s("test_notification_failed",this._lang))}finally{this._testingNotification=!1}}}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._loaded?(this._loaded=!0,this._userService=new ue(this.hass),this._loadSettings(),this._loadUsers()):e.has("hass")&&this.hass&&this._userService&&this._userService.updateHass(this.hass)}async _loadUsers(){if(this._userService)try{this._users=await this._userService.getUsers()}catch{this._users=[]}}async _loadSettings(){this._loading=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});this._settings=e,this._hydrateVacationFromSettings()}catch{}this._loading=!1}_hydrateVacationFromSettings(){let e=this._settings?.vacation;e&&(this._vacEnabled=e.enabled,this._vacStart=e.start||"",this._vacEnd=e.end||"",this._vacBuffer=e.buffer_days,this._vacExempt=new Set(e.exempt_task_ids||[]),this._vacIsActive=e.is_active,this._vacWindowEnd=e.window_end)}async _updateSetting(e,t){try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/update",settings:{[e]:t}});this._settings=i,this._showToast(s("settings_saved",this._lang)),this.dispatchEvent(new CustomEvent("settings-changed"))}catch{this._showToast(s("action_error",this._lang))}}_showToast(e){this._toast=e,setTimeout(()=>{this._toast=""},3e3)}_downloadFile(e,t,i){et(e,t,i)}render(){let e=this._lang;return this._loading||!this._settings?n`<div class="settings-loading">Loading…</div>`:n`
    ...
    `}_renderVacation(e){let t=this._vacEnabled&&!this._vacIsActive&&this._vacWindowEnd&&new Date(this._vacWindowEnd)<new Date,i=this._vacExempt.size;return n`
      <div class="settings-section vacation-section" data-section="vacation">
        <h3>
          ${s("vacation_title",e)}
          ${this._vacIsActive?n`<span class="vac-badge active">${s("vacation_active",e)}</span>`:p}
          ${t?n`<span class="vac-badge stale">${s("vacation_ended",e)}</span>`:p}
        </h3>
        ...
        <details class="vac-exempt-panel">
          <summary>
            ...
            ${i>0?n`<span class="section-badge">${i}</span>`:p}
          </summary>
        ...
        ${this._vacStart&&this._vacEnd?n`
          <div class="vac-preview-toolbar">
            ...
            ${this._vacPreview.length>0?n`<span class="vac-preview-count">${this._vacPreview.length} ${s("vacation_preview_affected",e)}</span>`:p}
          </div>
    ...
    `}_renderVacationTaskList(e){let t=new Map;for(let a of this._vacAllTasks){let l=t.get(a.object_name)||[];l.push(a),t.set(a.object_name,l)}return[...t.entries()].sort(([a],[l])=>a.localeCompare(l)).map(([a,l])=>n`
      <div class="vac-task-group">
        ...
        ${l.sort((c,d)=>c.task_name.localeCompare(d.task_name)).map(c=>n`
            ...
            <div class="vac-preview-row ${a?"exempt":""}">
              <div class="vac-preview-info">
                <div class="vac-preview-name">
                  ...
                  ${t.kind==="sensor_based"?n`<span class="vac-preview-kind">${s("vacation_sensor_based",e)}</span>`:p}
                </div>
              ...
              <div class="vac-preview-actions">
                ...
                ${t.kind==="time_based"?n`<button @click=${()=>this._previewActionSkip(t)}>${s("qr_action_skip",e)}</button>`:p}
...
<body>
  ...
  `,h([y({attribute:!1})],C.prototype,"hass",2),h([y({attribute:!1})],C.prototype,"features",2),h([y({attribute:!1})],C.prototype,"budget",2),h([u()],C.prototype,"_settings",2),h([u()],C.prototype,"_loading",2),h([u()],C.prototype,"_importCsv",2),h([u()],C.prototype,"_importLoading",2),h([u()],C.prototype,"_includeHistory",2),h([u()],C.prototype,"_toast",2),h([u()],C.prototype,"_testingNotification",2),h([u()],C.prototype,"_users",2),h([u()],C.prototype,"_vacEnabled",2),h([u()],C.prototype,"_vacStart",2),h([u()],C.prototype,"_vacEnd",2),h([u()],C.prototype,"_vacBuffer",2),h([u()],C.prototype,"_vacExempt",2),h([u()],C.prototype,"_vacIsActive",2),h([u()],C.prototype,"_vacWindowEnd",2),h([u()],C.prototype,"_vacAllTasks",2),h([u()],C.prototype,"_vacPreview",2),h([u()],C.prototype,"_vacPreviewLoading",2),h([u()],C.prototype,"_vacSaving",2),h([u()],C.prototype,"_qrObjects",2),h([u()],C.prototype,"_qrSelectedEntries",2),h([u()],C.prototype,"_qrActions",2),h([u()],C.prototype,"_qrUrlMode",2),h([u()],C.prototype,"_qrBatchLoading",2),h([u()],C.prototype,"_qrBatchResults",2),h([u()],C.prototype,"_qrObjectsLoaded",2);customElements.define("maintenance-settings-view",C);var G=class extends T{constructor(){super(...arguments);this.objects=[];this._summary=null;this._loaded=!1;this._busy=!1;this._error="";this._query="";this._results=[];this._expanded=!1;this._initiallyLoaded=!1;this._searchTimer=0}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._initiallyLoaded&&(this._initiallyLoaded=!0,this._load(),se(this._lang).then(()=>this.requestUpdate()))}async _load(){this._busy=!0;try{this._summary=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/storage"}),this._error=""}catch(e){this._error=M(e,this._lang)}finally{this._loaded=!0,this._busy=!1}}_nameFor(e){return this.objects.find(i=>i.object?.id===e)?.object?.name||e.slice(0,8)}_entryFor(e){return this.objects.find(t=>t.object?.id===e)?.entry_id}_toggle(){this._expanded=!this._expanded}_openObject(e){this.dispatchEvent(new CustomEvent("open-object",{detail:{entry_id:e},bubbles:!0,composed:!0}))}_onSearch(e){this._query=e.target.value,clearTimeout(this._searchTimer),this._searchTimer=window.setTimeout(()=>{this._doSearch()},250)}async _doSearch(){let e=this._query.trim();if(!e){this._results=[];return}try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/search",query:e});this._results=t.results||[]}catch(t){this._error=M(t,this._lang),this._results=[]}}async _openResult(e){if(e.kind==="weblink"){window.open(e.url,"_blank","noopener");return}let t=window.open("about:blank","_blank");try{let i=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300});t&&(t.location.href=new URL(i.path,window.location.origin).href)}catch(i){t&&t.close(),this._error=M(i,this._lang)}}_renderResult(e,t){return n`
    ...
    `}render(){if(!this._loaded||!this._summary)return p;let e=this._summary;if(e.document_count===0)return p;let t=this._lang,i=Object.entries(e.by_object).filter(([,a])=>a.files>0||a.links>0).map(([a,l])=>({id:a,name:this._nameFor(a),entry:this._entryFor(a),...l})).sort((a,l)=>l.bytes-a.bytes);return n`
      <ha-card>
        <div class="card-content">
          <div class="header">
            ...
            >
              ...
              <span class="header-summary">
                ${ae(e.total_bytes)}
                ${e.dedup_savings_bytes>0?n`<span class="saved">−${ae(e.dedup_savings_bytes)}</span>`:p}
              </span>
            </button>
            <button
              class="icon-btn"
          ...
          ${this._expanded?n`
                <div class="body">
                  ...
                  ${this._query.trim()?this._results.length?n`<div class="obj-list">${this._results.map(a=>this._renderResult(a,t))}</div>`:n`<div class="search-empty">${s("doc_search_none",t)}</div>`:i.length?n`<div class="obj-list">${i.map(a=>this._renderObjRow(a,t))}</div>`:p}
                </div>
              `:p}
        </div>
      </ha-card>
    `}_renderObjRow(e,t){let i=e.entry;return n`
      ...
      >
        ...
        <span class="obj-meta">
          ${e.files>0?n`<ha-icon icon="mdi:file-document-outline"></ha-icon>${e.files}`:p}
          ${e.links>0?n`<ha-icon icon="mdi:link-variant"></ha-icon>${e.links}`:p}
        </span>
        <span class="obj-size">${ae(e.bytes)}</span>
        ${i?n`<ha-icon class="obj-go" icon="mdi:chevron-right"></ha-icon>`:p}
      </div>
    `}};G.styles=A`
  ...
  `,h([y({attribute:!1})],Q.prototype,"hass",2),h([y({attribute:!1})],Q.prototype,"objects",2),h([u()],Q.prototype,"_open",2),h([u()],Q.prototype,"_loading",2),h([u()],Q.prototype,"_error",2),h([u()],Q.prototype,"_groupId",2),h([u()],Q.prototype,"_name",2),h([u()],Q.prototype,"_description",2),h([u()],Q.prototype,"_selected",2);customElements.get("maintenance-group-dialog")||customElements.define("maintenance-group-dialog",Q);function Fe(o,r,e=4){if(!isFinite(o)||!isFinite(r))return{ticks:[],niceMin:0,niceMax:1};if(o===r){let g=Math.abs(o)*.1||1;o-=g,r+=g}let t=r-o,i=Math.pow(10,Math.floor(Math.log10(t/Math.max(1,e)))),a=i;for(let g of[1,2,5,10])if(a=i*g,t/a<=e+.5)break;let l=Math.floor(o/a)*a,c=Math.ceil(r/a)*a,d=[];for(let g=l;g<=c+a*1e-6;g+=a)d.push(Math.abs(g)<a*1e-9?0:g);return{ticks:d,niceMin:l,niceMax:c}}function re(o){let r=Math.abs(o);return r>=1e6?Te((o/1e6).toFixed(r>=1e7?0:1))+"M":r>=1e4?Te((o/1e3).toFixed(0))+"k":r>=1e3?Te((o/1e3).toFixed(1))+"k":r>=100?o.toFixed(0):r>=10||r>=1?Te(o.toFixed(1)):r===0?"0":Te(o.toFixed(2))}function Te(o){return o.replace(/\.0+$/,"").replace(/(\.\d*[1-9])0+$/,"$1")}function fe(o,r,e){let t=o.toLocaleString(e,{maximumFractionDigits:Math.abs(o)>=100?0:1});return r?`${t} ${r}`:t}function Se(o,r,e){let t=new Date(o),i=e?{month:"short",day:"numeric",year:"2-digit"}:{month:"short",day:"numeric"};return t.toLocaleDateString(r,i)}function Ct(o,r){return new Date(o).toLocaleDateString(r,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function ot(o,r){return new Date(o).getFullYear()!==new Date(r).getFullYear()}function lt(o,r,e){if(e<2||r<=o)return[o,r];let t=[];for(let i=0;i<e;i++)t.push(o+(r-o)*i/(e-1));return t}var ct=210,te=46,ce=14,de=12,pi=14,us=20+pi,_s=[{days:7,key:"chart_range_7d"},{days:30,key:"chart_range_30d"},{days:90,key:"chart_range_90d"},{days:365,key:"chart_range_1y"}],N=class extends T{constructor(){super(...arguments);this.points=[];this.events=[];this.unit="";this.lang="en";this.thresholdAbove=null;this.thresholdBelow=null;this.targetValue=null;this.forceZero=!1;this.projection=null;this.rangeDays=30;this.showRange=!0;this.busy=!1;this._width=0;this._hover=null;this._ro=null}connectedCallback(){super.connectedCallback(),this._ro=new ResizeObserver(e=>{let t=Math.floor(e[0]?.contentRect?.width||0);t&&Math.abs(t-this._width)>2&&(this._width=t)}),this._ro.observe(this)}disconnectedCallback(){super.disconnectedCallback(),this._ro?.disconnect(),this._ro=null}_emitRange(e){e!==this.rangeDays&&this.dispatchEvent(new CustomEvent("range-change",{detail:{days:e},bubbles:!0,composed:!0}))}render(){let e=this._width||320,t=[...this.points].sort((a,l)=>a.ts-l.ts),i=this.lang;return n`
    ...
    `}_renderSvg(e,t){let i=this.lang,a=e-te-ce,l=ct-us,c=l-de,d=1/0,g=-1/0;for(let k of t)d=Math.min(d,k.min??k.val),g=Math.max(g,k.max??k.val);this.thresholdAbove!=null&&(d=Math.min(d,this.thresholdAbove),g=Math.max(g,this.thresholdAbove)),this.thresholdBelow!=null&&(d=Math.min(d,this.thresholdBelow),g=Math.max(g,this.thresholdBelow)),this.targetValue!=null&&(d=Math.min(d,this.targetValue),g=Math.max(g,this.targetValue)),this.forceZero&&(d=Math.min(d,0));let _=(g-d||1)*.06,m=this.forceZero&&d>=0?0:d-_,{ticks:v,niceMin:f,niceMax:x}=Fe(m,g+_,4);this.forceZero&&d>=0&&f<0&&(f=0,v=v.filter(k=>k>=0));let b=t[0].ts,j=t[t.length-1].ts,z=j-b||1,L=ot(b,j),E=k=>te+(k-b)/z*a,I=k=>de+(1-(k-f)/(x-f||1))*c,V=t.map(k=>`${E(k.ts).toFixed(1)},${I(k.val).toFixed(1)}`).join(" "),ie=`M${E(t[0].ts).toFixed(1)},${l} `+t.map(k=>`L${E(k.ts).toFixed(1)},${I(k.val).toFixed(1)}`).join(" ")+` L${E(t[t.length-1].ts).toFixed(1)},${l} Z`,W="",D=t.filter(k=>k.min!=null&&k.max!=null);if(D.length>=2){let k=D.map(R=>`${E(R.ts).toFixed(1)},${I(R.max).toFixed(1)}`),$=[...D].reverse().map(R=>`${E(R.ts).toFixed(1)},${I(R.min).toFixed(1)}`);W=`M${k[0]} `+k.slice(1).map(R=>`L${R}`).join(" ")+` L${$.join(" L")} Z`}let q=[];if(this.thresholdBelow!=null){let k=I(this.thresholdBelow);q.push({y:k,h:Math.max(0,l-k),lineY:k,label:`\u25BC ${re(this.thresholdBelow)}`,labelY:Math.min(l-4,k+13)})}if(this.thresholdAbove!=null){let k=I(this.thresholdAbove);q.push({y:de,h:Math.max(0,k-de),lineY:k,label:`\u25B2 ${re(this.thresholdAbove)}`,labelY:Math.max(de+11,k-5)})}let pe=t[t.length-1],_e=(this.events||[]).filter(k=>k.ts>=b&&k.ts<=j),ye=lt(b,j,Math.max(2,Math.min(5,Math.floor(a/110)+1))),U=this._hover;return n`
      <div class="svg-holder">
        <svg
          ...
          @pointermove=${k=>this._onPointer(k,t,E,I,e)}
          @pointerdown=${k=>this._onPointer(k,t,E,I,e)}
          @pointerleave=${()=>this._hover=null}
        >
          <defs>
            <clipPath id="plot"><rect x="${te}" y="${de}" width="${a}" height="${c}" /></clipPath>
            ${q.length?P`<clipPath id="danger">${q.map(k=>P`<rect x="${te}" y="${k.y.toFixed(1)}" width="${a}" height="${k.h.toFixed(1)}" />`)}</clipPath>`:p}
          </defs>

          ${v.map(k=>{let $=I(k);return $<de-1||$>l+1?p:P`
              <line x1="${te}" y1="${$.toFixed(1)}" x2="${e-ce}" y2="${$.toFixed(1)}"
                stroke="var(--divider-color)" stroke-width="1" opacity="0.6" />
              <text x="${te-7}" y="${($+3.5).toFixed(1)}" text-anchor="end" class="tick-label">${re(k)}</text>`})}

          ${q.map(k=>P`<rect x="${te}" y="${k.y.toFixed(1)}" width="${a}" height="${k.h.toFixed(1)}"
              fill="var(--error-color, #f44336)" opacity="0.07" />`)}

          ${W?P`<path d="${W}" fill="var(--primary-color)" opacity="0.08" clip-path="url(#plot)" />`:p}
          <path d="${ie}" fill="var(--primary-color)" opacity="0.10" clip-path="url(#plot)" />
          ...
          ${q.map(k=>P`
              <line x1="${te}" y1="${k.lineY.toFixed(1)}" x2="${e-ce}" y2="${k.lineY.toFixed(1)}"
          ...
          ${this.targetValue!=null?P`<line x1="${te}" y1="${I(this.targetValue).toFixed(1)}" x2="${e-ce}" y2="${I(this.targetValue).toFixed(1)}"
                stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="6,4" />
              <text x="${e-ce-4}" y="${(I(this.targetValue)-5).toFixed(1)}" text-anchor="end" class="zone-label">◆ ${re(this.targetValue)} ${this.unit}</text>`:p}

          ${this.projection&&this.projection.length===2?P`<line x1="${E(this.projection[0].ts).toFixed(1)}" y1="${I(this.projection[0].val).toFixed(1)}"
                x2="${Math.min(E(this.projection[1].ts),e-ce).toFixed(1)}" y2="${I(Math.max(f,Math.min(x,this.projection[1].val))).toFixed(1)}"
                stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.8" />`:p}

          ${ye.map((k,$)=>{let R=E(k),dt=$===0?"start":$===ye.length-1?"end":"middle";return P`<text x="${R.toFixed(1)}" y="${ct-5}" text-anchor="${dt}" class="tick-label">${Se(k,i,L)}</text>`})}

          <line x1="${te}" y1="${l}" x2="${e-ce}" y2="${l}" stroke="var(--divider-color)" stroke-width="1" />

          ${_e.map(k=>{let $=E(k.ts),R=k.type==="completed"?"var(--success-color, #4caf50)":k.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return P`
          ...
          ${U?P`
                ...
                <circle cx="${U.x.toFixed(1)}" cy="${U.y.toFixed(1)}" r="4.5" fill="var(--primary-color)"
                  stroke="var(--card-background-color, #fff)" stroke-width="2" />`:P`<circle cx="${E(pe.ts).toFixed(1)}" cy="${I(pe.val).toFixed(1)}" r="4" fill="var(--primary-color)"
        ...
        ${U?n`<div
            ...
            >
              ...
              <div class="hover-val">
                ...
                ${U.p.min!=null&&U.p.max!=null?n`<span class="hover-range">(${re(U.p.min)}–${re(U.p.max)})</span>`:p}
              </div>
            </div>`:p}
      </div>
    `}_onPointer(e,t,i,a,l){let d=e.currentTarget.getBoundingClientRect(),g=(e.clientX-d.left)/d.width*l;if(g<te-8||g>l-ce+8){this._hover=null;return}let _=t[0],m=1/0;for(let v of t){let f=Math.abs(i(v.ts)-g);f<m&&(m=f,_=v)}this._hover={x:i(_.ts),y:a(_.val),p:_}}};N.styles=A`
    ...
    <div class="trigger-card">
      <div class="trigger-header">
        ...
        <div>
          ${v?n`
            ...
            <div class="trigger-entity-id">${d.map((x,b)=>n`${b>0?", ":""}<span class="entity-link" @click=${j=>be(j,x)}>${x}</span>`)}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `:n`
      ...
      ${f?gs(o,g,r):_!=null?n`
              <div class="trigger-value-row">
                ...
                ${g?n`<span class="trigger-unit">${g}</span>`:p}
              </div>
            `:p}

      <div class="trigger-limits">
        ${m==="threshold"?n`
          ${e.trigger_above!=null?n`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("threshold_above",t)}: ${e.trigger_above} ${g}</span>`:p}
          ${e.trigger_below!=null?n`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("threshold_below",t)}: ${e.trigger_below} ${g}</span>`:p}
          ${e.trigger_for_minutes?n`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${s("for_minutes",t)}: ${e.trigger_for_minutes}</span>`:p}
        `:p}
        ${m==="state_change"?n`
          ${e.trigger_target_changes!=null?n`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("target_changes",t)}: ${e.trigger_target_changes}</span>`:p}
        `:p}
        ${m==="runtime"?n`
          ${e.trigger_runtime_hours!=null?n`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("runtime_hours",t)}: ${e.trigger_runtime_hours}h</span>`:p}
        `:p}
        ${m==="compound"?n`
  ...
  `}function _i(o,r){if(o.trigger_baseline_value!=null)return{value:o.trigger_baseline_value,ts:hi(o)};if(!r.length)return null;let e=hi(o);if(e==null)return{value:r[0].val,ts:null};let t=r[0],i=Math.abs(r[0].ts-e);for(let a of r){let l=Math.abs(a.ts-e);l<i&&(t=a,i=l)}return{value:t.val,ts:e}}function hi(o){let r=[...o.history].filter(e=>e.type==="completed"||e.type==="reset").sort((e,t)=>new Date(t.timestamp).getTime()-new Date(e.timestamp).getTime())[0];return r?new Date(r.timestamp).getTime():null}function gs(o,r,e){let i=o.trigger_config.trigger_target_value,a=o.trigger_current_value;if(a==null||i<=0)return p;let l=_i(o,gi(o,e)),c=Math.max(0,a-(l?.value??a)),d=Math.min(999,Math.round(c/i*100)),g=d>=100?"over":d>=75?"near":"ok",_=e.lang;return n`
    <div class="counter-progress">
      <div class="counter-progress-nums">
        <span class="counter-progress-main">${fe(c,"",_)}<span class="counter-progress-target"> / ${fe(i,r,_)}</span></span>
        <span class="counter-progress-pct ${g}">${d} %</span>
      </div>
      <div class="counter-progress-bar" role="progressbar" aria-valuenow=${d} aria-valuemin="0" aria-valuemax="100">
        <div class="counter-progress-fill ${g}" style="width:${Math.min(100,d)}%"></div>
      </div>
      <div class="counter-progress-caption">
        ${s("chart_since_service",_)} · ${s("current",_)}: ${fe(a,r,_)}
      </div>
    </div>
  `}function gi(o,r){let e=o.trigger_config;if(!e)return[];let t=e.entity_id||"",i=r.detailStatsData.get(t)||[],a=r.isCounterEntity(e),l=[];if(i.length>=2)for(let c of i){let d={ts:c.ts,val:c.val};!a&&c.min!=null&&c.max!=null&&(d.min=c.min,d.max=c.max),l.push(d)}else for(let c of o.history)c.trigger_value!=null&&l.push({ts:new Date(c.timestamp).getTime(),val:c.trigger_value});return o.trigger_current_value!=null&&l.push({ts:Date.now(),val:o.trigger_current_value}),l.sort((c,d)=>c.ts-d.ts),l}function ms(o,r,e){let t=o.trigger_config;if(!t)return p;let i=t.type||"threshold",a=t.entity_id||"",l=i==="counter"&&t.trigger_target_value!=null,c=gi(o,e),d=c.length<2&&!!a&&e.hasStatsService&&!e.detailStatsData.has(a);if(c.length<2&&!d)return p;let g=Date.now()-e.rangeDays*864e5,_=c.filter(b=>b.ts>=g);_.length>=2&&(c=_);let m=null,v=!1;if(l&&c.length){let b=_i(o,c);if(b){if(b.ts!=null){let j=c.filter(z=>z.ts>=b.ts);j.length>=2&&(c=j)}c=c.map(j=>({...j,val:Math.max(0,j.val-b.value)}))}m=t.trigger_target_value,v=!0}let f=null;if(!l&&o.degradation_rate!=null&&o.degradation_trend!=="stable"&&o.degradation_trend!=="insufficient_data"&&c.length>=2){let b=c[c.length-1];f=[b,{ts:b.ts+30*864e5,val:b.val+o.degradation_rate*30}]}let x=o.history.filter(b=>["completed","skipped","reset"].includes(b.type)).map(b=>({ts:new Date(b.timestamp).getTime(),type:b.type}));return n`
      ...
      <div class="prediction-grid">
        ${t?n`
          <div class="prediction-item">
            ...
            ${o.degradation_rate!=null?n`<span class="prediction-rate">${o.degradation_rate>0?"+":""}${Math.abs(o.degradation_rate)>=10?Math.round(o.degradation_rate).toLocaleString():o.degradation_rate.toFixed(1)} ${o.trigger_entity_info?.unit_of_measurement||""}/${s("day_short",r)}</span>`:p}
          </div>
        `:p}
        ${i?n`
          <div class="prediction-item">
            ...
            ${o.threshold_prediction_date?n`<span class="prediction-date">${Z(o.threshold_prediction_date,r)}</span>`:p}
            ${o.threshold_prediction_confidence?n`<span class="confidence-dot ${o.threshold_prediction_confidence}"></span>`:p}
          </div>
        `:p}
        ${a&&e.environmental?n`
          <div class="prediction-item">
            ...
            ${o.environmental_entity?n`<span class="prediction-entity entity-link" @click=${c=>be(c,o.environmental_entity)}>${o.environmental_entity}</span>`:p}
          </div>
        `:p}
      </div>
    </div>
  `}function vi(o,r){let e=o.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return p;let a=o.interval_days??0,l=o.suggested_interval??a;return n`
  ...
  `}function bs(o,r,e,t,i){let f=Math.max(e,t,r,1)*1.3,x=50,b=[];for(let D=0;D<=x;D++){let q=D/x*f,pe=1-Math.exp(-Math.pow(q/r,o)),_e=32+q/f*260,ye=136-pe*128;b.push([_e,ye])}let j=b.map(([D,q])=>`${D.toFixed(1)},${q.toFixed(1)}`).join(" "),z="M32,136 "+b.map(([D,q])=>`L${D.toFixed(1)},${q.toFixed(1)}`).join(" ")+` L${b[x][0].toFixed(1)},136 Z`,L=32+e/f*260,E=1-Math.exp(-Math.pow(e/r,o)),I=136-E*128,V=((1-E)*100).toFixed(0),ie=32+t/f*260,W=[0,.25,.5,.75,1];return n`
      ...
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_weibull",i)}">
        ${W.map(D=>{let q=136-D*128;return P`
            <line x1="${32}" y1="${q.toFixed(1)}" x2="${292}" y2="${q.toFixed(1)}"
              stroke="var(--divider-color)" stroke-width="0.5" stroke-dasharray="${D===.5?"4,3":p}" />
        ...
        ${e>0?P`
          ...
          <circle cx="${L.toFixed(1)}" cy="${I.toFixed(1)}" r="3"
            fill="var(--primary-color, #03a9f4)" />
          <text x="${(L+4).toFixed(1)}" y="${(I-6).toFixed(1)}" fill="var(--primary-color, #03a9f4)"
            font-size="9" font-weight="600">R=${V}%</text>
        `:p}

        ${t>0&&t!==e?P`
          <line x1="${ie.toFixed(1)}" y1="${8}" x2="${ie.toFixed(1)}" y2="${136 .toFixed(1)}"
    ...
    <div class="chart-legend">
      ...
      ${e>0?n`<span class="legend-item"><span class="legend-swatch" style="background:var(--primary-color, #03a9f4); opacity:0.5"></span> ${s("current_interval_marker",i)}</span>`:p}
      ${t>0&&t!==e?n`<span class="legend-item"><span class="legend-swatch" style="background:var(--success-color, #4caf50)"></span> ${s("recommended_marker",i)}</span>`:p}
    </div>
  `}function fs(o,r){return n`
  ...
  `}function ys(o,r,e){let t=o.confidence_interval_low,i=o.confidence_interval_high,a=r.suggested_interval??r.interval_days??0,l=r.interval_days??0,c=Math.max(0,t-5),g=i+5-c,_=(t-c)/g*100,m=(i-t)/g*100,v=(a-c)/g*100,f=l>0?(l-c)/g*100:-1;return n`
    <div class="confidence-range">
      ...
      <div class="confidence-bar">
        ...
        ${f>=0?n`<div class="confidence-marker current" style="left:${f.toFixed(1)}%"></div>`:p}
  ...
  `}var fi=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"];function yi(o,r,e){if(!e.seasonal||!o.seasonal_factor||o.seasonal_factor===1)return p;let t=fi.map(c=>s(c,r)),i=new Date().getMonth(),a=o.seasonal_factors||o.interval_analysis?.seasonal_factors||null,l=a&&a.length===12?a:t.map((c,d)=>{let g=o.seasonal_factor||1,_=Math.sin((d-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,g+_))});return n`
    <div class="seasonal-card-compact">
      <h4>${s("seasonal_awareness",r)}</h4>
      <div class="seasonal-mini-chart">
        ${l.map((c,d)=>{let g=c*40,_=c<.9?"low":c>1.1?"high":"normal";return n`
  ...
  `}function xi(o,r){return xs(o,r)}function xs(o,r){let e=o.seasonal_factors??o.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return p;let t=o.interval_analysis?.seasonal_reason,i=new Date().getMonth(),a=300,l=100,c=8,g=l-c-4,_=Math.max(...e,1.5),m=a/12,v=m*.65,f=c+g-1/_*g;return n`
    <div class="seasonal-chart">
      <div class="seasonal-chart-title">
        ...
        ${t?n`<span class="source-tag">${t==="learned"?s("seasonal_learned",r):s("seasonal_manual",r)}</span>`:p}
      </div>
      <svg viewBox="0 0 ${a} ${l}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",r)}">
        ...
        ${e.map((x,b)=>{let j=x/_*g,z=b*m+(m-v)/2,L=c+g-j,E=b===i,I=x<1?"var(--success-color, #4caf50)":x>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return P`
            <rect x="${z.toFixed(1)}" y="${L.toFixed(1)}"
              width="${v.toFixed(1)}" height="${j.toFixed(1)}"
              fill="${I}" opacity="${E?1:.5}" rx="2" />
          `})}
      </svg>
  ...
  `}var $s=200,Oe=10,ws=22;function $i(o,r,e,t){let i=o.history.filter(c=>c.type==="completed"&&(c.cost!=null||c.duration!=null));if(i.length<2)return p;let a=i.some(c=>(c.cost??0)>0),l=i.some(c=>(c.duration??0)>0);return!a&&!l?p:n`
  ...
  `}function ks(o,r,e){let t=o.history.filter($=>$.type==="completed"&&($.cost!=null||$.duration!=null)).map($=>({ts:new Date($.timestamp).getTime(),cost:$.cost??0,duration:$.duration??0})).sort(($,R)=>$.ts-R.ts);if(t.length<2)return p;let i=t.some($=>$.cost>0),a=t.some($=>$.duration>0);if(!i&&!a)return p;let l=e!=="duration"&&i,c=e!=="cost"&&a,d=l||!c&&i,g=c||!l&&a,_=640,m=$s,v=d?44:12,f=g?44:12,x=_-v-f,b=m-ws,j=b-Oe,z=t[0].ts,L=t[t.length-1].ts,E=(L-z||864e5)*.05,I=z-E,V=L+E,ie=ot(z,L),W=$=>v+($-I)/(V-I)*x,D=Fe(0,Math.max(...t.map($=>$.cost))||1,3),q=Fe(0,Math.max(...t.map($=>$.duration))||1,3),pe=$=>Oe+(1-$/(D.niceMax||1))*j,_e=$=>Oe+(1-$/(q.niceMax||1))*j,ye=t.length>1?Math.min(...t.slice(1).map(($,R)=>W($.ts)-W(t[R].ts))):x,U=Math.max(6,Math.min(22,ye*.55)),k=lt(z,L,Math.max(2,Math.min(4,t.length)));return n`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${_} ${m}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_history",r)}">
        ${d?D.ticks.map($=>{let R=pe($);return R<Oe-1||R>b+1?p:P`
        ...
        ${g?q.ticks.map($=>{let R=_e($);return R<Oe-1||R>b+1?p:P`<text x="${_-f+6}" y="${(R+3.5).toFixed(1)}" text-anchor="start" fill="var(--accent-color, #ff9800)" font-size="10.5">${re($)}m</text>`}):p}
  ...
  `}var Es=60,Ts=20,wi=30,S=class extends T{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._chartRangeDays=(()=>{try{let e=parseInt(localStorage.getItem("msp-chart-range")||"",10);return[7,30,90,365].includes(e)?e:30}catch{return 30}})();this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._adminPanelUserIds=[];this._operatorWriteEnabled=!1;this._defaultWarningDays=7;this._actionLoading=!1;this._moreMenuOpen=!1;this._toastMessage="";this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab="dashboard";this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._objectSortMode="alphabetical";this._groupByMode="none";this._objectViewMode="cards";this._objectsTableColumns=Xe;this._showArchived=!1;this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._deepLinkHandled=!1;this._onDialogEvent=async()=>{try{await this._loadData()}catch{}};this._onCalendarLlCustom=e=>{let t=e.detail;t?.type==="maintenance-supporter:open-task"&&t.entry_id&&t.task_id&&(e.stopPropagation(),this._showTask(t.entry_id,t.task_id))};this._onHistoryEntrySaved=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"en"}get _isOperator(){let e=this.hass?.user;return e?e.is_admin?!1:!(this._operatorWriteEnabled&&this._adminPanelUserIds.includes(e.id)):!0}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler);let e=localStorage.getItem("maintenance_supporter_sort");e&&["due_date","object","type","task_name","area","assigned_user","group"].includes(e)&&(this._sortMode=e);let t=localStorage.getItem("maintenance_supporter_object_sort");t&&["alphabetical","due_soonest","task_count"].includes(t)&&(this._objectSortMode=t);let i=localStorage.getItem("maintenance_supporter_groupby");i&&["none","area","group","user"].includes(i)&&(this._groupByMode=i);let a=localStorage.getItem("maintenance_supporter_object_view");(a==="cards"||a==="table")&&(this._objectViewMode=a)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){super.updated(e);let t=this.hass?.language;if(t&&!Qe(t)&&se(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new it(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new ue(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,i,a,l]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);if(e&&(this._objects=e.objects),t&&(this._stats=t),i&&(this._budget=i),a&&(this._groups=a.groups||{}),l){let c=l;this._features=c.features,this._adminPanelUserIds=c.admin_panel_user_ids||[],this._operatorWriteEnabled=c.operator_write_enabled??!1;let d=c.general?.default_warning_days;typeof d=="number"&&d>=0&&d<=365&&(this._defaultWarningDays=d),this._objectsTableColumns=He(c.objects_table_columns)}this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("ms_action"),i=()=>{let _=window.location.pathname+window.location.hash;history.replaceState(history.state,"",_)};if(t==="add_object"){this._deepLinkHandled=!0,i(),requestAnimationFrame(()=>{this.shadowRoot?.querySelector("maintenance-object-dialog")?.openCreate()});return}if(t==="open_vacation"||t==="open_budget"||t==="open_groups"||t==="open_settings"){this._deepLinkHandled=!0,i(),this._overviewTab="settings",requestAnimationFrame(()=>{let _=this.shadowRoot?.querySelector("maintenance-settings-view"),m=t.replace("open_","");_?.scrollToSection?.(m)});return}let a=e.get("entry_id");if(!a)return;this._deepLinkHandled=!0;let l=e.get("task_id"),c=e.get("action"),d=window.location.pathname+window.location.hash;history.replaceState(history.state,"",d);let g=this._getObject(a);if(!g){this._showOverview();return}if(l){let _=g.tasks.find(m=>m.id===l);if(!_){this._showObject(a);return}this._showTask(a,l),c==="complete"?requestAnimationFrame(()=>{this._openCompleteDialog(a,l,_.name,this._features.checklists?_.checklist:void 0,this._features.adaptive&&!!_.adaptive_config?.enabled)}):c==="quick_complete"&&requestAnimationFrame(()=>{this._handleQuickComplete(a,l,_)})}else this._showObject(a)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let i=await this._statsService.getDetailStats(e,t,this._chartRangeDays),a=new Map(this._detailStatsData);a.set(e,i),this._detailStatsData=a}_setChartRange(e){if(e===this._chartRangeDays)return;this._chartRangeDays=e;try{localStorage.setItem("msp-chart-range",String(e))}catch{}let t=this._selectedEntryId&&this._selectedTaskId?this._getTask(this._selectedEntryId,this._selectedTaskId):null,i=t?.trigger_config?.entity_id;if(i){let a=new Map(this._detailStatsData);a.delete(i),this._detailStatsData=a,this._fetchDetailStats(i,this._isCounterEntity(t.trigger_config))}}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let i of this._objects)for(let a of i.tasks){let l=a.trigger_config?.entity_id;l&&e.push({entityId:l,isCounter:this._isCounterEntity(a.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let m of this._objects)for(let v of m.tasks){if(!this._showArchived&&v.archived||this._filterStatus&&v.status!==this._filterStatus)continue;if(this._filterUser){let x=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(v.responsible_user_id!==x)continue}let f=[];for(let x of Object.values(this._groups))x.task_refs?.some(b=>b.entry_id===m.entry_id&&b.task_id===v.id)&&f.push(x.name);e.push({entry_id:m.entry_id,task_id:v.id,object_name:m.object.name,task_name:v.name,type:v.type,schedule_type:v.schedule_type,status:v.status,days_until_due:v.days_until_due??null,next_due:v.next_due??null,trigger_active:v.trigger_active,trigger_current_value:v.trigger_current_value??null,trigger_current_delta:v.trigger_current_delta??null,trigger_config:v.trigger_config??null,trigger_entity_info:v.trigger_entity_info??null,times_performed:v.times_performed,total_cost:v.total_cost,interval_days:v.interval_days??null,interval_unit:v.interval_unit??null,interval_anchor:v.interval_anchor??null,is_done:v.is_done??!1,archived:v.archived??!1,history:v.history||[],enabled:v.enabled,nfc_tag_id:v.nfc_tag_id??null,area_id:m.object.area_id??null,responsible_user_id:v.responsible_user_id??null,group_names:f})}let t={overdue:0,triggered:1,due_soon:2,ok:3},i=(m,v)=>(t[m.status]??9)-(t[v.status]??9),a=(m,v)=>(m.days_until_due??99999)-(v.days_until_due??99999),l=(m,v)=>i(m,v)||a(m,v),c=m=>m.area_id&&this.hass?.areas?.[m.area_id]?.name||"",d=m=>m.responsible_user_id&&this._userService?.getUserName(m.responsible_user_id)||"",g=m=>m.group_names[0]||"",_={due_date:l,object:(m,v)=>m.object_name.localeCompare(v.object_name)||l(m,v),type:(m,v)=>m.type.localeCompare(v.type)||l(m,v),task_name:(m,v)=>m.task_name.localeCompare(v.task_name),area:(m,v)=>{let f=c(m),x=c(v);return!f&&x?1:f&&!x?-1:f.localeCompare(x)||l(m,v)},assigned_user:(m,v)=>{let f=d(m),x=d(v);return!f&&x?1:f&&!x?-1:f.localeCompare(x)||l(m,v)},group:(m,v)=>{let f=g(m),x=g(v);return!f&&x?1:f&&!x?-1:f.localeCompare(x)||l(m,v)}};return e.sort(_[this._sortMode]),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(a=>a.id===t)}_pushPanelState(e,t,i){let a={msp_view:e,msp_entry:t||null,msp_task:i||null};history.pushState(a,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let i=this._getTask(t.msp_entry,t.msp_task);i?.trigger_config?.entity_id&&this._fetchDetailStats(i.trigger_config.entity_id,this._isCounterEntity(i.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1,this._scrollContentToTop()}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null,this._scrollContentToTop()}_filterByStatus(e){this._filterStatus=e,this._overviewTab!=="dashboard"&&(this._overviewTab="dashboard"),this._scrollContentToTop()}_scrollContentToTop(){requestAnimationFrame(()=>{let e=this.shadowRoot?.querySelector(".content");e&&e.scrollTo({top:0,behavior:"smooth"})})}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null,this._scrollContentToTop()}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null,this._scrollContentToTop();let i=this._getTask(e,t);if(i?.trigger_config?.entity_id){let a=i.trigger_config.entity_id,l=this._isCounterEntity(i.trigger_config);this._fetchDetailStats(a,l)}}_showToast(e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}async _deleteObject(e){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("delete",this._lang),message:s("confirm_delete_object",this._lang),confirmText:s("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _deleteTask(e,t){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("delete",this._lang),message:s("confirm_delete_task",this._lang),confirmText:s("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _toggleArchiveTask(e,t,i){this._actionLoading=!0;try{await this.hass.connection.sendMessagePromise({type:i?"maintenance_supporter/task/unarchive":"maintenance_supporter/task/archive",entry_id:e,task_id:t}),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}finally{this._actionLoading=!1}}async _toggleArchiveObject(e,t){if(!(!t&&!await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("archive_object",this._lang),message:s("confirm_archive_object",this._lang),confirmText:s("archive_object",this._lang)})))try{await this.hass.connection.sendMessagePromise({type:t?"maintenance_supporter/object/unarchive":"maintenance_supporter/object/archive",entry_id:e}),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _skipTask(e,t,i){this._actionLoading=!0;try{let a={type:"maintenance_supporter/task/skip",entry_id:e,task_id:t};i&&(a.reason=i),await this.hass.connection.sendMessagePromise(a),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}finally{this._actionLoading=!1}}async _resetTask(e,t,i){this._actionLoading=!0;try{let a={type:"maintenance_supporter/task/reset",entry_id:e,task_id:t};i&&(a.date=i),await this.hass.connection.sendMessagePromise(a),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}finally{this._actionLoading=!1}}async _applySuggestion(e,t,i){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:i}),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}_openSeasonalOverrides(e){let t=this.shadowRoot.querySelector("maintenance-seasonal-overrides-dialog");if(!t||!this._selectedEntryId)return;let i=e.adaptive_config?.seasonal_overrides;t.open(this._selectedEntryId,e.id,i)}async _reanalyzeInterval(e,t){try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:e,task_id:t});i.recommended_interval?this._showToast(`${s("reanalyze_result",this._lang)}: ${i.recommended_interval} ${s("days",this._lang)} (${s(`confidence_${i.confidence}`,this._lang)}, ${i.data_points} ${s("data_points",this._lang)})`):this._showToast(s("reanalyze_insufficient_data",this._lang)),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _promptSkipTask(e,t){let i=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!i)return;let a=await i.prompt({title:s("skip",this._lang),message:s("skip_reason_prompt",this._lang),confirmText:s("skip",this._lang),inputLabel:s("reason_optional",this._lang),inputType:"text"});a.confirmed&&this._skipTask(e,t,a.value||void 0)}async _promptResetTask(e,t){let i=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!i)return;let a=await i.prompt({title:s("reset",this._lang),message:s("reset_date_prompt",this._lang),confirmText:s("reset",this._lang),inputLabel:s("reset_date_optional",this._lang),inputType:"date"});a.confirmed&&this._resetTask(e,t,a.value||void 0)}_dismissSuggestion(e,t){e&&t&&this._dismissedSuggestions.add(`${e}_${t}`),this.requestUpdate()}async _handleQuickComplete(e,t,i){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/quick_complete",entry_id:e,task_id:t}),this._showToast(s("quick_complete_success",this._lang))}catch(a){(a?.code||"")==="no_defaults"?this._openCompleteDialog(e,t,i.name,this._features.checklists?i.checklist:void 0,this._features.adaptive&&!!i.adaptive_config?.enabled):this._showToast(s("action_error",this._lang))}}_openCompleteDialog(e,t,i,a,l){let c=this.shadowRoot.querySelector("maintenance-complete-dialog");c&&(c.entryId=e,c.taskId=t,c.taskName=i,c.lang=this._lang,c.checklist=a||[],c.adaptiveEnabled=!!l,c.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,i,a){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,i,a)}render(){return n`
      ...
      ></maintenance-group-dialog>
      ${this._toastMessage?n`<div class="toast">${this._toastMessage}</div>`:p}
    `}_renderHeader(){let e=[{label:s("maintenance",this._lang),action:()=>this._showOverview()}];if(this._view==="object"&&this._selectedEntryId){let t=this._getObject(this._selectedEntryId);e.push({label:t?.object.name||"Object"})}if(this._view==="task"&&this._selectedEntryId&&this._selectedTaskId){let t=this._getObject(this._selectedEntryId);e.push({label:t?.object.name||"Object",action:()=>this._showObject(this._selectedEntryId)});let i=this._getTask(this._selectedEntryId,this._selectedTaskId);e.push({label:i?.name||"Task"})}return n`
      <div class="header">
        ${this.narrow?n`<ha-menu-button .hass=${this.hass} .narrow=${this.narrow}></ha-menu-button>`:p}
    ...
    `}_renderDashboard(){let e=this._stats,t=this._taskRows,i=this._lang,a=this._isOperator,l=this._objects.reduce((c,d)=>c+d.tasks.filter(g=>g.archived).length,0);return n`
      ...
      <div class="filter-bar">
        ...
        <label class="filter-field">
          ...
          >
            ...
            ${this._features.groups?n`<option value="group" ?selected=${this._groupByMode==="group"}>${s("groupby_group",i)}</option>`:p}
      ...
      ${a?p:n`<maintenance-storage-section-card
            .hass=${this.hass}
            .objects=${this._objects}
            @open-object=${c=>{let d=c.detail?.entry_id;d&&this._showObject(d)}}
          ></maintenance-storage-section-card>`}
    `}_renderGroupedTasks(e,t){let i=new Map,a=s("unassigned",t);for(let d of e){let g=[];this._groupByMode==="area"?g=[(d.area_id?this.hass?.areas?.[d.area_id]?.name:null)||a]:this._groupByMode==="user"?g=[(d.responsible_user_id?this._userService?.getUserName(d.responsible_user_id):null)||a]:this._groupByMode==="group"&&(g=d.group_names.length>0?d.group_names:[a]);for(let _ of g)i.has(_)||i.set(_,[]),i.get(_).push(d)}let l=[...i.entries()].sort(([d],[g])=>d===a&&g!==a?1:g===a&&d!==a?-1:d.localeCompare(g)),c=this._groupByMode==="area"?"mdi:map-marker-outline":this._groupByMode==="group"?"mdi:folder-outline":"mdi:account-outline";return n`
      ${l.map(([d,g])=>n`
      ...
      `)}
    `}_warrantyLabel(e,t,i){return e.kind==="expired"?s("warranty_expired",i):e.kind==="expiring"?s("warranty_expires_in",i).replace("{days}",String(e.days??0)):s("warranty_valid_until",i).replace("{date}",Z(t,i))}_renderWarrantyMeta(e,t){let i=jt(e);return n`<p class="meta">${s("warranty",t)}:
      <span class="warranty-chip warranty-${i.kind}">${this._warrantyLabel(i,e,t)}</span></p>`}_renderAllObjects(){let e=this._lang,t=this._isOperator,i=this._objectViewMode==="table"&&!this.narrow,a=this._objects.filter(_=>_.object.archived).length,l=_=>{let m=1/0;for(let v of _.tasks){let f=v.days_until_due;f!=null&&f<m&&(m=f)}return m},c=this._objects.filter(_=>this._showArchived||!_.object.archived);this._objectSortMode==="alphabetical"?c.sort((_,m)=>_.object.name.localeCompare(m.object.name)):this._objectSortMode==="task_count"?c.sort((_,m)=>m.tasks.length-_.tasks.length||_.object.name.localeCompare(m.object.name)):c.sort((_,m)=>l(_)-l(m)||_.object.name.localeCompare(m.object.name));let d=()=>{let _=new Map;for(let m of c){let v=m.object.area_id,f=v?this.hass?.areas?.[v]?.name||s("unassigned",e):s("no_area",e);_.has(f)||_.set(f,[]),_.get(f).push(m)}return new Map([..._.entries()].sort(([m],[v])=>m.localeCompare(v)))},g=_=>{let m=_.tasks.some(v=>v.status==="overdue"||v.status==="triggered");return n`
        <div class="object-card${m?" object-card-overdue":""}" @click=${()=>this._showObject(_.entry_id)}>
          ${m?n`<span class="overdue-dot" title="${s("has_overdue",e)}"></span>`:p}
          ...
          ${_.object.manufacturer||_.object.model?n`<div class="object-card-meta">${[_.object.manufacturer,_.object.model].filter(Boolean).join(" ")}</div>`:p}
          ${_.tasks.length===0?n`<div class="object-card-empty">${s("no_tasks_yet",e)}</div>`:p}
        </div>
      `};return n`
    ...
    `}_setObjectViewMode(e){this._objectViewMode=e,localStorage.setItem("maintenance_supporter_object_view",e)}async _exportObjectsCsv(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects/csv"}),t=new Date().toISOString().slice(0,10);et(e.csv,`maintenance_objects_${t}.csv`,"text/csv;charset=utf-8")}catch{this._showToast(s("action_error",this._lang))}}_renderObjectsTable(e){let t=this._lang,i=this._objectsTableColumns;return n`
      <div class="objects-table-wrap">
        <table class="objects-table">
          <thead>
            <tr>
              ${i.map(a=>{let l=we.find(d=>d.key===a),c=l&&l.key!=="actions"?s(l.labelKey,t):"";return n`<th class="oc-${a}">${c}</th>`})}
            </tr>
          </thead>
          <tbody>
            ${e.map(a=>n`
            ...
            <div class="groups-grid">
              ${e.map(([i,a])=>{let l=a.task_refs.map(c=>this._getTask(c.entry_id,c.task_id)?.name).filter(Boolean);return n`
                  <div class="group-card">
                    ...
                    ${a.description?n`<div class="group-card-desc">${a.description}</div>`:p}
    ...
    `}_renderOverviewRow(e){let t=this._lang,i=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,a=0,l=Ge.ok,c=!1;if(i&&e.days_until_due!==null){let m=At(e.interval_days,e.days_until_due,e.interval_unit);a=m.pct,c=m.overflow,e.status==="overdue"?l=Ge.overdue:e.status==="due_soon"&&(l=Ge.due_soon)}let d=e.area_id?this.hass?.areas?.[e.area_id]?.name:null,g=e.responsible_user_id?this._userService?.getUserName(e.responsible_user_id):null,_=e.group_names.length>0||d||g;return n`
      <div class="task-row${e.enabled?"":" task-disabled"}">
        <span class="cell-badges">
          ...
          ${e.enabled?p:n`<span class="badge-disabled">${s("disabled",t)}</span>`}
          ${e.nfc_tag_id?n`<span class="nfc-badge" title="${s("nfc_linked",t)}"><ha-icon icon="mdi:nfc-variant"></ha-icon></span>`:p}
        </span>
        ...
        <span class="due-cell" @click=${()=>this._showTask(e.entry_id,e.task_id)}>
          ...
          ${i?n`<div class="days-bar"><div class="days-bar-fill${c?" overflow":""}" style="width:${a}%;background:${l}"></div></div>`:p}
          ${e.trigger_config?this._renderTriggerProgress(e):!i&&e.trigger_active?n`<span style="color:var(--maint-triggered-color);font-weight:600">⚡</span>`:p}
    ...
    `}_openCompleteDialogForRow(e){let i=this._objects.find(a=>a.entry_id===e.entry_id)?.tasks.find(a=>a.id===e.task_id);this._openCompleteDialog(e.entry_id,e.task_id,e.task_name,this._features.checklists?i?.checklist:void 0,this._features.adaptive&&!!i?.adaptive_config?.enabled)}_renderTriggerProgress(e){let t=e.trigger_config??null;if(!t)return p;let i=t.type||"threshold",a=e.trigger_entity_info?.unit_of_measurement??"",l=0,c="";if(i==="threshold"){let _=e.trigger_current_value??null;if(_==null)return p;let m=t.trigger_above,v=t.trigger_below;if(m!=null){let f=v??0,x=m-f||1;l=Math.min(100,Math.max(0,(_-f)/x*100)),c=`${_.toFixed(1)} / ${m} ${a}`}else if(v!=null){let x=e.trigger_entity_info?.max??(v*2||100),b=x-v||1;l=Math.min(100,Math.max(0,(x-_)/b*100)),c=`${_.toFixed(1)} / ${v} ${a}`}else return p}else if(i==="counter"){let _=t.trigger_target_value||1,v=e.trigger_current_delta??null??e.trigger_current_value??null;if(v==null)return p;l=Math.min(100,Math.max(0,v/_*100)),c=`${v.toFixed(1)} / ${_} ${a}`}else if(i==="state_change"){let _=t.trigger_target_changes||1,m=e.trigger_current_value??null;if(m==null)return p;l=Math.min(100,Math.max(0,m/_*100)),c=`${Math.round(m)} / ${_}`}else if(i==="runtime"){let _=t.trigger_runtime_hours||100,m=e.trigger_current_value??null;if(m==null)return p;l=Math.min(100,Math.max(0,m/_*100)),c=`${m.toFixed(1)}h / ${_}h`}else if(i==="compound"){let _=t.compound_logic||t.operator||"AND",m=t.conditions?.length||0;c=`${_} (${m})`,l=e.trigger_active?100:0}else return p;let d=l>=100,g=l>90?"var(--error-color, #f44336)":l>70?"var(--warning-color, #ff9800)":"var(--primary-color)";return n`
      <div class="trigger-progress">
        <div class="trigger-progress-bar">
          <div class="trigger-progress-fill${d?" overflow":""}" style="width:${l}%;background:${g}"></div>
        </div>
        <span class="trigger-progress-label">${c}</span>
      </div>
    `}_renderMiniSparkline(e){if(!e.trigger_config?.entity_id)return p;let t=e.trigger_config.entity_id,i=this._miniStatsData.get(t)||[],a=[];if(i.length>=2)a=i.map(E=>({ts:E.ts,val:E.val}));else{if(!e.history)return p;for(let E of e.history)E.trigger_value!=null&&a.push({ts:new Date(E.timestamp).getTime(),val:E.trigger_value})}if(e.trigger_current_value!=null&&a.push({ts:Date.now(),val:e.trigger_current_value}),a.length<2)return p;a.sort((E,I)=>E.ts-I.ts);let l=Es,c=Ts,d=a.map(E=>E.val),g=Math.min(...d),_=Math.max(...d),m=_-g||1;g-=m*.1,_+=m*.1;let v=a[0].ts,x=a[a.length-1].ts-v||1,b=E=>(E-v)/x*l,j=E=>2+(1-(E-g)/(_-g))*(c-4),z=a;if(z.length>wi){let E=Math.ceil(z.length/wi);z=z.filter((I,V)=>V%E===0||V===z.length-1)}let L=z.map(E=>`${b(E.ts).toFixed(1)},${j(E.val).toFixed(1)}`).join(" ");return n`
      <svg class="mini-sparkline" viewBox="0 0 ${l} ${c}" preserveAspectRatio="none" role="img" aria-label="${s("chart_mini_sparkline",this._lang)}">
        <polyline points="${L}" fill="none" stroke="var(--primary-color)" stroke-width="1.5" stroke-linejoin="round" />
      </svg>
    `}_renderDaysProgress(e){let t=this._lang;if(e.days_until_due==null||!e.interval_days||e.interval_days<=0)return p;let{pct:i,overflow:a}=At(e.interval_days,e.days_until_due,e.interval_unit),l="var(--success-color, #4caf50)";return e.status==="overdue"?l="var(--error-color, #f44336)":e.status==="due_soon"&&(l="var(--warning-color, #ff9800)"),n`
    ...
    `}_renderObjectDetail(){if(!this._selectedEntryId)return p;let e=this._getObject(this._selectedEntryId);if(!e)return n`<p>Object not found.</p>`;let t=e.object,i=this._lang,a=this._isOperator,l=e.tasks.filter(d=>d.archived).length,c=e.tasks.filter(d=>this._showArchived||!d.archived);return n`
      <div class="detail-section">
        ...
        ${t.manufacturer||t.model?n`<p class="meta">${[t.manufacturer,t.model].filter(Boolean).join(" ")}</p>`:p}
        ${t.serial_number?n`<p class="meta">${s("serial_number_label",i)}: ${t.serial_number}</p>`:p}
        ...
        ${t.installation_date?n`<p class="meta">${s("installed",i)}: ${Z(t.installation_date,i)}</p>`:p}
        ...
        ${e.tasks.length===0?n`<div class="empty-state-centered">
            ...
            </div>`:n`<div class="task-table">${[...c].sort((d,g)=>{let _={overdue:0,triggered:1,due_soon:2,ok:3};return(_[d.status]??9)-(_[g.status]??9)||(d.days_until_due??99999)-(g.days_until_due??99999)}).map(d=>n`
              <div class="task-row${d.enabled?"":" task-disabled"}">
                <span class="cell-badges">
                  ...
                  ${d.enabled?p:n`<span class="badge-disabled">${s("disabled",i)}</span>`}
                  ${d.nfc_tag_id?n`<span class="nfc-badge" title="${s("nfc_linked",i)}"><ha-icon icon="mdi:nfc-variant"></ha-icon></span>`:p}
                </span>
                ...
                <span class="due-cell" @click=${()=>this._showTask(e.entry_id,d.id)}>
                  ...
                  ${d.trigger_config?this._renderTriggerProgress(d):p}
                  ${this._renderMiniSparkline(d)}
                </span>
                <span class="row-actions">
                  <mwc-icon-button class="btn-complete" title="${s("complete",i)}" @click=${g=>{g.stopPropagation(),this._openCompleteDialog(e.entry_id,d.id,d.name,this._features.checklists?d.checklist:void 0,this._features.adaptive&&!!d.adaptive_config?.enabled)}}>
                    <ha-icon icon="mdi:check"></ha-icon>
                  </mwc-icon-button>
                  <mwc-icon-button class="btn-skip" title="${s("skip",i)}" .disabled=${this._actionLoading} @click=${g=>{g.stopPropagation(),this._promptSkipTask(e.entry_id,d.id)}}>
                    <ha-icon icon="mdi:skip-next"></ha-icon>
                  </mwc-icon-button>
                </span>
              </div>
    ...
    `}_renderTaskHeader(e){let t=this._lang,a=this._getObject(this._selectedEntryId)?.object.name||"",l=this._isOperator,c=e.archived?"archived":e.is_done?"done":e.status==="due_soon"?"warning":e.status||"ok",d=e.archived?s("archived",t):e.is_done?s("completed",t):s(e.status||"ok",t);return n`
      <div class="task-header">
        <div class="task-header-title">
          ...
          ${e.nfc_tag_id?n`<span class="nfc-badge" title="${s("nfc_tag_id",t)}: ${e.nfc_tag_id}"><ha-icon icon="mdi:nfc-variant"></ha-icon> NFC</span>`:l?p:n`<span class="nfc-badge unlinked" title="${s("nfc_link_hint",t)}"
    ...
    `:p}_renderTabBar(){let e=this._lang;return n`
      <div class="tab-bar">
    ...
    `}_renderChecklistCard(e){if(!this._features.checklists)return p;let t=e.checklist||[];if(t.length===0)return p;let i=this._lang;return n`
    ...
    `}_renderTaskMeta(e){let t=e.documentation_url&&/^https?:\/\//i.test(e.documentation_url)?e.documentation_url:null,i=this._selectedEntryId?this._getObject(this._selectedEntryId):void 0,a=i?.object?.documentation_url,l=a&&/^https?:\/\//i.test(a)?a:null;if(!e.notes&&!t&&!l)return p;let c=this._lang;return n`
    ...
    `}_renderKPIBar(e){let t=this._lang,i=e.times_performed>0?e.total_cost/e.times_performed:0,a=e.days_until_due!==null&&e.days_until_due!==void 0?e.days_until_due<0?"overdue":e.days_until_due<=e.warning_days?"warning":"":"";return n`
      <div class="kpi-bar">
        <div class="kpi-card">
          ...
          ${this._features.schedule_time&&e.schedule_time?n`<div class="kpi-subtext">${s("at_time",t)} ${e.schedule_time}</div>`:p}
        </div>
    ...
    `}_renderRecommendationCard(e){let t=this._lang;if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return p;if(this._selectedEntryId&&this._selectedTaskId&&this._dismissedSuggestions.has(`${this._selectedEntryId}_${this._selectedTaskId}`))return p;let i=e.suggested_interval;return n`
    ...
    `}_renderRecentActivities(e){let t=this._lang,i=e.history.slice(-3).reverse();if(i.length===0)return p;let a=l=>{switch(l){case"completed":return"\u2713";case"triggered":return"\u2297";case"skipped":return"\u21B7";case"reset":return"\u21BA";default:return"\xB7"}};return n`
      <div class="recent-activities">
        ...
        ${i.map(l=>n`
          <div class="activity-item">
            ...
            ${l.cost?n`<span class="activity-badge">${l.cost.toFixed(0)}${this._budget?.currency_symbol||"\u20AC"}</span>`:p}
            ${l.duration?n`<span class="activity-badge">${l.duration}min</span>`:p}
          </div>
        `)}
    ...
    `}_renderHistoryFilters(e){let t=this._lang;return n`
      <div class="history-filters-new">
        <div class="filter-chips">
          ${["completed","skipped","reset","triggered"].map(i=>{let a=e.history.filter(l=>l.type===i).length;return a===0?p:n`
              <span class="filter-chip ${this._historyFilter===i?"active":""}"
                @click=${()=>{this._historyFilter=this._historyFilter===i?null:i}}>
                ${s(i,t)} (${a})
              </span>
            `})}
          ${this._historyFilter?n`<span class="filter-chip clear" @click=${()=>{this._historyFilter=null}}>${s("show_all",t)}</span>`:p}
        </div>
    ...
    `}_renderTaskDetail(){if(!this._selectedEntryId||!this._selectedTaskId)return p;let e=this._getTask(this._selectedEntryId,this._selectedTaskId);if(!e)return n`<p>Task not found.</p>`;let t=this._lang;return n`
      <div class="detail-section">
    ...
    `}_renderHistoryEntry(e){let t=this._lang,i=["completed","reset","skipped"].includes(e.type);return n`
      <div class="history-entry">
        ...
        <div class="history-content">
          ...
          ${e.notes?n`<div>${e.notes}</div>`:p}
          <div class="history-details">
            ${e.cost!=null?n`<span>${s("cost",t)}: ${e.cost.toFixed(2)} ${this._budget?.currency_symbol||"\u20AC"}</span>`:p}
            ${e.duration!=null?n`<span>${s("duration",t)}: ${e.duration} min</span>`:p}
            ${e.trigger_value!=null?n`<span>${s("trigger_val",t)}: ${e.trigger_value}</span>`:p}
          </div>
        </div>
      </div>
