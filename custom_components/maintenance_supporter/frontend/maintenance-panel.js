...
`;function Ne(o){let l=window;l.customCards=l.customCards||[],l.customCards.some(t=>t.type===o.type)||l.customCards.push(o)}var st=class extends O{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._objectFilter="";this._configuredObjects=[];this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(t){if(this._config={...t},t.past_days&&[30,90].includes(t.past_days)?this._pastDays=t.past_days:t.window_days&&[7,14,30,365].includes(t.window_days)&&(this._windowDays=t.window_days,this._pastDays=0),typeof t.user_filter=="string"&&(this._userFilter=t.user_filter),typeof t.object_filter=="string")this._objectFilter=t.object_filter,this._configuredObjects=[];else if(Array.isArray(t.object_filter)){let e=t.object_filter.filter(i=>typeof i=="string"&&i!=="");this._objectFilter=e.length===1?e[0]:"",this._configuredObjects=e.length>1?e:[]}}getCardSize(){return 6}get _lang(){return L(this.hass)}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(t){if(super.updated(t),Lt(this,t),t.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[t,e]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=t.objects,this._stats=e}catch{}}async _subscribe(){try{let t=await this.hass.connection.subscribeMessage(e=>{let i=e;this._objects=i.objects},{type:"maintenance_supporter/subscribe"});if(!this.isConnected){t();return}this._unsub=t}catch{}}_onEventClick(t){if(t.history_timestamp){this._openHistoryEntry(t);return}Re(t.entry_id,t.task_id)||this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:t.entry_id,task_id:t.task_id},bubbles:!0,composed:!0}))}async _openHistoryEntry(t){try{let i=(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:t.entry_id})).tasks?.find(n=>n.id===t.task_id)?.history?.find(n=>n.timestamp===t.history_timestamp);if(!i||Ce({entry_id:t.entry_id,task_id:t.task_id,original_timestamp:t.history_timestamp,type:i.type||"completed",timestamp:i.timestamp||t.history_timestamp,notes:i.notes??null,cost:i.cost??null,duration:i.duration??null,completed_by:i.completed_by??null,used_parts:i.used_parts??null}))return}catch{}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:t.entry_id,task_id:t.task_id,original_timestamp:t.history_timestamp},bubbles:!0,composed:!0}))}render(){if(!this.hass)return d;let t=this._lang,e=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.title,n=null;this._userFilter&&(n=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let p=f=>{let Q=f.toLowerCase();return this._objects.find(it=>it.entry_id===f||it.object.name.toLowerCase()===Q)?.entry_id??null},c=new Set(this._configuredObjects.map(p).filter(f=>f!==null)),h=c.size?this._objects.filter(f=>c.has(f.entry_id)):this._objects,g=this._config.show_object_filter!==!1&&h.length>1,v=this._objectFilter?p(this._objectFilter):null,_=v&&h.some(f=>f.entry_id===v)?h.filter(f=>f.entry_id===v):h,y=new Date;y.setHours(0,0,0,0);let k=this._pastDays>0,j=k?ke(_,y,this._pastDays,n):$e(_,y,this._windowDays,n),M=we(y),E=this._windowDays===365||k,$=E?j.filter(f=>f.events.length>0):j,z=f=>{let Q=`cal-status-${f.status}`,et=f.projected?"cal-event-projected":"",it=f.status==="overdue"&&f.days_until_due!=null?` (${_t(f.days_until_due,t)})`:"",nt=f.projected&&f.interval_days?r`<span class="cal-event-recur">${f.interval_unit&&f.interval_unit!=="days"?`${f.interval_days} ${s("unit_"+f.interval_unit,t)}`:s("cal_every_n_days",t).replace("{n}",String(f.interval_days))}</span>`:d,U=f.schedule_type==="sensor_based",dt=U?r`<ha-icon class="cal-event-icon cal-source-sensor"
  ...
  `}function re(o,l,t){if(!o.trigger_config?.entity_id)return d;let e=o.trigger_config.entity_id,i=l.get(e)||[],a=[];if(i.length>=2)a=i.map(T=>({ts:T.ts,val:T.val}));else{if(!o.history)return d;for(let T of o.history)T.trigger_value!=null&&a.push({ts:new Date(T.timestamp).getTime(),val:T.trigger_value})}if(o.trigger_current_value!=null&&a.push({ts:Date.now(),val:o.trigger_current_value}),a.length<2)return d;a.sort((T,f)=>T.ts-f.ts);let n=gi,p=mi,c=a.map(T=>T.val),h=Math.min(...c),g=Math.max(...c),v=g-h||1;h-=v*.1,g+=v*.1;let _=a[0].ts,k=a[a.length-1].ts-_||1,j=T=>(T-_)/k*n,M=T=>2+(1-(T-h)/(g-h))*(p-4),E=a;if(E.length>Be){let T=Math.ceil(E.length/Be);E=E.filter((f,Q)=>Q%T===0||Q===E.length-1)}let $=E.map(T=>`${j(T.ts).toFixed(1)},${M(T.val).toFixed(1)}`).join(" "),z=o.trigger_active?"var(--error-color, #f44336)":"var(--primary-color)";return r`
    ...
    `}_renderSvg(t,e){let i=this.lang,a=t-ct-gt,n=Qt-_i,p=n-mt,c=1/0,h=-1/0;for(let b of e)c=Math.min(c,b.min??b.val),h=Math.max(h,b.max??b.val);this.thresholdAbove!=null&&(c=Math.min(c,this.thresholdAbove),h=Math.max(h,this.thresholdAbove)),this.thresholdBelow!=null&&(c=Math.min(c,this.thresholdBelow),h=Math.max(h,this.thresholdBelow)),this.targetValue!=null&&(c=Math.min(c,this.targetValue),h=Math.max(h,this.targetValue)),this.forceZero&&(c=Math.min(c,0));let g=(h-c||1)*.06,v=this.forceZero&&c>=0?0:c-g,{ticks:_,niceMin:y,niceMax:k}=Ot(v,h+g,4);this.forceZero&&c>=0&&y<0&&(y=0,_=_.filter(b=>b>=0));let j=e[0].ts,M=this.projection&&this.projection.length===2?this.projection[1].ts:null,E=M!=null?Math.max(e[e.length-1].ts,M):e[e.length-1].ts,$=E-j||1,z=Gt(j,E),T=b=>ct+(b-j)/$*a,f=b=>mt+(1-(b-y)/(k-y||1))*p,Q=e.map(b=>`${T(b.ts).toFixed(1)},${f(b.val).toFixed(1)}`).join(" "),et=`M${T(e[0].ts).toFixed(1)},${n} `+e.map(b=>`L${T(b.ts).toFixed(1)},${f(b.val).toFixed(1)}`).join(" ")+` L${T(e[e.length-1].ts).toFixed(1)},${n} Z`,it="",nt=e.filter(b=>b.min!=null&&b.max!=null);if(nt.length>=2){let b=nt.map(X=>`${T(X.ts).toFixed(1)},${f(X.max).toFixed(1)}`),C=[...nt].reverse().map(X=>`${T(X.ts).toFixed(1)},${f(X.min).toFixed(1)}`);it=`M${b[0]} `+b.slice(1).map(X=>`L${X}`).join(" ")+` L${C.join(" L")} Z`}let U=[];if(this.thresholdBelow!=null){let b=f(this.thresholdBelow);U.push({y:b,h:Math.max(0,n-b),lineY:b,label:`\u25BC ${ut(this.thresholdBelow)}`,labelY:Math.min(n-4,b+13)})}if(this.thresholdAbove!=null){let b=f(this.thresholdAbove);U.push({y:mt,h:Math.max(0,b-mt),lineY:b,label:`\u25B2 ${ut(this.thresholdAbove)}`,labelY:Math.max(mt+11,b-5)})}let dt=e[e.length-1],yt=(this.events||[]).filter(b=>b.ts>=j&&b.ts<=E),xt=Kt(j,E,Math.max(2,Math.min(5,Math.floor(a/110)+1))),B=this._hover;return r`
      <div class="svg-holder">
        ...
        >
          ...
          ${xt.map((b,C)=>{let X=T(b),ei=C===0?"start":C===xt.length-1?"end":"middle";return H`<text x="${X.toFixed(1)}" y="${Qt-5}" text-anchor="${ei}" class="tick-label">${St(b,i,z)}</text>`})}
      ...
      <div class="trigger-limits">
        ...
        ${v==="compound"?r`
          ...
          ${(t.conditions||[]).map((k,j)=>r`
            <span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${j+1}. ${s(k.type||"unknown",e)}: ${k.entity_id?r`<span class="entity-link" @click=${M=>Mt(M,k.entity_id)}>${k.entity_id}</span>`:""}</span>
          `)}
        `:d}
      </div>
  ...
  `}function Ti(o,l,t){let e=o.history.filter(b=>b.type==="completed"&&(b.cost!=null||b.duration!=null)).map(b=>({ts:new Date(b.timestamp).getTime(),cost:b.cost??0,duration:b.duration??0})).sort((b,C)=>b.ts-C.ts);if(e.length<2)return d;let i=e.some(b=>b.cost>0),a=e.some(b=>b.duration>0);if(!i&&!a)return d;let n=t!=="duration"&&i,p=t!=="cost"&&a,c=n||!p&&i,h=p||!n&&a,g=640,v=$i,_=c?44:12,y=h?44:12,k=g-_-y,j=v-ki,M=j-Dt,E=e[0].ts,$=e[e.length-1].ts,z=($-E||864e5)*.05,T=E-z,f=$+z,Q=Gt(E,$),et=b=>_+(b-T)/(f-T)*k,it=Ot(0,Math.max(...e.map(b=>b.cost))||1,3),nt=Ot(0,Math.max(...e.map(b=>b.duration))||1,3),U=b=>Dt+(1-b/(it.niceMax||1))*M,dt=b=>Dt+(1-b/(nt.niceMax||1))*M,yt=e.length>1?Math.min(...e.slice(1).map((b,C)=>et(b.ts)-et(e[C].ts))):k,xt=Math.max(6,Math.min(22,yt*.55)),B=Kt(E,$,Math.max(2,Math.min(4,e.length)));return r`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${g} ${v}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_history",l)}">
        ...
        ${h?nt.ticks.map(b=>{let C=dt(b);return C<Dt-1||C>j+1?d:H`<text x="${g-y+6}" y="${(C+3.5).toFixed(1)}" text-anchor="start" fill="var(--accent-color, #ff9800)" font-size="10.5">${ut(b)}m</text>`}):d}
  ...
  `,u([w({attribute:!1})],ft.prototype,"hass",2),u([w()],ft.prototype,"docId",2),u([m()],ft.prototype,"_url",2),u([m()],ft.prototype,"_failed",2);customElements.get("maintenance-history-photo")||customElements.define("maintenance-history-photo",ft);var ji=["completed","skipped","missed","reset","triggered","trigger_replaced","trigger_removed"];function Qe(o,l){let t=l.lang;return r`
    <div class="history-filters-new">
      <div class="filter-chips">
        ${ji.map(e=>{let i=o.history.filter(a=>a.type===e).length;return i===0?d:r`
  ...
  `}function Ii(o,l){let t=l.lang,e=l.features.adaptive&&o.suggested_interval&&o.suggested_interval!==o.interval_days,i=l.features.seasonal&&o.seasonal_factor&&o.seasonal_factor!==1,a=e||i,n=l.features.adaptive&&o.interval_analysis?.weibull_beta!=null&&o.interval_analysis?.weibull_eta!=null,p=l.features.seasonal&&(o.seasonal_factors?.length===12||o.interval_analysis?.seasonal_factors?.length===12);return r`
    <div class="tab-content overview-tab">
      ...
      ${n?Ze("weibull","weibull_reliability_curve",Te(o,t),l):d}
...
`),`maintenance_parts_${n}.csv`,"text/csv;charset=utf-8")}async _exportObjectsCsv(){try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects/csv"}),e=new Date().toISOString().slice(0,10);Jt(t.csv,`maintenance_objects_${e}.csv`,"text/csv;charset=utf-8")}catch{this._showToast(s("action_error",this._lang))}}_renderObjectsTable(t){let e=this._lang,i=this._objectsTableColumns;return r`
    ...
    `}_renderOverviewRow(t){let e=this._lang,i=t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0,a=0,n=zt.ok,p=!1;if(i&&t.days_until_due!==null){let _=Bt(t.interval_days,t.days_until_due,t.interval_unit);a=_.pct,p=_.overflow,t.status==="overdue"?n=zt.overdue:t.status==="due_soon"&&(n=zt.due_soon)}let c=t.area_id?this.hass?.areas?.[t.area_id]?.name:null,h=t.responsible_user_id?this._userService?.getUserName(t.responsible_user_id):null,g=t.group_names.length>0||c||h,v=this._bulkMode&&this._bulkSelected.has(this._bulkKey(t));return r`
      <div class="task-row${t.enabled?"":" task-disabled"}${v?" bulk-selected":""}">
        ...
        ${this._renderRowActions(e,()=>this._openCompleteDialogForRow(t),()=>this._promptSkipTask(t.entry_id,t.task_id))}
      </div>
    `}_actionStyle(){return this._rowActionStyle}async _dismissRowActionNotice(t){let e={row_action_notice_pending:!1};t&&(e.row_action_style="icons");try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/update",settings:e}),this._rowActionNotice=!1,t&&(this._rowActionStyle="icons")}catch(i){console.warn("[maintenance-supporter] row-action notice update failed",i)}}_renderRowActions(t,e,i){let a=this._actionStyle();return a==="buttons"||a==="buttons_compact"?a==="buttons_compact"&&(this.narrow||this.tight)?r`
          <span class="row-actions as-buttons compact">
            <ha-button size="small" appearance="accent" variant="success" title="${s("complete",t)}" @click=${p=>{p.stopPropagation(),e()}}>
              <ha-icon icon="mdi:check"></ha-icon>
            </ha-button>
            <ha-icon-button .label=${s("skip",t)} title="${s("skip",t)}" .disabled=${this._actionLoading} @click=${p=>{p.stopPropagation(),i()}}>
        ...
        <span class="row-actions as-buttons">
          ...
          <ha-button size="small" appearance="plain" variant="neutral" title="${s("skip",t)}" ?disabled=${this._actionLoading} @click=${p=>{p.stopPropagation(),i()}}>
            <ha-icon slot="start" icon="mdi:skip-next"></ha-icon>${s("skip",t)}
          </ha-button>
        </span>`:r`
      <span class="row-actions">
        <mwc-icon-button class="btn-complete" title="${s("complete",t)}" @click=${n=>{n.stopPropagation(),e()}}>
          <ha-icon icon="mdi:check"></ha-icon>
        </mwc-icon-button>
        <mwc-icon-button class="btn-skip" title="${s("skip",t)}" .disabled=${this._actionLoading} @click=${n=>{n.stopPropagation(),i()}}>
          <ha-icon icon="mdi:skip-next"></ha-icon>
        </mwc-icon-button>
      ...
      <div class="detail-section">
        ...
        ${t.tasks.length===0?r`<div class="empty-state-centered">
              ...
              <div class="task-row${c.enabled?"":" task-disabled"}">
                ...
                ${this._renderRowActions(i,()=>this._openCompleteDialog(t.entry_id,c.id,c.name,this._features.checklists?c.checklist:void 0,this._features.adaptive&&!!c.adaptive_config?.enabled),()=>this._promptSkipTask(t.entry_id,c.id))}
              </div>
            `)}</div>`}
