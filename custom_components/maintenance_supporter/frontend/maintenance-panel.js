...
`;var Ui={days:1,weeks:7,months:30.4368,years:365.25};function vt(o,r){return!o||o<=0?0:o*(Ui[r||"days"]??1)}function bt(o,r,e){let t=vt(o,e);if(t<=0||r==null)return{pct:0,overflow:!1};let i=(t-r)/t*100;return{pct:Math.max(0,Math.min(100,i)),overflow:i>100}}function ft(o,r=new Date){if(!o)return{kind:"none",days:null,date:null};let e=new Date(`${o}T00:00:00`);if(isNaN(e.getTime()))return{kind:"none",days:null,date:null};let t=Date.UTC(r.getFullYear(),r.getMonth(),r.getDate()),i=Date.UTC(e.getFullYear(),e.getMonth(),e.getDate()),a=Math.round((i-t)/864e5);return a<0?{kind:"expired",days:a,date:o}:a<=60?{kind:"expiring",days:a,date:o}:{kind:"valid",days:a,date:o}}var be=[{key:"name",labelKey:"name",required:!0},{key:"manufacturer",labelKey:"manufacturer"},{key:"model",labelKey:"model"},{key:"serial_number",labelKey:"serial_number_label"},{key:"installation_date",labelKey:"installed"},{key:"warranty_expiry",labelKey:"warranty"},{key:"area_id",labelKey:"area"},{key:"documentation_url",labelKey:"documentation_url_label"},{key:"notes",labelKey:"object_notes_label"},{key:"task_count",labelKey:"tasks"},{key:"actions",labelKey:"actions"}],Bi=be.map(o=>o.key),Be=["name","manufacturer","model","serial_number","installation_date","warranty_expiry","area_id","task_count","actions"];function Ce(o){if(!Array.isArray(o))return[...Be];let r=new Set,e=[];for(let t of o)typeof t=="string"&&Bi.includes(t)&&!r.has(t)&&(r.add(t),e.push(t));return e.length?(e.includes("name")||e.unshift("name"),e):[...Be]}function We(o,r,e){let t=new Blob([o],{type:e}),i=URL.createObjectURL(t),a=document.createElement("a");a.href=i,a.download=r,a.target="_blank",a.rel="noopener",a.style.display="none",document.body.appendChild(a),a.dispatchEvent(new MouseEvent("click")),document.body.removeChild(a),setTimeout(()=>URL.revokeObjectURL(i),6e4)}function Ve(o,r){let e=document.createElement("a");e.href=o,e.download=r,e.target="_blank",e.rel="noopener",e.style.display="none",document.body.appendChild(e),e.dispatchEvent(new MouseEvent("click")),document.body.removeChild(e)}var Vt=j`
  :host {
    display: block;
    height: 100%;
...
`;var Ke=class{constructor(r){this._cache=new Map;this._pending=new Map;this._hass=r}updateHass(r){this._hass=r}async getDetailStats(r,e){return this._getStats(r,"hour",30,e)}async getMiniStats(r,e){return this._getStats(r,"day",14,e)}async getBatchMiniStats(r){let e=new Map,t=[];for(let u of r){let g=`${u.entityId}:day`,m=this._cache.get(g);m&&Date.now()-m.fetchedAt<3e5?e.set(u.entityId,m.points):t.push(u)}if(t.length===0)return e;let i=t.filter(u=>u.isCounter).map(u=>u.entityId),a=t.filter(u=>!u.isCounter).map(u=>u.entityId),l=new Date(Date.now()-336*60*60*1e3).toISOString(),c=[];return i.length>0&&c.push(this._fetchBatch(i,"day",l,["state","sum","change"],!0,e)),a.length>0&&c.push(this._fetchBatch(a,"day",l,["mean","min","max"],!1,e)),await Promise.all(c),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(r,e,t,i){let a=`${r}:${e}`,l=this._cache.get(a);if(l&&Date.now()-l.fetchedAt<3e5)return l.points;if(this._pending.has(a))return this._pending.get(a);let c=this._fetchAndNormalize(r,e,t,i,a);this._pending.set(a,c);try{return await c}finally{this._pending.delete(a)}}async _fetchAndNormalize(r,e,t,i,a){let l=new Date(Date.now()-t*24*60*60*1e3).toISOString(),c=i?["state","sum","change"]:["mean","min","max"];try{let g=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:l,statistic_ids:[r],period:e,types:c}))[r]||[],m=this._normalizeRows(g,i);return this._cache.set(a,{entityId:r,fetchedAt:Date.now(),period:e,points:m}),m}catch(u){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${r}:`,u),[]}}async _fetchBatch(r,e,t,i,a,l){try{let c=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:r,period:e,types:i});for(let u of r){let g=c[u]||[],m=this._normalizeRows(g,a);l.set(u,m),this._cache.set(`${u}:${e}`,{entityId:u,fetchedAt:Date.now(),period:e,points:m})}}catch(c){console.warn("[maintenance-supporter] Batch statistics fetch failed:",c)}}_normalizeRows(r,e){let t=[];for(let i of r){let a=null;if(e?a=i.state??null:a=i.mean??null,a===null)continue;let l={ts:i.start,val:a};e||(i.min!=null&&(l.min=i.min),i.max!=null&&(l.max=i.max)),t.push(l)}return t.sort((i,a)=>i.ts-a.ts),t}};var de=class{constructor(r){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=r}updateHass(r){this.hass=r}async getUsers(r=!1){let e=Date.now();if(!r&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(r,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:r,task_id:e,user_id:t})}async getTasksByUser(r){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:r})).tasks}getUserName(r){return!r||!this.usersCache?null:this.usersCache.find(t=>t.id===r)?.name||null}getUser(r){return!r||!this.usersCache?null:this.usersCache.find(e=>e.id===r)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(r){return r?r===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var Wi={name:"name",task_type:"maintenance_type",schedule_type:"schedule_type",interval_days:"interval_days",interval_anchor:"interval_anchor",warning_days:"warning_days",last_performed:"last_performed_optional",notes:"notes_optional",documentation_url:"documentation_url_optional",custom_icon:"custom_icon_optional",nfc_tag_id:"nfc_tag_id_optional",responsible_user_id:"responsible_user",entity_slug:"entity_slug",entity_id:"entity_id",area_id:"area_id_optional",manufacturer:"manufacturer_optional",model:"model_optional",serial_number:"serial_number_optional",installation_date:"installation_date_optional",warranty_expiry:"warranty_expiry_optional",checklist:"checklist_steps_optional",reason:"reason",feedback:"feedback",cost:"cost",duration:"duration",description:"description_optional",group_name:"name",group_description:"description_optional",environmental_entity:"environmental_entity_optional",environmental_attribute:"environmental_attribute_optional",trigger_above:"trigger_above",trigger_below:"trigger_below",trigger_for_minutes:"trigger_for_minutes"};function Vi(o,r){let e=Wi[o];if(!e)return o;let t=s(e,r);return t&&t!==e?t:o}function Ki(o){let e=o.match(/data\['([^']+)'\]/)?.[1],t;return(t=o.match(/length of value must be at most (\d+)/))?{field:e,rule:"too_long",param:t[1]}:(t=o.match(/length of value must be at least (\d+)/))?{field:e,rule:"too_short",param:t[1]}:(t=o.match(/value must be at most (\S+)/))?{field:e,rule:"value_too_high",param:t[1]}:(t=o.match(/value must be at least (\S+)/))?{field:e,rule:"value_too_low",param:t[1]}:/required key not provided/.test(o)?{field:e,rule:"required"}:(t=o.match(/expected (\w+)/))?{field:e,rule:"wrong_type",param:t[1]}:/value must be one of/.test(o)?{field:e,rule:"invalid_choice"}:/not a valid value/.test(o)?{field:e,rule:"invalid_value"}:{field:e,rule:"unknown"}}function L(o,r,e){if(typeof o=="string")return o;if(typeof o!="object"||o===null)return e;let t=o,i=t.message||t.error?.message||"";if(!i)return e;let a=Ki(i),l=a.field?Vi(a.field,r):"",c=u=>s(u,r).replace("{field}",l).replace("{n}",a.param??"");switch(a.rule){case"too_long":return c("err_too_long");case"too_short":return c("err_too_short");case"value_too_high":return c("err_value_too_high");case"value_too_low":return c("err_value_too_low");case"required":return c("err_required");case"wrong_type":return c("err_wrong_type").replace("{type}",a.param??"");case"invalid_choice":return c("err_invalid_choice");case"invalid_value":return c("err_invalid_value");default:return i||e}}var V=class extends A{constructor(){super(...arguments);this.label="";this.value="";this.placeholder="";this.type="text";this.required=!1;this.disabled=!1}_onInput(e){let t=e.target.value;this.value=t,this.dispatchEvent(new CustomEvent("input",{bubbles:!0,composed:!0,detail:{value:t}}))}render(){return n`
      <label class="field">
        ${this.label?n`<span class="label">${this.label}${this.required?n`<span class="req">*</span>`:d}</span>`:d}
        <input
          .value=${this.value??""}
          .type=${this.type}
        ...
        />
        ${this.helper?n`<span class="helper">${this.helper}</span>`:d}
      </label>
    `}};V.styles=j`
    :host { display: block; }
    .field {
  ...
  `,p([w({attribute:!1})],B.prototype,"hass",2),p([h()],B.prototype,"_open",2),p([h()],B.prototype,"_loading",2),p([h()],B.prototype,"_error",2),p([h()],B.prototype,"_name",2),p([h()],B.prototype,"_manufacturer",2),p([h()],B.prototype,"_model",2),p([h()],B.prototype,"_serialNumber",2),p([h()],B.prototype,"_areaId",2),p([h()],B.prototype,"_installationDate",2),p([h()],B.prototype,"_warrantyExpiry",2),p([h()],B.prototype,"_documentationUrl",2),p([h()],B.prototype,"_notes",2),p([h()],B.prototype,"_entryId",2);customElements.get("maintenance-object-dialog")||customElements.define("maintenance-object-dialog",B);function pe(o){let r=o??0;return r<1024?`${r} B`:r<1024*1024?`${(r/1024).toFixed(1)} KB`:`${(r/(1024*1024)).toFixed(1)} MB`}var Ye=["manual","warranty","invoice","spare_parts","photo","other"],Yi={manual:"mdi:book-open-variant",warranty:"mdi:shield-check",invoice:"mdi:receipt-text-outline",spare_parts:"mdi:cog-outline",photo:"mdi:image-outline",other:"mdi:file-document-outline"},q=class extends A{constructor(){super(...arguments);this.canWrite=!1;this._docs=[];this._loaded=!1;this._busy=!1;this._error="";this._hint="";this._addingLink=!1;this._linkUrl="";this._linkTitle="";this._category="manual";this._thumbs={};this._lightboxUrl="";this._editingId="";this._editTitle="";this._editCategory="manual";this._dragOver=!1;this._loadedFor=null;this._localeReady=!1}_isImage(e){return e.kind==="file"&&(e.mime||"").startsWith("image/")}async _sign(e){return(await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300})).path}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),this.hass&&!this._localeReady&&(this._localeReady=!0,ie(this._lang).then(()=>this.requestUpdate())),this.hass&&this.entryId&&this._loadedFor!==this.entryId&&(this._loadedFor=this.entryId,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:this.entryId});this._docs=e.documents||[],this._loaded=!0,this._error="",this._thumbs={},this._loadThumbs()}catch(e){this._error=L(e,this._lang),this._loaded=!0}}async _loadThumbs(){await Promise.all(this._docs.filter(e=>this._isImage(e)).map(async e=>{try{let t=await this._sign(e);this._thumbs={...this._thumbs,[e.id]:t}}catch{}}))}_category_of(e){return(e.tags||[]).find(i=>Ye.includes(i))||"other"}_onFileInput(e){let t=e.target,i=Array.from(t.files??[]);i.length&&this._uploadFiles(i),t.value=""}_onCameraInput(e){let t=e.target,i=Array.from(t.files??[]);i.length&&this._uploadFiles(i,"photo"),t.value=""}_onDrop(e){if(e.preventDefault(),this._dragOver=!1,!this.canWrite||this._busy)return;let t=Array.from(e.dataTransfer?.files??[]);t.length&&this._uploadFiles(t)}_onDragOver(e){this.canWrite&&(e.preventDefault(),this._dragOver=!0)}_onDragLeave(e){let t=e.relatedTarget;(!t||!e.currentTarget.contains(t))&&(this._dragOver=!1)}async _uploadFiles(e,t){let i=t??this._category;this._busy=!0,this._error="",this._hint="";let a=0,l=0;try{for(let c of e){let u=new FormData;u.append("entry_id",this.entryId),u.append("tags",i),u.append("file",c,c.name);let g=await fetch("/api/maintenance_supporter/document/upload",{method:"POST",headers:{Authorization:`Bearer ${this.hass.auth?.data?.access_token??""}`},body:u});if(!g.ok){this._error=g.status===413?s("doc_too_large",this._lang):s("doc_upload_failed",this._lang);continue}let m=await g.json();m.duplicate_in_object?l++:m.deduped&&a++}l?this._hint=s("doc_dup_in_object",this._lang):a&&(this._hint=s("doc_deduped",this._lang)),await this._load()}catch{this._error=s("doc_upload_failed",this._lang)}finally{this._busy=!1}}async _download(e){try{let t=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:30});Ve(t.path,e.filename||e.title||"document")}catch(t){this._error=L(t,this._lang)}}async _preview(e){if(this._isImage(e)){this._lightboxUrl=this._thumbs[e.id]||await this._sign(e);return}let t=window.open("about:blank","_blank");try{let i=await this._sign(e);t&&(t.location.href=i)}catch(i){t&&t.close(),this._error=L(i,this._lang)}}_startEdit(e){this._editingId=e.id,this._editTitle=e.title||"",this._editCategory=this._category_of(e),this._addingLink=!1,this._error=""}_cancelEdit(){this._editingId=""}async _saveEdit(e){let t=(e.tags||[]).filter(a=>!Ye.includes(a)),i=e.kind==="file"?[this._editCategory,...t]:e.tags??[];this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,title:this._editTitle.trim()||e.filename||e.url||"",tags:i}),this._editingId="",await this._load()}catch(a){this._error=L(a,this._lang)}finally{this._busy=!1}}async _delete(e){let t=e.title||e.filename||e.url||"";if(window.confirm(s("doc_delete_confirm",this._lang).replace("{name}",t))){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/delete",doc_id:e.id}),await this._load()}catch(i){this._error=L(i,this._lang)}finally{this._busy=!1}}}async _addLink(){let e=this._linkUrl.trim();if(e){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/add_link",entry_id:this.entryId,url:e,title:this._linkTitle.trim()||null}),this._linkUrl="",this._linkTitle="",this._addingLink=!1,await this._load()}catch(t){this._error=L(t,this._lang,s("doc_link_invalid",this._lang))}finally{this._busy=!1}}}render(){let e=this._lang;return n`
      <div
      ...
      <div class="doc-header">
        ...
        ${this.canWrite?n`
              <div class="doc-actions">
                ...
                >
                  ${Ye.map(t=>n`<option value=${t}>${s(`doc_cat_${t}`,e)}</option>`)}
                </select>
                ...
                <label class="btn camera-btn ${this._busy?"disabled":""}" title=${s("doc_camera",e)}>
      ...
      ${this._loaded?this._docs.length===0?n`<div class="doc-empty">${s("documents_empty",e)}</div>`:n`
              <div class="doc-list">
                ${this._docs.map(t=>this._renderDoc(t,e))}
              </div>
            `:n`<div class="doc-empty">${s("loading",e)}</div>`}
    ...
    `}_renderDoc(e,t){if(this._editingId===e.id)return this._renderEdit(e,t);let i=e.kind==="file",a=this._category_of(e),l=i?`${s(`doc_cat_${a}`,t)} \xB7 ${pe(e.size)}`:s("doc_link_badge",t),c=this._thumbs[e.id];return n`
      <div class="doc-row">
        ${i&&c?n`<img
              class="doc-thumb"
              src=${c}
    ...
    `}_renderEdit(e,t){let i=e.kind==="file";return n`
      <div class="doc-row editing">
        ...
        ${i?n`<select
            ...
            >
              ${Ye.map(a=>n`<option value=${a} ?selected=${a===this._editCategory}>${s(`doc_cat_${a}`,t)}</option>`)}
            </select>`:d}
  ...
  `,p([w({attribute:!1})],q.prototype,"hass",2),p([w({attribute:!1})],q.prototype,"entryId",2),p([w({type:Boolean})],q.prototype,"canWrite",2),p([h()],q.prototype,"_docs",2),p([h()],q.prototype,"_loaded",2),p([h()],q.prototype,"_busy",2),p([h()],q.prototype,"_error",2),p([h()],q.prototype,"_hint",2),p([h()],q.prototype,"_addingLink",2),p([h()],q.prototype,"_linkUrl",2),p([h()],q.prototype,"_linkTitle",2),p([h()],q.prototype,"_category",2),p([h()],q.prototype,"_thumbs",2),p([h()],q.prototype,"_lightboxUrl",2),p([h()],q.prototype,"_editingId",2),p([h()],q.prototype,"_editTitle",2),p([h()],q.prototype,"_editCategory",2),p([h()],q.prototype,"_dragOver",2);customElements.get("maintenance-documents-section")||customElements.define("maintenance-documents-section",q);var Gi=["manual","warranty","invoice","spare_parts","photo","other"],Qi={manual:"mdi:book-open-variant",warranty:"mdi:shield-check",invoice:"mdi:receipt-text-outline",spare_parts:"mdi:cog-outline",photo:"mdi:image-outline",other:"mdi:file-document-outline"},Y=class extends A{constructor(){super(...arguments);this.canWrite=!1;this._docs=[];this._loaded=!1;this._busy=!1;this._error="";this._attachId="";this._loadedKey="";this._localeReady=!1}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),this.hass&&!this._localeReady&&(this._localeReady=!0,ie(this._lang).then(()=>this.requestUpdate()));let t=`${this.entryId}|${this.taskId}`;this.hass&&this.entryId&&this.taskId&&this._loadedKey!==t&&(this._loadedKey=t,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:this.entryId});this._docs=e.documents||[],this._loaded=!0,this._error=""}catch(e){this._error=L(e,this._lang),this._loaded=!0}}_linked(){return this._docs.filter(e=>(e.task_ids||[]).includes(this.taskId))}_available(){return this._docs.filter(e=>!(e.task_ids||[]).includes(this.taskId))}async _setTaskIds(e,t){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,task_ids:t}),await this._load()}catch(i){this._error=L(i,this._lang)}finally{this._busy=!1}}_link(){let e=this._docs.find(t=>t.id===this._attachId);e&&(this._attachId="",this._setTaskIds(e,[...e.task_ids||[],this.taskId]))}_unlink(e){this._setTaskIds(e,(e.task_ids||[]).filter(t=>t!==this.taskId))}async _open(e){if(e.kind==="weblink"){window.open(e.url,"_blank","noopener");return}let t=window.open("about:blank","_blank");try{let i=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300});t&&(t.location.href=i.path)}catch(i){t&&t.close(),this._error=L(i,this._lang)}}async _download(e){try{let t=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:30});Ve(t.path,e.filename||e.title||"document")}catch(t){this._error=L(t,this._lang)}}render(){if(!this._loaded||this._docs.length===0)return d;let e=this._lang,t=this._linked(),i=this._available();return n`
      <div class="task-docs">
        <h3><ha-icon icon="mdi:paperclip"></ha-icon> ${s("documents",e)} (${t.length})</h3>
        ${this._error?n`<div class="tdoc-error">${this._error}</div>`:d}
        ${t.length===0?n`<div class="tdoc-empty">${s("doc_task_none",e)}</div>`:n`<div class="tdoc-list">${t.map(a=>this._renderRow(a,e))}</div>`}
        ${this.canWrite&&i.length?n`<div class="tdoc-attach">
              ...
              >
                ...
                ${i.map(a=>n`<option value=${a.id} ?selected=${a.id===this._attachId}>${a.title||a.filename||a.url}</option>`)}
              </select>
    ...
    `}_renderRow(e,t){let i=e.kind==="file",a=(e.tags||[]).find(l=>Gi.includes(l))||"other";return n`
      <div class="tdoc-row">
        <ha-icon class="tdoc-icon" icon=${i?Qi[a]:"mdi:link-variant"}></ha-icon>
        <div class="tdoc-info">
          <div class="tdoc-title">${e.title||e.filename||e.url}</div>
          <div class="tdoc-meta">${i?pe(e.size):s("doc_link_badge",t)}</div>
        </div>
        <button class="icon-btn" title=${s("doc_open",t)} @click=${()=>this._open(e)}>
          <ha-icon icon=${i?"mdi:eye-outline":"mdi:open-in-new"}></ha-icon>
        </button>
        ${i?n`<button class="icon-btn" title=${s("doc_download",t)} @click=${()=>this._download(e)}>
              <ha-icon icon="mdi:download"></ha-icon>
            </button>`:d}
        ${this.canWrite?n`<button class="icon-btn" title=${s("doc_unlink",t)} ?disabled=${this._busy} @click=${()=>this._unlink(e)}>
...
`),this._scheduleTime=t.schedule_time||"";let a=t.on_complete_action;if(a&&a.service){this._actionService=a.service;let u=a.target?.entity_id;this._actionTargetEntity=Array.isArray(u)?u[0]||"":u||"",this._actionData=a.data&&typeof a.data=="object"?{...a.data}:{},this._actionDataJsonFallback=""}else this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="";let l=t.quick_complete_defaults;this._qcNotes=l?.notes||"",this._qcCost=l?.cost!=null?String(l.cost):"",this._qcDuration=l?.duration!=null?String(l.duration):"",this._qcFeedback=l?.feedback||"";let c=t.adaptive_config||{};if(this._environmentalEntity=c.environmental_entity||"",this._environmentalAttribute=c.environmental_attribute||"",this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute,t.trigger_config){let u=t.trigger_config;this._triggerEntityId=u.entity_id||"",this._triggerEntityIds=u.entity_ids||(u.entity_id?[u.entity_id]:[]),this._triggerEntityLogic=u.entity_logic||"any",this._triggerAttribute=u.attribute||"",this._triggerType=u.type||"threshold",this._triggerAbove=u.trigger_above?.toString()||"",this._triggerBelow=u.trigger_below?.toString()||"",this._triggerForMinutes=u.trigger_for_minutes?.toString()||"0",this._triggerTargetValue=u.trigger_target_value?.toString()||"",this._triggerDeltaMode=u.trigger_delta_mode||!1,this._triggerFromState=u.trigger_from_state||"",this._triggerToState=u.trigger_to_state||"",this._triggerTargetChanges=u.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=u.trigger_runtime_hours?.toString()||""}else this._resetTriggerFields();this._triggerEntityId&&this._fetchEntityAttributes(this._triggerEntityId),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._intervalUnit="days",this._dueDate="",this._warningDays=String(this.defaultWarningDays),this._intervalAnchor="completion",this._weekdays=[],this._nth="1",this._nthWeekday="5",this._domDay="1",this._notes="",this._documentationUrl="",this._customIcon="",this._enabled=!0,this._lastPerformed="",this._nfcTagId="",this._responsibleUserId=null,this._checklistText="",this._scheduleTime="",this._environmentalEntity="",this._environmentalAttribute="",this._environmentalInitial="",this._environmentalAttributeInitial="",this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="",this._actionTesting=!1,this._actionTestResult="",this._qcNotes="",this._qcCost="",this._qcDuration="",this._qcFeedback="",this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerForMinutes="0",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours=""}async _loadUsers(){this._userService||(this._userService=new de(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}async _testAction(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e)){this._actionTestResult="error",this._actionTestError="Invalid service format (expected 'domain.service')",setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3);return}let[t,i]=e.split(".");if(!this.hass?.services?.[t]?.[i]){this._actionTestResult="error",this._actionTestError=`Service "${e}" is not registered in Home Assistant. Check spelling and that the integration providing it is loaded.`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}let a=this._actionTargetEntity.trim();if(a){let l=a.split(".")[0];if(l!==t&&!new Set(["homeassistant","scene","notify","persistent_notification"]).has(t)){this._actionTestResult="error",this._actionTestError=`Service "${e}" only works on ${t}.* entities; entity "${a}" is in ${l}.* \u2014 pick a service that matches the entity domain (e.g. ${l}.${i})`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}if(!this.hass.states?.[a]){this._actionTestResult="error",this._actionTestError=`Target entity "${a}" not found in Home Assistant \u2014 the entity may have been renamed or its integration removed.`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}}this._actionTestResult="ok",setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3)}_buildActionData(){if(this._actionDataJsonFallback.trim())try{let e=JSON.parse(this._actionDataJsonFallback);if(e&&typeof e=="object"&&!Array.isArray(e))return e}catch{}return{...this._actionData}}_serviceSchema(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e))return null;let[t,i]=e.split("."),a=this.hass?.services?.[t]?.[i]?.fields;return!a||Object.keys(a).length===0?null:Object.entries(a).map(([l,c])=>({name:l,required:!!c.required,selector:c.selector||{text:{}}}))}_renderCompletionActionsSection(e){if(!this.completionActionsEnabled)return d;let t=this._serviceSchema();return n`
      <details class="ca-section">
        ...
        <div class="ca-test-row">
          ...
          ${this._actionTestResult==="ok"?n`<span class="ca-test-ok">${s("on_complete_action_test_success",e)}</span>`:d}
          ${this._actionTestResult==="error"?n`<div class="ca-test-error-block">
                ...
                ${this._actionTestError?n`<div class="ca-test-error-detail">${this._actionTestError}</div>`:d}
              </div>`:d}
        </div>
      </details>
...
`).map(c=>c.trim()).filter(Boolean).slice(0,100);e.checklist=l.length?l:null}if(this.completionActionsEnabled){let l=this._actionService.trim();if(l&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(l)){let m={service:l},_=this._actionTargetEntity.trim();_&&(m.target={entity_id:_});let v=this._buildActionData();Object.keys(v).length>0&&(m.data=v),e.on_complete_action=m}else e.on_complete_action=null;let c={};this._qcNotes.trim()&&(c.notes=this._qcNotes.trim());let u=parseFloat(this._qcCost);!isNaN(u)&&u>=0&&(c.cost=u);let g=parseInt(this._qcDuration,10);!isNaN(g)&&g>=0&&(c.duration=g),this._qcFeedback&&(c.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(c).length?c:null}let t=await this.hass.connection.sendMessagePromise(e),i=this._taskId||t?.task_id,a=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(i&&this._scheduleType==="sensor_based"&&a)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:i,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=L(e,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return d;let e=this._lang;return n`
      ...
      <ms-textfield
        ...
        @input=${t=>{let a=t.target.value.split(",").map(l=>l.trim()).filter(Boolean);this._triggerEntityId=a[0]||"",this._triggerEntityIds=a,a[0]&&this._fetchEntityAttributes(a[0])}}
      ></ms-textfield>
      ...
      ${this._availableAttributes.length>0?n`
          <div class="select-row">
            ...
            >
              ...
              ${this._suggestedAttributes.map(t=>n`<option value=${t} ?selected=${t===this._triggerAttribute}>${t} ★</option>`)}
              ${this._availableAttributes.filter(t=>!this._suggestedAttributes.includes(t.name)).map(t=>n`<option value=${t.name} ?selected=${t.name===this._triggerAttribute}>${t.name}${t.numeric?"":" (non-numeric)"}</option>`)}
            </select>
          </div>
        `:n`
          <ms-textfield
      ...
      <div class="select-row">
        ...
        >
          ${es.map(t=>n`<option value=${t} ?selected=${t===this._triggerType}>${s(t,e)}</option>`)}
        </select>
      </div>
    ...
    `}_renderUnitSelect(){let e=this._lang;return n`
      <div class="select-row">
        ...
        <div class="weekday-chips">
          ${t.map((i,a)=>n`
            ...
            >${i}</button>`)}
        </div>`;if(this._scheduleType==="nth_weekday"){let i=[["1",s("ord_1",e)],["2",s("ord_2",e)],["3",s("ord_3",e)],["4",s("ord_4",e)],["5",s("ord_5",e)],["-1",s("ord_last",e)]];return n`
        <div class="select-row">
          <label>${s("recurrence_occurrence",e)}</label>
          <select .value=${this._nth} @change=${a=>this._nth=a.target.value}>
            ${i.map(([a,l])=>n`<option value=${a} ?selected=${a===this._nth}>${l}</option>`)}
          </select>
        </div>
        <div class="select-row">
          <label>${s("recurrence_weekday",e)}</label>
          <select .value=${this._nthWeekday} @change=${a=>this._nthWeekday=a.target.value}>
            ${t.map((a,l)=>n`<option value=${String(l)} ?selected=${String(l)===this._nthWeekday}>${a}</option>`)}
          </select>
        </div>`}return this._scheduleType==="day_of_month"?n`
        <ms-textfield
        ...
        ></ms-textfield>`:d}_renderTriggerTypeFields(){let e=this._lang;return this._triggerType==="threshold"?n`
        <ms-textfield
      ...
      `:this._triggerType==="runtime"?n`
        ...
        ></ms-textfield>
      `:d}render(){if(!this._open)return n``;let e=this._lang,t=this._taskId?s("edit_task",e):s("new_task",e);return n`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
          ${this._error?n`<div class="error">${this._error}</div>`:d}
          ...
          <div class="select-row">
            ...
            >
              ${Ji.map(i=>n`<option value=${i} ?selected=${i===this._type}>${s(i,e)}</option>`)}
            </select>
          </div>
          <div class="select-row">
            ...
            >
              ${Zi.map(i=>n`<option value=${i} ?selected=${i===this._scheduleType}>${s(i,e)}</option>`)}
            </select>
          </div>
...
<div class="qr-row">
  <div class="qr-col">
    <img src="${Kt(this._viewResult.svg_data_uri)}" alt="QR Info" />
    <div class="qr-label">${m}</div>
  </div>
  ${g?`<div class="qr-col">
    <img src="${Kt(this._completeResult.svg_data_uri)}" alt="QR Complete" />
    <div class="qr-label">${_}</div>
  </div>`:""}
</div>
<div class="url">${fe(this._viewResult.url)}</div>
<script>setTimeout(()=>window.print(),300)<\/script>
</body></html>`),a.document.close()}_downloadSvg(e,t){let i=decodeURIComponent(e.svg_data_uri.replace("data:image/svg+xml,","")),a=new Blob([i],{type:"image/svg+xml"}),l=URL.createObjectURL(a),c=document.createElement("a");c.href=l;let u=this._taskName?`${this._objectName}-${this._taskName}`:this._objectName;c.download=`qr-${is(u)}-${t}.svg`,c.click(),URL.revokeObjectURL(l)}_close(){this._open=!1,this._viewResult=null,this._completeResult=null,this._error="",this._loading=!1}render(){if(!this._open)return n``;let e=this.lang||this.hass?.language||"en",t=this._taskName?`${s("qr_code",e)}: ${this._objectName} \u2014 ${this._taskName}`:`${s("qr_code",e)}: ${this._objectName}`,i=!!this._viewResult;return n`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
          ${this._loading?n`<div class="loading">${s("qr_generating",e)}</div>`:this._error?n`<div class="error">${this._error}</div>`:i?n`
                    <div class="qr-pair">
                      <div class="qr-item">
                        <img
  ...
  `,p([w({attribute:!1})],X.prototype,"hass",2),p([w()],X.prototype,"lang",2),p([h()],X.prototype,"_open",2),p([h()],X.prototype,"_loading",2),p([h()],X.prototype,"_error",2),p([h()],X.prototype,"_viewResult",2),p([h()],X.prototype,"_completeResult",2),p([h()],X.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",X);var Yt=5;function Ie(o){let r=o.getFullYear(),e=String(o.getMonth()+1).padStart(2,"0"),t=String(o.getDate()).padStart(2,"0");return`${r}-${e}-${t}`}function ss(o,r){let e=[];for(let t=0;t<r;t++){let i=new Date(o);i.setDate(i.getDate()+t),i.setHours(0,0,0,0),e.push(Ie(i))}return e}function Ge(o,r){let[e,t,i]=o.split("-").map(Number),a=new Date(e,t-1,i);return a.setDate(a.getDate()+r),Ie(a)}function as(o){if(!o||o.length===0)return null;let r=o.map(e=>e.cost).filter(e=>typeof e=="number");return r.length===0?null:r.reduce((e,t)=>e+t,0)/r.length}function rs(o){let{windowStart:r,windowEnd:e,task:t,entryId:i,objectName:a}=o,l=[],c=(_,v)=>({date:_,entry_id:i,task_id:t.id,task_name:t.name,object_name:a,status:v&&(t.status==="overdue"||t.status==="triggered")?"ok":t.status,days_until_due:v?null:t.days_until_due??null,projected:v,schedule_type:t.schedule_type,interval_days:t.interval_days??null,interval_unit:t.interval_unit??null,responsible_user_id:t.responsible_user_id??null,avg_cost:as(t.history),adaptive_enabled:!!t.adaptive_config?.enabled,prediction_confidence:t.threshold_prediction_confidence??null}),u=Math.max(1,Math.round(vt(t.interval_days,t.interval_unit)));if(t.status==="overdue"||t.status==="triggered"){if(l.push(c(r,!1)),t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let _=Ge(r,u),v=1;for(;_<=e&&v<Yt;)l.push(c(_,!0)),v++,_=Ge(_,u)}return l}let g=t.next_due;if(typeof g!="string"||!g)return l;let m=g.slice(0,10);if(m>=r&&m<=e)l.push(c(m,!1));else if(m>e)return l;if(t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let _=Ge(m,u),v=l.length;for(;_<=e&&v<Yt;)_>=r&&(l.push(c(_,!0)),v++),_=Ge(_,u)}return l}var Gt={overdue:0,triggered:1,due_soon:2,ok:3};function Qt(o,r,e,t=null){let i=ss(r,e),a=i[0],l=i[i.length-1],c=[];for(let g of o){let m=g.object?.name||"",_=g.entry_id,v=g.tasks||[];for(let b of v){if(t&&b.responsible_user_id!==t||b.enabled===!1)continue;let x=rs({windowStart:a,windowEnd:l,task:b,entryId:_,objectName:m});c.push(...x)}}let u=new Map;for(let g of i)u.set(g,[]);for(let g of c){let m=u.get(g.date);m&&m.push(g)}for(let[,g]of u)g.sort((m,_)=>{let v=Gt[m.status]??99,b=Gt[_.status]??99;if(v!==b)return v-b;if(m.projected!==_.projected)return m.projected?1:-1;let x=m.object_name.localeCompare(_.object_name);return x!==0?x:m.task_name.localeCompare(_.task_name)});return i.map(g=>({date:g,events:u.get(g)??[]}))}var ns={completed:"ok",reset:"ok",skipped:"due_soon",triggered:"triggered",trigger_replaced:"triggered"};function os(o,r){let e=[];for(let t=r-1;t>=0;t--){let i=new Date(o);i.setDate(i.getDate()-t),i.setHours(0,0,0,0),e.push(Ie(i))}return e}function Jt(o,r,e,t=null){let i=os(r,e),a=i[0],l=i[i.length-1],c=new Map;for(let g of i)c.set(g,[]);for(let g of o){let m=g.object?.name||"",_=g.entry_id,v=g.tasks||[];for(let b of v){if(t&&b.responsible_user_id!==t)continue;let x=b.history||[];for(let f of x){if(typeof f?.timestamp!="string")continue;let z=f.timestamp.slice(0,10);if(z<a||z>l)continue;let M=c.get(z);if(!M)continue;let S=f.type??"completed";M.push({date:z,entry_id:_,task_id:b.id,task_name:b.name,object_name:m,status:ns[S]??"ok",days_until_due:null,projected:!1,schedule_type:b.schedule_type,interval_days:b.interval_days??null,responsible_user_id:b.responsible_user_id??null,avg_cost:typeof f.cost=="number"?f.cost:null,adaptive_enabled:!!b.adaptive_config?.enabled,prediction_confidence:null,history_timestamp:f.timestamp,history_type:S,history_cost:typeof f.cost=="number"?f.cost:null,history_notes:typeof f.notes=="string"?f.notes:null,history_duration:typeof f.duration=="number"?f.duration:null})}}}let u={completed:0,reset:1,skipped:2,triggered:3,trigger_replaced:4};for(let[,g]of c)g.sort((m,_)=>{let v=u[m.history_type??""]??99,b=u[_.history_type??""]??99;if(v!==b)return v-b;let x=m.object_name.localeCompare(_.object_name);return x!==0?x:m.task_name.localeCompare(_.task_name)});return i.map(g=>({date:g,events:c.get(g)??[]}))}var Zt=j`
  .cal-controls {
    display: flex;
    gap: 12px;
    align-items: center;
...
`;var ee=class extends A{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter)}getCardSize(){return 6}get _lang(){return this.hass?.language||"en"}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){super.updated(e);let t=this.hass?.language;if(t&&!Fe(t)&&ie(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}_onEventClick(e){if(e.history_timestamp){this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}));return}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}render(){if(!this.hass)return d;let e=this._lang,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.title,l=null;this._userFilter&&(l=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let c=new Date;c.setHours(0,0,0,0);let u=this._pastDays>0,g=u?Jt(this._objects,c,this._pastDays,l):Qt(this._objects,c,this._windowDays,l),m=Ie(c),_=this._windowDays===365||u,v=_?g.filter(f=>f.events.length>0):g,b=f=>{let z=`cal-status-${f.status}`,M=f.projected?"cal-event-projected":"",S=f.status==="overdue"&&f.days_until_due!=null?` (${Math.abs(f.days_until_due)}d ${s("overdue",e).toLowerCase()})`:"",k=f.projected&&f.interval_days?n`<span class="cal-event-recur">${f.interval_unit&&f.interval_unit!=="days"?`${f.interval_days} ${s("unit_"+f.interval_unit,e)}`:s("cal_every_n_days",e).replace("{n}",String(f.interval_days))}</span>`:d,R=f.schedule_type==="sensor_based",N=R?n`<ha-icon class="cal-event-icon cal-source-sensor"
                title="${s("cal_source_sensor",e)}" icon="mdi:trending-up"></ha-icon>`:n`<ha-icon class="cal-event-icon cal-source-time"
                title="${f.adaptive_enabled?s("cal_source_time_adaptive",e):s("cal_source_time",e)}"
                icon="${f.adaptive_enabled?"mdi:clock-time-four-outline":"mdi:clock-outline"}"></ha-icon>`,K=R&&f.prediction_confidence&&f.status!=="triggered"&&!f.projected?n`<span class="cal-event-prediction cal-conf-${f.prediction_confidence}">
            ${s("cal_predicted",e)} · ${s(`cal_confidence_${f.prediction_confidence}`,e)}
          </span>`:d,J=this._stats?.budget?.currency_symbol||"\u20AC",H=f.history_type?s(f.history_type,e):s(f.status,e);return n`
        <div class="cal-event ${M}"
          @click=${()=>this._onEventClick(f)}>
          ${N}
          <span class="cal-status-pill ${z}">${H}</span>
          <div class="cal-event-body">
            <div class="cal-event-title">${f.object_name} · ${f.task_name}${S}</div>
            ${K}
            ${k}
          </div>
          ${f.avg_cost!=null&&f.avg_cost>0?n`<span class="cal-event-cost">${f.avg_cost.toFixed(0)} ${J}</span>`:d}
        </div>
      `},x=f=>{let[z,M,S]=f.date.split("-").map(Number),k=new Date(z,M-1,S),R=f.date===m,N=k.toLocaleDateString(e,{weekday:"short"}),K=k.toLocaleDateString(e,{month:"long"});return n`
        <div class="cal-day-row">
          ...
          <div class="cal-day-content">
            <div class="cal-day-header">
              ...
              ${R?n`<span class="cal-day-today-badge">${s("today",e)}</span>`:d}
            </div>
            ${f.events.length===0?n`<div class="cal-empty">${s("cal_no_events",e)}</div>`:f.events.map(b)}
          </div>
        </div>
      `};return n`
      <ha-card .header=${a}>
        ...
        <div class="cal-rolling">
          ${v.length===0&&_?n`<div class="cal-empty">${s("cal_no_events",e)}</div>`:v.map(x)}
        </div>
      </ha-card>
    `}};ee.styles=[Ue,Zt,j`
      :host { display: block; }
      ha-card { padding: 0; overflow: hidden; }
    `],p([w({attribute:!1})],ee.prototype,"hass",2),p([h()],ee.prototype,"_config",2),p([h()],ee.prototype,"_objects",2),p([h()],ee.prototype,"_stats",2),p([h()],ee.prototype,"_windowDays",2),p([h()],ee.prototype,"_pastDays",2),p([h()],ee.prototype,"_userFilter",2),p([h()],ee.prototype,"_unsub",2);var ls=[{value:7,label:"Week (7 days)"},{value:14,label:"Fortnight (14 days)"},{value:30,label:"Month (30 days, default)"},{value:365,label:"Year (365 days, empty days collapsed)"}],ye=class extends A{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"}}setConfig(e){this._config={...e}}_valueChanged(e,t){let i={...this._config,[e]:t};e==="show_window_chips"&&t===!0&&delete i.show_window_chips,e==="show_user_filter"&&t===!0&&delete i.show_user_filter,e==="title"&&(!t||typeof t=="string"&&t.trim()==="")&&delete i.title,e==="user_filter"&&t===""&&delete i.user_filter,this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}render(){let e=this._config.window_days??30,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.user_filter??"",l=this._config.title??"";return n`
      <div class="editor">
        <div class="row">
        ...
        <div class="row">
          ...
          >
            ${ls.map(c=>n`<option value="${c.value}" ?selected=${c.value===e}>${c.label}</option>`)}
          </select>
        </div>
  ...
  `,p([w({attribute:!1})],ye.prototype,"hass",2),p([h()],ye.prototype,"_config",2);customElements.get("maintenance-supporter-calendar-card")||customElements.define("maintenance-supporter-calendar-card",ee);customElements.get("maintenance-supporter-calendar-card-editor")||customElements.define("maintenance-supporter-calendar-card-editor",ye);var Qe=window;Qe.customCards=Qe.customCards||[];var Xt="maintenance-supporter-calendar-card",cs=Qe.customCards.some(o=>o.type===Xt);cs||Qe.customCards.push({type:Xt,name:"Maintenance Supporter \u2014 Calendar",description:"Rolling calendar of maintenance tasks with 7/14/30/365 day windows, source icons, and prediction-confidence pills.",preview:!0});var ne=class extends A{constructor(){super(...arguments);this._open=!1;this._saving=!1;this._error="";this._draft=null;this._originalSnapshot=null}get _lang(){return this.hass?.language||"en"}openEdit(e){this._draft={...e},this._originalSnapshot={...e},this._error="",this._open=!0}close(){this._open=!1,this._error="",this._draft=null,this._originalSnapshot=null}_set(e,t){this._draft&&(this._draft={...this._draft,[e]:t})}async _save(){if(!(!this._draft||!this._originalSnapshot)){this._saving=!0,this._error="";try{let e={type:"maintenance_supporter/task/history/update",entry_id:this._draft.entry_id,task_id:this._draft.task_id,original_timestamp:this._originalSnapshot.original_timestamp};if(this._draft.timestamp!==this._originalSnapshot.timestamp&&(e.timestamp=this._draft.timestamp),this._draft.notes!==this._originalSnapshot.notes&&(e.notes=this._draft.notes),this._draft.cost!==this._originalSnapshot.cost&&(e.cost=this._draft.cost),this._draft.duration!==this._originalSnapshot.duration&&(e.duration=this._draft.duration),this._draft.completed_by!==this._originalSnapshot.completed_by&&(e.completed_by=this._draft.completed_by),Object.keys(e).filter(i=>!["type","entry_id","task_id","original_timestamp"].includes(i)).length===0){this.close();return}await this.hass.connection.sendMessagePromise(e),this.dispatchEvent(new CustomEvent("history-entry-saved",{detail:{entry_id:this._draft.entry_id,task_id:this._draft.task_id,new_timestamp:this._draft.timestamp},bubbles:!0,composed:!0})),this.close()}catch(e){this._error=L(e,this._lang)}finally{this._saving=!1}}}render(){if(!this._open||!this._draft)return d;let e=this._lang,t=this._draft;return n`
  ...
  `,p([w({attribute:!1})],ne.prototype,"hass",2),p([h()],ne.prototype,"_open",2),p([h()],ne.prototype,"_saving",2),p([h()],ne.prototype,"_error",2),p([h()],ne.prototype,"_draft",2);customElements.get("maintenance-history-edit-dialog")||customElements.define("maintenance-history-edit-dialog",ne);var G=class extends A{constructor(){super(...arguments);this._open=!1;this._title="";this._message="";this._confirmText="";this._danger=!1;this._inputLabel="";this._inputType="";this._inputValue="";this._resolve=null;this._promptResolve=null}confirm(e){return this._title=e.title,this._message=e.message,this._confirmText=e.confirmText||"OK",this._danger=e.danger||!1,this._inputLabel="",this._inputType="",this._inputValue="",this._open=!0,new Promise(t=>{this._resolve=t,this._promptResolve=null})}prompt(e){return this._title=e.title,this._message=e.message,this._confirmText=e.confirmText||"OK",this._danger=e.danger||!1,this._inputLabel=e.inputLabel||"",this._inputType=e.inputType||"text",this._inputValue=e.inputValue||"",this._open=!0,new Promise(t=>{this._promptResolve=t,this._resolve=null})}_cancel(){this._open=!1,this._promptResolve&&(this._promptResolve({confirmed:!1,value:""}),this._promptResolve=null),this._resolve?.(!1),this._resolve=null}_confirmAction(){this._open=!1,this._promptResolve&&(this._promptResolve({confirmed:!0,value:this._inputValue}),this._promptResolve=null),this._resolve?.(!0),this._resolve=null}render(){if(!this._open)return d;let e=this.hass?.language||"en";return n`
  ...
  `,p([w({attribute:!1})],G.prototype,"hass",2),p([h()],G.prototype,"_open",2),p([h()],G.prototype,"_title",2),p([h()],G.prototype,"_message",2),p([h()],G.prototype,"_confirmText",2),p([h()],G.prototype,"_danger",2),p([h()],G.prototype,"_inputLabel",2),p([h()],G.prototype,"_inputType",2),p([h()],G.prototype,"_inputValue",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",G);var ei={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},ti=o=>(...r)=>({_$litDirective$:o,values:r}),Je=class{constructor(r){}get _$AU(){return this._$AM._$AU}_$AT(r,e,t){this._$Ct=r,this._$AM=e,this._$Ci=t}_$AS(r,e){return this.update(r,e)}update(r,e){return this.render(...e)}};var Me=class extends Je{constructor(r){if(super(r),this.it=d,r.type!==ei.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(r){if(r===d||r==null)return this._t=void 0,this.it=r;if(r===re)return r;if(typeof r!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(r===this.it)return this._t;this.it=r;let e=[r];return e.raw=e,this._t={_$litType$:this.constructor.resultType,strings:e,values:[]}}};Me.directiveName="unsafeHTML",Me.resultType=1;var ii=ti(Me);var ds=["EUR","USD","GBP","JPY","CHF","CAD","AUD","CNY","INR","BRL","CZK","PLN","RUB","SEK","NOK","DKK","UAH"],C=class extends A{constructor(){super(...arguments);this.budget=null;this._settings=null;this._loading=!0;this._importCsv="";this._importLoading=!1;this._includeHistory=!0;this._toast="";this._testingNotification=!1;this._users=[];this._vacEnabled=!1;this._vacStart="";this._vacEnd="";this._vacBuffer=3;this._vacExempt=new Set;this._vacIsActive=!1;this._vacWindowEnd=null;this._vacAllTasks=[];this._vacPreview=[];this._vacPreviewLoading=!1;this._vacSaving=!1;this._qrObjects=[];this._qrSelectedEntries=new Set;this._qrActions=new Set(["view"]);this._qrUrlMode="companion";this._qrBatchLoading=!1;this._qrBatchResults=[];this._qrObjectsLoaded=!1;this._loaded=!1;this._userService=null;this._sendTestNotification=async()=>{this._testingNotification=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/test_notification"}),t=e.message||(e.success?s("test_notification_success",this._lang):s("test_notification_failed",this._lang));this._showToast(t)}catch{this._showToast(s("test_notification_failed",this._lang))}finally{this._testingNotification=!1}}}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._loaded?(this._loaded=!0,this._userService=new de(this.hass),this._loadSettings(),this._loadUsers()):e.has("hass")&&this.hass&&this._userService&&this._userService.updateHass(this.hass)}async _loadUsers(){if(this._userService)try{this._users=await this._userService.getUsers()}catch{this._users=[]}}async _loadSettings(){this._loading=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});this._settings=e,this._hydrateVacationFromSettings()}catch{}this._loading=!1}_hydrateVacationFromSettings(){let e=this._settings?.vacation;e&&(this._vacEnabled=e.enabled,this._vacStart=e.start||"",this._vacEnd=e.end||"",this._vacBuffer=e.buffer_days,this._vacExempt=new Set(e.exempt_task_ids||[]),this._vacIsActive=e.is_active,this._vacWindowEnd=e.window_end)}async _updateSetting(e,t){try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/update",settings:{[e]:t}});this._settings=i,this._showToast(s("settings_saved",this._lang)),this.dispatchEvent(new CustomEvent("settings-changed"))}catch{this._showToast(s("action_error",this._lang))}}_showToast(e){this._toast=e,setTimeout(()=>{this._toast=""},3e3)}_downloadFile(e,t,i){We(e,t,i)}render(){let e=this._lang;return this._loading||!this._settings?n`<div class="settings-loading">Loading…</div>`:n`
      ...
      ${this._toast?n`<div class="settings-toast">${this._toast}</div>`:d}
    `}scrollToSection(e){requestAnimationFrame(()=>{let t=this.shadowRoot;if(!t)return;let i=t.querySelector(`[data-section="${e}"]`)??t.querySelector(`[data-section-alt="${e}"]`);i&&i.scrollIntoView({behavior:"smooth",block:"start"})})}_renderPanelAccess(e){let t=new Set(this._settings.admin_panel_user_ids||[]),i=this._users.filter(c=>!c.is_admin),a=this._settings.operator_write_enabled??!1,l=(c,u)=>{let g=new Set(t);u?g.add(c):g.delete(c),this._updateSetting("admin_panel_user_ids",[...g])};return n`
      <div class="settings-section">
        <h3>${s("settings_panel_access",e)} ${a&&t.size>0?n`<span class="section-badge">${t.size}</span>`:d}</h3>
        ...
        ${a?i.length===0?n`<div class="setting-row hint">${s("no_non_admin_users",e)}</div>`:i.map(c=>n`
              <label class="setting-row">
                <span>
    ...
    `}_renderObjectsColumns(e){let t=Ce(this._settings.objects_table_columns);return n`
      ...
      <div class="settings-section">
        ...
        <label class="setting-row">
          ...
          <input type="number" min="1" max="365" .value=${String(t.default_warning_days)}
            @change=${c=>{let u=parseInt(c.target.value,10);u>=1&&u<=365&&this._updateSetting("default_warning_days",u)}} />
        </label>
        <label class="setting-row">
          <span class="setting-label">${s("settings_currency",e)}</span>
          <select @change=${c=>this._updateSetting("budget_currency",c.target.value)}>
            ${ds.map(c=>n`<option value=${c} ?selected=${l.currency===c}>${c}</option>`)}
          </select>
        </label>
        <label class="setting-row">
        ...
        ${t.notifications_enabled?n`
          <label class="setting-row">
            ...
            <datalist id="ms-notify-services">
              ${a.map(c=>n`<option value=${c}></option>`)}
            </datalist>
          </label>
    ...
    `}_renderNotifications(e){let t=this._settings.notifications,i=this._settings.actions;return n`
      <div class="settings-section">
    ...
    `}_renderBudget(e){let t=this._settings.budget;return n`
    ...
    `}_renderArchive(e){let t=this._settings.archive??{oneoff_days:14,delete_archived_oneoff_days:0};return n`
    ...
    `}_renderVacation(e){let t=this._vacEnabled&&!this._vacIsActive&&this._vacWindowEnd&&new Date(this._vacWindowEnd)<new Date,i=this._vacExempt.size;return n`
      <div class="settings-section vacation-section" data-section="vacation">
        <h3>
          ${s("vacation_title",e)}
          ${this._vacIsActive?n`<span class="vac-badge active">${s("vacation_active",e)}</span>`:d}
          ${t?n`<span class="vac-badge stale">${s("vacation_ended",e)}</span>`:d}
        </h3>
        ...
        <details class="vac-exempt-panel">
          <summary>
            ...
            ${i>0?n`<span class="section-badge">${i}</span>`:d}
          </summary>
          <p class="section-desc">${s("vacation_exempt_desc",e)}</p>
          ${this._vacAllTasks.length===0?n`<button @click=${this._loadAllTasksForVacation}>${s("vacation_load_tasks",e)}</button>`:n`
        ...
        ${this._vacStart&&this._vacEnd?n`
          <div class="vac-preview-toolbar">
            ...
            ${this._vacPreview.length>0?n`<span class="vac-preview-count">${this._vacPreview.length} ${s("vacation_preview_affected",e)}</span>`:d}
          </div>
          ${this._vacPreview.length>0?this._renderVacationPreview(e):d}
        `:d}

        ${this._vacIsActive||t?n`<button class="vac-end-now" @click=${this._endVacationNow}>
              ${s("vacation_end_now",e)}
            </button>`:d}
      </div>
    `}_renderVacationTaskList(e){let t=new Map;for(let a of this._vacAllTasks){let l=t.get(a.object_name)||[];l.push(a),t.set(a.object_name,l)}return[...t.entries()].sort(([a],[l])=>a.localeCompare(l)).map(([a,l])=>n`
      <div class="vac-task-group">
        <div class="vac-task-group-name">${a||s("no_objects",e)}</div>
        ${l.sort((c,u)=>c.task_name.localeCompare(u.task_name)).map(c=>n`
    ...
    `)}_renderVacationPreview(e){return n`
      <div class="vac-preview-list">
        ${this._vacPreview.map(t=>{let i=t.events.map(l=>{let c=`vacation_event_${l.status}`;return`${l.date} (${s(c,e)})`}).join(" \xB7 "),a=!t.will_suppress;return n`
            <div class="vac-preview-row ${a?"exempt":""}">
              <div class="vac-preview-info">
                <div class="vac-preview-name">
                  ...
                  ${t.kind==="sensor_based"?n`<span class="vac-preview-kind">${s("vacation_sensor_based",e)}</span>`:d}
                </div>
              ...
              <div class="vac-preview-actions">
                ...
                ${t.kind==="time_based"?n`<button @click=${()=>this._previewActionSkip(t)}>${s("qr_action_skip",e)}</button>`:d}
    ...
    `}async _loadAllTasksForVacation(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),t=[];for(let i of e.objects||[])for(let a of i.tasks||[])t.push({entry_id:i.entry_id,object_name:i.object.name||"",task_id:a.id,task_name:a.name||""});this._vacAllTasks=t}catch{this._showToast(s("action_error",this._lang))}}async _saveVacation(e){if(!this._vacSaving){this._vacSaving=!0;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/vacation/update",...e});this._vacEnabled=t.enabled,this._vacStart=t.start||"",this._vacEnd=t.end||"",this._vacBuffer=t.buffer_days,this._vacExempt=new Set(t.exempt_task_ids||[]),this._vacIsActive=t.is_active,this._vacWindowEnd=t.window_end,this.dispatchEvent(new CustomEvent("settings-changed"))}catch(t){let i=t?.message||s("action_error",this._lang);this._showToast(i)}finally{this._vacSaving=!1}}}_toggleVacationEnabled(e){this._saveVacation({enabled:e})}_setVacationDate(e,t){let i={};i[e]=t||null,this._saveVacation(i)}_setVacationBuffer(e){e<0||e>14||this._saveVacation({buffer_days:e})}_toggleVacationExempt(e,t){let i=new Set(this._vacExempt);t?i.add(e):i.delete(e),this._saveVacation({exempt_task_ids:[...i]})}async _loadVacationPreview(){this._vacPreviewLoading=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/vacation/preview"});this._vacPreview=e.rows||[]}catch{this._showToast(s("action_error",this._lang))}finally{this._vacPreviewLoading=!1}}async _previewActionComplete(e){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/complete",entry_id:e.entry_id,task_id:e.task_id}),this._showToast(s("vacation_marked_complete",this._lang)),await this._loadVacationPreview()}catch{this._showToast(s("action_error",this._lang))}}async _previewActionSkip(e){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/skip",entry_id:e.entry_id,task_id:e.task_id,reason:"Skipped before vacation"}),this._showToast(s("vacation_marked_skip",this._lang)),await this._loadVacationPreview()}catch{this._showToast(s("action_error",this._lang))}}async _endVacationNow(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/vacation/end_now"});this._vacEnabled=e.enabled,this._vacEnd=e.end||"",this._vacIsActive=e.is_active,this._vacWindowEnd=e.window_end,this.dispatchEvent(new CustomEvent("settings-changed")),this._showToast(s("vacation_ended",this._lang))}catch{this._showToast(s("action_error",this._lang))}}_renderPrintQr(e){let t=this._qrSelectedEntries.size||this._qrObjects.length,i=this._qrActions.size,a=t*i,l=a>200;return n`
      <div class="settings-section qr-print-section">
        ...
        ${this._qrObjectsLoaded?n`
            <details open class="qr-filter-panel">
              ...
              <div class="qr-filter-group">
                ...
                <div class="qr-object-list">
                  ${this._qrObjects.length===0?n`<div class="hint">${s("no_objects",e)}</div>`:this._qrObjects.map(c=>n`
            ...
            ${this._qrBatchResults.length>0?n`
                ...
                <div class="qr-print-grid">
                  ${this._qrBatchResults.map(c=>n`
                    <div class="qr-print-cell">
                      <div class="qr-svg">${ii(c.svg)}</div>
                      <div class="qr-label">
              ...
              `:d}
          `:n`<button @click=${this._loadQrObjects}>${s("qr_print_load",e)}</button>`}
      </div>
...
<body>
...
</html>`,l=window.open("","_blank","width=900,height=1100");if(!l){window.print();return}l.document.open(),l.document.write(a),l.document.close()}_escapeHtml(e){return e.replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}_renderImportExport(e){return n`
      <div class="settings-section">
  ...
  `,p([w({attribute:!1})],C.prototype,"hass",2),p([w({attribute:!1})],C.prototype,"features",2),p([w({attribute:!1})],C.prototype,"budget",2),p([h()],C.prototype,"_settings",2),p([h()],C.prototype,"_loading",2),p([h()],C.prototype,"_importCsv",2),p([h()],C.prototype,"_importLoading",2),p([h()],C.prototype,"_includeHistory",2),p([h()],C.prototype,"_toast",2),p([h()],C.prototype,"_testingNotification",2),p([h()],C.prototype,"_users",2),p([h()],C.prototype,"_vacEnabled",2),p([h()],C.prototype,"_vacStart",2),p([h()],C.prototype,"_vacEnd",2),p([h()],C.prototype,"_vacBuffer",2),p([h()],C.prototype,"_vacExempt",2),p([h()],C.prototype,"_vacIsActive",2),p([h()],C.prototype,"_vacWindowEnd",2),p([h()],C.prototype,"_vacAllTasks",2),p([h()],C.prototype,"_vacPreview",2),p([h()],C.prototype,"_vacPreviewLoading",2),p([h()],C.prototype,"_vacSaving",2),p([h()],C.prototype,"_qrObjects",2),p([h()],C.prototype,"_qrSelectedEntries",2),p([h()],C.prototype,"_qrActions",2),p([h()],C.prototype,"_qrUrlMode",2),p([h()],C.prototype,"_qrBatchLoading",2),p([h()],C.prototype,"_qrBatchResults",2),p([h()],C.prototype,"_qrObjectsLoaded",2);customElements.define("maintenance-settings-view",C);var se=class extends A{constructor(){super(...arguments);this.objects=[];this._summary=null;this._loaded=!1;this._busy=!1;this._error="";this._initiallyLoaded=!1}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._initiallyLoaded&&(this._initiallyLoaded=!0,this._load(),ie(this._lang).then(()=>this.requestUpdate()))}async _load(){this._busy=!0;try{this._summary=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/storage"}),this._error=""}catch(e){this._error=L(e,this._lang)}finally{this._loaded=!0,this._busy=!1}}_nameFor(e){return this.objects.find(i=>i.object?.id===e)?.object?.name||e.slice(0,8)}render(){if(!this._loaded||!this._summary)return d;let e=this._summary;if(e.document_count===0)return d;let t=this._lang,i=Object.entries(e.by_object).filter(([,a])=>a.files>0||a.links>0).map(([a,l])=>({id:a,name:this._nameFor(a),...l})).sort((a,l)=>l.bytes-a.bytes);return n`
      <ha-card>
        <div class="card-content">
          <div class="header">
            <div class="title">
              <span class="emoji">🗄️</span>
              <span>${s("doc_storage_title",t)}</span>
            </div>
            <button
              class="icon-btn"
          ...
          <div class="totals">
            ...
            ${e.dedup_savings_bytes>0?n`<div class="stat">
          ...
          ${i.length?n`<div class="obj-list">
                ${i.map(a=>n`
                    <div class="obj-row">
                      <span class="obj-name">${a.name}</span>
                      <span class="obj-meta">
                        ${a.files>0?n`<ha-icon icon="mdi:file-document-outline"></ha-icon>${a.files}`:d}
                        ${a.links>0?n`<ha-icon icon="mdi:link-variant"></ha-icon>${a.links}`:d}
                      </span>
                      <span class="obj-size">${pe(a.bytes)}</span>
                    </div>
                  `)}
              </div>`:d}
        </div>
  ...
  `,p([w({attribute:!1})],se.prototype,"hass",2),p([w({attribute:!1})],se.prototype,"objects",2),p([h()],se.prototype,"_summary",2),p([h()],se.prototype,"_loaded",2),p([h()],se.prototype,"_busy",2),p([h()],se.prototype,"_error",2);customElements.get("maintenance-storage-section-card")||customElements.define("maintenance-storage-section-card",se);var ps=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"],te=class extends A{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._entryId="";this._taskId="";this._values=new Array(12).fill("");this._save=async()=>{let e=this._buildOverrides();if(e!==null){this._loading=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/seasonal_overrides",entry_id:this._entryId,task_id:this._taskId,overrides:e}),this._open=!1,this.dispatchEvent(new CustomEvent("overrides-saved"))}catch(t){this._error=L(t,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}};this._clearAll=async()=>{this._loading=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/seasonal_overrides",entry_id:this._entryId,task_id:this._taskId,overrides:{}}),this._values=new Array(12).fill(""),this._open=!1,this.dispatchEvent(new CustomEvent("overrides-saved"))}catch(e){this._error=L(e,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}open(e,t,i){if(this._entryId=e,this._taskId=t,this._values=new Array(12).fill(""),i)for(let[a,l]of Object.entries(i)){let c=parseInt(a,10);c>=1&&c<=12&&typeof l=="number"&&(this._values[c-1]=l.toString())}this._error="",this._open=!0}_close(){this._open=!1}_buildOverrides(){let e={};for(let t=0;t<12;t++){let i=this._values[t].trim();if(!i)continue;let a=parseFloat(i);if(Number.isNaN(a))return this._error=`${s("month_"+["jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec"][t],this._lang)}: ${s("seasonal_override_invalid",this._lang)}`,null;if(a<.1||a>5)return this._error=s("seasonal_override_range",this._lang),null;e[t+1]=a}return e}render(){if(!this._open)return n``;let e=this._lang;return n`
  ...
  `,p([w({attribute:!1})],te.prototype,"hass",2),p([h()],te.prototype,"_open",2),p([h()],te.prototype,"_loading",2),p([h()],te.prototype,"_error",2),p([h()],te.prototype,"_entryId",2),p([h()],te.prototype,"_taskId",2),p([h()],te.prototype,"_values",2);customElements.get("maintenance-seasonal-overrides-dialog")||customElements.define("maintenance-seasonal-overrides-dialog",te);var Q=class extends A{constructor(){super(...arguments);this.objects=[];this._open=!1;this._loading=!1;this._error="";this._groupId=null;this._name="";this._description="";this._selected=new Set;this._toggleTask=(e,t)=>{let i=`${e}:${t}`,a=new Set(this._selected);a.has(i)?a.delete(i):a.add(i),this._selected=a};this._save=async()=>{let e=this._name.trim();if(!e){this._error=s("group_name_required",this._lang);return}this._loading=!0,this._error="";try{let t=this._buildTaskRefs();this._groupId?await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/group/update",group_id:this._groupId,name:e,description:this._description,task_refs:t}):await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/group/create",name:e,description:this._description,task_refs:t}),this._open=!1,this.dispatchEvent(new CustomEvent("group-saved"))}catch(t){this._error=L(t,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}openCreate(){this._reset(),this._open=!0}openEdit(e,t){this._reset(),this._groupId=e,this._name=t.name,this._description=t.description||"",this._selected=new Set(t.task_refs.map(i=>`${i.entry_id}:${i.task_id}`)),this._open=!0}_reset(){this._groupId=null,this._name="",this._description="",this._selected=new Set,this._error=""}_close(){this._open=!1}_buildTaskRefs(){return[...this._selected].map(e=>{let[t,i]=e.split(":",2);return{entry_id:t,task_id:i}})}render(){if(!this._open)return n``;let e=this._lang,t=this._groupId?s("edit_group",e):s("new_group",e);return n`
      <ha-dialog open @closed=${this._close} heading="${t}">
        <div class="content">
          ...
          ${this.objects.length===0?n`<div class="hint">${s("no_objects",e)}</div>`:n`
              <div class="objects">
                ${[...this.objects].sort((i,a)=>i.object.name.localeCompare(a.object.name)).map(i=>n`
                  <div class="object-block">
                    <div class="object-name">${i.object.name}</div>
                    ${i.tasks.length===0?n`<div class="hint small">${s("no_tasks_short",e)}</div>`:[...i.tasks].sort((a,l)=>a.name.localeCompare(l.name)).map(a=>{let l=`${i.entry_id}:${a.id}`,c=this._selected.has(l);return n`
  ...
  `,p([w({attribute:!1})],Q.prototype,"hass",2),p([w({attribute:!1})],Q.prototype,"objects",2),p([h()],Q.prototype,"_open",2),p([h()],Q.prototype,"_loading",2),p([h()],Q.prototype,"_error",2),p([h()],Q.prototype,"_groupId",2),p([h()],Q.prototype,"_name",2),p([h()],Q.prototype,"_description",2),p([h()],Q.prototype,"_selected",2);customElements.get("maintenance-group-dialog")||customElements.define("maintenance-group-dialog",Q);var us=300,hs=140,yt=27;function ai(o,r){let e=o.trigger_config;if(!e)return d;let t=r.lang,i=o.trigger_entity_info,a=o.trigger_entity_infos,l=i?.friendly_name||e.entity_id||"\u2014",c=e.entity_id||"",u=e.entity_ids||(c?[c]:[]),g=i?.unit_of_measurement||"",m=o.trigger_current_value,_=e.type||"threshold",v=u.length>1;return n`
    <h3>${s("trigger",t)}</h3>
    <div class="trigger-card">
      <div class="trigger-header">
        ...
        <div>
          ${v?n`
            ...
            <div class="trigger-entity-id">${u.map((b,x)=>n`${x>0?", ":""}<span class="entity-link" @click=${f=>ge(f,b)}>${b}</span>`)}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `:n`
            <div class="trigger-entity-name">${l}</div>
            <div class="trigger-entity-id">${c?n`<span class="entity-link" @click=${b=>ge(b,c)}>${c}</span>`:""}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `}
        </div>
        <span class="status-badge ${o.trigger_active?"triggered":"ok"}" style="margin-left: auto;">
          ${o.trigger_active?s("triggered",t):s("ok",t)}
        </span>
      </div>
      ...
      ${m!=null?n`
            <div class="trigger-value-row">
              ...
              ${g?n`<span class="trigger-unit">${g}</span>`:d}
            </div>
          `:d}

      <div class="trigger-limits">
        ${_==="threshold"?n`
          ${e.trigger_above!=null?n`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("threshold_above",t)}: ${e.trigger_above} ${g}</span>`:d}
          ${e.trigger_below!=null?n`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("threshold_below",t)}: ${e.trigger_below} ${g}</span>`:d}
          ${e.trigger_for_minutes?n`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${s("for_minutes",t)}: ${e.trigger_for_minutes}</span>`:d}
        `:d}
        ${_==="counter"?n`
          ${e.trigger_target_value!=null?n`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("target_value",t)}: ${e.trigger_target_value} ${g}</span>`:d}
        `:d}
        ${_==="state_change"?n`
          ${e.trigger_target_changes!=null?n`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("target_changes",t)}: ${e.trigger_target_changes}</span>`:d}
        `:d}
        ${_==="runtime"?n`
          ${e.trigger_runtime_hours!=null?n`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("runtime_hours",t)}: ${e.trigger_runtime_hours}h</span>`:d}
        `:d}
        ${_==="compound"?n`
          ...
          ${(e.conditions||[]).map((b,x)=>n`
            <span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${x+1}. ${s(b.type||"unknown",t)}: ${b.entity_id?n`<span class="entity-link" @click=${f=>ge(f,b.entity_id)}>${b.entity_id}</span>`:""}</span>
          `)}
        `:d}
        ${i?.min!=null?n`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${s("min",t)}: ${i.min} ${g}</span>`:d}
        ${i?.max!=null?n`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${s("max",t)}: ${i.max} ${g}</span>`:d}
      </div>

      ${a&&a.length>1?n`
        <div class="trigger-entity-list">
          ${a.map(b=>n`
            <span class="trigger-entity-id">${b.friendly_name} (<span class="entity-link" @click=${x=>ge(x,b.entity_id)}>${b.entity_id}</span>)</span>
          `)}
        </div>
      `:d}
  ...
  `}function _s(o,r,e){let t=o.trigger_config;if(!t)return d;let i=t.type||"threshold",a=i==="counter"&&t.trigger_delta_mode,l=e.isCounterEntity(t),c=t.entity_id||"",u=e.detailStatsData.get(c)||[],g=[],m=!1;if(u.length>=2)for(let y of u){let I=y.val;a&&o.trigger_baseline_value!=null&&(I-=o.trigger_baseline_value);let U={ts:y.ts,val:I};!l&&y.min!=null&&y.max!=null&&(U.min=a&&o.trigger_baseline_value!=null?y.min-o.trigger_baseline_value:y.min,U.max=a&&o.trigger_baseline_value!=null?y.max-o.trigger_baseline_value:y.max,m=!0),g.push(U)}else for(let y of o.history)y.trigger_value!=null&&g.push({ts:new Date(y.timestamp).getTime(),val:y.trigger_value});if(o.trigger_current_value!=null){let y=o.trigger_current_value;a&&o.trigger_baseline_value!=null&&(y-=o.trigger_baseline_value),g.push({ts:Date.now(),val:y})}if(g.length<2&&c&&e.hasStatsService&&!e.detailStatsData.has(c))return n`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
      <ha-icon icon="mdi:chart-line" style="--mdc-icon-size:16px;margin-right:8px;"></ha-icon>
      ${s("loading_chart",e.lang)}
    </div>`;if(g.length<2)return d;g.sort((y,I)=>y.ts-I.ts);let _=us,v=hs,b=30,x=2,f=8,z=16,M=g.map(y=>y.val),S=Math.min(...M),k=Math.max(...M);if(m)for(let y of g)y.min!=null&&(S=Math.min(S,y.min)),y.max!=null&&(k=Math.max(k,y.max));t.trigger_above!=null&&(k=Math.max(k,t.trigger_above),S=Math.min(S,t.trigger_above)),t.trigger_below!=null&&(S=Math.min(S,t.trigger_below),k=Math.max(k,t.trigger_below));let R=null,N=null;if(i==="counter"&&t.trigger_target_value!=null){if(o.trigger_baseline_value!=null)R=o.trigger_baseline_value;else if(g.length>0){let y=[...o.history].filter(I=>I.type==="completed"||I.type==="reset").sort((I,U)=>new Date(U.timestamp).getTime()-new Date(I.timestamp).getTime())[0];if(y){let I=new Date(y.timestamp).getTime(),U=g[0],W=Math.abs(g[0].ts-I);for(let oe of g){let le=Math.abs(oe.ts-I);le<W&&(U=oe,W=le)}R=U.val}else R=g[0].val}R!=null?(N=R+t.trigger_target_value,k=Math.max(k,N),S=Math.min(S,R)):(k=Math.max(k,t.trigger_target_value),S=Math.min(S,0))}a&&o.trigger_baseline_value!=null&&(S=Math.min(S,0));let K=k-S||1;S-=K*.1,k+=K*.1;let J=g[0].ts,H=g[g.length-1].ts,E=H-J||1,D=y=>b+(y-J)/E*(_-b-x),P=y=>f+(1-(y-S)/(k-S))*(v-f-z),Ze=g.map(y=>`${D(y.ts).toFixed(1)},${P(y.val).toFixed(1)}`).join(" "),hi=`M${D(g[0].ts).toFixed(1)},${v-z} `+g.map(y=>`L${D(y.ts).toFixed(1)},${P(y.val).toFixed(1)}`).join(" ")+` L${D(g[g.length-1].ts).toFixed(1)},${v-z} Z`,Xe="";if(m){let y=g.filter(I=>I.min!=null&&I.max!=null);if(y.length>=2){let I=y.map(W=>`${D(W.ts).toFixed(1)},${P(W.max).toFixed(1)}`),U=[...y].reverse().map(W=>`${D(W.ts).toFixed(1)},${P(W.min).toFixed(1)}`);Xe=`M${I[0]} `+I.slice(1).map(W=>`L${W}`).join(" ")+` L${U[0]} `+U.slice(1).map(W=>`L${W}`).join(" ")+" Z"}}let xt=g[g.length-1],_i=D(xt.ts),gi=P(xt.val),$t=y=>Math.abs(y)>=1e4?(y/1e3).toFixed(0)+"k":y>=1e3?(y/1e3).toFixed(1)+"k":y.toFixed(y<10?1:0),mi=$t(k),vi=$t(S),bi=o.history.filter(y=>["completed","skipped","reset"].includes(y.type)).map(y=>({ts:new Date(y.timestamp).getTime(),type:y.type})).filter(y=>y.ts>=J&&y.ts<=H),et=g;if(g.length>yt){let y=(g.length-1)/(yt-1);et=[];for(let I=0;I<yt;I++)et.push(g[Math.round(I*y)])}return n`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${_} ${v}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_sparkline",e.lang)}">
        <text x="${b-3}" y="${f+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${mi}</text>
        <text x="${b-3}" y="${v-z+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${vi}</text>
        <text x="${b}" y="${v-1}" text-anchor="start" fill="var(--secondary-text-color)" font-size="7">${new Date(J).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        <text x="${_-x}" y="${v-1}" text-anchor="end" fill="var(--secondary-text-color)" font-size="7">${new Date(H).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        ${Xe?O`<path d="${Xe}" fill="var(--primary-color)" opacity="0.08" />`:d}
        <path d="${hi}" fill="var(--primary-color)" opacity="0.15" />
        <polyline points="${Ze}" fill="none" stroke="var(--primary-color)" stroke-width="2" stroke-linejoin="round" />
        ${o.degradation_rate!=null&&o.degradation_trend!=="stable"&&o.degradation_trend!=="insufficient_data"&&g.length>=2?(()=>{let y=g[g.length-1],I=30,U=y.ts+I*864e5,W=y.val+o.degradation_rate*I,oe=Math.min(U,H+(H-J)*.3),le=Math.max(S,Math.min(k,W)),fi=D(y.ts),yi=P(y.val),xi=D(oe),$i=P(le);return O`<line x1="${fi.toFixed(1)}" y1="${yi.toFixed(1)}" x2="${xi.toFixed(1)}" y2="${$i.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():d}
        ${i==="threshold"&&t.trigger_above!=null?O`<line x1="${b}" y1="${P(t.trigger_above).toFixed(1)}" x2="${_}" y2="${P(t.trigger_above).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${_-2}" y="${P(t.trigger_above)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25B2 ${t.trigger_above}</text>`:d}
        ${i==="threshold"&&t.trigger_below!=null?O`<line x1="${b}" y1="${P(t.trigger_below).toFixed(1)}" x2="${_}" y2="${P(t.trigger_below).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${_-2}" y="${P(t.trigger_below)+11}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25BC ${t.trigger_below}</text>`:d}
        ${i==="counter"&&N!=null?O`<line x1="${b}" y1="${P(N).toFixed(1)}" x2="${_}" y2="${P(N).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${_-2}" y="${P(N)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">${s("target_value",e.lang)}: +${t.trigger_target_value}</text>`:d}
        ${i==="counter"&&R!=null?O`<line x1="${b}" y1="${P(R).toFixed(1)}" x2="${_}" y2="${P(R).toFixed(1)}" stroke="var(--secondary-text-color)" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
                <text x="${b+2}" y="${P(R)+11}" text-anchor="start" fill="var(--secondary-text-color)" font-size="8">${s("baseline",e.lang)}</text>`:d}
        <circle cx="${_i.toFixed(1)}" cy="${gi.toFixed(1)}" r="3.5" fill="var(--primary-color)" />
        ${bi.map(y=>{let I=D(y.ts),U=y.type==="completed"?"var(--success-color, #4caf50)":y.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return O`
            <line x1="${I.toFixed(1)}" y1="${f}" x2="${I.toFixed(1)}" y2="${v-z}" stroke="${U}" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
            <circle cx="${I.toFixed(1)}" cy="${f+2}" r="5" fill="${U}" opacity="0.8" />
            <text x="${I.toFixed(1)}" y="${f+6}" text-anchor="middle" fill="white" font-size="7" font-weight="bold">${y.type==="completed"?"\u2713":y.type==="skipped"?"\u23ED":"\u21BA"}</text>
          `})}
        ${et.map(y=>{let I=D(y.ts),U=P(y.val),W=new Date(y.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),oe=`${y.val.toFixed(1)} ${r}`;return m&&y.min!=null&&y.max!=null&&(oe+=` (${y.min.toFixed(1)}\u2013${y.max.toFixed(1)})`),O`<circle cx="${I.toFixed(1)}" cy="${U.toFixed(1)}" r="8" fill="transparent" tabindex="0"
            @mouseenter=${le=>si(le,`${W}
${oe}`,e.setTooltip)}
            @focus=${le=>si(le,`${W}
${oe}`,e.setTooltip)}
...
`).map(y=>n`<div>${y}</div>`)}
  ...
  `}function si(o,r,e){let t=o.currentTarget,i=t.closest(".sparkline-container");if(!i)return;let a=i.getBoundingClientRect(),l=t.getBoundingClientRect();e({x:l.left-a.left+l.width/2,y:l.top-a.top-8,text:r})}function ri(o,r,e){let t=o.degradation_trend!=null&&o.degradation_trend!=="insufficient_data",i=o.days_until_threshold!=null,a=o.environmental_factor!=null&&o.environmental_factor!==1;if(!t&&!i&&!a)return d;let l=o.degradation_trend==="rising"?"M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z":o.degradation_trend==="falling"?"M16,18L18.29,15.71L13.41,10.83L9.41,14.83L2,7.41L3.41,6L9.41,12L13.41,8L19.71,14.29L22,12V18H16Z":"M22,12L18,8V11H3V13H18V16L22,12Z";return n`
    <div class="prediction-section">
      ${o.sensor_prediction_urgency?n`
        <div class="prediction-urgency-banner">
          <ha-svg-icon path="M1,21H23L12,2L1,21M12,18A1,1 0 0,1 11,17A1,1 0 0,1 12,16A1,1 0 0,1 13,17A1,1 0 0,1 12,18M13,15H11V10H13V15Z"></ha-svg-icon>
          ${s("sensor_prediction_urgency",r).replace("{days}",String(Math.round(o.days_until_threshold||0)))}
        </div>
      `:d}
      ...
      <div class="prediction-grid">
        ${t?n`
          <div class="prediction-item">
            ...
            <span class="prediction-value ${o.degradation_trend}">${s("trend_"+o.degradation_trend,r)}</span>
            ${o.degradation_rate!=null?n`<span class="prediction-rate">${o.degradation_rate>0?"+":""}${Math.abs(o.degradation_rate)>=10?Math.round(o.degradation_rate).toLocaleString():o.degradation_rate.toFixed(1)} ${o.trigger_entity_info?.unit_of_measurement||""}/${s("day_short",r)}</span>`:d}
          </div>
        `:d}
        ${i?n`
          <div class="prediction-item">
            ...
            <span class="prediction-value prediction-days${o.days_until_threshold===0?" exceeded":o.sensor_prediction_urgency?" urgent":""}">${o.days_until_threshold===0?s("threshold_exceeded",r):"~"+Math.round(o.days_until_threshold)+" "+s("days",r)}</span>
            ${o.threshold_prediction_date?n`<span class="prediction-date">${Z(o.threshold_prediction_date,r)}</span>`:d}
            ${o.threshold_prediction_confidence?n`<span class="confidence-dot ${o.threshold_prediction_confidence}"></span>`:d}
          </div>
        `:d}
        ${a&&e.environmental?n`
          <div class="prediction-item">
            ...
            ${o.environmental_entity?n`<span class="prediction-entity entity-link" @click=${c=>ge(c,o.environmental_entity)}>${o.environmental_entity}</span>`:d}
          </div>
        `:d}
      </div>
    </div>
  `}function ni(o,r){let e=o.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return d;let a=o.interval_days??0,l=o.suggested_interval??a;return n`
    <div class="weibull-section">
      ...
      ${ms(t,i,a,l,r)}
      ${vs(e,r)}
      ${e?.confidence_interval_low!=null?bs(e,o,r):d}
    </div>
  `}function gs(o,r){let e,t,i;return o<.8?(e="early_failures",t="M13,14H11V10H13M13,18H11V16H13M1,21H23L12,2L1,21Z",i="beta_early_failures"):o<=1.2?(e="random_failures",t="M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2M13,17H11V15H13V17M13,13H11V7H13V13Z",i="beta_random_failures"):o<=3.5?(e="wear_out",t="M12,2A10,10 0 0,1 22,12A10,10 0 0,1 12,22A10,10 0 0,1 2,12A10,10 0 0,1 12,2M12,4A8,8 0 0,0 4,12A8,8 0 0,0 12,20A8,8 0 0,0 20,12A8,8 0 0,0 12,4M12,6A6,6 0 0,1 18,12H12V6Z",i="beta_wear_out"):(e="highly_predictable",t="M12,2A10,10 0 0,1 22,12A10,10 0 0,1 12,22A10,10 0 0,1 2,12A10,10 0 0,1 12,2M11,16.5L18,9.5L16.59,8.09L11,13.67L7.91,10.59L6.5,12L11,16.5Z",i="beta_highly_predictable"),n`
    <span class="beta-badge ${e}">
      <ha-svg-icon path="${t}"></ha-svg-icon>
      ${s(i,r)} (\u03B2=${o.toFixed(2)})
    </span>
  `}function ms(o,r,e,t,i){let b=Math.max(e,t,r,1)*1.3,x=50,f=[];for(let H=0;H<=x;H++){let E=H/x*b,D=1-Math.exp(-Math.pow(E/r,o)),P=32+E/b*260,Ze=136-D*128;f.push([P,Ze])}let z=f.map(([H,E])=>`${H.toFixed(1)},${E.toFixed(1)}`).join(" "),M="M32,136 "+f.map(([H,E])=>`L${H.toFixed(1)},${E.toFixed(1)}`).join(" ")+` L${f[x][0].toFixed(1)},136 Z`,S=32+e/b*260,k=1-Math.exp(-Math.pow(e/r,o)),R=136-k*128,N=((1-k)*100).toFixed(0),K=32+t/b*260,J=[0,.25,.5,.75,1];return n`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_weibull",i)}">
        ${J.map(H=>{let E=136-H*128;return O`
        ...
        ${e>0?O`
          ...
          <text x="${(S+4).toFixed(1)}" y="${(R-6).toFixed(1)}" fill="var(--primary-color, #03a9f4)"
            font-size="9" font-weight="600">R=${N}%</text>
        `:d}
    ...
    <div class="chart-legend">
      ...
      ${e>0?n`<span class="legend-item"><span class="legend-swatch" style="background:var(--primary-color, #03a9f4); opacity:0.5"></span> ${s("current_interval_marker",i)}</span>`:d}
      ${t>0&&t!==e?n`<span class="legend-item"><span class="legend-swatch" style="background:var(--success-color, #4caf50)"></span> ${s("recommended_marker",i)}</span>`:d}
    </div>
  `}function vs(o,r){return n`
    <div class="weibull-info-row">
      <div class="weibull-info-item">
        <span>${s("characteristic_life",r)}</span>
        <span class="weibull-info-value">${Math.round(o.weibull_eta)} ${s("days",r)}</span>
      </div>
      ${o.weibull_r_squared!=null?n`
        <div class="weibull-info-item">
  ...
  `}function bs(o,r,e){let t=o.confidence_interval_low,i=o.confidence_interval_high,a=r.suggested_interval??r.interval_days??0,l=r.interval_days??0,c=Math.max(0,t-5),g=i+5-c,m=(t-c)/g*100,_=(i-t)/g*100,v=(a-c)/g*100,b=l>0?(l-c)/g*100:-1;return n`
    <div class="confidence-range">
      <div class="confidence-range-title">
        ${s("confidence_interval",e)}: ${a} ${s("days",e)} (${t}\u2013${i})
      </div>
      <div class="confidence-bar">
        <div class="confidence-fill" style="left:${m.toFixed(1)}%;width:${_.toFixed(1)}%"></div>
  ...
  `}function oi(o,r,e,t){let i=Math.max(o||1,r);return n`
    <div class="interval-comparison">
      <div class="interval-bar">
        <div class="interval-label">
          ${s("current",t)}: ${o??"\u2014"} ${o!=null?s("days",t):""}
        </div>
  ...
  `}var li=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"];function ci(o,r,e){if(!e.seasonal||!o.seasonal_factor||o.seasonal_factor===1)return d;let t=li.map(c=>s(c,r)),i=new Date().getMonth(),a=o.seasonal_factors||o.interval_analysis?.seasonal_factors||null,l=a&&a.length===12?a:t.map((c,u)=>{let g=o.seasonal_factor||1,m=Math.sin((u-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,g+m))});return n`
    <div class="seasonal-card-compact">
      <h4>${s("seasonal_awareness",r)}</h4>
      <div class="seasonal-mini-chart">
        ${l.map((c,u)=>{let g=c*40,m=c<.9?"low":c>1.1?"high":"normal";return n`
  ...
  `}function di(o,r){return fs(o,r)}function fs(o,r){let e=o.seasonal_factors??o.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return d;let t=o.interval_analysis?.seasonal_reason,i=new Date().getMonth(),a=300,l=100,c=8,g=l-c-4,m=Math.max(...e,1.5),_=a/12,v=_*.65,b=c+g-1/m*g;return n`
    <div class="seasonal-chart">
      <div class="seasonal-chart-title">
        ...
        ${t?n`<span class="source-tag">${t==="learned"?s("seasonal_learned",r):s("seasonal_manual",r)}</span>`:d}
      </div>
      <svg viewBox="0 0 ${a} ${l}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",r)}">
        ...
        ${e.map((x,f)=>{let z=x/m*g,M=f*_+(_-v)/2,S=c+g-z,k=f===i,R=x<1?"var(--success-color, #4caf50)":x>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return O`
            <rect x="${M.toFixed(1)}" y="${S.toFixed(1)}"
              width="${v.toFixed(1)}" height="${z.toFixed(1)}"
              fill="${R}" opacity="${k?1:.5}" rx="2" />
          `})}
      </svg>
      <div class="seasonal-labels">
        ${li.map((x,f)=>n`<span class="seasonal-label ${f===i?"active-month":""}">${s(x,r)}</span>`)}
      </div>
    </div>
  `}var ys=300,xs=200;function pi(o,r,e,t){let i=o.history.filter(c=>c.type==="completed"&&(c.cost!=null||c.duration!=null));if(i.length<2)return d;let a=i.some(c=>(c.cost??0)>0),l=i.some(c=>(c.duration??0)>0);return!a&&!l?d:n`
  ...
  `}var ws=60,ks=20,ui=30,T=class extends A{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._sparklineTooltip=null;this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._adminPanelUserIds=[];this._operatorWriteEnabled=!1;this._defaultWarningDays=7;this._actionLoading=!1;this._moreMenuOpen=!1;this._toastMessage="";this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab="dashboard";this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._objectSortMode="alphabetical";this._groupByMode="none";this._objectViewMode="cards";this._objectsTableColumns=Be;this._showArchived=!1;this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._deepLinkHandled=!1;this._onDialogEvent=async()=>{try{await this._loadData()}catch{}};this._onCalendarLlCustom=e=>{let t=e.detail;t?.type==="maintenance-supporter:open-task"&&t.entry_id&&t.task_id&&(e.stopPropagation(),this._showTask(t.entry_id,t.task_id))};this._onHistoryEntrySaved=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"en"}get _isOperator(){let e=this.hass?.user;return e?e.is_admin?!1:!(this._operatorWriteEnabled&&this._adminPanelUserIds.includes(e.id)):!0}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler);let e=localStorage.getItem("maintenance_supporter_sort");e&&["due_date","object","type","task_name","area","assigned_user","group"].includes(e)&&(this._sortMode=e);let t=localStorage.getItem("maintenance_supporter_object_sort");t&&["alphabetical","due_soonest","task_count"].includes(t)&&(this._objectSortMode=t);let i=localStorage.getItem("maintenance_supporter_groupby");i&&["none","area","group","user"].includes(i)&&(this._groupByMode=i);let a=localStorage.getItem("maintenance_supporter_object_view");(a==="cards"||a==="table")&&(this._objectViewMode=a)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){super.updated(e);let t=this.hass?.language;if(t&&!Fe(t)&&ie(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new Ke(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new de(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,i,a,l]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);if(e&&(this._objects=e.objects),t&&(this._stats=t),i&&(this._budget=i),a&&(this._groups=a.groups||{}),l){let c=l;this._features=c.features,this._adminPanelUserIds=c.admin_panel_user_ids||[],this._operatorWriteEnabled=c.operator_write_enabled??!1;let u=c.general?.default_warning_days;typeof u=="number"&&u>=0&&u<=365&&(this._defaultWarningDays=u),this._objectsTableColumns=Ce(c.objects_table_columns)}this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("ms_action"),i=()=>{let m=window.location.pathname+window.location.hash;history.replaceState(history.state,"",m)};if(t==="add_object"){this._deepLinkHandled=!0,i(),requestAnimationFrame(()=>{this.shadowRoot?.querySelector("maintenance-object-dialog")?.openCreate()});return}if(t==="open_vacation"||t==="open_budget"||t==="open_groups"||t==="open_settings"){this._deepLinkHandled=!0,i(),this._overviewTab="settings",requestAnimationFrame(()=>{let m=this.shadowRoot?.querySelector("maintenance-settings-view"),_=t.replace("open_","");m?.scrollToSection?.(_)});return}let a=e.get("entry_id");if(!a)return;this._deepLinkHandled=!0;let l=e.get("task_id"),c=e.get("action"),u=window.location.pathname+window.location.hash;history.replaceState(history.state,"",u);let g=this._getObject(a);if(!g){this._showOverview();return}if(l){let m=g.tasks.find(_=>_.id===l);if(!m){this._showObject(a);return}this._showTask(a,l),c==="complete"?requestAnimationFrame(()=>{this._openCompleteDialog(a,l,m.name,this._features.checklists?m.checklist:void 0,this._features.adaptive&&!!m.adaptive_config?.enabled)}):c==="quick_complete"&&requestAnimationFrame(()=>{this._handleQuickComplete(a,l,m)})}else this._showObject(a)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let i=await this._statsService.getDetailStats(e,t),a=new Map(this._detailStatsData);a.set(e,i),this._detailStatsData=a}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let i of this._objects)for(let a of i.tasks){let l=a.trigger_config?.entity_id;l&&e.push({entityId:l,isCounter:this._isCounterEntity(a.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let _ of this._objects)for(let v of _.tasks){if(!this._showArchived&&v.archived||this._filterStatus&&v.status!==this._filterStatus)continue;if(this._filterUser){let x=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(v.responsible_user_id!==x)continue}let b=[];for(let x of Object.values(this._groups))x.task_refs?.some(f=>f.entry_id===_.entry_id&&f.task_id===v.id)&&b.push(x.name);e.push({entry_id:_.entry_id,task_id:v.id,object_name:_.object.name,task_name:v.name,type:v.type,schedule_type:v.schedule_type,status:v.status,days_until_due:v.days_until_due??null,next_due:v.next_due??null,trigger_active:v.trigger_active,trigger_current_value:v.trigger_current_value??null,trigger_current_delta:v.trigger_current_delta??null,trigger_config:v.trigger_config??null,trigger_entity_info:v.trigger_entity_info??null,times_performed:v.times_performed,total_cost:v.total_cost,interval_days:v.interval_days??null,interval_unit:v.interval_unit??null,interval_anchor:v.interval_anchor??null,is_done:v.is_done??!1,archived:v.archived??!1,history:v.history||[],enabled:v.enabled,nfc_tag_id:v.nfc_tag_id??null,area_id:_.object.area_id??null,responsible_user_id:v.responsible_user_id??null,group_names:b})}let t={overdue:0,triggered:1,due_soon:2,ok:3},i=(_,v)=>(t[_.status]??9)-(t[v.status]??9),a=(_,v)=>(_.days_until_due??99999)-(v.days_until_due??99999),l=(_,v)=>i(_,v)||a(_,v),c=_=>_.area_id&&this.hass?.areas?.[_.area_id]?.name||"",u=_=>_.responsible_user_id&&this._userService?.getUserName(_.responsible_user_id)||"",g=_=>_.group_names[0]||"",m={due_date:l,object:(_,v)=>_.object_name.localeCompare(v.object_name)||l(_,v),type:(_,v)=>_.type.localeCompare(v.type)||l(_,v),task_name:(_,v)=>_.task_name.localeCompare(v.task_name),area:(_,v)=>{let b=c(_),x=c(v);return!b&&x?1:b&&!x?-1:b.localeCompare(x)||l(_,v)},assigned_user:(_,v)=>{let b=u(_),x=u(v);return!b&&x?1:b&&!x?-1:b.localeCompare(x)||l(_,v)},group:(_,v)=>{let b=g(_),x=g(v);return!b&&x?1:b&&!x?-1:b.localeCompare(x)||l(_,v)}};return e.sort(m[this._sortMode]),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(a=>a.id===t)}_pushPanelState(e,t,i){let a={msp_view:e,msp_entry:t||null,msp_task:i||null};history.pushState(a,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let i=this._getTask(t.msp_entry,t.msp_task);i?.trigger_config?.entity_id&&this._fetchDetailStats(i.trigger_config.entity_id,this._isCounterEntity(i.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1,this._scrollContentToTop()}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null,this._scrollContentToTop()}_filterByStatus(e){this._filterStatus=e,this._overviewTab!=="dashboard"&&(this._overviewTab="dashboard"),this._scrollContentToTop()}_scrollContentToTop(){requestAnimationFrame(()=>{let e=this.shadowRoot?.querySelector(".content");e&&e.scrollTo({top:0,behavior:"smooth"})})}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null,this._scrollContentToTop()}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null,this._scrollContentToTop();let i=this._getTask(e,t);if(i?.trigger_config?.entity_id){let a=i.trigger_config.entity_id,l=this._isCounterEntity(i.trigger_config);this._fetchDetailStats(a,l)}}_showToast(e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}async _deleteObject(e){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("delete",this._lang),message:s("confirm_delete_object",this._lang),confirmText:s("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _deleteTask(e,t){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("delete",this._lang),message:s("confirm_delete_task",this._lang),confirmText:s("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _toggleArchiveTask(e,t,i){this._actionLoading=!0;try{await this.hass.connection.sendMessagePromise({type:i?"maintenance_supporter/task/unarchive":"maintenance_supporter/task/archive",entry_id:e,task_id:t}),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}finally{this._actionLoading=!1}}async _toggleArchiveObject(e,t){if(!(!t&&!await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("archive_object",this._lang),message:s("confirm_archive_object",this._lang),confirmText:s("archive_object",this._lang)})))try{await this.hass.connection.sendMessagePromise({type:t?"maintenance_supporter/object/unarchive":"maintenance_supporter/object/archive",entry_id:e}),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _skipTask(e,t,i){this._actionLoading=!0;try{let a={type:"maintenance_supporter/task/skip",entry_id:e,task_id:t};i&&(a.reason=i),await this.hass.connection.sendMessagePromise(a),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}finally{this._actionLoading=!1}}async _resetTask(e,t,i){this._actionLoading=!0;try{let a={type:"maintenance_supporter/task/reset",entry_id:e,task_id:t};i&&(a.date=i),await this.hass.connection.sendMessagePromise(a),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}finally{this._actionLoading=!1}}async _applySuggestion(e,t,i){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:i}),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}_openSeasonalOverrides(e){let t=this.shadowRoot.querySelector("maintenance-seasonal-overrides-dialog");if(!t||!this._selectedEntryId)return;let i=e.adaptive_config?.seasonal_overrides;t.open(this._selectedEntryId,e.id,i)}async _reanalyzeInterval(e,t){try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:e,task_id:t});i.recommended_interval?this._showToast(`${s("reanalyze_result",this._lang)}: ${i.recommended_interval} ${s("days",this._lang)} (${s(`confidence_${i.confidence}`,this._lang)}, ${i.data_points} ${s("data_points",this._lang)})`):this._showToast(s("reanalyze_insufficient_data",this._lang)),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _promptSkipTask(e,t){let i=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!i)return;let a=await i.prompt({title:s("skip",this._lang),message:s("skip_reason_prompt",this._lang),confirmText:s("skip",this._lang),inputLabel:s("reason_optional",this._lang),inputType:"text"});a.confirmed&&this._skipTask(e,t,a.value||void 0)}async _promptResetTask(e,t){let i=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!i)return;let a=await i.prompt({title:s("reset",this._lang),message:s("reset_date_prompt",this._lang),confirmText:s("reset",this._lang),inputLabel:s("reset_date_optional",this._lang),inputType:"date"});a.confirmed&&this._resetTask(e,t,a.value||void 0)}_dismissSuggestion(e,t){e&&t&&this._dismissedSuggestions.add(`${e}_${t}`),this.requestUpdate()}async _handleQuickComplete(e,t,i){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/quick_complete",entry_id:e,task_id:t}),this._showToast(s("quick_complete_success",this._lang))}catch(a){(a?.code||"")==="no_defaults"?this._openCompleteDialog(e,t,i.name,this._features.checklists?i.checklist:void 0,this._features.adaptive&&!!i.adaptive_config?.enabled):this._showToast(s("action_error",this._lang))}}_openCompleteDialog(e,t,i,a,l){let c=this.shadowRoot.querySelector("maintenance-complete-dialog");c&&(c.entryId=e,c.taskId=t,c.taskName=i,c.lang=this._lang,c.checklist=a||[],c.adaptiveEnabled=!!l,c.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,i,a){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,i,a)}render(){return n`
      ...
      <div class="header">
        ...
        <div class="breadcrumbs">
          ${e.map((t,i)=>n`
              ...
              ${t.action?n`<a @click=${t.action}>${t.label}</a>`:n`<span class="current">${t.label}</span>`}
            `)}
        </div>
      </div>
    `}_renderOverview(){let e=this._lang,t=!!this.hass?.user?.is_admin,i=this._stats;return!t&&this._overviewTab==="settings"&&(this._overviewTab="dashboard"),n`
      ${i?n`
            <div class="stats-bar">
      ...
      ${this._overviewTab==="dashboard"?this._renderDashboard():this._overviewTab==="calendar"?n`
    ...
    `}_renderDashboard(){let e=this._stats,t=this._taskRows,i=this._lang,a=this._isOperator,l=this._objects.reduce((c,u)=>c+u.tasks.filter(g=>g.archived).length,0);return n`
      ...
      <div class="filter-bar">
        ...
        <label class="filter-field">
          ...
          <select
            ...
            @change=${c=>{let u=c.target.value;this._filterUser=u||null}}
          >
        ...
        <label class="filter-field">
          ...
          >
            ...
            ${this._features.groups?n`<option value="group" ?selected=${this._groupByMode==="group"}>${s("groupby_group",i)}</option>`:d}
      ...
      ${a?d:n`<maintenance-storage-section-card
            .hass=${this.hass}
            .objects=${this._objects}
          ></maintenance-storage-section-card>`}
    `}_renderGroupedTasks(e,t){let i=new Map,a=s("unassigned",t);for(let u of e){let g=[];this._groupByMode==="area"?g=[(u.area_id?this.hass?.areas?.[u.area_id]?.name:null)||a]:this._groupByMode==="user"?g=[(u.responsible_user_id?this._userService?.getUserName(u.responsible_user_id):null)||a]:this._groupByMode==="group"&&(g=u.group_names.length>0?u.group_names:[a]);for(let m of g)i.has(m)||i.set(m,[]),i.get(m).push(u)}let l=[...i.entries()].sort(([u],[g])=>u===a&&g!==a?1:g===a&&u!==a?-1:u.localeCompare(g)),c=this._groupByMode==="area"?"mdi:map-marker-outline":this._groupByMode==="group"?"mdi:folder-outline":"mdi:account-outline";return n`
      ${l.map(([u,g])=>n`
      ...
      `)}
    `}_warrantyLabel(e,t,i){return e.kind==="expired"?s("warranty_expired",i):e.kind==="expiring"?s("warranty_expires_in",i).replace("{days}",String(e.days??0)):s("warranty_valid_until",i).replace("{date}",Z(t,i))}_renderWarrantyMeta(e,t){let i=ft(e);return n`<p class="meta">${s("warranty",t)}:
      <span class="warranty-chip warranty-${i.kind}">${this._warrantyLabel(i,e,t)}</span></p>`}_renderAllObjects(){let e=this._lang,t=this._isOperator,i=this._objectViewMode==="table"&&!this.narrow,a=this._objects.filter(m=>m.object.archived).length,l=m=>{let _=1/0;for(let v of m.tasks){let b=v.days_until_due;b!=null&&b<_&&(_=b)}return _},c=this._objects.filter(m=>this._showArchived||!m.object.archived);this._objectSortMode==="alphabetical"?c.sort((m,_)=>m.object.name.localeCompare(_.object.name)):this._objectSortMode==="task_count"?c.sort((m,_)=>_.tasks.length-m.tasks.length||m.object.name.localeCompare(_.object.name)):c.sort((m,_)=>l(m)-l(_)||m.object.name.localeCompare(_.object.name));let u=()=>{let m=new Map;for(let _ of c){let v=_.object.area_id,b=v?this.hass?.areas?.[v]?.name||s("unassigned",e):s("no_area",e);m.has(b)||m.set(b,[]),m.get(b).push(_)}return new Map([...m.entries()].sort(([_],[v])=>_.localeCompare(v)))},g=m=>{let _=m.tasks.some(v=>v.status==="overdue"||v.status==="triggered");return n`
        <div class="object-card${_?" object-card-overdue":""}" @click=${()=>this._showObject(m.entry_id)}>
          ${_?n`<span class="overdue-dot" title="${s("has_overdue",e)}"></span>`:d}
          <div class="object-card-header">
            <span class="object-card-name">${m.object.name}</span>
            ${m.object.document_count?n`<span class="doc-badge" title="${m.object.document_count} ${s("documents",e)}">
                  <ha-icon icon="mdi:paperclip"></ha-icon>${m.object.document_count}
                </span>`:d}
            <span class="object-card-count">${m.tasks.length} ${s("tasks_lower",e)}</span>
          </div>
          ${m.object.manufacturer||m.object.model?n`<div class="object-card-meta">${[m.object.manufacturer,m.object.model].filter(Boolean).join(" ")}</div>`:d}
          ${m.tasks.length===0?n`<div class="object-card-empty">${s("no_tasks_yet",e)}</div>`:d}
        </div>
      `};return n`
      ...
      <div class="filter-bar">
        <label class="filter-field">
          ...
          <select
            ...
            @change=${m=>{this._objectSortMode=m.target.value,localStorage.setItem("maintenance_supporter_object_sort",this._objectSortMode)}}
          >
        ...
        <label class="filter-field">
          ...
          <select
            .value=${this._groupByMode}
            @change=${m=>{this._groupByMode=m.target.value,localStorage.setItem("maintenance_supporter_groupby",this._groupByMode)}}
          >
      ...
      ${i?this._renderObjectsTable(c):this._groupByMode==="area"?n`
          ${[...u().entries()].map(([m,_])=>n`
            <details class="group-section" open>
              ...
              <div class="objects-grid">${_.map(g)}</div>
            </details>
          `)}
        `:n`<div class="objects-grid">${c.map(g)}</div>`}
    `}_setObjectViewMode(e){this._objectViewMode=e,localStorage.setItem("maintenance_supporter_object_view",e)}async _exportObjectsCsv(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects/csv"}),t=new Date().toISOString().slice(0,10);We(e.csv,`maintenance_objects_${t}.csv`,"text/csv;charset=utf-8")}catch{this._showToast(s("action_error",this._lang))}}_renderObjectsTable(e){let t=this._lang,i=this._objectsTableColumns;return n`
      <div class="objects-table-wrap">
        <table class="objects-table">
          <thead>
            <tr>
              ${i.map(a=>{let l=be.find(u=>u.key===a),c=l&&l.key!=="actions"?s(l.labelKey,t):"";return n`<th class="oc-${a}">${c}</th>`})}
            </tr>
          </thead>
          <tbody>
            ${e.map(a=>n`
    ...
    `}_renderObjectCell(e,t,i){let a=t.object;switch(e){case"name":return n`<td class="oc-name">
          <span class="objects-table-name">${a.name}</span>
          ${a.document_count?n`<span class="doc-badge" title="${a.document_count} ${s("documents",i)}">
                <ha-icon icon="mdi:paperclip"></ha-icon>${a.document_count}
              </span>`:d}
        </td>`;case"manufacturer":return n`<td class="oc-manufacturer">${a.manufacturer||"\u2014"}</td>`;case"model":return n`<td class="oc-model">${a.model||"\u2014"}</td>`;case"serial_number":return n`<td class="oc-serial_number">${a.serial_number||"\u2014"}</td>`;case"installation_date":return n`<td class="oc-installation_date">${a.installation_date?Z(a.installation_date,i):"\u2014"}</td>`;case"warranty_expiry":return n`<td class="oc-warranty_expiry">${this._renderWarrantyCell(a.warranty_expiry,i)}</td>`;case"area_id":{let l=a.area_id?this.hass?.areas?.[a.area_id]?.name||a.area_id:"\u2014";return n`<td class="oc-area_id">${l}</td>`}case"documentation_url":return n`<td class="oc-documentation_url">${a.documentation_url&&/^https?:\/\//i.test(a.documentation_url)?n`<a href=${a.documentation_url} target="_blank" rel="noopener noreferrer"
                @click=${l=>l.stopPropagation()}><ha-icon icon="mdi:file-document-outline"></ha-icon></a>`:"\u2014"}</td>`;case"notes":return n`<td class="oc-notes" title=${a.notes||""}>${a.notes||"\u2014"}</td>`;case"task_count":return n`<td class="oc-task_count">${t.tasks.length}</td>`;case"actions":return n`<td class="oc-actions">
          <mwc-icon-button title="${s("qr_code",i)}" @click=${l=>{l.stopPropagation(),this._openQrForObject(t.entry_id,a.name)}}>
            <ha-icon icon="mdi:qrcode"></ha-icon>
          </mwc-icon-button>
        </td>`;default:return n`<td></td>`}}_renderWarrantyCell(e,t){let i=ft(e);return i.kind==="none"?n`<span class="warranty-none">—</span>`:n`<span class="warranty-chip warranty-${i.kind}">${this._warrantyLabel(i,e,t)}</span>`}async _onSettingsChanged(){await this._loadData()}_renderGroupsSection(){if(!this._features.groups)return d;let e=Object.entries(this._groups),t=this._lang;return n`
      <div class="groups-section">
        ...
        ${e.length===0?n`<div class="hint">${s("no_groups",t)}</div>`:n`
            <div class="groups-grid">
              ${e.map(([i,a])=>{let l=a.task_refs.map(c=>this._getTask(c.entry_id,c.task_id)?.name).filter(Boolean);return n`
                  <div class="group-card">
                    ...
                    ${a.description?n`<div class="group-card-desc">${a.description}</div>`:d}
                    <div class="group-card-tasks">
                      ${l.length>0?l.map(c=>n`<span class="group-task-chip">${c}</span>`):n`<span style="font-size:12px;color:var(--secondary-text-color)">${s("no_tasks_short",t)}</span>`}
                    </div>
                  </div>
                `})}
            </div>
    ...
    `}_renderOverviewRow(e){let t=this._lang,i=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,a=0,l=Oe.ok,c=!1;if(i&&e.days_until_due!==null){let _=bt(e.interval_days,e.days_until_due,e.interval_unit);a=_.pct,c=_.overflow,e.status==="overdue"?l=Oe.overdue:e.status==="due_soon"&&(l=Oe.due_soon)}let u=e.area_id?this.hass?.areas?.[e.area_id]?.name:null,g=e.responsible_user_id?this._userService?.getUserName(e.responsible_user_id):null,m=e.group_names.length>0||u||g;return n`
      <div class="task-row${e.enabled?"":" task-disabled"}">
        <span class="cell-badges">
          ...
          ${e.enabled?d:n`<span class="badge-disabled">${s("disabled",t)}</span>`}
          ${e.nfc_tag_id?n`<span class="nfc-badge" title="${s("nfc_linked",t)}"><ha-icon icon="mdi:nfc-variant"></ha-icon></span>`:d}
        </span>
        ...
        <span class="due-cell" @click=${()=>this._showTask(e.entry_id,e.task_id)}>
          <span class="due-text">${Ne(e.days_until_due,t)}</span>
          ${i?n`<div class="days-bar"><div class="days-bar-fill${c?" overflow":""}" style="width:${a}%;background:${l}"></div></div>`:d}
          ${e.trigger_config?this._renderTriggerProgress(e):!i&&e.trigger_active?n`<span style="color:var(--maint-triggered-color);font-weight:600">⚡</span>`:d}
          ${this._renderMiniSparkline(e)}
        </span>
        <span class="row-actions">
          <mwc-icon-button class="btn-complete" title="${s("complete",t)}" @click=${_=>{_.stopPropagation(),this._openCompleteDialogForRow(e)}}>
            <ha-icon icon="mdi:check"></ha-icon>
          </mwc-icon-button>
          <mwc-icon-button class="btn-skip" title="${s("skip",t)}" .disabled=${this._actionLoading} @click=${_=>{_.stopPropagation(),this._promptSkipTask(e.entry_id,e.task_id)}}>
            <ha-icon icon="mdi:skip-next"></ha-icon>
          </mwc-icon-button>
        </span>
      </div>
    `}_openCompleteDialogForRow(e){let i=this._objects.find(a=>a.entry_id===e.entry_id)?.tasks.find(a=>a.id===e.task_id);this._openCompleteDialog(e.entry_id,e.task_id,e.task_name,this._features.checklists?i?.checklist:void 0,this._features.adaptive&&!!i?.adaptive_config?.enabled)}_renderTriggerProgress(e){let t=e.trigger_config??null;if(!t)return d;let i=t.type||"threshold",a=e.trigger_entity_info?.unit_of_measurement??"",l=0,c="";if(i==="threshold"){let m=e.trigger_current_value??null;if(m==null)return d;let _=t.trigger_above,v=t.trigger_below;if(_!=null){let b=v??0,x=_-b||1;l=Math.min(100,Math.max(0,(m-b)/x*100)),c=`${m.toFixed(1)} / ${_} ${a}`}else if(v!=null){let x=e.trigger_entity_info?.max??(v*2||100),f=x-v||1;l=Math.min(100,Math.max(0,(x-m)/f*100)),c=`${m.toFixed(1)} / ${v} ${a}`}else return d}else if(i==="counter"){let m=t.trigger_target_value||1,v=e.trigger_current_delta??null??e.trigger_current_value??null;if(v==null)return d;l=Math.min(100,Math.max(0,v/m*100)),c=`${v.toFixed(1)} / ${m} ${a}`}else if(i==="state_change"){let m=t.trigger_target_changes||1,_=e.trigger_current_value??null;if(_==null)return d;l=Math.min(100,Math.max(0,_/m*100)),c=`${Math.round(_)} / ${m}`}else if(i==="runtime"){let m=t.trigger_runtime_hours||100,_=e.trigger_current_value??null;if(_==null)return d;l=Math.min(100,Math.max(0,_/m*100)),c=`${_.toFixed(1)}h / ${m}h`}else if(i==="compound"){let m=t.compound_logic||t.operator||"AND",_=t.conditions?.length||0;c=`${m} (${_})`,l=e.trigger_active?100:0}else return d;let u=l>=100,g=l>90?"var(--error-color, #f44336)":l>70?"var(--warning-color, #ff9800)":"var(--primary-color)";return n`
      <div class="trigger-progress">
        <div class="trigger-progress-bar">
          <div class="trigger-progress-fill${u?" overflow":""}" style="width:${l}%;background:${g}"></div>
        </div>
        <span class="trigger-progress-label">${c}</span>
      </div>
    `}_renderMiniSparkline(e){if(!e.trigger_config?.entity_id)return d;let t=e.trigger_config.entity_id,i=this._miniStatsData.get(t)||[],a=[];if(i.length>=2)a=i.map(k=>({ts:k.ts,val:k.val}));else{if(!e.history)return d;for(let k of e.history)k.trigger_value!=null&&a.push({ts:new Date(k.timestamp).getTime(),val:k.trigger_value})}if(e.trigger_current_value!=null&&a.push({ts:Date.now(),val:e.trigger_current_value}),a.length<2)return d;a.sort((k,R)=>k.ts-R.ts);let l=ws,c=ks,u=a.map(k=>k.val),g=Math.min(...u),m=Math.max(...u),_=m-g||1;g-=_*.1,m+=_*.1;let v=a[0].ts,x=a[a.length-1].ts-v||1,f=k=>(k-v)/x*l,z=k=>2+(1-(k-g)/(m-g))*(c-4),M=a;if(M.length>ui){let k=Math.ceil(M.length/ui);M=M.filter((R,N)=>N%k===0||N===M.length-1)}let S=M.map(k=>`${f(k.ts).toFixed(1)},${z(k.val).toFixed(1)}`).join(" ");return n`
      <svg class="mini-sparkline" viewBox="0 0 ${l} ${c}" preserveAspectRatio="none" role="img" aria-label="${s("chart_mini_sparkline",this._lang)}">
        <polyline points="${S}" fill="none" stroke="var(--primary-color)" stroke-width="1.5" stroke-linejoin="round" />
      </svg>
    `}_renderDaysProgress(e){let t=this._lang;if(e.days_until_due==null||!e.interval_days||e.interval_days<=0)return d;let{pct:i,overflow:a}=bt(e.interval_days,e.days_until_due,e.interval_unit),l="var(--success-color, #4caf50)";return e.status==="overdue"?l="var(--error-color, #f44336)":e.status==="due_soon"&&(l="var(--warning-color, #ff9800)"),n`
      <div class="days-progress">
        <div class="days-progress-labels">
          <span>${e.last_performed?`${s("last_performed",t)}: ${Z(e.last_performed,t)}`:""}</span>
          <span>${e.next_due?`${s("next_due",t)}: ${Z(e.next_due,t)}`:""}</span>
        </div>
        <div class="days-progress-bar" role="progressbar" aria-valuenow="${Math.round(i)}" aria-valuemin="0" aria-valuemax="100" aria-label="${s("days_progress",t)}">
          <div class="days-progress-fill${a?" overflow":""}" style="width:${i}%;background:${l}"></div>
        </div>
        <div class="days-progress-text">${Ne(e.days_until_due,t)}</div>
      </div>
    `}_renderObjectDetail(){if(!this._selectedEntryId)return d;let e=this._getObject(this._selectedEntryId);if(!e)return n`<p>Object not found.</p>`;let t=e.object,i=this._lang,a=this._isOperator,l=e.tasks.filter(u=>u.archived).length,c=e.tasks.filter(u=>this._showArchived||!u.archived);return n`
      <div class="detail-section">
        ...
        ${t.manufacturer||t.model?n`<p class="meta">${[t.manufacturer,t.model].filter(Boolean).join(" ")}</p>`:d}
        ${t.serial_number?n`<p class="meta">${s("serial_number_label",i)}: ${t.serial_number}</p>`:d}
        ${t.documentation_url&&/^https?:\/\//i.test(t.documentation_url)?n`<p class="meta">${s("documentation_url_label",i)}:
              <a href=${t.documentation_url} target="_blank" rel="noopener noreferrer">${t.documentation_url}</a>
            </p>`:d}
        ${t.installation_date?n`<p class="meta">${s("installed",i)}: ${Z(t.installation_date,i)}</p>`:d}
        ${t.warranty_expiry?this._renderWarrantyMeta(t.warranty_expiry,i):d}
        ${t.notes?n`<div class="object-notes">
        ...
        ${e.tasks.length===0?n`<div class="empty-state-centered">
            ...
            </div>`:n`<div class="task-table">${[...c].sort((u,g)=>{let m={overdue:0,triggered:1,due_soon:2,ok:3};return(m[u.status]??9)-(m[g.status]??9)||(u.days_until_due??99999)-(g.days_until_due??99999)}).map(u=>n`
              <div class="task-row${u.enabled?"":" task-disabled"}">
                <span class="cell-badges">
                  <span class="status-badge ${u.archived?"archived":u.is_done?"done":u.status}">${u.archived?s("archived",i):u.is_done?s("completed",i):s(u.status,i)}</span>
                  ${u.enabled?d:n`<span class="badge-disabled">${s("disabled",i)}</span>`}
                  ${u.nfc_tag_id?n`<span class="nfc-badge" title="${s("nfc_linked",i)}"><ha-icon icon="mdi:nfc-variant"></ha-icon></span>`:d}
                </span>
                <span class="cell task-name" @click=${()=>this._showTask(e.entry_id,u.id)}>${u.name}</span>
                <span class="task-sub${u.responsible_user_id?"":" task-sub-empty"}">${this._renderUserBadge(u)}</span>
                <span class="cell type">${s(u.type,i)}</span>
                <span class="due-cell" @click=${()=>this._showTask(e.entry_id,u.id)}>
                  <span class="due-text">${Ne(u.days_until_due,i)}</span>
                  ${u.trigger_config?this._renderTriggerProgress(u):d}
                  ${this._renderMiniSparkline(u)}
                </span>
                <span class="row-actions">
                  <mwc-icon-button class="btn-complete" title="${s("complete",i)}" @click=${g=>{g.stopPropagation(),this._openCompleteDialog(e.entry_id,u.id,u.name,this._features.checklists?u.checklist:void 0,this._features.adaptive&&!!u.adaptive_config?.enabled)}}>
                    <ha-icon icon="mdi:check"></ha-icon>
                  </mwc-icon-button>
                  <mwc-icon-button class="btn-skip" title="${s("skip",i)}" .disabled=${this._actionLoading} @click=${g=>{g.stopPropagation(),this._promptSkipTask(e.entry_id,u.id)}}>
                    <ha-icon icon="mdi:skip-next"></ha-icon>
                  </mwc-icon-button>
                </span>
              </div>
    ...
    `}_renderTaskHeader(e){let t=this._lang,a=this._getObject(this._selectedEntryId)?.object.name||"",l=this._isOperator,c=e.archived?"archived":e.is_done?"done":e.status==="due_soon"?"warning":e.status||"ok",u=e.archived?s("archived",t):e.is_done?s("completed",t):s(e.status||"ok",t);return n`
      <div class="task-header">
        <div class="task-header-title">
          ...
          ${e.nfc_tag_id?n`<span class="nfc-badge" title="${s("nfc_tag_id",t)}: ${e.nfc_tag_id}"><ha-icon icon="mdi:nfc-variant"></ha-icon> NFC</span>`:l?d:n`<span class="nfc-badge unlinked" title="${s("nfc_link_hint",t)}"
    ...
    `:d}_renderTabBar(){let e=this._lang;return n`
      <div class="tab-bar">
    ...
    `}_renderChecklistCard(e){if(!this._features.checklists)return d;let t=e.checklist||[];if(t.length===0)return d;let i=this._lang;return n`
    ...
    `}_renderHistoryTab(e){let t=this._lang;return n`
      <div class="tab-content history-tab">
        ${this._renderHistoryFilters(e)}
        ${this._renderHistoryList(e)}
      </div>
    `}_renderTaskMeta(e){let t=e.documentation_url&&/^https?:\/\//i.test(e.documentation_url)?e.documentation_url:null,i=this._selectedEntryId?this._getObject(this._selectedEntryId):void 0,a=i?.object?.documentation_url,l=a&&/^https?:\/\//i.test(a)?a:null;if(!e.notes&&!t&&!l)return d;let c=this._lang;return n`
    ...
    `}_renderKPIBar(e){let t=this._lang,i=e.times_performed>0?e.total_cost/e.times_performed:0,a=e.days_until_due!==null&&e.days_until_due!==void 0?e.days_until_due<0?"overdue":e.days_until_due<=e.warning_days?"warning":"":"";return n`
      <div class="kpi-bar">
        <div class="kpi-card">
          <div class="kpi-label">${s("next_due",t)}</div>
          <div class="kpi-value">${e.next_due?Z(e.next_due,t):"\u2014"}</div>
          ${this._features.schedule_time&&e.schedule_time?n`<div class="kpi-subtext">${s("at_time",t)} ${e.schedule_time}</div>`:d}
        </div>
        ...
        <div class="kpi-card">
          ...
          <div class="kpi-value">${Wt(e,t)}</div>
          ${this._features.adaptive&&e.suggested_interval&&e.suggested_interval!==e.interval_days?n`
    ...
    `}_renderRecommendationCard(e){let t=this._lang;if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return d;if(this._selectedEntryId&&this._selectedTaskId&&this._dismissedSuggestions.has(`${this._selectedEntryId}_${this._selectedTaskId}`))return d;let i=e.suggested_interval;return n`
      <div class="recommendation-card">
        <h4>${s("suggested_interval",t)}</h4>
        ${oi(e.interval_days,i,e.interval_confidence||"medium",t)}
    ...
    `}_renderRecentActivities(e){let t=this._lang,i=e.history.slice(-3).reverse();if(i.length===0)return d;let a=l=>{switch(l){case"completed":return"\u2713";case"triggered":return"\u2297";case"skipped":return"\u21B7";case"reset":return"\u21BA";default:return"\xB7"}};return n`
      <div class="recent-activities">
        <h3>${s("recent_activities",t)}</h3>
        ${i.map(l=>n`
          <div class="activity-item">
            <span class="activity-icon">${a(l.type)}</span>
            <span class="activity-date">${mt(l.timestamp,t)}</span>
            <span class="activity-note">${l.notes||"\u2014"}</span>
            ${l.cost?n`<span class="activity-badge">${l.cost.toFixed(0)}${this._budget?.currency_symbol||"\u20AC"}</span>`:d}
            ${l.duration?n`<span class="activity-badge">${l.duration}min</span>`:d}
          </div>
        `)}
    ...
    `}_renderHistoryFilters(e){let t=this._lang;return n`
      <div class="history-filters-new">
        <div class="filter-chips">
          ${["completed","skipped","reset","triggered"].map(i=>{let a=e.history.filter(l=>l.type===i).length;return a===0?d:n`
              <span class="filter-chip ${this._historyFilter===i?"active":""}"
                @click=${()=>{this._historyFilter=this._historyFilter===i?null:i}}>
                ${s(i,t)} (${a})
              </span>
            `})}
          ${this._historyFilter?n`<span class="filter-chip clear" @click=${()=>{this._historyFilter=null}}>${s("show_all",t)}</span>`:d}
        </div>
    ...
    `}_renderHistoryList(e){let t=this._lang,i=this._historyFilter?e.history.filter(a=>a.type===this._historyFilter):e.history;if(this._historySearch){let a=this._historySearch.toLowerCase();i=i.filter(l=>l.notes?.toLowerCase().includes(a))}return i.length===0?n`<p class="empty">${s("no_history",t)}</p>`:n`
      <div class="history-timeline">
        ${[...i].reverse().map(a=>this._renderHistoryEntry(a))}
      </div>
    `}_renderTaskDetail(){if(!this._selectedEntryId||!this._selectedTaskId)return d;let e=this._getTask(this._selectedEntryId,this._selectedTaskId);if(!e)return n`<p>Task not found.</p>`;let t=this._lang;return n`
      <div class="detail-section">
    ...
    `}_renderHistoryEntry(e){let t=this._lang,i=["completed","reset","skipped"].includes(e.type);return n`
      <div class="history-entry">
        <div class="history-icon ${e.type}">
          <ha-icon .icon=${Ut[e.type]||"mdi:circle"}></ha-icon>
        </div>
        <div class="history-content">
          <div class="history-row">
            <strong>${s(e.type,t)}</strong>
            ${i?n`<button class="history-edit-btn"
                       title=${s("history_edit_button",t)||"Edit entry"}
                       @click=${()=>this._openHistoryEdit(e)}>
                  <ha-icon icon="mdi:pencil"></ha-icon>
                </button>`:d}
          </div>
          <div class="history-date">${mt(e.timestamp,t)}</div>
          ${e.notes?n`<div>${e.notes}</div>`:d}
          <div class="history-details">
            ${e.cost!=null?n`<span>${s("cost",t)}: ${e.cost.toFixed(2)} ${this._budget?.currency_symbol||"\u20AC"}</span>`:d}
            ${e.duration!=null?n`<span>${s("duration",t)}: ${e.duration} min</span>`:d}
            ${e.trigger_value!=null?n`<span>${s("trigger_val",t)}: ${e.trigger_value}</span>`:d}
          </div>
        </div>
      </div>
    `}_openHistoryEdit(e){if(!this._selectedEntryId||!this._selectedTaskId)return;let t={entry_id:this._selectedEntryId,task_id:this._selectedTaskId,original_timestamp:e.timestamp,type:e.type,timestamp:e.timestamp,notes:e.notes??null,cost:e.cost??null,duration:e.duration??null,completed_by:e.completed_by??null};this.shadowRoot?.querySelector("maintenance-history-edit-dialog")?.openEdit(t)}};T.styles=[Ue,Vt],p([w({attribute:!1})],T.prototype,"hass",2),p([w({type:Boolean,reflect:!0})],T.prototype,"narrow",2),p([w({attribute:!1})],T.prototype,"panel",2),p([h()],T.prototype,"_objects",2),p([h()],T.prototype,"_stats",2),p([h()],T.prototype,"_view",2),p([h()],T.prototype,"_selectedEntryId",2),p([h()],T.prototype,"_selectedTaskId",2),p([h()],T.prototype,"_filterStatus",2),p([h()],T.prototype,"_filterUser",2),p([h()],T.prototype,"_unsub",2),p([h()],T.prototype,"_sparklineTooltip",2),p([h()],T.prototype,"_historyFilter",2),p([h()],T.prototype,"_budget",2),p([h()],T.prototype,"_groups",2),p([h()],T.prototype,"_detailStatsData",2),p([h()],T.prototype,"_miniStatsData",2),p([h()],T.prototype,"_features",2),p([h()],T.prototype,"_adminPanelUserIds",2),p([h()],T.prototype,"_operatorWriteEnabled",2),p([h()],T.prototype,"_defaultWarningDays",2),p([h()],T.prototype,"_actionLoading",2),p([h()],T.prototype,"_moreMenuOpen",2),p([h()],T.prototype,"_toastMessage",2),p([h()],T.prototype,"_overviewTab",2),p([h()],T.prototype,"_activeTab",2),p([h()],T.prototype,"_costDurationToggle",2),p([h()],T.prototype,"_historySearch",2),p([h()],T.prototype,"_sortMode",2),p([h()],T.prototype,"_objectSortMode",2),p([h()],T.prototype,"_groupByMode",2),p([h()],T.prototype,"_objectViewMode",2),p([h()],T.prototype,"_objectsTableColumns",2),p([h()],T.prototype,"_showArchived",2),T=p([Ot("maintenance-supporter-panel")],T);export{T as MaintenanceSupporterPanel};
