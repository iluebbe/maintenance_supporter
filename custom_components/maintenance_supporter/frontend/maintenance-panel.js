...
`;var Ne=class{constructor(s){this._cache=new Map;this._pending=new Map;this._hass=s}updateHass(s){this._hass=s}async getDetailStats(s,e){return this._getStats(s,"hour",30,e)}async getMiniStats(s,e){return this._getStats(s,"day",14,e)}async getBatchMiniStats(s){let e=new Map,t=[];for(let p of s){let u=`${p.entityId}:day`,h=this._cache.get(u);h&&Date.now()-h.fetchedAt<3e5?e.set(p.entityId,h.points):t.push(p)}if(t.length===0)return e;let a=t.filter(p=>p.isCounter).map(p=>p.entityId),n=t.filter(p=>!p.isCounter).map(p=>p.entityId),o=new Date(Date.now()-336*60*60*1e3).toISOString(),l=[];return a.length>0&&l.push(this._fetchBatch(a,"day",o,["state","sum","change"],!0,e)),n.length>0&&l.push(this._fetchBatch(n,"day",o,["mean","min","max"],!1,e)),await Promise.all(l),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(s,e,t,a){let n=`${s}:${e}`,o=this._cache.get(n);if(o&&Date.now()-o.fetchedAt<3e5)return o.points;if(this._pending.has(n))return this._pending.get(n);let l=this._fetchAndNormalize(s,e,t,a,n);this._pending.set(n,l);try{return await l}finally{this._pending.delete(n)}}async _fetchAndNormalize(s,e,t,a,n){let o=new Date(Date.now()-t*24*60*60*1e3).toISOString(),l=a?["state","sum","change"]:["mean","min","max"];try{let u=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:o,statistic_ids:[s],period:e,types:l}))[s]||[],h=this._normalizeRows(u,a);return this._cache.set(n,{entityId:s,fetchedAt:Date.now(),period:e,points:h}),h}catch(p){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${s}:`,p),[]}}async _fetchBatch(s,e,t,a,n,o){try{let l=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:s,period:e,types:a});for(let p of s){let u=l[p]||[],h=this._normalizeRows(u,n);o.set(p,h),this._cache.set(`${p}:${e}`,{entityId:p,fetchedAt:Date.now(),period:e,points:h})}}catch(l){console.warn("[maintenance-supporter] Batch statistics fetch failed:",l)}}_normalizeRows(s,e){let t=[];for(let a of s){let n=null;if(e?n=a.state??null:n=a.mean??null,n===null)continue;let o={ts:a.start,val:n};e||(a.min!=null&&(o.min=a.min),a.max!=null&&(o.max=a.max)),t.push(o)}return t.sort((a,n)=>a.ts-n.ts),t}};var oe=class{constructor(s){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=s}updateHass(s){this.hass=s}async getUsers(s=!1){let e=Date.now();if(!s&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(s,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:s,task_id:e,user_id:t})}async getTasksByUser(s){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:s})).tasks}getUserName(s){return!s||!this.usersCache?null:this.usersCache.find(t=>t.id===s)?.name||null}getUser(s){return!s||!this.usersCache?null:this.usersCache.find(e=>e.id===s)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(s){return s?s===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var Sa={name:"name",task_type:"maintenance_type",schedule_type:"schedule_type",interval_days:"interval_days",interval_anchor:"interval_anchor",warning_days:"warning_days",last_performed:"last_performed_optional",notes:"notes_optional",documentation_url:"documentation_url_optional",custom_icon:"custom_icon_optional",nfc_tag_id:"nfc_tag_id_optional",responsible_user_id:"responsible_user",entity_slug:"entity_slug",entity_id:"entity_id",area_id:"area_id_optional",manufacturer:"manufacturer_optional",model:"model_optional",serial_number:"serial_number_optional",installation_date:"installation_date_optional",checklist:"checklist_steps_optional",reason:"reason",feedback:"feedback",cost:"cost",duration:"duration",description:"description_optional",group_name:"name",group_description:"description_optional",environmental_entity:"environmental_entity_optional",environmental_attribute:"environmental_attribute_optional",trigger_above:"trigger_above",trigger_below:"trigger_below",trigger_for_minutes:"trigger_for_minutes"};function Ta(c,s){let e=Sa[c];if(!e)return c;let t=i(e,s);return t&&t!==e?t:c}function qa(c){let e=c.match(/data\['([^']+)'\]/)?.[1],t;return(t=c.match(/length of value must be at most (\d+)/))?{field:e,rule:"too_long",param:t[1]}:(t=c.match(/length of value must be at least (\d+)/))?{field:e,rule:"too_short",param:t[1]}:(t=c.match(/value must be at most (\S+)/))?{field:e,rule:"value_too_high",param:t[1]}:(t=c.match(/value must be at least (\S+)/))?{field:e,rule:"value_too_low",param:t[1]}:/required key not provided/.test(c)?{field:e,rule:"required"}:(t=c.match(/expected (\w+)/))?{field:e,rule:"wrong_type",param:t[1]}:/value must be one of/.test(c)?{field:e,rule:"invalid_choice"}:/not a valid value/.test(c)?{field:e,rule:"invalid_value"}:{field:e,rule:"unknown"}}function W(c,s,e){if(typeof c=="string")return c;if(typeof c!="object"||c===null)return e;let t=c,a=t.message||t.error?.message||"";if(!a)return e;let n=qa(a),o=n.field?Ta(n.field,s):"",l=p=>i(p,s).replace("{field}",o).replace("{n}",n.param??"");switch(n.rule){case"too_long":return l("err_too_long");case"too_short":return l("err_too_short");case"value_too_high":return l("err_value_too_high");case"value_too_low":return l("err_value_too_low");case"required":return l("err_required");case"wrong_type":return l("err_wrong_type").replace("{type}",n.param??"");case"invalid_choice":return l("err_invalid_choice");case"invalid_value":return l("err_invalid_value");default:return a||e}}var B=class extends T{constructor(){super(...arguments);this.label="";this.value="";this.placeholder="";this.type="text";this.required=!1;this.disabled=!1}_onInput(e){let t=e.target.value;this.value=t,this.dispatchEvent(new CustomEvent("input",{bubbles:!0,composed:!0,detail:{value:t}}))}render(){return r`
      <label class="field">
...
`),this._scheduleTime=t.schedule_time||"";let a=t.on_complete_action;if(a&&a.service){this._actionService=a.service;let l=a.target?.entity_id;this._actionTargetEntity=Array.isArray(l)?l[0]||"":l||"",this._actionData=a.data&&typeof a.data=="object"?{...a.data}:{},this._actionDataJsonFallback=""}else this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="";let n=t.quick_complete_defaults;this._qcNotes=n?.notes||"",this._qcCost=n?.cost!=null?String(n.cost):"",this._qcDuration=n?.duration!=null?String(n.duration):"",this._qcFeedback=n?.feedback||"";let o=t.adaptive_config||{};if(this._environmentalEntity=o.environmental_entity||"",this._environmentalAttribute=o.environmental_attribute||"",this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute,t.trigger_config){let l=t.trigger_config;this._triggerEntityId=l.entity_id||"",this._triggerEntityIds=l.entity_ids||(l.entity_id?[l.entity_id]:[]),this._triggerEntityLogic=l.entity_logic||"any",this._triggerAttribute=l.attribute||"",this._triggerType=l.type||"threshold",this._triggerAbove=l.trigger_above?.toString()||"",this._triggerBelow=l.trigger_below?.toString()||"",this._triggerForMinutes=l.trigger_for_minutes?.toString()||"0",this._triggerTargetValue=l.trigger_target_value?.toString()||"",this._triggerDeltaMode=l.trigger_delta_mode||!1,this._triggerFromState=l.trigger_from_state||"",this._triggerToState=l.trigger_to_state||"",this._triggerTargetChanges=l.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=l.trigger_runtime_hours?.toString()||""}else this._resetTriggerFields();this._triggerEntityId&&this._fetchEntityAttributes(this._triggerEntityId),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._warningDays=String(this.defaultWarningDays),this._intervalAnchor="completion",this._notes="",this._documentationUrl="",this._customIcon="",this._enabled=!0,this._lastPerformed="",this._nfcTagId="",this._responsibleUserId=null,this._checklistText="",this._scheduleTime="",this._environmentalEntity="",this._environmentalAttribute="",this._environmentalInitial="",this._environmentalAttributeInitial="",this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="",this._actionTesting=!1,this._actionTestResult="",this._qcNotes="",this._qcCost="",this._qcDuration="",this._qcFeedback="",this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerForMinutes="0",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours=""}async _loadUsers(){this._userService||(this._userService=new oe(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}async _testAction(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e)){this._actionTestResult="error",this._actionTestError="Invalid service format";return}let[t,a]=e.split("."),n=this._buildActionData(),o=this._actionTargetEntity.trim();if(o){let l=o.split(".")[0];if(l!==t&&!new Set(["homeassistant","scene","notify","persistent_notification"]).has(t)){this._actionTestResult="error",this._actionTestError=`Service "${e}" only works on ${t}.* entities; entity "${o}" is in ${l}.* \u2014 pick a service that matches the entity domain (e.g. ${l}.${a})`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}}this._actionTesting=!0,this._actionTestResult="",this._actionTestError="";try{let l=o?{entity_id:o}:void 0;await this.hass.callService(t,a,n,l),this._actionTestResult="ok"}catch(l){this._actionTestResult="error",this._actionTestError=String(l?.message??l).slice(0,240)}finally{this._actionTesting=!1,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3)}}_buildActionData(){if(this._actionDataJsonFallback.trim())try{let e=JSON.parse(this._actionDataJsonFallback);if(e&&typeof e=="object"&&!Array.isArray(e))return e}catch{}return{...this._actionData}}_serviceSchema(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e))return null;let[t,a]=e.split("."),n=this.hass?.services?.[t]?.[a]?.fields;return!n||Object.keys(n).length===0?null:Object.entries(n).map(([o,l])=>({name:o,required:!!l.required,selector:l.selector||{text:{}}}))}_renderCompletionActionsSection(e){if(!this.completionActionsEnabled)return d;let t=this._serviceSchema();return r`
      <details class="ca-section">
        ...
        <ha-service-picker
          ...
          @value-changed=${a=>{this._actionService=a.detail.value||"";let n=this._serviceSchema();if(n){let o=new Set(n.map(l=>l.name));this._actionData=Object.fromEntries(Object.entries(this._actionData).filter(([l])=>o.has(l)))}}}
        ></ha-service-picker>
        <ha-form
          .hass=${this.hass}
...
`).map(l=>l.trim()).filter(Boolean).slice(0,100);e.checklist=o.length?o:null}if(this.completionActionsEnabled){let o=this._actionService.trim();if(o&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(o)){let h={service:o},m=this._actionTargetEntity.trim();m&&(h.target={entity_id:m});let v=this._buildActionData();Object.keys(v).length>0&&(h.data=v),e.on_complete_action=h}else e.on_complete_action=null;let l={};this._qcNotes.trim()&&(l.notes=this._qcNotes.trim());let p=parseFloat(this._qcCost);!isNaN(p)&&p>=0&&(l.cost=p);let u=parseInt(this._qcDuration,10);!isNaN(u)&&u>=0&&(l.duration=u),this._qcFeedback&&(l.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(l).length?l:null}let t=await this.hass.connection.sendMessagePromise(e),a=this._taskId||t?.task_id,n=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(a&&this._scheduleType==="sensor_based"&&n)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:a,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=W(e,this._lang,i("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return d;let e=this._lang;return r`
      ...
      <div class="select-row">
        ...
        >
          ${Na.map(t=>r`<option value=${t} ?selected=${t===this._triggerType}>${i(t,e)}</option>`)}
        </select>
      </div>
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${Ca.map(a=>r`<option value=${a} ?selected=${a===this._type}>${i(a,e)}</option>`)}
            </select>
          </div>
          <div class="select-row">
            ...
            >
              ${Ma.map(a=>r`<option value=${a} ?selected=${a===this._scheduleType}>${i(a,e)}</option>`)}
            </select>
          </div>
  ...
  `,_([w({attribute:!1})],x.prototype,"hass",2),_([w({type:Boolean,attribute:"checklists-enabled"})],x.prototype,"checklistsEnabled",2),_([w({type:Boolean,attribute:"schedule-time-enabled"})],x.prototype,"scheduleTimeEnabled",2),_([w({type:Boolean,attribute:"completion-actions-enabled"})],x.prototype,"completionActionsEnabled",2),_([w({type:Number,attribute:"default-warning-days"})],x.prototype,"defaultWarningDays",2),_([g()],x.prototype,"_open",2),_([g()],x.prototype,"_loading",2),_([g()],x.prototype,"_error",2),_([g()],x.prototype,"_entryId",2),_([g()],x.prototype,"_taskId",2),_([g()],x.prototype,"_objectChoices",2),_([g()],x.prototype,"_name",2),_([g()],x.prototype,"_type",2),_([g()],x.prototype,"_scheduleType",2),_([g()],x.prototype,"_intervalDays",2),_([g()],x.prototype,"_warningDays",2),_([g()],x.prototype,"_intervalAnchor",2),_([g()],x.prototype,"_notes",2),_([g()],x.prototype,"_documentationUrl",2),_([g()],x.prototype,"_customIcon",2),_([g()],x.prototype,"_enabled",2),_([g()],x.prototype,"_triggerEntityId",2),_([g()],x.prototype,"_triggerEntityIds",2),_([g()],x.prototype,"_triggerEntityLogic",2),_([g()],x.prototype,"_triggerAttribute",2),_([g()],x.prototype,"_triggerType",2),_([g()],x.prototype,"_triggerAbove",2),_([g()],x.prototype,"_triggerBelow",2),_([g()],x.prototype,"_triggerForMinutes",2),_([g()],x.prototype,"_triggerTargetValue",2),_([g()],x.prototype,"_triggerDeltaMode",2),_([g()],x.prototype,"_triggerFromState",2),_([g()],x.prototype,"_triggerToState",2),_([g()],x.prototype,"_triggerTargetChanges",2),_([g()],x.prototype,"_triggerRuntimeHours",2),_([g()],x.prototype,"_suggestedAttributes",2),_([g()],x.prototype,"_availableAttributes",2),_([g()],x.prototype,"_entityDomain",2),_([g()],x.prototype,"_lastPerformed",2),_([g()],x.prototype,"_nfcTagId",2),_([g()],x.prototype,"_availableTags",2),_([g()],x.prototype,"_responsibleUserId",2),_([g()],x.prototype,"_availableUsers",2),_([g()],x.prototype,"_checklistText",2),_([g()],x.prototype,"_scheduleTime",2),_([g()],x.prototype,"_actionService",2),_([g()],x.prototype,"_actionTargetEntity",2),_([g()],x.prototype,"_actionData",2),_([g()],x.prototype,"_actionDataJsonFallback",2),_([g()],x.prototype,"_actionTesting",2),_([g()],x.prototype,"_actionTestResult",2),_([g()],x.prototype,"_actionTestError",2),_([g()],x.prototype,"_qcNotes",2),_([g()],x.prototype,"_qcCost",2),_([g()],x.prototype,"_qcDuration",2),_([g()],x.prototype,"_qcFeedback",2),_([g()],x.prototype,"_environmentalEntity",2),_([g()],x.prototype,"_environmentalAttribute",2);customElements.get("maintenance-task-dialog")||customElements.define("maintenance-task-dialog",x);var F=class extends T{constructor(){super(...arguments);this.entryId="";this.taskId="";this.taskName="";this.lang="en";this.checklist=[];this.adaptiveEnabled=!1;this._open=!1;this._notes="";this._cost="";this._duration="";this._loading=!1;this._error="";this._checklistState={};this._feedback="needed"}open(){this._open||(this._open=!0,this._notes="",this._cost="",this._duration="",this._error="",this._checklistState={},this._feedback="needed")}_toggleCheck(e){let t=String(e);this._checklistState={...this._checklistState,[t]:!this._checklistState[t]}}_setFeedback(e){this._feedback=e}async _complete(){this._loading=!0,this._error="";try{let e={type:"maintenance_supporter/task/complete",entry_id:this.entryId,task_id:this.taskId};if(this._notes&&(e.notes=this._notes),this._cost){let t=parseFloat(this._cost);!isNaN(t)&&t>=0&&(e.cost=t)}if(this._duration){let t=parseInt(this._duration,10);!isNaN(t)&&t>=0&&(e.duration=t)}this.checklist.length>0&&(e.checklist_state=this._checklistState),this.adaptiveEnabled&&(e.feedback=this._feedback),await this.hass.connection.sendMessagePromise(e),this._open=!1,this.dispatchEvent(new CustomEvent("task-completed"))}catch(e){this._error=W(e,this.lang,i("save_error",this.lang))}finally{this._loading=!1}}_close(){this._open=!1}render(){if(!this._open)return r``;let e=this.lang||this.hass?.language||"en";return r`
      <ha-dialog open @closed=${this._close}>
...
<script>setTimeout(()=>window.print(),300)<\/script>
</body></html>`),n.document.close()}_downloadSvg(e,t){let a=decodeURIComponent(e.svg_data_uri.replace("data:image/svg+xml,","")),n=new Blob([a],{type:"image/svg+xml"}),o=URL.createObjectURL(n),l=document.createElement("a");l.href=o;let p=this._taskName?`${this._objectName}-${this._taskName}`:this._objectName;l.download=`qr-${Ra(p)}-${t}.svg`,l.click(),URL.revokeObjectURL(o)}_close(){this._open=!1,this._viewResult=null,this._completeResult=null,this._error="",this._loading=!1}render(){if(!this._open)return r``;let e=this.lang||this.hass?.language||"en",t=this._taskName?`${i("qr_code",e)}: ${this._objectName} \u2014 ${this._taskName}`:`${i("qr_code",e)}: ${this._objectName}`,a=!!this._viewResult;return r`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
  ...
  `,_([w({attribute:!1})],Z.prototype,"hass",2),_([w()],Z.prototype,"lang",2),_([g()],Z.prototype,"_open",2),_([g()],Z.prototype,"_loading",2),_([g()],Z.prototype,"_error",2),_([g()],Z.prototype,"_viewResult",2),_([g()],Z.prototype,"_completeResult",2),_([g()],Z.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",Z);function we(c){let s=c.getFullYear(),e=String(c.getMonth()+1).padStart(2,"0"),t=String(c.getDate()).padStart(2,"0");return`${s}-${e}-${t}`}function Da(c,s){let e=[];for(let t=0;t<s;t++){let a=new Date(c);a.setDate(a.getDate()+t),a.setHours(0,0,0,0),e.push(we(a))}return e}function Re(c,s){let[e,t,a]=c.split("-").map(Number),n=new Date(e,t-1,a);return n.setDate(n.getDate()+s),we(n)}function Ia(c){if(!c||c.length===0)return null;let s=c.map(e=>e.cost).filter(e=>typeof e=="number");return s.length===0?null:s.reduce((e,t)=>e+t,0)/s.length}function Pa(c){let{windowStart:s,windowEnd:e,task:t,entryId:a,objectName:n}=c,o=[],l=(h,m)=>({date:h,entry_id:a,task_id:t.id,task_name:t.name,object_name:n,status:m&&(t.status==="overdue"||t.status==="triggered")?"ok":t.status,days_until_due:m?null:t.days_until_due??null,projected:m,schedule_type:t.schedule_type,interval_days:t.interval_days??null,responsible_user_id:t.responsible_user_id??null,avg_cost:Ia(t.history),adaptive_enabled:!!t.adaptive_config?.enabled,prediction_confidence:t.threshold_prediction_confidence??null});if(t.status==="overdue"||t.status==="triggered"){if(o.push(l(s,!1)),t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let h=Re(s,t.interval_days),m=1;for(;h<=e&&m<5;)o.push(l(h,!0)),m++,h=Re(h,t.interval_days)}return o}let p=t.next_due;if(typeof p!="string"||!p)return o;let u=p.slice(0,10);if(u>=s&&u<=e)o.push(l(u,!1));else if(u>e)return o;if(t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let h=Re(u,t.interval_days),m=o.length;for(;h<=e&&m<5;)h>=s&&(o.push(l(h,!0)),m++),h=Re(h,t.interval_days)}return o}var Et={overdue:0,triggered:1,due_soon:2,ok:3};function jt(c,s,e,t=null){let a=Da(s,e),n=a[0],o=a[a.length-1],l=[];for(let u of c){let h=u.object?.name||"",m=u.entry_id,v=u.tasks||[];for(let f of v){if(t&&f.responsible_user_id!==t||f.enabled===!1)continue;let k=Pa({windowStart:n,windowEnd:o,task:f,entryId:m,objectName:h});l.push(...k)}}let p=new Map;for(let u of a)p.set(u,[]);for(let u of l){let h=p.get(u.date);h&&h.push(u)}for(let[,u]of p)u.sort((h,m)=>{let v=Et[h.status]??99,f=Et[m.status]??99;if(v!==f)return v-f;if(h.projected!==m.projected)return h.projected?1:-1;let k=h.object_name.localeCompare(m.object_name);return k!==0?k:h.task_name.localeCompare(m.task_name)});return a.map(u=>({date:u,events:p.get(u)??[]}))}var Fa={completed:"ok",reset:"ok",skipped:"due_soon",triggered:"triggered",trigger_replaced:"triggered"};function La(c,s){let e=[];for(let t=s-1;t>=0;t--){let a=new Date(c);a.setDate(a.getDate()-t),a.setHours(0,0,0,0),e.push(we(a))}return e}function At(c,s,e,t=null){let a=La(s,e),n=a[0],o=a[a.length-1],l=new Map;for(let u of a)l.set(u,[]);for(let u of c){let h=u.object?.name||"",m=u.entry_id,v=u.tasks||[];for(let f of v){if(t&&f.responsible_user_id!==t)continue;let k=f.history||[];for(let b of k){if(typeof b?.timestamp!="string")continue;let N=b.timestamp.slice(0,10);if(N<n||N>o)continue;let C=l.get(N);if(!C)continue;let E=b.type??"completed";C.push({date:N,entry_id:m,task_id:f.id,task_name:f.name,object_name:h,status:Fa[E]??"ok",days_until_due:null,projected:!1,schedule_type:f.schedule_type,interval_days:f.interval_days??null,responsible_user_id:f.responsible_user_id??null,avg_cost:typeof b.cost=="number"?b.cost:null,adaptive_enabled:!!f.adaptive_config?.enabled,prediction_confidence:null,history_timestamp:b.timestamp,history_type:E,history_cost:typeof b.cost=="number"?b.cost:null,history_notes:typeof b.notes=="string"?b.notes:null,history_duration:typeof b.duration=="number"?b.duration:null})}}}let p={completed:0,reset:1,skipped:2,triggered:3,trigger_replaced:4};for(let[,u]of l)u.sort((h,m)=>{let v=p[h.history_type??""]??99,f=p[m.history_type??""]??99;if(v!==f)return v-f;let k=h.object_name.localeCompare(m.object_name);return k!==0?k:h.task_name.localeCompare(m.task_name)});return a.map(u=>({date:u,events:l.get(u)??[]}))}var St=q`
  .cal-controls {
    display: flex;
    gap: 12px;
    align-items: center;
...
`;var J=class extends T{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter)}getCardSize(){return 6}get _lang(){return this.hass?.language||"en"}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}_onEventClick(e){if(e.history_timestamp){this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}));return}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}render(){if(!this.hass)return d;let e=this._lang,t=this._config.show_window_chips!==!1,a=this._config.show_user_filter!==!1,n=this._config.title,o=null;this._userFilter&&(o=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let l=new Date;l.setHours(0,0,0,0);let p=this._pastDays>0,u=p?At(this._objects,l,this._pastDays,o):jt(this._objects,l,this._windowDays,o),h=we(l),m=this._windowDays===365||p,v=m?u.filter(b=>b.events.length>0):u,f=b=>{let N=`cal-status-${b.status}`,C=b.projected?"cal-event-projected":"",E=b.status==="overdue"&&b.days_until_due!=null?` (${Math.abs(b.days_until_due)}d ${i("overdue",e).toLowerCase()})`:"",$=b.projected&&b.interval_days?r`<span class="cal-event-recur">${i("cal_every_n_days",e).replace("{n}",String(b.interval_days))}</span>`:d,M=b.schedule_type==="sensor_based",L=M?r`<ha-icon class="cal-event-icon cal-source-sensor"
                ...
                icon="${b.adaptive_enabled?"mdi:clock-time-four-outline":"mdi:clock-outline"}"></ha-icon>`,V=M&&b.prediction_confidence&&b.status!=="triggered"&&!b.projected?r`<span class="cal-event-prediction cal-conf-${b.prediction_confidence}">
            ${i("cal_predicted",e)} · ${i(`cal_confidence_${b.prediction_confidence}`,e)}
          </span>`:d,K=this._stats?.budget?.currency_symbol||"\u20AC",R=b.history_type?i(b.history_type,e):i(b.status,e);return r`
        <div class="cal-event ${C}"
          @click=${()=>this._onEventClick(b)}>
          ${L}
          <span class="cal-status-pill ${N}">${R}</span>
          <div class="cal-event-body">
            <div class="cal-event-title">${b.object_name} · ${b.task_name}${E}</div>
            ${V}
            ${$}
          </div>
          ${b.avg_cost!=null&&b.avg_cost>0?r`<span class="cal-event-cost">${b.avg_cost.toFixed(0)} ${K}</span>`:d}
        </div>
      `},k=b=>{let[N,C,E]=b.date.split("-").map(Number),$=new Date(N,C-1,E),M=b.date===h,L=$.toLocaleDateString(e,{weekday:"short"}),V=$.toLocaleDateString(e,{month:"long"});return r`
    ...
    `],_([w({attribute:!1})],J.prototype,"hass",2),_([g()],J.prototype,"_config",2),_([g()],J.prototype,"_objects",2),_([g()],J.prototype,"_stats",2),_([g()],J.prototype,"_windowDays",2),_([g()],J.prototype,"_pastDays",2),_([g()],J.prototype,"_userFilter",2),_([g()],J.prototype,"_unsub",2);var Ha=[{value:7,label:"Week (7 days)"},{value:14,label:"Fortnight (14 days)"},{value:30,label:"Month (30 days, default)"},{value:365,label:"Year (365 days, empty days collapsed)"}],ge=class extends T{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"}}setConfig(e){this._config={...e}}_valueChanged(e,t){let a={...this._config,[e]:t};e==="show_window_chips"&&t===!0&&delete a.show_window_chips,e==="show_user_filter"&&t===!0&&delete a.show_user_filter,e==="title"&&(!t||typeof t=="string"&&t.trim()==="")&&delete a.title,e==="user_filter"&&t===""&&delete a.user_filter,this._config=a,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:a},bubbles:!0,composed:!0}))}render(){let e=this._config.window_days??30,t=this._config.show_window_chips!==!1,a=this._config.show_user_filter!==!1,n=this._config.user_filter??"",o=this._config.title??"";return r`
      <div class="editor">
        <div class="row">
        ...
        <div class="row">
          ...
          >
            ${Ha.map(l=>r`<option value="${l.value}" ?selected=${l.value===e}>${l.label}</option>`)}
          </select>
        </div>
  ...
  `,_([w({attribute:!1})],G.prototype,"hass",2),_([g()],G.prototype,"_open",2),_([g()],G.prototype,"_title",2),_([g()],G.prototype,"_message",2),_([g()],G.prototype,"_confirmText",2),_([g()],G.prototype,"_danger",2),_([g()],G.prototype,"_inputLabel",2),_([g()],G.prototype,"_inputType",2),_([g()],G.prototype,"_inputValue",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",G);var qt={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},Ct=c=>(...s)=>({_$litDirective$:c,values:s}),Ie=class{constructor(s){}get _$AU(){return this._$AM._$AU}_$AT(s,e,t){this._$Ct=s,this._$AM=e,this._$Ci=t}_$AS(s,e){return this.update(s,e)}update(s,e){return this.render(...e)}};var $e=class extends Ie{constructor(s){if(super(s),this.it=d,s.type!==qt.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(s){if(s===d||s==null)return this._t=void 0,this.it=s;if(s===ee)return s;if(typeof s!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(s===this.it)return this._t;this.it=s;let e=[s];return e.raw=e,this._t={_$litType$:this.constructor.resultType,strings:e,values:[]}}};$e.directiveName="unsafeHTML",$e.resultType=1;var Mt=Ct($e);var Ua=["EUR","USD","GBP","JPY","CHF","CAD","AUD","CNY","INR","BRL","CZK","PLN","RUB","SEK","NOK","DKK","UAH"],A=class extends T{constructor(){super(...arguments);this.budget=null;this._settings=null;this._loading=!0;this._importCsv="";this._importLoading=!1;this._includeHistory=!0;this._toast="";this._testingNotification=!1;this._users=[];this._vacEnabled=!1;this._vacStart="";this._vacEnd="";this._vacBuffer=3;this._vacExempt=new Set;this._vacIsActive=!1;this._vacWindowEnd=null;this._vacAllTasks=[];this._vacPreview=[];this._vacPreviewLoading=!1;this._vacSaving=!1;this._qrObjects=[];this._qrSelectedEntries=new Set;this._qrActions=new Set(["view"]);this._qrUrlMode="companion";this._qrBatchLoading=!1;this._qrBatchResults=[];this._qrObjectsLoaded=!1;this._loaded=!1;this._userService=null;this._sendTestNotification=async()=>{this._testingNotification=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/test_notification"}),t=e.message||(e.success?i("test_notification_success",this._lang):i("test_notification_failed",this._lang));this._showToast(t)}catch{this._showToast(i("test_notification_failed",this._lang))}finally{this._testingNotification=!1}}}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._loaded?(this._loaded=!0,this._userService=new oe(this.hass),this._loadSettings(),this._loadUsers()):e.has("hass")&&this.hass&&this._userService&&this._userService.updateHass(this.hass)}async _loadUsers(){if(this._userService)try{this._users=await this._userService.getUsers()}catch{this._users=[]}}async _loadSettings(){this._loading=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});this._settings=e,this._hydrateVacationFromSettings()}catch{}this._loading=!1}_hydrateVacationFromSettings(){let e=this._settings?.vacation;e&&(this._vacEnabled=e.enabled,this._vacStart=e.start||"",this._vacEnd=e.end||"",this._vacBuffer=e.buffer_days,this._vacExempt=new Set(e.exempt_task_ids||[]),this._vacIsActive=e.is_active,this._vacWindowEnd=e.window_end)}async _updateSetting(e,t){try{let a=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/update",settings:{[e]:t}});this._settings=a,this._showToast(i("settings_saved",this._lang)),this.dispatchEvent(new CustomEvent("settings-changed"))}catch{this._showToast(i("action_error",this._lang))}}_showToast(e){this._toast=e,setTimeout(()=>{this._toast=""},3e3)}_downloadFile(e,t,a){let n=new Blob([e],{type:a}),o=URL.createObjectURL(n),l=document.createElement("a");l.href=o,l.download=t,l.click(),URL.revokeObjectURL(o)}render(){let e=this._lang;return this._loading||!this._settings?r`<div class="settings-loading">Loading…</div>`:r`
    ...
    `}scrollToSection(e){requestAnimationFrame(()=>{let t=this.shadowRoot;if(!t)return;let a=t.querySelector(`[data-section="${e}"]`)??t.querySelector(`[data-section-alt="${e}"]`);a&&a.scrollIntoView({behavior:"smooth",block:"start"})})}_renderPanelAccess(e){let t=new Set(this._settings.admin_panel_user_ids||[]),a=this._users.filter(o=>!o.is_admin),n=(o,l)=>{let p=new Set(t);l?p.add(o):p.delete(o),this._updateSetting("admin_panel_user_ids",[...p])};return r`
      <div class="settings-section">
        ...
        ${a.length===0?r`<div class="setting-row hint">${i("no_non_admin_users",e)}</div>`:a.map(o=>r`
            <label class="setting-row">
              ...
              <input type="checkbox"
                .checked=${t.has(o.id)}
                @change=${l=>n(o.id,l.target.checked)} />
            </label>
          `)}
      </div>
      ...
      <div class="settings-section">
        ...
        <label class="setting-row">
          ...
          <select @change=${n=>this._updateSetting("budget_currency",n.target.value)}>
            ${Ua.map(n=>r`<option value=${n} ?selected=${a.currency===n}>${n}</option>`)}
          </select>
        </label>
        <label class="setting-row">
    ...
    `}_renderVacationTaskList(e){let t=new Map;for(let n of this._vacAllTasks){let o=t.get(n.object_name)||[];o.push(n),t.set(n.object_name,o)}return[...t.entries()].sort(([n],[o])=>n.localeCompare(o)).map(([n,o])=>r`
      <div class="vac-task-group">
        ...
        ${o.sort((l,p)=>l.task_name.localeCompare(p.task_name)).map(l=>r`
            <label class="vac-task-row">
              <input type="checkbox"
                .checked=${this._vacExempt.has(l.task_id)}
                @change=${p=>this._toggleVacationExempt(l.task_id,p.target.checked)} />
    ...
    `)}_renderVacationPreview(e){return r`
      <div class="vac-preview-list">
        ${this._vacPreview.map(t=>{let a=t.events.map(o=>{let l=`vacation_event_${o.status}`;return`${o.date} (${i(l,e)})`}).join(" \xB7 "),n=!t.will_suppress;return r`
        ...
        ${this._qrObjectsLoaded?r`
            <details open class="qr-filter-panel">
              ...
              <div class="qr-filter-group">
                ...
                <div class="qr-object-list">
                  ${this._qrObjects.length===0?r`<div class="hint">${i("no_objects",e)}</div>`:this._qrObjects.map(l=>r`
                      <label class="qr-object-row">
                        <input type="checkbox"
                          .checked=${this._qrSelectedEntries.size===0||this._qrSelectedEntries.has(l.entry_id)}
                          @change=${p=>this._toggleQrObject(l.entry_id,p.target.checked)} />
                        <span>${l.name}</span>
                        <span class="qr-task-count">${l.task_count}</span>
                      </label>
                    `)}
                </div>
              </div>
              ...
              <div class="qr-filter-group">
                ...
                <div class="qr-action-chips">
                  ${["view","complete","skip"].map(l=>r`
                    <label class="qr-action-chip ${this._qrActions.has(l)?"active":""}">
                      <input type="checkbox"
                        .checked=${this._qrActions.has(l)}
                        @change=${p=>this._toggleQrAction(l,p.target.checked)} />
                      ${i("qr_action_"+l,e)}
                    </label>
                  `)}
                </div>
              ...
              <div class="qr-filter-group">
                ...
                <select .value=${this._qrUrlMode}
                  @change=${l=>{this._qrUrlMode=l.target.value}}>
            ...
            ${this._qrBatchResults.length>0?r`
                ...
                <div class="qr-print-grid">
                  ${this._qrBatchResults.map(l=>r`
                    <div class="qr-print-cell">
                      <div class="qr-svg">${Mt(l.svg)}</div>
                      <div class="qr-label">
                        <div class="qr-label-obj">${l.object_name}</div>
                        <div class="qr-label-task">${l.task_name}</div>
                        <div class="qr-label-action">${i("qr_action_"+l.action,e)}</div>
                      </div>
                    </div>
                  `)}
                </div>
...
<body>
              ...
              <div class="objects">
                ${[...this.objects].sort((a,n)=>a.object.name.localeCompare(n.object.name)).map(a=>r`
                  <div class="object-block">
                    ...
                    ${a.tasks.length===0?r`<div class="hint small">${i("no_tasks_short",e)}</div>`:[...a.tasks].sort((n,o)=>n.name.localeCompare(o.name)).map(n=>{let o=`${a.entry_id}:${n.id}`,l=this._selected.has(o);return r`
  ...
  `,_([w({attribute:!1})],Q.prototype,"hass",2),_([w({attribute:!1})],Q.prototype,"objects",2),_([g()],Q.prototype,"_open",2),_([g()],Q.prototype,"_loading",2),_([g()],Q.prototype,"_error",2),_([g()],Q.prototype,"_groupId",2),_([g()],Q.prototype,"_name",2),_([g()],Q.prototype,"_description",2),_([g()],Q.prototype,"_selected",2);customElements.get("maintenance-group-dialog")||customElements.define("maintenance-group-dialog",Q);var Va=300,Wa=140,et=27;function Rt(c,s){let e=c.trigger_config;if(!e)return d;let t=s.lang,a=c.trigger_entity_info,n=c.trigger_entity_infos,o=a?.friendly_name||e.entity_id||"\u2014",l=e.entity_id||"",p=e.entity_ids||(l?[l]:[]),u=a?.unit_of_measurement||"",h=c.trigger_current_value,m=e.type||"threshold",v=p.length>1;return r`
    <h3>${i("trigger",t)}</h3>
      ...
      <div class="trigger-header">
        ...
        <div>
          ...
          `:r`
            ...
            <div class="trigger-entity-id">${l?r`<span class="entity-link" @click=${f=>de(f,l)}>${l}</span>`:""}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `}
        </div>
        <span class="status-badge ${c.trigger_active?"triggered":"ok"}" style="margin-left: auto;">
          ${c.trigger_active?i("triggered",t):i("ok",t)}
        </span>
      </div>

      ${h!=null?r`
            <div class="trigger-value-row">
              <span class="trigger-current ${c.trigger_active?"active":""}">${typeof h=="number"?h.toFixed(1):h}</span>
  ...
  `}function Ga(c,s,e){let t=c.trigger_config;if(!t)return d;let a=t.type||"threshold",n=a==="counter"&&t.trigger_delta_mode,o=e.isCounterEntity(t),l=t.entity_id||"",p=e.detailStatsData.get(l)||[],u=[],h=!1;if(p.length>=2)for(let y of p){let S=y.val;n&&c.trigger_baseline_value!=null&&(S-=c.trigger_baseline_value);let H={ts:y.ts,val:S};!o&&y.min!=null&&y.max!=null&&(H.min=n&&c.trigger_baseline_value!=null?y.min-c.trigger_baseline_value:y.min,H.max=n&&c.trigger_baseline_value!=null?y.max-c.trigger_baseline_value:y.max,h=!0),u.push(H)}else for(let y of c.history)y.trigger_value!=null&&u.push({ts:new Date(y.timestamp).getTime(),val:y.trigger_value});if(c.trigger_current_value!=null){let y=c.trigger_current_value;n&&c.trigger_baseline_value!=null&&(y-=c.trigger_baseline_value),u.push({ts:Date.now(),val:y})}if(u.length<2&&l&&e.hasStatsService&&!e.detailStatsData.has(l))return r`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
      <ha-icon icon="mdi:chart-line" style="--mdc-icon-size:16px;margin-right:8px;"></ha-icon>
      ${i("loading_chart",e.lang)}
    </div>`;if(u.length<2)return d;u.sort((y,S)=>y.ts-S.ts);let m=Va,v=Wa,f=30,k=2,b=8,N=16,C=u.map(y=>y.val),E=Math.min(...C),$=Math.max(...C);if(h)for(let y of u)y.min!=null&&(E=Math.min(E,y.min)),y.max!=null&&($=Math.max($,y.max));t.trigger_above!=null&&($=Math.max($,t.trigger_above),E=Math.min(E,t.trigger_above)),t.trigger_below!=null&&(E=Math.min(E,t.trigger_below),$=Math.max($,t.trigger_below));let M=null,L=null;if(a==="counter"&&t.trigger_target_value!=null){if(c.trigger_baseline_value!=null)M=c.trigger_baseline_value;else if(u.length>0){let y=[...c.history].filter(S=>S.type==="completed"||S.type==="reset").sort((S,H)=>new Date(H.timestamp).getTime()-new Date(S.timestamp).getTime())[0];if(y){let S=new Date(y.timestamp).getTime(),H=u[0],O=Math.abs(u[0].ts-S);for(let ae of u){let ie=Math.abs(ae.ts-S);ie<O&&(H=ae,O=ie)}M=H.val}else M=u[0].val}M!=null?(L=M+t.trigger_target_value,$=Math.max($,L),E=Math.min(E,M)):($=Math.max($,t.trigger_target_value),E=Math.min(E,0))}n&&c.trigger_baseline_value!=null&&(E=Math.min(E,0));let V=$-E||1;E-=V*.1,$+=V*.1;let K=u[0].ts,R=u[u.length-1].ts,z=R-K||1,I=y=>f+(y-K)/z*(m-f-k),D=y=>b+(1-(y-E)/($-E))*(v-b-N),Pe=u.map(y=>`${I(y.ts).toFixed(1)},${D(y.val).toFixed(1)}`).join(" "),Bt=`M${I(u[0].ts).toFixed(1)},${v-N} `+u.map(y=>`L${I(y.ts).toFixed(1)},${D(y.val).toFixed(1)}`).join(" ")+` L${I(u[u.length-1].ts).toFixed(1)},${v-N} Z`,Fe="";if(h){let y=u.filter(S=>S.min!=null&&S.max!=null);if(y.length>=2){let S=y.map(O=>`${I(O.ts).toFixed(1)},${D(O.max).toFixed(1)}`),H=[...y].reverse().map(O=>`${I(O.ts).toFixed(1)},${D(O.min).toFixed(1)}`);Fe=`M${S[0]} `+S.slice(1).map(O=>`L${O}`).join(" ")+` L${H[0]} `+H.slice(1).map(O=>`L${O}`).join(" ")+" Z"}}let tt=u[u.length-1],Vt=I(tt.ts),Wt=D(tt.val),at=y=>Math.abs(y)>=1e4?(y/1e3).toFixed(0)+"k":y>=1e3?(y/1e3).toFixed(1)+"k":y.toFixed(y<10?1:0),Gt=at($),Qt=at(E),Kt=c.history.filter(y=>["completed","skipped","reset"].includes(y.type)).map(y=>({ts:new Date(y.timestamp).getTime(),type:y.type})).filter(y=>y.ts>=K&&y.ts<=R),Le=u;if(u.length>et){let y=(u.length-1)/(et-1);Le=[];for(let S=0;S<et;S++)Le.push(u[Math.round(S*y)])}return r`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${m} ${v}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_sparkline",e.lang)}">
        <text x="${f-3}" y="${b+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${Gt}</text>
        <text x="${f-3}" y="${v-N+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${Qt}</text>
        ...
        <text x="${m-k}" y="${v-1}" text-anchor="end" fill="var(--secondary-text-color)" font-size="7">${new Date(R).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        ${Fe?P`<path d="${Fe}" fill="var(--primary-color)" opacity="0.08" />`:d}
        <path d="${Bt}" fill="var(--primary-color)" opacity="0.15" />
        <polyline points="${Pe}" fill="none" stroke="var(--primary-color)" stroke-width="2" stroke-linejoin="round" />
        ${c.degradation_rate!=null&&c.degradation_trend!=="stable"&&c.degradation_trend!=="insufficient_data"&&u.length>=2?(()=>{let y=u[u.length-1],S=30,H=y.ts+S*864e5,O=y.val+c.degradation_rate*S,ae=Math.min(H,R+(R-K)*.3),ie=Math.max(E,Math.min($,O)),Zt=I(y.ts),Jt=D(y.val),Yt=I(ae),Xt=D(ie);return P`<line x1="${Zt.toFixed(1)}" y1="${Jt.toFixed(1)}" x2="${Yt.toFixed(1)}" y2="${Xt.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():d}
        ${a==="threshold"&&t.trigger_above!=null?P`<line x1="${f}" y1="${D(t.trigger_above).toFixed(1)}" x2="${m}" y2="${D(t.trigger_above).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${m-2}" y="${D(t.trigger_above)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25B2 ${t.trigger_above}</text>`:d}
        ${a==="threshold"&&t.trigger_below!=null?P`<line x1="${f}" y1="${D(t.trigger_below).toFixed(1)}" x2="${m}" y2="${D(t.trigger_below).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${m-2}" y="${D(t.trigger_below)+11}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25BC ${t.trigger_below}</text>`:d}
        ${a==="counter"&&L!=null?P`<line x1="${f}" y1="${D(L).toFixed(1)}" x2="${m}" y2="${D(L).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${m-2}" y="${D(L)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">${i("target_value",e.lang)}: +${t.trigger_target_value}</text>`:d}
        ${a==="counter"&&M!=null?P`<line x1="${f}" y1="${D(M).toFixed(1)}" x2="${m}" y2="${D(M).toFixed(1)}" stroke="var(--secondary-text-color)" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
                <text x="${f+2}" y="${D(M)+11}" text-anchor="start" fill="var(--secondary-text-color)" font-size="8">${i("baseline",e.lang)}</text>`:d}
        <circle cx="${Vt.toFixed(1)}" cy="${Wt.toFixed(1)}" r="3.5" fill="var(--primary-color)" />
        ${Kt.map(y=>{let S=I(y.ts),H=y.type==="completed"?"var(--success-color, #4caf50)":y.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return P`
          ...
          `})}
        ${Le.map(y=>{let S=I(y.ts),H=D(y.val),O=new Date(y.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),ae=`${y.val.toFixed(1)} ${s}`;return h&&y.min!=null&&y.max!=null&&(ae+=` (${y.min.toFixed(1)}\u2013${y.max.toFixed(1)})`),P`<circle cx="${S.toFixed(1)}" cy="${H.toFixed(1)}" r="8" fill="transparent" tabindex="0"
            @mouseenter=${ie=>Nt(ie,`${O}
${ae}`,e.setTooltip)}
            @focus=${ie=>Nt(ie,`${O}
${ae}`,e.setTooltip)}
...
`).map(y=>r`<div>${y}</div>`)}
  ...
  `}function Nt(c,s,e){let t=c.currentTarget,a=t.closest(".sparkline-container");if(!a)return;let n=a.getBoundingClientRect(),o=t.getBoundingClientRect();e({x:o.left-n.left+o.width/2,y:o.top-n.top-8,text:s})}function Dt(c,s,e){let t=c.degradation_trend!=null&&c.degradation_trend!=="insufficient_data",a=c.days_until_threshold!=null,n=c.environmental_factor!=null&&c.environmental_factor!==1;if(!t&&!a&&!n)return d;let o=c.degradation_trend==="rising"?"M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z":c.degradation_trend==="falling"?"M16,18L18.29,15.71L13.41,10.83L9.41,14.83L2,7.41L3.41,6L9.41,12L13.41,8L19.71,14.29L22,12V18H16Z":"M22,12L18,8V11H3V13H18V16L22,12Z";return r`
    <div class="prediction-section">
      ${c.sensor_prediction_urgency?r`
        <div class="prediction-urgency-banner">
          <ha-svg-icon path="M1,21H23L12,2L1,21M12,18A1,1 0 0,1 11,17A1,1 0 0,1 12,16A1,1 0 0,1 13,17A1,1 0 0,1 12,18M13,15H11V10H13V15Z"></ha-svg-icon>
          ${i("sensor_prediction_urgency",s).replace("{days}",String(Math.round(c.days_until_threshold||0)))}
        </div>
      `:d}
      ...
      <div class="prediction-grid">
        ${t?r`
          <div class="prediction-item">
            ...
            <span class="prediction-value ${c.degradation_trend}">${i("trend_"+c.degradation_trend,s)}</span>
            ${c.degradation_rate!=null?r`<span class="prediction-rate">${c.degradation_rate>0?"+":""}${Math.abs(c.degradation_rate)>=10?Math.round(c.degradation_rate).toLocaleString():c.degradation_rate.toFixed(1)} ${c.trigger_entity_info?.unit_of_measurement||""}/${i("day_short",s)}</span>`:d}
          </div>
        `:d}
        ${a?r`
          <div class="prediction-item">
            ...
            <span class="prediction-value prediction-days${c.days_until_threshold===0?" exceeded":c.sensor_prediction_urgency?" urgent":""}">${c.days_until_threshold===0?i("threshold_exceeded",s):"~"+Math.round(c.days_until_threshold)+" "+i("days",s)}</span>
            ${c.threshold_prediction_date?r`<span class="prediction-date">${se(c.threshold_prediction_date,s)}</span>`:d}
            ${c.threshold_prediction_confidence?r`<span class="confidence-dot ${c.threshold_prediction_confidence}"></span>`:d}
          </div>
        `:d}
        ${n&&e.environmental?r`
          <div class="prediction-item">
            ...
            <span class="prediction-value">${c.environmental_factor.toFixed(2)}x</span>
            ${c.environmental_entity?r`<span class="prediction-entity entity-link" @click=${l=>de(l,c.environmental_entity)}>${c.environmental_entity}</span>`:d}
          </div>
        `:d}
      </div>
    </div>
  `}function It(c,s){let e=c.interval_analysis,t=e?.weibull_beta,a=e?.weibull_eta;if(t==null||a==null||a<=0)return d;let n=c.interval_days??0,o=c.suggested_interval??n;return r`
    <div class="weibull-section">
      ...
      ${Ka(t,a,n,o,s)}
      ${Za(e,s)}
      ${e?.confidence_interval_low!=null?Ja(e,c,s):d}
    </div>
  `}function Qa(c,s){let e,t,a;return c<.8?(e="early_failures",t="M13,14H11V10H13M13,18H11V16H13M1,21H23L12,2L1,21Z",a="beta_early_failures"):c<=1.2?(e="random_failures",t="M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2M13,17H11V15H13V17M13,13H11V7H13V13Z",a="beta_random_failures"):c<=3.5?(e="wear_out",t="M12,2A10,10 0 0,1 22,12A10,10 0 0,1 12,22A10,10 0 0,1 2,12A10,10 0 0,1 12,2M12,4A8,8 0 0,0 4,12A8,8 0 0,0 12,20A8,8 0 0,0 20,12A8,8 0 0,0 12,4M12,6A6,6 0 0,1 18,12H12V6Z",a="beta_wear_out"):(e="highly_predictable",t="M12,2A10,10 0 0,1 22,12A10,10 0 0,1 12,22A10,10 0 0,1 2,12A10,10 0 0,1 12,2M11,16.5L18,9.5L16.59,8.09L11,13.67L7.91,10.59L6.5,12L11,16.5Z",a="beta_highly_predictable"),r`
    <span class="beta-badge ${e}">
      <ha-svg-icon path="${t}"></ha-svg-icon>
      ${i(a,s)} (\u03B2=${c.toFixed(2)})
    </span>
  `}function Ka(c,s,e,t,a){let f=Math.max(e,t,s,1)*1.3,k=50,b=[];for(let R=0;R<=k;R++){let z=R/k*f,I=1-Math.exp(-Math.pow(z/s,c)),D=32+z/f*260,Pe=136-I*128;b.push([D,Pe])}let N=b.map(([R,z])=>`${R.toFixed(1)},${z.toFixed(1)}`).join(" "),C="M32,136 "+b.map(([R,z])=>`L${R.toFixed(1)},${z.toFixed(1)}`).join(" ")+` L${b[k][0].toFixed(1)},136 Z`,E=32+e/f*260,$=1-Math.exp(-Math.pow(e/s,c)),M=136-$*128,L=((1-$)*100).toFixed(0),V=32+t/f*260,K=[0,.25,.5,.75,1];return r`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_weibull",a)}">
        ${K.map(R=>{let z=136-R*128;return P`
            <line x1="${32}" y1="${z.toFixed(1)}" x2="${292}" y2="${z.toFixed(1)}"
              stroke="var(--divider-color)" stroke-width="0.5" stroke-dasharray="${R===.5?"4,3":d}" />
  ...
  `}function Za(c,s){return r`
    <div class="weibull-info-row">
      <div class="weibull-info-item">
        <span>${i("characteristic_life",s)}</span>
        <span class="weibull-info-value">${Math.round(c.weibull_eta)} ${i("days",s)}</span>
      </div>
      ${c.weibull_r_squared!=null?r`
        <div class="weibull-info-item">
          <span>${i("weibull_r_squared",s)}</span>
          <span class="weibull-info-value">${c.weibull_r_squared.toFixed(3)}</span>
        </div>
      `:d}
    </div>
  `}function Ja(c,s,e){let t=c.confidence_interval_low,a=c.confidence_interval_high,n=s.suggested_interval??s.interval_days??0,o=s.interval_days??0,l=Math.max(0,t-5),u=a+5-l,h=(t-l)/u*100,m=(a-t)/u*100,v=(n-l)/u*100,f=o>0?(o-l)/u*100:-1;return r`
  ...
  `}function Pt(c,s,e,t){let a=Math.max(c||1,s);return r`
    <div class="interval-comparison">
      <div class="interval-bar">
        <div class="interval-label">
          ${i("current",t)}: ${c??"\u2014"} ${c!=null?i("days",t):""}
        </div>
        <div class="interval-visual current"
          style="width: ${c!=null?Math.min(c/a*100,100):0}%"></div>
      </div>
      <div class="interval-bar">
        <div class="interval-label">
  ...
  `}var Ft=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"];function Lt(c,s,e){if(!e.seasonal||!c.seasonal_factor||c.seasonal_factor===1)return d;let t=Ft.map(l=>i(l,s)),a=new Date().getMonth(),n=c.seasonal_factors||c.interval_analysis?.seasonal_factors||null,o=n&&n.length===12?n:t.map((l,p)=>{let u=c.seasonal_factor||1,h=Math.sin((p-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,u+h))});return r`
    <div class="seasonal-card-compact">
      <h4>${i("seasonal_awareness",s)}</h4>
      <div class="seasonal-mini-chart">
        ${o.map((l,p)=>{let u=l*40,h=l<.9?"low":l>1.1?"high":"normal";return r`
            <div class="seasonal-bar ${h} ${p===a?"current":""}"
                 style="height: ${u}px"
                 title="${t[p]}: ${l.toFixed(2)}x">
            </div>
          `})}
      </div>
  ...
  `}function Ht(c,s){return Ya(c,s)}function Ya(c,s){let e=c.seasonal_factors??c.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return d;let t=c.interval_analysis?.seasonal_reason,a=new Date().getMonth(),n=300,o=100,l=8,u=o-l-4,h=Math.max(...e,1.5),m=n/12,v=m*.65,f=l+u-1/h*u;return r`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${n} ${o}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_seasonal",s)}">
        ...
        ${e.map((k,b)=>{let N=k/h*u,C=b*m+(m-v)/2,E=l+u-N,$=b===a,M=k<1?"var(--success-color, #4caf50)":k>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return P`
      ...
      <div class="seasonal-labels">
        ${Ft.map((k,b)=>r`<span class="seasonal-label ${b===a?"active-month":""}">${i(k,s)}</span>`)}
      </div>
    </div>
  `}var Xa=300,ei=200;function Ot(c,s,e,t){let a=c.history.filter(l=>l.type==="completed"&&(l.cost!=null||l.duration!=null));if(a.length<2)return d;let n=a.some(l=>(l.cost??0)>0),o=a.some(l=>(l.duration??0)>0);return!n&&!o?d:r`
  ...
  `}function ti(c,s,e){let t=c.history.filter(z=>z.type==="completed"&&(z.cost!=null||z.duration!=null)).map(z=>({ts:new Date(z.timestamp).getTime(),cost:z.cost??0,duration:z.duration??0})).sort((z,I)=>z.ts-I.ts);if(t.length<2)return d;let a=t.some(z=>z.cost>0),n=t.some(z=>z.duration>0);if(!a&&!n)return d;let o=e!=="duration"&&a,l=e!=="cost"&&n,p=o||!l&&a,u=l||!o&&n,h=Xa,m=ei,v=p?32:8,f=u?32:8,k=8,b=20,N=h-v-f,C=m-k-b,E=Math.max(...t.map(z=>z.cost))||1,$=Math.max(...t.map(z=>z.duration))||1,M=Math.min(20,N/t.length*.6),L=N/t.length,V=z=>v+L*z+L/2,K=z=>k+C-z/E*C,R=z=>k+C-z/$*C;return r`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${h} ${m}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_history",s)}">
        ...
        `):d}
        ${u?P`
          <polyline points="${t.map((z,I)=>`${V(I).toFixed(1)},${R(z.duration).toFixed(1)}`).join(" ")}"
            fill="none" stroke="var(--accent-color, #ff9800)" stroke-width="2" stroke-linejoin="round" />
          ${t.map((z,I)=>P`
            <circle cx="${V(I).toFixed(1)}" cy="${R(z.duration).toFixed(1)}" r="3" fill="var(--accent-color, #ff9800)" />
          `)}
        `:d}
  ...
  `}var ai=60,ii=20,Ut=30,j=class extends T{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._sparklineTooltip=null;this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._adminPanelUserIds=[];this._defaultWarningDays=7;this._actionLoading=!1;this._moreMenuOpen=!1;this._toastMessage="";this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab="dashboard";this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._objectSortMode="alphabetical";this._groupByMode="none";this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._deepLinkHandled=!1;this._onDialogEvent=async()=>{try{await this._loadData()}catch{}};this._onCalendarLlCustom=e=>{let t=e.detail;t?.type==="maintenance-supporter:open-task"&&t.entry_id&&t.task_id&&(e.stopPropagation(),this._showTask(t.entry_id,t.task_id))};this._onHistoryEntrySaved=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"en"}get _isOperator(){let e=this.hass?.user;return e?e.is_admin?!1:!this._adminPanelUserIds.includes(e.id):!0}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler);let e=localStorage.getItem("maintenance_supporter_sort");e&&["due_date","object","type","task_name","area","assigned_user","group"].includes(e)&&(this._sortMode=e);let t=localStorage.getItem("maintenance_supporter_object_sort");t&&["alphabetical","due_soonest","task_count"].includes(t)&&(this._objectSortMode=t);let a=localStorage.getItem("maintenance_supporter_groupby");a&&["none","area","group","user"].includes(a)&&(this._groupByMode=a)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new Ne(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new oe(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,a,n,o]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);if(e&&(this._objects=e.objects),t&&(this._stats=t),a&&(this._budget=a),n&&(this._groups=n.groups||{}),o){let l=o;this._features=l.features,this._adminPanelUserIds=l.admin_panel_user_ids||[];let p=l.general?.default_warning_days;typeof p=="number"&&p>=0&&p<=365&&(this._defaultWarningDays=p)}this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("ms_action"),a=()=>{let h=window.location.pathname+window.location.hash;history.replaceState(history.state,"",h)};if(t==="add_object"){this._deepLinkHandled=!0,a(),requestAnimationFrame(()=>{this.shadowRoot?.querySelector("maintenance-object-dialog")?.openCreate()});return}if(t==="open_vacation"||t==="open_budget"||t==="open_groups"||t==="open_settings"){this._deepLinkHandled=!0,a(),this._overviewTab="settings",requestAnimationFrame(()=>{let h=this.shadowRoot?.querySelector("maintenance-settings-view"),m=t.replace("open_","");h?.scrollToSection?.(m)});return}let n=e.get("entry_id");if(!n)return;this._deepLinkHandled=!0;let o=e.get("task_id"),l=e.get("action"),p=window.location.pathname+window.location.hash;history.replaceState(history.state,"",p);let u=this._getObject(n);if(!u){this._showOverview();return}if(o){let h=u.tasks.find(m=>m.id===o);if(!h){this._showObject(n);return}this._showTask(n,o),l==="complete"?requestAnimationFrame(()=>{this._openCompleteDialog(n,o,h.name,this._features.checklists?h.checklist:void 0,this._features.adaptive&&!!h.adaptive_config?.enabled)}):l==="quick_complete"&&requestAnimationFrame(()=>{this._handleQuickComplete(n,o,h)})}else this._showObject(n)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let a=await this._statsService.getDetailStats(e,t),n=new Map(this._detailStatsData);n.set(e,a),this._detailStatsData=n}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let a of this._objects)for(let n of a.tasks){let o=n.trigger_config?.entity_id;o&&e.push({entityId:o,isCounter:this._isCounterEntity(n.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let m of this._objects)for(let v of m.tasks){if(this._filterStatus&&v.status!==this._filterStatus)continue;if(this._filterUser){let k=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(v.responsible_user_id!==k)continue}let f=[];for(let k of Object.values(this._groups))k.task_refs?.some(b=>b.entry_id===m.entry_id&&b.task_id===v.id)&&f.push(k.name);e.push({entry_id:m.entry_id,task_id:v.id,object_name:m.object.name,task_name:v.name,type:v.type,schedule_type:v.schedule_type,status:v.status,days_until_due:v.days_until_due??null,next_due:v.next_due??null,trigger_active:v.trigger_active,trigger_current_value:v.trigger_current_value??null,trigger_current_delta:v.trigger_current_delta??null,trigger_config:v.trigger_config??null,trigger_entity_info:v.trigger_entity_info??null,times_performed:v.times_performed,total_cost:v.total_cost,interval_days:v.interval_days??null,interval_anchor:v.interval_anchor??null,history:v.history||[],enabled:v.enabled,nfc_tag_id:v.nfc_tag_id??null,area_id:m.object.area_id??null,responsible_user_id:v.responsible_user_id??null,group_names:f})}let t={overdue:0,triggered:1,due_soon:2,ok:3},a=(m,v)=>(t[m.status]??9)-(t[v.status]??9),n=(m,v)=>(m.days_until_due??99999)-(v.days_until_due??99999),o=(m,v)=>a(m,v)||n(m,v),l=m=>m.area_id&&this.hass?.areas?.[m.area_id]?.name||"",p=m=>m.responsible_user_id&&this._userService?.getUserName(m.responsible_user_id)||"",u=m=>m.group_names[0]||"",h={due_date:o,object:(m,v)=>m.object_name.localeCompare(v.object_name)||o(m,v),type:(m,v)=>m.type.localeCompare(v.type)||o(m,v),task_name:(m,v)=>m.task_name.localeCompare(v.task_name),area:(m,v)=>{let f=l(m),k=l(v);return!f&&k?1:f&&!k?-1:f.localeCompare(k)||o(m,v)},assigned_user:(m,v)=>{let f=p(m),k=p(v);return!f&&k?1:f&&!k?-1:f.localeCompare(k)||o(m,v)},group:(m,v)=>{let f=u(m),k=u(v);return!f&&k?1:f&&!k?-1:f.localeCompare(k)||o(m,v)}};return e.sort(h[this._sortMode]),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(n=>n.id===t)}_pushPanelState(e,t,a){let n={msp_view:e,msp_entry:t||null,msp_task:a||null};history.pushState(n,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let a=this._getTask(t.msp_entry,t.msp_task);a?.trigger_config?.entity_id&&this._fetchDetailStats(a.trigger_config.entity_id,this._isCounterEntity(a.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null}_filterByStatus(e){this._filterStatus=e,this._overviewTab!=="dashboard"&&(this._overviewTab="dashboard"),requestAnimationFrame(()=>{this.shadowRoot?.querySelector(".filter-bar")?.scrollIntoView({behavior:"smooth",block:"start"})})}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null;let a=this._getTask(e,t);if(a?.trigger_config?.entity_id){let n=a.trigger_config.entity_id,o=this._isCounterEntity(a.trigger_config);this._fetchDetailStats(n,o)}}_showToast(e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}async _deleteObject(e){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:i("delete",this._lang),message:i("confirm_delete_object",this._lang),confirmText:i("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}}async _deleteTask(e,t){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:i("delete",this._lang),message:i("confirm_delete_task",this._lang),confirmText:i("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}}async _skipTask(e,t,a){this._actionLoading=!0;try{let n={type:"maintenance_supporter/task/skip",entry_id:e,task_id:t};a&&(n.reason=a),await this.hass.connection.sendMessagePromise(n),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}finally{this._actionLoading=!1}}async _resetTask(e,t,a){this._actionLoading=!0;try{let n={type:"maintenance_supporter/task/reset",entry_id:e,task_id:t};a&&(n.date=a),await this.hass.connection.sendMessagePromise(n),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}finally{this._actionLoading=!1}}async _applySuggestion(e,t,a){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:a}),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}}_openSeasonalOverrides(e){let t=this.shadowRoot.querySelector("maintenance-seasonal-overrides-dialog");if(!t||!this._selectedEntryId)return;let a=e.adaptive_config?.seasonal_overrides;t.open(this._selectedEntryId,e.id,a)}async _reanalyzeInterval(e,t){try{let a=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:e,task_id:t});a.recommended_interval?this._showToast(`${i("reanalyze_result",this._lang)}: ${a.recommended_interval} ${i("days",this._lang)} (${i(`confidence_${a.confidence}`,this._lang)}, ${a.data_points} ${i("data_points",this._lang)})`):this._showToast(i("reanalyze_insufficient_data",this._lang)),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}}async _promptSkipTask(e,t){let a=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!a)return;let n=await a.prompt({title:i("skip",this._lang),message:i("skip_reason_prompt",this._lang),confirmText:i("skip",this._lang),inputLabel:i("reason_optional",this._lang),inputType:"text"});n.confirmed&&this._skipTask(e,t,n.value||void 0)}async _promptResetTask(e,t){let a=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!a)return;let n=await a.prompt({title:i("reset",this._lang),message:i("reset_date_prompt",this._lang),confirmText:i("reset",this._lang),inputLabel:i("reset_date_optional",this._lang),inputType:"date"});n.confirmed&&this._resetTask(e,t,n.value||void 0)}_dismissSuggestion(e,t){e&&t&&this._dismissedSuggestions.add(`${e}_${t}`),this.requestUpdate()}async _handleQuickComplete(e,t,a){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/quick_complete",entry_id:e,task_id:t}),this._showToast(i("quick_complete_success",this._lang))}catch(n){(n?.code||"")==="no_defaults"?this._openCompleteDialog(e,t,a.name,this._features.checklists?a.checklist:void 0,this._features.adaptive&&!!a.adaptive_config?.enabled):this._showToast(i("action_error",this._lang))}}_openCompleteDialog(e,t,a,n,o){let l=this.shadowRoot.querySelector("maintenance-complete-dialog");l&&(l.entryId=e,l.taskId=t,l.taskName=a,l.lang=this._lang,l.checklist=n||[],l.adaptiveEnabled=!!o,l.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,a,n){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,a,n)}render(){return r`
      ...
      <div class="filter-bar">
        ...
        <label class="filter-field">
          ...
          <select
            ...
            @change=${o=>{let l=o.target.value;this._filterUser=l||null}}
          >
    ...
    `}_renderGroupedTasks(e,t){let a=new Map,n=i("unassigned",t);for(let p of e){let u=[];this._groupByMode==="area"?u=[(p.area_id?this.hass?.areas?.[p.area_id]?.name:null)||n]:this._groupByMode==="user"?u=[(p.responsible_user_id?this._userService?.getUserName(p.responsible_user_id):null)||n]:this._groupByMode==="group"&&(u=p.group_names.length>0?p.group_names:[n]);for(let h of u)a.has(h)||a.set(h,[]),a.get(h).push(p)}let o=[...a.entries()].sort(([p],[u])=>p===n&&u!==n?1:u===n&&p!==n?-1:p.localeCompare(u)),l=this._groupByMode==="area"?"mdi:map-marker-outline":this._groupByMode==="group"?"mdi:folder-outline":"mdi:account-outline";return r`
      ${o.map(([p,u])=>r`
      ...
      `)}
    `}_renderAllObjects(){let e=this._lang,t=this._isOperator,a=p=>{let u=1/0;for(let h of p.tasks){let m=h.days_until_due;m!=null&&m<u&&(u=m)}return u},n=[...this._objects];this._objectSortMode==="alphabetical"?n.sort((p,u)=>p.object.name.localeCompare(u.object.name)):this._objectSortMode==="task_count"?n.sort((p,u)=>u.tasks.length-p.tasks.length||p.object.name.localeCompare(u.object.name)):n.sort((p,u)=>a(p)-a(u)||p.object.name.localeCompare(u.object.name));let o=()=>{let p=new Map;for(let u of n){let h=u.object.area_id,m=h?this.hass?.areas?.[h]?.name||i("unassigned",e):i("no_area",e);p.has(m)||p.set(m,[]),p.get(m).push(u)}return new Map([...p.entries()].sort(([u],[h])=>u.localeCompare(h)))},l=p=>{let u=p.tasks.some(h=>h.status==="overdue"||h.status==="triggered");return r`
        <div class="object-card${u?" object-card-overdue":""}" @click=${()=>this._showObject(p.entry_id)}>
          ${u?r`<span class="overdue-dot" title="${i("has_overdue",e)}"></span>`:d}
          <div class="object-card-header">
            <span class="object-card-name">${p.object.name}</span>
            <span class="object-card-count">${p.tasks.length} ${i("tasks_lower",e)}</span>
          </div>
          ...
          ${p.tasks.length===0?r`<div class="object-card-empty">${i("no_tasks_yet",e)}</div>`:d}
        </div>
      `};return r`
      ...
      ${this._groupByMode==="area"?r`
          ${[...o().entries()].map(([p,u])=>r`
            <details class="group-section" open>
              ...
              <div class="objects-grid">${u.map(l)}</div>
            </details>
          `)}
        `:r`<div class="objects-grid">${n.map(l)}</div>`}
      ...
      <div class="groups-section">
        ...
        ${e.length===0?r`<div class="hint">${i("no_groups",t)}</div>`:r`
            <div class="groups-grid">
              ${e.map(([a,n])=>{let o=n.task_refs.map(l=>this._getTask(l.entry_id,l.task_id)?.name).filter(Boolean);return r`
                  <div class="group-card">
                    ...
                    <div class="group-card-tasks">
                      ${o.length>0?o.map(l=>r`<span class="group-task-chip">${l}</span>`):r`<span style="font-size:12px;color:var(--secondary-text-color)">${i("no_tasks_short",t)}</span>`}
                    </div>
                  </div>
                `})}
            </div>
    ...
    `}_openGroupCreate(){this.shadowRoot.querySelector("maintenance-group-dialog")?.openCreate()}_openGroupEdit(e){let t=this._groups[e];t&&this.shadowRoot.querySelector("maintenance-group-dialog")?.openEdit(e,t)}async _deleteGroup(e,t){let a=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(a?await a.confirm({title:i("delete_group",this._lang),message:i("delete_group_confirm",this._lang).replace("{name}",t),confirmText:i("delete",this._lang)}):confirm(`${i("delete_group_confirm",this._lang).replace("{name}",t)}`))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/group/delete",group_id:e}),await this._loadData()}catch{this._showToast(i("action_error",this._lang))}}_renderBudgetBar(){let e=this._budget;if(!e)return d;let t=this._lang,a=e.currency_symbol||"\u20AC",n=[];return e.monthly_budget>0&&n.push({label:i("budget_monthly",t),spent:e.monthly_spent,budget:e.monthly_budget}),e.yearly_budget>0&&n.push({label:i("budget_yearly",t),spent:e.yearly_spent,budget:e.yearly_budget}),n.length===0?d:r`
      <div class="budget-bars">
        ${n.map(o=>{let l=Math.min(100,Math.max(0,o.spent/o.budget*100)),p=l>=100?"var(--error-color, #f44336)":l>=e.alert_threshold_pct?"var(--warning-color, #ff9800)":"var(--success-color, #4caf50)";return r`
            <div class="budget-item">
              ...
              <div class="budget-bar">
                <div class="budget-bar-fill" style="width:${l}%; background:${p}"></div>
              </div>
            </div>
          `})}
      </div>
    `}_renderOverviewRow(e){let t=this._lang,a=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,n=0,o=qe.ok,l=!1;if(a&&e.days_until_due!==null){let m=(e.interval_days-e.days_until_due)/e.interval_days*100;n=Math.max(0,Math.min(100,m)),l=m>100,e.status==="overdue"?o=qe.overdue:e.status==="due_soon"&&(o=qe.due_soon)}let p=e.area_id?this.hass?.areas?.[e.area_id]?.name:null,u=e.responsible_user_id?this._userService?.getUserName(e.responsible_user_id):null,h=e.group_names.length>0||p||u;return r`
      <div class="task-row${e.enabled?"":" task-disabled"}">
        ...
        <span class="due-cell" @click=${()=>this._showTask(e.entry_id,e.task_id)}>
          ...
          ${a?r`<div class="days-bar"><div class="days-bar-fill${l?" overflow":""}" style="width:${n}%;background:${o}"></div></div>`:d}
    ...
    `}_openCompleteDialogForRow(e){let a=this._objects.find(n=>n.entry_id===e.entry_id)?.tasks.find(n=>n.id===e.task_id);this._openCompleteDialog(e.entry_id,e.task_id,e.task_name,this._features.checklists?a?.checklist:void 0,this._features.adaptive&&!!a?.adaptive_config?.enabled)}_renderTriggerProgress(e){let t=e.trigger_config??null;if(!t)return d;let a=t.type||"threshold",n=e.trigger_entity_info?.unit_of_measurement??"",o=0,l="";if(a==="threshold"){let h=e.trigger_current_value??null;if(h==null)return d;let m=t.trigger_above,v=t.trigger_below;if(m!=null){let f=v??0,k=m-f||1;o=Math.min(100,Math.max(0,(h-f)/k*100)),l=`${h.toFixed(1)} / ${m} ${n}`}else if(v!=null){let k=e.trigger_entity_info?.max??(v*2||100),b=k-v||1;o=Math.min(100,Math.max(0,(k-h)/b*100)),l=`${h.toFixed(1)} / ${v} ${n}`}else return d}else if(a==="counter"){let h=t.trigger_target_value||1,v=e.trigger_current_delta??null??e.trigger_current_value??null;if(v==null)return d;o=Math.min(100,Math.max(0,v/h*100)),l=`${v.toFixed(1)} / ${h} ${n}`}else if(a==="state_change"){let h=t.trigger_target_changes||1,m=e.trigger_current_value??null;if(m==null)return d;o=Math.min(100,Math.max(0,m/h*100)),l=`${Math.round(m)} / ${h}`}else if(a==="runtime"){let h=t.trigger_runtime_hours||100,m=e.trigger_current_value??null;if(m==null)return d;o=Math.min(100,Math.max(0,m/h*100)),l=`${m.toFixed(1)}h / ${h}h`}else if(a==="compound"){let h=t.compound_logic||t.operator||"AND",m=t.conditions?.length||0;l=`${h} (${m})`,o=e.trigger_active?100:0}else return d;let p=o>=100,u=o>90?"var(--error-color, #f44336)":o>70?"var(--warning-color, #ff9800)":"var(--primary-color)";return r`
      <div class="trigger-progress">
        ...
        <span class="trigger-progress-label">${l}</span>
      </div>
    `}_renderMiniSparkline(e){if(!e.trigger_config?.entity_id)return d;let t=e.trigger_config.entity_id,a=this._miniStatsData.get(t)||[],n=[];if(a.length>=2)n=a.map($=>({ts:$.ts,val:$.val}));else{if(!e.history)return d;for(let $ of e.history)$.trigger_value!=null&&n.push({ts:new Date($.timestamp).getTime(),val:$.trigger_value})}if(e.trigger_current_value!=null&&n.push({ts:Date.now(),val:e.trigger_current_value}),n.length<2)return d;n.sort(($,M)=>$.ts-M.ts);let o=ai,l=ii,p=n.map($=>$.val),u=Math.min(...p),h=Math.max(...p),m=h-u||1;u-=m*.1,h+=m*.1;let v=n[0].ts,k=n[n.length-1].ts-v||1,b=$=>($-v)/k*o,N=$=>2+(1-($-u)/(h-u))*(l-4),C=n;if(C.length>Ut){let $=Math.ceil(C.length/Ut);C=C.filter((M,L)=>L%$===0||L===C.length-1)}let E=C.map($=>`${b($.ts).toFixed(1)},${N($.val).toFixed(1)}`).join(" ");return r`
      <svg class="mini-sparkline" viewBox="0 0 ${o} ${l}" preserveAspectRatio="none" role="img" aria-label="${i("chart_mini_sparkline",this._lang)}">
        <polyline points="${E}" fill="none" stroke="var(--primary-color)" stroke-width="1.5" stroke-linejoin="round" />
      </svg>
    `}_renderDaysProgress(e){let t=this._lang;if(e.days_until_due==null||!e.interval_days||e.interval_days<=0)return d;let n=(e.interval_days-e.days_until_due)/e.interval_days*100,o=Math.max(0,Math.min(100,n)),l=n>100,p="var(--success-color, #4caf50)";return e.status==="overdue"?p="var(--error-color, #f44336)":e.status==="due_soon"&&(p="var(--warning-color, #ff9800)"),r`
      <div class="days-progress">
        <div class="days-progress-labels">
          <span>${e.last_performed?`${i("last_performed",t)}: ${se(e.last_performed,t)}`:""}</span>
          <span>${e.next_due?`${i("next_due",t)}: ${se(e.next_due,t)}`:""}</span>
        </div>
        <div class="days-progress-bar" role="progressbar" aria-valuenow="${Math.round(o)}" aria-valuemin="0" aria-valuemax="100" aria-label="${i("days_progress",t)}">
          <div class="days-progress-fill${l?" overflow":""}" style="width:${o}%;background:${p}"></div>
        </div>
    ...
    `}_renderObjectDetail(){if(!this._selectedEntryId)return d;let e=this._getObject(this._selectedEntryId);if(!e)return r`<p>Object not found.</p>`;let t=e.object,a=this._lang,n=this._isOperator;return r`
      <div class="detail-section">
        ...
        ${t.installation_date?r`<p class="meta">${i("installed",a)}: ${se(t.installation_date,a)}</p>`:d}
        ...
        ${e.tasks.length===0?r`<div class="empty-state-centered">
            ...
            </div>`:[...e.tasks].sort((o,l)=>{let p={overdue:0,triggered:1,due_soon:2,ok:3};return(p[o.status]??9)-(p[l.status]??9)||(o.days_until_due??99999)-(l.days_until_due??99999)}).map(o=>r`
              <div class="task-row${o.enabled?"":" task-disabled"}">
                ...
                <span class="due-cell" @click=${()=>this._showTask(e.entry_id,o.id)}>
                  <span class="due-text">${Ce(o.days_until_due,a)}</span>
                  ${o.trigger_config?this._renderTriggerProgress(o):d}
                  ${this._renderMiniSparkline(o)}
                </span>
                <span class="row-actions">
                  <mwc-icon-button class="btn-complete" title="${i("complete",a)}" @click=${l=>{l.stopPropagation(),this._openCompleteDialog(e.entry_id,o.id,o.name,this._features.checklists?o.checklist:void 0,this._features.adaptive&&!!o.adaptive_config?.enabled)}}>
                    <ha-icon icon="mdi:check"></ha-icon>
                  </mwc-icon-button>
                  <mwc-icon-button class="btn-skip" title="${i("skip",a)}" .disabled=${this._actionLoading} @click=${l=>{l.stopPropagation(),this._promptSkipTask(e.entry_id,o.id)}}>
                    <ha-icon icon="mdi:skip-next"></ha-icon>
                  </mwc-icon-button>
                </span>
              </div>
    ...
    `}_renderTaskHeader(e){let t=this._lang,n=this._getObject(this._selectedEntryId)?.object.name||"",o=this._isOperator,l=e.status==="due_soon"?"warning":e.status||"ok",p=i(e.status||"ok",t);return r`
      <div class="task-header">
        <div class="task-header-title">
          ...
          <span class="status-chip ${l}">${p}</span>
    ...
    `}_renderTaskMeta(e){let t=e.documentation_url&&/^https?:\/\//i.test(e.documentation_url)?e.documentation_url:null,a=this._selectedEntryId?this._getObject(this._selectedEntryId):void 0,n=a?.object?.documentation_url,o=n&&/^https?:\/\//i.test(n)?n:null;if(!e.notes&&!t&&!o)return d;let l=this._lang;return r`
      <div class="task-meta-card">
        ...
        ${t?r`
          <div class="task-meta-row task-meta-link">
            ...
            <a href="${t}" target="_blank" rel="noopener noreferrer">${i("documentation_label",l)}</a>
          </div>
        `:d}
        ${o?r`
          <div class="task-meta-row task-meta-link">
            <ha-icon icon="mdi:book-open-variant"></ha-icon>
            <a href="${o}" target="_blank" rel="noopener noreferrer">${i("documentation_url_label",l)} (${a?.object?.name||""})</a>
          </div>
        `:d}
      </div>
    `}_renderKPIBar(e){let t=this._lang,a=e.times_performed>0?e.total_cost/e.times_performed:0,n=e.days_until_due!==null&&e.days_until_due!==void 0?e.days_until_due<0?"overdue":e.days_until_due<=e.warning_days?"warning":"":"";return r`
      <div class="kpi-bar">
        <div class="kpi-card">
          ...
          <div class="kpi-value">${e.next_due?se(e.next_due,t):"\u2014"}</div>
        ...
        <div class="kpi-card">
          ...
          <div class="kpi-value">${e.last_performed?se(e.last_performed,t):"\u2014"}</div>
        </div>
        <div class="kpi-card">
    ...
    `}_renderRecommendationCard(e){let t=this._lang;if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return d;if(this._selectedEntryId&&this._selectedTaskId&&this._dismissedSuggestions.has(`${this._selectedEntryId}_${this._selectedTaskId}`))return d;let a=e.suggested_interval;return r`
      <div class="recommendation-card">
        ...
        ${Pt(e.interval_days,a,e.interval_confidence||"medium",t)}
      ...
      <div class="recent-activities">
        ...
        ${a.map(o=>r`
          <div class="activity-item">
            ...
            <span class="activity-date">${Xe(o.timestamp,t)}</span>
    ...
    `}_renderHistoryEntry(e){let t=this._lang,a=["completed","reset","skipped"].includes(e.type);return r`
      <div class="history-entry">
        <div class="history-icon ${e.type}">
          <ha-icon .icon=${xt[e.type]||"mdi:circle"}></ha-icon>
        </div>
        <div class="history-content">
          ...
          <div class="history-date">${Xe(e.timestamp,t)}</div>
