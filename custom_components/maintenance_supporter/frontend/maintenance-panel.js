...
<body>
  ...
  `}function ps(o,r,e){let t=o.trigger_config;if(!t)return d;let i=t.type||"threshold",a=i==="counter"&&t.trigger_delta_mode,l=e.isCounterEntity(t),c=t.entity_id||"",u=e.detailStatsData.get(c)||[],g=[],m=!1;if(u.length>=2)for(let y of u){let M=y.val;a&&o.trigger_baseline_value!=null&&(M-=o.trigger_baseline_value);let N={ts:y.ts,val:M};!l&&y.min!=null&&y.max!=null&&(N.min=a&&o.trigger_baseline_value!=null?y.min-o.trigger_baseline_value:y.min,N.max=a&&o.trigger_baseline_value!=null?y.max-o.trigger_baseline_value:y.max,m=!0),g.push(N)}else for(let y of o.history)y.trigger_value!=null&&g.push({ts:new Date(y.timestamp).getTime(),val:y.trigger_value});if(o.trigger_current_value!=null){let y=o.trigger_current_value;a&&o.trigger_baseline_value!=null&&(y-=o.trigger_baseline_value),g.push({ts:Date.now(),val:y})}if(g.length<2&&c&&e.hasStatsService&&!e.detailStatsData.has(c))return n`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
    ...
    </div>`;if(g.length<2)return d;g.sort((y,M)=>y.ts-M.ts);let _=cs,v=ds,b=30,x=2,f=8,L=16,I=g.map(y=>y.val),S=Math.min(...I),k=Math.max(...I);if(m)for(let y of g)y.min!=null&&(S=Math.min(S,y.min)),y.max!=null&&(k=Math.max(k,y.max));t.trigger_above!=null&&(k=Math.max(k,t.trigger_above),S=Math.min(S,t.trigger_above)),t.trigger_below!=null&&(S=Math.min(S,t.trigger_below),k=Math.max(k,t.trigger_below));let R=null,O=null;if(i==="counter"&&t.trigger_target_value!=null){if(o.trigger_baseline_value!=null)R=o.trigger_baseline_value;else if(g.length>0){let y=[...o.history].filter(M=>M.type==="completed"||M.type==="reset").sort((M,N)=>new Date(N.timestamp).getTime()-new Date(M.timestamp).getTime())[0];if(y){let M=new Date(y.timestamp).getTime(),N=g[0],W=Math.abs(g[0].ts-M);for(let re of g){let ne=Math.abs(re.ts-M);ne<W&&(N=re,W=ne)}R=N.val}else R=g[0].val}R!=null?(O=R+t.trigger_target_value,k=Math.max(k,O),S=Math.min(S,R)):(k=Math.max(k,t.trigger_target_value),S=Math.min(S,0))}a&&o.trigger_baseline_value!=null&&(S=Math.min(S,0));let K=k-S||1;S-=K*.1,k+=K*.1;let Q=g[0].ts,H=g[g.length-1].ts,E=H-Q||1,q=y=>b+(y-Q)/E*(_-b-x),P=y=>f+(1-(y-S)/(k-S))*(v-f-L),Ge=g.map(y=>`${q(y.ts).toFixed(1)},${P(y.val).toFixed(1)}`).join(" "),ui=`M${q(g[0].ts).toFixed(1)},${v-L} `+g.map(y=>`L${q(y.ts).toFixed(1)},${P(y.val).toFixed(1)}`).join(" ")+` L${q(g[g.length-1].ts).toFixed(1)},${v-L} Z`,Qe="";if(m){let y=g.filter(M=>M.min!=null&&M.max!=null);if(y.length>=2){let M=y.map(W=>`${q(W.ts).toFixed(1)},${P(W.max).toFixed(1)}`),N=[...y].reverse().map(W=>`${q(W.ts).toFixed(1)},${P(W.min).toFixed(1)}`);Qe=`M${M[0]} `+M.slice(1).map(W=>`L${W}`).join(" ")+` L${N[0]} `+N.slice(1).map(W=>`L${W}`).join(" ")+" Z"}}let bt=g[g.length-1],hi=q(bt.ts),_i=P(bt.val),ft=y=>Math.abs(y)>=1e4?(y/1e3).toFixed(0)+"k":y>=1e3?(y/1e3).toFixed(1)+"k":y.toFixed(y<10?1:0),gi=ft(k),mi=ft(S),vi=o.history.filter(y=>["completed","skipped","reset"].includes(y.type)).map(y=>({ts:new Date(y.timestamp).getTime(),type:y.type})).filter(y=>y.ts>=Q&&y.ts<=H),Je=g;if(g.length>vt){let y=(g.length-1)/(vt-1);Je=[];for(let M=0;M<vt;M++)Je.push(g[Math.round(M*y)])}return n`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${_} ${v}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_sparkline",e.lang)}">
        ...
        ${Qe?z`<path d="${Qe}" fill="var(--primary-color)" opacity="0.08" />`:d}
        ...
        ${o.degradation_rate!=null&&o.degradation_trend!=="stable"&&o.degradation_trend!=="insufficient_data"&&g.length>=2?(()=>{let y=g[g.length-1],M=30,N=y.ts+M*864e5,W=y.val+o.degradation_rate*M,re=Math.min(N,H+(H-Q)*.3),ne=Math.max(S,Math.min(k,W)),bi=q(y.ts),fi=P(y.val),yi=q(re),xi=P(ne);return z`<line x1="${bi.toFixed(1)}" y1="${fi.toFixed(1)}" x2="${yi.toFixed(1)}" y2="${xi.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():d}
        ${i==="threshold"&&t.trigger_above!=null?z`<line x1="${b}" y1="${P(t.trigger_above).toFixed(1)}" x2="${_}" y2="${P(t.trigger_above).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${_-2}" y="${P(t.trigger_above)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25B2 ${t.trigger_above}</text>`:d}
        ${i==="threshold"&&t.trigger_below!=null?z`<line x1="${b}" y1="${P(t.trigger_below).toFixed(1)}" x2="${_}" y2="${P(t.trigger_below).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${_-2}" y="${P(t.trigger_below)+11}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25BC ${t.trigger_below}</text>`:d}
        ${i==="counter"&&O!=null?z`<line x1="${b}" y1="${P(O).toFixed(1)}" x2="${_}" y2="${P(O).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
        ...
        ${i==="counter"&&R!=null?z`<line x1="${b}" y1="${P(R).toFixed(1)}" x2="${_}" y2="${P(R).toFixed(1)}" stroke="var(--secondary-text-color)" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
        ...
        ${vi.map(y=>{let M=q(y.ts),N=y.type==="completed"?"var(--success-color, #4caf50)":y.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return z`
          ...
          `})}
        ${Je.map(y=>{let M=q(y.ts),N=P(y.val),W=new Date(y.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),re=`${y.val.toFixed(1)} ${r}`;return m&&y.min!=null&&y.max!=null&&(re+=` (${y.min.toFixed(1)}\u2013${y.max.toFixed(1)})`),z`<circle cx="${M.toFixed(1)}" cy="${N.toFixed(1)}" r="8" fill="transparent" tabindex="0"
...
`).map(y=>n`<div>${y}</div>`)}
  ...
  `}function hs(o,r,e,t,i){let b=Math.max(e,t,r,1)*1.3,x=50,f=[];for(let H=0;H<=x;H++){let E=H/x*b,q=1-Math.exp(-Math.pow(E/r,o)),P=32+E/b*260,Ge=136-q*128;f.push([P,Ge])}let L=f.map(([H,E])=>`${H.toFixed(1)},${E.toFixed(1)}`).join(" "),I="M32,136 "+f.map(([H,E])=>`L${H.toFixed(1)},${E.toFixed(1)}`).join(" ")+` L${f[x][0].toFixed(1)},136 Z`,S=32+e/b*260,k=1-Math.exp(-Math.pow(e/r,o)),R=136-k*128,O=((1-k)*100).toFixed(0),K=32+t/b*260,Q=[0,.25,.5,.75,1];return n`
  ...
  `}function ci(o,r){return ms(o,r)}function ms(o,r){let e=o.seasonal_factors??o.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return d;let t=o.interval_analysis?.seasonal_reason,i=new Date().getMonth(),a=300,l=100,c=8,g=l-c-4,m=Math.max(...e,1.5),_=a/12,v=_*.65,b=c+g-1/m*g;return n`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${a} ${l}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",r)}">
        ...
        ${e.map((x,f)=>{let L=x/m*g,I=f*_+(_-v)/2,S=c+g-L,k=f===i,R=x<1?"var(--success-color, #4caf50)":x>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return z`
