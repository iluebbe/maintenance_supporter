...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function bi(r,n,e){let t=r.trigger_config;if(!t)return d;let i=t.type||"threshold",s=i==="counter"&&t.trigger_delta_mode,l=e.isCounterEntity(t),c=t.entity_id||"",_=e.detailStatsData.get(c)||[],p=[],m=!1;if(_.length>=2)for(let h of _){let E=h.val;s&&r.trigger_baseline_value!=null&&(E-=r.trigger_baseline_value);let R={ts:h.ts,val:E};!l&&h.min!=null&&h.max!=null&&(R.min=s&&r.trigger_baseline_value!=null?h.min-r.trigger_baseline_value:h.min,R.max=s&&r.trigger_baseline_value!=null?h.max-r.trigger_baseline_value:h.max,m=!0),p.push(R)}else for(let h of r.history)h.trigger_value!=null&&p.push({ts:new Date(h.timestamp).getTime(),val:h.trigger_value});if(r.trigger_current_value!=null){let h=r.trigger_current_value;s&&r.trigger_baseline_value!=null&&(h-=r.trigger_baseline_value),p.push({ts:Date.now(),val:h})}if(p.length<2&&c&&e.hasStatsService&&!e.detailStatsData.has(c))return o`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
    ...
    </div>`;if(p.length<2)return d;p.sort((h,E)=>h.ts-E.ts);let v=vi,y=fi,f=30,k=2,T=8,q=16,C=p.map(h=>h.val),S=Math.min(...C),x=Math.max(...C);if(m)for(let h of p)h.min!=null&&(S=Math.min(S,h.min)),h.max!=null&&(x=Math.max(x,h.max));t.trigger_above!=null&&(x=Math.max(x,t.trigger_above),S=Math.min(S,t.trigger_above)),t.trigger_below!=null&&(S=Math.min(S,t.trigger_below),x=Math.max(x,t.trigger_below));let I=null,O=null;if(i==="counter"&&t.trigger_target_value!=null){if(r.trigger_baseline_value!=null)I=r.trigger_baseline_value;else if(p.length>0){let h=[...r.history].filter(E=>E.type==="completed"||E.type==="reset").sort((E,R)=>new Date(R.timestamp).getTime()-new Date(E.timestamp).getTime())[0];if(h){let E=new Date(h.timestamp).getTime(),R=p[0],P=Math.abs(p[0].ts-E);for(let Y of p){let X=Math.abs(Y.ts-E);X<P&&(R=Y,P=X)}I=R.val}else I=p[0].val}I!=null?(O=I+t.trigger_target_value,x=Math.max(x,O),S=Math.min(S,I)):(x=Math.max(x,t.trigger_target_value),S=Math.min(S,0))}s&&r.trigger_baseline_value!=null&&(S=Math.min(S,0));let Z=x-S||1;S-=Z*.1,x+=Z*.1;let J=p[0].ts,F=p[p.length-1].ts,$=F-J||1,j=h=>f+(h-J)/$*(v-f-k),z=h=>T+(1-(h-S)/(x-S))*(y-T-q),Te=p.map(h=>`${j(h.ts).toFixed(1)},${z(h.val).toFixed(1)}`).join(" "),At=`M${j(p[0].ts).toFixed(1)},${y-q} `+p.map(h=>`L${j(h.ts).toFixed(1)},${z(h.val).toFixed(1)}`).join(" ")+` L${j(p[p.length-1].ts).toFixed(1)},${y-q} Z`,Me="";if(m){let h=p.filter(E=>E.min!=null&&E.max!=null);if(h.length>=2){let E=h.map(P=>`${j(P.ts).toFixed(1)},${z(P.max).toFixed(1)}`),R=[...h].reverse().map(P=>`${j(P.ts).toFixed(1)},${z(P.min).toFixed(1)}`);Me=`M${E[0]} `+E.slice(1).map(P=>`L${P}`).join(" ")+` L${R[0]} `+R.slice(1).map(P=>`L${P}`).join(" ")+" Z"}}let Ve=p[p.length-1],St=j(Ve.ts),Tt=z(Ve.val),We=h=>Math.abs(h)>=1e4?(h/1e3).toFixed(0)+"k":h>=1e3?(h/1e3).toFixed(1)+"k":h.toFixed(h<10?1:0),Mt=We(x),zt=We(S),Ct=r.history.filter(h=>["completed","skipped","reset"].includes(h.type)).map(h=>({ts:new Date(h.timestamp).getTime(),type:h.type})).filter(h=>h.ts>=J&&h.ts<=F),ze=p;if(p.length>Be){let h=(p.length-1)/(Be-1);ze=[];for(let E=0;E<Be;E++)ze.push(p[Math.round(E*h)])}return o`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${v} ${y}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${a("chart_sparkline",e.lang)}">
        ...
        <text x="${v-k}" y="${y-1}" text-anchor="end" fill="var(--secondary-text-color)" font-size="7">${new Date(F).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        ...
        ${r.degradation_rate!=null&&r.degradation_trend!=="stable"&&r.degradation_trend!=="insufficient_data"&&p.length>=2?(()=>{let h=p[p.length-1],E=30,R=h.ts+E*864e5,P=h.val+r.degradation_rate*E,Y=Math.min(R,F+(F-J)*.3),X=Math.max(S,Math.min(x,P)),It=j(h.ts),jt=z(h.val),Lt=j(Y),Dt=z(X);return D`<line x1="${It.toFixed(1)}" y1="${jt.toFixed(1)}" x2="${Lt.toFixed(1)}" y2="${Dt.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():d}
...
`).map(h=>o`<div>${h}</div>`)}
  ...
  `}function xi(r,n,e,t,i){let f=Math.max(e,t,n,1)*1.3,k=50,T=[];for(let F=0;F<=k;F++){let $=F/k*f,j=1-Math.exp(-Math.pow($/n,r)),z=32+$/f*260,Te=136-j*128;T.push([z,Te])}let q=T.map(([F,$])=>`${F.toFixed(1)},${$.toFixed(1)}`).join(" "),C="M32,136 "+T.map(([F,$])=>`L${F.toFixed(1)},${$.toFixed(1)}`).join(" ")+` L${T[k][0].toFixed(1)},136 Z`,S=32+e/f*260,x=1-Math.exp(-Math.pow(e/n,r)),I=136-x*128,O=((1-x)*100).toFixed(0),Z=32+t/f*260,J=[0,.25,.5,.75,1];return o`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${a("chart_weibull",i)}">
        ${J.map(F=>{let $=136-F*128;return D`
  ...
  `}function Si(r,n,e){let t=r.history.filter($=>$.type==="completed"&&($.cost!=null||$.duration!=null)).map($=>({ts:new Date($.timestamp).getTime(),cost:$.cost??0,duration:$.duration??0})).sort(($,j)=>$.ts-j.ts);if(t.length<2)return d;let i=t.some($=>$.cost>0),s=t.some($=>$.duration>0);if(!i&&!s)return d;let l=e!=="duration"&&i,c=e!=="cost"&&s,_=l||!c&&i,p=c||!l&&s,m=Ei,v=Ai,y=_?32:8,f=p?32:8,k=8,T=20,q=m-y-f,C=v-k-T,S=Math.max(...t.map($=>$.cost))||1,x=Math.max(...t.map($=>$.duration))||1,I=Math.min(20,q/t.length*.6),O=q/t.length,Z=$=>y+O*$+O/2,J=$=>k+C-$/S*C,F=$=>k+C-$/x*C;return o`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${m} ${v}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${a("chart_history",n)}">
        ...
        `):d}
        ${p?D`
          <polyline points="${t.map(($,j)=>`${Z(j).toFixed(1)},${F($.duration).toFixed(1)}`).join(" ")}"
            fill="none" stroke="var(--accent-color, #ff9800)" stroke-width="2" stroke-linejoin="round" />
          ${t.map(($,j)=>D`
            <circle cx="${Z(j).toFixed(1)}" cy="${F($.duration).toFixed(1)}" r="3" fill="var(--accent-color, #ff9800)" />
          `)}
        `:d}
