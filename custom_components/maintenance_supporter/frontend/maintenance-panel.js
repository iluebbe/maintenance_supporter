...
`).map(g=>l`<div>${g}</div>`)}
  ...
  `}function li(s,a,e,t,i){let f=Math.max(e,t,a,1)*1.3,w=50,T=[];for(let z=0;z<=w;z++){let x=z/w*f,D=1-Math.exp(-Math.pow(x/a,s)),M=32+x/f*260,we=136-D*128;T.push([M,we])}let H=T.map(([z,x])=>`${z.toFixed(1)},${x.toFixed(1)}`).join(" "),C="M32,136 "+T.map(([z,x])=>`L${z.toFixed(1)},${x.toFixed(1)}`).join(" ")+` L${T[w][0].toFixed(1)},136 Z`,E=32+e/f*260,y=1-Math.exp(-Math.pow(e/a,s)),L=136-y*128,q=((1-y)*100).toFixed(0),W=32+t/f*260,B=[0,.25,.5,.75,1];return l`
