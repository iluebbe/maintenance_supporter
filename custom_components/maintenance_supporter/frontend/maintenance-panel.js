...
`;var Ae=class{constructor(n){this._cache=new Map;this._pending=new Map;this._hass=n}updateHass(n){this._hass=n}async getDetailStats(n,e){return this._getStats(n,"hour",30,e)}async getMiniStats(n,e){return this._getStats(n,"day",14,e)}async getBatchMiniStats(n){let e=new Map,t=[];for(let p of n){let u=`${p.entityId}:day`,m=this._cache.get(u);m&&Date.now()-m.fetchedAt<3e5?e.set(p.entityId,m.points):t.push(p)}if(t.length===0)return e;let i=t.filter(p=>p.isCounter).map(p=>p.entityId),s=t.filter(p=>!p.isCounter).map(p=>p.entityId),o=new Date(Date.now()-336*60*60*1e3).toISOString(),c=[];return i.length>0&&c.push(this._fetchBatch(i,"day",o,["state","sum","change"],!0,e)),s.length>0&&c.push(this._fetchBatch(s,"day",o,["mean","min","max"],!1,e)),await Promise.all(c),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(n,e,t,i){let s=`${n}:${e}`,o=this._cache.get(s);if(o&&Date.now()-o.fetchedAt<3e5)return o.points;if(this._pending.has(s))return this._pending.get(s);let c=this._fetchAndNormalize(n,e,t,i,s);this._pending.set(s,c);try{return await c}finally{this._pending.delete(s)}}async _fetchAndNormalize(n,e,t,i,s){let o=new Date(Date.now()-t*24*60*60*1e3).toISOString(),c=i?["state","sum","change"]:["mean","min","max"];try{let u=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:o,statistic_ids:[n],period:e,types:c}))[n]||[],m=this._normalizeRows(u,i);return this._cache.set(s,{entityId:n,fetchedAt:Date.now(),period:e,points:m}),m}catch(p){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${n}:`,p),[]}}async _fetchBatch(n,e,t,i,s,o){try{let c=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:n,period:e,types:i});for(let p of n){let u=c[p]||[],m=this._normalizeRows(u,s);o.set(p,m),this._cache.set(`${p}:${e}`,{entityId:p,fetchedAt:Date.now(),period:e,points:m})}}catch(c){console.warn("[maintenance-supporter] Batch statistics fetch failed:",c)}}_normalizeRows(n,e){let t=[];for(let i of n){let s=null;if(e?s=i.state??null:s=i.mean??null,s===null)continue;let o={ts:i.start,val:s};e||(i.min!=null&&(o.min=i.min),i.max!=null&&(o.max=i.max)),t.push(o)}return t.sort((i,s)=>i.ts-s.ts),t}};var le=class{constructor(n){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=n}updateHass(n){this.hass=n}async getUsers(n=!1){let e=Date.now();if(!n&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(n,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:n,task_id:e,user_id:t})}async getTasksByUser(n){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:n})).tasks}getUserName(n){return!n||!this.usersCache?null:this.usersCache.find(t=>t.id===n)?.name||null}getUser(n){return!n||!this.usersCache?null:this.usersCache.find(e=>e.id===n)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(n){return n?n===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var H=class extends M{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._name="";this._manufacturer="";this._model="";this._serialNumber="";this._areaId="";this._installationDate="";this._entryId=null}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}openCreate(){this._entryId=null,this._name="",this._manufacturer="",this._model="",this._serialNumber="",this._areaId="",this._installationDate="",this._error="",this._open=!0}openEdit(e,t){this._entryId=e,this._name=t.name||"",this._manufacturer=t.manufacturer||"",this._model=t.model||"",this._serialNumber=t.serial_number||"",this._areaId=t.area_id||"",this._installationDate=t.installation_date||"",this._error="",this._open=!0}async _save(){if(this._name.trim()){this._loading=!0,this._error="";try{this._entryId?await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/update",entry_id:this._entryId,name:this._name,manufacturer:this._manufacturer||null,model:this._model||null,serial_number:this._serialNumber||null,area_id:this._areaId||null,installation_date:this._installationDate||null}):await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/create",name:this._name,manufacturer:this._manufacturer||null,model:this._model||null,serial_number:this._serialNumber||null,area_id:this._areaId||null,installation_date:this._installationDate||null}),this._open=!1,this.dispatchEvent(new CustomEvent("object-saved"))}catch{this._error=a("save_error",this._lang)}finally{this._loading=!1}}}_close(){this._open=!1}render(){if(!this._open)return l``;let e=this._lang,t=this._entryId?a("edit_object",e):a("new_object",e);return l`
  ...
  `,_([E({attribute:!1})],H.prototype,"hass",2),_([g()],H.prototype,"_open",2),_([g()],H.prototype,"_loading",2),_([g()],H.prototype,"_error",2),_([g()],H.prototype,"_name",2),_([g()],H.prototype,"_manufacturer",2),_([g()],H.prototype,"_model",2),_([g()],H.prototype,"_serialNumber",2),_([g()],H.prototype,"_areaId",2),_([g()],H.prototype,"_installationDate",2),_([g()],H.prototype,"_entryId",2);customElements.get("maintenance-object-dialog")||customElements.define("maintenance-object-dialog",H);var oi=["cleaning","inspection","replacement","calibration","service","custom"],li=["time_based","sensor_based","manual"],di=["threshold","counter","state_change","runtime"],y=class extends M{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._entryId="";this._taskId=null;this._name="";this._type="custom";this._scheduleType="time_based";this._intervalDays="30";this._warningDays="7";this._intervalAnchor="completion";this._notes="";this._documentationUrl="";this._customIcon="";this._enabled=!0;this._triggerEntityId="";this._triggerEntityIds=[];this._triggerEntityLogic="any";this._triggerAttribute="";this._triggerType="threshold";this._triggerAbove="";this._triggerBelow="";this._triggerForMinutes="0";this._triggerTargetValue="";this._triggerDeltaMode=!1;this._triggerFromState="";this._triggerToState="";this._triggerTargetChanges="";this._triggerRuntimeHours="";this._suggestedAttributes=[];this._availableAttributes=[];this._entityDomain="";this._lastPerformed="";this._nfcTagId="";this._availableTags=[];this._responsibleUserId=null;this._availableUsers=[];this._environmentalEntity="";this._environmentalAttribute="";this._environmentalInitial="";this._userService=null}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}async openCreate(e){this._entryId=e,this._taskId=null,this._error="",this._resetFields(),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}async openEdit(e,t){this._entryId=e,this._taskId=t.id,this._error="",this._name=t.name,this._type=t.type,this._scheduleType=t.schedule_type,this._intervalDays=t.interval_days?.toString()||"30",this._warningDays=t.warning_days.toString(),this._intervalAnchor=t.interval_anchor||"completion",this._notes=t.notes||"",this._documentationUrl=t.documentation_url||"",this._customIcon=t.custom_icon||"",this._enabled=t.enabled!==!1,this._lastPerformed=t.last_performed||"",this._nfcTagId=t.nfc_tag_id||"",this._responsibleUserId=t.responsible_user_id||null;let i=t.adaptive_config||{};if(this._environmentalEntity=i.environmental_entity||"",this._environmentalAttribute=i.environmental_attribute||"",this._environmentalInitial=this._environmentalEntity,t.trigger_config){let s=t.trigger_config;this._triggerEntityId=s.entity_id||"",this._triggerEntityIds=s.entity_ids||(s.entity_id?[s.entity_id]:[]),this._triggerEntityLogic=s.entity_logic||"any",this._triggerAttribute=s.attribute||"",this._triggerType=s.type||"threshold",this._triggerAbove=s.trigger_above?.toString()||"",this._triggerBelow=s.trigger_below?.toString()||"",this._triggerForMinutes=s.trigger_for_minutes?.toString()||"0",this._triggerTargetValue=s.trigger_target_value?.toString()||"",this._triggerDeltaMode=s.trigger_delta_mode||!1,this._triggerFromState=s.trigger_from_state||"",this._triggerToState=s.trigger_to_state||"",this._triggerTargetChanges=s.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=s.trigger_runtime_hours?.toString()||""}else this._resetTriggerFields();this._triggerEntityId&&this._fetchEntityAttributes(this._triggerEntityId),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._warningDays="7",this._intervalAnchor="completion",this._notes="",this._documentationUrl="",this._customIcon="",this._enabled=!0,this._lastPerformed="",this._nfcTagId="",this._responsibleUserId=null,this._environmentalEntity="",this._environmentalAttribute="",this._environmentalInitial="",this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerForMinutes="0",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours=""}async _loadUsers(){this._userService||(this._userService=new le(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}async _loadTags(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tags/list"});this._availableTags=e.tags||[]}catch{this._availableTags=[]}}async _fetchEntityAttributes(e){if(!e||!this.hass){this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="";return}try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/entity/attributes",entity_id:e});this._entityDomain=t.domain||"",this._suggestedAttributes=t.suggested_attributes||[],this._availableAttributes=t.available_attributes||[]}catch{this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain=""}}async _save(){if(this._name.trim()){this._loading=!0,this._error="";try{let e={type:this._taskId?"maintenance_supporter/task/update":"maintenance_supporter/task/create",entry_id:this._entryId,name:this._name,task_type:this._type,schedule_type:this._scheduleType,warning_days:parseInt(this._warningDays,10)||7};if(this._taskId&&(e.task_id=this._taskId),this._scheduleType!=="manual"?this._intervalDays?(e.interval_days=parseInt(this._intervalDays,10),e.interval_anchor=this._intervalAnchor):this._taskId&&(e.interval_days=null,e.interval_anchor="completion"):this._taskId&&(e.interval_days=null,e.interval_anchor="completion"),e.notes=this._notes||null,e.documentation_url=this._documentationUrl||null,e.custom_icon=this._customIcon||null,e.enabled=this._enabled,e.last_performed=this._lastPerformed||null,e.nfc_tag_id=this._nfcTagId||null,e.responsible_user_id=this._responsibleUserId,this._scheduleType==="sensor_based"&&this._triggerEntityId){let s=this._triggerEntityIds.length>0?this._triggerEntityIds:[this._triggerEntityId],o={entity_id:s[0],entity_ids:s,type:this._triggerType};if(this._triggerAttribute&&(o.attribute=this._triggerAttribute),s.length>1&&(o.entity_logic=this._triggerEntityLogic),this._triggerType==="threshold"){if(this._triggerAbove){let c=parseFloat(this._triggerAbove);isNaN(c)||(o.trigger_above=c)}if(this._triggerBelow){let c=parseFloat(this._triggerBelow);isNaN(c)||(o.trigger_below=c)}if(this._triggerForMinutes){let c=parseInt(this._triggerForMinutes,10);isNaN(c)||(o.trigger_for_minutes=c)}}else if(this._triggerType==="counter"){if(this._triggerTargetValue){let c=parseFloat(this._triggerTargetValue);isNaN(c)||(o.trigger_target_value=c)}o.trigger_delta_mode=this._triggerDeltaMode}else if(this._triggerType==="state_change"){if(this._triggerFromState&&(o.trigger_from_state=this._triggerFromState),this._triggerToState&&(o.trigger_to_state=this._triggerToState),this._triggerTargetChanges){let c=parseInt(this._triggerTargetChanges,10);isNaN(c)||(o.trigger_target_changes=c)}}else if(this._triggerType==="runtime"&&this._triggerRuntimeHours){let c=parseFloat(this._triggerRuntimeHours);isNaN(c)||(o.trigger_runtime_hours=c)}e.trigger_config=o}else this._taskId&&(e.trigger_config=null);let t=await this.hass.connection.sendMessagePromise(e),i=this._taskId||t?.task_id;if(i&&this._scheduleType==="sensor_based"&&this._environmentalEntity!==this._environmentalInitial)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:i,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null})}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch{this._error=a("save_error",this._lang)}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return d;let e=this._lang;return l`
      ...
      <div class="select-row">
        ...
        >
          ${di.map(t=>l`<option value=${t} ?selected=${t===this._triggerType}>${a(t,e)}</option>`)}
        </select>
      </div>
    ...
    `}_renderTriggerTypeFields(){let e=this._lang;return this._triggerType==="threshold"?l`
      ...
      `:this._triggerType==="runtime"?l`
        ...
        ></ha-textfield>
      `:d}render(){if(!this._open)return l``;let e=this._lang,t=this._taskId?a("edit_task",e):a("new_task",e);return l`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
          ${this._error?l`<div class="error">${this._error}</div>`:d}
          ...
          <div class="select-row">
            ...
            >
              ${oi.map(i=>l`<option value=${i} ?selected=${i===this._type}>${a(i,e)}</option>`)}
            </select>
          </div>
          <div class="select-row">
            ...
            >
              ${li.map(i=>l`<option value=${i} ?selected=${i===this._scheduleType}>${a(i,e)}</option>`)}
            </select>
          </div>
  ...
  `,_([E({attribute:!1})],y.prototype,"hass",2),_([g()],y.prototype,"_open",2),_([g()],y.prototype,"_loading",2),_([g()],y.prototype,"_error",2),_([g()],y.prototype,"_entryId",2),_([g()],y.prototype,"_taskId",2),_([g()],y.prototype,"_name",2),_([g()],y.prototype,"_type",2),_([g()],y.prototype,"_scheduleType",2),_([g()],y.prototype,"_intervalDays",2),_([g()],y.prototype,"_warningDays",2),_([g()],y.prototype,"_intervalAnchor",2),_([g()],y.prototype,"_notes",2),_([g()],y.prototype,"_documentationUrl",2),_([g()],y.prototype,"_customIcon",2),_([g()],y.prototype,"_enabled",2),_([g()],y.prototype,"_triggerEntityId",2),_([g()],y.prototype,"_triggerEntityIds",2),_([g()],y.prototype,"_triggerEntityLogic",2),_([g()],y.prototype,"_triggerAttribute",2),_([g()],y.prototype,"_triggerType",2),_([g()],y.prototype,"_triggerAbove",2),_([g()],y.prototype,"_triggerBelow",2),_([g()],y.prototype,"_triggerForMinutes",2),_([g()],y.prototype,"_triggerTargetValue",2),_([g()],y.prototype,"_triggerDeltaMode",2),_([g()],y.prototype,"_triggerFromState",2),_([g()],y.prototype,"_triggerToState",2),_([g()],y.prototype,"_triggerTargetChanges",2),_([g()],y.prototype,"_triggerRuntimeHours",2),_([g()],y.prototype,"_suggestedAttributes",2),_([g()],y.prototype,"_availableAttributes",2),_([g()],y.prototype,"_entityDomain",2),_([g()],y.prototype,"_lastPerformed",2),_([g()],y.prototype,"_nfcTagId",2),_([g()],y.prototype,"_availableTags",2),_([g()],y.prototype,"_responsibleUserId",2),_([g()],y.prototype,"_availableUsers",2),_([g()],y.prototype,"_environmentalEntity",2),_([g()],y.prototype,"_environmentalAttribute",2);customElements.get("maintenance-task-dialog")||customElements.define("maintenance-task-dialog",y);var F=class extends M{constructor(){super(...arguments);this.entryId="";this.taskId="";this.taskName="";this.lang="en";this.checklist=[];this.adaptiveEnabled=!1;this._open=!1;this._notes="";this._cost="";this._duration="";this._loading=!1;this._error="";this._checklistState={};this._feedback="needed"}open(){this._open||(this._open=!0,this._notes="",this._cost="",this._duration="",this._error="",this._checklistState={},this._feedback="needed")}_toggleCheck(e){let t=String(e);this._checklistState={...this._checklistState,[t]:!this._checklistState[t]}}_setFeedback(e){this._feedback=e}async _complete(){this._loading=!0,this._error="";try{let e={type:"maintenance_supporter/task/complete",entry_id:this.entryId,task_id:this.taskId};if(this._notes&&(e.notes=this._notes),this._cost){let t=parseFloat(this._cost);!isNaN(t)&&t>=0&&(e.cost=t)}if(this._duration){let t=parseInt(this._duration,10);!isNaN(t)&&t>=0&&(e.duration=t)}this.checklist.length>0&&(e.checklist_state=this._checklistState),this.adaptiveEnabled&&(e.feedback=this._feedback),await this.hass.connection.sendMessagePromise(e),this._open=!1,this.dispatchEvent(new CustomEvent("task-completed"))}catch{this._error=a("save_error",this.lang)}finally{this._loading=!1}}_close(){this._open=!1}render(){if(!this._open)return l``;let e=this.lang||this.hass?.language||"en";return l`
      <ha-dialog open @closed=${this._close}>
...
<script>setTimeout(()=>window.print(),300)<\/script>
      ...
      <div class="settings-section">
        ...
        <label class="setting-row">
          ...
          <select @change=${i=>this._updateSetting("budget_currency",i.target.value)}>
            ${_i.map(i=>l`<option value=${i} ?selected=${t.currency===i}>${i}</option>`)}
          </select>
        </label>
        <label class="setting-row">
  ...
  `,_([E({attribute:!1})],W.prototype,"hass",2),_([g()],W.prototype,"_open",2),_([g()],W.prototype,"_loading",2),_([g()],W.prototype,"_error",2),_([g()],W.prototype,"_entryId",2),_([g()],W.prototype,"_taskId",2),_([g()],W.prototype,"_values",2);customElements.get("maintenance-seasonal-overrides-dialog")||customElements.define("maintenance-seasonal-overrides-dialog",W);var V=class extends M{constructor(){super(...arguments);this.objects=[];this._open=!1;this._loading=!1;this._error="";this._groupId=null;this._name="";this._description="";this._selected=new Set;this._toggleTask=(e,t)=>{let i=`${e}:${t}`,s=new Set(this._selected);s.has(i)?s.delete(i):s.add(i),this._selected=s};this._save=async()=>{let e=this._name.trim();if(!e){this._error=a("group_name_required",this._lang);return}this._loading=!0,this._error="";try{let t=this._buildTaskRefs();this._groupId?await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/group/update",group_id:this._groupId,name:e,description:this._description,task_refs:t}):await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/group/create",name:e,description:this._description,task_refs:t}),this._open=!1,this.dispatchEvent(new CustomEvent("group-saved"))}catch(t){this._error=t?.message||a("save_error",this._lang)}finally{this._loading=!1}}}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}openCreate(){this._reset(),this._open=!0}openEdit(e,t){this._reset(),this._groupId=e,this._name=t.name,this._description=t.description||"",this._selected=new Set(t.task_refs.map(i=>`${i.entry_id}:${i.task_id}`)),this._open=!0}_reset(){this._groupId=null,this._name="",this._description="",this._selected=new Set,this._error=""}_close(){this._open=!1}_buildTaskRefs(){return[...this._selected].map(e=>{let[t,i]=e.split(":",2);return{entry_id:t,task_id:i}})}render(){if(!this._open)return l``;let e=this._lang,t=this._groupId?a("edit_group",e):a("new_group",e);return l`
      <ha-dialog open @closed=${this._close} heading="${t}">
        <div class="content">
          ...
          ${this.objects.length===0?l`<div class="hint">${a("no_objects",e)}</div>`:l`
              <div class="objects">
                ${this.objects.map(i=>l`
                  <div class="object-block">
                    <div class="object-name">${i.object.name}</div>
                    ${i.tasks.length===0?l`<div class="hint small">${a("no_tasks_short",e)}</div>`:i.tasks.map(s=>{let o=`${i.entry_id}:${s.id}`,c=this._selected.has(o);return l`
  ...
  `,_([E({attribute:!1})],V.prototype,"hass",2),_([E({attribute:!1})],V.prototype,"objects",2),_([g()],V.prototype,"_open",2),_([g()],V.prototype,"_loading",2),_([g()],V.prototype,"_error",2),_([g()],V.prototype,"_groupId",2),_([g()],V.prototype,"_name",2),_([g()],V.prototype,"_description",2),_([g()],V.prototype,"_selected",2);customElements.get("maintenance-group-dialog")||customElements.define("maintenance-group-dialog",V);var pi=300,gi=140,Ue=27;function vt(r,n){let e=r.trigger_config;if(!e)return d;let t=n.lang,i=r.trigger_entity_info,s=r.trigger_entity_infos,o=i?.friendly_name||e.entity_id||"\u2014",c=e.entity_id||"",p=e.entity_ids||(c?[c]:[]),u=i?.unit_of_measurement||"",m=r.trigger_current_value,v=e.type||"threshold",b=p.length>1;return l`
    <h3>${a("trigger",t)}</h3>
    <div class="trigger-card">
      <div class="trigger-header">
        ...
        <div>
          ${b?l`
            <div class="trigger-entity-name">${p.length} ${a("entities",t)} (${e.entity_logic||"any"})</div>
            <div class="trigger-entity-id">${p.map((f,k)=>l`${k>0?", ":""}<span class="entity-link" @click=${T=>se(T,f)}>${f}</span>`)}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `:l`
            <div class="trigger-entity-name">${o}</div>
            <div class="trigger-entity-id">${c?l`<span class="entity-link" @click=${f=>se(f,c)}>${c}</span>`:""}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `}
        </div>
      ...
      <div class="trigger-limits">
        ${v==="threshold"?l`
          ${e.trigger_above!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${a("threshold_above",t)}: ${e.trigger_above} ${u}</span>`:d}
          ${e.trigger_below!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${a("threshold_below",t)}: ${e.trigger_below} ${u}</span>`:d}
          ${e.trigger_for_minutes?l`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${a("for_minutes",t)}: ${e.trigger_for_minutes}</span>`:d}
        `:d}
        ${v==="counter"?l`
          ${e.trigger_target_value!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${a("target_value",t)}: ${e.trigger_target_value} ${u}</span>`:d}
        `:d}
        ${v==="state_change"?l`
          ${e.trigger_target_changes!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${a("target_changes",t)}: ${e.trigger_target_changes}</span>`:d}
        `:d}
        ${v==="runtime"?l`
          ${e.trigger_runtime_hours!=null?l`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${a("runtime_hours",t)}: ${e.trigger_runtime_hours}h</span>`:d}
        `:d}
        ${v==="compound"?l`
          ...
          ${(e.conditions||[]).map((f,k)=>l`
            <span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${k+1}. ${a(f.type||"unknown",t)}: ${f.entity_id?l`<span class="entity-link" @click=${T=>se(T,f.entity_id)}>${f.entity_id}</span>`:""}</span>
          `)}
        `:d}
        ${i?.min!=null?l`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${a("min",t)}: ${i.min} ${u}</span>`:d}
        ${i?.max!=null?l`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${a("max",t)}: ${i.max} ${u}</span>`:d}
      </div>
  ...
  `}function hi(r,n,e){let t=r.trigger_config;if(!t)return d;let i=t.type||"threshold",s=i==="counter"&&t.trigger_delta_mode,o=e.isCounterEntity(t),c=t.entity_id||"",p=e.detailStatsData.get(c)||[],u=[],m=!1;if(p.length>=2)for(let h of p){let w=h.val;s&&r.trigger_baseline_value!=null&&(w-=r.trigger_baseline_value);let R={ts:h.ts,val:w};!o&&h.min!=null&&h.max!=null&&(R.min=s&&r.trigger_baseline_value!=null?h.min-r.trigger_baseline_value:h.min,R.max=s&&r.trigger_baseline_value!=null?h.max-r.trigger_baseline_value:h.max,m=!0),u.push(R)}else for(let h of r.history)h.trigger_value!=null&&u.push({ts:new Date(h.timestamp).getTime(),val:h.trigger_value});if(r.trigger_current_value!=null){let h=r.trigger_current_value;s&&r.trigger_baseline_value!=null&&(h-=r.trigger_baseline_value),u.push({ts:Date.now(),val:h})}if(u.length<2&&c&&e.hasStatsService&&!e.detailStatsData.has(c))return l`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
      <ha-icon icon="mdi:chart-line" style="--mdc-icon-size:16px;margin-right:8px;"></ha-icon>
      ${a("loading_chart",e.lang)}
    </div>`;if(u.length<2)return d;u.sort((h,w)=>h.ts-w.ts);let v=pi,b=gi,f=30,k=2,T=8,q=16,C=u.map(h=>h.val),S=Math.min(...C),x=Math.max(...C);if(m)for(let h of u)h.min!=null&&(S=Math.min(S,h.min)),h.max!=null&&(x=Math.max(x,h.max));t.trigger_above!=null&&(x=Math.max(x,t.trigger_above),S=Math.min(S,t.trigger_above)),t.trigger_below!=null&&(S=Math.min(S,t.trigger_below),x=Math.max(x,t.trigger_below));let I=null,O=null;if(i==="counter"&&t.trigger_target_value!=null){if(r.trigger_baseline_value!=null)I=r.trigger_baseline_value;else if(u.length>0){let h=[...r.history].filter(w=>w.type==="completed"||w.type==="reset").sort((w,R)=>new Date(R.timestamp).getTime()-new Date(w.timestamp).getTime())[0];if(h){let w=new Date(h.timestamp).getTime(),R=u[0],P=Math.abs(u[0].ts-w);for(let Q of u){let Y=Math.abs(Q.ts-w);Y<P&&(R=Q,P=Y)}I=R.val}else I=u[0].val}I!=null?(O=I+t.trigger_target_value,x=Math.max(x,O),S=Math.min(S,I)):(x=Math.max(x,t.trigger_target_value),S=Math.min(S,0))}s&&r.trigger_baseline_value!=null&&(S=Math.min(S,0));let J=x-S||1;S-=J*.1,x+=J*.1;let K=u[0].ts,N=u[u.length-1].ts,$=N-K||1,j=h=>f+(h-K)/$*(v-f-k),z=h=>T+(1-(h-S)/(x-S))*(b-T-q),Se=u.map(h=>`${j(h.ts).toFixed(1)},${z(h.val).toFixed(1)}`).join(" "),Et=`M${j(u[0].ts).toFixed(1)},${b-q} `+u.map(h=>`L${j(h.ts).toFixed(1)},${z(h.val).toFixed(1)}`).join(" ")+` L${j(u[u.length-1].ts).toFixed(1)},${b-q} Z`,Te="";if(m){let h=u.filter(w=>w.min!=null&&w.max!=null);if(h.length>=2){let w=h.map(P=>`${j(P.ts).toFixed(1)},${z(P.max).toFixed(1)}`),R=[...h].reverse().map(P=>`${j(P.ts).toFixed(1)},${z(P.min).toFixed(1)}`);Te=`M${w[0]} `+w.slice(1).map(P=>`L${P}`).join(" ")+` L${R[0]} `+R.slice(1).map(P=>`L${P}`).join(" ")+" Z"}}let Be=u[u.length-1],At=j(Be.ts),St=z(Be.val),Ve=h=>Math.abs(h)>=1e4?(h/1e3).toFixed(0)+"k":h>=1e3?(h/1e3).toFixed(1)+"k":h.toFixed(h<10?1:0),Tt=Ve(x),Mt=Ve(S),zt=r.history.filter(h=>["completed","skipped","reset"].includes(h.type)).map(h=>({ts:new Date(h.timestamp).getTime(),type:h.type})).filter(h=>h.ts>=K&&h.ts<=N),Me=u;if(u.length>Ue){let h=(u.length-1)/(Ue-1);Me=[];for(let w=0;w<Ue;w++)Me.push(u[Math.round(w*h)])}return l`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${v} ${b}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${a("chart_sparkline",e.lang)}">
        <text x="${f-3}" y="${T+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${Tt}</text>
        <text x="${f-3}" y="${b-q+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${Mt}</text>
        <text x="${f}" y="${b-1}" text-anchor="start" fill="var(--secondary-text-color)" font-size="7">${new Date(K).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        <text x="${v-k}" y="${b-1}" text-anchor="end" fill="var(--secondary-text-color)" font-size="7">${new Date(N).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        ${Te?D`<path d="${Te}" fill="var(--primary-color)" opacity="0.08" />`:d}
        ...
        ${zt.map(h=>{let w=j(h.ts),R=h.type==="completed"?"var(--success-color, #4caf50)":h.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return D`
          ...
          `})}
        ${Me.map(h=>{let w=j(h.ts),R=z(h.val),P=new Date(h.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),Q=`${h.val.toFixed(1)} ${n}`;return m&&h.min!=null&&h.max!=null&&(Q+=` (${h.min.toFixed(1)}\u2013${h.max.toFixed(1)})`),D`<circle cx="${w.toFixed(1)}" cy="${R.toFixed(1)}" r="8" fill="transparent" tabindex="0"
            @mouseenter=${Y=>mt(Y,`${P}
${Q}`,e.setTooltip)}
            @focus=${Y=>mt(Y,`${P}
${Q}`,e.setTooltip)}
...
`).map(h=>l`<div>${h}</div>`)}
  ...
  `}function mt(r,n,e){let t=r.currentTarget,i=t.closest(".sparkline-container");if(!i)return;let s=i.getBoundingClientRect(),o=t.getBoundingClientRect();e({x:o.left-s.left+o.width/2,y:o.top-s.top-8,text:n})}function ft(r,n,e){let t=r.degradation_trend!=null&&r.degradation_trend!=="insufficient_data",i=r.days_until_threshold!=null,s=r.environmental_factor!=null&&r.environmental_factor!==1;if(!t&&!i&&!s)return d;let o=r.degradation_trend==="rising"?"M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z":r.degradation_trend==="falling"?"M16,18L18.29,15.71L13.41,10.83L9.41,14.83L2,7.41L3.41,6L9.41,12L13.41,8L19.71,14.29L22,12V18H16Z":"M22,12L18,8V11H3V13H18V16L22,12Z";return l`
      ...
      <div class="prediction-grid">
        ${t?l`
          <div class="prediction-item">
            ...
            ${r.degradation_rate!=null?l`<span class="prediction-rate">${r.degradation_rate>0?"+":""}${Math.abs(r.degradation_rate)>=10?Math.round(r.degradation_rate).toLocaleString():r.degradation_rate.toFixed(1)} ${r.trigger_entity_info?.unit_of_measurement||""}/${a("day_short",n)}</span>`:d}
          </div>
        `:d}
        ${i?l`
          <div class="prediction-item">
            ...
            ${r.threshold_prediction_date?l`<span class="prediction-date">${ee(r.threshold_prediction_date,n)}</span>`:d}
        ...
        ${s&&e.environmental?l`
          <div class="prediction-item">
            ...
            ${r.environmental_entity?l`<span class="prediction-entity entity-link" @click=${c=>se(c,r.environmental_entity)}>${r.environmental_entity}</span>`:d}
          </div>
        `:d}
      </div>
    </div>
  `}function bt(r,n){let e=r.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return d;let s=r.interval_days??0,o=r.suggested_interval??s;return l`
    <div class="weibull-section">
      ...
      ${vi(t,i,s,o,n)}
      ${fi(e,n)}
      ${e?.confidence_interval_low!=null?bi(e,r,n):d}
    </div>
  ...
  `}function vi(r,n,e,t,i){let f=Math.max(e,t,n,1)*1.3,k=50,T=[];for(let N=0;N<=k;N++){let $=N/k*f,j=1-Math.exp(-Math.pow($/n,r)),z=32+$/f*260,Se=136-j*128;T.push([z,Se])}let q=T.map(([N,$])=>`${N.toFixed(1)},${$.toFixed(1)}`).join(" "),C="M32,136 "+T.map(([N,$])=>`L${N.toFixed(1)},${$.toFixed(1)}`).join(" ")+` L${T[k][0].toFixed(1)},136 Z`,S=32+e/f*260,x=1-Math.exp(-Math.pow(e/n,r)),I=136-x*128,O=((1-x)*100).toFixed(0),J=32+t/f*260,K=[0,.25,.5,.75,1];return l`
    ...
    <div class="chart-legend">
      ...
      ${e>0?l`<span class="legend-item"><span class="legend-swatch" style="background:var(--primary-color, #03a9f4); opacity:0.5"></span> ${a("current_interval_marker",i)}</span>`:d}
      ${t>0&&t!==e?l`<span class="legend-item"><span class="legend-swatch" style="background:var(--success-color, #4caf50)"></span> ${a("recommended_marker",i)}</span>`:d}
    </div>
  `}function fi(r,n){return l`
  ...
  `}function bi(r,n,e){let t=r.confidence_interval_low,i=r.confidence_interval_high,s=n.suggested_interval??n.interval_days??0,o=n.interval_days??0,c=Math.max(0,t-5),u=i+5-c,m=(t-c)/u*100,v=(i-t)/u*100,b=(s-c)/u*100,f=o>0?(o-c)/u*100:-1;return l`
  ...
  `}function $t(r,n){return yi(r,n)}function yi(r,n){let e=r.seasonal_factors??r.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return d;let t=r.interval_analysis?.seasonal_reason,i=new Date().getMonth(),s=300,o=100,c=8,u=o-c-4,m=Math.max(...e,1.5),v=s/12,b=v*.65,f=c+u-1/m*u;return l`
    <div class="seasonal-chart">
      <div class="seasonal-chart-title">
        ...
        ${t?l`<span class="source-tag">${t==="learned"?a("seasonal_learned",n):a("seasonal_manual",n)}</span>`:d}
      </div>
      <svg viewBox="0 0 ${s} ${o}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${a("chart_seasonal",n)}">
        <line x1="0" y1="${f.toFixed(1)}" x2="${s}" y2="${f.toFixed(1)}"
          stroke="var(--divider-color)" stroke-width="1" stroke-dasharray="4,3" />
        ${e.map((k,T)=>{let q=k/m*u,C=T*v+(v-b)/2,S=c+u-q,x=T===i,I=k<1?"var(--success-color, #4caf50)":k>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return D`
      ...
      <div class="seasonal-labels">
        ${yt.map((k,T)=>l`<span class="seasonal-label ${T===i?"active-month":""}">${a(k,n)}</span>`)}
      </div>
    </div>
  `}var xi=300,$i=200;function kt(r,n,e,t){let i=r.history.filter(c=>c.type==="completed"&&(c.cost!=null||c.duration!=null));if(i.length<2)return d;let s=i.some(c=>(c.cost??0)>0),o=i.some(c=>(c.duration??0)>0);return!s&&!o?d:l`
  ...
  `}function ki(r,n,e){let t=r.history.filter($=>$.type==="completed"&&($.cost!=null||$.duration!=null)).map($=>({ts:new Date($.timestamp).getTime(),cost:$.cost??0,duration:$.duration??0})).sort(($,j)=>$.ts-j.ts);if(t.length<2)return d;let i=t.some($=>$.cost>0),s=t.some($=>$.duration>0);if(!i&&!s)return d;let o=e!=="duration"&&i,c=e!=="cost"&&s,p=o||!c&&i,u=c||!o&&s,m=xi,v=$i,b=p?32:8,f=u?32:8,k=8,T=20,q=m-b-f,C=v-k-T,S=Math.max(...t.map($=>$.cost))||1,x=Math.max(...t.map($=>$.duration))||1,I=Math.min(20,q/t.length*.6),O=q/t.length,J=$=>b+O*$+O/2,K=$=>k+C-$/S*C,N=$=>k+C-$/x*C;return l`
    <div class="sparkline-container">
  ...
  `}var wi=60,Ei=20,wt=30,A=class extends M{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._sparklineTooltip=null;this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1};this._actionLoading=!1;this._moreMenuOpen=!1;this._toastMessage="";this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab="dashboard";this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._deepLinkHandled=!1;this._onDialogEvent=async()=>{try{await this._loadData()}catch{}}}get _lang(){return this.hass?.language||"en"}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler);let e=localStorage.getItem("maintenance_supporter_sort");e&&["due_date","object","type","task_name"].includes(e)&&(this._sortMode=e)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new Ae(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new le(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,i,s,o]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);e&&(this._objects=e.objects),t&&(this._stats=t),i&&(this._budget=i),s&&(this._groups=s.groups||{}),o&&(this._features=o.features),this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("entry_id");if(!t)return;this._deepLinkHandled=!0;let i=e.get("task_id"),s=e.get("action"),o=window.location.pathname+window.location.hash;history.replaceState(history.state,"",o);let c=this._getObject(t);if(!c){this._showOverview();return}if(i){let p=c.tasks.find(u=>u.id===i);if(!p){this._showObject(t);return}this._showTask(t,i),s==="complete"&&requestAnimationFrame(()=>{this._openCompleteDialog(t,i,p.name,this._features.checklists?p.checklist:void 0,this._features.adaptive&&!!p.adaptive_config?.enabled)})}else this._showObject(t)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let i=await this._statsService.getDetailStats(e,t),s=new Map(this._detailStatsData);s.set(e,i),this._detailStatsData=s}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let i of this._objects)for(let s of i.tasks){let o=s.trigger_config?.entity_id;o&&e.push({entityId:o,isCounter:this._isCounterEntity(s.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let p of this._objects)for(let u of p.tasks)if(!(this._filterStatus&&u.status!==this._filterStatus)){if(this._filterUser){let m=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(u.responsible_user_id!==m)continue}e.push({entry_id:p.entry_id,task_id:u.id,object_name:p.object.name,task_name:u.name,type:u.type,schedule_type:u.schedule_type,status:u.status,days_until_due:u.days_until_due??null,next_due:u.next_due??null,trigger_active:u.trigger_active,trigger_current_value:u.trigger_current_value??null,trigger_current_delta:u.trigger_current_delta??null,trigger_config:u.trigger_config??null,trigger_entity_info:u.trigger_entity_info??null,times_performed:u.times_performed,total_cost:u.total_cost,interval_days:u.interval_days??null,interval_anchor:u.interval_anchor??null,history:u.history||[],enabled:u.enabled,nfc_tag_id:u.nfc_tag_id??null})}let t={overdue:0,triggered:1,due_soon:2,ok:3},i=(p,u)=>(t[p.status]??9)-(t[u.status]??9),s=(p,u)=>(p.days_until_due??99999)-(u.days_until_due??99999),o=(p,u)=>i(p,u)||s(p,u),c={due_date:o,object:(p,u)=>p.object_name.localeCompare(u.object_name)||o(p,u),type:(p,u)=>p.type.localeCompare(u.type)||o(p,u),task_name:(p,u)=>p.task_name.localeCompare(u.task_name)};return e.sort(c[this._sortMode]),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(s=>s.id===t)}_pushPanelState(e,t,i){let s={msp_view:e,msp_entry:t||null,msp_task:i||null};history.pushState(s,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let i=this._getTask(t.msp_entry,t.msp_task);i?.trigger_config?.entity_id&&this._fetchDetailStats(i.trigger_config.entity_id,this._isCounterEntity(i.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null;let i=this._getTask(e,t);if(i?.trigger_config?.entity_id){let s=i.trigger_config.entity_id,o=this._isCounterEntity(i.trigger_config);this._fetchDetailStats(s,o)}}_showToast(e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}async _deleteObject(e){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:a("delete",this._lang),message:a("confirm_delete_object",this._lang),confirmText:a("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData()}catch{this._showToast(a("action_error",this._lang))}}async _deleteTask(e,t){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:a("delete",this._lang),message:a("confirm_delete_task",this._lang),confirmText:a("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData()}catch{this._showToast(a("action_error",this._lang))}}async _skipTask(e,t,i){this._actionLoading=!0;try{let s={type:"maintenance_supporter/task/skip",entry_id:e,task_id:t};i&&(s.reason=i),await this.hass.connection.sendMessagePromise(s),await this._loadData()}catch{this._showToast(a("action_error",this._lang))}finally{this._actionLoading=!1}}async _resetTask(e,t,i){this._actionLoading=!0;try{let s={type:"maintenance_supporter/task/reset",entry_id:e,task_id:t};i&&(s.date=i),await this.hass.connection.sendMessagePromise(s),await this._loadData()}catch{this._showToast(a("action_error",this._lang))}finally{this._actionLoading=!1}}async _applySuggestion(e,t,i){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:i}),await this._loadData()}catch{this._showToast(a("action_error",this._lang))}}_openSeasonalOverrides(e){let t=this.shadowRoot.querySelector("maintenance-seasonal-overrides-dialog");if(!t||!this._selectedEntryId)return;let i=e.adaptive_config?.seasonal_overrides;t.open(this._selectedEntryId,e.id,i)}async _reanalyzeInterval(e,t){try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:e,task_id:t});i.recommended_interval?this._showToast(`${a("reanalyze_result",this._lang)}: ${i.recommended_interval} ${a("days",this._lang)} (${a(`confidence_${i.confidence}`,this._lang)}, ${i.data_points} ${a("data_points",this._lang)})`):this._showToast(a("reanalyze_insufficient_data",this._lang)),await this._loadData()}catch{this._showToast(a("action_error",this._lang))}}async _promptSkipTask(e,t){let i=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!i)return;let s=await i.prompt({title:a("skip",this._lang),message:a("skip_reason_prompt",this._lang),confirmText:a("skip",this._lang),inputLabel:a("reason_optional",this._lang),inputType:"text"});s.confirmed&&this._skipTask(e,t,s.value||void 0)}async _promptResetTask(e,t){let i=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!i)return;let s=await i.prompt({title:a("reset",this._lang),message:a("reset_date_prompt",this._lang),confirmText:a("reset",this._lang),inputLabel:a("reset_date_optional",this._lang),inputType:"date"});s.confirmed&&this._resetTask(e,t,s.value||void 0)}_dismissSuggestion(e,t){e&&t&&this._dismissedSuggestions.add(`${e}_${t}`),this.requestUpdate()}_openCompleteDialog(e,t,i,s,o){let c=this.shadowRoot.querySelector("maintenance-complete-dialog");c&&(c.entryId=e,c.taskId=t,c.taskName=i,c.lang=this._lang,c.checklist=s||[],c.adaptiveEnabled=!!o,c.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,i,s){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,i,s)}render(){return l`
      ...
      <div class="objects-grid">
        ${this._objects.map(t=>l`
          <div class="object-card" @click=${()=>this._showObject(t.entry_id)}>
            ...
            ${t.tasks.length===0?l`<div class="object-card-empty">${a("no_tasks_yet",e)}</div>`:d}
          </div>
        `)}
      </div>
    `}async _onSettingsChanged(){await this._loadData()}_renderGroupsSection(){if(!this._features.groups)return d;let e=Object.entries(this._groups),t=this._lang;return l`
      <div class="groups-section">
        ...
        ${e.length===0?l`<div class="hint">${a("no_groups",t)}</div>`:l`
              ...
              ${e.map(([i,s])=>{let o=s.task_refs.map(c=>this._getTask(c.entry_id,c.task_id)?.name).filter(Boolean);return l`
                  <div class="group-card">
                    ...
                    <div class="group-card-tasks">
                      ${o.length>0?o.map(c=>l`<span class="group-task-chip">${c}</span>`):l`<span style="font-size:12px;color:var(--secondary-text-color)">${a("no_tasks_short",t)}</span>`}
                    </div>
                  </div>
                `})}
            </div>
    ...
    `}_renderOverviewRow(e){let t=this._lang,i=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,s=0,o=we.ok,c=!1;if(i&&e.days_until_due!==null){let p=(e.interval_days-e.days_until_due)/e.interval_days*100;s=Math.max(0,Math.min(100,p)),c=p>100,e.status==="overdue"?o=we.overdue:e.status==="due_soon"&&(o=we.due_soon)}return l`
      <div class="task-row${e.enabled?"":" task-disabled"}">
        <span class="status-badge ${e.status}">${a(e.status,t)}</span>
        ${e.enabled?d:l`<span class="badge-disabled">${a("disabled",t)}</span>`}
        ${e.nfc_tag_id?l`<span class="nfc-badge" title="${a("nfc_linked",t)}"><ha-icon icon="mdi:nfc-variant"></ha-icon></span>`:d}
        ...
        <span class="row-actions">
          <mwc-icon-button class="btn-complete" title="${a("complete",t)}" @click=${p=>{p.stopPropagation(),this._openCompleteDialogForRow(e)}}>
            <ha-icon icon="mdi:check"></ha-icon>
          </mwc-icon-button>
          <mwc-icon-button class="btn-skip" title="${a("skip",t)}" .disabled=${this._actionLoading} @click=${p=>{p.stopPropagation(),this._promptSkipTask(e.entry_id,e.task_id)}}>
    ...
    `}_renderMiniSparkline(e){if(!e.trigger_config?.entity_id)return d;let t=e.trigger_config.entity_id,i=this._miniStatsData.get(t)||[],s=[];if(i.length>=2)s=i.map(x=>({ts:x.ts,val:x.val}));else{if(!e.history)return d;for(let x of e.history)x.trigger_value!=null&&s.push({ts:new Date(x.timestamp).getTime(),val:x.trigger_value})}if(e.trigger_current_value!=null&&s.push({ts:Date.now(),val:e.trigger_current_value}),s.length<2)return d;s.sort((x,I)=>x.ts-I.ts);let o=wi,c=Ei,p=s.map(x=>x.val),u=Math.min(...p),m=Math.max(...p),v=m-u||1;u-=v*.1,m+=v*.1;let b=s[0].ts,k=s[s.length-1].ts-b||1,T=x=>(x-b)/k*o,q=x=>2+(1-(x-u)/(m-u))*(c-4),C=s;if(C.length>wt){let x=Math.ceil(C.length/wt);C=C.filter((I,O)=>O%x===0||O===C.length-1)}let S=C.map(x=>`${T(x.ts).toFixed(1)},${q(x.val).toFixed(1)}`).join(" ");return l`
      <svg class="mini-sparkline" viewBox="0 0 ${o} ${c}" preserveAspectRatio="none" role="img" aria-label="${a("chart_mini_sparkline",this._lang)}">
    ...
    `}_renderDaysProgress(e){let t=this._lang;if(e.days_until_due==null||!e.interval_days||e.interval_days<=0)return d;let s=(e.interval_days-e.days_until_due)/e.interval_days*100,o=Math.max(0,Math.min(100,s)),c=s>100,p="var(--success-color, #4caf50)";return e.status==="overdue"?p="var(--error-color, #f44336)":e.status==="due_soon"&&(p="var(--warning-color, #ff9800)"),l`
      <div class="days-progress">
        ...
        <div class="days-progress-bar" role="progressbar" aria-valuenow="${Math.round(o)}" aria-valuemin="0" aria-valuemax="100" aria-label="${a("days_progress",t)}">
    ...
    `}_renderObjectDetail(){if(!this._selectedEntryId)return d;let e=this._getObject(this._selectedEntryId);if(!e)return l`<p>Object not found.</p>`;let t=e.object,i=this._lang;return l`
      <div class="detail-section">
        ...
        ${t.serial_number?l`<p class="meta">${a("serial_number_label",i)}: ${t.serial_number}</p>`:d}
        ${t.installation_date?l`<p class="meta">${a("installed",i)}: ${ee(t.installation_date,i)}</p>`:d}
        ...
        ${e.tasks.length===0?l`<div class="empty-state-centered">
              ...
              <div class="task-row${s.enabled?"":" task-disabled"}">
                ...
                ${s.enabled?d:l`<span class="badge-disabled">${a("disabled",i)}</span>`}
                ${s.nfc_tag_id?l`<span class="nfc-badge" title="${a("nfc_linked",i)}"><ha-icon icon="mdi:nfc-variant"></ha-icon></span>`:d}
                ...
                <span class="row-actions">
                  <mwc-icon-button class="btn-complete" title="${a("complete",i)}" @click=${o=>{o.stopPropagation(),this._openCompleteDialog(e.entry_id,s.id,s.name,this._features.checklists?s.checklist:void 0,this._features.adaptive&&!!s.adaptive_config?.enabled)}}>
                    <ha-icon icon="mdi:check"></ha-icon>
                  </mwc-icon-button>
                  <mwc-icon-button class="btn-skip" title="${a("skip",i)}" .disabled=${this._actionLoading} @click=${o=>{o.stopPropagation(),this._promptSkipTask(e.entry_id,s.id)}}>
                    <ha-icon icon="mdi:skip-next"></ha-icon>
                  </mwc-icon-button>
                </span>
              </div>
    ...
    `}_renderTaskHeader(e){let t=this._lang,s=this._getObject(this._selectedEntryId)?.object.name||"",o=e.status==="due_soon"?"warning":e.status||"ok",c=a(e.status||"ok",t);return l`
      <div class="task-header">
        <div class="task-header-title">
          ...
          ${e.nfc_tag_id?l`<span class="nfc-badge" title="${a("nfc_tag_id",t)}: ${e.nfc_tag_id}"><ha-icon icon="mdi:nfc-variant"></ha-icon> NFC</span>`:l`<span class="nfc-badge unlinked" title="${a("nfc_link_hint",t)}"
          ...
          <div class="more-menu-wrapper">
            ...
            ${this._moreMenuOpen?l`
              <div class="popup-menu" @click=${p=>p.stopPropagation()}>
                ...
                <div class="popup-menu-item" @click=${()=>{this._closeMoreMenu();let p=this._getObject(this._selectedEntryId)?.object;this._openQrForTask(this._selectedEntryId,this._selectedTaskId,p?.name||"",e.name)}}><ha-icon icon="mdi:qrcode"></ha-icon> ${a("qr_code",t)}</div>
      ...
      <div class="tab-content overview-tab">
        ...
        <div class="two-column-layout ${o?"":"single-column"}">
          ...
          <div class="right-column">
            ${kt(e,t,this._costDurationToggle,u=>{this._costDurationToggle=u})}
          </div>
        </div>
        ${c?bt(e,t):d}
        ${p?l`
        ...
        <div class="interval-comparison">
          ...
          <div class="interval-bar">
            <div class="interval-label">${a("recommended",t)}: ${s} ${a("days",t)}
              <span class="confidence-badge ${o}">${a(`confidence_${o}`,t)}</span>
            </div>
      ...
      <div class="recent-activities">
        ...
        ${i.map(o=>l`
          <div class="activity-item">
            ...
            <span class="activity-date">${Oe(o.timestamp,t)}</span>
      ...
      <div class="history-filters-new">
        <div class="filter-chips">
          ${["completed","skipped","reset","triggered"].map(i=>{let s=e.history.filter(o=>o.type===i).length;return s===0?d:l`
            ...
            `})}
          ${this._historyFilter?l`<span class="filter-chip clear" @click=${()=>{this._historyFilter=null}}>${a("show_all",t)}</span>`:d}
        </div>
    ...
    `}_renderHistoryList(e){let t=this._lang,i=this._historyFilter?e.history.filter(s=>s.type===this._historyFilter):e.history;if(this._historySearch){let s=this._historySearch.toLowerCase();i=i.filter(o=>o.notes?.toLowerCase().includes(s))}return i.length===0?l`<p class="empty">${a("no_history",t)}</p>`:l`
      ...
      <div class="history-entry">
        ...
        <div class="history-content">
          ...
          <div class="history-details">
            ${e.cost!=null?l`<span>${a("cost",t)}: ${e.cost.toFixed(2)} ${this._budget?.currency_symbol||"\u20AC"}</span>`:d}
            ${e.duration!=null?l`<span>${a("duration",t)}: ${e.duration} min</span>`:d}
            ${e.trigger_value!=null?l`<span>${a("trigger_val",t)}: ${e.trigger_value}</span>`:d}
          </div>
        </div>
      </div>
    `}};A.styles=[pt,gt],_([E({attribute:!1})],A.prototype,"hass",2),_([E({type:Boolean,reflect:!0})],A.prototype,"narrow",2),_([E({attribute:!1})],A.prototype,"panel",2),_([g()],A.prototype,"_objects",2),_([g()],A.prototype,"_stats",2),_([g()],A.prototype,"_view",2),_([g()],A.prototype,"_selectedEntryId",2),_([g()],A.prototype,"_selectedTaskId",2),_([g()],A.prototype,"_filterStatus",2),_([g()],A.prototype,"_filterUser",2),_([g()],A.prototype,"_unsub",2),_([g()],A.prototype,"_sparklineTooltip",2),_([g()],A.prototype,"_historyFilter",2),_([g()],A.prototype,"_budget",2),_([g()],A.prototype,"_groups",2),_([g()],A.prototype,"_detailStatsData",2),_([g()],A.prototype,"_miniStatsData",2),_([g()],A.prototype,"_features",2),_([g()],A.prototype,"_actionLoading",2),_([g()],A.prototype,"_moreMenuOpen",2),_([g()],A.prototype,"_toastMessage",2),_([g()],A.prototype,"_overviewTab",2),_([g()],A.prototype,"_activeTab",2),_([g()],A.prototype,"_costDurationToggle",2),_([g()],A.prototype,"_historySearch",2),_([g()],A.prototype,"_sortMode",2),A=_([dt("maintenance-supporter-panel")],A);export{A as MaintenanceSupporterPanel};
