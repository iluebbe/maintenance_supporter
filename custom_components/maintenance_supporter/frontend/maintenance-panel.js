...
`;var B={overviewTab:"msp-overview-tab",collapsedSections:"msp-collapsed-sections",chartRange:"msp-chart-range",chartHideOutliers:"msp-chart-hide-outliers",taskSort:"maintenance_supporter_sort",objectSort:"maintenance_supporter_object_sort",groupBy:"maintenance_supporter_groupby",objectView:"maintenance_supporter_object_view"};var ys={days:1,weeks:7,months:30.4368,years:365.25};function Bt(n,r){return!n||n<=0?0:n*(ys[r||"days"]??1)}function vt(n,r,e){let t=Bt(n,e);if(t<=0||r==null)return{pct:0,overflow:!1};let i=(t-r)/t*100;return{pct:Math.max(0,Math.min(100,i)),overflow:i>100}}function z(n){return String(n??"").replace(/[&<>"']/g,r=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[r])}function fi(n,r,e,t,i,a){let l=[[e.manufacturer,n.manufacturer],[e.model,n.model],[e.serial,n.serial_number],[e.installed,n.installation_date?t(n.installation_date):null],[e.warranty,n.warranty_expiry?t(n.warranty_expiry):null]].filter(([,u])=>!!u),c=r.map(u=>{let g=e.scheduleLabel(u);return`<tr>
      <td>${z(u.name)}</td>
...
`;var xt=class{constructor(r){this._cache=new Map;this._pending=new Map;this._hass=r}updateHass(r){this._hass=r}async getDetailStats(r,e,t=30){return this._getStats(r,t<=35?"hour":"day",t,e)}async getMiniStats(r,e){return this._getStats(r,"day",14,e)}async getBatchMiniStats(r){let e=new Map,t=[];for(let h of r){let u=`${h.entityId}:day:14`,g=this._cache.get(u);g&&Date.now()-g.fetchedAt<3e5?e.set(h.entityId,g.points):t.push(h)}if(t.length===0)return e;let i=t.filter(h=>h.isCounter).map(h=>h.entityId),a=t.filter(h=>!h.isCounter).map(h=>h.entityId),l=new Date(Date.now()-336*60*60*1e3).toISOString(),c=[];return i.length>0&&c.push(this._fetchBatch(i,"day",l,["state","sum","change"],!0,e)),a.length>0&&c.push(this._fetchBatch(a,"day",l,["mean","min","max"],!1,e)),await Promise.all(c),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(r,e,t,i){let a=`${r}:${e}:${t}`,l=this._cache.get(a);if(l&&Date.now()-l.fetchedAt<3e5)return l.points;if(this._pending.has(a))return this._pending.get(a);let c=this._fetchAndNormalize(r,e,t,i,a);this._pending.set(a,c);try{return await c}finally{this._pending.delete(a)}}async _fetchAndNormalize(r,e,t,i,a){let l=new Date(Date.now()-t*24*60*60*1e3).toISOString(),c=i?["state","sum","change"]:["mean","min","max"];try{let u=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:l,statistic_ids:[r],period:e,types:c}))[r]||[],g=this._normalizeRows(u,i);return this._cache.set(a,{entityId:r,fetchedAt:Date.now(),period:e,points:g}),g}catch(h){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${r}:`,h),[]}}async _fetchBatch(r,e,t,i,a,l){try{let c=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:r,period:e,types:i});for(let h of r){let u=c[h]||[],g=this._normalizeRows(u,a);l.set(h,g),this._cache.set(`${h}:${e}:14`,{entityId:h,fetchedAt:Date.now(),period:e,points:g})}}catch(c){console.warn("[maintenance-supporter] Batch statistics fetch failed:",c)}}_normalizeRows(r,e){let t=[];for(let i of r){let a=null;if(e?a=i.state??null:a=i.mean??null,a===null)continue;let l={ts:i.start,val:a};e||(i.min!=null&&(l.min=i.min),i.max!=null&&(l.max=i.max)),t.push(l)}return t.sort((i,a)=>i.ts-a.ts),t}};var ye=class{constructor(r){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=r}updateHass(r){this.hass=r}async getUsers(r=!1){let e=Date.now();if(!r&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(r,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:r,task_id:e,user_id:t})}async getTasksByUser(r){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:r})).tasks}getUserName(r){return!r||!this.usersCache?null:this.usersCache.find(t=>t.id===r)?.name||null}getUser(r){return!r||!this.usersCache?null:this.usersCache.find(e=>e.id===r)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(r){return r?r===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var G=class extends S{constructor(){super(...arguments);this.label="";this.value="";this.placeholder="";this.type="text";this.required=!1;this.disabled=!1}_onInput(e){let t=e.target.value;this.value=t,this.dispatchEvent(new CustomEvent("input",{bubbles:!0,composed:!0,detail:{value:t}}))}render(){return o`
  ...
  `,d([y({attribute:!1})],F.prototype,"hass",2),d([y({attribute:!1})],F.prototype,"objects",2),d([_()],F.prototype,"_open",2),d([_()],F.prototype,"_loading",2),d([_()],F.prototype,"_error",2),d([_()],F.prototype,"_name",2),d([_()],F.prototype,"_manufacturer",2),d([_()],F.prototype,"_model",2),d([_()],F.prototype,"_serialNumber",2),d([_()],F.prototype,"_areaId",2),d([_()],F.prototype,"_installationDate",2),d([_()],F.prototype,"_warrantyExpiry",2),d([_()],F.prototype,"_documentationUrl",2),d([_()],F.prototype,"_notes",2),d([_()],F.prototype,"_haDeviceId",2),d([_()],F.prototype,"_parentEntryId",2),d([_()],F.prototype,"_entryId",2);customElements.get("maintenance-object-dialog")||customElements.define("maintenance-object-dialog",F);function de(n){let r=n??0;return r<1024?`${r} B`:r<1024*1024?`${(r/1024).toFixed(1)} KB`:`${(r/(1024*1024)).toFixed(1)} MB`}var $t=["manual","warranty","invoice","spare_parts","photo","other"],Es={manual:"mdi:book-open-variant",warranty:"mdi:shield-check",invoice:"mdi:receipt-text-outline",spare_parts:"mdi:cog-outline",photo:"mdi:image-outline",other:"mdi:file-document-outline"},q=class extends S{constructor(){super(...arguments);this.canWrite=!1;this._docs=[];this._loaded=!1;this._busy=!1;this._error="";this._hint="";this._addingLink=!1;this._linkUrl="";this._linkTitle="";this._category="manual";this._thumbs={};this._lightboxUrl="";this._editingId="";this._editTitle="";this._editCategory="manual";this._dragOver=!1;this._loadedFor=null;this._localeReady=!1}_isImage(e){return e.kind==="file"&&(e.mime||"").startsWith("image/")}async _sign(e){return(await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300})).path}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),this.hass&&!this._localeReady&&(this._localeReady=!0,Y(this._lang).then(()=>this.requestUpdate())),this.hass&&this.entryId&&this._loadedFor!==this.entryId&&(this._loadedFor=this.entryId,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:this.entryId});this._docs=e.documents||[],this._loaded=!0,this._error="",this._thumbs={},this._loadThumbs()}catch(e){this._error=I(e,this._lang),this._loaded=!0}}async _loadThumbs(){await Promise.all(this._docs.filter(e=>this._isImage(e)).map(async e=>{try{let t=await this._sign(e);this._thumbs={...this._thumbs,[e.id]:t}}catch{}}))}_category_of(e){return(e.tags||[]).find(i=>$t.includes(i))||"other"}_labelKeydown(e){(e.key==="Enter"||e.key===" ")&&(e.preventDefault(),e.currentTarget.querySelector("input")?.click())}_onFileInput(e){let t=e.target,i=Array.from(t.files??[]);i.length&&this._uploadFiles(i),t.value=""}_onCameraInput(e){let t=e.target,i=Array.from(t.files??[]);i.length&&this._uploadFiles(i,"photo"),t.value=""}_onDrop(e){if(e.preventDefault(),this._dragOver=!1,!this.canWrite||this._busy)return;let t=Array.from(e.dataTransfer?.files??[]);t.length&&this._uploadFiles(t)}_onDragOver(e){this.canWrite&&(e.preventDefault(),this._dragOver=!0)}_onDragLeave(e){let t=e.relatedTarget;(!t||!e.currentTarget.contains(t))&&(this._dragOver=!1)}async _uploadFiles(e,t){let i=t??this._category;this._busy=!0,this._error="",this._hint="";let a=0,l=0;try{for(let c of e){let h=new FormData;h.append("entry_id",this.entryId),h.append("tags",i),h.append("file",c,c.name);let u=await fetch("/api/maintenance_supporter/document/upload",{method:"POST",headers:{Authorization:`Bearer ${this.hass.auth?.data?.access_token??""}`},body:h});if(!u.ok){this._error=u.status===413?s("doc_too_large",this._lang):s("doc_upload_failed",this._lang);continue}let g=await u.json();g.duplicate_in_object?l++:g.deduped&&a++}l?this._hint=s("doc_dup_in_object",this._lang):a&&(this._hint=s("doc_deduped",this._lang)),await this._load()}catch{this._error=s("doc_upload_failed",this._lang)}finally{this._busy=!1}}async _download(e){try{let t=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:30});yt(t.path,e.filename||e.title||"document")}catch(t){this._error=I(t,this._lang)}}async _preview(e){if(this._isImage(e)){this._lightboxUrl=this._thumbs[e.id]||await this._sign(e);return}let t=window.open("about:blank","_blank");try{let i=await this._sign(e);t&&(t.location.href=new URL(i,window.location.origin).href)}catch(i){t&&t.close(),this._error=I(i,this._lang)}}_openDoc(e){e.kind==="file"?this._preview(e):ne(e.url)&&window.open(e.url,"_blank","noopener")}_startEdit(e){this._editingId=e.id,this._editTitle=e.title||"",this._editCategory=this._category_of(e),this._addingLink=!1,this._error=""}_cancelEdit(){this._editingId=""}async _saveEdit(e){let t=(e.tags||[]).filter(a=>!$t.includes(a)),i=e.kind==="file"?[this._editCategory,...t]:e.tags??[];this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,title:this._editTitle.trim()||e.filename||e.url||"",tags:i}),this._editingId="",await this._load()}catch(a){this._error=I(a,this._lang)}finally{this._busy=!1}}async _delete(e){let t=e.title||e.filename||e.url||"";if(window.confirm(s("doc_delete_confirm",this._lang).replace("{name}",t))){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/delete",doc_id:e.id}),await this._load()}catch(i){this._error=I(i,this._lang)}finally{this._busy=!1}}}async _addLink(){let e=this._linkUrl.trim();if(e){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/add_link",entry_id:this.entryId,url:e,title:this._linkTitle.trim()||null}),this._linkUrl="",this._linkTitle="",this._addingLink=!1,await this._load()}catch(t){this._error=I(t,this._lang,s("doc_link_invalid",this._lang))}finally{this._busy=!1}}}render(){let e=this._lang;return o`
      ...
      <div class="doc-header">
        ...
        ${this.canWrite?o`
              <div class="doc-actions">
                ...
                >
                  ${$t.map(t=>o`<option value=${t}>${s(`doc_cat_${t}`,e)}</option>`)}
                </select>
                <label
    ...
    `}_renderDoc(e,t){if(this._editingId===e.id)return this._renderEdit(e,t);let i=e.kind==="file",a=this._category_of(e),l=i?`${s(`doc_cat_${a}`,t)} \xB7 ${de(e.size)}`:s("doc_link_badge",t),c=this._thumbs[e.id];return o`
      <div class="doc-row">
        ${i&&c?o`<img
              class="doc-thumb"
              src=${c}
    ...
    `}_renderEdit(e,t){let i=e.kind==="file";return o`
      <div class="doc-row editing">
        ...
        ${i?o`<select
            ...
            >
              ${$t.map(a=>o`<option value=${a} ?selected=${a===this._editCategory}>${s(`doc_cat_${a}`,t)}</option>`)}
            </select>`:p}
  ...
  `,d([y({attribute:!1})],q.prototype,"hass",2),d([y({attribute:!1})],q.prototype,"entryId",2),d([y({type:Boolean})],q.prototype,"canWrite",2),d([_()],q.prototype,"_docs",2),d([_()],q.prototype,"_loaded",2),d([_()],q.prototype,"_busy",2),d([_()],q.prototype,"_error",2),d([_()],q.prototype,"_hint",2),d([_()],q.prototype,"_addingLink",2),d([_()],q.prototype,"_linkUrl",2),d([_()],q.prototype,"_linkTitle",2),d([_()],q.prototype,"_category",2),d([_()],q.prototype,"_thumbs",2),d([_()],q.prototype,"_lightboxUrl",2),d([_()],q.prototype,"_editingId",2),d([_()],q.prototype,"_editTitle",2),d([_()],q.prototype,"_editCategory",2),d([_()],q.prototype,"_dragOver",2);customElements.get("maintenance-documents-section")||customElements.define("maintenance-documents-section",q);var Ts=["manual","warranty","invoice","spare_parts","photo","other"],Ss={manual:"mdi:book-open-variant",warranty:"mdi:shield-check",invoice:"mdi:receipt-text-outline",spare_parts:"mdi:cog-outline",photo:"mdi:image-outline",other:"mdi:file-document-outline"},J=class extends S{constructor(){super(...arguments);this.canWrite=!1;this._docs=[];this._loaded=!1;this._busy=!1;this._error="";this._attachId="";this._loadedKey="";this._localeReady=!1}get _lang(){return this.hass?.language||"en"}get _refId(){return this.partId||this.taskId||""}get _linkField(){return this.partId?"part_ids":"task_ids"}updated(e){super.updated(e),this.hass&&!this._localeReady&&(this._localeReady=!0,Y(this._lang).then(()=>this.requestUpdate()));let t=`${this.entryId}|${this._refId}`;this.hass&&this.entryId&&this._refId&&this._loadedKey!==t&&(this._loadedKey=t,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:this.entryId});this._docs=e.documents||[],this._loaded=!0,this._error=""}catch(e){this._error=I(e,this._lang),this._loaded=!0}}_links(e){return e[this._linkField]||[]}_linked(){return this._docs.filter(e=>this._links(e).includes(this._refId))}_available(){return this._docs.filter(e=>!this._links(e).includes(this._refId))}async _setLinks(e,t){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,[this._linkField]:t}),await this._load()}catch(i){this._error=I(i,this._lang)}finally{this._busy=!1}}_link(){let e=this._docs.find(t=>t.id===this._attachId);e&&(this._attachId="",this._setLinks(e,[...this._links(e),this._refId]))}_unlink(e){this._setLinks(e,this._links(e).filter(t=>t!==this._refId))}_isPdf(e){return e.mime==="application/pdf"||(e.filename||"").toLowerCase().endsWith(".pdf")}_pageFor(e){return this._isPdf(e)&&this.taskId?e.task_pages?.[this.taskId]:void 0}async _open(e){if(e.kind==="weblink"){window.open(e.url,"_blank","noopener");return}let t=this._pageFor(e),i=t?`#page=${t}`:"",a=window.open("about:blank","_blank");try{let l=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300});a&&(a.location.href=new URL(l.path+i,window.location.origin).href)}catch(l){a&&a.close(),this._error=I(l,this._lang)}}async _setPage(e,t){if(this.taskId){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,task_pages:{[this.taskId]:t}}),await this._load()}catch(i){this._error=I(i,this._lang)}finally{this._busy=!1}}}async _download(e){try{let t=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:30});yt(t.path,e.filename||e.title||"document")}catch(t){this._error=I(t,this._lang)}}render(){if(!this._loaded||this._docs.length===0)return p;let e=this._lang,t=this._linked(),i=this._available();return o`
    ...
    `}_renderRow(e,t){let i=e.kind==="file",a=this._isPdf(e),l=this._pageFor(e),c=(e.tags||[]).find(u=>Ts.includes(u))||"other",h=i?de(e.size):s("doc_link_badge",t);return o`
      <div class="tdoc-row">
...
`).map(h=>h.trim()).filter(Boolean).slice(0,100);e.checklist=c.length?c:null}if(this.completionActionsEnabled){let c=this._actionService.trim();if(c&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(c)){let m={service:c},v=this._actionTargetEntity.trim();v&&(m.target={entity_id:v});let b=this._buildActionData();Object.keys(b).length>0&&(m.data=b),e.on_complete_action=m}else e.on_complete_action=null;let h={};this._qcNotes.trim()&&(h.notes=this._qcNotes.trim());let u=parseFloat(this._qcCost);!isNaN(u)&&u>=0&&(h.cost=u);let g=parseInt(this._qcDuration,10);!isNaN(g)&&g>=0&&(h.duration=g),this._qcFeedback&&(h.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(h).length?h:null}let i=await this.hass.connection.sendMessagePromise(e),a=this._taskId||i?.task_id,l=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(a&&this._scheduleType==="sensor_based"&&l)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:a,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=I(e,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return p;let e=this._lang,t=this._triggerType==="compound";return o`
    ...
    `}_patchCondition(e,t){this._compoundConditions=this._compoundConditions.map((i,a)=>a===e?{...i,...t}:i)}_addCondition(){this._compoundConditions=[...this._compoundConditions,Rs()]}_removeCondition(e){this._compoundConditions=this._compoundConditions.filter((t,i)=>i!==e)}_renderCompoundEditor(){let e=this._lang;return o`
      <div class="select-row">
    ...
    `}_renderCondition(e,t){let i=this._lang,a=t+1;return o`
      <div class="compound-condition">
        ...
        <div class="select-row">
          ...
          >
            ${wi.map(l=>o`<option value=${l} ?selected=${l===e.type}>${s(l,i)}</option>`)}
          </select>
        </div>
    ...
    `}_renderConditionTypeFields(e,t){let i=this._lang;return e.type==="threshold"?o`
      ...
      ${this._endsMode==="until"?o`
        ...
        ></ms-textfield>`:p}
    `}_renderCalendarFields(){let e=this._lang,t=zs(e);if(this._scheduleType==="weekdays")return o`
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${js.map(i=>o`<option value=${i} ?selected=${i===this._type}>${s(i,e)}</option>`)}
            </select>
          </div>
          ...
          <div class="select-row">
            ...
            >
              ${Cs.map(i=>o`<option value=${i} ?selected=${i===this._priority}>${s("priority_"+i,e)}</option>`)}
            </select>
          </div>
          <div class="field">
...
<div class="qr-row">
  <div class="qr-col">
    <img src="${ki(this._viewResult.svg_data_uri)}" alt="QR Info" />
    <div class="qr-label">${g}</div>
  </div>
  ${u?`<div class="qr-col">
    <img src="${ki(this._completeResult.svg_data_uri)}" alt="QR Complete" />
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `,d([y({attribute:!1})],oe.prototype,"hass",2),d([_()],oe.prototype,"_open",2),d([_()],oe.prototype,"_loading",2),d([_()],oe.prototype,"_adopting",2),d([_()],oe.prototype,"_error",2),d([_()],oe.prototype,"_sensors",2),d([_()],oe.prototype,"_selected",2);customElements.get("maintenance-adopt-problem-sensors-dialog")||customElements.define("maintenance-adopt-problem-sensors-dialog",oe);var Ei=5;function Je(n){let r=n.getFullYear(),e=String(n.getMonth()+1).padStart(2,"0"),t=String(n.getDate()).padStart(2,"0");return`${r}-${e}-${t}`}function Os(n,r){let e=[];for(let t=0;t<r;t++){let i=new Date(n);i.setDate(i.getDate()+t),i.setHours(0,0,0,0),e.push(Je(i))}return e}function wt(n,r){let[e,t,i]=n.split("-").map(Number),a=new Date(e,t-1,i);return a.setDate(a.getDate()+r),Je(a)}function qs(n){if(!n||n.length===0)return null;let r=n.map(e=>e.cost).filter(e=>typeof e=="number");return r.length===0?null:r.reduce((e,t)=>e+t,0)/r.length}function Ns(n){let{windowStart:r,windowEnd:e,task:t,entryId:i,objectName:a}=n,l=[],c=(m,v)=>({date:m,entry_id:i,task_id:t.id,task_name:t.name,object_name:a,status:v&&(t.status==="overdue"||t.status==="triggered")?"ok":t.status,days_until_due:v?null:t.days_until_due??null,projected:v,schedule_type:t.schedule_type,interval_days:t.interval_days??null,interval_unit:t.interval_unit??null,responsible_user_id:t.responsible_user_id??null,avg_cost:qs(t.history),adaptive_enabled:!!t.adaptive_config?.enabled,prediction_confidence:t.threshold_prediction_confidence??null}),h=Math.max(1,Math.round(Bt(t.interval_days,t.interval_unit)));if(t.status==="overdue"||t.status==="triggered"){if(l.push(c(r,!1)),t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let m=wt(r,h),v=1;for(;m<=e&&v<Ei;)l.push(c(m,!0)),v++,m=wt(m,h)}return l}let u=t.next_due;if(typeof u!="string"||!u)return l;let g=u.slice(0,10);if(g>=r&&g<=e)l.push(c(g,!1));else if(g>e)return l;if(t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let m=wt(g,h),v=l.length;for(;m<=e&&v<Ei;)m>=r&&(l.push(c(m,!0)),v++),m=wt(m,h)}return l}var Ti={overdue:0,triggered:1,due_soon:2,ok:3};function Si(n,r,e,t=null){let i=Os(r,e),a=i[0],l=i[i.length-1],c=[];for(let u of n){let g=u.object?.name||"",m=u.entry_id,v=u.tasks||[];for(let b of v){if(t&&b.responsible_user_id!==t||b.enabled===!1)continue;let $=Ns({windowStart:a,windowEnd:l,task:b,entryId:m,objectName:g});c.push(...$)}}let h=new Map;for(let u of i)h.set(u,[]);for(let u of c){let g=h.get(u.date);g&&g.push(u)}for(let[,u]of h)u.sort((g,m)=>{let v=Ti[g.status]??99,b=Ti[m.status]??99;if(v!==b)return v-b;if(g.projected!==m.projected)return g.projected?1:-1;let $=g.object_name.localeCompare(m.object_name);return $!==0?$:g.task_name.localeCompare(m.task_name)});return i.map(u=>({date:u,events:h.get(u)??[]}))}var Fs={completed:"ok",reset:"ok",skipped:"due_soon",triggered:"triggered",trigger_replaced:"triggered",trigger_removed:"ok"};function Us(n,r){let e=[];for(let t=r-1;t>=0;t--){let i=new Date(n);i.setDate(i.getDate()-t),i.setHours(0,0,0,0),e.push(Je(i))}return e}function Ai(n,r,e,t=null){let i=Us(r,e),a=i[0],l=i[i.length-1],c=new Map;for(let u of i)c.set(u,[]);for(let u of n){let g=u.object?.name||"",m=u.entry_id,v=u.tasks||[];for(let b of v){if(t&&b.responsible_user_id!==t)continue;let $=b.history||[];for(let f of $){if(typeof f?.timestamp!="string")continue;let j=f.timestamp.slice(0,10);if(j<a||j>l)continue;let C=c.get(j);if(!C)continue;let L=f.type??"completed";C.push({date:j,entry_id:m,task_id:b.id,task_name:b.name,object_name:g,status:Fs[L]??"ok",days_until_due:null,projected:!1,schedule_type:b.schedule_type,interval_days:b.interval_days??null,responsible_user_id:b.responsible_user_id??null,avg_cost:typeof f.cost=="number"?f.cost:null,adaptive_enabled:!!b.adaptive_config?.enabled,prediction_confidence:null,history_timestamp:f.timestamp,history_type:L,history_cost:typeof f.cost=="number"?f.cost:null,history_notes:typeof f.notes=="string"?f.notes:null,history_duration:typeof f.duration=="number"?f.duration:null})}}}let h={completed:0,reset:1,skipped:2,triggered:3,trigger_replaced:4};for(let[,u]of c)u.sort((g,m)=>{let v=h[g.history_type??""]??99,b=h[m.history_type??""]??99;if(v!==b)return v-b;let $=g.object_name.localeCompare(m.object_name);return $!==0?$:g.task_name.localeCompare(m.task_name)});return i.map(u=>({date:u,events:c.get(u)??[]}))}var ji=A`
  .cal-controls {
    display: flex;
    gap: 12px;
    align-items: center;
...
`;var re=class extends S{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter)}getCardSize(){return 6}get _lang(){return this.hass?.language||"en"}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){super.updated(e),e.has("hass")&&ut(this.hass?.locale);let t=this.hass?.language;if(t&&!ht(t)&&Y(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{let e=await this.hass.connection.subscribeMessage(t=>{let i=t;this._objects=i.objects},{type:"maintenance_supporter/subscribe"});if(!this.isConnected){e();return}this._unsub=e}catch{}}_onEventClick(e){if(e.history_timestamp){this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}));return}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}render(){if(!this.hass)return p;let e=this._lang,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.title,l=null;this._userFilter&&(l=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let c=new Date;c.setHours(0,0,0,0);let h=this._pastDays>0,u=h?Ai(this._objects,c,this._pastDays,l):Si(this._objects,c,this._windowDays,l),g=Je(c),m=this._windowDays===365||h,v=m?u.filter(f=>f.events.length>0):u,b=f=>{let j=`cal-status-${f.status}`,C=f.projected?"cal-event-projected":"",L=f.status==="overdue"&&f.days_until_due!=null?` (${Math.abs(f.days_until_due)}d ${s("overdue",e).toLowerCase()})`:"",R=f.projected&&f.interval_days?o`<span class="cal-event-recur">${f.interval_unit&&f.interval_unit!=="days"?`${f.interval_days} ${s("unit_"+f.interval_unit,e)}`:s("cal_every_n_days",e).replace("{n}",String(f.interval_days))}</span>`:p,T=f.schedule_type==="sensor_based",X=T?o`<ha-icon class="cal-event-icon cal-source-sensor"
          ...
          </span>`:p,Z=this._stats?.budget?.currency_symbol||Se,U=f.history_type?s(f.history_type,e):s(f.status,e);return o`
      ...
      `},$=f=>{let[j,C,L]=f.date.split("-").map(Number),R=new Date(j,C-1,L),T=f.date===g,X=R.toLocaleDateString(e,{weekday:"short"}),ee=R.toLocaleDateString(e,{month:"long"});return o`
    ...
    `],d([y({attribute:!1})],re.prototype,"hass",2),d([_()],re.prototype,"_config",2),d([_()],re.prototype,"_objects",2),d([_()],re.prototype,"_stats",2),d([_()],re.prototype,"_windowDays",2),d([_()],re.prototype,"_pastDays",2),d([_()],re.prototype,"_userFilter",2),d([_()],re.prototype,"_unsub",2);var Bs=[{value:7,label:"Week (7 days)"},{value:14,label:"Fortnight (14 days)"},{value:30,label:"Month (30 days, default)"},{value:365,label:"Year (365 days, empty days collapsed)"}],ze=class extends S{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"}}setConfig(e){this._config={...e}}_valueChanged(e,t){let i={...this._config,[e]:t};e==="show_window_chips"&&t===!0&&delete i.show_window_chips,e==="show_user_filter"&&t===!0&&delete i.show_user_filter,e==="title"&&(!t||typeof t=="string"&&t.trim()==="")&&delete i.title,e==="user_filter"&&t===""&&delete i.user_filter,this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}render(){let e=this._config.window_days??30,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.user_filter??"",l=this._config.title??"";return o`
      <div class="editor">
        <div class="row">
        ...
        <div class="row">
          ...
          >
            ${Bs.map(c=>o`<option value="${c.value}" ?selected=${c.value===e}>${c.label}</option>`)}
          </select>
        </div>
  ...
  `],d([y({attribute:!1})],te.prototype,"hass",2),d([_()],te.prototype,"_open",2),d([_()],te.prototype,"_title",2),d([_()],te.prototype,"_message",2),d([_()],te.prototype,"_confirmText",2),d([_()],te.prototype,"_danger",2),d([_()],te.prototype,"_inputLabel",2),d([_()],te.prototype,"_inputType",2),d([_()],te.prototype,"_inputValue",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",te);var Ii={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},Mi=n=>(...r)=>({_$litDirective$:n,values:r}),Et=class{constructor(r){}get _$AU(){return this._$AM._$AU}_$AT(r,e,t){this._$Ct=r,this._$AM=e,this._$Ci=t}_$AS(r,e){return this.update(r,e)}update(r,e){return this.render(...e)}};var Xe=class extends Et{constructor(r){if(super(r),this.it=p,r.type!==Ii.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(r){if(r===p||r==null)return this._t=void 0,this.it=r;if(r===_e)return r;if(typeof r!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(r===this.it)return this._t;this.it=r;let e=[r];return e.raw=e,this._t={_$litType$:this.constructor.resultType,strings:e,values:[]}}};Xe.directiveName="unsafeHTML",Xe.resultType=1;var Ri=Mi(Xe);var Ws=["EUR","USD","GBP","JPY","CHF","CAD","AUD","NZD","CNY","INR","BRL","CZK","PLN","RUB","SEK","NOK","DKK","UAH"],M=class extends S{constructor(){super(...arguments);this.budget=null;this._settings=null;this._loading=!0;this._importCsv="";this._importLoading=!1;this._includeHistory=!0;this._toast="";this._testingNotification=!1;this._users=[];this._savedViews=[];this._vacEnabled=!1;this._vacStart="";this._vacEnd="";this._vacBuffer=3;this._vacExempt=new Set;this._vacIsActive=!1;this._vacWindowEnd=null;this._vacAllTasks=[];this._vacPreview=[];this._vacPreviewLoading=!1;this._vacSaving=!1;this._qrObjects=[];this._qrSelectedEntries=new Set;this._qrActions=new Set(["view"]);this._qrUrlMode="companion";this._qrBatchLoading=!1;this._qrBatchResults=[];this._qrObjectsLoaded=!1;this._exportObjects=[];this._exportSelectedEntries=new Set;this._exportObjectsLoaded=!1;this._docArchiveLoading=!1;this._loaded=!1;this._userService=null;this._sendTestNotification=async()=>{this._testingNotification=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/test_notification"}),t=e.message||(e.success?s("test_notification_success",this._lang):s("test_notification_failed",this._lang));this._showToast(t)}catch{this._showToast(s("test_notification_failed",this._lang))}finally{this._testingNotification=!1}};this._allTemplates=[];this._templateCategories={};this._templatesRequested=!1}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._loaded?(this._loaded=!0,this._userService=new ye(this.hass),this._loadSettings(),this._loadUsers()):e.has("hass")&&this.hass&&this._userService&&this._userService.updateHass(this.hass)}async _loadUsers(){if(this._userService)try{this._users=await this._userService.getUsers()}catch{this._users=[]}}async _loadSettings(){this._loading=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});this._settings=e,this._hydrateVacationFromSettings()}catch{}try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"});this._savedViews=e.views||[]}catch{}this._loading=!1}_hydrateVacationFromSettings(){let e=this._settings?.vacation;e&&(this._vacEnabled=e.enabled,this._vacStart=e.start||"",this._vacEnd=e.end||"",this._vacBuffer=e.buffer_days,this._vacExempt=new Set(e.exempt_task_ids||[]),this._vacIsActive=e.is_active,this._vacWindowEnd=e.window_end)}async _updateSetting(e,t){try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/update",settings:{[e]:t}});this._settings=i,this._showToast(s("settings_saved",this._lang)),this.dispatchEvent(new CustomEvent("settings-changed"))}catch{this._showToast(s("action_error",this._lang))}}_showToast(e){this._toast=e,setTimeout(()=>{this._toast=""},3e3)}_downloadFile(e,t,i){ft(e,t,i)}render(){let e=this._lang;return this._loading||!this._settings?o`<div class="settings-loading">Loading…</div>`:o`
    ...
    `}_toggleColumn(e,t){let i=new Set(Ze(this._settings.objects_table_columns));t?i.add(e):i.delete(e);let a=Le.filter(l=>l.required||i.has(l.key)).map(l=>l.key);this._updateSetting("objects_table_columns",a)}_renderGeneral(e){let t=this._settings.general,i=t.notify_targets??[],a=this._settings.budget;return o`
      <div class="settings-section">
        ...
        <label class="setting-row">
          ...
          <select @change=${l=>this._updateSetting("budget_currency",l.target.value)}>
            ${Ws.map(l=>o`<option value=${l} ?selected=${a.currency===l}>${l}</option>`)}
          </select>
        </label>
        <label class="setting-row">
...
<body>
  ...
  `,d([y({attribute:!1})],M.prototype,"hass",2),d([y({attribute:!1})],M.prototype,"features",2),d([y({attribute:!1})],M.prototype,"budget",2),d([_()],M.prototype,"_settings",2),d([_()],M.prototype,"_loading",2),d([_()],M.prototype,"_importCsv",2),d([_()],M.prototype,"_importLoading",2),d([_()],M.prototype,"_includeHistory",2),d([_()],M.prototype,"_toast",2),d([_()],M.prototype,"_testingNotification",2),d([_()],M.prototype,"_users",2),d([_()],M.prototype,"_savedViews",2),d([_()],M.prototype,"_vacEnabled",2),d([_()],M.prototype,"_vacStart",2),d([_()],M.prototype,"_vacEnd",2),d([_()],M.prototype,"_vacBuffer",2),d([_()],M.prototype,"_vacExempt",2),d([_()],M.prototype,"_vacIsActive",2),d([_()],M.prototype,"_vacWindowEnd",2),d([_()],M.prototype,"_vacAllTasks",2),d([_()],M.prototype,"_vacPreview",2),d([_()],M.prototype,"_vacPreviewLoading",2),d([_()],M.prototype,"_vacSaving",2),d([_()],M.prototype,"_qrObjects",2),d([_()],M.prototype,"_qrSelectedEntries",2),d([_()],M.prototype,"_qrActions",2),d([_()],M.prototype,"_qrUrlMode",2),d([_()],M.prototype,"_qrBatchLoading",2),d([_()],M.prototype,"_qrBatchResults",2),d([_()],M.prototype,"_qrObjectsLoaded",2),d([_()],M.prototype,"_exportObjects",2),d([_()],M.prototype,"_exportSelectedEntries",2),d([_()],M.prototype,"_exportObjectsLoaded",2),d([_()],M.prototype,"_docArchiveLoading",2),d([_()],M.prototype,"_allTemplates",2),d([_()],M.prototype,"_templateCategories",2);customElements.define("maintenance-settings-view",M);var ie=class extends S{constructor(){super(...arguments);this.objects=[];this._summary=null;this._loaded=!1;this._busy=!1;this._error="";this._query="";this._results=[];this._expanded=!1;this._initiallyLoaded=!1;this._searchTimer=0}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._initiallyLoaded&&(this._initiallyLoaded=!0,this._load(),Y(this._lang).then(()=>this.requestUpdate()))}async _load(){this._busy=!0;try{this._summary=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/storage"}),this._error=""}catch(e){this._error=I(e,this._lang)}finally{this._loaded=!0,this._busy=!1}}_nameFor(e){return this.objects.find(i=>i.object?.id===e)?.object?.name||e.slice(0,8)}_entryFor(e){return this.objects.find(t=>t.object?.id===e)?.entry_id}_toggle(){this._expanded=!this._expanded}_openObject(e){this.dispatchEvent(new CustomEvent("open-object",{detail:{entry_id:e},bubbles:!0,composed:!0}))}_onSearch(e){this._query=e.target.value,clearTimeout(this._searchTimer),this._searchTimer=window.setTimeout(()=>{this._doSearch()},250)}async _doSearch(){let e=this._query.trim();if(!e){this._results=[];return}try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/search",query:e});this._results=t.results||[]}catch(t){this._error=I(t,this._lang),this._results=[]}}async _openResult(e){if(e.kind==="weblink"){window.open(e.url,"_blank","noopener");return}let t=window.open("about:blank","_blank");try{let i=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300});t&&(t.location.href=new URL(i.path,window.location.origin).href)}catch(i){t&&t.close(),this._error=I(i,this._lang)}}_renderResult(e,t){return o`
        ...
        <div class="card-content">
          <div class="header">
            ...
            >
              ...
              <span class="header-summary">
                ${de(e.total_bytes)}
                ${e.dedup_savings_bytes>0?o`<span class="saved">−${de(e.dedup_savings_bytes)}</span>`:p}
              </span>
            </button>
            <button
              class="icon-btn"
          ...
          ${this._expanded?o`
                <div class="body">
                  <div class="totals">
                    <div class="stat">
                      <div class="stat-value">${de(e.total_bytes)}</div>
                    ...
                    ${e.dedup_savings_bytes>0?o`<div class="stat">
                          <div class="stat-value saved">−${de(e.dedup_savings_bytes)}</div>
    ...
    `}_renderObjRow(e,t){let i=e.entry;return o`
      ...
      >
        ...
        <span class="obj-size">${de(e.bytes)}</span>
  ...
  `}function Kt(n,r,e){if(!n.trigger_config?.entity_id)return p;let t=n.trigger_config.entity_id,i=r.get(t)||[],a=[];if(i.length>=2)a=i.map(T=>({ts:T.ts,val:T.val}));else{if(!n.history)return p;for(let T of n.history)T.trigger_value!=null&&a.push({ts:new Date(T.timestamp).getTime(),val:T.trigger_value})}if(n.trigger_current_value!=null&&a.push({ts:Date.now(),val:n.trigger_current_value}),a.length<2)return p;a.sort((T,X)=>T.ts-X.ts);let l=Ys,c=Gs,h=a.map(T=>T.val),u=Math.min(...h),g=Math.max(...h),m=g-u||1;u-=m*.1,g+=m*.1;let v=a[0].ts,$=a[a.length-1].ts-v||1,f=T=>(T-v)/$*l,j=T=>2+(1-(T-u)/(g-u))*(c-4),C=a;if(C.length>Li){let T=Math.ceil(C.length/Li);C=C.filter((X,ee)=>ee%T===0||ee===C.length-1)}let L=C.map(T=>`${f(T.ts).toFixed(1)},${j(T.val).toFixed(1)}`).join(" "),R=n.trigger_active?"var(--error-color, #f44336)":"var(--primary-color)";return o`
  ...
  `}function Pi(n,r){let e=r;if(n.days_until_due==null||!n.interval_days||n.interval_days<=0)return p;let{pct:t,overflow:i}=vt(n.interval_days,n.days_until_due,n.interval_unit),a="var(--success-color, #4caf50)";return n.status==="overdue"?a="var(--error-color, #f44336)":n.status==="due_soon"&&(a="var(--warning-color, #ff9800)"),o`
  ...
  `}function et(n,r,e=4){if(!isFinite(n)||!isFinite(r))return{ticks:[],niceMin:0,niceMax:1};if(n===r){let u=Math.abs(n)*.1||1;n-=u,r+=u}let t=r-n,i=Math.pow(10,Math.floor(Math.log10(t/Math.max(1,e)))),a=i;for(let u of[1,2,5,10])if(a=i*u,t/a<=e+.5)break;let l=Math.floor(n/a)*a,c=Math.ceil(r/a)*a,h=[];for(let u=l;u<=c+a*1e-6;u+=a)h.push(Math.abs(u)<a*1e-9?0:u);return{ticks:h,niceMin:l,niceMax:c}}function he(n){let r=Math.abs(n);return r>=1e6?He((n/1e6).toFixed(r>=1e7?0:1))+"M":r>=1e4?He((n/1e3).toFixed(0))+"k":r>=1e3?He((n/1e3).toFixed(1))+"k":r>=100?n.toFixed(0):r>=10||r>=1?He(n.toFixed(1)):r===0?"0":He(n.toFixed(2))}function He(n){return n.replace(/\.0+$/,"").replace(/(\.\d*[1-9])0+$/,"$1")}function je(n,r,e){let t=n.toLocaleString(e,{maximumFractionDigits:Math.abs(n)>=100?0:1});return r?`${t} ${r}`:t}function De(n,r,e){let t=new Date(n),i=e?{month:"short",day:"numeric",year:"2-digit"}:{month:"short",day:"numeric"};return t.toLocaleDateString(r,i)}function Yt(n,r){return new Date(n).toLocaleDateString(r,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function Tt(n,r){return new Date(n).getFullYear()!==new Date(r).getFullYear()}function St(n,r,e){if(e<2||r<=n)return[n,r];let t=[];for(let i=0;i<e;i++)t.push(n+(r-n)*i/(e-1));return t}var At=210,ce=46,me=14,ve=12,zi=14,Qs=20+zi,Zs=[{days:7,key:"chart_range_7d"},{days:30,key:"chart_range_30d"},{days:90,key:"chart_range_90d"},{days:365,key:"chart_range_1y"}],V=class extends S{constructor(){super(...arguments);this.points=[];this.events=[];this.unit="";this.lang="en";this.thresholdAbove=null;this.thresholdBelow=null;this.targetValue=null;this.forceZero=!1;this.projection=null;this.rangeDays=30;this.showRange=!0;this.busy=!1;this.hideOutliers=!1;this.showOutlierToggle=!0;this._width=0;this._hover=null;this._ro=null}connectedCallback(){super.connectedCallback(),this._ro=new ResizeObserver(e=>{let t=Math.floor(e[0]?.contentRect?.width||0);t&&Math.abs(t-this._width)>2&&(this._width=t)}),this._ro.observe(this)}disconnectedCallback(){super.disconnectedCallback(),this._ro?.disconnect(),this._ro=null}_emitRange(e){e!==this.rangeDays&&this.dispatchEvent(new CustomEvent("range-change",{detail:{days:e},bubbles:!0,composed:!0}))}_toggleOutliers(){this.dispatchEvent(new CustomEvent("outlier-toggle",{detail:{hide:!this.hideOutliers},bubbles:!0,composed:!0}))}render(){let e=this._width||320,t=[...this.points].sort((a,l)=>a.ts-l.ts),i=this.lang;return o`
    ...
    `}_renderSvg(e,t){let i=this.lang,a=e-ce-me,l=At-Qs,c=l-ve,h=1/0,u=-1/0;for(let k of t)h=Math.min(h,k.min??k.val),u=Math.max(u,k.max??k.val);this.thresholdAbove!=null&&(h=Math.min(h,this.thresholdAbove),u=Math.max(u,this.thresholdAbove)),this.thresholdBelow!=null&&(h=Math.min(h,this.thresholdBelow),u=Math.max(u,this.thresholdBelow)),this.targetValue!=null&&(h=Math.min(h,this.targetValue),u=Math.max(u,this.targetValue)),this.forceZero&&(h=Math.min(h,0));let g=(u-h||1)*.06,m=this.forceZero&&h>=0?0:h-g,{ticks:v,niceMin:b,niceMax:$}=et(m,u+g,4);this.forceZero&&h>=0&&b<0&&(b=0,v=v.filter(k=>k>=0));let f=t[0].ts,j=t[t.length-1].ts,C=j-f||1,L=Tt(f,j),R=k=>ce+(k-f)/C*a,T=k=>ve+(1-(k-b)/($-b||1))*c,X=t.map(k=>`${R(k.ts).toFixed(1)},${T(k.val).toFixed(1)}`).join(" "),ee=`M${R(t[0].ts).toFixed(1)},${l} `+t.map(k=>`L${R(k.ts).toFixed(1)},${T(k.val).toFixed(1)}`).join(" ")+` L${R(t[t.length-1].ts).toFixed(1)},${l} Z`,Z="",U=t.filter(k=>k.min!=null&&k.max!=null);if(U.length>=2){let k=U.map(P=>`${R(P.ts).toFixed(1)},${T(P.max).toFixed(1)}`),w=[...U].reverse().map(P=>`${R(P.ts).toFixed(1)},${T(P.min).toFixed(1)}`);Z=`M${k[0]} `+k.slice(1).map(P=>`L${P}`).join(" ")+` L${w.join(" L")} Z`}let N=[];if(this.thresholdBelow!=null){let k=T(this.thresholdBelow);N.push({y:k,h:Math.max(0,l-k),lineY:k,label:`\u25BC ${he(this.thresholdBelow)}`,labelY:Math.min(l-4,k+13)})}if(this.thresholdAbove!=null){let k=T(this.thresholdAbove);N.push({y:ve,h:Math.max(0,k-ve),lineY:k,label:`\u25B2 ${he(this.thresholdAbove)}`,labelY:Math.max(ve+11,k-5)})}let be=t[t.length-1],$e=(this.events||[]).filter(k=>k.ts>=f&&k.ts<=j),Ce=St(f,j,Math.max(2,Math.min(5,Math.floor(a/110)+1))),K=this._hover;return o`
      <div class="svg-holder">
        ...
        >
          <defs>
            <clipPath id="plot"><rect x="${ce}" y="${ve}" width="${a}" height="${c}" /></clipPath>
            ${N.length?D`<clipPath id="danger">${N.map(k=>D`<rect x="${ce}" y="${k.y.toFixed(1)}" width="${a}" height="${k.h.toFixed(1)}" />`)}</clipPath>`:p}
          ...
          ${v.map(k=>{let w=T(k);return w<ve-1||w>l+1?p:D`
              <line x1="${ce}" y1="${w.toFixed(1)}" x2="${e-me}" y2="${w.toFixed(1)}"
                stroke="var(--divider-color)" stroke-width="1" opacity="0.6" />
              <text x="${ce-7}" y="${(w+3.5).toFixed(1)}" text-anchor="end" class="tick-label">${he(k)}</text>`})}

          ${N.map(k=>D`<rect x="${ce}" y="${k.y.toFixed(1)}" width="${a}" height="${k.h.toFixed(1)}"
          ...
          ${N.map(k=>D`
              <line x1="${ce}" y1="${k.lineY.toFixed(1)}" x2="${e-me}" y2="${k.lineY.toFixed(1)}"
                stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="6,4" />
              <text x="${e-me-4}" y="${k.labelY.toFixed(1)}" text-anchor="end" class="zone-label">${k.label}</text>`)}

          ${this.targetValue!=null?D`<line x1="${ce}" y1="${T(this.targetValue).toFixed(1)}" x2="${e-me}" y2="${T(this.targetValue).toFixed(1)}"
                stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="6,4" />
              <text x="${e-me-4}" y="${(T(this.targetValue)-5).toFixed(1)}" text-anchor="end" class="zone-label">◆ ${he(this.targetValue)} ${this.unit}</text>`:p}

          ${this.projection&&this.projection.length===2?D`<line x1="${R(this.projection[0].ts).toFixed(1)}" y1="${T(this.projection[0].val).toFixed(1)}"
                x2="${Math.min(R(this.projection[1].ts),e-me).toFixed(1)}" y2="${T(Math.max(b,Math.min($,this.projection[1].val))).toFixed(1)}"
                stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.8" />`:p}

          ${Ce.map((k,w)=>{let P=R(k),jt=w===0?"start":w===Ce.length-1?"end":"middle";return D`<text x="${P.toFixed(1)}" y="${At-5}" text-anchor="${jt}" class="tick-label">${De(k,i,L)}</text>`})}

          <line x1="${ce}" y1="${l}" x2="${e-me}" y2="${l}" stroke="var(--divider-color)" stroke-width="1" />

          ${$e.map(k=>{let w=R(k.ts),P=k.type==="completed"?"var(--success-color, #4caf50)":k.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return D`
              <line x1="${w.toFixed(1)}" y1="${ve}" x2="${w.toFixed(1)}" y2="${l}" stroke="${P}" stroke-width="1" opacity="0.14" />
              <rect x="${(w-1.5).toFixed(1)}" y="${l+3}" width="3" height="${zi-6}" rx="1.5" fill="${P}">
                <title>${Yt(k.ts,i)}</title>
              </rect>`})}

          ${K?D`
                <line x1="${K.x.toFixed(1)}" y1="${ve}" x2="${K.x.toFixed(1)}" y2="${l}"
                ...
                <circle cx="${K.x.toFixed(1)}" cy="${K.y.toFixed(1)}" r="4.5" fill="var(--primary-color)"
                  stroke="var(--card-background-color, #fff)" stroke-width="2" />`:D`<circle cx="${R(be.ts).toFixed(1)}" cy="${T(be.val).toFixed(1)}" r="4" fill="var(--primary-color)"
        ...
        ${K?o`<div
            ...
            >
              <div class="hover-date">${Yt(K.p.ts,i)}</div>
              <div class="hover-val">
                ${je(K.p.val,this.unit,i)}
                ${K.p.min!=null&&K.p.max!=null?o`<span class="hover-range">(${he(K.p.min)}–${he(K.p.max)})</span>`:p}
              </div>
            </div>`:p}
      </div>
    `}_onPointer(e,t,i,a,l){let h=e.currentTarget.getBoundingClientRect(),u=(e.clientX-h.left)/h.width*l;if(u<ce-8||u>l-me+8){this._hover=null;return}let g=t[0],m=1/0;for(let v of t){let b=Math.abs(i(v.ts)-u);b<m&&(m=b,g=v)}this._hover={x:i(g.ts),y:a(g.val),p:g}}};V.styles=A`
    ...
    <div class="trigger-card">
      <div class="trigger-header">
        ...
        <div>
          ${v?o`
            ...
            <div class="trigger-entity-id">${h.map(($,f)=>o`${f>0?", ":""}<span class="entity-link" @click=${j=>Ae(j,$)}>${$}</span>`)}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `:o`
            <div class="trigger-entity-name">${l}</div>
            <div class="trigger-entity-id">${c?o`<span class="entity-link" @click=${$=>Ae($,c)}>${c}</span>`:""}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `}
        </div>
      ...
      ${b?ea(b,t):g!=null?o`
              <div class="trigger-value-row">
                <span class="trigger-current ${n.trigger_active?"active":""}">${typeof g=="number"?je(g,"",t):g}</span>
      ...
      <div class="trigger-limits">
        ...
        ${m==="compound"?o`
          ...
          ${(e.conditions||[]).map(($,f)=>o`
            <span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${f+1}. ${s($.type||"unknown",t)}: ${$.entity_id?o`<span class="entity-link" @click=${j=>Ae(j,$.entity_id)}>${$.entity_id}</span>`:""}</span>
          `)}
        `:p}
      </div>
      ...
      ${a&&a.length>1?o`
        <div class="trigger-entity-list">
          ${a.map($=>o`
            <span class="trigger-entity-id">${$.friendly_name} (<span class="entity-link" @click=${f=>Ae(f,$.entity_id)}>${$.entity_id}</span>)</span>
          `)}
        </div>
      `:p}
  ...
  `}function Xs(n,r,e){let t=n.trigger_config,i=n.trigger_current_value;if(!t||i==null)return null;switch(t.type||"threshold"){case"counter":{let a=t.trigger_target_value;if(a==null||a<=0)return null;let l=Di(n,Oi(n,e));return{progress:Math.max(0,i-(l?.value??i)),target:a,unit:r,meter:i}}case"state_change":{let a=t.trigger_target_changes;return a==null||a<=0?null:{progress:Math.max(0,i),target:a,unit:"",meter:null}}case"runtime":{let a=t.trigger_runtime_hours;return a==null||a<=0?null:{progress:Math.max(0,i),target:a,unit:"h",meter:null}}}return null}function Di(n,r){if(n.trigger_baseline_value!=null)return{value:n.trigger_baseline_value,ts:Gt(n)};if(!r.length)return null;let e=Gt(n);if(e==null)return{value:r[0].val,ts:null};let t=r[0],i=Math.abs(r[0].ts-e);for(let a of r){let l=Math.abs(a.ts-e);l<i&&(t=a,i=l)}return{value:t.val,ts:e}}function Gt(n){let r=[...n.history].filter(e=>e.type==="completed"||e.type==="reset").sort((e,t)=>new Date(t.timestamp).getTime()-new Date(e.timestamp).getTime())[0];return r?new Date(r.timestamp).getTime():null}function ea(n,r){let e=Math.min(999,Math.round(n.progress/n.target*100)),t=e>=100?"over":e>=75?"near":"ok";return o`
    <div class="counter-progress">
      <div class="counter-progress-nums">
        <span class="counter-progress-main">${je(n.progress,"",r)}<span class="counter-progress-target"> / ${je(n.target,n.unit,r)}</span></span>
      ...
      <div class="counter-progress-caption">
        ${s("chart_since_service",r)}${n.meter!=null?o` · ${s("current",r)}: ${je(n.meter,n.unit,r)}`:p}
      </div>
    </div>
  `}function Oi(n,r){let e=n.trigger_config;if(!e)return[];let t=e.type||"threshold",i=e.entity_id||"",a=t==="runtime"?[]:r.detailStatsData.get(i)||[],l=r.isCounterEntity(e),c=[];if(a.length>=2)for(let h of a){let u={ts:h.ts,val:h.val};!l&&h.min!=null&&h.max!=null&&(u.min=h.min,u.max=h.max),c.push(u)}else for(let h of n.history)h.trigger_value!=null&&c.push({ts:new Date(h.timestamp).getTime(),val:h.trigger_value});return n.trigger_current_value!=null&&c.push({ts:Date.now(),val:n.trigger_current_value}),c.sort((h,u)=>h.ts-u.ts),c}function ta(n,r,e){let t=n.trigger_config;if(!t)return p;let i=t.type||"threshold",a=t.entity_id||"",l=Oi(n,e);i==="runtime"&&t.trigger_runtime_hours&&n.trigger_current_value!=null&&(l=[{ts:Gt(n)??l[0]?.ts??Date.now()-864e5,val:0},{ts:Date.now(),val:Math.max(0,n.trigger_current_value)}]),e.hideOutliers&&(l=Js(l));let c=l.length<2&&!!a&&e.hasStatsService&&!e.detailStatsData.has(a);if(l.length<2&&!c)return p;let h=!!a&&e.detailStatsData.has(a)&&(e.detailStatsData.get(a)?.length??0)<2,u=Date.now()-e.rangeDays*864e5,g=l.filter(f=>f.ts>=u);g.length>=2&&(l=g);let m=null,v=!1;if(i==="counter"&&t.trigger_target_value!=null&&l.length){let f=Di(n,l);if(f){if(f.ts!=null){let j=l.filter(C=>C.ts>=f.ts);j.length>=2&&(l=j)}l=l.map(j=>({...j,val:Math.max(0,j.val-f.value)}))}m=t.trigger_target_value,v=!0}else i==="state_change"&&t.trigger_target_changes?(m=t.trigger_target_changes,v=!0):i==="runtime"&&t.trigger_runtime_hours&&(m=t.trigger_runtime_hours,v=!0);let b=null;if(m==null&&n.degradation_rate!=null&&(n.degradation_trend!=="stable"||n.days_until_threshold!=null)&&n.degradation_trend!=="insufficient_data"&&l.length>=2){let f=l[l.length-1];b=[f,{ts:f.ts+30*864e5,val:f.val+n.degradation_rate*30}]}let $=n.history.filter(f=>["completed","skipped","reset"].includes(f.type)).map(f=>({ts:new Date(f.timestamp).getTime(),type:f.type}));return o`
  ...
  `}function qi(n,r,e){let t=n.degradation_trend!=null&&n.degradation_trend!=="insufficient_data",i=n.days_until_threshold!=null,a=n.environmental_factor!=null&&n.environmental_factor!==1;if(!t&&!i&&!a)return p;let l=n.degradation_trend==="rising"?"M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z":n.degradation_trend==="falling"?"M16,18L18.29,15.71L13.41,10.83L9.41,14.83L2,7.41L3.41,6L9.41,12L13.41,8L19.71,14.29L22,12V18H16Z":"M22,12L18,8V11H3V13H18V16L22,12Z";return o`
      ...
      <div class="prediction-grid">
        ...
        ${a&&e.environmental?o`
          <div class="prediction-item">
            ...
            ${n.environmental_entity?o`<span class="prediction-entity entity-link" @click=${c=>Ae(c,n.environmental_entity)}>${n.environmental_entity}</span>`:p}
          </div>
        `:p}
      </div>
    </div>
  `}function Ni(n,r){let e=n.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return p;let a=n.interval_days??0,l=n.suggested_interval??a;return o`
  ...
  `}function ra(n,r,e){let t=n.confidence_interval_low,i=n.confidence_interval_high,a=r.suggested_interval??r.interval_days??0,l=r.interval_days??0,c=Math.max(0,t-5),u=i+5-c,g=(t-c)/u*100,m=(i-t)/u*100,v=(a-c)/u*100,b=l>0?(l-c)/u*100:-1;return o`
  ...
  `}function Fi(n,r,e,t){let i=Math.max(n||1,r);return o`
  ...
  `}var Ui=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"];function Bi(n,r,e){if(!e.seasonal||!n.seasonal_factor||n.seasonal_factor===1)return p;let t=Ui.map(c=>s(c,r)),i=new Date().getMonth(),a=n.seasonal_factors||n.interval_analysis?.seasonal_factors||null,l=a&&a.length===12?a:t.map((c,h)=>{let u=n.seasonal_factor||1,g=Math.sin((h-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,u+g))});return o`
  ...
  `}function Vi(n,r){return na(n,r)}function na(n,r){let e=n.seasonal_factors??n.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return p;let t=n.interval_analysis?.seasonal_reason,i=new Date().getMonth(),a=300,l=100,c=8,u=l-c-4,g=Math.max(...e,1.5),m=a/12,v=m*.65,b=c+u-1/g*u;return o`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${a} ${l}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",r)}">
        ...
        ${e.map(($,f)=>{let j=$/g*u,C=f*m+(m-v)/2,L=c+u-j,R=f===i,T=$<1?"var(--success-color, #4caf50)":$>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return D`
      ...
      <div class="seasonal-labels">
        ${Ui.map(($,f)=>o`<span class="seasonal-label ${f===i?"active-month":""}">${s($,r)}</span>`)}
      </div>
    </div>
  `}var oa=200,tt=10,la=22;function Wi(n,r,e,t){let i=n.history.filter(c=>c.type==="completed"&&(c.cost!=null||c.duration!=null));if(i.length<2)return p;let a=i.some(c=>(c.cost??0)>0),l=i.some(c=>(c.duration??0)>0);return!a&&!l?p:o`
  ...
  `}function ca(n,r,e){let t=n.history.filter(w=>w.type==="completed"&&(w.cost!=null||w.duration!=null)).map(w=>({ts:new Date(w.timestamp).getTime(),cost:w.cost??0,duration:w.duration??0})).sort((w,P)=>w.ts-P.ts);if(t.length<2)return p;let i=t.some(w=>w.cost>0),a=t.some(w=>w.duration>0);if(!i&&!a)return p;let l=e!=="duration"&&i,c=e!=="cost"&&a,h=l||!c&&i,u=c||!l&&a,g=640,m=oa,v=h?44:12,b=u?44:12,$=g-v-b,f=m-la,j=f-tt,C=t[0].ts,L=t[t.length-1].ts,R=(L-C||864e5)*.05,T=C-R,X=L+R,ee=Tt(C,L),Z=w=>v+(w-T)/(X-T)*$,U=et(0,Math.max(...t.map(w=>w.cost))||1,3),N=et(0,Math.max(...t.map(w=>w.duration))||1,3),be=w=>tt+(1-w/(U.niceMax||1))*j,$e=w=>tt+(1-w/(N.niceMax||1))*j,Ce=t.length>1?Math.min(...t.slice(1).map((w,P)=>Z(w.ts)-Z(t[P].ts))):$,K=Math.max(6,Math.min(22,Ce*.55)),k=St(C,L,Math.max(2,Math.min(4,t.length)));return o`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${g} ${m}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_history",r)}">
        ${h?U.ticks.map(w=>{let P=be(w);return P<tt-1||P>f+1?p:D`
            <line x1="${v}" y1="${P.toFixed(1)}" x2="${g-b}" y2="${P.toFixed(1)}" stroke="var(--divider-color)" stroke-width="1" opacity="0.55" />
            <text x="${v-6}" y="${(P+3.5).toFixed(1)}" text-anchor="end" fill="var(--primary-color)" font-size="10.5">${he(w)}€</text>`}):p}
        ${u?N.ticks.map(w=>{let P=$e(w);return P<tt-1||P>f+1?p:D`<text x="${g-b+6}" y="${(P+3.5).toFixed(1)}" text-anchor="start" fill="var(--accent-color, #ff9800)" font-size="10.5">${he(w)}m</text>`}):p}

        ${h?t.filter(w=>w.cost>0).map(w=>D`
          <rect x="${(Z(w.ts)-K/2).toFixed(1)}" y="${be(w.cost).toFixed(1)}" width="${K.toFixed(1)}" height="${(f-be(w.cost)).toFixed(1)}"
            fill="var(--primary-color)" opacity="0.6" rx="2">
            <title>${De(w.ts,r,!0)}: ${w.cost.toLocaleString(r)}€${w.duration?` \xB7 ${w.duration}m`:""}</title>
          </rect>
        `):p}
        ${u?D`
          <polyline points="${t.map(w=>`${Z(w.ts).toFixed(1)},${$e(w.duration).toFixed(1)}`).join(" ")}"
            fill="none" stroke="var(--accent-color, #ff9800)" stroke-width="2" stroke-linejoin="round" />
          ${t.map(w=>D`
            <circle cx="${Z(w.ts).toFixed(1)}" cy="${$e(w.duration).toFixed(1)}" r="3.5" fill="var(--accent-color, #ff9800)">
              <title>${De(w.ts,r,!0)}: ${w.duration}m${w.cost?` \xB7 ${w.cost.toLocaleString(r)}\u20AC`:""}</title>
            </circle>
          `)}
        `:p}

        <line x1="${v}" y1="${f}" x2="${g-b}" y2="${f}" stroke="var(--divider-color)" stroke-width="1" />
        ${k.map((w,P)=>{let jt=P===0?"start":P===k.length-1?"end":"middle";return D`<text x="${Z(w).toFixed(1)}" y="${m-6}" text-anchor="${jt}" fill="var(--secondary-text-color)" font-size="10">${De(w,r,ee)}</text>`})}
      </svg>
    </div>
    <div class="chart-legend">
  ...
  `,d([y({attribute:!1})],xe.prototype,"hass",2),d([y()],xe.prototype,"docId",2),d([_()],xe.prototype,"_url",2),d([_()],xe.prototype,"_failed",2);customElements.get("maintenance-history-photo")||customElements.define("maintenance-history-photo",xe);var da=["completed","skipped","missed","reset","triggered"];function Ki(n,r){let e=r.lang;return o`
    <div class="history-filters-new">
      <div class="filter-chips">
        ${da.map(t=>{let i=n.history.filter(a=>a.type===t).length;return i===0?p:o`
  ...
  `}function Yi(n,r){let e=r.lang,t=r.filter?n.history.filter(i=>i.type===r.filter):n.history;if(r.search){let i=r.search.toLowerCase();t=t.filter(a=>a.notes?.toLowerCase().includes(i))}return t.length===0?o`<p class="empty">${s("no_history",e)}</p>`:o`
    <div class="history-timeline">
      ${[...t].reverse().map(i=>pa(i,r))}
    </div>
  `}function pa(n,r){let e=r.lang,t=["completed","reset","skipped"].includes(n.type);return o`
    <div class="history-entry">
      <div class="history-icon ${n.type}">
        <ha-icon .icon=${Ye[n.type]||"mdi:circle"}></ha-icon>
      </div>
      <div class="history-content">
        ...
        <div class="history-date">${_t(n.timestamp,e)}</div>
  ...
  `}function Qt(n,r){if(!n.responsible_user_id)return p;let e=r(n.responsible_user_id);return e?o`
    <span class="user-badge">
      <ha-icon icon="mdi:account"></ha-icon>
      ${e}
    </span>
  `:p}function ha(n,r){let e=r.lang,t=r.isOperator,i=n.archived?"archived":n.is_done?"done":n.status==="due_soon"?"warning":n.status||"ok",a=n.archived?s("archived",e):n.is_done?s("completed",e):s(n.status||"ok",e);return o`
  ...
  `}function ua(n){let r=n.lang;return o`
    <div class="tab-bar">
  ...
  `}function Gi(n,r,e,t){let i=t.collapsedSections.has(n);return o`
  ...
  `}function _a(n,r){if(!r.features.checklists)return p;let e=n.checklist||[];if(e.length===0)return p;let t=r.lang;return o`
  ...
  `}function ga(n,r){let e=ne(n.documentation_url)?n.documentation_url:null,t=ne(r.objectDocUrl)?r.objectDocUrl:null;if(!n.notes&&!e&&!t)return p;let i=r.lang;return o`
  ...
  `}function ma(n,r){let e=r.lang,t=n.times_performed>0?n.total_cost/n.times_performed:0,i=n.days_until_due!==null&&n.days_until_due!==void 0?n.days_until_due<0?"overdue":n.days_until_due<=n.warning_days?"warning":"":"";return o`
    <div class="kpi-bar">
      <div class="kpi-card">
      ...
      <div class="kpi-card">
        ...
        <div class="kpi-value">${Qe(n,e)}</div>
  ...
  `}function va(n,r){let e=r.lang;if(!r.features.adaptive||!n.suggested_interval||n.suggested_interval===n.interval_days)return p;if(r.suggestionDismissed)return p;let t=n.suggested_interval;return o`
    <div class="recommendation-card">
      <h4>${s("suggested_interval",e)}</h4>
      ${Fi(n.interval_days,t,n.interval_confidence||"medium",e)}
  ...
  `}function ba(n,r){let e=r.lang,t=n.history.slice(-3).reverse();if(t.length===0)return p;let i=a=>{switch(a){case"completed":return"\u2713";case"triggered":return"\u2297";case"skipped":return"\u21B7";case"reset":return"\u21BA";default:return"\xB7"}};return o`
    <div class="recent-activities">
      <h3>${s("recent_activities",e)}</h3>
      ${t.map(a=>o`
        <div class="activity-item">
          <span class="activity-icon">${i(a.type)}</span>
          <span class="activity-date">${_t(a.timestamp,e)}</span>
  ...
  `}function fa(n,r){let e=r.lang,t=r.features.adaptive&&n.suggested_interval&&n.suggested_interval!==n.interval_days,i=r.features.seasonal&&n.seasonal_factor&&n.seasonal_factor!==1,a=t||i,l=r.features.adaptive&&n.interval_analysis?.weibull_beta!=null&&n.interval_analysis?.weibull_eta!=null,c=r.features.seasonal&&(n.seasonal_factors?.length===12||n.interval_analysis?.seasonal_factors?.length===12);return o`
    <div class="tab-content overview-tab">
      ...
      <div class="two-column-layout ${a?"":"single-column"}">
        ...
        <div class="right-column">
          ${Wi(n,e,r.costDurationToggle,h=>r.setCostDurationToggle(h))}
        </div>
      </div>
      ${l?Gi("weibull","weibull_reliability_curve",Ni(n,e),r):p}
      ${c?Gi("seasonal","seasonal_chart_title",o`
            ${Vi(n,e)}
  ...
  `}function ya(n,r){return o`
    <div class="tab-content history-tab">
      ${Ki(n,r.history)}
      ${Yi(n,r.history)}
    </div>
  `}function xa(n,r){switch(r.activeTab){case"overview":return fa(n,r);case"history":return ya(n,r);default:return p}}function Qi(n,r){return o`
  ...
  `}var it=class extends S{createRenderRoot(){return this}render(){return!this.task||!this.ctx?p:o`${Qi(this.task,this.ctx)}`}};d([y({attribute:!1})],it.prototype,"task",2),d([y({attribute:!1})],it.prototype,"ctx",2);customElements.get("maintenance-task-detail-view")||customElements.define("maintenance-task-detail-view",it);function Zi(n){if(n.total<=0)return{start:0,end:0,padTop:0,padBottom:0};let r=n.overscan??12,e=Math.max(1,n.step??6),t=Math.max(1,n.rowHeight),i=Math.floor((n.scrollTop-n.listTop)/t),a=Math.ceil(n.viewportHeight/t)+1,l=Math.max(0,i-r);l=Math.floor(l/e)*e;let c=Math.min(n.total,Math.max(i,0)+a+r);return c=Math.min(n.total,Math.ceil(c/e)*e),l>=c&&(l=Math.min(l,Math.max(0,n.total-1)),c=Math.min(n.total,l+Math.max(a,1))),{start:l,end:c,padTop:l*t,padBottom:(n.total-c)*t}}var E=class extends S{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._filterLabel=null;this._savedViews=[];this._activeViewId="";this._unsub=null;this._chartRangeDays=(()=>{try{let e=parseInt(localStorage.getItem(B.chartRange)||"",10);return[7,30,90,365].includes(e)?e:30}catch{return 30}})();this._hideOutliers=(()=>{try{return localStorage.getItem(B.chartHideOutliers)==="1"}catch{return!1}})();this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._adminPanelUserIds=[];this._operatorWriteEnabled=!1;this._defaultWarningDays=7;this._actionLoading=!1;this._moreMenuOpen=!1;this._toastMessage="";this._toastUndo=null;this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab=(()=>{try{let e=localStorage.getItem(B.overviewTab);return e==="today"||e==="calendar"?e:"dashboard"}catch{return"dashboard"}})();this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._objectSortMode="alphabetical";this._groupByMode="none";this._objectViewMode="cards";this._objectsTableColumns=bt;this._showArchived=!1;this._bulkMode=!1;this._bulkSelected=new Set;this._virtStart=0;this._virtEnd=0;this._virtRowHeight=53;this._virtTotalRows=0;this._virtScrollAttached=!1;this._virtRaf=0;this._collapsedSections=(()=>{try{return new Set(JSON.parse(localStorage.getItem(B.collapsedSections)||"[]"))}catch{return new Set}})();this._paletteOpen=!1;this._paletteQuery="";this._paletteActive=0;this._templateGalleryOpen=!1;this._templates=[];this._templateCategories={};this._templateBusy=!1;this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._onVirtualScroll=()=>{this._virtRaf||(this._virtRaf=requestAnimationFrame(()=>{this._virtRaf=0,this._updateVirtualWindow()}))};this._deepLinkHandled=!1;this._paletteKeydown=e=>{if(e.key==="/"&&!e.ctrlKey&&!e.metaKey&&!e.altKey&&!this._paletteOpen){let i=e.composedPath()[0];if(i instanceof HTMLElement&&(i.tagName==="INPUT"||i.tagName==="TEXTAREA"||i.tagName==="SELECT"||i.isContentEditable))return;e.preventDefault(),this._openPalette();return}if(!this._paletteOpen)return;let t=this._paletteResults;if(e.key==="Escape")e.preventDefault(),this._closePalette();else if(e.key==="ArrowDown")e.preventDefault(),this._paletteActive=Math.min(this._paletteActive+1,t.length-1);else if(e.key==="ArrowUp")e.preventDefault(),this._paletteActive=Math.max(this._paletteActive-1,0);else if(e.key==="Enter"){e.preventDefault();let i=t[this._paletteActive];i&&this._selectPaletteResult(i)}};this._onDialogEvent=async()=>{try{await this._loadData()}catch{}};this._onCalendarLlCustom=e=>{let t=e.detail;t?.type==="maintenance-supporter:open-task"&&t.entry_id&&t.task_id&&(e.stopPropagation(),this._showTask(t.entry_id,t.task_id))};this._onHistoryEntrySaved=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"en"}get _isOperator(){let e=this.hass?.user;return e?e.is_admin?!1:!(this._operatorWriteEnabled&&this._adminPanelUserIds.includes(e.id)):!0}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler),window.addEventListener("keydown",this._paletteKeydown),window.addEventListener("resize",this._onVirtualScroll,{passive:!0});try{let e=localStorage.getItem(B.taskSort);e&&["due_date","object","type","task_name","area","assigned_user","group"].includes(e)&&(this._sortMode=e);let t=localStorage.getItem(B.objectSort);t&&["alphabetical","due_soonest","task_count"].includes(t)&&(this._objectSortMode=t);let i=localStorage.getItem(B.groupBy);i&&["none","area","group","user"].includes(i)&&(this._groupByMode=i);let a=localStorage.getItem(B.objectView);(a==="cards"||a==="table")&&(this._objectViewMode=a)}catch{}}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),window.removeEventListener("keydown",this._paletteKeydown),window.removeEventListener("resize",this._onVirtualScroll),this.shadowRoot?.querySelector(".content")?.removeEventListener("scroll",this._onVirtualScroll),this._virtScrollAttached=!1,this._virtRaf&&cancelAnimationFrame(this._virtRaf),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){super.updated(e),e.has("hass")&&ut(this.hass?.locale);let t=this.hass?.language;if(t&&!ht(t)&&Y(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new xt(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new ye(this.hass),this._userService.getUsers())}let i=this.shadowRoot?.querySelector(".content");i&&!this._virtScrollAttached&&(i.addEventListener("scroll",this._onVirtualScroll,{passive:!0}),this._virtScrollAttached=!0),this._updateVirtualWindow()}_updateVirtualWindow(){let e=this.shadowRoot?.querySelector(".content"),t=this.shadowRoot?.querySelector(".task-table.virtual");if(!e||!t)return;let i=t.querySelector(".task-row:not(.virt-sizer)");i&&i.offsetHeight>20&&(this._virtRowHeight=i.offsetHeight);let a=t.getBoundingClientRect().top-e.getBoundingClientRect().top+e.scrollTop,l=Zi({scrollTop:e.scrollTop,viewportHeight:e.clientHeight,listTop:a,rowHeight:this._virtRowHeight,total:this._virtTotalRows});(l.start!==this._virtStart||l.end!==this._virtEnd)&&(this._virtStart=l.start,this._virtEnd=l.end)}async _loadData(){let[e,t,i,a,l,c]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"}).catch(()=>null)]);if(c&&(this._savedViews=c.views||[]),e&&(this._objects=e.objects),t&&(this._stats=t),i&&(this._budget=i),a&&(this._groups=a.groups||{}),l){let h=l;this._features=h.features,this._adminPanelUserIds=h.admin_panel_user_ids||[],this._operatorWriteEnabled=h.operator_write_enabled??!1;let u=h.general?.default_warning_days;typeof u=="number"&&u>=0&&u<=365&&(this._defaultWarningDays=u),this._objectsTableColumns=Ze(h.objects_table_columns)}this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("ms_action"),i=()=>{let g=window.location.pathname+window.location.hash;history.replaceState(history.state,"",g)};if(t==="add_object"){this._deepLinkHandled=!0,i(),requestAnimationFrame(()=>{this.shadowRoot?.querySelector("maintenance-object-dialog")?.openCreate()});return}if(t==="open_vacation"||t==="open_budget"||t==="open_groups"||t==="open_settings"){this._deepLinkHandled=!0,i(),this._overviewTab="settings",requestAnimationFrame(()=>{let g=this.shadowRoot?.querySelector("maintenance-settings-view"),m=t.replace("open_","");g?.scrollToSection?.(m)});return}let a=e.get("entry_id");if(!a)return;this._deepLinkHandled=!0;let l=e.get("task_id"),c=e.get("action"),h=window.location.pathname+window.location.hash;history.replaceState(history.state,"",h);let u=this._getObject(a);if(!u){this._showOverview();return}if(l){let g=u.tasks.find(m=>m.id===l);if(!g){this._showObject(a);return}this._showTask(a,l),c==="complete"?requestAnimationFrame(()=>{this._openCompleteDialog(a,l,g.name,this._features.checklists?g.checklist:void 0,this._features.adaptive&&!!g.adaptive_config?.enabled)}):c==="quick_complete"&&requestAnimationFrame(()=>{this._handleQuickComplete(a,l,g)})}else this._showObject(a)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let i=await this._statsService.getDetailStats(e,t,this._chartRangeDays),a=new Map(this._detailStatsData);a.set(e,i),this._detailStatsData=a}_setChartRange(e){if(e===this._chartRangeDays)return;this._chartRangeDays=e;try{localStorage.setItem(B.chartRange,String(e))}catch{}let t=this._selectedEntryId&&this._selectedTaskId?this._getTask(this._selectedEntryId,this._selectedTaskId):null,i=t?.trigger_config?.entity_id;if(i){let a=new Map(this._detailStatsData);a.delete(i),this._detailStatsData=a,this._fetchDetailStats(i,this._isCounterEntity(t.trigger_config))}}_setHideOutliers(e){if(e!==this._hideOutliers){this._hideOutliers=e;try{localStorage.setItem(B.chartHideOutliers,e?"1":"0")}catch{}}}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let i of this._objects)for(let a of i.tasks){let l=a.trigger_config?.entity_id;l&&e.push({entityId:l,isCounter:this._isCounterEntity(a.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{let e=await this.hass.connection.subscribeMessage(t=>{let i=t;this._objects=i.objects},{type:"maintenance_supporter/subscribe"});if(!this.isConnected){e();return}this._unsub=e}catch{}}get _taskRows(){let e=[];for(let m of this._objects)for(let v of m.tasks){if(!this._showArchived&&v.archived||this._filterStatus&&v.status!==this._filterStatus)continue;if(this._filterUser){let $=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(v.responsible_user_id!==$)continue}if(this._filterLabel&&!(v.labels||[]).includes(this._filterLabel))continue;let b=[];for(let $ of Object.values(this._groups))$.task_refs?.some(f=>f.entry_id===m.entry_id&&f.task_id===v.id)&&b.push($.name);e.push({entry_id:m.entry_id,task_id:v.id,object_name:m.object.name,task_name:v.name,type:v.type,schedule_type:v.schedule_type,status:v.status,days_until_due:v.days_until_due??null,next_due:v.next_due??null,trigger_active:v.trigger_active,trigger_current_value:v.trigger_current_value??null,trigger_current_delta:v.trigger_current_delta??null,trigger_config:v.trigger_config??null,trigger_entity_info:v.trigger_entity_info??null,times_performed:v.times_performed,total_cost:v.total_cost,interval_days:v.interval_days??null,interval_unit:v.interval_unit??null,interval_anchor:v.interval_anchor??null,is_done:v.is_done??!1,archived:v.archived??!1,history:v.history||[],enabled:v.enabled,nfc_tag_id:v.nfc_tag_id??null,priority:v.priority??"normal",labels:v.labels??[],area_id:m.object.area_id??null,responsible_user_id:v.responsible_user_id??null,group_names:b})}let t={overdue:0,triggered:1,due_soon:2,ok:3},i=(m,v)=>(t[m.status]??9)-(t[v.status]??9),a=(m,v)=>(m.days_until_due??99999)-(v.days_until_due??99999),l=(m,v)=>i(m,v)||a(m,v),c=m=>m.area_id&&this.hass?.areas?.[m.area_id]?.name||"",h=m=>m.responsible_user_id&&this._userService?.getUserName(m.responsible_user_id)||"",u=m=>m.group_names[0]||"",g={due_date:l,object:(m,v)=>m.object_name.localeCompare(v.object_name)||l(m,v),type:(m,v)=>m.type.localeCompare(v.type)||l(m,v),task_name:(m,v)=>m.task_name.localeCompare(v.task_name),area:(m,v)=>{let b=c(m),$=c(v);return!b&&$?1:b&&!$?-1:b.localeCompare($)||l(m,v)},assigned_user:(m,v)=>{let b=h(m),$=h(v);return!b&&$?1:b&&!$?-1:b.localeCompare($)||l(m,v)},group:(m,v)=>{let b=u(m),$=u(v);return!b&&$?1:b&&!$?-1:b.localeCompare($)||l(m,v)}};return e.sort(g[this._sortMode]),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(a=>a.id===t)}_pushPanelState(e,t,i){let a={msp_view:e,msp_entry:t||null,msp_task:i||null};history.pushState(a,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let i=this._getTask(t.msp_entry,t.msp_task);i?.trigger_config?.entity_id&&this._fetchDetailStats(i.trigger_config.entity_id,this._isCounterEntity(i.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1,this._scrollContentToTop()}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null,this._scrollContentToTop()}_filterByStatus(e){this._filterStatus=e,this._activeViewId="",this._overviewTab!=="dashboard"&&(this._overviewTab="dashboard"),this._scrollContentToTop()}get _allLabels(){let e=new Set;for(let t of this._objects)for(let i of t.tasks)for(let a of i.labels||[])e.add(a);return[...e].sort((t,i)=>t.localeCompare(i))}get _currentFilters(){return{status:this._filterStatus,user_id:this._filterUser,label:this._filterLabel,archived:this._showArchived,sort_mode:this._sortMode,group_by:this._groupByMode}}_applyView(e){if(this._activeViewId=e,!e)return;let t=this._savedViews.find(a=>a.id===e);if(!t)return;let i=t.filters;this._filterStatus=i.status||"",this._filterUser=i.user_id||null,this._filterLabel=i.label||null,this._showArchived=!!i.archived,["due_date","object","type","task_name","area","assigned_user","group"].includes(i.sort_mode)&&(this._sortMode=i.sort_mode),["none","area","group","user"].includes(i.group_by)&&(this._groupByMode=i.group_by);try{localStorage.setItem(B.taskSort,this._sortMode),localStorage.setItem(B.groupBy,this._groupByMode)}catch{}this._overviewTab!=="dashboard"&&(this._overviewTab="dashboard")}_openSavedViewsDialog(){this.shadowRoot.querySelector("maintenance-saved-views-dialog")?.open(this._currentFilters,this._savedViews)}_onSavedViewsChanged(e){this._savedViews=e.detail.views||[],this._activeViewId&&!this._savedViews.some(t=>t.id===this._activeViewId)&&(this._activeViewId="")}_scrollContentToTop(){requestAnimationFrame(()=>{let e=this.shadowRoot?.querySelector(".content");e&&e.scrollTo({top:0,behavior:"smooth"})})}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null,this._scrollContentToTop()}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null,this._scrollContentToTop();let i=this._getTask(e,t);if(i?.trigger_config?.entity_id){let a=i.trigger_config.entity_id,l=this._isCounterEntity(i.trigger_config);this._fetchDetailStats(a,l)}}_showToast(e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastUndo=null,this._toastMessage=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}_showUndoToast(e,t){this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage=e,this._toastUndo=t,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastUndo=null,this._toastTimer=null},7e3)}_runToastUndo(){let e=this._toastUndo;this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage="",this._toastUndo=null,this._toastTimer=null,e?.()}_openPalette(){this._paletteQuery="",this._paletteActive=0,this._paletteOpen=!0,this.updateComplete.then(()=>{this.shadowRoot?.querySelector(".palette-input")?.focus()})}_closePalette(){this._paletteOpen=!1,this._paletteQuery=""}get _paletteResults(){let e=this._paletteQuery.trim().toLowerCase(),t=[];for(let i of this._objects){let a=i.object.name||"";(!e||a.toLowerCase().includes(e))&&t.push({kind:"object",entryId:i.entry_id,label:a,sub:s("object",this._lang)});for(let l of i.tasks){if(l.archived)continue;let c=l.name||"",h=(l.labels||[]).some(u=>u.toLowerCase().includes(e));if(!e||c.toLowerCase().includes(e)||a.toLowerCase().includes(e)||h){let u=(l.labels||[]).length?`  #${(l.labels||[]).join(" #")}`:"";t.push({kind:"task",entryId:i.entry_id,taskId:l.id,label:c,sub:a+u})}}if(t.length>60)break}return t.slice(0,40)}_selectPaletteResult(e){this._closePalette(),e.kind==="task"&&e.taskId?this._showTask(e.entryId,e.taskId):this._showObject(e.entryId)}_renderPalette(){if(!this._paletteOpen)return p;let e=this._lang,t=this._paletteResults;return o`
    ...
    `}_statusBadge(e,t,i){let a=this._lang,l=e?"archived":t?"done":i,c=e?"archived":t?"completed":i,h=e?s("archived",a):t?s("completed",a):s(i,a);return o`<span class="status-badge ${l}"><ha-icon icon="${Ye[c]||"mdi:circle-medium"}"></ha-icon>${h}</span>`}_setOverviewTab(e){this._overviewTab=e;try{localStorage.setItem(B.overviewTab,e)}catch{}this._scrollContentToTop()}_renderToday(){let e=this._lang,t=this._taskRows,i=u=>`${u.entry_id}:${u.task_id}`,a=t.filter(u=>u.status==="overdue"||u.trigger_active),l=new Set(a.map(i)),c=t.filter(u=>!l.has(i(u))&&u.days_until_due===0);c.forEach(u=>l.add(i(u)));let h=t.filter(u=>!l.has(i(u))&&u.days_until_due!=null&&u.days_until_due>0&&u.days_until_due<=7);return a.length+c.length+h.length===0?o`
        ...
        ${t.map(l=>o`
          <div class="today-row" @click=${()=>this._showTask(l.entry_id,l.task_id)}>
            ...
            <div class="today-main">
              ...
              <div class="today-object">${l.object_name} · ${Re(l.days_until_due,a)}</div>
            </div>
    ...
    `}_renderGroupedTasks(e,t){let i=new Map,a=s("unassigned",t);for(let h of e){let u=[];this._groupByMode==="area"?u=[(h.area_id?this.hass?.areas?.[h.area_id]?.name:null)||a]:this._groupByMode==="user"?u=[(h.responsible_user_id?this._userService?.getUserName(h.responsible_user_id):null)||a]:this._groupByMode==="group"&&(u=h.group_names.length>0?h.group_names:[a]);for(let g of u)i.has(g)||i.set(g,[]),i.get(g).push(h)}let l=[...i.entries()].sort(([h],[u])=>h===a&&u!==a?1:u===a&&h!==a?-1:h.localeCompare(u)),c=this._groupByMode==="area"?"mdi:map-marker-outline":this._groupByMode==="group"?"mdi:folder-outline":"mdi:account-outline";return o`
      ...
      `)}
    `}_warrantyLabel(e,t,i){return e.kind==="expired"?s("warranty_expired",i):e.kind==="expiring"?s("warranty_expires_in",i).replace("{days}",String(e.days??0)):s("warranty_valid_until",i).replace("{date}",W(t,i))}_renderWarrantyMeta(e,t){let i=Vt(e);return o`<p class="meta">${s("warranty",t)}:
    ...
    `}_renderOverviewRow(e){let t=this._lang,i=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,a=0,l=Ke.ok,c=!1;if(i&&e.days_until_due!==null){let v=vt(e.interval_days,e.days_until_due,e.interval_unit);a=v.pct,c=v.overflow,e.status==="overdue"?l=Ke.overdue:e.status==="due_soon"&&(l=Ke.due_soon)}let h=e.area_id?this.hass?.areas?.[e.area_id]?.name:null,u=e.responsible_user_id?this._userService?.getUserName(e.responsible_user_id):null,g=e.group_names.length>0||h||u,m=this._bulkMode&&this._bulkSelected.has(this._bulkKey(e));return o`
      <div class="task-row${e.enabled?"":" task-disabled"}${m?" bulk-selected":""}">
        ...
        <span class="due-cell" @click=${()=>this._showTask(e.entry_id,e.task_id)}>
          <span class="due-text">${Re(e.days_until_due,t)}</span>
          ${i?o`<div class="days-bar"><div class="days-bar-fill${c?" overflow":""}" style="width:${a}%;background:${l}"></div></div>`:p}
          ${e.trigger_config?Wt(e):!i&&e.trigger_active?o`<span style="color:var(--maint-triggered-color);font-weight:600">⚡</span>`:p}
          ${Kt(e,this._miniStatsData,this._lang)}
        </span>
      ...
      <div class="detail-section">
        ...
        ${e.tasks.length===0?o`<div class="empty-state-centered">
              ...
              <div class="task-row${h.enabled?"":" task-disabled"}">
                ...
                <span class="task-sub${h.responsible_user_id?"":" task-sub-empty"}">${Qt(h,u=>this._userService?.getUserName(u)??null)}</span>
                <span class="cell type">${s(h.type,i)}</span>
                <span class="due-cell" @click=${()=>this._showTask(e.entry_id,h.id)}>
                  <span class="due-text">${Re(h.days_until_due,i)}</span>
                  ${h.trigger_config?Wt(h):p}
                  ${Kt(h,this._miniStatsData,this._lang)}
                </span>
                <span class="row-actions">
