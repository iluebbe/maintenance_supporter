...
${B(l.notes)}</div>`:""}
</body></html>`}function ke(l,c=new Date){if(!l)return{kind:"none",days:null,date:null};let t=new Date(`${l}T00:00:00`);if(isNaN(t.getTime()))return{kind:"none",days:null,date:null};let e=Date.UTC(c.getFullYear(),c.getMonth(),c.getDate()),i=Date.UTC(t.getFullYear(),t.getMonth(),t.getDate()),a=Math.round((i-e)/864e5);return a<0?{kind:"expired",days:a,date:l}:a<=60?{kind:"expiring",days:a,date:l}:{kind:"valid",days:a,date:l}}var q=l=>String(l??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[c]);function xi(l,c,t,e,i,a,n,d,r,p=[],u=null){let b=[[t.object,q(c)],[t.type,q(t.typeLabel(l.type))],[t.interval,q(i(l))],[t.nextDue,l.next_due?q(e(l.next_due)):"\u2014"],[t.lastDone,l.last_performed?q(e(l.last_performed)):q(t.never)]];l.priority&&l.priority!=="normal"&&b.push([t.priority,q(l.priority)]);let _=(l.checklist||[]).map(f=>`<li><span class="box"></span>${q(f)}</li>`).join(""),v=(f,R)=>f?`<figure class="qr"><img src="${f}" alt="" /><figcaption>${q(R)}</figcaption></figure>`:"";return`<!DOCTYPE html>
...
`;var oe=class{constructor(c){this._cache=new Map;this._pending=new Map;this.historyFallbackIds=new Set;this._hass=c}updateHass(c){this._hass=c}async getDetailStats(c,t,e=30){return this._getStats(c,e<=35?"hour":"day",e,t)}async getMiniStats(c,t){return this._getStats(c,"day",14,t)}async getBatchMiniStats(c){let t=new Map,e=[];for(let r of c){let p=`${r.entityId}:day:14`,u=this._cache.get(p);u&&Date.now()-u.fetchedAt<3e5?t.set(r.entityId,u.points):e.push(r)}if(e.length===0)return t;let i=e.filter(r=>r.isCounter).map(r=>r.entityId),a=e.filter(r=>!r.isCounter).map(r=>r.entityId),n=new Date(Date.now()-336*60*60*1e3).toISOString(),d=[];return i.length>0&&d.push(this._fetchBatch(i,"day",n,["state","sum","change"],!0,t)),a.length>0&&d.push(this._fetchBatch(a,"day",n,["mean","min","max"],!1,t)),await Promise.all(d),t}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(c,t,e,i){let a=`${c}:${t}:${e}`,n=this._cache.get(a);if(n&&Date.now()-n.fetchedAt<3e5)return n.points;if(this._pending.has(a))return this._pending.get(a);let d=this._fetchAndNormalize(c,t,e,i,a);this._pending.set(a,d);try{return await d}finally{this._pending.delete(a)}}async _fetchAndNormalize(c,t,e,i,a){let n=new Date(Date.now()-e*24*60*60*1e3).toISOString(),d=i?["state","sum","change"]:["mean","min","max"];try{let p=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:n,statistic_ids:[c],period:t,types:d}))[c]||[],u=this._normalizeRows(p,i);if(u.length<2){let b=await this._fetchHistoryFallback(c,n);b.length>=2?(u=b,this.historyFallbackIds.add(c)):this.historyFallbackIds.delete(c)}else this.historyFallbackIds.delete(c);return this._cache.set(a,{entityId:c,fetchedAt:Date.now(),period:t,points:u}),u}catch(r){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${c}:`,r),[]}}async _fetchHistoryFallback(c,t){try{let i=(await this._hass.connection.sendMessagePromise({type:"history/history_during_period",start_time:t,end_time:new Date().toISOString(),entity_ids:[c],minimal_response:!0,no_attributes:!0}))?.[c]||[];if(i.length>1e3){let d=Math.ceil(i.length/500);i=i.filter((r,p)=>p%d===0||p===i.length-1)}let a=[],n=null;for(let d of i){let r=d.s??d.state;if(r==null||r==="unknown"||r==="unavailable")continue;let p;if(r==="on"||r==="open"||r==="true")p=1;else if(r==="off"||r==="closed"||r==="false")p=0;else if(p=parseFloat(r),!Number.isFinite(p))continue;let u=d.lu??d.last_updated??d.last_changed,b=typeof u=="number"?u*1e3:u!=null?Date.parse(u):NaN;Number.isFinite(b)&&(n!=null&&n!==p&&a.push({ts:b,val:n}),a.push({ts:b,val:p}),n=p)}return a.sort((d,r)=>d.ts-r.ts),a.length&&n!=null&&a.push({ts:Date.now(),val:n}),a}catch(e){return console.warn(`[maintenance-supporter] History fallback failed for ${c}:`,e),[]}}async _fetchBatch(c,t,e,i,a,n){try{let d=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:e,statistic_ids:c,period:t,types:i});for(let r of c){let p=d[r]||[],u=this._normalizeRows(p,a);n.set(r,u),this._cache.set(`${r}:${t}:14`,{entityId:r,fetchedAt:Date.now(),period:t,points:u})}}catch(d){console.warn("[maintenance-supporter] Batch statistics fetch failed:",d)}}_normalizeRows(c,t){let e=[];for(let i of c){let a=null;if(t?a=i.state??null:a=i.mean??null,a===null)continue;let n={ts:i.start,val:a};t||(i.min!=null&&(n.min=i.min),i.max!=null&&(n.max=i.max)),e.push(n)}return e.sort((i,a)=>i.ts-a.ts),e}};function vt(l,c){let t=l??0;return t<1024?`${t} B`:t<1024*1024?`${it(t/1024,c,1)} KB`:`${it(t/(1024*1024),c,1)} MB`}var Rt=["manual","warranty","invoice","spare_parts","photo","other"],re={manual:"mdi:book-open-variant",warranty:"mdi:shield-check",invoice:"mdi:receipt-text-outline",spare_parts:"mdi:cog-outline",photo:"mdi:image-outline",other:"mdi:file-document-outline"};function Tt(l){return l.title||l.filename||l.url||""}var ne=8,je=["newest","oldest","title","category"];function Te(l){return je.includes(l??"")?l:"newest"}var $e=["manual","warranty","invoice","spare_parts","photo","other"];function ki(l,c){let t=new Intl.Collator(void 0,{numeric:!0,sensitivity:"base"}),e=d=>(d.title||d.filename||d.url||"").trim(),i=(d,r)=>t.compare(e(d),e(r)),a=d=>{if(d.kind==="weblink")return $e.length+1;let r=(d.tags||[]).find(p=>$e.includes(p))||"other";return $e.indexOf(r)},n=[...l];switch(c){case"oldest":return n.sort((d,r)=>(d.added_at||"").localeCompare(r.added_at||""));case"title":return n.sort(i);case"category":return n.sort((d,r)=>a(d)-a(r)||i(d,r));default:return n.sort((d,r)=>(r.added_at||"").localeCompare(d.added_at||""))}}function le(l,c){let t=Gt(c);if(!t.length)return l;let e=[];for(let i of l){let a=Ct(t,[{text:i.title,weight:3},{text:i.filename,weight:2},{text:(i.tags||[]).join(" "),weight:2},{text:i.description,weight:2},{text:i.url,weight:1}]);a>0&&e.push({doc:i,score:a})}return e.sort((i,a)=>a.score-i.score).map(i=>i.doc)}var V=class extends H{constructor(){super(...arguments);this.canWrite=!1;this._docs=[];this._filter="";this._loaded=!1;this._busy=!1;this._error="";this._hint="";this._addingLink=!1;this._linkUrl="";this._linkTitle="";this._category="manual";this._thumbs={};this._lightboxUrl="";this._editingId="";this._editTitle="";this._editCategory="manual";this._editDescription="";this._linkDescription="";this._sort=Te(Z(D.docSort));this._dragOver=!1;this._loadedFor=null;this._localeReady=!1;this._singlePick=Ze()}_isImage(t){return t.kind==="file"&&(t.mime||"").startsWith("image/")}async _sign(t){return Xt(this.hass,t.id)}get _lang(){return U(this.hass)}updated(t){super.updated(t),this.hass&&!this._localeReady&&(this._localeReady=!0,ot(this._lang).then(()=>this.requestUpdate())),this.hass&&this.entryId&&this._loadedFor!==this.entryId&&(this._loadedFor=this.entryId,this._load())}async _load(){try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:this.entryId});this._docs=t.documents||[],this._loaded=!0,this._error="",this._thumbs={},this._loadThumbs()}catch(t){this._error=A(t,this._lang),this._loaded=!0}}async _loadThumbs(){await Promise.all(this._docs.filter(t=>this._isImage(t)).map(async t=>{try{let e=await this._sign(t);this._thumbs={...this._thumbs,[t.id]:e}}catch{}}))}_category_of(t){return(t.tags||[]).find(i=>Rt.includes(i))||"other"}_labelKeydown(t){(t.key==="Enter"||t.key===" ")&&(t.preventDefault(),t.currentTarget.querySelector("input")?.click())}_onFileInput(t){let e=t.target,i=Array.from(e.files??[]);i.length&&this._uploadFiles(i),e.value=""}_onDrop(t){if(t.preventDefault(),this._dragOver=!1,!this.canWrite||this._busy)return;let e=Array.from(t.dataTransfer?.files??[]);e.length&&this._uploadFiles(e)}_onDragOver(t){this.canWrite&&(t.preventDefault(),this._dragOver=!0)}_onDragLeave(t){let e=t.relatedTarget;(!e||!t.currentTarget.contains(e))&&(this._dragOver=!1)}async _uploadFiles(t,e){let i=e??this._category;this._busy=!0,this._error="",this._hint="";let a=0,n=0;try{for(let d of t){let r;try{r=await ei(this.hass,this.entryId,d,[i])}catch(p){let u=p instanceof Error?p.message:"";if(u!=="doc_too_large"&&u!=="doc_upload_failed")throw p;this._error=s(u,this._lang);continue}r.duplicate_in_object?n++:r.deduped&&a++}n?this._hint=s("doc_dup_in_object",this._lang):a&&(this._hint=s("doc_deduped",this._lang)),await this._load()}catch{this._error=s("doc_upload_failed",this._lang)}finally{this._busy=!1}}async _download(t){try{await Zt(this.hass,t.id,t.filename||t.title||"document")}catch(e){this._error=A(e,this._lang)}}async _preview(t){if(this._isImage(t)){this._lightboxUrl=this._thumbs[t.id]||await this._sign(t);return}try{await wt(this.hass,t.id)}catch(e){this._error=A(e,this._lang)}}_openDoc(t){t.kind==="file"?this._preview(t):at(t.url)&&window.open(t.url,"_blank","noopener")}_startEdit(t){this._editingId=t.id,this._editTitle=t.title||"",this._editCategory=this._category_of(t),this._editDescription=t.description||"",this._addingLink=!1,this._error=""}_cancelEdit(){this._editingId=""}_setSort(t){this._sort=Te(t),G(D.docSort,this._sort)}async _saveEdit(t){let e=(t.tags||[]).filter(a=>!Rt.includes(a)),i=t.kind==="file"?[this._editCategory,...e]:t.tags??[];this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:t.id,title:this._editTitle.trim()||t.filename||t.url||"",tags:i,description:this._editDescription.trim()}),this._editingId="",await this._load()}catch(a){this._error=A(a,this._lang)}finally{this._busy=!1}}async _delete(t){let e=Tt(t);if(window.confirm(s("doc_delete_confirm",this._lang).replace("{name}",e))){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/delete",doc_id:t.id}),await this._load()}catch(i){this._error=A(i,this._lang)}finally{this._busy=!1}}}async _addLink(){let t=this._linkUrl.trim();if(t){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/add_link",entry_id:this.entryId,url:t,title:this._linkTitle.trim()||null,description:this._linkDescription.trim()||null}),this._linkUrl="",this._linkTitle="",this._linkDescription="",this._addingLink=!1,await this._load()}catch(e){this._error=A(e,this._lang,s("doc_link_invalid",this._lang))}finally{this._busy=!1}}}render(){let t=this._lang;return o`
    ...
    `}_renderDoc(t,e){if(this._editingId===t.id)return this._renderEdit(t,e);let i=t.kind==="file",a=this._category_of(t),n=i?`${s(`doc_cat_${a}`,e)} \xB7 ${vt(t.size,e)}`:s("doc_link_badge",e),d=this._thumbs[t.id];return o`
      <div class="doc-row">
        ${i&&d?o`<img
              class="doc-thumb"
              src=${d}
  ...
  `],g([S({attribute:!1})],V.prototype,"hass",2),g([S({attribute:!1})],V.prototype,"entryId",2),g([S({type:Boolean})],V.prototype,"canWrite",2),g([m()],V.prototype,"_docs",2),g([m()],V.prototype,"_filter",2),g([m()],V.prototype,"_loaded",2),g([m()],V.prototype,"_busy",2),g([m()],V.prototype,"_error",2),g([m()],V.prototype,"_hint",2),g([m()],V.prototype,"_addingLink",2),g([m()],V.prototype,"_linkUrl",2),g([m()],V.prototype,"_linkTitle",2),g([m()],V.prototype,"_category",2),g([m()],V.prototype,"_thumbs",2),g([m()],V.prototype,"_lightboxUrl",2),g([m()],V.prototype,"_editingId",2),g([m()],V.prototype,"_editTitle",2),g([m()],V.prototype,"_editCategory",2),g([m()],V.prototype,"_editDescription",2),g([m()],V.prototype,"_linkDescription",2),g([m()],V.prototype,"_sort",2),g([m()],V.prototype,"_dragOver",2);customElements.get("maintenance-documents-section")||customElements.define("maintenance-documents-section",V);var nt=class extends H{constructor(){super(...arguments);this.canWrite=!1;this._docs=[];this._loaded=!1;this._busy=!1;this._error="";this._attachId="";this._filter="";this._loadedKey="";this._localeReady=!1}get _lang(){return U(this.hass)}get _refId(){return this.partId||this.taskId||""}get _linkField(){return this.partId?"part_ids":"task_ids"}updated(t){super.updated(t),this.hass&&!this._localeReady&&(this._localeReady=!0,ot(this._lang).then(()=>this.requestUpdate()));let e=`${this.entryId}|${this._refId}`;this.hass&&this.entryId&&this._refId&&this._loadedKey!==e&&(this._loadedKey=e,this._load())}async _load(){try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:this.entryId});this._docs=t.documents||[],this._loaded=!0,this._error=""}catch(t){this._error=A(t,this._lang),this._loaded=!0}}_links(t){return t[this._linkField]||[]}_linked(){return this._docs.filter(t=>this._links(t).includes(this._refId))}_available(){return this._docs.filter(t=>!this._links(t).includes(this._refId))}async _setLinks(t,e){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:t.id,[this._linkField]:e}),await this._load()}catch(i){this._error=A(i,this._lang)}finally{this._busy=!1}}_link(){let t=this._docs.find(e=>e.id===this._attachId);t&&(this._attachId="",this._setLinks(t,[...this._links(t),this._refId]))}_unlink(t){this._setLinks(t,this._links(t).filter(e=>e!==this._refId))}_isPdf(t){return t.mime==="application/pdf"||(t.filename||"").toLowerCase().endsWith(".pdf")}_pageFor(t){return this._isPdf(t)&&this.taskId?t.task_pages?.[this.taskId]:void 0}async _open(t){if(t.kind==="weblink"){at(t.url)&&window.open(t.url,"_blank","noopener");return}let e=this._pageFor(t);try{await wt(this.hass,t.id,e?`#page=${e}`:"")}catch(i){this._error=A(i,this._lang)}}async _setPage(t,e){if(this.taskId){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:t.id,task_pages:{[this.taskId]:e}}),await this._load()}catch(i){this._error=A(i,this._lang)}finally{this._busy=!1}}}async _download(t){try{await Zt(this.hass,t.id,t.filename||t.title||"document")}catch(e){this._error=A(e,this._lang)}}render(){if(!this._loaded||this._docs.length===0)return h;let t=this._lang,e=this._linked(),i=this._available();return o`
    ...
    `}_renderRow(t,e){let i=t.kind==="file",a=this._isPdf(t),n=this._pageFor(t),d=(t.tags||[]).find(p=>Rt.includes(p))||"other",r=i?vt(t.size,e):s("doc_link_badge",e);return o`
      <div class="tdoc-row">
...
`;function Ei(l){let c=window;c.customCards=c.customCards||[],c.customCards.some(t=>t.type===l.type)||c.customCards.push(l)}var dt=class extends H{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._objectFilter="";this._configuredObjects=[];this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(t){if(this._config={...t},t.past_days&&[30,90].includes(t.past_days)?this._pastDays=t.past_days:t.window_days&&[7,14,30,365].includes(t.window_days)&&(this._windowDays=t.window_days,this._pastDays=0),typeof t.user_filter=="string"&&(this._userFilter=t.user_filter),typeof t.object_filter=="string")this._objectFilter=t.object_filter,this._configuredObjects=[];else if(Array.isArray(t.object_filter)){let e=t.object_filter.filter(i=>typeof i=="string"&&i!=="");this._objectFilter=e.length===1?e[0]:"",this._configuredObjects=e.length>1?e:[]}}getCardSize(){return 6}get _lang(){return U(this.hass)}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(t){if(super.updated(t),At(this,t),t.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[t,e]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=t.objects,this._stats=e,Lt(this._stats.budget)}catch{}}async _subscribe(){try{let t=await this.hass.connection.subscribeMessage(e=>{let i=e;this._objects=i.objects},{type:"maintenance_supporter/subscribe"});if(!this.isConnected){t();return}this._unsub=t}catch{}}_onEventClick(t){if(t.history_timestamp){this._openHistoryEntry(t);return}bi(t.entry_id,t.task_id)||this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:t.entry_id,task_id:t.task_id},bubbles:!0,composed:!0}))}async _openHistoryEntry(t){try{let e=await ri(this.hass,t.entry_id,t.task_id,t.history_timestamp);if(!e||_i(e))return}catch{}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:t.entry_id,task_id:t.task_id,original_timestamp:t.history_timestamp},bubbles:!0,composed:!0}))}render(){if(!this.hass)return h;let t=this._lang,e=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.title,n=null;this._userFilter&&(n=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let d=y=>{let st=y.toLowerCase();return this._objects.find($=>$.entry_id===y||$.object.name.toLowerCase()===st)?.entry_id??null},r=new Set(this._configuredObjects.map(d).filter(y=>y!==null)),p=r.size?this._objects.filter(y=>r.has(y.entry_id)):this._objects,u=this._config.show_object_filter!==!1&&p.length>1,b=this._objectFilter?d(this._objectFilter):null,_=b&&p.some(y=>y.entry_id===b)?p.filter(y=>y.entry_id===b):p,v=new Date;v.setHours(0,0,0,0);let f=this._pastDays>0,R=f?ai(_,v,this._pastDays,n):si(_,v,this._windowDays,n),x=qt(v),M=this._windowDays===365||f,k=M?R.filter(y=>y.events.length>0):R,I=y=>{let st=`cal-status-${y.status}`,ut=y.projected?"cal-event-projected":"",$=y.status==="overdue"&&y.days_until_due!=null?` (${jt(y.days_until_due,t)})`:"",N=y.projected&&y.interval_days?o`<span class="cal-event-recur">${y.interval_unit&&y.interval_unit!=="days"?`${y.interval_days} ${s("unit_"+y.interval_unit,t)}`:s("cal_every_n_days",t).replace("{n}",String(y.interval_days))}</span>`:h,F=y.schedule_type==="sensor_based",P=F?o`<ha-icon class="cal-event-icon cal-source-sensor"
              ...
              <div class="cal-controls">
                ...
                ${u?o`
                      <select class="cal-user-filter"
                        ...
                        ${[...p].sort((y,st)=>y.object.name.localeCompare(st.object.name)).map(y=>o`<option value=${y.entry_id} ?selected=${y.entry_id===b}>${y.object.name}</option>`)}
                      </select>
                    `:h}
              </div>
            `:h}
        ...
        ${W?o`<div
            ...
            >
              ...
              <div class="hover-val">
                ...
                ${W.p.min!=null&&W.p.max!=null?o`<span class="hover-range">(${bt(W.p.min,i)}–${bt(W.p.max,i)})</span>`:h}
              </div>
            </div>`:h}
      </div>
    `}_onPointer(t,e,i,a,n){let r=t.currentTarget.getBoundingClientRect(),p=(t.clientX-r.left)/r.width*n;if(p<_t-8||p>n-kt+8){this._hover=null;return}let u=e[0],b=1/0;for(let _ of e){let v=Math.abs(i(_.ts)-p);v<b&&(b=v,u=_)}this._hover={x:i(u.ts),y:a(u.val),p:u}}};et.styles=L`
  ...
  `}var vs=200,Ut=10,fs=22;function Pi(l,c,t,e,i){let a=l.history.filter(r=>r.type==="completed"&&(r.cost!=null||r.duration!=null));if(a.length<2)return h;let n=a.some(r=>(r.cost??0)>0),d=a.some(r=>(r.duration??0)>0);return!n&&!d?h:o`
  ...
  `}function ys(l,c,t,e){let i=l.history.filter(j=>j.type==="completed"&&(j.cost!=null||j.duration!=null)).map(j=>({ts:new Date(j.timestamp).getTime(),cost:j.cost??0,duration:j.duration??0})).sort((j,z)=>j.ts-z.ts);if(i.length<2)return h;let a=i.some(j=>j.cost>0),n=i.some(j=>j.duration>0);if(!a&&!n)return h;let d=t!=="duration"&&a,r=t!=="cost"&&n,p=d||!r&&a,u=r||!d&&n,b=640,_=vs,v=p?44:12,f=u?44:12,R=b-v-f,x=_-fs,M=x-Ut,k=i[0].ts,I=i[i.length-1].ts,O=(I-k||864e5)*.05,y=k-O,st=I+O,ut=se(k,I),$=j=>v+(j-y)/(st-y)*R,N=Ft(0,Math.max(...i.map(j=>j.cost))||1,3),F=Ft(0,Math.max(...i.map(j=>j.duration))||1,3),P=j=>Ut+(1-j/(N.niceMax||1))*M,Y=j=>Ut+(1-j/(F.niceMax||1))*M,X=i.length>1?Math.min(...i.slice(1).map((j,z)=>$(j.ts)-$(i[z].ts))):R,W=Math.max(6,Math.min(22,X*.55)),T=ae(k,I,Math.max(2,Math.min(4,i.length)));return o`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${b} ${_}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_history",c)}">
        ...
        ${u?F.ticks.map(j=>{let z=Y(j);return z<Ut-1||z>x+1?h:J`<text x="${b-f+6}" y="${E(z+3.5)}" text-anchor="start" fill="var(--accent-color, #ff9800)" font-size="10.5">${bt(j,c)}m</text>`}):h}
      ...
      <div class="task-table grouped${this._bulkMode?" bulk":""}">
        ${n.map(([r,p])=>{let u=!this._collapsedGroups.has(r);return o`
            <div class="group-section" ?open=${u}>
              ...
              ${u?o`<div class="group-rows">${p.map(b=>this._renderOverviewRow(b))}</div>`:h}
            </div>
          `})}
      </div>
    `}_toggleGroup(t){let e=new Set(this._collapsedGroups);e.has(t)?e.delete(t):e.add(t),this._collapsedGroups=e}_warrantyLabel(t,e,i){return t.kind==="expired"?s("warranty_expired",i):t.kind==="expiring"?s("warranty_expires_in",i).replace("{days}",String(t.days??0)):s("warranty_valid_until",i).replace("{date}",Q(e,i))}_renderWarrantyMeta(t,e){let i=ke(t);return o`<p class="meta">${s("warranty",e)}:
      <span class="warranty-chip warranty-${i.kind}">${this._warrantyLabel(i,t,e)}</span></p>`}_renderAllObjects(){let t=this._lang,e=this._isOperator,i=this._objectViewMode==="table"&&!this.narrow,a=this._objects.filter(u=>u.object.archived).length,n=u=>{let b=1/0;for(let _ of u.tasks){let v=_.days_until_due;v!=null&&v<b&&(b=v)}return b},d=this._objects.filter(u=>this._showArchived||!u.object.archived);this._objectSortMode==="alphabetical"?d.sort((u,b)=>u.object.name.localeCompare(b.object.name)):this._objectSortMode==="task_count"?d.sort((u,b)=>b.tasks.length-u.tasks.length||u.object.name.localeCompare(b.object.name)):d.sort((u,b)=>n(u)-n(b)||u.object.name.localeCompare(b.object.name));let r=()=>{let u=new Map;for(let b of d){let _=b.object.area_id,v=_?this.hass?.areas?.[_]?.name||s("unassigned",t):s("no_area",t);u.has(v)||u.set(v,[]),u.get(v).push(b)}return new Map([...u.entries()].sort(([b],[_])=>b.localeCompare(_)))},p=u=>{let b=u.tasks.some(_=>_.status==="overdue"||_.status==="triggered");return o`
        <div class="object-card${b?" object-card-overdue":""}${this._objBulkMode?" selectable":""}${this._objBulkMode&&this._objBulkSelected.has(u.entry_id)?" bulk-selected":""}"
          ...
          ${b?o`<span class="overdue-dot" title="${s("has_overdue",t)}"></span>`:h}
