...
`;var Pt=class{constructor(o){this._cache=new Map;this._pending=new Map;this._hass=o}updateHass(o){this._hass=o}async getDetailStats(o,t,e=30){return this._getStats(o,e<=35?"hour":"day",e,t)}async getMiniStats(o,t){return this._getStats(o,"day",14,t)}async getBatchMiniStats(o){let t=new Map,e=[];for(let c of o){let p=`${c.entityId}:day:14`,u=this._cache.get(p);u&&Date.now()-u.fetchedAt<3e5?t.set(c.entityId,u.points):e.push(c)}if(e.length===0)return t;let i=e.filter(c=>c.isCounter).map(c=>c.entityId),a=e.filter(c=>!c.isCounter).map(c=>c.entityId),l=new Date(Date.now()-336*60*60*1e3).toISOString(),h=[];return i.length>0&&h.push(this._fetchBatch(i,"day",l,["state","sum","change"],!0,t)),a.length>0&&h.push(this._fetchBatch(a,"day",l,["mean","min","max"],!1,t)),await Promise.all(h),t}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(o,t,e,i){let a=`${o}:${t}:${e}`,l=this._cache.get(a);if(l&&Date.now()-l.fetchedAt<3e5)return l.points;if(this._pending.has(a))return this._pending.get(a);let h=this._fetchAndNormalize(o,t,e,i,a);this._pending.set(a,h);try{return await h}finally{this._pending.delete(a)}}async _fetchAndNormalize(o,t,e,i,a){let l=new Date(Date.now()-e*24*60*60*1e3).toISOString(),h=i?["state","sum","change"]:["mean","min","max"];try{let p=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:l,statistic_ids:[o],period:t,types:h}))[o]||[],u=this._normalizeRows(p,i);return this._cache.set(a,{entityId:o,fetchedAt:Date.now(),period:t,points:u}),u}catch(c){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${o}:`,c),[]}}async _fetchBatch(o,t,e,i,a,l){try{let h=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:e,statistic_ids:o,period:t,types:i});for(let c of o){let p=h[c]||[],u=this._normalizeRows(p,a);l.set(c,u),this._cache.set(`${c}:${t}:14`,{entityId:c,fetchedAt:Date.now(),period:t,points:u})}}catch(h){console.warn("[maintenance-supporter] Batch statistics fetch failed:",h)}}_normalizeRows(o,t){let e=[];for(let i of o){let a=null;if(t?a=i.state??null:a=i.mean??null,a===null)continue;let l={ts:i.start,val:a};t||(i.min!=null&&(l.min=i.min),i.max!=null&&(l.max=i.max)),e.push(l)}return e.sort((i,a)=>i.ts-a.ts),e}};function ot(n){let o=n??0;return o<1024?`${o} B`:o<1024*1024?`${(o/1024).toFixed(1)} KB`:`${(o/(1024*1024)).toFixed(1)} MB`}var zt=["manual","warranty","invoice","spare_parts","photo","other"],qe={manual:"mdi:book-open-variant",warranty:"mdi:shield-check",invoice:"mdi:receipt-text-outline",spare_parts:"mdi:cog-outline",photo:"mdi:image-outline",other:"mdi:file-document-outline"},B=class extends L{constructor(){super(...arguments);this.canWrite=!1;this._docs=[];this._loaded=!1;this._busy=!1;this._error="";this._hint="";this._addingLink=!1;this._linkUrl="";this._linkTitle="";this._category="manual";this._thumbs={};this._lightboxUrl="";this._editingId="";this._editTitle="";this._editCategory="manual";this._dragOver=!1;this._loadedFor=null;this._localeReady=!1}_isImage(t){return t.kind==="file"&&(t.mime||"").startsWith("image/")}async _sign(t){return(await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${t.id}`,expires:300})).path}get _lang(){return this.hass?.language||"en"}updated(t){super.updated(t),this.hass&&!this._localeReady&&(this._localeReady=!0,q(this._lang).then(()=>this.requestUpdate())),this.hass&&this.entryId&&this._loadedFor!==this.entryId&&(this._loadedFor=this.entryId,this._load())}async _load(){try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:this.entryId});this._docs=t.documents||[],this._loaded=!0,this._error="",this._thumbs={},this._loadThumbs()}catch(t){this._error=E(t,this._lang),this._loaded=!0}}async _loadThumbs(){await Promise.all(this._docs.filter(t=>this._isImage(t)).map(async t=>{try{let e=await this._sign(t);this._thumbs={...this._thumbs,[t.id]:e}}catch{}}))}_category_of(t){return(t.tags||[]).find(i=>zt.includes(i))||"other"}_labelKeydown(t){(t.key==="Enter"||t.key===" ")&&(t.preventDefault(),t.currentTarget.querySelector("input")?.click())}_onFileInput(t){let e=t.target,i=Array.from(e.files??[]);i.length&&this._uploadFiles(i),e.value=""}_onCameraInput(t){let e=t.target,i=Array.from(e.files??[]);i.length&&this._uploadFiles(i,"photo"),e.value=""}_onDrop(t){if(t.preventDefault(),this._dragOver=!1,!this.canWrite||this._busy)return;let e=Array.from(t.dataTransfer?.files??[]);e.length&&this._uploadFiles(e)}_onDragOver(t){this.canWrite&&(t.preventDefault(),this._dragOver=!0)}_onDragLeave(t){let e=t.relatedTarget;(!e||!t.currentTarget.contains(e))&&(this._dragOver=!1)}async _uploadFiles(t,e){let i=e??this._category;this._busy=!0,this._error="",this._hint="";let a=0,l=0;try{for(let h of t){let c=new FormData;c.append("entry_id",this.entryId),c.append("tags",i),c.append("file",h,h.name);let p=await fetch("/api/maintenance_supporter/document/upload",{method:"POST",headers:{Authorization:`Bearer ${this.hass.auth?.data?.access_token??""}`},body:c});if(!p.ok){this._error=p.status===413?s("doc_too_large",this._lang):s("doc_upload_failed",this._lang);continue}let u=await p.json();u.duplicate_in_object?l++:u.deduped&&a++}l?this._hint=s("doc_dup_in_object",this._lang):a&&(this._hint=s("doc_deduped",this._lang)),await this._load()}catch{this._error=s("doc_upload_failed",this._lang)}finally{this._busy=!1}}async _download(t){try{let e=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${t.id}`,expires:30});Ct(e.path,t.filename||t.title||"document")}catch(e){this._error=E(e,this._lang)}}async _preview(t){if(this._isImage(t)){this._lightboxUrl=this._thumbs[t.id]||await this._sign(t);return}let e=window.open("about:blank","_blank");try{let i=await this._sign(t);e&&(e.location.href=new URL(i,window.location.origin).href)}catch(i){e&&e.close(),this._error=E(i,this._lang)}}_openDoc(t){t.kind==="file"?this._preview(t):it(t.url)&&window.open(t.url,"_blank","noopener")}_startEdit(t){this._editingId=t.id,this._editTitle=t.title||"",this._editCategory=this._category_of(t),this._addingLink=!1,this._error=""}_cancelEdit(){this._editingId=""}async _saveEdit(t){let e=(t.tags||[]).filter(a=>!zt.includes(a)),i=t.kind==="file"?[this._editCategory,...e]:t.tags??[];this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:t.id,title:this._editTitle.trim()||t.filename||t.url||"",tags:i}),this._editingId="",await this._load()}catch(a){this._error=E(a,this._lang)}finally{this._busy=!1}}async _delete(t){let e=t.title||t.filename||t.url||"";if(window.confirm(s("doc_delete_confirm",this._lang).replace("{name}",e))){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/delete",doc_id:t.id}),await this._load()}catch(i){this._error=E(i,this._lang)}finally{this._busy=!1}}}async _addLink(){let t=this._linkUrl.trim();if(t){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/add_link",entry_id:this.entryId,url:t,title:this._linkTitle.trim()||null}),this._linkUrl="",this._linkTitle="",this._addingLink=!1,await this._load()}catch(e){this._error=E(e,this._lang,s("doc_link_invalid",this._lang))}finally{this._busy=!1}}}render(){let t=this._lang;return r`
  ...
  `,g([T({attribute:!1})],G.prototype,"hass",2),g([m()],G.prototype,"_ov",2),g([m()],G.prototype,"_loading",2),g([m()],G.prototype,"_marking",2),g([m()],G.prototype,"_error",2),g([m()],G.prototype,"_history",2),g([m()],G.prototype,"_rosterSort",2),g([m()],G.prototype,"_typeFilter",2),g([m()],G.prototype,"_recorded",2);var Gt=G;customElements.get("maintenance-battery-fleet-section")||customElements.define("maintenance-battery-fleet-section",Gt);var _e=5;function $t(n){let o=n.getFullYear(),t=String(n.getMonth()+1).padStart(2,"0"),e=String(n.getDate()).padStart(2,"0");return`${o}-${t}-${e}`}function Ze(n,o){let t=[];for(let e=0;e<o;e++){let i=new Date(n);i.setDate(i.getDate()+e),i.setHours(0,0,0,0),t.push($t(i))}return t}function It(n,o){let[t,e,i]=n.split("-").map(Number),a=new Date(t,e-1,i);return a.setDate(a.getDate()+o),$t(a)}function Qe(n){if(!n||n.length===0)return null;let o=n.map(t=>t.cost).filter(t=>typeof t=="number");return o.length===0?null:o.reduce((t,e)=>t+e,0)/o.length}function Xe(n){let{windowStart:o,windowEnd:t,task:e,entryId:i,objectName:a}=n,l=[],h=(_,v)=>({date:_,entry_id:i,task_id:e.id,task_name:e.name,object_name:a,status:v&&(e.status==="overdue"||e.status==="triggered")?"ok":e.status,days_until_due:v?null:e.days_until_due??null,projected:v,schedule_type:e.schedule_type,interval_days:e.interval_days??null,interval_unit:e.interval_unit??null,responsible_user_id:e.responsible_user_id??null,avg_cost:Qe(e.history),adaptive_enabled:!!e.adaptive_config?.enabled,prediction_confidence:e.threshold_prediction_confidence??null}),c=Math.max(1,Math.round(Yt(e.interval_days,e.interval_unit)));if(e.status==="overdue"||e.status==="triggered"){if(l.push(h(o,!1)),e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0){let _=It(o,c),v=1;for(;_<=t&&v<_e;)l.push(h(_,!0)),v++,_=It(_,c)}return l}let p=e.next_due;if(typeof p!="string"||!p)return l;let u=p.slice(0,10);if(u>=o&&u<=t)l.push(h(u,!1));else if(u>t)return l;if(e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0){let _=It(u,c),v=l.length;for(;_<=t&&v<_e;)_>=o&&(l.push(h(_,!0)),v++),_=It(_,c)}return l}var me={overdue:0,triggered:1,due_soon:2,ok:3};function ve(n,o,t,e=null){let i=Ze(o,t),a=i[0],l=i[i.length-1],h=[];for(let p of n){let u=p.object?.name||"",_=p.entry_id,v=p.tasks||[];for(let b of v){if(e&&b.responsible_user_id!==e||b.enabled===!1)continue;let x=Xe({windowStart:a,windowEnd:l,task:b,entryId:_,objectName:u});h.push(...x)}}let c=new Map;for(let p of i)c.set(p,[]);for(let p of h){let u=c.get(p.date);u&&u.push(p)}for(let[,p]of c)p.sort((u,_)=>{let v=me[u.status]??99,b=me[_.status]??99;if(v!==b)return v-b;if(u.projected!==_.projected)return u.projected?1:-1;let x=u.object_name.localeCompare(_.object_name);return x!==0?x:u.task_name.localeCompare(_.task_name)});return i.map(p=>({date:p,events:c.get(p)??[]}))}var Je={completed:"ok",reset:"ok",skipped:"due_soon",triggered:"triggered",trigger_replaced:"triggered",trigger_removed:"ok"};function ti(n,o){let t=[];for(let e=o-1;e>=0;e--){let i=new Date(n);i.setDate(i.getDate()-e),i.setHours(0,0,0,0),t.push($t(i))}return t}function be(n,o,t,e=null){let i=ti(o,t),a=i[0],l=i[i.length-1],h=new Map;for(let p of i)h.set(p,[]);for(let p of n){let u=p.object?.name||"",_=p.entry_id,v=p.tasks||[];for(let b of v){if(e&&b.responsible_user_id!==e)continue;let x=b.history||[];for(let f of x){if(typeof f?.timestamp!="string")continue;let M=f.timestamp.slice(0,10);if(M<a||M>l)continue;let S=h.get(M);if(!S)continue;let I=f.type??"completed";S.push({date:M,entry_id:_,task_id:b.id,task_name:b.name,object_name:u,status:Je[I]??"ok",days_until_due:null,projected:!1,schedule_type:b.schedule_type,interval_days:b.interval_days??null,responsible_user_id:b.responsible_user_id??null,avg_cost:typeof f.cost=="number"?f.cost:null,adaptive_enabled:!!b.adaptive_config?.enabled,prediction_confidence:null,history_timestamp:f.timestamp,history_type:I,history_cost:typeof f.cost=="number"?f.cost:null,history_notes:typeof f.notes=="string"?f.notes:null,history_duration:typeof f.duration=="number"?f.duration:null})}}}let c={completed:0,reset:1,skipped:2,triggered:3,trigger_replaced:4};for(let[,p]of h)p.sort((u,_)=>{let v=c[u.history_type??""]??99,b=c[_.history_type??""]??99;if(v!==b)return v-b;let x=u.object_name.localeCompare(_.object_name);return x!==0?x:u.task_name.localeCompare(_.task_name)});return i.map(p=>({date:p,events:h.get(p)??[]}))}var fe=D`
  .cal-controls {
    display: flex;
    gap: 12px;
    align-items: center;
...
`;var X=class extends L{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._objectFilter="";this._configuredObjects=[];this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(t){if(this._config={...t},t.past_days&&[30,90].includes(t.past_days)?this._pastDays=t.past_days:t.window_days&&[7,14,30,365].includes(t.window_days)&&(this._windowDays=t.window_days,this._pastDays=0),typeof t.user_filter=="string"&&(this._userFilter=t.user_filter),typeof t.object_filter=="string")this._objectFilter=t.object_filter,this._configuredObjects=[];else if(Array.isArray(t.object_filter)){let e=t.object_filter.filter(i=>typeof i=="string"&&i!=="");this._objectFilter=e.length===1?e[0]:"",this._configuredObjects=e.length>1?e:[]}}getCardSize(){return 6}get _lang(){return this.hass?.language||"en"}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(t){super.updated(t),t.has("hass")&&Et(this.hass?.locale);let e=this.hass?.language;if(e&&!St(e)&&q(e).then(()=>this.requestUpdate()),t.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[t,e]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=t.objects,this._stats=e}catch{}}async _subscribe(){try{let t=await this.hass.connection.subscribeMessage(e=>{let i=e;this._objects=i.objects},{type:"maintenance_supporter/subscribe"});if(!this.isConnected){t();return}this._unsub=t}catch{}}_onEventClick(t){if(t.history_timestamp){this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:t.entry_id,task_id:t.task_id,original_timestamp:t.history_timestamp},bubbles:!0,composed:!0}));return}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:t.entry_id,task_id:t.task_id},bubbles:!0,composed:!0}))}render(){if(!this.hass)return d;let t=this._lang,e=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.title,l=null;this._userFilter&&(l=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let h=y=>{let N=y.toLowerCase();return this._objects.find(H=>H.entry_id===y||H.object.name.toLowerCase()===N)?.entry_id??null},c=new Set(this._configuredObjects.map(h).filter(y=>y!==null)),p=c.size?this._objects.filter(y=>c.has(y.entry_id)):this._objects,u=this._config.show_object_filter!==!1&&p.length>1,_=this._objectFilter?h(this._objectFilter):null,v=_&&p.some(y=>y.entry_id===_)?p.filter(y=>y.entry_id===_):p,b=new Date;b.setHours(0,0,0,0);let x=this._pastDays>0,f=x?be(v,b,this._pastDays,l):ve(v,b,this._windowDays,l),M=$t(b),S=this._windowDays===365||x,I=S?f.filter(y=>y.events.length>0):f,R=y=>{let N=`cal-status-${y.status}`,W=y.projected?"cal-event-projected":"",H=y.status==="overdue"&&y.days_until_due!=null?` (${gt(y.days_until_due,t)})`:"",O=y.projected&&y.interval_days?r`<span class="cal-event-recur">${y.interval_unit&&y.interval_unit!=="days"?`${y.interval_days} ${s("unit_"+y.interval_unit,t)}`:s("cal_every_n_days",t).replace("{n}",String(y.interval_days))}</span>`:d,Z=y.schedule_type==="sensor_based",st=Z?r`<ha-icon class="cal-event-icon cal-source-sensor"
  ...
  `}function wt(n,o,t=4){if(!isFinite(n)||!isFinite(o))return{ticks:[],niceMin:0,niceMax:1};if(n===o){let p=Math.abs(n)*.1||1;n-=p,o+=p}let e=o-n,i=Math.pow(10,Math.floor(Math.log10(e/Math.max(1,t)))),a=i;for(let p of[1,2,5,10])if(a=i*p,e/a<=t+.5)break;let l=Math.floor(n/a)*a,h=Math.ceil(o/a)*a,c=[];for(let p=l;p<=h+a*1e-6;p+=a)c.push(Math.abs(p)<a*1e-9?0:p);return{ticks:c,niceMin:l,niceMax:h}}function ct(n){let o=Math.abs(n);return o>=1e6?ft((n/1e6).toFixed(o>=1e7?0:1))+"M":o>=1e4?ft((n/1e3).toFixed(0))+"k":o>=1e3?ft((n/1e3).toFixed(1))+"k":o>=100?n.toFixed(0):o>=10||o>=1?ft(n.toFixed(1)):o===0?"0":ft(n.toFixed(2))}function ft(n){return n.replace(/\.0+$/,"").replace(/(\.\d*[1-9])0+$/,"$1")}function vt(n,o,t){let e=n.toLocaleString(t,{maximumFractionDigits:Math.abs(n)>=100?0:1});return o?`${e} ${o}`:e}function yt(n,o,t){let e=new Date(n),i=t?{month:"short",day:"numeric",year:"2-digit"}:{month:"short",day:"numeric"};return e.toLocaleDateString(o,i)}function Xt(n,o){return new Date(n).toLocaleDateString(o,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function Ft(n,o){return new Date(n).getFullYear()!==new Date(o).getFullYear()}function Bt(n,o,t){if(t<2||o<=n)return[n,o];let e=[];for(let i=0;i<t;i++)e.push(n+(o-n)*i/(t-1));return e}var Vt=210,nt=46,pt=14,ht=12,we=14,ri=20+we,oi=[{days:7,key:"chart_range_7d"},{days:30,key:"chart_range_30d"},{days:90,key:"chart_range_90d"},{days:365,key:"chart_range_1y"}],V=class extends L{constructor(){super(...arguments);this.points=[];this.events=[];this.unit="";this.lang="en";this.thresholdAbove=null;this.thresholdBelow=null;this.targetValue=null;this.forceZero=!1;this.projection=null;this.rangeDays=30;this.showRange=!0;this.busy=!1;this.hideOutliers=!1;this.showOutlierToggle=!0;this._width=0;this._hover=null;this._ro=null}connectedCallback(){super.connectedCallback(),this._ro=new ResizeObserver(t=>{let e=Math.floor(t[0]?.contentRect?.width||0);e&&Math.abs(e-this._width)>2&&(this._width=e)}),this._ro.observe(this)}disconnectedCallback(){super.disconnectedCallback(),this._ro?.disconnect(),this._ro=null}_emitRange(t){t!==this.rangeDays&&this.dispatchEvent(new CustomEvent("range-change",{detail:{days:t},bubbles:!0,composed:!0}))}_toggleOutliers(){this.dispatchEvent(new CustomEvent("outlier-toggle",{detail:{hide:!this.hideOutliers},bubbles:!0,composed:!0}))}render(){let t=this._width||320,e=[...this.points].sort((a,l)=>a.ts-l.ts),i=this.lang;return r`
    ...
    `}_renderSvg(t,e){let i=this.lang,a=t-nt-pt,l=Vt-ri,h=l-ht,c=1/0,p=-1/0;for(let w of e)c=Math.min(c,w.min??w.val),p=Math.max(p,w.max??w.val);this.thresholdAbove!=null&&(c=Math.min(c,this.thresholdAbove),p=Math.max(p,this.thresholdAbove)),this.thresholdBelow!=null&&(c=Math.min(c,this.thresholdBelow),p=Math.max(p,this.thresholdBelow)),this.targetValue!=null&&(c=Math.min(c,this.targetValue),p=Math.max(p,this.targetValue)),this.forceZero&&(c=Math.min(c,0));let u=(p-c||1)*.06,_=this.forceZero&&c>=0?0:c-u,{ticks:v,niceMin:b,niceMax:x}=wt(_,p+u,4);this.forceZero&&c>=0&&b<0&&(b=0,v=v.filter(w=>w>=0));let f=e[0].ts,M=e[e.length-1].ts,S=M-f||1,I=Ft(f,M),R=w=>nt+(w-f)/S*a,j=w=>ht+(1-(w-b)/(x-b||1))*h,y=e.map(w=>`${R(w.ts).toFixed(1)},${j(w.val).toFixed(1)}`).join(" "),N=`M${R(e[0].ts).toFixed(1)},${l} `+e.map(w=>`L${R(w.ts).toFixed(1)},${j(w.val).toFixed(1)}`).join(" ")+` L${R(e[e.length-1].ts).toFixed(1)},${l} Z`,W="",H=e.filter(w=>w.min!=null&&w.max!=null);if(H.length>=2){let w=H.map(C=>`${R(C.ts).toFixed(1)},${j(C.max).toFixed(1)}`),$=[...H].reverse().map(C=>`${R(C.ts).toFixed(1)},${j(C.min).toFixed(1)}`);W=`M${w[0]} `+w.slice(1).map(C=>`L${C}`).join(" ")+` L${$.join(" L")} Z`}let O=[];if(this.thresholdBelow!=null){let w=j(this.thresholdBelow);O.push({y:w,h:Math.max(0,l-w),lineY:w,label:`\u25BC ${ct(this.thresholdBelow)}`,labelY:Math.min(l-4,w+13)})}if(this.thresholdAbove!=null){let w=j(this.thresholdAbove);O.push({y:ht,h:Math.max(0,w-ht),lineY:w,label:`\u25B2 ${ct(this.thresholdAbove)}`,labelY:Math.max(ht+11,w-5)})}let Z=e[e.length-1],st=(this.events||[]).filter(w=>w.ts>=f&&w.ts<=M),rt=Bt(f,M,Math.max(2,Math.min(5,Math.floor(a/110)+1))),U=this._hover;return r`
  ...
  `}function Se(n,o){let t=n.interval_analysis,e=t?.weibull_beta,i=t?.weibull_eta;if(e==null||i==null||i<=0)return d;let a=n.interval_days??0,l=n.suggested_interval??a;return r`
    <div class="weibull-section">
      ...
      ${ui(e,i,a,l,o)}
      ${gi(t,o)}
      ${t?.confidence_interval_low!=null?_i(t,n,o):d}
    </div>
  ...
  `}function _i(n,o,t){let e=n.confidence_interval_low,i=n.confidence_interval_high,a=o.suggested_interval??o.interval_days??0,l=o.interval_days??0,h=Math.max(0,e-5),p=i+5-h,u=(e-h)/p*100,_=(i-e)/p*100,v=(a-h)/p*100,b=l>0?(l-h)/p*100:-1;return r`
  ...
  `}function Ce(n,o){return mi(n,o)}function mi(n,o){let t=n.seasonal_factors??n.interval_analysis?.seasonal_factors;if(!t||t.length!==12)return d;let e=n.interval_analysis?.seasonal_reason,i=new Date().getMonth(),a=300,l=100,h=8,p=l-h-4,u=Math.max(...t,1.5),_=a/12,v=_*.65,b=h+p-1/u*p;return r`
  ...
  `}function fi(n,o,t){let e=n.history.filter($=>$.type==="completed"&&($.cost!=null||$.duration!=null)).map($=>({ts:new Date($.timestamp).getTime(),cost:$.cost??0,duration:$.duration??0})).sort(($,C)=>$.ts-C.ts);if(e.length<2)return d;let i=e.some($=>$.cost>0),a=e.some($=>$.duration>0);if(!i&&!a)return d;let l=t!=="duration"&&i,h=t!=="cost"&&a,c=l||!h&&i,p=h||!l&&a,u=640,_=vi,v=c?44:12,b=p?44:12,x=u-v-b,f=_-bi,M=f-kt,S=e[0].ts,I=e[e.length-1].ts,R=(I-S||864e5)*.05,j=S-R,y=I+R,N=Ft(S,I),W=$=>v+($-j)/(y-j)*x,H=wt(0,Math.max(...e.map($=>$.cost))||1,3),O=wt(0,Math.max(...e.map($=>$.duration))||1,3),Z=$=>kt+(1-$/(H.niceMax||1))*M,st=$=>kt+(1-$/(O.niceMax||1))*M,rt=e.length>1?Math.min(...e.slice(1).map(($,C)=>W($.ts)-W(e[C].ts))):x,U=Math.max(6,Math.min(22,rt*.55)),w=Bt(S,I,Math.max(2,Math.min(4,e.length)));return r`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${u} ${_}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_history",o)}">
        ...
        ${w.map(($,C)=>{let Ut=C===0?"start":C===w.length-1?"end":"middle";return P`<text x="${W($).toFixed(1)}" y="${_-6}" text-anchor="${Ut}" fill="var(--secondary-text-color)" font-size="10">${yt($,o,N)}</text>`})}
      </svg>
    </div>
    <div class="chart-legend">
  ...
  `,g([T({attribute:!1})],_t.prototype,"hass",2),g([T()],_t.prototype,"docId",2),g([m()],_t.prototype,"_url",2),g([m()],_t.prototype,"_failed",2);customElements.get("maintenance-history-photo")||customElements.define("maintenance-history-photo",_t);var yi=["completed","skipped","missed","reset","triggered"];function Re(n,o){let t=o.lang;return r`
