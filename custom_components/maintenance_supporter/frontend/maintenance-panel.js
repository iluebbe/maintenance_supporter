...
`;var re=class extends S{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._objectFilter="";this._configuredObjects=[];this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){if(this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter),typeof e.object_filter=="string")this._objectFilter=e.object_filter,this._configuredObjects=[];else if(Array.isArray(e.object_filter)){let t=e.object_filter.filter(i=>typeof i=="string"&&i!=="");this._objectFilter=t.length===1?t[0]:"",this._configuredObjects=t.length>1?t:[]}}getCardSize(){return 6}get _lang(){return this.hass?.language||"en"}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){super.updated(e),e.has("hass")&&vt(this.hass?.locale);let t=this.hass?.language;if(t&&!mt(t)&&W(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{let e=await this.hass.connection.subscribeMessage(t=>{let i=t;this._objects=i.objects},{type:"maintenance_supporter/subscribe"});if(!this.isConnected){e();return}this._unsub=e}catch{}}_onEventClick(e){if(e.history_timestamp){this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}));return}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}render(){if(!this.hass)return h;let e=this._lang,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.title,l=null;this._userFilter&&(l=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let c=w=>{let Q=w.toLowerCase();return this._objects.find(F=>F.entry_id===w||F.object.name.toLowerCase()===Q)?.entry_id??null},d=new Set(this._configuredObjects.map(c).filter(w=>w!==null)),u=d.size?this._objects.filter(w=>d.has(w.entry_id)):this._objects,g=this._config.show_object_filter!==!1&&u.length>1,m=this._objectFilter?c(this._objectFilter):null,v=m&&u.some(w=>w.entry_id===m)?u.filter(w=>w.entry_id===m):u,f=new Date;f.setHours(0,0,0,0);let $=this._pastDays>0,x=$?Hi(v,f,this._pastDays,l):Di(v,f,this._windowDays,l),M=st(f),L=this._windowDays===365||$,N=L?x.filter(w=>w.events.length>0):x,O=w=>{let Q=`cal-status-${w.status}`,Z=w.projected?"cal-event-projected":"",F=w.status==="overdue"&&w.days_until_due!=null?` (${Te(w.days_until_due,e)})`:"",D=w.projected&&w.interval_days?o`<span class="cal-event-recur">${w.interval_unit&&w.interval_unit!=="days"?`${w.interval_days} ${s("unit_"+w.interval_unit,e)}`:s("cal_every_n_days",e).replace("{n}",String(w.interval_days))}</span>`:h,te=w.schedule_type==="sensor_based",pe=te?o`<ha-icon class="cal-event-icon cal-source-sensor"
      ...
      `},C=w=>{let[Q,Z,F]=w.date.split("-").map(Number),D=new Date(Q,Z-1,F),te=w.date===M,pe=D.toLocaleDateString(e,{weekday:"short"}),_e=D.toLocaleDateString(e,{month:"long"});return o`
    ...
    `],p([b({attribute:!1})],re.prototype,"hass",2),p([_()],re.prototype,"_config",2),p([_()],re.prototype,"_objects",2),p([_()],re.prototype,"_stats",2),p([_()],re.prototype,"_windowDays",2),p([_()],re.prototype,"_pastDays",2),p([_()],re.prototype,"_userFilter",2),p([_()],re.prototype,"_objectFilter",2),p([_()],re.prototype,"_unsub",2);var sa=[{value:7,label:"Week (7 days)"},{value:14,label:"Fortnight (14 days)"},{value:30,label:"Month (30 days, default)"},{value:365,label:"Year (365 days, empty days collapsed)"}],He=class extends S{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"}}setConfig(e){this._config={...e}}_valueChanged(e,t){let i={...this._config,[e]:t};e==="show_window_chips"&&t===!0&&delete i.show_window_chips,e==="show_user_filter"&&t===!0&&delete i.show_user_filter,e==="show_object_filter"&&t===!0&&delete i.show_object_filter,e==="title"&&(!t||typeof t=="string"&&t.trim()==="")&&delete i.title,e==="user_filter"&&t===""&&delete i.user_filter,this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}render(){let e=this._config.window_days??30,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.user_filter??"",l=this._config.title??"";return o`
      <div class="editor">
        <div class="row">
...
<body>
    ...
    `}_renderSvg(e,t){let i=this.lang,a=e-ue-$e,l=Lt-ca,c=l-we,d=1/0,u=-1/0;for(let E of t)d=Math.min(d,E.min??E.val),u=Math.max(u,E.max??E.val);this.thresholdAbove!=null&&(d=Math.min(d,this.thresholdAbove),u=Math.max(u,this.thresholdAbove)),this.thresholdBelow!=null&&(d=Math.min(d,this.thresholdBelow),u=Math.max(u,this.thresholdBelow)),this.targetValue!=null&&(d=Math.min(d,this.targetValue),u=Math.max(u,this.targetValue)),this.forceZero&&(d=Math.min(d,0));let g=(u-d||1)*.06,m=this.forceZero&&d>=0?0:d-g,{ticks:v,niceMin:f,niceMax:$}=rt(m,u+g,4);this.forceZero&&d>=0&&f<0&&(f=0,v=v.filter(E=>E>=0));let x=t[0].ts,M=t[t.length-1].ts,L=M-x||1,N=Rt(x,M),O=E=>ue+(E-x)/L*a,C=E=>we+(1-(E-f)/($-f||1))*c,w=t.map(E=>`${O(E.ts).toFixed(1)},${C(E.val).toFixed(1)}`).join(" "),Q=`M${O(t[0].ts).toFixed(1)},${l} `+t.map(E=>`L${O(E.ts).toFixed(1)},${C(E.val).toFixed(1)}`).join(" ")+` L${O(t[t.length-1].ts).toFixed(1)},${l} Z`,Z="",F=t.filter(E=>E.min!=null&&E.max!=null);if(F.length>=2){let E=F.map(I=>`${O(I.ts).toFixed(1)},${C(I.max).toFixed(1)}`),k=[...F].reverse().map(I=>`${O(I.ts).toFixed(1)},${C(I.min).toFixed(1)}`);Z=`M${E[0]} `+E.slice(1).map(I=>`L${I}`).join(" ")+` L${k.join(" L")} Z`}let D=[];if(this.thresholdBelow!=null){let E=C(this.thresholdBelow);D.push({y:E,h:Math.max(0,l-E),lineY:E,label:`\u25BC ${fe(this.thresholdBelow)}`,labelY:Math.min(l-4,E+13)})}if(this.thresholdAbove!=null){let E=C(this.thresholdAbove);D.push({y:we,h:Math.max(0,E-we),lineY:E,label:`\u25B2 ${fe(this.thresholdAbove)}`,labelY:Math.max(we+11,E-5)})}let te=t[t.length-1],pe=(this.events||[]).filter(E=>E.ts>=x&&E.ts<=M),_e=Mt(x,M,Math.max(2,Math.min(5,Math.floor(a/110)+1))),G=this._hover;return o`
      <div class="svg-holder">
        ...
        >
          ...
          ${pe.map(E=>{let k=O(E.ts),I=E.type==="completed"?"var(--success-color, #4caf50)":E.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return H`
          ...
          ${G?H`
                ...
                <circle cx="${G.x.toFixed(1)}" cy="${G.y.toFixed(1)}" r="4.5" fill="var(--primary-color)"
                  stroke="var(--card-background-color, #fff)" stroke-width="2" />`:H`<circle cx="${O(te.ts).toFixed(1)}" cy="${C(te.val).toFixed(1)}" r="4" fill="var(--primary-color)"
  ...
  `}function ma(n,r,e,t,i){let f=Math.max(e,t,r,1)*1.3,$=50,x=[];for(let F=0;F<=$;F++){let D=F/$*f,te=1-Math.exp(-Math.pow(D/r,n)),pe=32+D/f*260,_e=136-te*128;x.push([pe,_e])}let M=x.map(([F,D])=>`${F.toFixed(1)},${D.toFixed(1)}`).join(" "),L="M32,136 "+x.map(([F,D])=>`L${F.toFixed(1)},${D.toFixed(1)}`).join(" ")+` L${x[$][0].toFixed(1)},136 Z`,N=32+e/f*260,O=1-Math.exp(-Math.pow(e/r,n)),C=136-O*128,w=((1-O)*100).toFixed(0),Q=32+t/f*260,Z=[0,.25,.5,.75,1];return o`
  ...
  `}function $a(n,r,e){let t=n.history.filter(k=>k.type==="completed"&&(k.cost!=null||k.duration!=null)).map(k=>({ts:new Date(k.timestamp).getTime(),cost:k.cost??0,duration:k.duration??0})).sort((k,I)=>k.ts-I.ts);if(t.length<2)return h;let i=t.some(k=>k.cost>0),a=t.some(k=>k.duration>0);if(!i&&!a)return h;let l=e!=="duration"&&i,c=e!=="cost"&&a,d=l||!c&&i,u=c||!l&&a,g=640,m=ya,v=d?44:12,f=u?44:12,$=g-v-f,x=m-xa,M=x-nt,L=t[0].ts,N=t[t.length-1].ts,O=(N-L||864e5)*.05,C=L-O,w=N+O,Q=Rt(L,N),Z=k=>v+(k-C)/(w-C)*$,F=rt(0,Math.max(...t.map(k=>k.cost))||1,3),D=rt(0,Math.max(...t.map(k=>k.duration))||1,3),te=k=>nt+(1-k/(F.niceMax||1))*M,pe=k=>nt+(1-k/(D.niceMax||1))*M,_e=t.length>1?Math.min(...t.slice(1).map((k,I)=>Z(k.ts)-Z(t[I].ts))):$,G=Math.max(6,Math.min(22,_e*.55)),E=Mt(L,N,Math.max(2,Math.min(4,t.length)));return o`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${g} ${m}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_history",r)}">
        ...
        ${u?D.ticks.map(k=>{let I=pe(k);return I<nt-1||I>x+1?h:H`<text x="${g-f+6}" y="${(I+3.5).toFixed(1)}" text-anchor="start" fill="var(--accent-color, #ff9800)" font-size="10.5">${fe(k)}m</text>`}):h}
