...
<body>
  ...
  `}function Pt(r,n,e){if(!r.trigger_config?.entity_id)return h;let t=r.trigger_config.entity_id,i=n.get(t)||[],a=[];if(i.length>=2)a=i.map(T=>({ts:T.ts,val:T.val}));else{if(!r.history)return h;for(let T of r.history)T.trigger_value!=null&&a.push({ts:new Date(T.timestamp).getTime(),val:T.trigger_value})}if(r.trigger_current_value!=null&&a.push({ts:Date.now(),val:r.trigger_current_value}),a.length<2)return h;a.sort((T,Y)=>T.ts-Y.ts);let l=Rs,c=Is,d=a.map(T=>T.val),u=Math.min(...d),g=Math.max(...d),m=g-u||1;u-=m*.1,g+=m*.1;let v=a[0].ts,w=a[a.length-1].ts-v||1,b=T=>(T-v)/w*l,C=T=>2+(1-(T-u)/(g-u))*(c-4),P=a;if(P.length>xi){let T=Math.ceil(P.length/xi);P=P.filter((Y,G)=>G%T===0||G===P.length-1)}let H=P.map(T=>`${b(T.ts).toFixed(1)},${C(T.val).toFixed(1)}`).join(" "),R=r.trigger_active?"var(--error-color, #f44336)":"var(--primary-color)";return o`
      ...
      <div class="svg-holder">
        ...
        >
          ...
          ${this.targetValue!=null?z`<line x1="${se}" y1="${T(this.targetValue).toFixed(1)}" x2="${e-de}" y2="${T(this.targetValue).toFixed(1)}"
                stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="6,4" />
              <text x="${e-de-4}" y="${(T(this.targetValue)-5).toFixed(1)}" text-anchor="end" class="zone-label">◆ ${re(this.targetValue)} ${this.unit}</text>`:h}
  ...
  `}function Ds(r,n,e){let t=r.trigger_config,i=r.trigger_current_value;if(!t||i==null)return null;switch(t.type||"threshold"){case"counter":{let a=t.trigger_target_value;if(a==null||a<=0)return null;let l=Ei(r,Ti(r,e));return{progress:Math.max(0,i-(l?.value??i)),target:a,unit:n,meter:i}}case"state_change":{let a=t.trigger_target_changes;return a==null||a<=0?null:{progress:Math.max(0,i),target:a,unit:"",meter:null}}case"runtime":{let a=t.trigger_runtime_hours;return a==null||a<=0?null:{progress:Math.max(0,i),target:a,unit:"h",meter:null}}}return null}function Ei(r,n){if(r.trigger_baseline_value!=null)return{value:r.trigger_baseline_value,ts:Ht(r)};if(!n.length)return null;let e=Ht(r);if(e==null)return{value:n[0].val,ts:null};let t=n[0],i=Math.abs(n[0].ts-e);for(let a of n){let l=Math.abs(a.ts-e);l<i&&(t=a,i=l)}return{value:t.val,ts:e}}function Ht(r){let n=[...r.history].filter(e=>e.type==="completed"||e.type==="reset").sort((e,t)=>new Date(t.timestamp).getTime()-new Date(e.timestamp).getTime())[0];return n?new Date(n.timestamp).getTime():null}function Hs(r,n){let e=Math.min(999,Math.round(r.progress/r.target*100)),t=e>=100?"over":e>=75?"near":"ok";return o`
  ...
  `}function Ns(r,n,e,t,i){let f=Math.max(e,t,n,1)*1.3,w=50,b=[];for(let F=0;F<=w;F++){let q=F/w*f,he=1-Math.exp(-Math.pow(q/n,r)),me=32+q/f*260,ke=136-he*128;b.push([me,ke])}let C=b.map(([F,q])=>`${F.toFixed(1)},${q.toFixed(1)}`).join(" "),P="M32,136 "+b.map(([F,q])=>`L${F.toFixed(1)},${q.toFixed(1)}`).join(" ")+` L${b[w][0].toFixed(1)},136 Z`,H=32+e/f*260,R=1-Math.exp(-Math.pow(e/n,r)),T=136-R*128,Y=((1-R)*100).toFixed(0),G=32+t/f*260,K=[0,.25,.5,.75,1];return o`
  ...
  `}function na(r,n){switch(n.activeTab){case"overview":return sa(r,n);case"history":return aa(r,n);default:return h}}function Di(r,n){return o`
