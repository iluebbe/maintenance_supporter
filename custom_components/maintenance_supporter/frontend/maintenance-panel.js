...
`;var we=class{constructor(a){this._cache=new Map;this._pending=new Map;this._hass=a}updateHass(a){this._hass=a}async getDetailStats(a,e){return this._getStats(a,"hour",30,e)}async getMiniStats(a,e){return this._getStats(a,"day",14,e)}async getBatchMiniStats(a){let e=new Map,t=[];for(let p of a){let u=`${p.entityId}:day`,m=this._cache.get(u);m&&Date.now()-m.fetchedAt<3e5?e.set(p.entityId,m.points):t.push(p)}if(t.length===0)return e;let i=t.filter(p=>p.isCounter).map(p=>p.entityId),n=t.filter(p=>!p.isCounter).map(p=>p.entityId),l=new Date(Date.now()-336*60*60*1e3).toISOString(),d=[];return i.length>0&&d.push(this._fetchBatch(i,"day",l,["state","sum","change"],!0,e)),n.length>0&&d.push(this._fetchBatch(n,"day",l,["mean","min","max"],!1,e)),await Promise.all(d),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(a,e,t,i){let n=`${a}:${e}`,l=this._cache.get(n);if(l&&Date.now()-l.fetchedAt<3e5)return l.points;if(this._pending.has(n))return this._pending.get(n);let d=this._fetchAndNormalize(a,e,t,i,n);this._pending.set(n,d);try{return await d}finally{this._pending.delete(n)}}async _fetchAndNormalize(a,e,t,i,n){let l=new Date(Date.now()-t*24*60*60*1e3).toISOString(),d=i?["state","sum","change"]:["mean","min","max"];try{let u=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:l,statistic_ids:[a],period:e,types:d}))[a]||[],m=this._normalizeRows(u,i);return this._cache.set(n,{entityId:a,fetchedAt:Date.now(),period:e,points:m}),m}catch(p){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${a}:`,p),[]}}async _fetchBatch(a,e,t,i,n,l){try{let d=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:a,period:e,types:i});for(let p of a){let u=d[p]||[],m=this._normalizeRows(u,n);l.set(p,m),this._cache.set(`${p}:${e}`,{entityId:p,fetchedAt:Date.now(),period:e,points:m})}}catch(d){console.warn("[maintenance-supporter] Batch statistics fetch failed:",d)}}_normalizeRows(a,e){let t=[];for(let i of a){let n=null;if(e?n=i.state??null:n=i.mean??null,n===null)continue;let l={ts:i.start,val:n};e||(i.min!=null&&(l.min=i.min),i.max!=null&&(l.max=i.max)),t.push(l)}return t.sort((i,n)=>i.ts-n.ts),t}};var re=class{constructor(a){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=a}updateHass(a){this.hass=a}async getUsers(a=!1){let e=Date.now();if(!a&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(a,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:a,task_id:e,user_id:t})}async getTasksByUser(a){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:a})).tasks}getUserName(a){return!a||!this.usersCache?null:this.usersCache.find(t=>t.id===a)?.name||null}getUser(a){return!a||!this.usersCache?null:this.usersCache.find(e=>e.id===a)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(a){return a?a===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var q=class extends L{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._name="";this._manufacturer="";this._model="";this._serialNumber="";this._areaId="";this._installationDate="";this._entryId=null}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}openCreate(){this._entryId=null,this._name="",this._manufacturer="",this._model="",this._serialNumber="",this._areaId="",this._installationDate="",this._error="",this._open=!0}openEdit(e,t){this._entryId=e,this._name=t.name||"",this._manufacturer=t.manufacturer||"",this._model=t.model||"",this._serialNumber=t.serial_number||"",this._areaId=t.area_id||"",this._installationDate=t.installation_date||"",this._error="",this._open=!0}async _save(){if(this._name.trim()){this._loading=!0,this._error="";try{this._entryId?await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/update",entry_id:this._entryId,name:this._name,manufacturer:this._manufacturer||null,model:this._model||null,serial_number:this._serialNumber||null,area_id:this._areaId||null,installation_date:this._installationDate||null}):await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/create",name:this._name,manufacturer:this._manufacturer||null,model:this._model||null,serial_number:this._serialNumber||null,area_id:this._areaId||null,installation_date:this._installationDate||null}),this._open=!1,this.dispatchEvent(new CustomEvent("object-saved"))}catch{this._error=s("save_error",this._lang)}finally{this._loading=!1}}}_close(){this._open=!1}render(){if(!this._open)return o``;let e=this._lang,t=this._entryId?s("edit_object",e):s("new_object",e);return o`
  ...
  `,_([S({attribute:!1})],q.prototype,"hass",2),_([g()],q.prototype,"_open",2),_([g()],q.prototype,"_loading",2),_([g()],q.prototype,"_error",2),_([g()],q.prototype,"_name",2),_([g()],q.prototype,"_manufacturer",2),_([g()],q.prototype,"_model",2),_([g()],q.prototype,"_serialNumber",2),_([g()],q.prototype,"_areaId",2),_([g()],q.prototype,"_installationDate",2),_([g()],q.prototype,"_entryId",2);customElements.get("maintenance-object-dialog")||customElements.define("maintenance-object-dialog",q);var ai=["cleaning","inspection","replacement","calibration","service","custom"],ni=["time_based","sensor_based","manual"],si=["threshold","counter","state_change","runtime"],$=class extends L{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._entryId="";this._taskId=null;this._name="";this._type="custom";this._scheduleType="time_based";this._intervalDays="30";this._warningDays="7";this._intervalAnchor="completion";this._notes="";this._documentationUrl="";this._customIcon="";this._enabled=!0;this._triggerEntityId="";this._triggerEntityIds=[];this._triggerEntityLogic="any";this._triggerAttribute="";this._triggerType="threshold";this._triggerAbove="";this._triggerBelow="";this._triggerForMinutes="0";this._triggerTargetValue="";this._triggerDeltaMode=!1;this._triggerFromState="";this._triggerToState="";this._triggerTargetChanges="";this._triggerRuntimeHours="";this._suggestedAttributes=[];this._availableAttributes=[];this._entityDomain="";this._nfcTagId="";this._availableTags=[];this._responsibleUserId=null;this._availableUsers=[];this._userService=null}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}async openCreate(e){this._entryId=e,this._taskId=null,this._error="",this._resetFields(),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}async openEdit(e,t){if(this._entryId=e,this._taskId=t.id,this._error="",this._name=t.name,this._type=t.type,this._scheduleType=t.schedule_type,this._intervalDays=t.interval_days?.toString()||"30",this._warningDays=t.warning_days.toString(),this._intervalAnchor=t.interval_anchor||"completion",this._notes=t.notes||"",this._documentationUrl=t.documentation_url||"",this._customIcon=t.custom_icon||"",this._enabled=t.enabled!==!1,this._nfcTagId=t.nfc_tag_id||"",this._responsibleUserId=t.responsible_user_id||null,t.trigger_config){let i=t.trigger_config;this._triggerEntityId=i.entity_id||"",this._triggerEntityIds=i.entity_ids||(i.entity_id?[i.entity_id]:[]),this._triggerEntityLogic=i.entity_logic||"any",this._triggerAttribute=i.attribute||"",this._triggerType=i.type||"threshold",this._triggerAbove=i.trigger_above?.toString()||"",this._triggerBelow=i.trigger_below?.toString()||"",this._triggerForMinutes=i.trigger_for_minutes?.toString()||"0",this._triggerTargetValue=i.trigger_target_value?.toString()||"",this._triggerDeltaMode=i.trigger_delta_mode||!1,this._triggerFromState=i.trigger_from_state||"",this._triggerToState=i.trigger_to_state||"",this._triggerTargetChanges=i.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=i.trigger_runtime_hours?.toString()||""}else this._resetTriggerFields();this._triggerEntityId&&this._fetchEntityAttributes(this._triggerEntityId),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._warningDays="7",this._intervalAnchor="completion",this._notes="",this._documentationUrl="",this._customIcon="",this._enabled=!0,this._nfcTagId="",this._responsibleUserId=null,this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerForMinutes="0",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours=""}async _loadUsers(){this._userService||(this._userService=new re(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}async _loadTags(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tags/list"});this._availableTags=e.tags||[]}catch{this._availableTags=[]}}async _fetchEntityAttributes(e){if(!e||!this.hass){this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="";return}try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/entity/attributes",entity_id:e});this._entityDomain=t.domain||"",this._suggestedAttributes=t.suggested_attributes||[],this._availableAttributes=t.available_attributes||[]}catch{this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain=""}}async _save(){if(this._name.trim()){this._loading=!0,this._error="";try{let e={type:this._taskId?"maintenance_supporter/task/update":"maintenance_supporter/task/create",entry_id:this._entryId,name:this._name,task_type:this._type,schedule_type:this._scheduleType,warning_days:parseInt(this._warningDays,10)||7};if(this._taskId&&(e.task_id=this._taskId),this._scheduleType!=="manual"?this._intervalDays?(e.interval_days=parseInt(this._intervalDays,10),e.interval_anchor=this._intervalAnchor):this._taskId&&(e.interval_days=null,e.interval_anchor="completion"):this._taskId&&(e.interval_days=null,e.interval_anchor="completion"),e.notes=this._notes||null,e.documentation_url=this._documentationUrl||null,e.custom_icon=this._customIcon||null,e.enabled=this._enabled,e.nfc_tag_id=this._nfcTagId||null,e.responsible_user_id=this._responsibleUserId,this._scheduleType==="sensor_based"&&this._triggerEntityId){let t=this._triggerEntityIds.length>0?this._triggerEntityIds:[this._triggerEntityId],i={entity_id:t[0],entity_ids:t,type:this._triggerType};if(this._triggerAttribute&&(i.attribute=this._triggerAttribute),t.length>1&&(i.entity_logic=this._triggerEntityLogic),this._triggerType==="threshold"){if(this._triggerAbove){let n=parseFloat(this._triggerAbove);isNaN(n)||(i.trigger_above=n)}if(this._triggerBelow){let n=parseFloat(this._triggerBelow);isNaN(n)||(i.trigger_below=n)}if(this._triggerForMinutes){let n=parseInt(this._triggerForMinutes,10);isNaN(n)||(i.trigger_for_minutes=n)}}else if(this._triggerType==="counter"){if(this._triggerTargetValue){let n=parseFloat(this._triggerTargetValue);isNaN(n)||(i.trigger_target_value=n)}i.trigger_delta_mode=this._triggerDeltaMode}else if(this._triggerType==="state_change"){if(this._triggerFromState&&(i.trigger_from_state=this._triggerFromState),this._triggerToState&&(i.trigger_to_state=this._triggerToState),this._triggerTargetChanges){let n=parseInt(this._triggerTargetChanges,10);isNaN(n)||(i.trigger_target_changes=n)}}else if(this._triggerType==="runtime"&&this._triggerRuntimeHours){let n=parseFloat(this._triggerRuntimeHours);isNaN(n)||(i.trigger_runtime_hours=n)}e.trigger_config=i}else this._taskId&&(e.trigger_config=null);await this.hass.connection.sendMessagePromise(e),this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch{this._error=s("save_error",this._lang)}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return c;let e=this._lang;return o`
      ...
      <ha-textfield
        ...
        @input=${t=>{let n=t.target.value.split(",").map(l=>l.trim()).filter(Boolean);this._triggerEntityId=n[0]||"",this._triggerEntityIds=n,n[0]&&this._fetchEntityAttributes(n[0])}}
      ></ha-textfield>
      ...
      ${this._availableAttributes.length>0?o`
          <div class="select-row">
            ...
            >
              ...
              ${this._suggestedAttributes.map(t=>o`<option value=${t}>${t} ★</option>`)}
              ${this._availableAttributes.filter(t=>!this._suggestedAttributes.includes(t.name)).map(t=>o`<option value=${t.name}>${t.name}${t.numeric?"":" (non-numeric)"}</option>`)}
            </select>
          </div>
        `:o`
          <ha-textfield
      ...
      <div class="select-row">
        ...
        >
          ${si.map(t=>o`<option value=${t}>${s(t,e)}</option>`)}
        </select>
      </div>
    ...
    `}_renderTriggerTypeFields(){let e=this._lang;return this._triggerType==="threshold"?o`
      ...
      `:this._triggerType==="runtime"?o`
        ...
        ></ha-textfield>
      `:c}render(){if(!this._open)return o``;let e=this._lang,t=this._taskId?s("edit_task",e):s("new_task",e);return o`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
          ${this._error?o`<div class="error">${this._error}</div>`:c}
          ...
          <div class="select-row">
            ...
            >
              ${ai.map(i=>o`<option value=${i} ?selected=${i===this._type}>${s(i,e)}</option>`)}
            </select>
          </div>
          <div class="select-row">
            ...
            >
              ${ni.map(i=>o`<option value=${i} ?selected=${i===this._scheduleType}>${s(i,e)}</option>`)}
            </select>
          </div>
          ...
          ${this._availableTags.length>0?o`
              <div class="select-row">
                ...
                >
                  ...
                  ${this._availableTags.map(i=>o`<option value=${i.id}>${i.name}</option>`)}
                </select>
              </div>
            `:o`
              <ha-textfield
...
<script>setTimeout(()=>window.print(),300)<\/script>
</body></html>`),n.document.close()}_downloadSvg(e,t){let i=decodeURIComponent(e.svg_data_uri.replace("data:image/svg+xml,","")),n=new Blob([i],{type:"image/svg+xml"}),l=URL.createObjectURL(n),d=document.createElement("a");d.href=l;let p=this._taskName?`${this._objectName}-${this._taskName}`:this._objectName;d.download=`qr-${ri(p)}-${t}.svg`,d.click(),URL.revokeObjectURL(l)}_close(){this._open=!1,this._viewResult=null,this._completeResult=null,this._error="",this._loading=!1}render(){if(!this._open)return o``;let e=this.lang||this.hass?.language||"en",t=this._taskName?`${s("qr_code",e)}: ${this._objectName} \u2014 ${this._taskName}`:`${s("qr_code",e)}: ${this._objectName}`,i=!!this._viewResult;return o`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
        ...
        ${i.map(n=>o`
          <label class="setting-row">
            ...
            <input type="checkbox" .checked=${t[n.key]}
              @change=${l=>this._updateSetting(n.settingKey,l.target.checked)} />
          </label>
        `)}
      </div>
      ...
      <div class="settings-section">
        ...
        <label class="setting-row">
          ...
          <select @change=${i=>this._updateSetting("budget_currency",i.target.value)}>
            ${oi.map(i=>o`<option value=${i} ?selected=${t.currency===i}>${i}</option>`)}
          </select>
        </label>
        <label class="setting-row">
    ...
    <div class="trigger-card">
      <div class="trigger-header">
        ...
        <div>
          ${b?o`
            ...
            <div class="trigger-entity-id">${p.map((f,k)=>o`${k>0?", ":""}<span class="entity-link" @click=${T=>ae(T,f)}>${f}</span>`)}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `:o`
            <div class="trigger-entity-name">${l}</div>
            <div class="trigger-entity-id">${d?o`<span class="entity-link" @click=${f=>ae(f,d)}>${d}</span>`:""}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `}
        </div>
      ...
      ${m!=null?o`
            <div class="trigger-value-row">
              ...
              ${u?o`<span class="trigger-unit">${u}</span>`:c}
            </div>
          `:c}

      <div class="trigger-limits">
        ${v==="threshold"?o`
          ${e.trigger_above!=null?o`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("threshold_above",t)}: ${e.trigger_above} ${u}</span>`:c}
          ${e.trigger_below!=null?o`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("threshold_below",t)}: ${e.trigger_below} ${u}</span>`:c}
          ${e.trigger_for_minutes?o`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${s("for_minutes",t)}: ${e.trigger_for_minutes}</span>`:c}
        `:c}
        ${v==="counter"?o`
          ${e.trigger_target_value!=null?o`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("target_value",t)}: ${e.trigger_target_value} ${u}</span>`:c}
        `:c}
        ${v==="state_change"?o`
          ${e.trigger_target_changes!=null?o`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("target_changes",t)}: ${e.trigger_target_changes}</span>`:c}
        `:c}
        ${v==="runtime"?o`
          ${e.trigger_runtime_hours!=null?o`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("runtime_hours",t)}: ${e.trigger_runtime_hours}h</span>`:c}
        `:c}
        ${v==="compound"?o`
          ...
          ${(e.conditions||[]).map((f,k)=>o`
            <span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${k+1}. ${s(f.type||"unknown",t)}: ${f.entity_id?o`<span class="entity-link" @click=${T=>ae(T,f.entity_id)}>${f.entity_id}</span>`:""}</span>
          `)}
        `:c}
        ${i?.min!=null?o`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${s("min",t)}: ${i.min} ${u}</span>`:c}
        ${i?.max!=null?o`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${s("max",t)}: ${i.max} ${u}</span>`:c}
      </div>
  ...
  `}function di(r,a,e){let t=r.trigger_config;if(!t)return c;let i=t.type||"threshold",n=i==="counter"&&t.trigger_delta_mode,l=e.isCounterEntity(t),d=t.entity_id||"",p=e.detailStatsData.get(d)||[],u=[],m=!1;if(p.length>=2)for(let h of p){let w=h.val;n&&r.trigger_baseline_value!=null&&(w-=r.trigger_baseline_value);let N={ts:h.ts,val:w};!l&&h.min!=null&&h.max!=null&&(N.min=n&&r.trigger_baseline_value!=null?h.min-r.trigger_baseline_value:h.min,N.max=n&&r.trigger_baseline_value!=null?h.max-r.trigger_baseline_value:h.max,m=!0),u.push(N)}else for(let h of r.history)h.trigger_value!=null&&u.push({ts:new Date(h.timestamp).getTime(),val:h.trigger_value});if(r.trigger_current_value!=null){let h=r.trigger_current_value;n&&r.trigger_baseline_value!=null&&(h-=r.trigger_baseline_value),u.push({ts:Date.now(),val:h})}if(u.length<2&&d&&e.hasStatsService&&!e.detailStatsData.has(d))return o`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
      <ha-icon icon="mdi:chart-line" style="--mdc-icon-size:16px;margin-right:8px;"></ha-icon>
      ${s("loading_chart",e.lang)}
    </div>`;if(u.length<2)return c;u.sort((h,w)=>h.ts-w.ts);let v=li,b=ci,f=30,k=2,T=8,H=16,C=u.map(h=>h.val),A=Math.min(...C),y=Math.max(...C);if(m)for(let h of u)h.min!=null&&(A=Math.min(A,h.min)),h.max!=null&&(y=Math.max(y,h.max));t.trigger_above!=null&&(y=Math.max(y,t.trigger_above),A=Math.min(A,t.trigger_above)),t.trigger_below!=null&&(A=Math.min(A,t.trigger_below),y=Math.max(y,t.trigger_below));let z=null,O=null;if(i==="counter"&&t.trigger_target_value!=null){if(r.trigger_baseline_value!=null)z=r.trigger_baseline_value;else if(u.length>0){let h=[...r.history].filter(w=>w.type==="completed"||w.type==="reset").sort((w,N)=>new Date(N.timestamp).getTime()-new Date(w.timestamp).getTime())[0];if(h){let w=new Date(h.timestamp).getTime(),N=u[0],R=Math.abs(u[0].ts-w);for(let J of u){let Z=Math.abs(J.ts-w);Z<R&&(N=J,R=Z)}z=N.val}else z=u[0].val}z!=null?(O=z+t.trigger_target_value,y=Math.max(y,O),A=Math.min(A,z)):(y=Math.max(y,t.trigger_target_value),A=Math.min(A,0))}n&&r.trigger_baseline_value!=null&&(A=Math.min(A,0));let G=y-A||1;A-=G*.1,y+=G*.1;let W=u[0].ts,D=u[u.length-1].ts,x=D-W||1,I=h=>f+(h-W)/x*(v-f-k),M=h=>T+(1-(h-A)/(y-A))*(b-T-H),Ee=u.map(h=>`${I(h.ts).toFixed(1)},${M(h.val).toFixed(1)}`).join(" "),kt=`M${I(u[0].ts).toFixed(1)},${b-H} `+u.map(h=>`L${I(h.ts).toFixed(1)},${M(h.val).toFixed(1)}`).join(" ")+` L${I(u[u.length-1].ts).toFixed(1)},${b-H} Z`,Ae="";if(m){let h=u.filter(w=>w.min!=null&&w.max!=null);if(h.length>=2){let w=h.map(R=>`${I(R.ts).toFixed(1)},${M(R.max).toFixed(1)}`),N=[...h].reverse().map(R=>`${I(R.ts).toFixed(1)},${M(R.min).toFixed(1)}`);Ae=`M${w[0]} `+w.slice(1).map(R=>`L${R}`).join(" ")+` L${N[0]} `+N.slice(1).map(R=>`L${R}`).join(" ")+" Z"}}let Oe=u[u.length-1],wt=I(Oe.ts),Et=M(Oe.val),Ue=h=>Math.abs(h)>=1e4?(h/1e3).toFixed(0)+"k":h>=1e3?(h/1e3).toFixed(1)+"k":h.toFixed(h<10?1:0),At=Ue(y),St=Ue(A),Tt=r.history.filter(h=>["completed","skipped","reset"].includes(h.type)).map(h=>({ts:new Date(h.timestamp).getTime(),type:h.type})).filter(h=>h.ts>=W&&h.ts<=D),Se=u;if(u.length>qe){let h=(u.length-1)/(qe-1);Se=[];for(let w=0;w<qe;w++)Se.push(u[Math.round(w*h)])}return o`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${v} ${b}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_sparkline",e.lang)}">
        ...
        ${r.degradation_rate!=null&&r.degradation_trend!=="stable"&&r.degradation_trend!=="insufficient_data"&&u.length>=2?(()=>{let h=u[u.length-1],w=30,N=h.ts+w*864e5,R=h.val+r.degradation_rate*w,J=Math.min(N,D+(D-W)*.3),Z=Math.max(A,Math.min(y,R)),Mt=I(h.ts),Ct=M(h.val),zt=I(J),It=M(Z);return j`<line x1="${Mt.toFixed(1)}" y1="${Ct.toFixed(1)}" x2="${zt.toFixed(1)}" y2="${It.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():c}
        ...
        ${Tt.map(h=>{let w=I(h.ts),N=h.type==="completed"?"var(--success-color, #4caf50)":h.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return j`
            <line x1="${w.toFixed(1)}" y1="${T}" x2="${w.toFixed(1)}" y2="${b-H}" stroke="${N}" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
            <circle cx="${w.toFixed(1)}" cy="${T+2}" r="5" fill="${N}" opacity="0.8" />
            <text x="${w.toFixed(1)}" y="${T+6}" text-anchor="middle" fill="white" font-size="7" font-weight="bold">${h.type==="completed"?"\u2713":h.type==="skipped"?"\u23ED":"\u21BA"}</text>
          `})}
        ${Se.map(h=>{let w=I(h.ts),N=M(h.val),R=new Date(h.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),J=`${h.val.toFixed(1)} ${a}`;return m&&h.min!=null&&h.max!=null&&(J+=` (${h.min.toFixed(1)}\u2013${h.max.toFixed(1)})`),j`<circle cx="${w.toFixed(1)}" cy="${N.toFixed(1)}" r="8" fill="transparent" tabindex="0"
            @mouseenter=${Z=>gt(Z,`${R}
${J}`,e.setTooltip)}
            @focus=${Z=>gt(Z,`${R}
${J}`,e.setTooltip)}
...
`).map(h=>o`<div>${h}</div>`)}
  ...
  `}function gt(r,a,e){let t=r.currentTarget,i=t.closest(".sparkline-container");if(!i)return;let n=i.getBoundingClientRect(),l=t.getBoundingClientRect();e({x:l.left-n.left+l.width/2,y:l.top-n.top-8,text:a})}function mt(r,a,e){let t=r.degradation_trend!=null&&r.degradation_trend!=="insufficient_data",i=r.days_until_threshold!=null,n=r.environmental_factor!=null&&r.environmental_factor!==1;if(!t&&!i&&!n)return c;let l=r.degradation_trend==="rising"?"M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z":r.degradation_trend==="falling"?"M16,18L18.29,15.71L13.41,10.83L9.41,14.83L2,7.41L3.41,6L9.41,12L13.41,8L19.71,14.29L22,12V18H16Z":"M22,12L18,8V11H3V13H18V16L22,12Z";return o`
      ...
      <div class="prediction-grid">
        ${t?o`
          <div class="prediction-item">
            ...
            ${r.degradation_rate!=null?o`<span class="prediction-rate">${r.degradation_rate>0?"+":""}${Math.abs(r.degradation_rate)>=10?Math.round(r.degradation_rate).toLocaleString():r.degradation_rate.toFixed(1)} ${r.trigger_entity_info?.unit_of_measurement||""}/${s("day_short",a)}</span>`:c}
          </div>
        `:c}
        ${i?o`
          <div class="prediction-item">
            ...
            ${r.threshold_prediction_date?o`<span class="prediction-date">${Q(r.threshold_prediction_date,a)}</span>`:c}
        ...
        ${n&&e.environmental?o`
          <div class="prediction-item">
            ...
            ${r.environmental_entity?o`<span class="prediction-entity entity-link" @click=${d=>ae(d,r.environmental_entity)}>${r.environmental_entity}</span>`:c}
          </div>
        `:c}
      </div>
    </div>
  `}function vt(r,a){let e=r.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return c;let n=r.interval_days??0,l=r.suggested_interval??n;return o`
  ...
  `}function pi(r,a,e,t,i){let f=Math.max(e,t,a,1)*1.3,k=50,T=[];for(let D=0;D<=k;D++){let x=D/k*f,I=1-Math.exp(-Math.pow(x/a,r)),M=32+x/f*260,Ee=136-I*128;T.push([M,Ee])}let H=T.map(([D,x])=>`${D.toFixed(1)},${x.toFixed(1)}`).join(" "),C="M32,136 "+T.map(([D,x])=>`L${D.toFixed(1)},${x.toFixed(1)}`).join(" ")+` L${T[k][0].toFixed(1)},136 Z`,A=32+e/f*260,y=1-Math.exp(-Math.pow(e/a,r)),z=136-y*128,O=((1-y)*100).toFixed(0),G=32+t/f*260,W=[0,.25,.5,.75,1];return o`
    ...
    <div class="chart-legend">
      ...
      ${e>0?o`<span class="legend-item"><span class="legend-swatch" style="background:var(--primary-color, #03a9f4); opacity:0.5"></span> ${s("current_interval_marker",i)}</span>`:c}
      ${t>0&&t!==e?o`<span class="legend-item"><span class="legend-swatch" style="background:var(--success-color, #4caf50)"></span> ${s("recommended_marker",i)}</span>`:c}
    </div>
  `}function _i(r,a){return o`
  ...
  `}function gi(r,a,e){let t=r.confidence_interval_low,i=r.confidence_interval_high,n=a.suggested_interval??a.interval_days??0,l=a.interval_days??0,d=Math.max(0,t-5),u=i+5-d,m=(t-d)/u*100,v=(i-t)/u*100,b=(n-d)/u*100,f=l>0?(l-d)/u*100:-1;return o`
  ...
  `}function yt(r,a){return hi(r,a)}function hi(r,a){let e=r.seasonal_factors??r.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return c;let t=r.interval_analysis?.seasonal_reason,i=new Date().getMonth(),n=300,l=100,d=8,u=l-d-4,m=Math.max(...e,1.5),v=n/12,b=v*.65,f=d+u-1/m*u;return o`
    <div class="seasonal-chart">
      <div class="seasonal-chart-title">
        ...
        ${t?o`<span class="source-tag">${t==="learned"?s("seasonal_learned",a):s("seasonal_manual",a)}</span>`:c}
      </div>
      <svg viewBox="0 0 ${n} ${l}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",a)}">
      ...
      <div class="seasonal-labels">
        ${ft.map((k,T)=>o`<span class="seasonal-label ${T===i?"active-month":""}">${s(k,a)}</span>`)}
      </div>
    </div>
  `}var mi=300,vi=200;function xt(r,a,e,t){let i=r.history.filter(d=>d.type==="completed"&&(d.cost!=null||d.duration!=null));if(i.length<2)return c;let n=i.some(d=>(d.cost??0)>0),l=i.some(d=>(d.duration??0)>0);return!n&&!l?c:o`
  ...
  `}var bi=60,yi=20,$t=30,E=class extends L{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._sparklineTooltip=null;this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1};this._actionLoading=!1;this._moreMenuOpen=!1;this._toastMessage="";this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab="dashboard";this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._deepLinkHandled=!1;this._onDialogEvent=async()=>{try{await this._loadData()}catch{}}}get _lang(){return this.hass?.language||"en"}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler);let e=localStorage.getItem("maintenance_supporter_sort");e&&["due_date","object","type","task_name"].includes(e)&&(this._sortMode=e)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new we(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new re(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,i,n,l]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);e&&(this._objects=e.objects),t&&(this._stats=t),i&&(this._budget=i),n&&(this._groups=n.groups||{}),l&&(this._features=l.features),this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("entry_id");if(!t)return;this._deepLinkHandled=!0;let i=e.get("task_id"),n=e.get("action"),l=window.location.pathname+window.location.hash;history.replaceState(history.state,"",l);let d=this._getObject(t);if(!d){this._showOverview();return}if(i){let p=d.tasks.find(u=>u.id===i);if(!p){this._showObject(t);return}this._showTask(t,i),n==="complete"&&requestAnimationFrame(()=>{this._openCompleteDialog(t,i,p.name,this._features.checklists?p.checklist:void 0,this._features.adaptive&&!!p.adaptive_config?.enabled)})}else this._showObject(t)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let i=await this._statsService.getDetailStats(e,t),n=new Map(this._detailStatsData);n.set(e,i),this._detailStatsData=n}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let i of this._objects)for(let n of i.tasks){let l=n.trigger_config?.entity_id;l&&e.push({entityId:l,isCounter:this._isCounterEntity(n.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let p of this._objects)for(let u of p.tasks)if(!(this._filterStatus&&u.status!==this._filterStatus)){if(this._filterUser){let m=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(u.responsible_user_id!==m)continue}e.push({entry_id:p.entry_id,task_id:u.id,object_name:p.object.name,task_name:u.name,type:u.type,schedule_type:u.schedule_type,status:u.status,days_until_due:u.days_until_due??null,next_due:u.next_due??null,trigger_active:u.trigger_active,trigger_current_value:u.trigger_current_value??null,trigger_current_delta:u.trigger_current_delta??null,trigger_config:u.trigger_config??null,trigger_entity_info:u.trigger_entity_info??null,times_performed:u.times_performed,total_cost:u.total_cost,interval_days:u.interval_days??null,interval_anchor:u.interval_anchor??null,history:u.history||[],enabled:u.enabled,nfc_tag_id:u.nfc_tag_id??null})}let t={overdue:0,triggered:1,due_soon:2,ok:3},i=(p,u)=>(t[p.status]??9)-(t[u.status]??9),n=(p,u)=>(p.days_until_due??99999)-(u.days_until_due??99999),l=(p,u)=>i(p,u)||n(p,u),d={due_date:l,object:(p,u)=>p.object_name.localeCompare(u.object_name)||l(p,u),type:(p,u)=>p.type.localeCompare(u.type)||l(p,u),task_name:(p,u)=>p.task_name.localeCompare(u.task_name)};return e.sort(d[this._sortMode]),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(n=>n.id===t)}_pushPanelState(e,t,i){let n={msp_view:e,msp_entry:t||null,msp_task:i||null};history.pushState(n,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let i=this._getTask(t.msp_entry,t.msp_task);i?.trigger_config?.entity_id&&this._fetchDetailStats(i.trigger_config.entity_id,this._isCounterEntity(i.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null;let i=this._getTask(e,t);if(i?.trigger_config?.entity_id){let n=i.trigger_config.entity_id,l=this._isCounterEntity(i.trigger_config);this._fetchDetailStats(n,l)}}_showToast(e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}async _deleteObject(e){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("delete",this._lang),message:s("confirm_delete_object",this._lang),confirmText:s("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _deleteTask(e,t){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("delete",this._lang),message:s("confirm_delete_task",this._lang),confirmText:s("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _skipTask(e,t,i){this._actionLoading=!0;try{let n={type:"maintenance_supporter/task/skip",entry_id:e,task_id:t};i&&(n.reason=i),await this.hass.connection.sendMessagePromise(n),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}finally{this._actionLoading=!1}}async _resetTask(e,t,i){this._actionLoading=!0;try{let n={type:"maintenance_supporter/task/reset",entry_id:e,task_id:t};i&&(n.date=i),await this.hass.connection.sendMessagePromise(n),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}finally{this._actionLoading=!1}}async _applySuggestion(e,t,i){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:i}),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _promptSkipTask(e,t){let i=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!i)return;let n=await i.prompt({title:s("skip",this._lang),message:s("skip_reason_prompt",this._lang),confirmText:s("skip",this._lang),inputLabel:s("reason_optional",this._lang),inputType:"text"});n.confirmed&&this._skipTask(e,t,n.value||void 0)}async _promptResetTask(e,t){let i=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!i)return;let n=await i.prompt({title:s("reset",this._lang),message:s("reset_date_prompt",this._lang),confirmText:s("reset",this._lang),inputLabel:s("reset_date_optional",this._lang),inputType:"date"});n.confirmed&&this._resetTask(e,t,n.value||void 0)}_dismissSuggestion(e,t){e&&t&&this._dismissedSuggestions.add(`${e}_${t}`),this.requestUpdate()}_openCompleteDialog(e,t,i,n,l){let d=this.shadowRoot.querySelector("maintenance-complete-dialog");d&&(d.entryId=e,d.taskId=t,d.taskName=i,d.lang=this._lang,d.checklist=n||[],d.adaptiveEnabled=!!l,d.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,i,n){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,i,n)}render(){return o`
    ...
    `}_renderDashboard(){let e=this._stats,t=this._taskRows,i=this._lang;return o`
      ...
      <div class="filter-bar">
        ...
        <select
          ...
          @change=${n=>{let l=n.target.value;this._filterUser=l||null}}
        >
      ...
      <div class="objects-grid">
        ${this._objects.map(t=>o`
          <div class="object-card" @click=${()=>this._showObject(t.entry_id)}>
            ...
            ${t.tasks.length===0?o`<div class="object-card-empty">${s("no_tasks_yet",e)}</div>`:c}
          </div>
        `)}
      </div>
    `}async _onSettingsChanged(){await this._loadData()}_renderGroupsSection(){let e=Object.entries(this._groups);if(e.length===0)return c;let t=this._lang;return o`
      <div class="groups-section">
        <h3>${s("groups",t)}</h3>
        <div class="groups-grid">
          ${e.map(([i,n])=>{let l=n.task_refs.map(d=>this._getTask(d.entry_id,d.task_id)?.name).filter(Boolean);return o`
              <div class="group-card">
                <div class="group-card-name">${n.name}</div>
                ${n.description?o`<div class="group-card-desc">${n.description}</div>`:c}
                <div class="group-card-tasks">
                  ${l.length>0?l.map(d=>o`<span class="group-task-chip">${d}</span>`):o`<span style="font-size:12px;color:var(--secondary-text-color)">${s("no_tasks_short",t)}</span>`}
                </div>
              </div>
            `})}
        </div>
      </div>
    `}_renderBudgetBar(){let e=this._budget;if(!e)return c;let t=this._lang,i=e.currency_symbol||"\u20AC",n=[];return e.monthly_budget>0&&n.push({label:s("budget_monthly",t),spent:e.monthly_spent,budget:e.monthly_budget}),e.yearly_budget>0&&n.push({label:s("budget_yearly",t),spent:e.yearly_spent,budget:e.yearly_budget}),n.length===0?c:o`
      <div class="budget-bars">
        ${n.map(l=>{let d=Math.min(100,Math.max(0,l.spent/l.budget*100)),p=d>=100?"var(--error-color, #f44336)":d>=e.alert_threshold_pct?"var(--warning-color, #ff9800)":"var(--success-color, #4caf50)";return o`
    ...
    `}_renderOverviewRow(e){let t=this._lang,i=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,n=0,l=$e.ok,d=!1;if(i&&e.days_until_due!==null){let p=(e.interval_days-e.days_until_due)/e.interval_days*100;n=Math.max(0,Math.min(100,p)),d=p>100,e.status==="overdue"?l=$e.overdue:e.status==="due_soon"&&(l=$e.due_soon)}return o`
      <div class="task-row${e.enabled?"":" task-disabled"}">
        <span class="status-badge ${e.status}">${s(e.status,t)}</span>
        ${e.enabled?c:o`<span class="badge-disabled">${s("disabled",t)}</span>`}
        ${e.nfc_tag_id?o`<span class="nfc-badge" title="${s("nfc_linked",t)}"><ha-icon icon="mdi:nfc-variant"></ha-icon></span>`:c}
        ...
        <span class="due-cell" @click=${()=>this._showTask(e.entry_id,e.task_id)}>
          ...
          ${i?o`<div class="days-bar"><div class="days-bar-fill${d?" overflow":""}" style="width:${n}%;background:${l}"></div></div>`:c}
    ...
    `}_openCompleteDialogForRow(e){let i=this._objects.find(n=>n.entry_id===e.entry_id)?.tasks.find(n=>n.id===e.task_id);this._openCompleteDialog(e.entry_id,e.task_id,e.task_name,this._features.checklists?i?.checklist:void 0,this._features.adaptive&&!!i?.adaptive_config?.enabled)}_renderTriggerProgress(e){let t=e.trigger_config??null;if(!t)return c;let i=t.type||"threshold",n=e.trigger_entity_info?.unit_of_measurement??"",l=0,d="";if(i==="threshold"){let m=e.trigger_current_value??null;if(m==null)return c;let v=t.trigger_above,b=t.trigger_below;if(v!=null){let f=b??0,k=v-f||1;l=Math.min(100,Math.max(0,(m-f)/k*100)),d=`${m.toFixed(1)} / ${v} ${n}`}else if(b!=null){let k=e.trigger_entity_info?.max??(b*2||100),T=k-b||1;l=Math.min(100,Math.max(0,(k-m)/T*100)),d=`${m.toFixed(1)} / ${b} ${n}`}else return c}else if(i==="counter"){let m=t.trigger_target_value||1,b=e.trigger_current_delta??null??e.trigger_current_value??null;if(b==null)return c;l=Math.min(100,Math.max(0,b/m*100)),d=`${b.toFixed(1)} / ${m} ${n}`}else if(i==="state_change"){let m=t.trigger_target_changes||1,v=e.trigger_current_value??null;if(v==null)return c;l=Math.min(100,Math.max(0,v/m*100)),d=`${Math.round(v)} / ${m}`}else if(i==="runtime"){let m=t.trigger_runtime_hours||100,v=e.trigger_current_value??null;if(v==null)return c;l=Math.min(100,Math.max(0,v/m*100)),d=`${v.toFixed(1)}h / ${m}h`}else if(i==="compound"){let m=t.compound_logic||t.operator||"AND",v=t.conditions?.length||0;d=`${m} (${v})`,l=e.trigger_active?100:0}else return c;let p=l>=100,u=l>90?"var(--error-color, #f44336)":l>70?"var(--warning-color, #ff9800)":"var(--primary-color)";return o`
      <div class="trigger-progress">
        <div class="trigger-progress-bar">
          <div class="trigger-progress-fill${p?" overflow":""}" style="width:${l}%;background:${u}"></div>
        </div>
        <span class="trigger-progress-label">${d}</span>
      </div>
    `}_renderMiniSparkline(e){if(!e.trigger_config?.entity_id)return c;let t=e.trigger_config.entity_id,i=this._miniStatsData.get(t)||[],n=[];if(i.length>=2)n=i.map(y=>({ts:y.ts,val:y.val}));else{if(!e.history)return c;for(let y of e.history)y.trigger_value!=null&&n.push({ts:new Date(y.timestamp).getTime(),val:y.trigger_value})}if(e.trigger_current_value!=null&&n.push({ts:Date.now(),val:e.trigger_current_value}),n.length<2)return c;n.sort((y,z)=>y.ts-z.ts);let l=bi,d=yi,p=n.map(y=>y.val),u=Math.min(...p),m=Math.max(...p),v=m-u||1;u-=v*.1,m+=v*.1;let b=n[0].ts,k=n[n.length-1].ts-b||1,T=y=>(y-b)/k*l,H=y=>2+(1-(y-u)/(m-u))*(d-4),C=n;if(C.length>$t){let y=Math.ceil(C.length/$t);C=C.filter((z,O)=>O%y===0||O===C.length-1)}let A=C.map(y=>`${T(y.ts).toFixed(1)},${H(y.val).toFixed(1)}`).join(" ");return o`
      <svg class="mini-sparkline" viewBox="0 0 ${l} ${d}" preserveAspectRatio="none" role="img" aria-label="${s("chart_mini_sparkline",this._lang)}">
        <polyline points="${A}" fill="none" stroke="var(--primary-color)" stroke-width="1.5" stroke-linejoin="round" />
      </svg>
    `}_renderDaysProgress(e){let t=this._lang;if(e.days_until_due==null||!e.interval_days||e.interval_days<=0)return c;let n=(e.interval_days-e.days_until_due)/e.interval_days*100,l=Math.max(0,Math.min(100,n)),d=n>100,p="var(--success-color, #4caf50)";return e.status==="overdue"?p="var(--error-color, #f44336)":e.status==="due_soon"&&(p="var(--warning-color, #ff9800)"),o`
      <div class="days-progress">
        ...
        <div class="days-progress-bar" role="progressbar" aria-valuenow="${Math.round(l)}" aria-valuemin="0" aria-valuemax="100" aria-label="${s("days_progress",t)}">
          <div class="days-progress-fill${d?" overflow":""}" style="width:${l}%;background:${p}"></div>
        </div>
    ...
    `}_renderObjectDetail(){if(!this._selectedEntryId)return c;let e=this._getObject(this._selectedEntryId);if(!e)return o`<p>Object not found.</p>`;let t=e.object,i=this._lang;return o`
      <div class="detail-section">
        ...
        ${t.serial_number?o`<p class="meta">${s("serial_number_label",i)}: ${t.serial_number}</p>`:c}
        ${t.installation_date?o`<p class="meta">${s("installed",i)}: ${Q(t.installation_date,i)}</p>`:c}
        ...
        ${e.tasks.length===0?o`<div class="empty-state-centered">
              ...
              <div class="task-row${n.enabled?"":" task-disabled"}">
                ...
                ${n.enabled?c:o`<span class="badge-disabled">${s("disabled",i)}</span>`}
                ${n.nfc_tag_id?o`<span class="nfc-badge" title="${s("nfc_linked",i)}"><ha-icon icon="mdi:nfc-variant"></ha-icon></span>`:c}
                ...
                <span class="due-cell" @click=${()=>this._showTask(e.entry_id,n.id)}>
                  <span class="due-text">${ke(n.days_until_due,i)}</span>
                  ${n.trigger_config?this._renderTriggerProgress(n):c}
                  ${this._renderMiniSparkline(n)}
                </span>
                <span class="row-actions">
                  <mwc-icon-button class="btn-complete" title="${s("complete",i)}" @click=${l=>{l.stopPropagation(),this._openCompleteDialog(e.entry_id,n.id,n.name,this._features.checklists?n.checklist:void 0,this._features.adaptive&&!!n.adaptive_config?.enabled)}}>
                    <ha-icon icon="mdi:check"></ha-icon>
                  </mwc-icon-button>
                  <mwc-icon-button class="btn-skip" title="${s("skip",i)}" .disabled=${this._actionLoading} @click=${l=>{l.stopPropagation(),this._promptSkipTask(e.entry_id,n.id)}}>
                    <ha-icon icon="mdi:skip-next"></ha-icon>
                  </mwc-icon-button>
                </span>
              </div>
    ...
    `}_renderTaskHeader(e){let t=this._lang,n=this._getObject(this._selectedEntryId)?.object.name||"",l=e.status==="due_soon"?"warning":e.status||"ok",d=s(e.status||"ok",t);return o`
      <div class="task-header">
        <div class="task-header-title">
          ...
          ${e.nfc_tag_id?o`<span class="nfc-badge" title="${s("nfc_tag_id",t)}: ${e.nfc_tag_id}"><ha-icon icon="mdi:nfc-variant"></ha-icon> NFC</span>`:o`<span class="nfc-badge unlinked" title="${s("nfc_link_hint",t)}"
    ...
    `}_renderKPIBar(e){let t=this._lang,i=e.times_performed>0?e.total_cost/e.times_performed:0,n=e.days_until_due!==null&&e.days_until_due!==void 0?e.days_until_due<0?"overdue":e.days_until_due<=e.warning_days?"warning":"":"";return o`
      <div class="kpi-bar">
        <div class="kpi-card">
    ...
    `}_renderRecommendationCard(e){let t=this._lang;if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return c;if(this._selectedEntryId&&this._selectedTaskId&&this._dismissedSuggestions.has(`${this._selectedEntryId}_${this._selectedTaskId}`))return c;let i=e.interval_days,n=e.suggested_interval,l=e.interval_confidence||"medium",d=Math.max(i||1,n);return o`
        ...
        <div class="interval-comparison">
          ...
          <div class="interval-bar">
            <div class="interval-label">${s("recommended",t)}: ${n} ${s("days",t)}
              <span class="confidence-badge ${l}">${s(`confidence_${l}`,t)}</span>
            </div>
    ...
    `}_renderRecentActivities(e){let t=this._lang,i=e.history.slice(0,3);if(i.length===0)return c;let n=l=>{switch(l){case"completed":return"\u2713";case"triggered":return"\u2297";case"skipped":return"\u21B7";case"reset":return"\u21BA";default:return"\xB7"}};return o`
      <div class="recent-activities">
        <h3>${s("recent_activities",t)}</h3>
        ${i.map(l=>o`
          <div class="activity-item">
            <span class="activity-icon">${n(l.type)}</span>
            <span class="activity-date">${He(l.timestamp,t)}</span>
    ...
    `}_renderHistoryFilters(e){let t=this._lang;return o`
      <div class="history-filters-new">
        <div class="filter-chips">
          ${["completed","skipped","reset","triggered"].map(i=>{let n=e.history.filter(l=>l.type===i).length;return n===0?c:o`
              <span class="filter-chip ${this._historyFilter===i?"active":""}"
                @click=${()=>{this._historyFilter=this._historyFilter===i?null:i}}>
                ${s(i,t)} (${n})
              </span>
            `})}
          ${this._historyFilter?o`<span class="filter-chip clear" @click=${()=>{this._historyFilter=null}}>${s("show_all",t)}</span>`:c}
        </div>
    ...
    `}_renderHistoryList(e){let t=this._lang,i=this._historyFilter?e.history.filter(n=>n.type===this._historyFilter):e.history;if(this._historySearch){let n=this._historySearch.toLowerCase();i=i.filter(l=>l.notes?.toLowerCase().includes(n))}return i.length===0?o`<p class="empty">${s("no_history",t)}</p>`:o`
      ...
      <div class="history-entry">
        ...
        <div class="history-content">
          ...
          <div class="history-details">
            ${e.cost!=null?o`<span>${s("cost",t)}: ${e.cost.toFixed(2)} ${this._budget?.currency_symbol||"\u20AC"}</span>`:c}
            ${e.duration!=null?o`<span>${s("duration",t)}: ${e.duration} min</span>`:c}
            ${e.trigger_value!=null?o`<span>${s("trigger_val",t)}: ${e.trigger_value}</span>`:c}
          </div>
        </div>
      </div>
