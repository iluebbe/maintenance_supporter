...
`;var ds={days:1,weeks:7,months:30.4368,years:365.25};function zt(r,n){return!r||r<=0?0:r*(ds[n||"days"]??1)}function ct(r,n,e){let t=zt(r,e);if(t<=0||n==null)return{pct:0,overflow:!1};let i=(t-n)/t*100;return{pct:Math.max(0,Math.min(100,i)),overflow:i>100}}function z(r){return String(r??"").replace(/[&<>"']/g,n=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[n])}function pi(r,n,e,t,i,a){let l=[[e.manufacturer,r.manufacturer],[e.model,r.model],[e.serial,r.serial_number],[e.installed,r.installation_date?t(r.installation_date):null],[e.warranty,r.warranty_expiry?t(r.warranty_expiry):null]].filter(([,h])=>!!h),c=n.map(h=>{let g=e.scheduleLabel(h);return`<tr>
      <td>${z(h.name)}</td>
      <td>${z(e.typeLabel(h.type))}</td>
      <td>${z(e.statusLabel(h.status))}</td>
      <td>${z(g)}</td>
      <td>${z(h.last_performed?t(h.last_performed):e.none)}</td>
      <td>${z(h.next_due?t(h.next_due):e.none)}</td>
      <td class="num">${h.times_performed??0}</td>
      <td class="num">${(h.total_cost??0).toFixed(2)} ${z(i)}</td>
    </tr>`}).join(""),u=n.reduce((h,g)=>h+(g.total_cost??0),0);return`<!DOCTYPE html><html><head><meta charset="utf-8">
...
<style>
  * { box-sizing: border-box; }
  ...
  ${l.length?`<div class="meta">${l.map(([h,g])=>`<div><div class="k">${z(h)}</div>${z(g)}</div>`).join("")}</div>`:""}
  ...
  <table>
    ...
    <tfoot><tr><td colspan="7">${z(e.totalCost)}</td><td class="num">${u.toFixed(2)} ${z(i)}</td></tr></tfoot>
  </table>
  ${r.notes?`<div class="notes"><strong>${z(e.notes)}:</strong>
${z(r.notes)}</div>`:""}
</body></html>`}function Ht(r,n=new Date){if(!r)return{kind:"none",days:null,date:null};let e=new Date(`${r}T00:00:00`);if(isNaN(e.getTime()))return{kind:"none",days:null,date:null};let t=Date.UTC(n.getFullYear(),n.getMonth(),n.getDate()),i=Date.UTC(e.getFullYear(),e.getMonth(),e.getDate()),a=Math.round((i-t)/864e5);return a<0?{kind:"expired",days:a,date:r}:a<=60?{kind:"expiring",days:a,date:r}:{kind:"valid",days:a,date:r}}var Ce=[{key:"name",labelKey:"name",required:!0},{key:"manufacturer",labelKey:"manufacturer"},{key:"model",labelKey:"model"},{key:"serial_number",labelKey:"serial_number_label"},{key:"installation_date",labelKey:"installed"},{key:"warranty_expiry",labelKey:"warranty"},{key:"area_id",labelKey:"area"},{key:"documentation_url",labelKey:"documentation_url_label"},{key:"notes",labelKey:"object_notes_label"},{key:"task_count",labelKey:"tasks"},{key:"actions",labelKey:"actions"}],ps=Ce.map(r=>r.key),dt=["name","manufacturer","model","serial_number","installation_date","warranty_expiry","area_id","task_count","actions"];function We(r){if(!Array.isArray(r))return[...dt];let n=new Set,e=[];for(let t of r)typeof t=="string"&&ps.includes(t)&&!n.has(t)&&(n.add(t),e.push(t));return e.length?(e.includes("name")||e.unshift("name"),e):[...dt]}function pt(r,n,e){let t=new Blob([r],{type:e}),i=URL.createObjectURL(t),a=document.createElement("a");a.href=i,a.download=n,a.target="_blank",a.rel="noopener",a.style.display="none",document.body.appendChild(a),a.dispatchEvent(new MouseEvent("click")),document.body.removeChild(a),setTimeout(()=>URL.revokeObjectURL(i),6e4)}function ut(r,n){let e=document.createElement("a");e.href=r,e.download=n,e.target="_blank",e.rel="noopener",e.style.display="none",document.body.appendChild(e),e.dispatchEvent(new MouseEvent("click")),document.body.removeChild(e)}var O=r=>String(r??"").replace(/[&<>"']/g,n=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[n]);function ui(r,n,e,t,i,a,l,c,u,h=[]){let g=[[e.object,O(n)],[e.type,O(e.typeLabel(r.type))],[e.interval,O(i(r))],[e.nextDue,r.next_due?O(t(r.next_due)):"\u2014"],[e.lastDone,r.last_performed?O(t(r.last_performed)):O(e.never)]];r.priority&&r.priority!=="normal"&&g.push([e.priority,O(r.priority)]);let m=(r.checklist||[]).map(f=>`<li><span class="box"></span>${O(f)}</li>`).join(""),v=(f,$)=>f?`<figure class="qr"><img src="${f}" alt="" /><figcaption>${O($)}</figcaption></figure>`:"";return`<!DOCTYPE html>
...
<body>
  ...
  ${h.length?`<h2>${O(e.parts)}</h2><ul class="check">${h.map(f=>`<li><span class="box"></span>${O(f)}</li>`).join("")}</ul>`:""}
...
`;var ht=class{constructor(n){this._cache=new Map;this._pending=new Map;this._hass=n}updateHass(n){this._hass=n}async getDetailStats(n,e,t=30){return this._getStats(n,t<=35?"hour":"day",t,e)}async getMiniStats(n,e){return this._getStats(n,"day",14,e)}async getBatchMiniStats(n){let e=new Map,t=[];for(let u of n){let h=`${u.entityId}:day:14`,g=this._cache.get(h);g&&Date.now()-g.fetchedAt<3e5?e.set(u.entityId,g.points):t.push(u)}if(t.length===0)return e;let i=t.filter(u=>u.isCounter).map(u=>u.entityId),a=t.filter(u=>!u.isCounter).map(u=>u.entityId),l=new Date(Date.now()-336*60*60*1e3).toISOString(),c=[];return i.length>0&&c.push(this._fetchBatch(i,"day",l,["state","sum","change"],!0,e)),a.length>0&&c.push(this._fetchBatch(a,"day",l,["mean","min","max"],!1,e)),await Promise.all(c),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(n,e,t,i){let a=`${n}:${e}:${t}`,l=this._cache.get(a);if(l&&Date.now()-l.fetchedAt<3e5)return l.points;if(this._pending.has(a))return this._pending.get(a);let c=this._fetchAndNormalize(n,e,t,i,a);this._pending.set(a,c);try{return await c}finally{this._pending.delete(a)}}async _fetchAndNormalize(n,e,t,i,a){let l=new Date(Date.now()-t*24*60*60*1e3).toISOString(),c=i?["state","sum","change"]:["mean","min","max"];try{let h=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:l,statistic_ids:[n],period:e,types:c}))[n]||[],g=this._normalizeRows(h,i);return this._cache.set(a,{entityId:n,fetchedAt:Date.now(),period:e,points:g}),g}catch(u){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${n}:`,u),[]}}async _fetchBatch(n,e,t,i,a,l){try{let c=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:n,period:e,types:i});for(let u of n){let h=c[u]||[],g=this._normalizeRows(h,a);l.set(u,g),this._cache.set(`${u}:${e}:14`,{entityId:u,fetchedAt:Date.now(),period:e,points:g})}}catch(c){console.warn("[maintenance-supporter] Batch statistics fetch failed:",c)}}_normalizeRows(n,e){let t=[];for(let i of n){let a=null;if(e?a=i.state??null:a=i.mean??null,a===null)continue;let l={ts:i.start,val:a};e||(i.min!=null&&(l.min=i.min),i.max!=null&&(l.max=i.max)),t.push(l)}return t.sort((i,a)=>i.ts-a.ts),t}};var me=class{constructor(n){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=n}updateHass(n){this.hass=n}async getUsers(n=!1){let e=Date.now();if(!n&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(n,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:n,task_id:e,user_id:t})}async getTasksByUser(n){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:n})).tasks}getUserName(n){return!n||!this.usersCache?null:this.usersCache.find(t=>t.id===n)?.name||null}getUser(n){return!n||!this.usersCache?null:this.usersCache.find(e=>e.id===n)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(n){return n?n===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var K=class extends A{constructor(){super(...arguments);this.label="";this.value="";this.placeholder="";this.type="text";this.required=!1;this.disabled=!1}_onInput(e){let t=e.target.value;this.value=t,this.dispatchEvent(new CustomEvent("input",{bubbles:!0,composed:!0,detail:{value:t}}))}render(){return o`
  ...
  `,d([y({attribute:!1})],F.prototype,"hass",2),d([y({attribute:!1})],F.prototype,"objects",2),d([_()],F.prototype,"_open",2),d([_()],F.prototype,"_loading",2),d([_()],F.prototype,"_error",2),d([_()],F.prototype,"_name",2),d([_()],F.prototype,"_manufacturer",2),d([_()],F.prototype,"_model",2),d([_()],F.prototype,"_serialNumber",2),d([_()],F.prototype,"_areaId",2),d([_()],F.prototype,"_installationDate",2),d([_()],F.prototype,"_warrantyExpiry",2),d([_()],F.prototype,"_documentationUrl",2),d([_()],F.prototype,"_notes",2),d([_()],F.prototype,"_haDeviceId",2),d([_()],F.prototype,"_parentEntryId",2),d([_()],F.prototype,"_entryId",2);customElements.get("maintenance-object-dialog")||customElements.define("maintenance-object-dialog",F);function oe(r){let n=r??0;return n<1024?`${n} B`:n<1024*1024?`${(n/1024).toFixed(1)} KB`:`${(n/(1024*1024)).toFixed(1)} MB`}var _t=["manual","warranty","invoice","spare_parts","photo","other"],gs={manual:"mdi:book-open-variant",warranty:"mdi:shield-check",invoice:"mdi:receipt-text-outline",spare_parts:"mdi:cog-outline",photo:"mdi:image-outline",other:"mdi:file-document-outline"},q=class extends A{constructor(){super(...arguments);this.canWrite=!1;this._docs=[];this._loaded=!1;this._busy=!1;this._error="";this._hint="";this._addingLink=!1;this._linkUrl="";this._linkTitle="";this._category="manual";this._thumbs={};this._lightboxUrl="";this._editingId="";this._editTitle="";this._editCategory="manual";this._dragOver=!1;this._loadedFor=null;this._localeReady=!1}_isImage(e){return e.kind==="file"&&(e.mime||"").startsWith("image/")}async _sign(e){return(await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300})).path}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),this.hass&&!this._localeReady&&(this._localeReady=!0,ie(this._lang).then(()=>this.requestUpdate())),this.hass&&this.entryId&&this._loadedFor!==this.entryId&&(this._loadedFor=this.entryId,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:this.entryId});this._docs=e.documents||[],this._loaded=!0,this._error="",this._thumbs={},this._loadThumbs()}catch(e){this._error=M(e,this._lang),this._loaded=!0}}async _loadThumbs(){await Promise.all(this._docs.filter(e=>this._isImage(e)).map(async e=>{try{let t=await this._sign(e);this._thumbs={...this._thumbs,[e.id]:t}}catch{}}))}_category_of(e){return(e.tags||[]).find(i=>_t.includes(i))||"other"}_labelKeydown(e){(e.key==="Enter"||e.key===" ")&&(e.preventDefault(),e.currentTarget.querySelector("input")?.click())}_onFileInput(e){let t=e.target,i=Array.from(t.files??[]);i.length&&this._uploadFiles(i),t.value=""}_onCameraInput(e){let t=e.target,i=Array.from(t.files??[]);i.length&&this._uploadFiles(i,"photo"),t.value=""}_onDrop(e){if(e.preventDefault(),this._dragOver=!1,!this.canWrite||this._busy)return;let t=Array.from(e.dataTransfer?.files??[]);t.length&&this._uploadFiles(t)}_onDragOver(e){this.canWrite&&(e.preventDefault(),this._dragOver=!0)}_onDragLeave(e){let t=e.relatedTarget;(!t||!e.currentTarget.contains(t))&&(this._dragOver=!1)}async _uploadFiles(e,t){let i=t??this._category;this._busy=!0,this._error="",this._hint="";let a=0,l=0;try{for(let c of e){let u=new FormData;u.append("entry_id",this.entryId),u.append("tags",i),u.append("file",c,c.name);let h=await fetch("/api/maintenance_supporter/document/upload",{method:"POST",headers:{Authorization:`Bearer ${this.hass.auth?.data?.access_token??""}`},body:u});if(!h.ok){this._error=h.status===413?s("doc_too_large",this._lang):s("doc_upload_failed",this._lang);continue}let g=await h.json();g.duplicate_in_object?l++:g.deduped&&a++}l?this._hint=s("doc_dup_in_object",this._lang):a&&(this._hint=s("doc_deduped",this._lang)),await this._load()}catch{this._error=s("doc_upload_failed",this._lang)}finally{this._busy=!1}}async _download(e){try{let t=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:30});ut(t.path,e.filename||e.title||"document")}catch(t){this._error=M(t,this._lang)}}async _preview(e){if(this._isImage(e)){this._lightboxUrl=this._thumbs[e.id]||await this._sign(e);return}let t=window.open("about:blank","_blank");try{let i=await this._sign(e);t&&(t.location.href=new URL(i,window.location.origin).href)}catch(i){t&&t.close(),this._error=M(i,this._lang)}}_openDoc(e){e.kind==="file"?this._preview(e):e.url&&/^https?:\/\//i.test(e.url)&&window.open(e.url,"_blank","noopener")}_startEdit(e){this._editingId=e.id,this._editTitle=e.title||"",this._editCategory=this._category_of(e),this._addingLink=!1,this._error=""}_cancelEdit(){this._editingId=""}async _saveEdit(e){let t=(e.tags||[]).filter(a=>!_t.includes(a)),i=e.kind==="file"?[this._editCategory,...t]:e.tags??[];this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,title:this._editTitle.trim()||e.filename||e.url||"",tags:i}),this._editingId="",await this._load()}catch(a){this._error=M(a,this._lang)}finally{this._busy=!1}}async _delete(e){let t=e.title||e.filename||e.url||"";if(window.confirm(s("doc_delete_confirm",this._lang).replace("{name}",t))){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/delete",doc_id:e.id}),await this._load()}catch(i){this._error=M(i,this._lang)}finally{this._busy=!1}}}async _addLink(){let e=this._linkUrl.trim();if(e){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/add_link",entry_id:this.entryId,url:e,title:this._linkTitle.trim()||null}),this._linkUrl="",this._linkTitle="",this._addingLink=!1,await this._load()}catch(t){this._error=M(t,this._lang,s("doc_link_invalid",this._lang))}finally{this._busy=!1}}}render(){let e=this._lang;return o`
      ...
      <div class="doc-header">
        ...
        ${this.canWrite?o`
              <div class="doc-actions">
                ...
                >
                  ${_t.map(t=>o`<option value=${t}>${s(`doc_cat_${t}`,e)}</option>`)}
                </select>
                <label
    ...
    `}_renderDoc(e,t){if(this._editingId===e.id)return this._renderEdit(e,t);let i=e.kind==="file",a=this._category_of(e),l=i?`${s(`doc_cat_${a}`,t)} \xB7 ${oe(e.size)}`:s("doc_link_badge",t),c=this._thumbs[e.id];return o`
      <div class="doc-row">
        ${i&&c?o`<img
              class="doc-thumb"
              src=${c}
    ...
    `}_renderEdit(e,t){let i=e.kind==="file";return o`
      <div class="doc-row editing">
        ...
        ${i?o`<select
            ...
            >
              ${_t.map(a=>o`<option value=${a} ?selected=${a===this._editCategory}>${s(`doc_cat_${a}`,t)}</option>`)}
            </select>`:p}
              ...
              ${this._restockFor===e.id?o`
                  ...
                  `:o`
                    <ha-icon-button
                      ...
                      .disabled=${this._busy}
                      @click=${()=>{this._restockFor=e.id,this._restockQty=String(e.restock_quantity||1)}}
  ...
  `,d([y({attribute:!1})],J.prototype,"hass",2),d([y({attribute:!1})],J.prototype,"entryId",2),d([y({attribute:!1})],J.prototype,"parts",2),d([y({type:Boolean})],J.prototype,"canWrite",2),d([_()],J.prototype,"_editing",2),d([_()],J.prototype,"_busy",2),d([_()],J.prototype,"_error",2),d([_()],J.prototype,"_restockFor",2),d([_()],J.prototype,"_restockQty",2);customElements.define("maintenance-parts-section",J);var vs=["manual","warranty","invoice","spare_parts","photo","other"],bs={manual:"mdi:book-open-variant",warranty:"mdi:shield-check",invoice:"mdi:receipt-text-outline",spare_parts:"mdi:cog-outline",photo:"mdi:image-outline",other:"mdi:file-document-outline"},Z=class extends A{constructor(){super(...arguments);this.canWrite=!1;this._docs=[];this._loaded=!1;this._busy=!1;this._error="";this._attachId="";this._loadedKey="";this._localeReady=!1}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),this.hass&&!this._localeReady&&(this._localeReady=!0,ie(this._lang).then(()=>this.requestUpdate()));let t=`${this.entryId}|${this.taskId}`;this.hass&&this.entryId&&this.taskId&&this._loadedKey!==t&&(this._loadedKey=t,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:this.entryId});this._docs=e.documents||[],this._loaded=!0,this._error=""}catch(e){this._error=M(e,this._lang),this._loaded=!0}}_linked(){return this._docs.filter(e=>(e.task_ids||[]).includes(this.taskId))}_available(){return this._docs.filter(e=>!(e.task_ids||[]).includes(this.taskId))}async _setTaskIds(e,t){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,task_ids:t}),await this._load()}catch(i){this._error=M(i,this._lang)}finally{this._busy=!1}}_link(){let e=this._docs.find(t=>t.id===this._attachId);e&&(this._attachId="",this._setTaskIds(e,[...e.task_ids||[],this.taskId]))}_unlink(e){this._setTaskIds(e,(e.task_ids||[]).filter(t=>t!==this.taskId))}_isPdf(e){return e.mime==="application/pdf"||(e.filename||"").toLowerCase().endsWith(".pdf")}_pageFor(e){return this._isPdf(e)?e.task_pages?.[this.taskId]:void 0}async _open(e){if(e.kind==="weblink"){window.open(e.url,"_blank","noopener");return}let t=this._pageFor(e),i=t?`#page=${t}`:"",a=window.open("about:blank","_blank");try{let l=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300});a&&(a.location.href=new URL(l.path+i,window.location.origin).href)}catch(l){a&&a.close(),this._error=M(l,this._lang)}}async _setPage(e,t){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,task_pages:{[this.taskId]:t}}),await this._load()}catch(i){this._error=M(i,this._lang)}finally{this._busy=!1}}async _download(e){try{let t=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:30});ut(t.path,e.filename||e.title||"document")}catch(t){this._error=M(t,this._lang)}}render(){if(!this._loaded||this._docs.length===0)return p;let e=this._lang,t=this._linked(),i=this._available();return o`
    ...
    `}_renderRow(e,t){let i=e.kind==="file",a=this._isPdf(e),l=e.task_pages?.[this.taskId],c=(e.tags||[]).find(h=>vs.includes(h))||"other",u=i?oe(e.size):s("doc_link_badge",t);return o`
      <div class="tdoc-row">
        <ha-icon class="tdoc-icon" icon=${i?bs[c]:"mdi:link-variant"}></ha-icon>
        <div
          class="tdoc-info"
          role="button"
          tabindex="0"
        ...
        >
          ...
          <div class="tdoc-meta">
            ${u}${l?o` · <span class="tdoc-pagetag">${s("doc_page",t)} ${l}</span>`:p}
          </div>
        </div>
...
`).map(u=>u.trim()).filter(Boolean).slice(0,100);e.checklist=c.length?c:null}if(this.completionActionsEnabled){let c=this._actionService.trim();if(c&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(c)){let m={service:c},v=this._actionTargetEntity.trim();v&&(m.target={entity_id:v});let f=this._buildActionData();Object.keys(f).length>0&&(m.data=f),e.on_complete_action=m}else e.on_complete_action=null;let u={};this._qcNotes.trim()&&(u.notes=this._qcNotes.trim());let h=parseFloat(this._qcCost);!isNaN(h)&&h>=0&&(u.cost=h);let g=parseInt(this._qcDuration,10);!isNaN(g)&&g>=0&&(u.duration=g),this._qcFeedback&&(u.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(u).length?u:null}let i=await this.hass.connection.sendMessagePromise(e),a=this._taskId||i?.task_id,l=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(a&&this._scheduleType==="sensor_based"&&l)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:a,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=M(e,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return p;let e=this._lang,t=this._triggerType==="compound";return o`
      ...
      <div class="select-row">
        ...
        >
          ${$s.map(i=>o`<option value=${i} ?selected=${i===this._triggerType}>${s(i,e)}</option>`)}
        </select>
      </div>
    ...
    `}_renderCondition(e,t){let i=this._lang,a=t+1;return o`
      <div class="compound-condition">
        ...
        <div class="select-row">
          ...
          >
            ${gi.map(l=>o`<option value=${l} ?selected=${l===e.type}>${s(l,i)}</option>`)}
          </select>
        </div>
    ...
    `}_renderConditionTypeFields(e,t){let i=this._lang;return e.type==="threshold"?o`
      ...
      ${this._endsMode==="until"?o`
        ...
        ></ms-textfield>`:p}
    `}_renderCalendarFields(){let e=this._lang,t=Ts(e);if(this._scheduleType==="weekdays")return o`
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${ys.map(i=>o`<option value=${i} ?selected=${i===this._priority}>${s("priority_"+i,e)}</option>`)}
            </select>
          </div>
          <div class="field">
          ...
          <div class="select-row">
            ...
            >
              ${xs.map(i=>o`<option value=${i} ?selected=${i===this._scheduleType}>${s(i,e)}</option>`)}
            </select>
          </div>
...
<div class="qr-row">
  <div class="qr-col">
    <img src="${mi(this._viewResult.svg_data_uri)}" alt="QR Info" />
    <div class="qr-label">${g}</div>
  </div>
  ${h?`<div class="qr-col">
    <img src="${mi(this._completeResult.svg_data_uri)}" alt="QR Complete" />
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `,d([y({attribute:!1})],se.prototype,"hass",2),d([y()],se.prototype,"lang",2),d([_()],se.prototype,"_open",2),d([_()],se.prototype,"_loading",2),d([_()],se.prototype,"_error",2),d([_()],se.prototype,"_viewResult",2),d([_()],se.prototype,"_completeResult",2),d([_()],se.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",se);var vi=5;function Ve(r){let n=r.getFullYear(),e=String(r.getMonth()+1).padStart(2,"0"),t=String(r.getDate()).padStart(2,"0");return`${n}-${e}-${t}`}function js(r,n){let e=[];for(let t=0;t<n;t++){let i=new Date(r);i.setDate(i.getDate()+t),i.setHours(0,0,0,0),e.push(Ve(i))}return e}function gt(r,n){let[e,t,i]=r.split("-").map(Number),a=new Date(e,t-1,i);return a.setDate(a.getDate()+n),Ve(a)}function Cs(r){if(!r||r.length===0)return null;let n=r.map(e=>e.cost).filter(e=>typeof e=="number");return n.length===0?null:n.reduce((e,t)=>e+t,0)/n.length}function Ms(r){let{windowStart:n,windowEnd:e,task:t,entryId:i,objectName:a}=r,l=[],c=(m,v)=>({date:m,entry_id:i,task_id:t.id,task_name:t.name,object_name:a,status:v&&(t.status==="overdue"||t.status==="triggered")?"ok":t.status,days_until_due:v?null:t.days_until_due??null,projected:v,schedule_type:t.schedule_type,interval_days:t.interval_days??null,interval_unit:t.interval_unit??null,responsible_user_id:t.responsible_user_id??null,avg_cost:Cs(t.history),adaptive_enabled:!!t.adaptive_config?.enabled,prediction_confidence:t.threshold_prediction_confidence??null}),u=Math.max(1,Math.round(zt(t.interval_days,t.interval_unit)));if(t.status==="overdue"||t.status==="triggered"){if(l.push(c(n,!1)),t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let m=gt(n,u),v=1;for(;m<=e&&v<vi;)l.push(c(m,!0)),v++,m=gt(m,u)}return l}let h=t.next_due;if(typeof h!="string"||!h)return l;let g=h.slice(0,10);if(g>=n&&g<=e)l.push(c(g,!1));else if(g>e)return l;if(t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let m=gt(g,u),v=l.length;for(;m<=e&&v<vi;)m>=n&&(l.push(c(m,!0)),v++),m=gt(m,u)}return l}var bi={overdue:0,triggered:1,due_soon:2,ok:3};function fi(r,n,e,t=null){let i=js(n,e),a=i[0],l=i[i.length-1],c=[];for(let h of r){let g=h.object?.name||"",m=h.entry_id,v=h.tasks||[];for(let f of v){if(t&&f.responsible_user_id!==t||f.enabled===!1)continue;let $=Ms({windowStart:a,windowEnd:l,task:f,entryId:m,objectName:g});c.push(...$)}}let u=new Map;for(let h of i)u.set(h,[]);for(let h of c){let g=u.get(h.date);g&&g.push(h)}for(let[,h]of u)h.sort((g,m)=>{let v=bi[g.status]??99,f=bi[m.status]??99;if(v!==f)return v-f;if(g.projected!==m.projected)return g.projected?1:-1;let $=g.object_name.localeCompare(m.object_name);return $!==0?$:g.task_name.localeCompare(m.task_name)});return i.map(h=>({date:h,events:u.get(h)??[]}))}var Rs={completed:"ok",reset:"ok",skipped:"due_soon",triggered:"triggered",trigger_replaced:"triggered",trigger_removed:"ok"};function Is(r,n){let e=[];for(let t=n-1;t>=0;t--){let i=new Date(r);i.setDate(i.getDate()-t),i.setHours(0,0,0,0),e.push(Ve(i))}return e}function yi(r,n,e,t=null){let i=Is(n,e),a=i[0],l=i[i.length-1],c=new Map;for(let h of i)c.set(h,[]);for(let h of r){let g=h.object?.name||"",m=h.entry_id,v=h.tasks||[];for(let f of v){if(t&&f.responsible_user_id!==t)continue;let $=f.history||[];for(let b of $){if(typeof b?.timestamp!="string")continue;let S=b.timestamp.slice(0,10);if(S<a||S>l)continue;let j=c.get(S);if(!j)continue;let L=b.type??"completed";j.push({date:S,entry_id:m,task_id:f.id,task_name:f.name,object_name:g,status:Rs[L]??"ok",days_until_due:null,projected:!1,schedule_type:f.schedule_type,interval_days:f.interval_days??null,responsible_user_id:f.responsible_user_id??null,avg_cost:typeof b.cost=="number"?b.cost:null,adaptive_enabled:!!f.adaptive_config?.enabled,prediction_confidence:null,history_timestamp:b.timestamp,history_type:L,history_cost:typeof b.cost=="number"?b.cost:null,history_notes:typeof b.notes=="string"?b.notes:null,history_duration:typeof b.duration=="number"?b.duration:null})}}}let u={completed:0,reset:1,skipped:2,triggered:3,trigger_replaced:4};for(let[,h]of c)h.sort((g,m)=>{let v=u[g.history_type??""]??99,f=u[m.history_type??""]??99;if(v!==f)return v-f;let $=g.object_name.localeCompare(m.object_name);return $!==0?$:g.task_name.localeCompare(m.task_name)});return i.map(h=>({date:h,events:c.get(h)??[]}))}var xi=C`
  .cal-controls {
    display: flex;
    gap: 12px;
    align-items: center;
...
`;var ae=class extends A{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter)}getCardSize(){return 6}get _lang(){return this.hass?.language||"en"}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){super.updated(e);let t=this.hass?.language;if(t&&!rt(t)&&ie(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{let e=await this.hass.connection.subscribeMessage(t=>{let i=t;this._objects=i.objects},{type:"maintenance_supporter/subscribe"});if(!this.isConnected){e();return}this._unsub=e}catch{}}_onEventClick(e){if(e.history_timestamp){this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}));return}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}render(){if(!this.hass)return p;let e=this._lang,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.title,l=null;this._userFilter&&(l=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let c=new Date;c.setHours(0,0,0,0);let u=this._pastDays>0,h=u?yi(this._objects,c,this._pastDays,l):fi(this._objects,c,this._windowDays,l),g=Ve(c),m=this._windowDays===365||u,v=m?h.filter(b=>b.events.length>0):h,f=b=>{let S=`cal-status-${b.status}`,j=b.projected?"cal-event-projected":"",L=b.status==="overdue"&&b.days_until_due!=null?` (${Math.abs(b.days_until_due)}d ${s("overdue",e).toLowerCase()})`:"",I=b.projected&&b.interval_days?o`<span class="cal-event-recur">${b.interval_unit&&b.interval_unit!=="days"?`${b.interval_days} ${s("unit_"+b.interval_unit,e)}`:s("cal_every_n_days",e).replace("{n}",String(b.interval_days))}</span>`:p,E=b.schedule_type==="sensor_based",G=E?o`<ha-icon class="cal-event-icon cal-source-sensor"
          ...
          </span>`:p,Y=this._stats?.budget?.currency_symbol||we,U=b.history_type?s(b.history_type,e):s(b.status,e);return o`
        <div class="cal-event ${j}"
          ...
          ${b.avg_cost!=null&&b.avg_cost>0?o`<span class="cal-event-cost">${b.avg_cost.toFixed(0)} ${Y}</span>`:p}
        </div>
      `},$=b=>{let[S,j,L]=b.date.split("-").map(Number),I=new Date(S,j-1,L),E=b.date===g,G=I.toLocaleDateString(e,{weekday:"short"}),Q=I.toLocaleDateString(e,{month:"long"});return o`
    ...
    `],d([y({attribute:!1})],ae.prototype,"hass",2),d([_()],ae.prototype,"_config",2),d([_()],ae.prototype,"_objects",2),d([_()],ae.prototype,"_stats",2),d([_()],ae.prototype,"_windowDays",2),d([_()],ae.prototype,"_pastDays",2),d([_()],ae.prototype,"_userFilter",2),d([_()],ae.prototype,"_unsub",2);var Ls=[{value:7,label:"Week (7 days)"},{value:14,label:"Fortnight (14 days)"},{value:30,label:"Month (30 days, default)"},{value:365,label:"Year (365 days, empty days collapsed)"}],Re=class extends A{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"}}setConfig(e){this._config={...e}}_valueChanged(e,t){let i={...this._config,[e]:t};e==="show_window_chips"&&t===!0&&delete i.show_window_chips,e==="show_user_filter"&&t===!0&&delete i.show_user_filter,e==="title"&&(!t||typeof t=="string"&&t.trim()==="")&&delete i.title,e==="user_filter"&&t===""&&delete i.user_filter,this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}render(){let e=this._config.window_days??30,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.user_filter??"",l=this._config.title??"";return o`
      <div class="editor">
        <div class="row">
        ...
        <div class="row">
          ...
          >
            ${Ls.map(c=>o`<option value="${c.value}" ?selected=${c.value===e}>${c.label}</option>`)}
          </select>
        </div>
  ...
  `,d([y({attribute:!1})],X.prototype,"hass",2),d([_()],X.prototype,"_open",2),d([_()],X.prototype,"_title",2),d([_()],X.prototype,"_message",2),d([_()],X.prototype,"_confirmText",2),d([_()],X.prototype,"_danger",2),d([_()],X.prototype,"_inputLabel",2),d([_()],X.prototype,"_inputType",2),d([_()],X.prototype,"_inputValue",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",X);var wi={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},ki=r=>(...n)=>({_$litDirective$:r,values:n}),vt=class{constructor(n){}get _$AU(){return this._$AM._$AU}_$AT(n,e,t){this._$Ct=n,this._$AM=e,this._$Ci=t}_$AS(n,e){return this.update(n,e)}update(n,e){return this.render(...e)}};var Ke=class extends vt{constructor(n){if(super(n),this.it=p,n.type!==wi.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(n){if(n===p||n==null)return this._t=void 0,this.it=n;if(n===de)return n;if(typeof n!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(n===this.it)return this._t;this.it=n;let e=[n];return e.raw=e,this._t={_$litType$:this.constructor.resultType,strings:e,values:[]}}};Ke.directiveName="unsafeHTML",Ke.resultType=1;var Ei=ki(Ke);var zs=["EUR","USD","GBP","JPY","CHF","CAD","AUD","CNY","INR","BRL","CZK","PLN","RUB","SEK","NOK","DKK","UAH"],R=class extends A{constructor(){super(...arguments);this.budget=null;this._settings=null;this._loading=!0;this._importCsv="";this._importLoading=!1;this._includeHistory=!0;this._toast="";this._testingNotification=!1;this._users=[];this._vacEnabled=!1;this._vacStart="";this._vacEnd="";this._vacBuffer=3;this._vacExempt=new Set;this._vacIsActive=!1;this._vacWindowEnd=null;this._vacAllTasks=[];this._vacPreview=[];this._vacPreviewLoading=!1;this._vacSaving=!1;this._qrObjects=[];this._qrSelectedEntries=new Set;this._qrActions=new Set(["view"]);this._qrUrlMode="companion";this._qrBatchLoading=!1;this._qrBatchResults=[];this._qrObjectsLoaded=!1;this._loaded=!1;this._userService=null;this._sendTestNotification=async()=>{this._testingNotification=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/test_notification"}),t=e.message||(e.success?s("test_notification_success",this._lang):s("test_notification_failed",this._lang));this._showToast(t)}catch{this._showToast(s("test_notification_failed",this._lang))}finally{this._testingNotification=!1}};this._allTemplates=[];this._templateCategories={};this._templatesRequested=!1}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._loaded?(this._loaded=!0,this._userService=new me(this.hass),this._loadSettings(),this._loadUsers()):e.has("hass")&&this.hass&&this._userService&&this._userService.updateHass(this.hass)}async _loadUsers(){if(this._userService)try{this._users=await this._userService.getUsers()}catch{this._users=[]}}async _loadSettings(){this._loading=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});this._settings=e,this._hydrateVacationFromSettings()}catch{}this._loading=!1}_hydrateVacationFromSettings(){let e=this._settings?.vacation;e&&(this._vacEnabled=e.enabled,this._vacStart=e.start||"",this._vacEnd=e.end||"",this._vacBuffer=e.buffer_days,this._vacExempt=new Set(e.exempt_task_ids||[]),this._vacIsActive=e.is_active,this._vacWindowEnd=e.window_end)}async _updateSetting(e,t){try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/update",settings:{[e]:t}});this._settings=i,this._showToast(s("settings_saved",this._lang)),this.dispatchEvent(new CustomEvent("settings-changed"))}catch{this._showToast(s("action_error",this._lang))}}_showToast(e){this._toast=e,setTimeout(()=>{this._toast=""},3e3)}_downloadFile(e,t,i){pt(e,t,i)}render(){let e=this._lang;return this._loading||!this._settings?o`<div class="settings-loading">Loading…</div>`:o`
    ...
    `}_toggleTemplate(e,t){let i=new Set(this._settings.disabled_template_ids||[]);t?i.delete(e):i.add(e),this._updateSetting("disabled_template_ids",[...i])}_renderObjectsColumns(e){let t=We(this._settings.objects_table_columns);return o`
    ...
    `}_toggleColumn(e,t){let i=new Set(We(this._settings.objects_table_columns));t?i.add(e):i.delete(e);let a=Ce.filter(l=>l.required||i.has(l.key)).map(l=>l.key);this._updateSetting("objects_table_columns",a)}_renderGeneral(e){let t=this._settings.general,i=t.notify_targets??[],a=this._settings.budget;return o`
      <div class="settings-section">
        ...
        <label class="setting-row">
          ...
          <select @change=${l=>this._updateSetting("budget_currency",l.target.value)}>
            ${zs.map(l=>o`<option value=${l} ?selected=${a.currency===l}>${l}</option>`)}
          </select>
        </label>
        <label class="setting-row">
    ...
    `}_renderVacationTaskList(e){let t=new Map;for(let a of this._vacAllTasks){let l=t.get(a.object_name)||[];l.push(a),t.set(a.object_name,l)}return[...t.entries()].sort(([a],[l])=>a.localeCompare(l)).map(([a,l])=>o`
      <div class="vac-task-group">
        ...
        ${l.sort((c,u)=>c.task_name.localeCompare(u.task_name)).map(c=>o`
...
<body>
  ...
  `,d([y({attribute:!1})],R.prototype,"hass",2),d([y({attribute:!1})],R.prototype,"features",2),d([y({attribute:!1})],R.prototype,"budget",2),d([_()],R.prototype,"_settings",2),d([_()],R.prototype,"_loading",2),d([_()],R.prototype,"_importCsv",2),d([_()],R.prototype,"_importLoading",2),d([_()],R.prototype,"_includeHistory",2),d([_()],R.prototype,"_toast",2),d([_()],R.prototype,"_testingNotification",2),d([_()],R.prototype,"_users",2),d([_()],R.prototype,"_vacEnabled",2),d([_()],R.prototype,"_vacStart",2),d([_()],R.prototype,"_vacEnd",2),d([_()],R.prototype,"_vacBuffer",2),d([_()],R.prototype,"_vacExempt",2),d([_()],R.prototype,"_vacIsActive",2),d([_()],R.prototype,"_vacWindowEnd",2),d([_()],R.prototype,"_vacAllTasks",2),d([_()],R.prototype,"_vacPreview",2),d([_()],R.prototype,"_vacPreviewLoading",2),d([_()],R.prototype,"_vacSaving",2),d([_()],R.prototype,"_qrObjects",2),d([_()],R.prototype,"_qrSelectedEntries",2),d([_()],R.prototype,"_qrActions",2),d([_()],R.prototype,"_qrUrlMode",2),d([_()],R.prototype,"_qrBatchLoading",2),d([_()],R.prototype,"_qrBatchResults",2),d([_()],R.prototype,"_qrObjectsLoaded",2),d([_()],R.prototype,"_allTemplates",2),d([_()],R.prototype,"_templateCategories",2);customElements.define("maintenance-settings-view",R);var ee=class extends A{constructor(){super(...arguments);this.objects=[];this._summary=null;this._loaded=!1;this._busy=!1;this._error="";this._query="";this._results=[];this._expanded=!1;this._initiallyLoaded=!1;this._searchTimer=0}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._initiallyLoaded&&(this._initiallyLoaded=!0,this._load(),ie(this._lang).then(()=>this.requestUpdate()))}async _load(){this._busy=!0;try{this._summary=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/storage"}),this._error=""}catch(e){this._error=M(e,this._lang)}finally{this._loaded=!0,this._busy=!1}}_nameFor(e){return this.objects.find(i=>i.object?.id===e)?.object?.name||e.slice(0,8)}_entryFor(e){return this.objects.find(t=>t.object?.id===e)?.entry_id}_toggle(){this._expanded=!this._expanded}_openObject(e){this.dispatchEvent(new CustomEvent("open-object",{detail:{entry_id:e},bubbles:!0,composed:!0}))}_onSearch(e){this._query=e.target.value,clearTimeout(this._searchTimer),this._searchTimer=window.setTimeout(()=>{this._doSearch()},250)}async _doSearch(){let e=this._query.trim();if(!e){this._results=[];return}try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/search",query:e});this._results=t.results||[]}catch(t){this._error=M(t,this._lang),this._results=[]}}async _openResult(e){if(e.kind==="weblink"){window.open(e.url,"_blank","noopener");return}let t=window.open("about:blank","_blank");try{let i=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300});t&&(t.location.href=new URL(i.path,window.location.origin).href)}catch(i){t&&t.close(),this._error=M(i,this._lang)}}_renderResult(e,t){return o`
        ...
        <div class="card-content">
          <div class="header">
            ...
            >
              ...
              <span class="header-summary">
                ${oe(e.total_bytes)}
                ${e.dedup_savings_bytes>0?o`<span class="saved">−${oe(e.dedup_savings_bytes)}</span>`:p}
              </span>
            </button>
            <button
              class="icon-btn"
          ...
          ${this._expanded?o`
                <div class="body">
                  <div class="totals">
                    <div class="stat">
                      <div class="stat-value">${oe(e.total_bytes)}</div>
                    ...
                    ${e.dedup_savings_bytes>0?o`<div class="stat">
                          <div class="stat-value saved">−${oe(e.dedup_savings_bytes)}</div>
    ...
    `}_renderObjRow(e,t){let i=e.entry;return o`
      ...
      >
        ...
        <span class="obj-size">${oe(e.bytes)}</span>
  ...
  `,d([y({attribute:!1})],te.prototype,"hass",2),d([y({attribute:!1})],te.prototype,"objects",2),d([_()],te.prototype,"_open",2),d([_()],te.prototype,"_loading",2),d([_()],te.prototype,"_error",2),d([_()],te.prototype,"_groupId",2),d([_()],te.prototype,"_name",2),d([_()],te.prototype,"_description",2),d([_()],te.prototype,"_selected",2);customElements.get("maintenance-group-dialog")||customElements.define("maintenance-group-dialog",te);var Ds=60,Os=20,Ti=30;function Dt(r){let n=r.trigger_config??null;if(!n)return p;let e=n.type||"threshold",t=r.trigger_entity_info?.unit_of_measurement??"",i=0,a="";if(e==="threshold"){let u=r.trigger_current_value??null;if(u==null)return p;let h=n.trigger_above,g=n.trigger_below;if(h!=null){let m=g??0,v=h-m||1;i=Math.min(100,Math.max(0,(u-m)/v*100)),a=`${u.toFixed(1)} / ${h} ${t}`}else if(g!=null){let v=r.trigger_entity_info?.max??(g*2||100),f=v-g||1;i=Math.min(100,Math.max(0,(v-u)/f*100)),a=`${u.toFixed(1)} / ${g} ${t}`}else return p}else if(e==="counter"){let u=n.trigger_target_value||1,g=r.trigger_current_delta??null??r.trigger_current_value??null;if(g==null)return p;i=Math.min(100,Math.max(0,g/u*100)),a=`${g.toFixed(1)} / ${u} ${t}`}else if(e==="state_change"){let u=n.trigger_target_changes||1,h=r.trigger_current_value??null;if(h==null)return p;i=Math.min(100,Math.max(0,h/u*100)),a=`${Math.round(h)} / ${u}`}else if(e==="runtime"){let u=n.trigger_runtime_hours||100,h=r.trigger_current_value??null;if(h==null)return p;i=Math.min(100,Math.max(0,h/u*100)),a=`${h.toFixed(1)}h / ${u}h`}else if(e==="compound"){let u=n.compound_logic||n.operator||"AND",h=n.conditions?.length||0;a=`${u} (${h})`,i=r.trigger_active?100:0}else return p;let l=i>=100,c=i>90?"var(--error-color, #f44336)":i>70?"var(--warning-color, #ff9800)":"var(--primary-color)";return o`
  ...
  `}function Ot(r,n,e){if(!r.trigger_config?.entity_id)return p;let t=r.trigger_config.entity_id,i=n.get(t)||[],a=[];if(i.length>=2)a=i.map(E=>({ts:E.ts,val:E.val}));else{if(!r.history)return p;for(let E of r.history)E.trigger_value!=null&&a.push({ts:new Date(E.timestamp).getTime(),val:E.trigger_value})}if(r.trigger_current_value!=null&&a.push({ts:Date.now(),val:r.trigger_current_value}),a.length<2)return p;a.sort((E,G)=>E.ts-G.ts);let l=Ds,c=Os,u=a.map(E=>E.val),h=Math.min(...u),g=Math.max(...u),m=g-h||1;h-=m*.1,g+=m*.1;let v=a[0].ts,$=a[a.length-1].ts-v||1,b=E=>(E-v)/$*l,S=E=>2+(1-(E-h)/(g-h))*(c-4),j=a;if(j.length>Ti){let E=Math.ceil(j.length/Ti);j=j.filter((G,Q)=>Q%E===0||Q===j.length-1)}let L=j.map(E=>`${b(E.ts).toFixed(1)},${S(E.val).toFixed(1)}`).join(" "),I=r.trigger_active?"var(--error-color, #f44336)":"var(--primary-color)";return o`
  ...
  `}function Si(r,n){let e=n;if(r.days_until_due==null||!r.interval_days||r.interval_days<=0)return p;let{pct:t,overflow:i}=ct(r.interval_days,r.days_until_due,r.interval_unit),a="var(--success-color, #4caf50)";return r.status==="overdue"?a="var(--error-color, #f44336)":r.status==="due_soon"&&(a="var(--warning-color, #ff9800)"),o`
    <div class="days-progress">
      ...
      <div class="days-progress-text">${je(r.days_until_due,e)}</div>
    </div>
  `}function Ye(r,n,e=4){if(!isFinite(r)||!isFinite(n))return{ticks:[],niceMin:0,niceMax:1};if(r===n){let h=Math.abs(r)*.1||1;r-=h,n+=h}let t=n-r,i=Math.pow(10,Math.floor(Math.log10(t/Math.max(1,e)))),a=i;for(let h of[1,2,5,10])if(a=i*h,t/a<=e+.5)break;let l=Math.floor(r/a)*a,c=Math.ceil(n/a)*a,u=[];for(let h=l;h<=c+a*1e-6;h+=a)u.push(Math.abs(h)<a*1e-9?0:h);return{ticks:u,niceMin:l,niceMax:c}}function le(r){let n=Math.abs(r);return n>=1e6?Ie((r/1e6).toFixed(n>=1e7?0:1))+"M":n>=1e4?Ie((r/1e3).toFixed(0))+"k":n>=1e3?Ie((r/1e3).toFixed(1))+"k":n>=100?r.toFixed(0):n>=10||n>=1?Ie(r.toFixed(1)):n===0?"0":Ie(r.toFixed(2))}function Ie(r){return r.replace(/\.0+$/,"").replace(/(\.\d*[1-9])0+$/,"$1")}function Ee(r,n,e){let t=r.toLocaleString(e,{maximumFractionDigits:Math.abs(r)>=100?0:1});return n?`${t} ${n}`:t}function Le(r,n,e){let t=new Date(r),i=e?{month:"short",day:"numeric",year:"2-digit"}:{month:"short",day:"numeric"};return t.toLocaleDateString(n,i)}function qt(r,n){return new Date(r).toLocaleDateString(n,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function bt(r,n){return new Date(r).getFullYear()!==new Date(n).getFullYear()}function ft(r,n,e){if(e<2||n<=r)return[r,n];let t=[];for(let i=0;i<e;i++)t.push(r+(n-r)*i/(e-1));return t}var yt=210,re=46,ue=14,he=12,Ai=14,qs=20+Ai,Ns=[{days:7,key:"chart_range_7d"},{days:30,key:"chart_range_30d"},{days:90,key:"chart_range_90d"},{days:365,key:"chart_range_1y"}],B=class extends A{constructor(){super(...arguments);this.points=[];this.events=[];this.unit="";this.lang="en";this.thresholdAbove=null;this.thresholdBelow=null;this.targetValue=null;this.forceZero=!1;this.projection=null;this.rangeDays=30;this.showRange=!0;this.busy=!1;this.hideOutliers=!1;this.showOutlierToggle=!0;this._width=0;this._hover=null;this._ro=null}connectedCallback(){super.connectedCallback(),this._ro=new ResizeObserver(e=>{let t=Math.floor(e[0]?.contentRect?.width||0);t&&Math.abs(t-this._width)>2&&(this._width=t)}),this._ro.observe(this)}disconnectedCallback(){super.disconnectedCallback(),this._ro?.disconnect(),this._ro=null}_emitRange(e){e!==this.rangeDays&&this.dispatchEvent(new CustomEvent("range-change",{detail:{days:e},bubbles:!0,composed:!0}))}_toggleOutliers(){this.dispatchEvent(new CustomEvent("outlier-toggle",{detail:{hide:!this.hideOutliers},bubbles:!0,composed:!0}))}render(){let e=this._width||320,t=[...this.points].sort((a,l)=>a.ts-l.ts),i=this.lang;return o`
    ...
    `}_renderSvg(e,t){let i=this.lang,a=e-re-ue,l=yt-qs,c=l-he,u=1/0,h=-1/0;for(let k of t)u=Math.min(u,k.min??k.val),h=Math.max(h,k.max??k.val);this.thresholdAbove!=null&&(u=Math.min(u,this.thresholdAbove),h=Math.max(h,this.thresholdAbove)),this.thresholdBelow!=null&&(u=Math.min(u,this.thresholdBelow),h=Math.max(h,this.thresholdBelow)),this.targetValue!=null&&(u=Math.min(u,this.targetValue),h=Math.max(h,this.targetValue)),this.forceZero&&(u=Math.min(u,0));let g=(h-u||1)*.06,m=this.forceZero&&u>=0?0:u-g,{ticks:v,niceMin:f,niceMax:$}=Ye(m,h+g,4);this.forceZero&&u>=0&&f<0&&(f=0,v=v.filter(k=>k>=0));let b=t[0].ts,S=t[t.length-1].ts,j=S-b||1,L=bt(b,S),I=k=>re+(k-b)/j*a,E=k=>he+(1-(k-f)/($-f||1))*c,G=t.map(k=>`${I(k.ts).toFixed(1)},${E(k.val).toFixed(1)}`).join(" "),Q=`M${I(t[0].ts).toFixed(1)},${l} `+t.map(k=>`L${I(k.ts).toFixed(1)},${E(k.val).toFixed(1)}`).join(" ")+` L${I(t[t.length-1].ts).toFixed(1)},${l} Z`,Y="",U=t.filter(k=>k.min!=null&&k.max!=null);if(U.length>=2){let k=U.map(P=>`${I(P.ts).toFixed(1)},${E(P.max).toFixed(1)}`),w=[...U].reverse().map(P=>`${I(P.ts).toFixed(1)},${E(P.min).toFixed(1)}`);Y=`M${k[0]} `+k.slice(1).map(P=>`L${P}`).join(" ")+` L${w.join(" L")} Z`}let N=[];if(this.thresholdBelow!=null){let k=E(this.thresholdBelow);N.push({y:k,h:Math.max(0,l-k),lineY:k,label:`\u25BC ${le(this.thresholdBelow)}`,labelY:Math.min(l-4,k+13)})}if(this.thresholdAbove!=null){let k=E(this.thresholdAbove);N.push({y:he,h:Math.max(0,k-he),lineY:k,label:`\u25B2 ${le(this.thresholdAbove)}`,labelY:Math.max(he+11,k-5)})}let _e=t[t.length-1],be=(this.events||[]).filter(k=>k.ts>=b&&k.ts<=S),Te=ft(b,S,Math.max(2,Math.min(5,Math.floor(a/110)+1))),V=this._hover;return o`
      <div class="svg-holder">
        ...
        >
          <defs>
            <clipPath id="plot"><rect x="${re}" y="${he}" width="${a}" height="${c}" /></clipPath>
            ${N.length?D`<clipPath id="danger">${N.map(k=>D`<rect x="${re}" y="${k.y.toFixed(1)}" width="${a}" height="${k.h.toFixed(1)}" />`)}</clipPath>`:p}
          ...
          ${v.map(k=>{let w=E(k);return w<he-1||w>l+1?p:D`
              <line x1="${re}" y1="${w.toFixed(1)}" x2="${e-ue}" y2="${w.toFixed(1)}"
                stroke="var(--divider-color)" stroke-width="1" opacity="0.6" />
              <text x="${re-7}" y="${(w+3.5).toFixed(1)}" text-anchor="end" class="tick-label">${le(k)}</text>`})}

          ${N.map(k=>D`<rect x="${re}" y="${k.y.toFixed(1)}" width="${a}" height="${k.h.toFixed(1)}"
              fill="url(#dangerHatch)" />`)}

          ${Y?D`<path d="${Y}" fill="var(--primary-color)" opacity="0.08" clip-path="url(#plot)" />`:p}
          ...
          ${N.length?D`<polyline points="${G}" fill="none" stroke="var(--error-color, #f44336)" stroke-width="2"
                stroke-linejoin="round" stroke-linecap="round" clip-path="url(#danger)" />`:p}

          ${N.map(k=>D`
              <line x1="${re}" y1="${k.lineY.toFixed(1)}" x2="${e-ue}" y2="${k.lineY.toFixed(1)}"
                stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="6,4" />
              <text x="${e-ue-4}" y="${k.labelY.toFixed(1)}" text-anchor="end" class="zone-label">${k.label}</text>`)}

          ${this.targetValue!=null?D`<line x1="${re}" y1="${E(this.targetValue).toFixed(1)}" x2="${e-ue}" y2="${E(this.targetValue).toFixed(1)}"
                stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="6,4" />
              <text x="${e-ue-4}" y="${(E(this.targetValue)-5).toFixed(1)}" text-anchor="end" class="zone-label">◆ ${le(this.targetValue)} ${this.unit}</text>`:p}

          ${this.projection&&this.projection.length===2?D`<line x1="${I(this.projection[0].ts).toFixed(1)}" y1="${E(this.projection[0].val).toFixed(1)}"
                x2="${Math.min(I(this.projection[1].ts),e-ue).toFixed(1)}" y2="${E(Math.max(f,Math.min($,this.projection[1].val))).toFixed(1)}"
                stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.8" />`:p}

          ${Te.map((k,w)=>{let P=I(k),xt=w===0?"start":w===Te.length-1?"end":"middle";return D`<text x="${P.toFixed(1)}" y="${yt-5}" text-anchor="${xt}" class="tick-label">${Le(k,i,L)}</text>`})}

          <line x1="${re}" y1="${l}" x2="${e-ue}" y2="${l}" stroke="var(--divider-color)" stroke-width="1" />

          ${be.map(k=>{let w=I(k.ts),P=k.type==="completed"?"var(--success-color, #4caf50)":k.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return D`
              <line x1="${w.toFixed(1)}" y1="${he}" x2="${w.toFixed(1)}" y2="${l}" stroke="${P}" stroke-width="1" opacity="0.14" />
              <rect x="${(w-1.5).toFixed(1)}" y="${l+3}" width="3" height="${Ai-6}" rx="1.5" fill="${P}">
                <title>${qt(k.ts,i)}</title>
              </rect>`})}

          ${V?D`
                <line x1="${V.x.toFixed(1)}" y1="${he}" x2="${V.x.toFixed(1)}" y2="${l}"
                  stroke="var(--secondary-text-color)" stroke-width="1" stroke-dasharray="3,3" opacity="0.7" />
                <circle cx="${V.x.toFixed(1)}" cy="${V.y.toFixed(1)}" r="4.5" fill="var(--primary-color)"
                  stroke="var(--card-background-color, #fff)" stroke-width="2" />`:D`<circle cx="${I(_e.ts).toFixed(1)}" cy="${E(_e.val).toFixed(1)}" r="4" fill="var(--primary-color)"
                stroke="var(--card-background-color, #fff)" stroke-width="1.5" />`}
        </svg>
        ${V?o`<div
              class="hover-chip"
              style="left:${Math.min(Math.max(V.x,70),e-70)}px"
            >
              <div class="hover-date">${qt(V.p.ts,i)}</div>
              <div class="hover-val">
                ${Ee(V.p.val,this.unit,i)}
                ${V.p.min!=null&&V.p.max!=null?o`<span class="hover-range">(${le(V.p.min)}–${le(V.p.max)})</span>`:p}
              </div>
            </div>`:p}
      </div>
    `}_onPointer(e,t,i,a,l){let u=e.currentTarget.getBoundingClientRect(),h=(e.clientX-u.left)/u.width*l;if(h<re-8||h>l-ue+8){this._hover=null;return}let g=t[0],m=1/0;for(let v of t){let f=Math.abs(i(v.ts)-h);f<m&&(m=f,g=v)}this._hover={x:i(g.ts),y:a(g.val),p:g}}};B.styles=C`
    ...
    <div class="trigger-card">
      <div class="trigger-header">
        ...
        <div>
          ${v?o`
            ...
            <div class="trigger-entity-id">${u.map(($,b)=>o`${b>0?", ":""}<span class="entity-link" @click=${S=>ke(S,$)}>${$}</span>`)}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `:o`
            <div class="trigger-entity-name">${l}</div>
            <div class="trigger-entity-id">${c?o`<span class="entity-link" @click=${$=>ke($,c)}>${c}</span>`:""}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `}
        </div>
      ...
      ${f?Bs(f,t):g!=null?o`
              <div class="trigger-value-row">
                <span class="trigger-current ${r.trigger_active?"active":""}">${typeof g=="number"?Ee(g,"",t):g}</span>
      ...
      <div class="trigger-limits">
        ${m==="threshold"?o`
          ${e.trigger_above!=null?o`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("threshold_above",t)}: ${e.trigger_above} ${h}</span>`:p}
          ${e.trigger_below!=null?o`<span class="trigger-limit-item"><span class="dot warn" aria-hidden="true"></span> ${s("threshold_below",t)}: ${e.trigger_below} ${h}</span>`:p}
        ...
        ${m==="compound"?o`
          ...
          ${(e.conditions||[]).map(($,b)=>o`
            <span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${b+1}. ${s($.type||"unknown",t)}: ${$.entity_id?o`<span class="entity-link" @click=${S=>ke(S,$.entity_id)}>${$.entity_id}</span>`:""}</span>
          `)}
        `:p}
      </div>
      ...
      ${a&&a.length>1?o`
        <div class="trigger-entity-list">
          ${a.map($=>o`
            <span class="trigger-entity-id">${$.friendly_name} (<span class="entity-link" @click=${b=>ke(b,$.entity_id)}>${$.entity_id}</span>)</span>
          `)}
        </div>
      `:p}
  ...
  `}function Us(r,n,e){let t=r.trigger_config,i=r.trigger_current_value;if(!t||i==null)return null;switch(t.type||"threshold"){case"counter":{let a=t.trigger_target_value;if(a==null||a<=0)return null;let l=Ci(r,Mi(r,e));return{progress:Math.max(0,i-(l?.value??i)),target:a,unit:n,meter:i}}case"state_change":{let a=t.trigger_target_changes;return a==null||a<=0?null:{progress:Math.max(0,i),target:a,unit:"",meter:null}}case"runtime":{let a=t.trigger_runtime_hours;return a==null||a<=0?null:{progress:Math.max(0,i),target:a,unit:"h",meter:null}}}return null}function Ci(r,n){if(r.trigger_baseline_value!=null)return{value:r.trigger_baseline_value,ts:Nt(r)};if(!n.length)return null;let e=Nt(r);if(e==null)return{value:n[0].val,ts:null};let t=n[0],i=Math.abs(n[0].ts-e);for(let a of n){let l=Math.abs(a.ts-e);l<i&&(t=a,i=l)}return{value:t.val,ts:e}}function Nt(r){let n=[...r.history].filter(e=>e.type==="completed"||e.type==="reset").sort((e,t)=>new Date(t.timestamp).getTime()-new Date(e.timestamp).getTime())[0];return n?new Date(n.timestamp).getTime():null}function Bs(r,n){let e=Math.min(999,Math.round(r.progress/r.target*100)),t=e>=100?"over":e>=75?"near":"ok";return o`
    <div class="counter-progress">
      <div class="counter-progress-nums">
        <span class="counter-progress-main">${Ee(r.progress,"",n)}<span class="counter-progress-target"> / ${Ee(r.target,r.unit,n)}</span></span>
      ...
      <div class="counter-progress-caption">
        ${s("chart_since_service",n)}${r.meter!=null?o` · ${s("current",n)}: ${Ee(r.meter,r.unit,n)}`:p}
      </div>
    </div>
  `}function Mi(r,n){let e=r.trigger_config;if(!e)return[];let t=e.type||"threshold",i=e.entity_id||"",a=t==="runtime"?[]:n.detailStatsData.get(i)||[],l=n.isCounterEntity(e),c=[];if(a.length>=2)for(let u of a){let h={ts:u.ts,val:u.val};!l&&u.min!=null&&u.max!=null&&(h.min=u.min,h.max=u.max),c.push(h)}else for(let u of r.history)u.trigger_value!=null&&c.push({ts:new Date(u.timestamp).getTime(),val:u.trigger_value});return r.trigger_current_value!=null&&c.push({ts:Date.now(),val:r.trigger_current_value}),c.sort((u,h)=>u.ts-h.ts),c}function Ws(r,n,e){let t=r.trigger_config;if(!t)return p;let i=t.type||"threshold",a=t.entity_id||"",l=Mi(r,e);i==="runtime"&&t.trigger_runtime_hours&&r.trigger_current_value!=null&&(l=[{ts:Nt(r)??l[0]?.ts??Date.now()-864e5,val:0},{ts:Date.now(),val:Math.max(0,r.trigger_current_value)}]),e.hideOutliers&&(l=Fs(l));let c=l.length<2&&!!a&&e.hasStatsService&&!e.detailStatsData.has(a);if(l.length<2&&!c)return p;let u=!!a&&e.detailStatsData.has(a)&&(e.detailStatsData.get(a)?.length??0)<2,h=Date.now()-e.rangeDays*864e5,g=l.filter(b=>b.ts>=h);g.length>=2&&(l=g);let m=null,v=!1;if(i==="counter"&&t.trigger_target_value!=null&&l.length){let b=Ci(r,l);if(b){if(b.ts!=null){let S=l.filter(j=>j.ts>=b.ts);S.length>=2&&(l=S)}l=l.map(S=>({...S,val:Math.max(0,S.val-b.value)}))}m=t.trigger_target_value,v=!0}else i==="state_change"&&t.trigger_target_changes?(m=t.trigger_target_changes,v=!0):i==="runtime"&&t.trigger_runtime_hours&&(m=t.trigger_runtime_hours,v=!0);let f=null;if(m==null&&r.degradation_rate!=null&&(r.degradation_trend!=="stable"||r.days_until_threshold!=null)&&r.degradation_trend!=="insufficient_data"&&l.length>=2){let b=l[l.length-1];f=[b,{ts:b.ts+30*864e5,val:b.val+r.degradation_rate*30}]}let $=r.history.filter(b=>["completed","skipped","reset"].includes(b.type)).map(b=>({ts:new Date(b.timestamp).getTime(),type:b.type}));return o`
    ...
    ></maintenance-trigger-chart>
    ${u&&!c?o`<div class="chart-note">
          <ha-icon icon="mdi:information-outline"></ha-icon>
          ${s("chart_no_stats",e.lang)}
        </div>`:p}
  `}function Ri(r,n,e){let t=r.degradation_trend!=null&&r.degradation_trend!=="insufficient_data",i=r.days_until_threshold!=null,a=r.environmental_factor!=null&&r.environmental_factor!==1;if(!t&&!i&&!a)return p;let l=r.degradation_trend==="rising"?"M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z":r.degradation_trend==="falling"?"M16,18L18.29,15.71L13.41,10.83L9.41,14.83L2,7.41L3.41,6L9.41,12L13.41,8L19.71,14.29L22,12V18H16Z":"M22,12L18,8V11H3V13H18V16L22,12Z";return o`
      ...
      <div class="prediction-grid">
        ...
        ${a&&e.environmental?o`
          <div class="prediction-item">
            ...
            ${r.environmental_entity?o`<span class="prediction-entity entity-link" @click=${c=>ke(c,r.environmental_entity)}>${r.environmental_entity}</span>`:p}
          </div>
        `:p}
      </div>
    </div>
  `}function Ii(r,n){let e=r.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return p;let a=r.interval_days??0,l=r.suggested_interval??a;return o`
  ...
  `}function Gs(r,n,e){let t=r.confidence_interval_low,i=r.confidence_interval_high,a=n.suggested_interval??n.interval_days??0,l=n.interval_days??0,c=Math.max(0,t-5),h=i+5-c,g=(t-c)/h*100,m=(i-t)/h*100,v=(a-c)/h*100,f=l>0?(l-c)/h*100:-1;return o`
  ...
  `}function Li(r,n,e,t){let i=Math.max(r||1,n);return o`
  ...
  `}function Hi(r,n){return Qs(r,n)}function Qs(r,n){let e=r.seasonal_factors??r.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return p;let t=r.interval_analysis?.seasonal_reason,i=new Date().getMonth(),a=300,l=100,c=8,h=l-c-4,g=Math.max(...e,1.5),m=a/12,v=m*.65,f=c+h-1/g*h;return o`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${a} ${l}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",n)}">
        ...
        ${e.map(($,b)=>{let S=$/g*h,j=b*m+(m-v)/2,L=c+h-S,I=b===i,E=$<1?"var(--success-color, #4caf50)":$>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return D`
            <rect x="${j.toFixed(1)}" y="${L.toFixed(1)}"
              width="${v.toFixed(1)}" height="${S.toFixed(1)}"
              fill="${E}" opacity="${I?1:.5}" rx="2" />
          `})}
      </svg>
      <div class="seasonal-labels">
        ${Pi.map(($,b)=>o`<span class="seasonal-label ${b===i?"active-month":""}">${s($,n)}</span>`)}
      </div>
    </div>
  `}var Js=200,Ge=10,Zs=22;function Di(r,n,e,t){let i=r.history.filter(c=>c.type==="completed"&&(c.cost!=null||c.duration!=null));if(i.length<2)return p;let a=i.some(c=>(c.cost??0)>0),l=i.some(c=>(c.duration??0)>0);return!a&&!l?p:o`
  ...
  `}function Xs(r,n,e){let t=r.history.filter(w=>w.type==="completed"&&(w.cost!=null||w.duration!=null)).map(w=>({ts:new Date(w.timestamp).getTime(),cost:w.cost??0,duration:w.duration??0})).sort((w,P)=>w.ts-P.ts);if(t.length<2)return p;let i=t.some(w=>w.cost>0),a=t.some(w=>w.duration>0);if(!i&&!a)return p;let l=e!=="duration"&&i,c=e!=="cost"&&a,u=l||!c&&i,h=c||!l&&a,g=640,m=Js,v=u?44:12,f=h?44:12,$=g-v-f,b=m-Zs,S=b-Ge,j=t[0].ts,L=t[t.length-1].ts,I=(L-j||864e5)*.05,E=j-I,G=L+I,Q=bt(j,L),Y=w=>v+(w-E)/(G-E)*$,U=Ye(0,Math.max(...t.map(w=>w.cost))||1,3),N=Ye(0,Math.max(...t.map(w=>w.duration))||1,3),_e=w=>Ge+(1-w/(U.niceMax||1))*S,be=w=>Ge+(1-w/(N.niceMax||1))*S,Te=t.length>1?Math.min(...t.slice(1).map((w,P)=>Y(w.ts)-Y(t[P].ts))):$,V=Math.max(6,Math.min(22,Te*.55)),k=ft(j,L,Math.max(2,Math.min(4,t.length)));return o`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${g} ${m}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_history",n)}">
        ${u?U.ticks.map(w=>{let P=_e(w);return P<Ge-1||P>b+1?p:D`
            <line x1="${v}" y1="${P.toFixed(1)}" x2="${g-f}" y2="${P.toFixed(1)}" stroke="var(--divider-color)" stroke-width="1" opacity="0.55" />
            <text x="${v-6}" y="${(P+3.5).toFixed(1)}" text-anchor="end" fill="var(--primary-color)" font-size="10.5">${le(w)}€</text>`}):p}
        ${h?N.ticks.map(w=>{let P=be(w);return P<Ge-1||P>b+1?p:D`<text x="${g-f+6}" y="${(P+3.5).toFixed(1)}" text-anchor="start" fill="var(--accent-color, #ff9800)" font-size="10.5">${le(w)}m</text>`}):p}

        ${u?t.filter(w=>w.cost>0).map(w=>D`
          <rect x="${(Y(w.ts)-V/2).toFixed(1)}" y="${_e(w.cost).toFixed(1)}" width="${V.toFixed(1)}" height="${(b-_e(w.cost)).toFixed(1)}"
            fill="var(--primary-color)" opacity="0.6" rx="2">
            <title>${Le(w.ts,n,!0)}: ${w.cost.toLocaleString(n)}€${w.duration?` \xB7 ${w.duration}m`:""}</title>
          </rect>
        `):p}
        ${h?D`
          <polyline points="${t.map(w=>`${Y(w.ts).toFixed(1)},${be(w.duration).toFixed(1)}`).join(" ")}"
            fill="none" stroke="var(--accent-color, #ff9800)" stroke-width="2" stroke-linejoin="round" />
          ${t.map(w=>D`
            <circle cx="${Y(w.ts).toFixed(1)}" cy="${be(w.duration).toFixed(1)}" r="3.5" fill="var(--accent-color, #ff9800)">
              <title>${Le(w.ts,n,!0)}: ${w.duration}m${w.cost?` \xB7 ${w.cost.toLocaleString(n)}\u20AC`:""}</title>
            </circle>
          `)}
        `:p}

        <line x1="${v}" y1="${b}" x2="${g-f}" y2="${b}" stroke="var(--divider-color)" stroke-width="1" />
        ${k.map((w,P)=>{let xt=P===0?"start":P===k.length-1?"end":"middle";return D`<text x="${Y(w).toFixed(1)}" y="${m-6}" text-anchor="${xt}" fill="var(--secondary-text-color)" font-size="10">${Le(w,n,Q)}</text>`})}
      </svg>
    </div>
    <div class="chart-legend">
      ${u?o`<span class="legend-item"><span class="legend-swatch" style="background:var(--primary-color);opacity:0.6"></span>${s("cost",n)}</span>`:p}
      ${h?o`<span class="legend-item"><span class="legend-swatch" style="background:var(--accent-color, #ff9800)"></span>${s("duration",n)}</span>`:p}
    </div>
  `}var ve=class extends A{constructor(){super(...arguments);this.docId="";this._url="";this._failed=!1;this._signedFor=""}updated(){this.hass&&this.docId&&this._signedFor!==this.docId&&(this._signedFor=this.docId,this._url="",this._failed=!1,this._sign())}async _sign(){try{let e=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${this.docId}`,expires:300});this._url=e.path}catch{this._failed=!0}}render(){return this._failed||!this.docId?p:this._url?o`
      <a href=${this._url} target="_blank" rel="noopener" class="wrap">
        <img src=${this._url} alt="" loading="lazy"
          @error=${()=>this._failed=!0} />
      </a>`:o`<div class="ph"></div>`}};ve.styles=C`
  ...
  `,d([y({attribute:!1})],ve.prototype,"hass",2),d([y()],ve.prototype,"docId",2),d([_()],ve.prototype,"_url",2),d([_()],ve.prototype,"_failed",2);customElements.get("maintenance-history-photo")||customElements.define("maintenance-history-photo",ve);var ea=["completed","skipped","missed","reset","triggered"];function Oi(r,n){let e=n.lang;return o`
    <div class="history-filters-new">
      <div class="filter-chips">
        ${ea.map(t=>{let i=r.history.filter(a=>a.type===t).length;return i===0?p:o`
  ...
  `}function qi(r,n){let e=n.lang,t=n.filter?r.history.filter(i=>i.type===n.filter):r.history;if(n.search){let i=n.search.toLowerCase();t=t.filter(a=>a.notes?.toLowerCase().includes(i))}return t.length===0?o`<p class="empty">${s("no_history",e)}</p>`:o`
    <div class="history-timeline">
      ${[...t].reverse().map(i=>ta(i,n))}
    </div>
  `}function ta(r,n){let e=n.lang,t=["completed","reset","skipped"].includes(r.type);return o`
    <div class="history-entry">
      <div class="history-icon ${r.type}">
        <ha-icon .icon=${nt[r.type]||"mdi:circle"}></ha-icon>
      </div>
      <div class="history-content">
        ...
        <div class="history-date">${ot(r.timestamp,e)}</div>
  ...
  `}function Ft(r,n){if(!r.responsible_user_id)return p;let e=n(r.responsible_user_id);return e?o`
    <span class="user-badge">
      <ha-icon icon="mdi:account"></ha-icon>
      ${e}
    </span>
  `:p}function ia(r,n){let e=n.lang,t=n.isOperator,i=r.archived?"archived":r.is_done?"done":r.status==="due_soon"?"warning":r.status||"ok",a=r.archived?s("archived",e):r.is_done?s("completed",e):s(r.status||"ok",e);return o`
  ...
  `}function sa(r){let n=r.lang;return o`
    <div class="tab-bar">
  ...
  `}function Ni(r,n,e,t){let i=t.collapsedSections.has(r);return o`
  ...
  `}function aa(r,n){if(!n.features.checklists)return p;let e=r.checklist||[];if(e.length===0)return p;let t=n.lang;return o`
  ...
  `}function na(r,n){let e=r.documentation_url&&/^https?:\/\//i.test(r.documentation_url)?r.documentation_url:null,t=n.objectDocUrl&&/^https?:\/\//i.test(n.objectDocUrl)?n.objectDocUrl:null;if(!r.notes&&!e&&!t)return p;let i=n.lang;return o`
  ...
  `}function ra(r,n){let e=n.lang,t=r.times_performed>0?r.total_cost/r.times_performed:0,i=r.days_until_due!==null&&r.days_until_due!==void 0?r.days_until_due<0?"overdue":r.days_until_due<=r.warning_days?"warning":"":"";return o`
    <div class="kpi-bar">
      <div class="kpi-card">
      ...
      <div class="kpi-card">
        ...
        <div class="kpi-value">${Be(r,e)}</div>
  ...
  `}function oa(r,n){let e=n.lang;if(!n.features.adaptive||!r.suggested_interval||r.suggested_interval===r.interval_days)return p;if(n.suggestionDismissed)return p;let t=r.suggested_interval;return o`
    <div class="recommendation-card">
      <h4>${s("suggested_interval",e)}</h4>
      ${Li(r.interval_days,t,r.interval_confidence||"medium",e)}
  ...
  `}function la(r,n){let e=n.lang,t=r.history.slice(-3).reverse();if(t.length===0)return p;let i=a=>{switch(a){case"completed":return"\u2713";case"triggered":return"\u2297";case"skipped":return"\u21B7";case"reset":return"\u21BA";default:return"\xB7"}};return o`
    <div class="recent-activities">
      <h3>${s("recent_activities",e)}</h3>
      ${t.map(a=>o`
        <div class="activity-item">
          <span class="activity-icon">${i(a.type)}</span>
          <span class="activity-date">${ot(a.timestamp,e)}</span>
  ...
  `}function ca(r,n){let e=n.lang,t=n.features.adaptive&&r.suggested_interval&&r.suggested_interval!==r.interval_days,i=n.features.seasonal&&r.seasonal_factor&&r.seasonal_factor!==1,a=t||i,l=n.features.adaptive&&r.interval_analysis?.weibull_beta!=null&&r.interval_analysis?.weibull_eta!=null,c=n.features.seasonal&&(r.seasonal_factors?.length===12||r.interval_analysis?.seasonal_factors?.length===12);return o`
    <div class="tab-content overview-tab">
      ...
      <div class="two-column-layout ${a?"":"single-column"}">
        ...
        <div class="right-column">
          ${Di(r,e,n.costDurationToggle,u=>n.setCostDurationToggle(u))}
        </div>
      </div>
      ${l?Ni("weibull","weibull_reliability_curve",Ii(r,e),n):p}
      ${c?Ni("seasonal","seasonal_chart_title",o`
            ${Hi(r,e)}
  ...
  `}function da(r,n){return o`
    <div class="tab-content history-tab">
      ${Oi(r,n.history)}
      ${qi(r,n.history)}
    </div>
  `}function pa(r,n){switch(n.activeTab){case"overview":return ca(r,n);case"history":return da(r,n);default:return p}}function Fi(r,n){return o`
  ...
  `}var Qe=class extends A{createRenderRoot(){return this}render(){return!this.task||!this.ctx?p:o`${Fi(this.task,this.ctx)}`}};d([y({attribute:!1})],Qe.prototype,"task",2),d([y({attribute:!1})],Qe.prototype,"ctx",2);customElements.get("maintenance-task-detail-view")||customElements.define("maintenance-task-detail-view",Qe);function Ui(r){if(r.total<=0)return{start:0,end:0,padTop:0,padBottom:0};let n=r.overscan??12,e=Math.max(1,r.step??6),t=Math.max(1,r.rowHeight),i=Math.floor((r.scrollTop-r.listTop)/t),a=Math.ceil(r.viewportHeight/t)+1,l=Math.max(0,i-n);l=Math.floor(l/e)*e;let c=Math.min(r.total,Math.max(i,0)+a+n);return c=Math.min(r.total,Math.ceil(c/e)*e),l>=c&&(l=Math.min(l,Math.max(0,r.total-1)),c=Math.min(r.total,l+Math.max(a,1))),{start:l,end:c,padTop:l*t,padBottom:(r.total-c)*t}}var T=class extends A{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._chartRangeDays=(()=>{try{let e=parseInt(localStorage.getItem("msp-chart-range")||"",10);return[7,30,90,365].includes(e)?e:30}catch{return 30}})();this._hideOutliers=(()=>{try{return localStorage.getItem("msp-chart-hide-outliers")==="1"}catch{return!1}})();this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._adminPanelUserIds=[];this._operatorWriteEnabled=!1;this._defaultWarningDays=7;this._actionLoading=!1;this._moreMenuOpen=!1;this._toastMessage="";this._toastUndo=null;this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab=(()=>{try{let e=localStorage.getItem("msp-overview-tab");return e==="today"||e==="calendar"?e:"dashboard"}catch{return"dashboard"}})();this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._objectSortMode="alphabetical";this._groupByMode="none";this._objectViewMode="cards";this._objectsTableColumns=dt;this._showArchived=!1;this._bulkMode=!1;this._bulkSelected=new Set;this._virtStart=0;this._virtEnd=0;this._virtRowHeight=53;this._virtTotalRows=0;this._virtScrollAttached=!1;this._virtRaf=0;this._collapsedSections=(()=>{try{return new Set(JSON.parse(localStorage.getItem("msp-collapsed-sections")||"[]"))}catch{return new Set}})();this._paletteOpen=!1;this._paletteQuery="";this._paletteActive=0;this._templateGalleryOpen=!1;this._templates=[];this._templateCategories={};this._templateBusy=!1;this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._onVirtualScroll=()=>{this._virtRaf||(this._virtRaf=requestAnimationFrame(()=>{this._virtRaf=0,this._updateVirtualWindow()}))};this._deepLinkHandled=!1;this._paletteKeydown=e=>{if(e.key==="/"&&!e.ctrlKey&&!e.metaKey&&!e.altKey&&!this._paletteOpen){let i=e.composedPath()[0];if(i instanceof HTMLElement&&(i.tagName==="INPUT"||i.tagName==="TEXTAREA"||i.tagName==="SELECT"||i.isContentEditable))return;e.preventDefault(),this._openPalette();return}if(!this._paletteOpen)return;let t=this._paletteResults;if(e.key==="Escape")e.preventDefault(),this._closePalette();else if(e.key==="ArrowDown")e.preventDefault(),this._paletteActive=Math.min(this._paletteActive+1,t.length-1);else if(e.key==="ArrowUp")e.preventDefault(),this._paletteActive=Math.max(this._paletteActive-1,0);else if(e.key==="Enter"){e.preventDefault();let i=t[this._paletteActive];i&&this._selectPaletteResult(i)}};this._onDialogEvent=async()=>{try{await this._loadData()}catch{}};this._onCalendarLlCustom=e=>{let t=e.detail;t?.type==="maintenance-supporter:open-task"&&t.entry_id&&t.task_id&&(e.stopPropagation(),this._showTask(t.entry_id,t.task_id))};this._onHistoryEntrySaved=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"en"}get _isOperator(){let e=this.hass?.user;return e?e.is_admin?!1:!(this._operatorWriteEnabled&&this._adminPanelUserIds.includes(e.id)):!0}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler),window.addEventListener("keydown",this._paletteKeydown),window.addEventListener("resize",this._onVirtualScroll,{passive:!0});let e=localStorage.getItem("maintenance_supporter_sort");e&&["due_date","object","type","task_name","area","assigned_user","group"].includes(e)&&(this._sortMode=e);let t=localStorage.getItem("maintenance_supporter_object_sort");t&&["alphabetical","due_soonest","task_count"].includes(t)&&(this._objectSortMode=t);let i=localStorage.getItem("maintenance_supporter_groupby");i&&["none","area","group","user"].includes(i)&&(this._groupByMode=i);let a=localStorage.getItem("maintenance_supporter_object_view");(a==="cards"||a==="table")&&(this._objectViewMode=a)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),window.removeEventListener("keydown",this._paletteKeydown),window.removeEventListener("resize",this._onVirtualScroll),this.shadowRoot?.querySelector(".content")?.removeEventListener("scroll",this._onVirtualScroll),this._virtScrollAttached=!1,this._virtRaf&&cancelAnimationFrame(this._virtRaf),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){super.updated(e);let t=this.hass?.language;if(t&&!rt(t)&&ie(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new ht(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new me(this.hass),this._userService.getUsers())}let i=this.shadowRoot?.querySelector(".content");i&&!this._virtScrollAttached&&(i.addEventListener("scroll",this._onVirtualScroll,{passive:!0}),this._virtScrollAttached=!0),this._updateVirtualWindow()}_updateVirtualWindow(){let e=this.shadowRoot?.querySelector(".content"),t=this.shadowRoot?.querySelector(".task-table.virtual");if(!e||!t)return;let i=t.querySelector(".task-row:not(.virt-sizer)");i&&i.offsetHeight>20&&(this._virtRowHeight=i.offsetHeight);let a=t.getBoundingClientRect().top-e.getBoundingClientRect().top+e.scrollTop,l=Ui({scrollTop:e.scrollTop,viewportHeight:e.clientHeight,listTop:a,rowHeight:this._virtRowHeight,total:this._virtTotalRows});(l.start!==this._virtStart||l.end!==this._virtEnd)&&(this._virtStart=l.start,this._virtEnd=l.end)}async _loadData(){let[e,t,i,a,l]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);if(e&&(this._objects=e.objects),t&&(this._stats=t),i&&(this._budget=i),a&&(this._groups=a.groups||{}),l){let c=l;this._features=c.features,this._adminPanelUserIds=c.admin_panel_user_ids||[],this._operatorWriteEnabled=c.operator_write_enabled??!1;let u=c.general?.default_warning_days;typeof u=="number"&&u>=0&&u<=365&&(this._defaultWarningDays=u),this._objectsTableColumns=We(c.objects_table_columns)}this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("ms_action"),i=()=>{let g=window.location.pathname+window.location.hash;history.replaceState(history.state,"",g)};if(t==="add_object"){this._deepLinkHandled=!0,i(),requestAnimationFrame(()=>{this.shadowRoot?.querySelector("maintenance-object-dialog")?.openCreate()});return}if(t==="open_vacation"||t==="open_budget"||t==="open_groups"||t==="open_settings"){this._deepLinkHandled=!0,i(),this._overviewTab="settings",requestAnimationFrame(()=>{let g=this.shadowRoot?.querySelector("maintenance-settings-view"),m=t.replace("open_","");g?.scrollToSection?.(m)});return}let a=e.get("entry_id");if(!a)return;this._deepLinkHandled=!0;let l=e.get("task_id"),c=e.get("action"),u=window.location.pathname+window.location.hash;history.replaceState(history.state,"",u);let h=this._getObject(a);if(!h){this._showOverview();return}if(l){let g=h.tasks.find(m=>m.id===l);if(!g){this._showObject(a);return}this._showTask(a,l),c==="complete"?requestAnimationFrame(()=>{this._openCompleteDialog(a,l,g.name,this._features.checklists?g.checklist:void 0,this._features.adaptive&&!!g.adaptive_config?.enabled)}):c==="quick_complete"&&requestAnimationFrame(()=>{this._handleQuickComplete(a,l,g)})}else this._showObject(a)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let i=await this._statsService.getDetailStats(e,t,this._chartRangeDays),a=new Map(this._detailStatsData);a.set(e,i),this._detailStatsData=a}_setChartRange(e){if(e===this._chartRangeDays)return;this._chartRangeDays=e;try{localStorage.setItem("msp-chart-range",String(e))}catch{}let t=this._selectedEntryId&&this._selectedTaskId?this._getTask(this._selectedEntryId,this._selectedTaskId):null,i=t?.trigger_config?.entity_id;if(i){let a=new Map(this._detailStatsData);a.delete(i),this._detailStatsData=a,this._fetchDetailStats(i,this._isCounterEntity(t.trigger_config))}}_setHideOutliers(e){if(e!==this._hideOutliers){this._hideOutliers=e;try{localStorage.setItem("msp-chart-hide-outliers",e?"1":"0")}catch{}}}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let i of this._objects)for(let a of i.tasks){let l=a.trigger_config?.entity_id;l&&e.push({entityId:l,isCounter:this._isCounterEntity(a.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{let e=await this.hass.connection.subscribeMessage(t=>{let i=t;this._objects=i.objects},{type:"maintenance_supporter/subscribe"});if(!this.isConnected){e();return}this._unsub=e}catch{}}get _taskRows(){let e=[];for(let m of this._objects)for(let v of m.tasks){if(!this._showArchived&&v.archived||this._filterStatus&&v.status!==this._filterStatus)continue;if(this._filterUser){let $=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(v.responsible_user_id!==$)continue}let f=[];for(let $ of Object.values(this._groups))$.task_refs?.some(b=>b.entry_id===m.entry_id&&b.task_id===v.id)&&f.push($.name);e.push({entry_id:m.entry_id,task_id:v.id,object_name:m.object.name,task_name:v.name,type:v.type,schedule_type:v.schedule_type,status:v.status,days_until_due:v.days_until_due??null,next_due:v.next_due??null,trigger_active:v.trigger_active,trigger_current_value:v.trigger_current_value??null,trigger_current_delta:v.trigger_current_delta??null,trigger_config:v.trigger_config??null,trigger_entity_info:v.trigger_entity_info??null,times_performed:v.times_performed,total_cost:v.total_cost,interval_days:v.interval_days??null,interval_unit:v.interval_unit??null,interval_anchor:v.interval_anchor??null,is_done:v.is_done??!1,archived:v.archived??!1,history:v.history||[],enabled:v.enabled,nfc_tag_id:v.nfc_tag_id??null,priority:v.priority??"normal",labels:v.labels??[],area_id:m.object.area_id??null,responsible_user_id:v.responsible_user_id??null,group_names:f})}let t={overdue:0,triggered:1,due_soon:2,ok:3},i=(m,v)=>(t[m.status]??9)-(t[v.status]??9),a=(m,v)=>(m.days_until_due??99999)-(v.days_until_due??99999),l=(m,v)=>i(m,v)||a(m,v),c=m=>m.area_id&&this.hass?.areas?.[m.area_id]?.name||"",u=m=>m.responsible_user_id&&this._userService?.getUserName(m.responsible_user_id)||"",h=m=>m.group_names[0]||"",g={due_date:l,object:(m,v)=>m.object_name.localeCompare(v.object_name)||l(m,v),type:(m,v)=>m.type.localeCompare(v.type)||l(m,v),task_name:(m,v)=>m.task_name.localeCompare(v.task_name),area:(m,v)=>{let f=c(m),$=c(v);return!f&&$?1:f&&!$?-1:f.localeCompare($)||l(m,v)},assigned_user:(m,v)=>{let f=u(m),$=u(v);return!f&&$?1:f&&!$?-1:f.localeCompare($)||l(m,v)},group:(m,v)=>{let f=h(m),$=h(v);return!f&&$?1:f&&!$?-1:f.localeCompare($)||l(m,v)}};return e.sort(g[this._sortMode]),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(a=>a.id===t)}_pushPanelState(e,t,i){let a={msp_view:e,msp_entry:t||null,msp_task:i||null};history.pushState(a,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let i=this._getTask(t.msp_entry,t.msp_task);i?.trigger_config?.entity_id&&this._fetchDetailStats(i.trigger_config.entity_id,this._isCounterEntity(i.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1,this._scrollContentToTop()}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null,this._scrollContentToTop()}_filterByStatus(e){this._filterStatus=e,this._overviewTab!=="dashboard"&&(this._overviewTab="dashboard"),this._scrollContentToTop()}_scrollContentToTop(){requestAnimationFrame(()=>{let e=this.shadowRoot?.querySelector(".content");e&&e.scrollTo({top:0,behavior:"smooth"})})}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null,this._scrollContentToTop()}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null,this._scrollContentToTop();let i=this._getTask(e,t);if(i?.trigger_config?.entity_id){let a=i.trigger_config.entity_id,l=this._isCounterEntity(i.trigger_config);this._fetchDetailStats(a,l)}}_showToast(e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastUndo=null,this._toastMessage=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}_showUndoToast(e,t){this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage=e,this._toastUndo=t,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastUndo=null,this._toastTimer=null},7e3)}_runToastUndo(){let e=this._toastUndo;this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage="",this._toastUndo=null,this._toastTimer=null,e?.()}_openPalette(){this._paletteQuery="",this._paletteActive=0,this._paletteOpen=!0,this.updateComplete.then(()=>{this.shadowRoot?.querySelector(".palette-input")?.focus()})}_closePalette(){this._paletteOpen=!1,this._paletteQuery=""}get _paletteResults(){let e=this._paletteQuery.trim().toLowerCase(),t=[];for(let i of this._objects){let a=i.object.name||"";(!e||a.toLowerCase().includes(e))&&t.push({kind:"object",entryId:i.entry_id,label:a,sub:s("object",this._lang)});for(let l of i.tasks){if(l.archived)continue;let c=l.name||"",u=(l.labels||[]).some(h=>h.toLowerCase().includes(e));if(!e||c.toLowerCase().includes(e)||a.toLowerCase().includes(e)||u){let h=(l.labels||[]).length?`  #${(l.labels||[]).join(" #")}`:"";t.push({kind:"task",entryId:i.entry_id,taskId:l.id,label:c,sub:a+h})}}if(t.length>60)break}return t.slice(0,40)}_selectPaletteResult(e){this._closePalette(),e.kind==="task"&&e.taskId?this._showTask(e.entryId,e.taskId):this._showObject(e.entryId)}_renderPalette(){if(!this._paletteOpen)return p;let e=this._lang,t=this._paletteResults;return o`
    ...
    `}_statusBadge(e,t,i){let a=this._lang,l=e?"archived":t?"done":i,c=e?"archived":t?"completed":i,u=e?s("archived",a):t?s("completed",a):s(i,a);return o`<span class="status-badge ${l}"><ha-icon icon="${nt[c]||"mdi:circle-medium"}"></ha-icon>${u}</span>`}_setOverviewTab(e){this._overviewTab=e;try{localStorage.setItem("msp-overview-tab",e)}catch{}this._scrollContentToTop()}_renderToday(){let e=this._lang,t=this._taskRows,i=h=>`${h.entry_id}:${h.task_id}`,a=t.filter(h=>h.status==="overdue"||h.trigger_active),l=new Set(a.map(i)),c=t.filter(h=>!l.has(i(h))&&h.days_until_due===0);c.forEach(h=>l.add(i(h)));let u=t.filter(h=>!l.has(i(h))&&h.days_until_due!=null&&h.days_until_due>0&&h.days_until_due<=7);return a.length+c.length+u.length===0?o`
      ...
      <div class="today-view">
        ...
        ${this._renderTodaySection("today_this_week",u,"")}
      </div>
        ...
        ${t.map(l=>o`
          <div class="today-row" @click=${()=>this._showTask(l.entry_id,l.task_id)}>
            ...
            <div class="today-main">
              ...
              <div class="today-object">${l.object_name} · ${je(l.days_until_due,a)}</div>
            </div>
    ...
    `}_renderDashboard(){let e=this._stats,t=this._taskRows,i=this._lang,a=this._isOperator,l=this._objects.reduce((c,u)=>c+u.tasks.filter(h=>h.archived).length,0);return o`
    ...
    `}_renderTaskTable(e){let t=this._bulkMode?" bulk":"";if(this._virtTotalRows=e.length,this.narrow||e.length<120)return o`
      ...
      `;let i=e.length,a=this._virtRowHeight,l=Math.max(0,Math.min(this._virtStart,i)),c=this._virtEnd>0?Math.min(this._virtEnd,i):Math.min(i,40);c<l&&(l=0,c=Math.min(i,40));let u=l*a,h=(i-c)*a;return o`
    ...
    `}_renderVirtSizerRow(e){let t=this._lang,i="",a=!1,l=!1,c=!1;for(let u of e){let h=u.archived?s("archived",t):u.is_done?s("completed",t):s(u.status,t);h.length>i.length&&(i=h),u.enabled||(a=!0),u.nfc_tag_id&&(l=!0),(u.priority==="high"||u.priority==="low")&&(c=!0)}return o`
    ...
    `}_renderGroupedTasks(e,t){let i=new Map,a=s("unassigned",t);for(let u of e){let h=[];this._groupByMode==="area"?h=[(u.area_id?this.hass?.areas?.[u.area_id]?.name:null)||a]:this._groupByMode==="user"?h=[(u.responsible_user_id?this._userService?.getUserName(u.responsible_user_id):null)||a]:this._groupByMode==="group"&&(h=u.group_names.length>0?u.group_names:[a]);for(let g of h)i.has(g)||i.set(g,[]),i.get(g).push(u)}let l=[...i.entries()].sort(([u],[h])=>u===a&&h!==a?1:h===a&&u!==a?-1:u.localeCompare(h)),c=this._groupByMode==="area"?"mdi:map-marker-outline":this._groupByMode==="group"?"mdi:folder-outline":"mdi:account-outline";return o`
      ${l.map(([u,h])=>o`
    ...
    `}_renderOverviewRow(e){let t=this._lang,i=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,a=0,l=at.ok,c=!1;if(i&&e.days_until_due!==null){let v=ct(e.interval_days,e.days_until_due,e.interval_unit);a=v.pct,c=v.overflow,e.status==="overdue"?l=at.overdue:e.status==="due_soon"&&(l=at.due_soon)}let u=e.area_id?this.hass?.areas?.[e.area_id]?.name:null,h=e.responsible_user_id?this._userService?.getUserName(e.responsible_user_id):null,g=e.group_names.length>0||u||h,m=this._bulkMode&&this._bulkSelected.has(this._bulkKey(e));return o`
      <div class="task-row${e.enabled?"":" task-disabled"}${m?" bulk-selected":""}">
        ...
        <span class="due-cell" @click=${()=>this._showTask(e.entry_id,e.task_id)}>
          ...
          ${e.trigger_config?Dt(e):!i&&e.trigger_active?o`<span style="color:var(--maint-triggered-color);font-weight:600">⚡</span>`:p}
          ${Ot(e,this._miniStatsData,this._lang)}
        </span>
    ...
    `}_openCompleteDialogForRow(e){let i=this._objects.find(a=>a.entry_id===e.entry_id)?.tasks.find(a=>a.id===e.task_id);this._openCompleteDialog(e.entry_id,e.task_id,e.task_name,this._features.checklists?i?.checklist:void 0,this._features.adaptive&&!!i?.adaptive_config?.enabled)}_renderObjectDetail(){if(!this._selectedEntryId)return p;let e=this._getObject(this._selectedEntryId);if(!e)return o`<p>Object not found.</p>`;let t=e.object,i=this._lang,a=this._isOperator,l=e.tasks.filter(u=>u.archived).length,c=e.tasks.filter(u=>this._showArchived||!u.archived);return o`
      <div class="detail-section">
        ...
        ${e.tasks.length===0?o`<div class="empty-state-centered">
            ...
            </div>`:o`<div class="task-table">${[...c].sort((u,h)=>{let g={overdue:0,triggered:1,due_soon:2,ok:3};return(g[u.status]??9)-(g[h.status]??9)||(u.days_until_due??99999)-(h.days_until_due??99999)}).map(u=>o`
              <div class="task-row${u.enabled?"":" task-disabled"}">
                <span class="cell-badges">
                  ${this._statusBadge(!!u.archived,!!u.is_done,u.status)}
                  ${u.enabled?p:o`<span class="badge-disabled">${s("disabled",i)}</span>`}
                  ${u.nfc_tag_id?o`<span class="nfc-badge" title="${s("nfc_linked",i)}"><ha-icon icon="mdi:nfc-variant"></ha-icon></span>`:p}
                </span>
                <span class="cell task-name" @click=${()=>this._showTask(e.entry_id,u.id)}>${u.name}</span>
                <span class="task-sub${u.responsible_user_id?"":" task-sub-empty"}">${Ft(u,h=>this._userService?.getUserName(h)??null)}</span>
                <span class="cell type">${s(u.type,i)}</span>
                <span class="due-cell" @click=${()=>this._showTask(e.entry_id,u.id)}>
                  <span class="due-text">${je(u.days_until_due,i)}</span>
                  ${u.trigger_config?Dt(u):p}
                  ${Ot(u,this._miniStatsData,this._lang)}
                </span>
                <span class="row-actions">
                  <mwc-icon-button class="btn-complete" title="${s("complete",i)}" @click=${h=>{h.stopPropagation(),this._openCompleteDialog(e.entry_id,u.id,u.name,this._features.checklists?u.checklist:void 0,this._features.adaptive&&!!u.adaptive_config?.enabled)}}>
                    <ha-icon icon="mdi:check"></ha-icon>
                  </mwc-icon-button>
                  <mwc-icon-button class="btn-skip" title="${s("skip",i)}" .disabled=${this._actionLoading} @click=${h=>{h.stopPropagation(),this._promptSkipTask(e.entry_id,u.id)}}>
                    <ha-icon icon="mdi:skip-next"></ha-icon>
                  </mwc-icon-button>
                </span>
              </div>
