...
`;var we=class{constructor(i){this._cache=new Map;this._pending=new Map;this._hass=i}updateHass(i){this._hass=i}async getDetailStats(i,e){return this._getStats(i,"hour",30,e)}async getMiniStats(i,e){return this._getStats(i,"day",14,e)}async getBatchMiniStats(i){let e=new Map,t=[];for(let u of i){let _=`${u.entityId}:day`,m=this._cache.get(_);m&&Date.now()-m.fetchedAt<3e5?e.set(u.entityId,m.points):t.push(u)}if(t.length===0)return e;let a=t.filter(u=>u.isCounter).map(u=>u.entityId),s=t.filter(u=>!u.isCounter).map(u=>u.entityId),o=new Date(Date.now()-336*60*60*1e3).toISOString(),c=[];return a.length>0&&c.push(this._fetchBatch(a,"day",o,["state","sum","change"],!0,e)),s.length>0&&c.push(this._fetchBatch(s,"day",o,["mean","min","max"],!1,e)),await Promise.all(c),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(i,e,t,a){let s=`${i}:${e}`,o=this._cache.get(s);if(o&&Date.now()-o.fetchedAt<3e5)return o.points;if(this._pending.has(s))return this._pending.get(s);let c=this._fetchAndNormalize(i,e,t,a,s);this._pending.set(s,c);try{return await c}finally{this._pending.delete(s)}}async _fetchAndNormalize(i,e,t,a,s){let o=new Date(Date.now()-t*24*60*60*1e3).toISOString(),c=a?["state","sum","change"]:["mean","min","max"];try{let _=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:o,statistic_ids:[i],period:e,types:c}))[i]||[],m=this._normalizeRows(_,a);return this._cache.set(s,{entityId:i,fetchedAt:Date.now(),period:e,points:m}),m}catch(u){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${i}:`,u),[]}}async _fetchBatch(i,e,t,a,s,o){try{let c=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:i,period:e,types:a});for(let u of i){let _=c[u]||[],m=this._normalizeRows(_,s);o.set(u,m),this._cache.set(`${u}:${e}`,{entityId:u,fetchedAt:Date.now(),period:e,points:m})}}catch(c){console.warn("[maintenance-supporter] Batch statistics fetch failed:",c)}}_normalizeRows(i,e){let t=[];for(let a of i){let s=null;if(e?s=a.state??null:s=a.mean??null,s===null)continue;let o={ts:a.start,val:s};e||(a.min!=null&&(o.min=a.min),a.max!=null&&(o.max=a.max)),t.push(o)}return t.sort((a,s)=>a.ts-s.ts),t}};var re=class{constructor(i){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=i}updateHass(i){this.hass=i}async getUsers(i=!1){let e=Date.now();if(!i&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(i,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:i,task_id:e,user_id:t})}async getTasksByUser(i){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:i})).tasks}getUserName(i){return!i||!this.usersCache?null:this.usersCache.find(t=>t.id===i)?.name||null}getUser(i){return!i||!this.usersCache?null:this.usersCache.find(e=>e.id===i)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(i){return i?i===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var H=class extends L{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._name="";this._manufacturer="";this._model="";this._serialNumber="";this._areaId="";this._installationDate="";this._entryId=null}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}openCreate(){this._entryId=null,this._name="",this._manufacturer="",this._model="",this._serialNumber="",this._areaId="",this._installationDate="",this._error="",this._open=!0}openEdit(e,t){this._entryId=e,this._name=t.name||"",this._manufacturer=t.manufacturer||"",this._model=t.model||"",this._serialNumber=t.serial_number||"",this._areaId=t.area_id||"",this._installationDate=t.installation_date||"",this._error="",this._open=!0}async _save(){if(this._name.trim()){this._loading=!0,this._error="";try{this._entryId?await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/update",entry_id:this._entryId,name:this._name,manufacturer:this._manufacturer||null,model:this._model||null,serial_number:this._serialNumber||null,area_id:this._areaId||null,installation_date:this._installationDate||null}):await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/create",name:this._name,manufacturer:this._manufacturer||null,model:this._model||null,serial_number:this._serialNumber||null,area_id:this._areaId||null,installation_date:this._installationDate||null}),this._open=!1,this.dispatchEvent(new CustomEvent("object-saved"))}catch{this._error=n("save_error",this._lang)}finally{this._loading=!1}}}_close(){this._open=!1}render(){if(!this._open)return l``;let e=this._lang,t=this._entryId?n("edit_object",e):n("new_object",e);return l`
  ...
  `,p([S({attribute:!1})],H.prototype,"hass",2),p([g()],H.prototype,"_open",2),p([g()],H.prototype,"_loading",2),p([g()],H.prototype,"_error",2),p([g()],H.prototype,"_name",2),p([g()],H.prototype,"_manufacturer",2),p([g()],H.prototype,"_model",2),p([g()],H.prototype,"_serialNumber",2),p([g()],H.prototype,"_areaId",2),p([g()],H.prototype,"_installationDate",2),p([g()],H.prototype,"_entryId",2);customElements.get("maintenance-object-dialog")||customElements.define("maintenance-object-dialog",H);var sa=["cleaning","inspection","replacement","calibration","service","custom"],ra=["time_based","sensor_based","manual"],oa=["threshold","counter","state_change","runtime"],$=class extends L{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._entryId="";this._taskId=null;this._name="";this._type="custom";this._scheduleType="time_based";this._intervalDays="30";this._warningDays="7";this._intervalAnchor="completion";this._notes="";this._documentationUrl="";this._customIcon="";this._enabled=!0;this._triggerEntityId="";this._triggerEntityIds=[];this._triggerEntityLogic="any";this._triggerAttribute="";this._triggerType="threshold";this._triggerAbove="";this._triggerBelow="";this._triggerForMinutes="0";this._triggerTargetValue="";this._triggerDeltaMode=!1;this._triggerFromState="";this._triggerToState="";this._triggerTargetChanges="";this._triggerRuntimeHours="";this._suggestedAttributes=[];this._availableAttributes=[];this._entityDomain="";this._lastPerformed="";this._nfcTagId="";this._availableTags=[];this._responsibleUserId=null;this._availableUsers=[];this._userService=null}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}async openCreate(e){this._entryId=e,this._taskId=null,this._error="",this._resetFields(),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}async openEdit(e,t){if(this._entryId=e,this._taskId=t.id,this._error="",this._name=t.name,this._type=t.type,this._scheduleType=t.schedule_type,this._intervalDays=t.interval_days?.toString()||"30",this._warningDays=t.warning_days.toString(),this._intervalAnchor=t.interval_anchor||"completion",this._notes=t.notes||"",this._documentationUrl=t.documentation_url||"",this._customIcon=t.custom_icon||"",this._enabled=t.enabled!==!1,this._lastPerformed=t.last_performed||"",this._nfcTagId=t.nfc_tag_id||"",this._responsibleUserId=t.responsible_user_id||null,t.trigger_config){let a=t.trigger_config;this._triggerEntityId=a.entity_id||"",this._triggerEntityIds=a.entity_ids||(a.entity_id?[a.entity_id]:[]),this._triggerEntityLogic=a.entity_logic||"any",this._triggerAttribute=a.attribute||"",this._triggerType=a.type||"threshold",this._triggerAbove=a.trigger_above?.toString()||"",this._triggerBelow=a.trigger_below?.toString()||"",this._triggerForMinutes=a.trigger_for_minutes?.toString()||"0",this._triggerTargetValue=a.trigger_target_value?.toString()||"",this._triggerDeltaMode=a.trigger_delta_mode||!1,this._triggerFromState=a.trigger_from_state||"",this._triggerToState=a.trigger_to_state||"",this._triggerTargetChanges=a.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=a.trigger_runtime_hours?.toString()||""}else this._resetTriggerFields();this._triggerEntityId&&this._fetchEntityAttributes(this._triggerEntityId),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._warningDays="7",this._intervalAnchor="completion",this._notes="",this._documentationUrl="",this._customIcon="",this._enabled=!0,this._lastPerformed="",this._nfcTagId="",this._responsibleUserId=null,this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerForMinutes="0",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours=""}async _loadUsers(){this._userService||(this._userService=new re(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}async _loadTags(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tags/list"});this._availableTags=e.tags||[]}catch{this._availableTags=[]}}async _fetchEntityAttributes(e){if(!e||!this.hass){this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="";return}try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/entity/attributes",entity_id:e});this._entityDomain=t.domain||"",this._suggestedAttributes=t.suggested_attributes||[],this._availableAttributes=t.available_attributes||[]}catch{this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain=""}}async _save(){if(this._name.trim()){this._loading=!0,this._error="";try{let e={type:this._taskId?"maintenance_supporter/task/update":"maintenance_supporter/task/create",entry_id:this._entryId,name:this._name,task_type:this._type,schedule_type:this._scheduleType,warning_days:parseInt(this._warningDays,10)||7};if(this._taskId&&(e.task_id=this._taskId),this._scheduleType!=="manual"?this._intervalDays?(e.interval_days=parseInt(this._intervalDays,10),e.interval_anchor=this._intervalAnchor):this._taskId&&(e.interval_days=null,e.interval_anchor="completion"):this._taskId&&(e.interval_days=null,e.interval_anchor="completion"),e.notes=this._notes||null,e.documentation_url=this._documentationUrl||null,e.custom_icon=this._customIcon||null,e.enabled=this._enabled,e.last_performed=this._lastPerformed||null,e.nfc_tag_id=this._nfcTagId||null,e.responsible_user_id=this._responsibleUserId,this._scheduleType==="sensor_based"&&this._triggerEntityId){let t=this._triggerEntityIds.length>0?this._triggerEntityIds:[this._triggerEntityId],a={entity_id:t[0],entity_ids:t,type:this._triggerType};if(this._triggerAttribute&&(a.attribute=this._triggerAttribute),t.length>1&&(a.entity_logic=this._triggerEntityLogic),this._triggerType==="threshold"){if(this._triggerAbove){let s=parseFloat(this._triggerAbove);isNaN(s)||(a.trigger_above=s)}if(this._triggerBelow){let s=parseFloat(this._triggerBelow);isNaN(s)||(a.trigger_below=s)}if(this._triggerForMinutes){let s=parseInt(this._triggerForMinutes,10);isNaN(s)||(a.trigger_for_minutes=s)}}else if(this._triggerType==="counter"){if(this._triggerTargetValue){let s=parseFloat(this._triggerTargetValue);isNaN(s)||(a.trigger_target_value=s)}a.trigger_delta_mode=this._triggerDeltaMode}else if(this._triggerType==="state_change"){if(this._triggerFromState&&(a.trigger_from_state=this._triggerFromState),this._triggerToState&&(a.trigger_to_state=this._triggerToState),this._triggerTargetChanges){let s=parseInt(this._triggerTargetChanges,10);isNaN(s)||(a.trigger_target_changes=s)}}else if(this._triggerType==="runtime"&&this._triggerRuntimeHours){let s=parseFloat(this._triggerRuntimeHours);isNaN(s)||(a.trigger_runtime_hours=s)}e.trigger_config=a}else this._taskId&&(e.trigger_config=null);await this.hass.connection.sendMessagePromise(e),this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch{this._error=n("save_error",this._lang)}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return d;let e=this._lang;return l`
      ...
      <div class="select-row">
        ...
        >
          ${oa.map(t=>l`<option value=${t} ?selected=${t===this._triggerType}>${n(t,e)}</option>`)}
        </select>
      </div>
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${sa.map(a=>l`<option value=${a} ?selected=${a===this._type}>${n(a,e)}</option>`)}
            </select>
          </div>
          <div class="select-row">
            ...
            >
              ${ra.map(a=>l`<option value=${a} ?selected=${a===this._scheduleType}>${n(a,e)}</option>`)}
            </select>
          </div>
      ...
      <div class="trigger-limits">
        ...
        ${v==="compound"?l`
          ...
          ${(e.conditions||[]).map((f,k)=>l`
            <span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${k+1}. ${n(f.type||"unknown",t)}: ${f.entity_id?l`<span class="entity-link" @click=${T=>ie(T,f.entity_id)}>${f.entity_id}</span>`:""}</span>
          `)}
        `:d}
    ...
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${v} ${b}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_sparkline",e.lang)}">
        ...
        ${Tt.map(h=>{let w=I(h.ts),F=h.type==="completed"?"var(--success-color, #4caf50)":h.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return j`
          ...
          `})}
        ${Se.map(h=>{let w=I(h.ts),F=M(h.val),R=new Date(h.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),J=`${h.val.toFixed(1)} ${i}`;return m&&h.min!=null&&h.max!=null&&(J+=` (${h.min.toFixed(1)}\u2013${h.max.toFixed(1)})`),j`<circle cx="${w.toFixed(1)}" cy="${F.toFixed(1)}" r="8" fill="transparent" tabindex="0"
            @mouseenter=${Z=>gt(Z,`${R}
${J}`,e.setTooltip)}
            @focus=${Z=>gt(Z,`${R}
${J}`,e.setTooltip)}
...
`).map(h=>l`<div>${h}</div>`)}
  ...
  `}function gt(r,i,e){let t=r.currentTarget,a=t.closest(".sparkline-container");if(!a)return;let s=a.getBoundingClientRect(),o=t.getBoundingClientRect();e({x:o.left-s.left+o.width/2,y:o.top-s.top-8,text:i})}function mt(r,i,e){let t=r.degradation_trend!=null&&r.degradation_trend!=="insufficient_data",a=r.days_until_threshold!=null,s=r.environmental_factor!=null&&r.environmental_factor!==1;if(!t&&!a&&!s)return d;let o=r.degradation_trend==="rising"?"M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z":r.degradation_trend==="falling"?"M16,18L18.29,15.71L13.41,10.83L9.41,14.83L2,7.41L3.41,6L9.41,12L13.41,8L19.71,14.29L22,12V18H16Z":"M22,12L18,8V11H3V13H18V16L22,12Z";return l`
  ...
  `}function vt(r,i){let e=r.interval_analysis,t=e?.weibull_beta,a=e?.weibull_eta;if(t==null||a==null||a<=0)return d;let s=r.interval_days??0,o=r.suggested_interval??s;return l`
  ...
  `}function ma(r,i,e){let t=r.confidence_interval_low,a=r.confidence_interval_high,s=i.suggested_interval??i.interval_days??0,o=i.interval_days??0,c=Math.max(0,t-5),_=a+5-c,m=(t-c)/_*100,v=(a-t)/_*100,b=(s-c)/_*100,f=o>0?(o-c)/_*100:-1;return l`
  ...
  `}function yt(r,i){return va(r,i)}function va(r,i){let e=r.seasonal_factors??r.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return d;let t=r.interval_analysis?.seasonal_reason,a=new Date().getMonth(),s=300,o=100,c=8,_=o-c-4,m=Math.max(...e,1.5),v=s/12,b=v*.65,f=c+_-1/m*_;return l`
    <div class="seasonal-chart">
      ...
      <div class="seasonal-labels">
        ${ft.map((k,T)=>l`<span class="seasonal-label ${T===a?"active-month":""}">${n(k,i)}</span>`)}
      </div>
    </div>
  `}var fa=300,ba=200;function xt(r,i,e,t){let a=r.history.filter(c=>c.type==="completed"&&(c.cost!=null||c.duration!=null));if(a.length<2)return d;let s=a.some(c=>(c.cost??0)>0),o=a.some(c=>(c.duration??0)>0);return!s&&!o?d:l`
  ...
  `}var xa=60,$a=20,$t=30,E=class extends L{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._sparklineTooltip=null;this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1};this._actionLoading=!1;this._moreMenuOpen=!1;this._toastMessage="";this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab="dashboard";this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._deepLinkHandled=!1;this._onDialogEvent=async()=>{try{await this._loadData()}catch{}}}get _lang(){return this.hass?.language||"en"}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler);let e=localStorage.getItem("maintenance_supporter_sort");e&&["due_date","object","type","task_name"].includes(e)&&(this._sortMode=e)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new we(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new re(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,a,s,o]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);e&&(this._objects=e.objects),t&&(this._stats=t),a&&(this._budget=a),s&&(this._groups=s.groups||{}),o&&(this._features=o.features),this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("entry_id");if(!t)return;this._deepLinkHandled=!0;let a=e.get("task_id"),s=e.get("action"),o=window.location.pathname+window.location.hash;history.replaceState(history.state,"",o);let c=this._getObject(t);if(!c){this._showOverview();return}if(a){let u=c.tasks.find(_=>_.id===a);if(!u){this._showObject(t);return}this._showTask(t,a),s==="complete"&&requestAnimationFrame(()=>{this._openCompleteDialog(t,a,u.name,this._features.checklists?u.checklist:void 0,this._features.adaptive&&!!u.adaptive_config?.enabled)})}else this._showObject(t)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let a=await this._statsService.getDetailStats(e,t),s=new Map(this._detailStatsData);s.set(e,a),this._detailStatsData=s}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let a of this._objects)for(let s of a.tasks){let o=s.trigger_config?.entity_id;o&&e.push({entityId:o,isCounter:this._isCounterEntity(s.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let u of this._objects)for(let _ of u.tasks)if(!(this._filterStatus&&_.status!==this._filterStatus)){if(this._filterUser){let m=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(_.responsible_user_id!==m)continue}e.push({entry_id:u.entry_id,task_id:_.id,object_name:u.object.name,task_name:_.name,type:_.type,schedule_type:_.schedule_type,status:_.status,days_until_due:_.days_until_due??null,next_due:_.next_due??null,trigger_active:_.trigger_active,trigger_current_value:_.trigger_current_value??null,trigger_current_delta:_.trigger_current_delta??null,trigger_config:_.trigger_config??null,trigger_entity_info:_.trigger_entity_info??null,times_performed:_.times_performed,total_cost:_.total_cost,interval_days:_.interval_days??null,interval_anchor:_.interval_anchor??null,history:_.history||[],enabled:_.enabled,nfc_tag_id:_.nfc_tag_id??null})}let t={overdue:0,triggered:1,due_soon:2,ok:3},a=(u,_)=>(t[u.status]??9)-(t[_.status]??9),s=(u,_)=>(u.days_until_due??99999)-(_.days_until_due??99999),o=(u,_)=>a(u,_)||s(u,_),c={due_date:o,object:(u,_)=>u.object_name.localeCompare(_.object_name)||o(u,_),type:(u,_)=>u.type.localeCompare(_.type)||o(u,_),task_name:(u,_)=>u.task_name.localeCompare(_.task_name)};return e.sort(c[this._sortMode]),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(s=>s.id===t)}_pushPanelState(e,t,a){let s={msp_view:e,msp_entry:t||null,msp_task:a||null};history.pushState(s,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let a=this._getTask(t.msp_entry,t.msp_task);a?.trigger_config?.entity_id&&this._fetchDetailStats(a.trigger_config.entity_id,this._isCounterEntity(a.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null;let a=this._getTask(e,t);if(a?.trigger_config?.entity_id){let s=a.trigger_config.entity_id,o=this._isCounterEntity(a.trigger_config);this._fetchDetailStats(s,o)}}_showToast(e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}async _deleteObject(e){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:n("delete",this._lang),message:n("confirm_delete_object",this._lang),confirmText:n("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}async _deleteTask(e,t){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:n("delete",this._lang),message:n("confirm_delete_task",this._lang),confirmText:n("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}async _skipTask(e,t,a){this._actionLoading=!0;try{let s={type:"maintenance_supporter/task/skip",entry_id:e,task_id:t};a&&(s.reason=a),await this.hass.connection.sendMessagePromise(s),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}finally{this._actionLoading=!1}}async _resetTask(e,t,a){this._actionLoading=!0;try{let s={type:"maintenance_supporter/task/reset",entry_id:e,task_id:t};a&&(s.date=a),await this.hass.connection.sendMessagePromise(s),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}finally{this._actionLoading=!1}}async _applySuggestion(e,t,a){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:a}),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}async _reanalyzeInterval(e,t){try{let a=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:e,task_id:t});a.recommended_interval?this._showToast(`${n("reanalyze_result",this._lang)}: ${a.recommended_interval} ${n("days",this._lang)} (${n(`confidence_${a.confidence}`,this._lang)}, ${a.data_points} ${n("data_points",this._lang)})`):this._showToast(n("reanalyze_insufficient_data",this._lang)),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}async _promptSkipTask(e,t){let a=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!a)return;let s=await a.prompt({title:n("skip",this._lang),message:n("skip_reason_prompt",this._lang),confirmText:n("skip",this._lang),inputLabel:n("reason_optional",this._lang),inputType:"text"});s.confirmed&&this._skipTask(e,t,s.value||void 0)}async _promptResetTask(e,t){let a=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!a)return;let s=await a.prompt({title:n("reset",this._lang),message:n("reset_date_prompt",this._lang),confirmText:n("reset",this._lang),inputLabel:n("reset_date_optional",this._lang),inputType:"date"});s.confirmed&&this._resetTask(e,t,s.value||void 0)}_dismissSuggestion(e,t){e&&t&&this._dismissedSuggestions.add(`${e}_${t}`),this.requestUpdate()}_openCompleteDialog(e,t,a,s,o){let c=this.shadowRoot.querySelector("maintenance-complete-dialog");c&&(c.entryId=e,c.taskId=t,c.taskName=a,c.lang=this._lang,c.checklist=s||[],c.adaptiveEnabled=!!o,c.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,a,s){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,a,s)}render(){return l`
    ...
    `}_renderOverviewRow(e){let t=this._lang,a=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,s=0,o=$e.ok,c=!1;if(a&&e.days_until_due!==null){let u=(e.interval_days-e.days_until_due)/e.interval_days*100;s=Math.max(0,Math.min(100,u)),c=u>100,e.status==="overdue"?o=$e.overdue:e.status==="due_soon"&&(o=$e.due_soon)}return l`
    ...
    `}_renderMiniSparkline(e){if(!e.trigger_config?.entity_id)return d;let t=e.trigger_config.entity_id,a=this._miniStatsData.get(t)||[],s=[];if(a.length>=2)s=a.map(y=>({ts:y.ts,val:y.val}));else{if(!e.history)return d;for(let y of e.history)y.trigger_value!=null&&s.push({ts:new Date(y.timestamp).getTime(),val:y.trigger_value})}if(e.trigger_current_value!=null&&s.push({ts:Date.now(),val:e.trigger_current_value}),s.length<2)return d;s.sort((y,z)=>y.ts-z.ts);let o=xa,c=$a,u=s.map(y=>y.val),_=Math.min(...u),m=Math.max(...u),v=m-_||1;_-=v*.1,m+=v*.1;let b=s[0].ts,k=s[s.length-1].ts-b||1,T=y=>(y-b)/k*o,q=y=>2+(1-(y-_)/(m-_))*(c-4),C=s;if(C.length>$t){let y=Math.ceil(C.length/$t);C=C.filter((z,O)=>O%y===0||O===C.length-1)}let A=C.map(y=>`${T(y.ts).toFixed(1)},${q(y.val).toFixed(1)}`).join(" ");return l`
    ...
    `}_renderObjectDetail(){if(!this._selectedEntryId)return d;let e=this._getObject(this._selectedEntryId);if(!e)return l`<p>Object not found.</p>`;let t=e.object,a=this._lang;return l`
      <div class="detail-section">
        ...
        ${t.installation_date?l`<p class="meta">${n("installed",a)}: ${Y(t.installation_date,a)}</p>`:d}
        ...
        ${e.tasks.length===0?l`<div class="empty-state-centered">
              ...
              <div class="task-row${s.enabled?"":" task-disabled"}">
                ...
                <span class="due-cell" @click=${()=>this._showTask(e.entry_id,s.id)}>
                  <span class="due-text">${ke(s.days_until_due,a)}</span>
