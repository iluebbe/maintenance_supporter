...
`;var xe=class{constructor(a){this._cache=new Map;this._pending=new Map;this._hass=a}updateHass(a){this._hass=a}async getDetailStats(a,e){return this._getStats(a,"hour",30,e)}async getMiniStats(a,e){return this._getStats(a,"day",14,e)}async getBatchMiniStats(a){let e=new Map,t=[];for(let u of a){let p=`${u.entityId}:day`,_=this._cache.get(p);_&&Date.now()-_.fetchedAt<3e5?e.set(u.entityId,_.points):t.push(u)}if(t.length===0)return e;let i=t.filter(u=>u.isCounter).map(u=>u.entityId),r=t.filter(u=>!u.isCounter).map(u=>u.entityId),o=new Date(Date.now()-336*60*60*1e3).toISOString(),d=[];return i.length>0&&d.push(this._fetchBatch(i,"day",o,["state","sum","change"],!0,e)),r.length>0&&d.push(this._fetchBatch(r,"day",o,["mean","min","max"],!1,e)),await Promise.all(d),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(a,e,t,i){let r=`${a}:${e}`,o=this._cache.get(r);if(o&&Date.now()-o.fetchedAt<3e5)return o.points;if(this._pending.has(r))return this._pending.get(r);let d=this._fetchAndNormalize(a,e,t,i,r);this._pending.set(r,d);try{return await d}finally{this._pending.delete(r)}}async _fetchAndNormalize(a,e,t,i,r){let o=new Date(Date.now()-t*24*60*60*1e3).toISOString(),d=i?["state","sum","change"]:["mean","min","max"];try{let p=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:o,statistic_ids:[a],period:e,types:d}))[a]||[],_=this._normalizeRows(p,i);return this._cache.set(r,{entityId:a,fetchedAt:Date.now(),period:e,points:_}),_}catch(u){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${a}:`,u),[]}}async _fetchBatch(a,e,t,i,r,o){try{let d=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:a,period:e,types:i});for(let u of a){let p=d[u]||[],_=this._normalizeRows(p,r);o.set(u,_),this._cache.set(`${u}:${e}`,{entityId:u,fetchedAt:Date.now(),period:e,points:_})}}catch(d){console.warn("[maintenance-supporter] Batch statistics fetch failed:",d)}}_normalizeRows(a,e){let t=[];for(let i of a){let r=null;if(e?r=i.state??null:r=i.mean??null,r===null)continue;let o={ts:i.start,val:r};e||(i.min!=null&&(o.min=i.min),i.max!=null&&(o.max=i.max)),t.push(o)}return t.sort((i,r)=>i.ts-r.ts),t}};var re=class{constructor(a){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=a}async getUsers(a=!1){let e=Date.now();if(!a&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(a,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:a,task_id:e,user_id:t})}async getTasksByUser(a){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:a})).tasks}getUserName(a){return!a||!this.usersCache?null:this.usersCache.find(t=>t.id===a)?.name||null}getUser(a){return!a||!this.usersCache?null:this.usersCache.find(e=>e.id===a)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(a){return a?a===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var O=class extends I{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._name="";this._manufacturer="";this._model="";this._entryId=null}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}openCreate(){this._entryId=null,this._name="",this._manufacturer="",this._model="",this._open=!0}openEdit(e,t){this._entryId=e,this._name=t.name||"",this._manufacturer=t.manufacturer||"",this._model=t.model||"",this._open=!0}async _save(){if(this._name.trim()){this._loading=!0;try{this._entryId?await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/update",entry_id:this._entryId,name:this._name,manufacturer:this._manufacturer||null,model:this._model||null}):await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/create",name:this._name,manufacturer:this._manufacturer||null,model:this._model||null}),this._open=!1,this.dispatchEvent(new CustomEvent("object-saved"))}finally{this._loading=!1}}}_close(){this._open=!1}render(){if(!this._open)return l``;let e=this._lang,t=this._entryId?s("edit_object",e):s("new_object",e);return l`
  ...
  `,h([H({attribute:!1})],O.prototype,"hass",2),h([v()],O.prototype,"_open",2),h([v()],O.prototype,"_loading",2),h([v()],O.prototype,"_name",2),h([v()],O.prototype,"_manufacturer",2),h([v()],O.prototype,"_model",2),h([v()],O.prototype,"_entryId",2),O=h([ae("maintenance-object-dialog")],O);var Qt=["cleaning","inspection","replacement","calibration","service","custom"],Jt=["time_based","sensor_based","manual"],Xt=["threshold","counter","state_change","runtime"],w=class extends I{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._entryId="";this._taskId=null;this._name="";this._type="custom";this._scheduleType="time_based";this._intervalDays="30";this._warningDays="7";this._notes="";this._documentationUrl="";this._triggerEntityId="";this._triggerEntityIds=[];this._triggerEntityLogic="any";this._triggerAttribute="";this._triggerType="threshold";this._triggerAbove="";this._triggerBelow="";this._triggerForMinutes="0";this._triggerTargetValue="";this._triggerDeltaMode=!1;this._triggerFromState="";this._triggerToState="";this._triggerTargetChanges="";this._triggerRuntimeHours="";this._responsibleUserId=null;this._availableUsers=[];this._userService=null}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}async openCreate(e){this._entryId=e,this._taskId=null,this._resetFields(),await this._loadUsers(),this._open=!0}async openEdit(e,t){if(this._entryId=e,this._taskId=t.id,this._name=t.name,this._type=t.type,this._scheduleType=t.schedule_type,this._intervalDays=t.interval_days?.toString()||"30",this._warningDays=t.warning_days.toString(),this._notes=t.notes||"",this._documentationUrl=t.documentation_url||"",this._responsibleUserId=t.responsible_user_id||null,t.trigger_config){let i=t.trigger_config;this._triggerEntityId=i.entity_id||"",this._triggerEntityIds=i.entity_ids||(i.entity_id?[i.entity_id]:[]),this._triggerEntityLogic=i.entity_logic||"any",this._triggerAttribute=i.attribute||"",this._triggerType=i.type||"threshold",this._triggerAbove=i.trigger_above?.toString()||"",this._triggerBelow=i.trigger_below?.toString()||"",this._triggerForMinutes=i.trigger_for_minutes?.toString()||"0",this._triggerTargetValue=i.trigger_target_value?.toString()||"",this._triggerDeltaMode=i.trigger_delta_mode||!1,this._triggerFromState=i.trigger_from_state||"",this._triggerToState=i.trigger_to_state||"",this._triggerTargetChanges=i.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=i.trigger_runtime_hours?.toString()||""}else this._resetTriggerFields();await this._loadUsers(),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._warningDays="7",this._notes="",this._documentationUrl="",this._responsibleUserId=null,this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerForMinutes="0",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours=""}async _loadUsers(){this._userService||(this._userService=new re(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}async _save(){if(this._name.trim()){this._loading=!0;try{let e={type:this._taskId?"maintenance_supporter/task/update":"maintenance_supporter/task/create",entry_id:this._entryId,name:this._name,task_type:this._type,schedule_type:this._scheduleType,warning_days:parseInt(this._warningDays,10)||7};if(this._taskId&&(e.task_id=this._taskId),this._scheduleType!=="manual"&&this._intervalDays&&(e.interval_days=parseInt(this._intervalDays,10)),this._notes&&(e.notes=this._notes),this._documentationUrl&&(e.documentation_url=this._documentationUrl),this._responsibleUserId&&(e.responsible_user_id=this._responsibleUserId),this._scheduleType==="sensor_based"&&this._triggerEntityId){let t=this._triggerEntityIds.length>0?this._triggerEntityIds:[this._triggerEntityId],i={entity_id:t[0],entity_ids:t,type:this._triggerType};this._triggerAttribute&&(i.attribute=this._triggerAttribute),t.length>1&&(i.entity_logic=this._triggerEntityLogic),this._triggerType==="threshold"?(this._triggerAbove&&(i.trigger_above=parseFloat(this._triggerAbove)),this._triggerBelow&&(i.trigger_below=parseFloat(this._triggerBelow)),this._triggerForMinutes&&(i.trigger_for_minutes=parseInt(this._triggerForMinutes,10))):this._triggerType==="counter"?(this._triggerTargetValue&&(i.trigger_target_value=parseFloat(this._triggerTargetValue)),i.trigger_delta_mode=this._triggerDeltaMode):this._triggerType==="state_change"?(this._triggerFromState&&(i.trigger_from_state=this._triggerFromState),this._triggerToState&&(i.trigger_to_state=this._triggerToState),this._triggerTargetChanges&&(i.trigger_target_changes=parseInt(this._triggerTargetChanges,10))):this._triggerType==="runtime"&&this._triggerRuntimeHours&&(i.trigger_runtime_hours=parseFloat(this._triggerRuntimeHours)),e.trigger_config=i}await this.hass.connection.sendMessagePromise(e),this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return c;let e=this._lang;return l`
      ...
      <div class="select-row">
        ...
        >
          ${Xt.map(t=>l`<option value=${t}>${s(t,e)}</option>`)}
        </select>
      </div>
      ...
      <ha-dialog open @closed=${this._close} .heading=${t}>
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${Jt.map(i=>l`<option value=${i}>${s(i,e)}</option>`)}
            </select>
          </div>
  ...
  `,h([H({attribute:!1})],w.prototype,"hass",2),h([v()],w.prototype,"_open",2),h([v()],w.prototype,"_loading",2),h([v()],w.prototype,"_entryId",2),h([v()],w.prototype,"_taskId",2),h([v()],w.prototype,"_name",2),h([v()],w.prototype,"_type",2),h([v()],w.prototype,"_scheduleType",2),h([v()],w.prototype,"_intervalDays",2),h([v()],w.prototype,"_warningDays",2),h([v()],w.prototype,"_notes",2),h([v()],w.prototype,"_documentationUrl",2),h([v()],w.prototype,"_triggerEntityId",2),h([v()],w.prototype,"_triggerEntityIds",2),h([v()],w.prototype,"_triggerEntityLogic",2),h([v()],w.prototype,"_triggerAttribute",2),h([v()],w.prototype,"_triggerType",2),h([v()],w.prototype,"_triggerAbove",2),h([v()],w.prototype,"_triggerBelow",2),h([v()],w.prototype,"_triggerForMinutes",2),h([v()],w.prototype,"_triggerTargetValue",2),h([v()],w.prototype,"_triggerDeltaMode",2),h([v()],w.prototype,"_triggerFromState",2),h([v()],w.prototype,"_triggerToState",2),h([v()],w.prototype,"_triggerTargetChanges",2),h([v()],w.prototype,"_triggerRuntimeHours",2),h([v()],w.prototype,"_responsibleUserId",2),h([v()],w.prototype,"_availableUsers",2),w=h([ae("maintenance-task-dialog")],w);var U=class extends I{constructor(){super(...arguments);this.lang="de";this._open=!1;this._loading=!1;this._error="";this._result=null;this._action="view";this._entryId="";this._taskId=null;this._objectName="";this._taskName=""}openForObject(e,t){this._entryId=e,this._taskId=null,this._objectName=t,this._taskName="",this._action="view",this._error="",this._result=null,this._open=!0,this._generate()}openForTask(e,t,i,r){this._entryId=e,this._taskId=t,this._objectName=i,this._taskName=r,this._action="view",this._error="",this._result=null,this._open=!0,this._generate()}async _generate(){this._loading=!0,this._error="";try{let e={type:"maintenance_supporter/qr/generate",entry_id:this._entryId,action:this._action};this._taskId&&(e.task_id=this._taskId);let t=await this.hass.connection.sendMessagePromise(e);this._result=t}catch{this._error=s("qr_error",this.lang)}finally{this._loading=!1}}_onActionChange(e){this._action=e.target.value,this._generate()}_print(){if(!this._result)return;let e=this._result,t=e.label.task_name?`${e.label.object_name} \u2014 ${e.label.task_name}`:e.label.object_name,i=[e.label.manufacturer,e.label.model].filter(Boolean).join(" "),r=window.open("","_blank","width=400,height=500");r&&(r.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8">
<title>${t}</title>
<style>
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `,h([H({attribute:!1})],U.prototype,"hass",2),h([H()],U.prototype,"lang",2),h([v()],U.prototype,"_open",2),h([v()],U.prototype,"_loading",2),h([v()],U.prototype,"_error",2),h([v()],U.prototype,"_result",2),h([v()],U.prototype,"_action",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",U);var V=class extends I{constructor(){super(...arguments);this._open=!1;this._title="";this._message="";this._confirmText="";this._danger=!1;this._resolve=null}confirm(e){return this._title=e.title,this._message=e.message,this._confirmText=e.confirmText||"OK",this._danger=e.danger||!1,this._open=!0,new Promise(t=>{this._resolve=t})}_cancel(){this._open=!1,this._resolve?.(!1),this._resolve=null}_confirmAction(){this._open=!1,this._resolve?.(!0),this._resolve=null}render(){if(!this._open)return c;let e=this.hass?.language||"de";return l`
  ...
  `,h([H({attribute:!1})],V.prototype,"hass",2),h([v()],V.prototype,"_open",2),h([v()],V.prototype,"_title",2),h([v()],V.prototype,"_message",2),h([v()],V.prototype,"_confirmText",2),h([v()],V.prototype,"_danger",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",V);var ei=300,ti=140,He=27;function dt(n,a){let e=n.trigger_config;if(!e)return c;let t=a.lang,i=n.trigger_entity_info,r=n.trigger_entity_infos,o=i?.friendly_name||e.entity_id||"\u2014",d=e.entity_id||"",u=e.entity_ids||(d?[d]:[]),p=i?.unit_of_measurement||"",_=n.trigger_current_value,m=e.type||"threshold",b=u.length>1;return l`
    <h3>${s("trigger",t)}</h3>
    <div class="trigger-card">
      ...
      ${_!=null?l`
            <div class="trigger-value-row">
              <span class="trigger-current ${n.trigger_active?"active":""}">${typeof _=="number"?_.toFixed(1):_}</span>
  ...
  `}function ii(n,a,e){let t=n.trigger_config;if(!t)return c;let i=t.type||"threshold",r=i==="counter"&&t.trigger_delta_mode,o=e.isCounterEntity(t),d=t.entity_id||"",u=e.detailStatsData.get(d)||[],p=[],_=!1;if(u.length>=2)for(let g of u){let k=g.val;r&&n.trigger_baseline_value!=null&&(k-=n.trigger_baseline_value);let z={ts:g.ts,val:k};!o&&g.min!=null&&g.max!=null&&(z.min=r&&n.trigger_baseline_value!=null?g.min-n.trigger_baseline_value:g.min,z.max=r&&n.trigger_baseline_value!=null?g.max-n.trigger_baseline_value:g.max,_=!0),p.push(z)}else for(let g of n.history)g.trigger_value!=null&&p.push({ts:new Date(g.timestamp).getTime(),val:g.trigger_value});if(n.trigger_current_value!=null){let g=n.trigger_current_value;r&&n.trigger_baseline_value!=null&&(g-=n.trigger_baseline_value),p.push({ts:Date.now(),val:g})}if(p.length<2&&d&&e.hasStatsService&&!e.detailStatsData.has(d))return l`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
      <ha-icon icon="mdi:chart-line" style="--mdc-icon-size:16px;margin-right:8px;"></ha-icon>
      ${s("loading_chart",e.lang)}
    </div>`;if(p.length<2)return c;p.sort((g,k)=>g.ts-k.ts);let m=ei,b=ti,f=30,$=2,S=8,P=16,M=p.map(g=>g.val),E=Math.min(...M),y=Math.max(...M);if(_)for(let g of p)g.min!=null&&(E=Math.min(E,g.min)),g.max!=null&&(y=Math.max(y,g.max));t.trigger_above!=null&&(y=Math.max(y,t.trigger_above),E=Math.min(E,t.trigger_above)),t.trigger_below!=null&&(E=Math.min(E,t.trigger_below),y=Math.max(y,t.trigger_below));let C=null,R=null;if(i==="counter"&&t.trigger_target_value!=null){if(n.trigger_baseline_value!=null)C=n.trigger_baseline_value;else if(p.length>0){let g=[...n.history].filter(k=>k.type==="completed"||k.type==="reset").sort((k,z)=>new Date(z.timestamp).getTime()-new Date(k.timestamp).getTime())[0];if(g){let k=new Date(g.timestamp).getTime(),z=p[0],F=Math.abs(p[0].ts-k);for(let G of p){let K=Math.abs(G.ts-k);K<F&&(z=G,F=K)}C=z.val}else C=p[0].val}C!=null?(R=C+t.trigger_target_value,y=Math.max(y,R),E=Math.min(E,C)):(y=Math.max(y,t.trigger_target_value),E=Math.min(E,0))}r&&n.trigger_baseline_value!=null&&(E=Math.min(E,0));let B=y-E||1;E-=B*.1,y+=B*.1;let q=p[0].ts,j=p[p.length-1].ts,x=j-q||1,D=g=>f+(g-q)/x*(m-f-$),T=g=>S+(1-(g-E)/(y-E))*(b-S-P),$e=p.map(g=>`${D(g.ts).toFixed(1)},${T(g.val).toFixed(1)}`).join(" "),ft=`M${D(p[0].ts).toFixed(1)},${b-P} `+p.map(g=>`L${D(g.ts).toFixed(1)},${T(g.val).toFixed(1)}`).join(" ")+` L${D(p[p.length-1].ts).toFixed(1)},${b-P} Z`,we="";if(_){let g=p.filter(k=>k.min!=null&&k.max!=null);if(g.length>=2){let k=g.map(F=>`${D(F.ts).toFixed(1)},${T(F.max).toFixed(1)}`),z=[...g].reverse().map(F=>`${D(F.ts).toFixed(1)},${T(F.min).toFixed(1)}`);we=`M${k[0]} `+k.slice(1).map(F=>`L${F}`).join(" ")+` L${z[0]} `+z.slice(1).map(F=>`L${F}`).join(" ")+" Z"}}let Re=p[p.length-1],bt=D(Re.ts),yt=T(Re.val),Ne=g=>Math.abs(g)>=1e4?(g/1e3).toFixed(0)+"k":g>=1e3?(g/1e3).toFixed(1)+"k":g.toFixed(g<10?1:0),xt=Ne(y),$t=Ne(E),wt=n.history.filter(g=>["completed","skipped","reset"].includes(g.type)).map(g=>({ts:new Date(g.timestamp).getTime(),type:g.type})).filter(g=>g.ts>=q&&g.ts<=j),ke=p;if(p.length>He){let g=(p.length-1)/(He-1);ke=[];for(let k=0;k<He;k++)ke.push(p[Math.round(k*g)])}return l`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${m} ${b}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_sparkline",e.lang)}">
        <text x="${f-3}" y="${S+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${xt}</text>
        <text x="${f-3}" y="${b-P+3}" text-anchor="end" fill="var(--secondary-text-color)" font-size="8">${$t}</text>
        <text x="${f}" y="${b-1}" text-anchor="start" fill="var(--secondary-text-color)" font-size="7">${new Date(q).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        <text x="${m-$}" y="${b-1}" text-anchor="end" fill="var(--secondary-text-color)" font-size="7">${new Date(j).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        ${we?L`<path d="${we}" fill="var(--primary-color)" opacity="0.08" />`:c}
        ...
        ${n.degradation_rate!=null&&n.degradation_trend!=="stable"&&n.degradation_trend!=="insufficient_data"&&p.length>=2?(()=>{let g=p[p.length-1],k=30,z=g.ts+k*864e5,F=g.val+n.degradation_rate*k,G=Math.min(z,j+(j-q)*.3),K=Math.max(E,Math.min(y,F)),kt=D(g.ts),Et=T(g.val),At=D(G),St=T(K);return L`<line x1="${kt.toFixed(1)}" y1="${Et.toFixed(1)}" x2="${At.toFixed(1)}" y2="${St.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():c}
        ${i==="threshold"&&t.trigger_above!=null?L`<line x1="${f}" y1="${T(t.trigger_above).toFixed(1)}" x2="${m}" y2="${T(t.trigger_above).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${m-2}" y="${T(t.trigger_above)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25B2 ${t.trigger_above}</text>`:c}
        ${i==="threshold"&&t.trigger_below!=null?L`<line x1="${f}" y1="${T(t.trigger_below).toFixed(1)}" x2="${m}" y2="${T(t.trigger_below).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${m-2}" y="${T(t.trigger_below)+11}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25BC ${t.trigger_below}</text>`:c}
        ${i==="counter"&&R!=null?L`<line x1="${f}" y1="${T(R).toFixed(1)}" x2="${m}" y2="${T(R).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${m-2}" y="${T(R)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">${s("target_value",e.lang)}: +${t.trigger_target_value}</text>`:c}
        ${i==="counter"&&C!=null?L`<line x1="${f}" y1="${T(C).toFixed(1)}" x2="${m}" y2="${T(C).toFixed(1)}" stroke="var(--secondary-text-color)" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
                <text x="${f+2}" y="${T(C)+11}" text-anchor="start" fill="var(--secondary-text-color)" font-size="8">${s("baseline",e.lang)}</text>`:c}
        <circle cx="${bt.toFixed(1)}" cy="${yt.toFixed(1)}" r="3.5" fill="var(--primary-color)" />
        ${wt.map(g=>{let k=D(g.ts),z=g.type==="completed"?"var(--success-color, #4caf50)":g.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return L`
            <line x1="${k.toFixed(1)}" y1="${S}" x2="${k.toFixed(1)}" y2="${b-P}" stroke="${z}" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
            <circle cx="${k.toFixed(1)}" cy="${S+2}" r="5" fill="${z}" opacity="0.8" />
            <text x="${k.toFixed(1)}" y="${S+6}" text-anchor="middle" fill="white" font-size="7" font-weight="bold">${g.type==="completed"?"\u2713":g.type==="skipped"?"\u23ED":"\u21BA"}</text>
          `})}
        ${ke.map(g=>{let k=D(g.ts),z=T(g.val),F=new Date(g.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),G=`${g.val.toFixed(1)} ${a}`;return _&&g.min!=null&&g.max!=null&&(G+=` (${g.min.toFixed(1)}\u2013${g.max.toFixed(1)})`),L`<circle cx="${k.toFixed(1)}" cy="${z.toFixed(1)}" r="8" fill="transparent" tabindex="0"
            @mouseenter=${K=>ct(K,`${F}
${G}`,e.setTooltip)}
            @focus=${K=>ct(K,`${F}
${G}`,e.setTooltip)}
...
`).map(g=>l`<div>${g}</div>`)}
      ...
      <div class="prediction-grid">
        ...
        ${i?l`
          <div class="prediction-item">
            ...
            ${n.threshold_prediction_date?l`<span class="prediction-date">${Y(n.threshold_prediction_date,a)}</span>`:c}
  ...
  `}function pt(n,a){let e=n.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null)return c;let r=n.interval_days??0,o=n.suggested_interval??r;return l`
    ...
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_weibull",i)}">
        ...
        ${e>0?L`
          ...
          <circle cx="${E.toFixed(1)}" cy="${C.toFixed(1)}" r="3"
            fill="var(--primary-color, #03a9f4)" />
          <text x="${(E+4).toFixed(1)}" y="${(C-6).toFixed(1)}" fill="var(--primary-color, #03a9f4)"
  ...
  `}function si(n,a,e){let t=n.confidence_interval_low,i=n.confidence_interval_high,r=a.suggested_interval??a.interval_days??0,o=a.interval_days??0,d=Math.max(0,t-5),p=i+5-d,_=(t-d)/p*100,m=(i-t)/p*100,b=(r-d)/p*100,f=o>0?(o-d)/p*100:-1;return l`
  ...
  `}function _t(n,a){return oi(n,a)}function oi(n,a){let e=n.seasonal_factors??n.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return c;let t=n.interval_analysis?.seasonal_reason,i=new Date().getMonth(),r=300,o=100,d=8,p=o-d-4,_=Math.max(...e,1.5),m=r/12,b=m*.65,f=d+p-1/_*p;return l`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${r} ${o}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",a)}">
        ...
        ${e.map(($,S)=>{let P=$/_*p,M=S*m+(m-b)/2,E=d+p-P,y=S===i,C=$<1?"var(--success-color, #4caf50)":$>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return L`
            <rect x="${M.toFixed(1)}" y="${E.toFixed(1)}"
              width="${b.toFixed(1)}" height="${P.toFixed(1)}"
              fill="${C}" opacity="${y?1:.5}" rx="2" />
          `})}
      </svg>
      <div class="seasonal-labels">
        ${gt.map(($,S)=>l`<span class="seasonal-label ${S===i?"active-month":""}">${s($,a)}</span>`)}
      </div>
    </div>
  `}var li=300,ci=200;function mt(n,a,e,t){let i=n.history.filter(d=>d.type==="completed"&&(d.cost!=null||d.duration!=null));if(i.length<2)return c;let r=i.some(d=>(d.cost??0)>0),o=i.some(d=>(d.duration??0)>0);return!r&&!o?c:l`
  ...
  `}function di(n,a,e){let t=n.history.filter(x=>x.type==="completed"&&(x.cost!=null||x.duration!=null)).map(x=>({ts:new Date(x.timestamp).getTime(),cost:x.cost??0,duration:x.duration??0})).sort((x,D)=>x.ts-D.ts);if(t.length<2)return c;let i=t.some(x=>x.cost>0),r=t.some(x=>x.duration>0);if(!i&&!r)return c;let o=e!=="duration"&&i,d=e!=="cost"&&r,u=o||!d&&i,p=d||!o&&r,_=li,m=ci,b=u?32:8,f=p?32:8,$=8,S=20,P=_-b-f,M=m-$-S,E=Math.max(...t.map(x=>x.cost))||1,y=Math.max(...t.map(x=>x.duration))||1,C=Math.min(20,P/t.length*.6),R=P/t.length,B=x=>b+R*x+R/2,q=x=>$+M-x/E*M,j=x=>$+M-x/y*M;return l`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${_} ${m}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_history",a)}">
        ${u?t.map((x,D)=>L`
          <rect x="${(B(D)-C/2).toFixed(1)}" y="${q(x.cost).toFixed(1)}" width="${C.toFixed(1)}" height="${($+M-q(x.cost)).toFixed(1)}"
            fill="var(--primary-color)" opacity="0.6" rx="2" />
        `):c}
        ${p?L`
          <polyline points="${t.map((x,D)=>`${B(D).toFixed(1)},${j(x.duration).toFixed(1)}`).join(" ")}"
            fill="none" stroke="var(--accent-color, #ff9800)" stroke-width="2" stroke-linejoin="round" />
          ${t.map((x,D)=>L`
            <circle cx="${B(D).toFixed(1)}" cy="${j(x.duration).toFixed(1)}" r="3" fill="var(--accent-color, #ff9800)" />
          `)}
        `:c}
        <text x="${b}" y="${m-2}" text-anchor="start" fill="var(--secondary-text-color)" font-size="7">${new Date(t[0].ts).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        <text x="${_-f}" y="${m-2}" text-anchor="end" fill="var(--secondary-text-color)" font-size="7">${new Date(t[t.length-1].ts).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        ${u?L`
        ...
        `:c}
        ${p?L`
          <text x="${_-f+3}" y="${$+4}" text-anchor="start" fill="var(--accent-color, #ff9800)" font-size="7">${y.toFixed(0)}m</text>
          <text x="${_-f+3}" y="${$+M+3}" text-anchor="start" fill="var(--accent-color, #ff9800)" font-size="7">0m</text>
        `:c}
      </svg>
    </div>
    <div class="chart-legend">
  ...
  `}var ui=60,pi=20,vt=30,A=class extends I{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._sparklineTooltip=null;this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1};this._actionLoading=!1;this._moreMenuOpen=!1;this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._deepLinkHandled=!1;this._onDialogEvent=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"de"}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new xe(this.hass),this._fetchMiniStatsForOverview()),this._userService||(this._userService=new re(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,i,r,o]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);e&&(this._objects=e.objects),t&&(this._stats=t),i&&(this._budget=i),r&&(this._groups=r.groups||{}),o&&(this._features=o.features),this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("entry_id");if(!t)return;this._deepLinkHandled=!0;let i=e.get("task_id"),r=e.get("action");i?(this._showTask(t,i),r==="complete"&&(history.replaceState(null,"",window.location.pathname+window.location.hash),requestAnimationFrame(()=>{let d=this._getObject(t)?.tasks.find(u=>u.id===i);d&&this._openCompleteDialog(t,i,d.name,this._features.checklists?d.checklist:void 0,this._features.adaptive&&!!d.adaptive_config?.enabled)}))):this._showObject(t)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let i=await this._statsService.getDetailStats(e,t),r=new Map(this._detailStatsData);r.set(e,i),this._detailStatsData=r}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let i of this._objects)for(let r of i.tasks){let o=r.trigger_config?.entity_id;o&&e.push({entityId:o,isCounter:this._isCounterEntity(r.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let i of this._objects)for(let r of i.tasks)if(!(this._filterStatus&&r.status!==this._filterStatus)){if(this._filterUser){let o=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(r.responsible_user_id!==o)continue}e.push({entry_id:i.entry_id,task_id:r.id,object_name:i.object.name,task_name:r.name,type:r.type,schedule_type:r.schedule_type,status:r.status,days_until_due:r.days_until_due??null,next_due:r.next_due??null,trigger_active:r.trigger_active,trigger_current_value:r.trigger_current_value??null,trigger_current_delta:r.trigger_current_delta??null,trigger_config:r.trigger_config??null,trigger_entity_info:r.trigger_entity_info??null,times_performed:r.times_performed,total_cost:r.total_cost,interval_days:r.interval_days??null,history:r.history||[],enabled:r.enabled})}let t={overdue:0,triggered:1,due_soon:2,ok:3};return e.sort((i,r)=>(t[i.status]??9)-(t[r.status]??9)),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(r=>r.id===t)}_pushPanelState(e,t,i){let r={msp_view:e,msp_entry:t||null,msp_task:i||null};history.pushState(r,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let i=this._getTask(t.msp_entry,t.msp_task);i?.trigger_config?.entity_id&&this._fetchDetailStats(i.trigger_config.entity_id,this._isCounterEntity(i.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._historyFilter=null;let i=this._getTask(e,t);if(i?.trigger_config?.entity_id){let r=i.trigger_config.entity_id,o=this._isCounterEntity(i.trigger_config);this._fetchDetailStats(r,o)}}async _deleteObject(e){await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("delete",this._lang),message:s("confirm_delete_object",this._lang),confirmText:s("delete",this._lang),danger:!0})&&(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData())}async _deleteTask(e,t){await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("delete",this._lang),message:s("confirm_delete_task",this._lang),confirmText:s("delete",this._lang),danger:!0})&&(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData())}async _skipTask(e,t){this._actionLoading=!0;try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/skip",entry_id:e,task_id:t}),await this._loadData()}finally{this._actionLoading=!1}}async _resetTask(e,t){this._actionLoading=!0;try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/reset",entry_id:e,task_id:t}),await this._loadData()}finally{this._actionLoading=!1}}async _applySuggestion(e,t,i){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:i}),await this._loadData()}_dismissSuggestion(){this._loadData()}_openCompleteDialog(e,t,i,r,o){let d=this.shadowRoot.querySelector("maintenance-complete-dialog");d&&(d.entryId=e,d.taskId=t,d.taskName=i,d.lang=this._lang,d.checklist=r||[],d.adaptiveEnabled=!!o,d.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,i,r){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,i,r)}async _exportCsv(){let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/csv/export"}),t=new Blob([e.csv],{type:"text/csv;charset=utf-8;"}),i=URL.createObjectURL(t),r=document.createElement("a");r.href=i,r.download="maintenance_export.csv",r.click(),URL.revokeObjectURL(i)}_triggerImportCsv(){let e=this.shadowRoot.querySelector("input[type=file]");e&&(e.value="",e.click())}async _handleCsvFile(e){let i=e.target.files?.[0];if(!i)return;let r=await i.text(),o=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/csv/import",csv_content:r});await this._loadData(),await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:"Import",message:s("import_result",this._lang).replace("{created}",String(o.created)).replace("{total}",String(o.total)),confirmText:"OK"})}render(){return l`
    ...
    `}_openCompleteDialogForRow(e){let i=this._objects.find(r=>r.entry_id===e.entry_id)?.tasks.find(r=>r.id===e.task_id);this._openCompleteDialog(e.entry_id,e.task_id,e.task_name,this._features.checklists?i?.checklist:void 0,this._features.adaptive&&!!i?.adaptive_config?.enabled)}_renderTriggerProgress(e){let t=e.trigger_config??null;if(!t)return c;let i=t.type||"threshold",r=e.trigger_entity_info?.unit_of_measurement??"",o=0,d="";if(i==="threshold"){let _=e.trigger_current_value??null;if(_==null)return c;let m=t.trigger_above,b=t.trigger_below;if(m!=null){let f=b??0,$=m-f||1;o=Math.min(100,Math.max(0,(_-f)/$*100)),d=`${_.toFixed(1)} / ${m} ${r}`}else if(b!=null){let $=e.trigger_entity_info?.max??(_*2||100),S=$-b||1;o=Math.min(100,Math.max(0,($-_)/S*100)),d=`${_.toFixed(1)} / ${b} ${r}`}else return c}else if(i==="counter"){let _=t.trigger_target_value||1,b=e.trigger_current_delta??null??e.trigger_current_value??null;if(b==null)return c;o=Math.min(100,Math.max(0,b/_*100)),d=`${b.toFixed(1)} / ${_} ${r}`}else if(i==="state_change"){let _=t.trigger_target_changes||1,m=e.trigger_current_value??null;if(m==null)return c;o=Math.min(100,Math.max(0,m/_*100)),d=`${Math.round(m)} / ${_}`}else if(i==="runtime"){let _=t.trigger_runtime_hours||100,m=e.trigger_current_value??null;if(m==null)return c;o=Math.min(100,Math.max(0,m/_*100)),d=`${m.toFixed(1)}h / ${_}h`}else if(i==="compound"){let _=t.compound_logic||t.operator||"AND",m=t.conditions?.length||0;d=`${_} (${m})`,o=e.trigger_active?100:0}else return c;let u=o>=100,p=o>90?"var(--error-color, #f44336)":o>70?"var(--warning-color, #ff9800)":"var(--primary-color)";return l`
    ...
    `}_renderObjectDetail(){if(!this._selectedEntryId)return c;let e=this._getObject(this._selectedEntryId);if(!e)return l`<p>Object not found.</p>`;let t=e.object,i=this._lang;return l`
      <div class="detail-section">
        ...
        ${t.installation_date?l`<p class="meta">${s("installed",i)}: ${Y(t.installation_date,i)}</p>`:c}
    ...
    `}};A.styles=[ot,lt],h([H({attribute:!1})],A.prototype,"hass",2),h([H({type:Boolean,reflect:!0})],A.prototype,"narrow",2),h([H({attribute:!1})],A.prototype,"panel",2),h([v()],A.prototype,"_objects",2),h([v()],A.prototype,"_stats",2),h([v()],A.prototype,"_view",2),h([v()],A.prototype,"_selectedEntryId",2),h([v()],A.prototype,"_selectedTaskId",2),h([v()],A.prototype,"_filterStatus",2),h([v()],A.prototype,"_filterUser",2),h([v()],A.prototype,"_unsub",2),h([v()],A.prototype,"_sparklineTooltip",2),h([v()],A.prototype,"_historyFilter",2),h([v()],A.prototype,"_budget",2),h([v()],A.prototype,"_groups",2),h([v()],A.prototype,"_detailStatsData",2),h([v()],A.prototype,"_miniStatsData",2),h([v()],A.prototype,"_features",2),h([v()],A.prototype,"_actionLoading",2),h([v()],A.prototype,"_moreMenuOpen",2),h([v()],A.prototype,"_activeTab",2),h([v()],A.prototype,"_costDurationToggle",2),h([v()],A.prototype,"_historySearch",2),A=h([ae("maintenance-supporter-panel")],A);export{A as MaintenanceSupporterPanel};
