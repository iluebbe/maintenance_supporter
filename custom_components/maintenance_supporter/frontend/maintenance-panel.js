...
`;var Bi={days:1,weeks:7,months:30.4368,years:365.25};function St(o,r){return!o||o<=0?0:o*(Bi[r||"days"]??1)}function At(o,r,e){let t=St(o,e);if(t<=0||r==null)return{pct:0,overflow:!1};let i=(t-r)/t*100;return{pct:Math.max(0,Math.min(100,i)),overflow:i>100}}function jt(o,r=new Date){if(!o)return{kind:"none",days:null,date:null};let e=new Date(`${o}T00:00:00`);if(isNaN(e.getTime()))return{kind:"none",days:null,date:null};let t=Date.UTC(r.getFullYear(),r.getMonth(),r.getDate()),i=Date.UTC(e.getFullYear(),e.getMonth(),e.getDate()),a=Math.round((i-t)/864e5);return a<0?{kind:"expired",days:a,date:o}:a<=60?{kind:"expiring",days:a,date:o}:{kind:"valid",days:a,date:o}}var we=[{key:"name",labelKey:"name",required:!0},{key:"manufacturer",labelKey:"manufacturer"},{key:"model",labelKey:"model"},{key:"serial_number",labelKey:"serial_number_label"},{key:"installation_date",labelKey:"installed"},{key:"warranty_expiry",labelKey:"warranty"},{key:"area_id",labelKey:"area"},{key:"documentation_url",labelKey:"documentation_url_label"},{key:"notes",labelKey:"object_notes_label"},{key:"task_count",labelKey:"tasks"},{key:"actions",labelKey:"actions"}],Wi=we.map(o=>o.key),Xe=["name","manufacturer","model","serial_number","installation_date","warranty_expiry","area_id","task_count","actions"];function He(o){if(!Array.isArray(o))return[...Xe];let r=new Set,e=[];for(let t of o)typeof t=="string"&&Wi.includes(t)&&!r.has(t)&&(r.add(t),e.push(t));return e.length?(e.includes("name")||e.unshift("name"),e):[...Xe]}function et(o,r,e){let t=new Blob([o],{type:e}),i=URL.createObjectURL(t),a=document.createElement("a");a.href=i,a.download=r,a.target="_blank",a.rel="noopener",a.style.display="none",document.body.appendChild(a),a.dispatchEvent(new MouseEvent("click")),document.body.removeChild(a),setTimeout(()=>URL.revokeObjectURL(i),6e4)}function tt(o,r){let e=document.createElement("a");e.href=o,e.download=r,e.target="_blank",e.rel="noopener",e.style.display="none",document.body.appendChild(e),e.dispatchEvent(new MouseEvent("click")),document.body.removeChild(e)}var ei=A`
  :host {
    display: block;
    height: 100%;
...
`;var it=class{constructor(r){this._cache=new Map;this._pending=new Map;this._hass=r}updateHass(r){this._hass=r}async getDetailStats(r,e,t=30){return this._getStats(r,t<=35?"hour":"day",t,e)}async getMiniStats(r,e){return this._getStats(r,"day",14,e)}async getBatchMiniStats(r){let e=new Map,t=[];for(let p of r){let g=`${p.entityId}:day:14`,_=this._cache.get(g);_&&Date.now()-_.fetchedAt<3e5?e.set(p.entityId,_.points):t.push(p)}if(t.length===0)return e;let i=t.filter(p=>p.isCounter).map(p=>p.entityId),a=t.filter(p=>!p.isCounter).map(p=>p.entityId),l=new Date(Date.now()-336*60*60*1e3).toISOString(),c=[];return i.length>0&&c.push(this._fetchBatch(i,"day",l,["state","sum","change"],!0,e)),a.length>0&&c.push(this._fetchBatch(a,"day",l,["mean","min","max"],!1,e)),await Promise.all(c),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(r,e,t,i){let a=`${r}:${e}:${t}`,l=this._cache.get(a);if(l&&Date.now()-l.fetchedAt<3e5)return l.points;if(this._pending.has(a))return this._pending.get(a);let c=this._fetchAndNormalize(r,e,t,i,a);this._pending.set(a,c);try{return await c}finally{this._pending.delete(a)}}async _fetchAndNormalize(r,e,t,i,a){let l=new Date(Date.now()-t*24*60*60*1e3).toISOString(),c=i?["state","sum","change"]:["mean","min","max"];try{let g=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:l,statistic_ids:[r],period:e,types:c}))[r]||[],_=this._normalizeRows(g,i);return this._cache.set(a,{entityId:r,fetchedAt:Date.now(),period:e,points:_}),_}catch(p){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${r}:`,p),[]}}async _fetchBatch(r,e,t,i,a,l){try{let c=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:r,period:e,types:i});for(let p of r){let g=c[p]||[],_=this._normalizeRows(g,a);l.set(p,_),this._cache.set(`${p}:${e}:14`,{entityId:p,fetchedAt:Date.now(),period:e,points:_})}}catch(c){console.warn("[maintenance-supporter] Batch statistics fetch failed:",c)}}_normalizeRows(r,e){let t=[];for(let i of r){let a=null;if(e?a=i.state??null:a=i.mean??null,a===null)continue;let l={ts:i.start,val:a};e||(i.min!=null&&(l.min=i.min),i.max!=null&&(l.max=i.max)),t.push(l)}return t.sort((i,a)=>i.ts-a.ts),t}};var ue=class{constructor(r){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=r}updateHass(r){this.hass=r}async getUsers(r=!1){let e=Date.now();if(!r&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(r,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:r,task_id:e,user_id:t})}async getTasksByUser(r){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:r})).tasks}getUserName(r){return!r||!this.usersCache?null:this.usersCache.find(t=>t.id===r)?.name||null}getUser(r){return!r||!this.usersCache?null:this.usersCache.find(e=>e.id===r)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(r){return r?r===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var Vi={name:"name",task_type:"maintenance_type",schedule_type:"schedule_type",interval_days:"interval_days",interval_anchor:"interval_anchor",warning_days:"warning_days",last_performed:"last_performed_optional",notes:"notes_optional",documentation_url:"documentation_url_optional",custom_icon:"custom_icon_optional",nfc_tag_id:"nfc_tag_id_optional",responsible_user_id:"responsible_user",entity_slug:"entity_slug",entity_id:"entity_id",area_id:"area_id_optional",manufacturer:"manufacturer_optional",model:"model_optional",serial_number:"serial_number_optional",installation_date:"installation_date_optional",warranty_expiry:"warranty_expiry_optional",checklist:"checklist_steps_optional",reason:"reason",feedback:"feedback",cost:"cost",duration:"duration",description:"description_optional",group_name:"name",group_description:"description_optional",environmental_entity:"environmental_entity_optional",environmental_attribute:"environmental_attribute_optional",trigger_above:"trigger_above",trigger_below:"trigger_below",trigger_for_minutes:"trigger_for_minutes"};function Ki(o,r){let e=Vi[o];if(!e)return o;let t=s(e,r);return t&&t!==e?t:o}function Yi(o){let e=o.match(/data\['([^']+)'\]/)?.[1],t;return(t=o.match(/length of value must be at most (\d+)/))?{field:e,rule:"too_long",param:t[1]}:(t=o.match(/length of value must be at least (\d+)/))?{field:e,rule:"too_short",param:t[1]}:(t=o.match(/value must be at most (\S+)/))?{field:e,rule:"value_too_high",param:t[1]}:(t=o.match(/value must be at least (\S+)/))?{field:e,rule:"value_too_low",param:t[1]}:/required key not provided/.test(o)?{field:e,rule:"required"}:(t=o.match(/expected (\w+)/))?{field:e,rule:"wrong_type",param:t[1]}:/value must be one of/.test(o)?{field:e,rule:"invalid_choice"}:/not a valid value/.test(o)?{field:e,rule:"invalid_value"}:{field:e,rule:"unknown"}}function I(o,r,e){if(e=e??s("action_error",r),typeof o=="string")return o;if(typeof o!="object"||o===null)return e;let t=o,i=t.message||t.error?.message||"";if(!i)return e;let a=Yi(i),l=a.field?Ki(a.field,r):"",c=p=>s(p,r).replace("{field}",l).replace("{n}",a.param??"");switch(a.rule){case"too_long":return c("err_too_long");case"too_short":return c("err_too_short");case"value_too_high":return c("err_value_too_high");case"value_too_low":return c("err_value_too_low");case"required":return c("err_required");case"wrong_type":return c("err_wrong_type").replace("{type}",a.param??"");case"invalid_choice":return c("err_invalid_choice");case"invalid_value":return c("err_invalid_value");default:return i||e}}var B=class extends T{constructor(){super(...arguments);this.label="";this.value="";this.placeholder="";this.type="text";this.required=!1;this.disabled=!1}_onInput(e){let t=e.target.value;this.value=t,this.dispatchEvent(new CustomEvent("input",{bubbles:!0,composed:!0,detail:{value:t}}))}render(){return n`
  ...
  `,h([y({attribute:!1})],O.prototype,"hass",2),h([u()],O.prototype,"_open",2),h([u()],O.prototype,"_loading",2),h([u()],O.prototype,"_error",2),h([u()],O.prototype,"_name",2),h([u()],O.prototype,"_manufacturer",2),h([u()],O.prototype,"_model",2),h([u()],O.prototype,"_serialNumber",2),h([u()],O.prototype,"_areaId",2),h([u()],O.prototype,"_installationDate",2),h([u()],O.prototype,"_warrantyExpiry",2),h([u()],O.prototype,"_documentationUrl",2),h([u()],O.prototype,"_notes",2),h([u()],O.prototype,"_entryId",2);customElements.get("maintenance-object-dialog")||customElements.define("maintenance-object-dialog",O);function ae(o){let r=o??0;return r<1024?`${r} B`:r<1024*1024?`${(r/1024).toFixed(1)} KB`:`${(r/(1024*1024)).toFixed(1)} MB`}var st=["manual","warranty","invoice","spare_parts","photo","other"],Gi={manual:"mdi:book-open-variant",warranty:"mdi:shield-check",invoice:"mdi:receipt-text-outline",spare_parts:"mdi:cog-outline",photo:"mdi:image-outline",other:"mdi:file-document-outline"},H=class extends T{constructor(){super(...arguments);this.canWrite=!1;this._docs=[];this._loaded=!1;this._busy=!1;this._error="";this._hint="";this._addingLink=!1;this._linkUrl="";this._linkTitle="";this._category="manual";this._thumbs={};this._lightboxUrl="";this._editingId="";this._editTitle="";this._editCategory="manual";this._dragOver=!1;this._loadedFor=null;this._localeReady=!1}_isImage(e){return e.kind==="file"&&(e.mime||"").startsWith("image/")}async _sign(e){return(await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300})).path}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),this.hass&&!this._localeReady&&(this._localeReady=!0,se(this._lang).then(()=>this.requestUpdate())),this.hass&&this.entryId&&this._loadedFor!==this.entryId&&(this._loadedFor=this.entryId,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:this.entryId});this._docs=e.documents||[],this._loaded=!0,this._error="",this._thumbs={},this._loadThumbs()}catch(e){this._error=I(e,this._lang),this._loaded=!0}}async _loadThumbs(){await Promise.all(this._docs.filter(e=>this._isImage(e)).map(async e=>{try{let t=await this._sign(e);this._thumbs={...this._thumbs,[e.id]:t}}catch{}}))}_category_of(e){return(e.tags||[]).find(i=>st.includes(i))||"other"}_labelKeydown(e){(e.key==="Enter"||e.key===" ")&&(e.preventDefault(),e.currentTarget.querySelector("input")?.click())}_onFileInput(e){let t=e.target,i=Array.from(t.files??[]);i.length&&this._uploadFiles(i),t.value=""}_onCameraInput(e){let t=e.target,i=Array.from(t.files??[]);i.length&&this._uploadFiles(i,"photo"),t.value=""}_onDrop(e){if(e.preventDefault(),this._dragOver=!1,!this.canWrite||this._busy)return;let t=Array.from(e.dataTransfer?.files??[]);t.length&&this._uploadFiles(t)}_onDragOver(e){this.canWrite&&(e.preventDefault(),this._dragOver=!0)}_onDragLeave(e){let t=e.relatedTarget;(!t||!e.currentTarget.contains(t))&&(this._dragOver=!1)}async _uploadFiles(e,t){let i=t??this._category;this._busy=!0,this._error="",this._hint="";let a=0,l=0;try{for(let c of e){let p=new FormData;p.append("entry_id",this.entryId),p.append("tags",i),p.append("file",c,c.name);let g=await fetch("/api/maintenance_supporter/document/upload",{method:"POST",headers:{Authorization:`Bearer ${this.hass.auth?.data?.access_token??""}`},body:p});if(!g.ok){this._error=g.status===413?s("doc_too_large",this._lang):s("doc_upload_failed",this._lang);continue}let _=await g.json();_.duplicate_in_object?l++:_.deduped&&a++}l?this._hint=s("doc_dup_in_object",this._lang):a&&(this._hint=s("doc_deduped",this._lang)),await this._load()}catch{this._error=s("doc_upload_failed",this._lang)}finally{this._busy=!1}}async _download(e){try{let t=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:30});tt(t.path,e.filename||e.title||"document")}catch(t){this._error=I(t,this._lang)}}async _preview(e){if(this._isImage(e)){this._lightboxUrl=this._thumbs[e.id]||await this._sign(e);return}let t=window.open("about:blank","_blank");try{let i=await this._sign(e);t&&(t.location.href=new URL(i,window.location.origin).href)}catch(i){t&&t.close(),this._error=I(i,this._lang)}}_openDoc(e){e.kind==="file"?this._preview(e):e.url&&window.open(e.url,"_blank","noopener")}_startEdit(e){this._editingId=e.id,this._editTitle=e.title||"",this._editCategory=this._category_of(e),this._addingLink=!1,this._error=""}_cancelEdit(){this._editingId=""}async _saveEdit(e){let t=(e.tags||[]).filter(a=>!st.includes(a)),i=e.kind==="file"?[this._editCategory,...t]:e.tags??[];this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,title:this._editTitle.trim()||e.filename||e.url||"",tags:i}),this._editingId="",await this._load()}catch(a){this._error=I(a,this._lang)}finally{this._busy=!1}}async _delete(e){let t=e.title||e.filename||e.url||"";if(window.confirm(s("doc_delete_confirm",this._lang).replace("{name}",t))){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/delete",doc_id:e.id}),await this._load()}catch(i){this._error=I(i,this._lang)}finally{this._busy=!1}}}async _addLink(){let e=this._linkUrl.trim();if(e){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/add_link",entry_id:this.entryId,url:e,title:this._linkTitle.trim()||null}),this._linkUrl="",this._linkTitle="",this._addingLink=!1,await this._load()}catch(t){this._error=I(t,this._lang,s("doc_link_invalid",this._lang))}finally{this._busy=!1}}}render(){let e=this._lang;return n`
      ...
      <div class="doc-header">
        ...
        ${this.canWrite?n`
              <div class="doc-actions">
                ...
                >
                  ${st.map(t=>n`<option value=${t}>${s(`doc_cat_${t}`,e)}</option>`)}
                </select>
                <label
    ...
    `}_renderDoc(e,t){if(this._editingId===e.id)return this._renderEdit(e,t);let i=e.kind==="file",a=this._category_of(e),l=i?`${s(`doc_cat_${a}`,t)} \xB7 ${ae(e.size)}`:s("doc_link_badge",t),c=this._thumbs[e.id];return n`
      <div class="doc-row">
        ${i&&c?n`<img
              class="doc-thumb"
              src=${c}
    ...
    `}_renderEdit(e,t){let i=e.kind==="file";return n`
      <div class="doc-row editing">
        ...
        ${i?n`<select
            ...
            >
              ${st.map(a=>n`<option value=${a} ?selected=${a===this._editCategory}>${s(`doc_cat_${a}`,t)}</option>`)}
            </select>`:d}
  ...
  `,h([y({attribute:!1})],H.prototype,"hass",2),h([y({attribute:!1})],H.prototype,"entryId",2),h([y({type:Boolean})],H.prototype,"canWrite",2),h([u()],H.prototype,"_docs",2),h([u()],H.prototype,"_loaded",2),h([u()],H.prototype,"_busy",2),h([u()],H.prototype,"_error",2),h([u()],H.prototype,"_hint",2),h([u()],H.prototype,"_addingLink",2),h([u()],H.prototype,"_linkUrl",2),h([u()],H.prototype,"_linkTitle",2),h([u()],H.prototype,"_category",2),h([u()],H.prototype,"_thumbs",2),h([u()],H.prototype,"_lightboxUrl",2),h([u()],H.prototype,"_editingId",2),h([u()],H.prototype,"_editTitle",2),h([u()],H.prototype,"_editCategory",2),h([u()],H.prototype,"_dragOver",2);customElements.get("maintenance-documents-section")||customElements.define("maintenance-documents-section",H);var Qi=["manual","warranty","invoice","spare_parts","photo","other"],Zi={manual:"mdi:book-open-variant",warranty:"mdi:shield-check",invoice:"mdi:receipt-text-outline",spare_parts:"mdi:cog-outline",photo:"mdi:image-outline",other:"mdi:file-document-outline"},Y=class extends T{constructor(){super(...arguments);this.canWrite=!1;this._docs=[];this._loaded=!1;this._busy=!1;this._error="";this._attachId="";this._loadedKey="";this._localeReady=!1}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),this.hass&&!this._localeReady&&(this._localeReady=!0,se(this._lang).then(()=>this.requestUpdate()));let t=`${this.entryId}|${this.taskId}`;this.hass&&this.entryId&&this.taskId&&this._loadedKey!==t&&(this._loadedKey=t,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:this.entryId});this._docs=e.documents||[],this._loaded=!0,this._error=""}catch(e){this._error=I(e,this._lang),this._loaded=!0}}_linked(){return this._docs.filter(e=>(e.task_ids||[]).includes(this.taskId))}_available(){return this._docs.filter(e=>!(e.task_ids||[]).includes(this.taskId))}async _setTaskIds(e,t){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,task_ids:t}),await this._load()}catch(i){this._error=I(i,this._lang)}finally{this._busy=!1}}_link(){let e=this._docs.find(t=>t.id===this._attachId);e&&(this._attachId="",this._setTaskIds(e,[...e.task_ids||[],this.taskId]))}_unlink(e){this._setTaskIds(e,(e.task_ids||[]).filter(t=>t!==this.taskId))}_isPdf(e){return e.mime==="application/pdf"||(e.filename||"").toLowerCase().endsWith(".pdf")}_pageFor(e){return this._isPdf(e)?e.task_pages?.[this.taskId]:void 0}async _open(e){if(e.kind==="weblink"){window.open(e.url,"_blank","noopener");return}let t=this._pageFor(e),i=t?`#page=${t}`:"",a=window.open("about:blank","_blank");try{let l=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300});a&&(a.location.href=new URL(l.path+i,window.location.origin).href)}catch(l){a&&a.close(),this._error=I(l,this._lang)}}async _setPage(e,t){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/update",doc_id:e.id,task_pages:{[this.taskId]:t}}),await this._load()}catch(i){this._error=I(i,this._lang)}finally{this._busy=!1}}async _download(e){try{let t=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:30});tt(t.path,e.filename||e.title||"document")}catch(t){this._error=I(t,this._lang)}}render(){if(!this._loaded||this._docs.length===0)return d;let e=this._lang,t=this._linked(),i=this._available();return n`
      <div class="task-docs">
    ...
    `}_renderRow(e,t){let i=e.kind==="file",a=this._isPdf(e),l=e.task_pages?.[this.taskId],c=(e.tags||[]).find(g=>Qi.includes(g))||"other",p=i?ae(e.size):s("doc_link_badge",t);return n`
      <div class="tdoc-row">
        <ha-icon class="tdoc-icon" icon=${i?Zi[c]:"mdi:link-variant"}></ha-icon>
        <div
          class="tdoc-info"
          role="button"
          tabindex="0"
          ...
          @click=${()=>this._open(e)}
          @keydown=${g=>{(g.key==="Enter"||g.key===" ")&&(g.preventDefault(),this._open(e))}}
        >
          <div class="tdoc-title">${e.title||e.filename||e.url}</div>
          <div class="tdoc-meta">
            ${p}${l?n` · <span class="tdoc-pagetag">${s("doc_page",t)} ${l}</span>`:d}
          </div>
        </div>
        ${this.canWrite&&a?n`<input
              class="tdoc-page"
              type="number"
              min="1"
              inputmode="numeric"
              ...
              .value=${l?String(l):""}
              ?disabled=${this._busy}
              @change=${g=>{let _=parseInt(g.target.value,10);this._setPage(e,Number.isFinite(_)&&_>=1?_:0)}}
            />`:d}
...
`).map(c=>c.trim()).filter(Boolean).slice(0,100);e.checklist=l.length?l:null}if(this.completionActionsEnabled){let l=this._actionService.trim();if(l&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(l)){let _={service:l},m=this._actionTargetEntity.trim();m&&(_.target={entity_id:m});let v=this._buildActionData();Object.keys(v).length>0&&(_.data=v),e.on_complete_action=_}else e.on_complete_action=null;let c={};this._qcNotes.trim()&&(c.notes=this._qcNotes.trim());let p=parseFloat(this._qcCost);!isNaN(p)&&p>=0&&(c.cost=p);let g=parseInt(this._qcDuration,10);!isNaN(g)&&g>=0&&(c.duration=g),this._qcFeedback&&(c.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(c).length?c:null}let t=await this.hass.connection.sendMessagePromise(e),i=this._taskId||t?.task_id,a=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(i&&this._scheduleType==="sensor_based"&&a)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:i,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=I(e,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return d;let e=this._lang;return n`
      ...
      <div class="select-row">
        ...
        >
          ${ts.map(t=>n`<option value=${t} ?selected=${t===this._triggerType}>${s(t,e)}</option>`)}
        </select>
      </div>
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${Xi.map(i=>n`<option value=${i} ?selected=${i===this._scheduleType}>${s(i,e)}</option>`)}
            </select>
          </div>
  ...
  `,h([y({attribute:!1})],w.prototype,"hass",2),h([y({type:Boolean,attribute:"checklists-enabled"})],w.prototype,"checklistsEnabled",2),h([y({type:Boolean,attribute:"schedule-time-enabled"})],w.prototype,"scheduleTimeEnabled",2),h([y({type:Boolean,attribute:"completion-actions-enabled"})],w.prototype,"completionActionsEnabled",2),h([y({type:Number,attribute:"default-warning-days"})],w.prototype,"defaultWarningDays",2),h([u()],w.prototype,"_open",2),h([u()],w.prototype,"_loading",2),h([u()],w.prototype,"_error",2),h([u()],w.prototype,"_entryId",2),h([u()],w.prototype,"_taskId",2),h([u()],w.prototype,"_objectChoices",2),h([u()],w.prototype,"_name",2),h([u()],w.prototype,"_type",2),h([u()],w.prototype,"_scheduleType",2),h([u()],w.prototype,"_intervalDays",2),h([u()],w.prototype,"_intervalUnit",2),h([u()],w.prototype,"_dueDate",2),h([u()],w.prototype,"_warningDays",2),h([u()],w.prototype,"_intervalAnchor",2),h([u()],w.prototype,"_weekdays",2),h([u()],w.prototype,"_nth",2),h([u()],w.prototype,"_nthWeekday",2),h([u()],w.prototype,"_domDay",2),h([u()],w.prototype,"_notes",2),h([u()],w.prototype,"_documentationUrl",2),h([u()],w.prototype,"_customIcon",2),h([u()],w.prototype,"_enabled",2),h([u()],w.prototype,"_triggerEntityId",2),h([u()],w.prototype,"_triggerEntityIds",2),h([u()],w.prototype,"_triggerEntityLogic",2),h([u()],w.prototype,"_triggerAttribute",2),h([u()],w.prototype,"_triggerType",2),h([u()],w.prototype,"_triggerAbove",2),h([u()],w.prototype,"_triggerBelow",2),h([u()],w.prototype,"_triggerForMinutes",2),h([u()],w.prototype,"_triggerTargetValue",2),h([u()],w.prototype,"_triggerDeltaMode",2),h([u()],w.prototype,"_triggerFromState",2),h([u()],w.prototype,"_triggerToState",2),h([u()],w.prototype,"_triggerTargetChanges",2),h([u()],w.prototype,"_triggerRuntimeHours",2),h([u()],w.prototype,"_suggestedAttributes",2),h([u()],w.prototype,"_availableAttributes",2),h([u()],w.prototype,"_entityDomain",2),h([u()],w.prototype,"_lastPerformed",2),h([u()],w.prototype,"_nfcTagId",2),h([u()],w.prototype,"_availableTags",2),h([u()],w.prototype,"_responsibleUserId",2),h([u()],w.prototype,"_availableUsers",2),h([u()],w.prototype,"_checklistText",2),h([u()],w.prototype,"_scheduleTime",2),h([u()],w.prototype,"_actionService",2),h([u()],w.prototype,"_actionTargetEntity",2),h([u()],w.prototype,"_actionData",2),h([u()],w.prototype,"_actionDataJsonFallback",2),h([u()],w.prototype,"_actionTesting",2),h([u()],w.prototype,"_actionTestResult",2),h([u()],w.prototype,"_actionTestError",2),h([u()],w.prototype,"_qcNotes",2),h([u()],w.prototype,"_qcCost",2),h([u()],w.prototype,"_qcDuration",2),h([u()],w.prototype,"_qcFeedback",2),h([u()],w.prototype,"_environmentalEntity",2),h([u()],w.prototype,"_environmentalAttribute",2);customElements.get("maintenance-task-dialog")||customElements.define("maintenance-task-dialog",w);var F=class extends T{constructor(){super(...arguments);this.entryId="";this.taskId="";this.taskName="";this.lang="en";this.checklist=[];this.adaptiveEnabled=!1;this._open=!1;this._notes="";this._cost="";this._duration="";this._loading=!1;this._error="";this._checklistState={};this._feedback="needed"}open(){this._open||(this._open=!0,this._notes="",this._cost="",this._duration="",this._error="",this._checklistState={},this._feedback="needed")}_toggleCheck(e){let t=String(e);this._checklistState={...this._checklistState,[t]:!this._checklistState[t]}}_setFeedback(e){this._feedback=e}async _complete(){this._loading=!0,this._error="";try{let e={type:"maintenance_supporter/task/complete",entry_id:this.entryId,task_id:this.taskId};if(this._notes&&(e.notes=this._notes),this._cost){let t=parseFloat(this._cost);!isNaN(t)&&t>=0&&(e.cost=t)}if(this._duration){let t=parseInt(this._duration,10);!isNaN(t)&&t>=0&&(e.duration=t)}this.checklist.length>0&&(e.checklist_state=this._checklistState),this.adaptiveEnabled&&(e.feedback=this._feedback),await this.hass.connection.sendMessagePromise(e),this._open=!1,this.dispatchEvent(new CustomEvent("task-completed"))}catch(e){this._error=I(e,this.lang,s("save_error",this.lang))}finally{this._loading=!1}}_close(){this._open=!1}render(){if(!this._open)return n``;let e=this.lang||this.hass?.language||"en";return n`
      <ha-dialog open @closed=${this._close}>
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `,h([y({attribute:!1})],X.prototype,"hass",2),h([y()],X.prototype,"lang",2),h([u()],X.prototype,"_open",2),h([u()],X.prototype,"_loading",2),h([u()],X.prototype,"_error",2),h([u()],X.prototype,"_viewResult",2),h([u()],X.prototype,"_completeResult",2),h([u()],X.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",X);var ii=5;function qe(o){let r=o.getFullYear(),e=String(o.getMonth()+1).padStart(2,"0"),t=String(o.getDate()).padStart(2,"0");return`${r}-${e}-${t}`}function as(o,r){let e=[];for(let t=0;t<r;t++){let i=new Date(o);i.setDate(i.getDate()+t),i.setHours(0,0,0,0),e.push(qe(i))}return e}function at(o,r){let[e,t,i]=o.split("-").map(Number),a=new Date(e,t-1,i);return a.setDate(a.getDate()+r),qe(a)}function rs(o){if(!o||o.length===0)return null;let r=o.map(e=>e.cost).filter(e=>typeof e=="number");return r.length===0?null:r.reduce((e,t)=>e+t,0)/r.length}function ns(o){let{windowStart:r,windowEnd:e,task:t,entryId:i,objectName:a}=o,l=[],c=(m,v)=>({date:m,entry_id:i,task_id:t.id,task_name:t.name,object_name:a,status:v&&(t.status==="overdue"||t.status==="triggered")?"ok":t.status,days_until_due:v?null:t.days_until_due??null,projected:v,schedule_type:t.schedule_type,interval_days:t.interval_days??null,interval_unit:t.interval_unit??null,responsible_user_id:t.responsible_user_id??null,avg_cost:rs(t.history),adaptive_enabled:!!t.adaptive_config?.enabled,prediction_confidence:t.threshold_prediction_confidence??null}),p=Math.max(1,Math.round(St(t.interval_days,t.interval_unit)));if(t.status==="overdue"||t.status==="triggered"){if(l.push(c(r,!1)),t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let m=at(r,p),v=1;for(;m<=e&&v<ii;)l.push(c(m,!0)),v++,m=at(m,p)}return l}let g=t.next_due;if(typeof g!="string"||!g)return l;let _=g.slice(0,10);if(_>=r&&_<=e)l.push(c(_,!1));else if(_>e)return l;if(t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let m=at(_,p),v=l.length;for(;m<=e&&v<ii;)m>=r&&(l.push(c(m,!0)),v++),m=at(m,p)}return l}var si={overdue:0,triggered:1,due_soon:2,ok:3};function ai(o,r,e,t=null){let i=as(r,e),a=i[0],l=i[i.length-1],c=[];for(let g of o){let _=g.object?.name||"",m=g.entry_id,v=g.tasks||[];for(let f of v){if(t&&f.responsible_user_id!==t||f.enabled===!1)continue;let x=ns({windowStart:a,windowEnd:l,task:f,entryId:m,objectName:_});c.push(...x)}}let p=new Map;for(let g of i)p.set(g,[]);for(let g of c){let _=p.get(g.date);_&&_.push(g)}for(let[,g]of p)g.sort((_,m)=>{let v=si[_.status]??99,f=si[m.status]??99;if(v!==f)return v-f;if(_.projected!==m.projected)return _.projected?1:-1;let x=_.object_name.localeCompare(m.object_name);return x!==0?x:_.task_name.localeCompare(m.task_name)});return i.map(g=>({date:g,events:p.get(g)??[]}))}var os={completed:"ok",reset:"ok",skipped:"due_soon",triggered:"triggered",trigger_replaced:"triggered"};function ls(o,r){let e=[];for(let t=r-1;t>=0;t--){let i=new Date(o);i.setDate(i.getDate()-t),i.setHours(0,0,0,0),e.push(qe(i))}return e}function ri(o,r,e,t=null){let i=ls(r,e),a=i[0],l=i[i.length-1],c=new Map;for(let g of i)c.set(g,[]);for(let g of o){let _=g.object?.name||"",m=g.entry_id,v=g.tasks||[];for(let f of v){if(t&&f.responsible_user_id!==t)continue;let x=f.history||[];for(let b of x){if(typeof b?.timestamp!="string")continue;let j=b.timestamp.slice(0,10);if(j<a||j>l)continue;let z=c.get(j);if(!z)continue;let L=b.type??"completed";z.push({date:j,entry_id:m,task_id:f.id,task_name:f.name,object_name:_,status:os[L]??"ok",days_until_due:null,projected:!1,schedule_type:f.schedule_type,interval_days:f.interval_days??null,responsible_user_id:f.responsible_user_id??null,avg_cost:typeof b.cost=="number"?b.cost:null,adaptive_enabled:!!f.adaptive_config?.enabled,prediction_confidence:null,history_timestamp:b.timestamp,history_type:L,history_cost:typeof b.cost=="number"?b.cost:null,history_notes:typeof b.notes=="string"?b.notes:null,history_duration:typeof b.duration=="number"?b.duration:null})}}}let p={completed:0,reset:1,skipped:2,triggered:3,trigger_replaced:4};for(let[,g]of c)g.sort((_,m)=>{let v=p[_.history_type??""]??99,f=p[m.history_type??""]??99;if(v!==f)return v-f;let x=_.object_name.localeCompare(m.object_name);return x!==0?x:_.task_name.localeCompare(m.task_name)});return i.map(g=>({date:g,events:c.get(g)??[]}))}var ni=A`
  .cal-controls {
    display: flex;
    gap: 12px;
    align-items: center;
...
`;var ee=class extends T{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter)}getCardSize(){return 6}get _lang(){return this.hass?.language||"en"}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){super.updated(e);let t=this.hass?.language;if(t&&!Qe(t)&&se(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}_onEventClick(e){if(e.history_timestamp){this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}));return}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}render(){if(!this.hass)return d;let e=this._lang,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.title,l=null;this._userFilter&&(l=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let c=new Date;c.setHours(0,0,0,0);let p=this._pastDays>0,g=p?ri(this._objects,c,this._pastDays,l):ai(this._objects,c,this._windowDays,l),_=qe(c),m=this._windowDays===365||p,v=m?g.filter(b=>b.events.length>0):g,f=b=>{let j=`cal-status-${b.status}`,z=b.projected?"cal-event-projected":"",L=b.status==="overdue"&&b.days_until_due!=null?` (${Math.abs(b.days_until_due)}d ${s("overdue",e).toLowerCase()})`:"",M=b.projected&&b.interval_days?n`<span class="cal-event-recur">${b.interval_unit&&b.interval_unit!=="days"?`${b.interval_days} ${s("unit_"+b.interval_unit,e)}`:s("cal_every_n_days",e).replace("{n}",String(b.interval_days))}</span>`:d,E=b.schedule_type==="sensor_based",V=E?n`<ha-icon class="cal-event-icon cal-source-sensor"
                ...
                icon="${b.adaptive_enabled?"mdi:clock-time-four-outline":"mdi:clock-outline"}"></ha-icon>`,K=E&&b.prediction_confidence&&b.status!=="triggered"&&!b.projected?n`<span class="cal-event-prediction cal-conf-${b.prediction_confidence}">
            ${s("cal_predicted",e)} · ${s(`cal_confidence_${b.prediction_confidence}`,e)}
          </span>`:d,W=this._stats?.budget?.currency_symbol||"\u20AC",D=b.history_type?s(b.history_type,e):s(b.status,e);return n`
        <div class="cal-event ${z}"
          @click=${()=>this._onEventClick(b)}>
          ${V}
          <span class="cal-status-pill ${j}">${D}</span>
          <div class="cal-event-body">
            <div class="cal-event-title">${b.object_name} · ${b.task_name}${L}</div>
            ${K}
            ${M}
          </div>
          ${b.avg_cost!=null&&b.avg_cost>0?n`<span class="cal-event-cost">${b.avg_cost.toFixed(0)} ${W}</span>`:d}
        </div>
      `},x=b=>{let[j,z,L]=b.date.split("-").map(Number),M=new Date(j,z-1,L),E=b.date===_,V=M.toLocaleDateString(e,{weekday:"short"}),K=M.toLocaleDateString(e,{month:"long"});return n`
        <div class="cal-day-row">
          <div class="cal-day-pill ${E?"cal-today":""}">
            <span class="cal-pill-weekday">${V}</span>
            <span class="cal-pill-day">${M.getDate()}</span>
          </div>
          <div class="cal-day-content">
            <div class="cal-day-header">
              <span class="cal-day-month">${K}</span>
              ${E?n`<span class="cal-day-today-badge">${s("today",e)}</span>`:d}
            </div>
            ${b.events.length===0?n`<div class="cal-empty">${s("cal_no_events",e)}</div>`:b.events.map(f)}
          </div>
        </div>
      `};return n`
      <ha-card .header=${a}>
        ...
        <div class="cal-rolling">
          ${v.length===0&&m?n`<div class="cal-empty">${s("cal_no_events",e)}</div>`:v.map(x)}
        </div>
      </ha-card>
    `}};ee.styles=[Je,ni,A`
      :host { display: block; }
      ha-card { padding: 0; overflow: hidden; }
    `],h([y({attribute:!1})],ee.prototype,"hass",2),h([u()],ee.prototype,"_config",2),h([u()],ee.prototype,"_objects",2),h([u()],ee.prototype,"_stats",2),h([u()],ee.prototype,"_windowDays",2),h([u()],ee.prototype,"_pastDays",2),h([u()],ee.prototype,"_userFilter",2),h([u()],ee.prototype,"_unsub",2);var cs=[{value:7,label:"Week (7 days)"},{value:14,label:"Fortnight (14 days)"},{value:30,label:"Month (30 days, default)"},{value:365,label:"Year (365 days, empty days collapsed)"}],Ee=class extends T{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"}}setConfig(e){this._config={...e}}_valueChanged(e,t){let i={...this._config,[e]:t};e==="show_window_chips"&&t===!0&&delete i.show_window_chips,e==="show_user_filter"&&t===!0&&delete i.show_user_filter,e==="title"&&(!t||typeof t=="string"&&t.trim()==="")&&delete i.title,e==="user_filter"&&t===""&&delete i.user_filter,this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}render(){let e=this._config.window_days??30,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,a=this._config.user_filter??"",l=this._config.title??"";return n`
      <div class="editor">
        <div class="row">
        ...
        <div class="row">
          ...
          >
            ${cs.map(c=>n`<option value="${c.value}" ?selected=${c.value===e}>${c.label}</option>`)}
          </select>
        </div>
  ...
  `,h([y({attribute:!1})],G.prototype,"hass",2),h([u()],G.prototype,"_open",2),h([u()],G.prototype,"_title",2),h([u()],G.prototype,"_message",2),h([u()],G.prototype,"_confirmText",2),h([u()],G.prototype,"_danger",2),h([u()],G.prototype,"_inputLabel",2),h([u()],G.prototype,"_inputType",2),h([u()],G.prototype,"_inputValue",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",G);var li={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},ci=o=>(...r)=>({_$litDirective$:o,values:r}),nt=class{constructor(r){}get _$AU(){return this._$AM._$AU}_$AT(r,e,t){this._$Ct=r,this._$AM=e,this._$Ci=t}_$AS(r,e){return this.update(r,e)}update(r,e){return this.render(...e)}};var De=class extends nt{constructor(r){if(super(r),this.it=d,r.type!==li.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(r){if(r===d||r==null)return this._t=void 0,this.it=r;if(r===oe)return r;if(typeof r!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(r===this.it)return this._t;this.it=r;let e=[r];return e.raw=e,this._t={_$litType$:this.constructor.resultType,strings:e,values:[]}}};De.directiveName="unsafeHTML",De.resultType=1;var di=ci(De);var ps=["EUR","USD","GBP","JPY","CHF","CAD","AUD","CNY","INR","BRL","CZK","PLN","RUB","SEK","NOK","DKK","UAH"],C=class extends T{constructor(){super(...arguments);this.budget=null;this._settings=null;this._loading=!0;this._importCsv="";this._importLoading=!1;this._includeHistory=!0;this._toast="";this._testingNotification=!1;this._users=[];this._vacEnabled=!1;this._vacStart="";this._vacEnd="";this._vacBuffer=3;this._vacExempt=new Set;this._vacIsActive=!1;this._vacWindowEnd=null;this._vacAllTasks=[];this._vacPreview=[];this._vacPreviewLoading=!1;this._vacSaving=!1;this._qrObjects=[];this._qrSelectedEntries=new Set;this._qrActions=new Set(["view"]);this._qrUrlMode="companion";this._qrBatchLoading=!1;this._qrBatchResults=[];this._qrObjectsLoaded=!1;this._loaded=!1;this._userService=null;this._sendTestNotification=async()=>{this._testingNotification=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/test_notification"}),t=e.message||(e.success?s("test_notification_success",this._lang):s("test_notification_failed",this._lang));this._showToast(t)}catch{this._showToast(s("test_notification_failed",this._lang))}finally{this._testingNotification=!1}}}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._loaded?(this._loaded=!0,this._userService=new ue(this.hass),this._loadSettings(),this._loadUsers()):e.has("hass")&&this.hass&&this._userService&&this._userService.updateHass(this.hass)}async _loadUsers(){if(this._userService)try{this._users=await this._userService.getUsers()}catch{this._users=[]}}async _loadSettings(){this._loading=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});this._settings=e,this._hydrateVacationFromSettings()}catch{}this._loading=!1}_hydrateVacationFromSettings(){let e=this._settings?.vacation;e&&(this._vacEnabled=e.enabled,this._vacStart=e.start||"",this._vacEnd=e.end||"",this._vacBuffer=e.buffer_days,this._vacExempt=new Set(e.exempt_task_ids||[]),this._vacIsActive=e.is_active,this._vacWindowEnd=e.window_end)}async _updateSetting(e,t){try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/update",settings:{[e]:t}});this._settings=i,this._showToast(s("settings_saved",this._lang)),this.dispatchEvent(new CustomEvent("settings-changed"))}catch{this._showToast(s("action_error",this._lang))}}_showToast(e){this._toast=e,setTimeout(()=>{this._toast=""},3e3)}_downloadFile(e,t,i){et(e,t,i)}render(){let e=this._lang;return this._loading||!this._settings?n`<div class="settings-loading">Loading…</div>`:n`
    ...
    `}scrollToSection(e){requestAnimationFrame(()=>{let t=this.shadowRoot;if(!t)return;let i=t.querySelector(`[data-section="${e}"]`)??t.querySelector(`[data-section-alt="${e}"]`);i&&i.scrollIntoView({behavior:"smooth",block:"start"})})}_renderPanelAccess(e){let t=new Set(this._settings.admin_panel_user_ids||[]),i=this._users.filter(c=>!c.is_admin),a=this._settings.operator_write_enabled??!1,l=(c,p)=>{let g=new Set(t);p?g.add(c):g.delete(c),this._updateSetting("admin_panel_user_ids",[...g])};return n`
    ...
    `}_renderObjectsColumns(e){let t=He(this._settings.objects_table_columns);return n`
      ...
      <div class="settings-section">
        ...
        <label class="setting-row">
          ...
          <input type="number" min="1" max="365" .value=${String(t.default_warning_days)}
            @change=${c=>{let p=parseInt(c.target.value,10);p>=1&&p<=365&&this._updateSetting("default_warning_days",p)}} />
        </label>
        <label class="setting-row">
          <span class="setting-label">${s("settings_currency",e)}</span>
          <select @change=${c=>this._updateSetting("budget_currency",c.target.value)}>
            ${ps.map(c=>n`<option value=${c} ?selected=${l.currency===c}>${c}</option>`)}
          </select>
        </label>
        <label class="setting-row">
    ...
    `}_renderVacationTaskList(e){let t=new Map;for(let a of this._vacAllTasks){let l=t.get(a.object_name)||[];l.push(a),t.set(a.object_name,l)}return[...t.entries()].sort(([a],[l])=>a.localeCompare(l)).map(([a,l])=>n`
      <div class="vac-task-group">
        ...
        ${l.sort((c,p)=>c.task_name.localeCompare(p.task_name)).map(c=>n`
...
<body>
  ...
  `,h([y({attribute:!1})],C.prototype,"hass",2),h([y({attribute:!1})],C.prototype,"features",2),h([y({attribute:!1})],C.prototype,"budget",2),h([u()],C.prototype,"_settings",2),h([u()],C.prototype,"_loading",2),h([u()],C.prototype,"_importCsv",2),h([u()],C.prototype,"_importLoading",2),h([u()],C.prototype,"_includeHistory",2),h([u()],C.prototype,"_toast",2),h([u()],C.prototype,"_testingNotification",2),h([u()],C.prototype,"_users",2),h([u()],C.prototype,"_vacEnabled",2),h([u()],C.prototype,"_vacStart",2),h([u()],C.prototype,"_vacEnd",2),h([u()],C.prototype,"_vacBuffer",2),h([u()],C.prototype,"_vacExempt",2),h([u()],C.prototype,"_vacIsActive",2),h([u()],C.prototype,"_vacWindowEnd",2),h([u()],C.prototype,"_vacAllTasks",2),h([u()],C.prototype,"_vacPreview",2),h([u()],C.prototype,"_vacPreviewLoading",2),h([u()],C.prototype,"_vacSaving",2),h([u()],C.prototype,"_qrObjects",2),h([u()],C.prototype,"_qrSelectedEntries",2),h([u()],C.prototype,"_qrActions",2),h([u()],C.prototype,"_qrUrlMode",2),h([u()],C.prototype,"_qrBatchLoading",2),h([u()],C.prototype,"_qrBatchResults",2),h([u()],C.prototype,"_qrObjectsLoaded",2);customElements.define("maintenance-settings-view",C);var Q=class extends T{constructor(){super(...arguments);this.objects=[];this._summary=null;this._loaded=!1;this._busy=!1;this._error="";this._query="";this._results=[];this._expanded=!1;this._initiallyLoaded=!1;this._searchTimer=0}get _lang(){return this.hass?.language||"en"}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._initiallyLoaded&&(this._initiallyLoaded=!0,this._load(),se(this._lang).then(()=>this.requestUpdate()))}async _load(){this._busy=!0;try{this._summary=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/storage"}),this._error=""}catch(e){this._error=I(e,this._lang)}finally{this._loaded=!0,this._busy=!1}}_nameFor(e){return this.objects.find(i=>i.object?.id===e)?.object?.name||e.slice(0,8)}_entryFor(e){return this.objects.find(t=>t.object?.id===e)?.entry_id}_toggle(){this._expanded=!this._expanded}_openObject(e){this.dispatchEvent(new CustomEvent("open-object",{detail:{entry_id:e},bubbles:!0,composed:!0}))}_onSearch(e){this._query=e.target.value,clearTimeout(this._searchTimer),this._searchTimer=window.setTimeout(()=>{this._doSearch()},250)}async _doSearch(){let e=this._query.trim();if(!e){this._results=[];return}try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/search",query:e});this._results=t.results||[]}catch(t){this._error=I(t,this._lang),this._results=[]}}async _openResult(e){if(e.kind==="weblink"){window.open(e.url,"_blank","noopener");return}let t=window.open("about:blank","_blank");try{let i=await this.hass.connection.sendMessagePromise({type:"auth/sign_path",path:`/api/maintenance_supporter/document/${e.id}`,expires:300});t&&(t.location.href=new URL(i.path,window.location.origin).href)}catch(i){t&&t.close(),this._error=I(i,this._lang)}}_renderResult(e,t){return n`
    ...
    `}render(){if(!this._loaded||!this._summary)return d;let e=this._summary;if(e.document_count===0)return d;let t=this._lang,i=Object.entries(e.by_object).filter(([,a])=>a.files>0||a.links>0).map(([a,l])=>({id:a,name:this._nameFor(a),entry:this._entryFor(a),...l})).sort((a,l)=>l.bytes-a.bytes);return n`
      <ha-card>
        <div class="card-content">
          <div class="header">
            ...
            >
              ...
              <span class="header-summary">
                ${ae(e.total_bytes)}
                ${e.dedup_savings_bytes>0?n`<span class="saved">−${ae(e.dedup_savings_bytes)}</span>`:d}
              </span>
            </button>
            <button
              class="icon-btn"
          ...
          ${this._expanded?n`
                <div class="body">
                  ...
                  ${this._query.trim()?this._results.length?n`<div class="obj-list">${this._results.map(a=>this._renderResult(a,t))}</div>`:n`<div class="search-empty">${s("doc_search_none",t)}</div>`:i.length?n`<div class="obj-list">${i.map(a=>this._renderObjRow(a,t))}</div>`:d}
                </div>
              `:d}
        </div>
      </ha-card>
    `}_renderObjRow(e,t){let i=e.entry;return n`
      <div
        class="obj-row ${i?"clickable":""}"
        role=${i?"button":d}
        tabindex=${i?"0":d}
        aria-label=${i?e.name:d}
      ...
      >
        ...
        <span class="obj-meta">
          ${e.files>0?n`<ha-icon icon="mdi:file-document-outline"></ha-icon>${e.files}`:d}
          ${e.links>0?n`<ha-icon icon="mdi:link-variant"></ha-icon>${e.links}`:d}
        </span>
  ...
  `,h([y({attribute:!1})],Z.prototype,"hass",2),h([y({attribute:!1})],Z.prototype,"objects",2),h([u()],Z.prototype,"_open",2),h([u()],Z.prototype,"_loading",2),h([u()],Z.prototype,"_error",2),h([u()],Z.prototype,"_groupId",2),h([u()],Z.prototype,"_name",2),h([u()],Z.prototype,"_description",2),h([u()],Z.prototype,"_selected",2);customElements.get("maintenance-group-dialog")||customElements.define("maintenance-group-dialog",Z);function Fe(o,r,e=4){if(!isFinite(o)||!isFinite(r))return{ticks:[],niceMin:0,niceMax:1};if(o===r){let g=Math.abs(o)*.1||1;o-=g,r+=g}let t=r-o,i=Math.pow(10,Math.floor(Math.log10(t/Math.max(1,e)))),a=i;for(let g of[1,2,5,10])if(a=i*g,t/a<=e+.5)break;let l=Math.floor(o/a)*a,c=Math.ceil(r/a)*a,p=[];for(let g=l;g<=c+a*1e-6;g+=a)p.push(Math.abs(g)<a*1e-9?0:g);return{ticks:p,niceMin:l,niceMax:c}}function re(o){let r=Math.abs(o);return r>=1e6?Te((o/1e6).toFixed(r>=1e7?0:1))+"M":r>=1e4?Te((o/1e3).toFixed(0))+"k":r>=1e3?Te((o/1e3).toFixed(1))+"k":r>=100?o.toFixed(0):r>=10||r>=1?Te(o.toFixed(1)):r===0?"0":Te(o.toFixed(2))}function Te(o){return o.replace(/\.0+$/,"").replace(/(\.\d*[1-9])0+$/,"$1")}function fe(o,r,e){let t=o.toLocaleString(e,{maximumFractionDigits:Math.abs(o)>=100?0:1});return r?`${t} ${r}`:t}function Se(o,r,e){let t=new Date(o),i=e?{month:"short",day:"numeric",year:"2-digit"}:{month:"short",day:"numeric"};return t.toLocaleDateString(r,i)}function Ct(o,r){return new Date(o).toLocaleDateString(r,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function ot(o,r){return new Date(o).getFullYear()!==new Date(r).getFullYear()}function lt(o,r,e){if(e<2||r<=o)return[o,r];let t=[];for(let i=0;i<e;i++)t.push(o+(r-o)*i/(e-1));return t}var ct=210,ie=46,ce=14,de=12,pi=14,us=20+pi,_s=[{days:7,key:"chart_range_7d"},{days:30,key:"chart_range_30d"},{days:90,key:"chart_range_90d"},{days:365,key:"chart_range_1y"}],N=class extends T{constructor(){super(...arguments);this.points=[];this.events=[];this.unit="";this.lang="en";this.thresholdAbove=null;this.thresholdBelow=null;this.targetValue=null;this.forceZero=!1;this.projection=null;this.rangeDays=30;this.showRange=!0;this.busy=!1;this._width=0;this._hover=null;this._ro=null}connectedCallback(){super.connectedCallback(),this._ro=new ResizeObserver(e=>{let t=Math.floor(e[0]?.contentRect?.width||0);t&&Math.abs(t-this._width)>2&&(this._width=t)}),this._ro.observe(this)}disconnectedCallback(){super.disconnectedCallback(),this._ro?.disconnect(),this._ro=null}_emitRange(e){e!==this.rangeDays&&this.dispatchEvent(new CustomEvent("range-change",{detail:{days:e},bubbles:!0,composed:!0}))}render(){let e=this._width||320,t=[...this.points].sort((a,l)=>a.ts-l.ts),i=this.lang;return n`
      <div class="chart-wrap">
        ...
        ${t.length<2?n`<div class="chart-empty">
              <ha-icon icon="mdi:chart-line"></ha-icon> ${s("loading_chart",i)}
            </div>`:this._renderSvg(e,t)}
      </div>
    `}_renderSvg(e,t){let i=this.lang,a=e-ie-ce,l=ct-us,c=l-de,p=1/0,g=-1/0;for(let k of t)p=Math.min(p,k.min??k.val),g=Math.max(g,k.max??k.val);this.thresholdAbove!=null&&(p=Math.min(p,this.thresholdAbove),g=Math.max(g,this.thresholdAbove)),this.thresholdBelow!=null&&(p=Math.min(p,this.thresholdBelow),g=Math.max(g,this.thresholdBelow)),this.targetValue!=null&&(p=Math.min(p,this.targetValue),g=Math.max(g,this.targetValue)),this.forceZero&&(p=Math.min(p,0));let _=(g-p||1)*.06,m=this.forceZero&&p>=0?0:p-_,{ticks:v,niceMin:f,niceMax:x}=Fe(m,g+_,4);this.forceZero&&p>=0&&f<0&&(f=0,v=v.filter(k=>k>=0));let b=t[0].ts,j=t[t.length-1].ts,z=j-b||1,L=ot(b,j),M=k=>ie+(k-b)/z*a,E=k=>de+(1-(k-f)/(x-f||1))*c,V=t.map(k=>`${M(k.ts).toFixed(1)},${E(k.val).toFixed(1)}`).join(" "),K=`M${M(t[0].ts).toFixed(1)},${l} `+t.map(k=>`L${M(k.ts).toFixed(1)},${E(k.val).toFixed(1)}`).join(" ")+` L${M(t[t.length-1].ts).toFixed(1)},${l} Z`,W="",D=t.filter(k=>k.min!=null&&k.max!=null);if(D.length>=2){let k=D.map(R=>`${M(R.ts).toFixed(1)},${E(R.max).toFixed(1)}`),$=[...D].reverse().map(R=>`${M(R.ts).toFixed(1)},${E(R.min).toFixed(1)}`);W=`M${k[0]} `+k.slice(1).map(R=>`L${R}`).join(" ")+` L${$.join(" L")} Z`}let q=[];if(this.thresholdBelow!=null){let k=E(this.thresholdBelow);q.push({y:k,h:Math.max(0,l-k),lineY:k,label:`\u25BC ${re(this.thresholdBelow)}`,labelY:Math.min(l-4,k+13)})}if(this.thresholdAbove!=null){let k=E(this.thresholdAbove);q.push({y:de,h:Math.max(0,k-de),lineY:k,label:`\u25B2 ${re(this.thresholdAbove)}`,labelY:Math.max(de+11,k-5)})}let pe=t[t.length-1],_e=(this.events||[]).filter(k=>k.ts>=b&&k.ts<=j),ye=lt(b,j,Math.max(2,Math.min(5,Math.floor(a/110)+1))),U=this._hover;return n`
      <div class="svg-holder">
        ...
        >
          <defs>
            ...
            ${q.length?P`<clipPath id="danger">${q.map(k=>P`<rect x="${ie}" y="${k.y.toFixed(1)}" width="${a}" height="${k.h.toFixed(1)}" />`)}</clipPath>`:d}
          </defs>

          ${v.map(k=>{let $=E(k);return $<de-1||$>l+1?d:P`
          ...
          ${q.map(k=>P`<rect x="${ie}" y="${k.y.toFixed(1)}" width="${a}" height="${k.h.toFixed(1)}"
          ...
          <polyline points="${V}" fill="none" stroke="var(--primary-color)" stroke-width="2"
            stroke-linejoin="round" stroke-linecap="round" clip-path="url(#plot)" />
          ${q.length?P`<polyline points="${V}" fill="none" stroke="var(--error-color, #f44336)" stroke-width="2"
          ...
          ${this.targetValue!=null?P`<line x1="${ie}" y1="${E(this.targetValue).toFixed(1)}" x2="${e-ce}" y2="${E(this.targetValue).toFixed(1)}"
                stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="6,4" />
              <text x="${e-ce-4}" y="${(E(this.targetValue)-5).toFixed(1)}" text-anchor="end" class="zone-label">◆ ${re(this.targetValue)} ${this.unit}</text>`:d}

          ${this.projection&&this.projection.length===2?P`<line x1="${M(this.projection[0].ts).toFixed(1)}" y1="${E(this.projection[0].val).toFixed(1)}"
                x2="${Math.min(M(this.projection[1].ts),e-ce).toFixed(1)}" y2="${E(Math.max(f,Math.min(x,this.projection[1].val))).toFixed(1)}"
                stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.8" />`:d}

          ${ye.map((k,$)=>{let R=M(k),dt=$===0?"start":$===ye.length-1?"end":"middle";return P`<text x="${R.toFixed(1)}" y="${ct-5}" text-anchor="${dt}" class="tick-label">${Se(k,i,L)}</text>`})}
          ...
          ${_e.map(k=>{let $=M(k.ts),R=k.type==="completed"?"var(--success-color, #4caf50)":k.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return P`
              ...
              <rect x="${($-1.5).toFixed(1)}" y="${l+3}" width="3" height="${pi-6}" rx="1.5" fill="${R}">
        ...
        ${U?n`<div
            ...
            >
              ...
              <div class="hover-val">
                ${fe(U.p.val,this.unit,i)}
                ${U.p.min!=null&&U.p.max!=null?n`<span class="hover-range">(${re(U.p.min)}–${re(U.p.max)})</span>`:d}
              </div>
            </div>`:d}
      </div>
    `}_onPointer(e,t,i,a,l){let p=e.currentTarget.getBoundingClientRect(),g=(e.clientX-p.left)/p.width*l;if(g<ie-8||g>l-ce+8){this._hover=null;return}let _=t[0],m=1/0;for(let v of t){let f=Math.abs(i(v.ts)-g);f<m&&(m=f,_=v)}this._hover={x:i(_.ts),y:a(_.val),p:_}}};N.styles=A`
  ...
  `,h([y({attribute:!1})],N.prototype,"points",2),h([y({attribute:!1})],N.prototype,"events",2),h([y()],N.prototype,"unit",2),h([y()],N.prototype,"lang",2),h([y({attribute:!1})],N.prototype,"thresholdAbove",2),h([y({attribute:!1})],N.prototype,"thresholdBelow",2),h([y({attribute:!1})],N.prototype,"targetValue",2),h([y({type:Boolean})],N.prototype,"forceZero",2),h([y({attribute:!1})],N.prototype,"projection",2),h([y({attribute:!1})],N.prototype,"rangeDays",2),h([y({type:Boolean})],N.prototype,"showRange",2),h([y({type:Boolean})],N.prototype,"busy",2),h([u()],N.prototype,"_width",2),h([u()],N.prototype,"_hover",2);customElements.get("maintenance-trigger-chart")||customElements.define("maintenance-trigger-chart",N);function ui(o,r){let e=o.trigger_config;if(!e)return d;let t=r.lang,i=o.trigger_entity_info,a=o.trigger_entity_infos,l=i?.friendly_name||e.entity_id||"\u2014",c=e.entity_id||"",p=e.entity_ids||(c?[c]:[]),g=i?.unit_of_measurement||"",_=o.trigger_current_value,m=e.type||"threshold",v=p.length>1,f=gs(o,g,r);return n`
    <h3>${s("trigger",t)}</h3>
    <div class="trigger-card">
      <div class="trigger-header">
        ...
        <div>
          ${v?n`
            ...
            <div class="trigger-entity-id">${p.map((x,b)=>n`${b>0?", ":""}<span class="entity-link" @click=${j=>be(j,x)}>${x}</span>`)}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `:n`
            <div class="trigger-entity-name">${l}</div>
            <div class="trigger-entity-id">${c?n`<span class="entity-link" @click=${x=>be(x,c)}>${c}</span>`:""}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `}
        </div>
      ...
      ${f?ms(f,t):_!=null?n`
              <div class="trigger-value-row">
                <span class="trigger-current ${o.trigger_active?"active":""}">${typeof _=="number"?fe(_,"",t):_}</span>
      ...
      <div class="trigger-limits">
        ...
        ${m==="compound"?n`
          ...
          ${(e.conditions||[]).map((x,b)=>n`
            <span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${b+1}. ${s(x.type||"unknown",t)}: ${x.entity_id?n`<span class="entity-link" @click=${j=>be(j,x.entity_id)}>${x.entity_id}</span>`:""}</span>
          `)}
        `:d}
      </div>
      ...
      ${a&&a.length>1?n`
        <div class="trigger-entity-list">
          ${a.map(x=>n`
            <span class="trigger-entity-id">${x.friendly_name} (<span class="entity-link" @click=${b=>be(b,x.entity_id)}>${x.entity_id}</span>)</span>
          `)}
        </div>
      `:d}
  ...
  `}function gs(o,r,e){let t=o.trigger_config,i=o.trigger_current_value;if(!t||i==null)return null;switch(t.type||"threshold"){case"counter":{let a=t.trigger_target_value;if(a==null||a<=0)return null;let l=_i(o,gi(o,e));return{progress:Math.max(0,i-(l?.value??i)),target:a,unit:r,meter:i}}case"state_change":{let a=t.trigger_target_changes;return a==null||a<=0?null:{progress:Math.max(0,i),target:a,unit:"",meter:null}}case"runtime":{let a=t.trigger_runtime_hours;return a==null||a<=0?null:{progress:Math.max(0,i),target:a,unit:"h",meter:null}}}return null}function _i(o,r){if(o.trigger_baseline_value!=null)return{value:o.trigger_baseline_value,ts:hi(o)};if(!r.length)return null;let e=hi(o);if(e==null)return{value:r[0].val,ts:null};let t=r[0],i=Math.abs(r[0].ts-e);for(let a of r){let l=Math.abs(a.ts-e);l<i&&(t=a,i=l)}return{value:t.val,ts:e}}function hi(o){let r=[...o.history].filter(e=>e.type==="completed"||e.type==="reset").sort((e,t)=>new Date(t.timestamp).getTime()-new Date(e.timestamp).getTime())[0];return r?new Date(r.timestamp).getTime():null}function ms(o,r){let e=Math.min(999,Math.round(o.progress/o.target*100)),t=e>=100?"over":e>=75?"near":"ok";return n`
    <div class="counter-progress">
      <div class="counter-progress-nums">
        <span class="counter-progress-main">${fe(o.progress,"",r)}<span class="counter-progress-target"> / ${fe(o.target,o.unit,r)}</span></span>
      ...
      <div class="counter-progress-caption">
        ${s("chart_since_service",r)}${o.meter!=null?n` · ${s("current",r)}: ${fe(o.meter,o.unit,r)}`:d}
      </div>
    </div>
  `}function gi(o,r){let e=o.trigger_config;if(!e)return[];let t=e.entity_id||"",i=r.detailStatsData.get(t)||[],a=r.isCounterEntity(e),l=[];if(i.length>=2)for(let c of i){let p={ts:c.ts,val:c.val};!a&&c.min!=null&&c.max!=null&&(p.min=c.min,p.max=c.max),l.push(p)}else for(let c of o.history)c.trigger_value!=null&&l.push({ts:new Date(c.timestamp).getTime(),val:c.trigger_value});return o.trigger_current_value!=null&&l.push({ts:Date.now(),val:o.trigger_current_value}),l.sort((c,p)=>c.ts-p.ts),l}function vs(o,r,e){let t=o.trigger_config;if(!t)return d;let i=t.type||"threshold",a=t.entity_id||"",l=gi(o,e),c=l.length<2&&!!a&&e.hasStatsService&&!e.detailStatsData.has(a);if(l.length<2&&!c)return d;let p=!!a&&e.detailStatsData.has(a)&&(e.detailStatsData.get(a)?.length??0)<2,g=Date.now()-e.rangeDays*864e5,_=l.filter(b=>b.ts>=g);_.length>=2&&(l=_);let m=null,v=!1;if(i==="counter"&&t.trigger_target_value!=null&&l.length){let b=_i(o,l);if(b){if(b.ts!=null){let j=l.filter(z=>z.ts>=b.ts);j.length>=2&&(l=j)}l=l.map(j=>({...j,val:Math.max(0,j.val-b.value)}))}m=t.trigger_target_value,v=!0}else i==="state_change"&&t.trigger_target_changes?(m=t.trigger_target_changes,v=!0):i==="runtime"&&t.trigger_runtime_hours&&(m=t.trigger_runtime_hours,v=!0);let f=null;if(m==null&&o.degradation_rate!=null&&(o.degradation_trend!=="stable"||o.days_until_threshold!=null)&&o.degradation_trend!=="insufficient_data"&&l.length>=2){let b=l[l.length-1];f=[b,{ts:b.ts+30*864e5,val:b.val+o.degradation_rate*30}]}let x=o.history.filter(b=>["completed","skipped","reset"].includes(b.type)).map(b=>({ts:new Date(b.timestamp).getTime(),type:b.type}));return n`
      ...
      <div class="prediction-grid">
        ...
        ${i?n`
          <div class="prediction-item">
            ...
            ${o.threshold_prediction_date?n`<span class="prediction-date">${J(o.threshold_prediction_date,r)}</span>`:d}
        ...
        ${a&&e.environmental?n`
          <div class="prediction-item">
            ...
            ${o.environmental_entity?n`<span class="prediction-entity entity-link" @click=${c=>be(c,o.environmental_entity)}>${o.environmental_entity}</span>`:d}
          </div>
        `:d}
      </div>
    </div>
  `}function vi(o,r){let e=o.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return d;let a=o.interval_days??0,l=o.suggested_interval??a;return n`
  ...
  `}function xs(o,r,e){let t=o.confidence_interval_low,i=o.confidence_interval_high,a=r.suggested_interval??r.interval_days??0,l=r.interval_days??0,c=Math.max(0,t-5),g=i+5-c,_=(t-c)/g*100,m=(i-t)/g*100,v=(a-c)/g*100,f=l>0?(l-c)/g*100:-1;return n`
    <div class="confidence-range">
      <div class="confidence-range-title">
        ${s("confidence_interval",e)}: ${a} ${s("days",e)} (${t}\u2013${i})
      </div>
      <div class="confidence-bar">
        <div class="confidence-fill" style="left:${_.toFixed(1)}%;width:${m.toFixed(1)}%"></div>
        ${f>=0?n`<div class="confidence-marker current" style="left:${f.toFixed(1)}%"></div>`:d}
  ...
  `}function bi(o,r,e,t){let i=Math.max(o||1,r);return n`
  ...
  `}var fi=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"];function yi(o,r,e){if(!e.seasonal||!o.seasonal_factor||o.seasonal_factor===1)return d;let t=fi.map(c=>s(c,r)),i=new Date().getMonth(),a=o.seasonal_factors||o.interval_analysis?.seasonal_factors||null,l=a&&a.length===12?a:t.map((c,p)=>{let g=o.seasonal_factor||1,_=Math.sin((p-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,g+_))});return n`
    <div class="seasonal-card-compact">
      <h4>${s("seasonal_awareness",r)}</h4>
      <div class="seasonal-mini-chart">
        ${l.map((c,p)=>{let g=c*40,_=c<.9?"low":c>1.1?"high":"normal";return n`
            <div class="seasonal-bar ${_} ${p===i?"current":""}"
                 style="height: ${g}px"
  ...
  `}function xi(o,r){return $s(o,r)}function $s(o,r){let e=o.seasonal_factors??o.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return d;let t=o.interval_analysis?.seasonal_reason,i=new Date().getMonth(),a=300,l=100,c=8,g=l-c-4,_=Math.max(...e,1.5),m=a/12,v=m*.65,f=c+g-1/_*g;return n`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${a} ${l}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",r)}">
        ...
        ${e.map((x,b)=>{let j=x/_*g,z=b*m+(m-v)/2,L=c+g-j,M=b===i,E=x<1?"var(--success-color, #4caf50)":x>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return P`
            <rect x="${z.toFixed(1)}" y="${L.toFixed(1)}"
              width="${v.toFixed(1)}" height="${j.toFixed(1)}"
              fill="${E}" opacity="${M?1:.5}" rx="2" />
          `})}
      </svg>
      <div class="seasonal-labels">
        ${fi.map((x,b)=>n`<span class="seasonal-label ${b===i?"active-month":""}">${s(x,r)}</span>`)}
      </div>
    </div>
  `}var ws=200,Oe=10,ks=22;function $i(o,r,e,t){let i=o.history.filter(c=>c.type==="completed"&&(c.cost!=null||c.duration!=null));if(i.length<2)return d;let a=i.some(c=>(c.cost??0)>0),l=i.some(c=>(c.duration??0)>0);return!a&&!l?d:n`
  ...
  `}var Ts=60,Ss=20,wi=30,S=class extends T{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._chartRangeDays=(()=>{try{let e=parseInt(localStorage.getItem("msp-chart-range")||"",10);return[7,30,90,365].includes(e)?e:30}catch{return 30}})();this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._adminPanelUserIds=[];this._operatorWriteEnabled=!1;this._defaultWarningDays=7;this._actionLoading=!1;this._moreMenuOpen=!1;this._toastMessage="";this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab="dashboard";this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._objectSortMode="alphabetical";this._groupByMode="none";this._objectViewMode="cards";this._objectsTableColumns=Xe;this._showArchived=!1;this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._deepLinkHandled=!1;this._onDialogEvent=async()=>{try{await this._loadData()}catch{}};this._onCalendarLlCustom=e=>{let t=e.detail;t?.type==="maintenance-supporter:open-task"&&t.entry_id&&t.task_id&&(e.stopPropagation(),this._showTask(t.entry_id,t.task_id))};this._onHistoryEntrySaved=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"en"}get _isOperator(){let e=this.hass?.user;return e?e.is_admin?!1:!(this._operatorWriteEnabled&&this._adminPanelUserIds.includes(e.id)):!0}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler);let e=localStorage.getItem("maintenance_supporter_sort");e&&["due_date","object","type","task_name","area","assigned_user","group"].includes(e)&&(this._sortMode=e);let t=localStorage.getItem("maintenance_supporter_object_sort");t&&["alphabetical","due_soonest","task_count"].includes(t)&&(this._objectSortMode=t);let i=localStorage.getItem("maintenance_supporter_groupby");i&&["none","area","group","user"].includes(i)&&(this._groupByMode=i);let a=localStorage.getItem("maintenance_supporter_object_view");(a==="cards"||a==="table")&&(this._objectViewMode=a)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){super.updated(e);let t=this.hass?.language;if(t&&!Qe(t)&&se(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new it(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new ue(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,i,a,l]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);if(e&&(this._objects=e.objects),t&&(this._stats=t),i&&(this._budget=i),a&&(this._groups=a.groups||{}),l){let c=l;this._features=c.features,this._adminPanelUserIds=c.admin_panel_user_ids||[],this._operatorWriteEnabled=c.operator_write_enabled??!1;let p=c.general?.default_warning_days;typeof p=="number"&&p>=0&&p<=365&&(this._defaultWarningDays=p),this._objectsTableColumns=He(c.objects_table_columns)}this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("ms_action"),i=()=>{let _=window.location.pathname+window.location.hash;history.replaceState(history.state,"",_)};if(t==="add_object"){this._deepLinkHandled=!0,i(),requestAnimationFrame(()=>{this.shadowRoot?.querySelector("maintenance-object-dialog")?.openCreate()});return}if(t==="open_vacation"||t==="open_budget"||t==="open_groups"||t==="open_settings"){this._deepLinkHandled=!0,i(),this._overviewTab="settings",requestAnimationFrame(()=>{let _=this.shadowRoot?.querySelector("maintenance-settings-view"),m=t.replace("open_","");_?.scrollToSection?.(m)});return}let a=e.get("entry_id");if(!a)return;this._deepLinkHandled=!0;let l=e.get("task_id"),c=e.get("action"),p=window.location.pathname+window.location.hash;history.replaceState(history.state,"",p);let g=this._getObject(a);if(!g){this._showOverview();return}if(l){let _=g.tasks.find(m=>m.id===l);if(!_){this._showObject(a);return}this._showTask(a,l),c==="complete"?requestAnimationFrame(()=>{this._openCompleteDialog(a,l,_.name,this._features.checklists?_.checklist:void 0,this._features.adaptive&&!!_.adaptive_config?.enabled)}):c==="quick_complete"&&requestAnimationFrame(()=>{this._handleQuickComplete(a,l,_)})}else this._showObject(a)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let i=await this._statsService.getDetailStats(e,t,this._chartRangeDays),a=new Map(this._detailStatsData);a.set(e,i),this._detailStatsData=a}_setChartRange(e){if(e===this._chartRangeDays)return;this._chartRangeDays=e;try{localStorage.setItem("msp-chart-range",String(e))}catch{}let t=this._selectedEntryId&&this._selectedTaskId?this._getTask(this._selectedEntryId,this._selectedTaskId):null,i=t?.trigger_config?.entity_id;if(i){let a=new Map(this._detailStatsData);a.delete(i),this._detailStatsData=a,this._fetchDetailStats(i,this._isCounterEntity(t.trigger_config))}}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let i of this._objects)for(let a of i.tasks){let l=a.trigger_config?.entity_id;l&&e.push({entityId:l,isCounter:this._isCounterEntity(a.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let m of this._objects)for(let v of m.tasks){if(!this._showArchived&&v.archived||this._filterStatus&&v.status!==this._filterStatus)continue;if(this._filterUser){let x=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(v.responsible_user_id!==x)continue}let f=[];for(let x of Object.values(this._groups))x.task_refs?.some(b=>b.entry_id===m.entry_id&&b.task_id===v.id)&&f.push(x.name);e.push({entry_id:m.entry_id,task_id:v.id,object_name:m.object.name,task_name:v.name,type:v.type,schedule_type:v.schedule_type,status:v.status,days_until_due:v.days_until_due??null,next_due:v.next_due??null,trigger_active:v.trigger_active,trigger_current_value:v.trigger_current_value??null,trigger_current_delta:v.trigger_current_delta??null,trigger_config:v.trigger_config??null,trigger_entity_info:v.trigger_entity_info??null,times_performed:v.times_performed,total_cost:v.total_cost,interval_days:v.interval_days??null,interval_unit:v.interval_unit??null,interval_anchor:v.interval_anchor??null,is_done:v.is_done??!1,archived:v.archived??!1,history:v.history||[],enabled:v.enabled,nfc_tag_id:v.nfc_tag_id??null,area_id:m.object.area_id??null,responsible_user_id:v.responsible_user_id??null,group_names:f})}let t={overdue:0,triggered:1,due_soon:2,ok:3},i=(m,v)=>(t[m.status]??9)-(t[v.status]??9),a=(m,v)=>(m.days_until_due??99999)-(v.days_until_due??99999),l=(m,v)=>i(m,v)||a(m,v),c=m=>m.area_id&&this.hass?.areas?.[m.area_id]?.name||"",p=m=>m.responsible_user_id&&this._userService?.getUserName(m.responsible_user_id)||"",g=m=>m.group_names[0]||"",_={due_date:l,object:(m,v)=>m.object_name.localeCompare(v.object_name)||l(m,v),type:(m,v)=>m.type.localeCompare(v.type)||l(m,v),task_name:(m,v)=>m.task_name.localeCompare(v.task_name),area:(m,v)=>{let f=c(m),x=c(v);return!f&&x?1:f&&!x?-1:f.localeCompare(x)||l(m,v)},assigned_user:(m,v)=>{let f=p(m),x=p(v);return!f&&x?1:f&&!x?-1:f.localeCompare(x)||l(m,v)},group:(m,v)=>{let f=g(m),x=g(v);return!f&&x?1:f&&!x?-1:f.localeCompare(x)||l(m,v)}};return e.sort(_[this._sortMode]),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(a=>a.id===t)}_pushPanelState(e,t,i){let a={msp_view:e,msp_entry:t||null,msp_task:i||null};history.pushState(a,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let i=this._getTask(t.msp_entry,t.msp_task);i?.trigger_config?.entity_id&&this._fetchDetailStats(i.trigger_config.entity_id,this._isCounterEntity(i.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1,this._scrollContentToTop()}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null,this._scrollContentToTop()}_filterByStatus(e){this._filterStatus=e,this._overviewTab!=="dashboard"&&(this._overviewTab="dashboard"),this._scrollContentToTop()}_scrollContentToTop(){requestAnimationFrame(()=>{let e=this.shadowRoot?.querySelector(".content");e&&e.scrollTo({top:0,behavior:"smooth"})})}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null,this._scrollContentToTop()}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null,this._scrollContentToTop();let i=this._getTask(e,t);if(i?.trigger_config?.entity_id){let a=i.trigger_config.entity_id,l=this._isCounterEntity(i.trigger_config);this._fetchDetailStats(a,l)}}_showToast(e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}async _deleteObject(e){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("delete",this._lang),message:s("confirm_delete_object",this._lang),confirmText:s("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _deleteTask(e,t){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("delete",this._lang),message:s("confirm_delete_task",this._lang),confirmText:s("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _toggleArchiveTask(e,t,i){this._actionLoading=!0;try{await this.hass.connection.sendMessagePromise({type:i?"maintenance_supporter/task/unarchive":"maintenance_supporter/task/archive",entry_id:e,task_id:t}),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}finally{this._actionLoading=!1}}async _toggleArchiveObject(e,t){if(!(!t&&!await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:s("archive_object",this._lang),message:s("confirm_archive_object",this._lang),confirmText:s("archive_object",this._lang)})))try{await this.hass.connection.sendMessagePromise({type:t?"maintenance_supporter/object/unarchive":"maintenance_supporter/object/archive",entry_id:e}),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _skipTask(e,t,i){this._actionLoading=!0;try{let a={type:"maintenance_supporter/task/skip",entry_id:e,task_id:t};i&&(a.reason=i),await this.hass.connection.sendMessagePromise(a),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}finally{this._actionLoading=!1}}async _resetTask(e,t,i){this._actionLoading=!0;try{let a={type:"maintenance_supporter/task/reset",entry_id:e,task_id:t};i&&(a.date=i),await this.hass.connection.sendMessagePromise(a),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}finally{this._actionLoading=!1}}async _applySuggestion(e,t,i){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:i}),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}_openSeasonalOverrides(e){let t=this.shadowRoot.querySelector("maintenance-seasonal-overrides-dialog");if(!t||!this._selectedEntryId)return;let i=e.adaptive_config?.seasonal_overrides;t.open(this._selectedEntryId,e.id,i)}async _reanalyzeInterval(e,t){try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:e,task_id:t});i.recommended_interval?this._showToast(`${s("reanalyze_result",this._lang)}: ${i.recommended_interval} ${s("days",this._lang)} (${s(`confidence_${i.confidence}`,this._lang)}, ${i.data_points} ${s("data_points",this._lang)})`):this._showToast(s("reanalyze_insufficient_data",this._lang)),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}async _promptSkipTask(e,t){let i=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!i)return;let a=await i.prompt({title:s("skip",this._lang),message:s("skip_reason_prompt",this._lang),confirmText:s("skip",this._lang),inputLabel:s("reason_optional",this._lang),inputType:"text"});a.confirmed&&this._skipTask(e,t,a.value||void 0)}async _promptResetTask(e,t){let i=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!i)return;let a=await i.prompt({title:s("reset",this._lang),message:s("reset_date_prompt",this._lang),confirmText:s("reset",this._lang),inputLabel:s("reset_date_optional",this._lang),inputType:"date"});a.confirmed&&this._resetTask(e,t,a.value||void 0)}_dismissSuggestion(e,t){e&&t&&this._dismissedSuggestions.add(`${e}_${t}`),this.requestUpdate()}async _handleQuickComplete(e,t,i){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/quick_complete",entry_id:e,task_id:t}),this._showToast(s("quick_complete_success",this._lang))}catch(a){(a?.code||"")==="no_defaults"?this._openCompleteDialog(e,t,i.name,this._features.checklists?i.checklist:void 0,this._features.adaptive&&!!i.adaptive_config?.enabled):this._showToast(s("action_error",this._lang))}}_openCompleteDialog(e,t,i,a,l){let c=this.shadowRoot.querySelector("maintenance-complete-dialog");c&&(c.entryId=e,c.taskId=t,c.taskName=i,c.lang=this._lang,c.checklist=a||[],c.adaptiveEnabled=!!l,c.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,i,a){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,i,a)}render(){return n`
    ...
    `}_renderDashboard(){let e=this._stats,t=this._taskRows,i=this._lang,a=this._isOperator,l=this._objects.reduce((c,p)=>c+p.tasks.filter(g=>g.archived).length,0);return n`
      ...
      <div class="filter-bar">
        ...
        <label class="filter-field">
          ...
          <select
            ...
            @change=${c=>{let p=c.target.value;this._filterUser=p||null}}
          >
    ...
    `}_renderGroupedTasks(e,t){let i=new Map,a=s("unassigned",t);for(let p of e){let g=[];this._groupByMode==="area"?g=[(p.area_id?this.hass?.areas?.[p.area_id]?.name:null)||a]:this._groupByMode==="user"?g=[(p.responsible_user_id?this._userService?.getUserName(p.responsible_user_id):null)||a]:this._groupByMode==="group"&&(g=p.group_names.length>0?p.group_names:[a]);for(let _ of g)i.has(_)||i.set(_,[]),i.get(_).push(p)}let l=[...i.entries()].sort(([p],[g])=>p===a&&g!==a?1:g===a&&p!==a?-1:p.localeCompare(g)),c=this._groupByMode==="area"?"mdi:map-marker-outline":this._groupByMode==="group"?"mdi:folder-outline":"mdi:account-outline";return n`
      ${l.map(([p,g])=>n`
        <details class="group-section" open>
          ...
          <div class="task-table">
            ${g.map(_=>this._renderOverviewRow(_))}
          </div>
        </details>
      `)}
    `}_warrantyLabel(e,t,i){return e.kind==="expired"?s("warranty_expired",i):e.kind==="expiring"?s("warranty_expires_in",i).replace("{days}",String(e.days??0)):s("warranty_valid_until",i).replace("{date}",J(t,i))}_renderWarrantyMeta(e,t){let i=jt(e);return n`<p class="meta">${s("warranty",t)}:
      <span class="warranty-chip warranty-${i.kind}">${this._warrantyLabel(i,e,t)}</span></p>`}_renderAllObjects(){let e=this._lang,t=this._isOperator,i=this._objectViewMode==="table"&&!this.narrow,a=this._objects.filter(_=>_.object.archived).length,l=_=>{let m=1/0;for(let v of _.tasks){let f=v.days_until_due;f!=null&&f<m&&(m=f)}return m},c=this._objects.filter(_=>this._showArchived||!_.object.archived);this._objectSortMode==="alphabetical"?c.sort((_,m)=>_.object.name.localeCompare(m.object.name)):this._objectSortMode==="task_count"?c.sort((_,m)=>m.tasks.length-_.tasks.length||_.object.name.localeCompare(m.object.name)):c.sort((_,m)=>l(_)-l(m)||_.object.name.localeCompare(m.object.name));let p=()=>{let _=new Map;for(let m of c){let v=m.object.area_id,f=v?this.hass?.areas?.[v]?.name||s("unassigned",e):s("no_area",e);_.has(f)||_.set(f,[]),_.get(f).push(m)}return new Map([..._.entries()].sort(([m],[v])=>m.localeCompare(v)))},g=_=>{let m=_.tasks.some(v=>v.status==="overdue"||v.status==="triggered");return n`
        <div class="object-card${m?" object-card-overdue":""}" @click=${()=>this._showObject(_.entry_id)}>
          ${m?n`<span class="overdue-dot" title="${s("has_overdue",e)}"></span>`:d}
          <div class="object-card-header">
            <span class="object-card-name">${_.object.name}</span>
            ${_.object.document_count?n`<span class="doc-badge" title="${_.object.document_count} ${s("documents",e)}">
                  <ha-icon icon="mdi:paperclip"></ha-icon>${_.object.document_count}
                </span>`:d}
            <span class="object-card-count">${_.tasks.length} ${s("tasks_lower",e)}</span>
          </div>
          ${_.object.manufacturer||_.object.model?n`<div class="object-card-meta">${[_.object.manufacturer,_.object.model].filter(Boolean).join(" ")}</div>`:d}
          ${_.tasks.length===0?n`<div class="object-card-empty">${s("no_tasks_yet",e)}</div>`:d}
        </div>
      `};return n`
    ...
    `}_openGroupCreate(){this.shadowRoot.querySelector("maintenance-group-dialog")?.openCreate()}_openGroupEdit(e){let t=this._groups[e];t&&this.shadowRoot.querySelector("maintenance-group-dialog")?.openEdit(e,t)}async _deleteGroup(e,t){let i=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(i?await i.confirm({title:s("delete_group",this._lang),message:s("delete_group_confirm",this._lang).replace("{name}",t),confirmText:s("delete",this._lang)}):confirm(`${s("delete_group_confirm",this._lang).replace("{name}",t)}`))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/group/delete",group_id:e}),await this._loadData()}catch{this._showToast(s("action_error",this._lang))}}_renderBudgetBar(){let e=this._budget;if(!e)return d;let t=this._lang,i=e.currency_symbol||"\u20AC",a=[];return e.monthly_budget>0&&a.push({label:s("budget_monthly",t),spent:e.monthly_spent,budget:e.monthly_budget}),e.yearly_budget>0&&a.push({label:s("budget_yearly",t),spent:e.yearly_spent,budget:e.yearly_budget}),a.length===0?d:n`
      <div class="budget-bars">
        ${a.map(l=>{let c=Math.min(100,Math.max(0,l.spent/l.budget*100)),p=c>=100?"var(--error-color, #f44336)":c>=e.alert_threshold_pct?"var(--warning-color, #ff9800)":"var(--success-color, #4caf50)";return n`
    ...
    `}_renderOverviewRow(e){let t=this._lang,i=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,a=0,l=Ge.ok,c=!1;if(i&&e.days_until_due!==null){let m=At(e.interval_days,e.days_until_due,e.interval_unit);a=m.pct,c=m.overflow,e.status==="overdue"?l=Ge.overdue:e.status==="due_soon"&&(l=Ge.due_soon)}let p=e.area_id?this.hass?.areas?.[e.area_id]?.name:null,g=e.responsible_user_id?this._userService?.getUserName(e.responsible_user_id):null,_=e.group_names.length>0||p||g;return n`
      <div class="task-row${e.enabled?"":" task-disabled"}">
        ...
        <span class="task-sub${_?"":" task-sub-empty"}">
    ...
    `}_openCompleteDialogForRow(e){let i=this._objects.find(a=>a.entry_id===e.entry_id)?.tasks.find(a=>a.id===e.task_id);this._openCompleteDialog(e.entry_id,e.task_id,e.task_name,this._features.checklists?i?.checklist:void 0,this._features.adaptive&&!!i?.adaptive_config?.enabled)}_renderTriggerProgress(e){let t=e.trigger_config??null;if(!t)return d;let i=t.type||"threshold",a=e.trigger_entity_info?.unit_of_measurement??"",l=0,c="";if(i==="threshold"){let _=e.trigger_current_value??null;if(_==null)return d;let m=t.trigger_above,v=t.trigger_below;if(m!=null){let f=v??0,x=m-f||1;l=Math.min(100,Math.max(0,(_-f)/x*100)),c=`${_.toFixed(1)} / ${m} ${a}`}else if(v!=null){let x=e.trigger_entity_info?.max??(v*2||100),b=x-v||1;l=Math.min(100,Math.max(0,(x-_)/b*100)),c=`${_.toFixed(1)} / ${v} ${a}`}else return d}else if(i==="counter"){let _=t.trigger_target_value||1,v=e.trigger_current_delta??null??e.trigger_current_value??null;if(v==null)return d;l=Math.min(100,Math.max(0,v/_*100)),c=`${v.toFixed(1)} / ${_} ${a}`}else if(i==="state_change"){let _=t.trigger_target_changes||1,m=e.trigger_current_value??null;if(m==null)return d;l=Math.min(100,Math.max(0,m/_*100)),c=`${Math.round(m)} / ${_}`}else if(i==="runtime"){let _=t.trigger_runtime_hours||100,m=e.trigger_current_value??null;if(m==null)return d;l=Math.min(100,Math.max(0,m/_*100)),c=`${m.toFixed(1)}h / ${_}h`}else if(i==="compound"){let _=t.compound_logic||t.operator||"AND",m=t.conditions?.length||0;c=`${_} (${m})`,l=e.trigger_active?100:0}else return d;let p=l>=100,g=l>90?"var(--error-color, #f44336)":l>70?"var(--warning-color, #ff9800)":"var(--primary-color)";return n`
      <div class="trigger-progress">
        <div class="trigger-progress-bar">
          <div class="trigger-progress-fill${p?" overflow":""}" style="width:${l}%;background:${g}"></div>
        </div>
        <span class="trigger-progress-label">${c}</span>
      </div>
    `}_renderMiniSparkline(e){if(!e.trigger_config?.entity_id)return d;let t=e.trigger_config.entity_id,i=this._miniStatsData.get(t)||[],a=[];if(i.length>=2)a=i.map(E=>({ts:E.ts,val:E.val}));else{if(!e.history)return d;for(let E of e.history)E.trigger_value!=null&&a.push({ts:new Date(E.timestamp).getTime(),val:E.trigger_value})}if(e.trigger_current_value!=null&&a.push({ts:Date.now(),val:e.trigger_current_value}),a.length<2)return d;a.sort((E,V)=>E.ts-V.ts);let l=Ts,c=Ss,p=a.map(E=>E.val),g=Math.min(...p),_=Math.max(...p),m=_-g||1;g-=m*.1,_+=m*.1;let v=a[0].ts,x=a[a.length-1].ts-v||1,b=E=>(E-v)/x*l,j=E=>2+(1-(E-g)/(_-g))*(c-4),z=a;if(z.length>wi){let E=Math.ceil(z.length/wi);z=z.filter((V,K)=>K%E===0||K===z.length-1)}let L=z.map(E=>`${b(E.ts).toFixed(1)},${j(E.val).toFixed(1)}`).join(" "),M=e.trigger_active?"var(--error-color, #f44336)":"var(--primary-color)";return n`
    ...
    `}_renderDaysProgress(e){let t=this._lang;if(e.days_until_due==null||!e.interval_days||e.interval_days<=0)return d;let{pct:i,overflow:a}=At(e.interval_days,e.days_until_due,e.interval_unit),l="var(--success-color, #4caf50)";return e.status==="overdue"?l="var(--error-color, #f44336)":e.status==="due_soon"&&(l="var(--warning-color, #ff9800)"),n`
      <div class="days-progress">
        <div class="days-progress-labels">
          <span>${e.last_performed?`${s("last_performed",t)}: ${J(e.last_performed,t)}`:""}</span>
          <span>${e.next_due?`${s("next_due",t)}: ${J(e.next_due,t)}`:""}</span>
        </div>
    ...
    `}_renderObjectDetail(){if(!this._selectedEntryId)return d;let e=this._getObject(this._selectedEntryId);if(!e)return n`<p>Object not found.</p>`;let t=e.object,i=this._lang,a=this._isOperator,l=e.tasks.filter(p=>p.archived).length,c=e.tasks.filter(p=>this._showArchived||!p.archived);return n`
      <div class="detail-section">
        ...
        ${t.installation_date?n`<p class="meta">${s("installed",i)}: ${J(t.installation_date,i)}</p>`:d}
        ...
        ${e.tasks.length===0?n`<div class="empty-state-centered">
            ...
            </div>`:n`<div class="task-table">${[...c].sort((p,g)=>{let _={overdue:0,triggered:1,due_soon:2,ok:3};return(_[p.status]??9)-(_[g.status]??9)||(p.days_until_due??99999)-(g.days_until_due??99999)}).map(p=>n`
              <div class="task-row${p.enabled?"":" task-disabled"}">
                <span class="cell-badges">
                  <span class="status-badge ${p.archived?"archived":p.is_done?"done":p.status}">${p.archived?s("archived",i):p.is_done?s("completed",i):s(p.status,i)}</span>
                  ${p.enabled?d:n`<span class="badge-disabled">${s("disabled",i)}</span>`}
                ...
                <span class="due-cell" @click=${()=>this._showTask(e.entry_id,p.id)}>
                  ...
                  ${p.trigger_config?this._renderTriggerProgress(p):d}
    ...
    `}_renderTaskHeader(e){let t=this._lang,a=this._getObject(this._selectedEntryId)?.object.name||"",l=this._isOperator,c=e.archived?"archived":e.is_done?"done":e.status==="due_soon"?"warning":e.status||"ok",p=e.archived?s("archived",t):e.is_done?s("completed",t):s(e.status||"ok",t);return n`
    ...
    `}_renderTabContent(e){switch(this._activeTab){case"overview":return this._renderOverviewTab(e);case"history":return this._renderHistoryTab(e);default:return d}}get _sparklineCtx(){return{lang:this._lang,detailStatsData:this._detailStatsData,hasStatsService:!!this._statsService,isCounterEntity:e=>this._isCounterEntity(e),rangeDays:this._chartRangeDays,setRangeDays:e=>this._setChartRange(e)}}_renderOverviewTab(e){let t=this._lang,i=this._features.adaptive&&e.suggested_interval&&e.suggested_interval!==e.interval_days,a=this._features.seasonal&&e.seasonal_factor&&e.seasonal_factor!==1,l=i||a,c=this._features.adaptive&&e.interval_analysis?.weibull_beta!=null&&e.interval_analysis?.weibull_eta!=null,p=this._features.seasonal&&(e.seasonal_factors?.length===12||e.interval_analysis?.seasonal_factors?.length===12);return n`
      <div class="tab-content overview-tab">
        ...
        ${ui(e,this._sparklineCtx)}
        ${mi(e,t,this._features)}
        <div class="two-column-layout ${l?"":"single-column"}">
          ...
          <div class="right-column">
            ${$i(e,t,this._costDurationToggle,g=>{this._costDurationToggle=g})}
          </div>
        </div>
        ${c?vi(e,t):d}
        ${p?n`
    ...
    `}_renderKPIBar(e){let t=this._lang,i=e.times_performed>0?e.total_cost/e.times_performed:0,a=e.days_until_due!==null&&e.days_until_due!==void 0?e.days_until_due<0?"overdue":e.days_until_due<=e.warning_days?"warning":"":"";return n`
      <div class="kpi-bar">
        ...
        <div class="kpi-card">
          ...
          <div class="kpi-value">${Xt(e,t)}</div>
    ...
    `}_renderRecommendationCard(e){let t=this._lang;if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return d;if(this._selectedEntryId&&this._selectedTaskId&&this._dismissedSuggestions.has(`${this._selectedEntryId}_${this._selectedTaskId}`))return d;let i=e.suggested_interval;return n`
      <div class="recommendation-card">
        ...
        ${bi(e.interval_days,i,e.interval_confidence||"medium",t)}
      ...
      <div class="recent-activities">
        ...
        ${i.map(l=>n`
          <div class="activity-item">
            ...
            <span class="activity-date">${Tt(l.timestamp,t)}</span>
    ...
    `}_renderHistoryEntry(e){let t=this._lang,i=["completed","reset","skipped"].includes(e.type);return n`
      <div class="history-entry">
        <div class="history-icon ${e.type}">
          <ha-icon .icon=${Zt[e.type]||"mdi:circle"}></ha-icon>
        </div>
        <div class="history-content">
          ...
          <div class="history-date">${Tt(e.timestamp,t)}</div>
    ...
    `}_openHistoryEdit(e){if(!this._selectedEntryId||!this._selectedTaskId)return;let t={entry_id:this._selectedEntryId,task_id:this._selectedTaskId,original_timestamp:e.timestamp,type:e.type,timestamp:e.timestamp,notes:e.notes??null,cost:e.cost??null,duration:e.duration??null,completed_by:e.completed_by??null};this.shadowRoot?.querySelector("maintenance-history-edit-dialog")?.openEdit(t)}};S.styles=[Je,ei],h([y({attribute:!1})],S.prototype,"hass",2),h([y({type:Boolean,reflect:!0})],S.prototype,"narrow",2),h([y({attribute:!1})],S.prototype,"panel",2),h([u()],S.prototype,"_objects",2),h([u()],S.prototype,"_stats",2),h([u()],S.prototype,"_view",2),h([u()],S.prototype,"_selectedEntryId",2),h([u()],S.prototype,"_selectedTaskId",2),h([u()],S.prototype,"_filterStatus",2),h([u()],S.prototype,"_filterUser",2),h([u()],S.prototype,"_unsub",2),h([u()],S.prototype,"_chartRangeDays",2),h([u()],S.prototype,"_historyFilter",2),h([u()],S.prototype,"_budget",2),h([u()],S.prototype,"_groups",2),h([u()],S.prototype,"_detailStatsData",2),h([u()],S.prototype,"_miniStatsData",2),h([u()],S.prototype,"_features",2),h([u()],S.prototype,"_adminPanelUserIds",2),h([u()],S.prototype,"_operatorWriteEnabled",2),h([u()],S.prototype,"_defaultWarningDays",2),h([u()],S.prototype,"_actionLoading",2),h([u()],S.prototype,"_moreMenuOpen",2),h([u()],S.prototype,"_toastMessage",2),h([u()],S.prototype,"_overviewTab",2),h([u()],S.prototype,"_activeTab",2),h([u()],S.prototype,"_costDurationToggle",2),h([u()],S.prototype,"_historySearch",2),h([u()],S.prototype,"_sortMode",2),h([u()],S.prototype,"_objectSortMode",2),h([u()],S.prototype,"_groupByMode",2),h([u()],S.prototype,"_objectViewMode",2),h([u()],S.prototype,"_objectsTableColumns",2),h([u()],S.prototype,"_showArchived",2),S=h([Yt("maintenance-supporter-panel")],S);export{S as MaintenanceSupporterPanel};
