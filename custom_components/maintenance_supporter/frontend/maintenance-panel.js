...
<script>setTimeout(()=>window.print(),300)<\/script>
          ...
          ${this._loading?l`<div class="loading">${n("qr_generating",e)}</div>`:this._error?l`<div class="error">${this._error}</div>`:i?l`
                    <div class="qr-pair">
                      <div class="qr-item">
                        ...
                        <button class="dl-btn" title="${n("qr_download",e)}"
      ...
      <div class="trigger-limits">
        ...
        ${v==="compound"?l`
          ...
          ${(e.conditions||[]).map((f,w)=>l`
            <span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${w+1}. ${n(f.trigger_config?.type||"unknown",t)}: ${f.trigger_config?.entity_id||""}</span>
          `)}
        `:c}
  ...
  `}function ni(s,a,e){let t=s.trigger_config;if(!t)return c;let i=t.type||"threshold",r=i==="counter"&&t.trigger_delta_mode,o=e.isCounterEntity(t),d=t.entity_id||"",p=e.detailStatsData.get(d)||[],_=[],m=!1;if(p.length>=2)for(let h of p){let k=h.val;r&&s.trigger_baseline_value!=null&&(k-=s.trigger_baseline_value);let F={ts:h.ts,val:k};!o&&h.min!=null&&h.max!=null&&(F.min=r&&s.trigger_baseline_value!=null?h.min-s.trigger_baseline_value:h.min,F.max=r&&s.trigger_baseline_value!=null?h.max-s.trigger_baseline_value:h.max,m=!0),_.push(F)}else for(let h of s.history)h.trigger_value!=null&&_.push({ts:new Date(h.timestamp).getTime(),val:h.trigger_value});if(s.trigger_current_value!=null){let h=s.trigger_current_value;r&&s.trigger_baseline_value!=null&&(h-=s.trigger_baseline_value),_.push({ts:Date.now(),val:h})}if(_.length<2&&d&&e.hasStatsService&&!e.detailStatsData.has(d))return l`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
      <ha-icon icon="mdi:chart-line" style="--mdc-icon-size:16px;margin-right:8px;"></ha-icon>
      ${n("loading_chart",e.lang)}
    </div>`;if(_.length<2)return c;_.sort((h,k)=>h.ts-k.ts);let v=ai,b=ri,f=30,w=2,T=8,H=16,C=_.map(h=>h.val),E=Math.min(...C),y=Math.max(...C);if(m)for(let h of _)h.min!=null&&(E=Math.min(E,h.min)),h.max!=null&&(y=Math.max(y,h.max));t.trigger_above!=null&&(y=Math.max(y,t.trigger_above),E=Math.min(E,t.trigger_above)),t.trigger_below!=null&&(E=Math.min(E,t.trigger_below),y=Math.max(y,t.trigger_below));let L=null,q=null;if(i==="counter"&&t.trigger_target_value!=null){if(s.trigger_baseline_value!=null)L=s.trigger_baseline_value;else if(_.length>0){let h=[...s.history].filter(k=>k.type==="completed"||k.type==="reset").sort((k,F)=>new Date(F.timestamp).getTime()-new Date(k.timestamp).getTime())[0];if(h){let k=new Date(h.timestamp).getTime(),F=_[0],P=Math.abs(_[0].ts-k);for(let K of _){let Z=Math.abs(K.ts-k);Z<P&&(F=K,P=Z)}L=F.val}else L=_[0].val}L!=null?(q=L+t.trigger_target_value,y=Math.max(y,q),E=Math.min(E,L)):(y=Math.max(y,t.trigger_target_value),E=Math.min(E,0))}r&&s.trigger_baseline_value!=null&&(E=Math.min(E,0));let W=y-E||1;E-=W*.1,y+=W*.1;let B=_[0].ts,z=_[_.length-1].ts,x=z-B||1,D=h=>f+(h-B)/x*(v-f-w),M=h=>T+(1-(h-E)/(y-E))*(b-T-H),we=_.map(h=>`${D(h.ts).toFixed(1)},${M(h.val).toFixed(1)}`).join(" "),yt=`M${D(_[0].ts).toFixed(1)},${b-H} `+_.map(h=>`L${D(h.ts).toFixed(1)},${M(h.val).toFixed(1)}`).join(" ")+` L${D(_[_.length-1].ts).toFixed(1)},${b-H} Z`,ke="";if(m){let h=_.filter(k=>k.min!=null&&k.max!=null);if(h.length>=2){let k=h.map(P=>`${D(P.ts).toFixed(1)},${M(P.max).toFixed(1)}`),F=[...h].reverse().map(P=>`${D(P.ts).toFixed(1)},${M(P.min).toFixed(1)}`);ke=`M${k[0]} `+k.slice(1).map(P=>`L${P}`).join(" ")+` L${F[0]} `+F.slice(1).map(P=>`L${P}`).join(" ")+" Z"}}let Ne=_[_.length-1],xt=D(Ne.ts),$t=M(Ne.val),qe=h=>Math.abs(h)>=1e4?(h/1e3).toFixed(0)+"k":h>=1e3?(h/1e3).toFixed(1)+"k":h.toFixed(h<10?1:0),wt=qe(y),kt=qe(E),Et=s.history.filter(h=>["completed","skipped","reset"].includes(h.type)).map(h=>({ts:new Date(h.timestamp).getTime(),type:h.type})).filter(h=>h.ts>=B&&h.ts<=z),Ee=_;if(_.length>He){let h=(_.length-1)/(He-1);Ee=[];for(let k=0;k<He;k++)Ee.push(_[Math.round(k*h)])}return l`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${v} ${b}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_sparkline",e.lang)}">
        ...
        ${Et.map(h=>{let k=D(h.ts),F=h.type==="completed"?"var(--success-color, #4caf50)":h.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return I`
          ...
          `})}
        ${Ee.map(h=>{let k=D(h.ts),F=M(h.val),P=new Date(h.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),K=`${h.val.toFixed(1)} ${a}`;return m&&h.min!=null&&h.max!=null&&(K+=` (${h.min.toFixed(1)}\u2013${h.max.toFixed(1)})`),I`<circle cx="${k.toFixed(1)}" cy="${F.toFixed(1)}" r="8" fill="transparent" tabindex="0"
            @mouseenter=${Z=>pt(Z,`${P}
${K}`,e.setTooltip)}
            @focus=${Z=>pt(Z,`${P}
${K}`,e.setTooltip)}
...
`).map(h=>l`<div>${h}</div>`)}
  ...
  `}function oi(s,a,e,t,i){let f=Math.max(e,t,a,1)*1.3,w=50,T=[];for(let z=0;z<=w;z++){let x=z/w*f,D=1-Math.exp(-Math.pow(x/a,s)),M=32+x/f*260,we=136-D*128;T.push([M,we])}let H=T.map(([z,x])=>`${z.toFixed(1)},${x.toFixed(1)}`).join(" "),C="M32,136 "+T.map(([z,x])=>`L${z.toFixed(1)},${x.toFixed(1)}`).join(" ")+` L${T[w][0].toFixed(1)},136 Z`,E=32+e/f*260,y=1-Math.exp(-Math.pow(e/a,s)),L=136-y*128,q=((1-y)*100).toFixed(0),W=32+t/f*260,B=[0,.25,.5,.75,1];return l`
  ...
  `}function vt(s,a){return di(s,a)}function di(s,a){let e=s.seasonal_factors??s.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return c;let t=s.interval_analysis?.seasonal_reason,i=new Date().getMonth(),r=300,o=100,d=8,_=o-d-4,m=Math.max(...e,1.5),v=r/12,b=v*.65,f=d+_-1/m*_;return l`
    <div class="seasonal-chart">
      ...
      <div class="seasonal-labels">
        ${ht.map((w,T)=>l`<span class="seasonal-label ${T===i?"active-month":""}">${n(w,a)}</span>`)}
      </div>
    </div>
