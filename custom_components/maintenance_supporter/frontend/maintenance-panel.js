...
`;var we=class{constructor(a){this._cache=new Map;this._pending=new Map;this._hass=a}updateHass(a){this._hass=a}async getDetailStats(a,e){return this._getStats(a,"hour",30,e)}async getMiniStats(a,e){return this._getStats(a,"day",14,e)}async getBatchMiniStats(a){let e=new Map,t=[];for(let u of a){let _=`${u.entityId}:day`,m=this._cache.get(_);m&&Date.now()-m.fetchedAt<3e5?e.set(u.entityId,m.points):t.push(u)}if(t.length===0)return e;let i=t.filter(u=>u.isCounter).map(u=>u.entityId),s=t.filter(u=>!u.isCounter).map(u=>u.entityId),o=new Date(Date.now()-336*60*60*1e3).toISOString(),c=[];return i.length>0&&c.push(this._fetchBatch(i,"day",o,["state","sum","change"],!0,e)),s.length>0&&c.push(this._fetchBatch(s,"day",o,["mean","min","max"],!1,e)),await Promise.all(c),e}clearCache(){this._cache.clear(),this._pending.clear()}async _getStats(a,e,t,i){let s=`${a}:${e}`,o=this._cache.get(s);if(o&&Date.now()-o.fetchedAt<3e5)return o.points;if(this._pending.has(s))return this._pending.get(s);let c=this._fetchAndNormalize(a,e,t,i,s);this._pending.set(s,c);try{return await c}finally{this._pending.delete(s)}}async _fetchAndNormalize(a,e,t,i,s){let o=new Date(Date.now()-t*24*60*60*1e3).toISOString(),c=i?["state","sum","change"]:["mean","min","max"];try{let _=(await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:o,statistic_ids:[a],period:e,types:c}))[a]||[],m=this._normalizeRows(_,i);return this._cache.set(s,{entityId:a,fetchedAt:Date.now(),period:e,points:m}),m}catch(u){return console.warn(`[maintenance-supporter] Failed to fetch statistics for ${a}:`,u),[]}}async _fetchBatch(a,e,t,i,s,o){try{let c=await this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:t,statistic_ids:a,period:e,types:i});for(let u of a){let _=c[u]||[],m=this._normalizeRows(_,s);o.set(u,m),this._cache.set(`${u}:${e}`,{entityId:u,fetchedAt:Date.now(),period:e,points:m})}}catch(c){console.warn("[maintenance-supporter] Batch statistics fetch failed:",c)}}_normalizeRows(a,e){let t=[];for(let i of a){let s=null;if(e?s=i.state??null:s=i.mean??null,s===null)continue;let o={ts:i.start,val:s};e||(i.min!=null&&(o.min=i.min),i.max!=null&&(o.max=i.max)),t.push(o)}return t.sort((i,s)=>i.ts-s.ts),t}};var re=class{constructor(a){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=a}updateHass(a){this.hass=a}async getUsers(a=!1){let e=Date.now();if(!a&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(a,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:a,task_id:e,user_id:t})}async getTasksByUser(a){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:a})).tasks}getUserName(a){return!a||!this.usersCache?null:this.usersCache.find(t=>t.id===a)?.name||null}getUser(a){return!a||!this.usersCache?null:this.usersCache.find(e=>e.id===a)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(a){return a?a===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var H=class extends L{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._name="";this._manufacturer="";this._model="";this._serialNumber="";this._areaId="";this._installationDate="";this._entryId=null}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}openCreate(){this._entryId=null,this._name="",this._manufacturer="",this._model="",this._serialNumber="",this._areaId="",this._installationDate="",this._error="",this._open=!0}openEdit(e,t){this._entryId=e,this._name=t.name||"",this._manufacturer=t.manufacturer||"",this._model=t.model||"",this._serialNumber=t.serial_number||"",this._areaId=t.area_id||"",this._installationDate=t.installation_date||"",this._error="",this._open=!0}async _save(){if(this._name.trim()){this._loading=!0,this._error="";try{this._entryId?await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/update",entry_id:this._entryId,name:this._name,manufacturer:this._manufacturer||null,model:this._model||null,serial_number:this._serialNumber||null,area_id:this._areaId||null,installation_date:this._installationDate||null}):await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/create",name:this._name,manufacturer:this._manufacturer||null,model:this._model||null,serial_number:this._serialNumber||null,area_id:this._areaId||null,installation_date:this._installationDate||null}),this._open=!1,this.dispatchEvent(new CustomEvent("object-saved"))}catch{this._error=n("save_error",this._lang)}finally{this._loading=!1}}}_close(){this._open=!1}render(){if(!this._open)return l``;let e=this._lang,t=this._entryId?n("edit_object",e):n("new_object",e);return l`
  ...
  `,p([S({attribute:!1})],H.prototype,"hass",2),p([g()],H.prototype,"_open",2),p([g()],H.prototype,"_loading",2),p([g()],H.prototype,"_error",2),p([g()],H.prototype,"_name",2),p([g()],H.prototype,"_manufacturer",2),p([g()],H.prototype,"_model",2),p([g()],H.prototype,"_serialNumber",2),p([g()],H.prototype,"_areaId",2),p([g()],H.prototype,"_installationDate",2),p([g()],H.prototype,"_entryId",2);customElements.get("maintenance-object-dialog")||customElements.define("maintenance-object-dialog",H);var si=["cleaning","inspection","replacement","calibration","service","custom"],ri=["time_based","sensor_based","manual"],oi=["threshold","counter","state_change","runtime"],y=class extends L{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._entryId="";this._taskId=null;this._name="";this._type="custom";this._scheduleType="time_based";this._intervalDays="30";this._warningDays="7";this._intervalAnchor="completion";this._notes="";this._documentationUrl="";this._customIcon="";this._enabled=!0;this._triggerEntityId="";this._triggerEntityIds=[];this._triggerEntityLogic="any";this._triggerAttribute="";this._triggerType="threshold";this._triggerAbove="";this._triggerBelow="";this._triggerForMinutes="0";this._triggerTargetValue="";this._triggerDeltaMode=!1;this._triggerFromState="";this._triggerToState="";this._triggerTargetChanges="";this._triggerRuntimeHours="";this._suggestedAttributes=[];this._availableAttributes=[];this._entityDomain="";this._lastPerformed="";this._nfcTagId="";this._availableTags=[];this._responsibleUserId=null;this._availableUsers=[];this._environmentalEntity="";this._environmentalAttribute="";this._environmentalInitial="";this._userService=null}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}async openCreate(e){this._entryId=e,this._taskId=null,this._error="",this._resetFields(),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}async openEdit(e,t){this._entryId=e,this._taskId=t.id,this._error="",this._name=t.name,this._type=t.type,this._scheduleType=t.schedule_type,this._intervalDays=t.interval_days?.toString()||"30",this._warningDays=t.warning_days.toString(),this._intervalAnchor=t.interval_anchor||"completion",this._notes=t.notes||"",this._documentationUrl=t.documentation_url||"",this._customIcon=t.custom_icon||"",this._enabled=t.enabled!==!1,this._lastPerformed=t.last_performed||"",this._nfcTagId=t.nfc_tag_id||"",this._responsibleUserId=t.responsible_user_id||null;let i=t.adaptive_config||{};if(this._environmentalEntity=i.environmental_entity||"",this._environmentalAttribute=i.environmental_attribute||"",this._environmentalInitial=this._environmentalEntity,t.trigger_config){let s=t.trigger_config;this._triggerEntityId=s.entity_id||"",this._triggerEntityIds=s.entity_ids||(s.entity_id?[s.entity_id]:[]),this._triggerEntityLogic=s.entity_logic||"any",this._triggerAttribute=s.attribute||"",this._triggerType=s.type||"threshold",this._triggerAbove=s.trigger_above?.toString()||"",this._triggerBelow=s.trigger_below?.toString()||"",this._triggerForMinutes=s.trigger_for_minutes?.toString()||"0",this._triggerTargetValue=s.trigger_target_value?.toString()||"",this._triggerDeltaMode=s.trigger_delta_mode||!1,this._triggerFromState=s.trigger_from_state||"",this._triggerToState=s.trigger_to_state||"",this._triggerTargetChanges=s.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=s.trigger_runtime_hours?.toString()||""}else this._resetTriggerFields();this._triggerEntityId&&this._fetchEntityAttributes(this._triggerEntityId),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._warningDays="7",this._intervalAnchor="completion",this._notes="",this._documentationUrl="",this._customIcon="",this._enabled=!0,this._lastPerformed="",this._nfcTagId="",this._responsibleUserId=null,this._environmentalEntity="",this._environmentalAttribute="",this._environmentalInitial="",this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerForMinutes="0",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours=""}async _loadUsers(){this._userService||(this._userService=new re(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}async _loadTags(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tags/list"});this._availableTags=e.tags||[]}catch{this._availableTags=[]}}async _fetchEntityAttributes(e){if(!e||!this.hass){this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="";return}try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/entity/attributes",entity_id:e});this._entityDomain=t.domain||"",this._suggestedAttributes=t.suggested_attributes||[],this._availableAttributes=t.available_attributes||[]}catch{this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain=""}}async _save(){if(this._name.trim()){this._loading=!0,this._error="";try{let e={type:this._taskId?"maintenance_supporter/task/update":"maintenance_supporter/task/create",entry_id:this._entryId,name:this._name,task_type:this._type,schedule_type:this._scheduleType,warning_days:parseInt(this._warningDays,10)||7};if(this._taskId&&(e.task_id=this._taskId),this._scheduleType!=="manual"?this._intervalDays?(e.interval_days=parseInt(this._intervalDays,10),e.interval_anchor=this._intervalAnchor):this._taskId&&(e.interval_days=null,e.interval_anchor="completion"):this._taskId&&(e.interval_days=null,e.interval_anchor="completion"),e.notes=this._notes||null,e.documentation_url=this._documentationUrl||null,e.custom_icon=this._customIcon||null,e.enabled=this._enabled,e.last_performed=this._lastPerformed||null,e.nfc_tag_id=this._nfcTagId||null,e.responsible_user_id=this._responsibleUserId,this._scheduleType==="sensor_based"&&this._triggerEntityId){let s=this._triggerEntityIds.length>0?this._triggerEntityIds:[this._triggerEntityId],o={entity_id:s[0],entity_ids:s,type:this._triggerType};if(this._triggerAttribute&&(o.attribute=this._triggerAttribute),s.length>1&&(o.entity_logic=this._triggerEntityLogic),this._triggerType==="threshold"){if(this._triggerAbove){let c=parseFloat(this._triggerAbove);isNaN(c)||(o.trigger_above=c)}if(this._triggerBelow){let c=parseFloat(this._triggerBelow);isNaN(c)||(o.trigger_below=c)}if(this._triggerForMinutes){let c=parseInt(this._triggerForMinutes,10);isNaN(c)||(o.trigger_for_minutes=c)}}else if(this._triggerType==="counter"){if(this._triggerTargetValue){let c=parseFloat(this._triggerTargetValue);isNaN(c)||(o.trigger_target_value=c)}o.trigger_delta_mode=this._triggerDeltaMode}else if(this._triggerType==="state_change"){if(this._triggerFromState&&(o.trigger_from_state=this._triggerFromState),this._triggerToState&&(o.trigger_to_state=this._triggerToState),this._triggerTargetChanges){let c=parseInt(this._triggerTargetChanges,10);isNaN(c)||(o.trigger_target_changes=c)}}else if(this._triggerType==="runtime"&&this._triggerRuntimeHours){let c=parseFloat(this._triggerRuntimeHours);isNaN(c)||(o.trigger_runtime_hours=c)}e.trigger_config=o}else this._taskId&&(e.trigger_config=null);let t=await this.hass.connection.sendMessagePromise(e),i=this._taskId||t?.task_id;if(i&&this._scheduleType==="sensor_based"&&this._environmentalEntity!==this._environmentalInitial)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:i,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null})}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch{this._error=n("save_error",this._lang)}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return d;let e=this._lang;return l`
      ...
      <div class="select-row">
        ...
        >
          ${oi.map(t=>l`<option value=${t} ?selected=${t===this._triggerType}>${n(t,e)}</option>`)}
        </select>
      </div>
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${si.map(i=>l`<option value=${i} ?selected=${i===this._type}>${n(i,e)}</option>`)}
            </select>
          </div>
          <div class="select-row">
            ...
            >
              ${ri.map(i=>l`<option value=${i} ?selected=${i===this._scheduleType}>${n(i,e)}</option>`)}
            </select>
          </div>
  ...
  `,p([S({attribute:!1})],y.prototype,"hass",2),p([g()],y.prototype,"_open",2),p([g()],y.prototype,"_loading",2),p([g()],y.prototype,"_error",2),p([g()],y.prototype,"_entryId",2),p([g()],y.prototype,"_taskId",2),p([g()],y.prototype,"_name",2),p([g()],y.prototype,"_type",2),p([g()],y.prototype,"_scheduleType",2),p([g()],y.prototype,"_intervalDays",2),p([g()],y.prototype,"_warningDays",2),p([g()],y.prototype,"_intervalAnchor",2),p([g()],y.prototype,"_notes",2),p([g()],y.prototype,"_documentationUrl",2),p([g()],y.prototype,"_customIcon",2),p([g()],y.prototype,"_enabled",2),p([g()],y.prototype,"_triggerEntityId",2),p([g()],y.prototype,"_triggerEntityIds",2),p([g()],y.prototype,"_triggerEntityLogic",2),p([g()],y.prototype,"_triggerAttribute",2),p([g()],y.prototype,"_triggerType",2),p([g()],y.prototype,"_triggerAbove",2),p([g()],y.prototype,"_triggerBelow",2),p([g()],y.prototype,"_triggerForMinutes",2),p([g()],y.prototype,"_triggerTargetValue",2),p([g()],y.prototype,"_triggerDeltaMode",2),p([g()],y.prototype,"_triggerFromState",2),p([g()],y.prototype,"_triggerToState",2),p([g()],y.prototype,"_triggerTargetChanges",2),p([g()],y.prototype,"_triggerRuntimeHours",2),p([g()],y.prototype,"_suggestedAttributes",2),p([g()],y.prototype,"_availableAttributes",2),p([g()],y.prototype,"_entityDomain",2),p([g()],y.prototype,"_lastPerformed",2),p([g()],y.prototype,"_nfcTagId",2),p([g()],y.prototype,"_availableTags",2),p([g()],y.prototype,"_responsibleUserId",2),p([g()],y.prototype,"_availableUsers",2),p([g()],y.prototype,"_environmentalEntity",2),p([g()],y.prototype,"_environmentalAttribute",2);customElements.get("maintenance-task-dialog")||customElements.define("maintenance-task-dialog",y);var N=class extends L{constructor(){super(...arguments);this.entryId="";this.taskId="";this.taskName="";this.lang="en";this.checklist=[];this.adaptiveEnabled=!1;this._open=!1;this._notes="";this._cost="";this._duration="";this._loading=!1;this._error="";this._checklistState={};this._feedback="needed"}open(){this._open||(this._open=!0,this._notes="",this._cost="",this._duration="",this._error="",this._checklistState={},this._feedback="needed")}_toggleCheck(e){let t=String(e);this._checklistState={...this._checklistState,[t]:!this._checklistState[t]}}_setFeedback(e){this._feedback=e}async _complete(){this._loading=!0,this._error="";try{let e={type:"maintenance_supporter/task/complete",entry_id:this.entryId,task_id:this.taskId};if(this._notes&&(e.notes=this._notes),this._cost){let t=parseFloat(this._cost);!isNaN(t)&&t>=0&&(e.cost=t)}if(this._duration){let t=parseInt(this._duration,10);!isNaN(t)&&t>=0&&(e.duration=t)}this.checklist.length>0&&(e.checklist_state=this._checklistState),this.adaptiveEnabled&&(e.feedback=this._feedback),await this.hass.connection.sendMessagePromise(e),this._open=!1,this.dispatchEvent(new CustomEvent("task-completed"))}catch{this._error=n("save_error",this.lang)}finally{this._loading=!1}}_close(){this._open=!1}render(){if(!this._open)return l``;let e=this.lang||this.hass?.language||"en";return l`
      <ha-dialog open @closed=${this._close}>
...
<script>setTimeout(()=>window.print(),300)<\/script>
      ...
      <div class="settings-section">
        ...
        <label class="setting-row">
          ...
          <select @change=${i=>this._updateSetting("budget_currency",i.target.value)}>
            ${di.map(i=>l`<option value=${i} ?selected=${t.currency===i}>${i}</option>`)}
          </select>
        </label>
        <label class="setting-row">
  ...
  `,p([S({attribute:!1})],U.prototype,"hass",2),p([S({attribute:!1})],U.prototype,"features",2),p([S({attribute:!1})],U.prototype,"budget",2),p([g()],U.prototype,"_settings",2),p([g()],U.prototype,"_loading",2),p([g()],U.prototype,"_importCsv",2),p([g()],U.prototype,"_importLoading",2),p([g()],U.prototype,"_includeHistory",2),p([g()],U.prototype,"_toast",2),p([g()],U.prototype,"_testingNotification",2);customElements.define("maintenance-settings-view",U);var ci=300,_i=140,He=27;function ht(r,a){let e=r.trigger_config;if(!e)return d;let t=a.lang,i=r.trigger_entity_info,s=r.trigger_entity_infos,o=i?.friendly_name||e.entity_id||"\u2014",c=e.entity_id||"",u=e.entity_ids||(c?[c]:[]),_=i?.unit_of_measurement||"",m=r.trigger_current_value,v=e.type||"threshold",b=u.length>1;return l`
    <h3>${n("trigger",t)}</h3>
      ...
      <div class="trigger-header">
        ...
        <div>
          ${b?l`
            ...
            <div class="trigger-entity-id">${u.map((f,k)=>l`${k>0?", ":""}<span class="entity-link" @click=${T=>ae(T,f)}>${f}</span>`)}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `:l`
            <div class="trigger-entity-name">${o}</div>
            <div class="trigger-entity-id">${c?l`<span class="entity-link" @click=${f=>ae(f,c)}>${c}</span>`:""}${e.attribute?` \u2192 ${e.attribute}`:""}</div>
          `}
        </div>
      ...
      <div class="trigger-limits">
        ...
        ${v==="compound"?l`
          ...
          ${(e.conditions||[]).map((f,k)=>l`
            <span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${k+1}. ${n(f.type||"unknown",t)}: ${f.entity_id?l`<span class="entity-link" @click=${T=>ae(T,f.entity_id)}>${f.entity_id}</span>`:""}</span>
          `)}
        `:d}
        ${i?.min!=null?l`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${n("min",t)}: ${i.min} ${_}</span>`:d}
        ${i?.max!=null?l`<span class="trigger-limit-item"><span class="dot range" aria-hidden="true"></span> ${n("max",t)}: ${i.max} ${_}</span>`:d}
      </div>

      ${s&&s.length>1?l`
        <div class="trigger-entity-list">
          ${s.map(f=>l`
            <span class="trigger-entity-id">${f.friendly_name} (<span class="entity-link" @click=${k=>ae(k,f.entity_id)}>${f.entity_id}</span>)</span>
          `)}
        </div>
      `:d}
  ...
  `}function ui(r,a,e){let t=r.trigger_config;if(!t)return d;let i=t.type||"threshold",s=i==="counter"&&t.trigger_delta_mode,o=e.isCounterEntity(t),c=t.entity_id||"",u=e.detailStatsData.get(c)||[],_=[],m=!1;if(u.length>=2)for(let h of u){let w=h.val;s&&r.trigger_baseline_value!=null&&(w-=r.trigger_baseline_value);let F={ts:h.ts,val:w};!o&&h.min!=null&&h.max!=null&&(F.min=s&&r.trigger_baseline_value!=null?h.min-r.trigger_baseline_value:h.min,F.max=s&&r.trigger_baseline_value!=null?h.max-r.trigger_baseline_value:h.max,m=!0),_.push(F)}else for(let h of r.history)h.trigger_value!=null&&_.push({ts:new Date(h.timestamp).getTime(),val:h.trigger_value});if(r.trigger_current_value!=null){let h=r.trigger_current_value;s&&r.trigger_baseline_value!=null&&(h-=r.trigger_baseline_value),_.push({ts:Date.now(),val:h})}if(_.length<2&&c&&e.hasStatsService&&!e.detailStatsData.has(c))return l`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
      <ha-icon icon="mdi:chart-line" style="--mdc-icon-size:16px;margin-right:8px;"></ha-icon>
      ${n("loading_chart",e.lang)}
    </div>`;if(_.length<2)return d;_.sort((h,w)=>h.ts-w.ts);let v=ci,b=_i,f=30,k=2,T=8,q=16,C=_.map(h=>h.val),A=Math.min(...C),x=Math.max(...C);if(m)for(let h of _)h.min!=null&&(A=Math.min(A,h.min)),h.max!=null&&(x=Math.max(x,h.max));t.trigger_above!=null&&(x=Math.max(x,t.trigger_above),A=Math.min(A,t.trigger_above)),t.trigger_below!=null&&(A=Math.min(A,t.trigger_below),x=Math.max(x,t.trigger_below));let z=null,O=null;if(i==="counter"&&t.trigger_target_value!=null){if(r.trigger_baseline_value!=null)z=r.trigger_baseline_value;else if(_.length>0){let h=[...r.history].filter(w=>w.type==="completed"||w.type==="reset").sort((w,F)=>new Date(F.timestamp).getTime()-new Date(w.timestamp).getTime())[0];if(h){let w=new Date(h.timestamp).getTime(),F=_[0],R=Math.abs(_[0].ts-w);for(let J of _){let Z=Math.abs(J.ts-w);Z<R&&(F=J,R=Z)}z=F.val}else z=_[0].val}z!=null?(O=z+t.trigger_target_value,x=Math.max(x,O),A=Math.min(A,z)):(x=Math.max(x,t.trigger_target_value),A=Math.min(A,0))}s&&r.trigger_baseline_value!=null&&(A=Math.min(A,0));let G=x-A||1;A-=G*.1,x+=G*.1;let W=_[0].ts,D=_[_.length-1].ts,$=D-W||1,I=h=>f+(h-W)/$*(v-f-k),M=h=>T+(1-(h-A)/(x-A))*(b-T-q),Ee=_.map(h=>`${I(h.ts).toFixed(1)},${M(h.val).toFixed(1)}`).join(" "),kt=`M${I(_[0].ts).toFixed(1)},${b-q} `+_.map(h=>`L${I(h.ts).toFixed(1)},${M(h.val).toFixed(1)}`).join(" ")+` L${I(_[_.length-1].ts).toFixed(1)},${b-q} Z`,Ae="";if(m){let h=_.filter(w=>w.min!=null&&w.max!=null);if(h.length>=2){let w=h.map(R=>`${I(R.ts).toFixed(1)},${M(R.max).toFixed(1)}`),F=[...h].reverse().map(R=>`${I(R.ts).toFixed(1)},${M(R.min).toFixed(1)}`);Ae=`M${w[0]} `+w.slice(1).map(R=>`L${R}`).join(" ")+` L${F[0]} `+F.slice(1).map(R=>`L${R}`).join(" ")+" Z"}}let Oe=_[_.length-1],wt=I(Oe.ts),Et=M(Oe.val),Ue=h=>Math.abs(h)>=1e4?(h/1e3).toFixed(0)+"k":h>=1e3?(h/1e3).toFixed(1)+"k":h.toFixed(h<10?1:0),At=Ue(x),St=Ue(A),Tt=r.history.filter(h=>["completed","skipped","reset"].includes(h.type)).map(h=>({ts:new Date(h.timestamp).getTime(),type:h.type})).filter(h=>h.ts>=W&&h.ts<=D),Se=_;if(_.length>He){let h=(_.length-1)/(He-1);Se=[];for(let w=0;w<He;w++)Se.push(_[Math.round(w*h)])}return l`
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${v} ${b}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_sparkline",e.lang)}">
        ...
        ${r.degradation_rate!=null&&r.degradation_trend!=="stable"&&r.degradation_trend!=="insufficient_data"&&_.length>=2?(()=>{let h=_[_.length-1],w=30,F=h.ts+w*864e5,R=h.val+r.degradation_rate*w,J=Math.min(F,D+(D-W)*.3),Z=Math.max(A,Math.min(x,R)),Mt=I(h.ts),Ct=M(h.val),zt=I(J),It=M(Z);return j`<line x1="${Mt.toFixed(1)}" y1="${Ct.toFixed(1)}" x2="${zt.toFixed(1)}" y2="${It.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():d}
        ${i==="threshold"&&t.trigger_above!=null?j`<line x1="${f}" y1="${M(t.trigger_above).toFixed(1)}" x2="${v}" y2="${M(t.trigger_above).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${v-2}" y="${M(t.trigger_above)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25B2 ${t.trigger_above}</text>`:d}
        ${i==="threshold"&&t.trigger_below!=null?j`<line x1="${f}" y1="${M(t.trigger_below).toFixed(1)}" x2="${v}" y2="${M(t.trigger_below).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${v-2}" y="${M(t.trigger_below)+11}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25BC ${t.trigger_below}</text>`:d}
        ${i==="counter"&&O!=null?j`<line x1="${f}" y1="${M(O).toFixed(1)}" x2="${v}" y2="${M(O).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
        ...
        ${i==="counter"&&z!=null?j`<line x1="${f}" y1="${M(z).toFixed(1)}" x2="${v}" y2="${M(z).toFixed(1)}" stroke="var(--secondary-text-color)" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
                <text x="${f+2}" y="${M(z)+11}" text-anchor="start" fill="var(--secondary-text-color)" font-size="8">${n("baseline",e.lang)}</text>`:d}
        <circle cx="${wt.toFixed(1)}" cy="${Et.toFixed(1)}" r="3.5" fill="var(--primary-color)" />
        ${Tt.map(h=>{let w=I(h.ts),F=h.type==="completed"?"var(--success-color, #4caf50)":h.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return j`
            <line x1="${w.toFixed(1)}" y1="${T}" x2="${w.toFixed(1)}" y2="${b-q}" stroke="${F}" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
            <circle cx="${w.toFixed(1)}" cy="${T+2}" r="5" fill="${F}" opacity="0.8" />
            <text x="${w.toFixed(1)}" y="${T+6}" text-anchor="middle" fill="white" font-size="7" font-weight="bold">${h.type==="completed"?"\u2713":h.type==="skipped"?"\u23ED":"\u21BA"}</text>
          `})}
        ${Se.map(h=>{let w=I(h.ts),F=M(h.val),R=new Date(h.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),J=`${h.val.toFixed(1)} ${a}`;return m&&h.min!=null&&h.max!=null&&(J+=` (${h.min.toFixed(1)}\u2013${h.max.toFixed(1)})`),j`<circle cx="${w.toFixed(1)}" cy="${F.toFixed(1)}" r="8" fill="transparent" tabindex="0"
            @mouseenter=${Z=>gt(Z,`${R}
${J}`,e.setTooltip)}
            @focus=${Z=>gt(Z,`${R}
${J}`,e.setTooltip)}
...
`).map(h=>l`<div>${h}</div>`)}
  ...
  `}function gt(r,a,e){let t=r.currentTarget,i=t.closest(".sparkline-container");if(!i)return;let s=i.getBoundingClientRect(),o=t.getBoundingClientRect();e({x:o.left-s.left+o.width/2,y:o.top-s.top-8,text:a})}function mt(r,a,e){let t=r.degradation_trend!=null&&r.degradation_trend!=="insufficient_data",i=r.days_until_threshold!=null,s=r.environmental_factor!=null&&r.environmental_factor!==1;if(!t&&!i&&!s)return d;let o=r.degradation_trend==="rising"?"M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z":r.degradation_trend==="falling"?"M16,18L18.29,15.71L13.41,10.83L9.41,14.83L2,7.41L3.41,6L9.41,12L13.41,8L19.71,14.29L22,12V18H16Z":"M22,12L18,8V11H3V13H18V16L22,12Z";return l`
    <div class="prediction-section">
      ${r.sensor_prediction_urgency?l`
        <div class="prediction-urgency-banner">
          <ha-svg-icon path="M1,21H23L12,2L1,21M12,18A1,1 0 0,1 11,17A1,1 0 0,1 12,16A1,1 0 0,1 13,17A1,1 0 0,1 12,18M13,15H11V10H13V15Z"></ha-svg-icon>
          ${n("sensor_prediction_urgency",a).replace("{days}",String(Math.round(r.days_until_threshold||0)))}
        </div>
      `:d}
      ...
      <div class="prediction-grid">
        ${t?l`
          <div class="prediction-item">
            ...
            <span class="prediction-value ${r.degradation_trend}">${n("trend_"+r.degradation_trend,a)}</span>
            ${r.degradation_rate!=null?l`<span class="prediction-rate">${r.degradation_rate>0?"+":""}${Math.abs(r.degradation_rate)>=10?Math.round(r.degradation_rate).toLocaleString():r.degradation_rate.toFixed(1)} ${r.trigger_entity_info?.unit_of_measurement||""}/${n("day_short",a)}</span>`:d}
          </div>
        `:d}
        ${i?l`
          <div class="prediction-item">
            ...
            <span class="prediction-value prediction-days${r.days_until_threshold===0?" exceeded":r.sensor_prediction_urgency?" urgent":""}">${r.days_until_threshold===0?n("threshold_exceeded",a):"~"+Math.round(r.days_until_threshold)+" "+n("days",a)}</span>
            ${r.threshold_prediction_date?l`<span class="prediction-date">${Y(r.threshold_prediction_date,a)}</span>`:d}
        ...
        ${s&&e.environmental?l`
          <div class="prediction-item">
            ...
            ${r.environmental_entity?l`<span class="prediction-entity entity-link" @click=${c=>ae(c,r.environmental_entity)}>${r.environmental_entity}</span>`:d}
          </div>
        `:d}
      </div>
    </div>
  `}function vt(r,a){let e=r.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return d;let s=r.interval_days??0,o=r.suggested_interval??s;return l`
  ...
  `}function gi(r,a,e,t,i){let f=Math.max(e,t,a,1)*1.3,k=50,T=[];for(let D=0;D<=k;D++){let $=D/k*f,I=1-Math.exp(-Math.pow($/a,r)),M=32+$/f*260,Ee=136-I*128;T.push([M,Ee])}let q=T.map(([D,$])=>`${D.toFixed(1)},${$.toFixed(1)}`).join(" "),C="M32,136 "+T.map(([D,$])=>`L${D.toFixed(1)},${$.toFixed(1)}`).join(" ")+` L${T[k][0].toFixed(1)},136 Z`,A=32+e/f*260,x=1-Math.exp(-Math.pow(e/a,r)),z=136-x*128,O=((1-x)*100).toFixed(0),G=32+t/f*260,W=[0,.25,.5,.75,1];return l`
    ...
    <div class="chart-legend">
      ...
      ${e>0?l`<span class="legend-item"><span class="legend-swatch" style="background:var(--primary-color, #03a9f4); opacity:0.5"></span> ${n("current_interval_marker",i)}</span>`:d}
      ${t>0&&t!==e?l`<span class="legend-item"><span class="legend-swatch" style="background:var(--success-color, #4caf50)"></span> ${n("recommended_marker",i)}</span>`:d}
    </div>
  `}function hi(r,a){return l`
    <div class="weibull-info-row">
      <div class="weibull-info-item">
        <span>${n("characteristic_life",a)}</span>
        <span class="weibull-info-value">${Math.round(r.weibull_eta)} ${n("days",a)}</span>
      </div>
  ...
  `}function mi(r,a,e){let t=r.confidence_interval_low,i=r.confidence_interval_high,s=a.suggested_interval??a.interval_days??0,o=a.interval_days??0,c=Math.max(0,t-5),_=i+5-c,m=(t-c)/_*100,v=(i-t)/_*100,b=(s-c)/_*100,f=o>0?(o-c)/_*100:-1;return l`
    <div class="confidence-range">
      <div class="confidence-range-title">
        ${n("confidence_interval",e)}: ${s} ${n("days",e)} (${t}\u2013${i})
      </div>
  ...
  `}function yt(r,a){return vi(r,a)}function vi(r,a){let e=r.seasonal_factors??r.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return d;let t=r.interval_analysis?.seasonal_reason,i=new Date().getMonth(),s=300,o=100,c=8,_=o-c-4,m=Math.max(...e,1.5),v=s/12,b=v*.65,f=c+_-1/m*_;return l`
    <div class="seasonal-chart">
      <div class="seasonal-chart-title">
        ...
        ${t?l`<span class="source-tag">${t==="learned"?n("seasonal_learned",a):n("seasonal_manual",a)}</span>`:d}
      </div>
      <svg viewBox="0 0 ${s} ${o}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_seasonal",a)}">
        <line x1="0" y1="${f.toFixed(1)}" x2="${s}" y2="${f.toFixed(1)}"
          stroke="var(--divider-color)" stroke-width="1" stroke-dasharray="4,3" />
        ${e.map((k,T)=>{let q=k/m*_,C=T*v+(v-b)/2,A=c+_-q,x=T===i,z=k<1?"var(--success-color, #4caf50)":k>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return j`
            <rect x="${C.toFixed(1)}" y="${A.toFixed(1)}"
              width="${b.toFixed(1)}" height="${q.toFixed(1)}"
              fill="${z}" opacity="${x?1:.5}" rx="2" />
          `})}
      </svg>
      <div class="seasonal-labels">
        ${ft.map((k,T)=>l`<span class="seasonal-label ${T===i?"active-month":""}">${n(k,a)}</span>`)}
      </div>
    </div>
  `}var fi=300,bi=200;function xt(r,a,e,t){let i=r.history.filter(c=>c.type==="completed"&&(c.cost!=null||c.duration!=null));if(i.length<2)return d;let s=i.some(c=>(c.cost??0)>0),o=i.some(c=>(c.duration??0)>0);return!s&&!o?d:l`
  ...
  `}function yi(r,a,e){let t=r.history.filter($=>$.type==="completed"&&($.cost!=null||$.duration!=null)).map($=>({ts:new Date($.timestamp).getTime(),cost:$.cost??0,duration:$.duration??0})).sort(($,I)=>$.ts-I.ts);if(t.length<2)return d;let i=t.some($=>$.cost>0),s=t.some($=>$.duration>0);if(!i&&!s)return d;let o=e!=="duration"&&i,c=e!=="cost"&&s,u=o||!c&&i,_=c||!o&&s,m=fi,v=bi,b=u?32:8,f=_?32:8,k=8,T=20,q=m-b-f,C=v-k-T,A=Math.max(...t.map($=>$.cost))||1,x=Math.max(...t.map($=>$.duration))||1,z=Math.min(20,q/t.length*.6),O=q/t.length,G=$=>b+O*$+O/2,W=$=>k+C-$/A*C,D=$=>k+C-$/x*C;return l`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${m} ${v}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_history",a)}">
        ${u?t.map(($,I)=>j`
          <rect x="${(G(I)-z/2).toFixed(1)}" y="${W($.cost).toFixed(1)}" width="${z.toFixed(1)}" height="${(k+C-W($.cost)).toFixed(1)}"
            fill="var(--primary-color)" opacity="0.6" rx="2" />
        `):d}
        ${_?j`
          <polyline points="${t.map(($,I)=>`${G(I).toFixed(1)},${D($.duration).toFixed(1)}`).join(" ")}"
            fill="none" stroke="var(--accent-color, #ff9800)" stroke-width="2" stroke-linejoin="round" />
          ${t.map(($,I)=>j`
            <circle cx="${G(I).toFixed(1)}" cy="${D($.duration).toFixed(1)}" r="3" fill="var(--accent-color, #ff9800)" />
          `)}
        `:d}
  ...
  `}var xi=60,$i=20,$t=30,E=class extends L{constructor(){super(...arguments);this.narrow=!1;this.panel={};this._objects=[];this._stats=null;this._view="overview";this._selectedEntryId=null;this._selectedTaskId=null;this._filterStatus="";this._filterUser=null;this._unsub=null;this._sparklineTooltip=null;this._historyFilter=null;this._budget=null;this._groups={};this._detailStatsData=new Map;this._miniStatsData=new Map;this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1};this._actionLoading=!1;this._moreMenuOpen=!1;this._toastMessage="";this._toastTimer=null;this._dismissedSuggestions=new Set;this._overviewTab="dashboard";this._activeTab="overview";this._costDurationToggle="both";this._historySearch="";this._sortMode="due_date";this._statsService=null;this._userService=null;this._dataLoaded=!1;this._lastConnection=null;this._popstateHandler=e=>this._onPopState(e);this._deepLinkHandled=!1;this._onDialogEvent=async()=>{try{await this._loadData()}catch{}}}get _lang(){return this.hass?.language||"en"}connectedCallback(){super.connectedCallback(),window.addEventListener("popstate",this._popstateHandler);let e=localStorage.getItem("maintenance_supporter_sort");e&&["due_date","object","type","task_name"].includes(e)&&(this._sortMode=e)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("popstate",this._popstateHandler),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null,this._deepLinkHandled=!1,this._statsService?.clearCache(),this._statsService=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,history.replaceState({msp_view:"overview",msp_entry:null,msp_task:null},""),this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}this._statsService?this._statsService.updateHass(this.hass):(this._statsService=new we(this.hass),this._fetchMiniStatsForOverview()),this._userService?this._userService.updateHass(this.hass):(this._userService=new re(this.hass),this._userService.getUsers())}}async _loadData(){let[e,t,i,s,o]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"}).catch(()=>null),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).catch(()=>null)]);e&&(this._objects=e.objects),t&&(this._stats=t),i&&(this._budget=i),s&&(this._groups=s.groups||{}),o&&(this._features=o.features),this._fetchMiniStatsForOverview(),this._handleDeepLink()}_handleDeepLink(){if(this._deepLinkHandled)return;let e=new URLSearchParams(window.location.search),t=e.get("entry_id");if(!t)return;this._deepLinkHandled=!0;let i=e.get("task_id"),s=e.get("action"),o=window.location.pathname+window.location.hash;history.replaceState(history.state,"",o);let c=this._getObject(t);if(!c){this._showOverview();return}if(i){let u=c.tasks.find(_=>_.id===i);if(!u){this._showObject(t);return}this._showTask(t,i),s==="complete"&&requestAnimationFrame(()=>{this._openCompleteDialog(t,i,u.name,this._features.checklists?u.checklist:void 0,this._features.adaptive&&!!u.adaptive_config?.enabled)})}else this._showObject(t)}_isCounterEntity(e){if(!e)return!1;let t=e.type||"threshold";return t==="counter"||t==="state_change"}async _fetchDetailStats(e,t){if(!this._statsService)return;let i=await this._statsService.getDetailStats(e,t),s=new Map(this._detailStatsData);s.set(e,i),this._detailStatsData=s}async _fetchMiniStatsForOverview(){if(!this._statsService)return;let e=[];for(let i of this._objects)for(let s of i.tasks){let o=s.trigger_config?.entity_id;o&&e.push({entityId:o,isCounter:this._isCounterEntity(s.trigger_config)})}if(e.length===0)return;let t=await this._statsService.getBatchMiniStats(e);this._miniStatsData=new Map([...this._miniStatsData,...t])}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _taskRows(){let e=[];for(let u of this._objects)for(let _ of u.tasks)if(!(this._filterStatus&&_.status!==this._filterStatus)){if(this._filterUser){let m=this._filterUser==="current_user"?this._userService?.getCurrentUserId():this._filterUser;if(_.responsible_user_id!==m)continue}e.push({entry_id:u.entry_id,task_id:_.id,object_name:u.object.name,task_name:_.name,type:_.type,schedule_type:_.schedule_type,status:_.status,days_until_due:_.days_until_due??null,next_due:_.next_due??null,trigger_active:_.trigger_active,trigger_current_value:_.trigger_current_value??null,trigger_current_delta:_.trigger_current_delta??null,trigger_config:_.trigger_config??null,trigger_entity_info:_.trigger_entity_info??null,times_performed:_.times_performed,total_cost:_.total_cost,interval_days:_.interval_days??null,interval_anchor:_.interval_anchor??null,history:_.history||[],enabled:_.enabled,nfc_tag_id:_.nfc_tag_id??null})}let t={overdue:0,triggered:1,due_soon:2,ok:3},i=(u,_)=>(t[u.status]??9)-(t[_.status]??9),s=(u,_)=>(u.days_until_due??99999)-(_.days_until_due??99999),o=(u,_)=>i(u,_)||s(u,_),c={due_date:o,object:(u,_)=>u.object_name.localeCompare(_.object_name)||o(u,_),type:(u,_)=>u.type.localeCompare(_.type)||o(u,_),task_name:(u,_)=>u.task_name.localeCompare(_.task_name)};return e.sort(c[this._sortMode]),e}_getObject(e){return this._objects.find(t=>t.entry_id===e)}_getTask(e,t){return this._getObject(e)?.tasks.find(s=>s.id===t)}_pushPanelState(e,t,i){let s={msp_view:e,msp_entry:t||null,msp_task:i||null};history.pushState(s,"")}_onPopState(e){let t=e.state;if(t?.msp_view&&(this._view=t.msp_view,this._selectedEntryId=t.msp_entry||null,this._selectedTaskId=t.msp_task||null,this._moreMenuOpen=!1,t.msp_view==="task"&&t.msp_entry&&t.msp_task)){this._historyFilter=null;let i=this._getTask(t.msp_entry,t.msp_task);i?.trigger_config?.entity_id&&this._fetchDetailStats(i.trigger_config.entity_id,this._isCounterEntity(i.trigger_config))}}_showOverview(){this._pushPanelState("overview"),this._view="overview",this._selectedEntryId=null,this._selectedTaskId=null,this._moreMenuOpen=!1}_showAllObjects(){this._pushPanelState("all_objects"),this._view="all_objects",this._selectedEntryId=null,this._selectedTaskId=null}_showObject(e){this._pushPanelState("object",e),this._view="object",this._selectedEntryId=e,this._selectedTaskId=null}_showTask(e,t){this._pushPanelState("task",e,t),this._view="task",this._selectedEntryId=e,this._selectedTaskId=t,this._activeTab="overview",this._historyFilter=null;let i=this._getTask(e,t);if(i?.trigger_config?.entity_id){let s=i.trigger_config.entity_id,o=this._isCounterEntity(i.trigger_config);this._fetchDetailStats(s,o)}}_showToast(e){this._toastTimer&&clearTimeout(this._toastTimer),this._toastMessage=e,this._toastTimer=setTimeout(()=>{this._toastMessage="",this._toastTimer=null},4e3)}async _deleteObject(e){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:n("delete",this._lang),message:n("confirm_delete_object",this._lang),confirmText:n("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:e}),this._showOverview(),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}async _deleteTask(e,t){if(await this.shadowRoot.querySelector("maintenance-confirm-dialog")?.confirm({title:n("delete",this._lang),message:n("confirm_delete_task",this._lang),confirmText:n("delete",this._lang),danger:!0}))try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/delete",entry_id:e,task_id:t}),this._showObject(e),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}async _skipTask(e,t,i){this._actionLoading=!0;try{let s={type:"maintenance_supporter/task/skip",entry_id:e,task_id:t};i&&(s.reason=i),await this.hass.connection.sendMessagePromise(s),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}finally{this._actionLoading=!1}}async _resetTask(e,t,i){this._actionLoading=!0;try{let s={type:"maintenance_supporter/task/reset",entry_id:e,task_id:t};i&&(s.date=i),await this.hass.connection.sendMessagePromise(s),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}finally{this._actionLoading=!1}}async _applySuggestion(e,t,i){try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/apply_suggestion",entry_id:e,task_id:t,interval:i}),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}async _reanalyzeInterval(e,t){try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:e,task_id:t});i.recommended_interval?this._showToast(`${n("reanalyze_result",this._lang)}: ${i.recommended_interval} ${n("days",this._lang)} (${n(`confidence_${i.confidence}`,this._lang)}, ${i.data_points} ${n("data_points",this._lang)})`):this._showToast(n("reanalyze_insufficient_data",this._lang)),await this._loadData()}catch{this._showToast(n("action_error",this._lang))}}async _promptSkipTask(e,t){let i=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!i)return;let s=await i.prompt({title:n("skip",this._lang),message:n("skip_reason_prompt",this._lang),confirmText:n("skip",this._lang),inputLabel:n("reason_optional",this._lang),inputType:"text"});s.confirmed&&this._skipTask(e,t,s.value||void 0)}async _promptResetTask(e,t){let i=this.shadowRoot.querySelector("maintenance-confirm-dialog");if(!i)return;let s=await i.prompt({title:n("reset",this._lang),message:n("reset_date_prompt",this._lang),confirmText:n("reset",this._lang),inputLabel:n("reset_date_optional",this._lang),inputType:"date"});s.confirmed&&this._resetTask(e,t,s.value||void 0)}_dismissSuggestion(e,t){e&&t&&this._dismissedSuggestions.add(`${e}_${t}`),this.requestUpdate()}_openCompleteDialog(e,t,i,s,o){let c=this.shadowRoot.querySelector("maintenance-complete-dialog");c&&(c.entryId=e,c.taskId=t,c.taskName=i,c.lang=this._lang,c.checklist=s||[],c.adaptiveEnabled=!!o,c.open())}_openQrForObject(e,t){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForObject(e,t)}_openQrForTask(e,t,i,s){this.shadowRoot.querySelector("maintenance-qr-dialog")?.openForTask(e,t,i,s)}render(){return l`
    ...
    `}_renderDashboard(){let e=this._stats,t=this._taskRows,i=this._lang;return l`
      ...
      <div class="filter-bar">
        ...
        >
          <option value="due_date" ?selected=${this._sortMode==="due_date"}>${n("sort_due_date",i)}</option>
          <option value="object" ?selected=${this._sortMode==="object"}>${n("sort_object",i)}</option>
          <option value="type" ?selected=${this._sortMode==="type"}>${n("sort_type",i)}</option>
          <option value="task_name" ?selected=${this._sortMode==="task_name"}>${n("sort_task_name",i)}</option>
        </select>
        <ha-button
    ...
    `}async _onSettingsChanged(){await this._loadData()}_renderGroupsSection(){let e=Object.entries(this._groups);if(e.length===0)return d;let t=this._lang;return l`
      <div class="groups-section">
        ...
        <div class="groups-grid">
          ${e.map(([i,s])=>{let o=s.task_refs.map(c=>this._getTask(c.entry_id,c.task_id)?.name).filter(Boolean);return l`
    ...
    `}_renderBudgetBar(){let e=this._budget;if(!e)return d;let t=this._lang,i=e.currency_symbol||"\u20AC",s=[];return e.monthly_budget>0&&s.push({label:n("budget_monthly",t),spent:e.monthly_spent,budget:e.monthly_budget}),e.yearly_budget>0&&s.push({label:n("budget_yearly",t),spent:e.yearly_spent,budget:e.yearly_budget}),s.length===0?d:l`
      <div class="budget-bars">
        ${s.map(o=>{let c=Math.min(100,Math.max(0,o.spent/o.budget*100)),u=c>=100?"var(--error-color, #f44336)":c>=e.alert_threshold_pct?"var(--warning-color, #ff9800)":"var(--success-color, #4caf50)";return l`
    ...
    `}_renderOverviewRow(e){let t=this._lang,i=e.schedule_type==="time_based"&&e.interval_days&&e.interval_days>0,s=0,o=$e.ok,c=!1;if(i&&e.days_until_due!==null){let u=(e.interval_days-e.days_until_due)/e.interval_days*100;s=Math.max(0,Math.min(100,u)),c=u>100,e.status==="overdue"?o=$e.overdue:e.status==="due_soon"&&(o=$e.due_soon)}return l`
      <div class="task-row${e.enabled?"":" task-disabled"}">
        ...
        <span class="due-cell" @click=${()=>this._showTask(e.entry_id,e.task_id)}>
          ...
          ${i?l`<div class="days-bar"><div class="days-bar-fill${c?" overflow":""}" style="width:${s}%;background:${o}"></div></div>`:d}
          ${e.trigger_config?this._renderTriggerProgress(e):!i&&e.trigger_active?l`<span style="color:var(--maint-triggered-color);font-weight:600">⚡</span>`:d}
          ${this._renderMiniSparkline(e)}
        </span>
        <span class="row-actions">
          <mwc-icon-button class="btn-complete" title="${n("complete",t)}" @click=${u=>{u.stopPropagation(),this._openCompleteDialogForRow(e)}}>
            <ha-icon icon="mdi:check"></ha-icon>
          </mwc-icon-button>
          <mwc-icon-button class="btn-skip" title="${n("skip",t)}" .disabled=${this._actionLoading} @click=${u=>{u.stopPropagation(),this._promptSkipTask(e.entry_id,e.task_id)}}>
            <ha-icon icon="mdi:skip-next"></ha-icon>
          </mwc-icon-button>
        </span>
      </div>
    `}_openCompleteDialogForRow(e){let i=this._objects.find(s=>s.entry_id===e.entry_id)?.tasks.find(s=>s.id===e.task_id);this._openCompleteDialog(e.entry_id,e.task_id,e.task_name,this._features.checklists?i?.checklist:void 0,this._features.adaptive&&!!i?.adaptive_config?.enabled)}_renderTriggerProgress(e){let t=e.trigger_config??null;if(!t)return d;let i=t.type||"threshold",s=e.trigger_entity_info?.unit_of_measurement??"",o=0,c="";if(i==="threshold"){let m=e.trigger_current_value??null;if(m==null)return d;let v=t.trigger_above,b=t.trigger_below;if(v!=null){let f=b??0,k=v-f||1;o=Math.min(100,Math.max(0,(m-f)/k*100)),c=`${m.toFixed(1)} / ${v} ${s}`}else if(b!=null){let k=e.trigger_entity_info?.max??(b*2||100),T=k-b||1;o=Math.min(100,Math.max(0,(k-m)/T*100)),c=`${m.toFixed(1)} / ${b} ${s}`}else return d}else if(i==="counter"){let m=t.trigger_target_value||1,b=e.trigger_current_delta??null??e.trigger_current_value??null;if(b==null)return d;o=Math.min(100,Math.max(0,b/m*100)),c=`${b.toFixed(1)} / ${m} ${s}`}else if(i==="state_change"){let m=t.trigger_target_changes||1,v=e.trigger_current_value??null;if(v==null)return d;o=Math.min(100,Math.max(0,v/m*100)),c=`${Math.round(v)} / ${m}`}else if(i==="runtime"){let m=t.trigger_runtime_hours||100,v=e.trigger_current_value??null;if(v==null)return d;o=Math.min(100,Math.max(0,v/m*100)),c=`${v.toFixed(1)}h / ${m}h`}else if(i==="compound"){let m=t.compound_logic||t.operator||"AND",v=t.conditions?.length||0;c=`${m} (${v})`,o=e.trigger_active?100:0}else return d;let u=o>=100,_=o>90?"var(--error-color, #f44336)":o>70?"var(--warning-color, #ff9800)":"var(--primary-color)";return l`
    ...
    `}_renderMiniSparkline(e){if(!e.trigger_config?.entity_id)return d;let t=e.trigger_config.entity_id,i=this._miniStatsData.get(t)||[],s=[];if(i.length>=2)s=i.map(x=>({ts:x.ts,val:x.val}));else{if(!e.history)return d;for(let x of e.history)x.trigger_value!=null&&s.push({ts:new Date(x.timestamp).getTime(),val:x.trigger_value})}if(e.trigger_current_value!=null&&s.push({ts:Date.now(),val:e.trigger_current_value}),s.length<2)return d;s.sort((x,z)=>x.ts-z.ts);let o=xi,c=$i,u=s.map(x=>x.val),_=Math.min(...u),m=Math.max(...u),v=m-_||1;_-=v*.1,m+=v*.1;let b=s[0].ts,k=s[s.length-1].ts-b||1,T=x=>(x-b)/k*o,q=x=>2+(1-(x-_)/(m-_))*(c-4),C=s;if(C.length>$t){let x=Math.ceil(C.length/$t);C=C.filter((z,O)=>O%x===0||O===C.length-1)}let A=C.map(x=>`${T(x.ts).toFixed(1)},${q(x.val).toFixed(1)}`).join(" ");return l`
    ...
    `}_renderDaysProgress(e){let t=this._lang;if(e.days_until_due==null||!e.interval_days||e.interval_days<=0)return d;let s=(e.interval_days-e.days_until_due)/e.interval_days*100,o=Math.max(0,Math.min(100,s)),c=s>100,u="var(--success-color, #4caf50)";return e.status==="overdue"?u="var(--error-color, #f44336)":e.status==="due_soon"&&(u="var(--warning-color, #ff9800)"),l`
      <div class="days-progress">
        <div class="days-progress-labels">
          <span>${e.last_performed?`${n("last_performed",t)}: ${Y(e.last_performed,t)}`:""}</span>
          <span>${e.next_due?`${n("next_due",t)}: ${Y(e.next_due,t)}`:""}</span>
        </div>
    ...
    `}_renderObjectDetail(){if(!this._selectedEntryId)return d;let e=this._getObject(this._selectedEntryId);if(!e)return l`<p>Object not found.</p>`;let t=e.object,i=this._lang;return l`
      <div class="detail-section">
        ...
        ${t.serial_number?l`<p class="meta">${n("serial_number_label",i)}: ${t.serial_number}</p>`:d}
        ${t.installation_date?l`<p class="meta">${n("installed",i)}: ${Y(t.installation_date,i)}</p>`:d}

        <h3>${n("tasks",i)} (${e.tasks.length})</h3>
        ${e.tasks.length===0?l`<div class="empty-state-centered">
              <p class="empty">${n("no_tasks_yet",i)}</p>
              <ha-button appearance="filled" @click=${()=>{this.shadowRoot.querySelector("maintenance-task-dialog")?.openCreate(e.entry_id)}}>${n("add_first_task",i)}</ha-button>
              ...
              <div class="task-row${s.enabled?"":" task-disabled"}">
                ...
                ${s.enabled?d:l`<span class="badge-disabled">${n("disabled",i)}</span>`}
                ${s.nfc_tag_id?l`<span class="nfc-badge" title="${n("nfc_linked",i)}"><ha-icon icon="mdi:nfc-variant"></ha-icon></span>`:d}
                ...
                <span class="due-cell" @click=${()=>this._showTask(e.entry_id,s.id)}>
                  <span class="due-text">${ke(s.days_until_due,i)}</span>
                  ${s.trigger_config?this._renderTriggerProgress(s):d}
                  ${this._renderMiniSparkline(s)}
                </span>
                <span class="row-actions">
                  <mwc-icon-button class="btn-complete" title="${n("complete",i)}" @click=${o=>{o.stopPropagation(),this._openCompleteDialog(e.entry_id,s.id,s.name,this._features.checklists?s.checklist:void 0,this._features.adaptive&&!!s.adaptive_config?.enabled)}}>
                    <ha-icon icon="mdi:check"></ha-icon>
                  </mwc-icon-button>
                  <mwc-icon-button class="btn-skip" title="${n("skip",i)}" .disabled=${this._actionLoading} @click=${o=>{o.stopPropagation(),this._promptSkipTask(e.entry_id,s.id)}}>
                    <ha-icon icon="mdi:skip-next"></ha-icon>
                  </mwc-icon-button>
                </span>
              </div>
          ...
          <div class="more-menu-wrapper">
            ...
            ${this._moreMenuOpen?l`
              <div class="popup-menu" @click=${u=>u.stopPropagation()}>
                ...
                <div class="popup-menu-item" @click=${()=>{this._closeMoreMenu();let u=this._getObject(this._selectedEntryId)?.object;this._openQrForTask(this._selectedEntryId,this._selectedTaskId,u?.name||"",e.name)}}><ha-icon icon="mdi:qrcode"></ha-icon> ${n("qr_code",t)}</div>
    ...
    `}_renderTaskMeta(e){let t=e.documentation_url&&/^https?:\/\//i.test(e.documentation_url)?e.documentation_url:null;if(!e.notes&&!t)return d;let i=this._lang;return l`
    ...
    `}_renderKPIBar(e){let t=this._lang,i=e.times_performed>0?e.total_cost/e.times_performed:0,s=e.days_until_due!==null&&e.days_until_due!==void 0?e.days_until_due<0?"overdue":e.days_until_due<=e.warning_days?"warning":"":"";return l`
      <div class="kpi-bar">
        <div class="kpi-card">
    ...
    `}_renderRecommendationCard(e){let t=this._lang;if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return d;if(this._selectedEntryId&&this._selectedTaskId&&this._dismissedSuggestions.has(`${this._selectedEntryId}_${this._selectedTaskId}`))return d;let i=e.interval_days,s=e.suggested_interval,o=e.interval_confidence||"medium",c=Math.max(i||1,s);return l`
      <div class="recommendation-card">
        <h4>${n("suggested_interval",t)}</h4>
        <div class="interval-comparison">
          <div class="interval-bar">
            <div class="interval-label">${n("current",t)||"Aktuell"}: ${i??"\u2014"} ${i!=null?n("days",t):""}</div>
            <div class="interval-visual current" style="width: ${i!=null?Math.min(i/c*100,100):0}%"></div>
          </div>
          <div class="interval-bar">
    ...
    `}_renderRecentActivities(e){let t=this._lang,i=e.history.slice(0,3);if(i.length===0)return d;let s=o=>{switch(o){case"completed":return"\u2713";case"triggered":return"\u2297";case"skipped":return"\u21B7";case"reset":return"\u21BA";default:return"\xB7"}};return l`
    ...
    `}_renderHistoryFilters(e){let t=this._lang;return l`
      <div class="history-filters-new">
        <div class="filter-chips">
          ${["completed","skipped","reset","triggered"].map(i=>{let s=e.history.filter(o=>o.type===i).length;return s===0?d:l`
    ...
    `}_renderHistoryList(e){let t=this._lang,i=this._historyFilter?e.history.filter(s=>s.type===this._historyFilter):e.history;if(this._historySearch){let s=this._historySearch.toLowerCase();i=i.filter(o=>o.notes?.toLowerCase().includes(s))}return i.length===0?l`<p class="empty">${n("no_history",t)}</p>`:l`
