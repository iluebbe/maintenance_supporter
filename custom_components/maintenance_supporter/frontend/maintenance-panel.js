...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `,g([S({attribute:!1})],B.prototype,"hass",2),g([_()],B.prototype,"_open",2),g([_()],B.prototype,"_title",2),g([_()],B.prototype,"_message",2),g([_()],B.prototype,"_confirmText",2),g([_()],B.prototype,"_danger",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",B);var ai=300,ri=140,Re=27;function pt(n,a){let e=n.trigger_config;if(!e)return c;let t=a.lang,i=n.trigger_entity_info,r=n.trigger_entity_infos,o=i?.friendly_name||e.entity_id||"\u2014",d=e.entity_id||"",u=e.entity_ids||(d?[d]:[]),p=i?.unit_of_measurement||"",m=n.trigger_current_value,v=e.type||"threshold",b=u.length>1;return l`
    <h3>${s("trigger",t)}</h3>
  ...
  `}function ni(n,a,e){let t=n.trigger_config;if(!t)return c;let i=t.type||"threshold",r=i==="counter"&&t.trigger_delta_mode,o=e.isCounterEntity(t),d=t.entity_id||"",u=e.detailStatsData.get(d)||[],p=[],m=!1;if(u.length>=2)for(let h of u){let k=h.val;r&&n.trigger_baseline_value!=null&&(k-=n.trigger_baseline_value);let I={ts:h.ts,val:k};!o&&h.min!=null&&h.max!=null&&(I.min=r&&n.trigger_baseline_value!=null?h.min-n.trigger_baseline_value:h.min,I.max=r&&n.trigger_baseline_value!=null?h.max-n.trigger_baseline_value:h.max,m=!0),p.push(I)}else for(let h of n.history)h.trigger_value!=null&&p.push({ts:new Date(h.timestamp).getTime(),val:h.trigger_value});if(n.trigger_current_value!=null){let h=n.trigger_current_value;r&&n.trigger_baseline_value!=null&&(h-=n.trigger_baseline_value),p.push({ts:Date.now(),val:h})}if(p.length<2&&d&&e.hasStatsService&&!e.detailStatsData.has(d))return l`<div class="sparkline-container" aria-live="polite" style="display:flex;align-items:center;justify-content:center;height:140px;color:var(--secondary-text-color);font-size:12px;">
    ...
    <div class="sparkline-container">
      <svg class="sparkline-svg" viewBox="0 0 ${v} ${b}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_sparkline",e.lang)}">
        ...
        <text x="${v-$}" y="${b-1}" text-anchor="end" fill="var(--secondary-text-color)" font-size="7">${new Date(j).toLocaleDateString(void 0,{month:"short",day:"numeric"})}</text>
        ...
        ${n.degradation_rate!=null&&n.degradation_trend!=="stable"&&n.degradation_trend!=="insufficient_data"&&p.length>=2?(()=>{let h=p[p.length-1],k=30,I=h.ts+k*864e5,H=h.val+n.degradation_rate*k,K=Math.min(I,j+(j-V)*.3),Z=Math.max(E,Math.min(y,H)),At=D(h.ts),St=M(h.val),Tt=D(K),Mt=M(Z);return z`<line x1="${At.toFixed(1)}" y1="${St.toFixed(1)}" x2="${Tt.toFixed(1)}" y2="${Mt.toFixed(1)}" stroke="var(--warning-color, #ff9800)" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.7" />`})():c}
        ${i==="threshold"&&t.trigger_above!=null?z`<line x1="${f}" y1="${M(t.trigger_above).toFixed(1)}" x2="${v}" y2="${M(t.trigger_above).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${v-2}" y="${M(t.trigger_above)-3}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25B2 ${t.trigger_above}</text>`:c}
        ${i==="threshold"&&t.trigger_below!=null?z`<line x1="${f}" y1="${M(t.trigger_below).toFixed(1)}" x2="${v}" y2="${M(t.trigger_below).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
                <text x="${v-2}" y="${M(t.trigger_below)+11}" text-anchor="end" fill="var(--error-color, #f44336)" font-size="9">\u25BC ${t.trigger_below}</text>`:c}
        ${i==="counter"&&O!=null?z`<line x1="${f}" y1="${M(O).toFixed(1)}" x2="${v}" y2="${M(O).toFixed(1)}" stroke="var(--error-color, #f44336)" stroke-width="1.5" stroke-dasharray="5,3" />
        ...
        ${i==="counter"&&L!=null?z`<line x1="${f}" y1="${M(L).toFixed(1)}" x2="${v}" y2="${M(L).toFixed(1)}" stroke="var(--secondary-text-color)" stroke-width="1" stroke-dasharray="3,3" opacity="0.5" />
        ...
        ${Et.map(h=>{let k=D(h.ts),I=h.type==="completed"?"var(--success-color, #4caf50)":h.type==="skipped"?"var(--warning-color, #ff9800)":"var(--info-color, #2196f3)";return z`
          ...
          `})}
        ${ke.map(h=>{let k=D(h.ts),I=M(h.val),H=new Date(h.ts).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),K=`${h.val.toFixed(1)} ${a}`;return m&&h.min!=null&&h.max!=null&&(K+=` (${h.min.toFixed(1)}\u2013${h.max.toFixed(1)})`),z`<circle cx="${k.toFixed(1)}" cy="${I.toFixed(1)}" r="8" fill="transparent" tabindex="0"
            @mouseenter=${Z=>ut(Z,`${H}
${K}`,e.setTooltip)}
            @focus=${Z=>ut(Z,`${H}
${K}`,e.setTooltip)}
...
`).map(h=>l`<div>${h}</div>`)}
  ...
  `}function oi(n,a,e,t,i){let f=Math.max(e,t,a,1)*1.3,$=50,T=[];for(let j=0;j<=$;j++){let x=j/$*f,D=1-Math.exp(-Math.pow(x/a,n)),M=32+x/f*260,$e=136-D*128;T.push([M,$e])}let R=T.map(([j,x])=>`${j.toFixed(1)},${x.toFixed(1)}`).join(" "),C="M32,136 "+T.map(([j,x])=>`L${j.toFixed(1)},${x.toFixed(1)}`).join(" ")+` L${T[$][0].toFixed(1)},136 Z`,E=32+e/f*260,y=1-Math.exp(-Math.pow(e/a,n)),L=136-y*128,O=((1-y)*100).toFixed(0),W=32+t/f*260,V=[0,.25,.5,.75,1];return l`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_weibull",i)}">
        ${V.map(j=>{let x=136-j*128;return z`
            <line x1="${32}" y1="${x.toFixed(1)}" x2="${292}" y2="${x.toFixed(1)}"
              stroke="var(--divider-color)" stroke-width="0.5" stroke-dasharray="${j===.5?"4,3":c}" />
  ...
  `}function vt(n,a){return di(n,a)}function di(n,a){let e=n.seasonal_factors??n.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return c;let t=n.interval_analysis?.seasonal_reason,i=new Date().getMonth(),r=300,o=100,d=8,p=o-d-4,m=Math.max(...e,1.5),v=r/12,b=v*.65,f=d+p-1/m*p;return l`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${r} ${o}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",a)}">
        ...
        ${e.map(($,T)=>{let R=$/m*p,C=T*v+(v-b)/2,E=d+p-R,y=T===i,L=$<1?"var(--success-color, #4caf50)":$>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return z`
  ...
  `}function gi(n,a,e){let t=n.history.filter(x=>x.type==="completed"&&(x.cost!=null||x.duration!=null)).map(x=>({ts:new Date(x.timestamp).getTime(),cost:x.cost??0,duration:x.duration??0})).sort((x,D)=>x.ts-D.ts);if(t.length<2)return c;let i=t.some(x=>x.cost>0),r=t.some(x=>x.duration>0);if(!i&&!r)return c;let o=e!=="duration"&&i,d=e!=="cost"&&r,u=o||!d&&i,p=d||!o&&r,m=ui,v=pi,b=u?32:8,f=p?32:8,$=8,T=20,R=m-b-f,C=v-$-T,E=Math.max(...t.map(x=>x.cost))||1,y=Math.max(...t.map(x=>x.duration))||1,L=Math.min(20,R/t.length*.6),O=R/t.length,W=x=>b+O*x+O/2,V=x=>$+C-x/E*C,j=x=>$+C-x/y*C;return l`
    <div class="sparkline-container">
      <svg class="history-chart" viewBox="0 0 ${m} ${v}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_history",a)}">
        ...
        `):c}
        ${p?z`
          <polyline points="${t.map((x,D)=>`${W(D).toFixed(1)},${j(x.duration).toFixed(1)}`).join(" ")}"
            fill="none" stroke="var(--accent-color, #ff9800)" stroke-width="2" stroke-linejoin="round" />
          ${t.map((x,D)=>z`
            <circle cx="${W(D).toFixed(1)}" cy="${j(x.duration).toFixed(1)}" r="3" fill="var(--accent-color, #ff9800)" />
          `)}
        `:c}
