...
`).map(s=>s.trim()).filter(Boolean).slice(0,100);e.checklist=l.length?l:null}if(this.completionActionsEnabled){let l=this._actionService.trim();if(l&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(l)){let v={service:l},h=this._actionTargetEntity.trim();h&&(v.target={entity_id:h});let x=this._buildActionData();Object.keys(x).length>0&&(v.data=x),e.on_complete_action=v}else e.on_complete_action=null;let s={};this._qcNotes.trim()&&(s.notes=this._qcNotes.trim());let u=parseFloat(this._qcCost);!isNaN(u)&&u>=0&&(s.cost=u);let g=parseInt(this._qcDuration,10);!isNaN(g)&&g>=0&&(s.duration=g),this._qcFeedback&&(s.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(s).length?s:null}let t=await this.hass.connection.sendMessagePromise(e),n=this._taskId||t?.task_id,r=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(n&&this._scheduleType==="sensor_based"&&r)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:n,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=y(e,this._lang,o("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return p;let e=this._lang;return c`
      ...
      <div class="select-row">
        ...
        >
          ${en.map(t=>c`<option value=${t} ?selected=${t===this._triggerType}>${o(t,e)}</option>`)}
        </select>
      </div>
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${Ya.map(n=>c`<option value=${n} ?selected=${n===this._type}>${o(n,e)}</option>`)}
            </select>
          </div>
          <div class="select-row">
            ...
            >
              ${Xa.map(n=>c`<option value=${n} ?selected=${n===this._scheduleType}>${o(n,e)}</option>`)}
            </select>
          </div>
...
<script>setTimeout(()=>window.print(),300)<\/script>
</body></html>`),r.document.close()}_downloadSvg(e,t){let n=decodeURIComponent(e.svg_data_uri.replace("data:image/svg+xml,","")),r=new Blob([n],{type:"image/svg+xml"}),l=URL.createObjectURL(r),s=document.createElement("a");s.href=l;let u=this._taskName?`${this._objectName}-${this._taskName}`:this._objectName;s.download=`qr-${tn(u)}-${t}.svg`,s.click(),URL.revokeObjectURL(l)}_close(){this._open=!1,this._viewResult=null,this._completeResult=null,this._error="",this._loading=!1}render(){if(!this._open)return c``;let e=this.lang||this.hass?.language||"en",t=this._taskName?`${o("qr_code",e)}: ${this._objectName} \u2014 ${this._taskName}`:`${o("qr_code",e)}: ${this._objectName}`,n=!!this._viewResult;return c`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
  ...
  `}function nn(i,a,e,t,n){let z=Math.max(e,t,a,1)*1.3,q=50,H=[];for(let R=0;R<=q;R++){let D=R/q*z,va=1-Math.exp(-Math.pow(D/a,i)),fa=32+D/z*260,ba=136-va*128;H.push([fa,ba])}let fe=H.map(([R,D])=>`${R.toFixed(1)},${D.toFixed(1)}`).join(" "),Ce="M32,136 "+H.map(([R,D])=>`L${R.toFixed(1)},${D.toFixed(1)}`).join(" ")+` L${H[q][0].toFixed(1)},136 Z`,X=32+e/z*260,be=1-Math.exp(-Math.pow(e/a,i)),ye=136-be*128,ma=((1-be)*100).toFixed(0),it=32+t/z*260,ha=[0,.25,.5,.75,1];return c`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${o("chart_weibull",n)}">
        ...
        <path d="${Ce}" fill="var(--primary-color, #03a9f4)" opacity="0.08" />
        ...
        ${e>0?ae`
          ...
          <circle cx="${X.toFixed(1)}" cy="${ye.toFixed(1)}" r="3"
            fill="var(--primary-color, #03a9f4)" />
          <text x="${(X+4).toFixed(1)}" y="${(ye-6).toFixed(1)}" fill="var(--primary-color, #03a9f4)"
            font-size="9" font-weight="600">R=${ma}%</text>
        `:p}
  ...
  `}function rn(i,a,e){let t=i.confidence_interval_low,n=i.confidence_interval_high,r=a.suggested_interval??a.interval_days??0,l=a.interval_days??0,s=Math.max(0,t-5),g=n+5-s,v=(t-s)/g*100,h=(n-t)/g*100,x=(r-s)/g*100,z=l>0?(l-s)/g*100:-1;return c`
      ...
      <div class="prediction-grid">
        ...
        ${n?c`
          <div class="prediction-item">
            ...
            ${i.threshold_prediction_date?c`<span class="prediction-date">${me(i.threshold_prediction_date,a)}</span>`:p}
        ...
        ${r&&e.environmental?c`
          <div class="prediction-item">
            ...
            ${i.environmental_entity?c`<span class="prediction-entity entity-link" @click=${s=>Mt(s,i.environmental_entity)}>${i.environmental_entity}</span>`:p}
          </div>
        `:p}
      </div>
    </div>
  `}var Kt=b(()=>{"use strict";A();S()});function Qt(i,a,e,t){let n=Math.max(i||1,a);return c`
  ...
  `}var Zt=b(()=>{"use strict";A();S()});function Yt(i,a,e){if(!e.seasonal||!i.seasonal_factor||i.seasonal_factor===1)return p;let t=Jt.map(s=>o(s,a)),n=new Date().getMonth(),r=i.seasonal_factors||i.interval_analysis?.seasonal_factors||null,l=r&&r.length===12?r:t.map((s,u)=>{let g=i.seasonal_factor||1,v=Math.sin((u-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,g+v))});return c`
  ...
  `}function Xt(i,a){return sn(i,a)}function sn(i,a){let e=i.seasonal_factors??i.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return p;let t=i.interval_analysis?.seasonal_reason,n=new Date().getMonth(),r=300,l=100,s=8,g=l-s-4,v=Math.max(...e,1.5),h=r/12,x=h*.65,z=s+g-1/v*g;return c`
  ...
  `}var Jt,ea=b(()=>{"use strict";A();S();Jt=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"]});var $,ta=b(()=>{"use strict";A();M();S();P();Wt();Kt();Zt();ea();$=class extends k{constructor(){super(...arguments);this._open=!1;this._entryId=null;this._taskId=null;this._task=null;this._objectName="";this._busy=!1;this._error="";this._showSkip=!1;this._showReset=!1;this._showDetails=!1;this._showAdaptive=!1;this._skipReason="";this._resetDate="";this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._toast="";this._featuresLoaded=!1}get _lang(){return this.hass?.language||"en"}async openFor(e,t){this._entryId=e,this._taskId=t,this._error="",this._showSkip=!1,this._showReset=!1,this._showAdaptive=!1,this._skipReason="",this._resetDate=new Date().toISOString().slice(0,10),this._open=!0,await Promise.all([this._loadTask(),this._loadFeatures()])}async _loadFeatures(){if(!this._featuresLoaded)try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});e?.features&&(this._features={...this._features,...e.features}),this._featuresLoaded=!0}catch{}}close(){this._open=!1,this._task=null,this._error=""}async _loadTask(){if(!(!this._entryId||!this._taskId))try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._objectName=e.object?.name||"";let t=(e.tasks||[]).find(n=>n.id===this._taskId);this._task=t??null}catch(e){this._error=y(e,this._lang)}}async _runWs(e){this._busy=!0,this._error="";try{return await this.hass.connection.sendMessagePromise(e),this._busy=!1,!0}catch(t){return this._error=y(t,this._lang),this._busy=!1,!1}}_notifyChanged(e){this.dispatchEvent(new CustomEvent("task-action-fired",{detail:{entry_id:this._entryId,task_id:this._taskId,action:e},bubbles:!0,composed:!0}))}_onComplete(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(L(),V)).then(({openCompleteDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,checklist:this._task.checklist||[],adaptive_enabled:!!this._task.adaptive_config?.enabled})&&(this._notifyChanged("complete"),this.close())})}async _onSkipConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/skip",entry_id:this._entryId,task_id:this._taskId,reason:this._skipReason.trim()||null})&&(this._notifyChanged("skip"),this.close())}async _onResetConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/reset",entry_id:this._entryId,task_id:this._taskId,date:this._resetDate||void 0})&&(this._notifyChanged("reset"),this.close())}_onEdit(){!this._entryId||!this._taskId||Promise.resolve().then(()=>(L(),V)).then(({openEditTaskDialog:e})=>{e(this._entryId,this._taskId),this.close()})}_onQr(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(L(),V)).then(({openQrDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,object_name:this._objectName}),this.close()})}async _onDelete(){if(!this._entryId||!this._taskId)return;let e=o("delete_task_confirm",this._lang)||`Delete "${this._task?.name}"?`;if(!window.confirm(e))return;await this._runWs({type:"maintenance_supporter/task/delete",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("delete"),this.close())}_onOpenInPanel(){if(!this._entryId||!this._taskId)return;let e=`/maintenance-supporter?entry_id=${encodeURIComponent(this._entryId)}&task_id=${encodeURIComponent(this._taskId)}`;history.pushState(null,"",e),window.dispatchEvent(new CustomEvent("location-changed")),this.close()}async _applySuggestion(){if(!this._entryId||!this._taskId||!this._task?.suggested_interval)return;await this._runWs({type:"maintenance_supporter/task/apply_suggestion",entry_id:this._entryId,task_id:this._taskId,interval:this._task.suggested_interval})&&(this._toast=o("suggestion_applied",this._lang)||"Applied",this._notifyChanged("apply_suggestion"),await this._loadTask(),setTimeout(()=>{this._toast=""},2500))}async _reanalyzeInterval(){if(!(!this._entryId||!this._taskId)){this._busy=!0,this._error="";try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:this._entryId,task_id:this._taskId});this._toast=e.recommended_interval?`${o("reanalyze_result",this._lang)||"Recomputed"}: ${e.recommended_interval}d (${e.data_points} pts)`:o("reanalyze_insufficient_data",this._lang)||"Not enough data",await this._loadTask(),setTimeout(()=>{this._toast=""},3500)}catch(e){this._error=y(e,this._lang)}finally{this._busy=!1}}}_onEditHistoryEntry(e){!this._entryId||!this._taskId||Promise.resolve().then(()=>(L(),V)).then(({openHistoryEditDialog:t})=>{t({entry_id:this._entryId,task_id:this._taskId,original_timestamp:e.timestamp,type:e.type,timestamp:e.timestamp,notes:e.notes??null,cost:e.cost??null,duration:e.duration??null,completed_by:e.completed_by??null})})}_renderRecommendation(e){if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return p;let t=this._lang;return c`
    ...
    `}_renderAdaptive(e){let t=this._lang,n=this._features.adaptive&&e.suggested_interval&&e.suggested_interval!==e.interval_days,r=e.degradation_trend!=null&&e.degradation_trend!=="insufficient_data"||e.days_until_threshold!=null||e.environmental_factor!=null&&e.environmental_factor!==1,l=this._features.adaptive&&e.interval_analysis?.weibull_beta!=null&&e.interval_analysis?.weibull_eta!=null,s=this._features.seasonal&&e.seasonal_factor&&e.seasonal_factor!==1;return!n&&!r&&!l&&!s?c`<div class="adaptive-empty">
      ...
      <div class="adaptive-stack">
        ...
        ${r?Gt(e,t,this._features):p}
        ${l?Bt(e,t):p}
        ${s?c`
          ${Yt(e,t,this._features)}
          ${e.seasonal_factors?.length===12||e.interval_analysis?.seasonal_factors?.length===12?Xt(e,t):p}
        `:p}
      </div>
                ...
                ${[...n].reverse().slice(0,20).map(u=>{let g=["completed","reset","skipped"].includes(u.type);return c`
                    <div class="history-entry">
                      <div class="history-line">
                        ...
                        <span class="history-date">${Rt(u.timestamp,t)}</span>
    ...
    `}render(){if(!this._open)return p;let e=this._lang,t=this._task,n=this.hass?.user?.is_admin??!0;return c`
      ...
      <div class="dialog" role="dialog" aria-modal="true">
        ${t?c`
              <div class="header">
                <div class="title">
                  <span class="status-dot" style="background: ${je[t.status]||"#ccc"}"></span>
                ...
                <div class="quick-info">
                  ${t.next_due?c`<span><strong>${o("next_due",e)||"Next due"}:</strong> ${me(t.next_due,e)}</span>`:p}
                  ${t.last_performed?c`<span><strong>${o("last_performed",e)||"Last"}:</strong> ${me(t.last_performed,e)}</span>`:p}
                      ...
                      <div class="task-list">
                        ${r.map(s=>c`
                          <div class="task-row" @click=${()=>this._onTaskClick(s.id)}>
                            <span class="status-dot" style="background: ${je[s.status]||"#ccc"}"></span>
  ...
  `,d([f({attribute:!1})],F.prototype,"hass",2),d([_()],F.prototype,"_open",2),d([_()],F.prototype,"_entryId",2),d([_()],F.prototype,"_data",2),d([_()],F.prototype,"_busy",2),d([_()],F.prototype,"_error",2);customElements.get("maintenance-object-quick-actions-dialog")||customElements.define("maintenance-object-quick-actions-dialog",F)});var V={};ka(V,{openCompleteDialog:()=>mn,openCreateObjectDialog:()=>We,openCreateTaskDialog:()=>gn,openEditObjectDialog:()=>pn,openEditTaskDialog:()=>Ge,openHistoryEditDialog:()=>Ke,openObjectQuickActions:()=>Ze,openQrDialog:()=>hn,openTaskQuickActions:()=>Qe});function oa(){return document.querySelector("home-assistant")?.hass}function B(i){let a=document.body.querySelector(i);return a||(a=document.createElement(i),document.body.appendChild(a)),a}function W(i){let a=oa();return a?(i.hass=a,!0):!1}function We(){let i=B(na);return W(i)?(i.openCreate(),!0):!1}function pn(i,a){let e=B(na);return W(e)?(e.openEdit(i,a),!0):!1}function gn(){let i=B(ia);return W(i)?(i.openCreate(),!0):!1}function Ge(i,a){let e=B(ia);return W(e)?(e.openEdit(i,{id:a}),!0):!1}function Ke(i){let a=B(ln);return W(a)?(a.openEdit(i),!0):!1}function mn(i){let a=B(dn);return W(a)?(a.entryId=i.entry_id,a.taskId=i.task_id,a.taskName=i.task_name,a.checklist=i.checklist??[],a.adaptiveEnabled=!!i.adaptive_enabled,a.lang=oa()?.language||"en",a.open(),!0):!1}function hn(i){let a=B(cn);return W(a)?(a.openForTask(i.entry_id,i.task_id,i.object_name,i.task_name),!0):!1}function Qe(i,a){let e=B(_n);return W(e)?(e.openFor(i,a),!0):!1}function Ze(i){let a=B(un);return W(a)?(a.openFor(i),!0):!1}var na,ia,ln,dn,cn,_n,un,L=b(()=>{"use strict";Pt();Ft();Ht();Ot();Vt();ta();aa();na="maintenance-object-dialog",ia="maintenance-task-dialog",ln="maintenance-history-edit-dialog",dn="maintenance-complete-dialog",cn="maintenance-qr-dialog",_n="maintenance-task-quick-actions-dialog",un="maintenance-object-quick-actions-dialog"});A();M();S();P();A();var ie=w`
...
`;var C=class extends k{constructor(){super(...arguments);this._config={type:""};this._state=null;this._busy=!1;this._error="";this._localStart="";this._localEnd="";this._localBuffer=7;this._dirty=!1;this._loaded=!1}setConfig(e){this._config=e}getCardSize(){return 2}get _lang(){return this.hass?.language||"en"}get _isAdmin(){return this.hass?.user?.is_admin??!0}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._loaded&&(this._loaded=!0,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/vacation/state"});this._state=e,this._localStart=e.start||"",this._localEnd=e.end||"",this._localBuffer=e.buffer_days??7,this._dirty=!1}catch(e){this._error=y(e,this._lang)}}async _toggleEnabled(e){this._busy=!0,this._error="";try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/vacation/update",enabled:e});this._state=t}catch(t){this._error=y(t,this._lang)}finally{this._busy=!1}}async _save(){if(this._isAdmin){this._busy=!0,this._error="";try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/vacation/update",start:this._localStart||null,end:this._localEnd||null,buffer_days:this._localBuffer});this._state=e,this._dirty=!1}catch(e){this._error=y(e,this._lang)}finally{this._busy=!1}}}async _endNow(){if(this._isAdmin&&window.confirm(o("vacation_end_now_confirm",this._lang)||"End vacation immediately?")){this._busy=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/vacation/end_now"});this._state=e,this._localStart=e.start||"",this._localEnd=e.end||""}catch(e){this._error=y(e,this._lang)}finally{this._busy=!1}}}_onDeepLink(){history.pushState(null,"","/maintenance-supporter?ms_action=open_vacation"),window.dispatchEvent(new CustomEvent("location-changed"))}render(){let e=this._lang,t=this._state;if(!t)return c`<ha-card><div class="loading">${o("loading",e)||"Loading\u2026"}</div></ha-card>`;let n=t.is_active===!0,r=t.enabled===!0,l=t.exempt_task_ids?.length??0,s=n?o("vacation_status_active",e)||"Active now":r?o("vacation_status_scheduled",e)||"Scheduled":o("vacation_status_inactive",e)||"Inactive",u=n?"active":r?"scheduled":"inactive";return c`
  ...
  `],d([f({attribute:!1})],T.prototype,"hass",2),d([_()],T.prototype,"_config",2),d([_()],T.prototype,"_status",2),d([_()],T.prototype,"_busy",2),d([_()],T.prototype,"_error",2),d([_()],T.prototype,"_localMonthly",2),d([_()],T.prototype,"_localYearly",2),d([_()],T.prototype,"_dirty",2);customElements.get("maintenance-budget-section-card")||customElements.define("maintenance-budget-section-card",T);window.customCards=window.customCards||[];window.customCards.push({type:"maintenance-budget-section-card",name:"Maintenance Supporter \u2014 Budget",description:"Inline monthly + yearly budget editor",preview:!1});A();M();S();P();var N=class extends k{constructor(){super(...arguments);this._config={type:""};this._groups={};this._loaded=!1;this._busy=!1;this._error="";this._newName="";this._editingId=null;this._editingName="";this._hasInitiallyLoaded=!1}setConfig(e){this._config=e}getCardSize(){return 2}get _lang(){return this.hass?.language||"en"}get _isAdmin(){return this.hass?.user?.is_admin??!0}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._hasInitiallyLoaded&&(this._hasInitiallyLoaded=!0,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"});this._groups=e.groups||{},this._loaded=!0}catch(e){this._error=y(e,this._lang)}}async _addGroup(){if(!this._isAdmin)return;let e=this._newName.trim();if(e){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/group/create",name:e}),this._newName="",await this._load()}catch(t){this._error=y(t,this._lang)}finally{this._busy=!1}}}_startEdit(e){this._editingId=e,this._editingName=this._groups[e]?.name||""}async _saveEdit(){if(!this._isAdmin||!this._editingId)return;let e=this._editingName.trim();if(e){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/group/update",group_id:this._editingId,name:e}),this._editingId=null,this._editingName="",await this._load()}catch(t){this._error=y(t,this._lang)}finally{this._busy=!1}}}async _deleteGroup(e,t){if(!this._isAdmin)return;let n=(o("group_delete_confirm",this._lang)||'Delete group "{name}"?').replace("{name}",t);if(window.confirm(n)){this._busy=!0;try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/group/delete",group_id:e}),await this._load()}catch(r){this._error=y(r,this._lang)}finally{this._busy=!1}}}_onDeepLink(){history.pushState(null,"","/maintenance-supporter?ms_action=open_groups"),window.dispatchEvent(new CustomEvent("location-changed"))}_onKeyDown(e,t){e.key==="Enter"?(e.preventDefault(),t()):e.key==="Escape"&&(e.preventDefault(),this._editingId=null,this._editingName="")}render(){let e=this._lang;if(!this._loaded)return c`<ha-card><div class="loading">${o("loading",e)||"Loading\u2026"}</div></ha-card>`;let t=Object.keys(this._groups);return c`
    ...
    .btn ha-icon { --mdc-icon-size: 16px; }
  `],d([f({attribute:!1})],N.prototype,"hass",2),d([_()],N.prototype,"_config",2),d([_()],N.prototype,"_groups",2),d([_()],N.prototype,"_loaded",2),d([_()],N.prototype,"_busy",2),d([_()],N.prototype,"_error",2),d([_()],N.prototype,"_newName",2),d([_()],N.prototype,"_editingId",2),d([_()],N.prototype,"_editingName",2);customElements.get("maintenance-groups-section-card")||customElements.define("maintenance-groups-section-card",N);window.customCards=window.customCards||[];window.customCards.push({type:"maintenance-groups-section-card",name:"Maintenance Supporter \u2014 Groups",description:"Inline group CRUD",preview:!1});L();var pa="maintenance-supporter",ra=`ll-strategy-dashboard-${pa}`,Je="hui-maintenance-supporter-strategy-editor",vn="NOT_RUNNING",fn=[{status:"overdue",title:"Overdue",icon:"mdi:alert-circle",path:"overdue"},{status:"triggered",title:"Triggered",icon:"mdi:flash",path:"triggered"},{status:"due_soon",title:"Due Soon",icon:"mdi:clock-alert-outline",path:"due-soon"},{status:"ok",title:"OK",icon:"mdi:check-circle-outline",path:"ok"}],sa=[{title:"Overdue",icon:"mdi:alert-circle",path:"overdue",filter:{filter_due_max_days:-1},matches:i=>i<=-1},{title:"Today",icon:"mdi:calendar-today",path:"today",filter:{filter_due_min_days:0,filter_due_max_days:0},matches:i=>i===0},{title:"This Week",icon:"mdi:calendar-week",path:"this-week",filter:{filter_due_min_days:1,filter_due_max_days:7},matches:i=>i>=1&&i<=7},{title:"This Month",icon:"mdi:calendar-month",path:"this-month",filter:{filter_due_min_days:8,filter_due_max_days:30},matches:i=>i>=8&&i<=30},{title:"Later",icon:"mdi:calendar-clock",path:"later",filter:{filter_due_min_days:31},matches:i=>i>=31}];function Y(i){return{type:"grid",cards:[i]}}var bn={condition:"view_columns",min:2};function yn(i){let a={overdue:0,triggered:0,due_soon:0,ok:0,total:0};for(let e of i)for(let t of e.tasks||[])a.total++,t.status==="overdue"?a.overdue++:t.status==="triggered"?a.triggered++:t.status==="due_soon"?a.due_soon++:t.status==="ok"&&a.ok++;return a}function kn(i){let a=[];return i.overdue&&a.push(`\u{1F534} **${i.overdue}** overdue`),i.triggered&&a.push(`\u26A1 **${i.triggered}** triggered`),i.due_soon&&a.push(`\u{1F7E1} **${i.due_soon}** due soon`),a.push(`\u{1F7E2} **${i.ok}** ok`),{type:"markdown",text_only:!0,content:a.join(" \xB7 ")}}function wn(){return{title:"Maintenance",type:"panel",cards:[{type:"empty-state",icon:"mdi:wrench-clock",icon_color:"primary",content_only:!0,title:"No maintenance objects yet",content:"Open the Maintenance panel to add your first object \u2014 pool pump, HVAC filter, vehicle, anything that needs scheduled care.",buttons:[{icon:"mdi:wrench",text:"Open Maintenance panel",appearance:"filled",variant:"brand",tap_action:{action:"navigate",navigation_path:"/maintenance-supporter"}},{icon:"mdi:plus",text:"Add object",appearance:"outlined",variant:"brand",tap_action:{action:"fire-dom-event",ll_custom:{type:"maintenance-supporter:add-object"}}}]}]}}function xn(i){let a=kn(i);return{title:"Overview",icon:"mdi:wrench-clock",path:"overview",type:"sections",header:{layout:"responsive",card:a},sidebar:{sections:[{type:"grid",cards:[{type:"heading",heading:"Status",heading_style:"title"},a]}],visibility:[bn],content_label:"Tasks",sidebar_label:"Status"},sections:[Y({type:"custom:maintenance-supporter-card",show_header:!1,filter_status:["overdue","triggered","due_soon"]})]}}function zn(i,a){let e=new Map;for(let l of i){let s=l.object.area_id||null;e.has(s)||e.set(s,[]),e.get(s).push(l)}let t=Array.from(e.keys()).filter(l=>l!==null);t.sort((l,s)=>{let u=a[l]?.name||l,g=a[s]?.name||s;return u.localeCompare(g)});let n=[];for(let l of t){let s=e.get(l);if(s.length===0)continue;let u=a[l];n.push({title:u?.name||l,icon:u?.icon||"mdi:floor-plan",path:`area-${l}`,type:"sections",sections:[Y({type:"custom:maintenance-supporter-card",show_header:!1,filter_objects:s.map(g=>g.object.name)})]})}let r=e.get(null);return r&&r.length>0&&n.push({title:"Unassigned",icon:"mdi:help-circle-outline",path:"unassigned",type:"sections",sections:[Y({type:"custom:maintenance-supporter-card",show_header:!1,filter_objects:r.map(l=>l.object.name)})]}),n}function $n(i){let a=new Set;for(let t of i)for(let n of t.tasks||[])n.status&&a.add(n.status);let e=[];for(let t of fn)a.has(t.status)&&e.push({title:t.title,icon:t.icon,path:t.path,type:"sections",sections:[Y({type:"custom:maintenance-supporter-card",show_header:!1,filter_status:[t.status]})]});return e}function En(i,a,e){let t=new Map;for(let s of i){let u=s.object.area_id,g=u&&a[u]?.floor_id||null;t.has(g)||t.set(g,[]),t.get(g).push(s)}let n=Array.from(t.keys()).filter(s=>s!==null);n.sort((s,u)=>{let g=e[s],v=e[u],h=g?.level??0,x=v?.level??0;return h!==x?h-x:(g?.name||s).localeCompare(v?.name||u)});let r=[];for(let s of n){let u=t.get(s);if(u.length===0)continue;let g=e[s];r.push({title:g?.name||s,icon:g?.icon||"mdi:home-floor-1",path:`floor-${s}`,type:"sections",sections:[Y({type:"custom:maintenance-supporter-card",show_header:!1,filter_objects:u.map(v=>v.object.name)})]})}let l=t.get(null);return l&&l.length>0&&r.push({title:"Other",icon:"mdi:help-circle-outline",path:"other",type:"sections",sections:[Y({type:"custom:maintenance-supporter-card",show_header:!1,filter_objects:l.map(s=>s.object.name)})]}),r}function An(i){return[{title:"Week",icon:"mdi:calendar-week",path:"cal-7",window_days:7},{title:"Fortnight",icon:"mdi:calendar-week-begin",path:"cal-14",window_days:14},{title:"Month",icon:"mdi:calendar-month",path:"cal-30",window_days:30},{title:"Year",icon:"mdi:calendar-clock",path:"cal-365",window_days:365}].map(e=>({title:e.title,icon:e.icon,path:e.path,type:"panel",cards:[{type:"custom:maintenance-supporter-calendar-card",window_days:e.window_days,show_window_chips:!1,show_user_filter:!0}]}))}function jn(i){let a=new Set;for(let t of i)for(let n of t.tasks||[]){let r=n.days_until_due;r!=null&&sa.forEach((l,s)=>{l.matches(r)&&a.add(s)})}let e=[];return sa.forEach((t,n)=>{a.has(n)&&e.push({title:t.title,icon:t.icon,path:t.path,type:"sections",sections:[Y({type:"custom:maintenance-supporter-card",show_header:!1,...t.filter})]})}),e}var Ye=class extends HTMLElement{static getCreateSuggestions(a){return{title:"Maintenance Supporter",icon:"mdi:wrench-clock"}}static async getConfigElement(){return document.createElement(Je)}static async generate(a,e){if(e.config?.state===vn)return{views:[{type:"sections",sections:[{cards:[{type:"starting"}]}]}]};if(e.config?.recovery_mode)return{views:[{type:"sections",sections:[{cards:[{type:"recovery-mode"}]}]}]};let t;try{t=await e.connection.sendMessagePromise({type:"maintenance_supporter/objects"})}catch{return{title:"Maintenance",views:[{title:"Maintenance",cards:[{type:"markdown",content:"**Maintenance Supporter** is not loaded. Install/enable the integration first."}]}]}}let n=t.objects||[];if(n.length===0)return{title:"Maintenance",views:[wn()]};let r=yn(n),l=a?.group_by??"area",s={area:()=>zn(n,e.areas||{}),status:()=>$n(n),floor:()=>En(n,e.areas||{},e.floors||{}),due_date:()=>jn(n),calendar:()=>An(n)};return{title:"Maintenance",views:[xn(r),...(s[l]??s.area)()]}}},Sn=[{value:"area",label:"By area (default)"},{value:"status",label:"By status (Overdue / Triggered / Due Soon / OK)"},{value:"floor",label:"By floor (uses HA floors)"},{value:"due_date",label:"By due date (Overdue / Today / Week / Month / Later)"},{value:"calendar",label:"Rolling calendar (Week / Fortnight / Month / Year)"}],Xe=class extends HTMLElement{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter"}}set hass(e){this._hass=e}setConfig(e){this._config=e,this._render()}connectedCallback(){this._render()}_render(){let e=this._config.group_by??"area",t=Sn.map(r=>`<option value="${r.value}"${r.value===e?" selected":""}>${r.label}</option>`).join("");this.innerHTML=`
      <style>
    ...
    `;let n=this.querySelector("#group-by");n?.addEventListener("change",()=>{let r={...this._config,group_by:n.value};this._config=r,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:r},bubbles:!0,composed:!0}))})}},ga="maintenance-supporter-section",la=`ll-strategy-section-${ga}`,et=class extends HTMLElement{static async generate(a,e){let t={type:"custom:maintenance-supporter-card",show_header:!1};a?.filter_status?.length&&(t.filter_status=a.filter_status),a?.max_items&&(t.max_items=a.max_items),a?.filter_due_min_days!==void 0&&(t.filter_due_min_days=a.filter_due_min_days),a?.filter_due_max_days!==void 0&&(t.filter_due_max_days=a.filter_due_max_days);let n=a?.filter_objects;if(a?.area_id&&!n)try{n=((await e.connection.sendMessagePromise({type:"maintenance_supporter/objects"})).objects||[]).filter(s=>s.object.area_id===a.area_id).map(s=>s.object.name)}catch{}n&&n.length>0&&(t.filter_objects=n);let r=[];return a?.title&&r.push({type:"heading",heading:a.title,heading_style:"title"}),r.push(t),{type:"grid",cards:r}}};customElements.get(ra)||customElements.define(ra,Ye);customElements.get(Je)||customElements.define(Je,Xe);customElements.get(la)||customElements.define(la,et);var tt=class extends HTMLElement{static async generate(a,e){return{type:"grid",cards:[{type:"custom:maintenance-vacation-section-card",title:a?.title}]}}},at=class extends HTMLElement{static async generate(a,e){return{type:"grid",cards:[{type:"custom:maintenance-budget-section-card",title:a?.title}]}}},nt=class extends HTMLElement{static async generate(a,e){return{type:"grid",cards:[{type:"custom:maintenance-groups-section-card",title:a?.title}]}}},da="ll-strategy-section-maintenance-supporter-vacation",ca="ll-strategy-section-maintenance-supporter-budget",_a="ll-strategy-section-maintenance-supporter-groups";customElements.get(da)||customElements.define(da,tt);customElements.get(ca)||customElements.define(ca,at);customElements.get(_a)||customElements.define(_a,nt);var ua="_msSupporterLlCustomBound";function he(i){history.pushState(null,"",i),window.dispatchEvent(new CustomEvent("location-changed"))}function qn(){let i=window;i[ua]||(i[ua]=!0,document.addEventListener("ll-custom",a=>{let e=a.detail;if(!e||typeof e.type!="string"||!e.type.startsWith("maintenance-supporter:"))return;let t=e.type.slice(22);if(t==="add-object"){if(We())return;he("/maintenance-supporter?ms_action=add_object");return}if(t==="open-task"&&typeof e.entry_id=="string"&&typeof e.task_id=="string"){if(Qe(e.entry_id,e.task_id)||Ge(e.entry_id,e.task_id))return;he(`/maintenance-supporter?entry_id=${encodeURIComponent(e.entry_id)}&task_id=${encodeURIComponent(e.task_id)}`);return}if(t==="open-object"&&typeof e.entry_id=="string"){if(Ze(e.entry_id))return;he(`/maintenance-supporter?entry_id=${encodeURIComponent(e.entry_id)}`);return}if(t==="deep-link"&&typeof e.path=="string"){he(e.path);return}if(t==="edit-history"&&typeof e.entry_id=="string"&&typeof e.task_id=="string"&&typeof e.original_timestamp=="string"){let r=document.querySelector("home-assistant")?.hass;if(!r)return;(async()=>{try{let u=(await r.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:e.entry_id})).tasks?.find(g=>g.id===e.task_id)?.history?.find(g=>g.timestamp===e.original_timestamp);if(!u)return;Ke({entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.original_timestamp,type:u.type||"completed",timestamp:u.timestamp||e.original_timestamp,notes:u.notes??null,cost:u.cost??null,duration:u.duration??null,completed_by:u.completed_by??null})}catch{he("/maintenance-supporter")}})();return}}))}qn();var qe=window;qe.customStrategies=qe.customStrategies||[];function ve(i){qe.customStrategies.some(e=>e.type===i.type&&e.strategyType===i.strategyType)||qe.customStrategies.push(i)}ve({type:pa,strategyType:"dashboard",name:"Maintenance Supporter",description:"Auto-generated dashboard. Group views by area, status, floor, or due date \u2014 picked from the strategy editor or YAML.",documentationURL:"https://github.com/iluebbe/maintenance_supporter#dashboard-strategy"});ve({type:ga,strategyType:"section",name:"Maintenance Supporter \u2014 Section",description:"Embed maintenance tasks (filterable by area, status, due date) as a section in any dashboard view.",documentationURL:"https://github.com/iluebbe/maintenance_supporter#section-strategy"});ve({type:"maintenance-supporter-vacation",strategyType:"section",name:"Maintenance Supporter \u2014 Vacation Status",description:"Compact vacation-mode status banner. Tap to open settings in the panel."});ve({type:"maintenance-supporter-budget",strategyType:"section",name:"Maintenance Supporter \u2014 Budget Status",description:"Monthly + yearly maintenance budget overview. Tap for details."});ve({type:"maintenance-supporter-groups",strategyType:"section",name:"Maintenance Supporter \u2014 Groups",description:"List of configured task groups with member counts."});
