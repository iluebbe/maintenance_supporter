...
`),this._scheduleTime=t.schedule_time||"";let n=t.on_complete_action;if(n&&n.service){this._actionService=n.service;let l=n.target?.entity_id;this._actionTargetEntity=Array.isArray(l)?l[0]||"":l||"",this._actionData=n.data&&typeof n.data=="object"?{...n.data}:{},this._actionDataJsonFallback=""}else this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="";let r=t.quick_complete_defaults;this._qcNotes=r?.notes||"",this._qcCost=r?.cost!=null?String(r.cost):"",this._qcDuration=r?.duration!=null?String(r.duration):"",this._qcFeedback=r?.feedback||"";let d=t.adaptive_config||{};if(this._environmentalEntity=d.environmental_entity||"",this._environmentalAttribute=d.environmental_attribute||"",this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute,t.trigger_config){let l=t.trigger_config;this._triggerEntityId=l.entity_id||"",this._triggerEntityIds=l.entity_ids||(l.entity_id?[l.entity_id]:[]),this._triggerEntityLogic=l.entity_logic||"any",this._triggerAttribute=l.attribute||"",this._triggerType=l.type||"threshold",this._triggerAbove=l.trigger_above?.toString()||"",this._triggerBelow=l.trigger_below?.toString()||"",this._triggerForMinutes=l.trigger_for_minutes?.toString()||"0",this._triggerTargetValue=l.trigger_target_value?.toString()||"",this._triggerDeltaMode=l.trigger_delta_mode||!1,this._triggerFromState=l.trigger_from_state||"",this._triggerToState=l.trigger_to_state||"",this._triggerTargetChanges=l.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=l.trigger_runtime_hours?.toString()||""}else this._resetTriggerFields();this._triggerEntityId&&this._fetchEntityAttributes(this._triggerEntityId),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._warningDays=String(this.defaultWarningDays),this._intervalAnchor="completion",this._notes="",this._documentationUrl="",this._customIcon="",this._enabled=!0,this._lastPerformed="",this._nfcTagId="",this._responsibleUserId=null,this._checklistText="",this._scheduleTime="",this._environmentalEntity="",this._environmentalAttribute="",this._environmentalInitial="",this._environmentalAttributeInitial="",this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="",this._actionTesting=!1,this._actionTestResult="",this._qcNotes="",this._qcCost="",this._qcDuration="",this._qcFeedback="",this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerForMinutes="0",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours=""}async _loadUsers(){this._userService||(this._userService=new be(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}async _testAction(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e)){this._actionTestResult="error";return}let[t,n]=e.split("."),r={...this._buildActionData()},d=this._actionTargetEntity.trim();d&&(r.entity_id=d),this._actionTesting=!0,this._actionTestResult="";try{await this.hass.callService(t,n,r),this._actionTestResult="ok"}catch{this._actionTestResult="error"}finally{this._actionTesting=!1,setTimeout(()=>{this._actionTestResult=""},3e3)}}_buildActionData(){if(this._actionDataJsonFallback.trim())try{let e=JSON.parse(this._actionDataJsonFallback);if(e&&typeof e=="object"&&!Array.isArray(e))return e}catch{}return{...this._actionData}}_serviceSchema(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e))return null;let[t,n]=e.split("."),r=this.hass?.services?.[t]?.[n]?.fields;return!r||Object.keys(r).length===0?null:Object.entries(r).map(([d,l])=>({name:d,required:!!l.required,selector:l.selector||{text:{}}}))}_renderCompletionActionsSection(e){if(!this.completionActionsEnabled)return p;let t=this._serviceSchema();return u`
      <details class="ca-section">
        ...
        <ha-service-picker
          ...
          @value-changed=${n=>{this._actionService=n.detail.value||"";let r=this._serviceSchema();if(r){let d=new Set(r.map(l=>l.name));this._actionData=Object.fromEntries(Object.entries(this._actionData).filter(([l])=>d.has(l)))}}}
        ></ha-service-picker>
        <ha-entity-picker
          .hass=${this.hass}
        ...
        <div class="ca-test-row">
          ...
          ${this._actionTestResult==="ok"?u`<span class="ca-test-ok">${o("on_complete_action_test_success",e)}</span>`:p}
          ${this._actionTestResult==="error"?u`<span class="ca-test-error">${o("on_complete_action_test_failed",e)}</span>`:p}
        </div>
      </details>
...
`).map(l=>l.trim()).filter(Boolean).slice(0,100);e.checklist=d.length?d:null}if(this.completionActionsEnabled){let d=this._actionService.trim();if(d&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(d)){let v={service:d},f=this._actionTargetEntity.trim();f&&(v.target={entity_id:f});let A=this._buildActionData();Object.keys(A).length>0&&(v.data=A),e.on_complete_action=v}else e.on_complete_action=null;let l={};this._qcNotes.trim()&&(l.notes=this._qcNotes.trim());let _=parseFloat(this._qcCost);!isNaN(_)&&_>=0&&(l.cost=_);let g=parseInt(this._qcDuration,10);!isNaN(g)&&g>=0&&(l.duration=g),this._qcFeedback&&(l.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(l).length?l:null}let t=await this.hass.connection.sendMessagePromise(e),n=this._taskId||t?.task_id,r=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(n&&this._scheduleType==="sensor_based"&&r)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:n,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=y(e,this._lang,o("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return p;let e=this._lang;return u`
      ...
      <ha-textfield
        ...
        @input=${t=>{let r=t.target.value.split(",").map(d=>d.trim()).filter(Boolean);this._triggerEntityId=r[0]||"",this._triggerEntityIds=r,r[0]&&this._fetchEntityAttributes(r[0])}}
      ></ha-textfield>
      ${this._triggerEntityIds.length>1?u`
      ...
      `:p}
      ${this._availableAttributes.length>0?u`
          <div class="select-row">
            ...
            >
              ...
              ${this._suggestedAttributes.map(t=>u`<option value=${t} ?selected=${t===this._triggerAttribute}>${t} ★</option>`)}
              ${this._availableAttributes.filter(t=>!this._suggestedAttributes.includes(t.name)).map(t=>u`<option value=${t.name} ?selected=${t.name===this._triggerAttribute}>${t.name}${t.numeric?"":" (non-numeric)"}</option>`)}
            </select>
          </div>
        `:u`
          <ha-textfield
      ...
      <div class="select-row">
        ...
        >
          ${Aa.map(t=>u`<option value=${t} ?selected=${t===this._triggerType}>${o(t,e)}</option>`)}
        </select>
      </div>
      ...
      ></ha-textfield>
    `}_renderTriggerTypeFields(){let e=this._lang;return this._triggerType==="threshold"?u`
        <ha-textfield
      ...
      `:this._triggerType==="state_change"?u`
        <ha-textfield
      ...
      `:this._triggerType==="runtime"?u`
        ...
        ></ha-textfield>
      `:p}render(){if(!this._open)return u``;let e=this._lang,t=this._taskId?o("edit_task",e):o("new_task",e);return u`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
          ${this._error?u`<div class="error">${this._error}</div>`:p}
          ${this._objectChoices.length>0?u`
            <div class="select-row">
              ...
              >
                ${this._objectChoices.map(n=>u`<option value=${n.entry_id} ?selected=${n.entry_id===this._entryId}>${n.name}</option>`)}
              </select>
            </div>
          `:p}
          <ha-textfield
          ...
          <div class="select-row">
            ...
            >
              ${za.map(n=>u`<option value=${n} ?selected=${n===this._type}>${o(n,e)}</option>`)}
            </select>
          </div>
          <div class="select-row">
            ...
            >
              ${Ea.map(n=>u`<option value=${n} ?selected=${n===this._scheduleType}>${o(n,e)}</option>`)}
            </select>
          </div>
          ${this._scheduleType==="time_based"?u`
                <ha-textfield
          ...
          <div class="select-row">
            ...
            >
              ...
              ${this._availableUsers.map(n=>u`<option value=${n.id} ?selected=${n.id===this._responsibleUserId}>${n.name}</option>`)}
            </select>
          </div>
          ${this._renderTriggerFields()}
          ${this._scheduleType==="sensor_based"?u`
            <ha-textfield
          ...
          ></ha-icon-picker>
          ${this._availableTags.length>0?u`
              <div class="select-row">
                ...
                >
                  ...
                  ${this._availableTags.map(n=>u`<option value=${n.id} ?selected=${n.id===this._nfcTagId}>${n.name}</option>`)}
                </select>
  ...
  `,s([h({attribute:!1})],m.prototype,"hass",2),s([h({type:Boolean,attribute:"checklists-enabled"})],m.prototype,"checklistsEnabled",2),s([h({type:Boolean,attribute:"schedule-time-enabled"})],m.prototype,"scheduleTimeEnabled",2),s([h({type:Boolean,attribute:"completion-actions-enabled"})],m.prototype,"completionActionsEnabled",2),s([h({type:Number,attribute:"default-warning-days"})],m.prototype,"defaultWarningDays",2),s([c()],m.prototype,"_open",2),s([c()],m.prototype,"_loading",2),s([c()],m.prototype,"_error",2),s([c()],m.prototype,"_entryId",2),s([c()],m.prototype,"_taskId",2),s([c()],m.prototype,"_objectChoices",2),s([c()],m.prototype,"_name",2),s([c()],m.prototype,"_type",2),s([c()],m.prototype,"_scheduleType",2),s([c()],m.prototype,"_intervalDays",2),s([c()],m.prototype,"_warningDays",2),s([c()],m.prototype,"_intervalAnchor",2),s([c()],m.prototype,"_notes",2),s([c()],m.prototype,"_documentationUrl",2),s([c()],m.prototype,"_customIcon",2),s([c()],m.prototype,"_enabled",2),s([c()],m.prototype,"_triggerEntityId",2),s([c()],m.prototype,"_triggerEntityIds",2),s([c()],m.prototype,"_triggerEntityLogic",2),s([c()],m.prototype,"_triggerAttribute",2),s([c()],m.prototype,"_triggerType",2),s([c()],m.prototype,"_triggerAbove",2),s([c()],m.prototype,"_triggerBelow",2),s([c()],m.prototype,"_triggerForMinutes",2),s([c()],m.prototype,"_triggerTargetValue",2),s([c()],m.prototype,"_triggerDeltaMode",2),s([c()],m.prototype,"_triggerFromState",2),s([c()],m.prototype,"_triggerToState",2),s([c()],m.prototype,"_triggerTargetChanges",2),s([c()],m.prototype,"_triggerRuntimeHours",2),s([c()],m.prototype,"_suggestedAttributes",2),s([c()],m.prototype,"_availableAttributes",2),s([c()],m.prototype,"_entityDomain",2),s([c()],m.prototype,"_lastPerformed",2),s([c()],m.prototype,"_nfcTagId",2),s([c()],m.prototype,"_availableTags",2),s([c()],m.prototype,"_responsibleUserId",2),s([c()],m.prototype,"_availableUsers",2),s([c()],m.prototype,"_checklistText",2),s([c()],m.prototype,"_scheduleTime",2),s([c()],m.prototype,"_actionService",2),s([c()],m.prototype,"_actionTargetEntity",2),s([c()],m.prototype,"_actionData",2),s([c()],m.prototype,"_actionDataJsonFallback",2),s([c()],m.prototype,"_actionTesting",2),s([c()],m.prototype,"_actionTestResult",2),s([c()],m.prototype,"_qcNotes",2),s([c()],m.prototype,"_qcCost",2),s([c()],m.prototype,"_qcDuration",2),s([c()],m.prototype,"_qcFeedback",2),s([c()],m.prototype,"_environmentalEntity",2),s([c()],m.prototype,"_environmentalAttribute",2);customElements.get("maintenance-task-dialog")||customElements.define("maintenance-task-dialog",m)});var x,St=b(()=>{"use strict";$();T();q();I();x=class extends k{constructor(){super(...arguments);this.entryId="";this.taskId="";this.taskName="";this.lang="en";this.checklist=[];this.adaptiveEnabled=!1;this._open=!1;this._notes="";this._cost="";this._duration="";this._loading=!1;this._error="";this._checklistState={};this._feedback="needed"}open(){this._open||(this._open=!0,this._notes="",this._cost="",this._duration="",this._error="",this._checklistState={},this._feedback="needed")}_toggleCheck(e){let t=String(e);this._checklistState={...this._checklistState,[t]:!this._checklistState[t]}}_setFeedback(e){this._feedback=e}async _complete(){this._loading=!0,this._error="";try{let e={type:"maintenance_supporter/task/complete",entry_id:this.entryId,task_id:this.taskId};if(this._notes&&(e.notes=this._notes),this._cost){let t=parseFloat(this._cost);!isNaN(t)&&t>=0&&(e.cost=t)}if(this._duration){let t=parseInt(this._duration,10);!isNaN(t)&&t>=0&&(e.duration=t)}this.checklist.length>0&&(e.checklist_state=this._checklistState),this.adaptiveEnabled&&(e.feedback=this._feedback),await this.hass.connection.sendMessagePromise(e),this._open=!1,this.dispatchEvent(new CustomEvent("task-completed"))}catch(e){this._error=y(e,this.lang,o("save_error",this.lang))}finally{this._loading=!1}}_close(){this._open=!1}render(){if(!this._open)return u``;let e=this.lang||this.hass?.language||"en";return u`
      <ha-dialog open @closed=${this._close}>
  ...
  `,s([h({attribute:!1})],x.prototype,"hass",2),s([h()],x.prototype,"entryId",2),s([h()],x.prototype,"taskId",2),s([h()],x.prototype,"taskName",2),s([h()],x.prototype,"lang",2),s([h({type:Array})],x.prototype,"checklist",2),s([h({type:Boolean})],x.prototype,"adaptiveEnabled",2),s([c()],x.prototype,"_open",2),s([c()],x.prototype,"_notes",2),s([c()],x.prototype,"_cost",2),s([c()],x.prototype,"_duration",2),s([c()],x.prototype,"_loading",2),s([c()],x.prototype,"_error",2),s([c()],x.prototype,"_checklistState",2),s([c()],x.prototype,"_feedback",2);customElements.get("maintenance-complete-dialog")||customElements.define("maintenance-complete-dialog",x)});var M,$t=b(()=>{"use strict";$();T();q();I();M=class extends k{constructor(){super(...arguments);this._open=!1;this._saving=!1;this._error="";this._draft=null;this._originalSnapshot=null}get _lang(){return this.hass?.language||"en"}openEdit(e){this._draft={...e},this._originalSnapshot={...e},this._error="",this._open=!0}close(){this._open=!1,this._error="",this._draft=null,this._originalSnapshot=null}_set(e,t){this._draft&&(this._draft={...this._draft,[e]:t})}async _save(){if(!(!this._draft||!this._originalSnapshot)){this._saving=!0,this._error="";try{let e={type:"maintenance_supporter/task/history/update",entry_id:this._draft.entry_id,task_id:this._draft.task_id,original_timestamp:this._originalSnapshot.original_timestamp};if(this._draft.timestamp!==this._originalSnapshot.timestamp&&(e.timestamp=this._draft.timestamp),this._draft.notes!==this._originalSnapshot.notes&&(e.notes=this._draft.notes),this._draft.cost!==this._originalSnapshot.cost&&(e.cost=this._draft.cost),this._draft.duration!==this._originalSnapshot.duration&&(e.duration=this._draft.duration),this._draft.completed_by!==this._originalSnapshot.completed_by&&(e.completed_by=this._draft.completed_by),Object.keys(e).filter(n=>!["type","entry_id","task_id","original_timestamp"].includes(n)).length===0){this.close();return}await this.hass.connection.sendMessagePromise(e),this.dispatchEvent(new CustomEvent("history-entry-saved",{detail:{entry_id:this._draft.entry_id,task_id:this._draft.task_id,new_timestamp:this._draft.timestamp},bubbles:!0,composed:!0})),this.close()}catch(e){this._error=y(e,this._lang)}finally{this._saving=!1}}}render(){if(!this._open||!this._draft)return p;let e=this._lang,t=this._draft;return u`
...
<div class="qr-row">
  <div class="qr-col">
    <img src="${qt(this._viewResult.svg_data_uri)}" alt="QR Info" />
    <div class="qr-label">${v}</div>
  </div>
  ${g?`<div class="qr-col">
    <img src="${qt(this._completeResult.svg_data_uri)}" alt="QR Complete" />
    <div class="qr-label">${f}</div>
  </div>`:""}
</div>
<div class="url">${Y(this._viewResult.url)}</div>
<script>setTimeout(()=>window.print(),300)<\/script>
</body></html>`),r.document.close()}_downloadSvg(e,t){let n=decodeURIComponent(e.svg_data_uri.replace("data:image/svg+xml,","")),r=new Blob([n],{type:"image/svg+xml"}),d=URL.createObjectURL(r),l=document.createElement("a");l.href=d;let _=this._taskName?`${this._objectName}-${this._taskName}`:this._objectName;l.download=`qr-${ja(_)}-${t}.svg`,l.click(),URL.revokeObjectURL(d)}_close(){this._open=!1,this._viewResult=null,this._completeResult=null,this._error="",this._loading=!1}render(){if(!this._open)return u``;let e=this.lang||this.hass?.language||"en",t=this._taskName?`${o("qr_code",e)}: ${this._objectName} \u2014 ${this._taskName}`:`${o("qr_code",e)}: ${this._objectName}`,n=!!this._viewResult;return u`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
          ${this._loading?u`<div class="loading">${o("qr_generating",e)}</div>`:this._error?u`<div class="error">${this._error}</div>`:n?u`
                    <div class="qr-pair">
                      <div class="qr-item">
                        <img
  ...
  `,s([h({attribute:!1})],C.prototype,"hass",2),s([h()],C.prototype,"lang",2),s([c()],C.prototype,"_open",2),s([c()],C.prototype,"_loading",2),s([c()],C.prototype,"_error",2),s([c()],C.prototype,"_viewResult",2),s([c()],C.prototype,"_completeResult",2),s([c()],C.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",C)});var E,Ct=b(()=>{"use strict";$();T();q();I();E=class extends k{constructor(){super(...arguments);this._open=!1;this._entryId=null;this._taskId=null;this._task=null;this._objectName="";this._busy=!1;this._error="";this._showSkip=!1;this._showReset=!1;this._showDetails=!1;this._skipReason="";this._resetDate=""}get _lang(){return this.hass?.language||"en"}async openFor(e,t){this._entryId=e,this._taskId=t,this._error="",this._showSkip=!1,this._showReset=!1,this._skipReason="",this._resetDate=new Date().toISOString().slice(0,10),this._open=!0,await this._loadTask()}close(){this._open=!1,this._task=null,this._error=""}async _loadTask(){if(!(!this._entryId||!this._taskId))try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._objectName=e.object?.name||"";let t=(e.tasks||[]).find(n=>n.id===this._taskId);this._task=t??null}catch(e){this._error=y(e,this._lang)}}async _runWs(e){this._busy=!0,this._error="";try{return await this.hass.connection.sendMessagePromise(e),this._busy=!1,!0}catch(t){return this._error=y(t,this._lang),this._busy=!1,!1}}_notifyChanged(e){this.dispatchEvent(new CustomEvent("task-action-fired",{detail:{entry_id:this._entryId,task_id:this._taskId,action:e},bubbles:!0,composed:!0}))}_onComplete(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(R(),F)).then(({openCompleteDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,checklist:this._task.checklist||[],adaptive_enabled:!!this._task.adaptive_config?.enabled})&&(this._notifyChanged("complete"),this.close())})}async _onSkipConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/skip",entry_id:this._entryId,task_id:this._taskId,reason:this._skipReason.trim()||null})&&(this._notifyChanged("skip"),this.close())}async _onResetConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/reset",entry_id:this._entryId,task_id:this._taskId,date:this._resetDate||void 0})&&(this._notifyChanged("reset"),this.close())}_onEdit(){!this._entryId||!this._taskId||Promise.resolve().then(()=>(R(),F)).then(({openEditTaskDialog:e})=>{e(this._entryId,this._taskId),this.close()})}_onQr(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(R(),F)).then(({openQrDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,object_name:this._objectName}),this.close()})}async _onDelete(){if(!this._entryId||!this._taskId)return;let e=o("delete_task_confirm",this._lang)||`Delete "${this._task?.name}"?`;if(!window.confirm(e))return;await this._runWs({type:"maintenance_supporter/task/delete",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("delete"),this.close())}_onOpenInPanel(){if(!this._entryId||!this._taskId)return;let e=`/maintenance-supporter?entry_id=${encodeURIComponent(this._entryId)}&task_id=${encodeURIComponent(this._taskId)}`;history.pushState(null,"",e),window.dispatchEvent(new CustomEvent("location-changed")),this.close()}_onEditHistoryEntry(e){!this._entryId||!this._taskId||Promise.resolve().then(()=>(R(),F)).then(({openHistoryEditDialog:t})=>{t({entry_id:this._entryId,task_id:this._taskId,original_timestamp:e.timestamp,type:e.type,timestamp:e.timestamp,notes:e.notes??null,cost:e.cost??null,duration:e.duration??null,completed_by:e.completed_by??null})})}_renderDetails(e){let t=this._lang,n=e.history||[],r=n.filter(_=>_.type==="completed"),d=r.reduce((_,g)=>_+(typeof g.cost=="number"?g.cost:0),0),l=(()=>{let _=r.map(g=>typeof g.duration=="number"?g.duration:null).filter(g=>g!=null);return _.length?Math.round(_.reduce((g,v)=>g+v,0)/_.length):null})();return u`
      <div class="details">
        <div class="stats-grid">
          ...
          <div class="stat">
            ...
            <span class="stat-value">${l!=null?`${l}m`:"\u2014"}</span>
          </div>
        </div>
        ...
        ${n.length===0?u`<div class="history-empty">${o("history_empty",t)||"No history yet."}</div>`:u`
              <div class="history-list">
                ${[...n].reverse().slice(0,20).map(_=>{let g=["completed","reset","skipped"].includes(_.type);return u`
                    <div class="history-entry">
                      <div class="history-line">
                        <span class="history-type type-${_.type}">${o(_.type,t)}</span>
                        <span class="history-date">${zt(_.timestamp,t)}</span>
                        ${g?u`<button class="history-edit"
                                   title="${o("history_edit_button",t)||"Edit"}"
                                   @click=${()=>this._onEditHistoryEntry(_)}>
                              <ha-icon icon="mdi:pencil"></ha-icon>
                            </button>`:p}
                      </div>
                      ${_.notes?u`<div class="history-notes">${_.notes}</div>`:p}
                      ${_.cost!=null||_.duration!=null?u`<div class="history-meta">
                            ${_.cost!=null?u`<span>💰 ${_.cost.toFixed(2)}</span>`:p}
                            ${_.duration!=null?u`<span>⏱️ ${_.duration}m</span>`:p}
                          </div>`:p}
                    </div>
                  `})}
                ${n.length>20?u`<div class="history-more">… +${n.length-20} ${o("older_entries",t)||"older"}</div>`:p}
              </div>
            `}
      </div>
    `}render(){if(!this._open)return p;let e=this._lang,t=this._task,n=this.hass?.user?.is_admin??!0;return u`
      <div class="backdrop" @click=${this.close}></div>
      <div class="dialog" role="dialog" aria-modal="true">
        ${t?u`
              <div class="header">
                ...
                <div class="object">
                  <button class="link-inline" @click=${()=>{this._entryId&&Promise.resolve().then(()=>(R(),F)).then(({openObjectQuickActions:r})=>{r(this._entryId),this.close()})}}>${this._objectName}</button>
                </div>
                <div class="quick-info">
                  ${t.next_due?u`<span><strong>${o("next_due",e)||"Next due"}:</strong> ${Re(t.next_due,e)}</span>`:p}
                  ${t.last_performed?u`<span><strong>${o("last_performed",e)||"Last"}:</strong> ${Re(t.last_performed,e)}</span>`:p}
                  ${t.interval_days!=null?u`<span><strong>${o("interval",e)||"Interval"}:</strong> ${t.interval_days}d</span>`:p}
                </div>
              </div>
              ...
              ${this._showSkip?u`
                  ...
                  `}
            `:u`<div class="loading">${o("loading",e)||"Loading\u2026"}</div>`}
      </div>
    `}};E.styles=w`
    :host { display: contents; }
    .backdrop {
  ...
  `,s([h({attribute:!1})],E.prototype,"hass",2),s([c()],E.prototype,"_open",2),s([c()],E.prototype,"_entryId",2),s([c()],E.prototype,"_taskId",2),s([c()],E.prototype,"_task",2),s([c()],E.prototype,"_objectName",2),s([c()],E.prototype,"_busy",2),s([c()],E.prototype,"_error",2),s([c()],E.prototype,"_showSkip",2),s([c()],E.prototype,"_showReset",2),s([c()],E.prototype,"_showDetails",2),s([c()],E.prototype,"_skipReason",2),s([c()],E.prototype,"_resetDate",2);customElements.get("maintenance-task-quick-actions-dialog")||customElements.define("maintenance-task-quick-actions-dialog",E)});var D,Tt=b(()=>{"use strict";$();T();q();I();D=class extends k{constructor(){super(...arguments);this._open=!1;this._entryId=null;this._data=null;this._busy=!1;this._error=""}get _lang(){return this.hass?.language||"en"}async openFor(e){this._entryId=e,this._error="",this._open=!0,await this._load()}close(){this._open=!1,this._data=null,this._error=""}async _load(){if(this._entryId)try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._data=e}catch(e){this._error=y(e,this._lang)}}_onEditObject(){!this._entryId||!this._data||Promise.resolve().then(()=>(R(),F)).then(({openEditObjectDialog:e})=>{e(this._entryId,this._data.object),this.close()})}_onAddTask(){this._entryId&&Promise.resolve().then(()=>(R(),F)).then(({openCreateTaskDialog:e})=>{e(),this.close()})}async _onDelete(){if(!this._entryId||!this._data)return;let e=o("delete_object_confirm",this._lang)||`Delete "${this._data.object.name}" and all its tasks?`;if(window.confirm(e)){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:this._entryId}),this.dispatchEvent(new CustomEvent("object-deleted",{detail:{entry_id:this._entryId},bubbles:!0,composed:!0})),this.close()}catch(t){this._error=y(t,this._lang)}finally{this._busy=!1}}}_onTaskClick(e){this._entryId&&Promise.resolve().then(()=>(R(),F)).then(({openTaskQuickActions:t})=>{t(this._entryId,e)})}render(){if(!this._open)return p;let e=this._lang,t=this._data,n=t?.object,r=t?.tasks||[],d=this.hass?.user?.is_admin??!0;return u`
      <div class="backdrop" @click=${this.close}></div>
      <div class="dialog" role="dialog" aria-modal="true">
        ${t&&n?u`
              <div class="header">
              ...
              <div class="tasks-section">
                ...
                ${r.length===0?u`<div class="empty">${o("no_tasks",e)||"No tasks yet."}</div>`:u`
                      <div class="task-list">
                        ${r.map(l=>u`
                          <div class="task-row" @click=${()=>this._onTaskClick(l.id)}>
                            <span class="status-dot" style="background: ${ve[l.status]||"#ccc"}"></span>
                            <span class="task-name">${l.name}</span>
                            <span class="task-status">${o(l.status||"ok",e)}</span>
                          </div>
                        `)}
                      </div>
                    `}
    ...
    `}_renderMetaRow(e){let t=this._lang,n=[];return e.area_id&&n.push([o("area",t)||"Area",e.area_id]),e.manufacturer&&n.push([o("manufacturer",t)||"Manufacturer",e.manufacturer]),e.model&&n.push([o("model",t)||"Model",e.model]),e.serial_number&&n.push([o("serial_number",t)||"Serial",e.serial_number]),e.installation_date&&n.push([o("installation_date",t)||"Installed",e.installation_date]),e.documentation_url&&n.push([o("documentation_url",t)||"Docs",e.documentation_url]),n.length===0?p:u`
      <div class="meta">
        ${n.map(([r,d])=>u`
            <div class="meta-item">
              <span class="meta-label">${r}</span>
              <span class="meta-value">${r.toLowerCase().includes("doc")||r.toLowerCase().includes("url")?u`<a href="${d}" target="_blank" rel="noopener">${d}</a>`:d}</span>
            </div>
          `)}
      </div>
    `}};D.styles=w`
  ...
  `,s([h({attribute:!1})],D.prototype,"hass",2),s([c()],D.prototype,"_open",2),s([c()],D.prototype,"_entryId",2),s([c()],D.prototype,"_data",2),s([c()],D.prototype,"_busy",2),s([c()],D.prototype,"_error",2);customElements.get("maintenance-object-quick-actions-dialog")||customElements.define("maintenance-object-quick-actions-dialog",D)});var F={};Wt(F,{openCompleteDialog:()=>Ra,openCreateObjectDialog:()=>De,openCreateTaskDialog:()=>Ia,openEditObjectDialog:()=>Ta,openEditTaskDialog:()=>Pe,openHistoryEditDialog:()=>Me,openObjectQuickActions:()=>Le,openQrDialog:()=>Da,openTaskQuickActions:()=>Fe});function Dt(){return document.querySelector("home-assistant")?.hass}function L(i){let a=document.body.querySelector(i);return a||(a=document.createElement(i),document.body.appendChild(a)),a}function O(i){let a=Dt();return a?(i.hass=a,!0):!1}function De(){let i=L(It);return O(i)?(i.openCreate(),!0):!1}function Ta(i,a){let e=L(It);return O(e)?(e.openEdit(i,a),!0):!1}function Ia(){let i=L(Rt);return O(i)?(i.openCreate(),!0):!1}function Pe(i,a){let e=L(Rt);return O(e)?(e.openEdit(i,{id:a}),!0):!1}function Me(i){let a=L(Sa);return O(a)?(a.openEdit(i),!0):!1}function Ra(i){let a=L($a);return O(a)?(a.entryId=i.entry_id,a.taskId=i.task_id,a.taskName=i.task_name,a.checklist=i.checklist??[],a.adaptiveEnabled=!!i.adaptive_enabled,a.lang=Dt()?.language||"en",a.open(),!0):!1}function Da(i){let a=L(qa);return O(a)?(a.openForTask(i.entry_id,i.task_id,i.object_name,i.task_name),!0):!1}function Fe(i,a){let e=L(Na);return O(e)?(e.openFor(i,a),!0):!1}function Le(i){let a=L(Ca);return O(a)?(a.openFor(i),!0):!1}var It,Rt,Sa,$a,qa,Na,Ca,R=b(()=>{"use strict";Et();jt();St();$t();Nt();Ct();Tt();It="maintenance-object-dialog",Rt="maintenance-task-dialog",Sa="maintenance-history-edit-dialog",$a="maintenance-complete-dialog",qa="maintenance-qr-dialog",Na="maintenance-task-quick-actions-dialog",Ca="maintenance-object-quick-actions-dialog"});$();T();q();I();var j=class extends k{constructor(){super(...arguments);this._config={type:""};this._state=null;this._busy=!1;this._error="";this._localStart="";this._localEnd="";this._localBuffer=7;this._dirty=!1;this._loaded=!1}setConfig(e){this._config=e}getCardSize(){return 2}get _lang(){return this.hass?.language||"en"}get _isAdmin(){return this.hass?.user?.is_admin??!0}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._loaded&&(this._loaded=!0,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/vacation/state"});this._state=e,this._localStart=e.start||"",this._localEnd=e.end||"",this._localBuffer=e.buffer_days??7,this._dirty=!1}catch(e){this._error=y(e,this._lang)}}async _toggleEnabled(e){this._busy=!0,this._error="";try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/vacation/update",enabled:e});this._state=t}catch(t){this._error=y(t,this._lang)}finally{this._busy=!1}}async _save(){if(this._isAdmin){this._busy=!0,this._error="";try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/vacation/update",start:this._localStart||null,end:this._localEnd||null,buffer_days:this._localBuffer});this._state=e,this._dirty=!1}catch(e){this._error=y(e,this._lang)}finally{this._busy=!1}}}async _endNow(){if(this._isAdmin&&window.confirm(o("vacation_end_now_confirm",this._lang)||"End vacation immediately?")){this._busy=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/vacation/end_now"});this._state=e,this._localStart=e.start||"",this._localEnd=e.end||""}catch(e){this._error=y(e,this._lang)}finally{this._busy=!1}}}_onDeepLink(){history.pushState(null,"","/maintenance-supporter?ms_action=open_vacation"),window.dispatchEvent(new CustomEvent("location-changed"))}render(){let e=this._lang,t=this._state;if(!t)return u`<ha-card><div class="loading">${o("loading",e)||"Loading\u2026"}</div></ha-card>`;let n=t.is_active===!0,r=t.enabled===!0,d=t.exempt_task_ids?.length??0,l=n?o("vacation_status_active",e)||"Active now":r?o("vacation_status_scheduled",e)||"Scheduled":o("vacation_status_inactive",e)||"Inactive",_=n?"active":r?"scheduled":"inactive";return u`
      <ha-card>
        <div class="card-content">
          <div class="header">
            ...
            <span class="status-pill ${_}">${l}</span>
          </div>
          ...
          ${this._isAdmin?u`
                ...
                <div class="actions">
                  ...
                  ${n?u`<button class="btn"
                        @click=${this._endNow}
                        ?disabled=${this._busy}>
                        ${o("vacation_end_now",e)||"End now"}
                      </button>`:p}
                  ${d>0?u`<button class="btn link"
                        @click=${this._onDeepLink}>
                        ${d} ${o("vacation_exempt_count",e)||"exempt"}…
                      </button>`:u`<button class="btn link"
                        @click=${this._onDeepLink}>
              ...
              `:u`
                <div class="readonly">
                  ${r&&t.start&&t.end?u`<div>${t.start} → ${t.end}</div>`:p}
    ...
    .loading { padding: 24px; text-align: center; color: var(--secondary-text-color); }
  `,s([h({attribute:!1})],j.prototype,"hass",2),s([c()],j.prototype,"_config",2),s([c()],j.prototype,"_state",2),s([c()],j.prototype,"_busy",2),s([c()],j.prototype,"_error",2),s([c()],j.prototype,"_localStart",2),s([c()],j.prototype,"_localEnd",2),s([c()],j.prototype,"_localBuffer",2),s([c()],j.prototype,"_dirty",2);customElements.get("maintenance-vacation-section-card")||customElements.define("maintenance-vacation-section-card",j);window.customCards=window.customCards||[];window.customCards.push({type:"maintenance-vacation-section-card",name:"Maintenance Supporter \u2014 Vacation",description:"Inline vacation mode toggle + dates",preview:!1});$();T();q();I();var N=class extends k{constructor(){super(...arguments);this._config={type:""};this._status=null;this._busy=!1;this._error="";this._localMonthly="";this._localYearly="";this._dirty=!1;this._loaded=!1}setConfig(e){this._config=e}getCardSize(){return 2}get _lang(){return this.hass?.language||"en"}get _isAdmin(){return this.hass?.user?.is_admin??!0}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._loaded&&(this._loaded=!0,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/budget_status"});this._status=e,this._localMonthly=e.monthly_budget?String(e.monthly_budget):"",this._localYearly=e.yearly_budget?String(e.yearly_budget):"",this._dirty=!1}catch(e){this._error=y(e,this._lang)}}async _save(){if(this._isAdmin){this._busy=!0,this._error="";try{let e=parseFloat(this._localMonthly),t=parseFloat(this._localYearly),n={};!isNaN(e)&&e>=0&&(n.budget_monthly=e),!isNaN(t)&&t>=0&&(n.budget_yearly=t),await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/update",settings:n}),await this._load()}catch(e){this._error=y(e,this._lang)}finally{this._busy=!1}}}_onDeepLink(){history.pushState(null,"","/maintenance-supporter?ms_action=open_budget"),window.dispatchEvent(new CustomEvent("location-changed"))}render(){let e=this._lang,t=this._status;if(!t)return u`<ha-card><div class="loading">${o("loading",e)||"Loading\u2026"}</div></ha-card>`;let n=t.currency_symbol||"\u20AC",r=t.monthly_budget?Math.min(100,(t.monthly_spent||0)/t.monthly_budget*100):0,d=t.yearly_budget?Math.min(100,(t.yearly_spent||0)/t.yearly_budget*100):0,l=r>=100?"danger":r>=80?"warning":"ok",_=d>=100?"danger":d>=80?"warning":"ok";return u`
      <ha-card>
        <div class="card-content">
          <div class="header">
            <div class="title">
          ...
          <div class="track">
            <div class="track-label-row">
              ...
              <span class="track-numbers ${l}">
                ${(t.monthly_spent||0).toFixed(0)} / ${(t.monthly_budget||0).toFixed(0)} ${n}
              </span>
            </div>
            <div class="bar"><div class="bar-fill ${l}" style="width:${r}%"></div></div>
          </div>

          <div class="track">
            <div class="track-label-row">
              <label>${o("budget_yearly",e)||"Yearly"}</label>
              <span class="track-numbers ${_}">
                ${(t.yearly_spent||0).toFixed(0)} / ${(t.yearly_budget||0).toFixed(0)} ${n}
              </span>
            </div>
            <div class="bar"><div class="bar-fill ${_}" style="width:${d}%"></div></div>
          </div>
  ...
  `,s([h({attribute:!1})],N.prototype,"hass",2),s([c()],N.prototype,"_config",2),s([c()],N.prototype,"_status",2),s([c()],N.prototype,"_busy",2),s([c()],N.prototype,"_error",2),s([c()],N.prototype,"_localMonthly",2),s([c()],N.prototype,"_localYearly",2),s([c()],N.prototype,"_dirty",2);customElements.get("maintenance-budget-section-card")||customElements.define("maintenance-budget-section-card",N);window.customCards=window.customCards||[];window.customCards.push({type:"maintenance-budget-section-card",name:"Maintenance Supporter \u2014 Budget",description:"Inline monthly + yearly budget editor",preview:!1});$();T();q();I();var S=class extends k{constructor(){super(...arguments);this._config={type:""};this._groups={};this._loaded=!1;this._busy=!1;this._error="";this._newName="";this._editingId=null;this._editingName="";this._hasInitiallyLoaded=!1}setConfig(e){this._config=e}getCardSize(){return 2}get _lang(){return this.hass?.language||"en"}get _isAdmin(){return this.hass?.user?.is_admin??!0}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._hasInitiallyLoaded&&(this._hasInitiallyLoaded=!0,this._load())}async _load(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/groups"});this._groups=e.groups||{},this._loaded=!0}catch(e){this._error=y(e,this._lang)}}async _addGroup(){if(!this._isAdmin)return;let e=this._newName.trim();if(e){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/group/create",name:e}),this._newName="",await this._load()}catch(t){this._error=y(t,this._lang)}finally{this._busy=!1}}}_startEdit(e){this._editingId=e,this._editingName=this._groups[e]?.name||""}async _saveEdit(){if(!this._isAdmin||!this._editingId)return;let e=this._editingName.trim();if(e){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/group/update",group_id:this._editingId,name:e}),this._editingId=null,this._editingName="",await this._load()}catch(t){this._error=y(t,this._lang)}finally{this._busy=!1}}}async _deleteGroup(e,t){if(!this._isAdmin)return;let n=(o("group_delete_confirm",this._lang)||'Delete group "{name}"?').replace("{name}",t);if(window.confirm(n)){this._busy=!0;try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/group/delete",group_id:e}),await this._load()}catch(r){this._error=y(r,this._lang)}finally{this._busy=!1}}}_onDeepLink(){history.pushState(null,"","/maintenance-supporter?ms_action=open_groups"),window.dispatchEvent(new CustomEvent("location-changed"))}_onKeyDown(e,t){e.key==="Enter"?(e.preventDefault(),t()):e.key==="Escape"&&(e.preventDefault(),this._editingId=null,this._editingName="")}render(){let e=this._lang;if(!this._loaded)return u`<ha-card><div class="loading">${o("loading",e)||"Loading\u2026"}</div></ha-card>`;let t=Object.keys(this._groups);return u`
      <ha-card>
        <div class="card-content">
          ...
          ${t.length===0?u`<div class="empty">${o("groups_empty",e)||"No groups yet."}</div>`:u`
                <div class="group-list">
                  ${t.map(n=>{let r=this._groups[n],d=r.task_refs?.length??0,l=this._editingId===n;return u`
                      <div class="group-row">
                        ${l?u`
                              <input class="edit-input" type="text"
                                .value=${this._editingName}
                                ?disabled=${this._busy}
                                @input=${_=>{this._editingName=_.target.value}}
                                @keydown=${_=>this._onKeyDown(_,this._saveEdit.bind(this))} />
    ...
    .loading { padding: 24px; text-align: center; color: var(--secondary-text-color); }
  `,s([h({attribute:!1})],S.prototype,"hass",2),s([c()],S.prototype,"_config",2),s([c()],S.prototype,"_groups",2),s([c()],S.prototype,"_loaded",2),s([c()],S.prototype,"_busy",2),s([c()],S.prototype,"_error",2),s([c()],S.prototype,"_newName",2),s([c()],S.prototype,"_editingId",2),s([c()],S.prototype,"_editingName",2);customElements.get("maintenance-groups-section-card")||customElements.define("maintenance-groups-section-card",S);window.customCards=window.customCards||[];window.customCards.push({type:"maintenance-groups-section-card",name:"Maintenance Supporter \u2014 Groups",description:"Inline group CRUD",preview:!1});R();var Vt="maintenance-supporter",Pt=`ll-strategy-dashboard-${Vt}`,Oe="hui-maintenance-supporter-strategy-editor",Pa="NOT_RUNNING",Ma=[{status:"overdue",title:"Overdue",icon:"mdi:alert-circle",path:"overdue"},{status:"triggered",title:"Triggered",icon:"mdi:flash",path:"triggered"},{status:"due_soon",title:"Due Soon",icon:"mdi:clock-alert-outline",path:"due-soon"},{status:"ok",title:"OK",icon:"mdi:check-circle-outline",path:"ok"}],Mt=[{title:"Overdue",icon:"mdi:alert-circle",path:"overdue",filter:{filter_due_max_days:-1},matches:i=>i<=-1},{title:"Today",icon:"mdi:calendar-today",path:"today",filter:{filter_due_min_days:0,filter_due_max_days:0},matches:i=>i===0},{title:"This Week",icon:"mdi:calendar-week",path:"this-week",filter:{filter_due_min_days:1,filter_due_max_days:7},matches:i=>i>=1&&i<=7},{title:"This Month",icon:"mdi:calendar-month",path:"this-month",filter:{filter_due_min_days:8,filter_due_max_days:30},matches:i=>i>=8&&i<=30},{title:"Later",icon:"mdi:calendar-clock",path:"later",filter:{filter_due_min_days:31},matches:i=>i>=31}];function Q(i){return{type:"grid",cards:[i]}}var Fa={condition:"view_columns",min:2};function La(i){let a={overdue:0,triggered:0,due_soon:0,ok:0,total:0};for(let e of i)for(let t of e.tasks||[])a.total++,t.status==="overdue"?a.overdue++:t.status==="triggered"?a.triggered++:t.status==="due_soon"?a.due_soon++:t.status==="ok"&&a.ok++;return a}function Oa(i){let a=[];return i.overdue&&a.push(`\u{1F534} **${i.overdue}** overdue`),i.triggered&&a.push(`\u26A1 **${i.triggered}** triggered`),i.due_soon&&a.push(`\u{1F7E1} **${i.due_soon}** due soon`),a.push(`\u{1F7E2} **${i.ok}** ok`),{type:"markdown",text_only:!0,content:a.join(" \xB7 ")}}function Ha(){return{title:"Maintenance",type:"panel",cards:[{type:"empty-state",icon:"mdi:wrench-clock",icon_color:"primary",content_only:!0,title:"No maintenance objects yet",content:"Open the Maintenance panel to add your first object \u2014 pool pump, HVAC filter, vehicle, anything that needs scheduled care.",buttons:[{icon:"mdi:wrench",text:"Open Maintenance panel",appearance:"filled",variant:"brand",tap_action:{action:"navigate",navigation_path:"/maintenance-supporter"}},{icon:"mdi:plus",text:"Add object",appearance:"outlined",variant:"brand",tap_action:{action:"fire-dom-event",ll_custom:{type:"maintenance-supporter:add-object"}}}]}]}}function Ua(i){let a=Oa(i);return{title:"Overview",icon:"mdi:wrench-clock",path:"overview",type:"sections",header:{layout:"responsive",card:a},sidebar:{sections:[{type:"grid",cards:[{type:"heading",heading:"Status",heading_style:"title"},a]}],visibility:[Fa],content_label:"Tasks",sidebar_label:"Status"},sections:[Q({type:"custom:maintenance-supporter-card",show_header:!1,filter_status:["overdue","triggered","due_soon"]})]}}function Va(i,a){let e=new Map;for(let d of i){let l=d.object.area_id||null;e.has(l)||e.set(l,[]),e.get(l).push(d)}let t=Array.from(e.keys()).filter(d=>d!==null);t.sort((d,l)=>{let _=a[d]?.name||d,g=a[l]?.name||l;return _.localeCompare(g)});let n=[];for(let d of t){let l=e.get(d);if(l.length===0)continue;let _=a[d];n.push({title:_?.name||d,icon:_?.icon||"mdi:floor-plan",path:`area-${d}`,type:"sections",sections:[Q({type:"custom:maintenance-supporter-card",show_header:!1,filter_objects:l.map(g=>g.object.name)})]})}let r=e.get(null);return r&&r.length>0&&n.push({title:"Unassigned",icon:"mdi:help-circle-outline",path:"unassigned",type:"sections",sections:[Q({type:"custom:maintenance-supporter-card",show_header:!1,filter_objects:r.map(d=>d.object.name)})]}),n}function Ba(i){let a=new Set;for(let t of i)for(let n of t.tasks||[])n.status&&a.add(n.status);let e=[];for(let t of Ma)a.has(t.status)&&e.push({title:t.title,icon:t.icon,path:t.path,type:"sections",sections:[Q({type:"custom:maintenance-supporter-card",show_header:!1,filter_status:[t.status]})]});return e}function Ga(i,a,e){let t=new Map;for(let l of i){let _=l.object.area_id,g=_&&a[_]?.floor_id||null;t.has(g)||t.set(g,[]),t.get(g).push(l)}let n=Array.from(t.keys()).filter(l=>l!==null);n.sort((l,_)=>{let g=e[l],v=e[_],f=g?.level??0,A=v?.level??0;return f!==A?f-A:(g?.name||l).localeCompare(v?.name||_)});let r=[];for(let l of n){let _=t.get(l);if(_.length===0)continue;let g=e[l];r.push({title:g?.name||l,icon:g?.icon||"mdi:home-floor-1",path:`floor-${l}`,type:"sections",sections:[Q({type:"custom:maintenance-supporter-card",show_header:!1,filter_objects:_.map(v=>v.object.name)})]})}let d=t.get(null);return d&&d.length>0&&r.push({title:"Other",icon:"mdi:help-circle-outline",path:"other",type:"sections",sections:[Q({type:"custom:maintenance-supporter-card",show_header:!1,filter_objects:d.map(l=>l.object.name)})]}),r}function Wa(i){return[{title:"Week",icon:"mdi:calendar-week",path:"cal-7",window_days:7},{title:"Fortnight",icon:"mdi:calendar-week-begin",path:"cal-14",window_days:14},{title:"Month",icon:"mdi:calendar-month",path:"cal-30",window_days:30},{title:"Year",icon:"mdi:calendar-clock",path:"cal-365",window_days:365}].map(e=>({title:e.title,icon:e.icon,path:e.path,type:"panel",cards:[{type:"custom:maintenance-supporter-calendar-card",window_days:e.window_days,show_window_chips:!1,show_user_filter:!0}]}))}function Qa(i){let a=new Set;for(let t of i)for(let n of t.tasks||[]){let r=n.days_until_due;r!=null&&Mt.forEach((d,l)=>{d.matches(r)&&a.add(l)})}let e=[];return Mt.forEach((t,n)=>{a.has(n)&&e.push({title:t.title,icon:t.icon,path:t.path,type:"sections",sections:[Q({type:"custom:maintenance-supporter-card",show_header:!1,...t.filter})]})}),e}var He=class extends HTMLElement{static getCreateSuggestions(a){return{title:"Maintenance Supporter",icon:"mdi:wrench-clock"}}static async getConfigElement(){return document.createElement(Oe)}static async generate(a,e){if(e.config?.state===Pa)return{views:[{type:"sections",sections:[{cards:[{type:"starting"}]}]}]};if(e.config?.recovery_mode)return{views:[{type:"sections",sections:[{cards:[{type:"recovery-mode"}]}]}]};let t;try{t=await e.connection.sendMessagePromise({type:"maintenance_supporter/objects"})}catch{return{title:"Maintenance",views:[{title:"Maintenance",cards:[{type:"markdown",content:"**Maintenance Supporter** is not loaded. Install/enable the integration first."}]}]}}let n=t.objects||[];if(n.length===0)return{title:"Maintenance",views:[Ha()]};let r=La(n),d=a?.group_by??"area",l={area:()=>Va(n,e.areas||{}),status:()=>Ba(n),floor:()=>Ga(n,e.areas||{},e.floors||{}),due_date:()=>Qa(n),calendar:()=>Wa(n)};return{title:"Maintenance",views:[Ua(r),...(l[d]??l.area)()]}}},Ka=[{value:"area",label:"By area (default)"},{value:"status",label:"By status (Overdue / Triggered / Due Soon / OK)"},{value:"floor",label:"By floor (uses HA floors)"},{value:"due_date",label:"By due date (Overdue / Today / Week / Month / Later)"},{value:"calendar",label:"Rolling calendar (Week / Fortnight / Month / Year)"}],Ue=class extends HTMLElement{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter"}}set hass(e){this._hass=e}setConfig(e){this._config=e,this._render()}connectedCallback(){this._render()}_render(){let e=this._config.group_by??"area",t=Ka.map(r=>`<option value="${r.value}"${r.value===e?" selected":""}>${r.label}</option>`).join("");this.innerHTML=`
      <style>
    ...
    `;let n=this.querySelector("#group-by");n?.addEventListener("change",()=>{let r={...this._config,group_by:n.value};this._config=r,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:r},bubbles:!0,composed:!0}))})}},Bt="maintenance-supporter-section",Ft=`ll-strategy-section-${Bt}`,Ve=class extends HTMLElement{static async generate(a,e){let t={type:"custom:maintenance-supporter-card",show_header:!1};a?.filter_status?.length&&(t.filter_status=a.filter_status),a?.max_items&&(t.max_items=a.max_items),a?.filter_due_min_days!==void 0&&(t.filter_due_min_days=a.filter_due_min_days),a?.filter_due_max_days!==void 0&&(t.filter_due_max_days=a.filter_due_max_days);let n=a?.filter_objects;if(a?.area_id&&!n)try{n=((await e.connection.sendMessagePromise({type:"maintenance_supporter/objects"})).objects||[]).filter(l=>l.object.area_id===a.area_id).map(l=>l.object.name)}catch{}n&&n.length>0&&(t.filter_objects=n);let r=[];return a?.title&&r.push({type:"heading",heading:a.title,heading_style:"title"}),r.push(t),{type:"grid",cards:r}}};customElements.get(Pt)||customElements.define(Pt,He);customElements.get(Oe)||customElements.define(Oe,Ue);customElements.get(Ft)||customElements.define(Ft,Ve);var Be=class extends HTMLElement{static async generate(a,e){return{type:"grid",cards:[{type:"custom:maintenance-vacation-section-card",title:a?.title}]}}},Ge=class extends HTMLElement{static async generate(a,e){return{type:"grid",cards:[{type:"custom:maintenance-budget-section-card",title:a?.title}]}}},We=class extends HTMLElement{static async generate(a,e){return{type:"grid",cards:[{type:"custom:maintenance-groups-section-card",title:a?.title}]}}},Lt="ll-strategy-section-maintenance-supporter-vacation",Ot="ll-strategy-section-maintenance-supporter-budget",Ht="ll-strategy-section-maintenance-supporter-groups";customElements.get(Lt)||customElements.define(Lt,Be);customElements.get(Ot)||customElements.define(Ot,Ge);customElements.get(Ht)||customElements.define(Ht,We);var Ut="_msSupporterLlCustomBound";function le(i){history.pushState(null,"",i),window.dispatchEvent(new CustomEvent("location-changed"))}function Ja(){let i=window;i[Ut]||(i[Ut]=!0,document.addEventListener("ll-custom",a=>{let e=a.detail;if(!e||typeof e.type!="string"||!e.type.startsWith("maintenance-supporter:"))return;let t=e.type.slice(22);if(t==="add-object"){if(De())return;le("/maintenance-supporter?ms_action=add_object");return}if(t==="open-task"&&typeof e.entry_id=="string"&&typeof e.task_id=="string"){if(Fe(e.entry_id,e.task_id)||Pe(e.entry_id,e.task_id))return;le(`/maintenance-supporter?entry_id=${encodeURIComponent(e.entry_id)}&task_id=${encodeURIComponent(e.task_id)}`);return}if(t==="open-object"&&typeof e.entry_id=="string"){if(Le(e.entry_id))return;le(`/maintenance-supporter?entry_id=${encodeURIComponent(e.entry_id)}`);return}if(t==="deep-link"&&typeof e.path=="string"){le(e.path);return}if(t==="edit-history"&&typeof e.entry_id=="string"&&typeof e.task_id=="string"&&typeof e.original_timestamp=="string"){let r=document.querySelector("home-assistant")?.hass;if(!r)return;(async()=>{try{let _=(await r.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:e.entry_id})).tasks?.find(g=>g.id===e.task_id)?.history?.find(g=>g.timestamp===e.original_timestamp);if(!_)return;Me({entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.original_timestamp,type:_.type||"completed",timestamp:_.timestamp||e.original_timestamp,notes:_.notes??null,cost:_.cost??null,duration:_.duration??null,completed_by:_.completed_by??null})}catch{le("/maintenance-supporter")}})();return}}))}Ja();var ye=window;ye.customStrategies=ye.customStrategies||[];function de(i){ye.customStrategies.some(e=>e.type===i.type&&e.strategyType===i.strategyType)||ye.customStrategies.push(i)}de({type:Vt,strategyType:"dashboard",name:"Maintenance Supporter",description:"Auto-generated dashboard. Group views by area, status, floor, or due date \u2014 picked from the strategy editor or YAML.",documentationURL:"https://github.com/iluebbe/maintenance_supporter#dashboard-strategy"});de({type:Bt,strategyType:"section",name:"Maintenance Supporter \u2014 Section",description:"Embed maintenance tasks (filterable by area, status, due date) as a section in any dashboard view.",documentationURL:"https://github.com/iluebbe/maintenance_supporter#section-strategy"});de({type:"maintenance-supporter-vacation",strategyType:"section",name:"Maintenance Supporter \u2014 Vacation Status",description:"Compact vacation-mode status banner. Tap to open settings in the panel."});de({type:"maintenance-supporter-budget",strategyType:"section",name:"Maintenance Supporter \u2014 Budget Status",description:"Monthly + yearly maintenance budget overview. Tap for details."});de({type:"maintenance-supporter-groups",strategyType:"section",name:"Maintenance Supporter \u2014 Groups",description:"List of configured task groups with member counts."});
