...
`;var kt={name:"name",task_type:"maintenance_type",schedule_type:"schedule_type",interval_days:"interval_days",interval_anchor:"interval_anchor",warning_days:"warning_days",last_performed:"last_performed_optional",notes:"notes_optional",documentation_url:"documentation_url_optional",custom_icon:"custom_icon_optional",nfc_tag_id:"nfc_tag_id_optional",responsible_user_id:"responsible_user",entity_slug:"entity_slug",entity_id:"entity_id",area_id:"area_id_optional",manufacturer:"manufacturer_optional",model:"model_optional",serial_number:"serial_number_optional",installation_date:"installation_date_optional",checklist:"checklist_steps_optional",reason:"reason",feedback:"feedback",cost:"cost",duration:"duration",description:"description_optional",group_name:"name",group_description:"description_optional",environmental_entity:"environmental_entity_optional",environmental_attribute:"environmental_attribute_optional",trigger_above:"trigger_above",trigger_below:"trigger_below",trigger_for_minutes:"trigger_for_minutes"};function wt(i,t){let e=kt[i];if(!e)return i;let a=s(e,t);return a&&a!==e?a:i}function xt(i){let e=i.match(/data\['([^']+)'\]/)?.[1],a;return(a=i.match(/length of value must be at most (\d+)/))?{field:e,rule:"too_long",param:a[1]}:(a=i.match(/length of value must be at least (\d+)/))?{field:e,rule:"too_short",param:a[1]}:(a=i.match(/value must be at most (\S+)/))?{field:e,rule:"value_too_high",param:a[1]}:(a=i.match(/value must be at least (\S+)/))?{field:e,rule:"value_too_low",param:a[1]}:/required key not provided/.test(i)?{field:e,rule:"required"}:(a=i.match(/expected (\w+)/))?{field:e,rule:"wrong_type",param:a[1]}:/value must be one of/.test(i)?{field:e,rule:"invalid_choice"}:/not a valid value/.test(i)?{field:e,rule:"invalid_value"}:{field:e,rule:"unknown"}}function S(i,t,e){if(typeof i=="string")return i;if(typeof i!="object"||i===null)return e;let a=i,n=a.message||a.error?.message||"";if(!n)return e;let o=xt(n),l=o.field?wt(o.field,t):"",r=c=>s(c,t).replace("{field}",l).replace("{n}",o.param??"");switch(o.rule){case"too_long":return r("err_too_long");case"too_short":return r("err_too_short");case"value_too_high":return r("err_value_too_high");case"value_too_low":return r("err_value_too_low");case"required":return r("err_required");case"wrong_type":return r("err_wrong_type").replace("{type}",o.param??"");case"invalid_choice":return r("err_invalid_choice");case"invalid_value":return r("err_invalid_value");default:return n||e}}var y=class extends w{constructor(){super(...arguments);this._open=!1;this._loading=!1;this._error="";this._name="";this._manufacturer="";this._model="";this._serialNumber="";this._areaId="";this._installationDate="";this._documentationUrl="";this._notes="";this._entryId=null}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}openCreate(){this._entryId=null,this._name="",this._manufacturer="",this._model="",this._serialNumber="",this._areaId="",this._installationDate="",this._documentationUrl="",this._notes="",this._error="",this._open=!0}openEdit(e,a){this._entryId=e,this._name=a.name||"",this._manufacturer=a.manufacturer||"",this._model=a.model||"",this._serialNumber=a.serial_number||"",this._areaId=a.area_id||"",this._installationDate=a.installation_date||"",this._documentationUrl=a.documentation_url||"",this._notes=a.notes||"",this._error="",this._open=!0}async _save(){if(this._name.trim()){this._loading=!0,this._error="";try{this._entryId?await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/update",entry_id:this._entryId,name:this._name,manufacturer:this._manufacturer||null,model:this._model||null,serial_number:this._serialNumber||null,area_id:this._areaId||null,installation_date:this._installationDate||null,documentation_url:this._documentationUrl.trim()||null,notes:this._notes.trim()||null}):await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/create",name:this._name,manufacturer:this._manufacturer||null,model:this._model||null,serial_number:this._serialNumber||null,area_id:this._areaId||null,installation_date:this._installationDate||null,documentation_url:this._documentationUrl.trim()||null,notes:this._notes.trim()||null}),this._open=!1,this.dispatchEvent(new CustomEvent("object-saved"))}catch(e){this._error=S(e,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}render(){if(!this._open)return p``;let e=this._lang,a=this._entryId?s("edit_object",e):s("new_object",e);return p`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${a}</div>
        <div class="content">
          ${this._error?p`<div class="error">${this._error}</div>`:g}
          <ha-textfield
            label="${s("name",e)}"
            required
            .value=${this._name}
          ...
          ></ha-textfield>
          <ha-textarea
            label="${s("object_notes_optional",e)}"
            autogrow
            rows="3"
            .value=${this._notes}
  ...
  `,_([v({attribute:!1})],y.prototype,"hass",2),_([d()],y.prototype,"_open",2),_([d()],y.prototype,"_loading",2),_([d()],y.prototype,"_error",2),_([d()],y.prototype,"_name",2),_([d()],y.prototype,"_manufacturer",2),_([d()],y.prototype,"_model",2),_([d()],y.prototype,"_serialNumber",2),_([d()],y.prototype,"_areaId",2),_([d()],y.prototype,"_installationDate",2),_([d()],y.prototype,"_documentationUrl",2),_([d()],y.prototype,"_notes",2),_([d()],y.prototype,"_entryId",2);customElements.get("maintenance-object-dialog")||customElements.define("maintenance-object-dialog",y);var Z=class{constructor(t){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=t}updateHass(t){this.hass=t}async getUsers(t=!1){let e=Date.now();if(!t&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let a=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=a.users,this.cacheTimestamp=e,this.usersCache}catch(a){return console.error("Failed to fetch users:",a),this.usersCache||[]}}async assignUser(t,e,a){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:t,task_id:e,user_id:a})}async getTasksByUser(t){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:t})).tasks}getUserName(t){return!t||!this.usersCache?null:this.usersCache.find(a=>a.id===t)?.name||null}getUser(t){return!t||!this.usersCache?null:this.usersCache.find(e=>e.id===t)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(t){return t?t===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}};var zt=["cleaning","inspection","replacement","calibration","service","custom"],Et=["time_based","sensor_based","manual"],At=["threshold","counter","state_change","runtime"],u=class extends w{constructor(){super(...arguments);this.checklistsEnabled=!1;this.scheduleTimeEnabled=!1;this.completionActionsEnabled=!1;this.defaultWarningDays=7;this._open=!1;this._loading=!1;this._error="";this._entryId="";this._taskId=null;this._objectChoices=[];this._name="";this._type="custom";this._scheduleType="time_based";this._intervalDays="30";this._warningDays="7";this._intervalAnchor="completion";this._notes="";this._documentationUrl="";this._customIcon="";this._enabled=!0;this._triggerEntityId="";this._triggerEntityIds=[];this._triggerEntityLogic="any";this._triggerAttribute="";this._triggerType="threshold";this._triggerAbove="";this._triggerBelow="";this._triggerForMinutes="0";this._triggerTargetValue="";this._triggerDeltaMode=!1;this._triggerFromState="";this._triggerToState="";this._triggerTargetChanges="";this._triggerRuntimeHours="";this._suggestedAttributes=[];this._availableAttributes=[];this._entityDomain="";this._lastPerformed="";this._nfcTagId="";this._availableTags=[];this._responsibleUserId=null;this._availableUsers=[];this._checklistText="";this._scheduleTime="";this._actionService="";this._actionTargetEntity="";this._actionData={};this._actionDataJsonFallback="";this._actionTesting=!1;this._actionTestResult="";this._qcNotes="";this._qcCost="";this._qcDuration="";this._qcFeedback="";this._environmentalEntity="";this._environmentalAttribute="";this._environmentalInitial="";this._environmentalAttributeInitial="";this._userService=null}get _lang(){return this.hass?.language??navigator.language.split("-")[0]??"en"}async openCreate(e,a){this._entryId=e,this._taskId=null,this._error="",!e&&a&&a.length>0?(this._objectChoices=a.map(n=>({entry_id:n.entry_id,name:n.object.name})).sort((n,o)=>n.name.localeCompare(o.name)),this._entryId=this._objectChoices[0].entry_id):this._objectChoices=[],this._resetFields(),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}async openEdit(e,a){this._entryId=e,this._taskId=a.id,this._error="",this._name=a.name,this._type=a.type,this._scheduleType=a.schedule_type,this._intervalDays=a.interval_days!=null?String(a.interval_days):"",this._warningDays=a.warning_days.toString(),this._intervalAnchor=a.interval_anchor||"completion",this._notes=a.notes||"",this._documentationUrl=a.documentation_url||"",this._customIcon=a.custom_icon||"",this._enabled=a.enabled!==!1,this._lastPerformed=a.last_performed||"",this._nfcTagId=a.nfc_tag_id||"",this._responsibleUserId=a.responsible_user_id||null,this._checklistText=(a.checklist||[]).join(`
`),this._scheduleTime=a.schedule_time||"";let n=a.on_complete_action;if(n&&n.service){this._actionService=n.service;let r=n.target?.entity_id;this._actionTargetEntity=Array.isArray(r)?r[0]||"":r||"",this._actionData=n.data&&typeof n.data=="object"?{...n.data}:{},this._actionDataJsonFallback=""}else this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="";let o=a.quick_complete_defaults;this._qcNotes=o?.notes||"",this._qcCost=o?.cost!=null?String(o.cost):"",this._qcDuration=o?.duration!=null?String(o.duration):"",this._qcFeedback=o?.feedback||"";let l=a.adaptive_config||{};if(this._environmentalEntity=l.environmental_entity||"",this._environmentalAttribute=l.environmental_attribute||"",this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute,a.trigger_config){let r=a.trigger_config;this._triggerEntityId=r.entity_id||"",this._triggerEntityIds=r.entity_ids||(r.entity_id?[r.entity_id]:[]),this._triggerEntityLogic=r.entity_logic||"any",this._triggerAttribute=r.attribute||"",this._triggerType=r.type||"threshold",this._triggerAbove=r.trigger_above?.toString()||"",this._triggerBelow=r.trigger_below?.toString()||"",this._triggerForMinutes=r.trigger_for_minutes?.toString()||"0",this._triggerTargetValue=r.trigger_target_value?.toString()||"",this._triggerDeltaMode=r.trigger_delta_mode||!1,this._triggerFromState=r.trigger_from_state||"",this._triggerToState=r.trigger_to_state||"",this._triggerTargetChanges=r.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=r.trigger_runtime_hours?.toString()||""}else this._resetTriggerFields();this._triggerEntityId&&this._fetchEntityAttributes(this._triggerEntityId),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._warningDays=String(this.defaultWarningDays),this._intervalAnchor="completion",this._notes="",this._documentationUrl="",this._customIcon="",this._enabled=!0,this._lastPerformed="",this._nfcTagId="",this._responsibleUserId=null,this._checklistText="",this._scheduleTime="",this._environmentalEntity="",this._environmentalAttribute="",this._environmentalInitial="",this._environmentalAttributeInitial="",this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="",this._actionTesting=!1,this._actionTestResult="",this._qcNotes="",this._qcCost="",this._qcDuration="",this._qcFeedback="",this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerForMinutes="0",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours=""}async _loadUsers(){this._userService||(this._userService=new Z(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}async _testAction(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e)){this._actionTestResult="error";return}let[a,n]=e.split("."),o={...this._buildActionData()},l=this._actionTargetEntity.trim();l&&(o.entity_id=l),this._actionTesting=!0,this._actionTestResult="";try{await this.hass.callService(a,n,o),this._actionTestResult="ok"}catch{this._actionTestResult="error"}finally{this._actionTesting=!1,setTimeout(()=>{this._actionTestResult=""},3e3)}}_buildActionData(){if(this._actionDataJsonFallback.trim())try{let e=JSON.parse(this._actionDataJsonFallback);if(e&&typeof e=="object"&&!Array.isArray(e))return e}catch{}return{...this._actionData}}_serviceSchema(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e))return null;let[a,n]=e.split("."),o=this.hass?.services?.[a]?.[n]?.fields;return!o||Object.keys(o).length===0?null:Object.entries(o).map(([l,r])=>({name:l,required:!!r.required,selector:r.selector||{text:{}}}))}_renderCompletionActionsSection(e){if(!this.completionActionsEnabled)return g;let a=this._serviceSchema();return p`
      <details class="ca-section">
        ...
        <ha-service-picker
          ...
          @value-changed=${n=>{this._actionService=n.detail.value||"";let o=this._serviceSchema();if(o){let l=new Set(o.map(r=>r.name));this._actionData=Object.fromEntries(Object.entries(this._actionData).filter(([r])=>l.has(r)))}}}
        ></ha-service-picker>
        <ha-entity-picker
          .hass=${this.hass}
          .value=${this._actionTargetEntity}
          .label=${s("on_complete_action_target",e)}
        ...
        <div class="ca-test-row">
          ...
          ${this._actionTestResult==="ok"?p`<span class="ca-test-ok">${s("on_complete_action_test_success",e)}</span>`:g}
          ${this._actionTestResult==="error"?p`<span class="ca-test-error">${s("on_complete_action_test_failed",e)}</span>`:g}
        </div>
      </details>
    ...
    `}async _loadTags(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tags/list"});this._availableTags=e.tags||[]}catch{this._availableTags=[]}}async _fetchEntityAttributes(e){if(!e||!this.hass){this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="";return}try{let a=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/entity/attributes",entity_id:e});this._entityDomain=a.domain||"",this._suggestedAttributes=a.suggested_attributes||[],this._availableAttributes=a.available_attributes||[]}catch{this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain=""}}async _save(){if(this._name.trim()){this._loading=!0,this._error="";try{let e={type:this._taskId?"maintenance_supporter/task/update":"maintenance_supporter/task/create",entry_id:this._entryId,name:this._name,task_type:this._type,schedule_type:this._scheduleType,warning_days:parseInt(this._warningDays,10)||7};if(this._taskId&&(e.task_id=this._taskId),this._scheduleType!=="manual"?this._intervalDays?(e.interval_days=parseInt(this._intervalDays,10),e.interval_anchor=this._intervalAnchor):this._taskId&&(e.interval_days=null,e.interval_anchor="completion"):this._taskId&&(e.interval_days=null,e.interval_anchor="completion"),e.notes=this._notes||null,e.documentation_url=this._documentationUrl||null,e.custom_icon=this._customIcon||null,e.enabled=this._enabled,e.last_performed=this._lastPerformed||null,e.nfc_tag_id=this._nfcTagId||null,e.responsible_user_id=this._responsibleUserId,this._scheduleType==="sensor_based"&&this._triggerEntityId){let l=this._triggerEntityIds.length>0?this._triggerEntityIds:[this._triggerEntityId],r={entity_id:l[0],entity_ids:l,type:this._triggerType};if(this._triggerAttribute&&(r.attribute=this._triggerAttribute),l.length>1&&(r.entity_logic=this._triggerEntityLogic),this._triggerType==="threshold"){if(this._triggerAbove){let c=parseFloat(this._triggerAbove);isNaN(c)||(r.trigger_above=c)}if(this._triggerBelow){let c=parseFloat(this._triggerBelow);isNaN(c)||(r.trigger_below=c)}if(this._triggerForMinutes){let c=parseInt(this._triggerForMinutes,10);isNaN(c)||(r.trigger_for_minutes=c)}}else if(this._triggerType==="counter"){if(this._triggerTargetValue){let c=parseFloat(this._triggerTargetValue);isNaN(c)||(r.trigger_target_value=c)}r.trigger_delta_mode=this._triggerDeltaMode}else if(this._triggerType==="state_change"){if(this._triggerFromState&&(r.trigger_from_state=this._triggerFromState),this._triggerToState&&(r.trigger_to_state=this._triggerToState),this._triggerTargetChanges){let c=parseInt(this._triggerTargetChanges,10);isNaN(c)||(r.trigger_target_changes=c)}}else if(this._triggerType==="runtime"&&this._triggerRuntimeHours){let c=parseFloat(this._triggerRuntimeHours);isNaN(c)||(r.trigger_runtime_hours=c)}e.trigger_config=r}else this._taskId&&(e.trigger_config=null);if(this.scheduleTimeEnabled&&this._scheduleType==="time_based"){let l=this._scheduleTime.trim();e.schedule_time=/^([01]\d|2[0-3]):[0-5]\d$/.test(l)?l:null}if(this.checklistsEnabled){let l=this._checklistText.split(`
`).map(r=>r.trim()).filter(Boolean).slice(0,100);e.checklist=l.length?l:null}if(this.completionActionsEnabled){let l=this._actionService.trim();if(l&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(l)){let f={service:l},h=this._actionTargetEntity.trim();h&&(f.target={entity_id:h});let k=this._buildActionData();Object.keys(k).length>0&&(f.data=k),e.on_complete_action=f}else e.on_complete_action=null;let r={};this._qcNotes.trim()&&(r.notes=this._qcNotes.trim());let c=parseFloat(this._qcCost);!isNaN(c)&&c>=0&&(r.cost=c);let m=parseInt(this._qcDuration,10);!isNaN(m)&&m>=0&&(r.duration=m),this._qcFeedback&&(r.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(r).length?r:null}let a=await this.hass.connection.sendMessagePromise(e),n=this._taskId||a?.task_id,o=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(n&&this._scheduleType==="sensor_based"&&o)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:n,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=S(e,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return g;let e=this._lang;return p`
      ...
      <ha-textfield
        ...
        @input=${a=>{let o=a.target.value.split(",").map(l=>l.trim()).filter(Boolean);this._triggerEntityId=o[0]||"",this._triggerEntityIds=o,o[0]&&this._fetchEntityAttributes(o[0])}}
      ></ha-textfield>
      ${this._triggerEntityIds.length>1?p`
        <div class="select-row">
          <label>${s("entity_logic",e)}</label>
          <select
            .value=${this._triggerEntityLogic}
            @change=${a=>this._triggerEntityLogic=a.target.value}
          >
      ...
      `:g}
      ${this._availableAttributes.length>0?p`
          <div class="select-row">
            <label>${s("attribute_optional",e)}</label>
            <select
              .value=${this._triggerAttribute}
              @change=${a=>this._triggerAttribute=a.target.value}
            >
              <option value="" ?selected=${!this._triggerAttribute}>${s("use_entity_state",e)}</option>
              ${this._suggestedAttributes.map(a=>p`<option value=${a} ?selected=${a===this._triggerAttribute}>${a} ★</option>`)}
              ${this._availableAttributes.filter(a=>!this._suggestedAttributes.includes(a.name)).map(a=>p`<option value=${a.name} ?selected=${a.name===this._triggerAttribute}>${a.name}${a.numeric?"":" (non-numeric)"}</option>`)}
            </select>
          </div>
        `:p`
          <ha-textfield
            ...
            .value=${this._triggerAttribute}
            @input=${a=>this._triggerAttribute=a.target.value}
          ></ha-textfield>
        `}
      <div class="select-row">
        ...
        <select
          ...
          @change=${a=>this._triggerType=a.target.value}
        >
          ${At.map(a=>p`<option value=${a} ?selected=${a===this._triggerType}>${s(a,e)}</option>`)}
        </select>
      </div>
      ...
      ></ha-textfield>
    `}_renderTriggerTypeFields(){let e=this._lang;return this._triggerType==="threshold"?p`
        <ha-textfield
          label="${s("trigger_above",e)}"
          type="number"
          step="any"
          .value=${this._triggerAbove}
          @input=${a=>this._triggerAbove=a.target.value}
        ></ha-textfield>
        <ha-textfield
          ...
          @input=${a=>this._triggerBelow=a.target.value}
        ></ha-textfield>
        <ha-textfield
          label="${s("for_at_least_minutes",e)}"
          type="number"
          .value=${this._triggerForMinutes}
          @input=${a=>this._triggerForMinutes=a.target.value}
        ></ha-textfield>
      `:this._triggerType==="counter"?p`
        <ha-textfield
          label="${s("target_value",e)}"
          type="number"
          step="any"
          .value=${this._triggerTargetValue}
          @input=${a=>this._triggerTargetValue=a.target.value}
        ></ha-textfield>
        <label>
          <input
            type="checkbox"
            .checked=${this._triggerDeltaMode}
            @change=${a=>this._triggerDeltaMode=a.target.checked}
          />
          ${s("delta_mode",e)}
        </label>
      `:this._triggerType==="state_change"?p`
        <ha-textfield
          label="${s("from_state_optional",e)}"
          .value=${this._triggerFromState}
          @input=${a=>this._triggerFromState=a.target.value}
        ></ha-textfield>
        <div class="field-help">${s("state_value_help",e)}</div>
        <ha-textfield
          label="${s("to_state_optional",e)}"
          .value=${this._triggerToState}
          @input=${a=>this._triggerToState=a.target.value}
        ></ha-textfield>
        <ha-textfield
          ...
          @input=${a=>this._triggerTargetChanges=a.target.value}
        ></ha-textfield>
        <div class="field-help">${s("target_changes_help",e)}</div>
      `:this._triggerType==="runtime"?p`
        <ha-textfield
          label="${s("runtime_hours",e)}"
          type="number"
          step="1"
          .value=${this._triggerRuntimeHours}
          @input=${a=>this._triggerRuntimeHours=a.target.value}
        ></ha-textfield>
      `:g}render(){if(!this._open)return p``;let e=this._lang,a=this._taskId?s("edit_task",e):s("new_task",e);return p`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${a}</div>
        <div class="content">
          ${this._error?p`<div class="error">${this._error}</div>`:g}
          ${this._objectChoices.length>0?p`
            <div class="select-row">
              ...
              >
                ${this._objectChoices.map(n=>p`<option value=${n.entry_id} ?selected=${n.entry_id===this._entryId}>${n.name}</option>`)}
              </select>
            </div>
          `:g}
          <ha-textfield
          ...
          <div class="select-row">
            ...
            >
              ${zt.map(n=>p`<option value=${n} ?selected=${n===this._type}>${s(n,e)}</option>`)}
            </select>
          </div>
          <div class="select-row">
            ...
            >
              ${Et.map(n=>p`<option value=${n} ?selected=${n===this._scheduleType}>${s(n,e)}</option>`)}
            </select>
          </div>
          ${this._scheduleType==="time_based"?p`
                <ha-textfield
          ...
          <div class="select-row">
            ...
            >
              ...
              ${this._availableUsers.map(n=>p`<option value=${n.id} ?selected=${n.id===this._responsibleUserId}>${n.name}</option>`)}
            </select>
          </div>
          ${this._renderTriggerFields()}
          ${this._scheduleType==="sensor_based"?p`
            <ha-textfield
          ...
          ></ha-icon-picker>
          ${this._availableTags.length>0?p`
              <div class="select-row">
                ...
                >
                  ...
                  ${this._availableTags.map(n=>p`<option value=${n.id} ?selected=${n.id===this._nfcTagId}>${n.name}</option>`)}
                </select>
  ...
  `,_([v({attribute:!1})],u.prototype,"hass",2),_([v({type:Boolean,attribute:"checklists-enabled"})],u.prototype,"checklistsEnabled",2),_([v({type:Boolean,attribute:"schedule-time-enabled"})],u.prototype,"scheduleTimeEnabled",2),_([v({type:Boolean,attribute:"completion-actions-enabled"})],u.prototype,"completionActionsEnabled",2),_([v({type:Number,attribute:"default-warning-days"})],u.prototype,"defaultWarningDays",2),_([d()],u.prototype,"_open",2),_([d()],u.prototype,"_loading",2),_([d()],u.prototype,"_error",2),_([d()],u.prototype,"_entryId",2),_([d()],u.prototype,"_taskId",2),_([d()],u.prototype,"_objectChoices",2),_([d()],u.prototype,"_name",2),_([d()],u.prototype,"_type",2),_([d()],u.prototype,"_scheduleType",2),_([d()],u.prototype,"_intervalDays",2),_([d()],u.prototype,"_warningDays",2),_([d()],u.prototype,"_intervalAnchor",2),_([d()],u.prototype,"_notes",2),_([d()],u.prototype,"_documentationUrl",2),_([d()],u.prototype,"_customIcon",2),_([d()],u.prototype,"_enabled",2),_([d()],u.prototype,"_triggerEntityId",2),_([d()],u.prototype,"_triggerEntityIds",2),_([d()],u.prototype,"_triggerEntityLogic",2),_([d()],u.prototype,"_triggerAttribute",2),_([d()],u.prototype,"_triggerType",2),_([d()],u.prototype,"_triggerAbove",2),_([d()],u.prototype,"_triggerBelow",2),_([d()],u.prototype,"_triggerForMinutes",2),_([d()],u.prototype,"_triggerTargetValue",2),_([d()],u.prototype,"_triggerDeltaMode",2),_([d()],u.prototype,"_triggerFromState",2),_([d()],u.prototype,"_triggerToState",2),_([d()],u.prototype,"_triggerTargetChanges",2),_([d()],u.prototype,"_triggerRuntimeHours",2),_([d()],u.prototype,"_suggestedAttributes",2),_([d()],u.prototype,"_availableAttributes",2),_([d()],u.prototype,"_entityDomain",2),_([d()],u.prototype,"_lastPerformed",2),_([d()],u.prototype,"_nfcTagId",2),_([d()],u.prototype,"_availableTags",2),_([d()],u.prototype,"_responsibleUserId",2),_([d()],u.prototype,"_availableUsers",2),_([d()],u.prototype,"_checklistText",2),_([d()],u.prototype,"_scheduleTime",2),_([d()],u.prototype,"_actionService",2),_([d()],u.prototype,"_actionTargetEntity",2),_([d()],u.prototype,"_actionData",2),_([d()],u.prototype,"_actionDataJsonFallback",2),_([d()],u.prototype,"_actionTesting",2),_([d()],u.prototype,"_actionTestResult",2),_([d()],u.prototype,"_qcNotes",2),_([d()],u.prototype,"_qcCost",2),_([d()],u.prototype,"_qcDuration",2),_([d()],u.prototype,"_qcFeedback",2),_([d()],u.prototype,"_environmentalEntity",2),_([d()],u.prototype,"_environmentalAttribute",2);customElements.get("maintenance-task-dialog")||customElements.define("maintenance-task-dialog",u);var b=class extends w{constructor(){super(...arguments);this.entryId="";this.taskId="";this.taskName="";this.lang="en";this.checklist=[];this.adaptiveEnabled=!1;this._open=!1;this._notes="";this._cost="";this._duration="";this._loading=!1;this._error="";this._checklistState={};this._feedback="needed"}open(){this._open||(this._open=!0,this._notes="",this._cost="",this._duration="",this._error="",this._checklistState={},this._feedback="needed")}_toggleCheck(e){let a=String(e);this._checklistState={...this._checklistState,[a]:!this._checklistState[a]}}_setFeedback(e){this._feedback=e}async _complete(){this._loading=!0,this._error="";try{let e={type:"maintenance_supporter/task/complete",entry_id:this.entryId,task_id:this.taskId};if(this._notes&&(e.notes=this._notes),this._cost){let a=parseFloat(this._cost);!isNaN(a)&&a>=0&&(e.cost=a)}if(this._duration){let a=parseInt(this._duration,10);!isNaN(a)&&a>=0&&(e.duration=a)}this.checklist.length>0&&(e.checklist_state=this._checklistState),this.adaptiveEnabled&&(e.feedback=this._feedback),await this.hass.connection.sendMessagePromise(e),this._open=!1,this.dispatchEvent(new CustomEvent("task-completed"))}catch(e){this._error=S(e,this.lang,s("save_error",this.lang))}finally{this._loading=!1}}_close(){this._open=!1}render(){if(!this._open)return p``;let e=this.lang||this.hass?.language||"en";return p`
      <ha-dialog open @closed=${this._close}>
  ...
  `,_([v({attribute:!1})],b.prototype,"hass",2),_([v()],b.prototype,"entryId",2),_([v()],b.prototype,"taskId",2),_([v()],b.prototype,"taskName",2),_([v()],b.prototype,"lang",2),_([v({type:Array})],b.prototype,"checklist",2),_([v({type:Boolean})],b.prototype,"adaptiveEnabled",2),_([d()],b.prototype,"_open",2),_([d()],b.prototype,"_notes",2),_([d()],b.prototype,"_cost",2),_([d()],b.prototype,"_duration",2),_([d()],b.prototype,"_loading",2),_([d()],b.prototype,"_error",2),_([d()],b.prototype,"_checklistState",2),_([d()],b.prototype,"_feedback",2);customElements.get("maintenance-complete-dialog")||customElements.define("maintenance-complete-dialog",b);var E=class extends w{constructor(){super(...arguments);this._open=!1;this._saving=!1;this._error="";this._draft=null;this._originalSnapshot=null}get _lang(){return this.hass?.language||"en"}openEdit(e){this._draft={...e},this._originalSnapshot={...e},this._error="",this._open=!0}close(){this._open=!1,this._error="",this._draft=null,this._originalSnapshot=null}_set(e,a){this._draft&&(this._draft={...this._draft,[e]:a})}async _save(){if(!(!this._draft||!this._originalSnapshot)){this._saving=!0,this._error="";try{let e={type:"maintenance_supporter/task/history/update",entry_id:this._draft.entry_id,task_id:this._draft.task_id,original_timestamp:this._originalSnapshot.original_timestamp};if(this._draft.timestamp!==this._originalSnapshot.timestamp&&(e.timestamp=this._draft.timestamp),this._draft.notes!==this._originalSnapshot.notes&&(e.notes=this._draft.notes),this._draft.cost!==this._originalSnapshot.cost&&(e.cost=this._draft.cost),this._draft.duration!==this._originalSnapshot.duration&&(e.duration=this._draft.duration),this._draft.completed_by!==this._originalSnapshot.completed_by&&(e.completed_by=this._draft.completed_by),Object.keys(e).filter(n=>!["type","entry_id","task_id","original_timestamp"].includes(n)).length===0){this.close();return}await this.hass.connection.sendMessagePromise(e),this.dispatchEvent(new CustomEvent("history-entry-saved",{detail:{entry_id:this._draft.entry_id,task_id:this._draft.task_id,new_timestamp:this._draft.timestamp},bubbles:!0,composed:!0})),this.close()}catch(e){this._error=S(e,this._lang)}finally{this._saving=!1}}}render(){if(!this._open||!this._draft)return g;let e=this._lang,a=this._draft;return p`
      <div class="backdrop" @click=${this.close}></div>
      <div class="dialog" role="dialog" aria-modal="true">
        <h2>${s("history_edit_title",e)||"Edit history entry"}</h2>
        <div class="entry-type">
          <ha-icon icon="mdi:tag-outline"></ha-icon>
          <span>${s(a.type,e)||a.type}</span>
        </div>
        <label>
          <span>${s("history_edit_timestamp",e)||"Timestamp"}</span>
          <input type="datetime-local"
            .value=${a.timestamp.length>=16?a.timestamp.slice(0,16):a.timestamp}
            @change=${n=>{let o=n.target.value;this._set("timestamp",o.length===16?`${o}:00`:o)}} />
        </label>
        <label>
          <span>${s("notes",e)||"Notes"}</span>
          <textarea
            rows="3"
            @input=${n=>{let o=n.target.value;this._set("notes",o||null)}}
            .value=${a.notes??""}></textarea>
        </label>
        <div class="row">
          <label>
            <span>${s("cost",e)||"Cost"}</span>
            <input type="number" min="0" step="0.01"
              .value=${a.cost!=null?String(a.cost):""}
              @input=${n=>{let o=n.target.value;this._set("cost",o?Number(o):null)}} />
          </label>
          <label>
            <span>${s("duration",e)||"Duration (min)"}</span>
            <input type="number" min="0"
              .value=${a.duration!=null?String(a.duration):""}
  ...
  `,_([v({attribute:!1})],E.prototype,"hass",2),_([d()],E.prototype,"_open",2),_([d()],E.prototype,"_saving",2),_([d()],E.prototype,"_error",2),_([d()],E.prototype,"_draft",2);customElements.get("maintenance-history-edit-dialog")||customElements.define("maintenance-history-edit-dialog",E);var jt="maintenance-object-dialog",St="maintenance-task-dialog",qt="maintenance-history-edit-dialog";function Nt(){return document.querySelector("home-assistant")?.hass}function de(i){let t=document.body.querySelector(i);return t||(t=document.createElement(i),document.body.appendChild(t)),t}function ue(i){let t=Nt();return t?(i.hass=t,!0):!1}function Re(){let i=de(jt);return ue(i)?(i.openCreate(),!0):!1}function Pe(i,t){let e=de(St);return ue(e)?(e.openEdit(i,{id:t}),!0):!1}function Me(i){let t=de(qt);return ue(t)?(t.openEdit(i),!0):!1}var Ue="maintenance-supporter",Fe=`ll-strategy-dashboard-${Ue}`,ge="hui-maintenance-supporter-strategy-editor",Ct="NOT_RUNNING",Tt=[{status:"overdue",title:"Overdue",icon:"mdi:alert-circle",path:"overdue"},{status:"triggered",title:"Triggered",icon:"mdi:flash",path:"triggered"},{status:"due_soon",title:"Due Soon",icon:"mdi:clock-alert-outline",path:"due-soon"},{status:"ok",title:"OK",icon:"mdi:check-circle-outline",path:"ok"}],Oe=[{title:"Overdue",icon:"mdi:alert-circle",path:"overdue",filter:{filter_due_max_days:-1},matches:i=>i<=-1},{title:"Today",icon:"mdi:calendar-today",path:"today",filter:{filter_due_min_days:0,filter_due_max_days:0},matches:i=>i===0},{title:"This Week",icon:"mdi:calendar-week",path:"this-week",filter:{filter_due_min_days:1,filter_due_max_days:7},matches:i=>i>=1&&i<=7},{title:"This Month",icon:"mdi:calendar-month",path:"this-month",filter:{filter_due_min_days:8,filter_due_max_days:30},matches:i=>i>=8&&i<=30},{title:"Later",icon:"mdi:calendar-clock",path:"later",filter:{filter_due_min_days:31},matches:i=>i>=31}];function $(i){return{type:"grid",cards:[i]}}var $t={condition:"view_columns",min:2};function It(i){let t={overdue:0,triggered:0,due_soon:0,ok:0,total:0};for(let e of i)for(let a of e.tasks||[])t.total++,a.status==="overdue"?t.overdue++:a.status==="triggered"?t.triggered++:a.status==="due_soon"?t.due_soon++:a.status==="ok"&&t.ok++;return t}function Dt(i){let t=[];return i.overdue&&t.push(`\u{1F534} **${i.overdue}** overdue`),i.triggered&&t.push(`\u26A1 **${i.triggered}** triggered`),i.due_soon&&t.push(`\u{1F7E1} **${i.due_soon}** due soon`),t.push(`\u{1F7E2} **${i.ok}** ok`),{type:"markdown",text_only:!0,content:t.join(" \xB7 ")}}function Rt(){return{title:"Maintenance",type:"panel",cards:[{type:"empty-state",icon:"mdi:wrench-clock",icon_color:"primary",content_only:!0,title:"No maintenance objects yet",content:"Open the Maintenance panel to add your first object \u2014 pool pump, HVAC filter, vehicle, anything that needs scheduled care.",buttons:[{icon:"mdi:wrench",text:"Open Maintenance panel",appearance:"filled",variant:"brand",tap_action:{action:"navigate",navigation_path:"/maintenance-supporter"}},{icon:"mdi:plus",text:"Add object",appearance:"outlined",variant:"brand",tap_action:{action:"fire-dom-event",ll_custom:{type:"maintenance-supporter:add-object"}}}]}]}}function Pt(i){let t=Dt(i);return{title:"Overview",icon:"mdi:wrench-clock",path:"overview",type:"sections",header:{layout:"responsive",card:t},sidebar:{sections:[{type:"grid",cards:[{type:"heading",heading:"Status",heading_style:"title"},t]}],visibility:[$t],content_label:"Tasks",sidebar_label:"Status"},sections:[$({type:"custom:maintenance-supporter-card",show_header:!1,filter_status:["overdue","triggered","due_soon"]})]}}function Mt(i,t){let e=new Map;for(let l of i){let r=l.object.area_id||null;e.has(r)||e.set(r,[]),e.get(r).push(l)}let a=Array.from(e.keys()).filter(l=>l!==null);a.sort((l,r)=>{let c=t[l]?.name||l,m=t[r]?.name||r;return c.localeCompare(m)});let n=[];for(let l of a){let r=e.get(l);if(r.length===0)continue;let c=t[l];n.push({title:c?.name||l,icon:c?.icon||"mdi:floor-plan",path:`area-${l}`,type:"sections",sections:[$({type:"custom:maintenance-supporter-card",show_header:!1,filter_objects:r.map(m=>m.object.name)})]})}let o=e.get(null);return o&&o.length>0&&n.push({title:"Unassigned",icon:"mdi:help-circle-outline",path:"unassigned",type:"sections",sections:[$({type:"custom:maintenance-supporter-card",show_header:!1,filter_objects:o.map(l=>l.object.name)})]}),n}function Ft(i){let t=new Set;for(let a of i)for(let n of a.tasks||[])n.status&&t.add(n.status);let e=[];for(let a of Tt)t.has(a.status)&&e.push({title:a.title,icon:a.icon,path:a.path,type:"sections",sections:[$({type:"custom:maintenance-supporter-card",show_header:!1,filter_status:[a.status]})]});return e}function Ot(i,t,e){let a=new Map;for(let r of i){let c=r.object.area_id,m=c&&t[c]?.floor_id||null;a.has(m)||a.set(m,[]),a.get(m).push(r)}let n=Array.from(a.keys()).filter(r=>r!==null);n.sort((r,c)=>{let m=e[r],f=e[c],h=m?.level??0,k=f?.level??0;return h!==k?h-k:(m?.name||r).localeCompare(f?.name||c)});let o=[];for(let r of n){let c=a.get(r);if(c.length===0)continue;let m=e[r];o.push({title:m?.name||r,icon:m?.icon||"mdi:home-floor-1",path:`floor-${r}`,type:"sections",sections:[$({type:"custom:maintenance-supporter-card",show_header:!1,filter_objects:c.map(f=>f.object.name)})]})}let l=a.get(null);return l&&l.length>0&&o.push({title:"Other",icon:"mdi:help-circle-outline",path:"other",type:"sections",sections:[$({type:"custom:maintenance-supporter-card",show_header:!1,filter_objects:l.map(r=>r.object.name)})]}),o}function Lt(i){return[{title:"Week",icon:"mdi:calendar-week",path:"cal-7",window_days:7},{title:"Fortnight",icon:"mdi:calendar-week-begin",path:"cal-14",window_days:14},{title:"Month",icon:"mdi:calendar-month",path:"cal-30",window_days:30},{title:"Year",icon:"mdi:calendar-clock",path:"cal-365",window_days:365}].map(e=>({title:e.title,icon:e.icon,path:e.path,type:"panel",cards:[{type:"custom:maintenance-supporter-calendar",window_days:e.window_days,show_window_chips:!1,show_user_filter:!0}]}))}function Ht(i){let t=new Set;for(let a of i)for(let n of a.tasks||[]){let o=n.days_until_due;o!=null&&Oe.forEach((l,r)=>{l.matches(o)&&t.add(r)})}let e=[];return Oe.forEach((a,n)=>{t.has(n)&&e.push({title:a.title,icon:a.icon,path:a.path,type:"sections",sections:[$({type:"custom:maintenance-supporter-card",show_header:!1,...a.filter})]})}),e}var me=class extends HTMLElement{static getCreateSuggestions(t){return{title:"Maintenance Supporter",icon:"mdi:wrench-clock"}}static async getConfigElement(){return document.createElement(ge)}static async generate(t,e){if(e.config?.state===Ct)return{views:[{type:"sections",sections:[{cards:[{type:"starting"}]}]}]};if(e.config?.recovery_mode)return{views:[{type:"sections",sections:[{cards:[{type:"recovery-mode"}]}]}]};let a;try{a=await e.connection.sendMessagePromise({type:"maintenance_supporter/objects"})}catch{return{title:"Maintenance",views:[{title:"Maintenance",cards:[{type:"markdown",content:"**Maintenance Supporter** is not loaded. Install/enable the integration first."}]}]}}let n=a.objects||[];if(n.length===0)return{title:"Maintenance",views:[Rt()]};let o=It(n),l=t?.group_by??"area",r={area:()=>Mt(n,e.areas||{}),status:()=>Ft(n),floor:()=>Ot(n,e.areas||{},e.floors||{}),due_date:()=>Ht(n),calendar:()=>Lt(n)};return{title:"Maintenance",views:[Pt(o),...(r[l]??r.area)()]}}},Ut=[{value:"area",label:"By area (default)"},{value:"status",label:"By status (Overdue / Triggered / Due Soon / OK)"},{value:"floor",label:"By floor (uses HA floors)"},{value:"due_date",label:"By due date (Overdue / Today / Week / Month / Later)"},{value:"calendar",label:"Rolling calendar (Week / Fortnight / Month / Year)"}],he=class extends HTMLElement{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter"}}set hass(e){this._hass=e}setConfig(e){this._config=e,this._render()}connectedCallback(){this._render()}_render(){let e=this._config.group_by??"area",a=Ut.map(o=>`<option value="${o.value}"${o.value===e?" selected":""}>${o.label}</option>`).join("");this.innerHTML=`
      <style>
      ...
      <div class="editor">
        ...
        <select id="group-by">${a}</select>
        <div class="help">
          The "Overview" view is always first. Empty groups are skipped.
        </div>
      </div>
    `;let n=this.querySelector("#group-by");n?.addEventListener("change",()=>{let o={...this._config,group_by:n.value};this._config=o,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:o},bubbles:!0,composed:!0}))})}},Ve="maintenance-supporter-section",Le=`ll-strategy-section-${Ve}`,fe=class extends HTMLElement{static async generate(t,e){let a={type:"custom:maintenance-supporter-card",show_header:!1};t?.filter_status?.length&&(a.filter_status=t.filter_status),t?.max_items&&(a.max_items=t.max_items),t?.filter_due_min_days!==void 0&&(a.filter_due_min_days=t.filter_due_min_days),t?.filter_due_max_days!==void 0&&(a.filter_due_max_days=t.filter_due_max_days);let n=t?.filter_objects;if(t?.area_id&&!n)try{n=((await e.connection.sendMessagePromise({type:"maintenance_supporter/objects"})).objects||[]).filter(r=>r.object.area_id===t.area_id).map(r=>r.object.name)}catch{}n&&n.length>0&&(a.filter_objects=n);let o=[];return t?.title&&o.push({type:"heading",heading:t.title,heading_style:"title"}),o.push(a),{type:"grid",cards:o}}};customElements.get(Fe)||customElements.define(Fe,me);customElements.get(ge)||customElements.define(ge,he);customElements.get(Le)||customElements.define(Le,fe);var He="_msSupporterLlCustomBound";function pe(i){history.pushState(null,"",i),window.dispatchEvent(new CustomEvent("location-changed"))}function Vt(){let i=window;i[He]||(i[He]=!0,document.addEventListener("ll-custom",t=>{let e=t.detail;if(!e||typeof e.type!="string"||!e.type.startsWith("maintenance-supporter:"))return;let a=e.type.slice(22);if(a==="add-object"){if(Re())return;pe("/maintenance-supporter?ms_action=add_object");return}if(a==="open-task"&&typeof e.entry_id=="string"&&typeof e.task_id=="string"){if(Pe(e.entry_id,e.task_id))return;pe(`/maintenance-supporter?entry_id=${encodeURIComponent(e.entry_id)}&task_id=${encodeURIComponent(e.task_id)}`);return}if(a==="edit-history"&&typeof e.entry_id=="string"&&typeof e.task_id=="string"&&typeof e.original_timestamp=="string"){let o=document.querySelector("home-assistant")?.hass;if(!o)return;(async()=>{try{let c=(await o.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:e.entry_id})).tasks?.find(m=>m.id===e.task_id)?.history?.find(m=>m.timestamp===e.original_timestamp);if(!c)return;Me({entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.original_timestamp,type:c.type||"completed",timestamp:c.timestamp||e.original_timestamp,notes:c.notes??null,cost:c.cost??null,duration:c.duration??null,completed_by:c.completed_by??null})}catch{pe("/maintenance-supporter")}})();return}}))}Vt();var Y=window;Y.customStrategies=Y.customStrategies||[];function Be(i){Y.customStrategies.some(e=>e.type===i.type&&e.strategyType===i.strategyType)||Y.customStrategies.push(i)}Be({type:Ue,strategyType:"dashboard",name:"Maintenance Supporter",description:"Auto-generated dashboard. Group views by area, status, floor, or due date \u2014 picked from the strategy editor or YAML.",documentationURL:"https://github.com/iluebbe/maintenance_supporter#dashboard-strategy"});Be({type:Ve,strategyType:"section",name:"Maintenance Supporter \u2014 Section",description:"Embed maintenance tasks (filterable by area, status, due date) as a section in any dashboard view.",documentationURL:"https://github.com/iluebbe/maintenance_supporter#section-strategy"});
