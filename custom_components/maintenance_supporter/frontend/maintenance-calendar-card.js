...
`).map(d=>d.trim()).filter(Boolean).slice(0,100);e.checklist=p.length?p:null}if(this.completionActionsEnabled){let p=this._actionService.trim();if(p&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(p)){let f={service:p},v=this._actionTargetEntity.trim();v&&(f.target={entity_id:v});let b=this._buildActionData();Object.keys(b).length>0&&(f.data=b),e.on_complete_action=f}else e.on_complete_action=null;let d={};this._qcNotes.trim()&&(d.notes=this._qcNotes.trim());let u=parseFloat(this._qcCost);!isNaN(u)&&u>=0&&(d.cost=u);let m=parseInt(this._qcDuration,10);!isNaN(m)&&m>=0&&(d.duration=m),this._qcFeedback&&(d.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(d).length?d:null}let i=await this.hass.connection.sendMessagePromise(e),n=this._taskId||i?.task_id,o=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(n&&this._scheduleType==="sensor_based"&&o)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:n,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}if(n&&this._adaptiveSnapshot()!==this._adaptiveInitial){let p=parseFloat(this._adaptiveAlpha),d=parseInt(this._adaptiveMin,10),u=parseInt(this._adaptiveMax,10);try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_adaptive",entry_id:this._entryId,task_id:n,enabled:this._adaptiveEnabled,...p>=.1&&p<=.9?{ewa_alpha:p}:{},...!isNaN(d)&&d>=1?{min_interval_days:d}:{},...!isNaN(u)&&u>=1?{max_interval_days:u}:{},seasonal_enabled:this._adaptiveSeasonal,sensor_prediction_enabled:this._adaptivePrediction}),this._adaptiveInitial=this._adaptiveSnapshot()}catch{}}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=N(e,this._lang,a("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1,this._pickerProbeTimer!==void 0&&(clearTimeout(this._pickerProbeTimer),this._pickerProbeTimer=void 0),this._pickerProbeStrikes=0}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return _;let e=this._lang,t=this._triggerType==="compound";return l`
      ...
      <div class="select-row">
        ...
        >
          ${Fs.map(i=>l`<option value=${i} ?selected=${i===this._triggerType}>${a(i,e)}</option>`)}
        </select>
      </div>
      ...
      ${this._intervalDays?l`
          ...
          `:_}
    `}_patchCondition(e,t){this._compoundConditions=this._compoundConditions.map((i,n)=>n===e?{...i,...t}:i)}_addCondition(){this._compoundConditions=[...this._compoundConditions,zs()]}_removeCondition(e){this._compoundConditions=this._compoundConditions.filter((t,i)=>i!==e)}_renderCompoundEditor(){let e=this._lang;return l`
      <div class="select-row">
    ...
    `}_buildSchedule(){let e=i=>{let n=parseInt(this._calOffset,10)||0;return n&&(i.offset=Math.max(-15,Math.min(n,15))),i};if(this._scheduleType==="weekdays")return e({kind:"weekdays",weekdays:[...this._weekdays].sort((i,n)=>i-n)});if(this._scheduleType==="nth_weekday")return e({kind:"nth_weekday",nth:parseInt(this._nth,10),weekday:parseInt(this._nthWeekday,10)});let t={kind:"day_of_month",day:this._domLastDay?-1:parseInt(this._domDay,10)||1};return this._domBusiness&&(t.business=!0),e(t)}_recurrenceExtras(){let e={};if(this._seasonMonths.length&&(e.season_months=[...this._seasonMonths].sort((t,i)=>t-i)),this._endsMode==="count"){let t=parseInt(this._endsCount,10);t>=1&&(e.ends={count:t})}else this._endsMode==="until"&&this._endsUntil&&(e.ends={until:this._endsUntil});return e}_toggleSeasonMonth(e){this._seasonMonths=this._seasonMonths.includes(e)?this._seasonMonths.filter(t=>t!==e):[...this._seasonMonths,e]}_renderRecurrenceExtras(){let e=this._lang;if(!(this._scheduleType==="time_based"||yt.includes(this._scheduleType)))return _;let i=Ks(e);return l`
      ...
      ${this._endsMode==="until"?l`
        ...
        ></ms-textfield>`:_}
    `}_renderCalendarFields(){let e=this._lang,t=Vs(e);if(this._scheduleType==="weekdays")return l`
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${Ds.map(i=>l`<option value=${i} ?selected=${i===this._type}>${a(i,e)}</option>`)}
            </select>
          </div>
          ...
          <div class="select-row">
            ...
            >
              ${Ms.map(i=>l`<option value=${i} ?selected=${i===this._priority}>${a("priority_"+i,e)}</option>`)}
            </select>
          </div>
          <div class="field">
          ...
          <div class="select-row">
            ...
            >
              ${Os.map(i=>l`<option value=${i} ?selected=${i===this._scheduleType}>${a(i,e)}</option>`)}
            </select>
          </div>
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function er(r,s,e){let t=r.confidence_interval_low,i=r.confidence_interval_high,n=s.suggested_interval??s.interval_days??0,o=s.interval_days??0,p=Math.max(0,t-5),u=i+5-p,m=(t-p)/u*100,f=(i-t)/u*100,v=(n-p)/u*100,b=o>0?(o-p)/u*100:-1;return l`
  ...
  `}var Mi=w(()=>{"use strict";P();R()});function Oi(r,s,e,t){let i=Math.max(r||1,s);return l`
  ...
  `}function Bi(r,s){return tr(r,s)}function tr(r,s){let e=r.seasonal_factors??r.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return _;let t=r.interval_analysis?.seasonal_reason,i=new Date().getMonth(),n=300,o=100,p=8,u=o-p-4,m=Math.max(...e,1.5),f=n/12,v=f*.65,b=p+u-1/m*u;return l`
        ...
        ${t&&i?l`
              ...
              ${i.notes?l`
                    <div class="notes-section">
                      ...
                      <div class="notes-body">${Yi(i.notes)}</div>
                    </div>
                  `:_}
  ...
  `,c([x({attribute:!1})],V.prototype,"hass",2),c([h()],V.prototype,"_open",2),c([h()],V.prototype,"_entryId",2),c([h()],V.prototype,"_data",2),c([h()],V.prototype,"_busy",2),c([h()],V.prototype,"_error",2);customElements.get("maintenance-object-quick-actions-dialog")||customElements.define("maintenance-object-quick-actions-dialog",V)});var J={};as(J,{openCompleteDialog:()=>ur,openCreateObjectDialog:()=>lr,openCreateTaskDialog:()=>dr,openEditObjectDialog:()=>cr,openEditTaskDialog:()=>pr,openHistoryEditDialog:()=>wt,openObjectQuickActions:()=>_r,openQrDialog:()=>hr,openTaskQuickActions:()=>$t});function Ze(){return document.querySelector("home-assistant")?.hass}function or(){return document.querySelector("home-assistant")?.shadowRoot??document.body}function Z(r){let s=or(),e=s.querySelector(r)??document.body.querySelector(r);return e?e.parentNode!==s&&s.appendChild(e):(e=document.createElement(r),s.appendChild(e)),e}function X(r){let s=Ze();if(!s)return!1;r.hass=s;let e=j(s);return Re(e)||Ne(e).then(()=>{r.requestUpdate?.()}),!0}function ts(r){return Je||(Je=r.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).then(s=>({features:s.features??Zi.features,defaultWarningDays:s.general?.default_warning_days??7})).catch(()=>Zi),Je)}function lr(){let r=Z(Xi);return X(r)?(r.openCreate(),!0):!1}function cr(r,s){let e=Z(Xi);return X(e)?(e.openEdit(r,s),!0):!1}function dr(r="",s){let e=Z(es);if(!X(e))return!1;let t=Ze();return t?((async()=>{let i=await ts(t),n=e;n.checklistsEnabled=i.features.checklists,n.scheduleTimeEnabled=i.features.schedule_time,n.completionActionsEnabled=i.features.completion_actions,n.defaultWarningDays=i.defaultWarningDays,n.openCreate(r,s)})(),!0):!1}function pr(r,s){let e=Z(es);if(!X(e))return!1;let t=Ze();return t?((async()=>{try{let[i,n]=await Promise.all([t.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:r}),ts(t)]),o=(i.tasks||[]).find(d=>d.id===s);if(!o){console.warn(`openEditTaskDialog: task ${s} not found in entry ${r}`);return}let p=e;p.checklistsEnabled=n.features.checklists,p.scheduleTimeEnabled=n.features.schedule_time,p.completionActionsEnabled=n.features.completion_actions,p.defaultWarningDays=n.defaultWarningDays,await p.openEdit(r,o)}catch(i){console.warn("openEditTaskDialog: failed to load task/features",i)}})(),!0):!1}function wt(r){let s=Z(ir);return X(s)?(s.openEdit(r),!0):!1}function ur(r){let s=Z(sr);return X(s)?(s.entryId=r.entry_id,s.taskId=r.task_id,s.taskName=r.task_name,s.checklist=r.checklist??[],s.adaptiveEnabled=!!r.adaptive_enabled,s.requiredFields=r.required_completion_fields??[],s.taskType=r.task_type??"",s.readingUnit=r.reading_unit??"",s.parts=r.parts??[],s.consumesParts=r.consumes_parts??[],s.phaseLabel=r.phase_label??"",s.lang=Ze()?.language||"en",s.open(),!0):!1}function hr(r){let s=Z(rr);return X(s)?(s.openForTask(r.entry_id,r.task_id,r.object_name,r.task_name),!0):!1}function $t(r,s){let e=Z(ar);return X(e)?(e.openFor(r,s),!0):!1}function _r(r){let s=Z(nr);return X(s)?(s.openFor(r),!0):!1}var Xi,es,ir,sr,rr,ar,nr,Zi,Je,W=w(()=>{"use strict";xi();Ti();Ii();Ci();Ri();Vi();Ji();R();Xi="maintenance-object-dialog",es="maintenance-task-dialog",ir="maintenance-history-edit-dialog",sr="maintenance-complete-dialog",rr="maintenance-qr-dialog",ar="maintenance-task-quick-actions-dialog",nr="maintenance-object-quick-actions-dialog";Zi={features:{adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1},defaultWarningDays:7},Je=null});P();U();ht();P();var ri=S`
  .cal-controls {
    display: flex;
    gap: 12px;
    align-items: center;
...
`;R();function bi(r){let s=window;s.customCards=s.customCards||[],s.customCards.some(e=>e.type===r.type)||s.customCards.push(r)}W();var H=class extends A{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._objectFilter="";this._configuredObjects=[];this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){if(this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter),typeof e.object_filter=="string")this._objectFilter=e.object_filter,this._configuredObjects=[];else if(Array.isArray(e.object_filter)){let t=e.object_filter.filter(i=>typeof i=="string"&&i!=="");this._objectFilter=t.length===1?t[0]:"",this._configuredObjects=t.length>1?t:[]}}getCardSize(){return 6}get _lang(){return j(this.hass)}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){if(super.updated(e),ui(this,e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{let e=await this.hass.connection.subscribeMessage(t=>{let i=t;this._objects=i.objects},{type:"maintenance_supporter/subscribe"});if(!this.isConnected){e();return}this._unsub=e}catch{}}_onEventClick(e){if(e.history_timestamp){this._openHistoryEntry(e);return}$t(e.entry_id,e.task_id)||this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}async _openHistoryEntry(e){try{let i=(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:e.entry_id})).tasks?.find(o=>o.id===e.task_id)?.history?.find(o=>o.timestamp===e.history_timestamp);if(!i||wt({entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp,type:i.type||"completed",timestamp:i.timestamp||e.history_timestamp,notes:i.notes??null,cost:i.cost??null,duration:i.duration??null,completed_by:i.completed_by??null,used_parts:i.used_parts??null}))return}catch{}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}))}render(){if(!this.hass)return _;let e=this._lang,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,n=this._config.title,o=null;this._userFilter&&(o=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let p=y=>{let z=y.toLowerCase();return this._objects.find(C=>C.entry_id===y||C.object.name.toLowerCase()===z)?.entry_id??null},d=new Set(this._configuredObjects.map(p).filter(y=>y!==null)),u=d.size?this._objects.filter(y=>d.has(y.entry_id)):this._objects,m=this._config.show_object_filter!==!1&&u.length>1,f=this._objectFilter?p(this._objectFilter):null,v=f&&u.some(y=>y.entry_id===f)?u.filter(y=>y.entry_id===f):u,b=new Date;b.setHours(0,0,0,0);let k=this._pastDays>0,E=k?si(v,b,this._pastDays,o):ii(v,b,this._windowDays,o),O=ne(b),K=this._windowDays===365||k,D=K?E.filter(y=>y.events.length>0):E,ce=y=>{let z=`cal-status-${y.status}`,pe=y.projected?"cal-event-projected":"",C=y.status==="overdue"&&y.days_until_due!=null?` (${mi(y.days_until_due,e)})`:"",q=y.projected&&y.interval_days?l`<span class="cal-event-recur">${y.interval_unit&&y.interval_unit!=="days"?`${y.interval_days} ${a("unit_"+y.interval_unit,e)}`:a("cal_every_n_days",e).replace("{n}",String(y.interval_days))}</span>`:_,te=y.schedule_type==="sensor_based",ye=te?l`<ha-icon class="cal-event-icon cal-source-sensor"
          ...
          </span>`:_,is=this._stats?.budget?.currency_symbol||di,ss=y.history_type?a(y.history_type,e):a(y.status,e);return l`
        <div class="cal-event ${pe}"
          @click=${()=>this._onEventClick(y)}>
          ${ye}
          <span class="cal-status-pill ${z}">${ss}</span>
          <div class="cal-event-body">
            <div class="cal-event-title">${y.object_name} · ${y.task_name}${C}</div>
            ${be}
            ${q}
          </div>
          ${y.avg_cost!=null&&y.avg_cost>0?l`<span class="cal-event-cost">${y.avg_cost.toFixed(0)} ${is}</span>`:_}
        </div>
      `},de=y=>{let[z,pe,C]=y.date.split("-").map(Number),q=new Date(z,pe-1,C),te=y.date===O,ye=q.toLocaleDateString(e,{weekday:"short"}),be=q.toLocaleDateString(e,{month:"long"});return l`
    ...
    `],c([x({attribute:!1})],H.prototype,"hass",2),c([h()],H.prototype,"_config",2),c([h()],H.prototype,"_objects",2),c([h()],H.prototype,"_stats",2),c([h()],H.prototype,"_windowDays",2),c([h()],H.prototype,"_pastDays",2),c([h()],H.prototype,"_userFilter",2),c([h()],H.prototype,"_objectFilter",2),c([h()],H.prototype,"_unsub",2);var gr=[{value:7,key:"cal_editor_window_week"},{value:14,key:"cal_editor_window_fortnight"},{value:30,key:"cal_editor_window_month"},{value:365,key:"cal_editor_window_year"}],ve=class extends A{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"}}get _lang(){return j(this.hass)}setConfig(e){this._config={...e}}updated(){let e=this._lang;e&&!Re(e)&&Ne(e).then(()=>this.requestUpdate())}_valueChanged(e,t){let i={...this._config,[e]:t};e==="show_window_chips"&&t===!0&&delete i.show_window_chips,e==="show_user_filter"&&t===!0&&delete i.show_user_filter,e==="show_object_filter"&&t===!0&&delete i.show_object_filter,e==="title"&&(!t||typeof t=="string"&&t.trim()==="")&&delete i.title,e==="user_filter"&&t===""&&delete i.user_filter,this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}render(){let e=this._lang,t=this._config.window_days??30,i=this._config.show_window_chips!==!1,n=this._config.show_user_filter!==!1,o=this._config.user_filter??"",p=this._config.title??"";return l`
      <div class="editor">
        ...
        <div class="row">
          ...
          >
            ${gr.map(d=>l`<option value="${d.value}" ?selected=${d.value===t}>${a(d.key,e)}</option>`)}
          </select>
        </div>
