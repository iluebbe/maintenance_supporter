...
`).map(d=>d.trim()).filter(Boolean).slice(0,100);e.checklist=p.length?p:null}if(this.completionActionsEnabled){let p=this._actionService.trim();if(p&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(p)){let f={service:p},b=this._actionTargetEntity.trim();b&&(f.target={entity_id:b});let v=this._buildActionData();Object.keys(v).length>0&&(f.data=v),e.on_complete_action=f}else e.on_complete_action=null;let d={};this._qcNotes.trim()&&(d.notes=this._qcNotes.trim());let u=parseFloat(this._qcCost);!isNaN(u)&&u>=0&&(d.cost=u);let m=parseInt(this._qcDuration,10);!isNaN(m)&&m>=0&&(d.duration=m),this._qcFeedback&&(d.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(d).length?d:null}let i=await this.hass.connection.sendMessagePromise(e),n=this._taskId||i?.task_id,l=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(n&&this._scheduleType==="sensor_based"&&l)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:n,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}if(n&&this._adaptiveSnapshot()!==this._adaptiveInitial){let p=parseFloat(this._adaptiveAlpha),d=parseInt(this._adaptiveMin,10),u=parseInt(this._adaptiveMax,10);try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_adaptive",entry_id:this._entryId,task_id:n,enabled:this._adaptiveEnabled,...p>=.1&&p<=.9?{ewa_alpha:p}:{},...!isNaN(d)&&d>=1?{min_interval_days:d}:{},...!isNaN(u)&&u>=1?{max_interval_days:u}:{},seasonal_enabled:this._adaptiveSeasonal,sensor_prediction_enabled:this._adaptivePrediction}),this._adaptiveInitial=this._adaptiveSnapshot()}catch{}}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=j(e,this._lang,a("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1,this._pickerProbeTimer!==void 0&&(clearTimeout(this._pickerProbeTimer),this._pickerProbeTimer=void 0),this._pickerProbeStrikes=0}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return _;let e=this._lang,t=this._triggerType==="compound";return o`
      ...
      <div class="select-row">
        ...
        >
          ${Ws.map(i=>o`<option value=${i} ?selected=${i===this._triggerType}>${a(i,e)}</option>`)}
        </select>
      </div>
    ...
    `}_renderCondition(e,t){let i=this._lang,n=t+1;return o`
      <div class="compound-condition">
        ...
        <div class="select-row">
          ...
          >
            ${qi.map(l=>o`<option value=${l} ?selected=${l===e.type}>${a(l,i)}</option>`)}
          </select>
        </div>
    ...
    `}_renderCalendarFields(){let e=this._lang,t=Qs(e);if(this._scheduleType==="weekdays")return o`
      ...
      ></ms-textfield>`}_thresholdLimitsOverlap(){let e=parseFloat(this._triggerAbove),t=parseFloat(this._triggerBelow);return!isNaN(e)&&!isNaN(t)&&t>e}_renderTriggerLiveHint(){if(this._triggerType==="compound")return _;let e=this._triggerType==="threshold"&&this._thresholdLimitsOverlap()?o`<div class="trigger-live-hint warn">${a("trigger_hint_overlap",this._lang)}</div>`:_,t=this._triggerEntityId||this._triggerEntityIds[0];if(!t||!this.hass?.states)return e;let i=this.hass.states[t];if(!i)return e;let n=this._lang,l=i.attributes?.unit_of_measurement,p=typeof l=="string"&&l?` ${l}`:"",d=this._triggerAttribute?i.attributes?.[this._triggerAttribute]:i.state,u=typeof d=="number"?d:parseFloat(String(d)),m=d!=="unknown"&&d!=="unavailable"&&d!=null&&!isNaN(u),f=v=>Number.isInteger(v)?String(v):String(Math.round(v*10)/10),b=[];if(this._triggerType==="threshold"){let v=parseFloat(this._triggerAbove),k=parseFloat(this._triggerBelow);if(isNaN(v)&&isNaN(k))return _;m&&b.push(a("trigger_hint_now",n).replace("{value}",f(u)+p)),isNaN(v)||b.push(a("trigger_hint_above",n).replace("{target}",f(v)+p)),isNaN(k)||b.push(a("trigger_hint_below",n).replace("{target}",f(k)+p))}else if(this._triggerType==="counter"){let v=parseFloat(this._triggerTargetValue);if(isNaN(v))return _;this._triggerDeltaMode?this._taskId?b.push(a("trigger_hint_counter_delta_edit",n).replace("{target}",f(v)+p)):m?b.push(a("trigger_hint_counter_delta",n).replace("{value}",f(u)+p).replace("{due}",f(u+v)+p).replace("{target}",f(v)+p)):b.push(a("trigger_hint_counter_delta_edit",n).replace("{target}",f(v)+p)):(m&&b.push(a("trigger_hint_now",n).replace("{value}",f(u)+p)),b.push(a("trigger_hint_counter_abs",n).replace("{target}",f(v)+p)))}else if(this._triggerType==="runtime"){let v=parseFloat(this._triggerRuntimeHours);if(isNaN(v))return _;b.push(a("trigger_hint_runtime",n).replace("{hours}",f(v))),b.push(a("trigger_hint_state_now",n).replace("{value}",String(i.state)))}else if(this._triggerType==="state_change"){let v=parseInt(this._triggerTargetChanges,10)||1,k=this._triggerToState.trim();b.push((k?a("trigger_hint_state_change_to",n).replace("{state}",k):a("trigger_hint_state_change",n)).replace("{count}",String(v))),b.push(a("trigger_hint_state_now",n).replace("{value}",String(i.state)))}return b.length?o`<div class="trigger-live-hint">${b.join(" ")}</div>${e}`:e}_renderTriggerTypeFields(){let e=this._lang;return this._triggerType==="threshold"?o`
        <ms-textfield
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${zs.map(i=>o`<option value=${i} ?selected=${i===this._type}>${a(i,e)}</option>`)}
            </select>
          </div>
          ...
          <div class="select-row">
            ...
            >
              ${Us.map(i=>o`<option value=${i} ?selected=${i===this._priority}>${a("priority_"+i,e)}</option>`)}
            </select>
          </div>
          <div class="field">
          ...
          <div class="select-row">
            ...
            >
              ${Bs.map(i=>o`<option value=${i} ?selected=${i===this._scheduleType}>${a(i,e)}</option>`)}
            </select>
          </div>
          ...
          ${this._scheduleType==="sensor_based"?o`
            ...
            <ha-form
              class="entity-picker-form"
              .hass=${this.hass}
              .schema=${[{name:"environmental_entity",selector:{entity:{domain:Ci,device_class:Pi}}}]}
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          ${this.parts.length?o`<div class="used-parts">
                ...
                ${this.parts.map(t=>{let i=V({part_id:t.id,entry_id:t.entry_id}),n=this._usedParts[i],l=n!==void 0,p=t.entry_id?{part_id:t.id,quantity:1,entry_id:t.entry_id}:{part_id:t.id,quantity:1};return o`<div class="used-part-row">
        ...
        <div class="dialog-actions">
          ...
          <ha-button
            ...
            title=${this._missingRequired.length?this._missingRequired.map(t=>a("err_required",e).replace("{field}",a(je[t]??t,e))).join(" \xB7 "):""}
          >
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function ir(r,s,e,t,i){let v=Math.max(e,t,s,1)*1.3,k=50,E=[];for(let C=0;C<=k;C++){let R=C/k*v,se=1-Math.exp(-Math.pow(R/s,r)),ye=32+R/v*260,be=136-se*128;E.push([ye,be])}let H=E.map(([C,R])=>`${C.toFixed(1)},${R.toFixed(1)}`).join(" "),M="M32,136 "+E.map(([C,R])=>`L${C.toFixed(1)},${R.toFixed(1)}`).join(" ")+` L${E[k][0].toFixed(1)},136 Z`,q=32+e/v*260,z=1-Math.exp(-Math.pow(e/s,r)),te=136-z*128,y=((1-z)*100).toFixed(0),B=32+t/v*260,pe=[0,.25,.5,.75,1];return o`
  ...
  `}function rr(r,s,e){let t=r.confidence_interval_low,i=r.confidence_interval_high,n=s.suggested_interval??s.interval_days??0,l=s.interval_days??0,p=Math.max(0,t-5),u=i+5-p,m=(t-p)/u*100,f=(i-t)/u*100,b=(n-p)/u*100,v=l>0?(l-p)/u*100:-1;return o`
      ...
      <div class="prediction-grid">
        ...
        ${i?o`
          <div class="prediction-item">
            ...
            ${r.threshold_prediction_date?o`<span class="prediction-date">${Q(r.threshold_prediction_date,s)}</span>`:_}
        ...
        ${n&&e.environmental?o`
          <div class="prediction-item">
            ...
            ${r.environmental_entity?o`<span class="prediction-entity entity-link" @click=${p=>wi(p,r.environmental_entity)}>${r.environmental_entity}</span>`:_}
          </div>
        `:_}
      </div>
    </div>
  `}var Vi=w(()=>{"use strict";P();N()});function Ki(r,s,e,t){let i=Math.max(r||1,s);return o`
  ...
  `}var Gi=w(()=>{"use strict";P();N()});function Qi(r,s,e){if(!e.seasonal||!r.seasonal_factor||r.seasonal_factor===1)return _;let t=Yi.map(p=>a(p,s)),i=new Date().getMonth(),n=r.seasonal_factors||r.interval_analysis?.seasonal_factors||null,l=n&&n.length===12?n:t.map((p,d)=>{let u=r.seasonal_factor||1,m=Math.sin((d-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,u+m))});return o`
  ...
  `}function Ji(r,s){return ar(r,s)}function ar(r,s){let e=r.seasonal_factors??r.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return _;let t=r.interval_analysis?.seasonal_reason,i=new Date().getMonth(),n=300,l=100,p=8,u=l-p-4,m=Math.max(...e,1.5),f=n/12,b=f*.65,v=p+u-1/m*u;return o`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${n} ${l}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${a("chart_seasonal",s)}">
        ...
        ${e.map((k,E)=>{let H=k/m*u,M=E*f+(f-b)/2,q=p+u-H,z=E===i,te=k<1?"var(--success-color, #4caf50)":k>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return _e`
  ...
  `}var Yi,Zi=w(()=>{"use strict";P();N();Yi=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"]});var I,Xi=w(()=>{"use strict";P();W();N();de();ht();$t();wt();Bi();Vi();Gi();Zi();I=class extends A{constructor(){super(...arguments);this._open=!1;this._entryId=null;this._taskId=null;this._task=null;this._objectName="";this._busy=!1;this._error="";this._showSkip=!1;this._showReset=!1;this._showDetails=!1;this._showAdaptive=!1;this._skipReason="";this._resetDate="";this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._toast="";this._featuresLoaded=!1;this._currencySymbol=""}get _lang(){return D(this.hass)}async openFor(e,t){this._entryId=e,this._taskId=t,this._error="",this._showSkip=!1,this._showReset=!1,this._showAdaptive=!1,this._skipReason="",this._resetDate=le(new Date),this._open=!0,await Promise.all([this._loadTask(),this._loadFeatures()])}async _loadFeatures(){if(!this._featuresLoaded)try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});e?.features&&(this._features={...this._features,...e.features}),this._currencySymbol=e?.budget?.currency_symbol||"",this._featuresLoaded=!0}catch{}}close(){this._open=!1,this._task=null,this._error=""}async _loadTask(){if(!(!this._entryId||!this._taskId))try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._objectName=e.object?.name||"";let t=(e.tasks||[]).find(i=>i.id===this._taskId);this._task=t??null}catch(e){this._error=j(e,this._lang)}}async _runWs(e){this._busy=!0,this._error="";try{return await this.hass.connection.sendMessagePromise(e),this._busy=!1,!0}catch(t){return this._error=j(t,this._lang),this._busy=!1,!1}}_notifyChanged(e){this.dispatchEvent(new CustomEvent("task-action-fired",{detail:{entry_id:this._entryId,task_id:this._taskId,action:e},bubbles:!0,composed:!0}))}_onComplete(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(K(),Z)).then(async({openCompleteDialog:e})=>{let t=this._task,i=[];try{i=(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects",compact:!0})).objects||[]}catch{}e(Fi({entryId:this._entryId,taskId:this._taskId,taskName:t.name,task:t,objects:i,lang:this._lang,checklist:t.checklist||[],adaptiveEnabled:!!t.adaptive_config?.enabled,currencySymbol:this._currencySymbol}))&&(this._notifyChanged("complete"),this.close())})}async _onSkipConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/skip",entry_id:this._entryId,task_id:this._taskId,reason:this._skipReason.trim()||null})&&(this._notifyChanged("skip"),this.close())}async _onResetConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/reset",entry_id:this._entryId,task_id:this._taskId,date:this._resetDate||void 0})&&(this._notifyChanged("reset"),this.close())}_onEdit(){!this._entryId||!this._taskId||Promise.resolve().then(()=>(K(),Z)).then(({openEditTaskDialog:e})=>{e(this._entryId,this._taskId),this.close()})}_onQr(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(K(),Z)).then(({openQrDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,object_name:this._objectName}),this.close()})}async _onDelete(){if(!this._entryId||!this._taskId)return;let e=a("delete_task_confirm",this._lang)||`Delete "${this._task?.name}"?`;if(!window.confirm(e))return;await this._runWs({type:"maintenance_supporter/task/delete",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("delete"),this.close())}async _onArchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/archive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("archive"),this.close())}async _onUnarchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/unarchive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("unarchive"),this.close())}_onOpenInPanel(){if(!this._entryId||!this._taskId)return;let e=`/maintenance-supporter?entry_id=${encodeURIComponent(this._entryId)}&task_id=${encodeURIComponent(this._taskId)}`;history.pushState(null,"",e),window.dispatchEvent(new CustomEvent("location-changed")),this.close()}async _applySuggestion(){if(!this._entryId||!this._taskId||!this._task?.suggested_interval)return;await this._runWs({type:"maintenance_supporter/task/apply_suggestion",entry_id:this._entryId,task_id:this._taskId,interval:this._task.suggested_interval})&&(this._toast=a("suggestion_applied",this._lang)||"Applied",this._notifyChanged("apply_suggestion"),await this._loadTask(),setTimeout(()=>{this._toast=""},2500))}async _reanalyzeInterval(){if(!(!this._entryId||!this._taskId)){this._busy=!0,this._error="";try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:this._entryId,task_id:this._taskId});this._toast=e.recommended_interval?`${a("reanalyze_result",this._lang)||"Recomputed"}: ${Ge(e.recommended_interval,"days",this._lang)} (${e.data_points} pts)`:a("reanalyze_insufficient_data",this._lang)||"Not enough data",await this._loadTask(),setTimeout(()=>{this._toast=""},3500)}catch(e){this._error=j(e,this._lang)}finally{this._busy=!1}}}_onEditHistoryEntry(e){!this._entryId||!this._taskId||Promise.resolve().then(()=>(K(),Z)).then(({openHistoryEditDialog:t})=>{t({entry_id:this._entryId,task_id:this._taskId,original_timestamp:e.timestamp,type:e.type,timestamp:e.timestamp,notes:e.notes??null,cost:e.cost??null,duration:e.duration??null,completed_by:e.completed_by??null,used_parts:e.used_parts??null})})}_renderRecommendation(e){if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return _;let t=this._lang;return o`
    ...
    `}_renderAdaptive(e){let t=this._lang,i=this._features.adaptive&&e.suggested_interval&&e.suggested_interval!==e.interval_days,n=e.degradation_trend!=null&&e.degradation_trend!=="insufficient_data"||e.days_until_threshold!=null||e.environmental_factor!=null&&e.environmental_factor!==1,l=this._features.adaptive&&e.interval_analysis?.weibull_beta!=null&&e.interval_analysis?.weibull_eta!=null,p=this._features.seasonal&&e.seasonal_factor&&e.seasonal_factor!==1;return!i&&!n&&!l&&!p?o`<div class="adaptive-empty">
      ...
      <div class="adaptive-stack">
        ...
        ${n?Wi(e,t,this._features):_}
        ${l?Ui(e,t):_}
        ${p?o`
          ${Qi(e,t,this._features)}
          ${e.seasonal_factors?.length===12||e.interval_analysis?.seasonal_factors?.length===12?Ji(e,t):_}
        `:_}
      </div>
                ...
                ${[...i].reverse().slice(0,20).map(d=>{let u=["completed","reset","skipped"].includes(d.type);return o`
                    <div class="history-entry">
                      <div class="history-line">
                        ...
                        <span class="history-date">${yi(d.timestamp,t)}</span>
        ...
        ${t?o`
              <div class="header">
                ...
                <div class="quick-info">
                  ${t.next_due?o`<span><strong>${a("next_due",e)||"Next due"}:</strong> ${Q(t.next_due,e)}</span>`:_}
                  ${t.last_performed?o`<span><strong>${a("last_performed",e)||"Last"}:</strong> ${Q(t.last_performed,e)}</span>`:_}
                  ${t.schedule?.kind&&!["manual","one_time"].includes(t.schedule.kind)||t.interval_days!=null?o`<span><strong>${a("interval",e)||"Interval"}:</strong> ${xi(t,e)}</span>`:_}
                  ${He(t)?o`<span><strong>${a("phase_current",e)}:</strong> ${He(t)}</span>`:_}
                </div>
              </div>
              ...
              ${this._showSkip?o`
                  ...
                  `:o`
                    <div class="actions primary-row">
                      ...
                      ${t.allow_skip!==!1?o`
                            <ha-button appearance="outlined" variant="neutral" @click=${()=>{this._showSkip=!0}} .disabled=${this._busy}>
        ...
        ${t&&i?o`
              ...
              ${i.notes?o`
                    <div class="notes-section">
                      ...
                      <div class="notes-body">${is(i.notes)}</div>
                    </div>
                  `:_}
      ...
      <div class="meta">
        ${i.map(([n,l])=>o`
            <div class="meta-item">
              ...
              <span class="meta-value">${es(l)?o`<a href="${l}" target="_blank" rel="noopener noreferrer">${l}</a>`:l}</span>
            </div>
          `)}
      </div>
    `}};G.styles=S`
  ...
  `,c([x({attribute:!1})],G.prototype,"hass",2),c([h()],G.prototype,"_open",2),c([h()],G.prototype,"_entryId",2),c([h()],G.prototype,"_data",2),c([h()],G.prototype,"_busy",2),c([h()],G.prototype,"_error",2);customElements.get("maintenance-object-quick-actions-dialog")||customElements.define("maintenance-object-quick-actions-dialog",G)});var Z={};ds(Z,{__resetSettingsCacheForTests:()=>hr,getRowActionStyle:()=>ur,openCompleteDialog:()=>vr,openCreateObjectDialog:()=>_r,openCreateTaskDialog:()=>mr,openEditObjectDialog:()=>gr,openEditTaskDialog:()=>fr,openHistoryEditDialog:()=>St,openObjectQuickActions:()=>br,openQrDialog:()=>yr,openTaskQuickActions:()=>At});function Ze(){return document.querySelector("home-assistant")?.hass}function pr(){return document.querySelector("home-assistant")?.shadowRoot??document.body}function X(r){let s=pr(),e=s.querySelector(r)??document.body.querySelector(r);return e?e.parentNode!==s&&s.appendChild(e):(e=document.createElement(r),s.appendChild(e)),e}function ee(r){let s=Ze();if(!s)return!1;r.hass=s;let e=D(s);return Re(e)||Ne(e).then(()=>{r.requestUpdate?.()}),!0}function ur(r){return Et(r).then(s=>s.rowActionStyle)}function hr(){De=null}function Et(r){return De||(De=r.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).then(s=>({features:s.features??kt.features,defaultWarningDays:s.general?.default_warning_days??7,rowActionStyle:s.general?.row_action_style??kt.rowActionStyle})).catch(()=>kt),De)}function _r(){let r=X(as);return ee(r)?(r.openCreate(),!0):!1}function gr(r,s){let e=X(as);return ee(e)?(e.openEdit(r,s),!0):!1}function mr(r="",s){let e=X(ns);if(!ee(e))return!1;let t=Ze();return t?((async()=>{let i=await Et(t),n=e;n.checklistsEnabled=i.features.checklists,n.scheduleTimeEnabled=i.features.schedule_time,n.completionActionsEnabled=i.features.completion_actions,n.defaultWarningDays=i.defaultWarningDays,n.openCreate(r,s)})(),!0):!1}function fr(r,s){let e=X(ns);if(!ee(e))return!1;let t=Ze();return t?((async()=>{try{let[i,n]=await Promise.all([t.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:r}),Et(t)]),l=(i.tasks||[]).find(d=>d.id===s);if(!l){console.warn(`openEditTaskDialog: task ${s} not found in entry ${r}`);return}let p=e;p.checklistsEnabled=n.features.checklists,p.scheduleTimeEnabled=n.features.schedule_time,p.completionActionsEnabled=n.features.completion_actions,p.defaultWarningDays=n.defaultWarningDays,await p.openEdit(r,l)}catch(i){console.warn("openEditTaskDialog: failed to load task/features",i)}})(),!0):!1}function St(r){let s=X(nr);return ee(s)?(s.openEdit(r),!0):!1}function vr(r){let s=X(or);return ee(s)?(zi(s,r,Ze()?.language||"en"),!0):!1}function yr(r){let s=X(lr);return ee(s)?(s.openForTask(r.entry_id,r.task_id,r.object_name,r.task_name),!0):!1}function At(r,s){let e=X(cr);return ee(e)?(e.openFor(r,s),!0):!1}function br(r){let s=X(dr);return ee(s)?(s.openFor(r),!0):!1}var as,ns,nr,or,lr,cr,dr,kt,De,K=w(()=>{"use strict";Ei();Ri();Ni();ji();Oi();Xi();rs();N();$t();as="maintenance-object-dialog",ns="maintenance-task-dialog",nr="maintenance-history-edit-dialog",or="maintenance-complete-dialog",lr="maintenance-qr-dialog",cr="maintenance-task-quick-actions-dialog",dr="maintenance-object-quick-actions-dialog";kt={features:{adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1},defaultWarningDays:7,rowActionStyle:"buttons_compact"};De=null});P();W();ht();P();var li=S`
  .cal-controls {
    display: flex;
    gap: 12px;
    align-items: center;
...
`;N();function ki(r){let s=window;s.customCards=s.customCards||[],s.customCards.some(e=>e.type===r.type)||s.customCards.push(r)}K();var O=class extends A{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._objectFilter="";this._configuredObjects=[];this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){if(this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter),typeof e.object_filter=="string")this._objectFilter=e.object_filter,this._configuredObjects=[];else if(Array.isArray(e.object_filter)){let t=e.object_filter.filter(i=>typeof i=="string"&&i!=="");this._objectFilter=t.length===1?t[0]:"",this._configuredObjects=t.length>1?t:[]}}getCardSize(){return 6}get _lang(){return D(this.hass)}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){if(super.updated(e),mi(this,e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{let e=await this.hass.connection.subscribeMessage(t=>{let i=t;this._objects=i.objects},{type:"maintenance_supporter/subscribe"});if(!this.isConnected){e();return}this._unsub=e}catch{}}_onEventClick(e){if(e.history_timestamp){this._openHistoryEntry(e);return}At(e.entry_id,e.task_id)||this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}async _openHistoryEntry(e){try{let i=(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:e.entry_id})).tasks?.find(l=>l.id===e.task_id)?.history?.find(l=>l.timestamp===e.history_timestamp);if(!i||St({entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp,type:i.type||"completed",timestamp:i.timestamp||e.history_timestamp,notes:i.notes??null,cost:i.cost??null,duration:i.duration??null,completed_by:i.completed_by??null,used_parts:i.used_parts??null}))return}catch{}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}))}render(){if(!this.hass)return _;let e=this._lang,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,n=this._config.title,l=null;this._userFilter&&(l=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let p=y=>{let B=y.toLowerCase();return this._objects.find(C=>C.entry_id===y||C.object.name.toLowerCase()===B)?.entry_id??null},d=new Set(this._configuredObjects.map(p).filter(y=>y!==null)),u=d.size?this._objects.filter(y=>d.has(y.entry_id)):this._objects,m=this._config.show_object_filter!==!1&&u.length>1,f=this._objectFilter?p(this._objectFilter):null,b=f&&u.some(y=>y.entry_id===f)?u.filter(y=>y.entry_id===f):u,v=new Date;v.setHours(0,0,0,0);let k=this._pastDays>0,E=k?oi(b,v,this._pastDays,l):ni(b,v,this._windowDays,l),H=le(v),M=this._windowDays===365||k,q=M?E.filter(y=>y.events.length>0):E,z=y=>{let B=`cal-status-${y.status}`,pe=y.projected?"cal-event-projected":"",C=y.status==="overdue"&&y.days_until_due!=null?` (${bi(y.days_until_due,e)})`:"",R=y.projected&&y.interval_days?o`<span class="cal-event-recur">${y.interval_unit&&y.interval_unit!=="days"?`${y.interval_days} ${a("unit_"+y.interval_unit,e)}`:a("cal_every_n_days",e).replace("{n}",String(y.interval_days))}</span>`:_,se=y.schedule_type==="sensor_based",ye=se?o`<ha-icon class="cal-event-icon cal-source-sensor"
                ...
                icon="${y.adaptive_enabled?"mdi:clock-time-four-outline":"mdi:clock-outline"}"></ha-icon>`,be=se&&y.prediction_confidence&&y.status!=="triggered"&&!y.projected?o`<span class="cal-event-prediction cal-conf-${y.prediction_confidence}">
            ${a("cal_predicted",e)} · ${a(`cal_confidence_${y.prediction_confidence}`,e)}
          </span>`:_,os=this._stats?.budget?.currency_symbol||_i,ls=y.history_type?a(y.history_type,e):a(y.status,e);return o`
        <div class="cal-event ${pe}"
          @click=${()=>this._onEventClick(y)}>
          ${ye}
          <span class="cal-status-pill ${B}">${ls}</span>
          <div class="cal-event-body">
            <div class="cal-event-title">${y.object_name} · ${y.task_name}${C}</div>
            ${be}
            ${R}
          </div>
          ${y.avg_cost!=null&&y.avg_cost>0?o`<span class="cal-event-cost">${y.avg_cost.toFixed(0)} ${os}</span>`:_}
        </div>
      `},te=y=>{let[B,pe,C]=y.date.split("-").map(Number),R=new Date(B,pe-1,C),se=y.date===H,ye=R.toLocaleDateString(e,{weekday:"short"}),be=R.toLocaleDateString(e,{month:"long"});return o`
        <div class="cal-day-row">
          ...
          <div class="cal-day-content">
            <div class="cal-day-header">
              ...
              ${se?o`<span class="cal-day-today-badge">${a("today",e)}</span>`:_}
            </div>
            ${y.events.length===0?o`<div class="cal-empty">${a("cal_no_events",e)}</div>`:y.events.map(z)}
          </div>
        </div>
      `};return o`
      <ha-card .header=${n}>
              ...
              <div class="cal-controls">
                ...
                ${m?o`
                      <select class="cal-user-filter"
                        ...
                        ${[...u].sort((y,B)=>y.object.name.localeCompare(B.object.name)).map(y=>o`<option value=${y.entry_id} ?selected=${y.entry_id===f}>${y.object.name}</option>`)}
                      </select>
                    `:_}
              </div>
            `:_}
        <div class="cal-rolling">
          ${q.length===0&&M?o`<div class="cal-empty">${a("cal_no_events",e)}</div>`:q.map(te)}
        </div>
      </ha-card>
    `}};O.styles=[Ye,li,S`
      :host { display: block; }
      ha-card { padding: 0; overflow: hidden; }
    `],c([x({attribute:!1})],O.prototype,"hass",2),c([h()],O.prototype,"_config",2),c([h()],O.prototype,"_objects",2),c([h()],O.prototype,"_stats",2),c([h()],O.prototype,"_windowDays",2),c([h()],O.prototype,"_pastDays",2),c([h()],O.prototype,"_userFilter",2),c([h()],O.prototype,"_objectFilter",2),c([h()],O.prototype,"_unsub",2);var xr=[{value:7,key:"cal_editor_window_week"},{value:14,key:"cal_editor_window_fortnight"},{value:30,key:"cal_editor_window_month"},{value:365,key:"cal_editor_window_year"}],ve=class extends A{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"}}get _lang(){return D(this.hass)}setConfig(e){this._config={...e}}updated(){let e=this._lang;e&&!Re(e)&&Ne(e).then(()=>this.requestUpdate())}_valueChanged(e,t){let i={...this._config,[e]:t};e==="show_window_chips"&&t===!0&&delete i.show_window_chips,e==="show_user_filter"&&t===!0&&delete i.show_user_filter,e==="show_object_filter"&&t===!0&&delete i.show_object_filter,e==="title"&&(!t||typeof t=="string"&&t.trim()==="")&&delete i.title,e==="user_filter"&&t===""&&delete i.user_filter,this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}render(){let e=this._lang,t=this._config.window_days??30,i=this._config.show_window_chips!==!1,n=this._config.show_user_filter!==!1,l=this._config.user_filter??"",p=this._config.title??"";return o`
      <div class="editor">
        <div class="row">
        ...
        <div class="row">
          ...
          >
            ${xr.map(d=>o`<option value="${d.value}" ?selected=${d.value===t}>${a(d.key,e)}</option>`)}
          </select>
        </div>
  ...
  `,c([x({attribute:!1})],ve.prototype,"hass",2),c([h()],ve.prototype,"_config",2);customElements.get("maintenance-supporter-calendar-card")||customElements.define("maintenance-supporter-calendar-card",O);customElements.get("maintenance-supporter-calendar-card-editor")||customElements.define("maintenance-supporter-calendar-card-editor",ve);ki({type:"maintenance-supporter-calendar-card",name:"Maintenance Supporter \u2014 Calendar",description:"Rolling calendar of maintenance tasks with 7/14/30/365 day windows, source icons, and prediction-confidence pills.",preview:!0});export{O as MaintenanceCalendarCard};
