...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function Or(a,r,e){let t=a.confidence_interval_low,i=a.confidence_interval_high,n=r.suggested_interval??r.interval_days??0,o=r.interval_days??0,p=Math.max(0,t-5),_=i+5-p,f=(t-p)/_*100,m=(i-t)/_*100,y=(n-p)/_*100,b=o>0?(o-p)/_*100:-1;return l`
  ...
  `}function ji(a,r){return Dr(a,r)}function Dr(a,r){let e=a.seasonal_factors??a.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return h;let t=a.interval_analysis?.seasonal_reason,i=new Date().getMonth(),n=300,o=100,p=8,_=o-p-4,f=Math.max(...e,1.5),m=n/12,y=m*.65,b=p+_-1/f*_;return l`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${n} ${o}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_seasonal",r)}">
        ...
        ${e.map((k,E)=>{let D=k/f*_,V=E*m+(m-y)/2,M=p+_-D,le=E===i,de=k<1?"var(--success-color, #4caf50)":k>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return _e`
              ...
              <div class="cal-controls">
                ...
                ${f?l`
                      <select class="cal-user-filter"
                        ...
                        ${[..._].sort((v,F)=>v.object.name.localeCompare(F.object.name)).map(v=>l`<option value=${v.entry_id} ?selected=${v.entry_id===m}>${v.object.name}</option>`)}
                      </select>
                    `:h}
              </div>
            `:h}
