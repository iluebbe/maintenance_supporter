      ...
      `},y=c=>{let[$,T,N]=c.date.split("-").map(Number),O=new Date($,T-1,N),L=c.date===u,re=O.toLocaleDateString(t,{weekday:"short"}),ne=O.toLocaleDateString(t,{month:"long"});return f`
        <div class="cal-day-row">
          ...
          <div class="cal-day-content">
            ...
            ${c.events.length===0?f`<div class="cal-empty">${m("cal_no_events",t)}</div>`:c.events.map(_)}
          </div>
        </div>
      `};return f`
      <ha-card .header=${a}>
