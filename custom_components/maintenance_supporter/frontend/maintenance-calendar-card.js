...
`).map(c=>c.trim()).filter(Boolean).slice(0,100);e.checklist=p.length?p:null}if(this.completionActionsEnabled){let p=this._actionService.trim();if(p&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(p)){let m={service:p},y=this._actionTargetEntity.trim();y&&(m.target={entity_id:y});let b=this._buildActionData();Object.keys(b).length>0&&(m.data=b),e.on_complete_action=m}else e.on_complete_action=null;let c={};this._qcNotes.trim()&&(c.notes=this._qcNotes.trim());let u=parseFloat(this._qcCost);!isNaN(u)&&u>=0&&(c.cost=u);let f=parseInt(this._qcDuration,10);!isNaN(f)&&f>=0&&(c.duration=f),this._qcFeedback&&(c.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(c).length?c:null}let i=await this.hass.connection.sendMessagePromise(e),n=this._taskId||i?.task_id,o=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(n&&this._scheduleType==="sensor_based"&&o)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:n,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}if(n&&this._adaptiveSnapshot()!==this._adaptiveInitial){let p=parseFloat(this._adaptiveAlpha),c=parseInt(this._adaptiveMin,10),u=parseInt(this._adaptiveMax,10);try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_adaptive",entry_id:this._entryId,task_id:n,enabled:this._adaptiveEnabled,...p>=.1&&p<=.9?{ewa_alpha:p}:{},...!isNaN(c)&&c>=1?{min_interval_days:c}:{},...!isNaN(u)&&u>=1?{max_interval_days:u}:{},seasonal_enabled:this._adaptiveSeasonal,sensor_prediction_enabled:this._adaptivePrediction}),this._adaptiveInitial=this._adaptiveSnapshot()}catch{}}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=N(e,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1,this._pickerProbeTimer!==void 0&&(clearTimeout(this._pickerProbeTimer),this._pickerProbeTimer=void 0),this._pickerProbeStrikes=0}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return h;let e=this._lang,t=this._triggerType==="compound";return l`
      ...
      <div class="select-row">
        ...
        >
          ${Tr.map(i=>l`<option value=${i} ?selected=${i===this._triggerType}>${s(i,e)}</option>`)}
        </select>
      </div>
      ...
      ${this._intervalDays?l`
          ...
          `:h}
    `}_patchCondition(e,t){this._compoundConditions=this._compoundConditions.map((i,n)=>n===e?{...i,...t}:i)}_addCondition(){this._compoundConditions=[...this._compoundConditions,Cr()]}_removeCondition(e){this._compoundConditions=this._compoundConditions.filter((t,i)=>i!==e)}_renderCompoundEditor(){let e=this._lang;return l`
      <div class="select-row">
    ...
    `}_renderCondition(e,t){let i=this._lang,n=t+1;return l`
      <div class="compound-condition">
        ...
        <div class="select-row">
          ...
          >
            ${xi.map(o=>l`<option value=${o} ?selected=${o===e.type}>${s(o,i)}</option>`)}
          </select>
        </div>
    ...
    `}_buildSchedule(){let e=i=>{let n=parseInt(this._calOffset,10)||0;return n&&(i.offset=Math.max(-15,Math.min(n,15))),i};if(this._scheduleType==="weekdays")return e({kind:"weekdays",weekdays:[...this._weekdays].sort((i,n)=>i-n)});if(this._scheduleType==="nth_weekday")return e({kind:"nth_weekday",nth:parseInt(this._nth,10),weekday:parseInt(this._nthWeekday,10)});let t={kind:"day_of_month",day:this._domLastDay?-1:parseInt(this._domDay,10)||1};return this._domBusiness&&(t.business=!0),e(t)}_recurrenceExtras(){let e={};if(this._seasonMonths.length&&(e.season_months=[...this._seasonMonths].sort((t,i)=>t-i)),this._endsMode==="count"){let t=parseInt(this._endsCount,10);t>=1&&(e.ends={count:t})}else this._endsMode==="until"&&this._endsUntil&&(e.ends={until:this._endsUntil});return e}_toggleSeasonMonth(e){this._seasonMonths=this._seasonMonths.includes(e)?this._seasonMonths.filter(t=>t!==e):[...this._seasonMonths,e]}_renderRecurrenceExtras(){let e=this._lang;if(!(this._scheduleType==="time_based"||mt.includes(this._scheduleType)))return h;let i=Nr(e);return l`
      ...
      ${this._endsMode==="until"?l`
        ...
        ></ms-textfield>`:h}
    `}_renderCalendarFields(){let e=this._lang,t=Rr(e);if(this._scheduleType==="weekdays")return l`
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${Er.map(i=>l`<option value=${i} ?selected=${i===this._type}>${s(i,e)}</option>`)}
            </select>
          </div>
          ...
          <div class="select-row">
            ...
            >
              ${Sr.map(i=>l`<option value=${i} ?selected=${i===this._priority}>${s("priority_"+i,e)}</option>`)}
            </select>
          </div>
          <div class="field">
          ...
          <div class="select-row">
            ...
            >
              ${Ar.map(i=>l`<option value=${i} ?selected=${i===this._scheduleType}>${s(i,e)}</option>`)}
            </select>
          </div>
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          ${this.parts.length?l`<div class="used-parts">
                ...
                ${this.parts.map(t=>{let i=ee({part_id:t.id,entry_id:t.entry_id}),n=this._usedParts[i],o=n!==void 0,p=t.entry_id?{part_id:t.id,quantity:1,entry_id:t.entry_id}:{part_id:t.id,quantity:1};return l`<div class="used-part-row">
                    ...
                    ${o?l`<input class="used-part-qty" type="number" min="0.01" max="999" step="0.01"
                          .value=${String(n.quantity)}
                          @input=${c=>{let u=parseFloat(c.target.value);this._usedParts={...this._usedParts,[i]:{...p,quantity:Number.isFinite(u)&&u>=.01?u:1}}}} />`:h}
                  </div>`})}
        ...
        <div class="dialog-actions">
          ...
          <ha-button
            ...
            title=${this._missingRequired.length?this._missingRequired.map(t=>s("err_required",e).replace("{field}",s(Ge[t]??t,e))).join(" \xB7 "):""}
          >
...
<script>setTimeout(()=>window.print(),300)<\/script>
</body></html>`),n.document.close()}_downloadSvg(e,t){let i=decodeURIComponent(e.svg_data_uri.replace("data:image/svg+xml,","")),n=new Blob([i],{type:"image/svg+xml"}),o=URL.createObjectURL(n),p=document.createElement("a");p.href=o;let c=this._taskName?`${this._objectName}-${this._taskName}`:this._objectName;p.download=`qr-${jr(c)}-${t}.svg`,p.click(),URL.revokeObjectURL(o)}_close(){this._open=!1,this._viewResult=null,this._completeResult=null,this._error="",this._loading=!1}render(){if(!this._open)return l``;let e=this.lang||this.hass?.language||"en",t=this._taskName?`${s("qr_code",e)}: ${this._objectName} \u2014 ${this._taskName}`:`${s("qr_code",e)}: ${this._objectName}`,i=!!this._viewResult;return l`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
  ...
  `,d([x({attribute:!1})],O.prototype,"hass",2),d([x()],O.prototype,"lang",2),d([_()],O.prototype,"_open",2),d([_()],O.prototype,"_loading",2),d([_()],O.prototype,"_error",2),d([_()],O.prototype,"_viewResult",2),d([_()],O.prototype,"_completeResult",2),d([_()],O.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",O)});function Ai(a,r){let e=a.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return h;let n=a.interval_days??0,o=a.suggested_interval??n;return l`
  ...
  `}function qr(a,r,e,t,i){let b=Math.max(e,t,r,1)*1.3,k=50,E=[];for(let I=0;I<=k;I++){let L=I/k*b,te=1-Math.exp(-Math.pow(L/r,a)),fe=32+L/b*260,ve=136-te*128;E.push([fe,ve])}let D=E.map(([I,L])=>`${I.toFixed(1)},${L.toFixed(1)}`).join(" "),V="M32,136 "+E.map(([I,L])=>`L${I.toFixed(1)},${L.toFixed(1)}`).join(" ")+` L${E[k][0].toFixed(1)},136 Z`,M=32+e/b*260,le=1-Math.exp(-Math.pow(e/r,a)),de=136-le*128,v=((1-le)*100).toFixed(0),F=32+t/b*260,ce=[0,.25,.5,.75,1];return l`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${s("chart_weibull",i)}">
        ...
        <path d="${V}" fill="var(--primary-color, #03a9f4)" opacity="0.08" />
        ...
        ${e>0?_e`
          ...
          <circle cx="${M.toFixed(1)}" cy="${de.toFixed(1)}" r="3"
            fill="var(--primary-color, #03a9f4)" />
          <text x="${(M+4).toFixed(1)}" y="${(de-6).toFixed(1)}" fill="var(--primary-color, #03a9f4)"
  ...
  `}function Or(a,r,e){let t=a.confidence_interval_low,i=a.confidence_interval_high,n=r.suggested_interval??r.interval_days??0,o=r.interval_days??0,p=Math.max(0,t-5),u=i+5-p,f=(t-p)/u*100,m=(i-t)/u*100,y=(n-p)/u*100,b=o>0?(o-p)/u*100:-1;return l`
      ...
      <div class="prediction-grid">
        ...
        ${i?l`
          <div class="prediction-item">
            ...
            ${a.threshold_prediction_date?l`<span class="prediction-date">${G(a.threshold_prediction_date,r)}</span>`:h}
        ...
        ${n&&e.environmental?l`
          <div class="prediction-item">
            ...
            ${a.environmental_entity?l`<span class="prediction-entity entity-link" @click=${p=>ui(p,a.environmental_entity)}>${a.environmental_entity}</span>`:h}
          </div>
        `:h}
      </div>
    </div>
  `}var Ii=w(()=>{"use strict";P();R()});function Li(a,r,e,t){let i=Math.max(a||1,r);return l`
  ...
  `}var Pi=w(()=>{"use strict";P();R()});function Ni(a,r,e){if(!e.seasonal||!a.seasonal_factor||a.seasonal_factor===1)return h;let t=Ri.map(p=>s(p,r)),i=new Date().getMonth(),n=a.seasonal_factors||a.interval_analysis?.seasonal_factors||null,o=n&&n.length===12?n:t.map((p,c)=>{let u=a.seasonal_factor||1,f=Math.sin((c-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,u+f))});return l`
    <div class="seasonal-card-compact">
      <h4>${s("seasonal_awareness",r)}</h4>
      <div class="seasonal-mini-chart">
        ${o.map((p,c)=>{let u=p*40,f=p<.9?"low":p>1.1?"high":"normal";return l`
  ...
  `}function ji(a,r){return Dr(a,r)}function Dr(a,r){let e=a.seasonal_factors??a.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return h;let t=a.interval_analysis?.seasonal_reason,i=new Date().getMonth(),n=300,o=100,p=8,u=o-p-4,f=Math.max(...e,1.5),m=n/12,y=m*.65,b=p+u-1/f*u;return l`
  ...
  `}var Ri,Hi=w(()=>{"use strict";P();R();Ri=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"]});var C,qi=w(()=>{"use strict";P();U();R();oe();Ti();Ii();Pi();Hi();C=class extends A{constructor(){super(...arguments);this._open=!1;this._entryId=null;this._taskId=null;this._task=null;this._objectName="";this._busy=!1;this._error="";this._showSkip=!1;this._showReset=!1;this._showDetails=!1;this._showAdaptive=!1;this._skipReason="";this._resetDate="";this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._toast="";this._featuresLoaded=!1}get _lang(){return H(this.hass)}async openFor(e,t){this._entryId=e,this._taskId=t,this._error="",this._showSkip=!1,this._showReset=!1,this._showAdaptive=!1,this._skipReason="",this._resetDate=new Date().toISOString().slice(0,10),this._open=!0,await Promise.all([this._loadTask(),this._loadFeatures()])}async _loadFeatures(){if(!this._featuresLoaded)try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});e?.features&&(this._features={...this._features,...e.features}),this._featuresLoaded=!0}catch{}}close(){this._open=!1,this._task=null,this._error=""}async _loadTask(){if(!(!this._entryId||!this._taskId))try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._objectName=e.object?.name||"";let t=(e.tasks||[]).find(i=>i.id===this._taskId);this._task=t??null}catch(e){this._error=N(e,this._lang)}}async _runWs(e){this._busy=!0,this._error="";try{return await this.hass.connection.sendMessagePromise(e),this._busy=!1,!0}catch(t){return this._error=N(t,this._lang),this._busy=!1,!1}}_notifyChanged(e){this.dispatchEvent(new CustomEvent("task-action-fired",{detail:{entry_id:this._entryId,task_id:this._taskId,action:e},bubbles:!0,composed:!0}))}_onComplete(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(B(),Q)).then(({openCompleteDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,checklist:this._task.checklist||[],adaptive_enabled:!!this._task.adaptive_config?.enabled})&&(this._notifyChanged("complete"),this.close())})}async _onSkipConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/skip",entry_id:this._entryId,task_id:this._taskId,reason:this._skipReason.trim()||null})&&(this._notifyChanged("skip"),this.close())}async _onResetConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/reset",entry_id:this._entryId,task_id:this._taskId,date:this._resetDate||void 0})&&(this._notifyChanged("reset"),this.close())}_onEdit(){!this._entryId||!this._taskId||Promise.resolve().then(()=>(B(),Q)).then(({openEditTaskDialog:e})=>{e(this._entryId,this._taskId),this.close()})}_onQr(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(B(),Q)).then(({openQrDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,object_name:this._objectName}),this.close()})}async _onDelete(){if(!this._entryId||!this._taskId)return;let e=s("delete_task_confirm",this._lang)||`Delete "${this._task?.name}"?`;if(!window.confirm(e))return;await this._runWs({type:"maintenance_supporter/task/delete",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("delete"),this.close())}async _onArchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/archive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("archive"),this.close())}async _onUnarchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/unarchive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("unarchive"),this.close())}_onOpenInPanel(){if(!this._entryId||!this._taskId)return;let e=`/maintenance-supporter?entry_id=${encodeURIComponent(this._entryId)}&task_id=${encodeURIComponent(this._taskId)}`;history.pushState(null,"",e),window.dispatchEvent(new CustomEvent("location-changed")),this.close()}async _applySuggestion(){if(!this._entryId||!this._taskId||!this._task?.suggested_interval)return;await this._runWs({type:"maintenance_supporter/task/apply_suggestion",entry_id:this._entryId,task_id:this._taskId,interval:this._task.suggested_interval})&&(this._toast=s("suggestion_applied",this._lang)||"Applied",this._notifyChanged("apply_suggestion"),await this._loadTask(),setTimeout(()=>{this._toast=""},2500))}async _reanalyzeInterval(){if(!(!this._entryId||!this._taskId)){this._busy=!0,this._error="";try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:this._entryId,task_id:this._taskId});this._toast=e.recommended_interval?`${s("reanalyze_result",this._lang)||"Recomputed"}: ${Be(e.recommended_interval,"days",this._lang)} (${e.data_points} pts)`:s("reanalyze_insufficient_data",this._lang)||"Not enough data",await this._loadTask(),setTimeout(()=>{this._toast=""},3500)}catch(e){this._error=N(e,this._lang)}finally{this._busy=!1}}}_onEditHistoryEntry(e){!this._entryId||!this._taskId||Promise.resolve().then(()=>(B(),Q)).then(({openHistoryEditDialog:t})=>{t({entry_id:this._entryId,task_id:this._taskId,original_timestamp:e.timestamp,type:e.type,timestamp:e.timestamp,notes:e.notes??null,cost:e.cost??null,duration:e.duration??null,completed_by:e.completed_by??null,used_parts:e.used_parts??null})})}_renderRecommendation(e){if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return h;let t=this._lang;return l`
    ...
    `}_renderAdaptive(e){let t=this._lang,i=this._features.adaptive&&e.suggested_interval&&e.suggested_interval!==e.interval_days,n=e.degradation_trend!=null&&e.degradation_trend!=="insufficient_data"||e.days_until_threshold!=null||e.environmental_factor!=null&&e.environmental_factor!==1,o=this._features.adaptive&&e.interval_analysis?.weibull_beta!=null&&e.interval_analysis?.weibull_eta!=null,p=this._features.seasonal&&e.seasonal_factor&&e.seasonal_factor!==1;return!i&&!n&&!o&&!p?l`<div class="adaptive-empty">
      ...
      <div class="adaptive-stack">
        ...
        ${n?Ci(e,t,this._features):h}
        ${o?Ai(e,t):h}
        ${p?l`
          ${Ni(e,t,this._features)}
          ${e.seasonal_factors?.length===12||e.interval_analysis?.seasonal_factors?.length===12?ji(e,t):h}
        `:h}
      </div>
    `}_renderDetails(e){let t=this._lang,i=e.history||[],n=i.filter(c=>c.type==="completed"),o=n.reduce((c,u)=>c+(typeof u.cost=="number"?u.cost:0),0),p=(()=>{let c=n.map(u=>typeof u.duration=="number"?u.duration:null).filter(u=>u!=null);return c.length?Math.round(c.reduce((u,f)=>u+f,0)/c.length):null})();return l`
      <div class="details">
        ...
        ${t?l`
              <div class="header">
                ...
                <div class="quick-info">
                  ${t.next_due?l`<span><strong>${s("next_due",e)||"Next due"}:</strong> ${G(t.next_due,e)}</span>`:h}
                  ${t.last_performed?l`<span><strong>${s("last_performed",e)||"Last"}:</strong> ${G(t.last_performed,e)}</span>`:h}
                  ${t.schedule?.kind&&!["manual","one_time"].includes(t.schedule.kind)||t.interval_days!=null?l`<span><strong>${s("interval",e)||"Interval"}:</strong> ${pi(t,e)}</span>`:h}
                </div>
              </div>
      ...
      <div class="meta">
        ${i.map(([n,o])=>l`
            <div class="meta-item">
              ...
              <span class="meta-value">${Mi(o)?l`<a href="${o}" target="_blank" rel="noopener noreferrer">${o}</a>`:o}</span>
            </div>
          `)}
      </div>
    `}};W.styles=S`
  ...
  `,d([x({attribute:!1})],W.prototype,"hass",2),d([_()],W.prototype,"_open",2),d([_()],W.prototype,"_entryId",2),d([_()],W.prototype,"_data",2),d([_()],W.prototype,"_busy",2),d([_()],W.prototype,"_error",2);customElements.get("maintenance-object-quick-actions-dialog")||customElements.define("maintenance-object-quick-actions-dialog",W)});var Q={};Gi(Q,{openCompleteDialog:()=>Jr,openCreateObjectDialog:()=>Kr,openCreateTaskDialog:()=>Yr,openEditObjectDialog:()=>Gr,openEditTaskDialog:()=>Qr,openHistoryEditDialog:()=>vt,openObjectQuickActions:()=>Xr,openQrDialog:()=>Zr,openTaskQuickActions:()=>yt});function Qe(){return document.querySelector("home-assistant")?.hass}function Vr(){return document.querySelector("home-assistant")?.shadowRoot??document.body}function J(a){let r=Vr(),e=r.querySelector(a)??document.body.querySelector(a);return e?e.parentNode!==r&&r.appendChild(e):(e=document.createElement(a),r.appendChild(e)),e}function Z(a){let r=Qe();if(!r)return!1;a.hass=r;let e=r.language||"en";return Re(e)||Ne(e).then(()=>{a.requestUpdate?.()}),!0}function Bi(a){return Ye||(Ye=a.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).then(r=>({features:r.features??zi.features,defaultWarningDays:r.general?.default_warning_days??7})).catch(()=>zi),Ye)}function Kr(){let a=J(Fi);return Z(a)?(a.openCreate(),!0):!1}function Gr(a,r){let e=J(Fi);return Z(e)?(e.openEdit(a,r),!0):!1}function Yr(){let a=J(Ui);if(!Z(a))return!1;let r=Qe();return r?((async()=>{let e=await Bi(r),t=a;t.checklistsEnabled=e.features.checklists,t.scheduleTimeEnabled=e.features.schedule_time,t.completionActionsEnabled=e.features.completion_actions,t.defaultWarningDays=e.defaultWarningDays,t.openCreate()})(),!0):!1}function Qr(a,r){let e=J(Ui);if(!Z(e))return!1;let t=Qe();return t?((async()=>{try{let[i,n]=await Promise.all([t.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:a}),Bi(t)]),o=(i.tasks||[]).find(c=>c.id===r);if(!o){console.warn(`openEditTaskDialog: task ${r} not found in entry ${a}`);return}let p=e;p.checklistsEnabled=n.features.checklists,p.scheduleTimeEnabled=n.features.schedule_time,p.completionActionsEnabled=n.features.completion_actions,p.defaultWarningDays=n.defaultWarningDays,await p.openEdit(a,o)}catch(i){console.warn("openEditTaskDialog: failed to load task/features",i)}})(),!0):!1}function vt(a){let r=J(zr);return Z(r)?(r.openEdit(a),!0):!1}function Jr(a){let r=J(Fr);return Z(r)?(r.entryId=a.entry_id,r.taskId=a.task_id,r.taskName=a.task_name,r.checklist=a.checklist??[],r.adaptiveEnabled=!!a.adaptive_enabled,r.requiredFields=a.required_completion_fields??[],r.lang=Qe()?.language||"en",r.open(),!0):!1}function Zr(a){let r=J(Ur);return Z(r)?(r.openForTask(a.entry_id,a.task_id,a.object_name,a.task_name),!0):!1}function yt(a,r){let e=J(Br);return Z(e)?(e.openFor(a,r),!0):!1}function Xr(a){let r=J(Wr);return Z(r)?(r.openFor(a),!0):!1}var Fi,Ui,zr,Fr,Ur,Br,Wr,zi,Ye,B=w(()=>{"use strict";gi();wi();$i();ki();Si();qi();Di();R();Fi="maintenance-object-dialog",Ui="maintenance-task-dialog",zr="maintenance-history-edit-dialog",Fr="maintenance-complete-dialog",Ur="maintenance-qr-dialog",Br="maintenance-task-quick-actions-dialog",Wr="maintenance-object-quick-actions-dialog";zi={features:{adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1},defaultWarningDays:7},Ye=null});P();U();var cr={days:1,weeks:7,months:30.4368,years:365.25};function Gt(a,r){return!a||a<=0?0:a*(cr[r||"days"]??1)}var Yt=5;function Te(a){let r=a.getFullYear(),e=String(a.getMonth()+1).padStart(2,"0"),t=String(a.getDate()).padStart(2,"0");return`${r}-${e}-${t}`}function pr(a,r){let e=[];for(let t=0;t<r;t++){let i=new Date(a);i.setDate(i.getDate()+t),i.setHours(0,0,0,0),e.push(Te(i))}return e}function Fe(a,r){let[e,t,i]=a.split("-").map(Number),n=new Date(e,t-1,i);return n.setDate(n.getDate()+r),Te(n)}function ur(a){if(!a||a.length===0)return null;let r=a.map(e=>e.cost).filter(e=>typeof e=="number");return r.length===0?null:r.reduce((e,t)=>e+t,0)/r.length}function _r(a){let{windowStart:r,windowEnd:e,task:t,entryId:i,objectName:n}=a,o=[],p=(m,y)=>({date:m,entry_id:i,task_id:t.id,task_name:t.name,object_name:n,status:y&&(t.status==="overdue"||t.status==="triggered")?"ok":t.status,days_until_due:y?null:t.days_until_due??null,projected:y,schedule_type:t.schedule_type,interval_days:t.interval_days??null,interval_unit:t.interval_unit??null,responsible_user_id:t.responsible_user_id??null,avg_cost:ur(t.history),adaptive_enabled:!!t.adaptive_config?.enabled,prediction_confidence:t.threshold_prediction_confidence??null}),c=Math.max(1,Math.round(Gt(t.interval_days,t.interval_unit)));if(t.status==="overdue"||t.status==="triggered"){if(o.push(p(r,!1)),t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let m=Fe(r,c),y=1;for(;m<=e&&y<Yt;)o.push(p(m,!0)),y++,m=Fe(m,c)}return o}let u=t.next_due;if(typeof u!="string"||!u)return o;let f=u.slice(0,10);if(f>=r&&f<=e)o.push(p(f,!1));else if(f>e)return o;if(t.schedule_type==="time_based"&&t.interval_days&&t.interval_days>0){let m=Fe(f,c),y=o.length;for(;m<=e&&y<Yt;)m>=r&&(o.push(p(m,!0)),y++),m=Fe(m,c)}return o}var Qt={overdue:0,triggered:1,due_soon:2,ok:3};function Jt(a,r,e,t=null){let i=pr(r,e),n=i[0],o=i[i.length-1],p=[];for(let u of a){let f=u.object?.name||"",m=u.entry_id,y=u.tasks||[];for(let b of y){if(t&&b.responsible_user_id!==t||b.enabled===!1)continue;let k=_r({windowStart:n,windowEnd:o,task:b,entryId:m,objectName:f});p.push(...k)}}let c=new Map;for(let u of i)c.set(u,[]);for(let u of p){let f=c.get(u.date);f&&f.push(u)}for(let[,u]of c)u.sort((f,m)=>{let y=Qt[f.status]??99,b=Qt[m.status]??99;if(y!==b)return y-b;if(f.projected!==m.projected)return f.projected?1:-1;let k=f.object_name.localeCompare(m.object_name);return k!==0?k:f.task_name.localeCompare(m.task_name)});return i.map(u=>({date:u,events:c.get(u)??[]}))}var hr={completed:"ok",reset:"ok",skipped:"due_soon",triggered:"triggered",trigger_replaced:"triggered",trigger_removed:"ok"};function gr(a,r){let e=[];for(let t=r-1;t>=0;t--){let i=new Date(a);i.setDate(i.getDate()-t),i.setHours(0,0,0,0),e.push(Te(i))}return e}function Zt(a,r,e,t=null){let i=gr(r,e),n=i[0],o=i[i.length-1],p=new Map;for(let u of i)p.set(u,[]);for(let u of a){let f=u.object?.name||"",m=u.entry_id,y=u.tasks||[];for(let b of y){if(t&&b.responsible_user_id!==t)continue;let k=b.history||[];for(let E of k){if(typeof E?.timestamp!="string")continue;let D=E.timestamp.slice(0,10);if(D<n||D>o)continue;let V=p.get(D);if(!V)continue;let M=E.type??"completed";V.push({date:D,entry_id:m,task_id:b.id,task_name:b.name,object_name:f,status:hr[M]??"ok",days_until_due:null,projected:!1,schedule_type:b.schedule_type,interval_days:b.interval_days??null,responsible_user_id:b.responsible_user_id??null,avg_cost:typeof E.cost=="number"?E.cost:null,adaptive_enabled:!!b.adaptive_config?.enabled,prediction_confidence:null,history_timestamp:E.timestamp,history_type:M,history_cost:typeof E.cost=="number"?E.cost:null,history_notes:typeof E.notes=="string"?E.notes:null,history_duration:typeof E.duration=="number"?E.duration:null})}}}let c={completed:0,reset:1,skipped:2,triggered:3,trigger_replaced:4};for(let[,u]of p)u.sort((f,m)=>{let y=c[f.history_type??""]??99,b=c[m.history_type??""]??99;if(y!==b)return y-b;let k=f.object_name.localeCompare(m.object_name);return k!==0?k:f.task_name.localeCompare(m.task_name)});return i.map(u=>({date:u,events:p.get(u)??[]}))}P();var Xt=S`
  .cal-controls {
    display: flex;
    gap: 12px;
    align-items: center;
...
`;R();function hi(a){let r=window;r.customCards=r.customCards||[],r.customCards.some(e=>e.type===a.type)||r.customCards.push(a)}B();var q=class extends A{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._objectFilter="";this._configuredObjects=[];this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){if(this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter),typeof e.object_filter=="string")this._objectFilter=e.object_filter,this._configuredObjects=[];else if(Array.isArray(e.object_filter)){let t=e.object_filter.filter(i=>typeof i=="string"&&i!=="");this._objectFilter=t.length===1?t[0]:"",this._configuredObjects=t.length>1?t:[]}}getCardSize(){return 6}get _lang(){return H(this.hass)}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){super.updated(e),e.has("hass")&&oi(this.hass?.locale);let t=this.hass?.language;if(t&&!Re(t)&&Ne(t).then(()=>this.requestUpdate()),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{let e=await this.hass.connection.subscribeMessage(t=>{let i=t;this._objects=i.objects},{type:"maintenance_supporter/subscribe"});if(!this.isConnected){e();return}this._unsub=e}catch{}}_onEventClick(e){if(e.history_timestamp){this._openHistoryEntry(e);return}yt(e.entry_id,e.task_id)||this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}async _openHistoryEntry(e){try{let i=(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:e.entry_id})).tasks?.find(o=>o.id===e.task_id)?.history?.find(o=>o.timestamp===e.history_timestamp);if(!i||vt({entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp,type:i.type||"completed",timestamp:i.timestamp||e.history_timestamp,notes:i.notes??null,cost:i.cost??null,duration:i.duration??null,completed_by:i.completed_by??null,used_parts:i.used_parts??null}))return}catch{}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}))}render(){if(!this.hass)return h;let e=this._lang,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,n=this._config.title,o=null;this._userFilter&&(o=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let p=v=>{let F=v.toLowerCase();return this._objects.find(I=>I.entry_id===v||I.object.name.toLowerCase()===F)?.entry_id??null},c=new Set(this._configuredObjects.map(p).filter(v=>v!==null)),u=c.size?this._objects.filter(v=>c.has(v.entry_id)):this._objects,f=this._config.show_object_filter!==!1&&u.length>1,m=this._objectFilter?p(this._objectFilter):null,y=m&&u.some(v=>v.entry_id===m)?u.filter(v=>v.entry_id===m):u,b=new Date;b.setHours(0,0,0,0);let k=this._pastDays>0,E=k?Zt(y,b,this._pastDays,o):Jt(y,b,this._windowDays,o),D=Te(b),V=this._windowDays===365||k,M=V?E.filter(v=>v.events.length>0):E,le=v=>{let F=`cal-status-${v.status}`,ce=v.projected?"cal-event-projected":"",I=v.status==="overdue"&&v.days_until_due!=null?` (${ci(v.days_until_due,e)})`:"",L=v.projected&&v.interval_days?l`<span class="cal-event-recur">${v.interval_unit&&v.interval_unit!=="days"?`${v.interval_days} ${s("unit_"+v.interval_unit,e)}`:s("cal_every_n_days",e).replace("{n}",String(v.interval_days))}</span>`:h,te=v.schedule_type==="sensor_based",fe=te?l`<ha-icon class="cal-event-icon cal-source-sensor"
                ...
                icon="${v.adaptive_enabled?"mdi:clock-time-four-outline":"mdi:clock-outline"}"></ha-icon>`,ve=te&&v.prediction_confidence&&v.status!=="triggered"&&!v.projected?l`<span class="cal-event-prediction cal-conf-${v.prediction_confidence}">
            ${s("cal_predicted",e)} · ${s(`cal_confidence_${v.prediction_confidence}`,e)}
          </span>`:h,Wi=this._stats?.budget?.currency_symbol||ai,Vi=v.history_type?s(v.history_type,e):s(v.status,e);return l`
        <div class="cal-event ${ce}"
          @click=${()=>this._onEventClick(v)}>
          ${fe}
          <span class="cal-status-pill ${F}">${Vi}</span>
          <div class="cal-event-body">
            <div class="cal-event-title">${v.object_name} · ${v.task_name}${I}</div>
            ${ve}
            ${L}
          </div>
          ${v.avg_cost!=null&&v.avg_cost>0?l`<span class="cal-event-cost">${v.avg_cost.toFixed(0)} ${Wi}</span>`:h}
        </div>
      `},de=v=>{let[F,ce,I]=v.date.split("-").map(Number),L=new Date(F,ce-1,I),te=v.date===D,fe=L.toLocaleDateString(e,{weekday:"short"}),ve=L.toLocaleDateString(e,{month:"long"});return l`
        <div class="cal-day-row">
          <div class="cal-day-pill ${te?"cal-today":""}">
            <span class="cal-pill-weekday">${fe}</span>
            <span class="cal-pill-day">${L.getDate()}</span>
          </div>
          <div class="cal-day-content">
            <div class="cal-day-header">
              <span class="cal-day-month">${ve}</span>
              ${te?l`<span class="cal-day-today-badge">${s("today",e)}</span>`:h}
            </div>
            ${v.events.length===0?l`<div class="cal-empty">${s("cal_no_events",e)}</div>`:v.events.map(le)}
          </div>
        </div>
      `};return l`
      <ha-card .header=${n}>
              ...
              <div class="cal-controls">
                ...
                ${f?l`
                      <select class="cal-user-filter"
                        ...
                        ${[...u].sort((v,F)=>v.object.name.localeCompare(F.object.name)).map(v=>l`<option value=${v.entry_id} ?selected=${v.entry_id===m}>${v.object.name}</option>`)}
                      </select>
                    `:h}
              </div>
            `:h}
        <div class="cal-rolling">
          ${M.length===0&&V?l`<div class="cal-empty">${s("cal_no_events",e)}</div>`:M.map(de)}
        </div>
      </ha-card>
    `}};q.styles=[Ve,Xt,S`
      :host { display: block; }
      ha-card { padding: 0; overflow: hidden; }
    `],d([x({attribute:!1})],q.prototype,"hass",2),d([_()],q.prototype,"_config",2),d([_()],q.prototype,"_objects",2),d([_()],q.prototype,"_stats",2),d([_()],q.prototype,"_windowDays",2),d([_()],q.prototype,"_pastDays",2),d([_()],q.prototype,"_userFilter",2),d([_()],q.prototype,"_objectFilter",2),d([_()],q.prototype,"_unsub",2);var es=[{value:7,key:"cal_editor_window_week"},{value:14,key:"cal_editor_window_fortnight"},{value:30,key:"cal_editor_window_month"},{value:365,key:"cal_editor_window_year"}],me=class extends A{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"}}get _lang(){return H(this.hass)}setConfig(e){this._config={...e}}updated(){let e=this._lang;e&&!Re(e)&&Ne(e).then(()=>this.requestUpdate())}_valueChanged(e,t){let i={...this._config,[e]:t};e==="show_window_chips"&&t===!0&&delete i.show_window_chips,e==="show_user_filter"&&t===!0&&delete i.show_user_filter,e==="show_object_filter"&&t===!0&&delete i.show_object_filter,e==="title"&&(!t||typeof t=="string"&&t.trim()==="")&&delete i.title,e==="user_filter"&&t===""&&delete i.user_filter,this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}render(){let e=this._lang,t=this._config.window_days??30,i=this._config.show_window_chips!==!1,n=this._config.show_user_filter!==!1,o=this._config.user_filter??"",p=this._config.title??"";return l`
      <div class="editor">
        <div class="row">
        ...
        <div class="row">
          ...
          >
            ${es.map(c=>l`<option value="${c.value}" ?selected=${c.value===t}>${s(c.key,e)}</option>`)}
          </select>
        </div>
  ...
  `,d([x({attribute:!1})],me.prototype,"hass",2),d([_()],me.prototype,"_config",2);customElements.get("maintenance-supporter-calendar-card")||customElements.define("maintenance-supporter-calendar-card",q);customElements.get("maintenance-supporter-calendar-card-editor")||customElements.define("maintenance-supporter-calendar-card-editor",me);hi({type:"maintenance-supporter-calendar-card",name:"Maintenance Supporter \u2014 Calendar",description:"Rolling calendar of maintenance tasks with 7/14/30/365 day windows, source icons, and prediction-confidence pills.",preview:!0});export{q as MaintenanceCalendarCard};
