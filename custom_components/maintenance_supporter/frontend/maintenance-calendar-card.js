...
`).map(d=>d.trim()).filter(Boolean).slice(0,100);e.checklist=p.length?p:null}if(this.completionActionsEnabled){let p=this._actionService.trim();if(p&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(p)){let f={service:p},x=this._actionTargetEntity.trim();x&&(f.target={entity_id:x});let v=this._buildActionData();Object.keys(v).length>0&&(f.data=v),e.on_complete_action=f}else e.on_complete_action=null;let d={};this._qcNotes.trim()&&(d.notes=this._qcNotes.trim());let u=parseFloat(this._qcCost);!isNaN(u)&&u>=0&&(d.cost=u);let m=parseInt(this._qcDuration,10);!isNaN(m)&&m>=0&&(d.duration=m),this._qcFeedback&&(d.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(d).length?d:null}let i=await this.hass.connection.sendMessagePromise(e),n=this._taskId||i?.task_id,l=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(n&&this._scheduleType==="sensor_based"&&l)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:n,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}if(n&&this._adaptiveSnapshot()!==this._adaptiveInitial){let p=parseFloat(this._adaptiveAlpha),d=parseInt(this._adaptiveMin,10),u=parseInt(this._adaptiveMax,10);try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_adaptive",entry_id:this._entryId,task_id:n,enabled:this._adaptiveEnabled,...p>=.1&&p<=.9?{ewa_alpha:p}:{},...!isNaN(d)&&d>=1?{min_interval_days:d}:{},...!isNaN(u)&&u>=1?{max_interval_days:u}:{},seasonal_enabled:this._adaptiveSeasonal,sensor_prediction_enabled:this._adaptivePrediction}),this._adaptiveInitial=this._adaptiveSnapshot()}catch{}}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=H(e,this._lang,a("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1,this._pickerProbeTimer!==void 0&&(clearTimeout(this._pickerProbeTimer),this._pickerProbeTimer=void 0),this._pickerProbeStrikes=0}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return _;let e=this._lang,t=this._triggerType==="compound";return o`
      ...
      <div class="select-row">
        ...
        >
          ${rs.map(i=>o`<option value=${i} ?selected=${i===this._triggerType}>${a(i,e)}</option>`)}
        </select>
      </div>
      ...
      ${this._intervalDays?o`
          ...
          `:_}
    `}_patchCondition(e,t){this._compoundConditions=this._compoundConditions.map((i,n)=>n===e?{...i,...t}:i)}_addCondition(){this._compoundConditions=[...this._compoundConditions,ss()]}_removeCondition(e){this._compoundConditions=this._compoundConditions.filter((t,i)=>i!==e)}_renderCompoundEditor(){let e=this._lang;return o`
      <div class="select-row">
    ...
    `}_renderCondition(e,t){let i=this._lang,n=t+1;return o`
      <div class="compound-condition">
        ...
        <div class="select-row">
          ...
          >
            ${Bi.map(l=>o`<option value=${l} ?selected=${l===e.type}>${a(l,i)}</option>`)}
          </select>
        </div>
    ...
    `}_buildSchedule(){let e=i=>{let n=parseInt(this._calOffset,10)||0;return n&&(i.offset=Math.max(-15,Math.min(n,15))),i};if(this._scheduleType==="weekdays")return e({kind:"weekdays",weekdays:[...this._weekdays].sort((i,n)=>i-n)});if(this._scheduleType==="nth_weekday")return e({kind:"nth_weekday",nth:parseInt(this._nth,10),weekday:parseInt(this._nthWeekday,10)});let t={kind:"day_of_month",day:this._domLastDay?-1:parseInt(this._domDay,10)||1};return this._domBusiness&&(t.business=!0),e(t)}_recurrenceExtras(){let e={};if(this._seasonMonths.length&&(e.season_months=[...this._seasonMonths].sort((t,i)=>t-i)),this._endsMode==="count"){let t=parseInt(this._endsCount,10);t>=1&&(e.ends={count:t})}else this._endsMode==="until"&&this._endsUntil&&(e.ends={until:this._endsUntil});return e}_toggleSeasonMonth(e){this._seasonMonths=this._seasonMonths.includes(e)?this._seasonMonths.filter(t=>t!==e):[...this._seasonMonths,e]}_renderRecurrenceExtras(){let e=this._lang;if(!(this._scheduleType==="time_based"||St.includes(this._scheduleType)))return _;let i=cs(e);return o`
      ...
      ${this._endsMode==="until"?o`
        ...
        ></ms-date-field>`:_}
    `}_renderCalendarFields(){let e=this._lang,t=ls(e);if(this._scheduleType==="weekdays")return o`
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${es.map(i=>o`<option value=${i} ?selected=${i===this._type}>${a(i,e)}</option>`)}
            </select>
          </div>
          ...
          <div class="select-row">
            ...
            >
              ${ts.map(i=>o`<option value=${i} ?selected=${i===this._priority}>${a("priority_"+i,e)}</option>`)}
            </select>
          </div>
          <div class="field">
          ...
          <div class="select-row">
            ...
            >
              ${is.map(i=>o`<option value=${i} ?selected=${i===this._scheduleType}>${a(i,e)}</option>`)}
            </select>
          </div>
          ...
          ${this._scheduleType==="sensor_based"?o`
            ...
            <ha-form
              class="entity-picker-form"
              .hass=${this.hass}
              .schema=${[{name:"environmental_entity",selector:{entity:{domain:zi,device_class:Fi}}}]}
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `,c([y({attribute:!1})],B.prototype,"hass",2),c([y()],B.prototype,"lang",2),c([h()],B.prototype,"_open",2),c([h()],B.prototype,"_loading",2),c([h()],B.prototype,"_error",2),c([h()],B.prototype,"_viewResult",2),c([h()],B.prototype,"_completeResult",2),c([h()],B.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",B)});function ps(s,r){if(r<=0)return 0;let e=typeof s=="number"&&Number.isFinite(s)?Math.trunc(s):0;return e<0?0:e%r}function us(s){return!!(s?.phases&&s.phase_sequence&&s.phase_sequence.length>0)}function Tt(s){if(!s||!us(s))return null;let r=s.phase_sequence,e=ps(s.phase_cursor,r.length),t=r[e],i=s.phases?.[t];return i?{id:t,name:i.name,index:e,count:r.length,notes:i.notes,checklist:i.checklist!==void 0?i.checklist:s.checklist??[],consumesParts:i.consumes_parts!==void 0?i.consumes_parts:s.consumes_parts??[],requiredFields:i.required_completion_fields!==void 0?i.required_completion_fields:s.required_completion_fields??[]}:null}function Fe(s){let r=Tt(s);return r?`${r.index+1}/${r.count} \xB7 ${r.name}`:""}var Ct=w(()=>{"use strict"});function Zi(s){let r=s.task??null,e=r?Tt(r):null,t=e?e.consumesParts:r?.consumes_parts||[],i=!!r?.part_ref,n=s.objects.find(d=>d.entry_id===s.entryId)?.parts||[],l=i?n.find(d=>d.id===r.part_ref.part_id):void 0,p=s.checklistsEnabled??!0;return{entry_id:s.entryId,task_id:s.taskId,task_name:s.taskName,checklist:e?p?e.checklist:[]:s.checklist??[],adaptive_enabled:!!s.adaptiveEnabled,required_completion_fields:e?e.requiredFields:r?.required_completion_fields||[],task_type:r?.type||"",reading_unit:r?.reading_unit||"",parts:i?[]:Oi({consumes_parts:t},s.entryId,s.objects,s.lang),consumes_parts:i?[]:t,phase_label:e?Fe(r):"",require_tag_scan:!!r?.require_tag_scan,restock_default:i?l?.restock_quantity??1:null,restock_unit_cost:i?l?.cost??null:null,currency_symbol:s.currencySymbol??"",consumes_info:t.map(d=>Mi(d,s.entryId,s.objects,s.lang)),checklist_prefill:r?.checklist_progress||{},via_tag_scan:!!s.viaTagScan}}function Xi(s,r,e){s.entryId=r.entry_id,s.taskId=r.task_id,s.taskName=r.task_name,s.lang=e,s.checklist=r.checklist??[],s.adaptiveEnabled=!!r.adaptive_enabled,s.requiredFields=r.required_completion_fields??[],s.taskType=r.task_type??"",s.readingUnit=r.reading_unit??"",s.parts=r.parts??[],s.consumesParts=r.consumes_parts??[],s.phaseLabel=r.phase_label??"",s.requireTagScan=!!r.require_tag_scan,s.restockDefault=r.restock_default??null,s.restockUnitCost=r.restock_unit_cost??null,s.currencySymbol=r.currency_symbol??"",s.consumesInfo=r.consumes_info??[],s.checklistPrefill=r.checklist_prefill??{},s.viaTagScan=!!r.via_tag_scan,s.open({viaTagScan:!!r.via_tag_scan})}var It=w(()=>{"use strict";it();Ct()});function E(s){return s.toFixed(1)}var Pt=w(()=>{"use strict";q()});function er(s,r){let e=s.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return _;let n=s.interval_days??0,l=s.suggested_interval??n;return o`
  ...
  `}function _s(s,r,e,t,i){let v=Math.max(e,t,r,1)*1.3,k=50,S=[];for(let P=0;P<=k;P++){let j=P/k*v,oe=1-Math.exp(-Math.pow(j/r,s)),Ee=32+j/v*260,Se=136-oe*128;S.push([Ee,Se])}let D=S.map(([P,j])=>`${E(P)},${E(j)}`).join(" "),z="M32,136 "+S.map(([P,j])=>`L${E(P)},${E(j)}`).join(" ")+` L${E(S[k][0])},136 Z`,N=32+e/v*260,V=1-Math.exp(-Math.pow(e/r,s)),ae=136-V*128,b=O((1-V)*100,i,0),Y=32+t/v*260,ge=[0,.25,.5,.75,1];return o`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${a("chart_weibull",i)}">
        ...
        ${e>0?ve`
          <line x1="${E(N)}" y1="${8}" x2="${E(N)}" y2="${E(136)}"
            stroke="var(--primary-color, #03a9f4)" stroke-width="1.5" stroke-dasharray="4,3" />
          <circle cx="${E(N)}" cy="${E(ae)}" r="3"
            fill="var(--primary-color, #03a9f4)" />
          <text x="${E(N+4)}" y="${E(ae-6)}" fill="var(--primary-color, #03a9f4)"
  ...
  `}function ms(s,r,e){let t=s.confidence_interval_low,i=s.confidence_interval_high,n=r.suggested_interval??r.interval_days??0,l=r.interval_days??0,p=Math.max(0,t-5),u=i+5-p,m=(t-p)/u*100,f=(i-t)/u*100,x=(n-p)/u*100,v=l>0?(l-p)/u*100:-1;return o`
      ...
      <div class="prediction-grid">
        ${t?o`
          <div class="prediction-item">
            ...
            ${s.degradation_rate!=null?o`<span class="prediction-rate">${s.degradation_rate>0?"+":""}${O(s.degradation_rate,r,Math.abs(s.degradation_rate)>=10?0:1)} ${s.trigger_entity_info?.unit_of_measurement||""}/${a("day_short",r)}</span>`:_}
          </div>
        `:_}
        ${i?o`
          <div class="prediction-item">
        ...
        ${n&&e.environmental?o`
          <div class="prediction-item">
            ...
            ${s.environmental_entity?o`<span class="prediction-entity entity-link" @click=${p=>qi(p,s.environmental_entity)}>${s.environmental_entity}</span>`:_}
          </div>
        `:_}
      </div>
    </div>
  `}var rr=w(()=>{"use strict";L();q()});function sr(s,r,e,t){let i=Math.max(s||1,r);return o`
  ...
  `}var ar=w(()=>{"use strict";L();q()});function or(s,r,e){if(!e.seasonal||!s.seasonal_factor||s.seasonal_factor===1)return _;let t=nr.map(p=>a(p,r)),i=new Date().getMonth(),n=s.seasonal_factors||s.interval_analysis?.seasonal_factors||null,l=n&&n.length===12?n:t.map((p,d)=>{let u=s.seasonal_factor||1,m=Math.sin((d-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,u+m))});return o`
  ...
  `}function lr(s,r){return fs(s,r)}function fs(s,r){let e=s.seasonal_factors??s.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return _;let t=s.interval_analysis?.seasonal_reason,i=new Date().getMonth(),n=300,l=100,p=8,u=l-p-4,m=Math.max(...e,1.5),f=n/12,x=f*.65,v=p+u-1/m*u;return o`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${n} ${l}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${a("chart_seasonal",r)}">
        ...
        ${e.map((k,S)=>{let D=k/m*u,z=S*f+(f-x)/2,N=p+u-D,V=S===i,ae=k<1?"var(--success-color, #4caf50)":k>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return ve`
            <rect x="${E(z)}" y="${E(N)}"
  ...
  `}var nr,cr=w(()=>{"use strict";L();q();Pt();nr=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"]});var I,dr=w(()=>{"use strict";L();W();q();_e();vt();It();Ct();tr();rr();ar();we();cr();I=class extends T{constructor(){super(...arguments);this._open=!1;this._entryId=null;this._taskId=null;this._task=null;this._objectName="";this._busy=!1;this._error="";this._showSkip=!1;this._showReset=!1;this._showDetails=!1;this._showAdaptive=!1;this._skipReason="";this._resetDate="";this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._toast="";this._featuresLoaded=!1;this._currencySymbol=""}get _lang(){return M(this.hass)}async openFor(e,t){this._entryId=e,this._taskId=t,this._error="",this._showSkip=!1,this._showReset=!1,this._showAdaptive=!1,this._skipReason="",this._resetDate=ue(new Date),this._open=!0,await Promise.all([this._loadTask(),this._loadFeatures()])}async _loadFeatures(){if(!this._featuresLoaded)try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});e?.features&&(this._features={...this._features,...e.features}),this._currencySymbol=e?.budget?.currency_symbol||"",this._featuresLoaded=!0}catch{}}close(){this._open=!1,this._task=null,this._error=""}async _loadTask(){if(!(!this._entryId||!this._taskId))try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._objectName=e.object?.name||"";let t=(e.tasks||[]).find(i=>i.id===this._taskId);this._task=t??null}catch(e){this._error=H(e,this._lang)}}async _runWs(e){this._busy=!0,this._error="";try{return await this.hass.connection.sendMessagePromise(e),this._busy=!1,!0}catch(t){return this._error=H(t,this._lang),this._busy=!1,!1}}_notifyChanged(e){this.dispatchEvent(new CustomEvent("task-action-fired",{detail:{entry_id:this._entryId,task_id:this._taskId,action:e},bubbles:!0,composed:!0}))}_onComplete(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(Q(),ie)).then(async({openCompleteDialog:e})=>{let t=this._task,i=[];try{i=(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects",compact:!0})).objects||[]}catch{}e(Zi({entryId:this._entryId,taskId:this._taskId,taskName:t.name,task:t,objects:i,lang:this._lang,checklist:t.checklist||[],adaptiveEnabled:!!t.adaptive_config?.enabled,currencySymbol:this._currencySymbol}))&&(this._notifyChanged("complete"),this.close())})}async _onSkipConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/skip",entry_id:this._entryId,task_id:this._taskId,reason:this._skipReason.trim()||null})&&(this._notifyChanged("skip"),this.close())}async _onResetConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/reset",entry_id:this._entryId,task_id:this._taskId,date:this._resetDate||void 0})&&(this._notifyChanged("reset"),this.close())}_onEdit(){!this._entryId||!this._taskId||Promise.resolve().then(()=>(Q(),ie)).then(({openEditTaskDialog:e})=>{e(this._entryId,this._taskId),this.close()})}_onQr(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(Q(),ie)).then(({openQrDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,object_name:this._objectName}),this.close()})}async _onDelete(){if(!this._entryId||!this._taskId)return;let e=a("delete_task_confirm",this._lang)||`Delete "${this._task?.name}"?`;if(!window.confirm(e))return;await this._runWs({type:"maintenance_supporter/task/delete",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("delete"),this.close())}async _onArchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/archive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("archive"),this.close())}async _onUnarchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/unarchive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("unarchive"),this.close())}_onOpenInPanel(){if(!this._entryId||!this._taskId)return;let e=`/maintenance-supporter?entry_id=${encodeURIComponent(this._entryId)}&task_id=${encodeURIComponent(this._taskId)}`;history.pushState(null,"",e),window.dispatchEvent(new CustomEvent("location-changed")),this.close()}async _applySuggestion(){if(!this._entryId||!this._taskId||!this._task?.suggested_interval)return;await this._runWs({type:"maintenance_supporter/task/apply_suggestion",entry_id:this._entryId,task_id:this._taskId,interval:this._task.suggested_interval})&&(this._toast=a("suggestion_applied",this._lang)||"Applied",this._notifyChanged("apply_suggestion"),await this._loadTask(),setTimeout(()=>{this._toast=""},2500))}async _reanalyzeInterval(){if(!(!this._entryId||!this._taskId)){this._busy=!0,this._error="";try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:this._entryId,task_id:this._taskId});this._toast=e.recommended_interval?`${a("reanalyze_result",this._lang)||"Recomputed"}: ${Ze(e.recommended_interval,"days",this._lang)} (${e.data_points} pts)`:a("reanalyze_insufficient_data",this._lang)||"Not enough data",await this._loadTask(),setTimeout(()=>{this._toast=""},3500)}catch(e){this._error=H(e,this._lang)}finally{this._busy=!1}}}_onEditHistoryEntry(e){!this._entryId||!this._taskId||Promise.resolve().then(()=>(Q(),ie)).then(({openHistoryEditDialog:t})=>{t({entry_id:this._entryId,task_id:this._taskId,original_timestamp:e.timestamp,type:e.type,timestamp:e.timestamp,notes:e.notes??null,cost:e.cost??null,duration:e.duration??null,completed_by:e.completed_by??null,used_parts:e.used_parts??null})})}_renderRecommendation(e){if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return _;let t=this._lang;return o`
    ...
    `}_renderAdaptive(e){let t=this._lang,i=this._features.adaptive&&e.suggested_interval&&e.suggested_interval!==e.interval_days,n=e.degradation_trend!=null&&e.degradation_trend!=="insufficient_data"||e.days_until_threshold!=null||e.environmental_factor!=null&&e.environmental_factor!==1,l=this._features.adaptive&&e.interval_analysis?.weibull_beta!=null&&e.interval_analysis?.weibull_eta!=null,p=this._features.seasonal&&e.seasonal_factor&&e.seasonal_factor!==1;return!i&&!n&&!l&&!p?o`<div class="adaptive-empty">
      ...
      <div class="adaptive-stack">
        ...
        ${n?ir(e,t,this._features):_}
        ${l?er(e,t):_}
        ${p?o`
          ${or(e,t,this._features)}
      ...
      <div class="details">
        <div class="stats-grid">
          ...
          <div class="stat">
            ...
            <span class="stat-value">${O(l,t,2)}</span>
          </div>
          <div class="stat">
                ...
                ${[...i].reverse().slice(0,20).map(d=>{let u=["completed","reset","skipped"].includes(d.type);return o`
                    <div class="history-entry">
                      <div class="history-line">
                        ...
                        <span class="history-date">${Ti(d.timestamp,t)}</span>
                      ...
                      ${d.cost!=null||d.duration!=null?o`<div class="history-meta">
                            ${d.cost!=null?o`<span>💰 ${O(d.cost,t,2)}</span>`:_}
        ...
        ${t?o`
              <div class="header">
                ...
                <div class="quick-info">
                  ...
                  ${t.schedule?.kind&&!["manual","one_time"].includes(t.schedule.kind)||t.interval_days!=null?o`<span><strong>${a("interval",e)||"Interval"}:</strong> ${Li(t,e)}</span>`:_}
      ...
      <div class="meta">
        ${i.map(([n,l])=>o`
            <div class="meta-item">
              ...
              <span class="meta-value">${pr(l)?o`<a href="${l}" target="_blank" rel="noopener noreferrer">${l}</a>`:l}</span>
            </div>
          `)}
      </div>
    `}};J.styles=A`
  ...
  `,c([y({attribute:!1})],J.prototype,"hass",2),c([h()],J.prototype,"_open",2),c([h()],J.prototype,"_entryId",2),c([h()],J.prototype,"_data",2),c([h()],J.prototype,"_busy",2),c([h()],J.prototype,"_error",2);customElements.get("maintenance-object-quick-actions-dialog")||customElements.define("maintenance-object-quick-actions-dialog",J)});var ie={};xr(ie,{__resetSettingsCacheForTests:()=>Es,getRowActionStyle:()=>ks,openCompleteDialog:()=>Is,openCreateObjectDialog:()=>Ss,openCreateTaskDialog:()=>Ts,openEditObjectDialog:()=>As,openEditTaskDialog:()=>Cs,openHistoryEditDialog:()=>Rt,openObjectQuickActions:()=>Ls,openQrDialog:()=>Ps,openTaskQuickActions:()=>Nt});function rt(){return document.querySelector("home-assistant")?.hass}function $s(){return document.querySelector("home-assistant")?.shadowRoot??document.body}function re(s){let r=$s(),e=r.querySelector(s)??document.body.querySelector(s);return e?e.parentNode!==r&&r.appendChild(e):(e=document.createElement(s),r.appendChild(e)),e}function se(s){let r=rt();if(!r)return!1;s.hass=r;let e=M(r);return Me(e)||Oe(e).then(()=>{s.requestUpdate?.()}),yt(r.locale,r.config?.country),!0}function ks(s){return qt(s).then(r=>r.rowActionStyle)}function Es(){Ue=null}function qt(s){return Ue||(Ue=s.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).then(r=>({features:r.features??Lt.features,defaultWarningDays:r.general?.default_warning_days??7,rowActionStyle:r.general?.row_action_style??Lt.rowActionStyle})).catch(()=>Lt),Ue)}function Ss(){let s=re(mr);return se(s)?(s.openCreate(),!0):!1}function As(s,r){let e=re(mr);return se(e)?(e.openEdit(s,r),!0):!1}function Ts(s="",r){let e=re(fr);if(!se(e))return!1;let t=rt();return t?((async()=>{let i=await qt(t),n=e;n.checklistsEnabled=i.features.checklists,n.scheduleTimeEnabled=i.features.schedule_time,n.completionActionsEnabled=i.features.completion_actions,n.defaultWarningDays=i.defaultWarningDays,n.openCreate(s,r)})(),!0):!1}function Cs(s,r){let e=re(fr);if(!se(e))return!1;let t=rt();return t?((async()=>{try{let[i,n]=await Promise.all([t.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:s}),qt(t)]),l=(i.tasks||[]).find(d=>d.id===r);if(!l){console.warn(`openEditTaskDialog: task ${r} not found in entry ${s}`);return}let p=e;p.checklistsEnabled=n.features.checklists,p.scheduleTimeEnabled=n.features.schedule_time,p.completionActionsEnabled=n.features.completion_actions,p.defaultWarningDays=n.defaultWarningDays,await p.openEdit(s,l)}catch(i){console.warn("openEditTaskDialog: failed to load task/features",i)}})(),!0):!1}function Rt(s){let r=re(vs);return se(r)?(r.openEdit(s),!0):!1}function Is(s){let r=re(bs);return se(r)?(Xi(r,s,rt()?.language||"en"),!0):!1}function Ps(s){let r=re(ys);return se(r)?(r.openForTask(s.entry_id,s.task_id,s.object_name,s.task_name),!0):!1}function Nt(s,r){let e=re(xs);return se(e)?(e.openFor(s,r),!0):!1}function Ls(s){let r=re(ws);return se(r)?(r.openFor(s),!0):!1}var mr,fr,vs,bs,ys,xs,ws,Lt,Ue,Q=w(()=>{"use strict";ji();Vi();Wi();Ki();Ji();dr();gr();q();It();mr="maintenance-object-dialog",fr="maintenance-task-dialog",vs="maintenance-history-edit-dialog",bs="maintenance-complete-dialog",ys="maintenance-qr-dialog",xs="maintenance-task-quick-actions-dialog",ws="maintenance-object-quick-actions-dialog";Lt={features:{adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1},defaultWarningDays:7,rowActionStyle:"buttons_compact"};Ue=null});L();W();vt();L();var mi=A`
  .cal-controls {
    display: flex;
    gap: 12px;
    align-items: center;
...
`;q();function Ni(s){let r=window;r.customCards=r.customCards||[],r.customCards.some(e=>e.type===s.type)||r.customCards.push(s)}Q();var U=class extends T{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"};this._objects=[];this._stats=null;this._windowDays=30;this._pastDays=0;this._userFilter="";this._objectFilter="";this._configuredObjects=[];this._unsub=null;this._dataLoaded=!1;this._lastConnection=null}static getConfigElement(){return document.createElement("maintenance-supporter-calendar-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-calendar-card",window_days:30,show_window_chips:!0,show_user_filter:!0}}setConfig(e){if(this._config={...e},e.past_days&&[30,90].includes(e.past_days)?this._pastDays=e.past_days:e.window_days&&[7,14,30,365].includes(e.window_days)&&(this._windowDays=e.window_days,this._pastDays=0),typeof e.user_filter=="string"&&(this._userFilter=e.user_filter),typeof e.object_filter=="string")this._objectFilter=e.object_filter,this._configuredObjects=[];else if(Array.isArray(e.object_filter)){let t=e.object_filter.filter(i=>typeof i=="string"&&i!=="");this._objectFilter=t.length===1?t[0]:"",this._configuredObjects=t.length>1?t:[]}}getCardSize(){return 6}get _lang(){return M(this.hass)}disconnectedCallback(){if(super.disconnectedCallback(),this._unsub){try{this._unsub()}catch{}this._unsub=null}this._dataLoaded=!1,this._lastConnection=null}updated(e){if(super.updated(e),ki(this,e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{let e=await this.hass.connection.subscribeMessage(t=>{let i=t;this._objects=i.objects},{type:"maintenance_supporter/subscribe"});if(!this.isConnected){e();return}this._unsub=e}catch{}}_onEventClick(e){if(e.history_timestamp){this._openHistoryEntry(e);return}Nt(e.entry_id,e.task_id)||this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:open-task",entry_id:e.entry_id,task_id:e.task_id},bubbles:!0,composed:!0}))}async _openHistoryEntry(e){try{let i=(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:e.entry_id})).tasks?.find(l=>l.id===e.task_id)?.history?.find(l=>l.timestamp===e.history_timestamp);if(!i||Rt({entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp,type:i.type||"completed",timestamp:i.timestamp||e.history_timestamp,notes:i.notes??null,cost:i.cost??null,duration:i.duration??null,completed_by:i.completed_by??null,used_parts:i.used_parts??null}))return}catch{}this.dispatchEvent(new CustomEvent("ll-custom",{detail:{type:"maintenance-supporter:edit-history",entry_id:e.entry_id,task_id:e.task_id,original_timestamp:e.history_timestamp},bubbles:!0,composed:!0}))}render(){if(!this.hass)return _;let e=this._lang,t=this._config.show_window_chips!==!1,i=this._config.show_user_filter!==!1,n=this._config.title,l=null;this._userFilter&&(l=this._userFilter==="current_user"?this.hass?.user?.id??null:this._userFilter);let p=b=>{let Y=b.toLowerCase();return this._objects.find(P=>P.entry_id===b||P.object.name.toLowerCase()===Y)?.entry_id??null},d=new Set(this._configuredObjects.map(p).filter(b=>b!==null)),u=d.size?this._objects.filter(b=>d.has(b.entry_id)):this._objects,m=this._config.show_object_filter!==!1&&u.length>1,f=this._objectFilter?p(this._objectFilter):null,x=f&&u.some(b=>b.entry_id===f)?u.filter(b=>b.entry_id===f):u,v=new Date;v.setHours(0,0,0,0);let k=this._pastDays>0,S=k?gi(x,v,this._pastDays,l):_i(x,v,this._windowDays,l),D=ue(v),z=this._windowDays===365||k,N=z?S.filter(b=>b.events.length>0):S,V=b=>{let Y=`cal-status-${b.status}`,ge=b.projected?"cal-event-projected":"",P=b.status==="overdue"&&b.days_until_due!=null?` (${Ci(b.days_until_due,e)})`:"",j=b.projected&&b.interval_days?o`<span class="cal-event-recur">${b.interval_unit&&b.interval_unit!=="days"?`${b.interval_days} ${a("unit_"+b.interval_unit,e)}`:a("cal_every_n_days",e).replace("{n}",String(b.interval_days))}</span>`:_,oe=b.schedule_type==="sensor_based",Ee=oe?o`<ha-icon class="cal-event-icon cal-source-sensor"
          ...
          </span>`:_,vr=this._stats?.budget?.currency_symbol||wi,br=b.history_type?a(b.history_type,e):a(b.status,e);return o`
        <div class="cal-event ${ge}"
          @click=${()=>this._onEventClick(b)}>
          ${Ee}
          <span class="cal-status-pill ${Y}">${br}</span>
          <div class="cal-event-body">
            <div class="cal-event-title">${b.object_name} · ${b.task_name}${P}</div>
            ${Se}
            ${j}
          </div>
          ${b.avg_cost!=null&&b.avg_cost>0?o`<span class="cal-event-cost">${Xe(b.avg_cost,vr,e,0)}</span>`:_}
        </div>
      `},ae=b=>{let[Y,ge,P]=b.date.split("-").map(Number),j=new Date(Y,ge-1,P),oe=b.date===D,Ee=Ii(j,e,"short"),Se=Pi(j,e,"long");return o`
        <div class="cal-day-row">
          <div class="cal-day-pill ${oe?"cal-today":""}">
            <span class="cal-pill-weekday">${Ee}</span>
            <span class="cal-pill-day">${j.getDate()}</span>
          </div>
    ...
    `],c([y({attribute:!1})],U.prototype,"hass",2),c([h()],U.prototype,"_config",2),c([h()],U.prototype,"_objects",2),c([h()],U.prototype,"_stats",2),c([h()],U.prototype,"_windowDays",2),c([h()],U.prototype,"_pastDays",2),c([h()],U.prototype,"_userFilter",2),c([h()],U.prototype,"_objectFilter",2),c([h()],U.prototype,"_unsub",2);var qs=[{value:7,key:"cal_editor_window_week"},{value:14,key:"cal_editor_window_fortnight"},{value:30,key:"cal_editor_window_month"},{value:365,key:"cal_editor_window_year"}],ke=class extends T{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-calendar-card"}}get _lang(){return M(this.hass)}setConfig(e){this._config={...e}}updated(){let e=this._lang;e&&!Me(e)&&Oe(e).then(()=>this.requestUpdate())}_valueChanged(e,t){let i={...this._config,[e]:t};e==="show_window_chips"&&t===!0&&delete i.show_window_chips,e==="show_user_filter"&&t===!0&&delete i.show_user_filter,e==="show_object_filter"&&t===!0&&delete i.show_object_filter,e==="title"&&(!t||typeof t=="string"&&t.trim()==="")&&delete i.title,e==="user_filter"&&t===""&&delete i.user_filter,this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}render(){let e=this._lang,t=this._config.window_days??30,i=this._config.show_window_chips!==!1,n=this._config.show_user_filter!==!1,l=this._config.user_filter??"",p=this._config.title??"";return o`
      <div class="editor">
        ...
        <div class="row">
          ...
          >
            ${qs.map(d=>o`<option value="${d.value}" ?selected=${d.value===t}>${a(d.key,e)}</option>`)}
          </select>
        </div>
