...
`),this._requiredCompletion=[...i.required_completion_fields||[]],this._scheduleTime=i.schedule_time||"";let d=i.on_complete_action;if(d&&d.service){this._actionService=d.service;let h=d.target?.entity_id;this._actionTargetEntity=Array.isArray(h)?h[0]||"":h||"",this._actionData=d.data&&typeof d.data=="object"?{...d.data}:{},this._actionDataJsonFallback=""}else this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="";let _=i.quick_complete_defaults;this._qcNotes=_?.notes||"",this._qcCost=_?.cost!=null?String(_.cost):"",this._qcDuration=_?.duration!=null?String(_.duration):"",this._qcFeedback=_?.feedback||"";let c=i.adaptive_config||{};if(this._environmentalEntity=c.environmental_entity||"",this._environmentalAttribute=c.environmental_attribute||"",this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute,this._adaptiveEnabled=!!c.enabled,this._adaptiveAlpha=(c.ewa_alpha??.3).toString(),this._adaptiveMin=(c.min_interval_days??7).toString(),this._adaptiveMax=(c.max_interval_days??365).toString(),this._adaptiveSeasonal=c.seasonal_enabled!==!1,this._adaptivePrediction=c.sensor_prediction_enabled!==!1,this._adaptiveInitial=this._adaptiveSnapshot(),i.trigger_config){let h=i.trigger_config;this._triggerEntityId=h.entity_id||h.entity_ids&&h.entity_ids[0]||"",this._triggerEntityIds=h.entity_ids||(h.entity_id?[h.entity_id]:[]),this._triggerEntityLogic=h.entity_logic||"any",this._triggerAttribute=h.attribute||"",this._triggerType=h.type||"threshold",this._triggerAbove=h.trigger_above?.toString()||"",this._triggerBelow=h.trigger_below?.toString()||"",this._triggerForMinutes=h.trigger_for_minutes?.toString()||"0",this._triggerTargetValue=h.trigger_target_value?.toString()||"",this._triggerDeltaMode=h.trigger_delta_mode||!1,this._triggerBaselineValue=h.trigger_baseline_value?.toString()||"",this._liveBaselineValue=i.trigger_baseline_value??null,this._autoCompleteOnRecovery=h.auto_complete_on_recovery||!1,this._triggerFromState=h.trigger_from_state||"",this._triggerToState=h.trigger_to_state||"",this._triggerTargetChanges=h.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=h.trigger_runtime_hours?.toString()||"",this._triggerOnStates=(h.trigger_on_states||[]).join(", "),h.type==="compound"?(this._compoundLogic=h.compound_logic==="OR"?"OR":"AND",this._compoundConditions=(h.conditions||[]).map(B)):(this._compoundLogic="AND",this._compoundConditions=[])}else this._resetTriggerFields();this._triggerEntityId&&this._fetchEntityAttributes(this._triggerEntityId),await Promise.all([this._loadUsers(),this._loadTags(),this._loadParts(),this._loadForeignPools()]),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._intervalUnit="days",this._dueDate="",this._warningDays=String(this.defaultWarningDays),this._earliestCompletionDays="",this._intervalAnchor="completion",this._weekdays=[],this._nth="1",this._nthWeekday="5",this._domDay="1",this._domLastDay=!1,this._domBusiness=!1,this._calOffset="0",this._seasonMonths=[],this._endsMode="never",this._endsCount="",this._endsUntil="",this._notes="",this._documentationUrl="",this._customIcon="",this._priority="normal",this._labels="",this._enabled=!0,this._lastPerformed="",this._nfcTagId="",this._readingUnit="",this._consumesParts={},this._responsibleUserId=null,this._assigneePool=[],this._rotationStrategy="",this._checklistText="",this._requiredCompletion=[],this._scheduleTime="",this._environmentalEntity="",this._environmentalAttribute="",this._environmentalInitial="",this._environmentalAttributeInitial="",this._adaptiveEnabled=!1,this._adaptiveAlpha="0.3",this._adaptiveMin="7",this._adaptiveMax="365",this._adaptiveSeasonal=!0,this._adaptivePrediction=!0,this._adaptiveInitial=this._adaptiveSnapshot(),this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="",this._actionTesting=!1,this._actionTestResult="",this._qcNotes="",this._qcCost="",this._qcDuration="",this._qcFeedback="",this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerForMinutes="0",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerBaselineValue="",this._liveBaselineValue=null,this._autoCompleteOnRecovery=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours="",this._triggerOnStates="",this._compoundLogic="AND",this._compoundConditions=[]}async _loadUsers(){this._userService||(this._userService=new A(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(t){console.error("Failed to load users:",t),this._availableUsers=[]}}_toggleAssignee(t){this._assigneePool=this._assigneePool.includes(t)?this._assigneePool.filter(i=>i!==t):[...this._assigneePool,t]}async _testAction(){let t=this._actionService.trim();if(!t||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(t)){this._actionTestResult="error",this._actionTestError="Invalid service format (expected 'domain.service')",setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3);return}let[i,e]=t.split(".");if(!this.hass?.services?.[i]?.[e]){this._actionTestResult="error",this._actionTestError=`Service "${t}" is not registered in Home Assistant. Check spelling and that the integration providing it is loaded.`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}let r=this._actionTargetEntity.trim();if(r){let d=r.split(".")[0];if(d!==i&&!new Set(["homeassistant","scene","notify","persistent_notification"]).has(i)){this._actionTestResult="error",this._actionTestError=`Service "${t}" only works on ${i}.* entities; entity "${r}" is in ${d}.* \u2014 pick a service that matches the entity domain (e.g. ${d}.${e})`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}if(!this.hass.states?.[r]){this._actionTestResult="error",this._actionTestError=`Target entity "${r}" not found in Home Assistant \u2014 the entity may have been renamed or its integration removed.`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}}this._actionTestResult="ok",setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3)}_buildActionData(){if(this._actionDataJsonFallback.trim())try{let t=JSON.parse(this._actionDataJsonFallback);if(t&&typeof t=="object"&&!Array.isArray(t))return t}catch{}return{...this._actionData}}_serviceSchema(){let t=this._actionService.trim();if(!t||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(t))return null;let[i,e]=t.split("."),r=this.hass?.services?.[i]?.[e]?.fields;return!r||Object.keys(r).length===0?null:Object.entries(r).map(([d,_])=>({name:d,required:!!_.required,selector:_.selector||{text:{}}}))}_renderCompletionActionsSection(t){if(!this.completionActionsEnabled)return u;let i=this._serviceSchema();return o`
      <details class="ca-section">
        ...
        <ha-service-picker
          ...
          @value-changed=${e=>{this._actionService=e.detail.value||"";let r=this._serviceSchema();if(r){let d=new Set(r.map(_=>_.name));this._actionData=Object.fromEntries(Object.entries(this._actionData).filter(([_])=>d.has(_)))}}}
        ></ha-service-picker>
        <ha-form
          .hass=${this.hass}
        ...
        <div class="ca-test-row">
          ...
          ${this._actionTestResult==="ok"?o`<span class="ca-test-ok">${s("on_complete_action_test_success",t)}</span>`:u}
      ...
      <div class="consumes-row">
        <label class="consumes-check">
          <input
            type="checkbox"
            .checked=${r!==void 0}
            @change=${_=>{let c={...this._consumesParts};_.target.checked?c[e]=c[e]||d:delete c[e],this._consumesParts=c}}
          />
...
`).map(c=>c.trim()).filter(Boolean).slice(0,100);t.checklist=_.length?_:null}if(this.completionActionsEnabled){let _=this._actionService.trim();if(_&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(_)){let v={service:_},g=this._actionTargetEntity.trim();g&&(v.target={entity_id:g});let f=this._buildActionData();Object.keys(f).length>0&&(v.data=f),t.on_complete_action=v}else t.on_complete_action=null;let c={};this._qcNotes.trim()&&(c.notes=this._qcNotes.trim());let h=parseFloat(this._qcCost);!isNaN(h)&&h>=0&&(c.cost=h);let m=parseInt(this._qcDuration,10);!isNaN(m)&&m>=0&&(c.duration=m),this._qcFeedback&&(c.feedback=this._qcFeedback),t.quick_complete_defaults=Object.keys(c).length?c:null}let e=await this.hass.connection.sendMessagePromise(t),r=this._taskId||e?.task_id,d=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(r&&this._scheduleType==="sensor_based"&&d)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:r,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}if(r&&this._adaptiveSnapshot()!==this._adaptiveInitial){let _=parseFloat(this._adaptiveAlpha),c=parseInt(this._adaptiveMin,10),h=parseInt(this._adaptiveMax,10);try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_adaptive",entry_id:this._entryId,task_id:r,enabled:this._adaptiveEnabled,..._>=.1&&_<=.9?{ewa_alpha:_}:{},...!isNaN(c)&&c>=1?{min_interval_days:c}:{},...!isNaN(h)&&h>=1?{max_interval_days:h}:{},seasonal_enabled:this._adaptiveSeasonal,sensor_prediction_enabled:this._adaptivePrediction}),this._adaptiveInitial=this._adaptiveSnapshot()}catch{}}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(t){this._error=C(t,this._lang,s("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1,this._pickerProbeTimer!==void 0&&(clearTimeout(this._pickerProbeTimer),this._pickerProbeTimer=void 0),this._pickerProbeStrikes=0}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return u;let t=this._lang,i=this._triggerType==="compound";return o`
      ...
      <div class="select-row">
        ...
        >
          ${q.map(e=>o`<option value=${e} ?selected=${e===this._triggerType}>${s(e,t)}</option>`)}
        </select>
      </div>
        ...
        ${this._availableAttributes.length>0?o`
            <div class="select-row">
              ...
              >
                ...
                ${this._suggestedAttributes.map(e=>o`<option value=${e} ?selected=${e===this._triggerAttribute}>${e} ★</option>`)}
                ${this._availableAttributes.filter(e=>!this._suggestedAttributes.includes(e.name)).map(e=>o`<option value=${e.name} ?selected=${e.name===this._triggerAttribute}>${e.name}${e.numeric?"":" (non-numeric)"}</option>`)}
              </select>
            </div>
          `:o`
            <ms-textfield
    ...
    `}_patchCondition(t,i){this._compoundConditions=this._compoundConditions.map((e,r)=>r===t?{...e,...i}:e)}_addCondition(){this._compoundConditions=[...this._compoundConditions,U()]}_removeCondition(t){this._compoundConditions=this._compoundConditions.filter((i,e)=>e!==t)}_renderCompoundEditor(){let t=this._lang;return o`
      <div class="select-row">
      ...
      ${this._compoundConditions.length===0?o`<div class="field-help">${s("compound_no_conditions",t)}</div>`:this._compoundConditions.map((i,e)=>this._renderCondition(i,e))}
      <button type="button" class="secondary-btn" @click=${()=>this._addCondition()}>
        + ${s("compound_add_condition",t)}
      </button>
    `}_renderCondition(t,i){let e=this._lang,r=i+1;return o`
      <div class="compound-condition">
        ...
        <div class="select-row">
          ...
          >
            ${R.map(d=>o`<option value=${d} ?selected=${d===t.type}>${s(d,e)}</option>`)}
          </select>
        </div>
        ${this._renderConditionTypeFields(t,i)}
      </div>
    `}_renderStateField(t){return this._entityPickerFallback||!t.entityId?o`
        <ms-textfield
          label=${t.label}
          .value=${t.value}
      ...
      ></ha-form>
    `}_renderOnStatesField(t){let i=this._lang;return this._entityPickerFallback||!t.entityId?o`
        <ms-textfield
      ...
      ></ha-form>
    `}_renderAdaptiveSection(t){return this._scheduleType==="one_time"||this._scheduleType==="manual"?u:o`
    ...
    `}_renderEnvironmentalAttribute(t){this._fetchConditionAttributes(this._environmentalEntity);let i=this._conditionAttrOptions[this._environmentalEntity];return i&&i.available.length>0?o`
        <div class="select-row">
          <label>${s("environmental_attribute_optional",t)}</label>
          <select
            .value=${this._environmentalAttribute}
            @change=${e=>this._environmentalAttribute=e.target.value}
          >
            <option value="" ?selected=${!this._environmentalAttribute}>${s("use_entity_state",t)}</option>
            ${i.suggested.map(e=>o`<option value=${e} ?selected=${e===this._environmentalAttribute}>${e} ★</option>`)}
            ${i.available.filter(e=>!i.suggested.includes(e.name)).map(e=>o`<option value=${e.name} ?selected=${e.name===this._environmentalAttribute}>${e.name}${e.numeric?"":" (non-numeric)"}</option>`)}
          </select>
        </div>
      `:o`
      <ms-textfield
        ...
        .value=${this._environmentalAttribute}
        @input=${e=>this._environmentalAttribute=e.target.value.trim()}
      ></ms-textfield>
    `}_renderConditionAttribute(t,i){let e=this._lang,r=t.entityIds.split(",")[0]?.trim()||"";r&&this._fetchConditionAttributes(r);let d=r?this._conditionAttrOptions[r]:void 0;return d&&d.available.length>0?o`
        <div class="select-row">
          <label>${s("attribute_optional",e)}</label>
          <select
            .value=${t.attribute}
            @change=${_=>this._patchCondition(i,{attribute:_.target.value})}
          >
            <option value="" ?selected=${!t.attribute}>${s("use_entity_state",e)}</option>
            ${d.suggested.map(_=>o`<option value=${_} ?selected=${_===t.attribute}>${_} ★</option>`)}
            ${d.available.filter(_=>!d.suggested.includes(_.name)).map(_=>o`<option value=${_.name} ?selected=${_.name===t.attribute}>${_.name}${_.numeric?"":" (non-numeric)"}</option>`)}
          </select>
        </div>
      `:o`
      <ms-textfield
        ...
        .value=${t.attribute}
        @input=${_=>this._patchCondition(i,{attribute:_.target.value.trim()})}
      ></ms-textfield>
    `}_renderConditionTypeFields(t,i){let e=this._lang;if(t.type==="threshold")return o`
      ...
      `;if(t.type==="state_change"){let r=t.entityIds.split(",")[0]?.trim()||"";return o`
      ...
      `}if(t.type==="runtime"){let r=t.entityIds.split(",")[0]?.trim()||"";return o`
      ...
      `}return u}_renderUnitSelect(){let t=this._lang;return o`
      <div class="select-row">
    ...
    `}_buildSchedule(){let t=e=>{let r=parseInt(this._calOffset,10)||0;return r&&(e.offset=Math.max(-15,Math.min(r,15))),e};if(this._scheduleType==="weekdays")return t({kind:"weekdays",weekdays:[...this._weekdays].sort((e,r)=>e-r)});if(this._scheduleType==="nth_weekday")return t({kind:"nth_weekday",nth:parseInt(this._nth,10),weekday:parseInt(this._nthWeekday,10)});let i={kind:"day_of_month",day:this._domLastDay?-1:parseInt(this._domDay,10)||1};return this._domBusiness&&(i.business=!0),t(i)}_recurrenceExtras(){let t={};if(this._seasonMonths.length&&(t.season_months=[...this._seasonMonths].sort((i,e)=>i-e)),this._endsMode==="count"){let i=parseInt(this._endsCount,10);i>=1&&(t.ends={count:i})}else this._endsMode==="until"&&this._endsUntil&&(t.ends={until:this._endsUntil});return t}_toggleSeasonMonth(t){this._seasonMonths=this._seasonMonths.includes(t)?this._seasonMonths.filter(i=>i!==t):[...this._seasonMonths,t]}_renderRecurrenceExtras(){let t=this._lang;if(!(this._scheduleType==="time_based"||w.includes(this._scheduleType)))return u;let e=z(t);return o`
      ...
      ${this._endsMode==="until"?o`
        ...
        ></ms-textfield>`:u}
    `}_renderCalendarFields(){let t=this._lang,i=D(t);if(this._scheduleType==="weekdays")return o`
        ...
        ${this._renderCalOffsetField()}`;if(this._scheduleType==="nth_weekday"){let e=[["1",s("ord_1",t)],["2",s("ord_2",t)],["3",s("ord_3",t)],["4",s("ord_4",t)],["5",s("ord_5",t)],["-1",s("ord_last",t)]];return o`
        <div class="select-row">
          <label>${s("recurrence_occurrence",t)}</label>
          <select .value=${this._nth} @change=${r=>this._nth=r.target.value}>
            ${e.map(([r,d])=>o`<option value=${r} ?selected=${r===this._nth}>${d}</option>`)}
          </select>
        </div>
        <div class="select-row">
          <label>${s("recurrence_weekday",t)}</label>
          <select .value=${this._nthWeekday} @change=${r=>this._nthWeekday=r.target.value}>
            ${i.map((r,d)=>o`<option value=${String(d)} ?selected=${String(d)===this._nthWeekday}>${r}</option>`)}
          </select>
        </div>
        ...
        ${this._renderCalOffsetField()}`:u}_renderCalOffsetField(){let t=this._lang;return o`
      <ms-textfield
      ...
      ></ms-textfield>`}_renderTriggerLiveHint(){if(this._triggerType==="compound")return u;let t=this._triggerEntityId||this._triggerEntityIds[0];if(!t||!this.hass?.states)return u;let i=this.hass.states[t];if(!i)return u;let e=this._lang,r=i.attributes?.unit_of_measurement,d=typeof r=="string"&&r?` ${r}`:"",_=this._triggerAttribute?i.attributes?.[this._triggerAttribute]:i.state,c=typeof _=="number"?_:parseFloat(String(_)),h=_!=="unknown"&&_!=="unavailable"&&_!=null&&!isNaN(c),m=g=>Number.isInteger(g)?String(g):String(Math.round(g*10)/10),v=[];if(this._triggerType==="threshold"){let g=parseFloat(this._triggerAbove),f=parseFloat(this._triggerBelow);if(isNaN(g)&&isNaN(f))return u;h&&v.push(s("trigger_hint_now",e).replace("{value}",m(c)+d)),isNaN(g)||v.push(s("trigger_hint_above",e).replace("{target}",m(g)+d)),isNaN(f)||v.push(s("trigger_hint_below",e).replace("{target}",m(f)+d))}else if(this._triggerType==="counter"){let g=parseFloat(this._triggerTargetValue);if(isNaN(g))return u;this._triggerDeltaMode?this._taskId?v.push(s("trigger_hint_counter_delta_edit",e).replace("{target}",m(g)+d)):h?v.push(s("trigger_hint_counter_delta",e).replace("{value}",m(c)+d).replace("{due}",m(c+g)+d).replace("{target}",m(g)+d)):v.push(s("trigger_hint_counter_delta_edit",e).replace("{target}",m(g)+d)):(h&&v.push(s("trigger_hint_now",e).replace("{value}",m(c)+d)),v.push(s("trigger_hint_counter_abs",e).replace("{target}",m(g)+d)))}else if(this._triggerType==="runtime"){let g=parseFloat(this._triggerRuntimeHours);if(isNaN(g))return u;v.push(s("trigger_hint_runtime",e).replace("{hours}",m(g))),v.push(s("trigger_hint_state_now",e).replace("{value}",String(i.state)))}else if(this._triggerType==="state_change"){let g=parseInt(this._triggerTargetChanges,10)||1,f=this._triggerToState.trim();v.push((f?s("trigger_hint_state_change_to",e).replace("{state}",f):s("trigger_hint_state_change",e)).replace("{count}",String(g))),v.push(s("trigger_hint_state_now",e).replace("{value}",String(i.state)))}return v.length?o`<div class="trigger-live-hint">${v.join(" ")}</div>`:u}_renderTriggerTypeFields(){let t=this._lang;return this._triggerType==="threshold"?o`
        <ms-textfield
      ...
      `:u}render(){if(!this._open)return o``;let t=this._lang,i=this._taskId?s("edit_task",t):s("new_task",t);return o`
        ...
        <div class="content">
          ...
          ${this._objectChoices.length>0?o`
            <div class="select-row">
              ...
              >
                ${this._objectChoices.map(e=>o`<option value=${e.entry_id} ?selected=${e.entry_id===this._entryId}>${e.name}</option>`)}
              </select>
            </div>
          `:u}
          <ms-textfield
          ...
          <div class="select-row">
            ...
            >
              ${F.map(e=>o`<option value=${e} ?selected=${e===this._type}>${s(e,t)}</option>`)}
            </select>
          </div>
          ${this._type==="reading"?o`
              ...
              `:u}
          ${this._partsLoadFailed?o`<div class="field-help parts-load-failed">${s("parts_load_failed",t)}</div>`:u}
          ...
          <div class="select-row">
            ...
            >
              ${M.map(e=>o`<option value=${e} ?selected=${e===this._priority}>${s("priority_"+e,t)}</option>`)}
            </select>
          </div>
          <div class="field">
          ...
          <div class="select-row">
            ...
            >
              ${O.map(e=>o`<option value=${e} ?selected=${e===this._scheduleType}>${s(e,t)}</option>`)}
            </select>
          </div>
          ...
          <div class="select-row">
            ...
            >
              ...
              ${this._availableUsers.map(e=>o`<option value=${e.id} ?selected=${e.id===this._responsibleUserId}>${e.name}</option>`)}
            </select>
          </div>
            ...
            ${this._assigneePool.length>=2?o`
              <div class="select-row">
                ...
                >
                  ...
                  ${["round_robin","least_completed","random"].map(e=>o`<option value=${e} ?selected=${e===this._rotationStrategy}>${s("rotation_"+e,t)}</option>`)}
                </select>
              </div>`:u}
          `:u}
          ...
          ${this._scheduleType==="sensor_based"?o`
            ...
            <ha-form
              class="entity-picker-form"
              .hass=${this.hass}
              .schema=${[{name:"environmental_entity",selector:{entity:{domain:H,device_class:N}}}]}
          ...
          ${this._availableTags.length>0?o`
              <div class="select-row">
                ...
                >
                  ...
                  ${this._availableTags.map(e=>o`<option value=${e.id} ?selected=${e.id===this._nfcTagId}>${e.name}</option>`)}
                </select>
