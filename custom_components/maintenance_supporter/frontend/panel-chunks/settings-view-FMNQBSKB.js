...
import{a as I,f as E,h as T}from"/maintenance_supporter_panelfiles/panel-chunks/chunk-OLTAFAWB.js";import{a as P,b as H}from"/maintenance_supporter_panelfiles/panel-chunks/chunk-Y23XZ6RW.js";import{a as z}from"/maintenance_supporter_panelfiles/panel-chunks/chunk-3YRDL7JR.js";import{a as c,b as q,c as r,e as m,f as p,g as A,h as S,l as $,m as d,q as t,s as j}from"/maintenance_supporter_panelfiles/panel-chunks/chunk-C7MO5BGD.js";var u={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},x=g=>(..._)=>({_$litDirective$:g,values:_}),f=class{constructor(_){}get _$AU(){return this._$AM._$AU}_$AT(_,e,s){this._$Ct=_,this._$AM=e,this._$Ci=s}_$AS(_,e){return this.update(_,e)}update(_,e){return this.render(...e)}};var y=class extends f{constructor(_){if(super(_),this.it=p,_.type!==u.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(_){if(_===p||_==null)return this._t=void 0,this.it=_;if(_===m)return _;if(typeof _!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(_===this.it)return this._t;this.it=_;let e=[_];return e.raw=e,this._t={_$litType$:this.constructor.resultType,strings:e,values:[]}}};y.directiveName="unsafeHTML",y.resultType=1;var C=x(y);var{I:Y}=A;var R=g=>g.strings===void 0;var M={},O=(g,_=M)=>g._$AH=_;var w=x(class extends f{constructor(g){if(super(g),g.type!==u.PROPERTY&&g.type!==u.ATTRIBUTE&&g.type!==u.BOOLEAN_ATTRIBUTE)throw Error("The `live` directive is not allowed on child or event bindings");if(!R(g))throw Error("`live` bindings can only contain a single expression")}render(g){return g}update(g,[_]){if(_===m||_===p)return _;let e=g.element,s=g.name;if(g.type===u.PROPERTY){if(_===e[s])return m}else if(g.type===u.BOOLEAN_ATTRIBUTE){if(!!_===e.hasAttribute(s))return m}else if(g.type===u.ATTRIBUTE&&e.getAttribute(s)===_+"")return m;return O(g),_}});var B=["EUR","USD","GBP","JPY","CHF","CAD","AUD","NZD","CNY","INR","BRL","CZK","PLN","RUB","SEK","NOK","DKK","UAH"],l=class extends S{constructor(){super(...arguments);this.budget=null;this._settings=null;this._loading=!0;this._importCsv="";this._importLoading=!1;this._includeHistory=!0;this._toast="";this._testingNotification=!1;this._personTargets=[];this._testingUser="";this._users=[];this._savedViews=[];this._vacEnabled=!1;this._vacStart="";this._vacEnd="";this._vacBuffer=3;this._vacExempt=new Set;this._vacIsActive=!1;this._vacWindowEnd=null;this._vacAllTasks=[];this._vacPreview=[];this._vacPreviewLoading=!1;this._vacSaving=!1;this._qrObjects=[];this._qrSelectedEntries=new Set;this._qrActions=new Set(["view"]);this._qrUrlMode="companion";this._qrBatchLoading=!1;this._qrBatchResults=[];this._qrObjectsLoaded=!1;this._exportObjects=[];this._exportSelectedEntries=new Set;this._exportObjectsLoaded=!1;this._docArchiveLoading=!1;this._loaded=!1;this._userService=null;this._sendTestNotification=async e=>{e?this._testingUser=e:this._testingNotification=!0;try{let s=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/test_notification",...e?{user_id:e}:{}}),i=s.message||(s.success?t("test_notification_success",this._lang):t("test_notification_failed",this._lang));this._showToast(i)}catch{this._showToast(t("test_notification_failed",this._lang))}finally{e?this._testingUser="":this._testingNotification=!1}};this._allTemplates=[];this._templateCategories={};this._tplOpenGroups=new Set;this._templatesRequested=!1}get _lang(){return j(this.hass)}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._loaded?(this._loaded=!0,this._userService=new z(this.hass),this._loadSettings(),this._loadUsers()):e.has("hass")&&this.hass&&this._userService&&this._userService.updateHass(this.hass)}async _loadUsers(){if(this._userService){try{this._users=await this._userService.getUsers()}catch{this._users=[]}this._loadNotifyTargets()}}async _loadNotifyTargets(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/notify/user_targets"});this._personTargets=e.targets||[]}catch{this._personTargets=[]}}async _loadSettings(){this._loading=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});this._settings=e,this._hydrateVacationFromSettings()}catch{}try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"});this._savedViews=e.views||[]}catch{}this._loading=!1}_hydrateVacationFromSettings(){let e=this._settings?.vacation;e&&(this._vacEnabled=e.enabled,this._vacStart=e.start||"",this._vacEnd=e.end||"",this._vacBuffer=e.buffer_days,this._vacExempt=new Set(e.exempt_task_ids||[]),this._vacIsActive=e.is_active,this._vacWindowEnd=e.window_end)}async _updateSetting(e,s){try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/update",settings:{[e]:s}});this._settings=i,this._showToast(t("settings_saved",this._lang)),this.dispatchEvent(new CustomEvent("settings-changed"))}catch{this._showToast(t("action_error",this._lang)),this.requestUpdate()}}_showToast(e){this._toast=e,setTimeout(()=>{this._toast=""},3e3)}_downloadFile(e,s,i){P(e,s,i)}render(){let e=this._lang;return this._loading||!this._settings?r`<div class="settings-loading">Loading…</div>`:r`
    ...
    `}scrollToSection(e){requestAnimationFrame(()=>{let s=this.shadowRoot;if(!s)return;let i=s.querySelector(`[data-section="${e}"]`)??s.querySelector(`[data-section-alt="${e}"]`);i&&i.scrollIntoView({behavior:"smooth",block:"start"})})}_renderPanelAccess(e){let s=new Set(this._settings.admin_panel_user_ids||[]),i=this._users.filter(o=>!o.is_admin),a=this._settings.operator_write_enabled??!1,n=(o,h)=>{let b=new Set(s);h?b.add(o):b.delete(o),this._updateSetting("admin_panel_user_ids",[...b])};return r`
      <div class="settings-section">
        <h3>${t("settings_panel_access",e)} ${a&&s.size>0?r`<span class="section-badge">${s.size}</span>`:p}</h3>
    ...
    `}async _loadTemplates(){if(!this._templatesRequested){this._templatesRequested=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/templates",language:this._lang});this._allTemplates=e.templates||[],this._templateCategories=e.categories||{}}catch{}}}_renderTemplateToggles(e){this._loadTemplates();let s=new Set(this._settings.disabled_template_ids||[]),i=new Map;for(let n of Object.keys(this._templateCategories))i.set(n,[]);for(let n of this._allTemplates)i.has(n.category)||i.set(n.category,[]),i.get(n.category).push(n);let a=n=>this._templateCategories[n]?.["name_"+e]||this._templateCategories[n]?.name_en||n;return r`
      <div class="settings-section" data-section="templates">
        ...
        ${[...i.entries()].filter(([,n])=>n.length>0).map(([n,o])=>{let h=o.filter(v=>!s.has(v.id)).length,b=this._tplOpenGroups.has(n);return r`
            <div class="tpl-group">
              <div
              ...
              >
                ...
                <input
                  ...
                  @change=${v=>this._toggleTemplateGroup(o.map(k=>k.id),v.target.checked)}
                />
              </div>
              ${b?o.map(v=>r`
    ...
    `}_toggleColumn(e,s){let i=new Set(T(this._settings.objects_table_columns));s?i.add(e):i.delete(e);let a=E.filter(n=>n.required||i.has(n.key)).map(n=>n.key);this._updateSetting("objects_table_columns",a)}_renderGeneral(e){let s=this._settings.general,i=s.notify_targets??[],a=this._settings.budget;return r`
      <div class="settings-section">
        ...
        <label class="setting-row">
          ...
          <select .value=${w(a.currency)} @change=${n=>this._updateSetting("budget_currency",n.target.value)}>
            ${B.map(n=>r`<option value=${n} ?selected=${a.currency===n}>${n}</option>`)}
          </select>
        </label>
        <label class="setting-row">
        ...
        <label class="setting-row">
          ...
          <select .value=${w(s.shopping_list_entity||"")}
      ...
      <div class="settings-section vacation-section" data-section="vacation">
        ...
        <details class="vac-exempt-panel">
          <summary>
            ...
            ${i>0?r`<span class="section-badge">${i}</span>`:p}
          </summary>
