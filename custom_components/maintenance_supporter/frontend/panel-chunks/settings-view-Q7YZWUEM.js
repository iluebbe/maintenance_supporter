...
import{a as A,c as j}from"/maintenance_supporter_panelfiles/panel-chunks/chunk-VNYLIDZN.js";import{a as O,g as F}from"/maintenance_supporter_panelfiles/panel-chunks/chunk-YUCXSTBQ.js";import"/maintenance_supporter_panelfiles/panel-chunks/chunk-UJ6W3NVH.js";import{a as M,b as R}from"/maintenance_supporter_panelfiles/panel-chunks/chunk-TVK2JN4G.js";import{a as B,b as N,c as U,e as D}from"/maintenance_supporter_panelfiles/panel-chunks/chunk-NVMWVJOE.js";import{L as C,a as d,b as P,c as r,e as x,f as l,g as H,h as I,l as q,m as p,q as t,s as z}from"/maintenance_supporter_panelfiles/panel-chunks/chunk-AWEHSJHD.js";var $={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},S=h=>(..._)=>({_$litDirective$:h,values:_}),w=class{constructor(_){}get _$AU(){return this._$AM._$AU}_$AT(_,e,s){this._$Ct=_,this._$AM=e,this._$Ci=s}_$AS(_,e){return this.update(_,e)}update(_,e){return this.render(...e)}};var k=class extends w{constructor(_){if(super(_),this.it=l,_.type!==$.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(_){if(_===l||_==null)return this._t=void 0,this.it=_;if(_===x)return _;if(typeof _!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(_===this.it)return this._t;this.it=_;let e=[_];return e.raw=e,this._t={_$litType$:this.constructor.resultType,strings:e,values:[]}}};k.directiveName="unsafeHTML",k.resultType=1;var K=S(k);var{I:se}=H;var Q=h=>h.strings===void 0;var Y={},G=(h,_=Y)=>h._$AH=_;var y=S(class extends w{constructor(h){if(super(h),h.type!==$.PROPERTY&&h.type!==$.ATTRIBUTE&&h.type!==$.BOOLEAN_ATTRIBUTE)throw Error("The `live` directive is not allowed on child or event bindings");if(!Q(h))throw Error("`live` bindings can only contain a single expression")}render(h){return h}update(h,[_]){if(_===x||_===l)return _;let e=h.element,s=h.name;if(h.type===$.PROPERTY){if(_===e[s])return x}else if(h.type===$.BOOLEAN_ATTRIBUTE){if(!!_===e.hasAttribute(s))return x}else if(h.type===$.ATTRIBUTE&&e.getAttribute(s)===_+"")return x;return G(h),_}});var W=["EUR","USD","GBP","JPY","CHF","CAD","AUD","NZD","CNY","INR","BRL","CZK","PLN","RUB","SEK","NOK","DKK","UAH"],c=class extends I{constructor(){super(...arguments);this.budget=null;this._settings=null;this._loading=!0;this._importCsv="";this._importLoading=!1;this._includeHistory=!0;this._toast="";this._testingNotification=!1;this._personTargets=[];this._testingUser="";this._users=[];this._savedViews=[];this._vacEnabled=!1;this._vacStart="";this._vacEnd="";this._vacBuffer=3;this._vacExempt=new Set;this._vacIsActive=!1;this._vacWindowEnd=null;this._vacAllTasks=[];this._vacPreview=[];this._vacPreviewLoading=!1;this._vacSaving=!1;this._qrObjects=[];this._qrSelectedEntries=new Set;this._qrActions=new Set(["view"]);this._qrUrlMode="companion";this._qrBatchLoading=!1;this._qrBatchResults=[];this._qrObjectsLoaded=!1;this._exportObjects=[];this._exportSelectedEntries=new Set;this._exportObjectsLoaded=!1;this._docArchiveLoading=!1;this._loaded=!1;this._userService=null;this._sendTestNotification=async e=>{e?this._testingUser=e:this._testingNotification=!0;try{let s=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/global/test_notification",...e?{user_id:e}:{}}),i=s.message||(s.success?t("test_notification_success",this._lang):t("test_notification_failed",this._lang));this._showToast(i)}catch{this._showToast(t("test_notification_failed",this._lang))}finally{e?this._testingUser="":this._testingNotification=!1}};this._allTemplates=[];this._templateCategories={};this._tplOpenGroups=new Set;this._templatesRequested=!1}get _lang(){return z(this.hass)}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._loaded?(this._loaded=!0,this._userService=new D(this.hass),this._loadSettings(),this._loadUsers()):e.has("hass")&&this.hass&&this._userService&&this._userService.updateHass(this.hass)}async _loadUsers(){if(this._userService){try{this._users=await this._userService.getUsers()}catch{this._users=[]}this._loadNotifyTargets()}}async _loadNotifyTargets(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/notify/user_targets"});this._personTargets=e.targets||[]}catch{this._personTargets=[]}}async _loadSettings(){this._loading=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});this._settings=e,this._hydrateVacationFromSettings()}catch{}try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"});this._savedViews=e.views||[]}catch{}this._loading=!1}_hydrateVacationFromSettings(){let e=this._settings?.vacation;e&&(this._vacEnabled=e.enabled,this._vacStart=e.start||"",this._vacEnd=e.end||"",this._vacBuffer=e.buffer_days,this._vacExempt=new Set(e.exempt_task_ids||[]),this._vacIsActive=e.is_active,this._vacWindowEnd=e.window_end)}_renderBatteryLifetimes(e){let s=this._settings?.general?.battery_lifetimes;if(!s||!s.length)return l;let i=this._settings?.general?.battery_lifetime_months??{},a=v=>t("lifetime_source_"+v.source,e),n=t("settings_battery_lifetime_months",e),o=v=>v.learned_models.length?r`<div class="bl-learned">${t("settings_battery_lifetime_learned_models",e).replace("{list}",v.learned_models.map(f=>`${f.model} ${f.months} ${n} (${f.samples})`).join(" \xB7 "))}</div>`:l,g=(v,f)=>{let T=Number.parseInt(f,10);!Number.isFinite(T)||T<1||T>240||this._updateSetting("battery_lifetime_months",{...i,[v]:T})},b=v=>{let f={...i};delete f[v],this._updateSetting("battery_lifetime_months",f)},u=s.filter(v=>v.in_fleet),m=s.filter(v=>!v.in_fleet),E=v=>r`
      <div class="bl-row ${v.source==="override"?"bl-override":""}">
        <span class="bl-type">${v.type}${v.in_fleet?r` <span class="bl-fleet">${t("settings_battery_lifetime_in_fleet",e)}</span>`:l}</span>
        <input class="bl-months" type="number" min="1" max="240" .value=${y(String(v.months))}
          @change=${f=>g(v.type,f.target.value)} />
        <span class="bl-unit">${t("settings_battery_lifetime_months",e)}</span>
        <span class="bl-source">${a(v)}</span>
        ${v.source==="override"?r`<button type="button" class="bl-reset" @click=${()=>b(v.type)}>${t("settings_battery_lifetime_reset",e)}</button>`:l}
        ${o(v)}
      </div>`;return r`
    ...
    `}_renderBatteryNotesHint(e){let s=this._settings?.general?.battery_notes;if(!s||!s.devices)return l;let i=this._settings?.general?.battery_low_percent??20,a=s.default>i,n=t("bn_summary",e).replace("{name}","Battery Notes").replace("{pct}",String(s.default)).replace("{n}",String(s.devices)),o=(a?t("bn_above_floor",e).replace("{name}","Battery Notes"):t("bn_floor_decides",e)).replace("{floor}",String(i)),g=n.search(/[:：]/),b=g<0?n:n.slice(0,g),u=g<0?"":n.slice(g);return r`
      <div class="bn-note${a?" warn":""}">
        <ha-icon icon="${a?"mdi:alert-outline":"mdi:battery-heart-variant"}"></ha-icon>
        <span>
          <a class="bn-link" href="/config/integrations/integration/battery_notes">${b}</a>${u}
      ...
      <div class="settings-section member-avatars" data-section="member_avatars">
        ...
        ${this._users.map(i=>{let a=N(i),n=s[i.id]||{};return r`
    ...
    `}scrollToSection(e){requestAnimationFrame(()=>{let s=this.shadowRoot;if(!s)return;let i=s.querySelector(`[data-section="${e}"]`)??s.querySelector(`[data-section-alt="${e}"]`);i&&i.scrollIntoView({behavior:"smooth",block:"start"})})}_renderPanelAccess(e){let s=new Set(this._settings.admin_panel_user_ids||[]),i=this._users.filter(o=>!o.is_admin),a=this._settings.operator_write_enabled??!1,n=(o,g)=>{let b=new Set(s);g?b.add(o):b.delete(o),this._updateSetting("admin_panel_user_ids",[...b])};return r`
    ...
    `}async _loadTemplates(){if(!this._templatesRequested){this._templatesRequested=!0;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/templates",language:this._lang});this._allTemplates=e.templates||[],this._templateCategories=e.categories||{}}catch{}}}_renderTemplateToggles(e){this._loadTemplates();let s=new Set(this._settings.disabled_template_ids||[]),i=new Map;for(let n of Object.keys(this._templateCategories))i.set(n,[]);for(let n of this._allTemplates)i.has(n.category)||i.set(n.category,[]),i.get(n.category).push(n);let a=n=>this._templateCategories[n]?.["name_"+e]||this._templateCategories[n]?.name_en||n;return r`
      <div class="settings-section" data-section="templates">
        ...
        ${[...i.entries()].filter(([,n])=>n.length>0).map(([n,o])=>{let g=o.filter(u=>!s.has(u.id)).length,b=this._tplOpenGroups.has(n);return r`
            <div class="tpl-group">
              <div
                ...
                @keydown=${u=>{(u.key==="Enter"||u.key===" ")&&(u.preventDefault(),this._toggleTplGroupOpen(n))}}
              >
                ...
                <input
                  ...
                  @change=${u=>this._toggleTemplateGroup(o.map(m=>m.id),u.target.checked)}
                />
              </div>
              ${b?o.map(u=>r`
                    <label class="setting-row tpl-row">
                      ...
                      <input
                        type="checkbox"
                        .checked=${!s.has(u.id)}
                        @change=${m=>this._toggleTemplate(u.id,m.target.checked)}
                      />
                    </label>
                  `):l}
            </div>
    ...
    `}_toggleColumn(e,s){let i=new Set(j(this._settings.objects_table_columns));s?i.add(e):i.delete(e);let a=A.filter(n=>n.required||i.has(n.key)).map(n=>n.key);this._updateSetting("objects_table_columns",a)}_renderGeneral(e){let s=this._settings.general,i=s.notify_targets??[],a=this._settings.budget;return r`
      <div class="settings-section">
        <h3>${t("settings_general",e)}</h3>
        <label class="setting-row">
          <span class="setting-label">${t("settings_default_warning",e)}</span>
          <input type="number" min="0" max="365" .value=${y(String(s.default_warning_days))}
        ...
        <label class="setting-row">
          ...
          <input type="number" min="1" max="90" .value=${y(String(s.default_consumable_threshold??10))}
        ...
        <label class="setting-row">
          ...
          <input type="number" min="1" max="90" .value=${y(String(s.battery_low_percent??20))}
        ...
        <label class="setting-row">
          ...
          <select .value=${y(s.row_action_style||"buttons_compact")}
        ...
        <label class="setting-row">
          ...
          <select .value=${y(a.currency)} @change=${n=>this._updateSetting("budget_currency",n.target.value)}>
            ${W.map(n=>r`<option value=${n} ?selected=${a.currency===n}>${n}</option>`)}
          </select>
        </label>
        <label class="setting-row">
        ...
        <label class="setting-row">
          ...
          <select .value=${y(s.shopping_list_entity||"")}
    ...
    `}_todoEntities(e){let s=Object.keys(this.hass?.states||{}).filter(i=>i.startsWith("todo.")).sort();return e&&!s.includes(e)&&s.unshift(e),s}_renderNotifications(e){let s=this._settings.notifications,i=this._settings.actions;return r`
      <div class="settings-section">
        ...
        <label class="setting-row">
          ...
          <select class="notify-completed" .value=${y(s.completed||"off")}
