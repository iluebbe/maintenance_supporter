...
import{a as b,e as E}from"/maintenance_supporter_panelfiles/panel-chunks/chunk-767A3O6T.js";import{a as P}from"/maintenance_supporter_panelfiles/panel-chunks/chunk-PHVGZO3C.js";import{K as w,a as r,b as k,c as o,f as c,h as y,l as n,m as p,q as a,w as x,x as $}from"/maintenance_supporter_panelfiles/panel-chunks/chunk-YCBGS4NI.js";function q(_){if(!_)return[];let g=[],e=_.photo_doc_id;typeof e=="string"&&g.push(e);let t=_.photo_doc_ids;Array.isArray(t)&&g.push(...t);let i=[];for(let l of g){if(typeof l!="string")continue;let d=l.trim();if(!(!d||i.includes(d))&&(i.push(d),i.length>=10))break}return i}async function S(_,g,e){let t=new FormData;t.append("entry_id",g),t.append("tags","photo"),t.append("file",e,e.name);let i=await fetch("/api/maintenance_supporter/document/upload",{method:"POST",headers:{Authorization:`Bearer ${_.auth?.data?.access_token??""}`},body:t});if(i.status===413)throw new Error("doc_too_large");if(!i.ok)throw new Error("doc_upload_failed");let l=await i.json();if(!l.id)throw new Error("doc_upload_failed");return l.id}async function v(_,g){await Promise.all(g.map(e=>_.connection.sendMessagePromise({type:"maintenance_supporter/documents/delete",doc_id:e}).catch(()=>{})))}var s=class extends y{constructor(){super(...arguments);this.entryId="";this.taskId="";this.taskName="";this.lang="en";this.checklist=[];this.adaptiveEnabled=!1;this.taskType="";this.readingUnit="";this.readings=[];this.lastReadings={};this.restockDefault=null;this.restockUnitCost=null;this.currencySymbol="";this.parts=[];this.consumesParts=[];this.consumesInfo=[];this.requiredFields=[];this.phaseLabel="";this.requireTagScan=!1;this.viaTagScan=!1;this._open=!1;this._notes="";this._cost="";this._duration="";this._loading=!1;this._error="";this._checklistState={};this._feedback="needed";this._photos=[];this._uploadedIds=[];this._photoUploading=!1;this._readingValue="";this._readingValues={};this._restockQty="";this._completedAt="";this._usedParts={};this.checklistPrefill={}}open(e={}){this._open||(this._open=!0,this.viaTagScan=!!e.viaTagScan,this._notes="",this._cost="",this._duration="",this._error="",this._checklistState=Object.fromEntries(this.checklist.map((t,i)=>[String(i),!!this.checklistPrefill[t]]).filter(([,t])=>t)),this._feedback="needed",this._photos.forEach(t=>URL.revokeObjectURL(t.preview)),this._photos=[],this._uploadedIds=[],this._photoUploading=!1,this._readingValue="",this._readingValues={},this._restockQty=this.restockDefault!==null?String(this.restockDefault):"",this._completedAt="",this._usedParts=Object.fromEntries(this.consumesParts.map(t=>[b(t),{...t}])))}_toggleCheck(e){let t=String(e);this._checklistState={...this._checklistState,[t]:!this._checklistState[t]}}_setFeedback(e){this._feedback=e}async _onPhotoInput(e){let t=e.target,i=Array.from(t.files??[]);if(t.value="",i.length===0)return;let l=10-this._photos.length,d=i.slice(0,Math.max(l,0));this._photoUploading=!0,this._error="";try{for(let h of d){let f=await S(this.hass,this.entryId,h);this._uploadedIds=[...this._uploadedIds,f],this._photos=[...this._photos,{id:f,preview:URL.createObjectURL(h)}]}i.length>d.length&&(this._error=a("photos_limit",this.lang).replace("{max}",String(10)))}catch(h){let f=h instanceof Error&&h.message==="doc_too_large"?"doc_too_large":"doc_upload_failed";this._error=a(f,this.lang)}finally{this._photoUploading=!1}}_removePhoto(e){let t=this._photos.find(i=>i.id===e);t&&URL.revokeObjectURL(t.preview),this._photos=this._photos.filter(i=>i.id!==e),this._uploadedIds.includes(e)&&(this._uploadedIds=this._uploadedIds.filter(i=>i!==e),v(this.hass,[e]))}async _complete(){this._loading=!0,this._error="";try{let e={type:"maintenance_supporter/task/complete",entry_id:this.entryId,task_id:this.taskId};if(this._notes&&(e.notes=this._notes),this._cost){let t=parseFloat(this._cost);!isNaN(t)&&t>=0&&(e.cost=t)}if(this._duration){let t=parseInt(this._duration,10);!isNaN(t)&&t>=0&&(e.duration=t)}if(this.checklist.length>0&&(e.checklist_state=this._checklistState),this.adaptiveEnabled&&(e.feedback=this._feedback),this._photos.length>0&&(e.photo_doc_ids=this._photos.map(t=>t.id)),this.viaTagScan&&(e.via_tag_scan=!0),this._completedAt){if(new Date(this._completedAt).getTime()>Date.now()){this._error=a("completed_at_future_error",this.lang),this._loading=!1;return}e.completed_at=this._completedAt.length===16?`${this._completedAt}:00`:this._completedAt}if(this.readings.length>0){let t={};for(let i of this.readings){let l=(this._readingValues[i.id]??"").trim();if(l==="")continue;let d=parseFloat(l.replace(",","."));isNaN(d)||(t[i.id]=d)}Object.keys(t).length>0&&(e.reading_values=t)}else if(this._readingValue!==""){let t=parseFloat(this._readingValue);isNaN(t)||(e.reading_value=t)}if(this.restockDefault!==null&&this._restockQty!==""){let t=parseFloat(this._restockQty);!isNaN(t)&&t>=1&&(e.restock_quantity=t)}this.parts.length>0&&(e.used_parts=Object.values(this._usedParts).filter(t=>Number.isFinite(t.quantity)&&t.quantity>0).map(t=>t.entry_id?{part_id:t.part_id,quantity:t.quantity,entry_id:t.entry_id}:{part_id:t.part_id,quantity:t.quantity})),await this.hass.connection.sendMessagePromise(e),this._uploadedIds=[],this._open=!1,this.dispatchEvent(new CustomEvent("task-completed"))}catch(e){this._error=P(e,this.lang,a("save_error",this.lang))}finally{this._loading=!1}}_renderReadingField(e,t){let i=this.lastReadings[e.id],l=e.unit||this.readingUnit,d=(this._readingValues[e.id]??"").trim(),h=d===""?NaN:parseFloat(d.replace(",",".")),f=i!==void 0&&!isNaN(h)&&h<i.value,u=i!==void 0?x(i.value,t,{maximumFractionDigits:3}):"";return o`
      ...
      <ha-dialog open @closed=${this._close}>
                ...
                ${this.parts.map(t=>{let i=b({part_id:t.id,entry_id:t.entry_id}),l=this._usedParts[i],d=l!==void 0,h=t.entry_id?{part_id:t.id,quantity:1,entry_id:t.entry_id}:{part_id:t.id,quantity:1};return o`<div class="used-part-row">
                    <label class="used-part-check">
                      <input type="checkbox" .checked=${d}
                        @change=${f=>{let u={...this._usedParts};f.target.checked?u[i]=u[i]||h:delete u[i],this._usedParts=u}} />
                      <span
                    ...
                    ${d?o`<input class="used-part-qty" type="number" min="0.01" max="999" step="0.01"
                          ...
                          @input=${f=>{let u=parseFloat(f.target.value);this._usedParts={...this._usedParts,[i]:{...h,quantity:Number.isFinite(u)&&u>=.01?u:1}}}} />`:c}
                  </div>`})}
        ...
        <div class="dialog-actions">
          ...
          <ha-button
            ...
            title=${this._missingRequired.length?this._missingRequired.map(t=>a("err_required",e).replace("{field}",a(E[t]??t,e))).join(" \xB7 "):""}
          >
