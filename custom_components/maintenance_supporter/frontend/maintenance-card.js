...
`).map(u=>u.trim()).filter(Boolean).slice(0,100);e.checklist=d.length?d:null}if(this.completionActionsEnabled){let d=this._actionService.trim();if(d&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(d)){let f={service:d},y=this._actionTargetEntity.trim();y&&(f.target={entity_id:y});let g=this._buildActionData();Object.keys(g).length>0&&(f.data=g),e.on_complete_action=f}else e.on_complete_action=null;let u={};this._qcNotes.trim()&&(u.notes=this._qcNotes.trim());let _=parseFloat(this._qcCost);!isNaN(_)&&_>=0&&(u.cost=_);let v=parseInt(this._qcDuration,10);!isNaN(v)&&v>=0&&(u.duration=v),this._qcFeedback&&(u.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(u).length?u:null}let i=await this.hass.connection.sendMessagePromise(e),n=this._taskId||i?.task_id,c=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(n&&this._scheduleType==="sensor_based"&&c)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:n,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=P(e,this._lang,r("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return h;let e=this._lang,t=this._triggerType==="compound";return o`
      ...
      <div class="select-row">
        ...
        >
          ${ns.map(i=>o`<option value=${i} ?selected=${i===this._triggerType}>${r(i,e)}</option>`)}
        </select>
      </div>
    ...
    `}_patchCondition(e,t){this._compoundConditions=this._compoundConditions.map((i,n)=>n===e?{...i,...t}:i)}_addCondition(){this._compoundConditions=[...this._compoundConditions,os()]}_removeCondition(e){this._compoundConditions=this._compoundConditions.filter((t,i)=>i!==e)}_renderCompoundEditor(){let e=this._lang;return o`
      <div class="select-row">
    ...
    `}_renderCondition(e,t){let i=this._lang,n=t+1;return o`
      <div class="compound-condition">
        ...
        <div class="select-row">
          ...
          >
            ${ii.map(c=>o`<option value=${c} ?selected=${c===e.type}>${r(c,i)}</option>`)}
          </select>
        </div>
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${ss.map(i=>o`<option value=${i} ?selected=${i===this._type}>${r(i,e)}</option>`)}
            </select>
          </div>
          ...
          <div class="select-row">
            ...
            >
              ${rs.map(i=>o`<option value=${i} ?selected=${i===this._priority}>${r("priority_"+i,e)}</option>`)}
            </select>
          </div>
          <div class="field">
          ...
          <div class="select-row">
            ...
            >
              ${as.map(i=>o`<option value=${i} ?selected=${i===this._scheduleType}>${r(i,e)}</option>`)}
            </select>
          </div>
...
<script>setTimeout(()=>window.print(),300)<\/script>
</body></html>`),n.document.close()}_downloadSvg(e,t){let i=decodeURIComponent(e.svg_data_uri.replace("data:image/svg+xml,","")),n=new Blob([i],{type:"image/svg+xml"}),c=URL.createObjectURL(n),d=document.createElement("a");d.href=c;let u=this._taskName?`${this._objectName}-${this._taskName}`:this._objectName;d.download=`qr-${us(u)}-${t}.svg`,d.click(),URL.revokeObjectURL(c)}_close(){this._open=!1,this._viewResult=null,this._completeResult=null,this._error="",this._loading=!1}render(){if(!this._open)return o``;let e=this.lang||this.hass?.language||"en",t=this._taskName?`${r("qr_code",e)}: ${this._objectName} \u2014 ${this._taskName}`:`${r("qr_code",e)}: ${this._objectName}`,i=!!this._viewResult;return o`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
  ...
  `,l([b({attribute:!1})],N.prototype,"hass",2),l([b()],N.prototype,"lang",2),l([p()],N.prototype,"_open",2),l([p()],N.prototype,"_loading",2),l([p()],N.prototype,"_error",2),l([p()],N.prototype,"_viewResult",2),l([p()],N.prototype,"_completeResult",2),l([p()],N.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",N)});function oi(a,s){let e=a.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return h;let n=a.interval_days??0,c=a.suggested_interval??n;return o`
    <div class="weibull-section">
      ...
      ${gs(t,i,n,c,s)}
      ${ms(e,s)}
      ${e?.confidence_interval_low!=null?fs(e,a,s):h}
    </div>
  ...
  `}function gs(a,s,e,t,i){let g=Math.max(e,t,s,1)*1.3,x=50,H=[];for(let M=0;M<=x;M++){let O=M/x*g,Ai=1-Math.exp(-Math.pow(O/s,a)),Ti=32+O/g*260,Ci=136-Ai*128;H.push([Ti,Ci])}let j=H.map(([M,O])=>`${M.toFixed(1)},${O.toFixed(1)}`).join(" "),S="M32,136 "+H.map(([M,O])=>`L${M.toFixed(1)},${O.toFixed(1)}`).join(" ")+` L${H[x][0].toFixed(1)},136 Z`,C=32+e/g*260,$e=1-Math.exp(-Math.pow(e/s,a)),ke=136-$e*128,Ei=((1-$e)*100).toFixed(0),ut=32+t/g*260,Si=[0,.25,.5,.75,1];return o`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${r("chart_weibull",i)}">
        ${Si.map(M=>{let O=136-M*128;return oe`
        ...
        ${e>0?oe`
          ...
          <text x="${(C+4).toFixed(1)}" y="${(ke-6).toFixed(1)}" fill="var(--primary-color, #03a9f4)"
            font-size="9" font-weight="600">R=${Ei}%</text>
        `:h}
  ...
  `}function fs(a,s,e){let t=a.confidence_interval_low,i=a.confidence_interval_high,n=s.suggested_interval??s.interval_days??0,c=s.interval_days??0,d=Math.max(0,t-5),_=i+5-d,v=(t-d)/_*100,f=(i-t)/_*100,y=(n-d)/_*100,g=c>0?(c-d)/_*100:-1;return o`
  ...
  `}var di=w(()=>{"use strict";I();L()});function pi(a,s,e,t){let i=Math.max(a||1,s);return o`
  ...
  `}function gi(a,s){return vs(a,s)}function vs(a,s){let e=a.seasonal_factors??a.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return h;let t=a.interval_analysis?.seasonal_reason,i=new Date().getMonth(),n=300,c=100,d=8,_=c-d-4,v=Math.max(...e,1.5),f=n/12,y=f*.65,g=d+_-1/v*_;return o`
    ...
    `}_renderAdaptive(e){let t=this._lang,i=this._features.adaptive&&e.suggested_interval&&e.suggested_interval!==e.interval_days,n=e.degradation_trend!=null&&e.degradation_trend!=="insufficient_data"||e.days_until_threshold!=null||e.environmental_factor!=null&&e.environmental_factor!==1,c=this._features.adaptive&&e.interval_analysis?.weibull_beta!=null&&e.interval_analysis?.weibull_eta!=null,d=this._features.seasonal&&e.seasonal_factor&&e.seasonal_factor!==1;return!i&&!n&&!c&&!d?o`<div class="adaptive-empty">
      ...
      <div class="adaptive-stack">
        ...
        ${d?o`
          ...
          ${e.seasonal_factors?.length===12||e.interval_analysis?.seasonal_factors?.length===12?gi(e,t):h}
        `:h}
      </div>
                ...
                ${[...i].reverse().slice(0,20).map(u=>{let _=["completed","reset","skipped"].includes(u.type);return o`
                    <div class="history-entry">
                      <div class="history-line">
                        ...
                        <span class="history-date">${Yt(u.timestamp,t)}</span>
        ...
        ${t?o`
              <div class="header">
                ...
                <div class="quick-info">
                  ...
                  ${t.schedule?.kind&&!["manual","one_time"].includes(t.schedule.kind)||t.interval_days!=null?o`<span><strong>${r("interval",e)||"Interval"}:</strong> ${Jt(t,e)}</span>`:h}
                </div>
              </div>
      ...
      <div class="meta">
        ${i.map(([n,c])=>o`
            <div class="meta-item">
              ...
              <span class="meta-value">${vi(c)?o`<a href="${c}" target="_blank" rel="noopener noreferrer">${c}</a>`:c}</span>
            </div>
          `)}
      </div>
    `}};U.styles=E`
  ...
  `,l([b({attribute:!1})],U.prototype,"hass",2),l([p()],U.prototype,"_open",2),l([p()],U.prototype,"_entryId",2),l([p()],U.prototype,"_data",2),l([p()],U.prototype,"_busy",2),l([p()],U.prototype,"_error",2);customElements.get("maintenance-object-quick-actions-dialog")||customElements.define("maintenance-object-quick-actions-dialog",U)});var G={};Ii(G,{openCompleteDialog:()=>As,openCreateObjectDialog:()=>dt,openCreateTaskDialog:()=>pt,openEditObjectDialog:()=>ks,openEditTaskDialog:()=>Es,openHistoryEditDialog:()=>Ss,openObjectQuickActions:()=>Cs,openQrDialog:()=>Ts,openTaskQuickActions:()=>ht});function De(){return document.querySelector("home-assistant")?.hass}function K(a){let s=document.body.querySelector(a);return s||(s=document.createElement(a),document.body.appendChild(s)),s}function Y(a){let s=De();if(!s)return!1;a.hass=s;let e=s.language||"en";return He(e)||Ne(e).then(()=>{a.requestUpdate?.()}),!0}function ki(a){return Oe||(Oe=a.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).then(s=>({features:s.features??xi.features,defaultWarningDays:s.general?.default_warning_days??7})).catch(()=>xi),Oe)}function dt(){let a=K(wi);return Y(a)?(a.openCreate(),!0):!1}function ks(a,s){let e=K(wi);return Y(e)?(e.openEdit(a,s),!0):!1}function pt(){let a=K($i);if(!Y(a))return!1;let s=De();return s?((async()=>{let e=await ki(s),t=a;t.checklistsEnabled=e.features.checklists,t.scheduleTimeEnabled=e.features.schedule_time,t.completionActionsEnabled=e.features.completion_actions,t.defaultWarningDays=e.defaultWarningDays,t.openCreate()})(),!0):!1}function Es(a,s){let e=K($i);if(!Y(e))return!1;let t=De();return t?((async()=>{try{let[i,n]=await Promise.all([t.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:a}),ki(t)]),c=(i.tasks||[]).find(u=>u.id===s);if(!c){console.warn(`openEditTaskDialog: task ${s} not found in entry ${a}`);return}let d=e;d.checklistsEnabled=n.features.checklists,d.scheduleTimeEnabled=n.features.schedule_time,d.completionActionsEnabled=n.features.completion_actions,d.defaultWarningDays=n.defaultWarningDays,await d.openEdit(a,c)}catch(i){console.warn("openEditTaskDialog: failed to load task/features",i)}})(),!0):!1}function Ss(a){let s=K(bs);return Y(s)?(s.openEdit(a),!0):!1}function As(a){let s=K(ys);return Y(s)?(s.entryId=a.entry_id,s.taskId=a.task_id,s.taskName=a.task_name,s.checklist=a.checklist??[],s.adaptiveEnabled=!!a.adaptive_enabled,s.requiredFields=a.required_completion_fields??[],s.lang=De()?.language||"en",s.open(),!0):!1}function Ts(a){let s=K(xs);return Y(s)?(s.openForTask(a.entry_id,a.task_id,a.object_name,a.task_name),!0):!1}function ht(a,s){let e=K(ws);return Y(e)?(e.openFor(a,s),!0):!1}function Cs(a){let s=K($s);return Y(s)?(s.openFor(a),!0):!1}var wi,$i,bs,ys,xs,ws,$s,xi,Oe,F=w(()=>{"use strict";ti();si();nt();ri();ni();fi();yi();L();wi="maintenance-object-dialog",$i="maintenance-task-dialog",bs="maintenance-history-edit-dialog",ys="maintenance-complete-dialog",xs="maintenance-qr-dialog",ws="maintenance-task-quick-actions-dialog",$s="maintenance-object-quick-actions-dialog";xi={features:{adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1},defaultWarningDays:7},Oe=null});I();function jt(a,s){if(s.objects)return s.objects;let e=s.delta||[],t=s.removed||[];if(!e.length&&!t.length)return null;let i=new Map(a.map(n=>[n.entry_id,n]));for(let n of e)i.set(n.entry_id,n);for(let n of t)i.delete(n);return[...i.values()]}z();L();rt();Me();I();z();L();var Xi=["overdue","triggered","due_soon","ok"],D=class extends k{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-card"};this._objects=[];this._loadingObjects=!0;this._loadError=!1;this._views=[];this._objectsLoaded=!1;this._onEntitiesChanged=e=>{this._valueChanged("entity_ids",e.detail.value||[])}}get _lang(){return this.hass?.language||"en"}setConfig(e){this._config={...e}}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._objectsLoaded&&(this._objectsLoaded=!0,this._loadObjects())}async _loadObjects(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"});this._objects=e.objects||[],this._loadError=!1}catch{this._objects=[],this._loadError=!0}this._loadingObjects=!1;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"});this._views=e.views||[]}catch{this._views=[]}}_valueChanged(e,t){let i={...this._config,[e]:t};(Array.isArray(t)&&t.length===0||t==="")&&delete i[e],this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i}}))}_toggleStatus(e,t){let i=new Set(this._config.filter_status||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_status",[...i])}_toggleObject(e,t){let i=new Set(this._config.filter_objects||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_objects",[...i])}_toggleLabel(e,t){let i=new Set(this._config.filter_labels||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_labels",[...i])}_toggleArea(e,t){let i=new Set(this._config.filter_areas||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_areas",[...i])}render(){let e=this._lang,t=new Set(this._config.filter_status||[]),i=new Set(this._config.filter_objects||[]),n=[...this._objects].map(g=>g.object.name).sort((g,x)=>g.localeCompare(x)),c=new Set(this._config.filter_areas||[]),d=[...new Set(this._objects.map(g=>g.object.area_id).filter(g=>!!g).concat([...c]))],u=g=>this.hass?.areas?.[g]?.name||g,_=d.map(g=>({id:g,name:u(g)})).sort((g,x)=>g.name.localeCompare(x.name)),v=new Set(this._config.filter_labels||[]),f=[...new Set(this._objects.flatMap(g=>g.tasks.flatMap(x=>x.labels||[])))].sort((g,x)=>g.localeCompare(x)),y=[];for(let g of this._objects)for(let x of g.tasks)x.sensor_entity_id&&y.push(x.sensor_entity_id),x.binary_sensor_entity_id&&y.push(x.binary_sensor_entity_id);return o`
      <div class="editor">
        <ha-textfield
                    ...
                    <div class="task-item clickable"
                      ...
                      ${n?o`
                            <mwc-icon-button
                              class="complete-btn"
                              title="${r("complete",e)}"
                              @click=${y=>{y.stopPropagation();let g=this.shadowRoot.querySelector("maintenance-complete-dialog");g.entryId=_,g.taskId=f.id,g.taskName=f.name,g.checklist=f.checklist||[],g.adaptiveEnabled=!!f.adaptive_config?.enabled,g.taskType=f.type||"",g.readingUnit=f.reading_unit||"",g.requiredFields=f.required_completion_fields||[],g.lang=e;let x=!!f.part_ref;g.parts=x?[]:Xt(f,_,this._objects,e),g.consumesParts=x?[]:f.consumes_parts||[],g.open()}}
                            >
                              <ha-icon icon="mdi:check"></ha-icon>
                            </mwc-icon-button>
                          `:h}
