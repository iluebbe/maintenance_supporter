...
`;var at=["overdue","triggered","due_soon","ok"],x=class extends y{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-card"};this._objects=[];this._loadingObjects=!0;this._loadError=!1;this._objectsLoaded=!1;this._onEntitiesChanged=t=>{this._valueChanged("entity_ids",t.detail.value||[])}}get _lang(){return this.hass?.language||"en"}setConfig(t){this._config={...t}}updated(t){super.updated(t),t.has("hass")&&this.hass&&!this._objectsLoaded&&(this._objectsLoaded=!0,this._loadObjects())}async _loadObjects(){try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"});this._objects=t.objects||[],this._loadError=!1}catch{this._objects=[],this._loadError=!0}this._loadingObjects=!1}_valueChanged(t,a){let n={...this._config,[t]:a};Array.isArray(a)&&a.length===0&&delete n[t],this._config=n,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:n}}))}_toggleStatus(t,a){let n=new Set(this._config.filter_status||[]);a?n.add(t):n.delete(t),this._valueChanged("filter_status",[...n])}_toggleObject(t,a){let n=new Set(this._config.filter_objects||[]);a?n.add(t):n.delete(t),this._valueChanged("filter_objects",[...n])}render(){let t=this._lang,a=new Set(this._config.filter_status||[]),n=new Set(this._config.filter_objects||[]),i=[...this._objects].map(s=>s.object.name).sort((s,l)=>s.localeCompare(l)),r=[];for(let s of this._objects)for(let l of s.tasks)l.sensor_entity_id&&r.push(l.sensor_entity_id),l.binary_sensor_entity_id&&r.push(l.binary_sensor_entity_id);return m`
      <div class="editor">
        <ha-textfield
        ...
        <div class="field">
          ...
          <div class="chip-row">
            ${at.map(s=>m`
              <label class="chip ${a.has(s)?"active":""}">
                <input type="checkbox"
                  .checked=${a.has(s)}
                  @change=${l=>this._toggleStatus(s,l.target.checked)} />
                ${_(s,t)}
              </label>
            `)}
          </div>
        ...
        <div class="field">
          ...
          ${this._loadingObjects?m`<div class="field-help">${_("card_loading_objects",t)}</div>`:this._loadError?m`<div class="field-help error-text">${_("card_load_error",t)}</div>`:i.length===0?m`<div class="field-help">${_("no_objects",t)}</div>`:m`
                <div class="object-list">
                  ${i.map(s=>m`
                    <label class="object-row">
                      <input type="checkbox"
                        .checked=${n.has(s)}
                        @change=${l=>this._toggleObject(s,l.target.checked)} />
                      <span>${s}</span>
                    </label>
                  `)}
                </div>
  ...
  `,u([v({attribute:!1})],f.prototype,"hass",2),u([v()],f.prototype,"entryId",2),u([v()],f.prototype,"taskId",2),u([v()],f.prototype,"taskName",2),u([v()],f.prototype,"lang",2),u([v({type:Array})],f.prototype,"checklist",2),u([v({type:Boolean})],f.prototype,"adaptiveEnabled",2),u([h()],f.prototype,"_open",2),u([h()],f.prototype,"_notes",2),u([h()],f.prototype,"_cost",2),u([h()],f.prototype,"_duration",2),u([h()],f.prototype,"_loading",2),u([h()],f.prototype,"_error",2),u([h()],f.prototype,"_checklistState",2),u([h()],f.prototype,"_feedback",2);customElements.get("maintenance-complete-dialog")||customElements.define("maintenance-complete-dialog",f);var z=class extends y{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-card"};this._objects=[];this._stats=null;this._unsub=null;this._dataLoaded=!1;this._lastConnection=null;this._onCompleted=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"en"}static getConfigElement(){return document.createElement("maintenance-supporter-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-card",show_header:!0,show_actions:!0,filter_status:["overdue","triggered","due_soon"],max_items:10}}setConfig(t){this._config=t}getCardSize(){return 3}connectedCallback(){super.connectedCallback()}disconnectedCallback(){super.disconnectedCallback(),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null}updated(t){if(super.updated(t),t.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[t,a]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=t.objects,this._stats=a}catch{}}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(t=>{let a=t;this._objects=a.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _flatTasks(){let t=[],{filter_status:a,filter_objects:n,entity_ids:i,max_items:r}=this._config,s=i?.length?new Set(i):null;for(let p of this._objects)if(!(n?.length&&!n.includes(p.object.name)))for(let g of p.tasks)a?.length&&!a.includes(g.status)||s&&!(g.sensor_entity_id&&s.has(g.sensor_entity_id)||g.binary_sensor_entity_id&&s.has(g.binary_sensor_entity_id))||t.push({entry_id:p.entry_id,object_name:p.object.name,task:g});let l={overdue:0,triggered:1,due_soon:2,ok:3};return t.sort((p,g)=>(l[p.task.status]??9)-(l[g.task.status]??9)),r&&r>0?t.slice(0,r):t}render(){let t=this._lang,a=this._config.title||_("maintenance",t),n=this._config.show_header!==!1,i=this._config.show_actions!==!1,r=this._config.compact||!1,s=this._flatTasks,l=this._stats;return m`
      <ha-card>
        ...
        ${s.length===0?m`<div class="empty-card">
              <div>${_("card_no_tasks_title",t)}</div>
              <a class="empty-link" href="/maintenance-supporter">${_("card_no_tasks_cta",t)}</a>
            </div>`:m`
              <div class="task-list ${r?"compact":""}">
                ${s.map(({entry_id:p,object_name:g,task:d})=>m`
                    <div class="task-item">
                      <div class="status-dot" style="background: ${ze[d.status]||"#ccc"}"></div>
                      <div class="task-info">
                        <div class="task-name">${d.name}</div>
                        ${r?c:m`<div class="task-meta">${g} · ${_(d.type,t)}</div>`}
                      </div>
                      <div class="task-due">
                      ...
                      ${i?m`
                            <mwc-icon-button
                              class="complete-btn"
                              title="${_("complete",t)}"
                              @click=${()=>{let b=this.shadowRoot.querySelector("maintenance-complete-dialog");b.entryId=p,b.taskId=d.id,b.taskName=d.name,b.checklist=d.checklist||[],b.adaptiveEnabled=!!d.adaptive_config?.enabled,b.lang=t,b.open()}}
                            >
