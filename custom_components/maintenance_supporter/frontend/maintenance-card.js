  ...
  `}function _i(a,s){return fs(a,s)}function fs(a,s){let e=a.seasonal_factors??a.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return h;let t=a.interval_analysis?.seasonal_reason,i=new Date().getMonth(),n=300,c=100,d=8,_=c-d-4,v=Math.max(...e,1.5),f=n/12,b=f*.65,g=d+_-1/v*_;return o`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${n} ${c}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${r("chart_seasonal",s)}">
        ...
        ${e.map((x,H)=>{let j=x/v*_,S=H*f+(f-b)/2,C=d+_-j,$e=H===i,ke=x<1?"var(--success-color, #4caf50)":x>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return oe`
