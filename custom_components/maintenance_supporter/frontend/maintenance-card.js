...
`});function jr(s,r,e){let t=new Blob([s],{type:e}),i=URL.createObjectURL(t),a=document.createElement("a");a.href=i,a.download=r,a.target="_blank",a.rel="noopener",a.style.display="none",document.body.appendChild(a),a.dispatchEvent(new MouseEvent("click")),document.body.removeChild(a),setTimeout(()=>URL.revokeObjectURL(i),6e4)}var pi=x(()=>{"use strict"});async function Nn(s,r,e=300){return(await s.connection.sendMessagePromise({type:"auth/sign_path",path:r,expires:e})).path}async function hi(s,r,e=300){return Nn(s,`/api/maintenance_supporter/document/${r}`,e)}async function Hr(s,r,e=""){let t=window.open("about:blank","_blank");try{let i=await hi(s,r);t&&(t.location.href=new URL(i+e,window.location.origin).href)}catch(i){throw t&&t.close(),i}}var ui=x(()=>{"use strict";pi()});function Tt(s){return!!s&&/^https?:\/\//i.test(s)}var _i=x(()=>{"use strict"});function jn(s){let r=(s||"").split(/\s+/).filter(Boolean);return r.length===0?"?":r.length===1?r[0][0].toUpperCase():(r[0][0]+r[r.length-1][0]).toUpperCase()}function Hn(s){let r=0;for(let e of s)r=r*31+e.charCodeAt(0)>>>0;return Or[r%Or.length]}function At(s){return s?{id:s.id,name:s.name,initials:s.initials||jn(s.name),color:s.color||Hn(s.id)}:null}function mi(s){return s?o`<span class="person-avatar" style="--person-color: ${s.color}" title=${s.name}>${s.initials}</span>`:c}var Or,gi=x(()=>{"use strict";C();Or=["#c62828","#ad1457","#6a1b9a","#4527a0","#283593","#1565c0","#00838f","#2e7d32","#558b2f","#ef6c00","#6d4c41","#546e7a"]});var Le,fi=x(()=>{"use strict";gi();Le=class{constructor(r){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=r}updateHass(r){this.hass=r}async getUsers(r=!1){let e=Date.now();if(!r&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(r,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:r,task_id:e,user_id:t})}async getTasksByUser(r){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:r})).tasks}getUserName(r){return!r||!this.usersCache?null:this.usersCache.find(t=>t.id===r)?.name||null}getPerson(r){return At(this.getUser(r))}getUser(r){return!r||!this.usersCache?null:this.usersCache.find(e=>e.id===r)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(r){return r?r===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}}});function On(s,r){if(r<=0)return 0;let e=typeof s=="number"&&Number.isFinite(s)?Math.trunc(s):0;return e<0?0:e%r}function qn(s){return!!(s?.phases&&s.phase_sequence&&s.phase_sequence.length>0)}function vi(s){if(!s||!qn(s))return null;let r=s.phase_sequence,e=On(s.phase_cursor,r.length),t=r[e],i=s.phases?.[t];return i?{id:t,name:i.name,index:e,count:r.length,notes:i.notes,checklist:i.checklist!==void 0?i.checklist:s.checklist??[],consumesParts:i.consumes_parts!==void 0?i.consumes_parts:s.consumes_parts??[],requiredFields:i.required_completion_fields!==void 0?i.required_completion_fields:s.required_completion_fields??[]}:null}function pe(s){let r=vi(s);return r?`${r.index+1}/${r.count} \xB7 ${r.name}`:""}var Ct=x(()=>{"use strict"});function Dr(){let s=new Uint8Array(4);return(globalThis.crypto??{getRandomValues:r=>r.map(()=>Math.floor(Math.random()*256))}).getRandomValues(s),Array.from(s,r=>r.toString(16).padStart(2,"0")).join("")}function he(s){let r=s?.reading_values;if(!Array.isArray(r))return[];let e=[],t=new Set;for(let i of r){if(!i||typeof i!="object")continue;let a=i;typeof a.id!="string"||!a.id||t.has(a.id)||typeof a.value!="number"||!Number.isFinite(a.value)||(t.add(a.id),e.push({id:a.id,name:typeof a.name=="string"?a.name:a.id,unit:typeof a.unit=="string"?a.unit:null,value:a.value}))}return e}function Mr(s){return s.filter(r=>r.type==="completed"&&(r.reading_value!=null||he(r).length>0)).sort((r,e)=>r.timestamp.localeCompare(e.timestamp))}function zr(s){return Mr(s??[]).map(r=>({timestamp:r.timestamp,values:he(r)})).filter(r=>r.values.length>0)}function Fr(s,r,e){let t;for(let i of s){if(e!==void 0){let l=new Date(i.timestamp).getTime();if(!isNaN(l)&&l>=e)break}let a=i.values.find(l=>l.id===r);a&&(t=a)}return t}function Ur(s,r,e){let t=he(r).find(a=>a.id===e);if(!t)return null;let i=null;for(let a of Mr(s)){if(a.timestamp>=r.timestamp)break;let l=he(a).find(h=>h.id===e);l&&(i=l.value)}return i==null?null:t.value-i}function Br(s){let r=new Set,e=new Set;for(let t of s){let i=(t.name||"").trim().toLowerCase();i&&(r.has(i)?e.add(t.id):r.add(i))}return e}function Vr(s){let r=[],e=new Set,t=new Set;for(let i of s){let a=(i.name||"").trim();if(!(!a||e.has(i.id)||t.has(a.toLowerCase()))&&(e.add(i.id),t.add(a.toLowerCase()),r.push({id:i.id,name:a,unit:(i.unit||"").trim()||null}),r.length>=20))break}return r}var xe=x(()=>{"use strict"});function Z(s){return`${s.entry_id??""}\0${s.part_id}`}function Wr(s,r,e,t){let i=!!s.entry_id&&s.entry_id!==r,a=i?s.entry_id:r,l=e.find(m=>m.entry_id===a),h=(l?.parts||[]).find(m=>m.id===s.part_id)||null,u=i&&l?.object?.name||"",_=h?.name||n("shared_part_unknown",t);return{part:h,foreign:i,ownerName:u,label:u?`${_} (${u})`:_}}function Kr(s,r,e,t){let{part:i,label:a}=Wr(s,r,e,t),l=i&&i.stock!==null&&i.stock!==void 0?` (${i.stock}${i.unit?" "+i.unit:""})`:"",h=i?.storage_location?` \u2014 ${i.storage_location}`:"";return`${s.quantity}\xD7 ${a}${l}${h}`}function Yr(s,r,e,t){let a=(e.find(h=>h.entry_id===r)?.parts||[]).map(h=>({...h})),l=new Set(a.map(h=>Z({part_id:h.id})));for(let h of s?.consumes_parts||[]){if(!h.entry_id||h.entry_id===r)continue;let u=Z(h);if(l.has(u))continue;l.add(u);let{part:_,ownerName:m}=Wr(h,r,e,t);a.push({id:h.part_id,name:_?.name||n("shared_part_unknown",t),unit:_?.unit,stock:_?.stock??null,storage_location:_?.storage_location,entry_id:h.entry_id,owner_name:m})}return a}var It=x(()=>{"use strict";I()});function Rt(s){let r=s.task??null,e=r?vi(r):null,t=e?e.consumesParts:r?.consumes_parts||[],i=!!r?.part_ref,a=s.objects.find(m=>m.entry_id===s.entryId)?.parts||[],l=i?a.find(m=>m.id===r.part_ref.part_id):void 0,h=s.features?s.features.checklists:s.checklistsEnabled??!0,u=s.features?s.features.checklists?s.checklist??r?.checklist??[]:[]:s.checklist??[],_=s.features?s.features.adaptive&&(s.adaptiveEnabled??!!r?.adaptive_config?.enabled):!!s.adaptiveEnabled;return{entry_id:s.entryId,task_id:s.taskId,task_name:s.taskName,checklist:e?h?e.checklist:[]:u,adaptive_enabled:_,required_completion_fields:e?e.requiredFields:r?.required_completion_fields||[],task_type:r?.type||"",reading_unit:r?.reading_unit||"",readings:r?.readings||[],reading_history:zr(r?.history),parts:i?[]:Yr({consumes_parts:t},s.entryId,s.objects,s.lang),consumes_parts:i?[]:t,phase_label:e?pe(r):"",require_tag_scan:!!r?.require_tag_scan,restock_default:i?l?.restock_quantity??1:null,restock_unit_cost:i?l?.cost??null:null,currency_symbol:ye({currency_symbol:s.currencySymbol}),consumes_info:t.map(m=>Kr(m,s.entryId,s.objects,s.lang)),checklist_prefill:r?.checklist_progress||{},via_tag_scan:!!s.viaTagScan}}function Pt(s,r,e){s.entryId=r.entry_id,s.taskId=r.task_id,s.taskName=r.task_name,s.lang=e,s.checklist=r.checklist??[],s.adaptiveEnabled=!!r.adaptive_enabled,s.requiredFields=r.required_completion_fields??[],s.taskType=r.task_type??"",s.readingUnit=r.reading_unit??"",s.readings=r.readings??[],s.readingHistory=r.reading_history??[],s.parts=r.parts??[],s.consumesParts=r.consumes_parts??[],s.phaseLabel=r.phase_label??"",s.requireTagScan=!!r.require_tag_scan,s.restockDefault=r.restock_default??null,s.restockUnitCost=r.restock_unit_cost??null,s.currencySymbol=ye(r),s.consumesInfo=r.consumes_info??[],s.checklistPrefill=r.checklist_prefill??{},s.viaTagScan=!!r.via_tag_scan,s.open({viaTagScan:!!r.via_tag_scan})}var Lt=x(()=>{"use strict";xe();It();Ct();I()});function Ne(s,r=we){return s?s.is_admin?!0:r.operatorWriteEnabled&&r.operatorIds.includes(s.id):!1}var we,ot=x(()=>{"use strict";we={operatorWriteEnabled:!1,operatorIds:[]}});function Dn(s){return{features:{...je.features,...s?.features??{}},defaultWarningDays:s?.general?.default_warning_days??je.defaultWarningDays,rowActionStyle:s?.general?.row_action_style??je.rowActionStyle,access:{operatorWriteEnabled:s?.operator_write_enabled===!0,operatorIds:Array.isArray(s?.admin_panel_user_ids)?s.admin_panel_user_ids.filter(r=>typeof r=="string"):[]},budget:s?.budget??null}}function Gr(){let s=window;return s.__msSettingsCache??={promise:null}}function X(s){let r=Gr();if(r.promise)return r.promise;let e=s.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).then(t=>Dn(t)).catch(()=>(r.promise===e&&(r.promise=null),je));return r.promise=e,e}function Qr(){Gr().promise=null}var je,lt=x(()=>{"use strict";ot();je={features:{adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1},defaultWarningDays:7,rowActionStyle:"buttons_compact",access:we,budget:null}});function Un(s){let r=s.replace(/_ids?$/,"").split("_").filter(Boolean);if(r.length===0)return s;let e=r.join(" ");return e.charAt(0).toUpperCase()+e.slice(1)}function Bn(s,r){let e=Fn[s];if(e){let t=n(e,r);if(t&&t!==e)return t}return Un(s)}function Vn(s){let r=[...s.matchAll(/\['([^']+)'\]/g)].map(i=>i[1]),e=r.length?r[r.length-1]:void 0,t;return(t=s.match(/length of value must be at most (\d+)/))?{field:e,rule:"too_long",param:t[1]}:(t=s.match(/length of value must be at least (\d+)/))?{field:e,rule:"too_short",param:t[1]}:(t=s.match(/value must be at most (\S+)/))?{field:e,rule:"value_too_high",param:t[1]}:(t=s.match(/value must be at least (\S+)/))?{field:e,rule:"value_too_low",param:t[1]}:/required key not provided/.test(s)?{field:e,rule:"required"}:(t=s.match(/expected (\w+)/))?{field:e,rule:"wrong_type",param:t[1]}:/value must be one of/.test(s)?{field:e,rule:"invalid_choice"}:/not a valid value/.test(s)?{field:e,rule:"invalid_value"}:{field:e,rule:"unknown"}}function Wn(s,r){let e=new Set(Jr(r));return Jr(s).every(t=>e.has(t))}function R(s,r,e){if(e=e??n("action_error",r),typeof s=="string")return s;if(typeof s!="object"||s===null)return e;let t=s,i=t.message||t.error?.message||"",a=t.code||t.error?.code||"",l=a?zn[a]:void 0;if(!i&&!l)return e;let h=Vn(i),u=h.field?Bn(h.field,r):"",_=m=>n(m,r).replace("{field}",u).replace("{n}",h.param??"");switch(h.rule){case"too_long":return _("err_too_long");case"too_short":return _("err_too_short");case"value_too_high":return _("err_value_too_high");case"value_too_low":return _("err_value_too_low");case"required":return _("err_required");case"wrong_type":return _("err_wrong_type").replace("{type}",h.param??"");case"invalid_choice":return _("err_invalid_choice");case"invalid_value":return _("err_invalid_value");default:if(l){let m=n(l,r);return i&&!Wn(i,a)?`${m} (${i})`:m}return i||e}}var zn,Fn,Jr,ne=x(()=>{"use strict";I();zn={already_archived:"ws_err_already_archived",already_paused:"ws_err_already_paused",archived:"ws_err_archived",create_failed:"ws_err_create_failed",duplicate_failed:"ws_err_duplicate_failed",empty:"ws_err_empty",empty_csv:"ws_err_empty_csv",invalid_cursor:"ws_err_invalid_cursor",invalid_date:"ws_err_invalid_date",invalid_device:"ws_err_invalid_device",invalid_entity_slug:"ws_err_invalid_entity_slug",invalid_format:"ws_err_invalid_format",invalid_input:"ws_err_invalid_input",invalid_mirror_todo:"ws_err_invalid_mirror_todo",invalid_parent:"ws_err_invalid_parent",invalid_icon:"ws_err_invalid_icon",invalid_range:"ws_err_invalid_range",invalid_search_template:"ws_err_invalid_search_template",invalid_target:"ws_err_invalid_target",invalid_trigger_config:"ws_err_invalid_trigger_config",invalid_url:"ws_err_invalid_url",invalid_user:"ws_err_invalid_user",invalid_view:"ws_err_invalid_view",limit_reached:"ws_err_limit_reached",no_defaults:"ws_err_no_defaults",no_phases:"ws_err_no_phases",no_url:"ws_err_no_url",not_archived:"ws_err_not_archived",not_available:"ws_err_not_available",not_configured:"ws_err_not_configured",not_found:"ws_err_not_found",not_loaded:"ws_err_not_loaded",not_paused:"ws_err_not_paused",replace_failed:"ws_err_replace_failed",self_link_device:"ws_err_self_link_device",storage_unavailable:"ws_err_storage_unavailable",too_early:"ws_err_too_early",too_large:"ws_err_too_large",too_many:"ws_err_too_many",too_many_views:"ws_err_too_many_views",unauthorized:"ws_err_unauthorized",unavailable:"ws_err_unavailable"},Fn={entry_id:"object",name:"name",task_type:"maintenance_type",schedule_type:"schedule_type",interval_days:"interval_days",interval_anchor:"interval_anchor",warning_days:"warning_days",last_performed:"last_performed_optional",notes:"notes_optional",documentation_url:"documentation_url_optional",custom_icon:"custom_icon_optional",nfc_tag_id:"nfc_tag_id_optional",responsible_user_id:"responsible_user",entity_slug:"entity_slug",entity_id:"entity_id",area_id:"area_id_optional",manufacturer:"manufacturer_optional",model:"model_optional",serial_number:"serial_number_optional",installation_date:"installation_date_optional",warranty_expiry:"warranty_expiry_optional",checklist:"checklist_steps_optional",reason:"reason",feedback:"feedback",cost:"cost",duration:"duration",description:"description_optional",environmental_entity:"environmental_entity_optional",environmental_attribute:"environmental_attribute_optional",trigger_above:"trigger_above",trigger_below:"trigger_below",trigger_equals:"trigger_equals",trigger_not_equals:"trigger_not_equals",trigger_for_minutes:"trigger_for_minutes"};Jr=s=>s.toLowerCase().split(/[^a-z0-9]+/).filter(r=>r.length>1)});var bi,dt,yi=x(()=>{"use strict";bi=["notes","cost","duration","photo","user"],dt={notes:"notes_label",cost:"cost",duration:"duration",photo:"photo_label",user:"user_label"}});function Nt(s){if(!s)return[];let r=[],e=s.photo_doc_id;typeof e=="string"&&r.push(e);let t=s.photo_doc_ids;Array.isArray(t)&&r.push(...t);let i=[];for(let a of r){if(typeof a!="string")continue;let l=a.trim();if(!(!l||i.includes(l))&&(i.push(l),i.length>=10))break}return i}var jt=x(()=>{"use strict"});async function Kn(s,r,e){let t=s.auth;if(t?.expired&&t.refreshAccessToken)try{await t.refreshAccessToken()}catch{}return fetch(r,{...e,headers:{...e.headers??{},Authorization:`Bearer ${t?.data?.access_token??""}`}})}async function Yn(s,r,e,t){let i=new FormData;i.append("entry_id",r);for(let h of t)i.append("tags",h);i.append("file",e,e.name);let a=await Kn(s,"/api/maintenance_supporter/document/upload",{method:"POST",body:i});if(a.status===413)throw new Error("doc_too_large");if(!a.ok)throw new Error("doc_upload_failed");let l=await a.json();if(!l.id)throw new Error("doc_upload_failed");return{id:l.id,deduped:!!l.deduped,duplicate_in_object:l.duplicate_in_object??null}}async function Xr(s,r,e){return(await Yn(s,r,e,["photo"])).id}async function xi(s,r){await Promise.all(r.map(e=>s.connection.sendMessagePromise({type:"maintenance_supporter/documents/delete",doc_id:e}).catch(()=>{})))}var es=x(()=>{"use strict";jt()});var He,wi=x(()=>{"use strict";I();es();He=class{constructor(r,e){this.host=r;this.opts=e;this.photos=[];this.uploadedIds=[];this.uploading=!1;this.errorKey="";this.max=e.max??10,r.addController(this)}hostConnected(){}get ids(){return this.photos.map(r=>r.id)}get remaining(){return Math.max(this.max-this.photos.length,0)}get full(){return this.remaining===0}errorText(r){return this.errorKey?n(this.errorKey,r).replace("{max}",String(this.max)):""}clearError(){this.errorKey=""}reset(r=[]){this._revokeAll(),this.photos=r.map(e=>({id:e,preview:""})),this.uploadedIds=[],this.uploading=!1,this.errorKey="",this.host.requestUpdate()}async addFiles(r){if(r.length===0)return;let e=r.slice(0,this.remaining);this.uploading=!0,this.errorKey="",this.host.requestUpdate();try{for(let t of e){let i=await Xr(this.opts.hass(),this.opts.entryId(),t);this.uploadedIds=[...this.uploadedIds,i],this.photos=[...this.photos,{id:i,preview:URL.createObjectURL(t)}],this.host.requestUpdate()}r.length>e.length&&(this.errorKey="photos_limit")}catch(t){this.errorKey=t instanceof Error&&t.message==="doc_too_large"?"doc_too_large":"doc_upload_failed"}finally{this.uploading=!1,this.host.requestUpdate()}}remove(r){let e=this.photos.find(t=>t.id===r);e?.preview&&URL.revokeObjectURL(e.preview),this.photos=this.photos.filter(t=>t.id!==r),this.uploadedIds.includes(r)&&(this.uploadedIds=this.uploadedIds.filter(t=>t!==r),xi(this.opts.hass(),[r])),this.host.requestUpdate()}discardOrphans(){if(this.uploadedIds.length===0)return;let r=this.uploadedIds;this.uploadedIds=[],xi(this.opts.hass(),r)}markAttached(){this.uploadedIds=[]}_revokeAll(){for(let r of this.photos)r.preview&&URL.revokeObjectURL(r.preview)}}});function Oe(s){let r=(s??"").trim();if(r==="")return null;let e=Number(r.replace(",","."));return!Number.isFinite(e)||e<0?null:Math.round(e)}var Ht=x(()=>{"use strict"});function Gn(s){let e=F("2001-02-03",s).match(/\d+/g)||[],t=h=>e.findIndex(u=>Number(u)===h),i=t(2001),a=t(2),l=t(3);return i===0?"YMD":a>=0&&l>=0&&a<l?"MDY":"DMY"}function qe(s,r,e){if(s<1e3||s>9999||r<1||r>12||e<1||e>31)return null;let t=new Date(s,r-1,e);return t.getFullYear()!==s||t.getMonth()!==r-1||t.getDate()!==e?null:`${s}-${String(r).padStart(2,"0")}-${String(e).padStart(2,"0")}`}function ts(s,r){let e=s.trim(),t;if(t=e.match(/^(\d{4})-(\d{1,2})-(\d{1,2})$/))return qe(+t[1],+t[2],+t[3]);if(t=e.match(/^(\d{4})$/))return qe(+t[1],1,1);if(t=e.match(/^(\d{1,2})[./](\d{4})$/))return qe(+t[2],+t[1],1);if(t=e.match(/^(\d{1,2})\.(\d{1,2})\.(\d{4})$/))return qe(+t[3],+t[2],+t[1]);if(t=e.match(/^(\d{1,2})[/-](\d{1,2})[/-](\d{4})$/)){let i=+t[1],a=+t[2],l=+t[3];return Gn(r)==="MDY"?qe(l,i,a):qe(l,a,i)}return null}var is=x(()=>{"use strict";I()});function Qn(s,r){if(r)switch(s){case"date":return r.split("T")[0];case"time":return r.length===5?`${r}:00`:r;case"datetime":{let[e,t="00:00:00"]=r.split("T");return`${e} ${t.length===5?`${t}:00`:t}`}}}function Jn(s,r){if(typeof r!="string"||!r)return"";switch(s){case"date":return r.slice(0,10);case"time":return r.slice(0,5);case"datetime":{let[e,t="00:00:00"]=r.split(" ");return`${e}T${t.length===5?`${t}:00`:t}`}}}var M,De=x(()=>{"use strict";C();q();I();is();M=class extends E{constructor(){super(...arguments);this.kind="date";this.label="";this.value="";this.clearable=!1;this.disabled=!1;this.required=!1;this.lang="en";this._typing=!1;this._typed="";this._typedInvalid=!1}_startTyping(){this._typed=this.value,this._typedInvalid=!1,this._typing=!0,this.updateComplete.then(()=>this.shadowRoot?.querySelector("input.typed")?.focus())}_commitTyped(){if(!this._typed.trim()){this._typing=!1;return}let e=ts(this._typed,this.lang);if(!e){this._typedInvalid=!0;return}this._typing=!1,this._typedInvalid=!1,this._emit(e)}_onTypedKey(e){e.key==="Enter"?(e.preventDefault(),this._commitTyped()):e.key==="Escape"&&(e.preventDefault(),this._typing=!1,this._typedInvalid=!1)}_selector(){switch(this.kind){case"date":return{date:{}};case"time":return{time:{no_second:!0}};case"datetime":return{datetime:{}}}}_onSelectorChange(e){e.stopPropagation(),this._emit(Jn(this.kind,e.detail?.value))}_clear(){this._emit("")}_emit(e){e!==this.value&&(this.value=e,this.dispatchEvent(new CustomEvent("value-changed",{bubbles:!0,composed:!0,detail:{value:e}})))}render(){return o`
      <div class="field">
  ...
  `,d([b({type:String})],te.prototype,"lang",2),d([p()],te.prototype,"_open",2),d([p()],te.prototype,"_busy",2),d([p()],te.prototype,"_devices",2),d([p()],te.prototype,"_deviceIndex",2),d([p()],te.prototype,"_switchFailed",2);customElements.get("ms-camera-capture")||customElements.define("ms-camera-capture",te)});var qt,K,ht=x(()=>{"use strict";C();q();I();$i();Ei();Ei();qt=k`
...
`,K=class extends E{constructor(){super(...arguments);this.lang="en";this.disabled=!1;this.busy=!1;this.accept="image/*";this.showCamera=!0;this.showGallery=!0;this.compact=!1;this.remaining=1/0;this._singlePick=Ot();this._inAppCamera=rs()}createRenderRoot(){return this}get _locked(){return this.disabled||this.busy}_emit(e){e.length!==0&&this.dispatchEvent(new CustomEvent("files-picked",{detail:{files:e},bubbles:!0,composed:!0}))}_onInput(e){let t=e.target,i=Array.from(t.files??[]);t.value="",this._emit(i)}_onCameraClick(e){!this._inAppCamera||this._locked||(e.preventDefault(),this.querySelector("ms-camera-capture")?.open())}_onCameraUnavailable(){this._inAppCamera=!1,this.querySelector(".photo-pick-camera input")?.click()}_onKeydown(e){e.key!=="Enter"&&e.key!==" "||(e.preventDefault(),!this._locked&&e.currentTarget.click())}render(){if(this.remaining<=0||!this.showCamera&&!this.showGallery)return c;let e=this.lang,t=this._locked?"disabled":"",i=this.busy?n("uploading",e):n("doc_camera",e),a=n(this._singlePick?"choose_photo":"choose_photos",e);return o`
    ...
    `}};d([b({type:String})],K.prototype,"lang",2),d([b({type:Boolean})],K.prototype,"disabled",2),d([b({type:Boolean})],K.prototype,"busy",2),d([b({type:String})],K.prototype,"accept",2),d([b({type:Boolean})],K.prototype,"showCamera",2),d([b({type:Boolean})],K.prototype,"showGallery",2),d([b({type:Boolean})],K.prototype,"compact",2),d([b({type:Number})],K.prototype,"remaining",2),d([p()],K.prototype,"_inAppCamera",2);customElements.get("ms-photo-picker")||customElements.define("ms-photo-picker",K)});var Me,ut,S,Si=x(()=>{"use strict";C();q();xe();I();ne();It();yi();wi();Ht();De();ht();ht();Me=[.01,999],ut=[.01,9999],S=class extends E{constructor(){super(...arguments);this.entryId="";this.taskId="";this.taskName="";this.lang="en";this.checklist=[];this.adaptiveEnabled=!1;this.taskType="";this.readingUnit="";this.readings=[];this.readingHistory=[];this.restockDefault=null;this.restockUnitCost=null;this.currencySymbol="";this.parts=[];this.consumesParts=[];this.consumesInfo=[];this.requiredFields=[];this.phaseLabel="";this.requireTagScan=!1;this.viaTagScan=!1;this._open=!1;this._notes="";this._cost="";this._duration="";this._loading=!1;this._error="";this._checklistState={};this._feedback="needed";this._photos=new He(this,{entryId:()=>this.entryId,hass:()=>this.hass});this._readingValue="";this._readingValues={};this._restockQty="";this._completedAt="";this._usedParts={};this._usedQtyText={};this.checklistPrefill={}}open(e={}){this._open||(this._open=!0,this.viaTagScan=!!e.viaTagScan,this._notes="",this._cost="",this._duration="",this._error="",this._checklistState=Object.fromEntries(this.checklist.map((t,i)=>[String(i),!!this.checklistPrefill[t]]).filter(([,t])=>t)),this._feedback="needed",this._photos.reset(),this._readingValue="",this._readingValues={},this._restockQty=this.restockDefault!==null?String(this.restockDefault):"",this._completedAt="",this._usedParts=Object.fromEntries(this.consumesParts.map(t=>[Z(t),{...t}])),this._usedQtyText={})}_rangeError(e,t){let i=this.lang;return n("settings_value_out_of_range",i).replace("{min}",j(e,i,{maximumFractionDigits:2})).replace("{max}",j(t,i,{maximumFractionDigits:2}))}_toggleCheck(e){let t=String(e);this._checklistState={...this._checklistState,[t]:!this._checklistState[t]}}_setFeedback(e){this._feedback=e}async _complete(){this._loading=!0,this._error="",this._photos.clearError();try{let e={type:"maintenance_supporter/task/complete",entry_id:this.entryId,task_id:this.taskId};if(this._notes&&(e.notes=this._notes),this._cost){let i=parseFloat(this._cost);!isNaN(i)&&i>=0&&(e.cost=i)}let t=Oe(this._duration);if(t!==null&&(e.duration=t),this.checklist.length>0&&(e.checklist_state=this._checklistState),this.adaptiveEnabled&&(e.feedback=this._feedback),this._photos.photos.length>0&&(e.photo_doc_ids=this._photos.ids),this.viaTagScan&&(e.via_tag_scan=!0),this._completedAt){if(new Date(this._completedAt).getTime()>Date.now()){this._error=n("completed_at_future_error",this.lang),this._loading=!1;return}e.completed_at=this._completedAt.length===16?`${this._completedAt}:00`:this._completedAt}if(this.readings.length>0){let i={};for(let a of this.readings){let l=(this._readingValues[a.id]??"").trim();if(l==="")continue;let h=parseFloat(l.replace(",","."));isNaN(h)||(i[a.id]=h)}Object.keys(i).length>0&&(e.reading_values=i)}else if(this._readingValue!==""){let i=parseFloat(this._readingValue);isNaN(i)||(e.reading_value=i)}if(this.restockDefault!==null&&this._restockQty.trim()!==""){let i=parseFloat(this._restockQty.replace(",","."));if(!Number.isFinite(i)||i<ut[0]||i>ut[1]){this._error=this._rangeError(...ut),this._loading=!1;return}e.restock_quantity=i}if(this.parts.length>0){let i=[];for(let[a,l]of Object.entries(this._usedParts)){let h=this._usedQtyText[a],u=h===void 0?l.quantity:parseFloat(h.replace(",","."));if(!Number.isFinite(u)||u<Me[0]||u>Me[1]){this._error=this._rangeError(...Me),this._loading=!1;return}i.push({...l,quantity:u})}e.used_parts=i.map(a=>a.entry_id?{part_id:a.part_id,quantity:a.quantity,entry_id:a.entry_id}:{part_id:a.part_id,quantity:a.quantity})}await this.hass.connection.sendMessagePromise(e),this._photos.markAttached(),this._open=!1,this.dispatchEvent(new CustomEvent("task-completed"))}catch(e){this._error=R(e,this.lang,n("save_error",this.lang))}finally{this._loading=!1}}_renderReadingField(e,t){let i=this._completedAt?new Date(this._completedAt).getTime():NaN,a=Fr(this.readingHistory,e.id,isNaN(i)?void 0:i),l=e.unit||this.readingUnit,h=(this._readingValues[e.id]??"").trim(),u=h===""?NaN:parseFloat(h.replace(",",".")),_=a!==void 0&&!isNaN(u)&&u<a.value,m=a!==void 0?j(a.value,t,{maximumFractionDigits:3}):"";return o`
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          ${this.parts.length?o`<div class="used-parts">
                ...
                ${this.parts.map(i=>{let a=Z({part_id:i.id,entry_id:i.entry_id}),l=this._usedParts[a],h=l!==void 0,u=i.entry_id?{part_id:i.id,quantity:1,entry_id:i.entry_id}:{part_id:i.id,quantity:1};return o`<div class="used-part-row">
                    <label class="used-part-check">
                      <input type="checkbox" .checked=${h}
                        @change=${_=>{let m={...this._usedParts};if(_.target.checked)m[a]=m[a]||u;else{delete m[a];let v={...this._usedQtyText};delete v[a],this._usedQtyText=v}this._usedParts=m}} />
                      <span
                    ...
                    ${h?o`<input class="used-part-qty" type="number" min=${Me[0]} max=${Me[1]} step="0.01"
                          .value=${this._usedQtyText[a]??String(l.quantity)}
                          @input=${_=>{let m=_.target.value;this._usedQtyText={...this._usedQtyText,[a]:m};let v=parseFloat(m.replace(",","."));Number.isFinite(v)&&v>=Me[0]&&(this._usedParts={...this._usedParts,[a]:{...u,quantity:v}})}} />`:c}
                  </div>`})}
          ...
          ${this.restockDefault!==null?o`
              <label class="field">
                ...
                <input type="number" step="0.01" min=${ut[0]} max=${ut[1]} class="field-input"
        ...
        <div class="dialog-actions">
          ...
          <ha-button
            ...
            title=${this._missingRequired.length?this._missingRequired.map(i=>n("err_required",e).replace("{field}",n(dt[i]??i,e))).join(" \xB7 "):""}
          >
  ...
  `],d([b({attribute:!1})],S.prototype,"hass",2),d([b()],S.prototype,"entryId",2),d([b()],S.prototype,"taskId",2),d([b()],S.prototype,"taskName",2),d([b()],S.prototype,"lang",2),d([b({type:Array})],S.prototype,"checklist",2),d([b({type:Boolean})],S.prototype,"adaptiveEnabled",2),d([b()],S.prototype,"taskType",2),d([b()],S.prototype,"readingUnit",2),d([b({attribute:!1})],S.prototype,"readings",2),d([b({attribute:!1})],S.prototype,"readingHistory",2),d([b({attribute:!1})],S.prototype,"restockDefault",2),d([b({attribute:!1})],S.prototype,"restockUnitCost",2),d([b()],S.prototype,"currencySymbol",2),d([b({attribute:!1})],S.prototype,"parts",2),d([b({attribute:!1})],S.prototype,"consumesParts",2),d([b({type:Array})],S.prototype,"consumesInfo",2),d([b({type:Array})],S.prototype,"requiredFields",2),d([b()],S.prototype,"phaseLabel",2),d([b({type:Boolean})],S.prototype,"requireTagScan",2),d([b({type:Boolean})],S.prototype,"viaTagScan",2),d([p()],S.prototype,"_open",2),d([p()],S.prototype,"_notes",2),d([p()],S.prototype,"_cost",2),d([p()],S.prototype,"_duration",2),d([p()],S.prototype,"_loading",2),d([p()],S.prototype,"_error",2),d([p()],S.prototype,"_checklistState",2),d([p()],S.prototype,"_feedback",2),d([p()],S.prototype,"_readingValue",2),d([p()],S.prototype,"_readingValues",2),d([p()],S.prototype,"_restockQty",2),d([p()],S.prototype,"_completedAt",2),d([p()],S.prototype,"_usedParts",2),d([p()],S.prototype,"_usedQtyText",2),d([b({attribute:!1})],S.prototype,"checklistPrefill",2);customElements.get("maintenance-complete-dialog")||customElements.define("maintenance-complete-dialog",S)});function T(s){return s.toFixed(1)}var Dt=x(()=>{"use strict";I()});var D,Ai=x(()=>{"use strict";C();q();D=class extends E{constructor(){super(...arguments);this.label="";this.value="";this.placeholder="";this.type="text";this.required=!1;this.disabled=!1;this.multiline=!1;this.rows=3}_onInput(e){let t=e.target.value;this.value=t,this.dispatchEvent(new CustomEvent("input",{bubbles:!0,composed:!0,detail:{value:t}}))}render(){return o`
      <label class="field">
        ${this.label?o`<span class="label">${this.label}${this.required?o`<span class="req">*</span>`:c}</span>`:c}
        ${this.multiline?o`
        <textarea
...
`),partId:A?.part_id||"",partQty:A?.quantity!=null?String(A.quantity):"",reqOverride:m.required_completion_fields!==void 0,reqFields:[...m.required_completion_fields||[]],extraParts:z.filter((O,Ee)=>Ee!==L).map(O=>({...O})),carry:$}}),this._phaseSeq=[...t.phase_sequence||[]],this._requiredCompletion=[...t.required_completion_fields||[]],this._scheduleTime=t.schedule_time||"",this._scheduleTimeOn=!!t.schedule_time;let l=t.on_complete_action;if(l&&l.service){this._actionService=l.service;let _=l.target?.entity_id;this._actionTargetEntity=Array.isArray(_)?_[0]||"":_||"",this._actionData=l.data&&typeof l.data=="object"?{...l.data}:{},this._actionDataJsonFallback=""}else this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="";let h=t.quick_complete_defaults;this._qcNotes=h?.notes||"",this._qcCost=h?.cost!=null?String(h.cost):"",this._qcDuration=h?.duration!=null?String(h.duration):"",this._qcFeedback=h?.feedback||"";let u=t.adaptive_config||{};if(this._environmentalEntity=u.environmental_entity||"",this._environmentalAttribute=u.environmental_attribute||"",this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute,this._adaptiveEnabled=!!u.enabled,this._adaptiveAlpha=u.ewa_alpha?.toString()??ae.alpha,this._adaptiveMin=u.min_interval_days?.toString()??ae.min,this._adaptiveMax=u.max_interval_days?.toString()??ae.max,this._adaptiveSeasonal=u.seasonal_enabled!==!1,this._adaptivePrediction=u.sensor_prediction_enabled!==!1,this._adaptiveInitial=this._adaptiveSnapshot(),t.trigger_config){let _=t.trigger_config;this._triggerEntityId=_.entity_id||_.entity_ids&&_.entity_ids[0]||"",this._triggerEntityIds=_.entity_ids||(_.entity_id?[_.entity_id]:[]),this._triggerEntityLogic=_.entity_logic||"any",this._triggerAttribute=_.attribute||"",this._triggerType=_.type||"threshold",this._triggerAbove=_.trigger_above?.toString()||"",this._triggerBelow=_.trigger_below?.toString()||"",this._triggerEquals=_.trigger_equals?.toString()||"",this._triggerNotEquals=_.trigger_not_equals?.toString()||"",this._triggerForMinutes=_.trigger_for_minutes?.toString()||"0",this._triggerCombinator=_.trigger_combinator==="all"?"all":"any",this._triggerTargetValue=_.trigger_target_value?.toString()||"",this._triggerDeltaMode=_.trigger_delta_mode||!1,this._triggerBaselineValue=_.trigger_baseline_value?.toString()||"",this._liveBaselineValue=t.trigger_baseline_value??null,this._autoCompleteOnRecovery=_.auto_complete_on_recovery||!1,this._triggerFromState=_.trigger_from_state||"",this._triggerToState=_.trigger_to_state||"",this._triggerTargetChanges=_.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=_.trigger_runtime_hours?.toString()||"",this._triggerRuntimeMaxSession=_.trigger_runtime_max_session_seconds?.toString()||"",this._triggerOnStates=(_.trigger_on_states||[]).join(", "),_.type==="compound"?(this._compoundLogic=_.compound_logic==="OR"?"OR":"AND",this._compoundConditions=(_.conditions||[]).map(da)):(this._compoundLogic="AND",this._compoundConditions=[])}else this._resetTriggerFields();this._triggerEntityId&&this._fetchEntityAttributes(this._triggerEntityId),await Promise.all([this._loadUsers(),this._loadTags(),this._loadParts(),this._loadForeignPools()]),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._intervalUnit="days",this._dueDate="",this._warningDays=String(this.defaultWarningDays),this._earliestCompletionDays="",this._intervalAnchor="completion",this._weekdays=[],this._nth="1",this._nthWeekday="5",this._domDay="1",this._domLastDay=!1,this._domBusiness=!1,this._calOffset="0",this._seasonMonths=[],this._endsMode="never",this._endsCount="",this._endsUntil="",this._notes="",this._documentationUrl="",this._customIcon="",this._notifyIcon="",this._priority="normal",this._labels="",this._mirrorTodoEntities=[],this._enabled=!0,this._lastPerformed="",this._nfcTagId="",this._requireTagScan=!1,this._allowSkip=!0,this._readingUnit="",this._readings=[],this._consumesParts={},this._responsibleUserId=null,this._assigneePool=[],this._rotationStrategy="",this._checklistText="",this._phaseDefs=[],this._phaseSeq=[],this._requiredCompletion=[],this._scheduleTime="",this._scheduleTimeOn=!1,this._environmentalEntity="",this._environmentalAttribute="",this._environmentalInitial="",this._environmentalAttributeInitial="",this._adaptiveEnabled=!1,this._adaptiveAlpha=ae.alpha,this._adaptiveMin=ae.min,this._adaptiveMax=ae.max,this._adaptiveSeasonal=!0,this._adaptivePrediction=!0,this._adaptiveInitial=this._adaptiveSnapshot(),this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="",this._actionTesting=!1,this._actionTestResult="",this._qcNotes="",this._qcCost="",this._qcDuration="",this._qcFeedback="",this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerEquals="",this._triggerNotEquals="",this._triggerForMinutes="0",this._triggerCombinator="any",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerBaselineValue="",this._liveBaselineValue=null,this._autoCompleteOnRecovery=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours="",this._triggerRuntimeMaxSession="",this._triggerOnStates="",this._compoundLogic="AND",this._compoundConditions=[]}async _loadUsers(){this._userService||(this._userService=new Le(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}_toggleAssignee(e){this._assigneePool=this._assigneePool.includes(e)?this._assigneePool.filter(t=>t!==e):[...this._assigneePool,e]}async _testAction(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e)){this._actionTestResult="error",this._actionTestError="Invalid service format (expected 'domain.service')",setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3);return}let[t,i]=e.split(".");if(!this.hass?.services?.[t]?.[i]){this._actionTestResult="error",this._actionTestError=`Service "${e}" is not registered in Home Assistant. Check spelling and that the integration providing it is loaded.`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}let a=this._actionTargetEntity.trim();if(a){let l=a.split(".")[0];if(l!==t&&!new Set(["homeassistant","scene","notify","persistent_notification"]).has(t)){this._actionTestResult="error",this._actionTestError=`Service "${e}" only works on ${t}.* entities; entity "${a}" is in ${l}.* \u2014 pick a service that matches the entity domain (e.g. ${l}.${i})`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}if(!this.hass.states?.[a]){this._actionTestResult="error",this._actionTestError=`Target entity "${a}" not found in Home Assistant \u2014 the entity may have been renamed or its integration removed.`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}}this._actionTestResult="ok",setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3)}_buildActionData(){if(this._actionDataJsonFallback.trim())try{let e=JSON.parse(this._actionDataJsonFallback);if(e&&typeof e=="object"&&!Array.isArray(e))return e}catch{}return{...this._actionData}}_serviceSchema(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e))return null;let[t,i]=e.split("."),a=this.hass?.services?.[t]?.[i]?.fields;return!a||Object.keys(a).length===0?null:Object.entries(a).map(([l,h])=>({name:l,required:!!h.required,selector:h.selector||{text:{}}}))}_renderCompletionActionsSection(e){if(!this.completionActionsEnabled)return c;let t=this._serviceSchema();return o`
      <details class="ca-section">
        ...
        <ha-service-picker
          ...
          @value-changed=${i=>{this._actionService=i.detail.value||"";let a=this._serviceSchema();if(a){let l=new Set(a.map(h=>h.name));this._actionData=Object.fromEntries(Object.entries(this._actionData).filter(([h])=>l.has(h)))}}}
        ></ha-service-picker>
        <ha-form
          .hass=${this.hass}
    ...
    `}async _loadParts(){if(this.parts=[],!!this._entryId)try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this.parts=e.parts||[],this._partsLoadFailed=!1}catch{this.parts=[],this._partsLoadFailed=!0}}async _loadForeignPools(){if(this._foreignOwners=[],!!this._entryId)try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"});this._foreignOwners=(e.objects||[]).filter(t=>t.entry_id!==this._entryId&&(t.parts||[]).length>0).map(t=>({entry_id:t.entry_id,name:t.object?.name||t.entry_id,parts:t.parts||[]})).sort((t,i)=>t.name.localeCompare(i.name))}catch{this._foreignOwners=[]}}async _loadTags(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tags/list"});this._availableTags=e.tags||[]}catch{this._availableTags=[]}}_fetchConditionAttributes(e){!e||!this.hass||this._conditionAttrOptions[e]||this._conditionAttrPending.has(e)||(this._conditionAttrPending.add(e),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/entity/attributes",entity_id:e}).then(t=>{let i=t;this._conditionAttrOptions={...this._conditionAttrOptions,[e]:{suggested:i.suggested_attributes||[],available:i.available_attributes||[]}}}).catch(()=>{this._conditionAttrOptions={...this._conditionAttrOptions,[e]:{suggested:[],available:[]}}}))}async _fetchEntityAttributes(e){if(!e||!this.hass){this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="";return}try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/entity/attributes",entity_id:e});this._entityDomain=t.domain||"",this._suggestedAttributes=t.suggested_attributes||[],this._availableAttributes=t.available_attributes||[]}catch{this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain=""}}get _hasForeignPick(){return Object.values(this._consumesParts).some(e=>!!e.entry_id)}_renderConsumesRow(e,t){let i=Z({part_id:e.id,entry_id:t}),a=this._consumesParts[i],l=t?{part_id:e.id,quantity:1,entry_id:t}:{part_id:e.id,quantity:1};return o`
      <div class="consumes-row">
        ...
        ${a!==void 0?o`<input
              ...
              @input=${h=>{let u=parseFloat(h.target.value);this._consumesParts={...this._consumesParts,[i]:{...l,quantity:Number.isFinite(u)&&u>=.01?u:1}}}}
            />`:c}
      </div>
    `}_toggleRequired(e,t){let i=new Set(this._requiredCompletion);t?i.add(e):i.delete(e),this._requiredCompletion=[...i]}_patchReading(e,t){this._readings=this._readings.map(i=>i.id===e?{...i,...t}:i)}_renderReadingsEditor(e){let t=Br(this._readings);return o`
      <div class="readings-editor">
        ...
        ${this._readings.length<20?o`
          <ha-button appearance="plain" class="reading-add"
            @click=${()=>this._readings=[...this._readings,{id:Dr(),name:"",unit:this._readings.length?this._readings[this._readings.length-1].unit:this._readingUnit}]}>
    ...
    `}_phaseSlug(e){let t=e.toLowerCase().replace(/[^a-z0-9_-]+/g,"-").replace(/^-+|-+$/g,"").slice(0,24)||"phase",i=t,a=2;for(;this._phaseDefs.some(l=>l.id===i);)i=`${t}-${a++}`;return i}_addPhaseDef(){let e=this._phaseSlug(`phase-${this._phaseDefs.length+1}`);this._phaseDefs=[...this._phaseDefs,{id:e,name:"",checklistText:"",partId:"",partQty:"",reqOverride:!1,reqFields:[],extraParts:[],carry:{}}]}_removePhaseDef(e){this._phaseDefs=this._phaseDefs.filter(t=>t.id!==e),this._phaseSeq=this._phaseSeq.filter(t=>t!==e)}_patchPhaseDef(e,t){this._phaseDefs=this._phaseDefs.map(i=>i.id===e?{...i,...t}:i)}_renderPhasesEditor(e){let t=i=>this._phaseDefs.find(a=>a.id===i)?.name||i;return o`
              ...
              ${bi.map(a=>o`
                <label class="req-option">
                  <input
                    ...
                    @change=${l=>{let h=l.target.checked,u=new Set(i.reqFields);h?u.add(a):u.delete(a),this._patchPhaseDef(i.id,{reqFields:[...u]})}}
                  />
                  <span>${n(dt[a],e)}</span>
                </label>
              `)}
        ...
        <div class="phase-seq">
          ${this._phaseSeq.map((i,a)=>o`
            <span class="phase-chip">
              ${a+1}. ${t(i)}
              <button class="phase-chip-x" @click=${()=>{this._phaseSeq=this._phaseSeq.filter((l,h)=>h!==a)}}>✕</button>
            </span>
          `)}
          <select
            class="phase-seq-add"
      ...
      `:c}
    `}async _save(){if(!this._loading&&this._name.trim()){if(this._adaptiveSnapshot()!==this._adaptiveInitial){let e=parseInt(this._adaptiveMin,10),t=parseInt(this._adaptiveMax,10);if(!isNaN(e)&&!isNaN(t)&&e>t){this._error=`${n("adaptive_min_interval",this._lang)} > ${n("adaptive_max_interval",this._lang)}`;return}}if(this._triggerType==="threshold"&&this._thresholdLimitsOverlap()){this._error=n("trigger_hint_overlap",this._lang);return}if(this._scheduleType==="calendar"&&!this._calendarEntity.trim().startsWith("calendar.")){this._error=n("calendar_entity_required",this._lang);return}this._loading=!0,this._error="";try{let e={type:this._taskId?"maintenance_supporter/task/update":"maintenance_supporter/task/create",entry_id:this._entryId,name:this._name,task_type:this._type,schedule_type:this._scheduleType,warning_days:Number.isNaN(parseInt(this._warningDays,10))?this.defaultWarningDays:Math.max(0,parseInt(this._warningDays,10))},t=this._earliestCompletionDays.trim();e.earliest_completion_days=t===""?null:Math.max(0,parseInt(t,10)||0),this._taskId&&(e.task_id=this._taskId),this._scheduleType==="one_time"?(e.due_date=this._dueDate||null,e.interval_days=null):Mt.includes(this._scheduleType)?(e.schedule={...this._buildSchedule(),...this._recurrenceExtras()},e.interval_days=null,this._taskId&&(e.due_date=null)):(this._taskId&&(e.due_date=null),this._scheduleType!=="manual"&&this._intervalDays?(e.interval_days=parseInt(this._intervalDays,10),e.interval_unit=this._intervalUnit,e.interval_anchor=this._intervalAnchor,this._scheduleType==="time_based"&&(e.schedule={kind:"interval",...this._recurrenceExtras()})):this._taskId&&(e.interval_days=null,e.interval_anchor="completion")),e.notes=this._notes||null,e.documentation_url=this._documentationUrl||null,e.custom_icon=this._customIcon||null,e.notify_icon=this._notifyIcon.trim()||null,e.priority=this._priority,e.labels=this._labels.split(",").map(u=>u.trim()).filter(Boolean),e.mirror_todo_entities=this._mirrorTodoEntities.filter(Boolean),e.enabled=this._enabled,(!this._taskId||this._lastPerformed!==this._loadedLastPerformed)&&(e.last_performed=this._lastPerformed||null),e.nfc_tag_id=this._nfcTagId||null,e.require_tag_scan=this._requireTagScan,e.allow_skip=this._allowSkip,e.notify_enabled=this._notifyEnabled,e.reading_unit=this._readingUnit.trim()||null,e.readings=Vr(this._readings);{let u={};for(let m of this._phaseDefs){if(!m.name.trim())continue;let v={...m.carry,name:m.name.trim()},w=m.checklistText.split(`
`).map(g=>g.trim()).filter(Boolean);w.length&&(v.checklist=w);let y=[];if(m.partId){let g=parseFloat(m.partQty);y.push({part_id:m.partId,quantity:Number.isFinite(g)&&g>0?g:1})}for(let g of m.extraParts)y.push(g.entry_id?{part_id:g.part_id,quantity:g.quantity,entry_id:g.entry_id}:{part_id:g.part_id,quantity:g.quantity});y.length&&(v.consumes_parts=y),m.reqOverride&&(v.required_completion_fields=[...m.reqFields]),u[m.id]=v}let _=this._phaseSeq.filter(m=>m in u);e.phases=Object.keys(u).length&&_.length?u:null,e.phase_sequence=e.phases?_:null}if((this.parts.length||this._foreignOwners.length)&&(e.consumes_parts=Object.values(this._consumesParts).map(u=>u.entry_id?{part_id:u.part_id,quantity:u.quantity,entry_id:u.entry_id}:{part_id:u.part_id,quantity:u.quantity})),(!this._taskId||this._responsibleUserId!==this._loadedResponsibleUserId)&&(e.responsible_user_id=this._responsibleUserId),e.assignee_pool=this._assigneePool,e.required_completion_fields=this._requiredCompletion,e.rotation_strategy=this._assigneePool.length>=2&&this._rotationStrategy?this._rotationStrategy:null,this._scheduleType==="sensor_based"&&this._triggerType==="compound"){let u=this._compoundConditions.map(ca).filter(_=>_!==null);if(u.length>0){let _={type:"compound",compound_logic:this._compoundLogic,conditions:u};this._autoCompleteOnRecovery&&(_.auto_complete_on_recovery=!0),this._triggerCombinator==="all"&&(_.trigger_combinator="all"),e.trigger_config=_}else this._taskId&&(e.trigger_config=null)}else if(this._scheduleType==="sensor_based"&&this._triggerEntityId){let u=this._triggerEntityIds.length>0?this._triggerEntityIds:[this._triggerEntityId],_={entity_id:u[0],entity_ids:u,type:this._triggerType};if(this._triggerAttribute&&(_.attribute=this._triggerAttribute),this._autoCompleteOnRecovery&&(_.auto_complete_on_recovery=!0),this._triggerCombinator==="all"&&(_.trigger_combinator="all"),u.length>1&&(_.entity_logic=this._triggerEntityLogic),this._triggerType==="threshold"){if(this._triggerAbove){let m=parseFloat(this._triggerAbove);isNaN(m)||(_.trigger_above=m)}if(this._triggerBelow){let m=parseFloat(this._triggerBelow);isNaN(m)||(_.trigger_below=m)}if(this._triggerEquals){let m=parseFloat(this._triggerEquals);isNaN(m)||(_.trigger_equals=m)}if(this._triggerNotEquals){let m=parseFloat(this._triggerNotEquals);isNaN(m)||(_.trigger_not_equals=m)}if(this._triggerForMinutes){let m=parseInt(this._triggerForMinutes,10);isNaN(m)||(_.trigger_for_minutes=m)}}else if(this._triggerType==="counter"){if(this._triggerTargetValue){let m=parseFloat(this._triggerTargetValue);isNaN(m)||(_.trigger_target_value=m)}if(_.trigger_delta_mode=this._triggerDeltaMode,this._triggerDeltaMode&&this._triggerBaselineValue){let m=parseFloat(this._triggerBaselineValue);!isNaN(m)&&m>=0&&(_.trigger_baseline_value=m)}}else if(this._triggerType==="state_change"){if(this._triggerFromState&&(_.trigger_from_state=this._triggerFromState),this._triggerToState&&(_.trigger_to_state=this._triggerToState),this._triggerTargetChanges){let m=parseInt(this._triggerTargetChanges,10);isNaN(m)||(_.trigger_target_changes=m)}if(this._triggerForMinutes){let m=parseInt(this._triggerForMinutes,10);isNaN(m)||(_.trigger_for_minutes=m)}}else if(this._triggerType==="runtime"){if(this._triggerRuntimeHours){let v=parseFloat(this._triggerRuntimeHours);isNaN(v)||(_.trigger_runtime_hours=v)}if(this._triggerRuntimeMaxSession){let v=parseInt(this._triggerRuntimeMaxSession,10);!isNaN(v)&&v>0&&(_.trigger_runtime_max_session_seconds=v)}let m=this._triggerOnStates.split(",").map(v=>v.trim()).filter(Boolean);m.length>0&&(_.trigger_on_states=m)}e.trigger_config=_}else this._taskId&&(e.trigger_config=null);if(this.scheduleTimeEnabled&&cs.includes(this._scheduleType)){let u=this._scheduleTimeOn?this._scheduleTime.trim():"";e.schedule_time=/^([01]\d|2[0-3]):[0-5]\d$/.test(u)?u:null}if(this.checklistsEnabled){let u=this._checklistText.split(`
`).map(_=>_.trim()).filter(Boolean).slice(0,100);e.checklist=u.length?u:null}if(this.completionActionsEnabled){let u=this._actionService.trim();if(u&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(u)){let w={service:u},y=this._actionTargetEntity.trim();y&&(w.target={entity_id:y});let g=this._buildActionData();Object.keys(g).length>0&&(w.data=g),e.on_complete_action=w}else e.on_complete_action=null;let _={};this._qcNotes.trim()&&(_.notes=this._qcNotes.trim());let m=parseFloat(this._qcCost);!isNaN(m)&&m>=0&&(_.cost=m);let v=Oe(this._qcDuration);v!==null&&(_.duration=v),this._qcFeedback&&(_.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(_).length?_:null}let i=await this.hass.connection.sendMessagePromise(e),a=this._taskId||i?.task_id,l=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;this._warning="";let h=u=>{this._warning=n("subsave_warning",this._lang).replace("{detail}",u)};if(a&&this._scheduleType==="sensor_based"&&l&&await Ii(this,{type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:a,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null},{onError:h})!==void 0&&(this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute),a&&this._adaptiveSnapshot()!==this._adaptiveInitial){let u=parseFloat(this._adaptiveAlpha),_=parseInt(this._adaptiveMin,10),m=parseInt(this._adaptiveMax,10);await Ii(this,{type:"maintenance_supporter/task/set_adaptive",entry_id:this._entryId,task_id:a,enabled:this._adaptiveEnabled,...u>=.1&&u<=.9?{ewa_alpha:u}:{},...!isNaN(_)&&_>=1?{min_interval_days:_}:{},...!isNaN(m)&&m>=1?{max_interval_days:m}:{},seasonal_enabled:this._adaptiveSeasonal,sensor_prediction_enabled:this._adaptivePrediction},{onError:h})!==void 0&&(this._adaptiveInitial=this._adaptiveSnapshot())}this._warning?a&&(this._taskId=a):this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=R(e,this._lang,n("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1,this._warning="",this._pickerProbeTimer!==void 0&&(clearTimeout(this._pickerProbeTimer),this._pickerProbeTimer=void 0),this._pickerProbeStrikes=0}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return c;let e=this._lang,t=this._triggerType==="compound";return o`
      ...
      <div class="select-row">
        ...
        >
          ${aa.map(i=>o`<option value=${i} ?selected=${i===this._triggerType}>${n(i,e)}</option>`)}
        </select>
      </div>
      ${t?this._renderCompoundEditor():o`
        ${this._entityPickerFallback?o`
          <ms-textfield
            ...
            @input=${i=>{let l=i.target.value.split(",").map(h=>h.trim()).filter(Boolean);this._triggerEntityId=l[0]||"",this._triggerEntityIds=l,l[0]&&this._fetchEntityAttributes(l[0])}}
          ></ms-textfield>
        `:o`
        <ha-form
          class="entity-picker-form"
          .hass=${this.hass}
          .schema=${[{name:"trigger_entities",selector:{entity:{multiple:!0,domain:Ci}}}]}
      ...
      ${this._intervalDays?o`
          ...
          `:c}
    `}_patchCondition(e,t){this._compoundConditions=this._compoundConditions.map((i,a)=>a===e?{...i,...t}:i)}_addCondition(){this._compoundConditions=[...this._compoundConditions,oa()]}_removeCondition(e){this._compoundConditions=this._compoundConditions.filter((t,i)=>i!==e)}_renderCompoundEditor(){let e=this._lang;return o`
      <div class="select-row">
    ...
    `}_renderCondition(e,t){let i=this._lang,a=t+1;return o`
      <div class="compound-condition">
        ...
        <ha-form
          class="entity-picker-form"
          .hass=${this.hass}
          .schema=${[{name:"condition_entities",selector:{entity:{multiple:!0,domain:Ci}}}]}
          .data=${{condition_entities:e.entityIds.split(",").map(l=>l.trim()).filter(Boolean)}}
          .computeLabel=${()=>n("entity_id",i)}
          @value-changed=${l=>{let h=(l.detail.value.condition_entities||[]).filter(Boolean);this._patchCondition(t,{entityIds:h.join(", ")})}}
        ></ha-form>`}
        ...
        <div class="select-row">
          ...
          >
            ${ps.map(l=>o`<option value=${l} ?selected=${l===e.type}>${n(l,i)}</option>`)}
          </select>
        </div>
    ...
    `}_buildSchedule(){let e=i=>{let a=parseInt(this._calOffset,10)||0;return a&&(i.offset=Math.max(-15,Math.min(a,15))),i};if(this._scheduleType==="weekdays")return e({kind:"weekdays",weekdays:[...this._weekdays].sort((i,a)=>i-a)});if(this._scheduleType==="nth_weekday")return e({kind:"nth_weekday",nth:parseInt(this._nth,10),weekday:parseInt(this._nthWeekday,10)});if(this._scheduleType==="calendar")return e({kind:"calendar",entity_id:this._calendarEntity.trim()});let t={kind:"day_of_month",day:this._domLastDay?-1:parseInt(this._domDay,10)||1};return this._domBusiness&&(t.business=!0),e(t)}_recurrenceExtras(){let e={};if(this._seasonMonths.length&&(e.season_months=[...this._seasonMonths].sort((t,i)=>t-i)),this._endsMode==="count"){let t=parseInt(this._endsCount,10);t>=1&&(e.ends={count:t})}else this._endsMode==="until"&&this._endsUntil&&(e.ends={until:this._endsUntil});return e}_toggleSeasonMonth(e){this._seasonMonths=this._seasonMonths.includes(e)?this._seasonMonths.filter(t=>t!==e):[...this._seasonMonths,e]}_renderRecurrenceExtras(){let e=this._lang;if(!(this._scheduleType==="time_based"||Mt.includes(this._scheduleType)))return c;let i=ha(e);return o`
      ...
      ${this._endsMode==="until"?o`
        ...
        ></ms-date-field>`:c}
    `}_renderCalendarFields(){let e=this._lang,t=pa(e);if(this._scheduleType==="weekdays")return o`
      ...
      ></ms-textfield>`}_thresholdLimitsOverlap(){let e=parseFloat(this._triggerAbove),t=parseFloat(this._triggerBelow);return!isNaN(e)&&!isNaN(t)&&t>e}_renderTriggerLiveHint(){if(this._triggerType==="compound")return c;let e=this._triggerType==="threshold"&&this._thresholdLimitsOverlap()?o`<div class="trigger-live-hint warn">${n("trigger_hint_overlap",this._lang)}</div>`:c,t=this._triggerEntityId||this._triggerEntityIds[0];if(!t||!this.hass?.states)return e;let i=this.hass.states[t];if(!i)return e;let a=this._lang,l=i.attributes?.unit_of_measurement,h=typeof l=="string"&&l?` ${l}`:"",u=this._triggerAttribute?i.attributes?.[this._triggerAttribute]:i.state,_=typeof u=="number"?u:parseFloat(String(u)),m=u!=="unknown"&&u!=="unavailable"&&u!=null&&!isNaN(_),v=y=>j(y,a,{maximumFractionDigits:1}),w=[];if(this._triggerType==="threshold"){let y=parseFloat(this._triggerAbove),g=parseFloat(this._triggerBelow);if(isNaN(y)&&isNaN(g))return c;m&&w.push(n("trigger_hint_now",a).replace("{value}",v(_)+h)),isNaN(y)||w.push(n("trigger_hint_above",a).replace("{target}",v(y)+h)),isNaN(g)||w.push(n("trigger_hint_below",a).replace("{target}",v(g)+h))}else if(this._triggerType==="counter"){let y=parseFloat(this._triggerTargetValue);if(isNaN(y))return c;this._triggerDeltaMode?this._taskId?w.push(n("trigger_hint_counter_delta_edit",a).replace("{target}",v(y)+h)):m?w.push(n("trigger_hint_counter_delta",a).replace("{value}",v(_)+h).replace("{due}",v(_+y)+h).replace("{target}",v(y)+h)):w.push(n("trigger_hint_counter_delta_edit",a).replace("{target}",v(y)+h)):(m&&w.push(n("trigger_hint_now",a).replace("{value}",v(_)+h)),w.push(n("trigger_hint_counter_abs",a).replace("{target}",v(y)+h)))}else if(this._triggerType==="runtime"){let y=parseFloat(this._triggerRuntimeHours);if(isNaN(y))return c;w.push(n("trigger_hint_runtime",a).replace("{hours}",v(y))),w.push(n("trigger_hint_state_now",a).replace("{value}",String(i.state)))}else if(this._triggerType==="state_change"){let y=parseInt(this._triggerTargetChanges,10)||1,g=this._triggerToState.trim();w.push((g?n("trigger_hint_state_change_to",a).replace("{state}",g):n("trigger_hint_state_change",a)).replace("{count}",String(y))),w.push(n("trigger_hint_state_now",a).replace("{value}",String(i.state)))}return w.length?o`<div class="trigger-live-hint">${w.join(" ")}</div>${e}`:e}_renderTriggerTypeFields(){let e=this._lang;return this._triggerType==="threshold"?o`
        <ms-textfield
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${ra.map(i=>o`<option value=${i} ?selected=${i===this._type}>${n(i,e)}</option>`)}
            </select>
          </div>
          ...
          <div class="select-row">
            ...
            >
              ${sa.map(i=>o`<option value=${i} ?selected=${i===this._priority}>${n("priority_"+i,e)}</option>`)}
            </select>
          </div>
          <div class="field">
          ...
          <div class="select-row">
            ...
            >
              ${na.map(i=>o`<option value=${i} ?selected=${i===this._scheduleType}>${n(i,e)}</option>`)}
            </select>
          </div>
          ...
          ${this._scheduleType==="one_time"?o`
                ...
                ></ms-date-field>
              `:c}
          ${this.scheduleTimeEnabled&&cs.includes(this._scheduleType)?o`
          ...
          ${this._scheduleType==="sensor_based"?o`
            ...
            <ha-form
              class="entity-picker-form"
              .hass=${this.hass}
              .schema=${[{name:"environmental_entity",selector:{entity:{domain:ns,device_class:as}}}]}
          ...
          <div class="field-help notify-icon-help">
            <ha-icon icon=${this._notifyIcon.trim()||Ri(this._type)}></ha-icon>
            ${n("notify_icon_hint",e).replace("{default}",Ri(this._type))}
          </div>
          ...
          ${this._inputLabel?o`
            ...
            <label class="field">
              ...
              ${this._options?o`<select class="field-input field-select"
                    ...
                    ${this._options.map(t=>o`<option value=${t.value} ?selected=${t.value===this._inputValue}>${t.label}</option>`)}
  ...
  `],d([b({attribute:!1})],V.prototype,"hass",2),d([p()],V.prototype,"_open",2),d([p()],V.prototype,"_title",2),d([p()],V.prototype,"_message",2),d([p()],V.prototype,"_confirmText",2),d([p()],V.prototype,"_danger",2),d([p()],V.prototype,"_inputLabel",2),d([p()],V.prototype,"_inputType",2),d([p()],V.prototype,"_options",2),d([p()],V.prototype,"_inputValue",2);customElements.get("maintenance-confirm-dialog")||customElements.define("maintenance-confirm-dialog",V)});function ua(){return document.querySelector("home-assistant")?.shadowRoot??document.body}function $e(s,r){let e=ua(),t=e.querySelector(`${_s}[${ms}]`);return t||(t=document.createElement(_s),t.setAttribute(ms,""),e.appendChild(t)),t.hass=s,t.confirm(r)}var _s,ms,zt=x(()=>{"use strict";us();_s="maintenance-confirm-dialog",ms="data-ms-lovelace-confirm"});var _e,Li=x(()=>{"use strict";C();q();ui();_e=class extends E{constructor(){super(...arguments);this.docId="";this._url="";this._failed=!1;this._signedFor=""}updated(){this.hass&&this.docId&&this._signedFor!==this.docId&&(this._signedFor=this.docId,this._url="",this._failed=!1,this._sign())}async _sign(){try{this._url=await hi(this.hass,this.docId)}catch{this._failed=!0}}render(){return this._failed||!this.docId?c:this._url?o`
      <a href=${this._url} target="_blank" rel="noopener" class="wrap">
        <img src=${this._url} alt="" loading="lazy"
          @error=${()=>this._failed=!0} />
      </a>`:o`<div class="ph"></div>`}};_e.styles=k`
      ...
      <div class="dialog" role="dialog" aria-modal="true">
        ...
        <div class="row">
          ...
          <label>
            ...
            <input type="number" min="0" step="1" inputmode="numeric"
              ...
              @input=${a=>{this._set("duration",Oe(a.target.value))}} />
          </label>
        </div>
        ...
        ${this._partOptions&&this._partOptions.length>0?o`
          <div class="parts-block">
            ...
            ${this._partOptions.map(a=>{let l=`${a.entry_id}:${a.part_id}`,h=this._partQty[l]||0;return o`
                <label class="part-row-edit">
                  <input type="checkbox" .checked=${h>0}
                    @change=${u=>{let _=u.target.checked;this._partQty={...this._partQty,[l]:_?1:0}}} />
                  ...
                  ${h>0?o`
                    <input class="part-qty" type="number" min="0.01" max="999" step="0.01"
                      .value=${String(h)}
                      @input=${u=>{let _=parseFloat(u.target.value);!isNaN(_)&&_>0&&(this._partQty={...this._partQty,[l]:_})}} />
                  `:c}
                </label>
              `})}
          </div>
...
<div class="qr-row">
  <div class="qr-col">
    <img src="${fs(this._viewResult.svg_data_uri)}" alt="QR Info" />
    <div class="qr-label">${m}</div>
  </div>
  ${_?`<div class="qr-col">
    <img src="${fs(this._completeResult.svg_data_uri)}" alt="QR Complete" />
    <div class="qr-label">${v}</div>
  </div>`:""}
</div>
<div class="url">${Fe(this._viewResult.url)}</div>
<script>setTimeout(()=>window.print(),300)<\/script>
</body></html>`),a.document.close()}_downloadSvg(e,t){let i=decodeURIComponent(e.svg_data_uri.replace("data:image/svg+xml,","")),a=this._taskName?`${this._objectName}-${this._taskName}`:this._objectName;jr(i,`qr-${_a(a)}-${t}.svg`,"image/svg+xml")}_close(){this._open=!1,this._viewResult=null,this._completeResult=null,this._error="",this._loading=!1}render(){if(!this._open)return o``;let e=this.lang||this.hass?.language||"en",t=this._taskName?`${n("qr_code",e)}: ${this._objectName} \u2014 ${this._taskName}`:`${n("qr_code",e)}: ${this._objectName}`,i=!!this._viewResult;return o`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
  ...
  `,d([b({attribute:!1})],Y.prototype,"hass",2),d([b()],Y.prototype,"lang",2),d([p()],Y.prototype,"_open",2),d([p()],Y.prototype,"_loading",2),d([p()],Y.prototype,"_error",2),d([p()],Y.prototype,"_viewResult",2),d([p()],Y.prototype,"_completeResult",2),d([p()],Y.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",Y)});var bs=x(()=>{"use strict"});function ys(s){let r=s.getFullYear(),e=String(s.getMonth()+1).padStart(2,"0"),t=String(s.getDate()).padStart(2,"0");return`${r}-${e}-${t}`}var xs=x(()=>{"use strict";bs()});function ws(s,r,e,t){let i=e,a=u=>typeof u=="string"?u:null,l=u=>typeof u=="number"?u:null,h=a(i.timestamp)??"";return{entry_id:s,task_id:r,original_timestamp:h,type:a(i.type)||"completed",timestamp:h,notes:a(i.notes),cost:l(i.cost),duration:l(i.duration),completed_by:a(i.completed_by),used_parts:Array.isArray(i.used_parts)?i.used_parts:null,photo_doc_ids:Nt(i),reading_value:l(i.reading_value),reading_values:he(i),readings:t?.readings??[],task_type:t?.type??null,reading_unit:t?.reading_unit??null}}var $s=x(()=>{"use strict";jt();xe()});function ma(s){return s&&ks(s.ref_no)?String(s.ref_no):null}function Es(s,r){let e=ma(s);return e&&r&&ks(r.ref_no)?`${e}.${r.ref_no}`:null}function Ss(s,r){return s?o`<span class="ref-chip" title=${r??""}>#${s}</span>`:c}var ks,Ni=x(()=>{"use strict";C();ks=s=>typeof s=="number"&&Number.isInteger(s)&&s>0});var Ts=x(()=>{"use strict"});function As(s){let r=e=>{let t=e.timestamp??"",i=Date.parse(t.length===10?`${t}T00:00:00`:t);return Number.isNaN(i)?-1/0:i};return(s??[]).map((e,t)=>({entry:e,index:t,at:r(e)})).sort((e,t)=>t.at-e.at||t.index-e.index).map(e=>e.entry)}function fa(s,r){return s.taskRef&&r.ref_no?`${s.taskRef}-${r.ref_no}`:null}function va(s,r){let e=Nt(r);return e.length>0?o`<div class="history-photos">
        ${e.map(t=>o`<maintenance-history-photo .hass=${s} .docId=${t}></maintenance-history-photo>`)}
      </div>`:c}function ba(s,r){let e=r.lang,t=l=>j(l,e,{maximumFractionDigits:3}),i=l=>l==null?"":` (${l>=0?"+":""}${t(l)})`,a=he(s);return a.length>0?o`<div class="history-readings">
    ...
    </span></div>`:c}function Cs(s,r,e={}){let t=r.lang,{compact:i=!1,showRef:a=!0,showBadges:l=!0,showEdit:h=!0}=e,u=h&&ga.includes(s.type);return o`
    <div class="history-entry${i?" compact":""}">
      ${i?c:o`<div class="history-icon ${s.type}">
            <ha-icon .icon=${tt[s.type]||"mdi:circle"}></ha-icon>
          </div>`}
      <div class="history-content">
        <div class="history-row">
          <strong>${n(s.type,t)}</strong>
          ${a?Ss(fa(r,s),n("ref_number",t)):c}
        ...
        <div class="history-date">${Rr(s.timestamp,t)}</div>
        ${s.notes?o`<div>${s.notes}</div>`:c}
        ${va(r.hass,s)}
        ${ba(s,r)}
        ${s.cost!=null||s.duration!=null||s.trigger_value!=null?o`<div class="history-details">
              ${s.cost!=null?o`<span>${n("cost",t)}: ${Re(s.cost,r.currencySymbol,t)}</span>`:c}
              ${s.duration!=null?o`<span>${n("duration",t)}: ${kt(s.duration,t)}</span>`:c}
              ${s.trigger_value!=null?o`<span>${n("trigger_val",t)}: ${s.trigger_value}</span>`:c}
            </div>`:c}
      </div>
    </div>
  `}var ga,Is=x(()=>{"use strict";C();I();Li();jt();Ni();xe();Ts();ga=["completed","reset","skipped"]});function _t(s){return s.archived?"archived":s.is_done?"done":s.status||"ok"}function ji(s,r){return n(s==="done"?"completed":s,r)}function Ft(s){let r=_t(s);return r==="done"?ya:et[r]||"var(--disabled-color, #9e9e9e)"}function xa(s){return tt[s==="done"?"completed":s]||"mdi:circle-medium"}function Rs(s,r,e="pill"){let t=_t(s),i=ji(t,r);return e==="chip"?o`<span class="status-chip ${t}">${i}</span>`:o`<span class="status-badge ${t}" role="img" title="${i}" aria-label="${i}"><ha-icon icon="${xa(t)}"></ha-icon><span class="status-label">${i}</span></span>`}var ya,Hi=x(()=>{"use strict";C();I();ai();ya="var(--maint-done-color, #78909c)"});var Ut,Ps=x(()=>{"use strict";Ut=class{constructor(){this._handle=null}schedule(r,e=4e3){this.clear(),this._handle=setTimeout(()=>{this._handle=null,r()},e)}clear(){this._handle!==null&&(clearTimeout(this._handle),this._handle=null)}get pending(){return this._handle!==null}}});function Ls(s,r){let e=s.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return c;let a=s.interval_days??0,l=s.suggested_interval??a;return o`
    ...
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_weibull",i)}">
        ...
        ${e>0?Ae`
          ...
          <circle cx="${T(A)}" cy="${T(Ee)}" r="3"
            fill="var(--primary-color, #03a9f4)" />
          <text x="${T(A+4)}" y="${T(Ee-6)}" fill="var(--primary-color, #03a9f4)"
            font-size="9" font-weight="600">R=${Gs}%</text>
        `:c}

        ${t>0&&t!==e?Ae`
          <line x1="${T(Vi)}" y1="${8}" x2="${T(Vi)}" y2="${T(136)}"
    ...
    <div class="weibull-info-row">
      ...
      ${s.weibull_r_squared!=null?o`
        <div class="weibull-info-item">
          ...
          <span class="weibull-info-value">${j(s.weibull_r_squared,r,3)}</span>
        </div>
      `:c}
    </div>
  `}function Ea(s,r,e){let t=s.confidence_interval_low,i=s.confidence_interval_high,a=r.suggested_interval??r.interval_days??0,l=r.interval_days??0,h=Math.max(0,t-5),_=i+5-h,m=(t-h)/_*100,v=(i-t)/_*100,w=(a-h)/_*100,y=l>0?(l-h)/_*100:-1;return o`
    <div class="confidence-range">
      <div class="confidence-range-title">
        ${n("confidence_interval",e)}: ${a} ${n("days",e)} (${t}\u2013${i})
      </div>
      <div class="confidence-bar">
        <div class="confidence-fill" style="left:${T(m)}%;width:${T(v)}%"></div>
        ${y>=0?o`<div class="confidence-marker current" style="left:${T(y)}%"></div>`:c}
      ...
      <div class="prediction-grid">
        ${t?o`
          <div class="prediction-item">
            ...
            ${s.degradation_rate!=null?o`<span class="prediction-rate">${s.degradation_rate>0?"+":""}${j(s.degradation_rate,r,Math.abs(s.degradation_rate)>=10?0:1)} ${s.trigger_entity_info?.unit_of_measurement||""}/${n("day_short",r)}</span>`:c}
          </div>
        `:c}
        ${i?o`
          <div class="prediction-item">
        ...
        ${a&&e.environmental?o`
          <div class="prediction-item">
            ...
            <span class="prediction-value">${j(s.environmental_factor,r,2)}x</span>
            ${s.environmental_entity?o`<span class="prediction-entity entity-link" @click=${h=>Nr(h,s.environmental_entity)}>${s.environmental_entity}</span>`:c}
          </div>
        `:c}
      </div>
    </div>
  `}var Hs=x(()=>{"use strict";C();I()});function Os(s,r,e,t){let i=Math.max(s||1,r);return o`
  ...
  `}var qs=x(()=>{"use strict";C();I()});function Ms(s,r,e){if(!e.seasonal||!s.seasonal_factor||s.seasonal_factor===1)return c;let t=Ds.map(h=>n(h,r)),i=new Date().getMonth(),a=s.seasonal_factors||s.interval_analysis?.seasonal_factors||null,l=a&&a.length===12?a:t.map((h,u)=>{let _=s.seasonal_factor||1,m=Math.sin((u-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,_+m))});return o`
    <div class="seasonal-card-compact">
      <h4>${n("seasonal_awareness",r)}</h4>
      <div class="seasonal-mini-chart">
        ${l.map((h,u)=>{let _=h*40,m=h<.9?"low":h>1.1?"high":"normal";return o`
  ...
  `}function zs(s,r){return Sa(s,r)}function Sa(s,r){let e=s.seasonal_factors??s.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return c;let t=s.interval_analysis?.seasonal_reason,i=new Date().getMonth(),a=300,l=100,h=8,_=l-h-4,m=Math.max(...e,1.5),v=a/12,w=v*.65,y=h+_-1/m*_;return o`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${a} ${l}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_seasonal",r)}">
        ...
        ${e.map((g,$)=>{let z=g/m*_,L=$*v+(v-w)/2,A=h+_-z,O=$===i,Ee=g<1?"var(--success-color, #4caf50)":g>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return Ae`
            <rect x="${T(L)}" y="${T(A)}"
              width="${T(w)}" height="${T(z)}"
              fill="${Ee}" opacity="${O?1:.5}" rx="2" />
          `})}
      </svg>
      <div class="seasonal-labels">
        ${Ds.map((g,$)=>o`<span class="seasonal-label ${$===i?"active-month":""}">${n(g,r)}</span>`)}
      </div>
    </div>
  `}var Ds,Fs=x(()=>{"use strict";C();I();Dt();Ds=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"]});var N,Us=x(()=>{"use strict";C();q();I();ne();xs();Lt();Ct();$s();xe();Ni();Is();Hi();lt();ot();zt();Ps();Ns();Hs();qs();De();Fs();N=class extends E{constructor(){super(...arguments);this._open=!1;this._entryId=null;this._taskId=null;this._task=null;this._objectName="";this._taskRef=null;this._busy=!1;this._error="";this._showSkip=!1;this._showReset=!1;this._showDetails=!1;this._showAdaptive=!1;this._skipReason="";this._resetDate="";this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._toast="";this._toastTimer=new Ut;this._access=we;this._currencySymbol=""}get _lang(){return P(this.hass)}async openFor(e,t){this._entryId=e,this._taskId=t,this._error="",this._showSkip=!1,this._showReset=!1,this._showAdaptive=!1,this._skipReason="",this._resetDate=ys(new Date),this._open=!0,await Promise.all([this._loadTask(),this._loadFeatures()])}async _loadFeatures(){let e=await X(this.hass);this._features={...this._features,...e.features},this._access=e.access,this._currencySymbol=ye(e.budget??void 0),$t(e.budget??void 0)}close(){this._open=!1,this._task=null,this._error="",this._toastTimer.clear(),this._toast=""}_showToast(e,t){this._toast=e,this._toastTimer.schedule(()=>{this._toast=""},t)}async _loadTask(){if(!(!this._entryId||!this._taskId))try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._objectName=e.object?.name||"";let t=(e.tasks||[]).find(i=>i.id===this._taskId);this._task=t??null,this._taskRef=Es(e.object,t)}catch(e){this._error=R(e,this._lang)}}async _runWs(e){this._busy=!0,this._error="";try{return await this.hass.connection.sendMessagePromise(e),this._busy=!1,!0}catch(t){return this._error=R(t,this._lang),this._busy=!1,!1}}_notifyChanged(e){this.dispatchEvent(new CustomEvent("task-action-fired",{detail:{entry_id:this._entryId,task_id:this._taskId,action:e},bubbles:!0,composed:!0}))}_onComplete(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(ie(),oe)).then(async({openCompleteDialog:e})=>{let t=this._task,i=[];try{i=(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects",compact:!0})).objects||[]}catch{}e(Rt({entryId:this._entryId,taskId:this._taskId,taskName:t.name,task:t,objects:i,lang:this._lang,features:this._features,currencySymbol:this._currencySymbol}))&&(this._notifyChanged("complete"),this.close())})}async _onSkipConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/skip",entry_id:this._entryId,task_id:this._taskId,reason:this._skipReason.trim()||null})&&(this._notifyChanged("skip"),this.close())}async _onResetConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/reset",entry_id:this._entryId,task_id:this._taskId,date:this._resetDate||void 0})&&(this._notifyChanged("reset"),this.close())}_onEdit(){!this._entryId||!this._taskId||Promise.resolve().then(()=>(ie(),oe)).then(({openEditTaskDialog:e})=>{e(this._entryId,this._taskId),this.close()})}_onQr(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(ie(),oe)).then(({openQrDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,object_name:this._objectName}),this.close()})}async _onDelete(){if(!this._entryId||!this._taskId||!await $e(this.hass,{title:n("delete",this._lang),message:n("delete_task_confirm",this._lang),confirmText:n("delete",this._lang),danger:!0}))return;await this._runWs({type:"maintenance_supporter/task/delete",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("delete"),this.close())}async _onArchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/archive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("archive"),this.close())}async _onUnarchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/unarchive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("unarchive"),this.close())}_onOpenInPanel(){if(!this._entryId||!this._taskId)return;let e=`/maintenance-supporter?entry_id=${encodeURIComponent(this._entryId)}&task_id=${encodeURIComponent(this._taskId)}`;history.pushState(null,"",e),window.dispatchEvent(new CustomEvent("location-changed")),this.close()}async _applySuggestion(){if(!this._entryId||!this._taskId||!this._task?.suggested_interval)return;await this._runWs({type:"maintenance_supporter/task/apply_suggestion",entry_id:this._entryId,task_id:this._taskId,interval:this._task.suggested_interval})&&(this._showToast(n("suggestion_applied",this._lang)),this._notifyChanged("apply_suggestion"),await this._loadTask())}async _reanalyzeInterval(){if(!(!this._entryId||!this._taskId)){this._busy=!0,this._error="";try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:this._entryId,task_id:this._taskId});this._showToast(e.recommended_interval?`${n("reanalyze_result",this._lang)}: ${rt(e.recommended_interval,"days",this._lang)} (${e.data_points} pts)`:n("reanalyze_insufficient_data",this._lang)),await this._loadTask()}catch(e){this._error=R(e,this._lang)}finally{this._busy=!1}}}_onEditHistoryEntry(e){if(!this._entryId||!this._taskId)return;let t=ws(this._entryId,this._taskId,e,this._task);Promise.resolve().then(()=>(ie(),oe)).then(({openHistoryEditDialog:i})=>i(t))}_renderRecommendation(e){if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return c;let t=this._lang;return o`
      <div class="recommendation-card">
        <h4>${n("suggested_interval",t)}</h4>
        ${Os(e.interval_days,e.suggested_interval,e.interval_confidence||"medium",t)}
    ...
    `}_renderAdaptive(e){let t=this._lang,i=this._features.adaptive&&e.suggested_interval&&e.suggested_interval!==e.interval_days,a=e.degradation_trend!=null&&e.degradation_trend!=="insufficient_data"||e.days_until_threshold!=null||e.environmental_factor!=null&&e.environmental_factor!==1,l=this._features.adaptive&&e.interval_analysis?.weibull_beta!=null&&e.interval_analysis?.weibull_eta!=null,h=this._features.seasonal&&e.seasonal_factor&&e.seasonal_factor!==1;return!i&&!a&&!l&&!h?o`<div class="adaptive-empty">
      ...
      <div class="adaptive-stack">
        ...
        ${a?js(e,t,this._features):c}
        ${l?Ls(e,t):c}
        ${h?o`
          ${Ms(e,t,this._features)}
          ${e.seasonal_factors?.length===12||e.interval_analysis?.seasonal_factors?.length===12?zs(e,t):c}
        `:c}
      </div>
    `}_renderDetails(e){let t=this._lang,i=e.history||[],a=e.history_count??i.length,l=As(i).slice(0,20);return o`
      <div class="details">
        <div class="stats-grid">
          ...
          <div class="stat">
            ...
            <span class="stat-value">${Re(e.total_cost??0,this._currencySymbol,t)}</span>
          </div>
          <div class="stat">
            <span class="stat-label">${n("avg_duration",t)}</span>
            <span class="stat-value">${kt(e.average_duration!=null?Math.round(e.average_duration):null,t)}</span>
          </div>
        </div>
        ...
        ${i.length===0?o`<div class="history-empty">${n("history_empty",t)}</div>`:o`
              <div class="history-list">
                ${l.map(h=>Cs(h,{lang:t,hass:this.hass,currencySymbol:this._currencySymbol,openEdit:u=>this._onEditHistoryEntry(u),readingUnit:e.reading_unit,readingSlotDelta:(u,_)=>Ur(i,u,_),taskRef:this._taskRef},{compact:!0}))}
                ${a>l.length?o`<div class="history-more">… +${a-l.length} ${n("older_entries",t)}</div>`:c}
              </div>
            `}
      </div>
    `}render(){if(!this._open)return c;let e=this._lang,t=this._task,i=Ne(this.hass?.user,this._access);return o`
      <div class="backdrop" @click=${this.close}></div>
      <div class="dialog" role="dialog" aria-modal="true">
        ${t?o`
              <div class="header">
                <div class="title">
                  <span class="status-dot" style="background: ${Ft(t)}"></span>
                  <span class="task-name">${t.name}</span>
                  ${Rs(t,e)}
                </div>
                <div class="object">
                  <button class="link-inline" @click=${()=>{this._entryId&&Promise.resolve().then(()=>(ie(),oe)).then(({openObjectQuickActions:a})=>{a(this._entryId),this.close()})}}>${this._objectName}</button>
                </div>
                <div class="quick-info">
                  ...
                  ${t.schedule?.kind&&!["manual","one_time"].includes(t.schedule.kind)||t.interval_days!=null?o`<span><strong>${n("interval",e)}:</strong> ${Lr(t,e)}</span>`:c}
                  ${pe(t)?o`<span><strong>${n("phase_current",e)}:</strong> ${pe(t)}</span>`:c}
                </div>
              </div>
              ...
              ${this._showSkip?o`
                  ...
                  `:o`
                    ...
                    <div class="actions secondary-row">
                      ${i?o`<ha-button size="small" appearance="outlined" variant="neutral" class="qa-edit" @click=${this._onEdit} .disabled=${this._busy}>
                      ...
                      ${i?o`<ha-button size="small" appearance="outlined" variant="neutral" class="qa-archive"
  ...
  `],d([b({attribute:!1})],N.prototype,"hass",2),d([p()],N.prototype,"_open",2),d([p()],N.prototype,"_entryId",2),d([p()],N.prototype,"_taskId",2),d([p()],N.prototype,"_task",2),d([p()],N.prototype,"_objectName",2),d([p()],N.prototype,"_taskRef",2),d([p()],N.prototype,"_busy",2),d([p()],N.prototype,"_error",2),d([p()],N.prototype,"_showSkip",2),d([p()],N.prototype,"_showReset",2),d([p()],N.prototype,"_showDetails",2),d([p()],N.prototype,"_showAdaptive",2),d([p()],N.prototype,"_skipReason",2),d([p()],N.prototype,"_resetDate",2),d([p()],N.prototype,"_features",2),d([p()],N.prototype,"_toast",2),d([p()],N.prototype,"_access",2);customElements.get("maintenance-task-quick-actions-dialog")||customElements.define("maintenance-task-quick-actions-dialog",N)});function Bs(s){return s?customElements.get("ha-markdown")?o`<ha-markdown class="notes-md" .content=${s} breaks></ha-markdown>`:o`${s}`:c}var Vs=x(()=>{"use strict";C()});var Q,Ws=x(()=>{"use strict";C();_i();Vs();q();I();ne();Hi();lt();ot();zt();Q=class extends E{constructor(){super(...arguments);this._open=!1;this._entryId=null;this._data=null;this._busy=!1;this._error="";this._access=we}get _lang(){return P(this.hass)}async openFor(e){this._entryId=e,this._error="",this._open=!0,await Promise.all([this._load(),X(this.hass).then(t=>{this._access=t.access})])}close(){this._open=!1,this._data=null,this._error=""}async _load(){if(this._entryId)try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._data=e}catch(e){this._error=R(e,this._lang)}}_onEditObject(){!this._entryId||!this._data||Promise.resolve().then(()=>(ie(),oe)).then(({openEditObjectDialog:e})=>{e(this._entryId,this._data.object),this.close()})}_onAddTask(){this._entryId&&Promise.resolve().then(()=>(ie(),oe)).then(({openCreateTaskDialog:e})=>{e(this._entryId),this.close()})}async _onDelete(){if(!(!this._entryId||!this._data||!await $e(this.hass,{title:n("delete",this._lang),message:n("delete_object_confirm",this._lang),confirmText:n("delete",this._lang),danger:!0}))){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:this._entryId}),this.dispatchEvent(new CustomEvent("object-deleted",{detail:{entry_id:this._entryId},bubbles:!0,composed:!0})),this.close()}catch(t){this._error=R(t,this._lang)}finally{this._busy=!1}}}async _onArchiveObject(){if(!this._entryId||!this._data)return;let e=!!this._data.object.archived;if(!(!e&&!await $e(this.hass,{title:n("archive_object",this._lang),message:n("confirm_archive_object",this._lang),confirmText:n("archive_object",this._lang)}))){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:e?"maintenance_supporter/object/unarchive":"maintenance_supporter/object/archive",entry_id:this._entryId}),this.dispatchEvent(new CustomEvent("object-changed",{detail:{entry_id:this._entryId},bubbles:!0,composed:!0})),this.close()}catch(t){this._error=R(t,this._lang)}finally{this._busy=!1}}}_onTaskClick(e){this._entryId&&Promise.resolve().then(()=>(ie(),oe)).then(({openTaskQuickActions:t})=>{t(this._entryId,e)})}render(){if(!this._open)return c;let e=this._lang,t=this._data,i=t?.object,a=t?.tasks||[],l=Ne(this.hass?.user,this._access);return o`
      <div class="backdrop" @click=${this.close}></div>
      <div class="dialog" role="dialog" aria-modal="true">
        ${t&&i?o`
              <div class="header">
                      ...
                      <div class="task-list">
                        ${a.map(h=>o`
                          <div class="task-row" @click=${()=>this._onTaskClick(h.id)}>
                            <span class="status-dot" style="background: ${Ft(h)}"></span>
                            <span class="task-name">${h.name}</span>
                            <span class="task-status ${_t(h)}">${ji(_t(h),e)}</span>
                          </div>
                        `)}
                      </div>
                    `}
              ...
              ${i.notes?o`
                    <div class="notes-section">
                      ...
                      <div class="notes-body">${Bs(i.notes)}</div>
                    </div>
                  `:c}
  ...
  `,d([b({attribute:!1})],Q.prototype,"hass",2),d([p()],Q.prototype,"_open",2),d([p()],Q.prototype,"_entryId",2),d([p()],Q.prototype,"_data",2),d([p()],Q.prototype,"_busy",2),d([p()],Q.prototype,"_error",2),d([p()],Q.prototype,"_access",2);customElements.get("maintenance-object-quick-actions-dialog")||customElements.define("maintenance-object-quick-actions-dialog",Q)});var oe={};tn(oe,{__resetSettingsCacheForTests:()=>Na,getRowActionStyle:()=>La,openCompleteDialog:()=>qa,openCreateObjectDialog:()=>Oi,openCreateTaskDialog:()=>qi,openEditObjectDialog:()=>ja,openEditTaskDialog:()=>Ha,openHistoryEditDialog:()=>Oa,openObjectQuickActions:()=>Ma,openQrDialog:()=>Da,openTaskQuickActions:()=>Di});function Bt(){return document.querySelector("home-assistant")?.hass}function Pa(){return document.querySelector("home-assistant")?.shadowRoot??document.body}function le(s){let r=Pa(),e=r.querySelector(s)??document.body.querySelector(s);return e?e.parentNode!==r&&r.appendChild(e):(e=document.createElement(s),r.appendChild(e)),e}function de(s){let r=Bt();if(!r)return!1;s.hass=r;let e=P(r);return li(e)||at(e).then(()=>{s.requestUpdate?.()}),di(r.locale,r.config?.country),!0}function La(s){return X(s).then(r=>r.rowActionStyle)}function Na(){Qr()}function Oi(){let s=le(Ks);return de(s)?(s.openCreate(),!0):!1}function ja(s,r){let e=le(Ks);return de(e)?(e.openEdit(s,r),!0):!1}function qi(s="",r){let e=le(Ys);if(!de(e))return!1;let t=Bt();return t?((async()=>{let i=await X(t),a=e;a.checklistsEnabled=i.features.checklists,a.scheduleTimeEnabled=i.features.schedule_time,a.completionActionsEnabled=i.features.completion_actions,a.defaultWarningDays=i.defaultWarningDays,a.openCreate(s,r)})(),!0):!1}function Ha(s,r){let e=le(Ys);if(!de(e))return!1;let t=Bt();return t?((async()=>{try{let[i,a]=await Promise.all([t.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:s}),X(t)]),l=(i.tasks||[]).find(u=>u.id===r);if(!l){console.warn(`openEditTaskDialog: task ${r} not found in entry ${s}`);return}let h=e;h.checklistsEnabled=a.features.checklists,h.scheduleTimeEnabled=a.features.schedule_time,h.completionActionsEnabled=a.features.completion_actions,h.defaultWarningDays=a.defaultWarningDays,await h.openEdit(s,l)}catch(i){console.warn("openEditTaskDialog: failed to load task/features",i)}})(),!0):!1}function Oa(s){let r=le(Ta);return de(r)?(r.openEdit(s),!0):!1}function qa(s){let r=le(Aa);return de(r)?(Pt(r,s,P(Bt())),!0):!1}function Da(s){let r=le(Ca);return de(r)?(r.openForTask(s.entry_id,s.task_id,s.object_name,s.task_name),!0):!1}function Di(s,r){let e=le(Ia);return de(e)?(e.openFor(s,r),!0):!1}function Ma(s){let r=le(Ra);return de(r)?(r.openFor(s),!0):!1}var Ks,Ys,Ta,Aa,Ca,Ia,Ra,ie=x(()=>{"use strict";ss();hs();Si();gs();vs();Us();Ws();I();Lt();lt();Ks="maintenance-object-dialog",Ys="maintenance-task-dialog",Ta="maintenance-history-edit-dialog",Aa="maintenance-complete-dialog",Ca="maintenance-qr-dialog",Ia="maintenance-task-quick-actions-dialog",Ra="maintenance-object-quick-actions-dialog"});C();var gn=["assignee_pool","required_completion_fields","checklist","labels","mirror_todo_entities","next_event_titles","history","readings"],fn=["checklist_progress"],vn=["tasks","parts"],bn=["manual_docs","battery_fleet_excluded"];function si(s,r,e=[]){for(let t of r)s[t]===void 0&&(s[t]=[]);for(let t of e)s[t]===void 0&&(s[t]={})}function yn(s){let r=s;si(r,vn),r.object&&typeof r.object=="object"&&si(r.object,bn);for(let e of r.tasks)si(e,gn,fn);return s}function Xe(s){for(let r of s)yn(r);return s}function xn(s,r){if(r.objects)return r.objects;let e=r.delta||[],t=r.removed||[];if(!e.length&&!t.length)return null;let i=new Map(s.map(a=>[a.entry_id,a]));for(let a of e)i.set(a.entry_id,a);for(let a of t)i.delete(a);return[...i.values()]}function hr(s,r){return r.objects&&Xe(r.objects),r.delta&&Xe(r.delta),xn(s,r)}q();I();ui();_i();function Pe(s){let r=window;r.customCards=r.customCards||[],r.customCards.some(e=>e.type===s.type)||r.customCards.push(s)}fi();gi();Ct();C();function qr(s){let r=(s??[]).filter(e=>!!e);return r.length?o`<span class="event-titles"> · ${r.join(", ")}</span>`:c}Lt();lt();ot();C();q();I();var Mn=["overdue","triggered","due_soon","ok"],ee=class extends E{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-card"};this._objects=[];this._loadingObjects=!0;this._loadError=!1;this._views=[];this._objectsLoaded=!1;this._onEntitiesChanged=e=>{this._valueChanged("entity_ids",e.detail.value||[])}}get _lang(){return P(this.hass)}setConfig(e){this._config={...e}}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._objectsLoaded&&(this._objectsLoaded=!0,this._loadObjects())}async _loadObjects(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"});this._objects=e.objects||[],this._loadError=!1}catch{this._objects=[],this._loadError=!0}this._loadingObjects=!1;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"});this._views=e.views||[]}catch{this._views=[]}}_valueChanged(e,t){let i={...this._config,[e]:t};(Array.isArray(t)&&t.length===0||t==="")&&delete i[e],this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i}}))}_toggleStatus(e,t){let i=new Set(this._config.filter_status||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_status",[...i])}_toggleObject(e,t){let i=new Set(this._config.filter_objects||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_objects",[...i])}_toggleLabel(e,t){let i=new Set(this._config.filter_labels||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_labels",[...i])}_toggleArea(e,t){let i=new Set(this._config.filter_areas||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_areas",[...i])}_togglePriority(e,t){let i=new Set(this._config.filter_priority||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_priority",[...i])}render(){let e=this._lang,t=new Set(this._config.filter_status||[]),i=new Set(this._config.filter_objects||[]),a=[...this._objects].map(g=>g.object.name).sort((g,$)=>g.localeCompare($)),l=new Set(this._config.filter_areas||[]),h=[...new Set(this._objects.map(g=>g.object.area_id).filter(g=>!!g).concat([...l]))],u=g=>this.hass?.areas?.[g]?.name||g,_=h.map(g=>({id:g,name:u(g)})).sort((g,$)=>g.name.localeCompare($.name)),m=new Set(this._config.filter_labels||[]),v=new Set(this._config.filter_priority||[]),w=[...new Set(this._objects.flatMap(g=>g.tasks.flatMap($=>$.labels||[])))].sort((g,$)=>g.localeCompare($)),y=[];for(let g of this._objects)for(let $ of g.tasks)$.sensor_entity_id&&y.push($.sensor_entity_id),$.binary_sensor_entity_id&&y.push($.binary_sensor_entity_id);return o`
      <div class="editor">
        <ha-textfield
  ...
  `,d([b({attribute:!1})],ee.prototype,"hass",2),d([p()],ee.prototype,"_config",2),d([p()],ee.prototype,"_objects",2),d([p()],ee.prototype,"_loadingObjects",2),d([p()],ee.prototype,"_loadError",2),d([p()],ee.prototype,"_views",2);customElements.get("maintenance-supporter-card-editor")||customElements.define("maintenance-supporter-card-editor",ee);Si();C();q();C();q();I();ki();ne();Dt();var B=class B extends E{constructor(){super(...arguments);this.flat=!1;this._ov=null;this._loading=!1;this._marking=!1;this._error="";this._history=null;this._rosterSort=B._storedSort();this._typeFilter=null;this._recorded=[];this._historyRequested=!1;this._localeReady=!1;this._markAll=async()=>{await this._mark(void 0)};this._repair=async()=>{if(!this._marking){this._marking=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/setup",language:this._lang}),await this._load()}catch(e){this._error=R(e,this._lang)}finally{this._marking=!1}}};this._loadHistory=async e=>{let t=e.target.open;if(pt(ue.batteryRosterOpen,t?"1":"0"),!(!t||this._historyRequested)){this._historyRequested=!0;try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/overview_history"});this._history=i.series}catch{this._history=null}}}}get _lang(){return P(this.hass)}connectedCallback(){super.connectedCallback(),this.hass&&this._load()}updated(e){wt(this,e),e.has("hass")&&this.hass&&!this._localeReady&&(this._localeReady=!0,at(this._lang).then(()=>this.requestUpdate()),this._ov===null&&!this._loading&&this._load())}async _load(){this._loading=!0,this._error="";try{this._ov=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/overview"})}catch(e){this._error=R(e,this._lang)}finally{this._loading=!1}}async _mark(e){if(!this._marking){this._marking=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/mark_replaced",...e?{entity_ids:e}:{}}),await this._load()}catch(t){this._error=R(t,this._lang)}finally{this._marking=!1}}}async _setExcluded(e,t){if(!this._marking){this._marking=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/set_excluded",entity_id:e,excluded:t}),await this._load()}catch(i){this._error=R(i,this._lang)}finally{this._marking=!1}}}async _addBattery(e){let t=e.detail?.value;if(!(!t||this._marking)){this._marking=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/set_included",entity_id:t,included:!0}),await this._load()}catch(i){this._error=R(i,this._lang)}finally{this._marking=!1}}}async _setTrackSelf(e){await this._setFleetOption("set_track_self_charging",e.target.checked)}async _setDueWithoutSensor(e){await this._setFleetOption("set_due_without_sensor",e.target.checked)}async _setFleetOption(e,t){if(!this._marking){this._marking=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:`maintenance_supporter/battery_fleet/${e}`,enabled:t}),await this._load()}catch(i){this._error=R(i,this._lang)}finally{this._marking=!1}}}_rosterOpen(){return ct(ue.batteryRosterOpen)!=="0"}_predictedTitle(e,t){if(e.forecast_overdue)return n("battery_fleet_forecast_overdue",t);let i=this._predictedDate(e.days_until??0);if(e.predicted_source==="trend")return n("battery_fleet_predicted_trend",t).replace("{date}",i).replace("{confidence}",n("cal_confidence_"+(e.prediction_confidence||"medium"),t));if(e.lifetime_months!=null&&e.lifetime_source){let a=n("lifetime_source_"+e.lifetime_source,t).replace("{n}",String(e.lifetime_samples??0));return n("battery_fleet_predicted_typical",t).replace("{date}",i).replace("{months}",String(e.lifetime_months)).replace("{type}",e.battery_type).replace("{source}",a)}return n("battery_fleet_predicted_on",t).replace("{date}",i)}_sparkline(e){let t=this._history?.[e.entity_id];if(!t||t.points.length<2)return c;let i=110,a=24,l=2,h=t.points[0][0],u=t.points[t.points.length-1][0],_=Date.now()/1e3,m=e.status!=="low"&&e.predicted_source==="trend"&&e.days_until!=null?_+e.days_until*86400:null,v=Math.max(u,m??u),w=L=>v===h?l:l+(L-h)/(v-h)*(i-2*l),y=L=>l+(1-Math.min(100,Math.max(0,L))/100)*(a-2*l),g=t.points.map(([L,A])=>`${T(w(L))},${T(y(A))}`).join(" "),$=t.points[t.points.length-1][1],z=T(y(t.threshold));return o`<svg
    ...
    >
      ...
      ${m!==null?o`<line
            ...
            y2=${z}
          ></line>`:c}
    </svg>`}static _storedSort(){return ct(ue.batteryRosterSort)==="name"?"name":"urgency"}_setSort(e){this._rosterSort=e,pt(ue.batteryRosterSort,e)}_sortedRoster(e){let t=this._typeFilter===null?e:e.filter(a=>a.battery_type===this._typeFilter);if(this._rosterSort==="name")return t;let i=a=>a.status==="low"?-1e3+(a.level??101)/101:a.days_until??1/0;return[...t].sort((a,l)=>i(a)-i(l)||a.device_name.localeCompare(l.device_name))}_predictedDate(e){return this._fmtDate(Date.now()+e*864e5)}_fmtDate(e){let t=new Date(e),i=a=>String(a).padStart(2,"0");return F(`${t.getFullYear()}-${i(t.getMonth()+1)}-${i(t.getDate())}`,this._lang)}_shoppingLine(e){return Object.entries(e).map(([t,i])=>o`<button
    ...
    </button>`}_renderRow(e,t,i){let a=e.available===!1?o`<span class="bf-offline">${n("battery_fleet_offline",t)}</span>`:e.no_sensor?o`<span class="bf-offline bf-nosensor">${n("battery_fleet_no_sensor",t)}</span>`:c,l=o`<span class="bf-type">${e.quantity}× ${e.battery_type}</span>`,h=i.mark==="always"||e.no_sensor||e.can_mark_replaced;return o`
      <div class="bf-row">
  ...
  `,d([b({attribute:!1})],B.prototype,"hass",2),d([b({type:Boolean})],B.prototype,"flat",2),d([p()],B.prototype,"_ov",2),d([p()],B.prototype,"_loading",2),d([p()],B.prototype,"_marking",2),d([p()],B.prototype,"_error",2),d([p()],B.prototype,"_history",2),d([p()],B.prototype,"_rosterSort",2),d([p()],B.prototype,"_typeFilter",2),d([p()],B.prototype,"_recorded",2);var Ti=B;customElements.get("maintenance-battery-fleet-section")||customElements.define("maintenance-battery-fleet-section",Ti);var ze=class extends E{constructor(){super(...arguments);this._config={type:"custom:maintenance-battery-fleet-card"}}static getStubConfig(){return{type:"custom:maintenance-battery-fleet-card"}}setConfig(e){this._config=e}getCardSize(){return 6}render(){return this.hass?o`
  ...
  `,d([b({attribute:!1})],ze.prototype,"hass",2),d([p()],ze.prototype,"_config",2);customElements.get("maintenance-battery-fleet-card")||customElements.define("maintenance-battery-fleet-card",ze);Pe({type:"maintenance-battery-fleet-card",name:"Battery Fleet",description:"All tracked batteries: what is low now, what runs out soon, and what to buy.",preview:!1});ie();C();q();var Mi=["today","dashboard","calendar","settings"];I();var Ue="maintenance-supporter-panel-card",zi="maintenance-supporter-panel",Ui="maintenance-supporter",Fi=870,za=320,Fa=480;function Ua(s){let e=s?.panels?.[Ui]?.config?._panel_custom?.module_url;return typeof e=="string"&&e.startsWith("/")?e:null}function Ba(s){if(s==null||s===""||s==="fill")return null;if(typeof s=="number")return s>0?`${Math.round(s)}px`:null;let r=String(s).trim();return/^\d+(\.\d+)?$/.test(r)?`${r}px`:r||null}var Bi=class extends HTMLElement{constructor(){super();this._config={type:`custom:${Ue}`};this._panel=null;this._loading=null;this._observer=null;this._lastWidth=-1;this._preview=!1;this._onResize=()=>this._layout(!0);let e=this.attachShadow({mode:"open"}),t=document.createElement("style");t.textContent=`
      ...
      ${zi} { display: block; height: 100%; }
    `,e.appendChild(t),this._status=document.createElement("div"),this._status.className="status",this._status.hidden=!0,e.appendChild(this._status)}static getConfigElement(){return document.createElement("maintenance-supporter-panel-card-editor")}static getStubConfig(){return{type:`custom:${Ue}`}}set preview(e){this._preview=!!e,this._layout(!0)}get preview(){return this._preview}_inEditorPreview(){if(this._preview)return!0;let e=this;for(let t=0;e&&t<40;t++){let i=e;if(i.tagName==="HUI-CARD-PREVIEW"||i.tagName==="HUI-DIALOG-EDIT-CARD")return!0;e=i.parentElement??i.getRootNode().host??null}return!1}setConfig(e){this._config={...e},this._panel&&(this._panel.presets=this._presets()),this._layout(!0)}getCardSize(){return 12}getGridOptions(){return{columns:"full",rows:"auto"}}set hass(e){this._hass=e,this._panel?this._panel.hass=e:this._mount()}get hass(){return this._hass}connectedCallback(){typeof ResizeObserver<"u"&&!this._observer&&(this._observer=new ResizeObserver(()=>this._layout(!1)),this._observer.observe(this)),window.addEventListener("resize",this._onResize),this._layout(!0),this._hass&&this._mount()}disconnectedCallback(){this._observer?.disconnect(),this._observer=null,window.removeEventListener("resize",this._onResize)}_presets(){let e={};return Mi.includes(this._config.tab??"")&&(e.tab=this._config.tab),typeof this._config.view=="string"&&this._config.view.trim()&&(e.view=this._config.view.trim()),e}_setStatus(e){this._status.textContent=e,this._status.hidden=!1}async _mount(){if(this._panel||this._loading||!this._hass||!this.isConnected)return;let e=P(this._hass);if(!customElements.get(zi)){let i=Ua(this._hass);if(!i){this._setStatus(n("panel_card_not_registered",e));return}this._loading=import(i).then(()=>{},()=>{throw this._setStatus(n("panel_card_load_failed",e)),new Error("panel bundle failed to load")});try{await this._loading}catch{return}finally{this._loading=null}if(this._panel||!this._hass||!this.isConnected)return}let t=document.createElement(zi);t.embedded=!0,t.panel=this._hass.panels?.[Ui]??{url_path:Ui},t.presets=this._presets(),t.narrow=this._narrow(),t.hass=this._hass,this._status.hidden=!0,this.shadowRoot.appendChild(t),this._panel=t,this._layout(!0)}_narrow(){let e=this.getBoundingClientRect().width;return e>0?e<Fi:window.innerWidth<Fi}_layout(e){let t=this.getBoundingClientRect(),i=Math.round(t.width)!==this._lastWidth;if(!e&&!i)return;this._lastWidth=Math.round(t.width),this._panel&&t.width>0&&(this._panel.narrow=t.width<Fi);let a=Ba(this._config.height);if(a){this.style.height=a;return}if(!this.isConnected)return;if(this._inEditorPreview()){this.style.height=`${Fa}px`;return}let l=Math.max(0,this._unscrolledTop(t)),h=Math.max(za,Math.floor(window.innerHeight-l));this.style.height=`${h}px`}_unscrolledTop(e){let t=e.top+window.scrollY,i=this.ownerDocument,a=this.parentElement??this.getRootNode().host??null;for(let l=0;a&&l<100;l++){let h=a;h!==i.documentElement&&h!==i.body&&typeof h.scrollTop=="number"&&h.scrollTop>0&&(t+=h.scrollTop),a=h.parentElement??h.getRootNode().host??null}return t}},ke=class extends E{constructor(){super(...arguments);this._config={type:`custom:${Ue}`};this._views=[];this._viewsLoaded=!1}setConfig(e){this._config={...e}}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._viewsLoaded&&(this._viewsLoaded=!0,this._loadViews())}async _loadViews(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"});this._views=e?.views||[]}catch{this._views=[]}}_set(e,t){let i={...this._config};t===""||t===void 0?delete i[e]:i[e]=t,this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}render(){let e=P(this.hass),t=i=>n(i==="today"?"tab_today":i==="calendar"?"tab_calendar":i,e);return o`
      <div class="editor">
        <div class="field">
          ...
          <select class="tab-select" .value=${this._config.tab||""} @change=${i=>this._set("tab",i.target.value)}>
            ...
            ${Mi.map(i=>o`<option value=${i} ?selected=${this._config.tab===i}>${t(i)}</option>`)}
          </select>
        </div>
  ...
  `,d([b({attribute:!1})],ke.prototype,"hass",2),d([p()],ke.prototype,"_config",2),d([p()],ke.prototype,"_views",2);customElements.get(Ue)||customElements.define(Ue,Bi);customElements.get("maintenance-supporter-panel-card-editor")||customElements.define("maintenance-supporter-panel-card-editor",ke);Pe({type:Ue,name:"Maintenance Supporter \u2014 Panel",description:"The complete Maintenance Supporter panel as a card \u2014 for a panel view / dashboard subview without the sidebar entry.",preview:!1});var U=class extends E{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-card"};this._globalRowStyle="buttons_compact";this._settings=je;this._firstLoadDone=!1;this._objects=[];this._stats=null;this._unsub=null;this._viewFilters=null;this._userNames={};this._userPersons={};this._userService=null;this._userNamesLoaded=!1;this._taskDocs={};this._docsLoadedFor=new Set;this._dataLoaded=!1;this._lastConnection=null;this._onCompleted=async()=>{await this._loadData()}}get _lang(){return P(this.hass)}static getConfigElement(){return document.createElement("maintenance-supporter-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-card",show_header:!0,show_actions:!0,filter_status:["overdue","triggered","due_soon"],max_items:10}}setConfig(e){let t=e.view_id!==this._config.view_id;this._config=e,t&&this._dataLoaded&&this.hass&&this._loadViewFilters()}getCardSize(){return 3}connectedCallback(){super.connectedCallback()}disconnectedCallback(){super.disconnectedCallback(),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null}updated(e){if(super.updated(e),wt(this,e),this.hass&&!this._userNamesLoaded&&this._config.show_assignee!==!1&&this._objects.some(t=>t.tasks.some(i=>i.responsible_user_id))&&this._loadUserNames(),this.hass&&this._config.show_documents!==!1)for(let t of this._objects)this._docsLoadedFor.has(t.entry_id)||t.tasks.some(i=>(i.document_count??0)>0)&&this._loadDocuments(t.entry_id);if(e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){X(this.hass).then(t=>{this._settings=t,this._globalRowStyle=t.rowActionStyle}).catch(()=>{});let e=this._loadViewFilters();try{let[t,i]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects",compact:!0}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);await e,this._objects=Xe(t.objects),this._stats=i,$t(this._stats.budget)}catch{}await e,this._firstLoadDone=!0}_assigneeName(e){if(this._config.show_assignee===!1)return"";let t=e.responsible_user_id;return t&&this._userNames[t]||""}async _loadUserNames(){this._userNamesLoaded=!0,this._userService?this._userService.updateHass(this.hass):this._userService=new Le(this.hass);try{let e=await this._userService.getUsers();this._userPersons=Object.fromEntries(e.map(t=>[t.id,At(t)])),this._userNames=Object.fromEntries(e.map(t=>[t.id,t.name]))}catch{}}async _loadDocuments(e){this._docsLoadedFor.add(e);try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:e}),i={};for(let a of t.documents||[])for(let l of a.task_ids||[])(i[l]||=[]).push({id:a.id,title:a.title,kind:a.kind,url:a.url});this._taskDocs={...this._taskDocs,[e]:i}}catch{}}_docsFor(e,t){if(this._config.show_documents===!1)return[];let a=[...this._taskDocs[e]?.[t.id]||[]];return t.documentation_url&&a.push({id:`url:${t.id}`,title:n("documentation_label",this._lang),kind:"weblink",url:t.documentation_url}),a}async _openDoc(e){if(e.kind==="weblink"&&e.url){Tt(e.url)&&window.open(e.url,"_blank","noopener");return}try{await Hr(this.hass,e.id)}catch{}}async _loadViewFilters(){if(!this._config.view_id){this._viewFilters=null;return}try{let t=((await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"})).views||[]).find(i=>i.id===this._config.view_id);this._viewFilters=t?t.filters:null}catch{this._viewFilters=null}}async _subscribe(){try{let e=await this.hass.connection.subscribeMessage(t=>{let i=hr(this._objects,t);i!==null&&(this._objects=i)},{type:"maintenance_supporter/subscribe",deltas:!0,compact:!0});if(!this.isConnected){e();return}this._unsub=e}catch{}}get _flatTasks(){let e=[],{filter_status:t,filter_objects:i,entity_ids:a,filter_due_min_days:l,filter_due_max_days:h,filter_labels:u,filter_priority:_,filter_areas:m,max_items:v}=this._config,w=a?.length?new Set(a):null,y=l!==void 0||h!==void 0,g=this._viewFilters,$=g?.user_id==="current_user"?this.hass.user?.id??null:g?.user_id??null;for(let L of this._objects)if(!(i?.length&&!i.includes(L.object.name))){if(m?.length){let A=L.object.area_id;if(!A||!m.includes(A))continue}for(let A of L.tasks)if(!A.is_done&&!(A.archived||L.object.archived)&&!(t?.length&&!t.includes(A.status))&&!(u?.length&&!(A.labels||[]).some(O=>u.includes(O)))&&!(_?.length&&!_.includes(A.priority||"normal"))&&!(w&&!(A.sensor_entity_id&&w.has(A.sensor_entity_id)||A.binary_sensor_entity_id&&w.has(A.binary_sensor_entity_id)))){if(y){let O=A.days_until_due;if(O==null||l!==void 0&&O<l||h!==void 0&&O>h)continue}g&&(g.status&&A.status!==g.status||g.label&&!(A.labels||[]).includes(g.label)||g.priority&&(A.priority||"normal")!==g.priority||$&&A.responsible_user_id!==$)||e.push({entry_id:L.entry_id,object_name:L.object.name,task:A})}}let z={overdue:0,triggered:1,due_soon:2,ok:3};return e.sort((L,A)=>{let O=(z[L.task.status]??9)-(z[A.task.status]??9);return O!==0?O:(L.task.days_until_due??1/0)-(A.task.days_until_due??1/0)}),v&&v>0?e.slice(0,v):e}_openComplete(e,t){let i=this.shadowRoot.querySelector("maintenance-complete-dialog");Pt(i,Rt({entryId:e,taskId:t.id,taskName:t.name,task:t,objects:this._objects,lang:this._lang,features:this._settings.features,currencySymbol:ye(this._stats?.budget)}),this._lang)}get _headerCounts(){let e={overdue:0,due_soon:0,triggered:0};for(let t of this._objects)for(let i of t.tasks)(i.status==="overdue"||i.status==="due_soon"||i.status==="triggered")&&(e[i.status]+=1);return e}_openTaskDetail(e,t){Di(e,t)}render(){let e=this._lang,t=this._config.title||n("maintenance",e),i=this._config.show_header!==!1,a=(this._config.action_style??(this._globalRowStyle==="icons"?"icons":"buttons"))==="buttons",l=this._config.show_actions!==!1,h=l&&Ne(this.hass?.user,this._settings.access),u=this._config.compact||!1,_=this._flatTasks,m=this._objects.length||this._stats?this._headerCounts:null;return o`
      <ha-card>
          ...
          <div class="header-right">
            ${i&&m?o`
                  <div class="header-stats">
                    ${m.overdue>0?o`<span class="badge overdue">${m.overdue}</span>`:c}
                    ${m.due_soon>0?o`<span class="badge due_soon">${m.due_soon}</span>`:c}
                    ${m.triggered>0?o`<span class="badge triggered">${m.triggered}</span>`:c}
                  </div>
                `:c}
            ${h?o`
                  <mwc-icon-button
        ...
        ${this._firstLoadDone?_.length===0?this._objects.some(v=>v.tasks.length>0)?o`<div class="empty-card">
              ...
              <div class="task-list ${u?"compact":""}">
                ${_.map(({entry_id:v,object_name:w,task:y})=>o`
                    <div class="task-item clickable"
                         @click=${()=>this._openTaskDetail(v,y.id)}
                         title="${n("open_task",e)}">
                      <div class="status-dot" style="background: ${et[y.status]||"#ccc"}"></div>
                      <div class="task-info">
                        <div class="task-name">
                          ${y.name}${qr(y.next_event_titles)}
                          ${y.due_override?o`<ha-icon
                                class="postponed-icon"
                                icon="mdi:calendar-clock"
                                title="${n("postponed",e)}"
                              ></ha-icon>`:c}
                        </div>
                        ${u?this._assigneeName(y)?o`<div class="task-meta compact-assignee" title="${this._assigneeName(y)}">
                                ${mi(this._userPersons[y.responsible_user_id])}${this._assigneeName(y)}
                              </div>`:c:o`<div class="task-meta">
                              ${w} · ${n(y.type,e)}${pe(y)?o` · ${pe(y)}`:c}${this._assigneeName(y)?o` · <span class="assignee"
                                    >${mi(this._userPersons[y.responsible_user_id])}${this._assigneeName(y)}</span
                                  >`:c}
                            </div>`}
                      </div>
                      ${this._docsFor(v,y).length?o`<div class="doc-chips">
                            ${this._docsFor(v,y).map(g=>o`
                              <button
                                type="button"
                                class="doc-chip"
                                title="${g.title}"
                                @click=${$=>{$.stopPropagation(),this._openDoc(g)}}
                              >
                                <ha-icon icon=${g.kind==="weblink"?"mdi:link-variant":"mdi:file-document-outline"}></ha-icon>
                                <span>${g.title}</span>
                              </button>
                            `)}
                          </div>`:c}
                      <div class="task-due">
                        ${y.days_until_due!==null&&y.days_until_due!==void 0?y.days_until_due<0?o`<span class="overdue-text">${ci(y.days_until_due,e)}</span>`:ci(y.days_until_due,e):y.trigger_active?"\u26A1":"\u2014"}
                      </div>
                      ${l&&a?o`
                            <ha-button
                              size="small"
                              ...
                              title="${n("complete",e)}"
                              @click=${g=>{g.stopPropagation(),this._openComplete(v,y)}}
                            >
            ...
            `:o`<div class="card-loading">${n("loading",e)}</div>`}
      </ha-card>
    ...
    `],d([b({attribute:!1})],U.prototype,"hass",2),d([p()],U.prototype,"_config",2),d([p()],U.prototype,"_globalRowStyle",2),d([p()],U.prototype,"_settings",2),d([p()],U.prototype,"_firstLoadDone",2),d([p()],U.prototype,"_objects",2),d([p()],U.prototype,"_stats",2),d([p()],U.prototype,"_unsub",2),d([p()],U.prototype,"_viewFilters",2),d([p()],U.prototype,"_userNames",2),d([p()],U.prototype,"_taskDocs",2);customElements.get("maintenance-supporter-card")||customElements.define("maintenance-supporter-card",U);Pe({type:"maintenance-supporter-card",name:"Maintenance Supporter",description:"Overview of your maintenance tasks with quick actions.",preview:!0});export{U as MaintenanceSupporterCard};
