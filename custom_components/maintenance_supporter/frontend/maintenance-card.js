...
`});function vr(s,r,e){let t=new Blob([s],{type:e}),i=URL.createObjectURL(t),a=document.createElement("a");a.href=i,a.download=r,a.target="_blank",a.rel="noopener",a.style.display="none",document.body.appendChild(a),a.dispatchEvent(new MouseEvent("click")),document.body.removeChild(a),setTimeout(()=>URL.revokeObjectURL(i),6e4)}var Wt=w(()=>{"use strict"});async function cn(s,r,e=300){return(await s.connection.sendMessagePromise({type:"auth/sign_path",path:r,expires:e})).path}async function Kt(s,r,e=300){return cn(s,`/api/maintenance_supporter/document/${r}`,e)}async function br(s,r,e=""){let t=window.open("about:blank","_blank");try{let i=await Kt(s,r);t&&(t.location.href=new URL(i+e,window.location.origin).href)}catch(i){throw t&&t.close(),i}}var Yt=w(()=>{"use strict";Wt()});function ut(s){return!!s&&/^https?:\/\//i.test(s)}var Gt=w(()=>{"use strict"});function pn(s){let r=(s||"").split(/\s+/).filter(Boolean);return r.length===0?"?":r.length===1?r[0][0].toUpperCase():(r[0][0]+r[r.length-1][0]).toUpperCase()}function hn(s){let r=0;for(let e of s)r=r*31+e.charCodeAt(0)>>>0;return yr[r%yr.length]}function _t(s){return s?{id:s.id,name:s.name,initials:s.initials||pn(s.name),color:s.color||hn(s.id)}:null}function Qt(s){return s?o`<span class="person-avatar" style="--person-color: ${s.color}" title=${s.name}>${s.initials}</span>`:c}var yr,Jt=w(()=>{"use strict";I();yr=["#c62828","#ad1457","#6a1b9a","#4527a0","#283593","#1565c0","#00838f","#2e7d32","#558b2f","#ef6c00","#6d4c41","#546e7a"]});var Ce,Zt=w(()=>{"use strict";Jt();Ce=class{constructor(r){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=r}updateHass(r){this.hass=r}async getUsers(r=!1){let e=Date.now();if(!r&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(r,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:r,task_id:e,user_id:t})}async getTasksByUser(r){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:r})).tasks}getUserName(r){return!r||!this.usersCache?null:this.usersCache.find(t=>t.id===r)?.name||null}getPerson(r){return _t(this.getUser(r))}getUser(r){return!r||!this.usersCache?null:this.usersCache.find(e=>e.id===r)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(r){return r?r===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}}});function un(s,r){if(r<=0)return 0;let e=typeof s=="number"&&Number.isFinite(s)?Math.trunc(s):0;return e<0?0:e%r}function _n(s){return!!(s?.phases&&s.phase_sequence&&s.phase_sequence.length>0)}function Xt(s){if(!s||!_n(s))return null;let r=s.phase_sequence,e=un(s.phase_cursor,r.length),t=r[e],i=s.phases?.[t];return i?{id:t,name:i.name,index:e,count:r.length,notes:i.notes,checklist:i.checklist!==void 0?i.checklist:s.checklist??[],consumesParts:i.consumes_parts!==void 0?i.consumes_parts:s.consumes_parts??[],requiredFields:i.required_completion_fields!==void 0?i.required_completion_fields:s.required_completion_fields??[]}:null}function de(s){let r=Xt(s);return r?`${r.index+1}/${r.count} \xB7 ${r.name}`:""}var gt=w(()=>{"use strict"});function xr(){let s=new Uint8Array(4);return(globalThis.crypto??{getRandomValues:r=>r.map(()=>Math.floor(Math.random()*256))}).getRandomValues(s),Array.from(s,r=>r.toString(16).padStart(2,"0")).join("")}function ce(s){let r=s?.reading_values;if(!Array.isArray(r))return[];let e=[],t=new Set;for(let i of r){if(!i||typeof i!="object")continue;let a=i;typeof a.id!="string"||!a.id||t.has(a.id)||typeof a.value!="number"||!Number.isFinite(a.value)||(t.add(a.id),e.push({id:a.id,name:typeof a.name=="string"?a.name:a.id,unit:typeof a.unit=="string"?a.unit:null,value:a.value}))}return e}function wr(s){return s.filter(r=>r.type==="completed"&&(r.reading_value!=null||ce(r).length>0)).sort((r,e)=>r.timestamp.localeCompare(e.timestamp))}function $r(s){return wr(s??[]).map(r=>({timestamp:r.timestamp,values:ce(r)})).filter(r=>r.values.length>0)}function kr(s,r,e){let t;for(let i of s){if(e!==void 0){let l=new Date(i.timestamp).getTime();if(!isNaN(l)&&l>=e)break}let a=i.values.find(l=>l.id===r);a&&(t=a)}return t}function Er(s,r,e){let t=ce(r).find(a=>a.id===e);if(!t)return null;let i=null;for(let a of wr(s)){if(a.timestamp>=r.timestamp)break;let l=ce(a).find(u=>u.id===e);l&&(i=l.value)}return i==null?null:t.value-i}function Sr(s){let r=new Set,e=new Set;for(let t of s){let i=(t.name||"").trim().toLowerCase();i&&(r.has(i)?e.add(t.id):r.add(i))}return e}function Tr(s){let r=[],e=new Set,t=new Set;for(let i of s){let a=(i.name||"").trim();if(!(!a||e.has(i.id)||t.has(a.toLowerCase()))&&(e.add(i.id),t.add(a.toLowerCase()),r.push({id:i.id,name:a,unit:(i.unit||"").trim()||null}),r.length>=20))break}return r}var ve=w(()=>{"use strict"});function Q(s){return`${s.entry_id??""}\0${s.part_id}`}function Ar(s,r,e,t){let i=!!s.entry_id&&s.entry_id!==r,a=i?s.entry_id:r,l=e.find(g=>g.entry_id===a),u=(l?.parts||[]).find(g=>g.id===s.part_id)||null,h=i&&l?.object?.name||"",_=u?.name||n("shared_part_unknown",t);return{part:u,foreign:i,ownerName:h,label:h?`${_} (${h})`:_}}function Cr(s,r,e,t){let{part:i,label:a}=Ar(s,r,e,t),l=i&&i.stock!==null&&i.stock!==void 0?` (${i.stock}${i.unit?" "+i.unit:""})`:"",u=i?.storage_location?` \u2014 ${i.storage_location}`:"";return`${s.quantity}\xD7 ${a}${l}${u}`}function Ir(s,r,e,t){let a=(e.find(u=>u.entry_id===r)?.parts||[]).map(u=>({...u})),l=new Set(a.map(u=>Q({part_id:u.id})));for(let u of s?.consumes_parts||[]){if(!u.entry_id||u.entry_id===r)continue;let h=Q(u);if(l.has(h))continue;l.add(h);let{part:_,ownerName:g}=Ar(u,r,e,t);a.push({id:u.part_id,name:_?.name||n("shared_part_unknown",t),unit:_?.unit,stock:_?.stock??null,storage_location:_?.storage_location,entry_id:u.entry_id,owner_name:g})}return a}var mt=w(()=>{"use strict";C()});function Je(s){let r=s.task??null,e=r?Xt(r):null,t=e?e.consumesParts:r?.consumes_parts||[],i=!!r?.part_ref,a=s.objects.find(h=>h.entry_id===s.entryId)?.parts||[],l=i?a.find(h=>h.id===r.part_ref.part_id):void 0,u=s.checklistsEnabled??!0;return{entry_id:s.entryId,task_id:s.taskId,task_name:s.taskName,checklist:e?u?e.checklist:[]:s.checklist??[],adaptive_enabled:!!s.adaptiveEnabled,required_completion_fields:e?e.requiredFields:r?.required_completion_fields||[],task_type:r?.type||"",reading_unit:r?.reading_unit||"",readings:r?.readings||[],reading_history:$r(r?.history),parts:i?[]:Ir({consumes_parts:t},s.entryId,s.objects,s.lang),consumes_parts:i?[]:t,phase_label:e?de(r):"",require_tag_scan:!!r?.require_tag_scan,restock_default:i?l?.restock_quantity??1:null,restock_unit_cost:i?l?.cost??null:null,currency_symbol:le({currency_symbol:s.currencySymbol}),consumes_info:t.map(h=>Cr(h,s.entryId,s.objects,s.lang)),checklist_prefill:r?.checklist_progress||{},via_tag_scan:!!s.viaTagScan}}function Ze(s,r,e){s.entryId=r.entry_id,s.taskId=r.task_id,s.taskName=r.task_name,s.lang=e,s.checklist=r.checklist??[],s.adaptiveEnabled=!!r.adaptive_enabled,s.requiredFields=r.required_completion_fields??[],s.taskType=r.task_type??"",s.readingUnit=r.reading_unit??"",s.readings=r.readings??[],s.readingHistory=r.reading_history??[],s.parts=r.parts??[],s.consumesParts=r.consumes_parts??[],s.phaseLabel=r.phase_label??"",s.requireTagScan=!!r.require_tag_scan,s.restockDefault=r.restock_default??null,s.restockUnitCost=r.restock_unit_cost??null,s.currencySymbol=le(r),s.consumesInfo=r.consumes_info??[],s.checklistPrefill=r.checklist_prefill??{},s.viaTagScan=!!r.via_tag_scan,s.open({viaTagScan:!!r.via_tag_scan})}var ft=w(()=>{"use strict";ve();mt();gt();C()});function vn(s){let r=s.replace(/_ids?$/,"").split("_").filter(Boolean);if(r.length===0)return s;let e=r.join(" ");return e.charAt(0).toUpperCase()+e.slice(1)}function bn(s,r){let e=fn[s];if(e){let t=n(e,r);if(t&&t!==e)return t}return vn(s)}function yn(s){let r=[...s.matchAll(/\['([^']+)'\]/g)].map(i=>i[1]),e=r.length?r[r.length-1]:void 0,t;return(t=s.match(/length of value must be at most (\d+)/))?{field:e,rule:"too_long",param:t[1]}:(t=s.match(/length of value must be at least (\d+)/))?{field:e,rule:"too_short",param:t[1]}:(t=s.match(/value must be at most (\S+)/))?{field:e,rule:"value_too_high",param:t[1]}:(t=s.match(/value must be at least (\S+)/))?{field:e,rule:"value_too_low",param:t[1]}:/required key not provided/.test(s)?{field:e,rule:"required"}:(t=s.match(/expected (\w+)/))?{field:e,rule:"wrong_type",param:t[1]}:/value must be one of/.test(s)?{field:e,rule:"invalid_choice"}:/not a valid value/.test(s)?{field:e,rule:"invalid_value"}:{field:e,rule:"unknown"}}function xn(s,r){let e=new Set(Rr(r));return Rr(s).every(t=>e.has(t))}function R(s,r,e){if(e=e??n("action_error",r),typeof s=="string")return s;if(typeof s!="object"||s===null)return e;let t=s,i=t.message||t.error?.message||"",a=t.code||t.error?.code||"",l=a?mn[a]:void 0;if(!i&&!l)return e;let u=yn(i),h=u.field?bn(u.field,r):"",_=g=>n(g,r).replace("{field}",h).replace("{n}",u.param??"");switch(u.rule){case"too_long":return _("err_too_long");case"too_short":return _("err_too_short");case"value_too_high":return _("err_value_too_high");case"value_too_low":return _("err_value_too_low");case"required":return _("err_required");case"wrong_type":return _("err_wrong_type").replace("{type}",u.param??"");case"invalid_choice":return _("err_invalid_choice");case"invalid_value":return _("err_invalid_value");default:if(l){let g=n(l,r);return i&&!xn(i,a)?`${g} (${i})`:g}return i||e}}var mn,fn,Rr,ie=w(()=>{"use strict";C();mn={already_archived:"ws_err_already_archived",already_paused:"ws_err_already_paused",archived:"ws_err_archived",create_failed:"ws_err_create_failed",duplicate_failed:"ws_err_duplicate_failed",empty:"ws_err_empty",empty_csv:"ws_err_empty_csv",invalid_cursor:"ws_err_invalid_cursor",invalid_date:"ws_err_invalid_date",invalid_device:"ws_err_invalid_device",invalid_entity_slug:"ws_err_invalid_entity_slug",invalid_format:"ws_err_invalid_format",invalid_input:"ws_err_invalid_input",invalid_parent:"ws_err_invalid_parent",invalid_range:"ws_err_invalid_range",invalid_target:"ws_err_invalid_target",invalid_trigger_config:"ws_err_invalid_trigger_config",invalid_url:"ws_err_invalid_url",invalid_user:"ws_err_invalid_user",invalid_view:"ws_err_invalid_view",limit_reached:"ws_err_limit_reached",no_defaults:"ws_err_no_defaults",no_phases:"ws_err_no_phases",no_url:"ws_err_no_url",not_archived:"ws_err_not_archived",not_available:"ws_err_not_available",not_configured:"ws_err_not_configured",not_found:"ws_err_not_found",not_loaded:"ws_err_not_loaded",not_paused:"ws_err_not_paused",replace_failed:"ws_err_replace_failed",self_link_device:"ws_err_self_link_device",storage_unavailable:"ws_err_storage_unavailable",too_early:"ws_err_too_early",too_large:"ws_err_too_large",too_many:"ws_err_too_many",too_many_views:"ws_err_too_many_views",unauthorized:"ws_err_unauthorized",unavailable:"ws_err_unavailable"},fn={entry_id:"object",name:"name",task_type:"maintenance_type",schedule_type:"schedule_type",interval_days:"interval_days",interval_anchor:"interval_anchor",warning_days:"warning_days",last_performed:"last_performed_optional",notes:"notes_optional",documentation_url:"documentation_url_optional",custom_icon:"custom_icon_optional",nfc_tag_id:"nfc_tag_id_optional",responsible_user_id:"responsible_user",entity_slug:"entity_slug",entity_id:"entity_id",area_id:"area_id_optional",manufacturer:"manufacturer_optional",model:"model_optional",serial_number:"serial_number_optional",installation_date:"installation_date_optional",warranty_expiry:"warranty_expiry_optional",checklist:"checklist_steps_optional",reason:"reason",feedback:"feedback",cost:"cost",duration:"duration",description:"description_optional",environmental_entity:"environmental_entity_optional",environmental_attribute:"environmental_attribute_optional",trigger_above:"trigger_above",trigger_below:"trigger_below",trigger_equals:"trigger_equals",trigger_not_equals:"trigger_not_equals",trigger_for_minutes:"trigger_for_minutes"};Rr=s=>s.toLowerCase().split(/[^a-z0-9]+/).filter(r=>r.length>1)});var ei,Xe,ti=w(()=>{"use strict";ei=["notes","cost","duration","photo","user"],Xe={notes:"notes_label",cost:"cost",duration:"duration",photo:"photo_label",user:"user_label"}});function vt(s){if(!s)return[];let r=[],e=s.photo_doc_id;typeof e=="string"&&r.push(e);let t=s.photo_doc_ids;Array.isArray(t)&&r.push(...t);let i=[];for(let a of r){if(typeof a!="string")continue;let l=a.trim();if(!(!l||i.includes(l))&&(i.push(l),i.length>=10))break}return i}var bt=w(()=>{"use strict"});async function wn(s,r,e,t){let i=new FormData;i.append("entry_id",r);for(let u of t)i.append("tags",u);i.append("file",e,e.name);let a=await fetch("/api/maintenance_supporter/document/upload",{method:"POST",headers:{Authorization:`Bearer ${s.auth?.data?.access_token??""}`},body:i});if(a.status===413)throw new Error("doc_too_large");if(!a.ok)throw new Error("doc_upload_failed");let l=await a.json();if(!l.id)throw new Error("doc_upload_failed");return{id:l.id,deduped:!!l.deduped,duplicate_in_object:l.duplicate_in_object??null}}async function Lr(s,r,e){return(await wn(s,r,e,["photo"])).id}async function ii(s,r){await Promise.all(r.map(e=>s.connection.sendMessagePromise({type:"maintenance_supporter/documents/delete",doc_id:e}).catch(()=>{})))}var Nr=w(()=>{"use strict";bt()});var Ie,ri=w(()=>{"use strict";C();Nr();Ie=class{constructor(r,e){this.host=r;this.opts=e;this.photos=[];this.uploadedIds=[];this.uploading=!1;this.errorKey="";this.max=e.max??10,r.addController(this)}hostConnected(){}get ids(){return this.photos.map(r=>r.id)}get remaining(){return Math.max(this.max-this.photos.length,0)}get full(){return this.remaining===0}errorText(r){return this.errorKey?n(this.errorKey,r).replace("{max}",String(this.max)):""}clearError(){this.errorKey=""}reset(r=[]){this._revokeAll(),this.photos=r.map(e=>({id:e,preview:""})),this.uploadedIds=[],this.uploading=!1,this.errorKey="",this.host.requestUpdate()}async addFiles(r){if(r.length===0)return;let e=r.slice(0,this.remaining);this.uploading=!0,this.errorKey="",this.host.requestUpdate();try{for(let t of e){let i=await Lr(this.opts.hass(),this.opts.entryId(),t);this.uploadedIds=[...this.uploadedIds,i],this.photos=[...this.photos,{id:i,preview:URL.createObjectURL(t)}],this.host.requestUpdate()}r.length>e.length&&(this.errorKey="photos_limit")}catch(t){this.errorKey=t instanceof Error&&t.message==="doc_too_large"?"doc_too_large":"doc_upload_failed"}finally{this.uploading=!1,this.host.requestUpdate()}}remove(r){let e=this.photos.find(t=>t.id===r);e?.preview&&URL.revokeObjectURL(e.preview),this.photos=this.photos.filter(t=>t.id!==r),this.uploadedIds.includes(r)&&(this.uploadedIds=this.uploadedIds.filter(t=>t!==r),ii(this.opts.hass(),[r])),this.host.requestUpdate()}discardOrphans(){if(this.uploadedIds.length===0)return;let r=this.uploadedIds;this.uploadedIds=[],ii(this.opts.hass(),r)}markAttached(){this.uploadedIds=[]}_revokeAll(){for(let r of this.photos)r.preview&&URL.revokeObjectURL(r.preview)}}});function $n(s){let e=F("2001-02-03",s).match(/\d+/g)||[],t=u=>e.findIndex(h=>Number(h)===u),i=t(2001),a=t(2),l=t(3);return i===0?"YMD":a>=0&&l>=0&&a<l?"MDY":"DMY"}function Re(s,r,e){if(s<1e3||s>9999||r<1||r>12||e<1||e>31)return null;let t=new Date(s,r-1,e);return t.getFullYear()!==s||t.getMonth()!==r-1||t.getDate()!==e?null:`${s}-${String(r).padStart(2,"0")}-${String(e).padStart(2,"0")}`}function jr(s,r){let e=s.trim(),t;if(t=e.match(/^(\d{4})-(\d{1,2})-(\d{1,2})$/))return Re(+t[1],+t[2],+t[3]);if(t=e.match(/^(\d{4})$/))return Re(+t[1],1,1);if(t=e.match(/^(\d{1,2})[./](\d{4})$/))return Re(+t[2],+t[1],1);if(t=e.match(/^(\d{1,2})\.(\d{1,2})\.(\d{4})$/))return Re(+t[3],+t[2],+t[1]);if(t=e.match(/^(\d{1,2})[/-](\d{1,2})[/-](\d{4})$/)){let i=+t[1],a=+t[2],l=+t[3];return $n(r)==="MDY"?Re(l,i,a):Re(l,a,i)}return null}var Hr=w(()=>{"use strict";C()});function kn(s,r){if(r)switch(s){case"date":return r.split("T")[0];case"time":return r.length===5?`${r}:00`:r;case"datetime":{let[e,t="00:00:00"]=r.split("T");return`${e} ${t.length===5?`${t}:00`:t}`}}}function En(s,r){if(typeof r!="string"||!r)return"";switch(s){case"date":return r.slice(0,10);case"time":return r.slice(0,5);case"datetime":{let[e,t="00:00:00"]=r.split(" ");return`${e}T${t.length===5?`${t}:00`:t}`}}}var M,Pe=w(()=>{"use strict";I();O();C();Hr();M=class extends E{constructor(){super(...arguments);this.kind="date";this.label="";this.value="";this.clearable=!1;this.disabled=!1;this.required=!1;this.lang="en";this._typing=!1;this._typed="";this._typedInvalid=!1}_startTyping(){this._typed=this.value,this._typedInvalid=!1,this._typing=!0,this.updateComplete.then(()=>this.shadowRoot?.querySelector("input.typed")?.focus())}_commitTyped(){if(!this._typed.trim()){this._typing=!1;return}let e=jr(this._typed,this.lang);if(!e){this._typedInvalid=!0;return}this._typing=!1,this._typedInvalid=!1,this._emit(e)}_onTypedKey(e){e.key==="Enter"?(e.preventDefault(),this._commitTyped()):e.key==="Escape"&&(e.preventDefault(),this._typing=!1,this._typedInvalid=!1)}_selector(){switch(this.kind){case"date":return{date:{}};case"time":return{time:{no_second:!0}};case"datetime":return{datetime:{}}}}_onSelectorChange(e){e.stopPropagation(),this._emit(En(this.kind,e.detail?.value))}_clear(){this._emit("")}_emit(e){e!==this.value&&(this.value=e,this.dispatchEvent(new CustomEvent("value-changed",{bubbles:!0,composed:!0,detail:{value:e}})))}render(){return o`
      <div class="field">
  ...
  `,d([v({attribute:!1})],M.prototype,"hass",2),d([v()],M.prototype,"kind",2),d([v()],M.prototype,"label",2),d([v()],M.prototype,"value",2),d([v()],M.prototype,"helper",2),d([v({type:Boolean})],M.prototype,"clearable",2),d([v({type:Boolean})],M.prototype,"disabled",2),d([v({type:Boolean})],M.prototype,"required",2),d([v()],M.prototype,"lang",2),d([p()],M.prototype,"_typing",2),d([p()],M.prototype,"_typed",2),d([p()],M.prototype,"_typedInvalid",2);customElements.get("ms-date-field")||customElements.define("ms-date-field",M)});function yt(){if(window.externalApp!==void 0)return!0;let r=typeof navigator<"u"&&navigator.userAgent||"";return/Home Assistant\//.test(r)&&/Android/.test(r)}var si=w(()=>{"use strict"});function qr(){if(!yt())return!1;let s=typeof navigator<"u"?navigator.mediaDevices:void 0;return!!s&&typeof s.getUserMedia=="function"}var Sn,Tn,be,ni=w(()=>{"use strict";I();O();C();si();Sn=1920,Tn=.88,be=class extends E{constructor(){super(...arguments);this.lang="en";this._open=!1;this._busy=!1;this._stream=null}async open(){if(this._open)return;let e=typeof navigator<"u"?navigator.mediaDevices:void 0;if(!e||typeof e.getUserMedia!="function"){this._unavailable("no_media_devices");return}try{this._stream=await e.getUserMedia({video:{facingMode:{ideal:"environment"}},audio:!1})}catch(i){this._unavailable(i instanceof Error?i.name||i.message:String(i));return}await this._preferMainBackCamera(e),this._open=!0,await this.updateComplete;let t=this._video;if(t&&this._stream){t.srcObject=this._stream;try{await t.play()}catch{}}}async _preferMainBackCamera(e){if(typeof e.enumerateDevices!="function")return;let t=[];try{t=await e.enumerateDevices()}catch{return}let i=g=>{let b=/camera2?\s*(\d+)/i.exec(g.label);return b?Number(b[1]):Number.MAX_SAFE_INTEGER},a=t.filter(g=>g.kind==="videoinput"&&g.deviceId&&/back|rear|environment|rück|hinten/i.test(g.label)).sort((g,b)=>i(g)-i(b)),u=this._stream?.getVideoTracks()[0]?.getSettings().deviceId;if(a.length>1&&a[0].deviceId!==u)try{let g=await e.getUserMedia({video:{deviceId:{exact:a[0].deviceId}},audio:!1});for(let b of this._stream?.getTracks()??[])b.stop();this._stream=g}catch{}let h=this._stream?.getVideoTracks()[0],_=h&&typeof h.getCapabilities=="function"?h.getCapabilities():void 0;if(_?.zoom&&typeof _.zoom.min=="number"&&_.zoom.min<1&&(_.zoom.max??1)>=1)try{await h.applyConstraints({advanced:[{zoom:1}]})}catch{}}close(){this._stopStream(),this._open=!1,this._busy=!1}disconnectedCallback(){super.disconnectedCallback(),this._stopStream()}get _video(){return this.shadowRoot?.querySelector("video")??null}_stopStream(){for(let t of this._stream?.getTracks()??[])t.stop();this._stream=null;let e=this._video;e&&(e.srcObject=null)}_unavailable(e){this._stopStream(),this._open=!1,this.dispatchEvent(new CustomEvent("capture-unavailable",{detail:{reason:e},bubbles:!0,composed:!0}))}async _shoot(){let e=this._video;if(!(!e||this._busy)){this._busy=!0;try{let t=e.videoWidth||640,i=e.videoHeight||480,a=Math.min(1,Sn/Math.max(t,i)),l=document.createElement("canvas");l.width=Math.round(t*a),l.height=Math.round(i*a);let u=l.getContext("2d");if(!u)throw new Error("no_canvas");u.drawImage(e,0,0,l.width,l.height);let h=await new Promise(b=>l.toBlob(b,"image/jpeg",Tn));if(!h)throw new Error("no_blob");let _=new Date().toISOString().replace(/[-:]/g,"").slice(0,15),g=new File([h],`photo-${_}.jpg`,{type:"image/jpeg"});this.close(),this.dispatchEvent(new CustomEvent("photo-captured",{detail:{file:g},bubbles:!0,composed:!0}))}catch(t){this._unavailable(t instanceof Error?t.message:String(t))}finally{this._busy=!1}}}render(){if(!this._open)return c;let e=this.lang;return o`
  ...
  `,d([v({type:String})],be.prototype,"lang",2),d([p()],be.prototype,"_open",2),d([p()],be.prototype,"_busy",2);customElements.get("ms-camera-capture")||customElements.define("ms-camera-capture",be)});var xt,W,et=w(()=>{"use strict";I();O();C();si();ni();ni();xt=k`
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          ${this.parts.length?o`<div class="used-parts">
                ...
                ${this.parts.map(i=>{let a=Q({part_id:i.id,entry_id:i.entry_id}),l=this._usedParts[a],u=l!==void 0,h=i.entry_id?{part_id:i.id,quantity:1,entry_id:i.entry_id}:{part_id:i.id,quantity:1};return o`<div class="used-part-row">
                    <label class="used-part-check">
                      <input type="checkbox" .checked=${u}
                        @change=${_=>{let g={...this._usedParts};_.target.checked?g[a]=g[a]||h:delete g[a],this._usedParts=g}} />
                      <span
                    ...
                    ${u?o`<input class="used-part-qty" type="number" min="0.01" max="999" step="0.01"
                          .value=${String(l.quantity)}
                          @input=${_=>{let g=parseFloat(_.target.value);this._usedParts={...this._usedParts,[a]:{...h,quantity:Number.isFinite(g)&&g>=.01?g:1}}}} />`:c}
                  </div>`})}
...
`),partId:A?.part_id||"",partQty:A?.quantity!=null?String(A.quantity):"",reqOverride:g.required_completion_fields!==void 0,reqFields:[...g.required_completion_fields||[]],extraParts:z.filter((H,xe)=>xe!==L).map(H=>({...H})),carry:$}}),this._phaseSeq=[...t.phase_sequence||[]],this._requiredCompletion=[...t.required_completion_fields||[]],this._scheduleTime=t.schedule_time||"",this._scheduleTimeOn=!!t.schedule_time;let l=t.on_complete_action;if(l&&l.service){this._actionService=l.service;let _=l.target?.entity_id;this._actionTargetEntity=Array.isArray(_)?_[0]||"":_||"",this._actionData=l.data&&typeof l.data=="object"?{...l.data}:{},this._actionDataJsonFallback=""}else this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="";let u=t.quick_complete_defaults;this._qcNotes=u?.notes||"",this._qcCost=u?.cost!=null?String(u.cost):"",this._qcDuration=u?.duration!=null?String(u.duration):"",this._qcFeedback=u?.feedback||"";let h=t.adaptive_config||{};if(this._environmentalEntity=h.environmental_entity||"",this._environmentalAttribute=h.environmental_attribute||"",this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute,this._adaptiveEnabled=!!h.enabled,this._adaptiveAlpha=h.ewa_alpha?.toString()??re.alpha,this._adaptiveMin=h.min_interval_days?.toString()??re.min,this._adaptiveMax=h.max_interval_days?.toString()??re.max,this._adaptiveSeasonal=h.seasonal_enabled!==!1,this._adaptivePrediction=h.sensor_prediction_enabled!==!1,this._adaptiveInitial=this._adaptiveSnapshot(),t.trigger_config){let _=t.trigger_config;this._triggerEntityId=_.entity_id||_.entity_ids&&_.entity_ids[0]||"",this._triggerEntityIds=_.entity_ids||(_.entity_id?[_.entity_id]:[]),this._triggerEntityLogic=_.entity_logic||"any",this._triggerAttribute=_.attribute||"",this._triggerType=_.type||"threshold",this._triggerAbove=_.trigger_above?.toString()||"",this._triggerBelow=_.trigger_below?.toString()||"",this._triggerEquals=_.trigger_equals?.toString()||"",this._triggerNotEquals=_.trigger_not_equals?.toString()||"",this._triggerForMinutes=_.trigger_for_minutes?.toString()||"0",this._triggerCombinator=_.trigger_combinator==="all"?"all":"any",this._triggerTargetValue=_.trigger_target_value?.toString()||"",this._triggerDeltaMode=_.trigger_delta_mode||!1,this._triggerBaselineValue=_.trigger_baseline_value?.toString()||"",this._liveBaselineValue=t.trigger_baseline_value??null,this._autoCompleteOnRecovery=_.auto_complete_on_recovery||!1,this._triggerFromState=_.trigger_from_state||"",this._triggerToState=_.trigger_to_state||"",this._triggerTargetChanges=_.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=_.trigger_runtime_hours?.toString()||"",this._triggerRuntimeMaxSession=_.trigger_runtime_max_session_seconds?.toString()||"",this._triggerOnStates=(_.trigger_on_states||[]).join(", "),_.type==="compound"?(this._compoundLogic=_.compound_logic==="OR"?"OR":"AND",this._compoundConditions=(_.conditions||[]).map(jn)):(this._compoundLogic="AND",this._compoundConditions=[])}else this._resetTriggerFields();this._triggerEntityId&&this._fetchEntityAttributes(this._triggerEntityId),await Promise.all([this._loadUsers(),this._loadTags(),this._loadParts(),this._loadForeignPools()]),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._intervalUnit="days",this._dueDate="",this._warningDays=String(this.defaultWarningDays),this._earliestCompletionDays="",this._intervalAnchor="completion",this._weekdays=[],this._nth="1",this._nthWeekday="5",this._domDay="1",this._domLastDay=!1,this._domBusiness=!1,this._calOffset="0",this._seasonMonths=[],this._endsMode="never",this._endsCount="",this._endsUntil="",this._notes="",this._documentationUrl="",this._customIcon="",this._priority="normal",this._labels="",this._enabled=!0,this._lastPerformed="",this._nfcTagId="",this._requireTagScan=!1,this._allowSkip=!0,this._readingUnit="",this._readings=[],this._consumesParts={},this._responsibleUserId=null,this._assigneePool=[],this._rotationStrategy="",this._checklistText="",this._phaseDefs=[],this._phaseSeq=[],this._requiredCompletion=[],this._scheduleTime="",this._scheduleTimeOn=!1,this._environmentalEntity="",this._environmentalAttribute="",this._environmentalInitial="",this._environmentalAttributeInitial="",this._adaptiveEnabled=!1,this._adaptiveAlpha=re.alpha,this._adaptiveMin=re.min,this._adaptiveMax=re.max,this._adaptiveSeasonal=!0,this._adaptivePrediction=!0,this._adaptiveInitial=this._adaptiveSnapshot(),this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="",this._actionTesting=!1,this._actionTestResult="",this._qcNotes="",this._qcCost="",this._qcDuration="",this._qcFeedback="",this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerEquals="",this._triggerNotEquals="",this._triggerForMinutes="0",this._triggerCombinator="any",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerBaselineValue="",this._liveBaselineValue=null,this._autoCompleteOnRecovery=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours="",this._triggerRuntimeMaxSession="",this._triggerOnStates="",this._compoundLogic="AND",this._compoundConditions=[]}async _loadUsers(){this._userService||(this._userService=new Ce(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}_toggleAssignee(e){this._assigneePool=this._assigneePool.includes(e)?this._assigneePool.filter(t=>t!==e):[...this._assigneePool,e]}async _testAction(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e)){this._actionTestResult="error",this._actionTestError="Invalid service format (expected 'domain.service')",setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3);return}let[t,i]=e.split(".");if(!this.hass?.services?.[t]?.[i]){this._actionTestResult="error",this._actionTestError=`Service "${e}" is not registered in Home Assistant. Check spelling and that the integration providing it is loaded.`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}let a=this._actionTargetEntity.trim();if(a){let l=a.split(".")[0];if(l!==t&&!new Set(["homeassistant","scene","notify","persistent_notification"]).has(t)){this._actionTestResult="error",this._actionTestError=`Service "${e}" only works on ${t}.* entities; entity "${a}" is in ${l}.* \u2014 pick a service that matches the entity domain (e.g. ${l}.${i})`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}if(!this.hass.states?.[a]){this._actionTestResult="error",this._actionTestError=`Target entity "${a}" not found in Home Assistant \u2014 the entity may have been renamed or its integration removed.`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}}this._actionTestResult="ok",setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3)}_buildActionData(){if(this._actionDataJsonFallback.trim())try{let e=JSON.parse(this._actionDataJsonFallback);if(e&&typeof e=="object"&&!Array.isArray(e))return e}catch{}return{...this._actionData}}_serviceSchema(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e))return null;let[t,i]=e.split("."),a=this.hass?.services?.[t]?.[i]?.fields;return!a||Object.keys(a).length===0?null:Object.entries(a).map(([l,u])=>({name:l,required:!!u.required,selector:u.selector||{text:{}}}))}_renderCompletionActionsSection(e){if(!this.completionActionsEnabled)return c;let t=this._serviceSchema();return o`
      <details class="ca-section">
        ...
        <ha-service-picker
          ...
          @value-changed=${i=>{this._actionService=i.detail.value||"";let a=this._serviceSchema();if(a){let l=new Set(a.map(u=>u.name));this._actionData=Object.fromEntries(Object.entries(this._actionData).filter(([u])=>l.has(u)))}}}
        ></ha-service-picker>
        <ha-form
          .hass=${this.hass}
      ...
      <div class="consumes-row">
        <label class="consumes-check">
          <input
            type="checkbox"
            .checked=${a!==void 0}
            @change=${u=>{let h={...this._consumesParts};u.target.checked?h[i]=h[i]||l:delete h[i],this._consumesParts=h}}
          />
        ...
        ${a!==void 0?o`<input
              ...
              @input=${u=>{let h=parseFloat(u.target.value);this._consumesParts={...this._consumesParts,[i]:{...l,quantity:Number.isFinite(h)&&h>=.01?h:1}}}}
            />`:c}
      </div>
    `}_toggleRequired(e,t){let i=new Set(this._requiredCompletion);t?i.add(e):i.delete(e),this._requiredCompletion=[...i]}_patchReading(e,t){this._readings=this._readings.map(i=>i.id===e?{...i,...t}:i)}_renderReadingsEditor(e){let t=Sr(this._readings);return o`
      <div class="readings-editor">
        ...
        ${this._readings.length<20?o`
          <ha-button appearance="plain" class="reading-add"
            @click=${()=>this._readings=[...this._readings,{id:xr(),name:"",unit:this._readings.length?this._readings[this._readings.length-1].unit:this._readingUnit}]}>
    ...
    `}_phaseSlug(e){let t=e.toLowerCase().replace(/[^a-z0-9_-]+/g,"-").replace(/^-+|-+$/g,"").slice(0,24)||"phase",i=t,a=2;for(;this._phaseDefs.some(l=>l.id===i);)i=`${t}-${a++}`;return i}_addPhaseDef(){let e=this._phaseSlug(`phase-${this._phaseDefs.length+1}`);this._phaseDefs=[...this._phaseDefs,{id:e,name:"",checklistText:"",partId:"",partQty:"",reqOverride:!1,reqFields:[],extraParts:[],carry:{}}]}_removePhaseDef(e){this._phaseDefs=this._phaseDefs.filter(t=>t.id!==e),this._phaseSeq=this._phaseSeq.filter(t=>t!==e)}_patchPhaseDef(e,t){this._phaseDefs=this._phaseDefs.map(i=>i.id===e?{...i,...t}:i)}_renderPhasesEditor(e){let t=i=>this._phaseDefs.find(a=>a.id===i)?.name||i;return o`
              ...
              ${ei.map(a=>o`
                <label class="req-option">
                  <input
                    ...
                    @change=${l=>{let u=l.target.checked,h=new Set(i.reqFields);u?h.add(a):h.delete(a),this._patchPhaseDef(i.id,{reqFields:[...h]})}}
                  />
                  <span>${n(Xe[a],e)}</span>
                </label>
              `)}
        ...
        <div class="phase-seq">
          ${this._phaseSeq.map((i,a)=>o`
            <span class="phase-chip">
              ${a+1}. ${t(i)}
              <button class="phase-chip-x" @click=${()=>{this._phaseSeq=this._phaseSeq.filter((l,u)=>u!==a)}}>✕</button>
            </span>
          `)}
          <select
            class="phase-seq-add"
      ...
      `:c}
    `}async _save(){if(!this._loading&&this._name.trim()){if(this._adaptiveSnapshot()!==this._adaptiveInitial){let e=parseInt(this._adaptiveMin,10),t=parseInt(this._adaptiveMax,10);if(!isNaN(e)&&!isNaN(t)&&e>t){this._error=`${n("adaptive_min_interval",this._lang)} > ${n("adaptive_max_interval",this._lang)}`;return}}if(this._triggerType==="threshold"&&this._thresholdLimitsOverlap()){this._error=n("trigger_hint_overlap",this._lang);return}this._loading=!0,this._error="";try{let e={type:this._taskId?"maintenance_supporter/task/update":"maintenance_supporter/task/create",entry_id:this._entryId,name:this._name,task_type:this._type,schedule_type:this._scheduleType,warning_days:Number.isNaN(parseInt(this._warningDays,10))?this.defaultWarningDays:Math.max(0,parseInt(this._warningDays,10))},t=this._earliestCompletionDays.trim();e.earliest_completion_days=t===""?null:Math.max(0,parseInt(t,10)||0),this._taskId&&(e.task_id=this._taskId),this._scheduleType==="one_time"?(e.due_date=this._dueDate||null,e.interval_days=null):$t.includes(this._scheduleType)?(e.schedule={...this._buildSchedule(),...this._recurrenceExtras()},e.interval_days=null,this._taskId&&(e.due_date=null)):(this._taskId&&(e.due_date=null),this._scheduleType!=="manual"&&this._intervalDays?(e.interval_days=parseInt(this._intervalDays,10),e.interval_unit=this._intervalUnit,e.interval_anchor=this._intervalAnchor,this._scheduleType==="time_based"&&(e.schedule={kind:"interval",...this._recurrenceExtras()})):this._taskId&&(e.interval_days=null,e.interval_anchor="completion")),e.notes=this._notes||null,e.documentation_url=this._documentationUrl||null,e.custom_icon=this._customIcon||null,e.priority=this._priority,e.labels=this._labels.split(",").map(h=>h.trim()).filter(Boolean),e.enabled=this._enabled,e.last_performed=this._lastPerformed||null,e.nfc_tag_id=this._nfcTagId||null,e.require_tag_scan=this._requireTagScan,e.allow_skip=this._allowSkip,e.notify_enabled=this._notifyEnabled,e.reading_unit=this._readingUnit.trim()||null,e.readings=Tr(this._readings);{let h={};for(let g of this._phaseDefs){if(!g.name.trim())continue;let b={...g.carry,name:g.name.trim()},y=g.checklistText.split(`
`).map(m=>m.trim()).filter(Boolean);y.length&&(b.checklist=y);let x=[];if(g.partId){let m=parseFloat(g.partQty);x.push({part_id:g.partId,quantity:Number.isFinite(m)&&m>0?m:1})}for(let m of g.extraParts)x.push(m.entry_id?{part_id:m.part_id,quantity:m.quantity,entry_id:m.entry_id}:{part_id:m.part_id,quantity:m.quantity});x.length&&(b.consumes_parts=x),g.reqOverride&&(b.required_completion_fields=[...g.reqFields]),h[g.id]=b}let _=this._phaseSeq.filter(g=>g in h);e.phases=Object.keys(h).length&&_.length?h:null,e.phase_sequence=e.phases?_:null}if((this.parts.length||this._foreignOwners.length)&&(e.consumes_parts=Object.values(this._consumesParts).map(h=>h.entry_id?{part_id:h.part_id,quantity:h.quantity,entry_id:h.entry_id}:{part_id:h.part_id,quantity:h.quantity})),e.responsible_user_id=this._responsibleUserId,e.assignee_pool=this._assigneePool,e.required_completion_fields=this._requiredCompletion,e.rotation_strategy=this._assigneePool.length>=2&&this._rotationStrategy?this._rotationStrategy:null,this._scheduleType==="sensor_based"&&this._triggerType==="compound"){let h=this._compoundConditions.map(Hn).filter(_=>_!==null);if(h.length>0){let _={type:"compound",compound_logic:this._compoundLogic,conditions:h};this._autoCompleteOnRecovery&&(_.auto_complete_on_recovery=!0),this._triggerCombinator==="all"&&(_.trigger_combinator="all"),e.trigger_config=_}else this._taskId&&(e.trigger_config=null)}else if(this._scheduleType==="sensor_based"&&this._triggerEntityId){let h=this._triggerEntityIds.length>0?this._triggerEntityIds:[this._triggerEntityId],_={entity_id:h[0],entity_ids:h,type:this._triggerType};if(this._triggerAttribute&&(_.attribute=this._triggerAttribute),this._autoCompleteOnRecovery&&(_.auto_complete_on_recovery=!0),this._triggerCombinator==="all"&&(_.trigger_combinator="all"),h.length>1&&(_.entity_logic=this._triggerEntityLogic),this._triggerType==="threshold"){if(this._triggerAbove){let g=parseFloat(this._triggerAbove);isNaN(g)||(_.trigger_above=g)}if(this._triggerBelow){let g=parseFloat(this._triggerBelow);isNaN(g)||(_.trigger_below=g)}if(this._triggerEquals){let g=parseFloat(this._triggerEquals);isNaN(g)||(_.trigger_equals=g)}if(this._triggerNotEquals){let g=parseFloat(this._triggerNotEquals);isNaN(g)||(_.trigger_not_equals=g)}if(this._triggerForMinutes){let g=parseInt(this._triggerForMinutes,10);isNaN(g)||(_.trigger_for_minutes=g)}}else if(this._triggerType==="counter"){if(this._triggerTargetValue){let g=parseFloat(this._triggerTargetValue);isNaN(g)||(_.trigger_target_value=g)}if(_.trigger_delta_mode=this._triggerDeltaMode,this._triggerDeltaMode&&this._triggerBaselineValue){let g=parseFloat(this._triggerBaselineValue);!isNaN(g)&&g>=0&&(_.trigger_baseline_value=g)}}else if(this._triggerType==="state_change"){if(this._triggerFromState&&(_.trigger_from_state=this._triggerFromState),this._triggerToState&&(_.trigger_to_state=this._triggerToState),this._triggerTargetChanges){let g=parseInt(this._triggerTargetChanges,10);isNaN(g)||(_.trigger_target_changes=g)}if(this._triggerForMinutes){let g=parseInt(this._triggerForMinutes,10);isNaN(g)||(_.trigger_for_minutes=g)}}else if(this._triggerType==="runtime"){if(this._triggerRuntimeHours){let b=parseFloat(this._triggerRuntimeHours);isNaN(b)||(_.trigger_runtime_hours=b)}if(this._triggerRuntimeMaxSession){let b=parseInt(this._triggerRuntimeMaxSession,10);!isNaN(b)&&b>0&&(_.trigger_runtime_max_session_seconds=b)}let g=this._triggerOnStates.split(",").map(b=>b.trim()).filter(Boolean);g.length>0&&(_.trigger_on_states=g)}e.trigger_config=_}else this._taskId&&(e.trigger_config=null);if(this.scheduleTimeEnabled&&Ur.includes(this._scheduleType)){let h=this._scheduleTimeOn?this._scheduleTime.trim():"";e.schedule_time=/^([01]\d|2[0-3]):[0-5]\d$/.test(h)?h:null}if(this.checklistsEnabled){let h=this._checklistText.split(`
`).map(_=>_.trim()).filter(Boolean).slice(0,100);e.checklist=h.length?h:null}if(this.completionActionsEnabled){let h=this._actionService.trim();if(h&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(h)){let y={service:h},x=this._actionTargetEntity.trim();x&&(y.target={entity_id:x});let m=this._buildActionData();Object.keys(m).length>0&&(y.data=m),e.on_complete_action=y}else e.on_complete_action=null;let _={};this._qcNotes.trim()&&(_.notes=this._qcNotes.trim());let g=parseFloat(this._qcCost);!isNaN(g)&&g>=0&&(_.cost=g);let b=parseInt(this._qcDuration,10);!isNaN(b)&&b>=0&&(_.duration=b),this._qcFeedback&&(_.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(_).length?_:null}let i=await this.hass.connection.sendMessagePromise(e),a=this._taskId||i?.task_id,l=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;this._warning="";let u=h=>{this._warning=n("subsave_warning",this._lang).replace("{detail}",h)};if(a&&this._scheduleType==="sensor_based"&&l&&await hi(this,{type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:a,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null},{onError:u})!==void 0&&(this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute),a&&this._adaptiveSnapshot()!==this._adaptiveInitial){let h=parseFloat(this._adaptiveAlpha),_=parseInt(this._adaptiveMin,10),g=parseInt(this._adaptiveMax,10);await hi(this,{type:"maintenance_supporter/task/set_adaptive",entry_id:this._entryId,task_id:a,enabled:this._adaptiveEnabled,...h>=.1&&h<=.9?{ewa_alpha:h}:{},...!isNaN(_)&&_>=1?{min_interval_days:_}:{},...!isNaN(g)&&g>=1?{max_interval_days:g}:{},seasonal_enabled:this._adaptiveSeasonal,sensor_prediction_enabled:this._adaptivePrediction},{onError:u})!==void 0&&(this._adaptiveInitial=this._adaptiveSnapshot())}this._warning?a&&(this._taskId=a):this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=R(e,this._lang,n("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1,this._warning="",this._pickerProbeTimer!==void 0&&(clearTimeout(this._pickerProbeTimer),this._pickerProbeTimer=void 0),this._pickerProbeStrikes=0}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return c;let e=this._lang,t=this._triggerType==="compound";return o`
      ...
      <div class="select-row">
        ...
        >
          ${Pn.map(i=>o`<option value=${i} ?selected=${i===this._triggerType}>${n(i,e)}</option>`)}
        </select>
      </div>
      ${t?this._renderCompoundEditor():o`
        ${this._entityPickerFallback?o`
          <ms-textfield
            ...
            @input=${i=>{let l=i.target.value.split(",").map(u=>u.trim()).filter(Boolean);this._triggerEntityId=l[0]||"",this._triggerEntityIds=l,l[0]&&this._fetchEntityAttributes(l[0])}}
          ></ms-textfield>
        `:o`
        <ha-form
          class="entity-picker-form"
          .hass=${this.hass}
          .schema=${[{name:"trigger_entities",selector:{entity:{multiple:!0,domain:pi}}}]}
    ...
    `}_renderCondition(e,t){let i=this._lang,a=t+1;return o`
      <div class="compound-condition">
        ...
        <ha-form
          class="entity-picker-form"
          .hass=${this.hass}
          .schema=${[{name:"condition_entities",selector:{entity:{multiple:!0,domain:pi}}}]}
          .data=${{condition_entities:e.entityIds.split(",").map(l=>l.trim()).filter(Boolean)}}
          .computeLabel=${()=>n("entity_id",i)}
          @value-changed=${l=>{let u=(l.detail.value.condition_entities||[]).filter(Boolean);this._patchCondition(t,{entityIds:u.join(", ")})}}
        ></ha-form>`}
        ...
        <div class="select-row">
          ...
          >
            ${Br.map(l=>o`<option value=${l} ?selected=${l===e.type}>${n(l,i)}</option>`)}
          </select>
        </div>
    ...
    `}_renderEnvironmentalAttribute(e){this._fetchConditionAttributes(this._environmentalEntity);let t=this._conditionAttrOptions[this._environmentalEntity];return this._renderAttributeSelect({label:n("environmental_attribute_optional",e),value:this._environmentalAttribute,suggested:t?.suggested??[],available:t?.available??[],onSelect:i=>this._environmentalAttribute=i})}_renderConditionAttribute(e,t){let i=e.entityIds.split(",")[0]?.trim()||"";i&&this._fetchConditionAttributes(i);let a=i?this._conditionAttrOptions[i]:void 0;return this._renderAttributeSelect({label:n("attribute_optional",this._lang),value:e.attribute,suggested:a?.suggested??[],available:a?.available??[],onSelect:l=>this._patchCondition(t,{attribute:l})})}_renderConditionTypeFields(e,t){let i=this._lang;if(e.type==="threshold")return o`
      ...
      </div>`}_toggleWeekday(e){this._weekdays=this._weekdays.includes(e)?this._weekdays.filter(t=>t!==e):[...this._weekdays,e]}_previewScheduleDict(){if(this._scheduleType==="one_time")return this._dueDate?{kind:"one_time",due_date:this._dueDate}:null;if($t.includes(this._scheduleType))return{...this._buildSchedule(),...this._recurrenceExtras()};let e=parseInt(this._intervalDays,10);return this._scheduleType==="manual"||!e||e<=0?null:{kind:"interval",every:e,unit:this._intervalUnit,anchor:this._intervalAnchor,...this._recurrenceExtras()}}updated(e){super.updated?.(e),this._scheduleEntityPickerProbe();for(let t of e.keys())if(f._PREVIEW_RELEVANT.has(String(t))){this._schedulePreviewRefresh();return}}_scheduleEntityPickerProbe(){this._entityPickerFallback||this._pickerProbeTimer!==void 0||!this._open||this._scheduleType!=="sensor_based"||(this._pickerProbeTimer=setTimeout(()=>this._probeEntityPickers(),1500))}_probeEntityPickers(){if(this._pickerProbeTimer=void 0,this._entityPickerFallback||!this._open)return;let e=this.shadowRoot?.querySelector("ha-form.entity-picker-form"),t=(this.shadowRoot?.querySelector(".content")?.offsetHeight??0)>0;if(!e||!t){this._pickerProbeStrikes=0;return}let i=(h,_,g=0)=>{if(!(!h||g>10)){(h.tagName?.toLowerCase()??"")==="ha-entity-picker"&&_.push(h);for(let b of[h.shadowRoot,h])if(b)for(let y of Array.from(b.children??[]))i(y,_,g+1)}},a=[...this.shadowRoot?.querySelectorAll("ha-form.entity-picker-form")??[]],l=[];for(let h of a)i(h,l);let u=l.length===0||l.some(h=>h.offsetHeight===0);if(e.offsetHeight===0||u){if(this._pickerProbeStrikes+=1,this._pickerProbeStrikes>=2){this._entityPickerFallback=!0;return}this._pickerProbeTimer=setTimeout(()=>this._probeEntityPickers(),700)}else this._pickerProbeStrikes=0}_schedulePreviewRefresh(){this._previewTimer&&clearTimeout(this._previewTimer),this._previewTimer=setTimeout(()=>{this._fetchSchedulePreview()},300)}async _fetchSchedulePreview(){let e=this._open?this._previewScheduleDict():null;if(!e){this._schedulePreview=[],this._schedulePreviewEnded=!1;return}let t=++this._previewSeq;try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/schedule/preview",schedule:e,...this._lastPerformed?{last_performed:this._lastPerformed}:{}});if(t!==this._previewSeq)return;this._schedulePreview=i.occurrences||[],this._schedulePreviewEnded=!!i.series_ended}catch{}}_renderSchedulePreview(){if(this._schedulePreview.length===0)return c;let e=this._lang,t=this.scheduleTimeEnabled&&this._scheduleTimeOn&&this._scheduleTime?` ${this._scheduleTime}`:"",i=this._schedulePreview.map((l,u)=>{let h=new Date(`${l}T12:00:00`).getDay();return`${Ye(h===0?6:h-1,e,"short")} ${F(l,e)}${u===0?t:""}`}).join(" \xB7 "),a=this._scheduleType==="time_based"&&this._intervalAnchor==="completion"?o`<div class="field-help">${n("schedule_preview_ontime",e)}</div>`:c;return o`
    ...
    `}_buildSchedule(){let e=i=>{let a=parseInt(this._calOffset,10)||0;return a&&(i.offset=Math.max(-15,Math.min(a,15))),i};if(this._scheduleType==="weekdays")return e({kind:"weekdays",weekdays:[...this._weekdays].sort((i,a)=>i-a)});if(this._scheduleType==="nth_weekday")return e({kind:"nth_weekday",nth:parseInt(this._nth,10),weekday:parseInt(this._nthWeekday,10)});let t={kind:"day_of_month",day:this._domLastDay?-1:parseInt(this._domDay,10)||1};return this._domBusiness&&(t.business=!0),e(t)}_recurrenceExtras(){let e={};if(this._seasonMonths.length&&(e.season_months=[...this._seasonMonths].sort((t,i)=>t-i)),this._endsMode==="count"){let t=parseInt(this._endsCount,10);t>=1&&(e.ends={count:t})}else this._endsMode==="until"&&this._endsUntil&&(e.ends={until:this._endsUntil});return e}_toggleSeasonMonth(e){this._seasonMonths=this._seasonMonths.includes(e)?this._seasonMonths.filter(t=>t!==e):[...this._seasonMonths,e]}_renderRecurrenceExtras(){let e=this._lang;if(!(this._scheduleType==="time_based"||$t.includes(this._scheduleType)))return c;let i=On(e);return o`
      ...
      ${this._endsMode==="until"?o`
        ...
        ></ms-date-field>`:c}
    `}_renderCalendarFields(){let e=this._lang,t=qn(e);if(this._scheduleType==="weekdays")return o`
      ...
      ></ms-textfield>`}_thresholdLimitsOverlap(){let e=parseFloat(this._triggerAbove),t=parseFloat(this._triggerBelow);return!isNaN(e)&&!isNaN(t)&&t>e}_renderTriggerLiveHint(){if(this._triggerType==="compound")return c;let e=this._triggerType==="threshold"&&this._thresholdLimitsOverlap()?o`<div class="trigger-live-hint warn">${n("trigger_hint_overlap",this._lang)}</div>`:c,t=this._triggerEntityId||this._triggerEntityIds[0];if(!t||!this.hass?.states)return e;let i=this.hass.states[t];if(!i)return e;let a=this._lang,l=i.attributes?.unit_of_measurement,u=typeof l=="string"&&l?` ${l}`:"",h=this._triggerAttribute?i.attributes?.[this._triggerAttribute]:i.state,_=typeof h=="number"?h:parseFloat(String(h)),g=h!=="unknown"&&h!=="unavailable"&&h!=null&&!isNaN(_),b=x=>q(x,a,{maximumFractionDigits:1}),y=[];if(this._triggerType==="threshold"){let x=parseFloat(this._triggerAbove),m=parseFloat(this._triggerBelow);if(isNaN(x)&&isNaN(m))return c;g&&y.push(n("trigger_hint_now",a).replace("{value}",b(_)+u)),isNaN(x)||y.push(n("trigger_hint_above",a).replace("{target}",b(x)+u)),isNaN(m)||y.push(n("trigger_hint_below",a).replace("{target}",b(m)+u))}else if(this._triggerType==="counter"){let x=parseFloat(this._triggerTargetValue);if(isNaN(x))return c;this._triggerDeltaMode?this._taskId?y.push(n("trigger_hint_counter_delta_edit",a).replace("{target}",b(x)+u)):g?y.push(n("trigger_hint_counter_delta",a).replace("{value}",b(_)+u).replace("{due}",b(_+x)+u).replace("{target}",b(x)+u)):y.push(n("trigger_hint_counter_delta_edit",a).replace("{target}",b(x)+u)):(g&&y.push(n("trigger_hint_now",a).replace("{value}",b(_)+u)),y.push(n("trigger_hint_counter_abs",a).replace("{target}",b(x)+u)))}else if(this._triggerType==="runtime"){let x=parseFloat(this._triggerRuntimeHours);if(isNaN(x))return c;y.push(n("trigger_hint_runtime",a).replace("{hours}",b(x))),y.push(n("trigger_hint_state_now",a).replace("{value}",String(i.state)))}else if(this._triggerType==="state_change"){let x=parseInt(this._triggerTargetChanges,10)||1,m=this._triggerToState.trim();y.push((m?n("trigger_hint_state_change_to",a).replace("{state}",m):n("trigger_hint_state_change",a)).replace("{count}",String(x))),y.push(n("trigger_hint_state_now",a).replace("{value}",String(i.state)))}return y.length?o`<div class="trigger-live-hint">${y.join(" ")}</div>${e}`:e}_renderTriggerTypeFields(){let e=this._lang;return this._triggerType==="threshold"?o`
        <ms-textfield
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${Cn.map(i=>o`<option value=${i} ?selected=${i===this._type}>${n(i,e)}</option>`)}
            </select>
          </div>
          ...
          <div class="select-row">
            ...
            >
              ${In.map(i=>o`<option value=${i} ?selected=${i===this._priority}>${n("priority_"+i,e)}</option>`)}
            </select>
          </div>
          <div class="field">
          ...
          <div class="select-row">
            ...
            >
              ${Rn.map(i=>o`<option value=${i} ?selected=${i===this._scheduleType}>${n(i,e)}</option>`)}
            </select>
          </div>
          ...
          ${this._scheduleType==="one_time"?o`
                ...
                ></ms-date-field>
              `:c}
          ${this.scheduleTimeEnabled&&Ur.includes(this._scheduleType)?o`
          ...
          ${this._scheduleType==="sensor_based"?o`
            ...
            <ha-form
              class="entity-picker-form"
              .hass=${this.hass}
              .schema=${[{name:"environmental_entity",selector:{entity:{domain:Dr,device_class:Mr}}}]}
    ...
    `}};f._PREVIEW_RELEVANT=new Set(["_open","_scheduleType","_intervalDays","_intervalUnit","_intervalAnchor","_dueDate","_weekdays","_nth","_nthWeekday","_domDay","_domLastDay","_domBusiness","_calOffset","_seasonMonths","_endsMode","_endsCount","_endsUntil","_lastPerformed"]),f.styles=k`
    .dialog-title {
      font-size: 18px;
      font-weight: 500;
      padding-bottom: 12px;
      ...
      <div class="dialog" role="dialog" aria-modal="true">
        ...
        ${this._partOptions&&this._partOptions.length>0?o`
          <div class="parts-block">
            ...
            ${this._partOptions.map(a=>{let l=`${a.entry_id}:${a.part_id}`,u=this._partQty[l]||0;return o`
                <label class="part-row-edit">
                  <input type="checkbox" .checked=${u>0}
                    @change=${h=>{let _=h.target.checked;this._partQty={...this._partQty,[l]:_?1:0}}} />
                  ...
                  ${u>0?o`
                    <input class="part-qty" type="number" min="0.01" max="999" step="0.01"
                      .value=${String(u)}
                      @input=${h=>{let _=parseFloat(h.target.value);!isNaN(_)&&_>0&&(this._partQty={...this._partQty,[l]:_})}} />
                  `:c}
                </label>
              `})}
          </div>
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `,d([v({attribute:!1})],K.prototype,"hass",2),d([v()],K.prototype,"lang",2),d([p()],K.prototype,"_open",2),d([p()],K.prototype,"_loading",2),d([p()],K.prototype,"_error",2),d([p()],K.prototype,"_viewResult",2),d([p()],K.prototype,"_completeResult",2),d([p()],K.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",K)});var Gr=w(()=>{"use strict"});function Qr(s){let r=s.getFullYear(),e=String(s.getMonth()+1).padStart(2,"0"),t=String(s.getDate()).padStart(2,"0");return`${r}-${e}-${t}`}var Jr=w(()=>{"use strict";Gr()});function Zr(s,r,e,t){let i=e,a=h=>typeof h=="string"?h:null,l=h=>typeof h=="number"?h:null,u=a(i.timestamp)??"";return{entry_id:s,task_id:r,original_timestamp:u,type:a(i.type)||"completed",timestamp:u,notes:a(i.notes),cost:l(i.cost),duration:l(i.duration),completed_by:a(i.completed_by),used_parts:Array.isArray(i.used_parts)?i.used_parts:null,photo_doc_ids:vt(i),reading_value:l(i.reading_value),reading_values:ce(i),readings:t?.readings??[],task_type:t?.type??null,reading_unit:t?.reading_unit??null}}var Xr=w(()=>{"use strict";bt();ve()});function Mn(s){return s&&es(s.ref_no)?String(s.ref_no):null}function ts(s,r){let e=Mn(s);return e&&r&&es(r.ref_no)?`${e}.${r.ref_no}`:null}function is(s,r){return s?o`<span class="ref-chip" title=${r??""}>#${s}</span>`:c}var es,gi=w(()=>{"use strict";I();es=s=>typeof s=="number"&&Number.isInteger(s)&&s>0});var rs=w(()=>{"use strict"});function Fn(s,r){return s.taskRef&&r.ref_no?`${s.taskRef}-${r.ref_no}`:null}function Un(s,r){let e=vt(r);return e.length>0?o`<div class="history-photos">
        ${e.map(t=>o`<maintenance-history-photo .hass=${s} .docId=${t}></maintenance-history-photo>`)}
      </div>`:c}function Bn(s,r){let e=r.lang,t=l=>q(l,e,{maximumFractionDigits:3}),i=l=>l==null?"":` (${l>=0?"+":""}${t(l)})`,a=ce(s);return a.length>0?o`<div class="history-readings">
    ...
    </span></div>`:c}function ss(s,r,e={}){let t=r.lang,{compact:i=!1,showRef:a=!0,showBadges:l=!0,showEdit:u=!0}=e,h=u&&zn.includes(s.type);return o`
    <div class="history-entry${i?" compact":""}">
      ${i?c:o`<div class="history-icon ${s.type}">
            <ha-icon .icon=${zt[s.type]||"mdi:circle"}></ha-icon>
          </div>`}
      <div class="history-content">
        <div class="history-row">
          <strong>${n(s.type,t)}</strong>
          ${a?is(Fn(r,s),n("ref_number",t)):c}
        ...
        <div class="history-date">${ur(s.timestamp,t)}</div>
        ${s.notes?o`<div>${s.notes}</div>`:c}
        ${Un(r.hass,s)}
        ${Bn(s,r)}
        ${s.cost!=null||s.duration!=null||s.trigger_value!=null?o`<div class="history-details">
              ${s.cost!=null?o`<span>${n("cost",t)}: ${Te(s.cost,r.currencySymbol,t)}</span>`:c}
              ${s.duration!=null?o`<span>${n("duration",t)}: ${pt(s.duration,t)}</span>`:c}
              ${s.trigger_value!=null?o`<span>${n("trigger_val",t)}: ${s.trigger_value}</span>`:c}
            </div>`:c}
      </div>
    </div>
  `}var zn,ns=w(()=>{"use strict";I();C();_i();bt();gi();ve();rs();zn=["completed","reset","skipped"]});function as(s,r){let e=s.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return c;let a=s.interval_days??0,l=s.suggested_interval??a;return o`
      ...
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_weibull",i)}">
        ${Ss.map(Y=>{let G=136-Y*128;return ke`
            ...
            <text x="${28}" y="${T(G+3)}" fill="var(--secondary-text-color)"
              font-size="8" text-anchor="end">${q(Y*100,i,0)}%</text>
          `})}
        ...
        ${e>0?ke`
          ...
          <circle cx="${T(A)}" cy="${T(xe)}" r="3"
            fill="var(--primary-color, #03a9f4)" />
          <text x="${T(A+4)}" y="${T(xe-6)}" fill="var(--primary-color, #03a9f4)"
            font-size="9" font-weight="600">R=${Es}%</text>
        `:c}

        ${t>0&&t!==e?ke`
          <line x1="${T(Si)}" y1="${8}" x2="${T(Si)}" y2="${T(136)}"
    ...
    <div class="weibull-info-row">
      ...
      ${s.weibull_r_squared!=null?o`
        <div class="weibull-info-item">
          ...
          <span class="weibull-info-value">${q(s.weibull_r_squared,r,3)}</span>
        </div>
      `:c}
    </div>
  `}function Yn(s,r,e){let t=s.confidence_interval_low,i=s.confidence_interval_high,a=r.suggested_interval??r.interval_days??0,l=r.interval_days??0,u=Math.max(0,t-5),_=i+5-u,g=(t-u)/_*100,b=(i-t)/_*100,y=(a-u)/_*100,x=l>0?(l-u)/_*100:-1;return o`
    <div class="confidence-range">
      <div class="confidence-range-title">
        ${n("confidence_interval",e)}: ${a} ${n("days",e)} (${t}\u2013${i})
      </div>
      <div class="confidence-bar">
        <div class="confidence-fill" style="left:${T(g)}%;width:${T(b)}%"></div>
        ${x>=0?o`<div class="confidence-marker current" style="left:${T(x)}%"></div>`:c}
        <div class="confidence-marker recommended" style="left:${T(y)}%"></div>
      </div>
      ...
      <div class="prediction-grid">
        ${t?o`
          <div class="prediction-item">
            ...
            ${s.degradation_rate!=null?o`<span class="prediction-rate">${s.degradation_rate>0?"+":""}${q(s.degradation_rate,r,Math.abs(s.degradation_rate)>=10?0:1)} ${s.trigger_entity_info?.unit_of_measurement||""}/${n("day_short",r)}</span>`:c}
          </div>
        `:c}
        ${i?o`
          <div class="prediction-item">
        ...
        ${a&&e.environmental?o`
          <div class="prediction-item">
            ...
            <span class="prediction-value">${q(s.environmental_factor,r,2)}x</span>
            ${s.environmental_entity?o`<span class="prediction-entity entity-link" @click=${u=>mr(u,s.environmental_entity)}>${s.environmental_entity}</span>`:c}
          </div>
        `:c}
      </div>
    </div>
  `}var ds=w(()=>{"use strict";I();C()});function cs(s,r,e,t){let i=Math.max(s||1,r);return o`
  ...
  `}var ps=w(()=>{"use strict";I();C()});function us(s,r,e){if(!e.seasonal||!s.seasonal_factor||s.seasonal_factor===1)return c;let t=hs.map(u=>n(u,r)),i=new Date().getMonth(),a=s.seasonal_factors||s.interval_analysis?.seasonal_factors||null,l=a&&a.length===12?a:t.map((u,h)=>{let _=s.seasonal_factor||1,g=Math.sin((h-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,_+g))});return o`
    <div class="seasonal-card-compact">
      <h4>${n("seasonal_awareness",r)}</h4>
      <div class="seasonal-mini-chart">
        ${l.map((u,h)=>{let _=u*40,g=u<.9?"low":u>1.1?"high":"normal";return o`
  ...
  `}function _s(s,r){return Gn(s,r)}function Gn(s,r){let e=s.seasonal_factors??s.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return c;let t=s.interval_analysis?.seasonal_reason,i=new Date().getMonth(),a=300,l=100,u=8,_=l-u-4,g=Math.max(...e,1.5),b=a/12,y=b*.65,x=u+_-1/g*_;return o`
  ...
  `}var hs,gs=w(()=>{"use strict";I();C();wt();hs=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"]});var j,ms=w(()=>{"use strict";I();O();C();ie();Jr();ft();gt();Xr();ve();gi();ns();os();ds();ps();Pe();gs();j=class extends E{constructor(){super(...arguments);this._open=!1;this._entryId=null;this._taskId=null;this._task=null;this._objectName="";this._taskRef=null;this._busy=!1;this._error="";this._showSkip=!1;this._showReset=!1;this._showDetails=!1;this._showAdaptive=!1;this._skipReason="";this._resetDate="";this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._toast="";this._featuresLoaded=!1;this._currencySymbol=""}get _lang(){return P(this.hass)}async openFor(e,t){this._entryId=e,this._taskId=t,this._error="",this._showSkip=!1,this._showReset=!1,this._showAdaptive=!1,this._skipReason="",this._resetDate=Qr(new Date),this._open=!0,await Promise.all([this._loadTask(),this._loadFeatures()])}async _loadFeatures(){if(!this._featuresLoaded)try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});e?.features&&(this._features={...this._features,...e.features}),this._currencySymbol=le(e?.budget),ct(e?.budget),this._featuresLoaded=!0}catch{}}close(){this._open=!1,this._task=null,this._error=""}async _loadTask(){if(!(!this._entryId||!this._taskId))try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._objectName=e.object?.name||"";let t=(e.tasks||[]).find(i=>i.id===this._taskId);this._task=t??null,this._taskRef=ts(e.object,t)}catch(e){this._error=R(e,this._lang)}}async _runWs(e){this._busy=!0,this._error="";try{return await this.hass.connection.sendMessagePromise(e),this._busy=!1,!0}catch(t){return this._error=R(t,this._lang),this._busy=!1,!1}}_notifyChanged(e){this.dispatchEvent(new CustomEvent("task-action-fired",{detail:{entry_id:this._entryId,task_id:this._taskId,action:e},bubbles:!0,composed:!0}))}_onComplete(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(Z(),se)).then(async({openCompleteDialog:e})=>{let t=this._task,i=[];try{i=(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects",compact:!0})).objects||[]}catch{}e(Je({entryId:this._entryId,taskId:this._taskId,taskName:t.name,task:t,objects:i,lang:this._lang,checklist:t.checklist||[],adaptiveEnabled:!!t.adaptive_config?.enabled,currencySymbol:this._currencySymbol}))&&(this._notifyChanged("complete"),this.close())})}async _onSkipConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/skip",entry_id:this._entryId,task_id:this._taskId,reason:this._skipReason.trim()||null})&&(this._notifyChanged("skip"),this.close())}async _onResetConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/reset",entry_id:this._entryId,task_id:this._taskId,date:this._resetDate||void 0})&&(this._notifyChanged("reset"),this.close())}_onEdit(){!this._entryId||!this._taskId||Promise.resolve().then(()=>(Z(),se)).then(({openEditTaskDialog:e})=>{e(this._entryId,this._taskId),this.close()})}_onQr(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(Z(),se)).then(({openQrDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,object_name:this._objectName}),this.close()})}async _onDelete(){if(!this._entryId||!this._taskId)return;let e=n("delete_task_confirm",this._lang)||`Delete "${this._task?.name}"?`;if(!window.confirm(e))return;await this._runWs({type:"maintenance_supporter/task/delete",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("delete"),this.close())}async _onArchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/archive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("archive"),this.close())}async _onUnarchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/unarchive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("unarchive"),this.close())}_onOpenInPanel(){if(!this._entryId||!this._taskId)return;let e=`/maintenance-supporter?entry_id=${encodeURIComponent(this._entryId)}&task_id=${encodeURIComponent(this._taskId)}`;history.pushState(null,"",e),window.dispatchEvent(new CustomEvent("location-changed")),this.close()}async _applySuggestion(){if(!this._entryId||!this._taskId||!this._task?.suggested_interval)return;await this._runWs({type:"maintenance_supporter/task/apply_suggestion",entry_id:this._entryId,task_id:this._taskId,interval:this._task.suggested_interval})&&(this._toast=n("suggestion_applied",this._lang),this._notifyChanged("apply_suggestion"),await this._loadTask(),setTimeout(()=>{this._toast=""},2500))}async _reanalyzeInterval(){if(!(!this._entryId||!this._taskId)){this._busy=!0,this._error="";try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:this._entryId,task_id:this._taskId});this._toast=e.recommended_interval?`${n("reanalyze_result",this._lang)}: ${Ke(e.recommended_interval,"days",this._lang)} (${e.data_points} pts)`:n("reanalyze_insufficient_data",this._lang),await this._loadTask(),setTimeout(()=>{this._toast=""},3500)}catch(e){this._error=R(e,this._lang)}finally{this._busy=!1}}}_onEditHistoryEntry(e){if(!this._entryId||!this._taskId)return;let t=Zr(this._entryId,this._taskId,e,this._task);Promise.resolve().then(()=>(Z(),se)).then(({openHistoryEditDialog:i})=>i(t))}_renderRecommendation(e){if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return c;let t=this._lang;return o`
    ...
    `}_renderAdaptive(e){let t=this._lang,i=this._features.adaptive&&e.suggested_interval&&e.suggested_interval!==e.interval_days,a=e.degradation_trend!=null&&e.degradation_trend!=="insufficient_data"||e.days_until_threshold!=null||e.environmental_factor!=null&&e.environmental_factor!==1,l=this._features.adaptive&&e.interval_analysis?.weibull_beta!=null&&e.interval_analysis?.weibull_eta!=null,u=this._features.seasonal&&e.seasonal_factor&&e.seasonal_factor!==1;return!i&&!a&&!l&&!u?o`<div class="adaptive-empty">
      ...
      <div class="adaptive-stack">
        ...
        ${a?ls(e,t,this._features):c}
        ${l?as(e,t):c}
        ${u?o`
          ${us(e,t,this._features)}
          ${e.seasonal_factors?.length===12||e.interval_analysis?.seasonal_factors?.length===12?_s(e,t):c}
        `:c}
      </div>
    `}_renderDetails(e){let t=this._lang,i=e.history||[],a=i.filter(h=>h.type==="completed"),l=a.reduce((h,_)=>h+(typeof _.cost=="number"?_.cost:0),0),u=(()=>{let h=a.map(_=>typeof _.duration=="number"?_.duration:null).filter(_=>_!=null);return h.length?Math.round(h.reduce((_,g)=>_+g,0)/h.length):null})();return o`
      <div class="details">
        <div class="stats-grid">
          ...
          <div class="stat">
            ...
            <span class="stat-value">${Te(l,this._currencySymbol,t)}</span>
          </div>
          <div class="stat">
            <span class="stat-label">${n("avg_duration",t)}</span>
            <span class="stat-value">${pt(u,t)}</span>
          </div>
        </div>
        ...
        ${i.length===0?o`<div class="history-empty">${n("history_empty",t)}</div>`:o`
              <div class="history-list">
                ${[...i].reverse().slice(0,20).map(h=>ss(h,{lang:t,hass:this.hass,currencySymbol:this._currencySymbol,openEdit:_=>this._onEditHistoryEntry(_),readingUnit:e.reading_unit,readingSlotDelta:(_,g)=>Er(i,_,g),taskRef:this._taskRef},{compact:!0}))}
        ...
        ${t?o`
              <div class="header">
                ...
                <div class="quick-info">
                  ...
                  ${t.schedule?.kind&&!["manual","one_time"].includes(t.schedule.kind)||t.interval_days!=null?o`<span><strong>${n("interval",e)}:</strong> ${gr(t,e)}</span>`:c}
                  ${de(t)?o`<span><strong>${n("phase_current",e)}:</strong> ${de(t)}</span>`:c}
                </div>
              </div>
        ...
        ${t&&i?o`
              ...
              <div class="tasks-section">
                ...
                ${a.length===0?o`<div class="empty">${n("no_tasks",e)}</div>`:o`
                      <div class="task-list">
                        ${a.map(u=>o`
                          <div class="task-row" @click=${()=>this._onTaskClick(u.id)}>
                            <span class="status-dot" style="background: ${me[u.status]||"#ccc"}"></span>
                            <span class="task-name">${u.name}</span>
                            <span class="task-status">${n(u.status||"ok",e)}</span>
                          </div>
                        `)}
                      </div>
                    `}
              ...
              ${i.notes?o`
                    <div class="notes-section">
                      ...
                      <div class="notes-body">${fs(i.notes)}</div>
                    </div>
                  `:c}
      ...
      <div class="meta">
        ${i.map(([a,l])=>o`
            <div class="meta-item">
              ...
              <span class="meta-value">${ut(l)?o`<a href="${l}" target="_blank" rel="noopener noreferrer">${l}</a>`:l}</span>
            </div>
          `)}
      </div>
    `}};X.styles=k`
  ...
  `,d([v({attribute:!1})],X.prototype,"hass",2),d([p()],X.prototype,"_open",2),d([p()],X.prototype,"_entryId",2),d([p()],X.prototype,"_data",2),d([p()],X.prototype,"_busy",2),d([p()],X.prototype,"_error",2);customElements.get("maintenance-object-quick-actions-dialog")||customElements.define("maintenance-object-quick-actions-dialog",X)});function ys(){let s=window;return s.__msSettingsCache??={promise:null}}function kt(s){let r=ys();if(r.promise)return r.promise;let e=s.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).then(t=>({features:t.features??mi.features,defaultWarningDays:t.general?.default_warning_days??7,rowActionStyle:t.general?.row_action_style??mi.rowActionStyle})).catch(()=>(r.promise===e&&(r.promise=null),mi));return r.promise=e,e}function xs(){ys().promise=null}var mi,ws=w(()=>{"use strict";mi={features:{adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1},defaultWarningDays:7,rowActionStyle:"buttons_compact"}});var se={};Rs(se,{__resetSettingsCacheForTests:()=>ia,getRowActionStyle:()=>fi,openCompleteDialog:()=>aa,openCreateObjectDialog:()=>vi,openCreateTaskDialog:()=>bi,openEditObjectDialog:()=>ra,openEditTaskDialog:()=>sa,openHistoryEditDialog:()=>na,openObjectQuickActions:()=>la,openQrDialog:()=>oa,openTaskQuickActions:()=>yi});function Et(){return document.querySelector("home-assistant")?.hass}function ta(){return document.querySelector("home-assistant")?.shadowRoot??document.body}function ne(s){let r=ta(),e=r.querySelector(s)??document.body.querySelector(s);return e?e.parentNode!==r&&r.appendChild(e):(e=document.createElement(s),r.appendChild(e)),e}function ae(s){let r=Et();if(!r)return!1;s.hass=r;let e=P(r);return Ut(e)||Qe(e).then(()=>{s.requestUpdate?.()}),Bt(r.locale,r.config?.country),!0}function fi(s){return kt(s).then(r=>r.rowActionStyle)}function ia(){xs()}function vi(){let s=ne($s);return ae(s)?(s.openCreate(),!0):!1}function ra(s,r){let e=ne($s);return ae(e)?(e.openEdit(s,r),!0):!1}function bi(s="",r){let e=ne(ks);if(!ae(e))return!1;let t=Et();return t?((async()=>{let i=await kt(t),a=e;a.checklistsEnabled=i.features.checklists,a.scheduleTimeEnabled=i.features.schedule_time,a.completionActionsEnabled=i.features.completion_actions,a.defaultWarningDays=i.defaultWarningDays,a.openCreate(s,r)})(),!0):!1}function sa(s,r){let e=ne(ks);if(!ae(e))return!1;let t=Et();return t?((async()=>{try{let[i,a]=await Promise.all([t.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:s}),kt(t)]),l=(i.tasks||[]).find(h=>h.id===r);if(!l){console.warn(`openEditTaskDialog: task ${r} not found in entry ${s}`);return}let u=e;u.checklistsEnabled=a.features.checklists,u.scheduleTimeEnabled=a.features.schedule_time,u.completionActionsEnabled=a.features.completion_actions,u.defaultWarningDays=a.defaultWarningDays,await u.openEdit(s,l)}catch(i){console.warn("openEditTaskDialog: failed to load task/features",i)}})(),!0):!1}function na(s){let r=ne(Qn);return ae(r)?(r.openEdit(s),!0):!1}function aa(s){let r=ne(Jn);return ae(r)?(Ze(r,s,P(Et())),!0):!1}function oa(s){let r=ne(Zn);return ae(r)?(r.openForTask(s.entry_id,s.task_id,s.object_name,s.task_name),!0):!1}function yi(s,r){let e=ne(Xn);return ae(e)?(e.openFor(s,r),!0):!1}function la(s){let r=ne(ea);return ae(r)?(r.openFor(s),!0):!1}var $s,ks,Qn,Jn,Zn,Xn,ea,Z=w(()=>{"use strict";Or();Vr();ai();Wr();Yr();ms();bs();C();ft();ws();$s="maintenance-object-dialog",ks="maintenance-task-dialog",Qn="maintenance-history-edit-dialog",Jn="maintenance-complete-dialog",Zn="maintenance-qr-dialog",Xn="maintenance-task-quick-actions-dialog",ea="maintenance-object-quick-actions-dialog"});I();var Vs=["assignee_pool","required_completion_fields","checklist","labels","history","readings"],Ws=["checklist_progress"],Ks=["tasks","parts"],Ys=["manual_docs","battery_fleet_excluded"];function Dt(s,r,e=[]){for(let t of r)s[t]===void 0&&(s[t]=[]);for(let t of e)s[t]===void 0&&(s[t]={})}function Gs(s){let r=s;Dt(r,Ks),r.object&&typeof r.object=="object"&&Dt(r.object,Ys);for(let e of r.tasks)Dt(e,Vs,Ws);return s}function Ve(s){for(let r of s)Gs(r);return s}function Qs(s,r){if(r.objects)return r.objects;let e=r.delta||[],t=r.removed||[];if(!e.length&&!t.length)return null;let i=new Map(s.map(a=>[a.entry_id,a]));for(let a of e)i.set(a.entry_id,a);for(let a of t)i.delete(a);return[...i.values()]}function Ki(s,r){return r.objects&&Ve(r.objects),r.delta&&Ve(r.delta),Qs(s,r)}O();C();Yt();Gt();function Ae(s){let r=window;r.customCards=r.customCards||[],r.customCards.some(e=>e.type===s.type)||r.customCards.push(s)}Zt();Jt();gt();ft();I();O();C();var gn=["overdue","triggered","due_soon","ok"],J=class extends E{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-card"};this._objects=[];this._loadingObjects=!0;this._loadError=!1;this._views=[];this._objectsLoaded=!1;this._onEntitiesChanged=e=>{this._valueChanged("entity_ids",e.detail.value||[])}}get _lang(){return P(this.hass)}setConfig(e){this._config={...e}}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._objectsLoaded&&(this._objectsLoaded=!0,this._loadObjects())}async _loadObjects(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"});this._objects=e.objects||[],this._loadError=!1}catch{this._objects=[],this._loadError=!0}this._loadingObjects=!1;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"});this._views=e.views||[]}catch{this._views=[]}}_valueChanged(e,t){let i={...this._config,[e]:t};(Array.isArray(t)&&t.length===0||t==="")&&delete i[e],this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i}}))}_toggleStatus(e,t){let i=new Set(this._config.filter_status||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_status",[...i])}_toggleObject(e,t){let i=new Set(this._config.filter_objects||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_objects",[...i])}_toggleLabel(e,t){let i=new Set(this._config.filter_labels||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_labels",[...i])}_toggleArea(e,t){let i=new Set(this._config.filter_areas||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_areas",[...i])}_togglePriority(e,t){let i=new Set(this._config.filter_priority||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_priority",[...i])}render(){let e=this._lang,t=new Set(this._config.filter_status||[]),i=new Set(this._config.filter_objects||[]),a=[...this._objects].map(m=>m.object.name).sort((m,$)=>m.localeCompare($)),l=new Set(this._config.filter_areas||[]),u=[...new Set(this._objects.map(m=>m.object.area_id).filter(m=>!!m).concat([...l]))],h=m=>this.hass?.areas?.[m]?.name||m,_=u.map(m=>({id:m,name:h(m)})).sort((m,$)=>m.name.localeCompare($.name)),g=new Set(this._config.filter_labels||[]),b=new Set(this._config.filter_priority||[]),y=[...new Set(this._objects.flatMap(m=>m.tasks.flatMap($=>$.labels||[])))].sort((m,$)=>m.localeCompare($)),x=[];for(let m of this._objects)for(let $ of m.tasks)$.sensor_entity_id&&x.push($.sensor_entity_id),$.binary_sensor_entity_id&&x.push($.binary_sensor_entity_id);return o`
      <div class="editor">
        <ha-textfield
  ...
  `,d([v({attribute:!1})],J.prototype,"hass",2),d([p()],J.prototype,"_config",2),d([p()],J.prototype,"_objects",2),d([p()],J.prototype,"_loadingObjects",2),d([p()],J.prototype,"_loadError",2),d([p()],J.prototype,"_views",2);customElements.get("maintenance-supporter-card-editor")||customElements.define("maintenance-supporter-card-editor",J);ai();I();O();I();O();C();var tt={printOptions:"msp-print-options",batteryRosterOpen:"msp-bf-roster-open",docSort:"msp-doc-sort",overviewTab:"msp-overview-tab",collapsedSections:"msp-collapsed-sections",chartRange:"msp-chart-range",chartHideOutliers:"msp-chart-hide-outliers",taskSort:"maintenance_supporter_sort",objectSort:"maintenance_supporter_object_sort",groupBy:"maintenance_supporter_groupby",objectView:"maintenance_supporter_object_view",objectsCache:"msp-objects-cache",gettingStartedDismissed:"msp-gs-dismissed",batteryRosterSort:"ms_bf_roster_sort"};function oi(s){try{return localStorage.getItem(s)}catch{return null}}function li(s,r){try{localStorage.setItem(s,r)}catch{}}ie();wt();var U=class U extends E{constructor(){super(...arguments);this.flat=!1;this._ov=null;this._loading=!1;this._marking=!1;this._error="";this._history=null;this._rosterSort=U._storedSort();this._typeFilter=null;this._recorded=[];this._historyRequested=!1;this._localeReady=!1;this._markAll=async()=>{await this._mark(void 0)};this._repair=async()=>{if(!this._marking){this._marking=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/setup",language:this._lang}),await this._load()}catch(e){this._error=R(e,this._lang)}finally{this._marking=!1}}};this._loadHistory=async e=>{let t=e.target.open;if(li(tt.batteryRosterOpen,t?"1":"0"),!(!t||this._historyRequested)){this._historyRequested=!0;try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/overview_history"});this._history=i.series}catch{this._history=null}}}}get _lang(){return P(this.hass)}connectedCallback(){super.connectedCallback(),this.hass&&this._load()}updated(e){dt(this,e),e.has("hass")&&this.hass&&!this._localeReady&&(this._localeReady=!0,Qe(this._lang).then(()=>this.requestUpdate()),this._ov===null&&!this._loading&&this._load())}async _load(){this._loading=!0,this._error="";try{this._ov=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/overview"})}catch(e){this._error=R(e,this._lang)}finally{this._loading=!1}}async _mark(e){if(!this._marking){this._marking=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/mark_replaced",...e?{entity_ids:e}:{}}),await this._load()}catch(t){this._error=R(t,this._lang)}finally{this._marking=!1}}}async _setExcluded(e,t){if(!this._marking){this._marking=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/set_excluded",entity_id:e,excluded:t}),await this._load()}catch(i){this._error=R(i,this._lang)}finally{this._marking=!1}}}async _addBattery(e){let t=e.detail?.value;if(!(!t||this._marking)){this._marking=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/set_included",entity_id:t,included:!0}),await this._load()}catch(i){this._error=R(i,this._lang)}finally{this._marking=!1}}}async _setTrackSelf(e){await this._setFleetOption("set_track_self_charging",e.target.checked)}async _setDueWithoutSensor(e){await this._setFleetOption("set_due_without_sensor",e.target.checked)}async _setFleetOption(e,t){if(!this._marking){this._marking=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:`maintenance_supporter/battery_fleet/${e}`,enabled:t}),await this._load()}catch(i){this._error=R(i,this._lang)}finally{this._marking=!1}}}_rosterOpen(){return oi(tt.batteryRosterOpen)!=="0"}_predictedTitle(e,t){if(e.forecast_overdue)return n("battery_fleet_forecast_overdue",t);let i=this._predictedDate(e.days_until??0);if(e.predicted_source==="trend")return n("battery_fleet_predicted_trend",t).replace("{date}",i).replace("{confidence}",n("cal_confidence_"+(e.prediction_confidence||"medium"),t));if(e.lifetime_months!=null&&e.lifetime_source){let a=n("lifetime_source_"+e.lifetime_source,t).replace("{n}",String(e.lifetime_samples??0));return n("battery_fleet_predicted_typical",t).replace("{date}",i).replace("{months}",String(e.lifetime_months)).replace("{type}",e.battery_type).replace("{source}",a)}return n("battery_fleet_predicted_on",t).replace("{date}",i)}_sparkline(e){let t=this._history?.[e.entity_id];if(!t||t.points.length<2)return c;let i=110,a=24,l=2,u=t.points[0][0],h=t.points[t.points.length-1][0],_=Date.now()/1e3,g=e.status!=="low"&&e.predicted_source==="trend"&&e.days_until!=null?_+e.days_until*86400:null,b=Math.max(h,g??h),y=L=>b===u?l:l+(L-u)/(b-u)*(i-2*l),x=L=>l+(1-Math.min(100,Math.max(0,L))/100)*(a-2*l),m=t.points.map(([L,A])=>`${T(y(L))},${T(x(A))}`).join(" "),$=t.points[t.points.length-1][1],z=T(x(t.threshold));return o`<svg
    ...
    >
      ...
      ${g!==null?o`<line
            ...
            y2=${z}
          ></line>`:c}
    </svg>`}static _storedSort(){return oi(tt.batteryRosterSort)==="name"?"name":"urgency"}_setSort(e){this._rosterSort=e,li(tt.batteryRosterSort,e)}_sortedRoster(e){let t=this._typeFilter===null?e:e.filter(a=>a.battery_type===this._typeFilter);if(this._rosterSort==="name")return t;let i=a=>a.status==="low"?-1e3+(a.level??101)/101:a.days_until??1/0;return[...t].sort((a,l)=>i(a)-i(l)||a.device_name.localeCompare(l.device_name))}_predictedDate(e){return this._fmtDate(Date.now()+e*864e5)}_fmtDate(e){let t=new Date(e),i=a=>String(a).padStart(2,"0");return F(`${t.getFullYear()}-${i(t.getMonth()+1)}-${i(t.getDate())}`,this._lang)}_shoppingLine(e){return Object.entries(e).map(([t,i])=>o`<button
    ...
    </button>`}_renderRow(e,t,i){let a=e.available===!1?o`<span class="bf-offline">${n("battery_fleet_offline",t)}</span>`:e.no_sensor?o`<span class="bf-offline bf-nosensor">${n("battery_fleet_no_sensor",t)}</span>`:c,l=o`<span class="bf-type">${e.quantity}× ${e.battery_type}</span>`,u=i.mark==="always"||e.no_sensor||e.can_mark_replaced;return o`
      <div class="bf-row">
  ...
  `,d([v({attribute:!1})],U.prototype,"hass",2),d([v({type:Boolean})],U.prototype,"flat",2),d([p()],U.prototype,"_ov",2),d([p()],U.prototype,"_loading",2),d([p()],U.prototype,"_marking",2),d([p()],U.prototype,"_error",2),d([p()],U.prototype,"_history",2),d([p()],U.prototype,"_rosterSort",2),d([p()],U.prototype,"_typeFilter",2),d([p()],U.prototype,"_recorded",2);var di=U;customElements.get("maintenance-battery-fleet-section")||customElements.define("maintenance-battery-fleet-section",di);var Le=class extends E{constructor(){super(...arguments);this._config={type:"custom:maintenance-battery-fleet-card"}}static getStubConfig(){return{type:"custom:maintenance-battery-fleet-card"}}setConfig(e){this._config=e}getCardSize(){return 6}render(){return this.hass?o`
  ...
  `,d([v({attribute:!1})],Le.prototype,"hass",2),d([p()],Le.prototype,"_config",2);customElements.get("maintenance-battery-fleet-card")||customElements.define("maintenance-battery-fleet-card",Le);Ae({type:"maintenance-battery-fleet-card",name:"Battery Fleet",description:"All tracked batteries: what is low now, what runs out soon, and what to buy.",preview:!1});Z();I();O();var xi=["today","dashboard","calendar","settings"];C();var je="maintenance-supporter-panel-card",wi="maintenance-supporter-panel",ki="maintenance-supporter",$i=870,da=320,ca=480;function pa(s){let e=s?.panels?.[ki]?.config?._panel_custom?.module_url;return typeof e=="string"&&e.startsWith("/")?e:null}function ha(s){if(s==null||s===""||s==="fill")return null;if(typeof s=="number")return s>0?`${Math.round(s)}px`:null;let r=String(s).trim();return/^\d+(\.\d+)?$/.test(r)?`${r}px`:r||null}var Ei=class extends HTMLElement{constructor(){super();this._config={type:`custom:${je}`};this._panel=null;this._loading=null;this._observer=null;this._lastWidth=-1;this._preview=!1;this._onResize=()=>this._layout(!0);let e=this.attachShadow({mode:"open"}),t=document.createElement("style");t.textContent=`
      ...
      ${wi} { display: block; height: 100%; }
    `,e.appendChild(t),this._status=document.createElement("div"),this._status.className="status",this._status.hidden=!0,e.appendChild(this._status)}static getConfigElement(){return document.createElement("maintenance-supporter-panel-card-editor")}static getStubConfig(){return{type:`custom:${je}`}}set preview(e){this._preview=!!e,this._layout(!0)}get preview(){return this._preview}_inEditorPreview(){if(this._preview)return!0;let e=this;for(let t=0;e&&t<40;t++){let i=e;if(i.tagName==="HUI-CARD-PREVIEW"||i.tagName==="HUI-DIALOG-EDIT-CARD")return!0;e=i.parentElement??i.getRootNode().host??null}return!1}setConfig(e){this._config={...e},this._panel&&(this._panel.presets=this._presets()),this._layout(!0)}getCardSize(){return 12}getGridOptions(){return{columns:"full",rows:"auto"}}set hass(e){this._hass=e,this._panel?this._panel.hass=e:this._mount()}get hass(){return this._hass}connectedCallback(){typeof ResizeObserver<"u"&&!this._observer&&(this._observer=new ResizeObserver(()=>this._layout(!1)),this._observer.observe(this)),window.addEventListener("resize",this._onResize),this._layout(!0),this._hass&&this._mount()}disconnectedCallback(){this._observer?.disconnect(),this._observer=null,window.removeEventListener("resize",this._onResize)}_presets(){let e={};return xi.includes(this._config.tab??"")&&(e.tab=this._config.tab),typeof this._config.view=="string"&&this._config.view.trim()&&(e.view=this._config.view.trim()),e}_setStatus(e){this._status.textContent=e,this._status.hidden=!1}async _mount(){if(this._panel||this._loading||!this._hass||!this.isConnected)return;let e=P(this._hass);if(!customElements.get(wi)){let i=pa(this._hass);if(!i){this._setStatus(n("panel_card_not_registered",e));return}this._loading=import(i).then(()=>{},()=>{throw this._setStatus(n("panel_card_load_failed",e)),new Error("panel bundle failed to load")});try{await this._loading}catch{return}finally{this._loading=null}if(this._panel||!this._hass||!this.isConnected)return}let t=document.createElement(wi);t.embedded=!0,t.panel=this._hass.panels?.[ki]??{url_path:ki},t.presets=this._presets(),t.narrow=this._narrow(),t.hass=this._hass,this._status.hidden=!0,this.shadowRoot.appendChild(t),this._panel=t,this._layout(!0)}_narrow(){let e=this.getBoundingClientRect().width;return e>0?e<$i:window.innerWidth<$i}_layout(e){let t=this.getBoundingClientRect(),i=Math.round(t.width)!==this._lastWidth;if(!e&&!i)return;this._lastWidth=Math.round(t.width),this._panel&&t.width>0&&(this._panel.narrow=t.width<$i);let a=ha(this._config.height);if(a){this.style.height=a;return}if(!this.isConnected)return;if(this._inEditorPreview()){this.style.height=`${ca}px`;return}let l=Math.max(0,this._unscrolledTop(t)),u=Math.max(da,Math.floor(window.innerHeight-l));this.style.height=`${u}px`}_unscrolledTop(e){let t=e.top+window.scrollY,i=this.ownerDocument,a=this.parentElement??this.getRootNode().host??null;for(let l=0;a&&l<100;l++){let u=a;u!==i.documentElement&&u!==i.body&&typeof u.scrollTop=="number"&&u.scrollTop>0&&(t+=u.scrollTop),a=u.parentElement??u.getRootNode().host??null}return t}},ye=class extends E{constructor(){super(...arguments);this._config={type:`custom:${je}`};this._views=[];this._viewsLoaded=!1}setConfig(e){this._config={...e}}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._viewsLoaded&&(this._viewsLoaded=!0,this._loadViews())}async _loadViews(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"});this._views=e?.views||[]}catch{this._views=[]}}_set(e,t){let i={...this._config};t===""||t===void 0?delete i[e]:i[e]=t,this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}render(){let e=P(this.hass),t=i=>n(i==="today"?"tab_today":i==="calendar"?"tab_calendar":i,e);return o`
      <div class="editor">
        <div class="field">
          ...
          <select class="tab-select" .value=${this._config.tab||""} @change=${i=>this._set("tab",i.target.value)}>
            ...
            ${xi.map(i=>o`<option value=${i} ?selected=${this._config.tab===i}>${t(i)}</option>`)}
          </select>
        </div>
  ...
  `,d([v({attribute:!1})],ye.prototype,"hass",2),d([p()],ye.prototype,"_config",2),d([p()],ye.prototype,"_views",2);customElements.get(je)||customElements.define(je,Ei);customElements.get("maintenance-supporter-panel-card-editor")||customElements.define("maintenance-supporter-panel-card-editor",ye);Ae({type:je,name:"Maintenance Supporter \u2014 Panel",description:"The complete Maintenance Supporter panel as a card \u2014 for a panel view / dashboard subview without the sidebar entry.",preview:!1});var V=class extends E{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-card"};this._globalRowStyle="buttons_compact";this._objects=[];this._stats=null;this._unsub=null;this._viewFilters=null;this._userNames={};this._userPersons={};this._userService=null;this._userNamesLoaded=!1;this._taskDocs={};this._docsLoadedFor=new Set;this._dataLoaded=!1;this._lastConnection=null;this._onCompleted=async()=>{await this._loadData()}}get _lang(){return P(this.hass)}static getConfigElement(){return document.createElement("maintenance-supporter-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-card",show_header:!0,show_actions:!0,filter_status:["overdue","triggered","due_soon"],max_items:10}}setConfig(e){let t=e.view_id!==this._config.view_id;this._config=e,t&&this._dataLoaded&&this.hass&&this._loadViewFilters()}getCardSize(){return 3}connectedCallback(){super.connectedCallback()}disconnectedCallback(){super.disconnectedCallback(),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null}updated(e){if(super.updated(e),dt(this,e),this.hass&&!this._userNamesLoaded&&this._config.show_assignee!==!1&&this._objects.some(t=>t.tasks.some(i=>i.responsible_user_id))&&this._loadUserNames(),this.hass&&this._config.show_documents!==!1)for(let t of this._objects)this._docsLoadedFor.has(t.entry_id)||t.tasks.some(i=>(i.document_count??0)>0)&&this._loadDocuments(t.entry_id);if(e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){fi(this.hass).then(e=>{this._globalRowStyle=e}).catch(()=>{});try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects",compact:!0}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=Ve(e.objects),this._stats=t,ct(this._stats.budget)}catch{}await this._loadViewFilters()}_assigneeName(e){if(this._config.show_assignee===!1)return"";let t=e.responsible_user_id;return t&&this._userNames[t]||""}async _loadUserNames(){this._userNamesLoaded=!0,this._userService?this._userService.updateHass(this.hass):this._userService=new Ce(this.hass);try{let e=await this._userService.getUsers();this._userPersons=Object.fromEntries(e.map(t=>[t.id,_t(t)])),this._userNames=Object.fromEntries(e.map(t=>[t.id,t.name]))}catch{}}async _loadDocuments(e){this._docsLoadedFor.add(e);try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:e}),i={};for(let a of t.documents||[])for(let l of a.task_ids||[])(i[l]||=[]).push({id:a.id,title:a.title,kind:a.kind,url:a.url});this._taskDocs={...this._taskDocs,[e]:i}}catch{}}_docsFor(e,t){if(this._config.show_documents===!1)return[];let a=[...this._taskDocs[e]?.[t.id]||[]];return t.documentation_url&&a.push({id:`url:${t.id}`,title:n("documentation_label",this._lang),kind:"weblink",url:t.documentation_url}),a}async _openDoc(e){if(e.kind==="weblink"&&e.url){ut(e.url)&&window.open(e.url,"_blank","noopener");return}try{await br(this.hass,e.id)}catch{}}async _loadViewFilters(){if(!this._config.view_id){this._viewFilters=null;return}try{let t=((await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"})).views||[]).find(i=>i.id===this._config.view_id);this._viewFilters=t?t.filters:null}catch{this._viewFilters=null}}async _subscribe(){try{let e=await this.hass.connection.subscribeMessage(t=>{let i=Ki(this._objects,t);i!==null&&(this._objects=i)},{type:"maintenance_supporter/subscribe",deltas:!0,compact:!0});if(!this.isConnected){e();return}this._unsub=e}catch{}}get _flatTasks(){let e=[],{filter_status:t,filter_objects:i,entity_ids:a,filter_due_min_days:l,filter_due_max_days:u,filter_labels:h,filter_priority:_,filter_areas:g,max_items:b}=this._config,y=a?.length?new Set(a):null,x=l!==void 0||u!==void 0,m=this._viewFilters,$=m?.user_id==="current_user"?this.hass.user?.id??null:m?.user_id??null;for(let L of this._objects)if(!(i?.length&&!i.includes(L.object.name))){if(g?.length){let A=L.object.area_id;if(!A||!g.includes(A))continue}for(let A of L.tasks)if(!A.is_done&&!(A.archived||L.object.archived)&&!(t?.length&&!t.includes(A.status))&&!(h?.length&&!(A.labels||[]).some(H=>h.includes(H)))&&!(_?.length&&!_.includes(A.priority||"normal"))&&!(y&&!(A.sensor_entity_id&&y.has(A.sensor_entity_id)||A.binary_sensor_entity_id&&y.has(A.binary_sensor_entity_id)))){if(x){let H=A.days_until_due;if(H==null||l!==void 0&&H<l||u!==void 0&&H>u)continue}m&&(m.status&&A.status!==m.status||m.label&&!(A.labels||[]).includes(m.label)||m.priority&&(A.priority||"normal")!==m.priority||$&&A.responsible_user_id!==$)||e.push({entry_id:L.entry_id,object_name:L.object.name,task:A})}}let z={overdue:0,triggered:1,due_soon:2,ok:3};return e.sort((L,A)=>{let H=(z[L.task.status]??9)-(z[A.task.status]??9);return H!==0?H:(L.task.days_until_due??1/0)-(A.task.days_until_due??1/0)}),b&&b>0?e.slice(0,b):e}_openTaskDetail(e,t){yi(e,t)}render(){let e=this._lang,t=this._config.title||n("maintenance",e),i=this._config.show_header!==!1,a=(this._config.action_style??(this._globalRowStyle==="icons"?"icons":"buttons"))==="buttons",l=this._config.show_actions!==!1,u=this._config.compact||!1,h=this._flatTasks,_=this._stats;return o`
      <ha-card>
          ...
          <div class="header-right">
            ${i&&_?o`
                  <div class="header-stats">
                    ${_.overdue>0?o`<span class="badge overdue">${_.overdue}</span>`:c}
                    ${_.due_soon>0?o`<span class="badge due_soon">${_.due_soon}</span>`:c}
                    ${_.triggered>0?o`<span class="badge triggered">${_.triggered}</span>`:c}
                  </div>
                `:c}
            ${l?o`
                  <mwc-icon-button
        ...
        ${h.length===0?this._objects.some(g=>g.tasks.length>0)?o`<div class="empty-card">
              ...
              <div class="task-list ${u?"compact":""}">
                ${h.map(({entry_id:g,object_name:b,task:y})=>o`
                    <div class="task-item clickable"
                         @click=${()=>this._openTaskDetail(g,y.id)}
                         title="${n("open_task",e)}">
                      <div class="status-dot" style="background: ${me[y.status]||"#ccc"}"></div>
                      <div class="task-info">
                        <div class="task-name">
                          ${y.name}
                        ...
                        ${u?this._assigneeName(y)?o`<div class="task-meta compact-assignee" title="${this._assigneeName(y)}">
                                ${Qt(this._userPersons[y.responsible_user_id])}${this._assigneeName(y)}
                              </div>`:c:o`<div class="task-meta">
                              ${b} · ${n(y.type,e)}${de(y)?o` · ${de(y)}`:c}${this._assigneeName(y)?o` · <span class="assignee"
                                    >${Qt(this._userPersons[y.responsible_user_id])}${this._assigneeName(y)}</span
                                  >`:c}
                            </div>`}
                      </div>
                      ${this._docsFor(g,y).length?o`<div class="doc-chips">
                            ${this._docsFor(g,y).map(x=>o`
                              <button
                                type="button"
                                class="doc-chip"
                                title="${x.title}"
                                @click=${m=>{m.stopPropagation(),this._openDoc(x)}}
                              >
                      ...
                      <div class="task-due">
                        ${y.days_until_due!==null&&y.days_until_due!==void 0?y.days_until_due<0?o`<span class="overdue-text">${Vt(y.days_until_due,e)}</span>`:Vt(y.days_until_due,e):y.trigger_active?"\u26A1":"\u2014"}
                      </div>
                      ${l&&a?o`
                            <ha-button
                              size="small"
                              ...
                              title="${n("complete",e)}"
                              @click=${x=>{x.stopPropagation();let m=this.shadowRoot.querySelector("maintenance-complete-dialog");Ze(m,Je({entryId:g,taskId:y.id,taskName:y.name,task:y,objects:this._objects,lang:e,checklist:y.checklist||[],adaptiveEnabled:!!y.adaptive_config?.enabled,currencySymbol:le(this._stats?.budget)}),e)}}
                            >
