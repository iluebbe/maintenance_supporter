...
`),this._scheduleTime=t.schedule_time||"";let r=t.on_complete_action;if(r&&r.service){this._actionService=r.service;let _=r.target?.entity_id;this._actionTargetEntity=Array.isArray(_)?_[0]||"":_||"",this._actionData=r.data&&typeof r.data=="object"?{...r.data}:{},this._actionDataJsonFallback=""}else this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="";let d=t.quick_complete_defaults;this._qcNotes=d?.notes||"",this._qcCost=d?.cost!=null?String(d.cost):"",this._qcDuration=d?.duration!=null?String(d.duration):"",this._qcFeedback=d?.feedback||"";let c=t.adaptive_config||{};if(this._environmentalEntity=c.environmental_entity||"",this._environmentalAttribute=c.environmental_attribute||"",this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute,t.trigger_config){let _=t.trigger_config;this._triggerEntityId=_.entity_id||"",this._triggerEntityIds=_.entity_ids||(_.entity_id?[_.entity_id]:[]),this._triggerEntityLogic=_.entity_logic||"any",this._triggerAttribute=_.attribute||"",this._triggerType=_.type||"threshold",this._triggerAbove=_.trigger_above?.toString()||"",this._triggerBelow=_.trigger_below?.toString()||"",this._triggerForMinutes=_.trigger_for_minutes?.toString()||"0",this._triggerTargetValue=_.trigger_target_value?.toString()||"",this._triggerDeltaMode=_.trigger_delta_mode||!1,this._triggerFromState=_.trigger_from_state||"",this._triggerToState=_.trigger_to_state||"",this._triggerTargetChanges=_.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=_.trigger_runtime_hours?.toString()||""}else this._resetTriggerFields();this._triggerEntityId&&this._fetchEntityAttributes(this._triggerEntityId),await Promise.all([this._loadUsers(),this._loadTags()]),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._intervalUnit="days",this._dueDate="",this._warningDays=String(this.defaultWarningDays),this._intervalAnchor="completion",this._weekdays=[],this._nth="1",this._nthWeekday="5",this._domDay="1",this._notes="",this._documentationUrl="",this._customIcon="",this._enabled=!0,this._lastPerformed="",this._nfcTagId="",this._responsibleUserId=null,this._checklistText="",this._scheduleTime="",this._environmentalEntity="",this._environmentalAttribute="",this._environmentalInitial="",this._environmentalAttributeInitial="",this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="",this._actionTesting=!1,this._actionTestResult="",this._qcNotes="",this._qcCost="",this._qcDuration="",this._qcFeedback="",this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerForMinutes="0",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours=""}async _loadUsers(){this._userService||(this._userService=new Ae(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}async _testAction(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e)){this._actionTestResult="error",this._actionTestError="Invalid service format (expected 'domain.service')",setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3);return}let[t,n]=e.split(".");if(!this.hass?.services?.[t]?.[n]){this._actionTestResult="error",this._actionTestError=`Service "${e}" is not registered in Home Assistant. Check spelling and that the integration providing it is loaded.`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}let r=this._actionTargetEntity.trim();if(r){let d=r.split(".")[0];if(d!==t&&!new Set(["homeassistant","scene","notify","persistent_notification"]).has(t)){this._actionTestResult="error",this._actionTestError=`Service "${e}" only works on ${t}.* entities; entity "${r}" is in ${d}.* \u2014 pick a service that matches the entity domain (e.g. ${d}.${n})`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}if(!this.hass.states?.[r]){this._actionTestResult="error",this._actionTestError=`Target entity "${r}" not found in Home Assistant \u2014 the entity may have been renamed or its integration removed.`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}}this._actionTestResult="ok",setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3)}_buildActionData(){if(this._actionDataJsonFallback.trim())try{let e=JSON.parse(this._actionDataJsonFallback);if(e&&typeof e=="object"&&!Array.isArray(e))return e}catch{}return{...this._actionData}}_serviceSchema(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e))return null;let[t,n]=e.split("."),r=this.hass?.services?.[t]?.[n]?.fields;return!r||Object.keys(r).length===0?null:Object.entries(r).map(([d,c])=>({name:d,required:!!c.required,selector:c.selector||{text:{}}}))}_renderCompletionActionsSection(e){if(!this.completionActionsEnabled)return p;let t=this._serviceSchema();return s`
      <details class="ca-section">
        ...
        <ha-service-picker
          ...
          @value-changed=${n=>{this._actionService=n.detail.value||"";let r=this._serviceSchema();if(r){let d=new Set(r.map(c=>c.name));this._actionData=Object.fromEntries(Object.entries(this._actionData).filter(([c])=>d.has(c)))}}}
        ></ha-service-picker>
        <ha-form
          .hass=${this.hass}
...
`).map(c=>c.trim()).filter(Boolean).slice(0,100);e.checklist=d.length?d:null}if(this.completionActionsEnabled){let d=this._actionService.trim();if(d&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(d)){let b={service:d},h=this._actionTargetEntity.trim();h&&(b.target={entity_id:h});let k=this._buildActionData();Object.keys(k).length>0&&(b.data=k),e.on_complete_action=b}else e.on_complete_action=null;let c={};this._qcNotes.trim()&&(c.notes=this._qcNotes.trim());let _=parseFloat(this._qcCost);!isNaN(_)&&_>=0&&(c.cost=_);let m=parseInt(this._qcDuration,10);!isNaN(m)&&m>=0&&(c.duration=m),this._qcFeedback&&(c.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(c).length?c:null}let t=await this.hass.connection.sendMessagePromise(e),n=this._taskId||t?.task_id,r=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(n&&this._scheduleType==="sensor_based"&&r)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:n,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=q(e,this._lang,i("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return p;let e=this._lang;return s`
      ...
      <div class="select-row">
        ...
        >
          ${La.map(t=>s`<option value=${t} ?selected=${t===this._triggerType}>${i(t,e)}</option>`)}
        </select>
      </div>
      ...
      <div class="select-row">
        ...
        <div class="select-row">
          ...
          <select .value=${this._nth} @change=${r=>this._nth=r.target.value}>
            ${n.map(([r,d])=>s`<option value=${r} ?selected=${r===this._nth}>${d}</option>`)}
          </select>
        </div>
        <div class="select-row">
          <label>${i("recurrence_weekday",e)}</label>
          <select .value=${this._nthWeekday} @change=${r=>this._nthWeekday=r.target.value}>
            ${t.map((r,d)=>s`<option value=${String(d)} ?selected=${String(d)===this._nthWeekday}>${r}</option>`)}
          </select>
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${Da.map(n=>s`<option value=${n} ?selected=${n===this._type}>${i(n,e)}</option>`)}
            </select>
          </div>
          <div class="select-row">
            ...
            >
              ${Ra.map(n=>s`<option value=${n} ?selected=${n===this._scheduleType}>${i(n,e)}</option>`)}
            </select>
          </div>
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function Va(o,a,e){let t=o.confidence_interval_low,n=o.confidence_interval_high,r=a.suggested_interval??a.interval_days??0,d=a.interval_days??0,c=Math.max(0,t-5),m=n+5-c,b=(t-c)/m*100,h=(n-t)/m*100,k=(r-c)/m*100,f=d>0?(d-c)/m*100:-1;return s`
      ...
      <div class="prediction-grid">
        ...
        ${n?s`
          <div class="prediction-item">
            ...
            ${o.threshold_prediction_date?s`<span class="prediction-date">${ge(o.threshold_prediction_date,a)}</span>`:p}
        ...
        ${r&&e.environmental?s`
          <div class="prediction-item">
            ...
            ${o.environmental_entity?s`<span class="prediction-entity entity-link" @click=${c=>jt(c,o.environmental_entity)}>${o.environmental_entity}</span>`:p}
          </div>
        `:p}
      </div>
    </div>
  `}var Lt=y(()=>{"use strict";j();S()});function Mt(o,a,e,t){let n=Math.max(o||1,a);return s`
  ...
  `}var Ft=y(()=>{"use strict";j();S()});function Ot(o,a,e){if(!e.seasonal||!o.seasonal_factor||o.seasonal_factor===1)return p;let t=Ht.map(c=>i(c,a)),n=new Date().getMonth(),r=o.seasonal_factors||o.interval_analysis?.seasonal_factors||null,d=r&&r.length===12?r:t.map((c,_)=>{let m=o.seasonal_factor||1,b=Math.sin((_-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,m+b))});return s`
    <div class="seasonal-card-compact">
      <h4>${i("seasonal_awareness",a)}</h4>
      <div class="seasonal-mini-chart">
        ${d.map((c,_)=>{let m=c*40,b=c<.9?"low":c>1.1?"high":"normal";return s`
  ...
  `}function Ut(o,a){return Ba(o,a)}function Ba(o,a){let e=o.seasonal_factors??o.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return p;let t=o.interval_analysis?.seasonal_reason,n=new Date().getMonth(),r=300,d=100,c=8,m=d-c-4,b=Math.max(...e,1.5),h=r/12,k=h*.65,f=c+m-1/b*m;return s`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${r} ${d}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${i("chart_seasonal",a)}">
        ...
        ${e.map((x,L)=>{let me=x/b*m,Se=L*h+(h-k)/2,Y=c+m-me,he=L===n,ve=x<1?"var(--success-color, #4caf50)":x>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return te`
  ...
  `}var Ht,Vt=y(()=>{"use strict";j();S();Ht=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"]});var $,Bt=y(()=>{"use strict";j();I();S();J();Rt();Lt();Ft();Vt();$=class extends w{constructor(){super(...arguments);this._open=!1;this._entryId=null;this._taskId=null;this._task=null;this._objectName="";this._busy=!1;this._error="";this._showSkip=!1;this._showReset=!1;this._showDetails=!1;this._showAdaptive=!1;this._skipReason="";this._resetDate="";this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._toast="";this._featuresLoaded=!1}get _lang(){return this.hass?.language||"en"}async openFor(e,t){this._entryId=e,this._taskId=t,this._error="",this._showSkip=!1,this._showReset=!1,this._showAdaptive=!1,this._skipReason="",this._resetDate=new Date().toISOString().slice(0,10),this._open=!0,await Promise.all([this._loadTask(),this._loadFeatures()])}async _loadFeatures(){if(!this._featuresLoaded)try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});e?.features&&(this._features={...this._features,...e.features}),this._featuresLoaded=!0}catch{}}close(){this._open=!1,this._task=null,this._error=""}async _loadTask(){if(!(!this._entryId||!this._taskId))try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._objectName=e.object?.name||"";let t=(e.tasks||[]).find(n=>n.id===this._taskId);this._task=t??null}catch(e){this._error=q(e,this._lang)}}async _runWs(e){this._busy=!0,this._error="";try{return await this.hass.connection.sendMessagePromise(e),this._busy=!1,!0}catch(t){return this._error=q(t,this._lang),this._busy=!1,!1}}_notifyChanged(e){this.dispatchEvent(new CustomEvent("task-action-fired",{detail:{entry_id:this._entryId,task_id:this._taskId,action:e},bubbles:!0,composed:!0}))}_onComplete(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(R(),O)).then(({openCompleteDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,checklist:this._task.checklist||[],adaptive_enabled:!!this._task.adaptive_config?.enabled})&&(this._notifyChanged("complete"),this.close())})}async _onSkipConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/skip",entry_id:this._entryId,task_id:this._taskId,reason:this._skipReason.trim()||null})&&(this._notifyChanged("skip"),this.close())}async _onResetConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/reset",entry_id:this._entryId,task_id:this._taskId,date:this._resetDate||void 0})&&(this._notifyChanged("reset"),this.close())}_onEdit(){!this._entryId||!this._taskId||Promise.resolve().then(()=>(R(),O)).then(({openEditTaskDialog:e})=>{e(this._entryId,this._taskId),this.close()})}_onQr(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(R(),O)).then(({openQrDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,object_name:this._objectName}),this.close()})}async _onDelete(){if(!this._entryId||!this._taskId)return;let e=i("delete_task_confirm",this._lang)||`Delete "${this._task?.name}"?`;if(!window.confirm(e))return;await this._runWs({type:"maintenance_supporter/task/delete",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("delete"),this.close())}_onOpenInPanel(){if(!this._entryId||!this._taskId)return;let e=`/maintenance-supporter?entry_id=${encodeURIComponent(this._entryId)}&task_id=${encodeURIComponent(this._taskId)}`;history.pushState(null,"",e),window.dispatchEvent(new CustomEvent("location-changed")),this.close()}async _applySuggestion(){if(!this._entryId||!this._taskId||!this._task?.suggested_interval)return;await this._runWs({type:"maintenance_supporter/task/apply_suggestion",entry_id:this._entryId,task_id:this._taskId,interval:this._task.suggested_interval})&&(this._toast=i("suggestion_applied",this._lang)||"Applied",this._notifyChanged("apply_suggestion"),await this._loadTask(),setTimeout(()=>{this._toast=""},2500))}async _reanalyzeInterval(){if(!(!this._entryId||!this._taskId)){this._busy=!0,this._error="";try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:this._entryId,task_id:this._taskId});this._toast=e.recommended_interval?`${i("reanalyze_result",this._lang)||"Recomputed"}: ${e.recommended_interval}d (${e.data_points} pts)`:i("reanalyze_insufficient_data",this._lang)||"Not enough data",await this._loadTask(),setTimeout(()=>{this._toast=""},3500)}catch(e){this._error=q(e,this._lang)}finally{this._busy=!1}}}_onEditHistoryEntry(e){!this._entryId||!this._taskId||Promise.resolve().then(()=>(R(),O)).then(({openHistoryEditDialog:t})=>{t({entry_id:this._entryId,task_id:this._taskId,original_timestamp:e.timestamp,type:e.type,timestamp:e.timestamp,notes:e.notes??null,cost:e.cost??null,duration:e.duration??null,completed_by:e.completed_by??null})})}_renderRecommendation(e){if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return p;let t=this._lang;return s`
    ...
    `}_renderDetails(e){let t=this._lang,n=e.history||[],r=n.filter(_=>_.type==="completed"),d=r.reduce((_,m)=>_+(typeof m.cost=="number"?m.cost:0),0),c=(()=>{let _=r.map(m=>typeof m.duration=="number"?m.duration:null).filter(m=>m!=null);return _.length?Math.round(_.reduce((m,b)=>m+b,0)/_.length):null})();return s`
      <div class="details">
        ...
        ${n.length===0?s`<div class="history-empty">${i("history_empty",t)||"No history yet."}</div>`:s`
              <div class="history-list">
                ${[...n].reverse().slice(0,20).map(_=>{let m=["completed","reset","skipped"].includes(_.type);return s`
                    <div class="history-entry">
                      ...
                      ${_.notes?s`<div class="history-notes">${_.notes}</div>`:p}
                      ${_.cost!=null||_.duration!=null?s`<div class="history-meta">
                            ${_.cost!=null?s`<span>💰 ${_.cost.toFixed(2)}</span>`:p}
                            ${_.duration!=null?s`<span>⏱️ ${_.duration}m</span>`:p}
                          </div>`:p}
                    </div>
                  `})}
        ...
        ${t?s`
              <div class="header">
                ...
                <div class="quick-info">
                  ${t.next_due?s`<span><strong>${i("next_due",e)||"Next due"}:</strong> ${ge(t.next_due,e)}</span>`:p}
                  ${t.last_performed?s`<span><strong>${i("last_performed",e)||"Last"}:</strong> ${ge(t.last_performed,e)}</span>`:p}
                  ${t.interval_days!=null?s`<span><strong>${i("interval",e)||"Interval"}:</strong> ${Et(t.interval_days,t.interval_unit,e)}</span>`:p}
                </div>
              </div>
                      ...
                      <div class="task-list">
                        ${r.map(c=>s`
                          <div class="task-row" @click=${()=>this._onTaskClick(c.id)}>
                            <span class="status-dot" style="background: ${ne[c.status]||"#ccc"}"></span>
                            <span class="task-name">${c.name}</span>
                            <span class="task-status">${i(c.status||"ok",e)}</span>
                          </div>
                        `)}
                      </div>
                    `}
      ...
      <div class="meta">
        ${n.map(([r,d])=>s`
            <div class="meta-item">
              ...
              <span class="meta-value">${r.toLowerCase().includes("doc")||r.toLowerCase().includes("url")?s`<a href="${d}" target="_blank" rel="noopener">${d}</a>`:d}</span>
            </div>
          `)}
      </div>
    `}};P.styles=z`
  ...
  `,l([v({attribute:!1})],P.prototype,"hass",2),l([u()],P.prototype,"_open",2),l([u()],P.prototype,"_entryId",2),l([u()],P.prototype,"_data",2),l([u()],P.prototype,"_busy",2),l([u()],P.prototype,"_error",2);customElements.get("maintenance-object-quick-actions-dialog")||customElements.define("maintenance-object-quick-actions-dialog",P)});var O={};na(O,{openCompleteDialog:()=>en,openCreateObjectDialog:()=>We,openCreateTaskDialog:()=>Ge,openEditObjectDialog:()=>Ja,openEditTaskDialog:()=>Ya,openHistoryEditDialog:()=>Xa,openObjectQuickActions:()=>an,openQrDialog:()=>tn,openTaskQuickActions:()=>Ke});function je(){return document.querySelector("home-assistant")?.hass}function U(o){let a=document.body.querySelector(o);return a||(a=document.createElement(o),document.body.appendChild(a)),a}function V(o){let a=je();return a?(o.hass=a,!0):!1}function Zt(o){return Ee||(Ee=o.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).then(a=>({features:a.features??Gt.features,defaultWarningDays:a.general?.default_warning_days??7})).catch(()=>Gt),Ee)}function We(){let o=U(Kt);return V(o)?(o.openCreate(),!0):!1}function Ja(o,a){let e=U(Kt);return V(e)?(e.openEdit(o,a),!0):!1}function Ge(){let o=U(Qt);if(!V(o))return!1;let a=je();return a?((async()=>{let e=await Zt(a),t=o;t.checklistsEnabled=e.features.checklists,t.scheduleTimeEnabled=e.features.schedule_time,t.completionActionsEnabled=e.features.completion_actions,t.defaultWarningDays=e.defaultWarningDays,t.openCreate()})(),!0):!1}function Ya(o,a){let e=U(Qt);if(!V(e))return!1;let t=je();return t?((async()=>{try{let[n,r]=await Promise.all([t.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:o}),Zt(t)]),d=(n.tasks||[]).find(_=>_.id===a);if(!d){console.warn(`openEditTaskDialog: task ${a} not found in entry ${o}`);return}let c=e;c.checklistsEnabled=r.features.checklists,c.scheduleTimeEnabled=r.features.schedule_time,c.completionActionsEnabled=r.features.completion_actions,c.defaultWarningDays=r.defaultWarningDays,await c.openEdit(o,d)}catch(n){console.warn("openEditTaskDialog: failed to load task/features",n)}})(),!0):!1}function Xa(o){let a=U(Wa);return V(a)?(a.openEdit(o),!0):!1}function en(o){let a=U(Ga);return V(a)?(a.entryId=o.entry_id,a.taskId=o.task_id,a.taskName=o.task_name,a.checklist=o.checklist??[],a.adaptiveEnabled=!!o.adaptive_enabled,a.lang=je()?.language||"en",a.open(),!0):!1}function tn(o){let a=U(Ka);return V(a)?(a.openForTask(o.entry_id,o.task_id,o.object_name,o.task_name),!0):!1}function Ke(o,a){let e=U(Qa);return V(e)?(e.openFor(o,a),!0):!1}function an(o){let a=U(Za);return V(a)?(a.openFor(o),!0):!1}var Kt,Qt,Wa,Ga,Ka,Qa,Za,Gt,Ee,R=y(()=>{"use strict";St();Tt();Ve();Nt();It();Bt();Wt();Kt="maintenance-object-dialog",Qt="maintenance-task-dialog",Wa="maintenance-history-edit-dialog",Ga="maintenance-complete-dialog",Ka="maintenance-qr-dialog",Qa="maintenance-task-quick-actions-dialog",Za="maintenance-object-quick-actions-dialog";Gt={features:{adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1},defaultWarningDays:7},Ee=null});j();I();S();j();I();S();var Ta=["overdue","triggered","due_soon","ok"],F=class extends w{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-card"};this._objects=[];this._loadingObjects=!0;this._loadError=!1;this._objectsLoaded=!1;this._onEntitiesChanged=e=>{this._valueChanged("entity_ids",e.detail.value||[])}}get _lang(){return this.hass?.language||"en"}setConfig(e){this._config={...e}}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._objectsLoaded&&(this._objectsLoaded=!0,this._loadObjects())}async _loadObjects(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"});this._objects=e.objects||[],this._loadError=!1}catch{this._objects=[],this._loadError=!0}this._loadingObjects=!1}_valueChanged(e,t){let n={...this._config,[e]:t};Array.isArray(t)&&t.length===0&&delete n[e],this._config=n,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:n}}))}_toggleStatus(e,t){let n=new Set(this._config.filter_status||[]);t?n.add(e):n.delete(e),this._valueChanged("filter_status",[...n])}_toggleObject(e,t){let n=new Set(this._config.filter_objects||[]);t?n.add(e):n.delete(e),this._valueChanged("filter_objects",[...n])}render(){let e=this._lang,t=new Set(this._config.filter_status||[]),n=new Set(this._config.filter_objects||[]),r=[...this._objects].map(c=>c.object.name).sort((c,_)=>c.localeCompare(_)),d=[];for(let c of this._objects)for(let _ of c.tasks)_.sensor_entity_id&&d.push(_.sensor_entity_id),_.binary_sensor_entity_id&&d.push(_.binary_sensor_entity_id);return s`
      <div class="editor">
        <ha-textfield
  ...
  `,l([v({attribute:!1})],F.prototype,"hass",2),l([u()],F.prototype,"_config",2),l([u()],F.prototype,"_objects",2),l([u()],F.prototype,"_loadingObjects",2),l([u()],F.prototype,"_loadError",2);customElements.get("maintenance-supporter-card-editor")||customElements.define("maintenance-supporter-card-editor",F);Ve();R();var B=class extends w{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-card"};this._objects=[];this._stats=null;this._unsub=null;this._dataLoaded=!1;this._lastConnection=null;this._onCompleted=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"en"}static getConfigElement(){return document.createElement("maintenance-supporter-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-card",show_header:!0,show_actions:!0,filter_status:["overdue","triggered","due_soon"],max_items:10}}setConfig(e){this._config=e}getCardSize(){return 3}connectedCallback(){super.connectedCallback()}disconnectedCallback(){super.disconnectedCallback(),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null}updated(e){if(super.updated(e),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(e=>{let t=e;this._objects=t.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _flatTasks(){let e=[],{filter_status:t,filter_objects:n,entity_ids:r,filter_due_min_days:d,filter_due_max_days:c,max_items:_}=this._config,m=r?.length?new Set(r):null,b=d!==void 0||c!==void 0;for(let k of this._objects)if(!(n?.length&&!n.includes(k.object.name))){for(let f of k.tasks)if(!f.is_done&&!(t?.length&&!t.includes(f.status))&&!(m&&!(f.sensor_entity_id&&m.has(f.sensor_entity_id)||f.binary_sensor_entity_id&&m.has(f.binary_sensor_entity_id)))){if(b){let x=f.days_until_due;if(x==null||d!==void 0&&x<d||c!==void 0&&x>c)continue}e.push({entry_id:k.entry_id,object_name:k.object.name,task:f})}}let h={overdue:0,triggered:1,due_soon:2,ok:3};return e.sort((k,f)=>{let x=(h[k.task.status]??9)-(h[f.task.status]??9);return x!==0?x:(k.task.days_until_due??1/0)-(f.task.days_until_due??1/0)}),_&&_>0?e.slice(0,_):e}_openTaskDetail(e,t){Ke(e,t)}render(){let e=this._lang,t=this._config.title||i("maintenance",e),n=this._config.show_header!==!1,r=this._config.show_actions!==!1,d=this._config.compact||!1,c=this._flatTasks,_=this._stats;return s`
      <ha-card>
          ...
          <div class="header-right">
            ${n&&_?s`
                  <div class="header-stats">
                    ${_.overdue>0?s`<span class="badge overdue">${_.overdue}</span>`:p}
                    ${_.due_soon>0?s`<span class="badge due_soon">${_.due_soon}</span>`:p}
                    ${_.triggered>0?s`<span class="badge triggered">${_.triggered}</span>`:p}
                  </div>
                `:p}
            ${r?s`
                  <mwc-icon-button
        ...
        ${c.length===0?s`<div class="empty-card">
              ...
              <div class="task-list ${d?"compact":""}">
                ${c.map(({entry_id:m,object_name:b,task:h})=>s`
                    <div class="task-item clickable"
                      ...
                      <div class="task-info">
                        ...
                        ${d?p:s`<div class="task-meta">${b} · ${i(h.type,e)}</div>`}
                      </div>
                      <div class="task-due">
