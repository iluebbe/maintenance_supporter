...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function ns(a,s,e,t,i){let b=Math.max(e,t,s,1)*1.3,E=50,w=[];for(let j=0;j<=E;j++){let z=j/E*b,bi=1-Math.exp(-Math.pow(z/s,a)),yi=32+z/b*260,xi=136-bi*128;w.push([yi,xi])}let P=w.map(([j,z])=>`${j.toFixed(1)},${z.toFixed(1)}`).join(" "),Ne="M32,136 "+w.map(([j,z])=>`L${j.toFixed(1)},${z.toFixed(1)}`).join(" ")+` L${w[E][0].toFixed(1)},136 Z`,ie=32+e/b*260,be=1-Math.exp(-Math.pow(e/s,a)),ye=136-be*128,fi=((1-be)*100).toFixed(0),nt=32+t/b*260,vi=[0,.25,.5,.75,1];return o`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${r("chart_weibull",i)}">
        ${vi.map(j=>{let z=136-j*128;return ae`
  ...
  `}function oi(a,s){return cs(a,s)}function cs(a,s){let e=a.seasonal_factors??a.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return h;let t=a.interval_analysis?.seasonal_reason,i=new Date().getMonth(),n=300,d=100,c=8,_=d-c-4,f=Math.max(...e,1.5),m=n/12,y=m*.65,b=c+_-1/f*_;return o`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${n} ${d}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${r("chart_seasonal",s)}">
        ...
        ${e.map((E,w)=>{let P=E/f*_,Ne=w*m+(m-y)/2,ie=c+_-P,be=w===i,ye=E<1?"var(--success-color, #4caf50)":E>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return ae`
