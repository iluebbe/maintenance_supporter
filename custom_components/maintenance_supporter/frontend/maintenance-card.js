...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function _s(a,s,e,t,i){let g=Math.max(e,t,s,1)*1.3,x=50,H=[];for(let z=0;z<=x;z++){let O=z/x*g,Si=1-Math.exp(-Math.pow(O/s,a)),Ai=32+O/g*260,Ti=136-Si*128;H.push([Ai,Ti])}let j=H.map(([z,O])=>`${z.toFixed(1)},${O.toFixed(1)}`).join(" "),S="M32,136 "+H.map(([z,O])=>`L${z.toFixed(1)},${O.toFixed(1)}`).join(" ")+` L${H[x][0].toFixed(1)},136 Z`,C=32+e/g*260,we=1-Math.exp(-Math.pow(e/s,a)),ke=136-we*128,ki=((1-we)*100).toFixed(0),ut=32+t/g*260,Ei=[0,.25,.5,.75,1];return o`
