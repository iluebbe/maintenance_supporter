...
`;var T=class extends y{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-card"}}setConfig(t){this._config={...t}}_valueChanged(t,i){let a={...this._config,[t]:i};this._config=a,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:a}}))}render(){return _`
      <div class="editor">
        <ha-textfield
          label="Title"
  ...
  `,u([v({attribute:!1})],m.prototype,"hass",2),u([v()],m.prototype,"entryId",2),u([v()],m.prototype,"taskId",2),u([v()],m.prototype,"taskName",2),u([v()],m.prototype,"lang",2),u([v({type:Array})],m.prototype,"checklist",2),u([v({type:Boolean})],m.prototype,"adaptiveEnabled",2),u([b()],m.prototype,"_open",2),u([b()],m.prototype,"_notes",2),u([b()],m.prototype,"_cost",2),u([b()],m.prototype,"_duration",2),u([b()],m.prototype,"_loading",2),u([b()],m.prototype,"_checklistState",2),u([b()],m.prototype,"_feedback",2);customElements.get("maintenance-complete-dialog")||customElements.define("maintenance-complete-dialog",m);var x=class extends y{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-card"};this._objects=[];this._stats=null;this._unsub=null;this._onCompleted=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"de"}static getConfigElement(){return document.createElement("maintenance-supporter-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-card",show_header:!0,show_actions:!0}}setConfig(t){this._config=t}getCardSize(){return 3}connectedCallback(){super.connectedCallback(),this._loadData(),this._subscribe()}disconnectedCallback(){super.disconnectedCallback(),this._unsub&&(this._unsub(),this._unsub=null)}async _loadData(){try{let[t,i]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=t.objects,this._stats=i}catch{}}async _subscribe(){try{this._unsub=await this.hass.connection.subscribeMessage(t=>{let i=t;this._objects=i.objects},{type:"maintenance_supporter/subscribe"})}catch{}}get _flatTasks(){let t=[],{filter_status:i,filter_objects:a,max_items:o}=this._config;for(let l of this._objects)if(!(a?.length&&!a.includes(l.object.name)))for(let s of l.tasks)i?.length&&!i.includes(s.status)||t.push({entry_id:l.entry_id,object_name:l.object.name,task:s});let r={overdue:0,triggered:1,due_soon:2,ok:3};return t.sort((l,s)=>(r[l.task.status]??9)-(r[s.task.status]??9)),o&&o>0?t.slice(0,o):t}render(){let t=this._lang,i=this._config.title||g("maintenance",t),a=this._config.show_header!==!1,o=this._config.show_actions!==!1,r=this._config.compact||!1,l=this._flatTasks,s=this._stats;return _`
      <ha-card>
        ...
        ${l.length===0?_`<div class="empty-card">${g("no_tasks_short",t)}</div>`:_`
              <div class="task-list ${r?"compact":""}">
                ${l.map(({entry_id:p,object_name:h,task:c})=>_`
                    <div class="task-item">
                      <div class="status-dot" style="background: ${Ae[c.status]||"#ccc"}"></div>
                      <div class="task-info">
                        <div class="task-name">${c.name}</div>
                        ${r?d:_`<div class="task-meta">${h} · ${g(c.type,t)}</div>`}
                      </div>
                      <div class="task-due">
                        ${c.days_until_due!==null&&c.days_until_due!==void 0?c.days_until_due<0?_`<span class="overdue-text">${Math.abs(c.days_until_due)}${t.startsWith("de")?"T":"d"}</span>`:c.days_until_due===0?g("today",t):`${c.days_until_due}${t.startsWith("de")?"T":"d"}`:c.trigger_active?"\u26A1":"\u2014"}
                      </div>
                      ${o?_`
                            <mwc-icon-button
                              class="complete-btn"
                              title="${g("complete",t)}"
                              @click=${()=>{let f=this.shadowRoot.querySelector("maintenance-complete-dialog");f.entryId=p,f.taskId=c.id,f.taskName=c.name,f.checklist=c.checklist||[],f.adaptiveEnabled=!!c.adaptive_config?.enabled,f.lang=t,f.open()}}
                            >
