...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function ms(a,s,e){let t=a.confidence_interval_low,i=a.confidence_interval_high,n=s.suggested_interval??s.interval_days??0,c=s.interval_days??0,d=Math.max(0,t-5),_=i+5-d,v=(t-d)/_*100,f=(i-t)/_*100,y=(n-d)/_*100,g=c>0?(c-d)/_*100:-1;return o`
  ...
  `}function _i(a,s){return fs(a,s)}function fs(a,s){let e=a.seasonal_factors??a.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return h;let t=a.interval_analysis?.seasonal_reason,i=new Date().getMonth(),n=300,c=100,d=8,_=c-d-4,v=Math.max(...e,1.5),f=n/12,y=f*.65,g=d+_-1/v*_;return o`
                    ...
                    <div class="task-item clickable"
                      ...
                      ${n?o`
                            <mwc-icon-button
                              class="complete-btn"
                              title="${r("complete",e)}"
                              @click=${y=>{y.stopPropagation();let g=this.shadowRoot.querySelector("maintenance-complete-dialog");g.entryId=_,g.taskId=f.id,g.taskName=f.name,g.checklist=f.checklist||[],g.adaptiveEnabled=!!f.adaptive_config?.enabled,g.taskType=f.type||"",g.readingUnit=f.reading_unit||"",g.requiredFields=f.required_completion_fields||[],g.lang=e;let x=!!f.part_ref;g.parts=x?[]:Zt(f,_,this._objects,e),g.consumesParts=x?[]:f.consumes_parts||[],g.open()}}
                            >
                              <ha-icon icon="mdi:check"></ha-icon>
                            </mwc-icon-button>
                          `:h}
