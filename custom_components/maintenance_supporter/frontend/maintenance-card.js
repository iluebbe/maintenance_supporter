...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `,l([f({attribute:!1})],R.prototype,"hass",2),l([f()],R.prototype,"lang",2),l([p()],R.prototype,"_open",2),l([p()],R.prototype,"_loading",2),l([p()],R.prototype,"_error",2),l([p()],R.prototype,"_viewResult",2),l([p()],R.prototype,"_completeResult",2),l([p()],R.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",R)});function Yt(a,s){let e=a.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return h;let o=a.interval_days??0,d=a.suggested_interval??o;return n`
  ...
  `}function Zi(a,s,e,t,i){let v=Math.max(e,t,s,1)*1.3,E=50,M=[];for(let H=0;H<=E;H++){let P=H/E*v,_i=1-Math.exp(-Math.pow(P/s,a)),ui=32+P/v*260,gi=136-_i*128;M.push([ui,gi])}let fe=M.map(([H,P])=>`${H.toFixed(1)},${P.toFixed(1)}`).join(" "),je="M32,136 "+M.map(([H,P])=>`L${H.toFixed(1)},${P.toFixed(1)}`).join(" ")+` L${M[E][0].toFixed(1)},136 Z`,te=32+e/v*260,ve=1-Math.exp(-Math.pow(e/s,a)),be=136-ve*128,pi=((1-ve)*100).toFixed(0),it=32+t/v*260,hi=[0,.25,.5,.75,1];return n`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${r("chart_weibull",i)}">
        ${hi.map(H=>{let P=136-H*128;return re`
