...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function Fs(a,s,e,t,i){let x=Math.max(e,t,s,1)*1.3,f=50,$=[];for(let F=0;F<=f;F++){let V=F/f*x,Bi=1-Math.exp(-Math.pow(V/s,a)),Wi=32+V/x*260,Ki=136-Bi*128;$.push([Wi,Ki])}let D=$.map(([F,V])=>`${F.toFixed(1)},${V.toFixed(1)}`).join(" "),L="M32,136 "+$.map(([F,V])=>`L${F.toFixed(1)},${V.toFixed(1)}`).join(" ")+` L${$[f][0].toFixed(1)},136 Z`,E=32+e/x*260,N=1-Math.exp(-Math.pow(e/s,a)),Ie=136-N*128,Ui=((1-N)*100).toFixed(0),$t=32+t/x*260,Vi=[0,.25,.5,.75,1];return o`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${r("chart_weibull",i)}">
        ...
        ${t>0&&t!==e?pe`
          <line x1="${$t.toFixed(1)}" y1="${8}" x2="${$t.toFixed(1)}" y2="${136 .toFixed(1)}"
  ...
  `}var Ti=w(()=>{"use strict";P();R()});function Ci(a,s,e,t){let i=Math.max(a||1,s);return o`
