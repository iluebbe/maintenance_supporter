...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function As(n,s,e,t,i){let g=Math.max(e,t,s,1)*1.3,x=50,N=[];for(let q=0;q<=x;q++){let O=q/x*g,Ri=1-Math.exp(-Math.pow(O/s,n)),ji=32+O/g*260,Ni=136-Ri*128;N.push([ji,Ni])}let j=N.map(([q,O])=>`${q.toFixed(1)},${O.toFixed(1)}`).join(" "),S="M32,136 "+N.map(([q,O])=>`L${q.toFixed(1)},${O.toFixed(1)}`).join(" ")+` L${N[x][0].toFixed(1)},136 Z`,C=32+e/g*260,ke=1-Math.exp(-Math.pow(e/s,n)),Ee=136-ke*128,Ii=((1-ke)*100).toFixed(0),mt=32+t/g*260,Pi=[0,.25,.5,.75,1];return o`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${r("chart_weibull",i)}">
        ${Pi.map(q=>{let O=136-q*128;return oe`
  ...
  `}var mi=$(()=>{"use strict";I();L()});function fi(n,s,e,t){let i=Math.max(n||1,s);return o`
