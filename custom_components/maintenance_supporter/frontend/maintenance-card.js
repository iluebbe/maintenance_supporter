...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function Ui(a,i,e,t,s){let v=Math.max(e,t,i,1)*1.3,w=50,N=[];for(let R=0;R<=w;R++){let z=R/w*v,li=1-Math.exp(-Math.pow(z/i,a)),ci=32+z/v*260,di=136-li*128;N.push([ci,di])}let me=N.map(([R,z])=>`${R.toFixed(1)},${z.toFixed(1)}`).join(" "),Le="M32,136 "+N.map(([R,z])=>`L${R.toFixed(1)},${z.toFixed(1)}`).join(" ")+` L${N[w][0].toFixed(1)},136 Z`,X=32+e/v*260,fe=1-Math.exp(-Math.pow(e/i,a)),ve=136-fe*128,ni=((1-fe)*100).toFixed(0),et=32+t/v*260,oi=[0,.25,.5,.75,1];return l`
  ...
  `}function Qt(a,i){return Vi(a,i)}function Vi(a,i){let e=a.seasonal_factors??a.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return u;let t=a.interval_analysis?.seasonal_reason,s=new Date().getMonth(),n=300,c=100,d=8,g=c-d-4,b=Math.max(...e,1.5),m=n/12,x=m*.65,v=d+g-1/b*g;return l`
    <div class="seasonal-chart">
      ...
      <div class="seasonal-labels">
        ${Jt.map((w,N)=>l`<span class="seasonal-label ${N===s?"active-month":""}">${r(w,i)}</span>`)}
      </div>
    </div>
              ...
              ${c?l`
                    <div class="actions">
                      ...
                      <button class="btn" @click=${this._onArchiveObject} ?disabled=${this._busy}>
                        ...
                        ${s.archived?r("unarchive",e)||"Unarchive":r("archive",e)||"Archive"}
                      </button>
