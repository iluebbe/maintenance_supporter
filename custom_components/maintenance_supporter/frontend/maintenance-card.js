      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          ${this.parts.length?o`<div class="used-parts">
                ...
                ${this.parts.map(t=>{let i=F({part_id:t.id,entry_id:t.entry_id}),n=this._usedParts[i],c=n!==void 0,d=t.entry_id?{part_id:t.id,quantity:1,entry_id:t.entry_id}:{part_id:t.id,quantity:1};return o`<div class="used-part-row">
                    <label class="used-part-check">
                      <input type="checkbox" .checked=${c}
                        @change=${h=>{let u={...this._usedParts};h.target.checked?u[i]=u[i]||d:delete u[i],this._usedParts=u}} />
                      <span
                    ...
                    ${c?o`<input class="used-part-qty" type="number" min="0.01" max="999" step="0.01"
                          ...
                          @input=${h=>{let u=parseFloat(h.target.value);this._usedParts={...this._usedParts,[i]:{...d,quantity:Number.isFinite(u)&&u>=.01?u:1}}}} />`:_}
                  </div>`})}
...
`).map(h=>h.trim()).filter(Boolean).slice(0,100);e.checklist=d.length?d:null}if(this.completionActionsEnabled){let d=this._actionService.trim();if(d&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(d)){let f={service:d},y=this._actionTargetEntity.trim();y&&(f.target={entity_id:y});let g=this._buildActionData();Object.keys(g).length>0&&(f.data=g),e.on_complete_action=f}else e.on_complete_action=null;let h={};this._qcNotes.trim()&&(h.notes=this._qcNotes.trim());let u=parseFloat(this._qcCost);!isNaN(u)&&u>=0&&(h.cost=u);let v=parseInt(this._qcDuration,10);!isNaN(v)&&v>=0&&(h.duration=v),this._qcFeedback&&(h.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(h).length?h:null}let i=await this.hass.connection.sendMessagePromise(e),n=this._taskId||i?.task_id,c=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(n&&this._scheduleType==="sensor_based"&&c)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:n,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}if(n&&this._adaptiveSnapshot()!==this._adaptiveInitial){let d=parseFloat(this._adaptiveAlpha),h=parseInt(this._adaptiveMin,10),u=parseInt(this._adaptiveMax,10);try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_adaptive",entry_id:this._entryId,task_id:n,enabled:this._adaptiveEnabled,...d>=.1&&d<=.9?{ewa_alpha:d}:{},...!isNaN(h)&&h>=1?{min_interval_days:h}:{},...!isNaN(u)&&u>=1?{max_interval_days:u}:{},seasonal_enabled:this._adaptiveSeasonal,sensor_prediction_enabled:this._adaptivePrediction}),this._adaptiveInitial=this._adaptiveSnapshot()}catch{}}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=P(e,this._lang,r("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1,this._pickerProbeTimer!==void 0&&(clearTimeout(this._pickerProbeTimer),this._pickerProbeTimer=void 0),this._pickerProbeStrikes=0}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return _;let e=this._lang,t=this._triggerType==="compound";return o`
    ...
    `}_renderConditionAttribute(e,t){let i=this._lang,n=e.entityIds.split(",")[0]?.trim()||"";n&&this._fetchConditionAttributes(n);let c=n?this._conditionAttrOptions[n]:void 0;return c&&c.available.length>0?o`
        <div class="select-row">
          <label>${r("attribute_optional",i)}</label>
          <select
            .value=${e.attribute}
            @change=${d=>this._patchCondition(t,{attribute:d.target.value})}
          >
            <option value="" ?selected=${!e.attribute}>${r("use_entity_state",i)}</option>
            ${c.suggested.map(d=>o`<option value=${d} ?selected=${d===e.attribute}>${d} ★</option>`)}
            ${c.available.filter(d=>!c.suggested.includes(d.name)).map(d=>o`<option value=${d.name} ?selected=${d.name===e.attribute}>${d.name}${d.numeric?"":" (non-numeric)"}</option>`)}
          </select>
        </div>
      `:o`
      <ms-textfield
        ...
        .value=${e.attribute}
        @input=${d=>this._patchCondition(t,{attribute:d.target.value.trim()})}
      ></ms-textfield>
    `}_renderConditionTypeFields(e,t){let i=this._lang;return e.type==="threshold"?o`
      ...
      `:_}_renderUnitSelect(){let e=this._lang;return o`
      <div class="select-row">
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function As(a,s,e,t,i){let g=Math.max(e,t,s,1)*1.3,x=50,N=[];for(let q=0;q<=x;q++){let z=q/x*g,Ri=1-Math.exp(-Math.pow(z/s,a)),ji=32+z/g*260,Ni=136-Ri*128;N.push([ji,Ni])}let j=N.map(([q,z])=>`${q.toFixed(1)},${z.toFixed(1)}`).join(" "),S="M32,136 "+N.map(([q,z])=>`L${q.toFixed(1)},${z.toFixed(1)}`).join(" ")+` L${N[x][0].toFixed(1)},136 Z`,C=32+e/g*260,ke=1-Math.exp(-Math.pow(e/s,a)),Ee=136-ke*128,Li=((1-ke)*100).toFixed(0),mt=32+t/g*260,Pi=[0,.25,.5,.75,1];return o`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${r("chart_weibull",i)}">
        ${Pi.map(q=>{let z=136-q*128;return oe`
  ...
  `}function Cs(a,s,e){let t=a.confidence_interval_low,i=a.confidence_interval_high,n=s.suggested_interval??s.interval_days??0,c=s.interval_days??0,d=Math.max(0,t-5),u=i+5-d,v=(t-d)/u*100,f=(i-t)/u*100,y=(n-d)/u*100,g=c>0?(c-d)/u*100:-1;return o`
  ...
  `}var ui=$(()=>{"use strict";L();I()});function gi(a,s,e){let t=a.degradation_trend!=null&&a.degradation_trend!=="insufficient_data",i=a.days_until_threshold!=null,n=a.environmental_factor!=null&&a.environmental_factor!==1;if(!t&&!i&&!n)return _;let c=a.degradation_trend==="rising"?"M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z":a.degradation_trend==="falling"?"M16,18L18.29,15.71L13.41,10.83L9.41,14.83L2,7.41L3.41,6L9.41,12L13.41,8L19.71,14.29L22,12V18H16Z":"M22,12L18,8V11H3V13H18V16L22,12Z";return o`
      ...
      <div class="prediction-grid">
        ...
        ${i?o`
          <div class="prediction-item">
            ...
            ${a.threshold_prediction_date?o`<span class="prediction-date">${W(a.threshold_prediction_date,s)}</span>`:_}
        ...
        ${n&&e.environmental?o`
          <div class="prediction-item">
            ...
            ${a.environmental_entity?o`<span class="prediction-entity entity-link" @click=${d=>ei(d,a.environmental_entity)}>${a.environmental_entity}</span>`:_}
          </div>
        `:_}
      </div>
    </div>
  `}var mi=$(()=>{"use strict";L();I()});function fi(a,s,e,t){let i=Math.max(a||1,s);return o`
  ...
  `}var vi=$(()=>{"use strict";L();I()});function yi(a,s,e){if(!e.seasonal||!a.seasonal_factor||a.seasonal_factor===1)return _;let t=bi.map(d=>r(d,s)),i=new Date().getMonth(),n=a.seasonal_factors||a.interval_analysis?.seasonal_factors||null,c=n&&n.length===12?n:t.map((d,h)=>{let u=a.seasonal_factor||1,v=Math.sin((h-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,u+v))});return o`
    <div class="seasonal-card-compact">
      <h4>${r("seasonal_awareness",s)}</h4>
      <div class="seasonal-mini-chart">
        ${c.map((d,h)=>{let u=d*40,v=d<.9?"low":d>1.1?"high":"normal";return o`
  ...
  `}function xi(a,s){return Is(a,s)}function Is(a,s){let e=a.seasonal_factors??a.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return _;let t=a.interval_analysis?.seasonal_reason,i=new Date().getMonth(),n=300,c=100,d=8,u=c-d-4,v=Math.max(...e,1.5),f=n/12,y=f*.65,g=d+u-1/v*u;return o`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${n} ${c}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${r("chart_seasonal",s)}">
        ...
        ${e.map((x,N)=>{let j=x/v*u,S=N*f+(f-y)/2,C=d+u-j,ke=N===i,Ee=x<1?"var(--success-color, #4caf50)":x>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return oe`
  ...
  `}var bi,$i=$(()=>{"use strict";L();I();bi=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"]});var T,wi=$(()=>{"use strict";L();O();I();re();ui();mi();vi();$i();T=class extends k{constructor(){super(...arguments);this._open=!1;this._entryId=null;this._taskId=null;this._task=null;this._objectName="";this._busy=!1;this._error="";this._showSkip=!1;this._showReset=!1;this._showDetails=!1;this._showAdaptive=!1;this._skipReason="";this._resetDate="";this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._toast="";this._featuresLoaded=!1}get _lang(){return this.hass?.language||"en"}async openFor(e,t){this._entryId=e,this._taskId=t,this._error="",this._showSkip=!1,this._showReset=!1,this._showAdaptive=!1,this._skipReason="",this._resetDate=new Date().toISOString().slice(0,10),this._open=!0,await Promise.all([this._loadTask(),this._loadFeatures()])}async _loadFeatures(){if(!this._featuresLoaded)try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});e?.features&&(this._features={...this._features,...e.features}),this._featuresLoaded=!0}catch{}}close(){this._open=!1,this._task=null,this._error=""}async _loadTask(){if(!(!this._entryId||!this._taskId))try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._objectName=e.object?.name||"";let t=(e.tasks||[]).find(i=>i.id===this._taskId);this._task=t??null}catch(e){this._error=P(e,this._lang)}}async _runWs(e){this._busy=!0,this._error="";try{return await this.hass.connection.sendMessagePromise(e),this._busy=!1,!0}catch(t){return this._error=P(t,this._lang),this._busy=!1,!1}}_notifyChanged(e){this.dispatchEvent(new CustomEvent("task-action-fired",{detail:{entry_id:this._entryId,task_id:this._taskId,action:e},bubbles:!0,composed:!0}))}_onComplete(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(U(),G)).then(({openCompleteDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,checklist:this._task.checklist||[],adaptive_enabled:!!this._task.adaptive_config?.enabled})&&(this._notifyChanged("complete"),this.close())})}async _onSkipConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/skip",entry_id:this._entryId,task_id:this._taskId,reason:this._skipReason.trim()||null})&&(this._notifyChanged("skip"),this.close())}async _onResetConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/reset",entry_id:this._entryId,task_id:this._taskId,date:this._resetDate||void 0})&&(this._notifyChanged("reset"),this.close())}_onEdit(){!this._entryId||!this._taskId||Promise.resolve().then(()=>(U(),G)).then(({openEditTaskDialog:e})=>{e(this._entryId,this._taskId),this.close()})}_onQr(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(U(),G)).then(({openQrDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,object_name:this._objectName}),this.close()})}async _onDelete(){if(!this._entryId||!this._taskId)return;let e=r("delete_task_confirm",this._lang)||`Delete "${this._task?.name}"?`;if(!window.confirm(e))return;await this._runWs({type:"maintenance_supporter/task/delete",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("delete"),this.close())}async _onArchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/archive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("archive"),this.close())}async _onUnarchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/unarchive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("unarchive"),this.close())}_onOpenInPanel(){if(!this._entryId||!this._taskId)return;let e=`/maintenance-supporter?entry_id=${encodeURIComponent(this._entryId)}&task_id=${encodeURIComponent(this._taskId)}`;history.pushState(null,"",e),window.dispatchEvent(new CustomEvent("location-changed")),this.close()}async _applySuggestion(){if(!this._entryId||!this._taskId||!this._task?.suggested_interval)return;await this._runWs({type:"maintenance_supporter/task/apply_suggestion",entry_id:this._entryId,task_id:this._taskId,interval:this._task.suggested_interval})&&(this._toast=r("suggestion_applied",this._lang)||"Applied",this._notifyChanged("apply_suggestion"),await this._loadTask(),setTimeout(()=>{this._toast=""},2500))}async _reanalyzeInterval(){if(!(!this._entryId||!this._taskId)){this._busy=!0,this._error="";try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:this._entryId,task_id:this._taskId});this._toast=e.recommended_interval?`${r("reanalyze_result",this._lang)||"Recomputed"}: ${je(e.recommended_interval,"days",this._lang)} (${e.data_points} pts)`:r("reanalyze_insufficient_data",this._lang)||"Not enough data",await this._loadTask(),setTimeout(()=>{this._toast=""},3500)}catch(e){this._error=P(e,this._lang)}finally{this._busy=!1}}}_onEditHistoryEntry(e){!this._entryId||!this._taskId||Promise.resolve().then(()=>(U(),G)).then(({openHistoryEditDialog:t})=>{t({entry_id:this._entryId,task_id:this._taskId,original_timestamp:e.timestamp,type:e.type,timestamp:e.timestamp,notes:e.notes??null,cost:e.cost??null,duration:e.duration??null,completed_by:e.completed_by??null})})}_renderRecommendation(e){if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return _;let t=this._lang;return o`
    ...
    `}_renderDetails(e){let t=this._lang,i=e.history||[],n=i.filter(h=>h.type==="completed"),c=n.reduce((h,u)=>h+(typeof u.cost=="number"?u.cost:0),0),d=(()=>{let h=n.map(u=>typeof u.duration=="number"?u.duration:null).filter(u=>u!=null);return h.length?Math.round(h.reduce((u,v)=>u+v,0)/h.length):null})();return o`
      <div class="details">
        ...
        ${i.length===0?o`<div class="history-empty">${r("history_empty",t)||"No history yet."}</div>`:o`
              <div class="history-list">
                ${[...i].reverse().slice(0,20).map(h=>{let u=["completed","reset","skipped"].includes(h.type);return o`
                    <div class="history-entry">
                      ...
                      ${h.notes?o`<div class="history-notes">${h.notes}</div>`:_}
                      ${h.cost!=null||h.duration!=null?o`<div class="history-meta">
                            ${h.cost!=null?o`<span>💰 ${h.cost.toFixed(2)}</span>`:_}
                            ${h.duration!=null?o`<span>⏱️ ${h.duration}m</span>`:_}
                          </div>`:_}
                    </div>
                  `})}
        ...
        ${t?o`
              <div class="header">
                ...
                <div class="quick-info">
                  ${t.next_due?o`<span><strong>${r("next_due",e)||"Next due"}:</strong> ${W(t.next_due,e)}</span>`:_}
                  ${t.last_performed?o`<span><strong>${r("last_performed",e)||"Last"}:</strong> ${W(t.last_performed,e)}</span>`:_}
  ...
  `,l([b({attribute:!1})],V.prototype,"hass",2),l([p()],V.prototype,"_open",2),l([p()],V.prototype,"_entryId",2),l([p()],V.prototype,"_data",2),l([p()],V.prototype,"_busy",2),l([p()],V.prototype,"_error",2);customElements.get("maintenance-object-quick-actions-dialog")||customElements.define("maintenance-object-quick-actions-dialog",V)});var G={};Mi(G,{openCompleteDialog:()=>zs,openCreateObjectDialog:()=>_t,openCreateTaskDialog:()=>ut,openEditObjectDialog:()=>Ms,openEditTaskDialog:()=>qs,openHistoryEditDialog:()=>Os,openObjectQuickActions:()=>Ds,openQrDialog:()=>Fs,openTaskQuickActions:()=>gt});function De(){return document.querySelector("home-assistant")?.hass}function Hs(){return document.querySelector("home-assistant")?.shadowRoot??document.body}function Y(a){let s=Hs(),e=s.querySelector(a)??document.body.querySelector(a);return e?e.parentNode!==s&&s.appendChild(e):(e=document.createElement(a),s.appendChild(e)),e}function J(a){let s=De();if(!s)return!1;a.hass=s;let e=s.language||"en";return He(e)||Me(e).then(()=>{a.requestUpdate?.()}),!0}function Ii(a){return Fe||(Fe=a.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).then(s=>({features:s.features??Ai.features,defaultWarningDays:s.general?.default_warning_days??7})).catch(()=>Ai),Fe)}function _t(){let a=Y(Ti);return J(a)?(a.openCreate(),!0):!1}function Ms(a,s){let e=Y(Ti);return J(e)?(e.openEdit(a,s),!0):!1}function ut(){let a=Y(Ci);if(!J(a))return!1;let s=De();return s?((async()=>{let e=await Ii(s),t=a;t.checklistsEnabled=e.features.checklists,t.scheduleTimeEnabled=e.features.schedule_time,t.completionActionsEnabled=e.features.completion_actions,t.defaultWarningDays=e.defaultWarningDays,t.openCreate()})(),!0):!1}function qs(a,s){let e=Y(Ci);if(!J(e))return!1;let t=De();return t?((async()=>{try{let[i,n]=await Promise.all([t.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:a}),Ii(t)]),c=(i.tasks||[]).find(h=>h.id===s);if(!c){console.warn(`openEditTaskDialog: task ${s} not found in entry ${a}`);return}let d=e;d.checklistsEnabled=n.features.checklists,d.scheduleTimeEnabled=n.features.schedule_time,d.completionActionsEnabled=n.features.completion_actions,d.defaultWarningDays=n.defaultWarningDays,await d.openEdit(a,c)}catch(i){console.warn("openEditTaskDialog: failed to load task/features",i)}})(),!0):!1}function Os(a){let s=Y(Ls);return J(s)?(s.openEdit(a),!0):!1}function zs(a){let s=Y(Ps);return J(s)?(s.entryId=a.entry_id,s.taskId=a.task_id,s.taskName=a.task_name,s.checklist=a.checklist??[],s.adaptiveEnabled=!!a.adaptive_enabled,s.requiredFields=a.required_completion_fields??[],s.lang=De()?.language||"en",s.open(),!0):!1}function Fs(a){let s=Y(Rs);return J(s)?(s.openForTask(a.entry_id,a.task_id,a.object_name,a.task_name),!0):!1}function gt(a,s){let e=Y(js);return J(e)?(e.openFor(a,s),!0):!1}function Ds(a){let s=Y(Ns);return J(s)?(s.openFor(a),!0):!1}var Ti,Ci,Ls,Ps,Rs,js,Ns,Ai,Fe,U=$(()=>{"use strict";ri();ci();lt();di();hi();wi();Si();I();Ti="maintenance-object-dialog",Ci="maintenance-task-dialog",Ls="maintenance-history-edit-dialog",Ps="maintenance-complete-dialog",Rs="maintenance-qr-dialog",js="maintenance-task-quick-actions-dialog",Ns="maintenance-object-quick-actions-dialog";Ai={features:{adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1},defaultWarningDays:7},Fe=null});L();var Qi=["assignee_pool","required_completion_fields","checklist","labels","history"],Zi=["checklist_progress"],Xi=["tasks","parts"],es=["manual_docs","battery_fleet_excluded"];function it(a,s,e=[]){for(let t of s)a[t]===void 0&&(a[t]=[]);for(let t of e)a[t]===void 0&&(a[t]={})}function ts(a){let s=a;it(s,Xi),s.object&&typeof s.object=="object"&&it(s.object,es);for(let e of s.tasks)it(e,Qi,Zi);return a}function ye(a){for(let s of a)ts(s);return a}function is(a,s){if(s.objects)return s.objects;let e=s.delta||[],t=s.removed||[];if(!e.length&&!t.length)return null;let i=new Map(a.map(n=>[n.entry_id,n]));for(let n of e)i.set(n.entry_id,n);for(let n of t)i.delete(n);return[...i.values()]}function Mt(a,s){return s.objects&&ye(s.objects),s.delta&&ye(s.delta),is(a,s)}O();I();nt();Oe();L();O();I();var ps=["overdue","triggered","due_soon","ok"],D=class extends k{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-card"};this._objects=[];this._loadingObjects=!0;this._loadError=!1;this._views=[];this._objectsLoaded=!1;this._onEntitiesChanged=e=>{this._valueChanged("entity_ids",e.detail.value||[])}}get _lang(){return this.hass?.language||"en"}setConfig(e){this._config={...e}}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._objectsLoaded&&(this._objectsLoaded=!0,this._loadObjects())}async _loadObjects(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"});this._objects=e.objects||[],this._loadError=!1}catch{this._objects=[],this._loadError=!0}this._loadingObjects=!1;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"});this._views=e.views||[]}catch{this._views=[]}}_valueChanged(e,t){let i={...this._config,[e]:t};(Array.isArray(t)&&t.length===0||t==="")&&delete i[e],this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i}}))}_toggleStatus(e,t){let i=new Set(this._config.filter_status||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_status",[...i])}_toggleObject(e,t){let i=new Set(this._config.filter_objects||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_objects",[...i])}_toggleLabel(e,t){let i=new Set(this._config.filter_labels||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_labels",[...i])}_toggleArea(e,t){let i=new Set(this._config.filter_areas||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_areas",[...i])}render(){let e=this._lang,t=new Set(this._config.filter_status||[]),i=new Set(this._config.filter_objects||[]),n=[...this._objects].map(g=>g.object.name).sort((g,x)=>g.localeCompare(x)),c=new Set(this._config.filter_areas||[]),d=[...new Set(this._objects.map(g=>g.object.area_id).filter(g=>!!g).concat([...c]))],h=g=>this.hass?.areas?.[g]?.name||g,u=d.map(g=>({id:g,name:h(g)})).sort((g,x)=>g.name.localeCompare(x.name)),v=new Set(this._config.filter_labels||[]),f=[...new Set(this._objects.flatMap(g=>g.tasks.flatMap(x=>x.labels||[])))].sort((g,x)=>g.localeCompare(x)),y=[];for(let g of this._objects)for(let x of g.tasks)x.sensor_entity_id&&y.push(x.sensor_entity_id),x.binary_sensor_entity_id&&y.push(x.binary_sensor_entity_id);return o`
      <div class="editor">
        <ha-textfield
          ...
          <div class="header-right">
            ${i&&h?o`
                  <div class="header-stats">
                    ${h.overdue>0?o`<span class="badge overdue">${h.overdue}</span>`:_}
                    ${h.due_soon>0?o`<span class="badge due_soon">${h.due_soon}</span>`:_}
                    ${h.triggered>0?o`<span class="badge triggered">${h.triggered}</span>`:_}
                  </div>
                `:_}
            ${n?o`
                  <mwc-icon-button
