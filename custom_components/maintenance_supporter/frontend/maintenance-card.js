                ...
                ${this.parts.map(t=>{let i=this._usedParts[t.id],n=i!==void 0;return o`<div class="used-part-row">
                    <label class="used-part-check">
                      <input type="checkbox" .checked=${n}
                        @change=${c=>{let d={...this._usedParts};c.target.checked?d[t.id]=d[t.id]||1:delete d[t.id],this._usedParts=d}} />
                    ...
                    ${n?o`<input class="used-part-qty" type="number" min="0.01" max="999" step="0.01"
                          .value=${String(i)}
                          @input=${c=>{let d=parseFloat(c.target.value);this._usedParts={...this._usedParts,[t.id]:Number.isFinite(d)&&d>=.01?d:1}}} />`:h}
                  </div>`})}
                      ...
                      <div class="consumes-row">
                        <label class="consumes-check">
                          <input
                            type="checkbox"
                            .checked=${n!==void 0}
                            @change=${c=>{let d={...this._consumesParts};c.target.checked?d[i.id]=d[i.id]||1:delete d[i.id],this._consumesParts=d}}
                          />
                        ...
                        ${n!==void 0?o`<input
                              ...
                              step="0.01"
                              .value=${String(n)}
                              @input=${c=>{let d=parseFloat(c.target.value);this._consumesParts={...this._consumesParts,[i.id]:Number.isFinite(d)&&d>=.01?d:1}}}
                            />`:h}
                      </div>
                    `})}
                </div>
...
<script>setTimeout(()=>window.print(),300)<\/script>
</body></html>`),n.document.close()}_downloadSvg(e,t){let i=decodeURIComponent(e.svg_data_uri.replace("data:image/svg+xml,","")),n=new Blob([i],{type:"image/svg+xml"}),c=URL.createObjectURL(n),d=document.createElement("a");d.href=c;let u=this._taskName?`${this._objectName}-${this._taskName}`:this._objectName;d.download=`qr-${rs(u)}-${t}.svg`,d.click(),URL.revokeObjectURL(c)}_close(){this._open=!1,this._viewResult=null,this._completeResult=null,this._error="",this._loading=!1}render(){if(!this._open)return o``;let e=this.lang||this.hass?.language||"en",t=this._taskName?`${r("qr_code",e)}: ${this._objectName} \u2014 ${this._taskName}`:`${r("qr_code",e)}: ${this._objectName}`,i=!!this._viewResult;return o`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
  ...
  `,l([v({attribute:!1})],H.prototype,"hass",2),l([v()],H.prototype,"lang",2),l([p()],H.prototype,"_open",2),l([p()],H.prototype,"_loading",2),l([p()],H.prototype,"_error",2),l([p()],H.prototype,"_viewResult",2),l([p()],H.prototype,"_completeResult",2),l([p()],H.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",H)});function Xt(a,s){let e=a.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return h;let n=a.interval_days??0,c=a.suggested_interval??n;return o`
    <div class="weibull-section">
      ...
      ${ns(t,i,n,c,s)}
      ${os(e,s)}
      ${e?.confidence_interval_low!=null?ls(e,a,s):h}
    </div>
  ...
  `}function ns(a,s,e,t,i){let b=Math.max(e,t,s,1)*1.3,E=50,w=[];for(let j=0;j<=E;j++){let q=j/E*b,bi=1-Math.exp(-Math.pow(q/s,a)),yi=32+q/b*260,xi=136-bi*128;w.push([yi,xi])}let P=w.map(([j,q])=>`${j.toFixed(1)},${q.toFixed(1)}`).join(" "),ze="M32,136 "+w.map(([j,q])=>`L${j.toFixed(1)},${q.toFixed(1)}`).join(" ")+` L${w[E][0].toFixed(1)},136 Z`,ie=32+e/b*260,xe=1-Math.exp(-Math.pow(e/s,a)),$e=136-xe*128,fi=((1-xe)*100).toFixed(0),ot=32+t/b*260,vi=[0,.25,.5,.75,1];return o`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${r("chart_weibull",i)}">
        ${vi.map(j=>{let q=136-j*128;return ae`
            <line x1="${32}" y1="${q.toFixed(1)}" x2="${292}" y2="${q.toFixed(1)}"
              stroke="var(--divider-color)" stroke-width="0.5" stroke-dasharray="${j===.5?"4,3":h}" />
        ...
        <path d="${ze}" fill="var(--primary-color, #03a9f4)" opacity="0.08" />
        ...
        ${e>0?ae`
          ...
          <circle cx="${ie.toFixed(1)}" cy="${$e.toFixed(1)}" r="3"
            fill="var(--primary-color, #03a9f4)" />
          <text x="${(ie+4).toFixed(1)}" y="${($e-6).toFixed(1)}" fill="var(--primary-color, #03a9f4)"
  ...
  `}function ls(a,s,e){let t=a.confidence_interval_low,i=a.confidence_interval_high,n=s.suggested_interval??s.interval_days??0,c=s.interval_days??0,d=Math.max(0,t-5),_=i+5-d,f=(t-d)/_*100,m=(i-t)/_*100,y=(n-d)/_*100,b=c>0?(c-d)/_*100:-1;return o`
      ...
      <div class="prediction-grid">
        ...
        ${i?o`
          <div class="prediction-item">
            ...
            ${a.threshold_prediction_date?o`<span class="prediction-date">${D(a.threshold_prediction_date,s)}</span>`:h}
        ...
        ${n&&e.environmental?o`
          <div class="prediction-item">
            ...
            ${a.environmental_entity?o`<span class="prediction-entity entity-link" @click=${d=>Bt(d,a.environmental_entity)}>${a.environmental_entity}</span>`:h}
          </div>
        `:h}
      </div>
    </div>
  `}var ii=x(()=>{"use strict";C();I()});function si(a,s,e,t){let i=Math.max(a||1,s);return o`
  ...
  `}function oi(a,s){return ds(a,s)}function ds(a,s){let e=a.seasonal_factors??a.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return h;let t=a.interval_analysis?.seasonal_reason,i=new Date().getMonth(),n=300,c=100,d=8,_=c-d-4,f=Math.max(...e,1.5),m=n/12,y=m*.65,b=d+_-1/f*_;return o`
  ...
  `}var ai,li=x(()=>{"use strict";C();I();ai=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"]});var T,di=x(()=>{"use strict";C();N();I();te();ei();ii();ri();li();T=class extends k{constructor(){super(...arguments);this._open=!1;this._entryId=null;this._taskId=null;this._task=null;this._objectName="";this._busy=!1;this._error="";this._showSkip=!1;this._showReset=!1;this._showDetails=!1;this._showAdaptive=!1;this._skipReason="";this._resetDate="";this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._toast="";this._featuresLoaded=!1}get _lang(){return this.hass?.language||"en"}async openFor(e,t){this._entryId=e,this._taskId=t,this._error="",this._showSkip=!1,this._showReset=!1,this._showAdaptive=!1,this._skipReason="",this._resetDate=new Date().toISOString().slice(0,10),this._open=!0,await Promise.all([this._loadTask(),this._loadFeatures()])}async _loadFeatures(){if(!this._featuresLoaded)try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});e?.features&&(this._features={...this._features,...e.features}),this._featuresLoaded=!0}catch{}}close(){this._open=!1,this._task=null,this._error=""}async _loadTask(){if(!(!this._entryId||!this._taskId))try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._objectName=e.object?.name||"";let t=(e.tasks||[]).find(i=>i.id===this._taskId);this._task=t??null}catch(e){this._error=L(e,this._lang)}}async _runWs(e){this._busy=!0,this._error="";try{return await this.hass.connection.sendMessagePromise(e),this._busy=!1,!0}catch(t){return this._error=L(t,this._lang),this._busy=!1,!1}}_notifyChanged(e){this.dispatchEvent(new CustomEvent("task-action-fired",{detail:{entry_id:this._entryId,task_id:this._taskId,action:e},bubbles:!0,composed:!0}))}_onComplete(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(O(),B)).then(({openCompleteDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,checklist:this._task.checklist||[],adaptive_enabled:!!this._task.adaptive_config?.enabled})&&(this._notifyChanged("complete"),this.close())})}async _onSkipConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/skip",entry_id:this._entryId,task_id:this._taskId,reason:this._skipReason.trim()||null})&&(this._notifyChanged("skip"),this.close())}async _onResetConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/reset",entry_id:this._entryId,task_id:this._taskId,date:this._resetDate||void 0})&&(this._notifyChanged("reset"),this.close())}_onEdit(){!this._entryId||!this._taskId||Promise.resolve().then(()=>(O(),B)).then(({openEditTaskDialog:e})=>{e(this._entryId,this._taskId),this.close()})}_onQr(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(O(),B)).then(({openQrDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,object_name:this._objectName}),this.close()})}async _onDelete(){if(!this._entryId||!this._taskId)return;let e=r("delete_task_confirm",this._lang)||`Delete "${this._task?.name}"?`;if(!window.confirm(e))return;await this._runWs({type:"maintenance_supporter/task/delete",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("delete"),this.close())}async _onArchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/archive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("archive"),this.close())}async _onUnarchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/unarchive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("unarchive"),this.close())}_onOpenInPanel(){if(!this._entryId||!this._taskId)return;let e=`/maintenance-supporter?entry_id=${encodeURIComponent(this._entryId)}&task_id=${encodeURIComponent(this._taskId)}`;history.pushState(null,"",e),window.dispatchEvent(new CustomEvent("location-changed")),this.close()}async _applySuggestion(){if(!this._entryId||!this._taskId||!this._task?.suggested_interval)return;await this._runWs({type:"maintenance_supporter/task/apply_suggestion",entry_id:this._entryId,task_id:this._taskId,interval:this._task.suggested_interval})&&(this._toast=r("suggestion_applied",this._lang)||"Applied",this._notifyChanged("apply_suggestion"),await this._loadTask(),setTimeout(()=>{this._toast=""},2500))}async _reanalyzeInterval(){if(!(!this._entryId||!this._taskId)){this._busy=!0,this._error="";try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:this._entryId,task_id:this._taskId});this._toast=e.recommended_interval?`${r("reanalyze_result",this._lang)||"Recomputed"}: ${e.recommended_interval}d (${e.data_points} pts)`:r("reanalyze_insufficient_data",this._lang)||"Not enough data",await this._loadTask(),setTimeout(()=>{this._toast=""},3500)}catch(e){this._error=L(e,this._lang)}finally{this._busy=!1}}}_onEditHistoryEntry(e){!this._entryId||!this._taskId||Promise.resolve().then(()=>(O(),B)).then(({openHistoryEditDialog:t})=>{t({entry_id:this._entryId,task_id:this._taskId,original_timestamp:e.timestamp,type:e.type,timestamp:e.timestamp,notes:e.notes??null,cost:e.cost??null,duration:e.duration??null,completed_by:e.completed_by??null})})}_renderRecommendation(e){if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return h;let t=this._lang;return o`
    ...
    `}_renderAdaptive(e){let t=this._lang,i=this._features.adaptive&&e.suggested_interval&&e.suggested_interval!==e.interval_days,n=e.degradation_trend!=null&&e.degradation_trend!=="insufficient_data"||e.days_until_threshold!=null||e.environmental_factor!=null&&e.environmental_factor!==1,c=this._features.adaptive&&e.interval_analysis?.weibull_beta!=null&&e.interval_analysis?.weibull_eta!=null,d=this._features.seasonal&&e.seasonal_factor&&e.seasonal_factor!==1;return!i&&!n&&!c&&!d?o`<div class="adaptive-empty">
      ...
      <div class="adaptive-stack">
        ...
        ${n?ti(e,t,this._features):h}
        ${c?Xt(e,t):h}
        ${d?o`
          ${ni(e,t,this._features)}
          ${e.seasonal_factors?.length===12||e.interval_analysis?.seasonal_factors?.length===12?oi(e,t):h}
        `:h}
      </div>
    `}_renderDetails(e){let t=this._lang,i=e.history||[],n=i.filter(u=>u.type==="completed"),c=n.reduce((u,_)=>u+(typeof _.cost=="number"?_.cost:0),0),d=(()=>{let u=n.map(_=>typeof _.duration=="number"?_.duration:null).filter(_=>_!=null);return u.length?Math.round(u.reduce((_,f)=>_+f,0)/u.length):null})();return o`
      <div class="details">
                ...
                ${[...i].reverse().slice(0,20).map(u=>{let _=["completed","reset","skipped"].includes(u.type);return o`
                    <div class="history-entry">
                      <div class="history-line">
                        ...
                        <span class="history-date">${Dt(u.timestamp,t)}</span>
    ...
    `}render(){if(!this._open)return h;let e=this._lang,t=this._task,i=this.hass?.user?.is_admin??!0;return o`
      ...
      <div class="dialog" role="dialog" aria-modal="true">
        ${t?o`
              <div class="header">
                <div class="title">
                  <span class="status-dot" style="background: ${X[t.status]||"#ccc"}"></span>
                  <span class="task-name">${t.name}</span>
                </div>
                <div class="object">
                  <button class="link-inline" @click=${()=>{this._entryId&&Promise.resolve().then(()=>(O(),B)).then(({openObjectQuickActions:n})=>{n(this._entryId),this.close()})}}>${this._objectName}</button>
                </div>
                <div class="quick-info">
                  ${t.next_due?o`<span><strong>${r("next_due",e)||"Next due"}:</strong> ${D(t.next_due,e)}</span>`:h}
                  ${t.last_performed?o`<span><strong>${r("last_performed",e)||"Last"}:</strong> ${D(t.last_performed,e)}</span>`:h}
                  ${t.schedule?.kind&&!["manual","one_time"].includes(t.schedule.kind)||t.interval_days!=null?o`<span><strong>${r("interval",e)||"Interval"}:</strong> ${Vt(t,e)}</span>`:h}
                </div>
              </div>
  ...
  `],l([v({attribute:!1})],T.prototype,"hass",2),l([p()],T.prototype,"_open",2),l([p()],T.prototype,"_entryId",2),l([p()],T.prototype,"_taskId",2),l([p()],T.prototype,"_task",2),l([p()],T.prototype,"_objectName",2),l([p()],T.prototype,"_busy",2),l([p()],T.prototype,"_error",2),l([p()],T.prototype,"_showSkip",2),l([p()],T.prototype,"_showReset",2),l([p()],T.prototype,"_showDetails",2),l([p()],T.prototype,"_showAdaptive",2),l([p()],T.prototype,"_skipReason",2),l([p()],T.prototype,"_resetDate",2),l([p()],T.prototype,"_features",2),l([p()],T.prototype,"_toast",2);customElements.get("maintenance-task-quick-actions-dialog")||customElements.define("maintenance-task-quick-actions-dialog",T)});function ci(a){return!!a&&/^https?:\/\//i.test(a)}var pi=x(()=>{"use strict"});var U,hi=x(()=>{"use strict";C();pi();N();I();te();U=class extends k{constructor(){super(...arguments);this._open=!1;this._entryId=null;this._data=null;this._busy=!1;this._error=""}get _lang(){return this.hass?.language||"en"}async openFor(e){this._entryId=e,this._error="",this._open=!0,await this._load()}close(){this._open=!1,this._data=null,this._error=""}async _load(){if(this._entryId)try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._data=e}catch(e){this._error=L(e,this._lang)}}_onEditObject(){!this._entryId||!this._data||Promise.resolve().then(()=>(O(),B)).then(({openEditObjectDialog:e})=>{e(this._entryId,this._data.object),this.close()})}_onAddTask(){this._entryId&&Promise.resolve().then(()=>(O(),B)).then(({openCreateTaskDialog:e})=>{e(),this.close()})}async _onDelete(){if(!this._entryId||!this._data)return;let e=r("delete_object_confirm",this._lang)||`Delete "${this._data.object.name}" and all its tasks?`;if(window.confirm(e)){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:this._entryId}),this.dispatchEvent(new CustomEvent("object-deleted",{detail:{entry_id:this._entryId},bubbles:!0,composed:!0})),this.close()}catch(t){this._error=L(t,this._lang)}finally{this._busy=!1}}}async _onArchiveObject(){if(!this._entryId||!this._data)return;let e=!!this._data.object.archived;if(!e){let t=r("confirm_archive_object",this._lang)||"Archive this object and its tasks?";if(!window.confirm(t))return}this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:e?"maintenance_supporter/object/unarchive":"maintenance_supporter/object/archive",entry_id:this._entryId}),this.dispatchEvent(new CustomEvent("object-changed",{detail:{entry_id:this._entryId},bubbles:!0,composed:!0})),this.close()}catch(t){this._error=L(t,this._lang)}finally{this._busy=!1}}_onTaskClick(e){this._entryId&&Promise.resolve().then(()=>(O(),B)).then(({openTaskQuickActions:t})=>{t(this._entryId,e)})}render(){if(!this._open)return h;let e=this._lang,t=this._data,i=t?.object,n=t?.tasks||[],c=this.hass?.user?.is_admin??!0;return o`
      <div class="backdrop" @click=${this.close}></div>
      <div class="dialog" role="dialog" aria-modal="true">
        ${t&&i?o`
              <div class="header">
                      ...
                      <div class="task-list">
                        ${n.map(d=>o`
                          <div class="task-row" @click=${()=>this._onTaskClick(d.id)}>
                            <span class="status-dot" style="background: ${X[d.status]||"#ccc"}"></span>
      ...
      <div class="meta">
        ${i.map(([n,c])=>o`
            <div class="meta-item">
              ...
              <span class="meta-value">${ci(c)?o`<a href="${c}" target="_blank" rel="noopener noreferrer">${c}</a>`:c}</span>
            </div>
          `)}
      </div>
    `}};U.styles=S`
  ...
  `,l([v({attribute:!1})],U.prototype,"hass",2),l([p()],U.prototype,"_open",2),l([p()],U.prototype,"_entryId",2),l([p()],U.prototype,"_data",2),l([p()],U.prototype,"_busy",2),l([p()],U.prototype,"_error",2);customElements.get("maintenance-object-quick-actions-dialog")||customElements.define("maintenance-object-quick-actions-dialog",U)});var B={};wi(B,{openCompleteDialog:()=>vs,openCreateObjectDialog:()=>rt,openCreateTaskDialog:()=>at,openEditObjectDialog:()=>gs,openEditTaskDialog:()=>ms,openHistoryEditDialog:()=>fs,openObjectQuickActions:()=>ys,openQrDialog:()=>bs,openTaskQuickActions:()=>nt});function Ne(){return document.querySelector("home-assistant")?.hass}function W(a){let s=document.body.querySelector(a);return s||(s=document.createElement(a),document.body.appendChild(s)),s}function G(a){let s=Ne();if(!s)return!1;a.hass=s;let e=s.language||"en";return Pe(e)||Re(e).then(()=>{a.requestUpdate?.()}),!0}function mi(a){return je||(je=a.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).then(s=>({features:s.features??ui.features,defaultWarningDays:s.general?.default_warning_days??7})).catch(()=>ui),je)}function rt(){let a=W(_i);return G(a)?(a.openCreate(),!0):!1}function gs(a,s){let e=W(_i);return G(e)?(e.openEdit(a,s),!0):!1}function at(){let a=W(gi);if(!G(a))return!1;let s=Ne();return s?((async()=>{let e=await mi(s),t=a;t.checklistsEnabled=e.features.checklists,t.scheduleTimeEnabled=e.features.schedule_time,t.completionActionsEnabled=e.features.completion_actions,t.defaultWarningDays=e.defaultWarningDays,t.openCreate()})(),!0):!1}function ms(a,s){let e=W(gi);if(!G(e))return!1;let t=Ne();return t?((async()=>{try{let[i,n]=await Promise.all([t.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:a}),mi(t)]),c=(i.tasks||[]).find(u=>u.id===s);if(!c){console.warn(`openEditTaskDialog: task ${s} not found in entry ${a}`);return}let d=e;d.checklistsEnabled=n.features.checklists,d.scheduleTimeEnabled=n.features.schedule_time,d.completionActionsEnabled=n.features.completion_actions,d.defaultWarningDays=n.defaultWarningDays,await d.openEdit(a,c)}catch(i){console.warn("openEditTaskDialog: failed to load task/features",i)}})(),!0):!1}function fs(a){let s=W(cs);return G(s)?(s.openEdit(a),!0):!1}function vs(a){let s=W(ps);return G(s)?(s.entryId=a.entry_id,s.taskId=a.task_id,s.taskName=a.task_name,s.checklist=a.checklist??[],s.adaptiveEnabled=!!a.adaptive_enabled,s.requiredFields=a.required_completion_fields??[],s.lang=Ne()?.language||"en",s.open(),!0):!1}function bs(a){let s=W(hs);return G(s)?(s.openForTask(a.entry_id,a.task_id,a.object_name,a.task_name),!0):!1}function nt(a,s){let e=W(us);return G(e)?(e.openFor(a,s),!0):!1}function ys(a){let s=W(_s);return G(s)?(s.openFor(a),!0):!1}var _i,gi,cs,ps,hs,us,_s,ui,je,O=x(()=>{"use strict";Gt();Yt();et();Jt();Qt();di();hi();I();_i="maintenance-object-dialog",gi="maintenance-task-dialog",cs="maintenance-history-edit-dialog",ps="maintenance-complete-dialog",hs="maintenance-qr-dialog",us="maintenance-task-quick-actions-dialog",_s="maintenance-object-quick-actions-dialog";ui={features:{adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1},defaultWarningDays:7},je=null});C();N();I();Xe();C();N();I();var Vi=["overdue","triggered","due_soon","ok"],M=class extends k{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-card"};this._objects=[];this._loadingObjects=!0;this._loadError=!1;this._views=[];this._objectsLoaded=!1;this._onEntitiesChanged=e=>{this._valueChanged("entity_ids",e.detail.value||[])}}get _lang(){return this.hass?.language||"en"}setConfig(e){this._config={...e}}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._objectsLoaded&&(this._objectsLoaded=!0,this._loadObjects())}async _loadObjects(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"});this._objects=e.objects||[],this._loadError=!1}catch{this._objects=[],this._loadError=!0}this._loadingObjects=!1;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"});this._views=e.views||[]}catch{this._views=[]}}_valueChanged(e,t){let i={...this._config,[e]:t};(Array.isArray(t)&&t.length===0||t==="")&&delete i[e],this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i}}))}_toggleStatus(e,t){let i=new Set(this._config.filter_status||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_status",[...i])}_toggleObject(e,t){let i=new Set(this._config.filter_objects||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_objects",[...i])}render(){let e=this._lang,t=new Set(this._config.filter_status||[]),i=new Set(this._config.filter_objects||[]),n=[...this._objects].map(d=>d.object.name).sort((d,u)=>d.localeCompare(u)),c=[];for(let d of this._objects)for(let u of d.tasks)u.sensor_entity_id&&c.push(u.sensor_entity_id),u.binary_sensor_entity_id&&c.push(u.binary_sensor_entity_id);return o`
      <div class="editor">
        <ha-textfield
          ...
          <div class="chip-row">
            ${Vi.map(d=>o`
              <label class="chip ${t.has(d)?"active":""}">
                <input type="checkbox"
                  .checked=${t.has(d)}
                  @change=${u=>this._toggleStatus(d,u.target.checked)} />
                ${r(d,e)}
              </label>
            `)}
          </div>
                ...
                <div class="object-list">
                  ${n.map(d=>o`
                    <label class="object-row">
                      <input type="checkbox"
                        .checked=${i.has(d)}
                        @change=${u=>this._toggleObject(d,u.target.checked)} />
                      <span>${d}</span>
                    </label>
                  `)}
                </div>
  ...
  `,l([v({attribute:!1})],M.prototype,"hass",2),l([p()],M.prototype,"_config",2),l([p()],M.prototype,"_objects",2),l([p()],M.prototype,"_loadingObjects",2),l([p()],M.prototype,"_loadError",2),l([p()],M.prototype,"_views",2);customElements.get("maintenance-supporter-card-editor")||customElements.define("maintenance-supporter-card-editor",M);et();O();var z=class extends k{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-card"};this._objects=[];this._stats=null;this._unsub=null;this._viewFilters=null;this._userNames={};this._userService=null;this._userNamesLoaded=!1;this._dataLoaded=!1;this._lastConnection=null;this._onCompleted=async()=>{await this._loadData()}}get _lang(){return this.hass?.language||"en"}static getConfigElement(){return document.createElement("maintenance-supporter-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-card",show_header:!0,show_actions:!0,filter_status:["overdue","triggered","due_soon"],max_items:10}}setConfig(e){let t=e.view_id!==this._config.view_id;this._config=e,t&&this._dataLoaded&&this.hass&&this._loadViewFilters()}getCardSize(){return 3}connectedCallback(){super.connectedCallback()}disconnectedCallback(){super.disconnectedCallback(),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null}updated(e){super.updated(e),e.has("hass")&&Ut(this.hass?.locale);let t=this.hass?.language;if(t&&!Pe(t)&&Re(t).then(()=>this.requestUpdate()),this.hass&&!this._userNamesLoaded&&this._config.show_assignee!==!1&&this._objects.some(i=>i.tasks.some(n=>n.responsible_user_id))&&this._loadUserNames(),e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=e.objects,this._stats=t}catch{}await this._loadViewFilters()}_assigneeName(e){if(this._config.show_assignee===!1)return"";let t=e.responsible_user_id;return t&&this._userNames[t]||""}async _loadUserNames(){this._userNamesLoaded=!0,this._userService?this._userService.updateHass(this.hass):this._userService=new oe(this.hass);try{let e=await this._userService.getUsers();this._userNames=Object.fromEntries(e.map(t=>[t.id,t.name]))}catch{}}async _loadViewFilters(){if(!this._config.view_id){this._viewFilters=null;return}try{let t=((await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"})).views||[]).find(i=>i.id===this._config.view_id);this._viewFilters=t?t.filters:null}catch{this._viewFilters=null}}async _subscribe(){try{let e=await this.hass.connection.subscribeMessage(t=>{let i=t;this._objects=i.objects},{type:"maintenance_supporter/subscribe"});if(!this.isConnected){e();return}this._unsub=e}catch{}}get _flatTasks(){let e=[],{filter_status:t,filter_objects:i,entity_ids:n,filter_due_min_days:c,filter_due_max_days:d,max_items:u}=this._config,_=n?.length?new Set(n):null,f=c!==void 0||d!==void 0,m=this._viewFilters,y=m?.user_id==="current_user"?this.hass.user?.id??null:m?.user_id??null;for(let E of this._objects)if(!(i?.length&&!i.includes(E.object.name))){for(let w of E.tasks)if(!w.is_done&&!(w.archived||E.object.archived)&&!(t?.length&&!t.includes(w.status))&&!(_&&!(w.sensor_entity_id&&_.has(w.sensor_entity_id)||w.binary_sensor_entity_id&&_.has(w.binary_sensor_entity_id)))){if(f){let P=w.days_until_due;if(P==null||c!==void 0&&P<c||d!==void 0&&P>d)continue}m&&(m.status&&w.status!==m.status||m.label&&!(w.labels||[]).includes(m.label)||y&&w.responsible_user_id!==y)||e.push({entry_id:E.entry_id,object_name:E.object.name,task:w})}}let b={overdue:0,triggered:1,due_soon:2,ok:3};return e.sort((E,w)=>{let P=(b[E.task.status]??9)-(b[w.task.status]??9);return P!==0?P:(E.task.days_until_due??1/0)-(w.task.days_until_due??1/0)}),u&&u>0?e.slice(0,u):e}_openTaskDetail(e,t){nt(e,t)}render(){let e=this._lang,t=this._config.title||r("maintenance",e),i=this._config.show_header!==!1,n=this._config.show_actions!==!1,c=this._config.compact||!1,d=this._flatTasks,u=this._stats;return o`
      <ha-card>
        ...
        ${d.length===0?this._objects.some(_=>_.tasks.length>0)?o`<div class="empty-card">
              ...
              <div class="task-list ${c?"compact":""}">
                ${d.map(({entry_id:_,object_name:f,task:m})=>o`
                    <div class="task-item clickable"
                         @click=${()=>this._openTaskDetail(_,m.id)}
                         title="${r("open_task",e)||"Open task"}">
                      <div class="status-dot" style="background: ${X[m.status]||"#ccc"}"></div>
                      <div class="task-info">
                        <div class="task-name">
                          ${m.name}
                      ...
                      ${n?o`
                            <mwc-icon-button
                              class="complete-btn"
                              title="${r("complete",e)}"
                              @click=${y=>{y.stopPropagation();let b=this.shadowRoot.querySelector("maintenance-complete-dialog");b.entryId=_,b.taskId=m.id,b.taskName=m.name,b.checklist=m.checklist||[],b.adaptiveEnabled=!!m.adaptive_config?.enabled,b.taskType=m.type||"",b.readingUnit=m.reading_unit||"",b.requiredFields=m.required_completion_fields||[],b.lang=e;let E=this._objects.find(P=>P.entry_id===_),w=!!m.part_ref;b.parts=w?[]:E?.parts||[],b.consumesParts=w?[]:m.consumes_parts||[],b.open()}}
                            >
                              <ha-icon icon="mdi:check"></ha-icon>
                            </mwc-icon-button>
                          `:h}
