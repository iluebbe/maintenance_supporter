      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${zi.map(s=>l`<option value=${s} ?selected=${s===this._type}>${a(s,e)}</option>`)}
            </select>
          </div>
          <div class="select-row">
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function Ui(r,i,e,t,s){let v=Math.max(e,t,i,1)*1.3,w=50,N=[];for(let R=0;R<=w;R++){let z=R/w*v,li=1-Math.exp(-Math.pow(z/i,r)),ci=32+z/v*260,di=136-li*128;N.push([ci,di])}let me=N.map(([R,z])=>`${R.toFixed(1)},${z.toFixed(1)}`).join(" "),Le="M32,136 "+N.map(([R,z])=>`L${R.toFixed(1)},${z.toFixed(1)}`).join(" ")+` L${N[w][0].toFixed(1)},136 Z`,X=32+e/v*260,fe=1-Math.exp(-Math.pow(e/i,r)),ve=136-fe*128,ni=((1-fe)*100).toFixed(0),et=32+t/v*260,oi=[0,.25,.5,.75,1];return l`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${a("chart_weibull",s)}">
        ${oi.map(R=>{let z=136-R*128;return ie`
