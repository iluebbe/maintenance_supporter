...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function Ui(a,i,e,t,s){let v=Math.max(e,t,i,1)*1.3,w=50,N=[];for(let j=0;j<=w;j++){let z=j/w*v,li=1-Math.exp(-Math.pow(z/i,a)),ci=32+z/v*260,di=136-li*128;N.push([ci,di])}let me=N.map(([j,z])=>`${j.toFixed(1)},${z.toFixed(1)}`).join(" "),Le="M32,136 "+N.map(([j,z])=>`L${j.toFixed(1)},${z.toFixed(1)}`).join(" ")+` L${N[w][0].toFixed(1)},136 Z`,X=32+e/v*260,fe=1-Math.exp(-Math.pow(e/i,a)),ve=136-fe*128,ni=((1-fe)*100).toFixed(0),et=32+t/v*260,oi=[0,.25,.5,.75,1];return l`
