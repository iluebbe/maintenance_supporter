      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          ${this.parts.length?o`<div class="used-parts">
                ...
                ${this.parts.map(t=>{let i=D({part_id:t.id,entry_id:t.entry_id}),n=this._usedParts[i],c=n!==void 0,d=t.entry_id?{part_id:t.id,quantity:1,entry_id:t.entry_id}:{part_id:t.id,quantity:1};return o`<div class="used-part-row">
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function ws(a,s,e,t,i){let g=Math.max(e,t,s,1)*1.3,x=50,H=[];for(let z=0;z<=x;z++){let O=z/x*g,Ci=1-Math.exp(-Math.pow(O/s,a)),Li=32+O/g*260,Ii=136-Ci*128;H.push([Li,Ii])}let j=H.map(([z,O])=>`${z.toFixed(1)},${O.toFixed(1)}`).join(" "),S="M32,136 "+H.map(([z,O])=>`L${z.toFixed(1)},${O.toFixed(1)}`).join(" ")+` L${H[x][0].toFixed(1)},136 Z`,C=32+e/g*260,$e=1-Math.exp(-Math.pow(e/s,a)),ke=136-$e*128,Ai=((1-$e)*100).toFixed(0),gt=32+t/g*260,Ti=[0,.25,.5,.75,1];return o`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${r("chart_weibull",i)}">
        ${Ti.map(z=>{let O=136-z*128;return oe`
            <line x1="${32}" y1="${O.toFixed(1)}" x2="${292}" y2="${O.toFixed(1)}"
              stroke="var(--divider-color)" stroke-width="0.5" stroke-dasharray="${z===.5?"4,3":h}" />
      ...
      <div class="prediction-grid">
        ...
        ${i?o`
          <div class="prediction-item">
            ...
            ${a.threshold_prediction_date?o`<span class="prediction-date">${W(a.threshold_prediction_date,s)}</span>`:h}
        ...
        ${t?o`
              <div class="header">
                ...
                <div class="quick-info">
                  ${t.next_due?o`<span><strong>${r("next_due",e)||"Next due"}:</strong> ${W(t.next_due,e)}</span>`:h}
                  ${t.last_performed?o`<span><strong>${r("last_performed",e)||"Last"}:</strong> ${W(t.last_performed,e)}</span>`:h}
