    ...
    `}_renderCondition(e,t){let i=this._lang,a=t+1;return o`
      <div class="compound-condition">
        ...
        <div class="select-row">
          ...
          >
            ${Ts.map(l=>o`<option value=${l} ?selected=${l===e.type}>${n(l,i)}</option>`)}
          </select>
        </div>
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          ${this._scheduleType==="one_time"?o`
                ...
                ></ms-date-field>
              `:p}
          ${this.scheduleTimeEnabled&&As.includes(this._scheduleType)?o`
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function pn(r,s,e,t,i){let y=Math.max(e,t,s,1)*1.3,m=50,$=[];for(let K=0;K<=m;K++){let Y=K/m*y,rr=1-Math.exp(-Math.pow(Y/s,r)),nr=32+Y/y*260,ar=136-rr*128;$.push([nr,ar])}let z=$.map(([K,Y])=>`${E(K)},${E(Y)}`).join(" "),I="M32,136 "+$.map(([K,Y])=>`L${E(K)},${E(Y)}`).join(" ")+` L${E($[m][0])},136 Z`,T=32+e/y*260,j=1-Math.exp(-Math.pow(e/s,r)),ve=136-j*128,ir=q((1-j)*100,i,0),ui=32+t/y*260,sr=[0,.25,.5,.75,1];return o`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_weibull",i)}">
        ...
        ${e>0?xe`
          <line x1="${E(T)}" y1="${8}" x2="${E(T)}" y2="${E(136)}"
            stroke="var(--primary-color, #03a9f4)" stroke-width="1.5" stroke-dasharray="4,3" />
          <circle cx="${E(T)}" cy="${E(ve)}" r="3"
            fill="var(--primary-color, #03a9f4)" />
          <text x="${E(T+4)}" y="${E(ve-6)}" fill="var(--primary-color, #03a9f4)"
  ...
  `}function Bs(r,s){return _n(r,s)}function _n(r,s){let e=r.seasonal_factors??r.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return p;let t=r.interval_analysis?.seasonal_reason,i=new Date().getMonth(),a=300,l=100,c=8,_=l-c-4,f=Math.max(...e,1.5),x=a/12,v=x*.65,y=c+_-1/f*_;return o`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${a} ${l}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_seasonal",s)}">
        ...
        ${e.map((m,$)=>{let z=m/f*_,I=$*x+(x-v)/2,T=c+_-z,j=$===i,ve=m<1?"var(--success-color, #4caf50)":m>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return xe`
            <rect x="${E(I)}" y="${E(T)}"
  ...
  `,d([b({attribute:!1})],B.prototype,"hass",2),d([b({type:Boolean})],B.prototype,"flat",2),d([u()],B.prototype,"_ov",2),d([u()],B.prototype,"_loading",2),d([u()],B.prototype,"_marking",2),d([u()],B.prototype,"_error",2),d([u()],B.prototype,"_history",2),d([u()],B.prototype,"_rosterSort",2),d([u()],B.prototype,"_typeFilter",2),d([u()],B.prototype,"_recorded",2);var Xt=B;customElements.get("maintenance-battery-fleet-section")||customElements.define("maintenance-battery-fleet-section",Xt);var Ce=class extends A{constructor(){super(...arguments);this._config={type:"custom:maintenance-battery-fleet-card"}}static getStubConfig(){return{type:"custom:maintenance-battery-fleet-card"}}setConfig(e){this._config=e}getCardSize(){return 6}render(){return this.hass?o`
