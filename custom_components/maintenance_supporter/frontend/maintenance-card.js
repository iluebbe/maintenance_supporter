...
`).map(_=>_.trim()).filter(Boolean).slice(0,100);e.checklist=c.length?c:null}if(this.completionActionsEnabled){let c=this._actionService.trim();if(c&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(c)){let m={service:c},x=this._actionTargetEntity.trim();x&&(m.target={entity_id:x});let v=this._buildActionData();Object.keys(v).length>0&&(m.data=v),e.on_complete_action=m}else e.on_complete_action=null;let _={};this._qcNotes.trim()&&(_.notes=this._qcNotes.trim());let u=parseFloat(this._qcCost);!isNaN(u)&&u>=0&&(_.cost=u);let b=parseInt(this._qcDuration,10);!isNaN(b)&&b>=0&&(_.duration=b),this._qcFeedback&&(_.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(_).length?_:null}let i=await this.hass.connection.sendMessagePromise(e),o=this._taskId||i?.task_id,d=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(o&&this._scheduleType==="sensor_based"&&d)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:o,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=I(e,this._lang,r("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return h;let e=this._lang,t=this._triggerType==="compound";return n`
      ...
      <div class="select-row">
        ...
        >
          ${Fi.map(i=>n`<option value=${i} ?selected=${i===this._triggerType}>${r(i,e)}</option>`)}
        </select>
      </div>
    ...
    `}_patchCondition(e,t){this._compoundConditions=this._compoundConditions.map((i,o)=>o===e?{...i,...t}:i)}_addCondition(){this._compoundConditions=[...this._compoundConditions,Wi()]}_removeCondition(e){this._compoundConditions=this._compoundConditions.filter((t,i)=>i!==e)}_renderCompoundEditor(){let e=this._lang;return n`
      <div class="select-row">
    ...
    `}_renderConditionTypeFields(e,t){let i=this._lang;return e.type==="threshold"?n`
      ...
      ${this._endsMode==="until"?n`
        ...
        ></ms-textfield>`:h}
    `}_renderCalendarFields(){let e=this._lang,t=Gi(e);if(this._scheduleType==="weekdays")return n`
      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="content">
          ...
          <div class="select-row">
            ...
            >
              ${Ui.map(i=>n`<option value=${i} ?selected=${i===this._type}>${r(i,e)}</option>`)}
            </select>
          </div>
          ...
          <div class="select-row">
            ...
            >
              ${Oi.map(i=>n`<option value=${i} ?selected=${i===this._priority}>${r("priority_"+i,e)}</option>`)}
            </select>
          </div>
          <div class="field">
          ...
          <div class="select-row">
            ...
            >
              ${Di.map(i=>n`<option value=${i} ?selected=${i===this._scheduleType}>${r(i,e)}</option>`)}
            </select>
          </div>
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function Zi(a,s,e,t,i){let v=Math.max(e,t,s,1)*1.3,E=50,M=[];for(let R=0;R<=E;R++){let P=R/E*v,_i=1-Math.exp(-Math.pow(P/s,a)),ui=32+P/v*260,gi=136-_i*128;M.push([ui,gi])}let fe=M.map(([R,P])=>`${R.toFixed(1)},${P.toFixed(1)}`).join(" "),je="M32,136 "+M.map(([R,P])=>`L${R.toFixed(1)},${P.toFixed(1)}`).join(" ")+` L${M[E][0].toFixed(1)},136 Z`,te=32+e/v*260,ve=1-Math.exp(-Math.pow(e/s,a)),be=136-ve*128,pi=((1-ve)*100).toFixed(0),it=32+t/v*260,hi=[0,.25,.5,.75,1];return n`
  ...
  `}function Xi(a,s,e){let t=a.confidence_interval_low,i=a.confidence_interval_high,o=s.suggested_interval??s.interval_days??0,d=s.interval_days??0,c=Math.max(0,t-5),u=i+5-c,b=(t-c)/u*100,m=(i-t)/u*100,x=(o-c)/u*100,v=d>0?(d-c)/u*100:-1;return n`
  ...
  `}function si(a,s){return es(a,s)}function es(a,s){let e=a.seasonal_factors??a.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return h;let t=a.interval_analysis?.seasonal_reason,i=new Date().getMonth(),o=300,d=100,c=8,u=d-c-4,b=Math.max(...e,1.5),m=o/12,x=m*.65,v=c+u-1/b*u;return n`
