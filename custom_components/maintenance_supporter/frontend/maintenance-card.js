...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function ns(a,s,e,t,i){let b=Math.max(e,t,s,1)*1.3,E=50,w=[];for(let j=0;j<=E;j++){let z=j/E*b,bi=1-Math.exp(-Math.pow(z/s,a)),yi=32+z/b*260,xi=136-bi*128;w.push([yi,xi])}let P=w.map(([j,z])=>`${j.toFixed(1)},${z.toFixed(1)}`).join(" "),ze="M32,136 "+w.map(([j,z])=>`L${j.toFixed(1)},${z.toFixed(1)}`).join(" ")+` L${w[E][0].toFixed(1)},136 Z`,ie=32+e/b*260,ye=1-Math.exp(-Math.pow(e/s,a)),xe=136-ye*128,fi=((1-ye)*100).toFixed(0),nt=32+t/b*260,vi=[0,.25,.5,.75,1];return o`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${r("chart_weibull",i)}">
        ...
        ${e>0?ae`
          ...
          <circle cx="${ie.toFixed(1)}" cy="${xe.toFixed(1)}" r="3"
            fill="var(--primary-color, #03a9f4)" />
          <text x="${(ie+4).toFixed(1)}" y="${(xe-6).toFixed(1)}" fill="var(--primary-color, #03a9f4)"
  ...
  `}function oi(a,s){return ds(a,s)}function ds(a,s){let e=a.seasonal_factors??a.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return h;let t=a.interval_analysis?.seasonal_reason,i=new Date().getMonth(),n=300,c=100,d=8,_=c-d-4,f=Math.max(...e,1.5),m=n/12,y=m*.65,b=d+_-1/f*_;return o`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${n} ${c}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${r("chart_seasonal",s)}">
        ...
        ${e.map((E,w)=>{let P=E/f*_,ze=w*m+(m-y)/2,ie=d+_-P,ye=w===i,xe=E<1?"var(--success-color, #4caf50)":E>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return ae`
            <rect x="${ze.toFixed(1)}" y="${ie.toFixed(1)}"
              width="${y.toFixed(1)}" height="${P.toFixed(1)}"
              fill="${xe}" opacity="${ye?1:.5}" rx="2" />
          `})}
      </svg>
