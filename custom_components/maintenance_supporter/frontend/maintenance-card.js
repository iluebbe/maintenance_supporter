...
`});function cs(r,s,e){let t=new Blob([r],{type:e}),i=URL.createObjectURL(t),a=document.createElement("a");a.href=i,a.download=s,a.target="_blank",a.rel="noopener",a.style.display="none",document.body.appendChild(a),a.dispatchEvent(new MouseEvent("click")),document.body.removeChild(a),setTimeout(()=>URL.revokeObjectURL(i),6e4)}var Dt=w(()=>{"use strict"});async function Ur(r,s,e=300){return(await r.connection.sendMessagePromise({type:"auth/sign_path",path:s,expires:e})).path}async function Mt(r,s,e=300){return Ur(r,`/api/maintenance_supporter/document/${s}`,e)}async function ps(r,s,e=""){let t=window.open("about:blank","_blank");try{let i=await Mt(r,s);t&&(t.location.href=new URL(i+e,window.location.origin).href)}catch(i){throw t&&t.close(),i}}var zt=w(()=>{"use strict";Dt()});function dt(r){return!!r&&/^https?:\/\//i.test(r)}var Ut=w(()=>{"use strict"});function Fr(r){let s=(r||"").split(/\s+/).filter(Boolean);return s.length===0?"?":s.length===1?s[0][0].toUpperCase():(s[0][0]+s[s.length-1][0]).toUpperCase()}function Br(r){let s=0;for(let e of r)s=s*31+e.charCodeAt(0)>>>0;return hs[s%hs.length]}function ct(r){return r?{id:r.id,name:r.name,initials:r.initials||Fr(r.name),color:r.color||Br(r.id)}:null}function Ft(r){return r?o`<span class="person-avatar" style="--person-color: ${r.color}" title=${r.name}>${r.initials}</span>`:p}var hs,Bt=w(()=>{"use strict";I();hs=["#c62828","#ad1457","#6a1b9a","#4527a0","#283593","#1565c0","#00838f","#2e7d32","#558b2f","#ef6c00","#6d4c41","#546e7a"]});var Se,Vt=w(()=>{"use strict";Bt();Se=class{constructor(s){this.usersCache=null;this.cacheTimestamp=0;this.CACHE_TTL_MS=6e4;this.hass=s}updateHass(s){this.hass=s}async getUsers(s=!1){let e=Date.now();if(!s&&this.usersCache&&e-this.cacheTimestamp<this.CACHE_TTL_MS)return this.usersCache;try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/users/list"});return this.usersCache=t.users,this.cacheTimestamp=e,this.usersCache}catch(t){return console.error("Failed to fetch users:",t),this.usersCache||[]}}async assignUser(s,e,t){await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/assign_user",entry_id:s,task_id:e,user_id:t})}async getTasksByUser(s){return(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tasks/by_user",user_id:s})).tasks}getUserName(s){return!s||!this.usersCache?null:this.usersCache.find(t=>t.id===s)?.name||null}getPerson(s){return ct(this.getUser(s))}getUser(s){return!s||!this.usersCache?null:this.usersCache.find(e=>e.id===s)||null}getCurrentUserId(){return this.hass.user?.id||null}isCurrentUser(s){return s?s===this.getCurrentUserId():!1}clearCache(){this.usersCache=null,this.cacheTimestamp=0}}});function Vr(r,s){if(s<=0)return 0;let e=typeof r=="number"&&Number.isFinite(r)?Math.trunc(r):0;return e<0?0:e%s}function Wr(r){return!!(r?.phases&&r.phase_sequence&&r.phase_sequence.length>0)}function Wt(r){if(!r||!Wr(r))return null;let s=r.phase_sequence,e=Vr(r.phase_cursor,s.length),t=s[e],i=r.phases?.[t];return i?{id:t,name:i.name,index:e,count:s.length,notes:i.notes,checklist:i.checklist!==void 0?i.checklist:r.checklist??[],consumesParts:i.consumes_parts!==void 0?i.consumes_parts:r.consumes_parts??[],requiredFields:i.required_completion_fields!==void 0?i.required_completion_fields:r.required_completion_fields??[]}:null}function oe(r){let s=Wt(r);return s?`${s.index+1}/${s.count} \xB7 ${s.name}`:""}var pt=w(()=>{"use strict"});function us(){let r=new Uint8Array(4);return(globalThis.crypto??{getRandomValues:s=>s.map(()=>Math.floor(Math.random()*256))}).getRandomValues(r),Array.from(r,s=>s.toString(16).padStart(2,"0")).join("")}function me(r){let s=r?.reading_values;if(!Array.isArray(s))return[];let e=[],t=new Set;for(let i of s){if(!i||typeof i!="object")continue;let a=i;typeof a.id!="string"||!a.id||t.has(a.id)||typeof a.value!="number"||!Number.isFinite(a.value)||(t.add(a.id),e.push({id:a.id,name:typeof a.name=="string"?a.name:a.id,unit:typeof a.unit=="string"?a.unit:null,value:a.value}))}return e}function _s(r){return r.filter(s=>s.type==="completed"&&(s.reading_value!=null||me(s).length>0)).sort((s,e)=>s.timestamp.localeCompare(e.timestamp))}function gs(r){return _s(r??[]).map(s=>({timestamp:s.timestamp,values:me(s)})).filter(s=>s.values.length>0)}function ms(r,s,e){let t;for(let i of r){if(e!==void 0){let l=new Date(i.timestamp).getTime();if(!isNaN(l)&&l>=e)break}let a=i.values.find(l=>l.id===s);a&&(t=a)}return t}function fs(r,s,e){let t=me(s).find(a=>a.id===e);if(!t)return null;let i=null;for(let a of _s(r)){if(a.timestamp>=s.timestamp)break;let l=me(a).find(c=>c.id===e);l&&(i=l.value)}return i==null?null:t.value-i}function vs(r){let s=new Set,e=new Set;for(let t of r){let i=(t.name||"").trim().toLowerCase();i&&(s.has(i)?e.add(t.id):s.add(i))}return e}function bs(r){let s=[],e=new Set,t=new Set;for(let i of r){let a=(i.name||"").trim();if(!(!a||e.has(i.id)||t.has(a.toLowerCase()))&&(e.add(i.id),t.add(a.toLowerCase()),s.push({id:i.id,name:a,unit:(i.unit||"").trim()||null}),s.length>=20))break}return s}var Ye=w(()=>{"use strict"});function G(r){return`${r.entry_id??""}\0${r.part_id}`}function ys(r,s,e,t){let i=!!r.entry_id&&r.entry_id!==s,a=i?r.entry_id:s,l=e.find(f=>f.entry_id===a),c=(l?.parts||[]).find(f=>f.id===r.part_id)||null,h=i&&l?.object?.name||"",_=c?.name||n("shared_part_unknown",t);return{part:c,foreign:i,ownerName:h,label:h?`${_} (${h})`:_}}function xs(r,s,e,t){let{part:i,label:a}=ys(r,s,e,t),l=i&&i.stock!==null&&i.stock!==void 0?` (${i.stock}${i.unit?" "+i.unit:""})`:"",c=i?.storage_location?` \u2014 ${i.storage_location}`:"";return`${r.quantity}\xD7 ${a}${l}${c}`}function ws(r,s,e,t){let a=(e.find(c=>c.entry_id===s)?.parts||[]).map(c=>({...c})),l=new Set(a.map(c=>G({part_id:c.id})));for(let c of r?.consumes_parts||[]){if(!c.entry_id||c.entry_id===s)continue;let h=G(c);if(l.has(h))continue;l.add(h);let{part:_,ownerName:f}=ys(c,s,e,t);a.push({id:c.part_id,name:_?.name||n("shared_part_unknown",t),unit:_?.unit,stock:_?.stock??null,storage_location:_?.storage_location,entry_id:c.entry_id,owner_name:f})}return a}var ht=w(()=>{"use strict";R()});function Ge(r){let s=r.task??null,e=s?Wt(s):null,t=e?e.consumesParts:s?.consumes_parts||[],i=!!s?.part_ref,a=r.objects.find(h=>h.entry_id===r.entryId)?.parts||[],l=i?a.find(h=>h.id===s.part_ref.part_id):void 0,c=r.checklistsEnabled??!0;return{entry_id:r.entryId,task_id:r.taskId,task_name:r.taskName,checklist:e?c?e.checklist:[]:r.checklist??[],adaptive_enabled:!!r.adaptiveEnabled,required_completion_fields:e?e.requiredFields:s?.required_completion_fields||[],task_type:s?.type||"",reading_unit:s?.reading_unit||"",readings:s?.readings||[],reading_history:gs(s?.history),parts:i?[]:ws({consumes_parts:t},r.entryId,r.objects,r.lang),consumes_parts:i?[]:t,phase_label:e?oe(s):"",require_tag_scan:!!s?.require_tag_scan,restock_default:i?l?.restock_quantity??1:null,restock_unit_cost:i?l?.cost??null:null,currency_symbol:r.currencySymbol??"",consumes_info:t.map(h=>xs(h,r.entryId,r.objects,r.lang)),checklist_prefill:s?.checklist_progress||{},via_tag_scan:!!r.viaTagScan}}function Qe(r,s,e){r.entryId=s.entry_id,r.taskId=s.task_id,r.taskName=s.task_name,r.lang=e,r.checklist=s.checklist??[],r.adaptiveEnabled=!!s.adaptive_enabled,r.requiredFields=s.required_completion_fields??[],r.taskType=s.task_type??"",r.readingUnit=s.reading_unit??"",r.readings=s.readings??[],r.readingHistory=s.reading_history??[],r.parts=s.parts??[],r.consumesParts=s.consumes_parts??[],r.phaseLabel=s.phase_label??"",r.requireTagScan=!!s.require_tag_scan,r.restockDefault=s.restock_default??null,r.restockUnitCost=s.restock_unit_cost??null,r.currencySymbol=s.currency_symbol??"",r.consumesInfo=s.consumes_info??[],r.checklistPrefill=s.checklist_prefill??{},r.viaTagScan=!!s.via_tag_scan,r.open({viaTagScan:!!s.via_tag_scan})}var ut=w(()=>{"use strict";Ye();ht();pt()});function Gr(r,s){let e=Yr[r];if(!e)return r;let t=n(e,s);return t&&t!==e?t:r}function Qr(r){let e=r.match(/data\['([^']+)'\]/)?.[1],t;return(t=r.match(/length of value must be at most (\d+)/))?{field:e,rule:"too_long",param:t[1]}:(t=r.match(/length of value must be at least (\d+)/))?{field:e,rule:"too_short",param:t[1]}:(t=r.match(/value must be at most (\S+)/))?{field:e,rule:"value_too_high",param:t[1]}:(t=r.match(/value must be at least (\S+)/))?{field:e,rule:"value_too_low",param:t[1]}:/required key not provided/.test(r)?{field:e,rule:"required"}:(t=r.match(/expected (\w+)/))?{field:e,rule:"wrong_type",param:t[1]}:/value must be one of/.test(r)?{field:e,rule:"invalid_choice"}:/not a valid value/.test(r)?{field:e,rule:"invalid_value"}:{field:e,rule:"unknown"}}function C(r,s,e){if(e=e??n("action_error",s),typeof r=="string")return r;if(typeof r!="object"||r===null)return e;let t=r,i=t.message||t.error?.message||"";if(!i)return e;let a=Qr(i),l=a.field?Gr(a.field,s):"",c=h=>n(h,s).replace("{field}",l).replace("{n}",a.param??"");switch(a.rule){case"too_long":return c("err_too_long");case"too_short":return c("err_too_short");case"value_too_high":return c("err_value_too_high");case"value_too_low":return c("err_value_too_low");case"required":return c("err_required");case"wrong_type":return c("err_wrong_type").replace("{type}",a.param??"");case"invalid_choice":return c("err_invalid_choice");case"invalid_value":return c("err_invalid_value");default:return i||e}}var Yr,le=w(()=>{"use strict";R();Yr={entry_id:"object",name:"name",task_type:"maintenance_type",schedule_type:"schedule_type",interval_days:"interval_days",interval_anchor:"interval_anchor",warning_days:"warning_days",last_performed:"last_performed_optional",notes:"notes_optional",documentation_url:"documentation_url_optional",custom_icon:"custom_icon_optional",nfc_tag_id:"nfc_tag_id_optional",responsible_user_id:"responsible_user",entity_slug:"entity_slug",entity_id:"entity_id",area_id:"area_id_optional",manufacturer:"manufacturer_optional",model:"model_optional",serial_number:"serial_number_optional",installation_date:"installation_date_optional",warranty_expiry:"warranty_expiry_optional",checklist:"checklist_steps_optional",reason:"reason",feedback:"feedback",cost:"cost",duration:"duration",description:"description_optional",group_name:"name",group_description:"description_optional",environmental_entity:"environmental_entity_optional",environmental_attribute:"environmental_attribute_optional",trigger_above:"trigger_above",trigger_below:"trigger_below",trigger_equals:"trigger_equals",trigger_not_equals:"trigger_not_equals",trigger_for_minutes:"trigger_for_minutes"}});function _t(){if(window.externalApp!==void 0)return!0;let s=typeof navigator<"u"&&navigator.userAgent||"";return/Home Assistant\//.test(s)&&/Android/.test(s)}var Kt=w(()=>{"use strict"});var Yt,Je,Gt=w(()=>{"use strict";Yt=["notes","cost","duration","photo","user"],Je={notes:"notes_label",cost:"cost",duration:"duration",photo:"photo_label",user:"user_label"}});function Qt(r){if(!r)return[];let s=[],e=r.photo_doc_id;typeof e=="string"&&s.push(e);let t=r.photo_doc_ids;Array.isArray(t)&&s.push(...t);let i=[];for(let a of s){if(typeof a!="string")continue;let l=a.trim();if(!(!l||i.includes(l))&&(i.push(l),i.length>=10))break}return i}var Jt=w(()=>{"use strict"});async function gt(r,s,e){let t=new FormData;t.append("entry_id",s),t.append("tags","photo"),t.append("file",e,e.name);let i=await fetch("/api/maintenance_supporter/document/upload",{method:"POST",headers:{Authorization:`Bearer ${r.auth?.data?.access_token??""}`},body:t});if(i.status===413)throw new Error("doc_too_large");if(!i.ok)throw new Error("doc_upload_failed");let a=await i.json();if(!a.id)throw new Error("doc_upload_failed");return a.id}async function Te(r,s){await Promise.all(s.map(e=>r.connection.sendMessagePromise({type:"maintenance_supporter/documents/delete",doc_id:e}).catch(()=>{})))}var Zt=w(()=>{"use strict";Jt()});function Jr(r){let e=U("2001-02-03",r).match(/\d+/g)||[],t=c=>e.findIndex(h=>Number(h)===c),i=t(2001),a=t(2),l=t(3);return i===0?"YMD":a>=0&&l>=0&&a<l?"MDY":"DMY"}function Ae(r,s,e){if(r<1e3||r>9999||s<1||s>12||e<1||e>31)return null;let t=new Date(r,s-1,e);return t.getFullYear()!==r||t.getMonth()!==s-1||t.getDate()!==e?null:`${r}-${String(s).padStart(2,"0")}-${String(e).padStart(2,"0")}`}function $s(r,s){let e=r.trim(),t;if(t=e.match(/^(\d{4})-(\d{1,2})-(\d{1,2})$/))return Ae(+t[1],+t[2],+t[3]);if(t=e.match(/^(\d{4})$/))return Ae(+t[1],1,1);if(t=e.match(/^(\d{1,2})[./](\d{4})$/))return Ae(+t[2],+t[1],1);if(t=e.match(/^(\d{1,2})\.(\d{1,2})\.(\d{4})$/))return Ae(+t[3],+t[2],+t[1]);if(t=e.match(/^(\d{1,2})[/-](\d{1,2})[/-](\d{4})$/)){let i=+t[1],a=+t[2],l=+t[3];return Jr(s)==="MDY"?Ae(l,i,a):Ae(l,a,i)}return null}var ks=w(()=>{"use strict";R()});function Zr(r,s){if(s)switch(r){case"date":return s.split("T")[0];case"time":return s.length===5?`${s}:00`:s;case"datetime":{let[e,t="00:00:00"]=s.split("T");return`${e} ${t.length===5?`${t}:00`:t}`}}}function Xr(r,s){if(typeof s!="string"||!s)return"";switch(r){case"date":return s.slice(0,10);case"time":return s.slice(0,5);case"datetime":{let[e,t="00:00:00"]=s.split(" ");return`${e}T${t.length===5?`${t}:00`:t}`}}}var M,Ce=w(()=>{"use strict";I();O();R();ks();M=class extends S{constructor(){super(...arguments);this.kind="date";this.label="";this.value="";this.clearable=!1;this.disabled=!1;this.required=!1;this.lang="en";this._typing=!1;this._typed="";this._typedInvalid=!1}_startTyping(){this._typed=this.value,this._typedInvalid=!1,this._typing=!0,this.updateComplete.then(()=>this.shadowRoot?.querySelector("input.typed")?.focus())}_commitTyped(){if(!this._typed.trim()){this._typing=!1;return}let e=$s(this._typed,this.lang);if(!e){this._typedInvalid=!0;return}this._typing=!1,this._typedInvalid=!1,this._emit(e)}_onTypedKey(e){e.key==="Enter"?(e.preventDefault(),this._commitTyped()):e.key==="Escape"&&(e.preventDefault(),this._typing=!1,this._typedInvalid=!1)}_selector(){switch(this.kind){case"date":return{date:{}};case"time":return{time:{no_second:!0}};case"datetime":return{datetime:{}}}}_onSelectorChange(e){e.stopPropagation(),this._emit(Xr(this.kind,e.detail?.value))}_clear(){this._emit("")}_emit(e){e!==this.value&&(this.value=e,this.dispatchEvent(new CustomEvent("value-changed",{bubbles:!0,composed:!0,detail:{value:e}})))}render(){return o`
      <div class="field">
        ...
        <div class="row">
          ${this._typing?o`<input class="typed" type="text" inputmode="numeric" autocomplete="off"
                .value=${this._typed}
                placeholder=${U("1978-03-15",this.lang)}
        ...
        ${this._typedInvalid?o`<span class="helper invalid">${n("date_type_invalid",this.lang)}</span>`:this.helper?o`<span class="helper">${this.helper}</span>`:p}
      </div>
    `}};M.styles=E`
    :host { display: block; }
    .field {
  ...
  `,d([b({attribute:!1})],M.prototype,"hass",2),d([b()],M.prototype,"kind",2),d([b()],M.prototype,"label",2),d([b()],M.prototype,"value",2),d([b()],M.prototype,"helper",2),d([b({type:Boolean})],M.prototype,"clearable",2),d([b({type:Boolean})],M.prototype,"disabled",2),d([b({type:Boolean})],M.prototype,"required",2),d([b()],M.prototype,"lang",2),d([u()],M.prototype,"_typing",2),d([u()],M.prototype,"_typed",2),d([u()],M.prototype,"_typedInvalid",2);customElements.get("ms-date-field")||customElements.define("ms-date-field",M)});function Es(){if(!_t())return!1;let r=typeof navigator<"u"?navigator.mediaDevices:void 0;return!!r&&typeof r.getUserMedia=="function"}var en,tn,fe,Xt=w(()=>{"use strict";I();O();R();Kt();en=1920,tn=.88,fe=class extends S{constructor(){super(...arguments);this.lang="en";this._open=!1;this._busy=!1;this._stream=null}async open(){if(this._open)return;let e=typeof navigator<"u"?navigator.mediaDevices:void 0;if(!e||typeof e.getUserMedia!="function"){this._unavailable("no_media_devices");return}try{this._stream=await e.getUserMedia({video:{facingMode:{ideal:"environment"}},audio:!1})}catch(i){this._unavailable(i instanceof Error?i.name||i.message:String(i));return}this._open=!0,await this.updateComplete;let t=this._video;if(t&&this._stream){t.srcObject=this._stream;try{await t.play()}catch{}}}close(){this._stopStream(),this._open=!1,this._busy=!1}disconnectedCallback(){super.disconnectedCallback(),this._stopStream()}get _video(){return this.shadowRoot?.querySelector("video")??null}_stopStream(){for(let t of this._stream?.getTracks()??[])t.stop();this._stream=null;let e=this._video;e&&(e.srcObject=null)}_unavailable(e){this._stopStream(),this._open=!1,this.dispatchEvent(new CustomEvent("capture-unavailable",{detail:{reason:e},bubbles:!0,composed:!0}))}async _shoot(){let e=this._video;if(!(!e||this._busy)){this._busy=!0;try{let t=e.videoWidth||640,i=e.videoHeight||480,a=Math.min(1,en/Math.max(t,i)),l=document.createElement("canvas");l.width=Math.round(t*a),l.height=Math.round(i*a);let c=l.getContext("2d");if(!c)throw new Error("no_canvas");c.drawImage(e,0,0,l.width,l.height);let h=await new Promise(x=>l.toBlob(x,"image/jpeg",tn));if(!h)throw new Error("no_blob");let _=new Date().toISOString().replace(/[-:]/g,"").slice(0,15),f=new File([h],`photo-${_}.jpg`,{type:"image/jpeg"});this.close(),this.dispatchEvent(new CustomEvent("photo-captured",{detail:{file:f},bubbles:!0,composed:!0}))}catch(t){this._unavailable(t instanceof Error?t.message:String(t))}finally{this._busy=!1}}}render(){if(!this._open)return p;let e=this.lang;return o`
  ...
  `,d([b({type:String})],fe.prototype,"lang",2),d([u()],fe.prototype,"_open",2),d([u()],fe.prototype,"_busy",2);customElements.get("ms-camera-capture")||customElements.define("ms-camera-capture",fe)});var k,ei=w(()=>{"use strict";I();O();Ye();R();le();ht();Kt();Gt();Zt();Ce();Xt();Xt();k=class extends S{constructor(){super(...arguments);this.entryId="";this.taskId="";this.taskName="";this.lang="en";this.checklist=[];this.adaptiveEnabled=!1;this.taskType="";this.readingUnit="";this.readings=[];this.readingHistory=[];this.restockDefault=null;this.restockUnitCost=null;this.currencySymbol="";this.parts=[];this.consumesParts=[];this.consumesInfo=[];this.requiredFields=[];this.phaseLabel="";this.requireTagScan=!1;this.viaTagScan=!1;this._open=!1;this._notes="";this._cost="";this._duration="";this._loading=!1;this._error="";this._checklistState={};this._feedback="needed";this._photos=[];this._uploadedIds=[];this._photoUploading=!1;this._readingValue="";this._readingValues={};this._restockQty="";this._completedAt="";this._usedParts={};this.checklistPrefill={};this._singlePick=_t();this._inAppCamera=Es()}open(e={}){this._open||(this._open=!0,this.viaTagScan=!!e.viaTagScan,this._notes="",this._cost="",this._duration="",this._error="",this._checklistState=Object.fromEntries(this.checklist.map((t,i)=>[String(i),!!this.checklistPrefill[t]]).filter(([,t])=>t)),this._feedback="needed",this._photos.forEach(t=>URL.revokeObjectURL(t.preview)),this._photos=[],this._uploadedIds=[],this._photoUploading=!1,this._readingValue="",this._readingValues={},this._restockQty=this.restockDefault!==null?String(this.restockDefault):"",this._completedAt="",this._usedParts=Object.fromEntries(this.consumesParts.map(t=>[G(t),{...t}])))}_toggleCheck(e){let t=String(e);this._checklistState={...this._checklistState,[t]:!this._checklistState[t]}}_setFeedback(e){this._feedback=e}_onCameraClick(e){!this._inAppCamera||this._photoUploading||(e.preventDefault(),this.shadowRoot?.querySelector("ms-camera-capture")?.open())}_onCameraUnavailable(){this._inAppCamera=!1,this.shadowRoot?.querySelector(".photo-pick-camera input")?.click()}async _onPhotoInput(e){let t=e.target,i=Array.from(t.files??[]);t.value="",await this._addPhotoFiles(i)}async _addPhotoFiles(e){if(e.length===0)return;let t=10-this._photos.length,i=e.slice(0,Math.max(t,0));this._photoUploading=!0,this._error="";try{for(let a of i){let l=await gt(this.hass,this.entryId,a);this._uploadedIds=[...this._uploadedIds,l],this._photos=[...this._photos,{id:l,preview:URL.createObjectURL(a)}]}e.length>i.length&&(this._error=n("photos_limit",this.lang).replace("{max}",String(10)))}catch(a){let l=a instanceof Error&&a.message==="doc_too_large"?"doc_too_large":"doc_upload_failed";this._error=n(l,this.lang)}finally{this._photoUploading=!1}}_removePhoto(e){let t=this._photos.find(i=>i.id===e);t&&URL.revokeObjectURL(t.preview),this._photos=this._photos.filter(i=>i.id!==e),this._uploadedIds.includes(e)&&(this._uploadedIds=this._uploadedIds.filter(i=>i!==e),Te(this.hass,[e]))}async _complete(){this._loading=!0,this._error="";try{let e={type:"maintenance_supporter/task/complete",entry_id:this.entryId,task_id:this.taskId};if(this._notes&&(e.notes=this._notes),this._cost){let t=parseFloat(this._cost);!isNaN(t)&&t>=0&&(e.cost=t)}if(this._duration){let t=parseInt(this._duration,10);!isNaN(t)&&t>=0&&(e.duration=t)}if(this.checklist.length>0&&(e.checklist_state=this._checklistState),this.adaptiveEnabled&&(e.feedback=this._feedback),this._photos.length>0&&(e.photo_doc_ids=this._photos.map(t=>t.id)),this.viaTagScan&&(e.via_tag_scan=!0),this._completedAt){if(new Date(this._completedAt).getTime()>Date.now()){this._error=n("completed_at_future_error",this.lang),this._loading=!1;return}e.completed_at=this._completedAt.length===16?`${this._completedAt}:00`:this._completedAt}if(this.readings.length>0){let t={};for(let i of this.readings){let a=(this._readingValues[i.id]??"").trim();if(a==="")continue;let l=parseFloat(a.replace(",","."));isNaN(l)||(t[i.id]=l)}Object.keys(t).length>0&&(e.reading_values=t)}else if(this._readingValue!==""){let t=parseFloat(this._readingValue);isNaN(t)||(e.reading_value=t)}if(this.restockDefault!==null&&this._restockQty!==""){let t=parseFloat(this._restockQty);!isNaN(t)&&t>=1&&(e.restock_quantity=t)}this.parts.length>0&&(e.used_parts=Object.values(this._usedParts).filter(t=>Number.isFinite(t.quantity)&&t.quantity>0).map(t=>t.entry_id?{part_id:t.part_id,quantity:t.quantity,entry_id:t.entry_id}:{part_id:t.part_id,quantity:t.quantity})),await this.hass.connection.sendMessagePromise(e),this._uploadedIds=[],this._open=!1,this.dispatchEvent(new CustomEvent("task-completed"))}catch(e){this._error=C(e,this.lang,n("save_error",this.lang))}finally{this._loading=!1}}_renderReadingField(e,t){let i=this._completedAt?new Date(this._completedAt).getTime():NaN,a=ms(this.readingHistory,e.id,isNaN(i)?void 0:i),l=e.unit||this.readingUnit,c=(this._readingValues[e.id]??"").trim(),h=c===""?NaN:parseFloat(c.replace(",",".")),_=a!==void 0&&!isNaN(h)&&h<a.value,f=a!==void 0?q(a.value,t,{maximumFractionDigits:3}):"";return o`
      <label class="field reading-field">
        <span class="field-label">${e.name}${l?` (${l})`:""}</span>
        <input type="text" inputmode="decimal" class="field-input"
          placeholder=${a!==void 0?n("reading_last",t).replace("{value}",f):""}
          .value=${this._readingValues[e.id]??""}
          @input=${x=>{this._readingValues={...this._readingValues,[e.id]:x.target.value}}} />
        ${_?o`<span class="reading-warn">${n("reading_below_last",t).replace("{value}",f)}</span>`:p}
      </label>`}get _missingRequired(){let e={notes:this._notes.trim()!=="",cost:this._cost.trim()!=="",duration:this._duration.trim()!=="",photo:this._photos.length>0,user:!!this.hass?.user};return this.requiredFields.filter(t=>!e[t])}_req(e){return this.requiredFields.includes(e)?o`<span class="req-mark" aria-hidden="true">*</span>`:p}_partsCostSuggestion(){if(this.restockDefault!==null){let i=parseFloat(this._restockQty);return this.restockUnitCost==null||!Number.isFinite(i)||i<=0?null:Math.round(this.restockUnitCost*i*100)/100}if(!this.parts.length)return null;let e=0,t=!1;for(let i of Object.values(this._usedParts)){let a=this.parts.find(l=>G({part_id:l.id,entry_id:l.entry_id})===G(i));a?.cost!=null&&(e+=a.cost*(i.quantity||1),t=!0)}return t?Math.round(e*100)/100:null}_renderCostSuggestion(e){if(this._cost.trim()!=="")return p;let t=this._partsCostSuggestion();if(t==null||t<=0)return p;let i=is(t,this.currencySymbol,e);return o`<button
      type="button"
      class="cost-suggestion"
      @click=${()=>this._cost=String(Math.round(t*100)/100)}
    >${n("cost_from_parts",e).replace("{amount}",i)}</button>`}_close(){if(this._open=!1,this._uploadedIds.length>0){let e=this._uploadedIds;this._uploadedIds=[],Te(this.hass,e)}}_pickCompletedAt(){let e=new Date,t=i=>String(i).padStart(2,"0");this._completedAt=`${e.getFullYear()}-${t(e.getMonth()+1)}-${t(e.getDate())}T${t(e.getHours())}:${t(e.getMinutes())}:00`}render(){if(!this._open)return o``;let e=this.lang||this.hass?.language||"en";return o`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${n("complete_title",e)}${this.taskName}</div>
        ${this.phaseLabel?o`<div class="phase-line">${n("phase_current",e)}: ${this.phaseLabel}</div>`:p}
        ${this.requireTagScan&&!this.viaTagScan?o`<div class="scan-required-note">${n("require_tag_scan_hint",e)}</div>`:p}
        <div class="content">
          ...
          ${this.parts.length?o`<div class="used-parts">
                ...
                ${this.parts.map(t=>{let i=G({part_id:t.id,entry_id:t.entry_id}),a=this._usedParts[i],l=a!==void 0,c=t.entry_id?{part_id:t.id,quantity:1,entry_id:t.entry_id}:{part_id:t.id,quantity:1};return o`<div class="used-part-row">
                    <label class="used-part-check">
                      <input type="checkbox" .checked=${l}
                        @change=${h=>{let _={...this._usedParts};h.target.checked?_[i]=_[i]||c:delete _[i],this._usedParts=_}} />
                      <span
                    ...
                    ${l?o`<input class="used-part-qty" type="number" min="0.01" max="999" step="0.01"
                          ...
                          @input=${h=>{let _=parseFloat(h.target.value);this._usedParts={...this._usedParts,[i]:{...c,quantity:Number.isFinite(_)&&_>=.01?_:1}}}} />`:p}
                  </div>`})}
          ...
          <div class="field">
            ...
            ${this._photos.length<10?o`<div class="photo-pickers">
                  <label class="photo-pick photo-pick-camera" @click=${this._onCameraClick}>
                    ...
                    <span>${this._photoUploading?n("uploading",e):n("doc_camera",e)}</span>
                  ...
                  <label class="photo-pick photo-pick-gallery">
                    ...
                    <span>${n(this._singlePick?"choose_photo":"choose_photos",e)}</span>
                    <input type="file" accept="image/*" ?multiple=${!this._singlePick}
                      ?disabled=${this._photoUploading}
                      @change=${this._onPhotoInput} />
                  </label>
                </div>
                ${this._singlePick?o`<div class="photo-limit photo-android-hint">${n("photos_android_hint",e)}</div>`:p}
                ${this._inAppCamera?o`<ms-camera-capture .lang=${e}
                  @photo-captured=${t=>this._addPhotoFiles([t.detail.file])}
                  @capture-unavailable=${this._onCameraUnavailable}></ms-camera-capture>`:p}`:o`<div class="photo-limit">${n("photos_limit",e).replace("{max}",String(10))}</div>`}
          </div>
        ...
        <div class="dialog-actions">
          ...
          <ha-button
            ...
            title=${this._missingRequired.length?this._missingRequired.map(t=>n("err_required",e).replace("{field}",n(Je[t]??t,e))).join(" \xB7 "):""}
          >
  ...
  `],d([b({attribute:!1})],k.prototype,"hass",2),d([b()],k.prototype,"entryId",2),d([b()],k.prototype,"taskId",2),d([b()],k.prototype,"taskName",2),d([b()],k.prototype,"lang",2),d([b({type:Array})],k.prototype,"checklist",2),d([b({type:Boolean})],k.prototype,"adaptiveEnabled",2),d([b()],k.prototype,"taskType",2),d([b()],k.prototype,"readingUnit",2),d([b({attribute:!1})],k.prototype,"readings",2),d([b({attribute:!1})],k.prototype,"readingHistory",2),d([b({attribute:!1})],k.prototype,"restockDefault",2),d([b({attribute:!1})],k.prototype,"restockUnitCost",2),d([b()],k.prototype,"currencySymbol",2),d([b({attribute:!1})],k.prototype,"parts",2),d([b({attribute:!1})],k.prototype,"consumesParts",2),d([b({type:Array})],k.prototype,"consumesInfo",2),d([b({type:Array})],k.prototype,"requiredFields",2),d([b()],k.prototype,"phaseLabel",2),d([b({type:Boolean})],k.prototype,"requireTagScan",2),d([b({type:Boolean})],k.prototype,"viaTagScan",2),d([u()],k.prototype,"_open",2),d([u()],k.prototype,"_notes",2),d([u()],k.prototype,"_cost",2),d([u()],k.prototype,"_duration",2),d([u()],k.prototype,"_loading",2),d([u()],k.prototype,"_error",2),d([u()],k.prototype,"_checklistState",2),d([u()],k.prototype,"_feedback",2),d([u()],k.prototype,"_photos",2),d([u()],k.prototype,"_photoUploading",2),d([u()],k.prototype,"_readingValue",2),d([u()],k.prototype,"_readingValues",2),d([u()],k.prototype,"_restockQty",2),d([u()],k.prototype,"_completedAt",2),d([u()],k.prototype,"_usedParts",2),d([b({attribute:!1})],k.prototype,"checklistPrefill",2),d([u()],k.prototype,"_inAppCamera",2);customElements.get("maintenance-complete-dialog")||customElements.define("maintenance-complete-dialog",k)});function T(r){return r.toFixed(1)}var mt=w(()=>{"use strict";R()});var D,ri=w(()=>{"use strict";I();O();D=class extends S{constructor(){super(...arguments);this.label="";this.value="";this.placeholder="";this.type="text";this.required=!1;this.disabled=!1;this.multiline=!1;this.rows=3}_onInput(e){let t=e.target.value;this.value=t,this.dispatchEvent(new CustomEvent("input",{bubbles:!0,composed:!0,detail:{value:t}}))}render(){return o`
      <label class="field">
        ${this.label?o`<span class="label">${this.label}${this.required?o`<span class="req">*</span>`:p}</span>`:p}
        ${this.multiline?o`
        <textarea
...
`),partId:A?.part_id||"",partQty:A?.quantity!=null?String(A.quantity):"",reqOverride:f.required_completion_fields!==void 0,reqFields:[...f.required_completion_fields||[]],extraParts:z.filter((j,be)=>be!==P).map(j=>({...j})),carry:$}}),this._phaseSeq=[...t.phase_sequence||[]],this._requiredCompletion=[...t.required_completion_fields||[]],this._scheduleTime=t.schedule_time||"",this._scheduleTimeOn=!!t.schedule_time;let l=t.on_complete_action;if(l&&l.service){this._actionService=l.service;let _=l.target?.entity_id;this._actionTargetEntity=Array.isArray(_)?_[0]||"":_||"",this._actionData=l.data&&typeof l.data=="object"?{...l.data}:{},this._actionDataJsonFallback=""}else this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="";let c=t.quick_complete_defaults;this._qcNotes=c?.notes||"",this._qcCost=c?.cost!=null?String(c.cost):"",this._qcDuration=c?.duration!=null?String(c.duration):"",this._qcFeedback=c?.feedback||"";let h=t.adaptive_config||{};if(this._environmentalEntity=h.environmental_entity||"",this._environmentalAttribute=h.environmental_attribute||"",this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute,this._adaptiveEnabled=!!h.enabled,this._adaptiveAlpha=h.ewa_alpha?.toString()??ie.alpha,this._adaptiveMin=h.min_interval_days?.toString()??ie.min,this._adaptiveMax=h.max_interval_days?.toString()??ie.max,this._adaptiveSeasonal=h.seasonal_enabled!==!1,this._adaptivePrediction=h.sensor_prediction_enabled!==!1,this._adaptiveInitial=this._adaptiveSnapshot(),t.trigger_config){let _=t.trigger_config;this._triggerEntityId=_.entity_id||_.entity_ids&&_.entity_ids[0]||"",this._triggerEntityIds=_.entity_ids||(_.entity_id?[_.entity_id]:[]),this._triggerEntityLogic=_.entity_logic||"any",this._triggerAttribute=_.attribute||"",this._triggerType=_.type||"threshold",this._triggerAbove=_.trigger_above?.toString()||"",this._triggerBelow=_.trigger_below?.toString()||"",this._triggerEquals=_.trigger_equals?.toString()||"",this._triggerNotEquals=_.trigger_not_equals?.toString()||"",this._triggerForMinutes=_.trigger_for_minutes?.toString()||"0",this._triggerCombinator=_.trigger_combinator==="all"?"all":"any",this._triggerTargetValue=_.trigger_target_value?.toString()||"",this._triggerDeltaMode=_.trigger_delta_mode||!1,this._triggerBaselineValue=_.trigger_baseline_value?.toString()||"",this._liveBaselineValue=t.trigger_baseline_value??null,this._autoCompleteOnRecovery=_.auto_complete_on_recovery||!1,this._triggerFromState=_.trigger_from_state||"",this._triggerToState=_.trigger_to_state||"",this._triggerTargetChanges=_.trigger_target_changes?.toString()||"",this._triggerRuntimeHours=_.trigger_runtime_hours?.toString()||"",this._triggerRuntimeMaxSession=_.trigger_runtime_max_session_seconds?.toString()||"",this._triggerOnStates=(_.trigger_on_states||[]).join(", "),_.type==="compound"?(this._compoundLogic=_.compound_logic==="OR"?"OR":"AND",this._compoundConditions=(_.conditions||[]).map(cn)):(this._compoundLogic="AND",this._compoundConditions=[])}else this._resetTriggerFields();this._triggerEntityId&&this._fetchEntityAttributes(this._triggerEntityId),await Promise.all([this._loadUsers(),this._loadTags(),this._loadParts(),this._loadForeignPools()]),this._open=!0}_resetFields(){this._name="",this._type="custom",this._scheduleType="time_based",this._intervalDays="30",this._intervalUnit="days",this._dueDate="",this._warningDays=String(this.defaultWarningDays),this._earliestCompletionDays="",this._intervalAnchor="completion",this._weekdays=[],this._nth="1",this._nthWeekday="5",this._domDay="1",this._domLastDay=!1,this._domBusiness=!1,this._calOffset="0",this._seasonMonths=[],this._endsMode="never",this._endsCount="",this._endsUntil="",this._notes="",this._documentationUrl="",this._customIcon="",this._priority="normal",this._labels="",this._enabled=!0,this._lastPerformed="",this._nfcTagId="",this._requireTagScan=!1,this._allowSkip=!0,this._readingUnit="",this._readings=[],this._consumesParts={},this._responsibleUserId=null,this._assigneePool=[],this._rotationStrategy="",this._checklistText="",this._phaseDefs=[],this._phaseSeq=[],this._requiredCompletion=[],this._scheduleTime="",this._scheduleTimeOn=!1,this._environmentalEntity="",this._environmentalAttribute="",this._environmentalInitial="",this._environmentalAttributeInitial="",this._adaptiveEnabled=!1,this._adaptiveAlpha=ie.alpha,this._adaptiveMin=ie.min,this._adaptiveMax=ie.max,this._adaptiveSeasonal=!0,this._adaptivePrediction=!0,this._adaptiveInitial=this._adaptiveSnapshot(),this._actionService="",this._actionTargetEntity="",this._actionData={},this._actionDataJsonFallback="",this._actionTesting=!1,this._actionTestResult="",this._qcNotes="",this._qcCost="",this._qcDuration="",this._qcFeedback="",this._resetTriggerFields()}_resetTriggerFields(){this._triggerEntityId="",this._triggerEntityIds=[],this._triggerEntityLogic="any",this._triggerAttribute="",this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="",this._triggerType="threshold",this._triggerAbove="",this._triggerBelow="",this._triggerEquals="",this._triggerNotEquals="",this._triggerForMinutes="0",this._triggerCombinator="any",this._triggerTargetValue="",this._triggerDeltaMode=!1,this._triggerBaselineValue="",this._liveBaselineValue=null,this._autoCompleteOnRecovery=!1,this._triggerFromState="",this._triggerToState="",this._triggerTargetChanges="",this._triggerRuntimeHours="",this._triggerRuntimeMaxSession="",this._triggerOnStates="",this._compoundLogic="AND",this._compoundConditions=[]}async _loadUsers(){this._userService||(this._userService=new Se(this.hass));try{this._availableUsers=await this._userService.getUsers()}catch(e){console.error("Failed to load users:",e),this._availableUsers=[]}}_toggleAssignee(e){this._assigneePool=this._assigneePool.includes(e)?this._assigneePool.filter(t=>t!==e):[...this._assigneePool,e]}async _testAction(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e)){this._actionTestResult="error",this._actionTestError="Invalid service format (expected 'domain.service')",setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3);return}let[t,i]=e.split(".");if(!this.hass?.services?.[t]?.[i]){this._actionTestResult="error",this._actionTestError=`Service "${e}" is not registered in Home Assistant. Check spelling and that the integration providing it is loaded.`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}let a=this._actionTargetEntity.trim();if(a){let l=a.split(".")[0];if(l!==t&&!new Set(["homeassistant","scene","notify","persistent_notification"]).has(t)){this._actionTestResult="error",this._actionTestError=`Service "${e}" only works on ${t}.* entities; entity "${a}" is in ${l}.* \u2014 pick a service that matches the entity domain (e.g. ${l}.${i})`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}if(!this.hass.states?.[a]){this._actionTestResult="error",this._actionTestError=`Target entity "${a}" not found in Home Assistant \u2014 the entity may have been renamed or its integration removed.`,setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},8e3);return}}this._actionTestResult="ok",setTimeout(()=>{this._actionTestResult="",this._actionTestError=""},5e3)}_buildActionData(){if(this._actionDataJsonFallback.trim())try{let e=JSON.parse(this._actionDataJsonFallback);if(e&&typeof e=="object"&&!Array.isArray(e))return e}catch{}return{...this._actionData}}_serviceSchema(){let e=this._actionService.trim();if(!e||!/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(e))return null;let[t,i]=e.split("."),a=this.hass?.services?.[t]?.[i]?.fields;return!a||Object.keys(a).length===0?null:Object.entries(a).map(([l,c])=>({name:l,required:!!c.required,selector:c.selector||{text:{}}}))}_renderCompletionActionsSection(e){if(!this.completionActionsEnabled)return p;let t=this._serviceSchema();return o`
      <details class="ca-section">
        ...
        <div class="ca-test-row">
          ...
          ${this._actionTestResult==="ok"?o`<span class="ca-test-ok">${n("on_complete_action_test_success",e)}</span>`:p}
    ...
    `}async _loadParts(){if(this.parts=[],!!this._entryId)try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this.parts=e.parts||[],this._partsLoadFailed=!1}catch{this.parts=[],this._partsLoadFailed=!0}}async _loadForeignPools(){if(this._foreignOwners=[],!!this._entryId)try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"});this._foreignOwners=(e.objects||[]).filter(t=>t.entry_id!==this._entryId&&(t.parts||[]).length>0).map(t=>({entry_id:t.entry_id,name:t.object?.name||t.entry_id,parts:t.parts||[]})).sort((t,i)=>t.name.localeCompare(i.name))}catch{this._foreignOwners=[]}}async _loadTags(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/tags/list"});this._availableTags=e.tags||[]}catch{this._availableTags=[]}}_fetchConditionAttributes(e){!e||!this.hass||this._conditionAttrOptions[e]||this._conditionAttrPending.has(e)||(this._conditionAttrPending.add(e),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/entity/attributes",entity_id:e}).then(t=>{let i=t;this._conditionAttrOptions={...this._conditionAttrOptions,[e]:{suggested:i.suggested_attributes||[],available:i.available_attributes||[]}}}).catch(()=>{this._conditionAttrOptions={...this._conditionAttrOptions,[e]:{suggested:[],available:[]}}}))}async _fetchEntityAttributes(e){if(!e||!this.hass){this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain="";return}try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/entity/attributes",entity_id:e});this._entityDomain=t.domain||"",this._suggestedAttributes=t.suggested_attributes||[],this._availableAttributes=t.available_attributes||[]}catch{this._suggestedAttributes=[],this._availableAttributes=[],this._entityDomain=""}}get _hasForeignPick(){return Object.values(this._consumesParts).some(e=>!!e.entry_id)}_renderConsumesRow(e,t){let i=G({part_id:e.id,entry_id:t}),a=this._consumesParts[i],l=t?{part_id:e.id,quantity:1,entry_id:t}:{part_id:e.id,quantity:1};return o`
      <div class="consumes-row">
        ...
        ${a!==void 0?o`<input
              ...
              step="0.01"
              .value=${String(a.quantity)}
              @input=${c=>{let h=parseFloat(c.target.value);this._consumesParts={...this._consumesParts,[i]:{...l,quantity:Number.isFinite(h)&&h>=.01?h:1}}}}
            />`:p}
      </div>
    `}_toggleRequired(e,t){let i=new Set(this._requiredCompletion);t?i.add(e):i.delete(e),this._requiredCompletion=[...i]}_patchReading(e,t){this._readings=this._readings.map(i=>i.id===e?{...i,...t}:i)}_renderReadingsEditor(e){let t=vs(this._readings);return o`
      <div class="readings-editor">
        ...
        ${this._readings.map(i=>o`
          ${t.has(i.id)?o`<div class="field-help reading-dup">${n("reading_duplicate_name",e)}</div>`:p}
        ...
        ${this._readings.length<20?o`
          <ha-button appearance="plain" class="reading-add"
            @click=${()=>this._readings=[...this._readings,{id:us(),name:"",unit:this._readings.length?this._readings[this._readings.length-1].unit:this._readingUnit}]}>
              ...
              ${Yt.map(a=>o`
                <label class="req-option">
                  <input
                    ...
                    @change=${l=>{let c=l.target.checked,h=new Set(i.reqFields);c?h.add(a):h.delete(a),this._patchPhaseDef(i.id,{reqFields:[...h]})}}
                  />
                  <span>${n(Je[a],e)}</span>
                </label>
              `)}
        ...
        <div class="phase-seq">
          ${this._phaseSeq.map((i,a)=>o`
            <span class="phase-chip">
              ${a+1}. ${t(i)}
              <button class="phase-chip-x" @click=${()=>{this._phaseSeq=this._phaseSeq.filter((l,c)=>c!==a)}}>✕</button>
            </span>
          `)}
          <select
            class="phase-seq-add"
...
`).map(h=>h.trim()).filter(Boolean).slice(0,100);e.checklist=c.length?c:null}if(this.completionActionsEnabled){let c=this._actionService.trim();if(c&&/^[a-z][a-z0-9_]*\.[a-z0-9_]+$/.test(c)){let x={service:c},v=this._actionTargetEntity.trim();v&&(x.target={entity_id:v});let y=this._buildActionData();Object.keys(y).length>0&&(x.data=y),e.on_complete_action=x}else e.on_complete_action=null;let h={};this._qcNotes.trim()&&(h.notes=this._qcNotes.trim());let _=parseFloat(this._qcCost);!isNaN(_)&&_>=0&&(h.cost=_);let f=parseInt(this._qcDuration,10);!isNaN(f)&&f>=0&&(h.duration=f),this._qcFeedback&&(h.feedback=this._qcFeedback),e.quick_complete_defaults=Object.keys(h).length?h:null}let i=await this.hass.connection.sendMessagePromise(e),a=this._taskId||i?.task_id,l=this._environmentalEntity!==this._environmentalInitial||this._environmentalAttribute!==this._environmentalAttributeInitial;if(a&&this._scheduleType==="sensor_based"&&l)try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_environmental_entity",entry_id:this._entryId,task_id:a,environmental_entity:this._environmentalEntity||null,environmental_attribute:this._environmentalAttribute||null}),this._environmentalInitial=this._environmentalEntity,this._environmentalAttributeInitial=this._environmentalAttribute}catch{}if(a&&this._adaptiveSnapshot()!==this._adaptiveInitial){let c=parseFloat(this._adaptiveAlpha),h=parseInt(this._adaptiveMin,10),_=parseInt(this._adaptiveMax,10);try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/set_adaptive",entry_id:this._entryId,task_id:a,enabled:this._adaptiveEnabled,...c>=.1&&c<=.9?{ewa_alpha:c}:{},...!isNaN(h)&&h>=1?{min_interval_days:h}:{},...!isNaN(_)&&_>=1?{max_interval_days:_}:{},seasonal_enabled:this._adaptiveSeasonal,sensor_prediction_enabled:this._adaptivePrediction}),this._adaptiveInitial=this._adaptiveSnapshot()}catch{}}this._open=!1,this.dispatchEvent(new CustomEvent("task-saved"))}catch(e){this._error=C(e,this._lang,n("save_error",this._lang))}finally{this._loading=!1}}}_close(){this._open=!1,this._pickerProbeTimer!==void 0&&(clearTimeout(this._pickerProbeTimer),this._pickerProbeTimer=void 0),this._pickerProbeStrikes=0}_renderTriggerFields(){if(this._scheduleType!=="sensor_based")return p;let e=this._lang,t=this._triggerType==="compound";return o`
      ...
      <div class="select-row">
        ...
        >
          ${on.map(i=>o`<option value=${i} ?selected=${i===this._triggerType}>${n(i,e)}</option>`)}
        </select>
      </div>
      ...
      ${this._intervalDays?o`
          ...
          `:p}
    `}_patchCondition(e,t){this._compoundConditions=this._compoundConditions.map((i,a)=>a===e?{...i,...t}:i)}_addCondition(){this._compoundConditions=[...this._compoundConditions,ln()]}_removeCondition(e){this._compoundConditions=this._compoundConditions.filter((t,i)=>i!==e)}_renderCompoundEditor(){let e=this._lang;return o`
      <div class="select-row">
    ...
    `}_buildSchedule(){let e=i=>{let a=parseInt(this._calOffset,10)||0;return a&&(i.offset=Math.max(-15,Math.min(a,15))),i};if(this._scheduleType==="weekdays")return e({kind:"weekdays",weekdays:[...this._weekdays].sort((i,a)=>i-a)});if(this._scheduleType==="nth_weekday")return e({kind:"nth_weekday",nth:parseInt(this._nth,10),weekday:parseInt(this._nthWeekday,10)});let t={kind:"day_of_month",day:this._domLastDay?-1:parseInt(this._domDay,10)||1};return this._domBusiness&&(t.business=!0),e(t)}_recurrenceExtras(){let e={};if(this._seasonMonths.length&&(e.season_months=[...this._seasonMonths].sort((t,i)=>t-i)),this._endsMode==="count"){let t=parseInt(this._endsCount,10);t>=1&&(e.ends={count:t})}else this._endsMode==="until"&&this._endsUntil&&(e.ends={until:this._endsUntil});return e}_toggleSeasonMonth(e){this._seasonMonths=this._seasonMonths.includes(e)?this._seasonMonths.filter(t=>t!==e):[...this._seasonMonths,e]}_renderRecurrenceExtras(){let e=this._lang;if(!(this._scheduleType==="time_based"||ft.includes(this._scheduleType)))return p;let i=un(e);return o`
      ...
      ${this._endsMode==="until"?o`
        ...
        ></ms-date-field>`:p}
    `}_renderCalendarFields(){let e=this._lang,t=hn(e);if(this._scheduleType==="weekdays")return o`
      ...
      ></ms-textfield>`}_thresholdLimitsOverlap(){let e=parseFloat(this._triggerAbove),t=parseFloat(this._triggerBelow);return!isNaN(e)&&!isNaN(t)&&t>e}_renderTriggerLiveHint(){if(this._triggerType==="compound")return p;let e=this._triggerType==="threshold"&&this._thresholdLimitsOverlap()?o`<div class="trigger-live-hint warn">${n("trigger_hint_overlap",this._lang)}</div>`:p,t=this._triggerEntityId||this._triggerEntityIds[0];if(!t||!this.hass?.states)return e;let i=this.hass.states[t];if(!i)return e;let a=this._lang,l=i.attributes?.unit_of_measurement,c=typeof l=="string"&&l?` ${l}`:"",h=this._triggerAttribute?i.attributes?.[this._triggerAttribute]:i.state,_=typeof h=="number"?h:parseFloat(String(h)),f=h!=="unknown"&&h!=="unavailable"&&h!=null&&!isNaN(_),x=y=>q(y,a,{maximumFractionDigits:1}),v=[];if(this._triggerType==="threshold"){let y=parseFloat(this._triggerAbove),m=parseFloat(this._triggerBelow);if(isNaN(y)&&isNaN(m))return p;f&&v.push(n("trigger_hint_now",a).replace("{value}",x(_)+c)),isNaN(y)||v.push(n("trigger_hint_above",a).replace("{target}",x(y)+c)),isNaN(m)||v.push(n("trigger_hint_below",a).replace("{target}",x(m)+c))}else if(this._triggerType==="counter"){let y=parseFloat(this._triggerTargetValue);if(isNaN(y))return p;this._triggerDeltaMode?this._taskId?v.push(n("trigger_hint_counter_delta_edit",a).replace("{target}",x(y)+c)):f?v.push(n("trigger_hint_counter_delta",a).replace("{value}",x(_)+c).replace("{due}",x(_+y)+c).replace("{target}",x(y)+c)):v.push(n("trigger_hint_counter_delta_edit",a).replace("{target}",x(y)+c)):(f&&v.push(n("trigger_hint_now",a).replace("{value}",x(_)+c)),v.push(n("trigger_hint_counter_abs",a).replace("{target}",x(y)+c)))}else if(this._triggerType==="runtime"){let y=parseFloat(this._triggerRuntimeHours);if(isNaN(y))return p;v.push(n("trigger_hint_runtime",a).replace("{hours}",x(y))),v.push(n("trigger_hint_state_now",a).replace("{value}",String(i.state)))}else if(this._triggerType==="state_change"){let y=parseInt(this._triggerTargetChanges,10)||1,m=this._triggerToState.trim();v.push((m?n("trigger_hint_state_change_to",a).replace("{state}",m):n("trigger_hint_state_change",a)).replace("{count}",String(y))),v.push(n("trigger_hint_state_now",a).replace("{value}",String(i.state)))}return v.length?o`<div class="trigger-live-hint">${v.join(" ")}</div>${e}`:e}_renderTriggerTypeFields(){let e=this._lang;return this._triggerType==="threshold"?o`
        <ms-textfield
      ...
      `:this._triggerType==="state_change"?o`
        ...
        ${(this._triggerTargetChanges||"1")==="1"&&(this._triggerFromState||this._triggerToState)?o`<div class="field-help">${n("state_latch_help",e)}</div>`:p}
        <ms-textfield
          label="${n("for_at_least_minutes",e)}"
          type="number"
          min="0"
      ...
      `:p}render(){if(!this._open)return o``;let e=this._lang,t=this._taskId?n("edit_task",e):n("new_task",e);return o`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
          ${this._error?o`<div class="error">${this._error}</div>`:p}
          ...
          <div class="select-row">
            ...
            >
              ${rn.map(i=>o`<option value=${i} ?selected=${i===this._type}>${n(i,e)}</option>`)}
            </select>
          </div>
          ${this._type==="reading"?o`
              ...
              `:p}
          ${this._partsLoadFailed?o`<div class="field-help parts-load-failed">${n("parts_load_failed",e)}</div>`:p}
          ...
          <div class="select-row">
            ...
            >
              ${nn.map(i=>o`<option value=${i} ?selected=${i===this._priority}>${n("priority_"+i,e)}</option>`)}
            </select>
          </div>
          <div class="field">
          ...
          <div class="select-row">
            ...
            >
              ${an.map(i=>o`<option value=${i} ?selected=${i===this._scheduleType}>${n(i,e)}</option>`)}
            </select>
          </div>
          ...
          ${this._scheduleType==="one_time"?o`
                ...
                ></ms-date-field>
              `:p}
          ${this.scheduleTimeEnabled&&Is.includes(this._scheduleType)?o`
          ...
          ${this._scheduleType==="sensor_based"?o`
            ...
            <ha-form
              class="entity-picker-form"
              .hass=${this.hass}
              .schema=${[{name:"environmental_entity",selector:{entity:{domain:Ts,device_class:As}}}]}
          ...
          ${this._requireTagScan?o`<div class="field-help">${n("require_tag_scan_help",e)}</div>`:p}
          <label class="req-option">
            <input
              type="checkbox"
          ...
          ${this._allowSkip?p:o`<div class="field-help">${n("disallow_skip_help",e)}</div>`}
          <label class="req-option">
            <input
              type="checkbox"
          ...
          ${this._notifyEnabled?p:o`<div class="field-help">${n("no_notifications_help",e)}</div>`}
  ...
  `,d([b({attribute:!1})],de.prototype,"hass",2),d([b()],de.prototype,"docId",2),d([u()],de.prototype,"_url",2),d([u()],de.prototype,"_failed",2);customElements.get("maintenance-history-photo")||customElements.define("maintenance-history-photo",de)});var F,Ls=w(()=>{"use strict";I();O();R();le();Zt();Ce();oi();F=class extends S{constructor(){super(...arguments);this._open=!1;this._saving=!1;this._error="";this._draft=null;this._originalSnapshot=null;this._partOptions=null;this._partQty={};this._partQtyOriginal="";this._photos=[];this._photosOriginal="";this._readingRows=[];this._readingText={};this._readingsOriginal="";this._uploadedIds=[];this._photoUploading=!1}get _lang(){return L(this.hass)}openEdit(e){this._draft={...e},this._originalSnapshot={...e},this._error="",this._open=!0,this._partOptions=null,this._partQty={},this._partQtyOriginal="",this._photos=[...e.photo_doc_ids??[]],this._photosOriginal=JSON.stringify(this._photos),this._uploadedIds=[],this._photoUploading=!1,this._seedReadings(e),this._loadPartOptions()}_seedReadings(e){let t=[],i=new Set;for(let l of e.readings??[])i.has(l.id)||(i.add(l.id),t.push({id:l.id,name:l.name,unit:l.unit??null}));for(let l of e.reading_values??[])i.has(l.id)||(i.add(l.id),t.push({id:l.id,name:l.name,unit:l.unit??null}));this._readingRows=t;let a={};for(let l of e.reading_values??[])a[l.id]=String(l.value);this._readingText=a,this._readingsOriginal=JSON.stringify(this._readingNumbers())}_readingNumbers(){let e={};for(let t of this._readingRows){let i=(this._readingText[t.id]??"").trim();if(i==="")continue;let a=parseFloat(i.replace(",","."));isNaN(a)||(e[t.id]=a)}return e}async _onPhotoInput(e){let t=e.target,i=Array.from(t.files??[]);t.value="";let a=this._draft;if(i.length===0||!a)return;let l=10-this._photos.length,c=i.slice(0,Math.max(l,0));this._photoUploading=!0,this._error="";try{for(let h of c){let _=await gt(this.hass,a.entry_id,h);this._uploadedIds=[...this._uploadedIds,_],this._photos=[...this._photos,_]}i.length>c.length&&(this._error=n("photos_limit",this._lang).replace("{max}",String(10)))}catch(h){let _=h instanceof Error&&h.message==="doc_too_large"?"doc_too_large":"doc_upload_failed";this._error=n(_,this._lang)}finally{this._photoUploading=!1}}_removePhoto(e){this._photos=this._photos.filter(t=>t!==e),this._uploadedIds.includes(e)&&(this._uploadedIds=this._uploadedIds.filter(t=>t!==e),Te(this.hass,[e]))}async _loadPartOptions(){let e=this._draft;if(e)try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/parts/overview"}),i=[];for(let l of t.parts||[]){let c=l.entry_id===e.entry_id,h=l.consumers.some(_=>_.entry_id===e.entry_id&&_.task_id===e.task_id);!c&&!h||i.push({part_id:l.part_id,name:l.name,entry_id:l.entry_id,foreign:!c,object_name:l.object_name})}for(let l of e.used_parts||[]){let c=l.entry_id||e.entry_id;i.some(h=>h.part_id===l.part_id&&h.entry_id===c)||i.push({part_id:l.part_id,name:l.name||l.part_id,entry_id:c,foreign:c!==e.entry_id,object_name:null})}let a={};for(let l of e.used_parts||[])a[`${l.entry_id||e.entry_id}:${l.part_id}`]=l.quantity??1;this._partOptions=i,this._partQty=a,this._partQtyOriginal=this._partSelectionKey()}catch{this._partOptions=[]}}_partSelectionKey(){return JSON.stringify(Object.entries(this._partQty).filter(([,e])=>e>0).sort(([e],[t])=>e.localeCompare(t)))}close(){if(this._open=!1,this._error="",this._draft=null,this._originalSnapshot=null,this._uploadedIds.length>0){let e=this._uploadedIds;this._uploadedIds=[],Te(this.hass,e)}}_set(e,t){this._draft&&(this._draft={...this._draft,[e]:t})}async _delete(){if(!this._draft||!this._originalSnapshot)return;let e=this._lang;if(window.confirm(n("history_delete_confirm",e))){this._saving=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/history/delete",entry_id:this._draft.entry_id,task_id:this._draft.task_id,timestamp:this._originalSnapshot.original_timestamp}),this.dispatchEvent(new CustomEvent("history-entry-saved",{detail:{entry_id:this._draft.entry_id,task_id:this._draft.task_id,deleted:!0},bubbles:!0,composed:!0})),this.close()}catch(t){this._error=C(t,e)}finally{this._saving=!1}}}async _save(){if(!(!this._draft||!this._originalSnapshot)){this._saving=!0,this._error="";try{let e={type:"maintenance_supporter/task/history/update",entry_id:this._draft.entry_id,task_id:this._draft.task_id,original_timestamp:this._originalSnapshot.original_timestamp};if(this._draft.timestamp!==this._originalSnapshot.timestamp&&(e.timestamp=this._draft.timestamp),this._draft.notes!==this._originalSnapshot.notes&&(e.notes=this._draft.notes),this._draft.cost!==this._originalSnapshot.cost&&(e.cost=this._draft.cost),this._draft.duration!==this._originalSnapshot.duration&&(e.duration=this._draft.duration),this._draft.completed_by!==this._originalSnapshot.completed_by&&(e.completed_by=this._draft.completed_by),this._partOptions!==null&&this._partSelectionKey()!==this._partQtyOriginal&&(e.used_parts=(this._partOptions||[]).filter(i=>(this._partQty[`${i.entry_id}:${i.part_id}`]||0)>0).map(i=>({part_id:i.part_id,quantity:this._partQty[`${i.entry_id}:${i.part_id}`],...i.foreign?{entry_id:i.entry_id}:{}}))),this._draft.reading_value!==this._originalSnapshot.reading_value&&(e.reading_value=this._draft.reading_value??null),this._readingRows.length>0&&JSON.stringify(this._readingNumbers())!==this._readingsOriginal){let i=this._readingNumbers(),a={};for(let l of this._readingRows)a[l.id]=i[l.id]??null;e.reading_values=a}if(JSON.stringify(this._photos)!==this._photosOriginal&&(e.photo_doc_ids=[...this._photos]),Object.keys(e).filter(i=>!["type","entry_id","task_id","original_timestamp"].includes(i)).length===0){this.close();return}await this.hass.connection.sendMessagePromise(e),this._uploadedIds=[],this.dispatchEvent(new CustomEvent("history-entry-saved",{detail:{entry_id:this._draft.entry_id,task_id:this._draft.task_id,new_timestamp:this._draft.timestamp},bubbles:!0,composed:!0})),this.close()}catch(e){this._error=C(e,this._lang)}finally{this._saving=!1}}}render(){if(!this._open||!this._draft)return p;let e=this._lang,t=this._draft;return o`
      ...
      <div class="dialog" role="dialog" aria-modal="true">
        ...
        <ms-date-field
          ...
          .hass=${this.hass}
          .lang=${e}
          .label=${n("history_edit_timestamp",e)||"Timestamp"}
        ...
        ${this._partOptions&&this._partOptions.length>0?o`
          <div class="parts-block">
            ...
            ${this._partOptions.map(i=>{let a=`${i.entry_id}:${i.part_id}`,l=this._partQty[a]||0;return o`
                <label class="part-row-edit">
                  <input type="checkbox" .checked=${l>0}
                    @change=${c=>{let h=c.target.checked;this._partQty={...this._partQty,[a]:h?1:0}}} />
                  ...
                  ${l>0?o`
                    <input class="part-qty" type="number" min="0.01" max="999" step="0.01"
                      .value=${String(l)}
                      @input=${c=>{let h=parseFloat(c.target.value);!isNaN(h)&&h>0&&(this._partQty={...this._partQty,[a]:h})}} />
                  `:p}
                </label>
              `})}
          </div>
        ...
        <div class="photos-block">
          ...
          ${this._photos.length<10?o`
            <label class="photo-add">
              ...
              <span>${this._photoUploading?n("uploading",e):n("add_photos",e)}</span>
              <input type="file" accept="image/*" multiple
                ?disabled=${this._photoUploading}
                @change=${this._onPhotoInput} />
            </label>`:o`<span class="photos-hint">${n("photos_limit",e).replace("{max}",String(10))}</span>`}
    ...
    `}_renderReadings(e,t){return this._readingRows.length>0?o`
        <div class="readings-block">
          ...
          `)}
        </div>`:e.reading_value==null&&e.task_type!=="reading"?p:o`
      <label>
...
<div class="qr-row">
  <div class="qr-col">
    <img src="${Ns(this._viewResult.svg_data_uri)}" alt="QR Info" />
    <div class="qr-label">${f}</div>
  </div>
  ${_?`<div class="qr-col">
    <img src="${Ns(this._completeResult.svg_data_uri)}" alt="QR Complete" />
    <div class="qr-label">${x}</div>
  </div>`:""}
</div>
<div class="url">${Pe(this._viewResult.url)}</div>
<script>setTimeout(()=>window.print(),300)<\/script>
</body></html>`),a.document.close()}_downloadSvg(e,t){let i=decodeURIComponent(e.svg_data_uri.replace("data:image/svg+xml,","")),a=this._taskName?`${this._objectName}-${this._taskName}`:this._objectName;cs(i,`qr-${_n(a)}-${t}.svg`,"image/svg+xml")}_close(){this._open=!1,this._viewResult=null,this._completeResult=null,this._error="",this._loading=!1}render(){if(!this._open)return o``;let e=this.lang||this.hass?.language||"en",t=this._taskName?`${n("qr_code",e)}: ${this._objectName} \u2014 ${this._taskName}`:`${n("qr_code",e)}: ${this._objectName}`,i=!!this._viewResult;return o`
      <ha-dialog open @closed=${this._close}>
        <div class="dialog-title">${t}</div>
        <div class="content">
  ...
  `,d([b({attribute:!1})],W.prototype,"hass",2),d([b()],W.prototype,"lang",2),d([u()],W.prototype,"_open",2),d([u()],W.prototype,"_loading",2),d([u()],W.prototype,"_error",2),d([u()],W.prototype,"_viewResult",2),d([u()],W.prototype,"_completeResult",2),d([u()],W.prototype,"_urlMode",2);customElements.get("maintenance-qr-dialog")||customElements.define("maintenance-qr-dialog",W)});var qs=w(()=>{"use strict"});function Hs(r){let s=r.getFullYear(),e=String(r.getMonth()+1).padStart(2,"0"),t=String(r.getDate()).padStart(2,"0");return`${s}-${e}-${t}`}var Os=w(()=>{"use strict";qs()});function Ds(r,s){let e=r.interval_analysis,t=e?.weibull_beta,i=e?.weibull_eta;if(t==null||i==null||i<=0)return p;let a=r.interval_days??0,l=r.suggested_interval??a;return o`
  ...
  `}function mn(r,s,e,t,i){let y=Math.max(e,t,s,1)*1.3,m=50,$=[];for(let K=0;K<=m;K++){let Y=K/m*y,or=1-Math.exp(-Math.pow(Y/s,r)),lr=32+Y/y*260,dr=136-or*128;$.push([lr,dr])}let z=$.map(([K,Y])=>`${T(K)},${T(Y)}`).join(" "),P="M32,136 "+$.map(([K,Y])=>`L${T(K)},${T(Y)}`).join(" ")+` L${T($[m][0])},136 Z`,A=32+e/y*260,j=1-Math.exp(-Math.pow(e/s,r)),be=136-j*128,nr=q((1-j)*100,i,0),fi=32+t/y*260,ar=[0,.25,.5,.75,1];return o`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_weibull",i)}">
        ...
        ${e>0?we`
          ...
          <circle cx="${T(A)}" cy="${T(be)}" r="3"
            fill="var(--primary-color, #03a9f4)" />
          <text x="${T(A+4)}" y="${T(be-6)}" fill="var(--primary-color, #03a9f4)"
            font-size="9" font-weight="600">R=${nr}%</text>
        `:p}

        ${t>0&&t!==e?we`
          <line x1="${T(fi)}" y1="${8}" x2="${T(fi)}" y2="${T(136)}"
    ...
    <div class="chart-legend">
      ...
      ${e>0?o`<span class="legend-item"><span class="legend-swatch" style="background:var(--primary-color, #03a9f4); opacity:0.5"></span> ${n("current_interval_marker",i)}</span>`:p}
      ${t>0&&t!==e?o`<span class="legend-item"><span class="legend-swatch" style="background:var(--success-color, #4caf50)"></span> ${n("recommended_marker",i)}</span>`:p}
    </div>
  `}function fn(r,s){return o`
    <div class="weibull-info-row">
      ...
      ${r.weibull_r_squared!=null?o`
        <div class="weibull-info-item">
          ...
          <span class="weibull-info-value">${q(r.weibull_r_squared,s,3)}</span>
        </div>
      `:p}
    </div>
  `}function vn(r,s,e){let t=r.confidence_interval_low,i=r.confidence_interval_high,a=s.suggested_interval??s.interval_days??0,l=s.interval_days??0,c=Math.max(0,t-5),_=i+5-c,f=(t-c)/_*100,x=(i-t)/_*100,v=(a-c)/_*100,y=l>0?(l-c)/_*100:-1;return o`
    <div class="confidence-range">
      <div class="confidence-range-title">
        ${n("confidence_interval",e)}: ${a} ${n("days",e)} (${t}\u2013${i})
      </div>
      <div class="confidence-bar">
        <div class="confidence-fill" style="left:${T(f)}%;width:${T(x)}%"></div>
        ${y>=0?o`<div class="confidence-marker current" style="left:${T(y)}%"></div>`:p}
        <div class="confidence-marker recommended" style="left:${T(v)}%"></div>
      </div>
  ...
  `}var Ms=w(()=>{"use strict";I();R();mt()});function zs(r,s,e){let t=r.degradation_trend!=null&&r.degradation_trend!=="insufficient_data",i=r.days_until_threshold!=null,a=r.environmental_factor!=null&&r.environmental_factor!==1;if(!t&&!i&&!a)return p;let l=r.degradation_trend==="rising"?"M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z":r.degradation_trend==="falling"?"M16,18L18.29,15.71L13.41,10.83L9.41,14.83L2,7.41L3.41,6L9.41,12L13.41,8L19.71,14.29L22,12V18H16Z":"M22,12L18,8V11H3V13H18V16L22,12Z";return o`
      ...
      <div class="prediction-grid">
        ${t?o`
          <div class="prediction-item">
            ...
            ${r.degradation_rate!=null?o`<span class="prediction-rate">${r.degradation_rate>0?"+":""}${q(r.degradation_rate,s,Math.abs(r.degradation_rate)>=10?0:1)} ${r.trigger_entity_info?.unit_of_measurement||""}/${n("day_short",s)}</span>`:p}
          </div>
        `:p}
        ${i?o`
          <div class="prediction-item">
            ...
            ${r.threshold_prediction_date?o`<span class="prediction-date">${U(r.threshold_prediction_date,s)}</span>`:p}
            ${r.threshold_prediction_confidence?o`<span class="confidence-dot ${r.threshold_prediction_confidence}"></span>`:p}
            ${(r.prediction_cycles??0)>0?o`<span class="prediction-cycles">${n("prediction_cycles",s)}: ${r.prediction_cycles}</span>`:p}
          </div>
        `:p}
        ${a&&e.environmental?o`
          <div class="prediction-item">
            ...
            <span class="prediction-value">${q(r.environmental_factor,s,2)}x</span>
            ${r.environmental_entity?o`<span class="prediction-entity entity-link" @click=${c=>ls(c,r.environmental_entity)}>${r.environmental_entity}</span>`:p}
          </div>
        `:p}
      </div>
    </div>
  `}var Us=w(()=>{"use strict";I();R()});function Fs(r,s,e,t){let i=Math.max(r||1,s);return o`
  ...
  `}var Bs=w(()=>{"use strict";I();R()});function Ws(r,s,e){if(!e.seasonal||!r.seasonal_factor||r.seasonal_factor===1)return p;let t=Vs.map(c=>n(c,s)),i=new Date().getMonth(),a=r.seasonal_factors||r.interval_analysis?.seasonal_factors||null,l=a&&a.length===12?a:t.map((c,h)=>{let _=r.seasonal_factor||1,f=Math.sin((h-6)*Math.PI/6)*.3;return Math.max(.7,Math.min(1.3,_+f))});return o`
    <div class="seasonal-card-compact">
      <h4>${n("seasonal_awareness",s)}</h4>
      <div class="seasonal-mini-chart">
        ${l.map((c,h)=>{let _=c*40,f=c<.9?"low":c>1.1?"high":"normal";return o`
  ...
  `}function Ks(r,s){return bn(r,s)}function bn(r,s){let e=r.seasonal_factors??r.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return p;let t=r.interval_analysis?.seasonal_reason,i=new Date().getMonth(),a=300,l=100,c=8,_=l-c-4,f=Math.max(...e,1.5),x=a/12,v=x*.65,y=c+_-1/f*_;return o`
    <div class="seasonal-chart">
      <div class="seasonal-chart-title">
        ...
        ${t?o`<span class="source-tag">${t==="learned"?n("seasonal_learned",s):n("seasonal_manual",s)}</span>`:p}
      </div>
      <svg viewBox="0 0 ${a} ${l}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${n("chart_seasonal",s)}">
        <line x1="0" y1="${T(y)}" x2="${a}" y2="${T(y)}"
          stroke="var(--divider-color)" stroke-width="1" stroke-dasharray="4,3" />
        ${e.map((m,$)=>{let z=m/f*_,P=$*x+(x-v)/2,A=c+_-z,j=$===i,be=m<1?"var(--success-color, #4caf50)":m>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return we`
            <rect x="${T(P)}" y="${T(A)}"
              width="${T(v)}" height="${T(z)}"
              fill="${be}" opacity="${j?1:.5}" rx="2" />
          `})}
      </svg>
      <div class="seasonal-labels">
        ${Vs.map((m,$)=>o`<span class="seasonal-label ${$===i?"active-month":""}">${n(m,s)}</span>`)}
      </div>
    </div>
  `}var Vs,Ys=w(()=>{"use strict";I();R();mt();Vs=["month_jan","month_feb","month_mar","month_apr","month_may","month_jun","month_jul","month_aug","month_sep","month_oct","month_nov","month_dec"]});var H,Gs=w(()=>{"use strict";I();O();R();le();Os();ut();pt();Jt();Ye();oi();Ms();Us();Bs();Ce();Ys();H=class extends S{constructor(){super(...arguments);this._open=!1;this._entryId=null;this._taskId=null;this._task=null;this._objectName="";this._busy=!1;this._error="";this._showSkip=!1;this._showReset=!1;this._showDetails=!1;this._showAdaptive=!1;this._skipReason="";this._resetDate="";this._features={adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1};this._toast="";this._featuresLoaded=!1;this._currencySymbol=""}get _lang(){return L(this.hass)}async openFor(e,t){this._entryId=e,this._taskId=t,this._error="",this._showSkip=!1,this._showReset=!1,this._showAdaptive=!1,this._skipReason="",this._resetDate=Hs(new Date),this._open=!0,await Promise.all([this._loadTask(),this._loadFeatures()])}async _loadFeatures(){if(!this._featuresLoaded)try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/settings"});e?.features&&(this._features={...this._features,...e.features}),this._currencySymbol=e?.budget?.currency_symbol||"",ot(e?.budget),this._featuresLoaded=!0}catch{}}close(){this._open=!1,this._task=null,this._error=""}async _loadTask(){if(!(!this._entryId||!this._taskId))try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._objectName=e.object?.name||"";let t=(e.tasks||[]).find(i=>i.id===this._taskId);this._task=t??null}catch(e){this._error=C(e,this._lang)}}async _runWs(e){this._busy=!0,this._error="";try{return await this.hass.connection.sendMessagePromise(e),this._busy=!1,!0}catch(t){return this._error=C(t,this._lang),this._busy=!1,!1}}_notifyChanged(e){this.dispatchEvent(new CustomEvent("task-action-fired",{detail:{entry_id:this._entryId,task_id:this._taskId,action:e},bubbles:!0,composed:!0}))}_onComplete(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(Z(),se)).then(async({openCompleteDialog:e})=>{let t=this._task,i=[];try{i=(await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects",compact:!0})).objects||[]}catch{}e(Ge({entryId:this._entryId,taskId:this._taskId,taskName:t.name,task:t,objects:i,lang:this._lang,checklist:t.checklist||[],adaptiveEnabled:!!t.adaptive_config?.enabled,currencySymbol:this._currencySymbol}))&&(this._notifyChanged("complete"),this.close())})}async _onSkipConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/skip",entry_id:this._entryId,task_id:this._taskId,reason:this._skipReason.trim()||null})&&(this._notifyChanged("skip"),this.close())}async _onResetConfirm(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/reset",entry_id:this._entryId,task_id:this._taskId,date:this._resetDate||void 0})&&(this._notifyChanged("reset"),this.close())}_onEdit(){!this._entryId||!this._taskId||Promise.resolve().then(()=>(Z(),se)).then(({openEditTaskDialog:e})=>{e(this._entryId,this._taskId),this.close()})}_onQr(){!this._entryId||!this._taskId||!this._task||Promise.resolve().then(()=>(Z(),se)).then(({openQrDialog:e})=>{e({entry_id:this._entryId,task_id:this._taskId,task_name:this._task.name,object_name:this._objectName}),this.close()})}async _onDelete(){if(!this._entryId||!this._taskId)return;let e=n("delete_task_confirm",this._lang)||`Delete "${this._task?.name}"?`;if(!window.confirm(e))return;await this._runWs({type:"maintenance_supporter/task/delete",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("delete"),this.close())}async _onArchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/archive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("archive"),this.close())}async _onUnarchive(){if(!this._entryId||!this._taskId)return;await this._runWs({type:"maintenance_supporter/task/unarchive",entry_id:this._entryId,task_id:this._taskId})&&(this._notifyChanged("unarchive"),this.close())}_onOpenInPanel(){if(!this._entryId||!this._taskId)return;let e=`/maintenance-supporter?entry_id=${encodeURIComponent(this._entryId)}&task_id=${encodeURIComponent(this._taskId)}`;history.pushState(null,"",e),window.dispatchEvent(new CustomEvent("location-changed")),this.close()}async _applySuggestion(){if(!this._entryId||!this._taskId||!this._task?.suggested_interval)return;await this._runWs({type:"maintenance_supporter/task/apply_suggestion",entry_id:this._entryId,task_id:this._taskId,interval:this._task.suggested_interval})&&(this._toast=n("suggestion_applied",this._lang)||"Applied",this._notifyChanged("apply_suggestion"),await this._loadTask(),setTimeout(()=>{this._toast=""},2500))}async _reanalyzeInterval(){if(!(!this._entryId||!this._taskId)){this._busy=!0,this._error="";try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/task/analyze_interval",entry_id:this._entryId,task_id:this._taskId});this._toast=e.recommended_interval?`${n("reanalyze_result",this._lang)||"Recomputed"}: ${Be(e.recommended_interval,"days",this._lang)} (${e.data_points} pts)`:n("reanalyze_insufficient_data",this._lang)||"Not enough data",await this._loadTask(),setTimeout(()=>{this._toast=""},3500)}catch(e){this._error=C(e,this._lang)}finally{this._busy=!1}}}_onEditHistoryEntry(e){!this._entryId||!this._taskId||Promise.resolve().then(()=>(Z(),se)).then(({openHistoryEditDialog:t})=>{t({entry_id:this._entryId,task_id:this._taskId,original_timestamp:e.timestamp,type:e.type,timestamp:e.timestamp,notes:e.notes??null,cost:e.cost??null,duration:e.duration??null,completed_by:e.completed_by??null,used_parts:e.used_parts??null,photo_doc_ids:Qt(e),reading_value:e.reading_value??null,reading_values:me(e),readings:this._task?.readings??[],task_type:this._task?.type??null,reading_unit:this._task?.reading_unit??null})})}_renderRecommendation(e){if(!this._features.adaptive||!e.suggested_interval||e.suggested_interval===e.interval_days)return p;let t=this._lang;return o`
      <div class="recommendation-card">
        <h4>${n("suggested_interval",t)}</h4>
        ${Fs(e.interval_days,e.suggested_interval,e.interval_confidence||"medium",t)}
    ...
    `}_renderAdaptive(e){let t=this._lang,i=this._features.adaptive&&e.suggested_interval&&e.suggested_interval!==e.interval_days,a=e.degradation_trend!=null&&e.degradation_trend!=="insufficient_data"||e.days_until_threshold!=null||e.environmental_factor!=null&&e.environmental_factor!==1,l=this._features.adaptive&&e.interval_analysis?.weibull_beta!=null&&e.interval_analysis?.weibull_eta!=null,c=this._features.seasonal&&e.seasonal_factor&&e.seasonal_factor!==1;return!i&&!a&&!l&&!c?o`<div class="adaptive-empty">
        ${n("adaptive_no_data",t)||"Not enough completion history yet for adaptive analysis."}
      </div>`:o`
      <div class="adaptive-stack">
        ${this._toast?o`<div class="toast">${this._toast}</div>`:p}
        ${i?this._renderRecommendation(e):p}
        ${a?zs(e,t,this._features):p}
        ${l?Ds(e,t):p}
        ${c?o`
          ${Ws(e,t,this._features)}
          ${e.seasonal_factors?.length===12||e.interval_analysis?.seasonal_factors?.length===12?Ks(e,t):p}
        `:p}
      </div>
    `}_renderHistoryReadings(e,t,i){let a=me(e),l=c=>q(c,i,{maximumFractionDigits:3});if(a.length>0)return o`<div class="history-readings">
        ${a.map(c=>{let h=fs(t,e,c.id);return o`<span class="history-reading"><span class="history-reading-name">${c.name}</span>
            <span class="history-reading-value">${l(c.value)}${c.unit?` ${c.unit}`:""}${h==null?"":` (${h>=0?"+":""}${l(h)})`}</span></span>`})}
      </div>`;if(e.reading_value!=null){let c=this._task?.reading_unit?` ${this._task.reading_unit}`:"";return o`<div class="history-readings"><span class="history-reading">
        <span class="history-reading-name">${n("reading_label",i)}</span>
        <span class="history-reading-value">${l(e.reading_value)}${c}</span></span></div>`}return p}_renderDetails(e){let t=this._lang,i=e.history||[],a=i.filter(h=>h.type==="completed"),l=a.reduce((h,_)=>h+(typeof _.cost=="number"?_.cost:0),0),c=(()=>{let h=a.map(_=>typeof _.duration=="number"?_.duration:null).filter(_=>_!=null);return h.length?Math.round(h.reduce((_,f)=>_+f,0)/h.length):null})();return o`
      <div class="details">
        <div class="stats-grid">
          <div class="stat">
            <span class="stat-label">${n("times_performed",t)||"Performed"}</span>
            <span class="stat-value">${a.length}</span>
          </div>
          <div class="stat">
            <span class="stat-label">${n("total_cost",t)||"Total cost"}</span>
            <span class="stat-value">${q(l,t,2)}</span>
          </div>
          <div class="stat">
            <span class="stat-label">${n("avg_duration",t)||"Avg duration"}</span>
            <span class="stat-value">${c!=null?`${c}m`:"\u2014"}</span>
          </div>
        </div>
        <div class="history-header">
          <strong>${n("history",t)||"History"}</strong>
          <span class="history-count">${i.length}</span>
        </div>
        ${i.length===0?o`<div class="history-empty">${n("history_empty",t)||"No history yet."}</div>`:o`
              <div class="history-list">
                ${[...i].reverse().slice(0,20).map(h=>{let _=["completed","reset","skipped"].includes(h.type);return o`
                    <div class="history-entry">
                      <div class="history-line">
                        <span class="history-type type-${h.type}">${n(h.type,t)}</span>
                        <span class="history-date">${ns(h.timestamp,t)}</span>
                        ${_?o`<button class="history-edit"
                                   title="${n("history_edit_button",t)||"Edit"}"
                                   @click=${()=>this._onEditHistoryEntry(h)}>
                              <ha-icon icon="mdi:pencil"></ha-icon>
                            </button>`:p}
                      </div>
                      ${h.notes?o`<div class="history-notes">${h.notes}</div>`:p}
                      ${this._renderHistoryReadings(h,i,t)}
                      ${(()=>{let f=Qt(h);return f.length?o`<div class="history-photos">
                              ${f.map(x=>o`<maintenance-history-photo .hass=${this.hass} .docId=${x}></maintenance-history-photo>`)}
                            </div>`:p})()}
                      ${h.cost!=null||h.duration!=null?o`<div class="history-meta">
                            ${h.cost!=null?o`<span>💰 ${q(h.cost,t,2)}</span>`:p}
                            ${h.duration!=null?o`<span>⏱️ ${h.duration}m</span>`:p}
                          </div>`:p}
                    </div>
                  `})}
                ${i.length>20?o`<div class="history-more">… +${i.length-20} ${n("older_entries",t)||"older"}</div>`:p}
              </div>
            `}
      </div>
    `}render(){if(!this._open)return p;let e=this._lang,t=this._task,i=this.hass?.user?.is_admin??!0;return o`
      ...
      <div class="dialog" role="dialog" aria-modal="true">
        ${t?o`
              <div class="header">
                <div class="title">
                  <span class="status-dot" style="background: ${_e[t.status]||"#ccc"}"></span>
                  <span class="task-name">${t.name}</span>
                </div>
                <div class="object">
                  <button class="link-inline" @click=${()=>{this._entryId&&Promise.resolve().then(()=>(Z(),se)).then(({openObjectQuickActions:a})=>{a(this._entryId),this.close()})}}>${this._objectName}</button>
                </div>
                <div class="quick-info">
                  ${t.next_due?o`<span><strong>${n("next_due",e)||"Next due"}:</strong> ${U(t.next_due,e)}</span>`:p}
                  ${t.last_performed?o`<span><strong>${n("last_performed",e)||"Last"}:</strong> ${U(t.last_performed,e)}</span>`:p}
                  ${t.schedule?.kind&&!["manual","one_time"].includes(t.schedule.kind)||t.interval_days!=null?o`<span><strong>${n("interval",e)||"Interval"}:</strong> ${os(t,e)}</span>`:p}
                  ${oe(t)?o`<span><strong>${n("phase_current",e)}:</strong> ${oe(t)}</span>`:p}
                </div>
              </div>
              ...
              ${this._showSkip?o`
                    <div class="inline-form">
                      <label>${n("skip_reason",e)||"Skip reason (optional)"}</label>
                  ...
                  `:this._showReset?o`
                    <div class="inline-form">
                      <label>${n("reset_to_date",e)||"Reset last_performed to"}</label>
                      <ms-date-field
                        kind="date"
                        .hass=${this.hass}
                        .lang=${e}
                  ...
                  `:o`
                    ...
                    ${i?o`
                          <div class="actions secondary-row">
                            ...
                            <ha-button size="small" appearance="outlined" variant="neutral"
                              ...
                              ${t.archived?n("unarchive",e)||"Unarchive":n("archive",e)||"Archive"}
                            </ha-button>
                    ...
                    <div class="details-toggle">
                      <button class="link" @click=${()=>{this._showDetails=!this._showDetails}}>
                        ...
                        ${this._showDetails?n("hide_details",e)||"Hide details":n("show_details",e)||"Show history + stats"}
                      </button>
                      ${this._features.adaptive||this._features.seasonal||this._features.environmental?o`<button class="link" @click=${()=>{this._showAdaptive=!this._showAdaptive}}>
                            ...
                            ${this._showAdaptive?n("hide_stats",e)||"Hide stats":n("show_stats",e)||"Show stats + graphs"}
                          </button>`:p}
                    </div>
                    ...
                    <div class="footer">
                      <button class="link" @click=${this._onOpenInPanel}>
                        ...
                        ${n("open_in_panel",e)||"Open in Maintenance panel"}
                      </button>
                    </div>
                  `}
            `:o`<div class="loading">${n("loading",e)||"Loading\u2026"}</div>`}
      </div>
    `}};H.styles=[lt,E`
    :host { display: contents; }
    .backdrop {
  ...
  `],d([b({attribute:!1})],H.prototype,"hass",2),d([u()],H.prototype,"_open",2),d([u()],H.prototype,"_entryId",2),d([u()],H.prototype,"_taskId",2),d([u()],H.prototype,"_task",2),d([u()],H.prototype,"_objectName",2),d([u()],H.prototype,"_busy",2),d([u()],H.prototype,"_error",2),d([u()],H.prototype,"_showSkip",2),d([u()],H.prototype,"_showReset",2),d([u()],H.prototype,"_showDetails",2),d([u()],H.prototype,"_showAdaptive",2),d([u()],H.prototype,"_skipReason",2),d([u()],H.prototype,"_resetDate",2),d([u()],H.prototype,"_features",2),d([u()],H.prototype,"_toast",2);customElements.get("maintenance-task-quick-actions-dialog")||customElements.define("maintenance-task-quick-actions-dialog",H)});function Qs(r){return r?customElements.get("ha-markdown")?o`<ha-markdown class="notes-md" .content=${r} breaks></ha-markdown>`:o`${r}`:p}var Js=w(()=>{"use strict";I()});var X,Zs=w(()=>{"use strict";I();Ut();Js();O();R();le();X=class extends S{constructor(){super(...arguments);this._open=!1;this._entryId=null;this._data=null;this._busy=!1;this._error=""}get _lang(){return L(this.hass)}async openFor(e){this._entryId=e,this._error="",this._open=!0,await this._load()}close(){this._open=!1,this._data=null,this._error=""}async _load(){if(this._entryId)try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:this._entryId});this._data=e}catch(e){this._error=C(e,this._lang)}}_onEditObject(){!this._entryId||!this._data||Promise.resolve().then(()=>(Z(),se)).then(({openEditObjectDialog:e})=>{e(this._entryId,this._data.object),this.close()})}_onAddTask(){this._entryId&&Promise.resolve().then(()=>(Z(),se)).then(({openCreateTaskDialog:e})=>{e(this._entryId),this.close()})}async _onDelete(){if(!this._entryId||!this._data)return;let e=n("delete_object_confirm",this._lang)||`Delete "${this._data.object.name}" and all its tasks?`;if(window.confirm(e)){this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/object/delete",entry_id:this._entryId}),this.dispatchEvent(new CustomEvent("object-deleted",{detail:{entry_id:this._entryId},bubbles:!0,composed:!0})),this.close()}catch(t){this._error=C(t,this._lang)}finally{this._busy=!1}}}async _onArchiveObject(){if(!this._entryId||!this._data)return;let e=!!this._data.object.archived;if(!e){let t=n("confirm_archive_object",this._lang)||"Archive this object and its tasks?";if(!window.confirm(t))return}this._busy=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:e?"maintenance_supporter/object/unarchive":"maintenance_supporter/object/archive",entry_id:this._entryId}),this.dispatchEvent(new CustomEvent("object-changed",{detail:{entry_id:this._entryId},bubbles:!0,composed:!0})),this.close()}catch(t){this._error=C(t,this._lang)}finally{this._busy=!1}}_onTaskClick(e){this._entryId&&Promise.resolve().then(()=>(Z(),se)).then(({openTaskQuickActions:t})=>{t(this._entryId,e)})}render(){if(!this._open)return p;let e=this._lang,t=this._data,i=t?.object,a=t?.tasks||[],l=this.hass?.user?.is_admin??!0;return o`
      <div class="backdrop" @click=${this.close}></div>
      <div class="dialog" role="dialog" aria-modal="true">
        ${t&&i?o`
              <div class="header">
              ...
              <div class="tasks-section">
                <div class="section-header">
                  <strong>${n("tasks",e)||"Tasks"}</strong>
                  <span class="count">${a.length}</span>
                </div>
                ${a.length===0?o`<div class="empty">${n("no_tasks",e)||"No tasks yet."}</div>`:o`
                      <div class="task-list">
                        ${a.map(c=>o`
                          <div class="task-row" @click=${()=>this._onTaskClick(c.id)}>
                            <span class="status-dot" style="background: ${_e[c.status]||"#ccc"}"></span>
                            <span class="task-name">${c.name}</span>
                            <span class="task-status">${n(c.status||"ok",e)}</span>
                          </div>
                        `)}
                      </div>
                    `}
              ...
              ${i.notes?o`
                    <div class="notes-section">
                      ...
                      <div class="notes-body">${Qs(i.notes)}</div>
                    </div>
                  `:p}
              ...
              ${l?o`
                    <div class="actions">
                      ...
                      <button class="btn" @click=${this._onArchiveObject} ?disabled=${this._busy}>
                        ...
                        ${i.archived?n("unarchive_object",e)||"Unarchive object":n("archive_object",e)||"Archive object"}
                      </button>
  ...
  `,d([b({attribute:!1})],X.prototype,"hass",2),d([u()],X.prototype,"_open",2),d([u()],X.prototype,"_entryId",2),d([u()],X.prototype,"_data",2),d([u()],X.prototype,"_busy",2),d([u()],X.prototype,"_error",2);customElements.get("maintenance-object-quick-actions-dialog")||customElements.define("maintenance-object-quick-actions-dialog",X)});function Xs(){let r=window;return r.__msSettingsCache??={promise:null}}function vt(r){let s=Xs();if(s.promise)return s.promise;let e=r.connection.sendMessagePromise({type:"maintenance_supporter/settings"}).then(t=>({features:t.features??li.features,defaultWarningDays:t.general?.default_warning_days??7,rowActionStyle:t.general?.row_action_style??li.rowActionStyle})).catch(()=>(s.promise===e&&(s.promise=null),li));return s.promise=e,e}function er(){Xs().promise=null}var li,tr=w(()=>{"use strict";li={features:{adaptive:!1,predictions:!1,seasonal:!1,environmental:!1,budget:!1,groups:!1,checklists:!1,schedule_time:!1,completion_actions:!1},defaultWarningDays:7,rowActionStyle:"buttons_compact"}});var se={};pr(se,{__resetSettingsCacheForTests:()=>Sn,getRowActionStyle:()=>di,openCompleteDialog:()=>In,openCreateObjectDialog:()=>ci,openCreateTaskDialog:()=>pi,openEditObjectDialog:()=>Tn,openEditTaskDialog:()=>An,openHistoryEditDialog:()=>Cn,openObjectQuickActions:()=>Rn,openQrDialog:()=>Pn,openTaskQuickActions:()=>hi});function bt(){return document.querySelector("home-assistant")?.hass}function En(){return document.querySelector("home-assistant")?.shadowRoot??document.body}function re(r){let s=En(),e=s.querySelector(r)??document.body.querySelector(r);return e?e.parentNode!==s&&s.appendChild(e):(e=document.createElement(r),s.appendChild(e)),e}function ne(r){let s=bt();if(!s)return!1;r.hass=s;let e=L(s);return qt(e)||Ke(e).then(()=>{r.requestUpdate?.()}),Ht(s.locale,s.config?.country),!0}function di(r){return vt(r).then(s=>s.rowActionStyle)}function Sn(){er()}function ci(){let r=re(ir);return ne(r)?(r.openCreate(),!0):!1}function Tn(r,s){let e=re(ir);return ne(e)?(e.openEdit(r,s),!0):!1}function pi(r="",s){let e=re(sr);if(!ne(e))return!1;let t=bt();return t?((async()=>{let i=await vt(t),a=e;a.checklistsEnabled=i.features.checklists,a.scheduleTimeEnabled=i.features.schedule_time,a.completionActionsEnabled=i.features.completion_actions,a.defaultWarningDays=i.defaultWarningDays,a.openCreate(r,s)})(),!0):!1}function An(r,s){let e=re(sr);if(!ne(e))return!1;let t=bt();return t?((async()=>{try{let[i,a]=await Promise.all([t.connection.sendMessagePromise({type:"maintenance_supporter/object",entry_id:r}),vt(t)]),l=(i.tasks||[]).find(h=>h.id===s);if(!l){console.warn(`openEditTaskDialog: task ${s} not found in entry ${r}`);return}let c=e;c.checklistsEnabled=a.features.checklists,c.scheduleTimeEnabled=a.features.schedule_time,c.completionActionsEnabled=a.features.completion_actions,c.defaultWarningDays=a.defaultWarningDays,await c.openEdit(r,l)}catch(i){console.warn("openEditTaskDialog: failed to load task/features",i)}})(),!0):!1}function Cn(r){let s=re(yn);return ne(s)?(s.openEdit(r),!0):!1}function In(r){let s=re(xn);return ne(s)?(Qe(s,r,bt()?.language||"en"),!0):!1}function Pn(r){let s=re(wn);return ne(s)?(s.openForTask(r.entry_id,r.task_id,r.object_name,r.task_name),!0):!1}function hi(r,s){let e=re($n);return ne(e)?(e.openFor(r,s),!0):!1}function Rn(r){let s=re(kn);return ne(s)?(s.openFor(r),!0):!1}var ir,sr,yn,xn,wn,$n,kn,Z=w(()=>{"use strict";Ss();Rs();ei();Ls();js();Gs();Zs();R();ut();tr();ir="maintenance-object-dialog",sr="maintenance-task-dialog",yn="maintenance-history-edit-dialog",xn="maintenance-complete-dialog",wn="maintenance-qr-dialog",$n="maintenance-task-quick-actions-dialog",kn="maintenance-object-quick-actions-dialog"});I();var Er=["assignee_pool","required_completion_fields","checklist","labels","history","readings"],Sr=["checklist_progress"],Tr=["tasks","parts"],Ar=["manual_docs","battery_fleet_excluded"];function Lt(r,s,e=[]){for(let t of s)r[t]===void 0&&(r[t]=[]);for(let t of e)r[t]===void 0&&(r[t]={})}function Cr(r){let s=r;Lt(s,Tr),s.object&&typeof s.object=="object"&&Lt(s.object,Ar);for(let e of s.tasks)Lt(e,Er,Sr);return r}function Ue(r){for(let s of r)Cr(s);return r}function Ir(r,s){if(s.objects)return s.objects;let e=s.delta||[],t=s.removed||[];if(!e.length&&!t.length)return null;let i=new Map(r.map(a=>[a.entry_id,a]));for(let a of e)i.set(a.entry_id,a);for(let a of t)i.delete(a);return[...i.values()]}function Oi(r,s){return s.objects&&Ue(s.objects),s.delta&&Ue(s.delta),Ir(r,s)}O();R();zt();Ut();function Ee(r){let s=window;s.customCards=s.customCards||[],s.customCards.some(e=>e.type===r.type)||s.customCards.push(r)}Vt();Bt();pt();ut();I();O();R();var Kr=["overdue","triggered","due_soon","ok"],Q=class extends S{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-card"};this._objects=[];this._loadingObjects=!0;this._loadError=!1;this._views=[];this._objectsLoaded=!1;this._onEntitiesChanged=e=>{this._valueChanged("entity_ids",e.detail.value||[])}}get _lang(){return L(this.hass)}setConfig(e){this._config={...e}}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._objectsLoaded&&(this._objectsLoaded=!0,this._loadObjects())}async _loadObjects(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects"});this._objects=e.objects||[],this._loadError=!1}catch{this._objects=[],this._loadError=!0}this._loadingObjects=!1;try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"});this._views=e.views||[]}catch{this._views=[]}}_valueChanged(e,t){let i={...this._config,[e]:t};(Array.isArray(t)&&t.length===0||t==="")&&delete i[e],this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i}}))}_toggleStatus(e,t){let i=new Set(this._config.filter_status||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_status",[...i])}_toggleObject(e,t){let i=new Set(this._config.filter_objects||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_objects",[...i])}_toggleLabel(e,t){let i=new Set(this._config.filter_labels||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_labels",[...i])}_toggleArea(e,t){let i=new Set(this._config.filter_areas||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_areas",[...i])}_togglePriority(e,t){let i=new Set(this._config.filter_priority||[]);t?i.add(e):i.delete(e),this._valueChanged("filter_priority",[...i])}render(){let e=this._lang,t=new Set(this._config.filter_status||[]),i=new Set(this._config.filter_objects||[]),a=[...this._objects].map(m=>m.object.name).sort((m,$)=>m.localeCompare($)),l=new Set(this._config.filter_areas||[]),c=[...new Set(this._objects.map(m=>m.object.area_id).filter(m=>!!m).concat([...l]))],h=m=>this.hass?.areas?.[m]?.name||m,_=c.map(m=>({id:m,name:h(m)})).sort((m,$)=>m.name.localeCompare($.name)),f=new Set(this._config.filter_labels||[]),x=new Set(this._config.filter_priority||[]),v=[...new Set(this._objects.flatMap(m=>m.tasks.flatMap($=>$.labels||[])))].sort((m,$)=>m.localeCompare($)),y=[];for(let m of this._objects)for(let $ of m.tasks)$.sensor_entity_id&&y.push($.sensor_entity_id),$.binary_sensor_entity_id&&y.push($.binary_sensor_entity_id);return o`
      <div class="editor">
        <ha-textfield
  ...
  `,d([b({attribute:!1})],Q.prototype,"hass",2),d([u()],Q.prototype,"_config",2),d([u()],Q.prototype,"_objects",2),d([u()],Q.prototype,"_loadingObjects",2),d([u()],Q.prototype,"_loadError",2),d([u()],Q.prototype,"_views",2);customElements.get("maintenance-supporter-card-editor")||customElements.define("maintenance-supporter-card-editor",Q);ei();I();O();I();O();R();var Ze={printOptions:"msp-print-options",batteryRosterOpen:"msp-bf-roster-open",docSort:"msp-doc-sort",overviewTab:"msp-overview-tab",collapsedSections:"msp-collapsed-sections",chartRange:"msp-chart-range",chartHideOutliers:"msp-chart-hide-outliers",taskSort:"maintenance_supporter_sort",objectSort:"maintenance_supporter_object_sort",groupBy:"maintenance_supporter_groupby",objectView:"maintenance_supporter_object_view",objectsCache:"msp-objects-cache",gettingStartedDismissed:"msp-gs-dismissed",batteryRosterSort:"ms_bf_roster_sort"};function ti(r){try{return localStorage.getItem(r)}catch{return null}}function ii(r,s){try{localStorage.setItem(r,s)}catch{}}le();mt();var B=class B extends S{constructor(){super(...arguments);this.flat=!1;this._ov=null;this._loading=!1;this._marking=!1;this._error="";this._history=null;this._rosterSort=B._storedSort();this._typeFilter=null;this._recorded=[];this._historyRequested=!1;this._localeReady=!1;this._markAll=async()=>{await this._mark(void 0)};this._repair=async()=>{if(!this._marking){this._marking=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/setup",language:this._lang}),await this._load()}catch(e){this._error=C(e,this._lang)}finally{this._marking=!1}}};this._loadHistory=async e=>{let t=e.target.open;if(ii(Ze.batteryRosterOpen,t?"1":"0"),!(!t||this._historyRequested)){this._historyRequested=!0;try{let i=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/overview_history"});this._history=i.series}catch{this._history=null}}}}get _lang(){return L(this.hass)}connectedCallback(){super.connectedCallback(),this.hass&&this._load()}updated(e){at(this,e),e.has("hass")&&this.hass&&!this._localeReady&&(this._localeReady=!0,Ke(this._lang).then(()=>this.requestUpdate()),this._ov===null&&!this._loading&&this._load())}async _load(){this._loading=!0,this._error="";try{this._ov=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/overview"})}catch(e){this._error=C(e,this._lang)}finally{this._loading=!1}}async _mark(e){if(!this._marking){this._marking=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/mark_replaced",...e?{entity_ids:e}:{}}),await this._load()}catch(t){this._error=C(t,this._lang)}finally{this._marking=!1}}}async _setExcluded(e,t){if(!this._marking){this._marking=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/set_excluded",entity_id:e,excluded:t}),await this._load()}catch(i){this._error=C(i,this._lang)}finally{this._marking=!1}}}async _addBattery(e){let t=e.detail?.value;if(!(!t||this._marking)){this._marking=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/battery_fleet/set_included",entity_id:t,included:!0}),await this._load()}catch(i){this._error=C(i,this._lang)}finally{this._marking=!1}}}async _setTrackSelf(e){await this._setFleetOption("set_track_self_charging",e.target.checked)}async _setDueWithoutSensor(e){await this._setFleetOption("set_due_without_sensor",e.target.checked)}async _setFleetOption(e,t){if(!this._marking){this._marking=!0,this._error="";try{await this.hass.connection.sendMessagePromise({type:`maintenance_supporter/battery_fleet/${e}`,enabled:t}),await this._load()}catch(i){this._error=C(i,this._lang)}finally{this._marking=!1}}}_rosterOpen(){return ti(Ze.batteryRosterOpen)!=="0"}_predictedTitle(e,t){if(e.forecast_overdue)return n("battery_fleet_forecast_overdue",t);let i=this._predictedDate(e.days_until??0);if(e.predicted_source==="trend")return n("battery_fleet_predicted_trend",t).replace("{date}",i).replace("{confidence}",n("cal_confidence_"+(e.prediction_confidence||"medium"),t));if(e.lifetime_months!=null&&e.lifetime_source){let a=n("lifetime_source_"+e.lifetime_source,t).replace("{n}",String(e.lifetime_samples??0));return n("battery_fleet_predicted_typical",t).replace("{date}",i).replace("{months}",String(e.lifetime_months)).replace("{type}",e.battery_type).replace("{source}",a)}return n("battery_fleet_predicted_on",t).replace("{date}",i)}_sparkline(e){let t=this._history?.[e.entity_id];if(!t||t.points.length<2)return p;let i=110,a=24,l=2,c=t.points[0][0],h=t.points[t.points.length-1][0],_=Date.now()/1e3,f=e.status!=="low"&&e.predicted_source==="trend"&&e.days_until!=null?_+e.days_until*86400:null,x=Math.max(h,f??h),v=P=>x===c?l:l+(P-c)/(x-c)*(i-2*l),y=P=>l+(1-Math.min(100,Math.max(0,P))/100)*(a-2*l),m=t.points.map(([P,A])=>`${T(v(P))},${T(y(A))}`).join(" "),$=t.points[t.points.length-1][1],z=T(y(t.threshold));return o`<svg
    ...
    >
      ...
      ${f!==null?o`<line
            ...
            y2=${z}
          ></line>`:p}
    </svg>`}static _storedSort(){return ti(Ze.batteryRosterSort)==="name"?"name":"urgency"}_setSort(e){this._rosterSort=e,ii(Ze.batteryRosterSort,e)}_sortedRoster(e){let t=this._typeFilter===null?e:e.filter(a=>a.battery_type===this._typeFilter);if(this._rosterSort==="name")return t;let i=a=>a.status==="low"?-1e3+(a.level??101)/101:a.days_until??1/0;return[...t].sort((a,l)=>i(a)-i(l)||a.device_name.localeCompare(l.device_name))}_predictedDate(e){return this._fmtDate(Date.now()+e*864e5)}_fmtDate(e){let t=new Date(e),i=a=>String(a).padStart(2,"0");return U(`${t.getFullYear()}-${i(t.getMonth()+1)}-${i(t.getDate())}`,this._lang)}_shoppingLine(e){return Object.entries(e).map(([t,i])=>o`<button
              ...
              <div class="bf-rows">
                ${t.low.map(a=>o`
                    <div class="bf-row">
                      ...
                      ${a.available===!1?o`<span class="bf-offline">${n("battery_fleet_offline",e)}</span>`:a.no_sensor?o`<span class="bf-offline bf-nosensor">${n("battery_fleet_no_sensor",e)}</span>`:p}
                      <span class="bf-type">${a.quantity}× ${a.battery_type}</span>
                      ${a.rechargeable?o`<span class="bf-recharge" title=${n("battery_fleet_rechargeable",e)}
                            ><ha-icon icon="mdi:battery-charging-outline"></ha-icon
                          ></span>`:p}
                      ${this._levelBar(a)}
                      ${a.level!=null?o`<span class="bf-level">${a.level}%</span>`:p}
                      <button
                        class="bf-mark"
                        title=${a.rechargeable?n("battery_fleet_mark_recharged",e):n("battery_fleet_mark_one",e)}
                      ...
                      <button
                        ...
                        title=${n("battery_fleet_exclude",e)}
                        .disabled=${this._marking}
        ...
        ${t.soon.length?o`
              ...
              <div class="bf-rows bf-soon-rows">
                ${t.soon.map(a=>o`
                    <div class="bf-row">
                      <span class="bf-dev">${a.device_name}</span>
                      ${a.no_sensor?o`<span class="bf-offline bf-nosensor">${n("battery_fleet_no_sensor",e)}</span>`:p}
                      <span class="bf-type">${a.quantity}× ${a.battery_type}</span>
                      ${this._levelBar(a)}
                      ${a.level!=null?o`<span class="bf-level">${a.level}%</span>`:p}
                      ${a.no_sensor||a.can_mark_replaced?o`<button class="bf-mark bf-replaced" title=${n("battery_fleet_mark_one",e)} .disabled=${this._marking} @click=${()=>this._mark([a.entity_id])}>
                            <ha-icon icon="mdi:battery-sync"></ha-icon>
                          </button>`:p}
                      ${a.days_until!=null?o`<span class="bf-predicted ${a.predicted_source==="trend"?"bf-trend":""} ${a.forecast_overdue?"bf-overdue":""}"
                            title=${this._predictedTitle(a,e)}
                            >${a.forecast_overdue?o`<ha-icon icon="mdi:calendar-alert"></ha-icon>`:p}~${this._predictedDate(a.days_until)}</span>`:p}
                    </div>
                  `)}
              </div>
            `:p}
              ...
              <details class="bf-roster" ?open=${this._rosterOpen()} @toggle=${this._loadHistory}>
                ...
                <div class="bf-rows">
                  ${this._sortedRoster(t.all).map(a=>o`
                      <div class="bf-row">
                        <span class="bf-dev">${a.device_name}</span>
                        <span class="bf-status bf-${a.status}"
                          >${a.no_sensor&&a.status==="low"?n("battery_fleet_status_due",e):n("battery_fleet_status_"+a.status,e)}</span
                        >
                        <span class="bf-type">${a.quantity}× ${a.battery_type}</span>
                        ${a.no_sensor?o`<span class="bf-offline bf-nosensor">${n("battery_fleet_no_sensor",e)}</span>`:p}
                        ${a.rechargeable?o`<span class="bf-recharge" title=${n("battery_fleet_rechargeable",e)}
                        ...
                        ${this._levelBar(a)}
                        ${a.level!=null?o`<span class="bf-level">${a.level}%</span>`:p}
                        ${a.no_sensor||a.can_mark_replaced?o`<button
                              class="bf-mark bf-replaced"
                              title=${n("battery_fleet_mark_one",e)}
                              .disabled=${this._marking}
                              @click=${()=>this._mark([a.entity_id])}
                            >
                              <ha-icon icon="mdi:battery-sync"></ha-icon>
                            </button>`:p}
                        ${(()=>{let l=this._history?.[a.entity_id]?.jump;return!l||this._recorded.includes(a.entity_id)?p:o`<button
                            class="bf-mark bf-jump"
                            title=${n("battery_fleet_record_replacement",e).replace("{date}",this._fmtDate(l.at*1e3))}
                            .disabled=${this._marking}
                        ...
                        ${a.days_until!=null?o`<span
                              ...
                              title=${this._predictedTitle(a,e)}
                              >${a.forecast_overdue?o`<ha-icon icon="mdi:calendar-alert"></ha-icon>`:p}~${this._predictedDate(a.days_until)}</span
                            >`:p}
                        <button
                          class="bf-mark bf-exclude"
                          title=${n("battery_fleet_exclude",e)}
  ...
  `,d([b({attribute:!1})],B.prototype,"hass",2),d([b({type:Boolean})],B.prototype,"flat",2),d([u()],B.prototype,"_ov",2),d([u()],B.prototype,"_loading",2),d([u()],B.prototype,"_marking",2),d([u()],B.prototype,"_error",2),d([u()],B.prototype,"_history",2),d([u()],B.prototype,"_rosterSort",2),d([u()],B.prototype,"_typeFilter",2),d([u()],B.prototype,"_recorded",2);var si=B;customElements.get("maintenance-battery-fleet-section")||customElements.define("maintenance-battery-fleet-section",si);var Ie=class extends S{constructor(){super(...arguments);this._config={type:"custom:maintenance-battery-fleet-card"}}static getStubConfig(){return{type:"custom:maintenance-battery-fleet-card"}}setConfig(e){this._config=e}getCardSize(){return 6}render(){return this.hass?o`
  ...
  `,d([b({attribute:!1})],Ie.prototype,"hass",2),d([u()],Ie.prototype,"_config",2);customElements.get("maintenance-battery-fleet-card")||customElements.define("maintenance-battery-fleet-card",Ie);Ee({type:"maintenance-battery-fleet-card",name:"Battery Fleet",description:"All tracked batteries: what is low now, what runs out soon, and what to buy.",preview:!1});Z();I();O();R();var Re="maintenance-supporter-panel-card",ui="maintenance-supporter-panel",gi="maintenance-supporter",_i=870,Ln=320,Nn=480,rr=["today","dashboard","calendar","settings"];function jn(r){let e=r?.panels?.[gi]?.config?._panel_custom?.module_url;return typeof e=="string"&&e.startsWith("/")?e:null}function qn(r){if(r==null||r===""||r==="fill")return null;if(typeof r=="number")return r>0?`${Math.round(r)}px`:null;let s=String(r).trim();return/^\d+(\.\d+)?$/.test(s)?`${s}px`:s||null}var mi=class extends HTMLElement{constructor(){super();this._config={type:`custom:${Re}`};this._panel=null;this._loading=null;this._observer=null;this._lastWidth=-1;this._preview=!1;this._onResize=()=>this._layout(!0);let e=this.attachShadow({mode:"open"}),t=document.createElement("style");t.textContent=`
      ...
      ${ui} { display: block; height: 100%; }
    `,e.appendChild(t),this._status=document.createElement("div"),this._status.className="status",this._status.hidden=!0,e.appendChild(this._status)}static getConfigElement(){return document.createElement("maintenance-supporter-panel-card-editor")}static getStubConfig(){return{type:`custom:${Re}`}}set preview(e){this._preview=!!e,this._layout(!0)}get preview(){return this._preview}_inEditorPreview(){if(this._preview)return!0;let e=this;for(let t=0;e&&t<40;t++){let i=e;if(i.tagName==="HUI-CARD-PREVIEW"||i.tagName==="HUI-DIALOG-EDIT-CARD")return!0;e=i.parentElement??i.getRootNode().host??null}return!1}setConfig(e){this._config={...e},this._panel&&(this._panel.presets=this._presets()),this._layout(!0)}getCardSize(){return 12}getGridOptions(){return{columns:"full",rows:"auto"}}set hass(e){this._hass=e,this._panel?this._panel.hass=e:this._mount()}get hass(){return this._hass}connectedCallback(){typeof ResizeObserver<"u"&&!this._observer&&(this._observer=new ResizeObserver(()=>this._layout(!1)),this._observer.observe(this)),window.addEventListener("resize",this._onResize),this._layout(!0),this._hass&&this._mount()}disconnectedCallback(){this._observer?.disconnect(),this._observer=null,window.removeEventListener("resize",this._onResize)}_presets(){let e={};return rr.includes(this._config.tab??"")&&(e.tab=this._config.tab),typeof this._config.view=="string"&&this._config.view.trim()&&(e.view=this._config.view.trim()),e}_setStatus(e){this._status.textContent=e,this._status.hidden=!1}async _mount(){if(this._panel||this._loading||!this._hass||!this.isConnected)return;let e=L(this._hass);if(!customElements.get(ui)){let i=jn(this._hass);if(!i){this._setStatus(n("panel_card_not_registered",e));return}this._loading=import(i).then(()=>{},()=>{throw this._setStatus(n("panel_card_load_failed",e)),new Error("panel bundle failed to load")});try{await this._loading}catch{return}finally{this._loading=null}if(this._panel||!this._hass||!this.isConnected)return}let t=document.createElement(ui);t.embedded=!0,t.panel=this._hass.panels?.[gi]??{url_path:gi},t.presets=this._presets(),t.narrow=this._narrow(),t.hass=this._hass,this._status.hidden=!0,this.shadowRoot.appendChild(t),this._panel=t,this._layout(!0)}_narrow(){let e=this.getBoundingClientRect().width;return e>0?e<_i:window.innerWidth<_i}_layout(e){let t=this.getBoundingClientRect(),i=Math.round(t.width)!==this._lastWidth;if(!e&&!i)return;this._lastWidth=Math.round(t.width),this._panel&&t.width>0&&(this._panel.narrow=t.width<_i);let a=qn(this._config.height);if(a){this.style.height=a;return}if(!this.isConnected)return;if(this._inEditorPreview()){this.style.height=`${Nn}px`;return}let l=Math.max(0,this._unscrolledTop(t)),c=Math.max(Ln,Math.floor(window.innerHeight-l));this.style.height=`${c}px`}_unscrolledTop(e){let t=e.top+window.scrollY,i=this.ownerDocument,a=this.parentElement??this.getRootNode().host??null;for(let l=0;a&&l<100;l++){let c=a;c!==i.documentElement&&c!==i.body&&typeof c.scrollTop=="number"&&c.scrollTop>0&&(t+=c.scrollTop),a=c.parentElement??c.getRootNode().host??null}return t}},ve=class extends S{constructor(){super(...arguments);this._config={type:`custom:${Re}`};this._views=[];this._viewsLoaded=!1}setConfig(e){this._config={...e}}updated(e){super.updated(e),e.has("hass")&&this.hass&&!this._viewsLoaded&&(this._viewsLoaded=!0,this._loadViews())}async _loadViews(){try{let e=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"});this._views=e?.views||[]}catch{this._views=[]}}_set(e,t){let i={...this._config};t===""||t===void 0?delete i[e]:i[e]=t,this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}render(){let e=L(this.hass),t=i=>n(i==="today"?"tab_today":i==="calendar"?"tab_calendar":i,e);return o`
      <div class="editor">
        <div class="field">
          ...
          <select class="tab-select" .value=${this._config.tab||""} @change=${i=>this._set("tab",i.target.value)}>
            ...
            ${rr.map(i=>o`<option value=${i} ?selected=${this._config.tab===i}>${t(i)}</option>`)}
          </select>
        </div>
  ...
  `,d([b({attribute:!1})],ve.prototype,"hass",2),d([u()],ve.prototype,"_config",2),d([u()],ve.prototype,"_views",2);customElements.get(Re)||customElements.define(Re,mi);customElements.get("maintenance-supporter-panel-card-editor")||customElements.define("maintenance-supporter-panel-card-editor",ve);Ee({type:Re,name:"Maintenance Supporter \u2014 Panel",description:"The complete Maintenance Supporter panel as a card \u2014 for a panel view / dashboard subview without the sidebar entry.",preview:!1});var V=class extends S{constructor(){super(...arguments);this._config={type:"custom:maintenance-supporter-card"};this._globalRowStyle="buttons_compact";this._objects=[];this._stats=null;this._unsub=null;this._viewFilters=null;this._userNames={};this._userPersons={};this._userService=null;this._userNamesLoaded=!1;this._taskDocs={};this._docsLoadedFor=new Set;this._dataLoaded=!1;this._lastConnection=null;this._onCompleted=async()=>{await this._loadData()}}get _lang(){return L(this.hass)}static getConfigElement(){return document.createElement("maintenance-supporter-card-editor")}static getStubConfig(){return{type:"custom:maintenance-supporter-card",show_header:!0,show_actions:!0,filter_status:["overdue","triggered","due_soon"],max_items:10}}setConfig(e){let t=e.view_id!==this._config.view_id;this._config=e,t&&this._dataLoaded&&this.hass&&this._loadViewFilters()}getCardSize(){return 3}connectedCallback(){super.connectedCallback()}disconnectedCallback(){super.disconnectedCallback(),this._unsub&&(this._unsub(),this._unsub=null),this._dataLoaded=!1,this._lastConnection=null}updated(e){if(super.updated(e),at(this,e),this.hass&&!this._userNamesLoaded&&this._config.show_assignee!==!1&&this._objects.some(t=>t.tasks.some(i=>i.responsible_user_id))&&this._loadUserNames(),this.hass&&this._config.show_documents!==!1)for(let t of this._objects)this._docsLoadedFor.has(t.entry_id)||t.tasks.some(i=>(i.document_count??0)>0)&&this._loadDocuments(t.entry_id);if(e.has("hass")&&this.hass){if(!this._dataLoaded)this._dataLoaded=!0,this._lastConnection=this.hass.connection,this._loadData(),this._subscribe();else if(this.hass.connection!==this._lastConnection){if(this._lastConnection=this.hass.connection,this._unsub){try{this._unsub()}catch{}this._unsub=null}this._subscribe(),this._loadData()}}}async _loadData(){di(this.hass).then(e=>{this._globalRowStyle=e}).catch(()=>{});try{let[e,t]=await Promise.all([this.hass.connection.sendMessagePromise({type:"maintenance_supporter/objects",compact:!0}),this.hass.connection.sendMessagePromise({type:"maintenance_supporter/statistics"})]);this._objects=Ue(e.objects),this._stats=t,ot(this._stats.budget)}catch{}await this._loadViewFilters()}_assigneeName(e){if(this._config.show_assignee===!1)return"";let t=e.responsible_user_id;return t&&this._userNames[t]||""}async _loadUserNames(){this._userNamesLoaded=!0,this._userService?this._userService.updateHass(this.hass):this._userService=new Se(this.hass);try{let e=await this._userService.getUsers();this._userPersons=Object.fromEntries(e.map(t=>[t.id,ct(t)])),this._userNames=Object.fromEntries(e.map(t=>[t.id,t.name]))}catch{}}async _loadDocuments(e){this._docsLoadedFor.add(e);try{let t=await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/documents/list",entry_id:e}),i={};for(let a of t.documents||[])for(let l of a.task_ids||[])(i[l]||=[]).push({id:a.id,title:a.title,kind:a.kind,url:a.url});this._taskDocs={...this._taskDocs,[e]:i}}catch{}}_docsFor(e,t){if(this._config.show_documents===!1)return[];let a=[...this._taskDocs[e]?.[t.id]||[]];return t.documentation_url&&a.push({id:`url:${t.id}`,title:n("documentation_label",this._lang),kind:"weblink",url:t.documentation_url}),a}async _openDoc(e){if(e.kind==="weblink"&&e.url){dt(e.url)&&window.open(e.url,"_blank","noopener");return}try{await ps(this.hass,e.id)}catch{}}async _loadViewFilters(){if(!this._config.view_id){this._viewFilters=null;return}try{let t=((await this.hass.connection.sendMessagePromise({type:"maintenance_supporter/views/list"})).views||[]).find(i=>i.id===this._config.view_id);this._viewFilters=t?t.filters:null}catch{this._viewFilters=null}}async _subscribe(){try{let e=await this.hass.connection.subscribeMessage(t=>{let i=Oi(this._objects,t);i!==null&&(this._objects=i)},{type:"maintenance_supporter/subscribe",deltas:!0,compact:!0});if(!this.isConnected){e();return}this._unsub=e}catch{}}get _flatTasks(){let e=[],{filter_status:t,filter_objects:i,entity_ids:a,filter_due_min_days:l,filter_due_max_days:c,filter_labels:h,filter_priority:_,filter_areas:f,max_items:x}=this._config,v=a?.length?new Set(a):null,y=l!==void 0||c!==void 0,m=this._viewFilters,$=m?.user_id==="current_user"?this.hass.user?.id??null:m?.user_id??null;for(let P of this._objects)if(!(i?.length&&!i.includes(P.object.name))){if(f?.length){let A=P.object.area_id;if(!A||!f.includes(A))continue}for(let A of P.tasks)if(!A.is_done&&!(A.archived||P.object.archived)&&!(t?.length&&!t.includes(A.status))&&!(h?.length&&!(A.labels||[]).some(j=>h.includes(j)))&&!(_?.length&&!_.includes(A.priority||"normal"))&&!(v&&!(A.sensor_entity_id&&v.has(A.sensor_entity_id)||A.binary_sensor_entity_id&&v.has(A.binary_sensor_entity_id)))){if(y){let j=A.days_until_due;if(j==null||l!==void 0&&j<l||c!==void 0&&j>c)continue}m&&(m.status&&A.status!==m.status||m.label&&!(A.labels||[]).includes(m.label)||m.priority&&(A.priority||"normal")!==m.priority||$&&A.responsible_user_id!==$)||e.push({entry_id:P.entry_id,object_name:P.object.name,task:A})}}let z={overdue:0,triggered:1,due_soon:2,ok:3};return e.sort((P,A)=>{let j=(z[P.task.status]??9)-(z[A.task.status]??9);return j!==0?j:(P.task.days_until_due??1/0)-(A.task.days_until_due??1/0)}),x&&x>0?e.slice(0,x):e}_openTaskDetail(e,t){hi(e,t)}render(){let e=this._lang,t=this._config.title||n("maintenance",e),i=this._config.show_header!==!1,a=(this._config.action_style??(this._globalRowStyle==="icons"?"icons":"buttons"))==="buttons",l=this._config.show_actions!==!1,c=this._config.compact||!1,h=this._flatTasks,_=this._stats;return o`
      <ha-card>
              ...
              <div class="task-list ${c?"compact":""}">
                ${h.map(({entry_id:f,object_name:x,task:v})=>o`
                    <div class="task-item clickable"
                         ...
                         title="${n("open_task",e)||"Open task"}">
                      <div class="status-dot" style="background: ${_e[v.status]||"#ccc"}"></div>
                      <div class="task-info">
                        <div class="task-name">
                          ${v.name}
                          ${v.due_override?o`<ha-icon
                                ...
                                title="${n("postponed",e)||"Postponed"}"
                              ></ha-icon>`:p}
                        </div>
                        ${c?this._assigneeName(v)?o`<div class="task-meta compact-assignee" title="${this._assigneeName(v)}">
                                ${Ft(this._userPersons[v.responsible_user_id])}${this._assigneeName(v)}
                              </div>`:p:o`<div class="task-meta">
                              ${x} · ${n(v.type,e)}${oe(v)?o` · ${oe(v)}`:p}${this._assigneeName(v)?o` · <span class="assignee"
                                    >${Ft(this._userPersons[v.responsible_user_id])}${this._assigneeName(v)}</span
                                  >`:p}
                            </div>`}
                      </div>
                      ...
                      <div class="task-due">
                        ${v.days_until_due!==null&&v.days_until_due!==void 0?v.days_until_due<0?o`<span class="overdue-text">${Ot(v.days_until_due,e)}</span>`:Ot(v.days_until_due,e):v.trigger_active?"\u26A1":"\u2014"}
                      </div>
                      ${l&&a?o`
                            <ha-button
                              size="small"
                              ...
                              title="${n("complete",e)}"
                              @click=${y=>{y.stopPropagation();let m=this.shadowRoot.querySelector("maintenance-complete-dialog");Qe(m,Ge({entryId:f,taskId:v.id,taskName:v.name,task:v,objects:this._objects,lang:e,checklist:v.checklist||[],adaptiveEnabled:!!v.adaptive_config?.enabled,currencySymbol:this._stats?.budget?.currency_symbol||""}),e)}}
                            >
    ...
    `],d([b({attribute:!1})],V.prototype,"hass",2),d([u()],V.prototype,"_config",2),d([u()],V.prototype,"_globalRowStyle",2),d([u()],V.prototype,"_objects",2),d([u()],V.prototype,"_stats",2),d([u()],V.prototype,"_unsub",2),d([u()],V.prototype,"_viewFilters",2),d([u()],V.prototype,"_userNames",2),d([u()],V.prototype,"_taskDocs",2);customElements.get("maintenance-supporter-card")||customElements.define("maintenance-supporter-card",V);Ee({type:"maintenance-supporter-card",name:"Maintenance Supporter",description:"Overview of your maintenance tasks with quick actions.",preview:!0});export{V as MaintenanceSupporterCard};
