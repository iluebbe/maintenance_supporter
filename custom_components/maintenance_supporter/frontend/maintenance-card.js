      ...
      <ha-dialog open @closed=${this._close}>
        ...
        <div class="dialog-actions">
          ...
          <ha-button
            ...
            title=${this._missingRequired.length?this._missingRequired.map(t=>r("err_required",e).replace("{field}",r(je[t]??t,e))).join(" \xB7 "):""}
          >
...
<script>setTimeout(()=>window.print(),300)<\/script>
  ...
  `}function er(a,s,e,t,i){let y=Math.max(e,t,s,1)*1.3,f=50,$=[];for(let F=0;F<=f;F++){let B=F/f*y,es=1-Math.exp(-Math.pow(B/s,a)),ts=32+B/y*260,is=136-es*128;$.push([ts,is])}let H=$.map(([F,B])=>`${F.toFixed(1)},${B.toFixed(1)}`).join(" "),P="M32,136 "+$.map(([F,B])=>`L${F.toFixed(1)},${B.toFixed(1)}`).join(" ")+` L${$[f][0].toFixed(1)},136 Z`,E=32+e/y*260,q=1-Math.exp(-Math.pow(e/s,a)),qe=136-q*128,Zi=((1-q)*100).toFixed(0),Pt=32+t/y*260,Xi=[0,.25,.5,.75,1];return o`
    <div class="weibull-chart">
      <svg viewBox="0 0 ${300} ${160}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${r("chart_weibull",i)}">
        ...
        ${e>0?he`
          ...
          <circle cx="${E.toFixed(1)}" cy="${qe.toFixed(1)}" r="3"
            fill="var(--primary-color, #03a9f4)" />
          <text x="${(E+4).toFixed(1)}" y="${(qe-6).toFixed(1)}" fill="var(--primary-color, #03a9f4)"
  ...
  `}function Bi(a,s){return sr(a,s)}function sr(a,s){let e=a.seasonal_factors??a.interval_analysis?.seasonal_factors;if(!e||e.length!==12)return h;let t=a.interval_analysis?.seasonal_reason,i=new Date().getMonth(),n=300,l=100,d=8,_=l-d-4,v=Math.max(...e,1.5),m=n/12,b=m*.65,y=d+_-1/v*_;return o`
    <div class="seasonal-chart">
      ...
      <svg viewBox="0 0 ${n} ${l}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${r("chart_seasonal",s)}">
        ...
        ${e.map((f,$)=>{let H=f/v*_,P=$*m+(m-b)/2,E=d+_-H,q=$===i,qe=f<1?"var(--success-color, #4caf50)":f>1?"var(--warning-color, #ff9800)":"var(--secondary-text-color)";return he`
            <rect x="${P.toFixed(1)}" y="${E.toFixed(1)}"
              width="${b.toFixed(1)}" height="${H.toFixed(1)}"
              fill="${qe}" opacity="${q?1:.5}" rx="2" />
          `})}
      </svg>
