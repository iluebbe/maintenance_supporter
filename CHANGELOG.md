...
- **Home profile** (Settings → Home profile): shows whether your home looks like a house or an apartment and why
  (several floors, a garden or garage area, a lawn mower …), your country and the climate of your home location,
...
- **Template tasks get real seasons:** tasks that belong to a season are created with a seasonal window or a fixed
...
- **Days-until-due countdown sensor per task** (forum request): a numeric
  ...
  cannot read the status sensor's `days_until_due` attribute. Disabled by
...
- **Vaillant (myVAILLANT)** joins the heating-pressure family — "Refill
  Heating Water" below 1 bar, from either the system- or device-level
  pressure sensor.
- **Whirlpool washers** get the tub-clean duty even though the
  integration has no cycle counter: the engine counts actual washing time
  (60 hours ≈ 30 typical cycles).

Catalog: **94 integrations / 174 verified signatures**.
