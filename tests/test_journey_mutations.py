...
async def test_owner_reorganizes_everything_after_a_year(hass: HomeAssistant, global_entry: MockConfigEntry) -> None:
    ...
    assert sorted(after.values()) == sorted(baseline.values()), "object rename changed entity_ids"
    ...
    entry = _entry(hass, entry_id)
    assert sorted(registry_snapshot(hass, entry).values()) == sorted(baseline.values())
    resp = _object_response(hass, entry)
    task_resp = next(t for t in resp["tasks"] if t["id"] == task_id)
...
async def test_feature_toggle_off_mutate_on_keeps_hidden_data(hass: HomeAssistant, global_entry: MockConfigEntry) -> None:
...
async def test_overdue_task_stays_overdue_across_restart(hass: HomeAssistant, global_entry: MockConfigEntry) -> None:
