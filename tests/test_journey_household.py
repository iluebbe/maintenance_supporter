...
def _make_entry(
    hass: HomeAssistant, unique_id: str, name: str, task: dict[str, Any]
) -> MockConfigEntry:
    entry = MockConfigEntry(
        version=1, minor_version=1, domain=DOMAIN,
        title=name,
...
async def test_double_tap_from_two_devices_counts_once(
    ...
    for i in (1, 2):
        await call_ws_handler(ws_complete_task, hass, _conn(), {
            "id": 10 + i, "type": "maintenance_supporter/task/complete",
            "entry_id": obj_entry.entry_id, "task_id": TASK_ID_1,
            "cost": 5.0,
        })
    await hass.async_block_till_done()
    ...
    assert (
        obj_entry.data[CONF_TASKS][TASK_ID_1]["responsible_user_id"] == "m1-ben"
    ), "double tap double-advanced the rotation"


async def test_completion_after_the_window_is_a_real_completion(
    hass: HomeAssistant, global_entry: MockConfigEntry
) -> None:
...
async def test_read_parity_across_all_surfaces(
    hass: HomeAssistant, global_entry: MockConfigEntry
) -> None:
    last = (dt_util.now().date() - timedelta(days=20)).isoformat()
    task = build_task_data(
        task_id=TASK_ID_1, name="Descale",
        last_performed=last, interval_days=30,
    )
    ...
    listed = await hass.services.async_call(
        DOMAIN, "list_tasks", {"entry_id": obj_entry.entry_id},
    ...
    item = next(
        i for i in (todo_entity.todo_items or [])
        if i.uid == f"{obj_entry.entry_id}:{TASK_ID_1}"
    )
    ...
    cal_eid = next(
        e.entity_id for e in reg.entities.values() if e.domain == "calendar"
    )
    cal_entity = hass.data["calendar"].get_entity(cal_eid)
    events = await cal_entity.async_get_events(
        hass, dtu.now(), dtu.now() + timedelta(days=40),
    )
