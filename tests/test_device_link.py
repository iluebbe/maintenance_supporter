...
async def test_linked_object_attaches_entities_to_foreign_device(
    hass: HomeAssistant, global_entry: MockConfigEntry
) -> None:
    device = _foreign_device(hass)
    obj_entry = _make_entry(
        hass, "linked", name="Washer Maintenance",
    ...
    sensors = [
        e for e in er.async_entries_for_config_entry(ent_reg, obj_entry.entry_id)
        if e.domain == "sensor"
    ]
    ...
    own = dr.async_get(hass).async_get_device(
...
async def test_vanished_linked_device_falls_back_to_own_device(
    hass: HomeAssistant, global_entry: MockConfigEntry
) -> None:
    obj_entry = _make_entry(
        hass, "dangling", name="Orphan",
...
async def test_parent_object_nests_via_device(
    hass: HomeAssistant, global_entry: MockConfigEntry
) -> None:
    parent_entry = _make_entry(hass, "parent", name="Water Heater")
    child_entry = _make_entry(
        hass, "child", name="Anode Rod",
        extra_obj={"parent_entry_id": parent_entry.entry_id},
    )
    await setup_integration(hass, global_entry, parent_entry, child_entry)

    reg = dr.async_get(hass)
    parent_dev = reg.async_get_device(
        identifiers={(DOMAIN, parent_entry.unique_id or "")}
    )
    child_dev = reg.async_get_device(
        identifiers={(DOMAIN, child_entry.unique_id or "")}
    )
...
async def test_validate_rejects_unknown_device_and_parent(
    hass: HomeAssistant, global_entry: MockConfigEntry
) -> None:
    conn = _FakeConnection()
    assert not _validate_device_link(
        hass, conn, {"id": 1, "ha_device_id": "nope"}, self_entry_id=None
    )
    assert conn.errors[-1][0] == "invalid_device"

    assert not _validate_device_link(
        hass, conn, {"id": 2, "parent_entry_id": "nope"}, self_entry_id=None
    )
    assert conn.errors[-1][0] == "invalid_parent"

    # The global entry is not a valid parent either.
    assert not _validate_device_link(
        hass, conn, {"id": 3, "parent_entry_id": global_entry.entry_id},
        self_entry_id=None,
    )
    assert conn.errors[-1][0] == "invalid_parent"


async def test_validate_rejects_parent_cycle(
    hass: HomeAssistant, global_entry: MockConfigEntry
) -> None:
    a = _make_entry(hass, "cyc_a", name="A")
    b = _make_entry(
        hass, "cyc_b", name="B", extra_obj={"parent_entry_id": a.entry_id}
    )
    conn = _FakeConnection()
    # Making B the parent of A closes the loop A→B→A.
    assert not _validate_device_link(
        hass, conn, {"id": 1, "parent_entry_id": b.entry_id},
        self_entry_id=a.entry_id,
    )
    assert conn.errors[-1][0] == "invalid_parent"
    # Direct self-parenting is a cycle of length one.
    assert not _validate_device_link(
        hass, conn, {"id": 2, "parent_entry_id": a.entry_id},
        self_entry_id=a.entry_id,
    )
    assert conn.errors[-1][0] == "invalid_parent"
    # A legitimate parent passes.
    assert _validate_device_link(
        hass, conn, {"id": 3, "parent_entry_id": a.entry_id},
        self_entry_id=b.entry_id,
    )


async def test_ws_response_exposes_link_fields(
    hass: HomeAssistant, global_entry: MockConfigEntry
) -> None:
    parent_entry = _make_entry(hass, "resp_parent", name="Parent")
    obj_entry = _make_entry(
        hass, "resp_child", name="Child",
