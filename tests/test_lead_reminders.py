    ...
    obj = _object(
        hass,
        {
            ...
            TASK_ID_1: build_task_data(
                name="Hit",
                interval_days=30,
                last_performed=_iso_days_ago(16),
            ),
    ...
    obj = _object(
        hass,
        {
            TASK_ID_1: build_task_data(
                name="Due Today",
                interval_days=30,
                last_performed=_iso_days_ago(30),
            ),
        },
        uid="lead_zero",
    )
...
async def test_send_lead_reminder_uses_due_soon_strings(
    ...
    with patch.object(nm, "hass") as mock_hass, patch.object(nm, "_is_quiet_hours", return_value=False):
...
async def test_send_lead_reminder_respects_quiet_hours(
    ...
    with patch.object(nm, "hass") as mock_hass, patch.object(nm, "_is_quiet_hours", return_value=True):
        mock_hass.services = MagicMock()
        mock_hass.services.async_call = AsyncMock()
