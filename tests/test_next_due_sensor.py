...
def _make_entry(
    hass: HomeAssistant, task_data: dict[str, Any], unique_id: str
) -> MockConfigEntry:
    entry = MockConfigEntry(
        version=1, minor_version=1, domain=DOMAIN,
        title="Test Object",
...
def _next_due_registry_entry(
    ...
    matches = [
        ...
        if e.unique_id.endswith("_next_due")
    ]
    assert len(matches) == 1
    return matches[0]


async def test_registered_but_disabled_by_default(
...
async def test_enabled_sensor_reports_midnight_local(
    hass: HomeAssistant, global_entry: MockConfigEntry
) -> None:
    last = dt_util.now().date() - timedelta(days=10)
    task = build_task_data(
        task_id=TASK_ID_1, last_performed=last.isoformat(), interval_days=30
    )
...
async def test_schedule_time_shifts_the_instant(
    hass: HomeAssistant
) -> None:
