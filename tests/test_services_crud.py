...
async def _create_object_with_task(
    hass: HomeAssistant, name: str = "Boiler"
) -> tuple[str, str]:
    obj = await hass.services.async_call(
        DOMAIN, "add_object", {"name": name}, blocking=True, return_response=True
    )
    await hass.async_block_till_done()
    entry_id = obj["entry_id"]
    res = await hass.services.async_call(
...
async def test_update_task_service_patches_fields(
    hass: HomeAssistant, global_config_entry: MockConfigEntry
) -> None:
    ...
    await hass.services.async_call(
        ...
        {"entry_id": entry_id, "task_id": task_id,
         "name": "Biennial service", "interval_days": 730,
         "priority": "high", "labels": ["heating"]},
        blocking=True,
    )
    await hass.async_block_till_done()
...
async def test_update_task_service_partial_edit_keeps_unit(
    hass: HomeAssistant, global_config_entry: MockConfigEntry
) -> None:
    ...
    res = await hass.services.async_call(
        DOMAIN, "add_task",
        {"entry_id": entry_id, "name": "Filter", "interval_days": 3,
         "interval_unit": "months"},
        blocking=True, return_response=True,
    )
    await hass.async_block_till_done()
    ...
    fields = read_legacy_fields(
        hass.config_entries.async_get_entry(entry_id).data[CONF_TASKS][res["task_id"]]
    )
    assert fields["interval_days"] == 6
    assert fields["interval_unit"] == "months"


async def test_update_task_service_unknown_task(
    hass: HomeAssistant, global_config_entry: MockConfigEntry
) -> None:
    await setup_integration(hass, global_config_entry)
...
async def test_delete_task_service_removes_task(
    hass: HomeAssistant, global_config_entry: MockConfigEntry
) -> None:
    await setup_integration(hass, global_config_entry)
...
async def test_list_tasks_service_snapshot_and_filters(
    hass: HomeAssistant, global_config_entry: MockConfigEntry
) -> None:
    ...
    result = await hass.services.async_call(
        DOMAIN, "list_tasks", {}, blocking=True, return_response=True
    )
