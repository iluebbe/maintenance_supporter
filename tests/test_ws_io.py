...
async def test_export_objects_csv_one_row_per_object(
    ...
    entry2 = MockConfigEntry(
        ...
        source="user",
        unique_id="maintenance_supporter_taskless_asset",
    )
...
async def test_export_objects_csv_roundtrips_via_import(
    ...
    entry = MockConfigEntry(
        ...
        source="user",
        unique_id="maintenance_supporter_roundtrip_asset",
    )
    entry.add_to_hass(hass)
...
async def test_json_export_import_roundtrips_doc_url_and_notes(
    ...
    entry = MockConfigEntry(
        ...
        data=build_object_entry_data(object_data=obj, tasks={}),
        source="user",
        unique_id="maintenance_supporter_docs_asset",
    )
    entry.add_to_hass(hass)
    await setup_integration(hass, global_entry, entry)
    ...
    entries = [e for e in hass.config_entries.async_entries(DOMAIN) if e.title == "Docs Asset Copy"]
    assert entries
...
async def test_export_handles_null_checklist(
    ...
    entry = MockConfigEntry(
        ...
        data=build_object_entry_data(tasks={TASK_ID_1: task}),
        source="user",
        unique_id="maintenance_supporter_null_checklist",
    )
    entry.add_to_hass(hass)
    await setup_integration(hass, global_entry, entry)
...
async def test_json_import_preserves_interval_unit_and_due_date(
            ...
            "objects": [
                {
                    ...
                    "tasks": [
                        {"name": "Monthly", "schedule_type": "time_based", "interval_days": 3, "interval_unit": "months"},
                        {"name": "OneShot", "schedule_type": "one_time", "due_date": "2026-09-01"},
                    ],
                }
            ],
        }
    ...
    entry = next(
        e
        for e in hass.config_entries.async_entries("maintenance_supporter")
...
async def test_import_json_caps_checklist(
    ...
    assert all(isinstance(x, str) and len(x) <= 500 for x in cl), "non-strings or oversize items leaked through"
...
async def test_csv_roundtrip_preserves_interval_unit_and_due_date(
    ...
    parsed = import_objects_csv(csv_text)
    entry_parsed = next(o for o in parsed if o["object"]["name"] == "CSV IntervalUnit")
